...
/// This is set to true just before Finale app calls start on the personality
/// script
...
bool FinaleApp::init()
{
    ...
    if (ret && !(disablePreloads_ = configSection->readBool(
                   "entities/disablePreloads", false))) {
#if ENABLE_WATCHERS
        ...
        if (DeviceApp::s_pGUIProgress_) {
            ...
            ProgressTask preloadTask = ProgressTask(
              DeviceApp::s_pProgress_, "Preloading Resources", 1.f);
    ...
    BW::string persName =
      configSection->readString("personality", Personality::DEFAULT_NAME);
...
void FinaleApp::runPreloads(ProgressTask& preloadTask)
{
    ...
    for (int i = 0; i < listlen; i++, preloadTask.step(dStep)) {
        ...
        if (PyString_Check(pString)) {
            ...
            if (pend != NULL &&
                (!strcmp(pend, ".bmp") || !strcmp(pend, ".tga") ||
                 !strcmp(pend, ".dds"))) {
        ...
        char* str = pModelStr ? PyString_AsString(pModelStr.getObject()) : NULL;
                            ...
                            BW::vector<BW::string>& ret) const
{
    ...
    if (pFolder) {
        ...
        const size_t remainingSize =
          ARRAY_SIZE(resourceID) -
          std::min(ARRAY_SIZE(resourceID), folderName.size());
        ...
        for (int i = 0; i < numChildren; ++i) {
            ...
            if (BWResource::getExtension(childName) == FX_EXTN) {
                bw_str_copy(pathEnd, remainingSize, childName);
...
{
    BW_GUARD;
    // TODO: if this system has alot of ram... preload the shaders as well as
    // compiling?
