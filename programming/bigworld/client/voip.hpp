...
class VOIPDataDict
{
  public:
     ...
     *
     *	@return				A pointer to the value corresponding to the given
...
class VOIPResponse
{
  public:
     ...
     *
     *	@param	message		An integer with a VoIP implementation defined
     *meaning.
...
/**
 *	This class forms an interface to VoIP services implemented in the DLL
 *voip.dll. The BigWorld client implements a python wrapper to this interface
 *that can be accessed in script though BigWorld.VOIP
 */
class VOIP_DLL_EXPORT VOIPClient
{
  private:
     ...
     *	Private constructor
     *
     *	VOIPClient objects should only be constructed by the createVOIPClient
     *function
     */
    VOIPClient();
     ...
     *	Private destructor
     *
     *	VOIPClient objects should only be destroyed by the destroyVOIPClient
     *function
     */
    ~VOIPClient();
  ...
  public:
     ...
     *	The returned VOIPClient should only be deleted using the
     *destroyVOIPClient() method
     *
    ...
    /**
     *	Returns whether the VOIPClient has already been initialised and not
     *finalised.
     *
     ...
     *
     *	@param	x			The location of the player in on the X axis. +X is
     *right
     *	@param	y			The location of the player in on the Y axis. +Y is
     *up
     *	@param	z			The location of the player in on the Z axis. +Z is
     *forward
     *	@param	yaw			The current yaw of the player. 0.0f yaw is forward,
     *+/- PI is backard
     *	@param	pitch		The current pitch of the player. 0.0f pitch is
     *level, +ve pitch is up (clockwise in Z-Y plane)
     *	@param	roll
     *	@param	spaceID		An integer uniquely identifying the space in which
     *the player currently resides
     */
    void tick(float x,
              float y,
              float z,
     ...
     *
     *	@param	data		Optional: Additional information in the form of
     *string value pairs
     */
     ...
     *	will be sent back to the application via the response callback
     *mechanism.
     */
    void getStatistics();
     ...
     *							interface. Passing NULL will clear the current
     *handler.
     */
     ...
     *
     *	@param	username	The string that uniquely identifies the user's
     *account
     ...
     *	Default password is "".
     *
     *	@param	channel			The string that will uniquely identify the new
     *channel
     *	@param	password		Optional: The password that will be needed to
     *access the channel
     *	@param	data			Optional: Additional information in the form of
     *string value pairs
     */
     ...
     *string value pairs
     */
    void inviteUser(const char*         username,
                    const char*         channel,
                    const VOIPDataDict* data = NULL);
     ...
     *string value pairs
     */
    void kickUser(const char*         username,
                  const char*         channel,
                  const VOIPDataDict* data = NULL);
     ...
     *string value pairs
     */
    void enableActiveChannel(const char*         channelID,
                             const VOIPDataDict* data = NULL);
     ...
     *string value pairs
     */
    void disableActiveChannel(const char*         channelID,
                              const VOIPDataDict* data = NULL);
     ...
     *
     *	@param	attenuationDB	The new speaker volume attenuation in
     *decibels(dB)
     *	@param	data			Optional: Additional information in the form of
     *string value pairs
     */
     ...
     *string value pairs
     */
    void setChannelVolume(const char*         channelName,
                          float               attenuationDB,
                          const VOIPDataDict* data = NULL);
     ...
     *string value pairs
     */
    void setMicrophoneVolume(float               attenuationDB,
                             const VOIPDataDict* data = NULL);
