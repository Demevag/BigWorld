...
void AvatarDropFilter::onEntityPositionUpdated(Entity& entity)
{
    ...
    if (alignToGround_) {
        ...
        Quaternion slopeRotation(
          slopeAxes, 1 + Vector3(0, 1, 0).dotProduct(groundNormal_));
    ...
    entity.setSpaceVehiclePositionAndDirection(
      spaceID, NULL_ENTITY_ID, localPos, localDir);
...
namespace {
     ...
     */
    void drawPlane(const Vector3& center,
                   ...
                   bool           drawAlways)
    {
        ...
        Geometrics::drawLine(
          center + v1 - v2, center + v1 + v2, colour, drawAlways);
        Geometrics::drawLine(
          center - v1 - v2, center - v1 + v2, colour, drawAlways);
        Geometrics::drawLine(
          center - v1 - v2, center + v1 - v2, colour, drawAlways);
...
PY_AUTO_MODULE_FUNCTION(RETOK,
                        debugDropFilter,
                        OPTARG(PyAvatarDropFilterPtr,
                               PyAvatarDropFilterPtr(),
                               END),
                        BigWorld)
