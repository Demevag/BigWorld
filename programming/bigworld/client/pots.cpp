    ...
    while (it != end) {
        ...
        if (pot.matrixProvider_) {
            ...
            if ((dSq < pot.distanceSq_) != pot.inside_) {
                ...
                Script::call(
                  pot.function_.getObject(),
                  Py_BuildValue("(ii)", pot.inside_ ? 1 : 0, it->first),
                  "Pots::tick");
            }
        }
    ...
    while (it != end) {
        ...
        if (mat.matrixProvider_) {
            ...
            if ((checkPos.lengthSquared() < 1.f) != mat.inside_) {
                ...
                Script::call(
                  mat.function_.getObject(),
                  Py_BuildValue("(ii)", mat.inside_ ? 1 : 0, it->first),
                  "Pots::tick");
            }
        }
 ...
 *	@}
 *	In this example, the hitPot function will get called whenever the player
 *enters or leaves a stationary sphere with a radius of 5 metres (the position
 *of the sphere is centred around where the player was when the addPot function
 *was called).
 *
 ...
 *	@param	callback	the script function to be called ( must take two int
 *params (in/out flag and potID) )
 *
 *	@return handle	a unique identifier for the pot
 */
uint32 Pots::addPot(MatrixProviderPtr      source,
                    float                  dist,
                    SmartPointer<PyObject> callback)
{
    BW_GUARD;
    ...
    {
        ERROR_MSG("Assert failure 'PyCallable_Check( callback.getObject() )' - "
                  "pot not created %i\n",
                  id);
        return id;
    }
 ...
 *	@param	source	the location, as given by a matrix provider
 *	@param	callback	the script function to be called ( must take two int
 *params (in/out flag and matID) )
 */
uint32 Pots::addMat(MatrixProviderPtr      source,
                    SmartPointer<PyObject> callback,
                    MatrixProviderPtr      target)
{
    BW_GUARD;
    uint32 id = s_instance_.nextHandle_++;
 ...
 *
 *	This method adds a matrix trap to the active list.  Rather than having a
 *Player Entity trigger the trap, as in a pot (Player-Only Trap) it uses a
 *MatrixProvider to trigger the trap.  It could use the Player's matrix to
 *	perform the exact same task as a pot, however a mat has the added
 *flexibility of being able to use any point in the world as the triggering
 *element (be it an Entity or otherwise).  If the target matrix is not provided,
 *the MatrixProvider of the current Camera will be used.  The scale of the
 *source MatrixProvider represents the size of the mat.
 *
 *	When the matrix trap is triggered, it will notfiy the given callback
 *function, defined as;
 *
 ...
 *
 *	@param	source		The centre and size of the mat, as given by a
 *MatrixProvider (size = scale of matrix)
 *	@param	callback	The script function to be called when the mat is
 *triggered (to receive in flag and mat ID)
 *	@param	target		The optional MatrixProvider that will trigger the mat
 *(uses current Camera if not provided)
 *
 ...
 *
 *	@param	matID	The integer ID of the mat to remove.  Corresponds to the
 *handle returned by BigWorld.addMat()
 */
