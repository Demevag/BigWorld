...
namespace // (anonymous)
{
    ...
    /**
     *	This is a collision callback used by filterDropPoint() to find the
     *closest solid triangle.
     */
    class SolidCollisionCallback : public CollisionCallback
    {
      public:
        ...
        float distance() const { return closestDistance_; }

        const WorldTriangle& triangle() const { return closestTriangle_; }
...
bool BWClientFilterEnvironment::filterDropPoint(SpaceID           spaceID,
                                                const Position3D& startPosition,
                                                Position3D&       landPosition,
...
PY_AUTO_MODULE_FUNCTION(RETVOID,
                        adfTest,
                        ARG(uint32, ARG(Vector4ProviderPtr, END)),
                        BigWorld)
