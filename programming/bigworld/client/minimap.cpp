...
void Minimap::update(float dTime, float relParentWidth, float relParentHeight)
{
    ...
    // because the gui is drawn in clip-space, but the screen is not usually
    // square we must take this into account when rotating, otherwise we end up
    // accidentally squashing the rotation matrix
    float w, h;
    ...
    // calculate the runTimeTransform for ourselves, or the border if we are not
    // spinning.
    if (this->rotate_) {
 ...
 */
void Minimap::doMousePick(const Vector3& camPos,
                          ...
                          const Vector2& size)
{
    ...
    if (fabsf(mpos.x) >= (1.f - margin_) ||
        (fabsf(mpos.y) >= (1.f - margin_))) {
        this->onEntryBlur();
        return;
    }
...
/**
 *	This method returns the distance to the given entry in clip coordinates,
 *given a mouse pos relative to the minimap, and a camera position in world
 *coordinates.
 *
 ...
 */
float Minimap::distanceToEntry(const Vector2&  mpos,
                               const Vector3&  cpos,
                               MatrixProvider* pMatrix)
{
    if (pMatrix) {
        Matrix p;
        pMatrix->matrix(p);
 ...
 *	@param	relParentHeight	see SimpleGUIComponent::update
 */
void Minimap::updateSimpleEntries(const Vector3& camPos,
 ...
 *	@param	relParentHeight	see SimpleGUIComponent::update
 */
void Minimap::updateEntries(const Vector3& camPos,
                            float          relParentWidth,
                            float          relParentHeight)
{
    float modX = this->widthInClip(relParentWidth) / 2.f;
...
#if INCLUDE_CELL_BOUNDARY_VIZ
void Minimap::updateCellBoundaries(const Vector3& camPos,
                                   float          relParentWidth,
                                   float          relParentHeight)
{
    ...
    if (s_debugMinimap) {
        DEBUG_MSG("Clipping Bounds min : %0.2f, %0.2f, %0.2f \tmax: %0.2f, "
                  "%0.2f, %0.2f\n",
                  bbmin.x,
                  bbmin.y,
                  bbmin.z,
    ...
    for (size_t i = 3; i < cellBounds_.size(); i += 4) {
        float a =
          Math::clamp(-CELL_MAX_VALUE, cellBounds_[i - 3], CELL_MAX_VALUE);
        float b =
          Math::clamp(-CELL_MAX_VALUE, cellBounds_[i - 2], CELL_MAX_VALUE);
        float c =
          Math::clamp(-CELL_MAX_VALUE, cellBounds_[i - 1], CELL_MAX_VALUE);
    ...
    for (size_t i = 0; i < cellBoundaries.size(); i++) {
        ...
        for (uint8 j = 0; j < 4; j++) {
            ...
            if (s_debugMinimap) {
                DEBUG_MSG("Line %d %d : A : %0.2f, %0.2f, %0.2f \tB : %0.2f, "
                          "%0.2f, %0.2f\t",
                          i,
                          j,
                          lineA.x,
            ...
            if (bounds.clip(lineA, lineB)) {
                if (s_debugMinimap) {
                    DEBUG_MSG("Clipped! : A : %0.2f, %0.2f, %0.2f \tB : %0.2f, "
                              "%0.2f, %0.2f\n",
                              lineA.x,
                              lineA.y,
                              lineA.z,
 ...
 *
 *	@param  reallyDraw 	Boolean representing whether we want to really draw or
 *just do all calculation without drawing.
 *	@param	overlay	Boolean representing whether we are drawing as a GUI overlay
 *	across the screen (the default), and False if we are drawing in the 3D
 *scene.
 */
void Minimap::drawSelf(bool reallyDraw, bool overlay)
{
    ...
    if (material_->begin()) {
        ...
        if (reallyDraw) {
            for (uint32 i = 0; i < material_->numPasses() && valid; i++) {
                ...
                material_->pEffect()->pEffect()->SetVector("mapCoords",
                                                           &mapCoords_);
        ...
        while (it != en) {
            ...
            // Clip region for layers.  Use the GUI clipping functionality to
            // make sure we are only drawing the area the layer covers. We are
            // using this technique to avoid using a pixel shader to do the same
            // thing - this is so the minimap still works on non-pshader cards.
            ...
            // The bottom-right of the rect is shifted down one pixel, since D3D
            // RECT's are right hand exclusive.
            ...
            if (SimpleGUI::instance().pushClipRegion(clipRegion)) {
                ...
                // the worldMapAnchor specifies where the center of the texture
                // map is, in world coordinates.
                /// The map coords are shader constants, representing (xOffset
                /// (uv), yOffset (uv), xScale (world -> uv), yScale (world ->
                /// uv)
                mapCoords_.set(
                ...
                if (reallyDraw) {
                    for (uint32 i = 0; i < material_->numPasses() && valid;
                         i++) {
                        material_->beginPass(i);
                        material_->pEffect()->pEffect()->SetVector("mapCoords",
                                                                   &mapCoords_);
                        ...
                        Moo::rc().setTexture(
                          0,
                          layer.pTexture_
                            ? layer.pTexture_->texture()->pTexture()
                            : NULL);
                        Moo::rc().setTexture(1,
                                             mask_ ? mask_->pTexture() : NULL);
                        if (tiled_) {
                            Moo::rc().setSamplerState(
                              0, D3DSAMP_ADDRESSU, D3DTADDRESS_WRAP);
                            Moo::rc().setSamplerState(
                        ...
                        if (!overlay) {
                            Moo::rc().setRenderState(D3DRS_ZENABLE, TRUE);
                            Moo::rc().setRenderState(D3DRS_ZWRITEENABLE, FALSE);
                            Moo::rc().setRenderState(D3DRS_CULLMODE,
                                                     D3DCULL_NONE);
                            Moo::rc().setRenderState(D3DRS_ZFUNC, D3DCMP_LESS);
                        }
    ...
    if (reallyDraw && simpleEntriesVisible_) {
        ...
        if (almostZero(pointSizeScaling_.x) &&
            almostZero(pointSizeScaling_.y)) {
            Moo::rc().setRenderState(D3DRS_POINTSIZE,
                                     *((DWORD*)&simpleEntrySize_));
        } else {
            float pSize =
              pointSizeScaling_.x + pointSizeScaling_.y * currentRange_;
            Moo::rc().setRenderState(D3DRS_POINTSIZE, *((DWORD*)&pSize));
        }
        Moo::rc().setTexture(
          0, simpleEntryMap_.hasObject() ? simpleEntryMap_->pTexture() : NULL);
 ...
 *	on the map.
 *	@param	pMatrix		MatrixProvider to use for position, rotation
 *information.
 *	@param	colour		colour to draw with.
 *	@return	MinimapHandle	Handle that used to remove the entry later on.
 */
MinimapHandle Minimap::addSimple(MatrixProviderPtr pMatrix,
 ...
 *	@return	MinimapHandle	Handle that used to remove the entry later on.
 */
MinimapHandle Minimap::add(MatrixProviderPtr     pMatrix,
                           SimpleGUIComponentPtr pComponent)
{
    MinimapHandle h = this->nextHandle();
 ...
 */
bool Minimap::load(DataSectionPtr    pSect,
                   ...
                   LoadBindings&     bindings)
{
    ...
    this->simpleEntryMap(
      pSect->readString("simpleEntryMap", this->simpleEntryMap()));
    this->simpleEntrySize_ =
      pSect->readFloat("simpleEntrySize", this->simpleEntrySize_);
 ...
 */
void Minimap::addTextureLayer(uint32               layerNum,
