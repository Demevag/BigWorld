                                             ...
                                             DataSectionPtr pSection)
{
    ...
    if (!pEntity->load(pSection, pChunk)) {
        ...
        return ChunkItemFactory::Result(NULL,
                                        "Failed to load entity of type " +
                                          pSection->readString("type"));
    } else {
...
void ChunkEntity::makeEntity()
{
    ...
    pEntity_ = EntityUDOFactory::createEntityWithType(
      pChunk_->space()->id(), pType_, worldTransform_, pPropertiesDS_);
...
{
    BW_GUARD;
    for (ChunkEntities::iterator it = entities_.begin(); it != entities_.end();
         it++) {
        (*it)->bind();
    }
}
