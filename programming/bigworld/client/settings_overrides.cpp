...
ScreenSettingsOverrides readCommandLineScreenSettingsOverrides()
{
    ...
    if (setX && setY) {
        ...
        ERROR_MSG(
          ...
          CMD_PARAM_SCREEN_RES_X,
          CMD_PARAM_SCREEN_RES_Y);
    }
    ...
    if (cmd.hasParam(CMD_PARAM_FULLSCREEN)) {
        ...
        uint32 forcedFullscreen = atouint32(cmd.getParam(CMD_PARAM_FULLSCREEN),
                                            CMD_FULLSCREEN_BAD_PARAM);
        if (forcedFullscreen == 0 || forcedFullscreen == 1) {
            ...
            ERROR_MSG(
              "DeviceApp: Value should should be 0 or 1 for param '-%s'\n",
