...
class Minimap : public SimpleGUIComponent
{
    Py_Header(Minimap, SimpleGUIComponent)
      ...
      public : Minimap(PyTypeObject* pType = &s_type_);
    ~Minimap();

    MinimapHandle add(MatrixProviderPtr     pMatrix,
    ...
    void addTextureLayer(uint32               layer,
    ...
    PY_AUTO_METHOD_DECLARE(RETDATA,
                           ...
                           NZARG(MatrixProviderPtr,
                                 NZARG(SimpleGUIComponentPtr, END)))
    PY_AUTO_METHOD_DECLARE(RETDATA,
                           addSimple,
                           NZARG(MatrixProviderPtr, ARG(Vector4, END)))
    PY_AUTO_METHOD_DECLARE(RETVOID, remove, ARG(MinimapHandle, END))
    PY_AUTO_METHOD_DECLARE(
      RETVOID,
      addTextureLayer,
      ARG(uint32, ARG(PyTextureProviderPtr, ARG(Vector2, ARG(Vector2, END)))))
  ...
  private:
    ...
    bool          load(DataSectionPtr    pSect,
                       const BW::string& ownerName,
                       LoadBindings&     bindings);
    void          save(DataSectionPtr pSect, SaveBindings& bindings);
    void          updateEntries(const Vector3& camPos,
                                float          relParentWidth,
                                float          relParentHeight);
    void          updateSimpleEntries(const Vector3& camPos,
                                      float          relParentWidth,
                                      float          relParentHeight);
    void          doMousePick(const Vector3& camPos,
                              const Vector3& center,
                              const Vector2& size);
    float         distanceToEntry(const Vector2&  mpos,
                                  const Vector3&  cpos,
    ...
    {
      public:
        virtual void matrix(Matrix& m) const { m = m_; }

        Matrix m_;
    };

    /// This is a simple class that stores a Matrix Provider, a gui component
    /// ptr, and a simple matrix provider
    class EntryInfo
    {
      public:
    ...
    void                      updateCellBoundaries(const Vector3& camPos,
                                                   float          relParentWidth,
                                                   float          relParentHeight);
