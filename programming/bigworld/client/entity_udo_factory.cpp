  ...
  const DataSectionPtr& ds)
{
    ...
    Entity* pEntity = EntityUDOFactory::createEntityWithType(
      spaceID, pType, transform, pPropertiesDS);
  ...
  const DataSectionPtr& ds)
{
    ...
    if (!pTransform) {
        ERROR_MSG("EntityUDOFactory::createUDO: "
                  "UserDataObject (of type '%s') has no position",
                  type.c_str());
        return 0;
    }
...
// ----------------------------------------------------------------------------
/* static */ Entity* EntityUDOFactory::createEntityWithType(
  SpaceID               spaceID,
