 ...
 *
 *	This attribute is used for client controlling entities to smooth their
 *motion when the ActionMatcher is switched on after being off (which is done by
 ...
 *	the entity is falling. This will be the case when the falling checkbox is
 *selected in the model editor for an action.
 *
 ...
 *
 *	The boredNotifer is the callback function to call once fuse exceeds
 *patience. As an animation plays, fuse will increase.  When a new animation is
 *played, fuse will be reset.  The patience parameter is set by script.  If 0,
 *the boredNotifier will not be triggered.  The callback can be defined as
 *follows:
 *
 *	@{
 ...
 *
 *	This function returns a new ActionMatcher Motor object, which is used to
 *match the movements and orientation of an entity to the actions defined on its
 *primary model. Note that the primary Model will be assigned an ActionMatcher
 *Motor by default.
 *
...
void ActionMatcher::attached()
{
    ...
    currModelScale.setScale(
      pOwner_->worldTransform().applyToUnitAxisVector(0).length(),
    ...
    if (ret == 0 && pOwner_ != NULL && pPyEntity_.good() &&
        ...
        if (matcherCoupled_) {
            ...
            if (pEntity->isControlled() && inheritOnRecouple_ &&
                ...
                pEntity->getLatestMove(localPosition,
                                       vehicleID,
                                       spaceID,
                                       localDirection,
                                       dummyPositionErr);
            ...
            } else // either inheritOnRecouple_ is false or non-self-controlled
                   // entity
            {
 ...
 */
void worldToVehicle(const Vector3&     vehiclePos,
 ...
 */
void worldToVehicle(const Direction3D& vehicleDir, Vector3& velocity)
{
    Matrix m;
    m.setRotateInverse(vehicleDir.yaw, vehicleDir.pitch, vehicleDir.roll);
...
void ActionMatcher::rev(float dTime)
{
    ...
    Matrix currModelScale;
    currModelScale.setScale(
      model.worldTransform().applyToUnitAxisVector(0).length(),
    ...
    const bool startedMoving =
      lastSpeed_ < 0.1f && vHorizontalVelocity.length() >= 0.1f;
    ...
    if (pEntity == ScriptPlayer::entity() &&
        ...
        if (lastSpeedFor_ <= smoothingTime_) {
            ...
            float estimateSpeed =
              lastSpeed_ + newSpeedDelta *
                             ((smoothingTime_ - lastSpeedFor_) + dTime) / dTime;
    ...
    if (matchModelScale_ && pEntity->pPrimaryModel()) {
        modelScaleFactor = pEntity->pPrimaryModel()
                             ->worldTransform()
    ...
    float rLen =
      vDeltaRot.length() * blendingSpeedFactorEstimate * modelScaleFactor;
    ...
    for (i = pActions_.begin(); i != pActions_.end(); i++) {
        ...
        if (mi.oneShot) {
            float matchCycleCount =
              model.actionQueue().matchCycleCount() +
    ...
    if (i == pActions_.end()) {
        ...
        if ((*i)->pSource_->matchInfo_.scalePlaybackSpeed) {
            ...
            if (dbLen != 0.0f) {
                ...
                inTime =
                  model.moveScale() * drLen / dbLen; // cancelled out 'dTime'
            }
        }
            ...
            if ((*i)->pSource_->isMovement_) {
                const Vector3 naturalDir =
                  model.initialItinerantContextInverse().applyVector(
                    (*i)->pFirstAnim_->flagFactorBit(0).applyToOrigin()) *
                  -1.f;
                ...
                // rYaw variable from above is calculated in vehicle / local
                // space and thus you can subtract the two.
        ...
        if (turnModelToEntity) {
            ...
            wNewModelYaw =
              wLastYaw +
              Math::clamp(bodyTwistMax, float(Angle(wCurrYaw - wLastYaw)));

            if (useEntityPitchAndRoll_) {
                wNewModelPitch =
                  wLastPitch +
                  Math::clamp(bodyTwistMax,
                              float(Angle(wCurrPitch - wLastPitch)));

                wNewModelRoll =
                  wLastRoll + Math::clamp(bodyTwistMax,
        ...
        newFootTwist             = lastFootTwist_ +
                       Math::clamp(footTwistMax, newFootTwist - lastFootTwist_);

        // DEBUG_MSG( "AM: rLen %g, rYaw %g, rAux1 %g: %s\n", rLen, rYaw, rAux1,
        //	(*i)->pSource->name_.c_str() );
        model.actionQueue().setMatch(
          *i, dTime, inTime, (*i)->pSource_->promoteMotion());
    ...
    if (startedMoving && startMovingNotifier_) {
        ...
        if (i != pActions_.end()) {
            Script::call(&*startMovingNotifier_,
...
void ActionMatcher::doEntityCollisions(Vector3& wCurrPos)
{
    ...
    for (; iter != end; iter++) {
        ...
        if (!pMotor || !ActionMatcher::Check(pMotor))
            continue;
        ...
        if (pAM->entityCollision_) {
            ...
            if (distSQ < bubbleRadius * bubbleRadius) {
                if (distSQ == 0.0f) {
                    lookAt = Vector2(
                      0.0f,
                      (iter->entityID() < pEntity->entityID()) ? 1.0f : -1.0f);
                } else {
                    lookAt.normalise();
                }
...
PyObject* ActionMatcher::pyGet_actions()
{
    ...
    for (size_t i = 0; i < pActions_.size(); i++) {
        PyTuple_SetItem(
          result,
          i,
          PyString_FromString(pActions_[i]->pSource_->name_.c_str()));
    }
...
PyObject* ActionMatcher::matchInfo(const BW::string& actionName)
{
    for (size_t i = 0; i < pActions_.size(); i++) {
        if (pActions_[i]->pSource_->name_ == actionName) {
            ...
            PyTuple_SetItem(
              result, 0, Script::getData(mi.trigger.minEntitySpeed));
            PyTuple_SetItem(
              result, 1, Script::getData(mi.trigger.maxEntitySpeed));
            PyTuple_SetItem(
              result, 2, Script::getData(mi.trigger.minEntityAux1));
            PyTuple_SetItem(
              result, 3, Script::getData(mi.trigger.maxEntityAux1));
            ...
            PyTuple_SetItem(
              result, 6, Script::getData(mi.cancel.minEntitySpeed));
            PyTuple_SetItem(
              result, 7, Script::getData(mi.cancel.maxEntitySpeed));
            PyTuple_SetItem(
              result, 8, Script::getData(mi.cancel.minEntityAux1));
            PyTuple_SetItem(
              result, 9, Script::getData(mi.cancel.maxEntityAux1));
