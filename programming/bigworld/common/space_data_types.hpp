...
const uint16 SPACE_DATA_RECORDING = SPACE_DATA_FIRST_NON_USER_CELL_ONLY_KEY;
...
struct SpaceData_LoadBounds
{
    ...
    /**
     *  This function converts a string buffer to a SpaceData_LoadBounds
     * representation
     *  @param input string that acts as a buffer
     *  @param loadBounds reference to a pointer that receives the casted value
     */
    static bool castFromString(const BW::string&      input,
                               SpaceData_LoadBounds*& loadBounds)
    {
     ...
     */
    static void copyToString(const SpaceData_LoadBounds& loadBounds,
                             BW::string&                 output)
    {
