...
namespace SimpleClientEntity {
    ...
    class StaticWatcherInitialiser
    {
      public:
        ...
        {
            BW_GUARD;
            MF_WATCH("Debug/dumpEntityMessages",
                     g_verbose,
                     Watcher::WT_READ_WRITE,
                     "If true, all entity property and method messages from "
                     "the server "
    ...
    class EntityPropertyOwner : public TopLevelPropertyOwner
    {
      public:
        ...
        virtual ScriptObject setOwnedProperty(int ref, BinaryIStream& data)
        {
            ...
            ScriptDataSink sink;
            if (!pDD->createFromStream(
                  data, sink, /* isPersistentOnly */ false)) {
     ...
     */
    bool propertyEvent(ScriptObject             pEntity,
    ...
    namespace {
        ...
        {
          public:
            ResetVisitor(ScriptObject&            rEntity,
                         const EntityDescription& rEdesc,
                         BinaryIStream&           rData)
              : rEntity_(rEntity)
              , rEdesc_(rEdesc)
              , rData_(rData){};
          ...
          private:
            virtual bool visit(const DataDescription& propDesc)
            {
                ...
                ScriptObject oldValue = king.setOwnedProperty(
                  propDesc.clientServerFullIndex(), rData_);
                ...
                if (!rEntity_.hasAttribute(methodName.c_str())) {
                    ...
                    if (!rEntity_.hasAttribute(methodName.c_str())) {
                        // No implicit property callback exists, early-out
                        // successfully.
                        return true;
                    }
                }
                             ...
                             bool                     isSlice)
    {
        ...
        int topLevelIndex = king.setNestedPropertyFromExternalStream(
          data, isSlice, ppOldValue, ppChangePath);
        ...
        if (shouldUseCallback) {
            ...
            pDataDescription->callSetterCallback(
              pEntity, pOldValue, pChangePath, isSlice);
        }
     ...
     */
    bool methodEvent(ScriptObject             pEntity,
                     const EntityDescription& edesc,
                     int                      methodID,
                     BinaryIStream&           data)
    {
