...
PyObject* PyTimer::addTimer(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(
          args, "f|fi", &initialOffset, &repeatOffset, &userArg)) {
        return NULL;
    }

    int id = ScriptTimersUtil::addTimer(
      &pTimers_, initialOffset, repeatOffset, userArg, &timerHandler_);
...
void PyTimer::handleTimeout(TimerHandle handle, void* pUser)
{
    ...
    if (id != 0) {
        ...
        PyObject* pResult =
          PyObject_CallMethod(pyObject_, "onTimer", "ik", id, uintptr(pUser));
