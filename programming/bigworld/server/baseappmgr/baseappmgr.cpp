...
namespace {
    ...
    /**
     * This static method sanitises pickled representation of a global base key
 ...
 *				the predicate.
 */
BaseAppsIterator::BaseAppsIterator(const BaseApps& apps, const Predicate& pred)
  : iter_(apps.begin())
  , end_(apps.end())
  , predicate_(pred)
{
...
bool BaseAppMgr::init(int argc, char* argv[])
{
...

    {
        Mercury::MachineDaemon::registerBirthListener(
          interface_.address(),
          BaseAppMgrInterface::handleCellAppMgrBirth,
          "CellAppMgrInterface");
    ...
    BW_REGISTER_WATCHER(
      0, "baseappmgr", "baseAppMgr", mainDispatcher_, interface_.address());
...
void BaseAppMgr::onBaseAppRetire(BaseApp& baseApp)
{
    ...
    this->getSubSet(baseApp).adjustBackupLocations(
      baseApp, ADJUST_BACKUP_LOCATIONS_OP_RETIRE);
}
...
void BaseAppSubSet::updateBestBaseApp()
{
    ...
    if ((pBest != NULL) && (bestBaseAppAddr_ != pBest->addr()) &&
...
namespace {
    ...
    /**
     *  This function sends a Mercury message to all known baseapps.  The
     * message payload is taken from the provided MemoryOStream.  If pExclude is
     * non-NULL, nothing will be sent to that app.  If pReplyHandler is
     * non-NULL, we start a request instead of starting a regular message.
     */
    void sendToBaseApps(const Mercury::InterfaceElement& ifElt,
                        MemoryOStream&                   args,
                        BaseAppsIterator                 iter,
                        Mercury::ReplyMessageHandler*    pHandler = NULL)
    {
        while (const BaseAppPtr& pBaseApp = iter.next()) {
            ...
            // Note: This does not stream off from "args". This is so that we
    ...
    /**
     *	This class is a functor to be used as a predicate for @ref
     *BaseAppsIterator. The predicate excludes the given app.
     */
    class BaseAppsExcluding : public std::unary_function<const BaseApp&, bool>
    {
      public:
        BaseAppsExcluding(const BaseApp* exclude)
          : exclude_(exclude)
        {
        }
        bool operator()(const BaseApp& app) const { return &app != exclude_; }
...
void BaseAppMgr::addWatchers()
{
    ...
    MF_WATCH(
      "serviceAppLoad/average", serviceApps_, &ServiceAppSubSet::avgAppLoad);
    ...
    MF_WATCH(
      "lastServiceAppIDAllocated", serviceApps_, &ServiceAppSubSet::lastAppID);

    pRoot->addChild(
      "cellAppMgr", Mercury::ChannelOwner::pWatcher(), &cellAppMgr_);
...
void BaseAppMgr::checkForDeadBaseApps()
{
    ...
    while (iter != baseAndServiceApps_.end()) {
        lastHeardTime =
          std::max(lastHeardTime, iter->second->channel().lastReceivedTime());
        ++iter;
    }
                              ...
                              BinaryIStream&                        data)
{
    ...
    if (pBest == NULL) {
        ...
        baseAppAddr.port = BaseAppMgrInterface::CREATE_ENTITY_ERROR_NO_BASEAPPS;
                     ...
                     const BaseAppMgrInterface::addArgs&   args)
{
    ...
    const BaseAppID id = this->getSubSet(args.isServiceApp).getNextAppID();

    const BaseAppPtr pBaseApp(new BaseApp(
      *this, args.addrForCells, args.addrForClients, id, args.isServiceApp));
...
void BaseAppSubSet::updateCreateBaseInfo()
{
    ...
    if (destSize > Config::maxDestinationsInCreateBaseInfo()) {
        ...
        // the machine.
        using namespace std::placeholders;
        std::partial_sort(infoPairs.begin(),
                          infoPairs.begin() + destSize,
                          infoPairs.end(),
    ...
    sendToBaseApps(BaseAppIntInterface::setCreateBaseInfo,
                   infoStream,
 ...
 */
void BaseAppMgr::recoverBaseApp(
...
/**
 *	This method reconstructs a @ref BaseApp after the death of an old
 *BaseAppMgr.
                                      ...
                                      BinaryIStream&          data)
{
    ...
    BaseAppPtr pBaseApp(
      new BaseApp(baseAppMgr, addrForCells, addrForClients, id, isServiceApp));
 ...
 *	@param data		Recovery data stream.
 */
void ServiceAppSubSet::recoverServiceFragments(const Mercury::Address& srcAddr,
                                               BinaryIStream&          data)
{
    uint32 numServices;
    data >> numServices;
                     ...
                     const BaseAppMgrInterface::delArgs&   args)
{
    ...
    if (this->onBaseAppDeath(addr)) {
        ...
        PROC_IP_ERROR_MSG(
          "BaseAppMgr: Error deleting %s id = %u\n", addr.c_str(), args.id);
    }
}
...
void BaseAppMgr::synchronize(BaseAppPtr baseApp)
{
    ...
    for (SharedData::const_iterator iSharedData = sharedBaseAppData_.begin();
         iSharedData != sharedBaseAppData_.end();
         ++iSharedData) {
    ...
    for (SharedData::const_iterator iSharedData = sharedGlobalData_.begin();
         iSharedData != sharedGlobalData_.end();
         ++iSharedData) {
        bundle.startMessage(BaseAppIntInterface::setSharedData);
        bundle << SharedDataType(SHARED_DATA_TYPE_GLOBAL) << iSharedData->first
               << iSharedData->second;
    }
...
void ManagedAppSubSet::killApp(const BaseApp& baseApp)
{
    ...
    if (!Mercury::MachineDaemon::sendSignalViaMachined(addr, SIGQUIT)) {
        ERROR_MSG(
          "ManagedAppSubSet::killApp: Failed to send SIGQUIT to %s %s\n",
          this->appName(),
          addr.c_str());
    }
...
void BaseAppMgr::sendBaseAppDeathNotifications(const BaseApp& baseApp)
{
    ...
    if (baseAndServiceApps_.size() > 0) {
        ...
        sendToBaseApps(
          BaseAppIntInterface::handleBaseAppDeath,
          args,
          BaseAppsIterator(baseAndServiceApps_, BaseAppsExcluding(&baseApp)));
...
bool ServiceAppSubSet::onBaseAppDeath(BaseApp& baseApp)
{
    ...
    int                numRemoved =
      services_.removeFragmentsForAddress(baseApp.addr(), &lostServices);
    ...
    if (!lostServices.empty()) {
        ServicesMap::Names::const_iterator iLostService = lostServices.begin();
 ...
 */
void BaseAppMgr::removeControlledShutdownBaseApp(const Mercury::Address& addr)
{
...
/**
 *	This method informs base and service apps of the controlled shutdown
 *process.
  ...
  const BaseAppMgrInterface::controlledShutDownArgs& args)
{
    ...
    SyncControlledShutDownHandler* pHandler =
      new SyncControlledShutDownHandler(args.stage, baseAndServiceApps_.size());
    MemoryOStream payload;
    payload << args.stage << args.shutDownTime;
    sendToBaseApps(BaseAppIntInterface::controlledShutDown,
                   payload,
...
void BaseAppMgr::controlledShutDownServer()
{
    if (shutDownStage_ != SHUTDOWN_NONE) {
        DEBUG_MSG(
          "BaseAppMgr::controlledShutDownServer: "
                               ...
                               BinaryIStream&                        data)
{
    ...
    if (shouldSendToBaseApps) {
        ...
        sendToBaseApps(BaseAppIntInterface::setSharedData,
                       payload,
                       BaseAppsIterator(baseAndServiceApps_));
    }
}
                               ...
                               BinaryIStream&                        data)
{
    ...
    if (shouldSendToBaseApps) {
        ...
        sendToBaseApps(BaseAppIntInterface::delSharedData,
                       payload,
                       BaseAppsIterator(baseAndServiceApps_));
    }
}
 ...
 */
void BaseAppMgr::informOfArchiveComplete(
 ...
 *	requesting address.
 */
void BaseAppMgr::requestBackupHashChain(
  const Mercury::Address&               addr,
  const Mercury::UnpackedMessageHeader& header,
  BinaryIStream&                        data)
{
                         ...
                         BinaryIStream&                        data)
{
    ...
    // Start the BaseApps.
    {
        ...
        for (BaseApps::const_iterator it = baseAndServiceApps_.begin();
             it != baseAndServiceApps_.end();
             ++it) {
            this->startupBaseApp(
              *it->second, shouldSendBootstrap, didAutoLoadEntitiesFromDB);
        }
 ...
 */
void BaseAppMgr::startupBaseApp(BaseApp& baseApp,
 ...
 */
void BaseAppMgr::handleCellAppDeath(
  const Mercury::Address& /*addr*/,
  const Mercury::UnpackedMessageHeader& /*header*/,
  BinaryIStream& data)
{
    ...
    sendToBaseApps(BaseAppIntInterface::handleCellAppDeath,
                   payload,
                   BaseAppsIterator(baseAndServiceApps_));
}
 ...
 */
void BaseAppMgr::registerBaseGlobally(
  const Mercury::Address&               srcAddr,
  const Mercury::UnpackedMessageHeader& header,
  BinaryIStream&                        data)
{
    ...
    {
        ERROR_MSG(
          "BaseAppMgr::registerBaseGlobally: "
    ...
    if (globalBases_.insert(value).second) {
        ...
        sendToBaseApps(
          BaseAppIntInterface::addGlobalBase,
          args,
          BaseAppsIterator(baseAndServiceApps_, BaseAppsExcluding(pSender)));
    } else {
 ...
 */
void BaseAppMgr::deregisterBaseGlobally(
  const Mercury::Address& srcAddr,
  const Mercury::UnpackedMessageHeader& /*header*/,
  BinaryIStream& data)
{
    ...
    if (it != globalBases_.end()) {
        ...
        sendToBaseApps(
          BaseAppIntInterface::delGlobalBase,
          payload,
 ...
 */
void BaseAppMgr::registerServiceFragment(
  const Mercury::Address&               srcAddr,
  const Mercury::UnpackedMessageHeader& header,
  BinaryIStream&                        data)
{
 ...
 */
void ServiceAppSubSet::addServiceFragmentFromStream(
  const Mercury::Address& srcAddr,
  BinaryIStream&          data,
  bool                    shouldInformOthers)
{
    ...
    {
        ERROR_MSG(
          "ServiceAppSubSet::registerServiceFragment: "
          "Got message from unregistered app @ %s, registration aborted\n",
          srcAddr.c_str());
    ...
    INFO_MSG(
      "ServiceAppSubSet::registerServiceFragment: Registered %s from %s\n",
      serviceName.c_str(),
      srcAddr.c_str());
    ...
    if (shouldInformOthers) {
        ...
        sendToBaseApps(BaseAppIntInterface::addServiceFragment,
                       args,
                       BaseAppsIterator(apps_));

        args.rewind();
        Mercury::Bundle& bundle = cellAppMgr_.bundle();
        bundle.startMessage(CellAppMgrInterface::addServiceFragment);
  ...
  ShutDownStage           shutDownStage)
{
    ...
    int  numDeleted =
      services_.removeFragment(serviceName, srcAddr, &didLoseService);
    ...
    sendToBaseApps(BaseAppIntInterface::delServiceFragment,
                   payload,
                   BaseAppsIterator(apps_));

    payload.rewind();
    Mercury::Bundle& bundle = cellAppMgr_.bundle();
    bundle.startMessage(CellAppMgrInterface::delServiceFragment);
 ...
 */
void BaseAppMgr::deregisterServiceFragment(
  const Mercury::Address& srcAddr,
  const Mercury::UnpackedMessageHeader& /*header*/,
  BinaryIStream& data)
{
    const bool controlledShutDown =
      serviceApps_.deregisterServiceFragment(srcAddr, data, shutDownStage_);
    if (controlledShutDown) {
 ...
 *	address.
 */
Mercury::ChannelOwner* BaseAppMgr::findChannelOwner(
  const Mercury::Address& addr)
{
