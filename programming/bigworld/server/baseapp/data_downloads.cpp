 ...
 */
int DataDownloads::addToBundle(Mercury::Bundle&    bundle,
                               int&                remaining,
 ...
 */
DataDownloads::Container::iterator DataDownloads::erase(Container::iterator it)
{
    // Reclaim the ID
                                        ...
                                        int                  id)
{
    ...
    if (!pDL->good()) {
        PyErr_Format(PyExc_ValueError, "Supplied ID (%d) already in use", id);
 ...
 */
BinaryOStream& operator<<(BinaryOStream& b, const DataDownloads& rDownloads)
{
 ...
 */
DataDownload* DataDownloadFactory::create(PyObjectPtr    pDesc,
                                          uint16         id,
                                          DataDownloads& dls)
{
...
bool DataDownloadFactory::checkDescription(PyObjectPtr& rpDesc)
{
    if (rpDesc == NULL) {
        rpDesc =
          PyObjectPtr(PyString_FromString(""), PyObjectPtr::STEAL_REFERENCE);
    }
 ...
 */
DataDownload::DataDownload(PyObjectPtr pDesc, uint16 id, DataDownloads& dls)
  : id_(id)
  , seq_(0)
  , pDesc_(pDesc)
  , bytesSent_(0)
...
{
  public:
    StringDataDownload(PyObjectPtr    pData,
                       PyObjectPtr    pDesc,
                       uint16         id,
                       DataDownloads& dls);
 ...
 *	Constructor.
 */
StringDataDownload::StringDataDownload(PyObjectPtr    pData,
                                       PyObjectPtr    pDesc,
                                       uint16         id,
                                       DataDownloads& dls)
                                                  ...
                                                  DataDownloads& dls)
{
    if (!PyString_Check(pData_.get())) {
        PyErr_SetString(PyExc_TypeError, "Data argument must be a string.");
        return NULL;
    }
...
{
  public:
    FileDataDownload(const BW::string& path,
                     PyObjectPtr       pDesc,
                     uint16            id,
                     DataDownloads&    dls);
 ...
 */
DataDownload* FileDataDownloadFactory::onCreate(PyObjectPtr    pDesc,
                                                uint16         id,
                                                DataDownloads& dls)
{
