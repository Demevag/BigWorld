...
namespace {
    ...
    {
        return BaseApp::instance().pEntityCreator()->createBaseAnywhere(args,
                                                                        kwargs);
    }
    /*~ function BigWorld.createBaseAnywhere
     *	@components{ base }
     *	The function createBaseAnywhere creates a new Base entity in the
     *BigWorld server system. The server may choose to create the Base entity on
     *any BaseApp.
     *
     ...
     *
     *	The function takes the name of the entity type to be created and
     *optionally a Python dictionary or keyword arguments to be used to provide
     *initial values for the entity.
     *
     *	Any values not provided will default to the entity's default value
     *provided by the ".def" file.
     *
     *	@param entityType Specifies the type of base entity to be	created. It
     *must be a string specifying the name of the entity type. Valid entity
     *types are those listed in scripts/entities.xml that have a base entity
     *component.
     *
     ...
     *  key is a Base property, its value will be used to initialise the Base
     *entity property. If the key is a Cell property, it will be added into the
     *	'cellData' attribute on the base entity, which is a Python dictionary,
     *and later be sent to initialise the cell entity properties.
     *
     *	If the same properties are provided multiple times via a DataSection,
     *	dictionary or keyword argument, the keyword argument has precedence and
     *then the other arguments from left to right.
     *
     *	Note: If the last argument is callable, it is not included in *params.
     *
     *	@param callback callback is an optional callable object that will be
     *called when the entity has been created. It is called with one argument
     *that is a mailbox to the new base entity on success and None on failure.
     *
    ...
    {
        return BaseApp::instance().pEntityCreator()->createBaseRemotely(args,
                                                                        kwargs);
    }
    /*~ function BigWorld.createBaseRemotely
     *	@components{ base }
     *	The function createBaseRemotely creates a new Base entity in the
     *BigWorld server system on the BaseApp specified by the baseMB argument.
     *The function takes the name of the entity type to be created and
     *optionally a Python dictionary or keyword arguments to be used to provide
     *initial values for the entity.
     *
     *	BigWorld.createBaseAnywhere should be preferred to this method and
     *	BigWorld.createBaseLocally as the server has the flexibility to create
     *the entity on an appropriately loaded BaseApp.
     *
     *	Any values not provided will default to the entity's default value
     *provided by the ".def" file.
    ...
    {
        return BaseApp::instance().pEntityCreator()->createBaseLocally(args,
                                                                       kwargs);
    }
    /*~ function BigWorld.createBaseLocally
     *	@components{ base }
     *	The function createBaseLocally creates a new Base entity in the BigWorld
     *	server system. The function takes the name of the entity type to be
     *created and optionally Python dictionaries, PyDataSections and keyword
     *arguments to be used to provide initial values for the entity.
     *
     *	Any values not provided will default to the entity's default value
     *provided by the ".def" file.
     *
     ...
     *and later be sent to initialise the cell entity properties.
     *
     *	If not specified in the ".def" file, the property will be added to the
     *base entity as a normal attribute. This behaviour is deprecated. You
     *should look to add these values to the ".def" file.
     *
     *	If the same properties are provided multiple times via a DataSection,
     *	dictionary or keyword argument, the keyword argument has precedence and
     *then the other arguments from left to right.
    ...
    PyObject* py_createBase(PyObject* args, PyObject* kwargs)
    {
        ...
        if (isFirst) {
            isFirst = false;
            WARNING_MSG(
              "py_createBase: BigWorld.createBase is deprecated as of "
     ...
     *	@components{ base }
     *	The function hasStarted returns whether or not the server has started.
     *This can be useful for identifying entities that are being created from
     *the database during a controlled start-up.
     */
     ...
     *	@components{ base }
     *	The function isShuttingDown returns whether or not the server is
     *shutting down. This return True after the onBaseAppShuttingDown
     *personality callback has been called. It can be useful for not allowing
     *operations to start when in the process of shutting down.
     */
     ...
     * 	this function was called. If the entity is already active then a
     *reference to the existing Base entity will be returned.
     *
     ...
     *	                   the entity to load. The identifier property of an
     *entity is specified by using the <Identifier> tag in the entity definition
     *file.
     *
     ...
     * 	entity, databaseID will be the database ID of the entity and wasActive
     *will indicate whether the entity was already active. If wasExisting is
     *true, then baseRef is referring to a pre-existing Base entity and may be a
     * 	mailbox rather than a direct reference to a base entity. If the
     *operation failed, then baseRef will be None, databaseID will be 0 and
     *wasActive will be false. The most common reason for failure is the that
     *entity does not exist in the database but intermittent failures like
     ...
     */
    PY_MODULE_FUNCTION_ALIAS(createBaseLocallyFromDB,
                             createBaseFromDB,
                             BigWorld)
     ...
     * 	create. The database ID of an entity is stored in the databaseID
     ...
     *timeouts or unable to allocate IDs may also occur.
     */
    bool createBaseLocallyFromDBID(const BW::string& entityType,
                                   DatabaseID        id,
                                   PyObjectPtr       pResultHandler)
    {
        return BaseApp::instance().pEntityCreator()->createBaseFromDB(
          entityType, id, pResultHandler);
    }
    PY_AUTO_MODULE_FUNCTION(
      RETOK,
      createBaseLocallyFromDBID,
     ...
     */
    PY_MODULE_FUNCTION_ALIAS(createBaseLocallyFromDBID,
                             createBaseFromDBID,
                             BigWorld)

    /*~ function BigWorld.createBaseAnywhereFromDB
     *	@components{ base }
     *	This function creates a new Base entity from data loaded from the
     *database. BigWorld can choose any BaseApp on which to create the Base
     *entity, with the aim of balancing the load between all the BaseApps.  If
     *the entity is already active then a reference to the existing Base entity
     *will be returned.
     *
     *	@param entityType This is a string specifying the type of Base entity to
     * 	load. Entity types are listed in <res>/scripts/entities.xml.
     ...
     * 	active. If wasActive is true then baseRef is a reference to a
     *pre-existing entity. If the operation failed, then baseRef will be None,
     *databaseID will be 0 and wasActive will be false. The most common reason
     *for failure is the that entity does not exist in the database but
     *intermittent failures like timeouts or unable to allocate IDs may also
     *occur.
     */
    ...
    PyObject* py_createBaseAnywhereFromDBID(PyObject* args)
    {
        ...
        PyObject*   pCallback = NULL;
        if (!PyArg_ParseTuple(args,
                              "sL|O:createBaseAnywhereFromDBID",
                              &typeStr,
                              &dbID,
                              &pCallback)) {
            return NULL;
     ...
     *	@components{ base }
     *	This function creates a new Base entity from data loaded from the
     *database. The new Base entity will be created on the same BaseApp as an
     *existing Base entity specified by the caller. If the entity is already
     *active then a reference to the active Base entity will be returned and
     *there is no guarantee that the active is on the same BaseApp as the Base
     *entity passed to this function.
     *
     *	@param entityType This is a string specifying the type of Base entity to
     * 	load. Entity types are listed in <res>/scripts/entities.xml.
     *
    ...
    PyObject* py_createBaseRemotelyFromDB(PyObject* args)
    {
        ...
        PyObject*   pCallback = NULL;
        if (!PyArg_ParseTuple(args,
                              "ssO|O:createBaseRemotelyFromDB",
                              &typeStr,
                              &identifierStr,
                              &pBaseMB,
                              &pCallback)) {
    ...
    PyObject* py_createBaseRemotelyFromDBID(PyObject* args)
    {
        ...
        PyObject*   pCallback = NULL;
        if (!PyArg_ParseTuple(args,
                              "sLO|O:createBaseRemotelyFromDBID",
                              &typeStr,
                              &dbID,
                              &pBaseMB,
                              &pCallback)) {
     ...
     *	in the BigWorld server system using a template to populate entity
     *properties. The server may choose to create the Base entity on any
     *BaseApp.
     *
     *	The function takes the name of the entity type to be created and id of
     *template to be used to provide initial values for the entity properties.
     *
     *	@param entityType Specifies the type of base entity to be created. It
     *must be a string specifying the name of the entity type. Valid entity
     *types are those listed in scripts/entities.xml that have a base entity
     *component.
     *
     *	@param templateID Specifies id of template to create entity from. It
     *must be a BLOB specifying the template.
     *
     *	@param callback callback is an optional callable object that will be
     *called when the entity has been created. It is called with one argument
     *that is a mailbox to the new base entity on success and None on failure.
     ...
     *	@return A mailbox to the new Base entity.
     */
    PyObject* createBaseAnywhereFromTemplate(const BW::string&   entityType,
                                             const BW::string&   templateID,
                                             const ScriptObject& callback)
    {
        return BaseApp::instance()
          .pEntityCreator()
          ->createBaseAnywhereFromTemplate(entityType, templateID, callback)
          .newRef();
    }
    PY_AUTO_MODULE_FUNCTION(
      RETOWN,
      ...
      ARG(BW::string,
          ARG(BW::string, OPTARG(ScriptObject, ScriptObject::none(), END))),
      BigWorld)
     ...
     */
    PyObject* createBaseLocallyFromTemplate(const BW::string& entityType,
                                            const BW::string& templateID)
    {
        return BaseApp::instance()
          .pEntityCreator()
          ->createBaseLocallyFromTemplate(entityType, templateID)
          .newRef();
    }
     ...
     *must be a BLOB specifying the template.
     *
     *	@param baseMB This could be a direct base entity reference or a mailbox
     *of type 'base', 'cell via base' or 'client via base'. The new base entity
     *will be	created on the same BaseApp as the corresponding base entity.
     *
     *	@param callback callback is an optional callable object that will be
     *called when the entity has been created. It is called with one argument
     *that is a mailbox to the new base entity on success and None on failure.
     ...
     *	@return A mailbox to the new Base entity.
     */
    PyObject* createBaseRemotelyFromTemplate(const BW::string&       entityType,
                                             const BW::string&       templateID,
                                             ...
                                             const ScriptObject&     callback)
    {
        if (EntityMailBoxRef::BASE != baseMB.component() &&
            ...
            PyErr_SetString(PyExc_ValueError,
                            ...
                            "Argument 3 must be a 'base', 'cell via base' or "
                            "'client via base' mailbox");
            return NULL;
        }
        ...
        return BaseApp::instance()
          ...
          ->createBaseRemotelyFromTemplate(
            baseMB.addr, entityType, templateID, callback)
          .newRef();
    }
    PY_AUTO_MODULE_FUNCTION(
      RETOWN,
    ...
    /*
     *	This is a bit hacky. It is used by the templatised class DBResultWaiter
     *to convert a response. This class is only currently used by one type,
                                         ...
                                         PyObject*         pAsScript)
    {
        if (pAsScript == Py_None) {
            WARNING_MSG("DBResultWaiter::handleMessage: "
                        ...
                        mailboxRef.id,
                        mailboxRef.addr.c_str());
            WARNING_MSG(
              "\tThis can occur if an entity on the local BaseApp was "
     ...
     */
    inline PyObject* convertDBResultWaiterReply(DatabaseID dbID,
                                                PyObject*  pAsScript)
    {
        return pAsScript;
    }
    ...
    */
     ...
     */
    template <class C>
    class DBResultWaiter : public Mercury::ReplyMessageHandler
    {
      public:
      ...
      private:
                           ...
                           void* /*arg*/)
        {
            ...
            if (header.length == 0) {
                ...
                // None can be returned if the mailbox is local but the entity
                // does not exist.
                if (callArg == Py_None) {
        ...
        void handleShuttingDown(const Mercury::NubException& /*ne*/,
                                void* /*arg*/)
        {
        ...
        void done(PyObject* callArg)
        {
            if (pResultHandler_) {
                ...
                Script::call(pResultHandler_,
                             Py_BuildValue("(O)", callArg),
                             "DBResultWaiter callback",
                             /*okIfFnNull:*/ false);
     ...
     *	@param callback callback is a callable object (e.g. a function) that
     *takes one argument. If deletion is successful, the callback is called with
     *the value True, if it is unsuccessful because the specified entry is
     *currently checked out, it is called with the mailbox of that base entity,
     *and if it is unsuccessful for any other reason (e.g. specified entry does
     *not exist) then it is called with the value False.
     */
    bool deleteBaseByDBID(const BW::string& entityType,
                          DatabaseID        dbID,
                          PyObjectPtr       pResultHandler)
    {
        if (dbID == 0) {
            PyErr_SetString(PyExc_ValueError,
                            "BigWorld.deleteBaseByDBID() "
                            "DatabaseID must be non-zero");
            return false;
        }

        EntityTypePtr pType =
          EntityType::getType(entityType.c_str(),
        ...
        DBAppInterface::deleteEntityArgs& dea =
          DBAppInterface::deleteEntityArgs::startRequest(
            bundle, new DBResultWaiter<EntityMailBoxRef>(pResultHandler));
     ...
     *	@param callback callback is a callable object (e.g. a function) that
     *takes one argument. If the lookup is successful, the callback is called
     *with the base mailbox; If the entity is not currently checked out, the
     *callback is called with the value True, regardless of whether the entity
     *exists in the database or not; For any other cases, it is called with the
     *value False.
     */
    bool lookUpBaseByDBID(const BW::string& entityType,
                          DatabaseID        dbID,
                          PyObjectPtr       pResultHandler)
    {
        if (dbID == 0) {
        ...
        if (dbApp.address().isNone()) {
            PyErr_Format(PyExc_RuntimeError,
                         "BigWorld.lookUpBaseByDBID() "
                         "No DBApp is available, data for %" PRI64
                         " has been lost",
                         dbID);
            return false;
        }
        ...
        DBAppInterface::lookupEntityArgs& lea =
          DBAppInterface::lookupEntityArgs::startRequest(
            bundle, new DBResultWaiter<EntityMailBoxRef>(pResultHandler));
    ...
    PY_AUTO_MODULE_FUNCTION(RETOK,
                            lookUpBaseByDBID,
                            ARG(BW::string,
                                ARG(DatabaseID, ARG(PyObjectPtr, END))),
                            BigWorld)
     ...
     *	@param callback callback is a callable object (e.g. a function) that
     *takes one argument. If the lookup is successful, the callback is called
     *with the base mailbox; If the entity is not currently checked out, but
     *exists in the database, the callback is called with the value True; For
     *any other cases, it is called with the value False.
     */
                          ...
                          PyObjectPtr       pResultHandler)
    {
        ...
        bundle.startRequest(
          DBAppInterface::lookupEntityByName,
     ...
     *	@param name Specifies the name of the entity to look up.
     *	@param callback callback is a callable object (e.g. a function) that
     *takes one argument: the database ID. If the specified entity does not
     *exist then the database ID will be 0.
     */
    ...
    /**
     *	This class represents handlers for the asynchronous calls to the
     *database to look up entities by a single property.
     */
    ...
    /**
     *	Override for Mercury::ReplyMessageHandler for accepting a reply message
     *for this request.
     */
    void LookUpBasesByIndexHandler::handleMessage(
      ...
      void*                           arg)
    {
        ...
        Script::call(
          pHandler_, PyTuple_Pack(1, pList), "BigWorld.lookUpBasesByIndex: ");
    ...
    {
        Script::call(
          pHandler_, PyTuple_Pack(1, Py_None), "BigWorld.lookUpBasesByIndex: ");
     ...
     *	@components{ base }
     *
     *	Perform an asynchronous look-up of persistent base entities stored in
     the *	primary database based on matching some of its properties. For
     efficiency, *	query properties should be an indexed property as defined in
     the entity *	definitions for the given entity type. They are specified in
     the search *	as keyword arguments to this method. The property matches
     ...
     *	If the entity is currently checked out, the base mailbox is returned as
     the *	second element of the pair. If the entity is not current checked
     out, the *	second element of the pair will be None.
     *
     ...
     *	will be called with None instead. Examples include if the property is of
     an *	inappropriate type to compare with a string (e.g. sequence and
     structured *	data types, PYTHON types, FLOAT* types).
     *
     *	Note that this method queries the primary database. If secondary
     databases *	are used, the data may be out-of-date with respect to what
     is contained in *	the secondary databases.
     *
     *	@param entityType		The name of the entity type to query for.
     *	@param callback			A callable object that is called back when
     matched *							results are received. *	@param filter
     (Optional) One of the BigWorld.LOOK_UP_ENTITIES_FILTER_constants. (default
     is	BigWorld.LOOK_UP_ENTITIES_FILTER_ALL). *	@param sortProperty
     (Optional) The property name to sort on. *	@param sortAscending
     (Optional) If True, sort in ascending or descending *
     order. Default True. *	@param limit			(Optional) Limit for the
     number of results *							returned, defaults to -1,
     indicating no limit. *	@param offset			(Optional) Offset for the
     results returned, *							defaults to 0.
     */
    PyObject* py_lookUpBasesByIndex(PyObject* args, PyObject* kwargs)
    {
        ...
        if (pEntityType == NULL) {
            PyErr_Format(
              PyExc_ValueError, "Invalid entity type \"%s\"", entityType);
            return NULL;
        }
        ...
        if (!criteria.sortProperty().empty()) {
            DataDescription* pDataDesc =
              entityDesc.findCompoundProperty(criteria.sortProperty().c_str());
            if (pDataDesc == NULL) {
                PyErr_Format(PyExc_ValueError,
        ...
        while (kwargs && PyDict_Next(kwargs, &pos, &key, &value)) {
            ...
            if (pDataDesc == NULL) {
                PyErr_Format(PyExc_ValueError,
                             ...
                             entityType);
                return NULL;
            } else if (!pDataDesc->dataType()->isSameType(ScriptObject(
                         value, ScriptObject::FROM_BORROWED_REFERENCE))) {
    ...
    PY_MODULE_FUNCTION_WITH_KEYWORDS_WITH_DOC(
      ...
      "\n"
      "If the request resulted in a database error of some kind (this does not "
      "include the case where zero entities are matched), the callback "
      "argument "
      "will be called with None instead. Examples include if the property is "
      "of an "
      "inappropriate type to compare with a string (e.g. sequence and "
      "structured "
                                   ...
                                   PyObjectPtr       pResultHandler)
    {
        ...
        return DBAppInterfaceUtils::executeRawDatabaseCommand(
          command, pResultHandler, baseApp.dbApp().channel());
    }
    PY_AUTO_MODULE_FUNCTION(RETOK,
    ...
    bool reloadScript(bool isFullReload)
    {
        ...
        if (isOK) {
            // Restore original Python interpreter. EntityType::init() got a
            // copy of the loaded modules. And EntityType::migrate() will
            // replace the modules in the original Python interpreter with new
            // modules.
            ...
            if (!isFullReload) {
                ...
                // NOTE: This works (mostly) because when we try to import a
                // module that's already imported, it doesn't try to read it
                // from the disk.
     ...
     *	BigWorld client needs to use CapsLock + F11 in order to reload the
     *scripts.
     *
     ...
     *	after reloadScript() completes. For example, if we have a custom data
     *type named "CustomClass":
     *
     *  @{
     ...
     *	@components{ base }
     *
     *	This method starts a service with the given name. If the service is
    ...
    PyObject* startService(const BW::string& serviceName)
    {
        ...
        Base* pBase =
          app.localServiceFragments().findInstanceWithType(serviceName.c_str());

        if (pBase) {
            PyErr_Format(
              PyExc_ValueError, "%s is already running", serviceName.c_str());
            return NULL;
        }
        ...
        if (!pResult) {
            PyErr_Format(
              PyExc_ValueError, "Failed to start %s", serviceName.c_str());
            return NULL;
        }
    ...
    PY_AUTO_MODULE_FUNCTION(RETOK, stopService, ARG(BW::string, END), BigWorld)

    /*
     *	This class is used to handle the response in
     *BigWorld.authenticateAccount.
     */
      ...
      public:
                                   ...
                                   void*                           arg)
        {
            ...
            if (status == LogOnStatus::LOGGED_ON) {
                ...
                ScriptObject name =
                  ScriptObject::createFrom(pType->description().name());
                ...
                isOkay =
                  deferred_.errback("BWAuthenticateError", errorMsg.c_str());
            }
     ...
     *
     *	In recovery mode, the recoveryData keyword-only argument is specified,
     *and is mutually exclusive with the other keyword-only arguments used for
     *	creation mode.
     *
     *	@param resPath 			The resource path to write out to.
     *	@param privateKey		The private key to use, in PEM string form.
     *Should only be specified for new files, should be omitted for recovery.
     *	@param recoveryData		This is a keyword-only argument, to be used in
     *							recovery mode. This parameter is used to supply
     *the serialised recovery data.
     *	@param shouldOverwrite 	This is an optional keyword-only argument, to be
     *							used in creation mode. If set to True, then any
     *							existing data will be lost. If False (the
     *default), an error is raised (via the callback) when the existing file is
     ...
     *							used in creation mode and specifies the number
     *of ticks to sign in each signing chunk. Defaults to 10.
    ...
    PyObject* py_createReplayDataFileWriter(PyObject* args, PyObject* kwargs)
    {
        ...
        if (!PyArg_ParseTuple(args,
                              "s#s#:BigWorld.createReplayDataFileWriter",
                              &resPathString,
                              &resPathStringLength,
                              &privateKeyString,
                              &privateKeyStringLength)) {
            return NULL;
        }
        ...
        if (ReplayDataFileWriter::existsForPath(path)) {
            PyErr_Format(
              PyExc_ValueError, "Already writing to path: %s", path.c_str());
            return NULL;
        }
        ...
        if ((parseResult = creationArgs.parse(kwargs)) ==
            ...
            return NULL;
        } else if (haveRecoveryData &&
                   (parseResult != KeywordParser::NONE_FOUND)) {
            PyErr_Format(
              PyExc_ValueError,
              "Cannot specify "
        ...
        if (!pChecksumScheme || !pChecksumScheme->isGood()) {
            PyErr_Format(
              PyExc_ValueError,
              "Invalid EC private key string%s%s",
        ...
        while (pMetaDataDict &&
               PyDict_Next(pMetaDataDict, &pos, &pKey, &pValue)) {
            BW::string keyString;
        ...
        pBackgroundFileWriter->initWithPath(
          pPath,
          /* shouldOverwrite */ (shouldOverwrite || haveRecoveryData),
          /* shouldTruncate */ shouldOverwrite);

        if (!haveRecoveryData) {
            return new PyReplayDataFileWriter(
              path,
              pBackgroundFileWriter,
              pChecksumScheme,
              numTicksToSign,
            ...
            return new PyReplayDataFileWriter(
              path, pBackgroundFileWriter, pChecksumScheme, recoveryData);
        }
    }
     ...
     *	@components{ base }
     *	This constant should be returned by onLogOnAttempt to allow the new
     ...
     *	attempt to log on should wait until the current Proxy has logged off.
     *Either Proxy.destroy or Proxy.destroyCellEntity should be called before
     *this is returned.
     */
    /*~ attribute BigWorld EXPOSE_BASE_SERVICE_APPS
     *	@components{ base }
     *	This flag is used by callable watchers to indicate that it is to be run
     *on all BaseApp and ServiceApp components owned by the manager.
     */
    /*~ attribute BigWorld EXPOSE_BASE_APPS
     *	@components{ base }
     *	This flag is used by callable watchers to indicate that it is to be run
     *on all BaseApp components owned by the manager.
     */
    /*~ attribute BigWorld EXPOSE_SERVICE_APPS
     *	@components{ base }
     *	This flag is used by callable watchers to indicate that it is to be run
     *on all ServiceApp components owned by the manager.
     */
    /*~ attribute BigWorld EXPOSE_LEAST_LOADED
     *	@components{ base }
     *	This flag is used by callable watchers to indicate that it is to be run
     *on the least loaded BaseApp component owned by the manager.
     */
...
namespace BigWorldBaseAppScript {
              ...
              bool                       isServiceApp)
    {
        ...
        if (PyObject_SetAttrString(
              pModule, "Base", reinterpret_cast<PyObject*>(&Base::s_type_)) ==
              -1 ||
            PyObject_SetAttrString(
              pModule, "Proxy", reinterpret_cast<PyObject*>(&Proxy::s_type_)) ==
              -1 ||
            PyObject_SetAttrString(
              pModule,
              "Service",
        ...
        addToModule(
          pModule, LOG_ON_WAIT_FOR_DESTROY, "LOG_ON_WAIT_FOR_DESTROY");

        // Add the watcher forwarding hints
        addToModule(pModule,
                    ForwardingWatcher::BASE_SERVICE_APPS,
                    "EXPOSE_BASE_SERVICE_APPS");
        addToModule(pModule, ForwardingWatcher::BASE_APPS, "EXPOSE_BASE_APPS");
        addToModule(
          pModule, ForwardingWatcher::SERVICE_APPS, "EXPOSE_SERVICE_APPS");
        addToModule(
        ...
        addToModule(
          pModule, LOOK_UP_ENTITIES_FILTER_ALL, "LOOK_UP_ENTITIES_FILTER_ALL");
        addToModule(pModule,
        ...
        addToModule(
          pModule, BW_COMPRESSION_ZIP_BEST_SPEED, "COMPRESSION_ZIP_BEST_SPEED");
        addToModule(pModule,
                    BW_COMPRESSION_ZIP_BEST_COMPRESSION,
        ...
        if (pServices) {
            ...
            if (PyObject_SetAttrString(
                  pModule, "localServices", pBases.get()) == -1) {
                return false;
            }
        }
 ...
 *  @components{ base }
 *  localServices contains a list of all the service fragments which currently
 * reside on the ServiceApp.
 *  @type PyBases
 */
