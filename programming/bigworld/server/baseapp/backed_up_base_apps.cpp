...
void BackedUpBaseApps::startAppBackup(const Mercury::Address& realBaseAppAddr,
                                      ...
                                      bool                    isInitial)
{
    INFO_MSG("BackedUpBaseApps::startAppBackup: %s backup on %s hash(id)%%%u = "
             "index %u\n",
    ...
    if (iter != apps_.end()) {
        ...
        if (!isPending) {
            ...
            if (iter->second.hasHadCurrentBackup()) {
                ...
                INFO_MSG("BackedUpBaseApps::stopAppBackup: was pending move to "
                         "new backup, "
...
/*static*/ WatcherPtr BackedUpBaseApps::pWatcher()
{
    ...
    Watcher* pMapWatcher = new MapWatcher<Container>(pNULL->apps_);
