...
class FetchDataSectionJob : public TickedWorkerJob
{
  public:
     ...
     *	loaded data section.
     */
    FetchDataSectionJob(const BW::string& path, PyObjectPtr pHandler)
      : path_(path)
      , pHandler_(pHandler)
  ...
  private:
    ...
    bool tick()
    {
        if (loadingFinished_ && pHandler_) {
            ...
            PyObject* pRes =
              PyObject_CallFunctionObjArgs(pHandler_.get(), pArg.get(), NULL);
 ...
 *one argument, the data section specified by path. If the data section could
...
class FetchFromChunksJob : public TickedWorkerJob
{
  public:
    ...
    {
      public:
        SimpleFilter(const BW::string& match)
          : match_(match)
        {
        }
    ...
    {
      public:
        void addMatch(const BW::string& match) { set_.insert(match); }
  ...
  public:
    FetchFromChunksJob(const BW::string& path,
  ...
  private:
    ...
    virtual float operator()()
    {
        ...
        if (!loadingStarted_) {
            ...
            if (pSpaceDS) {
                gridSize_ =
                  pSpaceDS->readFloat("chunkSize", g_defaultGridResolution);
    ...
    bool getNextOutdoorChunk()
    {
        ...
        m.setTranslate(
          (float)currX_ * gridSize_, 0.f, (float)currY_ * gridSize_);
    ...
    bool tick()
    {
            ...
            if (pMainThreadDS_) {
                while ((mainThreadIter_ != pMainThreadDS_->end()) &&
                    ...
                    if (pFilter_->shouldAccept(pLocalDS.get())) {
                        Matrix transform =
                          pLocalDS->readMatrix34("transform", Matrix::identity);
                        ...
                        PyObject* pResult = PyObject_CallMethod(
                          pHandler_.get(), "onSection", "OO", pPyDS, pMatrix);
        ...
        if (doneForever && pHandler_) {
            Script::call(PyObject_GetAttrString(pHandler_.get(), "onFinish"),
                         PyTuple_New(0),
                         "onFinish",
                         true);
...
PyObject* py_fetchEntitiesFromChunks(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(
          args, "sO:fetchEntitiesFromChunks", &path, &pHandler)) {
        return NULL;
    }
    ...
    // This object will be deleted when it has finished being ticked.
    new FetchFromChunksJob(
      path, pHandler, new FetchFromChunksJob::EntityFilter());
...
PyObject* py_fetchFromChunks(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(
          args, "OsO:fetchFromChunks", &pMatches, &path, &pHandler)) {
        return NULL;
    }
    ...
    if (PyString_Check(pMatches)) {
        pFilter =
          new FetchFromChunksJob::SimpleFilter(PyString_AS_STRING(pMatches));
    } else if (PySequence_Check(pMatches)) {
        std::auto_ptr<FetchFromChunksJob::SetFilter> pSetFilter(
          new FetchFromChunksJob::SetFilter());
        ...
        for (int i = 0; i < size; ++i) {
            ...
            if (PyString_Check(pItem.get())) {
                ...
                PyErr_Format(PyExc_TypeError,
                             "Item %d of the matches sequence is not a string",
                             i);
                return NULL;
            }
        }
 ...
 *	@param matches If a string is passed in, sections named this are passed to
 *the callback. If a sequence of strings is passed, any section with a name in
 *the sequence is passed to the callback.
 *	@param path The path to the space that is to be loaded.
 *	@param handler A Python object with an onSection method and optionally an
 *	onFinish method.
