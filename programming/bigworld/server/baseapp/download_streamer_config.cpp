...
{
    bytesPerTickTotal.set(ServerUtil::bpsToBytesPerTick(
      bitsPerSecondTotal(), BaseAppConfig::updateHertz()));

    bytesPerTickPerClient.set(ServerUtil::bpsToBytesPerTick(
      bitsPerSecondPerClient(), BaseAppConfig::updateHertz()));

    rampUpRateBytesPerTick.set(ServerUtil::bpsToBytesPerTick(
      rampUpRate(), BaseAppConfig::updateHertz()));

    if (bytesPerTickPerClient() <= DataDownload::packetOverhead()) {
        ERROR_MSG(
          "DownloadStreamerConfig::postInit: Download streaming will "
          "not operate, as only %u bytes can be sent per tick per client, "
          "but download streaming has a %u bytes-per-packet overhead\n",
          bytesPerTickPerClient(),
          DataDownload::packetOverhead());
        return false;
        ...
        WARNING_MSG(
          "DownloadStreamerConfig::postInit: Download streaming has "
