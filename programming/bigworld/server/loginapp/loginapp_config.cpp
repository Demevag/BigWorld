...
ServerAppOption<int> LoginAppConfig::numStartupRetries(60,
                                                       "numStartupRetries",
                                                       "");
...
{
    if (!ServerAppConfig::postInit() && ExternalAppConfig::postInit()) {
        return false;
    }

    BWConfig::update("loginApp/externalLatencyMin",
                     externalLatencyMin.getRef());
    BWConfig::update("loginApp/externalLatencyMax",
                     externalLatencyMax.getRef());
