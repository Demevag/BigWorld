...
bool DBAppMgr::init(int argc, char* argv[]) /* override */
{
    ...
    Mercury::Reason  reason = Mercury::MachineDaemon::findInterface(
      "CellAppMgrInterface", 0, cellAppMgrAddress, Config::numStartupRetries());
    ...
    reason = Mercury::MachineDaemon::findInterface(
      "BaseAppMgrInterface", 0, baseAppMgrAddress, Config::numStartupRetries());
    ...
    if (hasStarted) {
        ...
        // DBApps
        gatherLoginAppsTimer_ = mainDispatcher_.addOnceOffTimer(
          2000000,
          this,
    ...
    Mercury::MachineDaemon::registerBirthListener(
      ...
      DBAppMgrInterface::handleDBAppMgrBirth,
      "DBAppMgrInterface");

    // Register dead app callback with machined
    Mercury::MachineDaemon::registerDeathListener(
      this->interface().address(),
      DBAppMgrInterface::handleDBAppDeath,
      "DBAppInterface");

    Mercury::MachineDaemon::registerDeathListener(
      this->interface().address(),
      DBAppMgrInterface::handleLoginAppDeath,
      "LoginIntInterface");

    // Register revived manager app callbacks with machined
    Mercury::MachineDaemon::registerBirthListener(
      this->interface().address(),
      DBAppMgrInterface::handleBaseAppMgrBirth,
      "BaseAppMgrInterface");

    Mercury::MachineDaemon::registerBirthListener(
      this->interface().address(),
      DBAppMgrInterface::handleCellAppMgrBirth,
      "CellAppMgrInterface");
    ...
    BW_REGISTER_WATCHER(
      0, "dbappmgr", "dbAppMgr", mainDispatcher_, this->interface().address());
...
void DBAppMgr::handleBaseAppDeath(BinaryIStream& data)
{
    ...
    for (DBApps::const_iterator iter = dbApps_.begin(); iter != dbApps_.end();
         ++iter) {
                        ...
                        const Mercury::UnpackedMessageHeader& header)
{
    ...
    if (startupState_ == STARTUP_STATE_INDETERMINATE) {
        ...
        // should retry.
        Mercury::Channel& channel = this->getChannel<DBAppMgr>(srcAddr);
    ...
    DBAppPtr pDBApp =
      new DBApp(interface_, srcAddr, lastDBAppID_, header.replyID);
                            ...
                            const DBAppMgrInterface::recoverDBAppArgs& args)
{
    ...
    DBAppPtr pDBApp =
      new DBApp(interface_, srcAddr, id, Mercury::REPLY_ID_NONE);
 ...
 */
void DBAppMgr::handleDBAppDeath(
...
void DBAppMgr::sendDBAppHashUpdate(bool haveNewAlpha)
{
    ...
    if (haveNewAlpha) {
        const Mercury::Address newAlpha =
          (!dbApps_.empty() ? this->dbAppAlpha()->address()
        ...
        for (LoginApps::iterator iter = loginApps_.begin();
             iter != loginApps_.end();
             ++iter) {
            Mercury::Channel& channel = this->getChannel<DBAppMgr>(*iter);
...
{
    const Mercury::Address alphaAddress =
      (!dbApps_.empty() ? this->dbAppAlpha()->address()
                        : Mercury::Address::NONE);
                           ...
                           BinaryIStream& /* data */)
{
    ...
    Mercury::Address dbAppAlphaAddress =
      !dbApps_.empty() ? this->dbAppAlpha()->address() : Mercury::Address::NONE;
...
void DBAppMgr::serverHasStarted(bool hasStarted /* = true */)
{
    ...
    startupState_ =
      (hasStarted ? STARTUP_STATE_STARTED : STARTUP_STATE_NOT_STARTED);
...
void DBAppMgr::addWatchers()
{
    ...
    dbAppsWatcher->addChild(
      "*", new SmartPointerDereferenceWatcher(DBApp::pWatcher()));

    root.addChild("dbApps", dbAppsWatcher, &dbApps_);

    root.addChild("alphaAppID", makeWatcher(&DBAppMgr::alphaID), this);
}
