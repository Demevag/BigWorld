...
void Cluster::chooseBuddy()
{
    ...
    for (Addresses::iterator it = machines_.begin(); it != machines_.end();
         ++it) {
...
TimeQueue::TimeStamp Cluster::FloodTriggerHandler::delay() const
{
    ...
    TimeQueue::TimeStamp max = std::max(
      min + 1,
...
void Cluster::FloodReplyHandler::handleTimeout(TimerHandle handle, void* pUser)
{
    ...
    bool births = false;
    for (Addresses::iterator it = replied_.begin(); it != replied_.end();
         ++it) {
        if (cluster_.machines_.find(*it) == cluster_.machines_.end()) {
            births = true;
            cluster_.machines_.insert(*it);
            syslog(
              LOG_INFO, "Discovered new machine %s", inet_ntoa((in_addr&)*it));
        }
    }
...
{
    if (!wmm_.sendto(cluster_.machined_.endpoint(),
                     htons(PORT_MACHINED),
                     BROADCAST,
...
void Cluster::BirthReplyHandler::addTimer()
{
    ...
    mam.count_ = 0;
    mam.sendto(cluster_.machined_.endpoint(),
               htons(PORT_MACHINED),
               BROADCAST,
               MGMPacket::PACKET_STAGGER_REPLIES);
