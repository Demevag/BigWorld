...
void BWMachined::initNetworkInterfaces()
{
    ...
    if (broadcastAddr_ == 0 && !this->findBroadcastInterface()) {
        syslog(LOG_CRIT,
               "Failed to determine default broadcast interface. "
...
bool BWMachined::findBroadcastInterface()
{
    ...
    if (!epListen.good() ||
        ...
        syslog(LOG_CRIT,
               "Couldn't bind broadcast listener socket to port %d",
               PORT_BROADCAST_DISCOVERY);
        return false;
    }
    ...
    bool                  ok = qim.sendto(epListen,
                         htons(PORT_BROADCAST_DISCOVERY),
                         BROADCAST,
    ...
    while (1) {
        ...
        if (selgot == 0) {
            ...
            syslog(LOG_CRIT,
                   "Broadcast discovery select error. %s.",
                   strerror(errno));
            return false;
        } else {
            sockaddr_in sin;
            ...
            if (len == -1) {
                syslog(LOG_ERR,
                       "Broadcast discovery recvfrom error. %s.",
                       strerror(errno));
                continue;
            }

            syslog(LOG_INFO,
                   "Broadcast discovery receipt from %s.",
...
void BWMachined::save()
{
    ...
    if (out.error()) {
        syslog(LOG_ERR,
               "Couldn't write process state to %s: %s",
               STATE_FILE,
               out.strerror());
        unlink(STATE_FILE);
        return;
    ...
    if (out.error()) {
        syslog(LOG_ERR,
               "Failed to write process table to %s: %s",
               STATE_FILE,
               out.strerror());
        unlink(STATE_FILE);
        return;
    ...
    if (procs_.size() > 0) {
        syslog(LOG_INFO,
               "Wrote %" PRIzu " entries to %s prior to shutdown",
               procs_.size(),
               STATE_FILE);
    }
}
...
void BWMachined::load()
{
    ...
    if (in.stat(&statinfo) == 0) {
        ...
        if (age > 10 * 60) {
            syslog(LOG_INFO,
                   "Ignoring out-of-date %s (%d seconds old)",
                   STATE_FILE,
                   (int)age);
            unlink(STATE_FILE);
            return;
        ...
        syslog(LOG_ERR, "Couldn't stat %s: %s", STATE_FILE, in.strerror());
        return;
    }
    ...
    while (in.tell() < len) {
        ...
        MemoryIStream processBuffer(processBufferStr.data(),
                                    processBufferStr.size());
        ...
        if (!validateProcessInfo(pi)) {
            syslog(LOG_ERR,
                   "Failed to restore %s (pid:%d uid:%d).",
                   pi.m.name_.c_str(),
                   pi.m.pid_,
                   pi.m.uid_);
        } else {
    ...
    if (len > 0) {
        syslog(LOG_INFO,
               "Restored %" PRIzu " of %u entries from %s",
               procs_.size(),
               totalEntries,
               STATE_FILE);
    }
 ...
 */
const char* BWMachined::findOption(const char* optionName,
                                   const char* oldOptionName)
{
    ...
    while (isOkay && (fgets(buf, sizeof(buf), file) != NULL)) {
        ...
        if (buf[0] == '[') {
            ...
            if (buf[len - 1] == '\n') {
                ...
                syslog(
                  LOG_ERR, "Max tag length is %" PRIzu "\n", sizeof(buf) - 1);
                isOkay = false;
            }
        }
    }
...
bool BWMachined::readConfigFile()
{
    ...
    if (file == NULL) {
        syslog(
          LOG_WARNING, "Global config file %s doesn't exist", machinedConfFile);
        return true;
    }
    ...
    if (file == NULL) {
        syslog(
          LOG_WARNING, "Global config file %s doesn't exist", bigworldConfFile);
    } else {
    ...
    if ((optionValue != NULL) && (optionValue[0] != '\0')) {
        syslog(LOG_INFO,
               "The 'architecture' configuration option has been "
               "deprecated. Ignoring.");
    }
    ...
    if (optionValue) {
        ...
        if (inet_aton(internalIfaceStr.c_str(), (in_addr*)(&broadcastAddr_))) {
            Interfaces::iterator iIface = interfaces.find(broadcastAddr_);

            if (iIface == interfaces.end()) {
                syslog(LOG_ERR,
                       "Could not find configured internal interface "
                ...
                syslog(LOG_INFO,
                       "Using configured internal interface "
            ...
            if (!found) {
                syslog(LOG_ERR,
                       "Could not find configured internal "
    ...
    if (optionValue) {
        ...
        if (sscanf(optionValue, "%d", &tmpMaxPacketDelay) != 1) {
            syslog(LOG_ERR,
                   "Failed to parse MaxPacketDelay value. "
                   "Using default value %dms",
                   maxPacketDelayMillisec_);
        } else {
            syslog(LOG_INFO,
                   "Setting maximum packet delay to %dms",
                   tmpMaxPacketDelay);
...
int BWMachined::run()
{
    ...
    if (!pidPath_.empty()) {
        ...
        if (pidFile == NULL) {
            syslog(LOG_INFO,
                   "Cannot access PID file %s, error: %s",
                   pidPath_.c_str(),
                   strerror(errno));
        ...
        if (fprintf(pidFile, "%d", mf_getpid()) < 0) {
            syslog(LOG_INFO,
                   "Cannot write to PID file %s, error: %s",
                   pidPath_.c_str(),
                   strerror(errno));
            exitCode = EXIT_FAILURE;
            fclose(pidFile);
    ...
    while (g_serverRunning) {
        ...
        int selgot =
          select(std::max(maxfd + 1, osmaxfd + 1), &fds, NULL, NULL, &tv);
...
void BWMachined::update()
{
    ...
    for (unsigned int i = 0; i < procs_.size(); i++) {
        ...
        if (!updateProcessStats(pi) && errno == ENOENT) {
            syslog(LOG_ERR,
                   "%s (uid:%d) died without deregistering!\n",
                   pi.m.c_str(),
                   pi.m.uid_);
...
bool BWMachined::broadcastToListeners(ProcessMessage& pm, int type)
{
    ...
    bool ok = pm.sendto(
      ep_, htons(PORT_MACHINED), BROADCAST, MGMPacket::PACKET_STAGGER_REPLIES);
...
void BWMachined::removeRegisteredProc(unsigned index)
{
    if (index >= procs_.size()) {
        syslog(LOG_ERR,
               "Can't remove reg proc at index %d/%" PRIzu "",
               index,
               procs_.size());
        return;
    }
...
void BWMachined::sendSignal(const SignalMessage& sm)
{
    for (uint i = 0; i < procs_.size(); i++) {
        ...
        if (pm.m.matches(sm)) {
            ...
            syslog(LOG_INFO,
                   "sendSignal: signal = %d pid = %d uid = %d",
                   sm.signal_,
                   pm.m.pid_,
                   pm.m.uid_);
        }
 ...
 */
void BWMachined::handlePacket(Endpoint& ep, sockaddr_in& sin, MGMPacket& packet)
{
    MGMPacket replies;
    ...
    if (replies.messages_.size() > 0) {
        ...
        if (replies.write(os)) {
            ...
            // MGMPacket.
            syslog(LOG_ERR,
                   "handlePacket: Unable to generate reponse to "
                   "message from %s.",
                   ep.c_str());
        }
    }
 ...
 *
 */
bool BWMachined::handleMessage(Endpoint&            ep,
                               sockaddr_in&         sin,
    ...
    switch (mgm.message_) {
        ...
        case MachineGuardMessage::LISTENER_MESSAGE: {
            ...
            if (lm.param_ == (lm.ADD_BIRTH_LISTENER | lm.PARAM_IS_MSGTYPE)) {
                birthListeners_.add(lm, sin.sin_addr.s_addr);
                       ...
                       (lm.ADD_DEATH_LISTENER | lm.PARAM_IS_MSGTYPE)) {
                deathListeners_.add(lm, sin.sin_addr.s_addr);
            } else {
                syslog(LOG_ERR,
                       "Unrecognised param field for ListenerMessage: %x",
                       lm.param_);
                return false;
            }
        ...
        case MachineGuardMessage::WHOLE_MACHINE_MESSAGE: {
            ...
            if (mgm.outgoing()) {
                ...
                if (cluster_.pFloodReplyHandler_ &&
                    ...
                    syslog(LOG_ERR,
                           "Unsolicited WMM from %s:%d",
                           inet_ntoa((in_addr&)inaddr),
                           ntohs(sin.sin_port));
                }
            }

            // These are regular requests that could be coming from tools or
            // other machineds
            else {
        ...
        case MachineGuardMessage::MACHINE_PLATFORM_MESSAGE: {
            MachinePlatformMessage& mpm =
              static_cast<MachinePlatformMessage&>(mgm);
        ...
        case MachineGuardMessage::PROCESS_MESSAGE: {
            ...
            if (!(pm.param_ & pm.PARAM_IS_MSGTYPE)) {
                syslog(LOG_ERR,
                       "PROCESS_MESSAGE tried to use param filters! (%x)",
                       pm.param_);
                return false;
            }
            ...
            if ((msgtype == pm.REGISTER || msgtype == pm.DEREGISTER) &&
                ...
                syslog(LOG_ERR,
                       "%s tried to register a process here!",
            ...
            switch (msgtype) {
                case ProcessMessage::REGISTER: {
                    // Make sure this new proc doesn't clash with an existing
                    // one
                    unsigned int i = 0;
                    while (i < procs_.size()) {
                    ...
                    if (i < procs_.size()) {
                        syslog(LOG_ERR,
                               "Received re-registration for %s\n",
                               pm.c_str());
                    ...
                    // Write the registration info into the ProcessStatsMessage
                    // and mark it as outgoing for future sends
                    ProcessInfo& pi = procs_[i];
                    pi.m << pm;
                    pi.m.outgoing(true);
                    ...
                    syslog(LOG_INFO,
                           "Added %s for uid:%d (debug: at index %d)\n",
                           pm.c_str(),
                           pm.uid_,
                           i);
                    return true;
                ...
                case ProcessMessage::DEREGISTER: {
                    ...
                    if (i >= procs_.size())
                        syslog(LOG_ERR,
                               "Couldn't find pid %d to deregister it\n",
                               pm.pid_);
                    else
                        removeRegisteredProc(i);
                ...
                case ProcessMessage::NOTIFY_BIRTH: {
                    birthListeners_.handleNotify(
                      this->endpoint(), pm, sin.sin_addr);
                    return true;
                }

                case ProcessMessage::NOTIFY_DEATH: {
                    deathListeners_.handleNotify(
                      this->endpoint(), pm, sin.sin_addr);
                    return true;
                }
                ...
                default:
                    syslog(
                      LOG_ERR, "Unrecognised ProcessMessage type: %d", msgtype);
                    return false;
            }
        }
            ...
            for (BW::vector<ProcessInfo>::iterator it = procs_.begin();
                ...
                if (pi.m.matches(query)) {
                    ...
                    uint64      cpuDiff = std::max(
                      si.cpu[pi.affinity % si.nCpus].max.delta(), (uint64)1);

                    if (pi.affinity >= (int)si.nCpus)
                        syslog(LOG_ERR,
                               "ProcessInfo (%s) has invalid affinity %d",
                               pi.m.c_str(),
                               pi.affinity);
                    ...
                    reply.mem_ =
                      (uint8)(pi.mem.cur() * 0xff / si.mem.max.cur());
        ...
        case MachineGuardMessage::TAGS_MESSAGE: {
            ...
            if (tm.tags_.size() != 1) {
                syslog(LOG_ERR,
                       "Tags queries must pass only one tag (%" PRIzu
                       " passed)",
                       tm.tags_.size());
                return false;
            }
            ...
            // An empty query indicates we're asking for the list of tag
            // categories
            if (query == "") {
                for (TagsMap::iterator it = tags_.begin(); it != tags_.end();
                     ++it)
        ...
        return true;                                                           \
            ...
            for (unsigned i = 0; i < matches.size(); i++) {
                ...
                if (um.param_ & um.PARAM_CHECK_COREDUMPS) {
                    pServerPlatform_->checkCoreDumps(
                      um.uid_, matches[i]->mfroot_, (*matches[i]).coredumps_);
                }

                if (um.param_ & um.PARAM_GET_VERSION) {
                    pServerPlatform_->determineVersion(
                      um.uid_, matches[i]->mfroot_, matches[i]->versionString_);
                }
...
#undef FAIL
        ...
        case MachineGuardMessage::RESET_MESSAGE: {
            ...
            users_.flush();
            syslog(LOG_INFO,
                   "Flushing tags and user mapping at %s's request",
                   inet_ntoa(sin.sin_addr));
            rm.outgoing(true);
            replies.append(rm);
            return true;
        ...
        case MachineGuardMessage::MACHINED_ANNOUNCE_MESSAGE: {
            ...
            if (mam.type_ == mam.ANNOUNCE_BIRTH) {
                ...
                // When machines initially announce their own birth, we tell
                // them how many machines are on the cluster
                else {
                    uint32 inaddr = sin.sin_addr.s_addr;
            ...
            // This is confirming that a particular host exists
                ...
                if (cluster_.machines_.count(mam.addr_) == 0) {
                    syslog(LOG_INFO,
                           "Apparently %s is running machined",
        ...
        case MachineGuardMessage::QUERY_INTERFACE_MESSAGE: {
            QueryInterfaceMessage& qim =
              static_cast<QueryInterfaceMessage&>(mgm);

            if (qim.address_ == QueryInterfaceMessage::INTERNAL) {
                qim.address_ = broadcastAddr_;
            } else {
                syslog(LOG_ERR,
                       "Received QIF request for unknown type %d\n",
                       qim.address_);
            }
        ...
        default:
            syslog(LOG_ERR,
                   "Unknown message (%d) not marked as "
 ...
 *
 */
bool BWMachined::handleCreateMessage(Endpoint&            ep,
                                     sockaddr_in&         sin,
                                     MachineGuardMessage& mgm,
                                     MGMPacket&           replies)
{
    ...
    if (pUm == NULL) {
        ...
        if (ent == NULL) {
            syslog(LOG_ERR,
                   "UID %d doesn't exist on this system, "
                   "not starting %s",
                   cm.uid_,
                   cm.name_.c_str());
            pPmwd->running_ = 0;
    ...
    if (!users_.getEnv(*pUm)) {
        syslog(LOG_ERR,
               "Couldn't get env for user %s, not starting %s",
               pUm->username_.c_str(),
               cm.c_str());
        pPmwd->running_ = 0;
        replies.append(*pPmwd, true);
    ...
    if ((bw_stricmp(BW_CONFIG_HYBRID.c_str(), pConfig) != 0) &&
        ...
        syslog(LOG_ERR,
               "Rejected process start request for user %s for "
    ...
    if (mgm.message_ == MachineGuardMessage::CREATE_MESSAGE) {
        ...
        if (argc >= MAX_ARGC) {
            syslog(LOG_ERR,
                   "Not starting process %s for uid %d: "
                   "too many args (%d)",
                   cm.name_.c_str(),
                   cm.uid_,
                   argc);
        ...
        if (!pServerPlatform_->findUserBinaryDirForConfig(
              pUm->mfroot_, cm.config_, binaryDir)) {
            syslog(LOG_ERR,
                   "Not starting process %s for uid %d: "
    ...
    {
        CreateWithArgsMessage& cwam = static_cast<CreateWithArgsMessage&>(cm);

        unsigned int argc = 0;
        // +2 args for process name and -recover
        const char** argv =
          new const char*[cwam.args_.size() + NUM_SPARE_ARGS_FOR_STARTPROCESS +
                          2];
        ...
        for (CreateWithArgsMessage::Args::const_iterator i = cwam.args_.begin();
             i != cwam.args_.end();
             ++i) {
            argv[argc++] = i->c_str();
        }
 ...
 *
 */
void BWMachined::PacketTimeoutHandler::handleTimeout(TimerHandle handle,
                                                     void*       pUser)
{
