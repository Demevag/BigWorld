...
bool getProcessTimes(FILE*              f,
                     unsigned long int* utimePtr,
                     unsigned long int* stimePtr,
                     unsigned long int* vsizePtr,
                     unsigned long int* starttimePtr,
                     int*               cpuPtr);
...
namespace {
    ...
    typedef BW::map<int, std::pair<PidMessageWithDestination*, BW::string>>
                             PendingProcessMap;
...
void handleInterestingFds(fd_set* readfds, fd_set* writefds, fd_set* exceptfds)
{
    ...
    while (it != s_pendingProcesses.end()) {
        ...
        if (readlen == 0) {
            ...
            syslog(LOG_ERR,
                   "Error checking child status: %s, assuming exec "
                   "for %s failed",
                   strerror(errno),
                   binaryName.c_str());
            pPmwd->pid_     = 0;
...
void checkSocketBufferSizes()
{
    ...
    if ((writeBufSize != -1) && (writeBufSize < MIN_SND_SKT_BUF_SIZE)) {
        syslog(LOG_ERR, errorMsg, "Write", MIN_SND_SKT_BUF_SIZE, writeBufSize);
    }
}
...
bool updateProcessStats(ProcessInfo& pi)
{
    ...
    bw_snprintf(
      pinfoFilename, sizeof(pinfoFilename), "/proc/%d/stat", (int)pi.m.pid_);

    FILE* pinfo;
    if ((pinfo = fopen(pinfoFilename, "r")) == NULL) {
        if (errno != ENOENT) {
            syslog(
              LOG_ERR, "Couldn't open %s: %s", pinfoFilename, strerror(errno));
        }
    ...
    if (!getProcessTimes(pinfo, &utime, &stime, &vsize, &starttime, &cpu)) {
        syslog(LOG_ERR,
               "Failed to update process stats for '%s': %s",
               pinfoFilename,
               strerror(errno));
        fclose(pinfo);
        return false;
    ...
    if ((pi.starttime) && (pi.starttime != starttime)) {
        syslog(LOG_ERR,
               "updateProcessStats: Process %d starttime differs from "
                  ...
                  PidMessageWithDestination*  pPmwd)
{
    ...
    if ((childpid = fork()) == 0) {
        ...
        if (setuid(uid) == -1) {
            syslog(LOG_ERR,
                   "Failed to setuid to %d, aborting exec for %s\n",
                   uid,
                   binaryName);
            write(statusPipe[1], &errno, sizeof(errno));
            exit(EXIT_FAILURE);
        ...
        if (stat(path, &statbuffer) != 0) {
            syslog(LOG_ERR, "Unable to stat '%s': %s", path, strerror(errno));
        }
...
bool validateProcessInfo(const ProcessInfo& processInfo)
{
    // try to open the file
    char pinfoFilename[64];
    bw_snprintf(pinfoFilename,
                sizeof(pinfoFilename),
                "/proc/%d/stat",
                (int)processInfo.m.pid_);
    ...
    if ((status) && (processInfo.starttime) &&
        (starttime != processInfo.starttime)) {
        syslog(LOG_ERR,
               "validateProcessInfo: Process %d starttime differs "
...
bool raiseFileDescriptorHardLimit(unsigned long desiredLimit)
{
    ...
    if (fp != NULL) {
        ...
        if (numLinesRead != 1) {
            syslog(
              LOG_ERR, "Invalid line in %s: '%s'", kernel_nr_open_file, line);
            return false;
        }
        ...
        if (desiredLimit > nr_open) {
            syslog(LOG_WARNING,
                   "Requested file descriptor limit "
    ...
    if (bw_prlimit(/*pid*/ 0, RLIMIT_NOFILE, NULL, &nofile) == -1) {
        syslog(
          LOG_ERR, "Unable to get file descriptor limits: %s", strerror(errno));
        return false;
    }
    ...
    if (nofile.rlim_max < desiredLimit) {
        ...
        if (bw_prlimit(/*pid*/ 0, RLIMIT_NOFILE, &nofile, NULL) == -1) {
            syslog(LOG_ERR,
                   "Unable to set file descriptor limits to %lu: %s",
                   desiredLimit,
                   strerror(errno));
            return false;
        }
