 ...
 *	Constructor.
 */
FileReceiver::FileReceiver(int              socket,
                           uint32           ip,
                           uint16           port,
                           FileReceiverMgr& mgr)
  : mgr_(mgr)
  ...
  , destFile_(NULL)
{
    //	DEBUG_MSG( "FileReceiver::FileReceiver: Accepted incoming connection
    // from "
    //			"%s\n", srcAddr_.c_str() );
    ...
    mgr_.dispatcher().registerFileDescriptor(
      endPoint_.fileno(), this, "FileReceiver");
}
...
size_t FileReceiver::recvCommand()
{
    ...
    const char* pCommand = reinterpret_cast<const char*>(msgReceiver_.msg());
...
size_t FileReceiver::recvSrcPath()
{
    ...
    if (pos == BW::string::npos)
        pos = 0;
    destPath_.insert(
      destPath_.end(), srcPath_.begin() + pos + 1, srcPath_.end());
