  ...
  , public BWStreamDataHandler
{
    Py_Header(ClientApp, PyObjectPlusWithWeakReference)
      ...
      public
      : ClientApp(const BW::string&   name,
    ...
    bool setMovementController(const BW::string& type, const BW::string& data);
  ...
  private:
    ...
    class TimerRec
    {
      public:
        ...
        bool elapsed(float gameTime) const { return finTime() <= gameTime; }
    ...
    PY_AUTO_METHOD_DECLARE(RETVOID, moveTo, ARG(Vector3, END));
    PY_AUTO_METHOD_DECLARE(RETVOID, faceTowards, ARG(Vector3, END));
    PY_AUTO_METHOD_DECLARE(RETVOID, snapTo, ARG(Vector3, END));
    PY_AUTO_METHOD_DECLARE(RETVOID, stop, END);

    PY_AUTO_METHOD_DECLARE(RETDATA,
                           addTimer,
                           ARG(float, ARG(PyObjectPtr, ARG(bool, END))));
    PY_AUTO_METHOD_DECLARE(RETVOID, delTimer, ARG(int, END));
    ...
    void onGeometryMapping(BW::SpaceID       spaceID,
                           BW::Matrix        matrix,
