...
bool EntityType::init(const BW::string& standInEntity)
{
    ...
    if (!standInEntity.empty()) {
        ScriptModule standInEntityModule =
          ScriptModule::import(standInEntity.c_str(), errorPrint);
    ...
    for (EntityTypeID i = 0; i < entityDescriptionMap_.size(); ++i) {
        ...
        if (currDesc.name() != standInEntity) {
            ScriptModule entityModule =
              ScriptModule::import(currDesc.name().c_str(), errorPrint);

            if (entityModule) {
                entityType = entityModule.getAttribute(currDesc.name().c_str(),
                                                       ScriptErrorRetain());
            } else {
        ...
        s_types_[currDesc.clientIndex()] = new EntityType(currDesc, entityType);
...
{
    return EntityDefConstants(
      entityDescriptionMap_.digest(),
                                    ...
                                    bool           isPlayer) const
{
    ...
    for (uint i = 0; i < description_.propertyCount(); i++) {
        ...
        PyDict_SetItemString(
          pDict, const_cast<char*>(pDD->name().c_str()), pValue.get());
    }
                                    ...
                                    StreamContents contents) const
{
    ...
    if (contents < TAGGED_CELL_PLAYER_DATA) {
        ...
        for (uint8 i = 0; i < size; i++) {
            ...
            if (pDD != NULL) {
                ScriptDataSink sink;
                MF_VERIFY(pDD->createFromStream(stream,
                                                sink,
                ...
                if (!pDict.setItem(
                      pDD->name().c_str(), pValue, ScriptErrorRetain())) {
