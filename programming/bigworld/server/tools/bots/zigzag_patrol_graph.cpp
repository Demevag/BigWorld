        ...
        class ZigzagPatrolFactory : public MovementFactory
        {
          public:
                                       ...
                                       Vector3&          position)
            {
                ...
                if (!pGraph) {
                    PyErr_Format(
                      PyExc_IOError, "Unable to load %s", graphName.c_str());
                    return NULL;
                } else if (corridorWidth > 0)
                    return new ZigzagGraphTraverser(
                      *pGraph, speed, position, corridorWidth);
                else
