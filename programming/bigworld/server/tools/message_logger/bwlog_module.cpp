...
PyMODINIT_FUNC init_bwlog()
{
    ...
    PyObject*   m =
      Py_InitModule3((char*)MODULE_NAME, NULL, "Interface to BWLog files");
...
#define INSERT_CONSTANT(NAME) PyModule_AddIntConstant(m, #NAME, NAME);
...
#define INSERT_SEVERITY(LEVEL)                                                 \
    PyDict_SetItemString(                                                      \
      severityLevels, messagePrefix(LEVEL), PyInt_FromLong(LEVEL));
    ...
    for (int messageSourceIndex = 0; messageSourceIndex < NUM_MESSAGE_SOURCE;
         ++messageSourceIndex) {
        PyDict_SetItemString(
          sourceTypes,
