  ...
  MemoryIStream&                inputStream)
{
    ...
    BW::string    hostname;
    GetHostResult result =
      this->getHostnames()->getHostByAddr(address.ip, hostname);
    ...
    return this->writeLogToDB(
      componentMessage, address, inputStream, header, pHandler, categoryID);
}
