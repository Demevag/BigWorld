...
namespace MLMetadata {
    ...
    namespace {
        /**
         * Check whether the most recent operation on the memory stream was
         * successful. Generates a message if there was an error.
         *
     ...
     */
    MLDBMetadataStream::MLDBMetadataStream()
      : jsonAdded_(false)
    {
    }
        ...
        while (inputStream.remainingLength()) {
            ...
            if (type == LogMetaData::MLVT_JSON_STRING) {
                ...
                if (outputStream.setJSON(buffer)) {
                    // A valid JSON argument overrides all others, so nothing
                    // left to process.
                    return;
                }
