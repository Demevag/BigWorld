...
namespace MongoDB {
    ...
    // Map of Per-user log buffers which will be appended to until they are
    // consumed by a flush (ie. copied to a WriteTask).
  ...
  , public TimerHandler
{
  public:
    ...
    bool setAppInstanceID(const Mercury::Address& addr, ServerAppInstanceID id);
  ...
  private:
    ...
    mongo::BSONObj createBson(const LoggerComponentMessage& componentMessage,
                              const Mercury::Address&       address,
                              MemoryIStream&                inputStream,
                              const LoggerMessageHeader&    header,
                              LogStringInterpolator*        pHandler,
