...
LogStorageMongoDB::LogStorageMongoDB(Logger& logger)
  ...
  , loggerID_(logger.getLoggerID().empty() ? MongoDB::LOGGER_ID_NA
                                           : logger.getLoggerID())
  ...
  , canAddLogs_(false)
  , hostnames_(mongoDBTaskMgr_,
               conn_,
               commonDBName_ + "." + MongoDB::HOST_NAMES_COLL)
  , componentNames_(mongoDBTaskMgr_,
                    conn_,
                    commonDBName_ + "." + MongoDB::COMPONENTS_COLL)
  , formatStrings_(mongoDBTaskMgr_,
                   conn_,
                   commonDBName_ + "." + MongoDB::FMT_STRS_COLL)
  , categories_(mongoDBTaskMgr_,
                conn_,
                commonDBName_ + "." + MongoDB::CATEGORIES_COLL)
  , currentSecond_(0)
  , currentCounter_(0)
{
}
 ...
 */
bool LogStorageMongoDB::init(const ConfigReader& config, const char* /*pRoot*/)
{
    // init MongoDB config
    ...
    pConnectionThreadData_ = new MongoDB::ConnectionThreadData(
      connectionInfo_, this->getExpireLogsSeconds(), loggerID_);

    mongoDBTaskMgr_.startThreads(
      "MongoDBTaskManager", 1, pConnectionThreadData_);
 ...
 */
bool LogStorageMongoDB::stopLoggingFromComponent(const Mercury::Address& addr)
{
  ...
  MemoryIStream&                inputStream)
{
    ...
    if (connected && !canAddLogs_) {
        INFO_MSG(
          "LogStorageMongoDB::addLogMessage: "
    ...
    if (!canAddLogs_) {
        ...
        if (mongoDBTaskMgr_.bgTaskListSize() == 0) {
            ...
            mongoDBTaskMgr_.addBackgroundTask(pReconnectTask, TaskManager::MAX);
        }
  ...
  MessageLogger::CategoryID     categoryID)
{
    ...
    if (it != userLogs_.end()) {
        ...
        if (reason != Mercury::REASON_SUCCESS) {
            ERROR_MSG(
              "LogStorageMongoDB::writeLogToDB: "
    ...
    try {
        logBson = this->createBson(componentMessage,
                                   address,
                                   inputStream,
                                   header,
                                   pHandler,
    ...
    if (BW_TOOLS_COMPONENT_NAME == componentMessage.componentName_) {
        BW::string msg = logBson.getStringField(MongoDB::ENTRIES_KEY_MESSAGE);

        if (BW_TOOLS_SERVER_START_MSG == msg) {
            MongoDB::AddServerStartTask* pAddServerStartTask =
              new MongoDB::AddServerStartTask(
                componentMessage.uid_, pUserLogBuffer->getUsername(), currTime);
...
bool LogStorageMongoDB::initFromConfig(const ConfigReader& config)
{
    ...
    // host
    if (config.getValue(
          MongoDB::CONFIG_NAME_MONGO, "host", connectionInfo_.dbHost_)) {
    ...
    // user name
    if (config.getValue(
          MongoDB::CONFIG_NAME_MONGO, "user", connectionInfo_.dbUser_)) {
    ...
    // password
    if (config.getValue(
          MongoDB::CONFIG_NAME_MONGO, "password", connectionInfo_.dbPasswd_)) {
    ...
    // tcp time out
    if (config.getValue(
          MongoDB::CONFIG_NAME_MONGO, "tcp_time_out", tmpString)) {
        if (sscanf(tmpString.c_str(), "%u", &connectionInfo_.tcpTimeout_) !=
            1) {
            ERROR_MSG("LogStorageMongoDB::initFromConfig: "
    ...
    if (config.getValue(
          MongoDB::CONFIG_NAME_MONGO, "wait_between_reconnects", tmpString)) {
        if (sscanf(tmpString.c_str(),
                   "%hu",
    ...
    // max log buffer size
    if (config.getValue(
          MongoDB::CONFIG_NAME_MONGO, "max_buffered_lines", tmpString)) {
        if (sscanf(tmpString.c_str(), "%u", &maxBufSize_) != 1) {
            ERROR_MSG(
              "LogStorageMongoDB::initFromConfig: "
    ...
    // flush interval
    if (config.getValue(
          MongoDB::CONFIG_NAME_MONGO, "flush_interval", tmpString)) {
    ...
    if (config.getValue(
          MongoDB::CONFIG_NAME_MONGO, "expire_logs_days", tmpString)) {
                                    ...
                                    MongoDB::ConnectionInfo&   connectionInfo)
{
    try {
        ...
        if (!connection.auth(MongoDB::ADMIN_DB_NAME,
                             ...
                             connectionInfo.dbPasswd_.c_str(),
                             errMsg)) {
            ERROR_MSG("LogStorageMongoDB::connectToDB: "
...
bool LogStorageMongoDB::initCommonCollections()
{
    ...
    try {
        ...
        if (!this->initSourcesColl(collList) || !initSeveritiesColl(collList) ||
 ...
 */
bool LogStorageMongoDB::initSourcesColl(const MongoDB::CollList& collList)
{
...
bool LogStorageMongoDB::insertSourceToDB(DebugMessageSource type)
{
    ...
    objBuilder.append(MongoDB::SOURCES_KEY_NAME, messageSourceAsCString(type));
 ...
 * This is mainly for query
 */
bool LogStorageMongoDB::initSeveritiesColl(const MongoDB::CollList& collList)
{
    BW::string severitiesCollName =
      commonDBName_ + "." + MongoDB::SEVERITIES_COLL;
    MongoDB::CollList::const_iterator iter =
      std::find(collList.begin(), collList.end(), severitiesCollName.c_str());
...
bool LogStorageMongoDB::insertSeverityToDB(DebugMessagePriority level)
{
    ...
    objBuilder.append(MongoDB::SEVERITIES_KEY_NAME, messagePrefix(level));
 ...
 */
bool LogStorageMongoDB::initVersionColl(const MongoDB::CollList& collList)
{
    BW::string versionCollName = commonDBName_ + "." + MongoDB::VERSION_COLL;
    MongoDB::CollList::const_iterator iter =
      std::find(collList.begin(), collList.end(), versionCollName.c_str());
    ...
    if (iter == collList.end()) {
        ...
        std::auto_ptr<mongo::DBClientCursor> cursor =
          conn_.query(versionCollName.c_str(), mongo::BSONObj());
        ...
        if (cursor->more()) {
            ...
            uint32 version     = rec.getIntField(MongoDB::VERSION_KEY_VERSION);
...
bool LogStorageMongoDB::insertVersion()
{
    BW::string versionCollName = commonDBName_ + "." + MongoDB::VERSION_COLL;
    mongo::BSONObjBuilder objBuilder;
    objBuilder.append(MongoDB::VERSION_KEY_VERSION, MongoDB::SCHEMA_VERSION);
  ...
  uint64                        timestamp)
{
    ...
    objBuilder.append(MongoDB::ENTRIES_KEY_SOURCE, header.messageSource_);
    objBuilder.append(MongoDB::ENTRIES_KEY_SEVERITY, header.messagePriority_);
    objBuilder.append(MongoDB::ENTRIES_KEY_HOST, address.ip);
    objBuilder.append(MongoDB::ENTRIES_KEY_PID, componentMessage.pid_);
    ...
    objBuilder.append(
      MongoDB::ENTRIES_KEY_COMPONENT,
      componentNames_.getIDOfComponentName(componentMessage.componentName_));

    objBuilder.append(MongoDB::ENTRIES_KEY_APP_ID, this->getAppID(address));

    BW::string     msg;
    mongo::BSONObj metadataObj;
    this->parseMessage(pHandler,
                       inputStream,
                       address,
...
void LogStorageMongoDB::startFlushTimer()
{
    ...
    flushTimer_ = (logger_.getDispatcher())
                    ->addTimer(flushInterval_ * 1000,
                               this,
                               reinterpret_cast<void*>(TIMEOUT_FLUSH_LOG),
                               "FlushLog");
}
