...
namespace MongoDB {
    ...
    bool ConnectionThreadData::onStart(BackgroundTaskThread& thread)
    {
        ...
        DEBUG_MSG(
          "ConnectionThreadData::onStart: Initialising cluster status.\n");
        ...
        DEBUG_MSG(
          "ConnectionThreadData::onStart: Initialising UserLogWriters.\n");
    ...
    bool ConnectionThreadData::initClusterStatus()
    {
        ...
        try {
            bool ok = conn_.runCommand(
              ADMIN_DB_NAME,
              mongo::BSONObjBuilder().append("ismaster", 1).obj(),
              result);
            ...
            if (ok) {
                ...
                ERROR_MSG(
                  "ConnectionThreadData::initClusterStatus: "
    ...
    /*
     * Init user log collections, create a map of uid and user log collection
     * name
     */
        ...
        while (iter != userDBNameMap.end()) {
            ...
            if (!this->getUidFromDB(dbName, uid)) {
                ...
                try {
                    ...
                    ERROR_MSG("ConnectionThreadData::initUserLogWriters: "
                              "Got error ( %s ) when trying to read log from "
                              "db (%s).\n",
                              ex.what(),
                              dbName.c_str());
                ...
                userLogWriters_[uid] =
                  new UserLogWriter(*this, uid, username, loggerID_);
            }
        }
      ...
      const BW::string&  username)
    {
        ...
        // lookup-if-exists/insert-if-doesn't. We can set the iterator later
        // since a new one has been returned.
        std::pair<UserLogWriterMap::iterator, bool> insertResult =
          userLogWriters_.insert(
            std::pair<MessageLogger::UID, UserLogWriterPtr>(uid,
                                                            pUserLogWriter));
    ...
    /**
     * Get the user database name list. User database name starts with
     * bw_ml_user_
     */
      ...
      ConnectionThreadData::UserDbNameMap& userDBNameMap)
    {
        ...
        try {
            conn_.runCommand(
              ADMIN_DB_NAME,
              mongo::BSONObjBuilder().append("listDatabases", 1).obj(),
              info);
        ...
        mongo::BSONObjIterator iter(
          info["databases"].embeddedObjectUserCheck());
        while (iter.more()) {
            ...
            if (dbName.compare(0, prefixLen, prefix) == 0 &&
                (dbNameLen > suffixLen) &&
                dbName.compare(dbNameLen - suffixLen, suffixLen, suffix) == 0) {
                                            ...
                                            MessageLogger::UID& uid)
    {
        ...
        try {
            std::auto_ptr<mongo::DBClientCursor> cursor =
              conn_.query(uidCollName.c_str(), mongo::BSONObj());
    ...
    DBTaskStatus ConnectionThreadData::addRecordToCollection(
      const BW::string&     collectionName,
      const mongo::BSONObj& newRecord)
    {
        try {
            ...
            ERROR_MSG(
              "ConnectionThreadData::addRecordToCollection: "
    ...
    {
        try {
            conn_.update(collectionName.c_str(), existingRecord, newRecord);
     ...
     *
     *  This function should be checked at the beginning of background tasks
     * that are going to need to utilise the connection.
     *
     *  It performs two services:
     *  - It allows the main thread to be notified of a successful reconnect (so
     *  that it can resume logging)
     *  - It prevents randomly spammed error messages from various background
     * tasks attempting to repeatedly use the connection and failing.
     *
     *  It doesn't need to be called before every single connection usage
     * because the connection has an auto reconnect and tasks should handle a
     * failed connection mid-task gracefully.
     */
    ...
    bool ConnectionThreadData::getCollectionNames(
      mongo::DBClientConnection& conn,
      const BW::string&          dbName,
      CollList&                  rCollList)
    {
        try {
            // This is for ticket BWT-28917. This is to work around a crash
            // issue caused by MonogDB driver when target MongoDB server is
            // unreachable. This code simulated how MongoDB driver retrieves
            // collections names but avoid the crash issue. This fix may get
            // obsolete in future if MongoDB changes its own db schema. However
            // this is easy to test out, so we just adopt this solution.
            BW::string ns = dbName + ".system.namespaces";
            std::auto_ptr<mongo::DBClientCursor> cursor =
              conn.query(ns.c_str(), mongo::BSONObj());

            if (!cursor.get()) {
                ERROR_MSG(
                  "ConnectionThreadData::getCollectionNames: "
