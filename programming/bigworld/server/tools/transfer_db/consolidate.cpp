...
void Consolidate::error(BW::string errorStr, BW::string supplementalStr)
{
    ...
    if (hasSupplementalString) {
        ...
        responseStream.addBlob(supplementalStr.c_str(), supplementalStr.size());
        responseStream << ')';
    }
...
bool Consolidate::prepareDBForSend()
{
    ...
    char* sqlErrStr = NULL;
    if (sqlite3_exec(
          db, "SELECT * FROM tbl_checksum", NULL, NULL, &sqlErrStr) !=
        SQLITE_OK) {
    ...
    do {
        ...
        if (amountRead == -1) {
            ...
            while (remainingToSend > 0 && fatalError == false) {
                int lastSend =
                  endpoint_.send(workingBuffer + sentAmount, remainingToSend);
                if (lastSend == -1) {
                    if (errno == EINTR) {
                        continue;
                    }
