...
class ClientCaller : public PyObjectPlus
{
     ...
     *	Destructor.
     */
    ~ClientCaller() { Py_DECREF(&entity_); }
...
PyObject* ClientCaller::pyCall(PyObject* args)
{
    ...
    if (entity_.isReal()) {
        succeeded = entity_.sendToClient(destID_,
                                         methodDescription_,
                                         stream,
                                         isForOwn_,
                                         isForOthers_,
        ...
        succeeded = entity_.sendToClientViaReal(destID_,
                                                methodDescription_,
                                                stream,
                                                isForOwn_,
                                                isForOthers_,
    ...
    if (entity_.id() != destID_) {
        return PyString_FromFormat(
          ...
          ((isForOwn_ && isForOthers_) ? "allClients"
           : !isForOwn_                ? "otherClients"
                                       : "ownClient"),
          (isExposedForReplay_
             ? ((!isForOwn_ && !isForOthers_) ? "shouldRecordOnly=True"
...
ScriptObject PyClient::pyGetAttribute(const ScriptString& attrObj)
{
    ...
    return ScriptObject(new ClientCaller(clientEntity_,
                                         *pDescription,
                                         isForOwn_,
                                         isForOthers_,
                                         recordingOption_,
...
PyObject* PyClient::pyCall(PyObject* args, PyObject* kwargs)
{
    ...
    if (!PyEntityMailBox::parseRecordingOptionFromPythonCall(
    ...
    return new PyClient(
      clientEntity_, destEntity_, isForOwn_, isForOthers_, recordingOption);
}
