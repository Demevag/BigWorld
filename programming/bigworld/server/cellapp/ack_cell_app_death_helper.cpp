 ...
 */
void AckCellAppDeathHelper::handleMessage(
...
void AckCellAppDeathHelper::startTimer()
{
    ...
    for (RecentOnloads::iterator iter = recentOnloads_.begin();
         iter != recentOnloads_.end();
         ++iter) {
        EntityPtr pEntity = *iter;
        avgRTT +=
          std::min(pEntity->pReal()->channel().roundTripTime(), tickTime);
    }
    ...
    timerHandle_ = app_.mainDispatcher().addTimer(
      checkPeriod_, this, NULL, "AckCellAppDeathHelper");
...
void AckCellAppDeathHelper::checkFinished()
{
    ...
    if (recentOnloads_.empty() && numBadGhosts_ == 0 &&
