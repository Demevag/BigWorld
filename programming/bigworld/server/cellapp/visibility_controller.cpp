 ...
 *  @components{ cell }
 *  The addVisibility function creates a controller that handles whether an
 * Entity can be seen by any active vision controllers. These vision controllers
 * make use of the visibleHeight provided by this controller. This should be
 * updated whenever the visible height changes (eg, Entity crouches or jumps).
 *
 *  @see addVision
 *  @see addScanVision
 *  @see cancel
 *
 *  @param visibleHeight visibleHeight is a float which gives the current height
 * of the model.
 ...
 */
Controller::FactoryFnRet VisibilityController::New(float visibleHeight,
                                                   int   userArg)
{
    ...
    if (EntityVision::instance(*s_factoryFnEntity_).getVisibility() != NULL) {
        PyErr_SetString(PyExc_TypeError,
                        "Entity.addVisibilityController: "
