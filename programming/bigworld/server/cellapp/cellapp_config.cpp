...
{
    bool result = EntityAppConfig::postInit() && NoiseConfig::postInit() &&
...
{
    const float maxPackedRelativePositionValue =
      floorf(std::min(PackedXZ::maxLimit(1.f), PackedXYZ::maxLimit(1.f)));
    ...
    maxTimeBetweenPlayerUpdatesInTicks.set(
      secondsToTicks(maxTimeBetweenPlayerUpdates(), -1));
