...
ReplayDataCollector::~ReplayDataCollector()
{
    if (!bufferedEntityData_.empty()) {
        ...
        BufferedEntityData::iterator iter = bufferedEntityData_.begin();
...
void ReplayDataCollector::addEntityState(const Entity& entity)
{
    ...
    BufferedEntityData::iterator iter = bufferedEntityData_.find(entity.id());
    if (iter != bufferedEntityData_.end()) {
        data_.transfer(*(iter->second), iter->second->size());
        bw_safe_delete(iter->second);
    ...
    while (iMovedEntity != movedEntities_.end()) {
        ...
        if (entity.isOnGround()) {
            data_ << ReplayData::BlockType(
                  ...
                  << entity.id() << entity.localPosition().x
                  << entity.localPosition().z << entity.localDirection()
                  << entity.vehicleID();
        } else {
            data_ << ReplayData::BlockType(
                       ReplayData::BLOCK_TYPE_ENTITY_VOLATILE)
                  << entity.id() << entity.localPosition()
                  << entity.localDirection() << entity.vehicleID();
        }
 ...
 */
void ReplayDataCollector::addEntityMethod(
  const Entity&            entity,
 ...
 */
void ReplayDataCollector::addEntityProperty(
  const Entity&          entity,
  ...
  PropertyChange&        change)
{
    ...
    // TODO: Not const cast
    if (!(const_cast<Entity&>(entity))
           .getTopLevelOwner(change, propertyOwner)) {
                                              ...
                                              ScriptDict    properties)
{
    ...
    for (uint i = 0; i < propertyCount; ++i) {
        ...
        if (value) {
            ...
            MemoryOStream data;
            change.addToExternalStream(
              data, pDataDesc->clientServerFullIndex(), propertyCount);
            data_ << int32(data.remainingLength());
            data_.transfer(data, data.remainingLength());
        }
    }
...
namespace // (anonymous)
{
    ...
    class AoIEntityIDCollector : public EntityCacheVisitor
    {
      public:
         ...
         */
        const Collection& entityIDs() const { return entityIDs_; }
...
void ReplayDataCollector::addEntityPlayerStateChange(const Entity& entity)
{
    ...
    data_ << ReplayData::BlockType(
               ReplayData::BLOCK_TYPE_ENTITY_PLAYER_STATE_CHANGE)
          << entity.id();
                                             ...
                                             bool          hasEnteredAoI)
{
    ...
    *pStream << ReplayData::BlockType(ReplayData::BLOCK_TYPE_PLAYER_AOI_CHANGE)
