...
IMPLEMENT_EXCLUSIVE_CONTROLLER_TYPE(NavigationController,
                                    DOMAIN_REAL,
                                    "Movement")
 ...
 *							we don't support an entity on vehicle to navigate
 *yet, so it's global position.
...
NavigationController::NavigationStatus NavigationController::move()
{
    ...
    bool reachedNextPos = (fabs(position.x - nextPosition_.x) < 0.001f) &&
    ...
    if (reachedNextPos) {
        ...
        if (!nens.getNavigatePosition(srcLoc,
                                      *pDstLoc_,
                                      maxDistance_,
                                      nextPosition_,
...
void NavigationController::startReal(bool /*isInitialStart*/)
{
    ...
    // figure out the source navloc
    NavLoc srcLoc(
      this->entity().pChunkSpace(), this->entity().position(), girth_);
...
{
    stream << destination_ << metresPerTick_ << faceMovement_ << maxDistance_
           << girth_;
}
