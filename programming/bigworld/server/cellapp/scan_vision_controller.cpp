 ...
 */
ScanVisionController::ScanVisionController(float visionAngle,
                                           float visionRange,
                                           float seeingHeight,
                                           float amplitude,
                                           float scanPeriod,
...
float ScanVisionController::getYawOffset()
{
    ...
    double timeNow =
      (double)CellApp::instance().time() / CellAppConfig::updateHertz();
    float posInCycle = float(fmod(timeNow / scanPeriod_ + timeOffset_, 1.0));
