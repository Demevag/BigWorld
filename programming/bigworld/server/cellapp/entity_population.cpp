 ...
 */
void EntityPopulation::rememberRealChannel(EntityID id, CellAppChannel& channel)
{
 ...
 */
void EntityPopulation::rememberBaseChannel(Entity&                 entity,
                                           const Mercury::Address& addr) const
{
    if (entity.baseAddr() != Mercury::Address::NONE) {
        RecentTeleportData newData = { entity.baseAddr(),
                                       entity.space().id(),
                                       entity.id() };
        currRecentTeleportData_.insert(
          std::pair<Mercury::Address, RecentTeleportData>(addr, newData));
    }
}
  ...
  AckCellAppDeathHelper*     pAckHelper) const
{
    ...
    for (iter = range.first; iter != range.second; iter++) {
        ...
        if (pCellChannel && pCellChannel->addr() == addr) {
            ...
            BaseAppIntInterface::setClientArgs setClientArgs = { entityID };
  ...
  AckCellAppDeathHelper*     pAckHelper) const
{
    ...
    this->notifyBaseRangeOfCellAppDeath(
      prevRecentTeleportData_, addr, bundles, pAckHelper);
    this->notifyBaseRangeOfCellAppDeath(
      currRecentTeleportData_, addr, bundles, pAckHelper);
}
