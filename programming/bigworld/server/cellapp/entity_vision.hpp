...
class EntityVision : public EntityExtra
{
    ...
    PY_AUTO_METHOD_DECLARE(RETOK, setVisionRange, ARG(float, ARG(float, END)))
    bool setVisionRange(float visionAngle, float range);

    PY_AUTO_METHOD_DECLARE(
      RETOWN,
      addVision,
      ARG(float, ARG(float, ARG(float, OPTARG(int, 10, OPTARG(int, 0, END))))))
    PyObject* addVision(float fov,
                        float range,
                        float seeingHeight,
                        int   period  = 10,
                        int   userArg = 0);
      ...
      ARG(float,
          ARG(float,
              ARG(float,
                  ARG(float,
                      ARG(float,
                          ARG(float, OPTARG(int, 1, OPTARG(int, 0, END)))))))))
    PyObject* addScanVision(float fov,
                            float range,
                            float seeingHeight,
                            float amplitude,
                            float scanPeriod,
