 ...
 */
CellApp* CellApps::add(const Mercury::Address& addr,
                       uint16                  viewerPort,
                       CellAppID               appID)
{
...
CellApp* CellApps::leastLoaded(CellApp* pExclude)
{
    ...
    while (iter != endIter) {
        ...
        if (pCurr->addr().ip != pExclude->addr().ip) {
            if (!pBest || (pCurr->smoothedLoad() < pBest->smoothedLoad())) {
                pBest = pCurr;
            }
        }
...
CellApp* CellApps::findLeastLoadedCellApp() const
{
    ...
    while (iter != map_.end()) {
        if (!pBest ||
            (iter->second->estimatedLoad() < pBest->estimatedLoad())) {
            pBest = iter->second;
        }
 ...
 *
 */
void CellApps::setSharedData(uint8                   dataType,
                             const BW::string&       key,
 ...
 *
 */
void CellApps::delSharedData(uint8                   dataType,
                             const BW::string&       key,
                             const Mercury::Address& originalSrcAddr)
{
    Map::iterator iter = map_.begin();
...
WatcherPtr CellApps::pWatcher()
{
    ...
    pWatcher->addChild("*", new BaseDereferenceWatcher(CellApp::pWatcher()));
