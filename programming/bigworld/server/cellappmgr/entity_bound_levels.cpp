...
{
    return entityBounds_[Math::clamp(0, level, (int)entityBounds_.size() - 1)];
}
...
{
    return entityLoads_[Math::clamp(0, level, (int)entityLoads_.size() - 1)];
}
 ...
 */
void EntityBoundLevels::takeSingleBranch(const EntityBoundLevels& single,
                                         bool                     isY,
                                         bool                     isMax,
                                         float partitionPosition)
{
    ...
    for (size_t level = 0; level < entityBounds_.size(); ++level) {
        float boundToTake = single.entityBounds(level).range1D(isY)[isMax];
 ...
 */
void EntityBoundLevels::mergeTwoBranches(const EntityBoundLevels& left,
                                         const EntityBoundLevels& right,
                                         bool                     isY,
                                         bool                     isMax)
{
                                                ...
                                                float defaultPosition) const
{
    ...
    for (int level = 0; level < (int)entityLoads_.size(); ++level) {
        if (entityLoads_[level].range1D(isHorizontal)[isMax] <= absLoadDiff) {
