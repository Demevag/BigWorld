 ...
 *	Constructor.
 */
CellApp::CellApp(const Mercury::Address& addr, uint16 viewerPort, CellAppID id)
                ...
                if (pAlternate->cellApp().addr().ip == this->addr().ip) {
                    ...
                    if (pBest) {
                        INFO_MSG("CellApp::handleUnexpectedDeath: "
                                 ...
                                 space.id(),
                                 pBest->addr().c_str());
                        pAlternate = space.addCell(*pBest);
                    }
                }
            }
...
WatcherPtr CellApp::pWatcher()
{
    ...
    if (pWatcher == NULL) {
        ...
        pWatcher->addChild("numEntities", makeWatcher(&CellApp::numEntities_));
        ...
        pWatcher->addChild(
          "isRetiring",
          makeNonRefWatcher(MF_ACCESSORS(bool, CellApp, isRetiring)));
