 ...
 *	Constructor.
 */
CellData::CellData(CellApp& cellApp, Space& space)
  : CM::BSPNode(BW::Rect(-std::numeric_limits<float>::max(),
...
WatcherPtr CellData::pWatcher()
{
    ...
    if (pWatcher == NULL) {
        ...
        pWatcher->addChild("chunkBounds", makeWatcher(&CellData::chunkBounds_));

        pWatcher->addChild("isRetiring",
                           makeWatcher(MF_ACCESSORS_EX(
                             bool, CellData, isRetiring, setIsRetiring)));
        ...
        pWatcher->addChild(
          "isLeaf",
          makeWatcher(MF_ACCESSORS_EX(bool, CellData, isLeaf, setIsLeaf)));
    }
...
CM::BSPNode* CellData::addCell(CellData* pCell, bool isHorizontal)
{
    ...
    return new CM::InternalNode(this, pCell, isHorizontal, range_, partitionPt);
}
 ...
 */
inline float CellData::calculateAreaNotLoaded() const
{
...
{
    if ((range.xRange().length() <= 0.f) || (range.yRange().length() <= 0.f)) {
        return 0.f;
    }
    ...
    desiredRect.safeInflateBy(CellAppMgrConfig::ghostDistance() -
    ...
    if (shouldDebugIfNonZero && (areaNotLoaded > 0.f)) {
        ...
        WARNING_MSG("CellData::calculateAreaNotLoaded: Cell %d in Space %d "
                    "reverted to not having chunks loaded.\n"
                    "\tareaNotLoaded = %f\n"
                    ...
                    spaceBounds.xMax(),
                    spaceBounds.yMax(),
                    clippedDesiredRect.xMin(),
                    clippedDesiredRect.yMin(),
                    clippedDesiredRect.xMax(),
                    clippedDesiredRect.yMax(),
                    chunkBounds_.xMin(),
                    chunkBounds_.yMin(),
                    chunkBounds_.xMax(),
                    chunkBounds_.yMax(),
                    clippedChunkRect.xMin(),
                    clippedChunkRect.yMin(),
                    clippedChunkRect.xMax(),
                    clippedChunkRect.yMax(),
                    outerArea,
                    innerArea);
 ...
 *	new internal node.
 */
CM::BSPNode* CellData::addCellTo(CellData* pNewCell,
                       ...
                       bool            isShrinking)
{
    ...
    // to make it smaller.
    if (!hasArea && (isShrinking || numRetiring_) &&
        (this->numEntities() == 0)) {
        Space::addCellToDelete(this);
    }
}
...
{
    float smoothedLoad = pCellApp_ ? pCellApp_->smoothedLoad() : 0.f;
...
const BW::Rect& CellData::balanceChunkBounds() const
{
    ...
    return chunkBounds_.contains(desiredRect) ? chunkBounds_ : range_;
}
...
void CellData::debugPrint(int indent)
{
    ...
    DEBUG_MSG(
      "%*sCell: %d. (%f,%f)->(%f,%f). (%f,%f)->(%f,%f). (%f,%f)->(%f,%f)\n",
      2 * indent + 1,
      "+",
      this->cellApp().id(),
      chunkBounds_.xMin(),
