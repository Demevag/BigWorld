...
class PyBillingResponse : public PyObjectPlus
{
  ...
  private:
    bool loadEntity(const BW::string& entityType,
                    DatabaseID        dbID,
    ...
    bool loadEntityByName(const BW::string& entityType,
                          ...
                          const BW::string& dataForClient,
                          const BW::string& dataForBaseEntity);
    bool createNewEntity(const BW::string& entityType,
                         bool              shouldRemember,
                         const BW::string& dataForClient,
                         const BW::string& dataForBaseEntity);
                                   ...
                                   const BW::string& dataForBaseEntity)
{
    ...
    pHandler_->onGetEntityKeyForAccountSuccess(
      entityKey, dataForClient, dataForBaseEntity);
    pHandler_ = NULL;
                                        ...
                                        const BW::string& dataForBaseEntity)
{
    ...
    pHandler_->onGetEntityKeyForAccountCreateNew(
      entityTypeID, shouldRemember, dataForClient, dataForBaseEntity);
    pHandler_ = NULL;
                                        ...
                                        EntityTypeID&     resultID) const
{
    ...
    if (resultID == INVALID_ENTITY_TYPE_ID) {
        PyErr_Format(
          PyExc_ValueError, "Invalid entity type '%s'\n", entityType.c_str());
        return false;
    }
...
{
    return this->sendFailure(LogOnStatus::LOGIN_REJECTED_INVALID_PASSWORD,
                             errorMsg);
}

bool PyBillingResponse::failureNoSuchUser(const BW::string& errorMsg)
{
    return this->sendFailure(LogOnStatus::LOGIN_REJECTED_NO_SUCH_USER,
                             errorMsg);
}

bool PyBillingResponse::failureDBError(const BW::string& errorMsg)
{
    return this->sendFailure(LogOnStatus::LOGIN_REJECTED_DB_GENERAL_FAILURE,
                             errorMsg);
}
...
bool PyBillingResponse::failure(int statusAsInt, const BW::string& errorMsg)
{
    ...
    if (status.okay() || (statusAsInt != status.value())) {
        PyErr_Format(
          PyExc_ValueError, "Invalid failure status %d", statusAsInt);
        return false;
    }
...
{
    PyObjectPtr pMember(
      PyObject_GetAttrString(pPyObject, const_cast<char*>(member)),
 ...
 */
void PyBillingSystem::setEntityKeyForAccount(
  const BW::string&               username,
  const BW::string&               password,
  ...
  ISetEntityKeyForAccountHandler& handler)
{
    ...
    PyObjectPtr pArgs(PyTuple_New(4), PyObjectPtr::STEAL_REFERENCE);

    PyTuple_SET_ITEM(pArgs.get(), 0, Script::getData(username));
    PyTuple_SET_ITEM(pArgs.get(), 1, Script::getData(password));
    PyTuple_SET_ITEM(pArgs.get(), 2, Script::getData(entityDesc.name()));
    PyTuple_SET_ITEM(pArgs.get(), 3, Script::getData(ekey.dbID));
