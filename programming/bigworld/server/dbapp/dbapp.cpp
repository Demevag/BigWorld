...
namespace // (anonymous)
{
    ...
    int s_moduleTokens BW_UNUSED_ATTRIBUTE =
      ResMgr_token | PyScript_token | force_link_UDO_REF | BgTaskManager_token |
      BlobOrNull_token | PyURLRequest_token;
...
bool DBApp::init(int argc, char* argv[])
{
    ...
    return this->initNetwork() && this->initBaseAppMgr() &&
           this->initScript(argc, argv) && this->initEntityDefs() &&
           this->initExtensions() && this->initDatabaseCreation() &&
...
bool DBApp::initBaseAppMgr()
{
    ...
    Mercury::MachineDaemon::registerBirthListener(
      interface_.address(),
      DBAppInterface::handleBaseAppMgrBirth,
      "BaseAppMgrInterface");
...
bool DBApp::initScript(int argc, char** argv)
{
    ...
    ScriptModule module = ScriptModule::import(
      "BigWorld", ScriptErrorPrint("Failed to import BigWorld module"));
...
bool DBApp::initDatabaseCreation()
{
    ...
    DatabaseEngineData dbEngineData(
      this->interface(), this->mainDispatcher(), DBAppConfig::isProduction());

    pDatabase_ =
      DatabaseEngineCreator::createInstance(databaseType, dbEngineData);
...
bool DBApp::initDBAppMgrAsync()
{
    ...
    if (!dbAppMgr_.init("DBAppMgrInterface",
...
bool DBApp::onDBAppMgrRegistrationCompleted(BinaryIStream& data)
{
    ...
    if (this->isAlpha()) {
        CONFIG_INFO_MSG("Alpha = true (%s reset of game server state)\n",
                        shouldAlphaResetGameServerState ? "Performing"
                                                        : "Skipping");
    ...
    // If changing the order or adding/removing initialisation steps, be sure
    // to update the order in the header too, as it is a handy reference.
    if (!(this->initAppIDRegistration() && this->initDatabaseStartup() &&
          this->initBirthDeathListeners() && this->initWatchers() &&
          this->initConfig() && this->initReviver() &&
    ...
    if (this->isAlpha()) {
        ...
        if (!(this->initTimers() && this->initScriptAppReady())) {
            return false;
        }
...
bool DBApp::initDatabaseStartup()
{
    ...
    if (!pDatabase_->startup(
          *pEntityDefs_, mainDispatcher_, DBAppConfig::numDBLockRetries())) {
        return false;
    }
...
bool DBApp::initBirthDeathListeners()
{
    ...
    Mercury::Reason reason = Mercury::MachineDaemon::registerBirthListener(
      interface_.address(),
      DBAppInterface::handleDBAppMgrBirth,
      "DBAppMgrInterface");
    ...
    reason = Mercury::MachineDaemon::registerDeathListener(
      interface_.address(),
      DBAppInterface::handleDBAppMgrDeath,
      "DBAppMgrInterface");
...
bool DBApp::initWatchers()
{
    ...
    BW_REGISTER_WATCHER(
      id_, abrv, "dbApp", mainDispatcher_, this->interface().address());
...
bool DBApp::initConfig()
{
    ...
    shouldCacheLogOnRecords_ =
      (Config::shouldCacheLogOnRecords() && shouldAlphaResetGameServerState_);
...
bool DBApp::initBillingSystem()
{
    ...
    pBillingSystem_ =
      BillingSystemCreator::createFromConfig(*pEntityDefs_, *this);
...
bool DBApp::onAppsReady()
{
    ...
    if (shouldAlphaResetGameServerState_) {
        ...
        return this->initSendSpaceData() && this->initEntityAutoLoadingAsync();
    }
...
bool DBApp::initScriptAppReady()
{
    ...
    if (!this->scriptEvents().triggerTwoEvents(
          "onAppReady", "onDBAppReady", PyTuple_New(0))) {
        return false;
    }
...
bool DBApp::initTimers()
{
    ...
    // be ready.
    statusCheckTimer_ =
      mainDispatcher_.addTimer(1000000,
                               this,
...
void DBApp::consolidateData()
{
    if (status_.status() <= DBStatus::STARTING) {
        ...
        SECONDARYDB_CRITICAL_MSG(
          "DBApp::consolidateData: "
...
bool DBApp::startConsolidationProcess()
{
    if (this->isConsolidating()) {
        SECONDARYDB_TRACE_MSG(
          "DBApp::startConsolidationProcess: "
...
void DBApp::handleStatusCheck(BinaryIStream& data)
{
    ...
    INFO_MSG("DBApp::handleStatusCheck: "
             ...
             numBaseApps,
             Config::desiredBaseApps(),
             numCellApps,
             Config::desiredCellApps(),
             numServiceApps,
             Config::desiredServiceApps());

    // Ignore other status information
    data.finish();

    if ((status_.status() <= DBStatus::WAITING_FOR_APPS) && !data.error() &&
...
{
  public:
    CheckStatusReplyHandler(DBApp& dbApp)
      : dbApp_(dbApp)
    {
    }
 ...
 */
void DBApp::secondaryDBRegistration(
  ...
  const IDatabase::SecondaryDBEntries& removedEntries)
{
    for (IDatabase::SecondaryDBEntries::const_iterator pEntry =
           removedEntries.begin();
         pEntry != removedEntries.end();
         ++pEntry) {
        if (this->sendRemoveDBCmd(pEntry->addr.ip, pEntry->location)) {
            SECONDARYDB_TRACE_MSG("DBApp::onUpdateSecondaryDBsComplete: "
                                  ...
                                  pEntry->location.c_str(),
                                  pEntry->addr.ipAsString());
        } else {
            SECONDARYDB_ERROR_MSG(
              "DBApp::onUpdateSecondaryDBsComplete: "
    ...
    while (iter != pendingAttempts_.end()) {
        ...
        if (age > maxPendingLoginAge) {
            ERROR_MSG("DBApp::checkPendingLoginAttempts: Removing login "
                      ...
                      entityKey.dbID,
                      age);
            this->onCompleteRelogonAttempt(entityKey.typeID, entityKey.dbID);
        }
    }
}
...
{
    INFO_MSG(
      "DBApp::handleDBAppMgrBirth( id = %d ): %s\n", id_, args.addr.c_str());
  ...
  const DBAppInterface::handleDBAppMgrDeathArgs& args)
{
    ...
    NOTICE_MSG(
      "DBApp::handleDBAppMgrDeath( id = %d ): %s\n", id_, args.addr.c_str());
}
                      ...
                      IDatabase::IPutEntityHandler& handler)
{
    ...
    pDatabase_->putEntity(entityKey,
                          entityID,
                          pStream,
                          pBaseMailbox,
                          removeBaseMailbox,
 ...
 *  we can muck around with stuff before passing it to IDatabase.
 */
void DBApp::delEntity(const EntityDBKey&            ekey,
                      EntityID                      entityID,
                  ...
                  BinaryIStream&                        data)
{
    ...
    if (pParams->digest() != this->getEntityDefs().getDigest()) {
        ...
        this->sendFailure(header.replyID,
                          srcAddr,
 ...
 *  already logged on.
 */
void DBApp::onLogOnLoggedOnUser(EntityTypeID            typeID,
                                DatabaseID              dbID,
                                LogOnParamsPtr          pParams,
                                ...
                                const BW::string&       dataForBaseEntity)
{
    ...
    Mercury::ChannelSender sender(DBApp::getChannel(pExistingBase->addr));

    Mercury::Bundle& bundle = sender.bundle();
    bundle.startRequest(BaseAppIntInterface::logOnAttempt,
                        new RelogonAttemptHandler(pExistingBase->type(),
                                                  dbID,
                                                  replyAddr,
                                                  replyID,
                                                  pParams,
                                ...
                                BinaryOStream&    strm) const
{
    ...
    if (isCreated) {
        ...
        desc.addSectionToStream(pSection,
                                strm,
 ...
 */
DatabaseID* DBApp::prepareCreateEntityBundle(
  EntityTypeID                  typeID,
  DatabaseID                    dbID,
  const Mercury::Address&       addrForProxy,
  Mercury::ReplyMessageHandler* pHandler,
  ...
  LogOnParamsPtr                pParams,
  BW::string*                   pDataForBaseEntity)
{
    bundle.startRequest(BaseAppMgrInterface::createEntity,
                        pHandler,
                        0,
                        Mercury::DEFAULT_REQUEST_TIMEOUT +
                          1000000); // 1 second extra
                        ...
                        BinaryIStream&                        data)
{
    ...
    if (!isOkay) {
        ...
        WriteEntityHandler* pHandler = new WriteEntityHandler(
          ekey, entityID, flags, header.replyID, srcAddr);
...
void DBApp::authenticateAccount(const Mercury::Address&               srcAddr,
                                ...
                                BinaryIStream&                        data)
{
    AuthenticateAccountHandler::handle(
      srcAddr, header, data, *pBillingSystem_, this->interface());
}
                       ...
                       BinaryIStream&                        input)
{
    ...
    if (byName) {
        if (this->getEntityDefs()
              .getEntityDescription(ekey.typeID)
              .pIdentifier() == NULL) {
            ERROR_MSG(
              "DBApp::loadEntity: Attempted to load entity type "
...
{
    DeleteEntityHandler* pHandler = new DeleteEntityHandler(
      args.entityTypeID, args.dbid, srcAddr, header.replyID);
    pHandler->deleteEntity();
}
...
{
    LookUpEntityHandler* pHandler =
      new LookUpEntityHandler(srcAddr, header.replyID);
...
{
  public:
    ExecuteRawCommandHandler(DBApp&                 dbApp,
                             const Mercury::Address srcAddr,
                             Mercury::ReplyID       replyID)
      : dbApp_(dbApp)
      , replyBundle_()
...
class GetIDsHandler : public IDatabase::IGetIDsHandler
{
  public:
    ...
    void getIDs(int numIDs) { dbApp_.getIDatabase().getIDs(numIDs, *this); }
                   ...
                   BinaryIStream&                        input)
{
    ...
    input >> numIDs;
    INFO_MSG(
      "DBApp::getIDs: fetching %d id's for %s\n", numIDs, srcAddr.c_str());

    GetIDsHandler* pHandler = new GetIDsHandler(*this, srcAddr, header.replyID);
...
void DBApp::handleBaseAppDeath(BinaryIStream& data)
{
    ...
    if (shouldCacheLogOnRecords_) {
        logOnRecordsCache_.remapMailboxes(remappingSrcAddr, remappingDstAddrs);
    }
...
void DBApp::remapMailbox(EntityMailBoxRef& mailbox) const
{
    ...
    if (found != mailboxRemapInfo_.end()) {
        const Mercury::Address& newAddr = found->second.addressFor(mailbox.id);
                                     ...
                                     const EntityMailBoxRef* pBaseRef)
{
    ...
    if (isErased && (checkoutCompletionListeners_.size() > 0)) {
        std::pair<CheckoutCompletionListeners::iterator,
                  CheckoutCompletionListeners::iterator>
          listeners = checkoutCompletionListeners_.equal_range(entityID);
        for (CheckoutCompletionListeners::const_iterator iter = listeners.first;
             iter != listeners.second;
             ++iter) {
            iter->second->onCheckoutCompleted(pBaseRef);
        }
        checkoutCompletionListeners_.erase(listeners.first, listeners.second);
    }
 ...
 */
bool DBApp::registerCheckoutCompletionListener(
  EntityTypeID                        typeID,
  DatabaseID                          dbID,
  DBApp::ICheckoutCompletionListener& listener)
{
