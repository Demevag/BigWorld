...
bool EntityType::init(const BW::string& clientPath)
{
    ...
    if (PyObject_SetAttrString(
          pBWModule, "Entity", (PyObject*)&BW::PyEntity::s_type_) == -1) {
        return false;
    }

    // Initialise the entity descriptions.
    BW::DataSectionPtr pEntitiesList = BW::BWResource::openSection(clientPath);
    ...
    for (int i = 0; i < numEntityTypes; i++) {
        ...
        PyObject* pClass =
          PyObject_GetAttrString(pModule, const_cast<char*>(name));
        if (pClass == NULL) {
            ERROR_MSG("EntityType::init: Could not find class %s\n", name);
            succeeded = false;
            continue;
        } else if (PyObject_IsSubclass(
                     pClass, (PyObject*)&BW::PyEntity::s_type_) != 1) {
            ...
            continue;
                     ...
                     BW::ScriptObject(
                       pClass, BW::ScriptObject::FROM_BORROWED_REFERENCE))) {
            ERROR_MSG("EntityType::init: "
                      "Class %s does not have correct methods\n",
                      name);
            // don't fail here because it's not *completely* critical that
            // client does not have all the methods (but it might be for, say, a
            // real client) succeeded = false; continue;
        }
...
{
    static BW::EntityDefConstants s_constants(
