...
class MyConnectionHandler : public BW::ServerConnectionHandler
{
  public:
    ...
    virtual void onLoggedOn() { errorMessage_.clear(); }
...
class SimplePythonClient : public Singleton<SimplePythonClient>
{
  public:
    SimplePythonClient(const BW::string& serverName,
      ...
      : pSpaceDataStorage_(new SimpleSpaceDataStorage(new SpaceDataMappings()))
    ...
    void logout() { pConnection_->logOff(); }
    ...
    bool init(int argc, char** argv)
    {
        ...
        pConnectionHelper_ =
          new BWConnectionHelper(entityFactory_,
                                 *pSpaceDataStorage_,
...
// 4. Setup the public key to encrypt client / server communication
        ...
        ScriptModule bigWorld = ScriptModule::import(
    ...
    BWServerConnection* pConnection() { return pConnection_; }
  ...
  private:
    void tick()
    {
        ...
        if (!wasOnline_ && pConnection_->isOnline()) {
            ...
            std::cout << "Connected to "
                      << pConnection_->serverAddress().c_str() << std::endl;
    ...
    static void onSigInt(int signum) { SimplePythonClient::instance().exit(); }
                              ...
                              const ScriptDict& dict)
{
    ...
    if (pType == NULL) {
        PyErr_Format(PyExc_ValueError,
                     ...
                     "Class %s is not an entity class",
                     className.c_str());
        return NULL;
    }
    ...
    *pNumProps              = 0;
    ...
    for (uint8 i = 0; i < nprops; i++) {
        const DataDescription* pDD =
          pType->description().clientServerProperty(i);
    ...
    BWEntity*           pEntity =
      pConn->createLocalEntity(pType->description().clientIndex(),
                               spaceID,
                               vehicleID,
                               position,
                               direction,
...
PY_AUTO_MODULE_FUNCTION(
  ...
  ARG(BW::string,
      ARG(SpaceID,
          ARG(EntityID,
              ARG(Position3D,
                  ARG(Vector3, OPTARG(ScriptDict, ScriptDict(), END)))))),
  BigWorld);
...
PY_AUTO_MODULE_FUNCTION(RETDATA, destroyEntity, ARG(EntityID, END), BigWorld);
BW_END_NAMESPACE

void printUsage(const char* argv0)
{
    static const char USAGE_STRING[] =
      "Usage: %s -server <server> "
...
int main(int argc, char* argv[])
{
    ...
    SimplePythonClient client(serverName, userName, password, autoDisconnect);
