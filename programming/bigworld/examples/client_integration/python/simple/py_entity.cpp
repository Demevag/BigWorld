  ...
  , pPyBase_(NULL)
{
    ...
    if (!pNewDict || !pCurrDict || PyDict_Update(pCurrDict, pNewDict) < 0) {
        PY_ERROR_CHECK();
    }
    ...
    {                                                                          \
        if (pEntity_ == NULL) {                                                \
            PyErr_Format(PyExc_TypeError,                                      \
                         "Sorry, the attribute " #NAME " in "                  \
                         "%s is no longer available as its entity no longer "  \
                         "exists on this "                                     \
                         "client.",                                            \
                         this->typeName());                                    \
            return NULL;                                                       \
...
PY_RO_ATTRIBUTE_GET_IF_NOT_DESTROYED(this->pEntity_->direction().pitch, pitch)
...
{
    PyObject* pNewDict = this->pEntity()->type().newDictionary(
      stream, EntityDescription::FROM_BASE_TO_CLIENT_DATA);
    PyObject* pCurrDict = PyObject_GetAttrString(this, "__dict__");
...
{
    PyObject* pCellData = this->pEntity()->type().newDictionary(
      stream, EntityDescription::FROM_CELL_TO_CLIENT_DATA);
