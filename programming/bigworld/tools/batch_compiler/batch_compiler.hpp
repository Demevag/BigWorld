  ...
  , public PluginLoader
{
  ...
  private:
    DataSectionPtr writeSection(DataSectionPtr   pDataSection,
                                const StringRef& id,
                                const StringRef& classType);
    DataSectionPtr writeBreak(DataSectionPtr pDataSection);
    DataSectionPtr writeHeading(DataSectionPtr   pDataSection,
                                const StringRef& heading,
                                uint8            size = 1);
    DataSectionPtr writeText(DataSectionPtr   pDataSection,
                             const StringRef& text,
                             const StringRef& style);
    DataSectionPtr writeLink(DataSectionPtr   pDataSection,
