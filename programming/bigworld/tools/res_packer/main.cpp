...
static int s_chunkTokenSet = 0 | XmlPacker_token | ImagePacker_token |
                             FxPacker_token | ChunkPacker_token |
                             CDataPacker_token | ModelAnimPacker_token;
...
void printUsage(char* exeFullName)
{
    ...
    printf("Compatible usage: %s [-r|--res search_paths] input_file "
    ...
    printf(
      ...
      "\n"
      "In the old compatible mode, If an output file is specified, the input "
      "data\n"
      "is packed depending on the file type. If an output file is not "
      "specified,\n"
      ...
      "destination folder, so if packing the a file 'dragon.bmp' to the "
      "folder\n"
      "'/bw/finalgame/res/maps/monsters/dragon.bmp', and the root of the game\n"
      "will be '/bw/finalgame/res', then base_path must be "
      "'/bw/finalgame/res'\n"
...
void doBgTaskManagerInit()
{
    ...
    if (bSuccessfulInit && BgTaskManager::instance().numRunningThreads() == 0) {
        BgTaskManager::instance().startThreads("Background Thread: res_packer",
                                               1);
    }
#endif // MF_SERVER
}
...
void doFileIOTaskManagerInit()
{
    ...
    if (bSuccessfulInit &&
        ...
        FileIOTaskManager::instance().startThreads("File IO Thread: res_packer",
                                                   1);
    }
#endif // MF_SERVER
}
...
{
  public:
    ScopedWriteToConsole() { DebugFilter::shouldWriteToConsole(true); }

    ~ScopedWriteToConsole() { DebugFilter::shouldWriteToConsole(false); }
};
...
int main(int argc, char* argv[])
{
    ...
    // we don't want the memory leak reporting to pollute the console output on
    // exit.
    ...
    *outPath = '\0';
    ...
    while (processedFlag) {
        ...
        if ((numArgs > 1) &&
            (strcmp(pArgs[0], "--res") == 0 || strcmp(pArgs[0], "-r") == 0)) {
            // skip command-line-specified paths
            pArgs += 2;
            numArgs -= 2;
            processedFlag = true;
        } else if ((numArgs > 1) && (strcmp(pArgs[0], "--list") == 0 ||
                                     strcmp(pArgs[0], "-l") == 0)) {
            hasAssetList = true;
            ...
            numArgs -= 2;
            processedFlag = true;
        } else if ((numArgs > 1) && (strcmp(pArgs[0], "--in") == 0 ||
                                     strcmp(pArgs[0], "-i") == 0)) {
            hasInPath = true;
            ...
            numArgs -= 2;
            processedFlag = true;
        } else if ((numArgs > 1) && (strcmp(pArgs[0], "--out") == 0 ||
                                     strcmp(pArgs[0], "-o") == 0)) {
            hasOutPath = true;
            ...
            numArgs -= 2;
            processedFlag = true;
        } else if ((numArgs > 1) && (strcmp(pArgs[0], "--err") == 0 ||
                                     strcmp(pArgs[0], "-e") == 0)) {
            useErrorLog = true;
    ...
    if ((hasAssetList &&
         (!hasInPath || !hasOutPath || numArgs > 0)) ||  // list mode
    ...
    if (hasAssetList) {
        ...
        while (fgets(buf, MAX_FILEPATH, assetList) != 0) {
            ...
            BW::string pOutputName =
              BW::string(outPath) + "\\" + BW::string(buf);
            ...
            BasePacker* packer =
              Packers::instance().find(pInputName, pOutputName);
            if (numArgs == 1) {
                if (!packer)
                    return EXIT_SUCCESS; // file type not known, nothing to
                                         // print
