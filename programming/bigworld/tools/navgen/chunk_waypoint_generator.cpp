...
namespace {
    ...
    void getEntityPts(Chunk* pChunk, BW::vector<Vector3>& entityPts)
    {
        ...
        for (WPEntities::iterator it = wec.begin(); it != wec.end(); ++it) {
            seedPt = pChunk->transform().applyPoint((*it)->position() +
                                                    Vector3(0, 1.f, 0));
            ++seedPointCount;
  ...
  , modified_(true)
{
    ...
    DataSectionPtr chunkBinSection =
      BWResource::openSection(pChunk_->binFileName());
    if (chunkBinSection) {
...
void ChunkWaypointGenerator::outputDirtyFlag(bool dirty /*= false*/)
{
    ...
    if (chunkBinSection) {
        ...
        if (navmeshDirtySection) {
            navmeshDirtySection->setBinary(new BinaryBlock(
              &dirty, sizeof(dirty), "BinaryBlock/ChunkWaypointGenerator"));
            navmeshDirtySection->setParent(chunkBinSection);
            navmeshDirtySection->save();
            navmeshDirtySection->setParent(NULL);
        } else {
            ERROR_MSG("Couldn't create NavMesh dirty section for chunk %s",
                      pChunk_->identifier());
        }
    }
...
/*static*/ bool ChunkWaypointGenerator::canProcess(Chunk* chunk)
{
    ...
    if (!chunk->isOutsideChunk()) {
        ...
        Chunk* outside = ChunkManager::instance().findChunkByName(
          outsideChunkName, chunk->mapping());
    ...
    for (int x = -1; x < 2; ++x) {
        for (int z = -1; z < 2; ++z) {
            Vector3 pos(
              centre.x + x * gridSize, centre.y, centre.z + z * gridSize);
            BW::string chunkName =
              chunk->mapping()->outsideChunkIdentifier(pos);
                ...
                for (ChunkOverlappers::Overlappers::iterator it = coo.begin();
                    ...
                    if (!(*it)->pOverlapper()->isBound()) {
                        if (!(*it)->pOverlapper()->loading()) {
                            ChunkManager::instance().loadChunkExplicitly(
                              (*it)->pOverlapper()->identifier(),
                              (*it)->pOverlapper()->mapping());
                            ChunkManager::instance().checkLoadingChunks();
                        }
                        return false;
                    }
