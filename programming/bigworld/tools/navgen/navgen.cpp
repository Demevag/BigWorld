...
namespace {
    LRESULT CALLBACK reannotatingWndProc(HWND   hwnd,
                                         UINT   uMsg,
                                         WPARAM wParam,
                                         LPARAM lParam)
    {
...
class NavgenStatusWindow
{
  ...
  public:
    bool create(HWND parent)
    {
        ...
        int partWidth[] = { 150, // action: clearing/...
                            320, // processed
                            420, // skip
                            540, // elapsed time
                            640, // average time
        ...
        SendMessage(
          hwnd_, SB_SETPARTS, ARRAY_SIZE(partWidth), (LPARAM)partWidth);
        return !!hwnd_;
    }
    ...
    BW::wstring status() const
    {
        ...
        wchar_t buf[1024];
        DWORD   average =
          (GetTickCount() - start_) / (processed_ ? processed_ : 1);
        if (allProcessed_ == processed_) {
            bw_snwprintf(buf,
                         ARRAY_SIZE(buf),
                         L"%s finished with %d chunks processed, skipped %d "
                         L"chunks, takes %s, average %s.\n",
                         action_.c_str(),
                         allProcessed_,
                         skip_,
                         tickToTime(GetTickCount() - start_).c_str(),
                         tickToTime(average).c_str());
        } else {
            bw_snwprintf(
              buf,
              ARRAY_SIZE(buf),
              L"%s finished with %d chunks processed, %d new navmeshes were "
              L"created, %d navmeshes were already up-to-date,\n"
              L"skipped %d chunks, takes %s, average %s.\n",
              action_.c_str(),
              allProcessed_,
              processed_,
              allProcessed_ - processed_,
              skip_,
              tickToTime(GetTickCount() - start_).c_str(),
              tickToTime(average).c_str());
        }
        ...
        if (action_.empty()) {
            ...
            if (chunk_.empty())
                ...
                SendMessage(
                  ...
                  SB_SETTEXT,
                  0,
                  (LPARAM)(action_ + L": " + chunk_.substr(chunk_.size() - 9))
                    .c_str());
            wchar_t buf[256];
            if (allProcessed_ == processed_)
                bw_snwprintf(
                  buf, ARRAY_SIZE(buf), L"processed: %d", processed_);
            else
                bw_snwprintf(buf,
                             ARRAY_SIZE(buf),
                             L"processed: %d/%d",
    ...
    BW::wstring tickToTime(DWORD tick) const
    {
        ...
        if (tick > 3600) {
            ...
            bw_snwprintf(
              time, ARRAY_SIZE(time), L"%d:%02d", tick / 60, tick % 60);
        } else {
...
BW::map<float, Girth>
  g_girthSpecs; // value : width, height, depth from navgen_settings.xml.
                // this is a vector 'cause I'm a little worried about
                // having a float as lookup value.
BW::vector<float>
  g_girthsAlwaysGenerate; // always generate girths with values in this list.
...
const static std::pair<UINT, UINT> s_scrnSaverList[] = {
    ...
    std::make_pair<UINT, UINT>(SPI_GETSCREENSAVETIMEOUT,
                               SPI_SETSCREENSAVETIMEOUT)
};
const static size_t s_scrnSaverListSz =
  sizeof(s_scrnSaverList) / sizeof(s_scrnSaverList[0]);
...
class NavGenGenerating
{
  public:
    NavGenGenerating()
    {
        ...
        for (size_t i = 0; i < s_scrnSaverListSz; ++i) {
            SystemParametersInfo(
              s_scrnSaverList[i].first, 0, &scrnParams_[i], 0);
    ...
    ~NavGenGenerating()
    {
        ...
        for (size_t i = 0; i < s_scrnSaverListSz; ++i) {
            SystemParametersInfo(
              s_scrnSaverList[i].second, scrnParams_[i], NULL, 0);
        }
    }
...
namespace {
    void refreshWindows(bool ignoreInput = true,
  ...
  , public DebugMessageCallback
{
  public:
      ...
      : error_(false)
    {
        ...
        printMsg(
          (BW::wstring(L"Logging to : ") + getLogFileName() + L"\n").c_str());
    }
    ~ErrorLogHandler()
    {
        BW_GUARD;
        ...
        for (BW::set<wchar_t*>::iterator message = unhandledMessages_.begin();
             message != unhandledMessages_.end();
             ++message) {
            bw_free(*message);
        }
                               ...
                               va_list     argPtr)
    {
        ...
        if (messagePriority > MESSAGE_PRIORITY_INFO) {
            ...
            PostMessage(g_hWindow, WM_SETSTATUSTEXT, 0, (LPARAM)sendMessage);
        }
        return false;
    }
...
void setCalculating(size_t            done,
                    size_t            todo,
                    const BW::string& operation = "Calculating")
{
    ...
    if (todo == 0)
        ...
        message = BW::string(
          sformat("{0} {1} of {2} chunks", operation, done, todo).c_str());
        ...
        for (z = 0; z < g_pView->gridZ(); ++z) {
            ...
            for (a = 0; a < 8; ++a) {
                if (mask & (1 << a)) {
                    ...
                    glVertex2f(GLfloat(x + g_dx[a] * 0.3f),
                               GLfloat(z + g_dz[a] * 0.3f));
                }
            }
        }
    }
...
void drawBSPNodes()
{
    ...
    while (cursor < cursorMax) {
        ...
        if (!bni.internal_ && !bni.waypoint_)
            continue;
    ...
    if (HMENU menu = GetSubMenu(GetMenu(g_hWindow), 1)) // view)
    {
        if (g_pView) {
            ...
            for (int i = 0; i < g_pView->getSets(); ++i) {
                ...
                AppendMenu(
                  menu, MF_CHECKED | MF_STRING, ID_VIEW_SET_NONE + i + 1, s);
            }
        } else {
        ...
        if (g_pView) {
            for (int i = 0; i < g_pView->getSets(); ++i) {
                CheckMenuItem(menu,
                              i + 3,
                              MF_BYPOSITION |
                                (setAll ? MF_CHECKED : MF_UNCHECKED));
            }
        }
    }
}
...
void toggleVisibleSet(int index)
{
    ...
    if (menu && g_pView) {
        ...
        bool checked =
          !(GetMenuState(menu, index + 3, MF_BYPOSITION) & MF_CHECKED);
...
void doFileOpen(const char* inputFileName = NULL, Chunk* pChunk = NULL)
{
    ...
    if (inputFileName == NULL) {
        ...
        ofn.lpstrFilter =
          L"Adjacency Maps\0*.tga\0Chunk Files\0*.chunk\0All\0*.*\0";
        ofn.nFilterIndex    = 1;
...
void drawGenerateAllProgress()
{
    ...
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    glOrtho(viewCent.x - viewSize.x / 2,
            viewCent.x + viewSize.x / 2,
            viewCent.z - viewSize.z / 2,
            viewCent.z + viewSize.z / 2,
            -1.0f,
            1.0f);
    ...
    for (float x = spacebb.minBounds().x; x <= spacebb.maxBounds().x;
         x += gridSize) {
        glBegin(GL_LINES);
    ...
    for (float z = spacebb.minBounds().z; z <= spacebb.maxBounds().z;
         z += gridSize) {
        glBegin(GL_LINES);
    ...
    for (ChunkMap::iterator it = chunks.begin(); it != chunks.end(); it++) {
        ...
        for (; j != chunks.end(); ++j) {
            ...
            if (!doneIt) {
                glBegin(GL_POLYGON);
                glVertex2f(GLfloat(bb.minBounds().x),
                           GLfloat(bb.minBounds().z));
                glVertex2f(GLfloat(bb.maxBounds().x),
                           GLfloat(bb.minBounds().z));
                glVertex2f(GLfloat(bb.maxBounds().x),
                           GLfloat(bb.maxBounds().z));
                glVertex2f(GLfloat(bb.minBounds().x),
                           GLfloat(bb.maxBounds().z));
                glEnd();
            }
    ...
    for (ChunkMap::iterator it = chunks.begin(); it != chunks.end(); it++) {
        ...
        for (; j != chunks.end(); ++j) {
            ...
            if (doneIt) {
                bool calced =
                  g_calcChunksDone.find(pChunk) != g_calcChunksDone.end();
                bool notDirty = g_calcChunksNotDirty.find(pChunk) !=
                                g_calcChunksNotDirty.end();
    ...
    for (BW::set<Chunk*>::iterator it = chunkSet.begin(); it != chunkSet.end();
         ++it) {
        const BoundingBox& bb = (*it)->boundingBox();

        bool notDirty =
          g_calcChunksNotDirty.find(*it) != g_calcChunksNotDirty.end();
    ...
    if (g_currentChunk->loaded()) {
        ...
        if (g_currentChunkStatus == 1) {
            ...
            if (prop > 1.f)
                prop = 1.f;
            float zmax =
              bb.minBounds().z * (1.f - prop) + bb.maxBounds().z * prop;
...
bool isChunkLocked(Chunk* chunk)
{
    ...
    // ok, we've got got a coord between minX and maxX, it now needs to be 0
    // based
...
BW::vector<float> compileGirthsList(Chunk* pChunk)
{
    ...
    while (it != NavGenUDO::s_girthSeeds.end()) {
        ...
        // check to see if chunk boundary intersects with girth UDO bounding
        // box.
...
class NavgenScopedMenuDisabler
{
  ...
  public:
    NavgenScopedMenuDisabler()
      ...
      , generateAllOverwrite_(GetSubMenu(GetMenu(g_hWindow), 0),
                              ID_FILE_GENERATE_ALL_OVERWRITE)
      , clusterGenerate_(GetSubMenu(GetMenu(g_hWindow), 0),
                         ID_FILE_CLUSTERGENERATE)
...
// generate now calculates all girths required for the chunk, not just the
// selected one.
void doGenerate(Chunk* pChunk)
{
    ...
    ModelessInfoDialog dlg(
      g_hWindow,
      L"Please Wait",
      L"Please wait while NavGen generates the navigation mesh.\n\n"
      L"This process can take several minutes, during which time the program "
      L"may appear unresponsive.\n\n"
    ...
    for (uint gi = 0; gi < girthsToCalculate.size(); ++gi) {
        ...
        INFO_MSG(
          "Generating navPolys for girth size: %f (w: %f  h: %f  d: %f)\n",
          girthsToCalculate[gi],
          gSpec.width(),
          gSpec.height(),
          gSpec.depth());
...
bool waitChunkLoaded(Chunk* chunk, bool neighbourhood)
{
    ...
    ChunkManager::instance().camera(g_camMatrix,
...
void doClearAll()
{
    ...
    for (ChunkSpaceTraverser cst; !cst.done(); cst.next()) {
        ...
        if (hash(g_currentChunk->identifier().c_str()) % g_totalComputers !=
            g_myIndex) {
            g_statusWindow.skip();
            continue;
        }
        ...
        DataSectionPtr pChunkSect =
          BWResource::openSection(g_currentChunk->resourceID());
        ...
        DataSectionPtr pChunkBin =
          BWResource::openSection(g_currentChunk->binFileName(), true);
        ...
        BWResource::instance().purge(
          g_currentChunk->binFileName(), true, pChunkBin);
...
void doStatisticsAll()
{
    ...
    for (ChunkSpaceTraverser cst; !cst.done(); cst.next()) {
        ...
        DataSectionPtr pChunkBin =
          BWResource::openSection(g_currentChunk->binFileName());

        if (DataSectionPtr ds =
              pChunkBin->openSection("auxData/worldNavmesh")) {
            ++navmeshChunk;
...
void doGenerateAll(bool overwrite = false)
{
    ...
    if (g_totalComputers == 1) {
        ...
        BW::string msg(
          sformat("Start Cluster Generate (computer {0} of {1}):\n",
                  g_myIndex + 1,
                  g_totalComputers)
            .c_str());
    ...
    for (ChunkSpaceTraverser cst; !cst.done(); cst.next()) {
        ...
        if (g_currentChunk->isOutsideChunk()) {
            bool           modified = true;
            DataSectionPtr chunkBinSection =
              BWResource::openSection(g_currentChunk->binFileName());

            if (chunkBinSection) {
                modified =
                  !chunkBinSection->openSection("auxData/worldNavmesh");
            }
        ...
        NoticeMsg().write("%s\n",
                          (BW::string("Generating chunk: ") +
                           g_currentChunk->identifier() + '\n')
                            .c_str());
...
void showCursorPos()
{
    ...
    if (pChunk) {
        ...
        if (r.right && r.bottom && g_radiusX > 0 && g_radiusY > 0) {
            ...
            float x1 =
              (g_centreX - g_radiusX) * gridResolution + g_pView->gridMin().x;
            float y1 =
              (g_centreY - g_radiusY) * gridResolution + g_pView->gridMin().z;
            ...
            bw_snwprintf(buf,
                         ARRAY_SIZE(buf),
                         L"Girth %g LPoint(%0.2f, %0.2f) Point (%0.2f, %0.2f) "
                         L"Grid (%d,%d)",
                         g_girthsViewable[g_selectedGirth],
                         lx,
                         ly,
                         x,
                         y,
...
void selectNavPolyCommon(int newSel, bool showInfo)
{
    ...
    if (g_selectedIndex != -1) {
        SendDlgItemMessage(g_hInfoDialog, IDC_LIST1, LVM_DELETEALLITEMS, 0, 0);
        ...
        bw_snwprintf(buf,
                     ARRAY_SIZE(buf),
                     L"%f to %f",
                     g_pView->getMinHeight(g_selectedIndex),
                     g_pView->getMaxHeight(g_selectedIndex));
        ...
        for (i = 0; i < vertices; ++i) {
            ...
            bw_snwprintf(buf,
                         ARRAY_SIZE(buf),
                         L"%f",
                         v.x * gridResolution + g_pView->gridMin().x);
            item.iSubItem = 3;
            ...
            if (a > 0) {
                bw_snwprintf(
                  buf + wcslen(buf), ARRAY_SIZE(buf) - wcslen(buf), L"%d", a);
            }
            if (a < 0) {
                ...
                if (adjFlags == 0x0FFF)
                    ...
                    wchar_t ahtrans[] =
                      L"SMRC"; // solid, mixed, reserved, clear
                    bw_snwprintf(nbuf,
                                 ARRAY_SIZE(buf) - wcslen(buf),
                                 L"Ahead: %c%c %c%c %c%c",
                                 ahtrans[(adjFlags >> 0) & 3],
                                 ahtrans[(adjFlags >> 2) & 3],
                                 ahtrans[(adjFlags >> 4) & 3],
                                 ahtrans[(adjFlags >> 8) & 3],
                                 ahtrans[(adjFlags >> 8) & 3],
                                 ahtrans[(adjFlags >> 10) & 3]);
                }
            }
...
void selectNavPoly(bool doubleClicked)
{
    ...
    if (r.right && r.bottom && g_radiusX > 0 && g_radiusY > 0 && g_pView) {
        ...
        BW::vector<int> npFound =
          g_pView->findWaypoints(v, g_girthsViewable[g_selectedGirth]);
...
void displayHelp()
{
    ...
    if (g_hHelpWindow != NULL) {
        ...
        g_hHelpWindow = CreateDialog(GetModuleHandle(NULL),
        ...
        SetWindowPos(g_hHelpWindow,
                     HWND_TOPMOST,
                     (mainRect.left + mainRect.right) / 2 -
                       (helpRect.right - helpRect.left) / 2,
                     (mainRect.top + mainRect.bottom) / 2 -
                       (helpRect.bottom - helpRect.top) / 2,
                     0,
                     0,
    ...
    switch (msg) {
        ...
        case WM_CLOSE:
            if (!g_generating ||
                MessageBox(
                  hwnd,
                  L"Closing NavPoly will stop any generation process in "
            ...
            switch (LOWORD(w)) {
                ...
                case ']':
                    ...
                    if (g_selectedGirth >= (int)g_girthsViewable.size()) {
                        g_selectedGirth =
                          static_cast<int>(g_girthsViewable.size() - 1);
                    }
                    InvalidateRect(g_hWindow, NULL, FALSE);
                    showCursorPos();
                    break;
                case '[':
                    // g_selectedGirth =
                    // std::min(g_selectedGirth+1,g_girths.size()-1);
                    g_selectedGirth -= 1;
        ...
        case WM_COMMAND: {
            ...
            switch (LOWORD(w)) {
                ...
                case ID_FILE_OPENSPACE: {
                    BW::string space =
                      g_spaceManager->browseForSpaces(g_hWindow);
                ...
                case ID_FILE_CLEAR_ALL:
                    if (::MessageBox(
                          GetForegroundWindow(),
                          L"Are you sure you want to clear out all the current "
                          L"navigation polygon data?\n"
                          L"This could take a long time to regenerate",
                          L"Confirm Clearing of Navigation Data",
                          MB_YESNO) == IDNO)
                        break;
                    doClearAll();
                    InvalidateRect(g_hWindow, NULL, FALSE);
                ...
                case ID_FILE_GENERATE_ALL_OVERWRITE:
                    if (::MessageBox(
                          GetForegroundWindow(),
                          L"Are you sure you want to overwrite all the current "
                          L"navigation polygon data?\n"
                          L"This could take a long time to regenerate",
                          L"Confirm Overwriting of Navigation Data",
                          MB_YESNO) == IDNO)
                        break;
                    doGenerateAll(true);
                    InvalidateRect(g_hWindow, NULL, FALSE);
                ...
                case ID_FILE_CLUSTERGENERATE: {
                    if (DialogBox(GetModuleHandle(NULL),
                                  MAKEINTRESOURCE(IDD_CLUSTERGENDIALOG),
                                  g_hWindow,
                                  clusterDialogProc) == IDOK) {
                        doGenerateAll(false);
                    }
                    g_totalComputers = 1;
                    g_myIndex        = 0;
                ...
                case ID_VIEW_POLYGONBORDERS:
                    ...
                    CheckMenuItem(g_hMenu,
                                  LOWORD(w),
                                  g_viewPolygonBorders ? MF_CHECKED
                                                       : MF_UNCHECKED);
                ...
                case ID_VIEW_ERRORLOG:
                    CheckMenuItem(g_hMenu,
                                  LOWORD(w),
                                  AsyncMessage().isShow() ? MF_UNCHECKED
                                                          : MF_CHECKED);
...
static int s_chunkTokenSet =
  ChunkModel_token | ChunkLight_token | ChunkTerrain_token | ChunkTree_token;
...
void setupBWLockd(const BW::string& spacePath, float gridSize)
{
    ...
    if (!g_bigbangd.empty() && !g_username.empty()) {
        ...
        static const int   xExtent =
          (int)((MAX_TERRAIN_SHADOW_RANGE + 1.f) / gridSize);
...
#endif
{
    ...
    g_floodResultPath =
      drRoot->readString("floodResultPath", g_floodResultPath);
    ...
    Vector3 rot =
      mcRoot->readVector3(spacePath + "/space.settings/startDirection");
        ...
        if (!g_spaceManager->num() || !changeSpace(g_spaceManager->entry(0))) {
            for (;;) {
                MsgBox mb(L"Open space",
                          L"NavGen cannot find a valid default space to open, "
                          L"do you want to open "
...
bool changeSpace(const BW::string& space)
{
    ...
    gSpaceLock = CreateFile(wfilename.c_str(),
    ...
    if (gSpaceLock == INVALID_HANDLE_VALUE) {
        MsgBox mb(
          ...
          Localise(
            L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/UNABLE_TO_OPEN_SPACE",
            space),
    ...
    g_currentNavmeshGenerator = pSpaceSettings->readString("navmeshGenerator");
    if (!isCorrectNavmeshGenerator()) {
        ErrorLogHandler.print(
          "NOTE: '" + space +
            "' is not configured to be generated using NavGen"
            " (generator is set to '" +
    ...
    DataSectionPtr spaceSection =
      BWResource::openSection(space + "/space.localsettings");
    if (spaceSection) {
        Vector3 dir, pos;
        pos =
          spaceSection->readVector3("startPosition", Vector3(0.f, 2.f, 0.f));
    ...
    for (;;) {
        ...
        if (pChunk != NULL) {
            BW::string filePath =
              BWResolver::resolveFilename(pChunk->resourceID());
            doFileOpen(filePath.c_str(), pChunk);
            break;
        }
...
void updateMoo(float dTime, bool allowMovement)
{
    ...
    g_conn.tick();
    if (allowMovement)
        moveMooCameraFromKeys(dTime);

    Matrix viewMatrix;
    viewMatrix.invert(g_camMatrix);
    ...
    ChunkManager::instance().cameraSpace()->enviro().tick(dTime,
                                                          isCameraOutside());
...
void drawMooPolygon(int pidx)
{
    ...
    if (g_viewPolygonBorders) {
        ...
        for (v = 0; v <= vertexCount; v++)
            polygonBuf[v].colour_ = 0x99ffff00;
...
void drawMooBSPNodes()
{
    ...
    while (cursor < cursorMax) {
        ...
        Moo::rc().drawPrimitiveUP(
          D3DPT_TRIANGLEFAN, nv - 2, &polygonBuf[0], sizeof(Moo::VertexXYZL));
        ...
        Moo::rc().drawPrimitiveUP(
          D3DPT_TRIANGLESTRIP, nv * 2, &polygonBuf[0], sizeof(Moo::VertexXYZL));
...
void drawMooGridPiece(int cursor)
{
    ...
    Vector3 pos;
    Vector4 ahgts;
    if (!g_pView->gridPoll(cursor, pos, ahgts))
        return;
    ...
    for (int i = 0; i < 4; i++) {
        ...
        lineBuf[nlines * 2 + 1].pos_ =
          Vector3(pos.x + dx[i], ahgts[i], pos.z + dz[i]);
...
void drawMooColTri(Vector3* pVertex)
{
    ...
    Moo::rc().drawPrimitiveUP(
      D3DPT_LINESTRIP, 3, &triBuf[0], sizeof(Moo::VertexXYZL));
}
...
void drawMoo(float dTime)
{
    ...
    Moo::rc().device()->Clear(
      0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, 0x00ffffff, 1, 0);
    ...
    // if we hit assert because of it then we need to move renderRompPostScene
    // higher
    navgenDrawContext.begin(Moo::DrawContext::ALL_CHANNELS_MASK);
    renderRompPostScene(dTime);
    navgenDrawContext.end(Moo::DrawContext::ALL_CHANNELS_MASK);
    navgenDrawContext.flush(Moo::DrawContext::TRANSPARENT_CHANNEL_MASK |
...
void reannotate(Chunk* pChunk)
{
    ...
    LONG_PTR oldWndProc = SetWindowLongPtr(
      g_hMooWindow, GWLP_WNDPROC, (LONG_PTR)reannotatingWndProc);
...
void selectMooWaypoint(bool doubleClicked)
{
    ...
    for (pidx = 0; pidx < pc; pidx++) {
        ...
        for (v = 0; v < vc; v++) {
            ...
            g_pView->getVertex(
              pidx, (v + 1) % vc, thisV, adj, adjToAnotherChunk);
    ...
    switch (msg) {
        ...
        case WM_CHAR:
            switch (LOWORD(w)) {
                ...
                case '[':
                    //                  g_selectedGirth =
                    //                  std::max(g_selectedGirth,uint(1))-1;
                    g_selectedGirth -= 1;
                    if (g_selectedGirth < 0) {
                        g_selectedGirth = 0;
                    }
...
int bwWinMain(HINSTANCE hInstance, HINSTANCE, LPWSTR commandLine, int)
{
    ...
    BW::string settingsFilename =
      BWResource::appDirectory() + "navgen_settings.xml";
    ...
    g_annotate =
      g_navgenSettings->getRootSection()->readBool("annotate", false);
    ...
    if (!languages.empty()) {
        ...
        StringProvider::instance().load(
          BWResource::openSection("helpers/languages/navgen_rc_en.xml"));
        StringProvider::instance().load(
    ...
    bw_utf8tow(
      g_navgenSettings->getRootSection()->readString("currentLanguage", ""),
      currentLanguage);
    bw_utf8tow(
      g_navgenSettings->getRootSection()->readString("currentCountry", ""),
      currentCountry);
    ...
    g_hMooWindow = CreateWindow(L"navgenMoo",
                                L"NavPoly Renderer",
                                WS_OVERLAPPEDWINDOW,
                                CW_USEDEFAULT,
                                CW_USEDEFAULT,
    ...
    g_hInfoDialog = CreateDialog(
      hInstance, MAKEINTRESOURCE(IDD_WAYPOINT_INFO), g_hWindow, infoDialogProc);
    ...
    g_viewAdjacencies =
      (GetMenuState(g_hMenu, ID_VIEW_ADJACENCIES, MF_BYCOMMAND) & MF_CHECKED) !=
      0;
    g_viewBSPNodes =
      (GetMenuState(g_hMenu, ID_VIEW_BSPNODES, MF_BYCOMMAND) & MF_CHECKED) != 0;
    g_viewPolygonArea =
      (GetMenuState(g_hMenu, ID_VIEW_POLYGONAREA, MF_BYCOMMAND) & MF_CHECKED) !=
      0;
    g_viewPolygonBorders =
      (GetMenuState(g_hMenu, ID_VIEW_POLYGONBORDERS, MF_BYCOMMAND) &
       MF_CHECKED) != 0;

    g_processor = g_navgenSettings->getRootSection()->readInt("processor", 1);
    g_writeTGAs =
      g_navgenSettings->getRootSection()->readBool("writeTGAs", true);
    g_reannotation =
      g_navgenSettings->getRootSection()->readBool("reannotation", false);
    ...
    g_chunkLoadDistance = g_navgenSettings->getRootSection()->readFloat(
      "loadDistance", g_chunkLoadDistance);
    BgTaskManager::init();
    ...
    if (strstr(cmdLine.c_str(), "/s")) {
        ...
        if (!BWResource::openSection(space + "/space.settings")) {
            ...
            BW::wstring wmsg;
            bw_utf8tow(msg, wmsg);
            MessageBox(GetForegroundWindow(), wmsg.c_str(), L"NavGen", MB_OK);
            return 3;
        }
    ...
    g_accelerators = LoadAccelerators(::GetModuleHandle(NULL),
    ...
    if (workInCommandLine) {
        ...
        while (1) {
            ...
            float  dTime =
              float(double(timeNow - timeLast) / stampsPerSecondD());
            timeLast = timeNow;
    ...
    switch (msg) {
        case WM_INITDIALOG:
            for (int i = 2; i < 50; ++i) {
                ...
                SendDlgItemMessage(
                  hwnd, IDC_TOTALCOMPUTER, CB_ADDSTRING, 0, (LPARAM)s);
            }
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                SendDlgItemMessage(
                  hwnd, IDC_MYINDEX, CB_ADDSTRING, 0, (LPARAM)s);
            }
        ...
        case WM_COMMAND:
            if (HIWORD(w) == CBN_SELCHANGE) {
                if (LOWORD(w) == IDC_TOTALCOMPUTER) {
                    SendDlgItemMessage(
                      hwnd, IDC_MYINDEX, CB_RESETCONTENT, 0, 0);

                    g_totalComputers =
                      SendDlgItemMessage(
                        hwnd, IDC_TOTALCOMPUTER, CB_GETCURSEL, 0, 0) +
                      2;
                    ...
                    g_myIndex =
                      SendDlgItemMessage(hwnd, IDC_MYINDEX, CB_GETCURSEL, 0, 0);
                }
...
int WINAPI wWinMain(HINSTANCE hInstance,
                    ...
                    int       cmdShow)
{
    return CallWithExceptionFilter(
      bwWinMain, hInstance, hPrev, commandLine, cmdShow);
}
