...
static BW::map<VisualMesh::BloatVertex,
               BW::set<VisualMesh::BVVector::size_type>>
  verticeSetMap;
static BW::map<int, BW::vector<VisualMesh::BVVector::size_type>>
  verticeIndexSetMap;
                           ...
                           int           index)
{
    if (verticeIndexSetMap.find(realIndex) != verticeIndexSetMap.end()) {
        BW::vector<BVVector::size_type>& verticeIndexSet =
          verticeIndexSetMap[realIndex];
        for (BW::vector<BVVector::size_type>::iterator iter =
               verticeIndexSet.begin();
             iter != verticeIndexSet.end();
             ++iter) {
...
void VisualMesh::getNormalsFromModifier(INode* pNode)
{
    ...
    if (pNormsMod) {
        DEBUG_MSG(
          "MFXExport::preProcess - found edit normals modifier on object %s\n",
          pNode->GetName());
        ...
        for (int i = 0; i < nFaces; i++) {
            ...
            int                      nMatching = 0;
            while (it != end) {
                if (std::find(face.begin(), face.end(), it->realIndex[0]) !=
                      face.end() &&
                    std::find(face.begin(), face.end(), it->realIndex[1]) !=
                      face.end() &&
                    std::find(face.begin(), face.end(), it->realIndex[2]) !=
                      face.end()) {
                    if (vertexNormals[it->realIndex[0]] != NULL) {
                        int ind = static_cast<int>(vertices_.size());
                        vertices_.push_back(vertices_[it->index[0]]);
                        vertices_.back().normal =
                          Normalize(*vertexNormals[it->realIndex[0]]);
                        it->index[0] = ind;
                    }
                    if (vertexNormals[it->realIndex[1]] != NULL) {
                        ...
                        vertices_.back().normal =
                          Normalize(*vertexNormals[it->realIndex[1]]);
                        it->index[1] = ind;
                    }
                    if (vertexNormals[it->realIndex[2]] != NULL) {
                        ...
                        vertices_.back().normal =
                          Normalize(*vertexNormals[it->realIndex[2]]);
                        it->index[2] = ind;
                    }
                }
                it++;
        ...
        switch (def.type) {
            ...
            case TYPE_BITMAP: {
                ...
                if (b && bw_wtoutf8(b->bi.Name()) != BW::string("None")) {
                    m.textureOverrides_.push_back(
                      std::make_pair(BW::string(bw_wtoutf8(def.int_name)),
                                     BW::string(bw_wtoutf8(b->bi.Name()))));
                }
#else
                if (b && b->bi.Name() != BW::string("None")) {

                    m.textureOverrides_.push_back(std::make_pair(
                      BW::string(def.int_name), BW::string(b->bi.Name())));
                }
#endif
                break;
            }
...
bool VisualMesh::createMeshParticlePrimGroups(PGVector& primGroups)
{
    ...
    if (primGroups.size() != 1) {
        ...
        MessageBox(GetForegroundWindow(),
                   L"VisualMesh::createMeshParticlePrimGroups - Mesh particles "
                   L"must use only 1 material/mesh.\n",
                   L"Visual Exporter",
                   MB_OK | MB_ICONEXCLAMATION);
#else
        MessageBox(GetForegroundWindow(),
                   "VisualMesh::createMeshParticlePrimGroups - Mesh particles "
                   "must use only 1 material/mesh.\n",
                   "Visual Exporter",
                   MB_OK | MB_ICONEXCLAMATION);
#endif
 ...
 *	@param spExistingVisual	DataSection of the existing visual to get the
 material from. *	@param primitiveFile	The filename of the primitive
 record. *	@param useIdentifier	Flag use identifier.
 */
                      ...
                      bool              useIdentifier)
{
    ...
    if (!pFile)
        pFile = new BinSection(primitiveFile,
                               new BinaryBlock(0, 0, "BinaryBlock/VisualMesh"));
        ...
        if (pRenderSet) {
            ...
            if (pGeometry) {
                ...
                for (uint32 i = 0; i < materials_.size(); i++) {
                    ...
                    ;
                    if (mat.inUse)
                        mat.save(pGeometry, spExistingVisual, primGroup++);
                }
            }
        }
    }
...
bool VisualMesh::savePrimXml(const BW::string& xmlFile)
{
    ...
    if (ExportSettings::instance().exportMode() ==
        ...
        for (vvIt = vertices.begin(); vvIt != vertices.end(); ++vvIt) {
            ...
            Vector3  colour((float)((c & 0x00ff0000) >> 16) / 255.f,
                           (float)((c & 0x0000ff00) >> 8) / 255.f,
                           (float)(c & 0x000000ff) / 255.f);
...
void VisualMesh::Material::resources(BW::vector<BW::string>& retResources)
{
    if (this->fxFile_.length()) {
        ...
        for (uint32 i = 0; i < textureOverrides_.size(); i++) {
            BW::string textureName =
              unifySlashes(toLower(textureOverrides_[i].second));
 ...
 *	@return	Success or failure.
 */
bool VisualMesh::Material::checkFilename(BW::string&       fxName,
                                         const BW::string& fxPostfix)
{
    ...
    if (pPG) {
        ...
        if (!this->findAndCopyExistingMaterial(
            ...
            if (ExportSettings::instance().exportMode() ==
                ...
                pMaterial->writeString("fx",
                                       BWResolver::dissolveFilename(fxName));
                for (uint32 i = 0; i < textureOverrides_.size(); i++) {
                    DataSectionPtr pProp = pMaterial->newSection("property");
                    pProp->setString(textureOverrides_[i].first);
                    BW::string textureName =
                      unifySlashes(textureOverrides_[i].second);
                    pProp->writeString(
                      "Texture", BWResolver::dissolveFilename(textureName));
                }
                for (uint32 i = 0; i < vectorOverrides_.size(); i++) {
                    DataSectionPtr pProp = pMaterial->newSection("property");
                    pProp->setString(vectorOverrides_[i].first);
                    pProp->writeVector4(
                      "Vector4", *(Vector4*)(&vectorOverrides_[i].second));
                }
                ...
                if (mapIdentifier.length() > 4 && mapIdMeaning != 2) {
                    BW::string mfmID =
                      mapIdentifier.substr(0, mapIdentifier.length() - 4) +
                      ".mfm";
                    DataSectionPtr mfmsect = BWResource::openSection(mfmID);
                    if (mfmsect) {
                        if ((mfmsect->openSection("mfm") ||
                ...
                if (mapIdMeaning == 1) {
                    if (skin == NO_SKIN) {
                        ...
                        if (mapIdentifier.length() > 8 &&
                            mapIdentifier.substr(mapIdentifier.length() - 8,
                                                 4) == "_afx") {
                            fx += "_add";
                        }
                        // RA: changing the default shader for materials with a
                        // .tga file
                        /*else if (mapIdentifier.length() > 4 &&
                            mapIdentifier.substr( mapIdentifier.length() - 4, 4
                        ) == ".tga")
                        {
                            fx += "_alpha";
                        }*/
                    ...
                    if (mapIdentifier.length()) {
                        DataSectionPtr pProp =
                          pMaterial->newSection("property");
                        pProp->setString("diffuseMap");
                        pProp->writeString("Texture", mapIdentifier);
                    }
                } else if (mapIdMeaning == 2) {
...
bool VisualMesh::checkDXMaterials(const BW::string& fxPostfix)
{
    ...
    for (i = 0; i < materials_.size(); i++) {
        if ((ExportSettings::instance().exportMode() !=
            ...
            materials_[i].fxFile_.length()) {
            BW::string fxName;
            if (!materials_[i].checkFilename(fxName, fxPostfix)) {
#ifdef _UNICODE
                MessageBox(GetForegroundWindow(),
                           L"VisualMesh::Material::save - Skinned shader "
                           L"applied to a non-skinned mesh and the required "
                           L"shader variant doesn't exist.\n",
                           L"Visual Exporter",
                           MB_OK | MB_ICONEXCLAMATION);
#else
                MessageBox(GetForegroundWindow(),
                           "VisualMesh::Material::save - Skinned shader "
                           "applied to a non-skinned mesh and the required "
                           "shader variant doesn't exist.\n",
                           "Visual Exporter",
                           MB_OK | MB_ICONEXCLAMATION);
#endif
                return false;
...
bool VisualMesh::init(INode* node, bool checkMats)
{
    ...
    if (mesh->getNumFaces() && mesh->getNumVerts()) {
        ...
        vertexColours_ = (mesh->getNumVertCol() && mesh->vcFace) ||
                         (vertexAlpha && vertexAlphaFaces);
        if (checkMats && !checkDXMaterials(""))
            return false;

        if (!hasUVs && (ExportSettings::instance().bumpMapped() &&
                        !(ExportSettings::instance().exportMode() ==
                          ExportSettings::MESH_PARTICLES))) {
        ...
        Matrix3 scaleMatrix =
          isStatic ? meshMatrix : meshMatrix * Inverse(nodeMatrix);
        ...
        for (int i = 0; i < mesh->getNumFaces(); i++) {
            ...
            for (int j = 0; j < 3; j++) {
                ...
                if (hasUVs) {
                    ...
                    bv.uv =
                      reinterpret_cast<Point2&>(mapVerts[mapFaces[i].t[j]]);
                    bv.uv.y = 1 - bv.uv.y;
                }
                if (dualUV_) {
                    bv.uv2 = reinterpret_cast<Point2&>(
                      uv2MapVerts[uv2MapFaces[i].t[j]]);
                    bv.uv2.y = 1 - bv.uv2.y;
                }
                if (vertexColours_) {
                    Point3 colour = (mesh->getNumVertCol() && mesh->vcFace)
                                      ? reinterpret_cast<Point3&>(
                                          mesh->vertCol[mesh->vcFace[i].t[j]])
                                      : Point3(1.f, 1.f, 1.f);
                    float  alpha  = vertexAlpha
                                      ? vertexAlpha[vertexAlphaFaces[i].t[j]].x
                                      : 1.0f;
