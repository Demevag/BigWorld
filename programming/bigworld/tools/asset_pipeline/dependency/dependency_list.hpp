...
class DependencyList
{
  ...
  public:
    ...
    const BW::vector<Input>& secondaryInputs() const
    {
        return secondaryInputs_;
    }
    const BW::vector<Output>& intermediateOutputs() const
    {
        return intermediateOutputs_;
    }
    ...
    void addPrimaryConverterDependency(uint64            converterId,
                                       const BW::string& converterVersion);
    ...
    void addSecondarySourceFileDependency(const BW::string& filename,
                                          bool              critical);
    /// Add a secondary intermediate file dependency.
    /// \param filename the name of the compiled file to depend on.
    /// \param critical true if this dependency is a critical dependency.
    void addSecondaryIntermediateFileDependency(const BW::string& filename,
                                                bool              critical);
    /// Add a secondary output file dependency.
    /// \param filename the name of the compiled file to depend on.
    /// \param critical true if this dependency is a critical dependency.
    void addSecondaryOutputFileDependency(const BW::string& filename,
                                          bool              critical);
    ...
    /// \param includeSecondary should the secondary inputs be included in the
    /// combined hash
  ...
  private:
    /// Helper function for serialising in a vector of dependencies from a data
    /// section. \param pSection the data section to serialise in from. \param
    /// dependencies the vector to store the serialised in dependencies.
    bool serialiseIn(DataSectionPtr pSection, BW::vector<Input>& dependencies);
    /// Helper function for serialising out a vector of dependencies to a data
    /// section. \param pSection the data section to serialise out to. \param
    /// dependencies the vector of dependencies to serialise out.
    bool serialiseOut(DataSectionPtr           pSection,
                      const BW::vector<Input>& dependencies) const;
    /// Helper function for serialising in a vector of outputs from a data
    /// section. \param pSection the data section to serialise in from. \param
    /// outputs the vector to store the serialised in outputs.
    bool serialiseIn(DataSectionPtr pSection, BW::vector<Output>& outputs);
    /// Helper function for serialising out a vector of outputs to a data
    /// section. \param pSection the data section to serialise out to. \param
    /// outputs the vector of outputs to serialise out.
    bool serialiseOut(DataSectionPtr            pSection,
