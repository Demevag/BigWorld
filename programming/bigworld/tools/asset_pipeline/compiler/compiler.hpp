...
class Compiler
{
  ...
  public:
    /// Register conversion rules that will be used by the asset pipeline to
    /// discover tasks. \param conversionRule the conversion rule to register.
    virtual void registerConversionRule(ConversionRule& conversionRule) = 0;
    /// Register converters that will be used by the asset pipeline to convert
    /// tasks. \param converterInfo struct containing the details of the
    ...
    virtual void registerResourceCallbacks(
      ResourceCallbacks& resourceCallbacks) = 0;
    ...
    /// \param conversionTask an optional conversion task that will return the
    /// task associated with this dependency. \return true if the dependency is
    /// up to date at the time of the function returning
    ///		  false if the dependency is not yet up to date at the time of the
    ///function returning,
    ///        or if there was an error processing the dependency.
    virtual bool ensureUpToDate(const Dependency&      dependency,
                                const ConversionTask*& conversionTask) = 0;
    /// Ask the asset pipeline for the source of an file.
    /// \param file the file to find the source for.
    /// \param source output set to true if an error was encountered processing
    /// the dependency. \return true if the source for the file could be found.
    virtual bool getSourceFile(const BW::StringRef& file,
                               BW::string&          sourceFile) = 0;
    ...
    /// \param forceUpdate if false, allow the asset pipeline to return a cached
    /// value of the hash \return the hash of the file.
    virtual uint64 getFileHash(const BW::string& fileName,
                               bool              forceUpdate) = 0;
    /// Check if the stored hash of a file is out of date and update it if it
    /// is. \param fileName the name of the file on disk to hash. \return true
    /// if the stored hash is out of date.
    ...
    /// \param recursive specify whether the directory should be recursively
    /// hashed \return the hash of the file.
    ...
    /// Callback when the asset pipeline attempts to iterate a file searching
    /// for tasks. \param file the file iterated. \return true if this file
    /// should be iterated
    virtual bool shouldIterateFile(const BW::StringRef& file) = 0;
    /// Callback when the asset pipeline attempts to iterate a directory
    /// searching for tasks. \param directory the directory iterated. \return
    /// true if this directory should be iterated
    ...
    /// Callback before the asset pipeline starts creating the dependencies of a
    /// task. \param conversionTask the task the dependencies will be created
    /// for.
    virtual void onPreCreateDependencies(ConversionTask& conversionTask) = 0;
    /// Callback after the asset pipeline finishes creating the dependencies of
    /// a task. \param conversionTask the task the dependencies were created
    /// for.
    ...
    /// Callback when the asset pipeline successfully reads a file from the
    /// cache. \param filename the name of the file read.
    virtual void onCacheRead(const BW::string& filename) = 0;
    /// Callback when the asset pipeline unsuccessfully reads a file from the
    /// cache. \param filename the name of the file attempted to be read.
    virtual void onCacheReadMiss(const BW::string& filename) = 0;
    /// Callback when the asset pipeline successfully writes a file to the
    /// cache. \param filename the name of the file written.
    virtual void onCacheWrite(const BW::string& filename) = 0;
    /// Callback when the asset pipeline unsuccessfully writes a file to the
    /// cache. \param filename the name of the file attempted to be written.
