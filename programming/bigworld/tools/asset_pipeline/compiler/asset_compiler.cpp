...
{
    BW::vector<BW::wstring> baseResourcePaths =
      AssetPipe::getBaseResourcePaths();
    for (BW::vector<BW::wstring>::iterator it = baseResourcePaths.begin();
         it != baseResourcePaths.end();
         ++it) {
        BW::wstring mutexName     = L"Local\\AssetPipeline" + *it;
        HANDLE      compilerMutex = CreateMutexW(0, TRUE, mutexName.c_str());
        if (GetLastError() == ERROR_ALREADY_EXISTS) {
            ::MessageBox(NULL,
                         L"Only one instance of the AssetPipeline is allowed "
                         L"to be active per resource path.",
                         L"AssetPipeline",
                         MB_OK);
            exit(-1);
        }
        ...
        while (true) {
            ...
            if (WaitForSingleObject(compilerMutex, 100) != WAIT_OBJECT_0) {
                ::MessageBox(NULL,
                             L"Only one instance of the AssetPipeline is "
                             L"allowed to be active per resource path.",
                             L"AssetPipeline",
                             MB_OK);
                exit(-1);
            }
...
AssetCompiler::~AssetCompiler()
{
    ...
    for (BW::vector<HANDLE>::const_iterator it = compilerMutexes_.begin();
         it != compilerMutexes_.end();
         ++it) {
        CloseHandle(*it);
    }
...
void AssetCompiler::initCompiler()
{
    ...
    taskSemaphore_ = ::CreateSemaphore(NULL, numThreads_, numThreads_, NULL);

    // Start the background task manager
    BgTaskManager::init();
    BgTaskManager::instance().startThreads(ASSET_PIPELINE_CATEGORY,
                                           numThreads_);
...
void AssetCompiler::registerConverter(ConverterInfo& converterInfo)
{
    ...
    std::pair<ConverterMap::iterator, bool> insertIt = converterMap_.insert(
      std::make_pair(converterInfo.typeId_, &converterInfo));
    MF_ASSERT(insertIt.second);
}

void AssetCompiler::registerResourceCallbacks(
...
bool AssetCompiler::ensureUpToDate(const Dependency&      dependency,
    ...
    switch (dependency.getType()) {
        ...
        case IntermediateFileDependencyType: {
            ...
            return ensureCompiled(
              intermediatefileDependency.getFileName(), false, conversionTask);
        }
        ...
        case OutputFileDependencyType: {
            ...
            return ensureCompiled(
              outputfileDependency.getFileName(), true, conversionTask);
        }
...
bool AssetCompiler::getSourceFile(const BW::StringRef& file,
                                  BW::string&          sourceFile)
{
    ConversionRules::const_reverse_iterator it;
    for (it = conversionRules_.crbegin(); it != conversionRules_.crend();
         ++it) {
        ...
        if (BWResource::pathIsRelative(sourceFile)) {
            ERROR_MSG("Conversion rule returned a relative source file %s",
                      sourceFile.c_str());
        }
        return true;
    }
...
uint64 AssetCompiler::getHash(const Dependency& dependency)
{
    switch (dependency.getType()) {
        ...
        case IntermediateFileDependencyType: {
            // The hash of a compiled file dependency is a hash of its compiled
            // file on disk
            const IntermediateFileDependency& intermediatefileDependency =
              static_cast<const IntermediateFileDependency&>(dependency);
        ...
        case ConverterDependencyType: {
            // The hash of a converter dependency is a combination of its id and
            // version
            const ConverterDependency& converterDependency =
              static_cast<const ConverterDependency&>(dependency);
        ...
        case ConverterParamsDependencyType: {
            // The hash of a converter params dependency is a hash of the
            // parameter string
            const ConverterParamsDependency& converterParamsDependency =
              static_cast<const ConverterParamsDependency&>(dependency);
            return Hash64::compute(
...
uint64 AssetCompiler::getFileHash(const BW::string& fileName, bool forceUpdate)
{
    ...
    if (!forceUpdate) {
        ...
        StringHashMap<uint64>::const_iterator hashIt =
          fileHashes_.find(fileName);
...
bool AssetCompiler::checkFileHashUpToDate(const BW::string& fileName)
{
...

    {
        ...
        if (it == fileHashes_.end()) {
            // Cannot find the file in our stored hashes so it must be out of
            // date.
            return false;
        }
        oldHash = it->second;
...
uint64 AssetCompiler::getDirectoryHash(const BW::string& directory,
                                       ...
                                       bool              recursive)
{
    ...
    for (IFileSystem::Directory::iterator it = dir.begin(); it != dir.end();
        ...
        if (ft == IFileSystem::FT_FILE) {
            ...
            uint64 subHash =
              getDirectoryHash(subPath, pattern, regex, recursive);
            if (subHash != 0) {
...
bool AssetCompiler::ensureCompiled(const BW::string&      compiledFileName,
                                   bool                   output,
                                   const ConversionTask*& o_conversionTask)
{
    BW::string sourceFile;
    if (getSourceFile(compiledFileName, sourceFile)) {
        ...
        if (recursive_) {
            ...
            if (task.status_ == ConversionTask::NEW) {
                // Grab a mutex to ensure that only we are going to trigger a
                // build for this task.
            ...
            if (task.status_ == ConversionTask::QUEUED) {
                ...
                if (shouldProcess) {
                    ...
                    for (it = taskQueue_.begin(); it != taskQueue_.end();
                         ++it) {
                        if (*it == &task) {
                            break;
                        }
            ...
            // If shouldProcess is false it means another thread managed to
            // trigger a build for this task before we did.
            if (shouldProcess) {
            ...
            while (task.status_ != ConversionTask::DONE &&
                ...
                if (numThreads_ == 1) {
                    // We can't be waiting on a task on a different thread as we
                    // aren't running multi-threaded. Must have encountered a
                    // cyclic dependency.
                    ERROR_MSG("Cyclic dependency found %s\n",
                              task.source_.c_str());
                    break;
                } else if (task.threadId_ != 0) {
                    // Set our task thread id to the thread id of the task we
                    // are waiting for.
    ...
    if (output) {
        // Don't know how to compile this file, check if we can copy it from
        // somewhere
        BW::string sourcePath = compiledFileName;
        resolveSourcePath(sourcePath);
        if (BWResource::fileExists(sourcePath)) {
            if (outputPath_.empty()) {
                // A source for this file exists and we are not using an output
                // path. We can just use this file in place.
                return true;
            }
            ...
            if (BWResource::fileExists(outputPath)) {
                if (getFileHash(sourcePath, false) ==
                    getFileHash(outputPath, false)) {
...
void AssetCompiler::onTaskStarted(ConversionTask& conversionTask)
{
    ...
    while (true) {
        s_currentTask->fileHandle_ =
          ::CreateFileW(bw_utf8tow(conversionTask.source_).c_str(),
                        GENERIC_READ,
                        FILE_SHARE_READ,
                        0,
                        OPEN_EXISTING,
...
void AssetCompiler::onTaskResumed(ConversionTask& conversionTask)
{
    ...
    if (recursive_) {
        ...
        for (ConversionTask::SubTaskList::const_iterator it =
               s_currentTask->subTasks_.begin();
             it != s_currentTask->subTasks_.end();
             ++it) {
...
void AssetCompiler::onTaskSuspended(ConversionTask& conversionTask)
{
    ...
    if (recursive_) {
        ...
        for (it = s_currentTaskQueue->begin(); it != s_currentTaskQueue->end();
             ++it) {
            if ((*it)->status_ != ConversionTask::QUEUED) {
                continue;
            }

            for (itQueued = taskQueue_.begin(); itQueued != taskQueue_.end();
                 ++itQueued) {
                if (*it == *itQueued) {
                    taskQueue_.erase(itQueued);
                    // Set the status of this task back to new as it is no
                    // longer in the queue.
        ...
        // Find the tasks that we depend on that have already begun processing
        // and make sure that we insert our task behind them in the queue. We
        // cannot remove these tasks from the queue and push them to the front
        // because these tasks may in turn be dependent on other tasks before
        // them in the queue.
        ...
        for (it = s_currentTaskQueue->begin(); it != s_currentTaskQueue->end();
            ...
            for (itQueued = itInsert; itQueued != taskQueue_.end();
                 ++itQueued) {
                if (*it == *itQueued) {
                    itInsert = itQueued + 1;
                    break;
                                  ...
                                  va_list /*argPtr*/)
{
    ...
    // If this thread is creating dependencies or converting and an error or
    // warning message was triggered, Notify the asset pipeline that an error or
    // warning was detected for the current task.
    MF_ASSERT(s_currentTask != NULL);
    ...
    if (messagePriority == DebugMessagePriority::MESSAGE_PRIORITY_ERROR ||
        ...
        setError();
               ...
               DebugMessagePriority::MESSAGE_PRIORITY_WARNING) {
        setWarning();
    }
    return true;
}
...
void AssetCompiler::handleCritical(const char* msg)
{
    ...
    // If an assert was fired while creating dependencies or converting, throw
    // an exception to cause the current task to error but not crash the
    // application.
    throw msg;
#endif
}
...
bool AssetCompiler::resolveRelativePath(BW::string& path) const
{
    ...
    if (!intermediatePath_.empty() &&
        strncmp(path.c_str(),
                intermediatePath_.c_str(),
                intermediatePath_.length()) == 0) {
...
bool AssetCompiler::resolveCustomPath(const BW::string& basePath,
                                      BW::string&       path) const
{
    ...
    if (strncmp(path.c_str(), actualBasePath.data(), actualBasePath.length()) ==
        0) {
