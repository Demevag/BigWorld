...
const uint64 TestConverter::s_TypeId =
  Hash64::compute(TestConverter::getTypeName());
const char* TestConverter::s_Version = "2.6";

namespace {
    SimpleMutex g_ResourceLock;

    bool ExtractFiles(const BW::string&       sourcefile,
                      BW::string&             directory,
                                       ...
                                       DependencyList&   dependencies)
{
    ...
    for (BW::vector<BW::string>::const_iterator it = files.cbegin();
         it != files.cend();
         ++it) {
                            ...
                            BW::vector<BW::string>& outputFiles)
{
    ...
    BW::string outputFile =
      BWResource::changeExtension(sourcefile, ".testcompiledasset");
