...
TaskFinder::TaskFinder(Compiler& compiler, ConversionRules& rules)
  : compiler_(compiler)
  , rules_(rules)
{
}
...
ConversionTask& TaskFinder::getTask(const BW::StringRef& filename)
{
    ...
    if (!compiler_.shouldIterateDirectory(directory) ||
        ...
        pNewTask->converterId_ =
          ConversionTask::s_unknownId; // unknown converter type
                                    ...
                                    const bool           bRoot)
{
    ...
    //		not tasks for files on disk that have tasks created via
    // dependencies.
    //
