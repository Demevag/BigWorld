// VS2019: warning C4996: 'std::tr1': warning STL4002: The non-Standard std::tr1
...
const uint64 DefaultSpaceConverter::s_TypeId =
  Hash64::compute(DefaultSpaceConverter::getTypeName());
                                        ...
                                        DependencyList&   dependencies)
{
    ...
    dependencies.addSecondaryDirectoryDependency(
      spaceDir, ".*\\.(chunk|cdata)$", true, true, true);
                                       ...
                                       CommandLine&          commandLine)
{
    ...
    for (WriterList::iterator iter = writers_.begin(); iter != writers_.end();
         ++iter) {
                ...
                [CompiledSpace::StaticSceneTypes::eVLO])
{
    ...
    settings().addBoundGenerator(std::tr1::bind(
      &CompiledSpace::ParticleSystemWriter::boundBox, &particles_));
    settings().addBoundGenerator(std::tr1::bind(
      &CompiledSpace::StaticSceneWriter::boundBox, &staticScene_));
    settings().addBoundGenerator(
      std::tr1::bind(&CompiledSpace::StaticSceneWriter::boundBox, &vloScene_));
    settings().addBoundGenerator(std::tr1::bind(
      &CompiledSpace::LightSceneWriter::boundBox, &lightWriter_));
    ...
    converter().addItemHandler(
      "terrain", &terrain2_, &CompiledSpace::Terrain2Writer::convertTerrain);
    converter().addItemHandler(
      "model",
      &staticModels_,
      &CompiledSpace::StaticSceneModelWriter::convertModel);
    converter().addItemHandler(
      "entity", &entities_, &CompiledSpace::EntityWriter::convertEntity);
    converter().addItemHandler(
      "UserDataObject", &entities_, &CompiledSpace::EntityWriter::convertUDO);
#if SPEEDTREE_SUPPORT
    converter().addItemHandler(
      "speedtree",
      &speedTrees_,
      &CompiledSpace::StaticSceneSpeedTreeWriter::convertSpeedTree);
#endif
    converter().addItemHandler(
      "flare",
      &staticFlares_,
      &CompiledSpace::StaticSceneFlareWriter::convertFlare);
    converter().addItemHandler(
      "shell",
      &staticModels_,
      &CompiledSpace::StaticSceneModelWriter::convertShell);
    converter().addItemHandler(
      "particles",
      &particles_,
      &CompiledSpace::ParticleSystemWriter::convertParticles);
    converter().addItemHandler(
      "staticDecal",
      &decals_,
      &CompiledSpace::StaticSceneDecalWriter::convertStaticDecal);
    converter().addItemHandler(
      "water", &water_, &CompiledSpace::StaticSceneWaterWriter::convertWater);
    ...
    converter().addItemHandler(
      "omniLight",
      &lightWriter_,
      &CompiledSpace::LightSceneWriter::convertOmniLight);
    converter().addItemHandler(
      "spotLight",
      &lightWriter_,
      &CompiledSpace::LightSceneWriter::convertSpotLight);
    converter().addItemHandler(
      "pulseLight",
      &lightWriter_,
      &CompiledSpace::LightSceneWriter::convertPulseLight);

    // Texture streaming handlers
    converter().addItemHandler(
      "speedtree",
      &textureStreamingWriter_,
