...
const uint64 TextureConverter::s_TypeId =
  Hash64::compute(TextureConverter::getTypeName());
                                          ...
                                          DependencyList&   dependencies)
{
    ...
    BW::string texformatFile =
      BWResource::changeExtension(sourcefile, ".texformat");
                               ...
                               BW::vector<BW::string>& outputFiles)
{
    ...
    BW::string baseName =
      Moo::removeTextureExtension(relativeSourcefile).to_string();
    for (uint32 i = 1; TextureHelpers::textureExts[i] != NULL; i++) {
        ...
        if (BWResource::fileExists(filename)) {
            ERROR_MSG("Could not convert texture %s. A higher priorty texture "
                      "was found %s.",
                      sourcefile.c_str(),
    ...
    BW::string textureDetailsName =
      textureDetailLevelManager_.getTextureDetailLevelsName();
