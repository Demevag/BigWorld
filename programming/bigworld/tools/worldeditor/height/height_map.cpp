...
namespace {
    ...
    {
        return Math::lerp(Math::clamp(MIN_HEIGHT, height, MAX_HEIGHT),
                          MIN_HEIGHT,
                          MAX_HEIGHT,
                          (uint16)0,
                          (uint16)65535);
    ...
    {
        return Math::lerp(
          theight, (uint16)0, (uint16)65535, MIN_HEIGHT, MAX_HEIGHT);
    }
    ...
    bool rectanglesIntersect(int32  left1,
                             int32  top1,
                             int32  right1,
                             int32  bottom1,
                             int32  left2,
 ...
 */
void HeightMap::spaceInformation(BW::string const& spaceName,
                                 int32             localToWorldX,
                                 int32             localToWorldY,
                                 uint16            width,
                                 uint16            height,
...
)
{
    ...
    const TerrainUtils::TerrainFormat& ti =
      WorldManager::instance().getTerrainInfo();
 ...
 */
void HeightMap::drawImportedTerrain(float               left,
                                    float               top,
                                    float               right,
                                    float               bottom,
                                    ...
                                    ImportImage const&  image)
{
    ...
    restoreSubimage(
      impTerrLeft_, impTerrTop_, impTerrRight_, impTerrBottom_, impTerrSubImg_);
...
void HeightMap::dirtyThumbnail(int32 gridX, int32 gridY, bool justLoaded)
{
    ...
    Chunk* chunk = ChunkManager::instance().findChunkByGrid(
      gridX, gridY, WorldManager::instance().geometryMapping());
    ...
    for (BW::list<GridPos>::iterator it = undrawnChunks_.begin();
...
float HeightMap::heightAtGridPos(Vector2 const& pos) const
{
    ...
    if (pos.x < 0 || pos.x >= (float)gridWidth_ || pos.y < 0 ||
        pos.y >= (float)gridHeight_ || !hasTexture()) {
 ...
 */
void HeightMap::heightRange(Vector2 const& pos1,
                            Vector2 const& pos2,
                            float&         minv,
                            float&         maxv) const
{
    ...
    try {
        if (showProgress &&
            ...
            genHMTask = new ProgressBarTask(
              LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/HEIGHT/HEIGHT_MAP/"
                           L"GENERATING_TEXTURE"),
...
void HeightMap::doNotSaveHeightMap()
{
    ...
    BW::string filename = BWResource::resolveFilename(cachedHeightMapName());
 ...
 */
void HeightMap::extractSubimage(float               leftf,
                                float               topf,
                                float               rightf,
                                float               bottomf,
                                Moo::Image<uint16>& subImage) const
{
    BW_GUARD;
 ...
 */
void HeightMap::restoreSubimage(float                     leftf,
                                float                     topf,
                                float                     rightf,
                                float                     bottomf,
 ...
 */
void HeightMap::addTerrain(float               leftf,
                           float               topf,
                           float               rightf,
                           float               bottomf,
                           ElevationBlit::Mode mode,
...
bool HeightMap::chunkToHeightMap(int32 gx, int32 gy, bool force)
{
    ...
    uint32 dx1 =
      Math::lerp(gx, (int32)0, (int32)gridWidth_, (uint32)0U, twidth - 1U);
    uint32 dy1 =
      Math::lerp(gy, (int32)0, (int32)gridHeight_, (uint32)0U, theight - 1U);
    uint32 dx2 =
      Math::lerp(gx + 1, (int32)0, (int32)gridWidth_, (uint32)0U, twidth - 1U);
    uint32 dy2 = Math::lerp(
      gy + 1, (int32)0, (int32)gridHeight_, (uint32)0U, theight - 1U);
    ...
    if (!terrainImage.isEmpty()) {
        ...
        for (uint32 dy = dy1; dy <= dy2; ++dy) {
            ...
            for (uint32 dx = dx1; dx <= dx2; ++dx, ++p) {
                float sx =
                  Math::lerp(dx, dx1, dx2, 0.0f, (float)numPolesY_ - my);
 ...
 */
bool HeightMap::toTexCoords(float  leftf,
                            float  topf,
                            float  rightf,
                            float  bottomf,
                            int32& texLeft,
...
) const
{
    ...
    // overlap:
    bool result = rectanglesIntersect(texLeft,
                                      texTop,
                                      texRight,
                                      texBottom,
                                      0,
 ...
 */
void HeightMap::blitAdditive(ImportImage const& image,
                             int32              left,
                             int32              top,
                             int32              right,
                             int32              bottom,
                             ...
                             float              offset)
{
    ...
    for (int32 y = bottom; y <= top; ++y) {
        uint16* p = bits + y * pitch + left;
        int32   sy =
          (int32)Math::lerp<int64, int64>(y, top, bottom, itop, ibottom);
        const uint16* row = image.getRow(sy);
        for (int32 x = left; x <= right; ++x, ++p) {
            int32 sx =
              (int32)Math::lerp<int64, int64>(x, left, right, ileft, iright);
 ...
 *  @param offset       The offset to apply to the imported terrain.
 */
void HeightMap::blitSubtractive(ImportImage const& image,
                                int32              left,
                                int32              top,
                                int32              right,
                                int32              bottom,
 ...
 *  @param offset       The offset to apply to the imported terrain.
 */
void HeightMap::blitMin(ImportImage const& image,
                        int32              left,
                        int32              top,
                        int32              right,
                        int32              bottom,
 ...
 *  @param offset       The offset to apply to the imported terrain.
 */
void HeightMap::blitMax(ImportImage const& image,
                        int32              left,
                        int32              top,
                        int32              right,
                        int32              bottom,
 ...
 *  @param offset       The offset to apply to the imported terrain.
 */
void HeightMap::blitReplace(ImportImage const& image,
                            int32              left,
                            int32              top,
                            int32              right,
                            int32              bottom,
...
bool HeightMap::loadCachedHeightMap()
{
    ...
    BW::string textureName = BWResource::resolveFilename(cachedHeightMapName());
    FILE*      file        = NULL;
    try {
        file =
          BWResource::instance().fileSystem()->posixFileOpen(textureName, "rb");
        if (file == NULL)
            return false;
        ...
        long height = width;
        if (isPS2_) {
            texture_           = Moo::rc().createTexture(width,
                                               height,
                                               1, // no mipmap levels
                                               0, // usage
                                               D3DFMT_L16,
                                               D3DPOOL_MANAGED);
            textureSubstitute_ = Moo::rc().createTexture(
              width, height, 1, 0, D3DFMT_L16, D3DPOOL_SYSTEMMEM);
        } else {
...
void HeightMap::redrawAllChunks()
{
    ...
    if (isPS2_) {
        texture_ = Moo::rc().createTexture(recommendSize_,
                                           recommendSize_,
                                           1, // no mipmap levels
                                           0, // usage
                                           D3DFMT_L16,
        ...
        MF_ASSERT(texture_);
        textureSubstitute_ = Moo::rc().createTexture(
          recommendSize_, recommendSize_, 1, 0, D3DFMT_L16, D3DPOOL_SYSTEMMEM);
...
void HeightMap::ensureValidTexture() const
{
    ...
    if (!texture_) {
        ...
        if (isEightBitTex_) {
            texture_ = Moo::rc().createTexture(heightMap_.width(),
                                               heightMap_.height(),
                                               1, // no mipmap levels
                                               0, // usage
                                               D3DFMT_L8,
    ...
    if (isEightBitTex_) {
        ...
        if (minh == maxh) {
            ...
            for (uint32 y = 0; y < heightMap_.height(); ++y) {
                ...
                for (uint32 x = 0; x < heightMap_.width(); ++x) {
                    *dest =
                      Math::clamp(0, Math::lerp(*src, minh, maxh, 0, 255), 255);
                    ++dest;
                    ++src;
                }
            }
 ...
 *	@param readOnly		If this texture is only used for read purpose in this
 *access
 */
void HeightMap::beginTexture(uint16** base, uint32* pitch, bool readOnly) const
{
    ...
    if (isPS2_) {
        ...
        *base = const_cast<uint16*>(
          heightMap_.getRow(0)); // TODO fix this const_cast<>, see BWT-26710
