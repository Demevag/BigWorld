...
namespace {
    ...
    template <class V>
    void vertQuad(CustomMesh<V>& mesh,
                  float          x,
                  float          hx,
                  DWORD          colour,
                  bool           asLines = false)
    ...
    template <class V>
    void horzQuad(CustomMesh<V>& mesh,
                  float          y,
                  float          hy,
                  DWORD          colour,
                  bool           asLines = false)
...
void HeightModule::onStart()
{
    ...
    if (lastSpace != space) {
        ...
        maskTopLeft_ = Options::getOptionVector2(
          "render/height/maskTopLeft",
          Vector2(0.0f, static_cast<float>(gridHeight_)));
        maskBottomRight_ = Options::getOptionVector2(
          "render/height/maskBottomRight",
          Vector2(static_cast<float>(gridWidth_), 0.0f));

        terrainTopLeft_ = Options::getOptionVector2(
          "render/height/selTopLeft",
          Vector2(0.0f, static_cast<float>(gridHeight_)));
        terrainBottomRight_ = Options::getOptionVector2(
    ...
    HeightMap::instance().spaceInformation(
      space, localToWorld_.x, localToWorld_.y, gridWidth_, gridHeight_);
...
int HeightModule::onStop()
{
    ...
    Options::setOptionVector2("render/height/selTopLeft", terrainTopLeft_);
    Options::setOptionVector2("render/height/selBottomRight",
                              terrainBottomRight_);

    Options::setOptionVector2("render/height/maskTopLeft", maskTopLeft_);
    Options::setOptionVector2("render/height/maskBottomRight",
                              maskBottomRight_);
...
void HeightModule::render(float dTime)
{
    ...
    if (HeightMap::instance().aborted()) {
        ...
        abortedText_->update(
          dTime, Moo::rc().screenWidth(), Moo::rc().screenHeight());
    ...
    selectionShader_->beginPass(0);
    {
        ...
        Vector2 br =
          (mode_ == IMPORT_MASK) ? maskBottomRight_ : terrainBottomRight_;
...
bool HeightModule::handleKeyEvent(const KeyEvent& keyEvent)
{
    ...
    if (keyEvent.key() == KeyCode::KEY_LEFTMOUSE && keyEvent.isKeyDown() &&
        (mode_ == IMPORT_TERRAIN || mode_ == EXPORT_TERRAIN) &&
        (shouldGetMaxHeight() || shouldGetMinHeight())) {
    ...
    if (keyEvent.key() == KeyCode::KEY_1 && keyEvent.isKeyDown() &&
        !keyEvent.modifiers()) {
        PanelManager::instance().setToolMode(L"Objects");
    }
    if (keyEvent.key() == KeyCode::KEY_2 && keyEvent.isKeyDown() &&
        !keyEvent.modifiers()) {
        PanelManager::instance().setToolMode(L"TerrainTexture");
    }
    if (keyEvent.key() == KeyCode::KEY_3 && keyEvent.isKeyDown() &&
        !keyEvent.modifiers()) {
        PanelManager::instance().setToolMode(L"TerrainHeight");
    }
    if (keyEvent.key() == KeyCode::KEY_4 && keyEvent.isKeyDown() &&
        !keyEvent.modifiers()) {
        PanelManager::instance().setToolMode(L"TerrainFilter");
    }
    if (keyEvent.key() == KeyCode::KEY_5 && keyEvent.isKeyDown() &&
        !keyEvent.modifiers()) {
        PanelManager::instance().setToolMode(L"TerrainMesh");
    }
    if (keyEvent.key() == KeyCode::KEY_7 && keyEvent.isKeyDown() &&
        !keyEvent.modifiers()) {
        PanelManager::instance().setToolMode(L"Project");
    }
    if (keyEvent.key() == KeyCode::KEY_8 && keyEvent.isKeyDown() &&
        !keyEvent.modifiers()) {
        PanelManager::instance().setToolMode(L"TerrainRuler");
    }
    if (keyEvent.key() == KeyCode::KEY_9 && keyEvent.isKeyDown() &&
        !keyEvent.modifiers()) {
...
void HeightModule::gotoWorldPos(const KeyEvent& /*keyEvent*/)
{
    ...
    if (pModule != NULL) {
        ...
        if (pScriptObject != NULL) {
            Script::call(PyObject_GetAttrString(pScriptObject, "changeToMode"),
...
bool HeightModule::handleMouseEvent(const MouseEvent& mouseEvent)
{
    ...
    if ((mouseEvent.dx() != 0 || mouseEvent.dy() != 0) &&
        ...
        mouseDownOn3DWindow_ && !gettingHeight_) {
...
{
    viewPosition_.z +=
      (mouseEvent.dz() > 0) ? -minViewZ_ / 25.0f : minViewZ_ / 25.0f;
...
void HeightModule::handleTerrainEdit(const MouseEvent& /*mouseEvent*/)
{
    ...
    switch (terrainDir_) {
        ...
        case TERRAIN_MIDDLE: {
            ...
            if (fabs(snapTL.x - tl.x) <= fabs(snapBR.x - br.x)) {
            ...
            if (fabs(snapTL.y - tl.y) <= fabs(snapBR.y - br.y)) {
...
void HeightModule::importData(ImportImagePtr image,
                              float*         left /* = NULL */,
                              float*         top /* = NULL */,
                              float*         right /* = NULL */,
                              float*         bottom /* = NULL */
)
{
    ...
    if (tl.x < 0 || br.x > gridWidth_ || tl.y > gridHeight_ || br.y < 0) {
        ...
        if (w <= gridWidth_ && h <= gridHeight_) {
            WARNING_MSG("Imported terrain outside space, it was moved to the "
                        "top-left.\n");
        ...
        // The imported area doesn't fit, import into the entire space:
            ...
            WARNING_MSG("Imported terrain does not fit into space, it was "
                        "rescaled to fit.\n");
            tl = Vector2(0.0f, static_cast<float>(gridHeight_));
...
void HeightModule::terrainImportOptions(ElevationBlit::Mode mode,
                                        float               minV,
                                        float               maxV,
                                        float               strength,
                                        bool                absolute)
...
void HeightModule::textureImportOptions(BW::string const& texture,
...
size_t HeightModule::terrainUndoSize() const
{
    ...
    return ElevationBlit::importUndoSize(terrainTopLeft_.x,
                                         terrainTopLeft_.y,
                                         terrainBottomRight_.x,
                                         terrainBottomRight_.y);
}

bool HeightModule::doTerrainImport(bool doUndo,
                                   bool showProgress,
                                   bool forceToMem)
{
    BW_GUARD;
    ...
    if (importImage() != NULL) {
        bool result = true;
        if (doUndo) {
            result = ElevationBlit::saveUndoForImport(
              terrainTopLeft_.x,
              terrainTopLeft_.y,
              terrainBottomRight_.x,
              terrainBottomRight_.y,
        ...
        if (result) {
            ...
            result = ElevationBlit::import(*importImage(),
                                           terrainTopLeft_.x,
                                           terrainTopLeft_.y,
                                           terrainBottomRight_.x,
                                           terrainBottomRight_.y,
...
bool HeightModule::doTerrainExport(char const* filename, bool showProgress)
{
    BW_GUARD;
    ...
    if (absoluteHeights_) {
        ...
        ElevationBlit::heightRange(terrainTopLeft_.x,
                                   terrainTopLeft_.y,
                                   terrainBottomRight_.x,
                                   terrainBottomRight_.y,
                                   minHeight,
...
void HeightModule::onQuickStart()
{
    ...
    HeightMap::instance().spaceInformation(
      space, localToWorld_.x, localToWorld_.y, gridWidth_, gridHeight_, false);
}
...
void HeightModule::writeStatus()
{
    ...
    if (!dirtyChunks.empty()) {
        WorldManager::instance().setStatusMessage(
          5,
          LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/HEIGHT/HEIGHT_MODULE/"
...
{
    return Vector2(pos.x / gridSize_ - float(localToWorld_.x),
...
HeightModule::TerrainDir HeightModule::pointToDir(POINT pt) const
{
    ...
    if (fabsf(left - worldPos.x) < 3 * scaleX_ &&
        fabsf(top - worldPos.y) < 3 * scaleY_) {
        return TERRAIN_NORTH_WEST;
    }
    if (fabsf(right - worldPos.x) < 3 * scaleX_ &&
        fabsf(top - worldPos.y) < 3 * scaleY_) {
    ...
    if (left <= worldPos.x && worldPos.x <= right &&
        fabsf(top - worldPos.y) < 3 * scaleY_) {
        return TERRAIN_NORTH;
    }
    if (bottom <= worldPos.y && worldPos.y <= top &&
    ...
    if (left <= worldPos.x && worldPos.x <= right && bottom <= worldPos.y &&
        worldPos.y <= top) {
...
void HeightModule::drawRect(Vector2 const& ul,
                            Vector2 const& br,
                            uint32         colour,
                            int            ht)
{
...
void HeightModule::drawRect(Vector2 const& ul, Vector2 const& br, uint32 colour)
{
    BW_GUARD;
...
void HeightModule::drawHandle(Vector2 const& v,
                              uint32         colour,
                              int            hx,
                              int            hy) const
{
...
void HeightModule::redrawImportedTerrain()
{
    ...
    if (importImage() != NULL) {
        if (mode_ == IMPORT_TERRAIN) {
            ...
            HeightMap::instance().drawImportedTerrain(terrainTopLeft_.x,
                                                      terrainTopLeft_.y,
                                                      terrainBottomRight_.x,
                                                      terrainBottomRight_.y,
                                                      impMode_,
...
void HeightModule::heightQuery()
{
    ...
    HeightMap::instance().heightRange(
      heightDownPt_, heightCurrentPt_, minv, maxv);
...
void HeightModule::updateCursor()
{
    ...
    if ((mode_ == IMPORT_TERRAIN || mode_ == EXPORT_TERRAIN) &&
        (shouldGetMaxHeight() || shouldGetMinHeight())) {
        HCURSOR cursor =
          ::AfxGetApp()->LoadCursor(MAKEINTRESOURCE(IDC_HEIGHTPICKER));
        SetCursor(cursor);
    }
...
void HeightModule::validateWorldRect()
{
    ...
    terrainBottomRight_.x =
      max(0.f, min(terrainBottomRight_.x, (float)gridWidth_));
    terrainBottomRight_.y =
      max(0.f, min(terrainBottomRight_.y, (float)gridHeight_));
}
