...
class HeightModule : public FrameworkModule
{
  public:
     ...
     * **/
    void                  importData(ImportImagePtr image,
                                     float*         left   = NULL,
                                     float*         top    = NULL,
                                     float*         right  = NULL,
                                     float*         bottom = NULL);
    ...
    void terrainImportOptions(ElevationBlit::Mode mode,
                              float               minV,
                              float               maxV,
                              float               strength,
                              bool                absolute);
    ...
    void         textureImportOptions(BW::string const& texture,
  ...
  protected:
    ...
    /** Validate the world rectangle to ensure the selection never leaves the
     * world bounds */
  ...
  private:
    ...
    Vector3 viewPosition_; /** Camera position, X & Y specify position, Z
                              specifies zoom */
    float minViewZ_;
    float savedFarPlane_; /** Saved far-plane, allows restoration when returning
                             to main editing */
    float        gridSize_;
    int          minX_;
    int          minY_;
    unsigned int gridWidth_; /** Extent of the grid, in number of chunks. It
                                starts at 0,0 */
    unsigned int gridHeight_;
    GridCoord
      localToWorld_; /** Translation from local to world grid coordinates */
    ...
    Moo::Image<uint16>
      subTexture_; /** Sub texture of height map when importing and editing */
    ...
    Moo::EffectMaterialPtr terrainShader_; /** Shader to draw terrain and blend
                                              with hand drawn map */
    Moo::EffectMaterialPtr
      selectionShader_;                /** Shader to draw selection rectangle */
    ...
    static HeightModule*
      s_currentInstance_; /** The last created HeightModule */
    static Vector2
      s_terrainTopLeft_; /** Top-left of terrain in world coordinates */
    static Vector2
      s_terrainBottomRight_; /** Bottom-right of terrain in world coordinates */
    static Vector2 s_maskTopLeft_; /** Top-left of mask in world coordinates */
    static Vector2
      s_maskBottomRight_; /** Bottom-right of mask in world coordinates */
};
