...
WorldEditorCamera::WorldEditorCamera()
{
...

    {
        ...
        mlc->turboSpeed(Options::getOptionFloat(
          "camera/speed/" + speedName + "/turbo", 120.0f));
...
void WorldEditorCamera::changeToCamera(CameraType type)
{
    ...
    if (type == CT_Orthographic) {
        ...
        Matrix newM;
        newM.lookAt(
          currentPosition, Vector3(0.f, -1.f, 0.f), Vector3(0.f, 0.f, 1.f));
        // newM.invert();
...
void WorldEditorCamera::zoomExtents(ChunkItemPtr item, float sizeFactor)
{
    ...
    const Vector3 worldPosition =
      chunk->transform().applyPoint(item->edTransform().applyToOrigin());
