...
void FencesToolView::render(Moo::DrawContext& drawContext, const Tool& tool)
{
    ...
    if ((drawContext.collectChannelMask() &
         Moo::DrawContext::OPAQUE_CHANNEL_MASK) == 0) {
...
/**
 * Calculates nextSectionMatrix_ and update flags, according to the current
 * cursor position.
 */
void FencesToolView::calcNextSectionPlacement()
{
    ...
    if ((distToN <= distToP && prevSectionAttachmentEdge_ == 0) ||
        ...
        Vector3 attachDir = this->isFixedDirection()
                              ? fixedAttachmentDirection_
                              : cursorPos_ - (ptNW + offset);
        attachDir.y       = 0;
        ...
        if (InputDevices::instance().isShiftDown()) {
            ...
            nextSectionPosWorld =
              ptPW + attachDir * (fenceLenW + sectionStep) + offset;
            nextSectionPosWorld.y = cursorPos_.y;
        }
    ...
    mxRotate.setRotateY(-prevSectionMatrix.yaw());
    nextSectionOffset_ =
      mxRotate.applyPoint(Vector3(nextSectionStart - attachmentPoint));
...
/**
 * Breaks current sequence, so the next section will be placed independently,
 * starting new fence
 */
...
void FencesToolView::setCursorPos(const Vector3& pos)
{
    ...
    if (InputDevices::isKeyDown(KeyCode::KEY_LEFTMOUSE)) {
        if (posForDragChecking_.x == 0 && posForDragChecking_.z == 0) {
            posForDragChecking_ = cursorPos_;
        }
        if ((posForDragChecking_ - cursorPos_).lengthSquared() >=
            DRAG_DISTANCE) {
            draggingMode_ = true;
        }
    } else {
...
/**
 * Returns true if current cursor position and pressed button allows to place
 * next fence section. Uses flags updated in calcNextSectionPlacement()
 */
...
Matrix FencesToolView::addSectionToSequence()
{
    ...
    // Lock from placing new fences automatically unless Left-mouse key is
    // released or draggingMode is activated. This can prevent placing fences by
    // mistake.
    if (!draggingMode_) {
        this->lock(true);
    }
...
bool FencesToolView::groundPoint(Vector3& pnt, ChunkSpacePtr pSpace)
{
    ...
    pSpace->collide(pnt + Vector3(0, 100, 0), pnt - Vector3(0, 200, 0), cb);
    if (cb.hasCollided()) {
...
void FencesToolStateOperation::undo()
{
    ...
    size_t newSize =
      it == EditorChunkModel::s_fences.end() ? 0 : it->second.size();
...
GhostModelFashion::GhostModelFashion(SuperModel* pSuperModel)
{
    ...
    hWVPParam_ = replacementMaterial_->baseMaterial()
                   ->pEffect()
                   ->pEffect()
                   ->GetParameterByName(NULL, "world");
    hColorParam_ = replacementMaterial_->baseMaterial()
                     ->pEffect()
                     ->pEffect()
                     ->GetParameterByName(NULL, "color");
}
...
void GhostModelFashion::dress(SuperModel& model)
{
    ...
    for (Bindings::iterator it = bindings_.begin(); it != bindings_.end();
         it++) {
    ...
    Matrix              world =
      WorldManager::instance().getFenceToolView()->ghostModelMatrix_;
    replacementMaterial_->baseMaterial()->pEffect()->pEffect()->SetValue(
      hWVPParam_, &world, sizeof(D3DXMATRIX));

    float     sin = 0.7f + 0.3f * sinf(3.0f * (float)((double)timestamp() /
                                                  (double)stampsPerSecond()));
    D3DXCOLOR color(sin, sin, sin, sin);
    replacementMaterial_->baseMaterial()->pEffect()->pEffect()->SetValue(
      hColorParam_, &color, sizeof(D3DXCOLOR));
}
