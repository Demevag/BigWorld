...
namespace Graph {
     ...
     */
    void Graph::updateEdge(EdgePtr&       edge,
                           const NodePtr& start,
                           const NodePtr& end)
    {
        BW_GUARD;
    ...
    /**
     *	This method returns all the nodes that have edges from the specified
     *node.
     *
     *	@param start	Node used as the start of edges to find adjacent nodes.
     *	@param retNodes	Return param, nodes that are then end of edges from
     *start.
     */
    void Graph::adjacentNodes(const NodePtr& start, NodesSet& retNodes) const
    {
        ...
        for (EdgesMap::const_iterator it = range.first; it != range.second;
             ++it) {
     ...
     *	@param retNodes	Return param, nodes that are then start of edges from
     *end.
     */
    ...
    void Graph::removeEdgeFromMap(const EdgePtr& edge, EdgesMap& edgesMap)
    {
        ...
        for (int i = 0; i < 2; ++i) {
            std::pair<EdgesMap::iterator, EdgesMap::iterator> range =
              edgesMap.equal_range(nodes[i]);

            for (EdgesMap::iterator it = range.first; it != range.second;
                 ++it) {
