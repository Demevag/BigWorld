...
namespace {
     ...
     */
    bool nodeZOrderComparer(const Graph::NodeViewPtr& view1,
                            const Graph::NodeViewPtr& view2)
    {
...
namespace Graph {
    ...
    void GraphView::init(GraphPtr newGraph, COLORREF bkColour)
    {
        ...
        // Create the "Clone" cursor by copying the standard cursor used for
        // dragging and adding a little "[+]" at the bottom-right corner.
        if (!cloneCursor_) {
            cloneCursor_ =
              controls::addPlusSignToCursor(LoadCursor(NULL, IDC_SIZEALL));
            if (!cloneCursor_) {
                ERROR_MSG(
                  "GraphView::init: Could not create custom 'Clone' "
    ...
    void GraphView::update()
    {
        ...
        if (CWnd::WindowFromPoint(srcPoint) == this || GetCapture() == this) {
            ...
            if (isMouseDown) {
                if (btnDownNode_) {
                    if (!dragging_ && abs(btnDownPt_.x - point.x) > 5 ||
            ...
            if (prevHoverNode != hoverNode_ || dragging_ || panning_) {
                RedrawWindow();
            }
    ...
    /**
     *	This MFC method is called when the control is repainted, and draws all
     *the views in the view.
     */
    void GraphView::OnPaint()
    {
        ...
        if (!footerText_.empty() && footerFont_.GetLogFont(&logFont)) {
            ...
            memDC_.DrawText(
              footerText_.c_str(), &textRect, DT_CALCRECT | DT_WORDBREAK);
        ...
        if (graph_) {
            ...
            for (NodesSet::const_iterator itNode = nodes.begin();
                 itNode != nodes.end();
                 ++itNode) {
                NodeViewMap::const_iterator itView =
                  nodeViews_.find((*itNode).get());
            ...
            if (needsSort) {
                std::sort(
                  visibleNodes.begin(), visibleNodes.end(), nodeZOrderComparer);
            }
            ...
            if (isTransparent) {
                ...
                alphaBmp.CreateCompatibleBitmap(
                  &paintDC, fullExtents.Width(), fullExtents.Height());
            ...
            for (BW::vector<NodeViewPtr>::iterator itView =
                   visibleNodes.begin();
                 itView != visibleNodes.end();
                 ++itView) {
                NodeView::STATE state = ((*itView) == selectedNode_)
                                          ? NodeView::SELECTED
                                          : NodeView::NORMAL;
                ...
                if ((*itView)->alpha() != 255) {
                    // draw the node transparent (useful for drag & dropping for
                    // example)
     ...
     *  @return			TRUE - the background is erased - which is not needed
     *for this control.
     */
     ...
     *	@return		TRUE to prevent further processing, FALSE to relay to the
     *parent.
     */
     ...
     *
     *  @param point	Current mouse position.
     *  @param acceptDrag	Whether or not this button press could result in a
