    ...
    if (material_->begin()) {
        for (uint32 i = 0; i < material_->numPasses(); i++) {
            ...
            while (it != end) {
                ...
                biasGrid(
                  info_.localToWorld_, di.gridX_, di.gridZ_, biasedX, biasedZ);

                Vector2 offSet(1.f / Moo::rc().screenWidth(),
                               1.f / Moo::rc().screenHeight());
                ...
                static CustomMesh<Moo::VertexXYZL> s_gridSquare(
                  D3DPT_TRIANGLEFAN);
