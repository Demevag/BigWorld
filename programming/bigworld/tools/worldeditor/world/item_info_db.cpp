...
namespace {
    ...
    class ItemInfoDBEditor : public GeneralEditor
    {
      public:
        ...
        virtual bool useFullDateFormat() { return true; }
                    ...
                    if (valueType.toString(pVal, strVal)) {
                        ...
                        if (addingAssetMetadata_) {
                            ...
                            static BW::string s_assetPrefix =
                              LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/"
                                           L"SCENE_BROWSER/ASSET_MD_PREFIX") +
                              " ";
                            name = s_assetPrefix + pProp->name().c_str();
                        } else {
                            name = pProp->name().c_str();
                        }
                        dbProperties_.push_back(new ItemInfoDB::PropertyPair(
                          ItemInfoDB::Type(name, valueType), strVal));
                    }
    ...
    class AssetNameComparer : public ItemInfoDB::ComparerHelper
    {
      public:
        ...
        {
            int cmp =
              strcmpi(pItem1->assetName().c_str(), pItem2->assetName().c_str());
            if (cmp == 0) {
      ...
      public:
                    ...
                    const ItemInfoDB::ItemPtr& pItem2) const
        {
            ...
            if (cmp == 0) {
                ...
                cmp = strcmpi(pItem1->filePath().c_str(),
                              pItem2->filePath().c_str());
            }
            return cmp;
        }
    };
    ...
    class FilePathComparer : public ItemInfoDB::ComparerHelper
    {
      public:
        int compare(const ItemInfoDB::ItemPtr& pItem1,
                    const ItemInfoDB::ItemPtr& pItem2) const
        {
            return strcmpi(pItem1->filePath().c_str(),
                           pItem2->filePath().c_str());
        }
    };
    ...
    class ItemIntComparer : public ItemInfoDB::ComparerHelper
    {
      public:
        ...
        ItemIntComparer(const ItemInfoDB::Type& type)
          : type_(type)
        {
            if (type_ == ItemInfoDB::Type::builtinType(
                           ItemInfoDB::Type::TYPEID_NUMTRIS)) {
                intType_ = NUMTRIS;
            } else if (type_ == ItemInfoDB::Type::builtinType(
    ...
    {
      public:
        ItemFloatComparer(const ItemInfoDB::Type& type)
          : type_(type)
        {
        }
    ...
    {
      public:
        ItemStringComparer(const ItemInfoDB::Type& type)
          : type_(type)
        {
        }
    ...
    class DateStringComparer : public ItemInfoDB::ComparerHelper
    {
      ...
      public:
        DateStringComparer(const ItemInfoDB::Type& type)
          : type_(type)
        {
        }
                    ...
                    const ItemInfoDB::ItemPtr& pItem2) const
        {
            ...
            return int(cachedItemDateIndex(pItem2) -
                       cachedItemDateIndex(pItem1));
        }
        ...
        uint64 cachedItemDateIndex(const ItemInfoDB::ItemPtr& pItem) const
        {
            ...
            if (it != itemDateMap_.end()) {
                ...
                if (DateTimeUtils::parse(itemTime, date)) {
                    ...
                    static const uint64 SECONDS_PER_HOUR =
                      60 * SECONDS_PER_MINUTE;
                    static const uint64 SECONDS_PER_DAY = 24 * SECONDS_PER_HOUR;
                    static const uint64 SECONDS_PER_MONTH =
                      31 * SECONDS_PER_DAY;
                    static const uint64 SECONDS_PER_YEAR =
                      12 * SECONDS_PER_MONTH;

                    dateIndex = itemTime.year * SECONDS_PER_YEAR +
                    ...
                    itemDateMap_.insert(std::make_pair(pItem.get(), dateIndex))
                      .first;
                }
            }
            return dateIndex;
 ...
 */
ItemInfoDB::Type::Type(const BW::string& name, const ValueType& valueType)
  : name_(name)
 ...
 */
ItemInfoDB::Item::Item(const BW::string&     chunkId,
                       ChunkItem*            pChunkItem,
                       int                   numTris,
                       int                   numPrimitives,
                       const BW::string&     assetName,
                       const BW::string&     assetType,
  ...
  , onDeleteCallback_(NULL)
{
    ...
    onModifyCallback_ = new BWFunctor1<ItemInfoDB, EditorChunkItem*>(
      this, &ItemInfoDB::itemModified);
    EditorChunkItem::addOnModifyCallback(onModifyCallback_);

    onDeleteCallback_ = new BWFunctor1<ItemInfoDB, EditorChunkItem*>(
      this, &ItemInfoDB::itemDeleted);
    EditorChunkItem::addOnDeleteCallback(onDeleteCallback_);

    knownTypes_.insert(
      TypeUsagePair(Type::builtinType(Type::TYPEID_ASSETNAME), n));
    knownTypes_.insert(
      TypeUsagePair(Type::builtinType(Type::TYPEID_CHUNKID), n));
    knownTypes_.insert(
      TypeUsagePair(Type::builtinType(Type::TYPEID_NUMTRIS), n));
    knownTypes_.insert(
      TypeUsagePair(Type::builtinType(Type::TYPEID_NUMPRIMS), n));
    knownTypes_.insert(
      TypeUsagePair(Type::builtinType(Type::TYPEID_ASSETTYPE), n));
    knownTypes_.insert(
      TypeUsagePair(Type::builtinType(Type::TYPEID_FILEPATH), n));
    knownTypes_.insert(
      TypeUsagePair(Type::builtinType(Type::TYPEID_HIDDEN), n));
    knownTypes_.insert(
      TypeUsagePair(Type::builtinType(Type::TYPEID_FROZEN), n));
    knownTypes_.insert(
      TypeUsagePair(Type::builtinType(Type::TYPEID_TEAMID), n));
}
...
void ItemInfoDB::tick(int maxMillis)
{
    ...
    while (itChunkItem != pendingChunkItems_.end() && timestamp() < endTime) {
        ...
        ItemPtr pItem = new ItemInfoDB::Item(pChunkItem->chunk()->identifier(),
                                             pChunkItem,
                                             pChunkItem->edNumTriangles(),
                                             pChunkItem->edNumPrimitives(),
                                             pChunkItem->edAssetName(),
                                             pChunkItem->edClassName().c_str(),
                                             pChunkItem->edFilePath(),
        ...
        ChunkItemsMap::iterator itChunkItemMap = chunkItems_.find(pChunkItem);
 ...
 */
void ItemInfoDB::toss(ChunkItem* pChunkItem,
                      bool       tossingIn,
                      bool       ignoreExistence /*= true*/,
...
void ItemInfoDB::updateItemType(ItemPtr pItem, int usageAdd)
{
    ...
    for (ItemProperties::const_iterator itProp = pItem->properties().begin();
         itProp != pItem->properties().end();
         ++itProp) {
        updateType((*itProp)->first,
                   usageAdd,
 ...
 */
void ItemInfoDB::updateType(const Type&       type,
                            int               usageAdd,
