...
namespace {
    ...
    {
      public:
        SameUniqueId(UniqueID linkableId)
          : linkableId_(linkableId)
        {
        }
    ...
    {
      public:
        SameLink(EditorChunkItemLinkableManager::ULPair linkPair)
          : linkPair_(linkPair)
        {
        }
         ...
         *
         *	@return	True if the link is the same as the link stored by this
         *class instance.
         */
        bool operator()(
          const EditorChunkItemLinkableManager::ULPair& linkPair) const
        {
...
void EditorChunkItemLinkableManager::tick()
{
    ...
    //
    // 1)	Outstanding links: These are outgoing links from newly loaded
    // Linkers. 	If the target Linker is loaded, the link is removed from the
    // list and a 	standard link is created.  If it is not loaded, a chunk link
    // is created 	and the link is moved to the chunk links list.
    //
    ...
    //
    // 3)	Chunk links: These are the chunk links from loaded source Linker
    // objects 	to unloaded target Linker objects.  Target Linkers are checked
    // to see if 	they have been loaded.  If a target is loaded, the chunk
    // link is 	destroyed, a standard link is created, and the link is removed
    // from the 	chunk links list.
    //
    // 4)	Chunk back links: These are chunk back links from unloaded source
    // Linker 	objects to loaded target Linker objects.  Source Linkers are
    // checked to 	see if they have been loaded.  If a source is loaded, the
    // chunk back 	link is destroyed, a standard link is created (if needed),
    // and the link 	is removed from the chunk back links list.
    ...
    uint64 maxTime   = (stampsPerSecond() / 1000) *
                     Options::getOptionInt("render/links/maxProcessingMillis",
                                           MAX_LINK_PROCESSING_MILLIS);
    ...
    // Limiting the scope of the outstanding links list mutex
    {
        ...
        while (olIt != outstandingLinks_.end() &&
            ...
            MF_ASSERT(pLinkFromLinker != NULL &&
            ...
            if (pLinkToLinker) {
                ...
                Link   link(olIt->second.fromUID_,
                          olIt->second.fromCID_,
                          olIt->second.toUID_,
                          olIt->second.toCID_);
                ULPair linkPair(olIt->second.fromUID_, link);

                SameLink     linkEquality(linkPair);
                ULMultimapIt chunkLinkIt =
                  find_if(chunkLinks_.begin(), chunkLinks_.end(), linkEquality);
        ...
        while (olIt != outstandingBackLinks_.end() &&
            ...
            if (pLinkFromLinker) {
                ...
                ULMultimapIt chunkLinkIt = find_if(
                  chunkBackLinks_.begin(), chunkBackLinks_.end(), linkEquality);
    ...
    for (ulIt = chunkBackLinks_.begin(); ulIt != chunkBackLinks_.end() &&
                                         (timestamp() - startTime) < maxTime;) {
        // Check if source is registered
        EditorChunkItemLinkable* pLinkerFrom;
 ...
 */
void EditorChunkItemLinkableManager::tossRemove(
 ...
 */
void EditorChunkItemLinkableManager::idPreChange(const UniqueID& from,
                                                 const UniqueID& to)
{
    BW_GUARD;
...
void EditorChunkItemLinkableManager::deleted(EditorChunkItemLinkable* pLinkable)
{
    ...
    if (!pLinkable) {
        MF_ASSERT(
          0 && "EditorChunkItemLinkableManager::deleted : pLinkable is NULL!");
        return;
    }
    ...
    // Need to run through pLinkable's back links removing all references to
    // pLinkable from each.
    EditorChunkItemLinkable::Links backLinksCopy;
    backLinksCopy.insert(pLinkable->getBackLinksBegin(),
                         pLinkable->getBackLinksEnd());
 ...
 */
void EditorChunkItemLinkableManager::addLink(EditorChunkItemLinkable* start,
...
bool EditorChunkItemLinkableManager::addLinkInternal(
  EditorChunkItemLinkable*  start,
  EditorChunkItemLinkable*  end,
  const PropertyIndex&      propIdx,
  EditorChunkItemLinkable** old,
  bool                      updatePrevious /*= true*/)
{
    BW_GUARD;

    // Start and end should be registered prior to calling this method
    MF_ASSERT(start && end && isRegistered(start->guid()) &&
              isRegistered(end->guid()) &&
    ...
    if (previous) {
        ...
        if (previous->chunkItem()->chunk())
            WorldManager::instance().changedChunk(
              previous->chunkItem()->chunk(), InvalidateFlags::FLAG_NONE);
    }
  ...
  PropertyIndex            propIdx)
{
    ...
    if (pOldLinkable) {
        UndoRedo::instance().add(new LinkerUndoDeleteLinkOperation(
          pLinkable, pOldLinkable, data, propIdx));
    }
}
 ...
 *
 *	@param	pLinkable		The linker whos property is to be deleted.
 *	@param	pOldLinkable	The linker that the property points to, possibly
 *NULL.
 *	@param	propIdx		The id of the property to be deleted.
 */
  ...
  PropertyIndex            propIdx)
{
    ...
    MF_ASSERT(pLinkable && isRegistered(pLinkable->guid()) &&
    ...
    if (pLinkable->propHelper()->isUserDataObjectLink(propIdx.valueAt(0))) {
    ...
    } else if (pLinkable->propHelper()->isUserDataObjectLinkArray(
                 propIdx.valueAt(0))) {
        PyObjectPtr pValue(
        ...
        propArray.init(pLinkable->chunkItem(),
    ...
    if (pOldLinkable) {
        ...
        // Save the changes
        pOldLinkable->chunkItem()->edSave(
          pOldLinkable->chunkItem()->pOwnSect());
        if (pOldLinkable->chunkItem()->chunk())
            WorldManager::instance().changedChunk(
              pOldLinkable->chunkItem()->chunk(), InvalidateFlags::FLAG_NONE);
    }
}
...
void EditorChunkItemLinkableManager::deleteAllLinks(
  EditorChunkItemLinkable* pLinkable1,
  EditorChunkItemLinkable* pLinkable2)
{
    ...
    // method
    MF_ASSERT(isRegistered(pLinkable1->guid()) &&
              isRegistered(pLinkable2->guid()) &&
              "EditorChunkItemLinkableManager::deleteLink : "
  ...
  const BW::string& chunkId)
{
    ...
    if (linkableId == UniqueID() || chunkId == "")
        return 0;
    ...
    if (pLinkable == 0) {
        ...
        while (!(pLinkable = getRegistered(linkableId)) &&
        ...
        if (!pLinkable) {
            ERROR_MSG("EditorChunkItemLinkableManager::forceLoad : "
                      "Failed to load linked object %s from chunk %s.\n",
                      linkableId.toString().c_str(),
                      chunkId.c_str());
        }
    }
  ...
  EditorChunkItemLinkable* pLinkable)
{
    ...
    // Lock outstanding list mutex and limit scope
    {
        ...
        // Remove all outstanding back links to pLinkable in
        // outstandingBackLinks_
  ...
  GuidToGuidMap& linkerGuidMapping)
{
    ...
    if (linkerGuidMapping.size() == 0) {
        ERROR_MSG("EditorChunkItemLinkableManager::linkClonedLinkers : "
    ...
    // each
    for (it = linkerGuidMapping.begin(); it != linkerGuidMapping.end(); ++it) {
        // It should be registered
        EditorChunkItemLinkable* pClonedLinkable = getRegistered(it->second);
        MF_ASSERT(pClonedLinkable &&
  ...
  EditorChunkItemLinkable* pLinkable)
{
    ...
    MF_ASSERT(pLinkable && "EditorChunkItemLinkableManager::registerLinker : "
                           "pLinkable is NULL");
  ...
  EditorChunkItemLinkable* pLinkable)
{
    ...
    // The object should be registered prior to calling this method
    MF_ASSERT(pLinkable && "EditorChunkItemLinkableManager::unregisterLinker : "
                           "pLinkable is NULL");
    ...
    if (!isRegistered(pLinkable->guid())) {
        WorldManager::instance().addError(
          pLinkable->chunkItem()->chunk(),
          pLinkable->chunkItem(),
          "Linked object is using an GUID that is already registered, %s. "
  ...
  const UniqueID& linkableId)
{
    ...
    LinkerSetIt it;
    it = find_if(registeredLinkers_.begin(),
...
bool EditorChunkItemLinkableManager::isLinkValid(
  const EditorChunkItemLinkable* start,
  ...
  PyObjectPtr pPyLinkProp)
{
    ...
    BW::string linkableId =
      PyString_AsString(PyTuple_GetItem(pPyLinkProp.getObject(), 0));
    BW::string chunkId =
      PyString_AsString(PyTuple_GetItem(pPyLinkProp.getObject(), 1));
 ...
 *	@param	pPyLinkProp	The PyObject property.
 *	@return				Bool indicating whether the pPy property refer to
 *pLinkable
 */
bool EditorChunkItemLinkableManager::isLinkerReferencedInProp(
  const EditorChunkItemLinkable* pLinkable,
  ...
  EditorChunkItemLinkable* pLinkable2)
{
    ...
    MF_ASSERT(
      pLinkable1 && isRegistered(pLinkable1->guid()) && pLinkable2 &&
      isRegistered(pLinkable2->guid()) &&
    ...
    for (uint i = 0; i < 2; i++) {
        ...
        for (int j = 0; j < propCounter; j++) {
            ...
            if (pLinkables[i]->propHelper()->isUserDataObjectLink(j)) {
                PyObjectPtr pValue(pLinkables[i]->propHelper()->propGetPy(j),
                                   PyObjectPtr::STEAL_REFERENCE);

                // Need to obtain the unique ids and chunk ids of the linked
                // pLinkable
                BW::string uniqueId =
                  PyString_AsString(PyTuple_GetItem(pValue.getObject(), 0));
                if (uniqueId == pLinkables[(i + 1) % 2]->guid().toString()) {
                    BW::string chunkId =
                      PyString_AsString(PyTuple_GetItem(pValue.getObject(), 1));

                    if (isLinkValid(pLinkables[i % 2],
                                    pLinkables[(i + 1) % 2],
                                    chunkId)) {
            ...
            if (pLinkables[i]->propHelper()->isUserDataObjectLinkArray(j)) {
                ...
                ArrayPropertiesHelper propArray;
                propArray.init(pLinkables[i]->chunkItem(),
                               &(dataType->getElemType()),
                               pValue.getObject());

                // Iterate through the array of links
                for (int k = 0; k < propArray.propCount(); k++) {
                    PyObjectPtr link(PySequence_GetItem(pValue.getObject(), k),
                                     PyObjectPtr::STEAL_REFERENCE);
                    ...
                    if (uniqueId ==
                        pLinkables[(i + 1) % 2]->guid().toString()) {
  ...
  EditorChunkItemLinkable* pLinkable)
{
    ...
    for (int i = 0; i < propCounter; i++) {
        ...
        if (pLinkable->propHelper()->isUserDataObjectLink(i)) {
            PyObjectPtr ob(pLinkable->propHelper()->propGetPy(i),
                           PyObjectPtr::STEAL_REFERENCE);
            ...
            if (pLinkableBackLink) {
                ...
                pLinkableBackLink->chunkItem()->edSave(
                  pLinkableBackLink->chunkItem()->pOwnSect());
        ...
        // Is this property an array of object links
            ...
            for (int j = 0; j < propArray.propCount(); j++) {
                PyObjectPtr ob(propArray.propGetPy(j),
                               PyObjectPtr::STEAL_REFERENCE);
  ...
  EditorChunkItemLinkable* pLinkable)
{
    ...
    // Need to run through pLinkable's back links, updating pLinkable's chunkId
    // in all properties that point the pLinkable.
  ...
  EditorChunkItemLinkable* pLinkable)
{
    ...
    // method
    MF_ASSERT(isRegistered(pLinkableID) &&
    ...
    // Lock outstanding back list mutex and limit scope
    {
        ...
        // Update all outstanding back links to pLinkable in
        // outstandingBackLinks_
        for (it = outstandingBackLinks_.begin();
             it != outstandingBackLinks_.end();
             ++it) {
    ...
    // Lock chunk link mutex and limit scope
    {
        ...
        for (it = chunkLinks_.begin(); it != chunkLinks_.end(); ++it) {
            if (it->first == pLinkableID)
                it->second.fromCID_ = outsideChunkId;
        }
    }
    ...
    // Lock chunk back link mutex and limit scope
    {
        ...
        // Remove all chunk links from pLinkable in chunkLinks_
        for (it = chunkBackLinks_.begin(); it != chunkBackLinks_.end(); ++it) {
            if (it->second.toUID_ == pLinkableID)
                it->second.toCID_ = outsideChunkId;
        }
    }
  ...
  GuidToGuidMap&           linkerCloneGuidMapping)
{
    ...
    for (int i = 0; i < propCounter; i++) {
        DataDescription* pDD =
          pCloneLinkable->propHelper()->pType()->property(i);
        if (!pDD->editable())
            continue;

        // Is this property a object link
        if (pCloneLinkable->propHelper()->isUserDataObjectLink(i)) {
            PyObjectPtr ob(pCloneLinkable->propHelper()->propGetPy(i),
                           PyObjectPtr::STEAL_REFERENCE);
            ...
            if (propGuid == UniqueID()) {
                ...
                EditorChunkItemLinkable* pLinkableCloneLink =
                  getRegistered(propGuid);
                MF_ASSERT(
                  pLinkableCloneLink &&
        ...
        // Is this property an array of object links
            ...
            SequenceDataType* dataType =
              static_cast<SequenceDataType*>(pDD->dataType());
            ArrayPropertiesHelper propArray;
            propArray.init(pCloneLinkable->chunkItem(),
                           &(dataType->getElemType()),
                           ob.getObject());
            ...
            for (int j = 0; j < propArray.propCount(); j++) {
                ...
                if (propGuid == UniqueID()) {
                    ...
                    propIndex.append(j);
                    addLink(
                      pCloneLinkable, pLinkableCloneLink, propIndex, false);
                }
            }
        }
    }
  ...
  GuidToGuidMap&           linkerCloneGuidMapping)
{
    ...
    for (it = pCloneLinkable->getBackLinksBegin();
        ...
        // is contained in the domain of the map.  If it was not in the map,
        // delete the newly added GUID
        ...
        // As long as this is not the case add the new link and delete the
        // previous link.
  ...
  EditorChunkItemLinkable* chunkChangedLinker)
{
    ...
    if (!updatingLinker) {
        ERROR_MSG("EditorChunkItemLinkable::updateChunkIdsInProps : Cannot "
                  "update, updateLinker is NULL.\n");
        return;
    }
    ...
    for (int i = 0; i < propCounter; i++) {
        DataDescription* pDD =
          updatingLinker->propHelper()->pType()->property(i);
        if (!pDD->editable())
            continue;

        // Is this property a object link
        if (updatingLinker->propHelper()->isUserDataObjectLink(i)) {
            PyObjectPtr ob(updatingLinker->propHelper()->propGetPy(i),
                           PyObjectPtr::STEAL_REFERENCE);
        ...
        // Is this property an array of object links
            ...
            SequenceDataType* dataType =
              static_cast<SequenceDataType*>(pDD->dataType());
            ArrayPropertiesHelper propArray;
            propArray.init(updatingLinker->chunkItem(),
                           &(dataType->getElemType()),
                           ob.getObject());
            ...
            for (int j = 0; j < propArray.propCount(); j++) {
                ...
                if (isLinkerReferencedInProp(chunkChangedLinker, ob)) {
                    ...
                    propIndex.append(j);
                    addLinkInternal(
                      updatingLinker, chunkChangedLinker, propIndex);
                }
            }
        }
    }
    ...
    // Save changes
    updatingLinker->chunkItem()->edSave(
      updatingLinker->chunkItem()->pOwnSect());
    if (updatingLinker->chunkItem()->chunk() != NULL)
        WorldManager::instance().changedChunk(
          updatingLinker->chunkItem()->chunk(), InvalidateFlags::FLAG_NONE);
}
 ...
 */
void EditorChunkItemLinkableManager::addLink(const UniqueID&   linkFromUID,
                                             ...
                                             const BW::string& linkToCID)
{
    ...
    if (find_if(outstandingLinks_.begin(),
                outstandingLinks_.end(),
                linkEquality) == outstandingLinks_.end() &&
        find_if(chunkLinks_.begin(), chunkLinks_.end(), linkEquality) ==
          chunkLinks_.end())
  ...
  const BW::string& linkToCID)
{
    ...
    if (isRegistered(linkToUID)) {
        // Add the link to the list of outstanding back links if the link is not
        // in the chunk back links list or outstanding back links list already
        Link     link(linkFromUID, linkFromCID, linkToUID, linkToCID);
        ULPair   linkPair(linkFromUID, link);
        SameLink linkEquality(linkPair);
        ...
        if (find_if(outstandingBackLinks_.begin(),
                    outstandingBackLinks_.end(),
                    linkEquality) == outstandingBackLinks_.end() &&
            find_if(chunkBackLinks_.begin(),
                    chunkBackLinks_.end(),
                    linkEquality) == chunkBackLinks_.end())
  ...
  PyObjectPtr                    pPyLinkProp)
{
    ...
    addLink(pLinkable->guid(),
            pLinkable->getOutsideChunkId(),
            UniqueID(uniqueId),
            chunkId);
}
  ...
  PyObjectPtr                    pPyLinkProp)
{
    ...
    addLinkToOutstandingBackLinks(pLinkable->guid(),
                                  pLinkable->getOutsideChunkId(),
                                  UniqueID(uniqueId),
                                  chunkId);
}
  ...
  EditorChunkItemLinkable* pLinkable)
{
    ...
    MF_ASSERT(isRegistered(pLinkable->guid()) &&
              ...
              "addAllBackLinksToOutstandingBackLinksList : "
              "pLinkable is not registered!");

    // Iterate through pLinkable's back links adding each to the outstanding
    // back links list
    EditorChunkItemLinkable::LinksConstIter it;
    for (it = pLinkable->getBackLinksBegin();
...
void EditorChunkItemLinkableManager::removeAllRefs(
  EditorChunkItemLinkable* from,
  EditorChunkItemLinkable* pointingTo)
{
    BW_GUARD;

    MF_ASSERT(from && "EditorChunkItemLinkableManager::removeAllRefs : "
    ...
    for (int i = 0; i < propCounter; i++) {
        ...
        if (from->propHelper()->isUserDataObjectLink(i)) {
            PyObjectPtr ob(from->propHelper()->propGetPy(i),
                           PyObjectPtr::STEAL_REFERENCE);
        ...
        // Is this property an array of object links
            ...
            ArrayPropertiesHelper propArray;
            propArray.init(
              from->chunkItem(), &(dataType->getElemType()), ob.getObject());
...
/** Constructor */
EditorChunkItemLinkableManager::Link::Link(UniqueID   fromUID,
                                           BW::string fromCID,
                                           UniqueID   toUID,
                                           BW::string toCID)
  : fromUID_(fromUID)
...
{
    return (this->fromUID_ == rhs.fromUID_ && this->fromCID_ == rhs.fromCID_ &&
 ...
 */
static void udoCreateLink(const BW::string& guid1,
                          const BW::string& chunkId1,
                          const BW::string& guid2,
                          const BW::string& chunkId2,
                          const BW::string& propName)
{
    ...
    EditorChunkItemLinkable* startIL =
      WorldManager::instance().linkerManager().forceLoad(guid1, chunkId1);
    EditorChunkItemLinkable* endIL =
      WorldManager::instance().linkerManager().forceLoad(guid2, chunkId2);

    if (!startIL) {
        ERROR_MSG("Call to udoCreateLink failed: Couldn't find linked object "
                  "%s in chunk %s.\n",
                  guid1.c_str(),
                  chunkId1.c_str());
        return;
    ...
    if (!startIL->chunkItem()->isEditorUserDataObject()) {
        ERROR_MSG("Call to udoCreateLink failed: start item %s in chunk %s is "
                  "not a User Data Object.\n",
                  guid1.c_str(),
                  chunkId1.c_str());
        return;
    }
    ...
    if (!endIL->chunkItem()->isEditorUserDataObject()) {
        ERROR_MSG("Call to udoCreateLink failed: end item %s in chunk %s is "
                  "not a User Data Object.\n",
                  guid2.c_str(),
                  chunkId2.c_str());
        return;
    ...
    EditorChunkUserDataObject* start =
      static_cast<EditorChunkUserDataObject*>(startIL->chunkItem());
    EditorChunkUserDataObject* end =
      static_cast<EditorChunkUserDataObject*>(endIL->chunkItem());
    ...
    if (start->propHelper()->isUserDataObjectLinkArray(propIdx.valueAt(0))) {
        ...
        PyObjectPtr       ob(start->propHelper()->propGetPy(propIdx),
                       PyObjectPtr::STEAL_REFERENCE);
        SequenceDataType* dataType =
          static_cast<SequenceDataType*>(start->propHelper()
                                           ->pType()
                                           ->property(propIdx.valueAt(0))
                                           ->dataType());
        ArrayPropertiesHelper propArray;
    ...
    WorldManager::instance().linkerManager().addLink(
      start->chunkItemLinker(), end->chunkItemLinker(), propIdx);
}
 ...
 */
static PyObject* udoCreateAtPosition(const BW::string& guid,
                                     const BW::string& chunkId,
                                     const Vector3&    pos,
                                     bool              snapToTerrain)
{
    ...
    if (!pLinkable) {
        ERROR_MSG("Call to udoCreateAtPosition failed: Couldn't find linked "
                  "object %s in chunk %s.\n",
                  guid.c_str(),
                  chunkId.c_str());
        return NULL;
    ...
    if (!pLinkable->chunkItem()->isEditorUserDataObject()) {
        ERROR_MSG("Call to udoCreateAtPosition failed: %s in chunk %s is not a "
                  "User Data Object.\n",
                  guid.c_str(),
                  chunkId.c_str());
        return NULL;
    ...
    // copy it and clear its properties and guid (a new guid will be generated
    // on load)
    ...
    if (snapToTerrain) {
        ChunkTerrain* terrain =
          ChunkTerrainCache::instance(*newChunk).pTerrain();
        if (terrain != NULL)
 ...
 */
static void udoDeleteLinks(const BW::string& guid1,
                           const BW::string& chunkId1,
                           const BW::string& guid2,
                           const BW::string& chunkId2)
{
    ...
    EditorChunkItemLinkable* udo1 =
      WorldManager::instance().linkerManager().forceLoad(guid1, chunkId1);
    EditorChunkItemLinkable* udo2 =
      WorldManager::instance().linkerManager().forceLoad(guid2, chunkId2);

    if (!udo1) {
        ERROR_MSG("Call to udoDeleteLinks failed: Couldn't find linked object "
                  "%s in chunk %s.\n",
                  guid1.c_str(),
                  chunkId1.c_str());
        return;
...
static PyObject* udoGet(const BW::string& guid, const BW::string& chunkId)
{
    ...
    if (!pLinkable) {
        ERROR_MSG("Call to udoGet failed: Couldn't find linked object %s in "
                  "chunk %s.\n",
                  guid.c_str(),
                  chunkId.c_str());
        return NULL;
    ...
    return static_cast<EditorChunkUserDataObject*>(pLinkable->chunkItem())
      ->infoDict();
}
PY_AUTO_MODULE_FUNCTION(RETOWN,
                        udoGet,
                        ARG(BW::string /*guid*/,
                            ARG(BW::string /*chunkId*/, END)),
                        WorldEditor);
