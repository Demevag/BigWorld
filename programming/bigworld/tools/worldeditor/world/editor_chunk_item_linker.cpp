...
namespace {
    bool isLinkOwner(EditorChunkItemLinkable* linker,
                     EditorChunkItemLinkable* endLinker)
    {
        BW_GUARD;
 ...
 *	@param CID	The chunk id of the item being linked to.
 */
EditorChunkItemLinkable::EditorChunkItemLinkable(ChunkItem*        chunkItem,
                                                 UniqueID          guid,
                                                 PropertiesHelper* propHelper)
  : chunkItem_(chunkItem)
  , guid_(guid)
  , propHelper_(propHelper)
{
}
...
void EditorChunkItemLinkable::clearAllLinks()
{
    ...
    for (int i = 0; i < propCounter; i++) {
        ...
        // If this property is an array of linkable properties
            ...
            PyObjectPtr ob(propHelper()->propGetPy(i),
            ...
            propArray.init(
              chunkItem(), &(dataType->getElemType()), ob.getObject());
...
bool EditorChunkItemLinkable::linkedChunksWriteable()
{
    ...
    for (int i = 0; i < propCounter; i++) {
        ...
        if (propHelper()->isUserDataObjectLink(i)) {
            ...
            if (linkableUDO && linkableUDO->chunkItem()) {
                ...
                ERROR_MSG("EditorChunkItemLinkable::linkedChunksWriteable : "
        ...
        // Is this property an array of user data object links
            ...
            ArrayPropertiesHelper propArray;
            propArray.init(
              this->chunkItem(), &(dataType->getElemType()), ob.getObject());

            // Iterate through the array of links
            for (int j = 0; j < propArray.propCount(); j++) {
                PyObjectPtr aob(propArray.propGetPy(j),
                                PyObjectPtr::STEAL_REFERENCE);
                BW::string  id =
                  PyString_AsString(PyTuple_GetItem(aob.getObject(), 0));
                BW::string chunkId =
                  PyString_AsString(PyTuple_GetItem(aob.getObject(), 1));
                if (id == "" || chunkId == "")
                    continue;

                // Force load the linker since we need to examine its parent
                // chunk
                EditorChunkItemLinkable* linkableUDO =
                  WorldManager::instance().linkerManager().forceLoad(id,
                                                                     chunkId);
                ...
                if (linkableUDO && linkableUDO->chunkItem()) {
                    ...
                    if (!dirMap->gridFromChunkName(chunkId, gridX, gridZ) ||
                        !EditorChunk::outsideChunkWriteable(
                          gridX, gridZ, false)) {
                        return false;
                    }
                }
    ...
    // process back links
    for (LinksConstIter it = getBackLinksBegin(); it != getBackLinksEnd();
         ++it) {
        // Force load the linker since we need to examine its parent chunk
        EditorChunkItemLinkable* linkableUDO =
          WorldManager::instance().linkerManager().forceLoad((*it).UID_,
                                                             (*it).CID_);
...
void EditorChunkItemLinkable::createChunkLink(const BW::string& chunkId)
{
    ...
    Vector3 nextChunkDir = Vector3((gridX + 0.5f) * gridSize - chunkPos[0],
                                   0,
...
void EditorChunkItemLinkable::removeChunkLinks()
{
    ...
    for (BW::map<BW::string, ChunkLinkPtr>::iterator it =
           chunkPointLinks_.begin();
         it != chunkPointLinks_.end();
         ++it) {
...
void EditorChunkItemLinkable::addBackLink(EditorChunkItemLinkable* pEcil)
{
    ...
    if (this == pEcil) {
        ERROR_MSG("EditorChunkItemLinkable::addBackLink : Should not be "
                  "creating a back link to itself (%s).\n",
 ...
 */
void EditorChunkItemLinkable::addBackLink(const UniqueID&   guid,
 ...
 *  @param chunkId	Chunk Id of the link to remove
 */
void EditorChunkItemLinkable::removeBackLink(const UniqueID&   guid,
                                             const BW::string& chunkId)
{
    BW_GUARD;
 ...
 */
EditorChunkItemLinkable::LinksConstIter
EditorChunkItemLinkable::getBackLinksBegin()
{
 ...
 */
EditorChunkItemLinkable::LinksConstIter
EditorChunkItemLinkable::getBackLinksEnd()
{
 ...
 */
bool EditorChunkItemLinkable::hasLinksTo(const UniqueID&   guid,
                                         const BW::string& chunkId)
{
    BW_GUARD;
 ...
 */
ChunkLinkPtr EditorChunkItemLinkable::createLink(ChunkLink::Direction linkType,
                                                 EditorChunkItemLinkable* other)
{
    BW_GUARD;
    ...
    if (other == this) {
        ERROR_MSG("EditorChunkItemLinkable::createLink : Cannot create link to "
                  "ourselves (%s).\n",
                  this->guid().toString().c_str());
        return NULL;
    }
    ...
    if (linkType != ChunkLink::DIR_NONE && chunkItem() &&
        chunkItem()->chunk() && other->chunkItem() &&
        other->chunkItem()->chunk() && chunkItem()->isEditorUserDataObject() &&
        other->chunkItem()->isEditorUserDataObject()) {
        ...
        static float s_minLinkDistance =
          Options::getOptionFloat("render/links/minLinkLength", 0.2f);

        float minLengthSquared = s_minLinkDistance * s_minLinkDistance;
        if ((endPos - startPos).lengthSquared() < minLengthSquared) {
            WARNING_MSG(
              "A link from '%s' to '%s' is too short, which can cause "
              "problems if used for entity navigation.\n",
              this->guid().toString().c_str(),
              other->guid().toString().c_str());
        }
    }
...
bool EditorChunkItemLinkable::isLinkedTo(UniqueID const& other) const
{
    ...
    EditorChunkItemLinkable* myself =
      const_cast<EditorChunkItemLinkable*>(this);
 ...
 */
ChunkLinkPtr EditorChunkItemLinkable::setLink(EditorChunkItemLinkable* other,
                                              bool canTraverse)
{
    BW_GUARD;
    ...
    if (canTraverse) {
        ...
        if (owns) {
            link->direction((ChunkLink::Direction)(
...
void EditorChunkItemLinkable::delLink(ChunkLinkPtr link)
{
    ...
    for (BW::vector<ChunkLinkPtr>::iterator it = links_.begin();
         it != links_.end();
         ++it) {
        if (*it == link) {
            links_.erase(it);
 ...
 *                      is no link.
 */
ChunkLinkPtr EditorChunkItemLinkable::findLink(
  EditorChunkItemLinkable const* other) const
{
    BW_GUARD;

    for (BW::vector<ChunkLinkPtr>::const_iterator it = links_.begin();
         it != links_.end();
         ++it) {
        ChunkLinkPtr link = *it;
        if (link->startItem() == other->chunkItem() ||
            link->endItem() == other->chunkItem()) {
            return link;
        }
    }
...
void EditorChunkItemLinkable::removeLink(ChunkLinkPtr link)
{
    ...
    ChunkItem* otherCI = static_cast<ChunkItem*>(link->endItem().getObject());
    ChunkItem* myselfCI =
    ...
    if (otherCI->isEditorEntity())
        otherLinker =
          static_cast<EditorChunkEntity*>(otherCI)->chunkItemLinker();
    else if (otherCI->isEditorUserDataObject())
        otherLinker =
          static_cast<EditorChunkUserDataObject*>(otherCI)->chunkItemLinker();

    if (myselfCI->isEditorEntity())
        myselfLinker =
          static_cast<EditorChunkEntity*>(myselfCI)->chunkItemLinker();
    else if (myselfCI->isEditorUserDataObject())
        myselfLinker =
          static_cast<EditorChunkUserDataObject*>(myselfCI)->chunkItemLinker();
    ...
    if (otherLinker->chunkItem() == chunkItem()) {
        if (myselfCI->isEditorEntity())
            ...
            otherLinker = static_cast<EditorChunkUserDataObject*>(myselfCI)
                            ->chunkItemLinker();
    }
