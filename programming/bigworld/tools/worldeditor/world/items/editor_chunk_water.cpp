...
namespace {
    AutoConfigString s_chunkWaterMetaDataConfig(
...
class ChunkWaterConfig
{
  public:
    ...
    {
        return numIndicesInternal(
          state_.tessellation_, state_.size_.x, state_.size_.y);
    }
    ...
    {
        // TODO: the cell size needs to be limited based on tessellation and
        // size
    ...
    bool validateSize(float sizeX, float sizeY)
    {
        ...
        if ((count < MAX_INDEX_COUNT) &&
            equalOrGreater(sizeX, state_.tessellation_) &&
 ...
 */
class MyChunkItemMatrix
  : public ChunkWaterConfig
  , public ChunkItemMatrix
{
  public:
...
bool MyChunkItemMatrix::setMatrix(const Matrix& m)
{
    ...
    if (pItem_->chunk()) {
        spaceSizeX = pItem_->chunk()->space()->maxCoordX() -
                     pItem_->chunk()->space()->minCoordX();
        spaceSizeZ = pItem_->chunk()->space()->maxCoordZ() -
                     pItem_->chunk()->space()->minCoordZ();
    }

    if ((width <= spaceSizeX && height <= spaceSizeZ) &&
...
EditorChunkWater::~EditorChunkWater()
{
    ...
    BW::vector<EditorChunkWater*>::iterator p =
...
// bool EditorChunkSubstance<ChunkWater>::load(DataSectionPtr pSect, Chunk *
// pChunk )
//{
//	pOwnSect_ = pSect;
//
...
////	bb.addBounds( ( ( EditorChunkSubstance<LT>* )
/// this)->edTransform().applyToOrigin().y );
//	return true;
//}
...
bool EditorChunkWater::load(DataSectionPtr pSection, Chunk* pChunk)
{
    ...
    if (ok) {
        ...
        origin_.setIdentity();
        Matrix rtr;
        rtr.setRotateY(state_.orientation_);
        origin_.postMultiply(rtr);
        origin_.translation(state_.position_);

        bb_.setBounds(
          state_.position_ +
            Vector3(-state_.size_.x * 0.5f, 0, -state_.size_.y * 0.5f),
          state_.position_ +
            Vector3(state_.size_.x * 0.5f, 0, state_.size_.y * 0.5f));
...
void EditorChunkWater::save()
{
    ...
    waterSection->writeVector3("size",
                               Vector3(state_.size_.x, 0.f, state_.size_.y));
    ...
    waterSection->writeFloat("textureTessellation",
                             state_.textureTessellation_);
    ...
    waterSection->writeString("reflectionTexture",
                              resources_.reflectionTexture_);
...
{
  public:
    explicit ChunkWaterFloat(EditorChunkWaterPtr pItem,
                             float&              f,
                             float               min      = 0.f,
                             float               max      = 1.f,
                             int                 digits   = 1,
    ...
    virtual bool setPermanent(float f)
    {
        ...
        if ((limitMin_ && (f < minValue_)) || (limitMax_ && (f > maxValue_)))
            return false;
    ...
    virtual BW::string opName()
    {
        ...
        return LocaliseUTF8("WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/"
                            "SET_FLOAT_PROPERTY");
    }
...
class ChunkWaterTessellation
  : public ChunkWaterConfig
  , public ChunkWaterFloat
{
  public:
    ChunkWaterTessellation(EditorChunkWaterPtr pItem, Water::WaterState& config)
      : ChunkWaterFloat(pItem,
                        config.tessellation_,
                        1.f,
                        1000.f,
                        1,
                        true,
    ...
    virtual bool setPermanent(float f)
    {
        ...
        if (validateTessellation(f)) {
            ...
            WorldManager::instance().addCommentaryMsg(
              LocaliseUTF8("WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/"
                           "BAD_MESH_SIZE"));
            return false;
        }
    }
...
class ChunkWaterData : public UndoableDataProxy<DTYPE>
{
  public:
    explicit ChunkWaterData(EditorChunkWaterPtr   pItem,
                            typename DTYPE::Data& v,
                            bool                  updateReferences = false,
                            bool                  doSyncInit       = false)
      : pItem_(pItem)
      , v_(v)
    ...
    virtual typename DTYPE::Data get() const { return v_; }

    virtual void setTransient(typename DTYPE::Data v) { v_ = v; }
    ...
    virtual BW::string opName()
    {
        ...
        return LocaliseUTF8(
          L"WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/SET_DATA");
    }
...
// need a new matrix proxy / float proxy etc etc..... and use them to update the
// right variables... perhaps i can get the scaling to work from the reference
// transformations ALA the translation?

/**
 *	Add the properties to the given editor
 */
bool EditorChunkWater::edEdit(class GeneralEditor& editor,
                              const ChunkItemPtr   pItem)
{
    ...
    STATIC_LOCALISE_NAME(
      s_position, "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/POSITION");
    STATIC_LOCALISE_NAME(
      s_rotation,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/ORIENTATION");
    STATIC_LOCALISE_NAME(
      s_size, "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/SIZE");
    STATIC_LOCALISE_NAME(
      s_fresnelConstant,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/FRESNEL_CONSTANT");
    STATIC_LOCALISE_NAME(
      s_fresnelExponent,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/FRESNEL_EXPONENT");
    STATIC_LOCALISE_NAME(
      s_reflectionTint,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/REFLECTION_TINT");
    STATIC_LOCALISE_NAME(
      s_reflectionStrength,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/REFLECTION_STRENGTH");
    STATIC_LOCALISE_NAME(
      s_refractionTint,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/REFRACTION_TINT");
    STATIC_LOCALISE_NAME(
      s_refractionStrength,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/REFRACTION_STRENGTH");
    STATIC_LOCALISE_NAME(
      s_meshSize, "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/MESH_SIZE");
    STATIC_LOCALISE_NAME(
      s_consistency,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/CONSISTENCY");
    STATIC_LOCALISE_NAME(
      s_textureSize,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/TEXTURE_SIZE");
    STATIC_LOCALISE_NAME(
      s_scrollSpeed1U,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/SCROLL_SPEED_1_U");
    STATIC_LOCALISE_NAME(
      s_scrollSpeed1V,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/SCROLL_SPEED_1_V");
    STATIC_LOCALISE_NAME(
      s_scrollSpeed2U,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/SCROLL_SPEED_2_U");
    STATIC_LOCALISE_NAME(
      s_scrollSpeed2V,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/SCROLL_SPEED_2_V");
    STATIC_LOCALISE_NAME(
      s_waveScaleU,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/WAVE_SCALE_U");
    STATIC_LOCALISE_NAME(
      s_waveScaleV,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/WAVE_SCALE_V");
    STATIC_LOCALISE_NAME(
      s_windSpeed,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/WIND_SPEED");
    STATIC_LOCALISE_NAME(
      s_surfaceSmoothness,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/SURFACE_SMOOTHNESS");
    STATIC_LOCALISE_NAME(
      s_sunPower, "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/SUN_POWER");
    STATIC_LOCALISE_NAME(
      s_sunScale, "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/SUN_SCALE");
    STATIC_LOCALISE_NAME(
      s_deepColour,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/DEEP_COLOUR");
    STATIC_LOCALISE_NAME(
      s_fadeDepth,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/FADE_DEPTH");
    STATIC_LOCALISE_NAME(
      s_foamIntersection,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/FOAM_INTERSECTION");
    STATIC_LOCALISE_NAME(
      s_foamMultiplier,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/FOAM_MULTIPLIER");
    STATIC_LOCALISE_NAME(
      s_foamTiling,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/FOAM_TILING");
    STATIC_LOCALISE_NAME(
      s_bypassDepth,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/BYPASS_DEPTH");
    STATIC_LOCALISE_NAME(
      s_reflectionTexture,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/REFLECTION_TEXTURE");
    STATIC_LOCALISE_NAME(
      s_foamTexture,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/FOAM_TEXTURE");
    STATIC_LOCALISE_NAME(
      s_waveTexture,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/WAVE_TEXTURE");
    STATIC_LOCALISE_NAME(
      s_simCellSize,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/SIM_CELL_SIZE");
    STATIC_LOCALISE_NAME(
      s_useCubeMap,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/USE_CUBE_MAP");
    STATIC_LOCALISE_NAME(
      s_useEdgeAlpha,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/USE_EDGE_ALPHA");
    STATIC_LOCALISE_NAME(
      s_useSimulation,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/USE_SIMULATION");
    STATIC_LOCALISE_NAME(
      s_reflectionBottom,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/REFLECT_BOTTOM");
    STATIC_LOCALISE_NAME(
      s_alwaysVisible,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/ALWAYS_VISIBLE");
    STATIC_LOCALISE_NAME(
      s_insideOnly,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/INSIDE_ONLY");
    STATIC_LOCALISE_NAME(
      s_outsideOnly,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/OUTSIDE_ONLY");
    STATIC_LOCALISE_NAME(
      s_visibility,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/VISIBILITY");
    STATIC_LOCALISE_NAME(
      s_freqX, "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/FREQ_X");
    STATIC_LOCALISE_NAME(
      s_freqZ, "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/FREQ_Z");
    STATIC_LOCALISE_NAME(
      s_waveHeight,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/WAVE_HEIGHT");
    STATIC_LOCALISE_NAME(
      s_causticsPower,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/CAUSTICS_POWER");
    STATIC_LOCALISE_NAME(
      s_refractionPower,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/WATER_CONTRAST");
    STATIC_LOCALISE_NAME(
      s_animationSpeed,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/ANIMATION_SPEED");
    STATIC_LOCALISE_NAME(
      s_foamFrequency,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/FOAM_FREQ");
    STATIC_LOCALISE_NAME(
      s_foamAmplitude,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/FOAM_AMPLITUDE");
    STATIC_LOCALISE_NAME(
      s_foamWidth,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/FOAM_WIDTH");
    STATIC_LOCALISE_NAME(
      s_useShadows,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/USE_SHADOWS");
    STATIC_LOCALISE_NAME(
      s_waveTextureNumber,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_WATER/WAVE_TEXTURE_NUMBER");
    ...
    editor.addProperty(new GenFloatProperty(
      s_fresnelConstant, new ChunkWaterFloat(this, state_.fresnelConstant_)));
    editor.addProperty(new GenFloatProperty(
      s_fresnelExponent,
      new ChunkWaterFloat(this, state_.fresnelExponent_, 0.f, 7.f)));

    editor.addProperty(new GenFloatProperty(
      s_freqX, new ChunkWaterFloat(this, state_.freqX, 0.0f, 3.0f)));

    editor.addProperty(new GenFloatProperty(
      s_freqZ, new ChunkWaterFloat(this, state_.freqZ, 0.0f, 3.0f)));

    editor.addProperty(new GenFloatProperty(
      s_waveHeight, new ChunkWaterFloat(this, state_.waveHeight, 0.0f, 3.0f)));
    ...
    editor.addProperty(
      new GenFloatProperty(s_reflectionStrength,
    ...
    editor.addProperty(new GenFloatProperty(
      s_meshSize, new ChunkWaterTessellation(this, state_)));
    ...
    editor.addProperty(new GenFloatProperty(
      s_textureSize,
      new ChunkWaterFloat(
        this, state_.textureTessellation_, 0.f, 1000.f, 1, true, false)));
    ...
    editor.addProperty(new GenFloatProperty(
      s_waveScaleU, new ChunkWaterData<FloatProxy>(this, state_.waveScale_.x)));

    editor.addProperty(new GenFloatProperty(
      s_waveScaleV, new ChunkWaterData<FloatProxy>(this, state_.waveScale_.y)));

    editor.addProperty(new GenFloatProperty(
      s_windSpeed, new ChunkWaterData<FloatProxy>(this, state_.windVelocity_)));

    editor.addProperty(new GenFloatProperty(
      s_surfaceSmoothness, new ChunkWaterFloat(this, state_.smoothness_)));

    editor.addProperty(new GenFloatProperty(
      s_sunPower, new ChunkWaterFloat(this, state_.sunPower_, 1.f, 256.f)));

    editor.addProperty(new GenFloatProperty(
      s_sunScale, new ChunkWaterFloat(this, state_.sunScale_, 0.f, 10.f)));
    ...
    editor.addProperty(new GenFloatProperty(
      s_fadeDepth, new ChunkWaterData<FloatProxy>(this, state_.fadeDepth_)));
    ...
    editor.addProperty(new GenFloatProperty(
      s_foamTiling, new ChunkWaterData<FloatProxy>(this, state_.foamTiling_)));
    ...
    editor.addProperty(new GenFloatProperty(
      s_foamWidth, new ChunkWaterFloat(this, state_.foamWidth_, 0.0f, 50.0f)));

    editor.addProperty(new GenBoolProperty(
      s_bypassDepth, new ChunkWaterData<BoolProxy>(this, state_.bypassDepth_)));

    editor.addProperty(new GenBoolProperty(
      s_useShadows, new ChunkWaterData<BoolProxy>(this, state_.useShadows_)));
    ...
    pTProp->fileFilter(
      Name("Texture files(*.jpg;*.tga;*.bmp;*.dds)|*.jpg;*.tga;*.bmp;*.dds||"));
    ...
    editor.addProperty(new GenUIntProperty(
      s_waveTextureNumber,
      new ChunkWaterData<UIntProxy>(
        this, (UIntProxy::Data&)state_.waveTextureNumber_)));

    editor.addProperty(new GenFloatProperty(
      s_simCellSize, new ChunkWaterSimCellSize(this, state_)));
    ...
    editor.addProperty(new GenBoolProperty(
      s_reflectionBottom,
      new ChunkWaterData<BoolProxy>(
        this, state_.reflectBottom_, false, true /* do sync init */)));
...
{
    return state_.visibility_ == Water::ALWAYS_VISIBLE ||
 ...
 *
 */
bool EditorChunkWater::create(Chunk*            pChunk,
...
ChunkItemFactory::Result EditorChunkWater::oldCreate(Chunk*         pChunk,
                                                     DataSectionPtr pSection)
{
    BW_GUARD;
...
ChunkItemFactory
  EditorChunkWater::oldWaterFactory_("water", 0, EditorChunkWater::oldCreate);
