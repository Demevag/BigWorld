...
static AutoConfigString s_defaultModel("editor/entityModel",
                                       "resources/models/entity.model");
static AutoConfigString s_legacyDefaultModel("editor/entityModelLegacy",
...
EditorEntityType::EditorEntityType()
{
    ...
    eMap_.parse(BWResource::openSection(EntityDef::Constants::entitiesFile()));
    // load markers with properties that are pretending to be entities
    DataSectionPtr markers =
      BWResource::openSection(EntityDef::Constants::markerCategoriesFile());
    if (markers) {
        for (DataSectionIterator mit = markers->begin(); mit != markers->end();
             mit++) {
    ...
    INFO_MSG(
      "EditorEntityType constructor - Importing editor entity scripts\n");
    DataSectionPtr edScripts =
      BWResource::openSection(EntityDef::Constants::entitiesEditorPath());

    if (edScripts) {
        for (EntityDescriptionMap::DescriptionMap::const_iterator it =
               eMap_.begin();
             it != eMap_.end();
             it++) {
            ...
            if (PyErr_Occurred()) {
                ERROR_MSG(
                  "EditorEntityType - fail to import editor script %s\n",
                  name.c_str());
                PyErr_Print();
                continue;
            }
...
class SetupEditorPropertyTypes
{
  public:
    ...
    {
      public:
        EditorPatrolPathDataType(MetaDataType* pMetaType)
          : StringDataType(pMetaType)
        {
        }

        GeneralProperty* createEditorProperty(const BW::string& name,
                                              ChunkItem* editorChunkEntity,
 ...
 */
bool EditorChunkEntity::canLinkTo(const BW::string& propName,
                                  PyObject*         otherInfo) const
{
    BW_GUARD;
 ...
 */
bool EditorChunkEntity::load(DataSectionPtr pSection,
                             Chunk*         pChunk,
...
bool EditorChunkEntity::edLoad(const BW::string type,
...
bool EditorChunkEntity::edLoad(DataSectionPtr pSection, bool loadTransform)
{
    ...
    if (!surrogateMarker_) {
        ...
        if (!clientOnlyDS) {
            ...
            if (pType_ != NULL && !pType_->canBeOnBase() &&
                !pType_->canBeOnCell())
                clientOnly_ = true;
            else
                clientOnly_ = false;
...
bool EditorChunkEntity::edSave(DataSectionPtr pSection)
{
    ...
    for (uint i = 0; i < pType_->propertyCount(); i++) {
        ...
        ScriptObject pValue = ScriptObject(
...
/*virtual*/ bool EditorChunkEntity::edCanDelete()
{
    ...
    if (!chunkItemLinker()->linkedChunksWriteable()) {
        ERROR_MSG(
          LocaliseUTF8(
            L"WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_ENTITY/CANNOT_DELETE",
            guid().toString())
            .c_str());
        return false;
    }
...
Name EditorChunkEntity::edDescription()
{
    ...
    return Name(LocaliseUTF8(
      L"WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_ENTITY/ED_DESCRIPTION",
      name));
}

BW::vector<BW::string> EditorChunkEntity::edCommand(
...
bool EditorChunkEntity::edExecuteCommand(
...
BW::string EditorChunkEntity::edFilePath() const
{
    ...
    return BW::string(EntityDef::Constants::entitiesDefsPath()) + "/" +
 ...
 */
bool EditorChunkEntity::patrolListRelink(const BW::string& newGraph,
...
bool EditorChunkEntity::edEdit(class GeneralEditor& editor)
{
    ...
    STATIC_LOCALISE_NAME(
      s_type, "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_ENTITY/TYPE");
    STATIC_LOCALISE_NAME(
      s_position, "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_ENTITY/POSITION");
    STATIC_LOCALISE_NAME(
      s_direction,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_ENTITY/DIRECTION");
    STATIC_LOCALISE_NAME(
      s_clientOnly,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_ENTITY/CLIENT_ONLY");
    STATIC_LOCALISE_NAME(
      s_lastScriptError,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_ENTITY/LAST_SCRIPT_ERROR");
    STATIC_LOCALISE_NAME(
      s_graph, "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_ENTITY/GRAPH");
    STATIC_LOCALISE_NAME(
      s_node, "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_ENTITY/NODE");
    ...
    editor.addProperty(new StaticTextProperty(
      Name("guid"),
      new AccessorDataProxy<EditorChunkEntity, StringProxy>(
        this, "guid", &EditorChunkEntity::idGet, &EditorChunkEntity::idSet)));
    ...
    editor.addProperty(
      new GenBoolProperty(s_clientOnly,
...
bool EditorChunkEntity::edEditProperties(class GeneralEditor& editor,
                                         MatrixProxy*         pMP)
{
    BW_GUARD;
    ...
    if (pType_ == NULL) {
        if (surrogateMarker_) {
            ...
            ERROR_MSG(
              "EditorChunkEntity::edEdit - no properties for entity!\n");
            return false;
        }
    }
...
EditorChunkStationNode* EditorChunkEntity::connectedNode()
{
    ...
    for (; i != nodes.end(); ++i) {
        ...
        Vector3 to = dest->chunk()->transform().applyPoint(
                       dest->edTransform().applyToOrigin()) +
...
void EditorChunkEntity::removeFromDirtyList()
{
    ...
    BW::vector<EditorChunkEntity*>::iterator it = std::find(
...
void EditorChunkEntity::calculateDirtyModels()
{
    ...
    while (i < EditorChunkEntity::s_dirtyModelEntities_.size()) {
        ...
        if (!ent->chunk() || !ent->chunk()->isBound()) {
            ...
            ent->calculateModel();
            EditorChunkEntity::s_dirtyModelEntities_.erase(
              EditorChunkEntity::s_dirtyModelEntities_.begin() + i);
        }
    }
}
...
/*static*/ void EditorChunkEntity::loadModelTaskDone(void* self)
{
    ...
    if (!entity->model_) {
        BW::string errMsg = LocaliseUTF8(L"Unable to load entity's model %0",
                                         entity->modelToLoad_);
