...
ChunkItemFactory EditorChunkModelVLORef::factory_(
  ChunkModelVLORef::getSectionName(),
  1,
  EditorChunkModelVLORef::create);
...
ChunkItemFactory::Result EditorChunkModelVLORef::create(Chunk*         pChunk,
  ...
  DataSectionPtr pModelSection)
{
    ...
    xmlSectionPtr->writeString(ChunkVLO::getTypeAttrName(),
                               ChunkModelVLO::getLargeObjectType());
    xmlSectionPtr->writeString(ChunkVLO::getUIDAttrName(),
                               VeryLargeObject::generateUID());
...
/*virtual */ void EditorChunkModelVLORef::edBounds(BoundingBox& bbRet) const
{
  ...
  bool          transient /*= false */)
{
    ...
    return VeryLargeObjectBehaviour::edTransform(
      *pObject_, pChunk_, vloBB, this, transform, drawTransient_, m, transient);
}
...
bool EditorChunkModelVLORef::load(DataSectionPtr pSection,
                                  Chunk*         pChunk,
...
{
    return EditorChunkModelVLOParent::edShouldDraw() && !this->edHidden();
}
...
/*virtual */ bool EditorChunkModelVLORef::edSave(DataSectionPtr pSection)
{
    ...
    pSection->writeString(ChunkVLO::getTypeAttrName(),
                          ChunkModelVLO::getLargeObjectType());
...
/*static */ ChunkItemPtr EditorChunkModelVLORef::convertToChunkModelVLO(
  ChunkItemPtr  chunkModel,
  const Matrix& world)
{
    DataSectionPtr xmlSectionPtr =
      EditorChunkModelVLORef::createDataSection(chunkModel->pOwnSect());
