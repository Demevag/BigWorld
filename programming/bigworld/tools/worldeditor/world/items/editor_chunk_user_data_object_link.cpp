  ...
  BW::string const& path) const
{
    ...
    if (startCI->edIsEditable() && endCI->edIsEditable()) {
        ...
        if (!startCI->isEditorUserDataObject() ||
            !endCI->isEditorUserDataObject())
            return commands;
  ...
  BW::vector<BW::string>::size_type index)
{
    ...
    if (startCI->isEditorEntity() && endCI->isEditorUserDataObject()) {
        ...
        if (index == 0) {
            ...
            EditorChunkUserDataObject* end =
              static_cast<EditorChunkUserDataObject*>(endItem().getObject());
            // At the moment, it doesn't matter if "executeLinkCommand" is
            // called from start or end, and it only works if both UDOs are the
            // same type.
...
void EditorChunkUserDataObjectLink::deleteCommand()
{
    ...
    if (startCI->isEditorEntity())
        startLinker =
          static_cast<EditorChunkEntity*>(startCI)->chunkItemLinker();
    else if (startCI->isEditorUserDataObject())
        startLinker =
          static_cast<EditorChunkUserDataObject*>(startCI)->chunkItemLinker();

    if (endCI->isEditorEntity())
        endLinker = static_cast<EditorChunkEntity*>(endCI)->chunkItemLinker();
    else if (endCI->isEditorUserDataObject())
        endLinker =
          static_cast<EditorChunkUserDataObject*>(endCI)->chunkItemLinker();

    // Inform the linker manager that all links are to be deleted
    WorldManager::instance().linkerManager().deleteAllLinks(startLinker,
                                                            endLinker);
  ...
  WorldTriangle& wt) const
{
    ...
    if (start == NULL || end == NULL || !start->edIsEditable() ||
        !end->edIsEditable()) {
