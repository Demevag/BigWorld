...
static AutoConfigString s_blankTerrainTexture("system/emptyTerrainTextureBmp",
                                              "helpers/maps/aid_builder.tga");
static AutoConfigString s_dxEnumPath("system/dxenum");

// Preload models

static AutoConfigString s_notFoundModel("system/notFoundModel");
static AutoConfigString s_entityModel("editor/entityModel",
                                      "resources/models/entity.model");
static AutoConfigString s_legacyEntityModel("editor/entityModelLegacy",
                                            "helpers/props/standin.model");
...
class WEChunkLoader : public UnsavedChunks::IChunkLoader
{
  ...
  public:
    WEChunkLoader(GeometryMapping* mapping)
      : mapping_(mapping)
    {
    }
    ...
    {
        return ChunkManager::instance().findChunkByName(chunk, mapping_, false);
    }
};
...
static int s_chunkTokenSet =
  ChunkModel_token | ChunkLight_token | ChunkTerrain_token | ChunkFlare_token |
  ChunkWater_token | EditorChunkOverlapper_token | ChunkParticles_token |
  ChunkAttachment_token | ChunkTree_token | PyResourceRefs_token | Servo_token |
...
static int s_toolFunctorTokenSet =
  PipeFunctor_token | ScriptedToolFunctor_token | TeeFunctor_token |
  TerrainHeightFilterFunctor_token | TerrainHoleFunctor_token |
  TerrainSetHeightFunctor_token | TerrainTextureFunctor_token;
...
namespace {
    ...
    void allChunkItems(ChunkItemRevealer::ChunkItems& items)
    {
        ...
        for (ChunkMap::iterator i =
               ChunkManager::instance().cameraSpace()->chunks().begin();
             i != ChunkManager::instance().cameraSpace()->chunks().end();
             i++) {
     ...
     */
    void filterSelectableItems(ChunkItemRevealer::ChunkItems& unfilteredItems,
                               bool                           ignoreVis,
                               bool                           ignoreFrozen,
                               ChunkItemRevealer::ChunkItems& retItems)
    {
        ...
        for (ChunkItemRevealer::ChunkItems::iterator it =
               unfilteredItems.begin();
             it != unfilteredItems.end();
             ++it) {
            ChunkItemPtr item = *it;

            if (item->isShellModel()) {
                if (SelectionFilter::canSelect(
                      item.get(), true, false, ignoreVis, ignoreFrozen)) {
        ...
        for (ChunkItemRevealer::ChunkItems::iterator it =
            ...
            if (!item->isShellModel() && !item->chunk()->isOutsideChunk() &&
                selectableShells.find(item->chunk()) !=
                  selectableShells.end()) {
                // If the object is inside a shell, and the shell is already
                // selected, then we shouldn't select the shell's contents (this
                // item)
                continue;
            }
            ...
            if (item->edIsVLO()) {
                ...
                std::pair<BW::set<VeryLargeObjectPtr>::iterator, bool>
                  insertIt = selectedVLO.insert(vlo);
...
{
  public:
    SelectionOperation(const BW::vector<ChunkItemPtr>& before,
                       const BW::vector<ChunkItemPtr>& after);
...
WorldManager::WorldManager()
  ...
  , GUI::ActionMaker<WorldManager>(
      "changeSpace|newSpace|editSpace|recreateSpace|recentSpace|"
      "clearUndoRedoHistory|doExternalEditor|"
      "doReloadAllTextures|doReloadAllChunks|doExit|setLanguage|"
      "recalcCurrentChunk",
      &WorldManager::handleGUIAction)
  , GUI::UpdaterMaker<WorldManager>("updateRecreateSpace|updateUndo|updateRedo|"
                                    "updateExternalEditor|updateLanguage",
...

{
    ...
    MF_WATCH(
      "Chunk Processor/Time Slice",
      s_minimumAllowTimeSliceMS_,
      Watcher::WT_READ_WRITE,
      "Minimum amount of time per frame to dedicate to the chunk processors. "
      "A value of 0 means no dedicated time.");

    MF_WATCH(
      "Chunk Processor/Wait Warn Threshold",
      s_timeSlicePauseWarnThresholdMS_,
      Watcher::WT_READ_WRITE,
...
void WorldManager::fini()
{
    ...
    //        The world manager hangs on to unsaved terrain blocks, and needs
    //        Moo to free their renderer resources. Do this prior to performing
    //        shut down.
    ...
    if (inited_) {
        ...
        while (ToolManager::instance().tool()) {
            WARNING_MSG("WorldManager::fini : There is still a tool on the "
                        "stack that should have been cleaned up\n");
...
namespace {
    ...
    struct isChunkFileExists
    {
        bool operator()(const BW::string& filename, GeometryMapping* dirMapping)
        {
            ...
            return BWResource::fileExists(dirMapping->path() + filename +
                                          ".chunk");
        }
    } isChunkFileExists;
...
void WorldManager::update(float dTime)
{
    ...
    // GIZMOS
    if (InputDevices::isShiftDown() || InputDevices::isCtrlDown() ||
    ...
    bool drawFlora =
      !!Options::getOptionInt("render/environment/drawDetailObjects", 1);
    ...
    if (timeLastUpdateTexLod_ >
        Options::getOptionFloat("terrain/texture/lodregentime", 1.0f)) {
        timeLastUpdateTexLod_ = 0.0f;
...
bool WorldManager::isMemoryLow(bool testNow) const
{
    ...
    if (testNow ||
        curTime > (s_lastMemoryAllocTest + s_memoryAllocTestInterval)) {
        ...
        if (s_memoryIsFragmented) {
            WARNING_MSG("Memory is fragmented, failed to alloc %d MB (memory "
                        "load is %d %%).\n",
                        testSize,
                        memoryLoad);
        }
...
bool WorldManager::isMemoryLoadWarningNeeded() const
{
    ...
    return warningOnLowMemory_ &&
           (isMemoryLow() || Moo::rc().memoryCritical() ||
...
void WorldManager::checkMemoryLoad()
{
    ...
    if (!canLoadChunks) {
        ...
        if (pTextureQuality && pTextureQuality->activeOption() != 2) {
            ShowCursor(TRUE);
            CWaitCursor wait;
            ERROR_MSG("Memory is Low, setting texture quality to 'Low' to save "
                      "memory.\n");
            WorldEditorApp::instance().mainWnd()->RedrawWindow();
            pTextureQuality->selectOption(2, true);
        }
        if (s_lastCanLoadChunksWarning == 0 ||
            timestamp() >
              (s_lastCanLoadChunksWarning + s_canLoadChunksWarningInterval)) {
            s_lastCanLoadChunksWarning = timestamp();
            ERROR_MSG("Memory is Low, please reduce the far plane, save and/or "
...
BW::string WorldManager::describeDirtyChunkStatus() const
{
    ...
    for (size_t i = 0; i < dirtyChunkLists.size(); ++i) {
        if (dirtyChunkLists[i].num()) {
            ss << dirtyChunkLists.name(i) << ": " << dirtyChunkLists[i].num()
               << ' ';
        }
    }
...
void WorldManager::writeStatus()
{
    ...
    LocaliseUTF8(statusMessage,
                 L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/MEMORY_LOAD",
    ...
    statusMessage.clear();
    LocaliseUTF8(statusMessage,
                 L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/TRIS",
                 Moo::rc().lastFrameProfilingData().nPrimitives_);
    ...
    if (ToolManager::instance().tool() &&
        ...
        Vector3 pos = ToolManager::instance()
                        .tool()
                        ->locator()
                        ->transform()
                        .applyToOrigin();
        ...
        if (chunk && EditorChunkCache::instance(*chunk).pChunkSection()) {
            ...
            EditorChunkCache::instance(*chunk).pChunkSection()->openSections(
              "model", modelSects);

            LocaliseUTF8(statusMessage,
                         L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/"
                         L"CHUNK_LOCATOR_POSITION",
    ...
    if (s_countDown < 0.f) {
        statusMessage.clear();
        LocaliseUTF8(statusMessage,
                     L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/FPS",
    ...
    if (!dirtyChunks.empty()) {
        LocaliseUTF8(
          statusMessage,
          L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/CHUNK_LOADED_WITH_DIRTY",
          EditorChunkCache::chunks_.size(),
          dirtyChunks);
    } else {
        LocaliseUTF8(statusMessage,
                     L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/CHUNK_LOADED",
                     EditorChunkCache::chunks_.size());
    }
...
void WorldManager::render(float dTime)
{
    ...
    if (farPlane() !=
        Options::getOptionInt("graphics/farclip", (int)farPlane()))
        farPlane((float)Options::getOptionInt("graphics/farclip"));

    EditorChunkItem::hideAllOutside(
      !!Options::getOptionInt("render/hideOutsideObjects", 0));
    ...
    colourDrawContext_->end(Moo::DrawContext::TRANSPARENT_CHANNEL_MASK |
                            Moo::DrawContext::SHIMMER_CHANNEL_MASK);
    colourDrawContext_->flush(Moo::DrawContext::TRANSPARENT_CHANNEL_MASK |
                              Moo::DrawContext::SHIMMER_CHANNEL_MASK);
...
void WorldManager::renderChunks()
{
    ...
    if (chunkManagerInited_) {
        if (drawSelection_) {
            WorldManager::instance().colourDrawContext().pushOverrideBlock(
              selectionOverride_);
        }

        s_chunkDraw.start();
        uint32 scenaryWireFrameStatus_ =
          Options::getOptionInt("render/scenery/wireFrame", 0);
        uint32 terrainWireFrameStatus_ =
          Options::getOptionInt("render/terrain/wireFrame", 0);
        ...
        if (ChunkManager::instance().umbra()->umbraEnabled()) {
            ...
            Moo::rc().setRenderState(D3DRS_FILLMODE,
                                     scenaryWireFrameStatus_ ? D3DFILL_WIREFRAME
                                                             : D3DFILL_SOLID);
            ...
            if (scenaryWireFrameStatus_ || terrainWireFrameStatus_) {
                ...
                Moo::rc().device()->Clear(
                  0, NULL, D3DCLEAR_ZBUFFER, Colour::getUint32(bgColour), 1, 0);
        ...
        if (cc) {
            ...
            for (BW::vector<Chunk*>::iterator ci =
                   EditorChunkOverlapper::drawList.begin();
                 ci != EditorChunkOverlapper::drawList.end();
                 ++ci) {
                Chunk* c = *ci;
                if (!c->isBound()) {
                    // TODO : this shoudln't happen, chunks should get out
                    //  of the drawList when they are offline.
                    DEBUG_MSG("WorldManager::renderChunks: Trying to draw "
                              "chunk %s while it's offline!\n",
                              c->resourceID().c_str());
                    continue;
                }
        ...
        if (cc) {
            for (BW::vector<ChunkItemPtr>::iterator iter =
                   selectedItems_.begin();
                 iter != selectedItems_.end();
                 ++iter) {
                Chunk* c = (*iter)->chunk();
        ...
        // inside chunks will not render if they are not reacheable through
        // portals. If game visibility is off, the overlappers are used to
        // render not-connected chunks. But, with the visibility bounding box
        // culling, the overlapper may not be rendered, causing the stray shell
        // to be invisible, even if it is itself inside the camera frustum. To
        // fix this situation, when game visibility is turned off, after
        // rendering the chunks, it goes through all loaded chunks, trying to
        // render those that are inside and haven't been rendered for this
        // frame. Visibility bounding box culling still applies.
        bool drawAllShells =
          Options::getOptionInt("render/scenery/shells/gameVisibility") == 0;
        bool hideAllOutside =
          Options::getOptionInt("render/hideOutsideObjects") == 1;
        ...
        if (space.exists()) {
            ...
            for (ChunkMap::iterator it = begin; it != end; ++it) {
                for (uint i = 0; i < it->second.size(); i++) {
                    if (it->second[i] != NULL && it->second[i]->isBound() &&
                        OptionsMisc::readOnlyVisible() &&
                        !EditorChunkCache::instance(*it->second[i])
                           .edIsWriteable()) {
                        showReadOnlyRegion(it->second[i]->visibilityBox(),
                                           Matrix::identity);
                    }
                }
            }
        ...
        if (drawSelection_) {
            WorldManager::instance().colourDrawContext().popOverrideBlock(
              selectionOverride_);
        }
...
void WorldManager::renderTerrain()
{
    ...
    if (Options::getOptionInt("render/terrain", 1)) {
        ...
        bool wireFrameTerrain =
          Options::getOptionInt("render/terrain/wireFrame", 0) != 0;
        ...
        Moo::rc().setRenderState(
          D3DRS_FILLMODE, wireFrameTerrain ? D3DFILL_WIREFRAME : D3DFILL_SOLID);

        if (drawSelection()) {
            Moo::EffectMaterialPtr selectionMaterial = NULL;
            // TODO: We should get the FX name automatically with the version
            // number, instead of having to 'if' the version numbers.
            ...
            if (selectionMaterial != NULL) {
                ...
                ERROR_MSG("WorldManager::renderTerrain: There is no valid "
                          "selection shader for the current terrain\n");
            }
        } else {
            Terrain::BaseTerrainRenderer::instance()->drawAll(
              Moo::RENDERING_PASS_COLOR);
        ...
        if (!readOnlyTerrainBlocks_.empty()) {
            AVectorNoDestructor<BlockInPlace>::iterator i =
              readOnlyTerrainBlocks_.begin();
            for (; i != readOnlyTerrainBlocks_.end(); ++i) {
                Terrain::BaseTerrainRenderer::instance()->addBlock(
                  i->second.getObject(), i->first);
            }
...
bool WorldManager::init(HINSTANCE hInst, HWND hwndApp, HWND hwndGraphics)
{
    ...
    if (!inited_) {
        ...
        SceneBrowser::instance().callbackSelChange(
          new BWFunctor1<WorldManager, const SceneBrowser::ItemList&>(
        ...
        Personality::import(
          Options::getOptionString("personality", "Personality"));
        ...
        if (!spaceManager_->num() ||
            !changeSpace(spaceManager_->entry(0), false)) {
            ...
            for (;;) {
                ...
                MsgBox mb(Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/"
                                   L"OPEN_SPACE_TITLE"),
        ...
        Terrain::EditorTerrainBlock2::blendBuildInterval(Options::getOptionInt(
          "terrain2/blendsBuildInterval",
...
bool WorldManager::cursorOverGraphicsWnd() const
{
    ...
    if (fore != mainFrame->GetSafeHwnd() &&
        ::GetParent(fore) != mainFrame->GetSafeHwnd())
...
void WorldManager::addError(Chunk*      chunk,
                            ChunkItem*  item,
                            const char* format,
                            ...)
{
 ...
 *   @param retChunks        out, returned chunks, it's not cleared but
 * appended.
 */
                                         ...
                                         BW::set<Chunk*>&   retChunks)
{
    ...
    if (!pResidentChunk || pResidentChunk->isOutsideChunk()) {
        ...
        // Sampling on every gridSize distance since the bounding box of a
        // particle can be large than chunk size
        for (float x = bb.minBounds().x; x < bb.maxBounds().x; x += gridSize) {
            for (float z = bb.minBounds().z; z < bb.maxBounds().z;
                 z += gridSize) {
                retChunks.insert(
        ...
        // max x, max z
        retChunks.insert(EditorChunk::findOutsideChunk(
          Vector3(bb.maxBounds().x, 0, bb.maxBounds().z)));
 ...
 *                               might spread out and find more chunks to be
 * updated.
 *   @param flags            the flags to notify which ChunkCaches to be updated
 *                               only the ChunkCaches whose flag are included in
 * flags will be updated,if not provided, we only update thumbnail.
 */
void WorldManager::changedChunks(
  BW::set<Chunk*>& primaryChunks,
  InvalidateFlags  flags /*= InvalidateFlags::FLAG_THUMBNAIL*/)
{
    BW_GUARD;

    for (BW::set<Chunk*>::iterator i = primaryChunks.begin();
         i != primaryChunks.end();
         ++i) {
                                 ...
                                 EditorChunkItem& changedItem)
{
    ...
    for (BW::set<Chunk*>::iterator i = primaryChunks.begin();
         i != primaryChunks.end();
         ++i) {
        this->internalChangedChunk(
          *i, changedItem.edInvalidateFlags(), &changedItem);
    }
}

/**
 *   Update a list of chunks, this will be called when you changed a chunk item,
 *   but the invalidateFlags that you wish to update is different from the
 * item's default invalidate flags, if flags == changedItem.edInvalidateFlags(),
 * then use changedChunks( BW::set<Chunk*>& chunks, EditorChunkItem&
 * changedItem) instead.
 *
 *   @param primaryChunks    the chunks which have been changed.
 *                               note: from the primary chunks, the ChunkCache
...
/**
 *   This will be called when you did a change that's not caused by a chunk
 * item, ie.: change terrain
 ...
 * flags will be updated, if not provided, we only update thumbnail.
 */
void WorldManager::changedChunk(
  Chunk*          pPrimaryChunk,
  InvalidateFlags flags /*= InvalidateFlags::FLAG_THUMBNAIL*/)
{
    BW_GUARD;
                                ...
                                EditorChunkItem& changedItem)
{
    ...
    this->internalChangedChunk(
      pPrimaryChunk, changedItem.edInvalidateFlags(), &changedItem);
}
 ...
 *   but the invalidateFlags that you wish to update is different from the
 * item's default invalidate flags, if flags == changedItem.edInvalidateFlags(),
 * then use changedChunks( Chunk * pPrimaryChunk, EditorChunkItem& changedItem)
 * instead.
 *
 *   @param pPrimaryChunk    the chunk which has been changed.
 *                               note: from the primary chunk, the ChunkCache
 ...
 *   @param flags            the flags to notify which ChunkCaches to be updated
 *                               only the ChunkCaches whose flag are included in
 * pFlags will be updated, if NULL, we only update thumbnail by default.
 *   @param changedItem      the changed item that caused the chunk changed.
 */
void WorldManager::changedChunk(Chunk*           pPrimaryChunk,
                                InvalidateFlags  flags,
                                        ...
                                        EditorChunkItem* pChangedItem)
{
    ...
    ClientSpacePtr cs =
      SpaceManager::instance().space(pPrimaryChunk->space()->id());
    if (cs) {
                                       ...
                                       bool                      addAsBinary)
{
    ...
    if (add) {
        CVSWrapper(currentSpace_)
          .addFile(resourceID + ".cdata", addAsBinary, false);
        CVSWrapper(currentSpace_)
          .addFile(resourceID + ".chunk", addAsBinary, false);
    }
...
void WorldManager::eraseAndRemoveFile(const BW::string& resourceID)
{
    ...
    if (fileName.size() > strlen("i.chunk") &&
        strcmp(fileName.c_str() + fileName.size() - strlen("i.chunk"),
               "i.chunk") == 0)
        backupFileName =
          fileName.substr(0, fileName.size() - strlen("i.chunk")) + "i.~chunk~";
...
void WorldManager::changedTerrainBlock(Chunk* pChunk,
...
void WorldManager::resaveAllTerrainBlocks()
{
    ...
    ProgressBarTask terrainTask(
      LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/RESAVE_TERRAIN"),
      float(steps),
      false);
    ...
    for (int32 z = mapping_->minLGridY(); z <= mapping_->maxLGridY(); z++) {
        for (int32 x = mapping_->minLGridX(); x <= mapping_->maxLGridX(); x++) {
            ...
            if (pChunk) {
                EditorChunkTerrainBase* pEct =
                  EditorChunkTerrainCache::instance(*pChunk).pTerrain();
                ...
                xform.setTranslate(mapping_->pSpace()->gridToPoint(x),
                                   0.0f,
                                   mapping_->pSpace()->gridToPoint(z));

                // The current chunk is not in the chunk cache, load up the
                // terrain block and save it out again
                BW::string resourceName =
                  mapping_->path() + chunkIdentifier + ".cdata/terrain";
                Terrain::EditorBaseTerrainBlockPtr pEtb =
                  EditorChunkTerrain::loadBlock(
                    resourceName, NULL, xform.applyToOrigin());

                if (pEtb) {
                    resourceName =
                      mapping_->path() + chunkIdentifier + ".cdata";

                    DataSectionPtr pCDataSection =
                      BWResource::openSection(resourceName);
                    if (pCDataSection) {
                        // We must save to a terrain2 section
                        DataSectionPtr dataSection = pCDataSection->openSection(
                          pEtb->dataSectionName(), true);
                        if (dataSection) {
...
void WorldManager::restitchAllTerrainBlocks()
{
    ...
    ProgressBarTask terrainTask(
      LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/"
                   L"RE_STITCHING_ALL_TERRAIN_BLOCKS"),
      float(steps),
      false);
        ...
        for (int32 x = mapping_->minLGridX(); x <= mapping_->maxLGridX(); x++) {
            ...
            for (int32 i = -1; i <= 1; i++) {
                for (int32 j = -1; j <= 1; j++) {
                    ...
                    pChunk[ci] = ChunkManager::instance().findChunkByName(
                      chunkIdentifier[ci], mapping_);
                    ++ci;
                }
            }
            ...
            EditorChunkTerrain* pEct =
              (EditorChunkTerrain*)EditorChunkTerrainCache::instance(*pChunk[4])
                .pTerrain();
            MF_ASSERT(pEct);
            ...
            BW::string                       resourceName =
              mapping_->path() + chunkIdentifier[4] + ".cdata";
...
void WorldManager::createSpaceImage()
{
    ...
    dlg.setSpaceInfo(currentSpace_,
                     0,
                     0,
    ...
    if (result) {
        if (!canClose(LocaliseUTF8(
              L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/TAKEPHOTOS"))) {
            return;
        }
        BW::string targetPath =
          BWResource::resolveFilename(mapping_->path()) + "space_viewer/";
...
void WorldManager::createSpaceImageInternal(
  const BW::string&              targetPath,
  const TakePhotoDlg::PhotoInfo& photoInfo)
{
    ...
    float steps = 1.0f * (mapping_->maxLGridY() - mapping_->minLGridY() + 1) *
                  (mapping_->maxLGridX() - mapping_->minLGridX() + 1);
    ...
    ScopedProgressBar helper(2);
    {
        ProgressBarTask createSpaceImageProgress(
          LocaliseUTF8(
            L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/CREATING_SPACE_IMAGE"),
          steps,
          true);
        NearbyChunkLoader ncManager(mapping_->minLGridX(),
                                    mapping_->maxLGridX(),
                                    mapping_->minLGridY(),
                                    mapping_->maxLGridY());

        for (int32 z = mapping_->minLGridY();
             z <= mapping_->maxLGridY() &&
             !createSpaceImageProgress.isCancelled();
             ++z) {
            if (Moo::rc().device()->TestCooperativeLevel() != D3D_OK) {
                ERROR_MSG(
                  "Device is lost, regenerating thumbnails has been stopped\n");
                return;
            }

            for (int32 x = mapping_->minLGridX();
                 x <= mapping_->maxLGridX() &&
                 !createSpaceImageProgress.isCancelled();
                 ++x) {
                BW::string chunkName;
                ...
                if (!this->takePhotoOfChunk(chunkName,
                                            targetPath,
                                            photoPosX,
                                            photoPosY,
                                            pixelsPerMetre,
    ...
    if (!cancelled) {
        if (photoInfo.mode == TakePhotoDlg::LEVEL_MAP) {
            result =
              WorldEditorApp::instance().pythonAdapter()->createSpaceLevelImage(
                targetPath,
                imgSize,
                mapping_->maxLGridY() - mapping_->minLGridY() + 1,
                mapping_->maxLGridX() - mapping_->minLGridX() + 1,
            ...
            result =
              WorldEditorApp::instance().pythonAdapter()->createSpaceSingleMap(
                targetPath,
                imgSize,
                mapping_->maxLGridX() - mapping_->minLGridX() + 1,
                mapping_->maxLGridY() - mapping_->minLGridY() + 1,
                photoInfo.spaceMapWidth,
                photoInfo.spaceMapHeight);
        }
    }

    if (result) {
        this->addCommentaryMsg(LocaliseUTF8(
          L"WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK/SPACE_IMAGE_CREATED",
          targetPath));
    } else {
        this->addCommentaryMsg(
          LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK/"
                       L"SPACE_IMAGE_CREAT_FAILED"));
    }

    // DEBUG_MSG("create space image finished, time spent: %d\n", (timestamp() -
    // startTime) * 1000 / stampsPerSecond());
}
...
void WorldManager::regenerateThumbnailsOffline()
{
    ...
    float steps = (mapping_->maxLGridY() - mapping_->minLGridY() + 1.0f) *
                  (mapping_->maxLGridX() - mapping_->minLGridX() + 1.0f);

    ScopedProgressBar progressTaskHelper(5);
    ProgressBarTask   thumbProgress(
      LocaliseUTF8(
        L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/REGENERATING_THUMBNAILS"),
      steps,
      true);

    for (int32 z = mapping_->minLGridY();
         z <= mapping_->maxLGridY() && !thumbProgress.isCancelled();
         ++z) {
        if (Moo::rc().device()->TestCooperativeLevel() != D3D_OK) {
            ERROR_MSG(
              "Device is lost, regenerating thumbnails has been stopped\n");
        ...
        for (int32 x = mapping_->minLGridX();
             x <= mapping_->maxLGridX() && !thumbProgress.isCancelled();
             ++x) {
            BW::string chunkName;
            chunkID(chunkName, (int16)x, (int16)z);
...
void WorldManager::convertSpaceToZip()
{
    ...
    if (!canClose(
          LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/CONVERT")))
        return;
    ...
    ProgressBarTask zipProgress(
      LocaliseUTF8(
        L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/CONVERTING_TO_ZIP"),
      (float)cdataFiles.size(),
      false);
    for (BW::vector<BW::wstring>::const_iterator it = cdataFiles.begin();
         it != cdataFiles.end();
         ++it) {
        BW::string nfilename;
...
void WorldManager::regenerateLODsOffline()
{
    ...
    ProgressBarTask* thumbProgress = new ProgressBarTask(
      LocaliseUTF8(
        L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/REGENERATING_LODS"),
      steps,
      true);
    ...
    for (int32 z = mapping_->minLGridY();
        ...
        for (int32 x = mapping_->minLGridX();
            ...
            Chunk* chunk = ChunkManager::instance().findChunkByName(
              chunkName, this->geometryMapping());
            ...
            if (chunk != NULL) {
                ...
                // Force to memory:
                if (!inMemory) {
                    ChunkManager::instance().loadChunkNow(chunkName,
                                                          geometryMapping());
...
bool WorldManager::saveChangedFiles()
{
    ...
    // Save terrain chunks
    {
        ProgressBarTask terrainTask(
          LocaliseUTF8(
            L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/SAVE_TERRAIN"),
          0,
          false);
    ...
    if (pDS) {
        ...
        // Personality script.  Allow scripts to save any files they've
        // modified, and allow them to save the space.settings file too.
    ...
    {
        ProgressBarTask thumbnailTask(
          LocaliseUTF8(
            L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/SAVE_THUMBNAIL_DATA"),
          100,
          false);
    ...
    if (pEct) {
        ...
        for (size_t i = 0; i < b.numberTextureLayers(); ++i) {
            ...
            if (!TerrainUtils::sourceTextureHasAlpha(ttl.textureName())) {
                BW::string warningMsg = LocaliseUTF8(
        ...
        if (pChunk->isOutsideChunk() &&
            !terrainFormatStorage_.hasTerrainInfo()) {
...
void WorldManager::onUnloadChunk(Chunk* pChunk)
{
    ...
    for (ChunkBoundaries::const_iterator bit = pChunk->joints().begin();
        ...
        for (uint i = 0; i < (*bit)->boundPortals_.size(); i++) {
            ...
            if (pPortal->hasChunk()) {
                //              DEBUG_MSG( "WorldManager::onUnloadChunk( %s ):
                //              Still bound\n",
                //                  pChunk->identifier().c_str() );
                return;
            }
        ...
        if (ptraceback != NULL) {
            ...
            if (pModule != NULL) {
                PyObject* formatFunction =
                  PyObject_GetAttrString(pModule, "format_exception");

                if (formatFunction != NULL) {
                    PyObject* list = Script::ask(
                      formatFunction,
                      Py_BuildValue("(OOO)", ptype, pvalue, ptraceback),
                      "WorldEditor",
                      false,
                      false);
                    ...
                    if (list != NULL) {
                        for (int i = 0; i < PyList_Size(list); ++i) {
                            stack +=
                              PyString_AS_STRING(PyList_GetItem(list, i));
                        }
                        Py_DECREF(list);
                    }
                }
...
void WorldManager::quickSave()
{
    ...
    Commentary::instance().addMsg(
      Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/QUICK_SAVING"), 1);
...
void updateDirtyChunks(WorldManager::ChunkSet& chunks,
                       const DirtyChunkList&   dcl)
{
    for (DirtyChunkList::const_iterator iter = dcl.begin(); iter != dcl.end();
         ++iter) {
...
void WorldManager::tickSavingChunks()
{
    ...
    if (this->isMemoryLow(true)) {
        UndoRedo::instance().clear();
        INFO_MSG("Process Data: Memory is running low, clearing the Undo/Redo "
                 "stack.\n");
    }
...
void WorldManager::save()
{
    ...
    if (Options::getOptionBool("fullSave/warnUndoOnProcessData", true)) {
        ...
        if (result == IDOK) {
            ...
            Options::setOptionBool("fullSave/warnUndoOnProcessData",
                                   !undoWarn.dontRepeat());
    ...
    Commentary::instance().addMsg(
      Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/SAVING"), 1);
    ...
    {
        BW::string syncCleanList = LocaliseUTF8(
          L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/SYNC_CLEAN_LIST");
    ...
    BW::string recalcShadow =
      LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/RECALC_SHADOW");
    float            taskTotal    = float(dirtyChunks.size());
    float            taskProgress = 0.f;
    ProgressBarTask* savingTask =
      new ProgressBarTask(recalcShadow, taskTotal, true);
    ...
    CpuInfo cpuInfo;
    this->startThreads("WE Saving Thread", cpuInfo.numberOfSystemCores());
    disableClose();
    ...
    // clear the pendingChangedChunks_ list as chunks may have been added to it
    // during processing
    {
        SimpleMutexHolder smh(changeMutex_);
        pendingChangedChunks_.clear();
    ...
    if (errors) {
        if (mainFrame) {
            MessageBox(*mainFrame,
                       Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/"
                                L"FULL_SAVE_ERROR_TEXT"),
                       Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/"
                                L"FULL_SAVE_ERROR_TITLE"),
                       MB_ICONERROR);
        }
        ...
        Commentary::instance().addMsg(
          Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/SAVE_COMPLETE"),
          1);
    }
...
void WorldManager::setPlayerPreviewMode(bool enable)
{
    ...
    if (enable) {
        GUI::ItemPtr hideOrtho =
          GUI::Manager::instance()("/MainToolbar/Edit/ViewOrtho/HideOrthoMode");
...
bool WorldManager::touchAllChunks()
{
    ...
    if (!canClose(LocaliseUTF8(
          L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/INVALIDATE"))) {
        return false;
    }
    ...
    ProgressBarTask progressTask(
      LocaliseUTF8(
        L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/TOUCH_ALL_CHUNKS"),
      100,
      true);

    invalidateAllChunks(WorldManager::instance().geometryMapping(),
                        &progressTask);
...
void WorldManager::markChunks()
{
    ...
    for (BW::set<Chunk*>::iterator iter = EditorChunkCache::chunks_.begin();
         iter != EditorChunkCache::chunks_.end();
         ++iter)
        (*iter)->removable(true);
...
WorldManager::ScopedDeferSelectionReplacement::
                                         ...
                                         const ChunkItemPtr& itemToAdd)
{
    ...
    MF_ASSERT(
      std::find(selectedItems_.begin(), selectedItems_.end(), itemToAdd) ==
        selectedItems_.end() ||
      std::find(itemsToRemove_.begin(), itemsToRemove_.end(), itemToAdd) !=
        itemsToRemove_.end());
    // Check we haven't already tried to add the item to add
    MF_ASSERT(std::find(itemsToAdd_.begin(), itemsToAdd_.end(), itemToAdd) ==
              itemsToAdd_.end());
...
void WorldManager::flushSelectionReplacement()
{
    ...
    for (BW::vector<ChunkItemPtr>::const_iterator it = itemsToAdd_.begin();
         it != itemsToAdd_.end();
         ++it) {
        items.push_back(*it);
    }
    ...
    if (addUndoBarrier_) {
        ...
        UndoRedo::instance().barrier(LocaliseUTF8(L"GIZMO/UNDO/MOVE_PROPERTY") +
                                       " " +
                                       items.front()->edDescription().str(),
                                     false);
...
void WorldManager::setSelection(const BW::vector<ChunkItemPtr>& items,
    ...
    if (pModule != NULL) {
        ...
        if (pScriptObject != NULL) {
            ChunkItemGroupPtr group = ChunkItemGroupPtr(
              new ChunkItemGroup(items), PyObjectPtr::STEAL_REFERENCE);
...
void WorldManager::getSelection()
{
    ...
    if (pModule != NULL) {
        ...
        if (pScriptObject != NULL) {
            ...
            Script::call(PyObject_GetAttrString(pScriptObject, "getSelection"),
...
ChunkItem* WorldManager::getDrawSelectionItem(DWORD index) const
{
    ...
    return index > 0 && index <= drawSelectionItems_.size()
             ? drawSelectionItems_[index - 1]
             : NULL;
}

/**
 *  return a loaded chunk, if wasForcedIntoMemory is specified, it represents
 * whether this chunk is loaded in this function or not.
 */
Chunk* WorldManager::getChunk(const BW::string& chunkID,
                              bool              forceIntoMemory,
                              bool*             wasForcedIntoMemory)
{
    BW_GUARD;

    Chunk* pChunk = ChunkManager::instance().findChunkByName(
      chunkID, this->geometryMapping());
    if (!pChunk) {
        return NULL;
    }
    ...
    if (!inMemory) {
        ChunkManager::instance().loadChunkNow(chunkID, this->geometryMapping());
        ChunkManager::instance().checkLoadingChunks();
                                   ...
                                   bool              forceIntoMemory)
{
    ...
    Chunk* pChunk =
      this->getChunk(chunkID, forceIntoMemory, &wasForcedIntoMemory);
    if (pChunk == NULL) {
        return false;
    }
...
bool WorldManager::recalcThumbnail(Chunk* pChunk)
{
    ...
    return EditorChunkThumbnailCache::instance(*pChunk).recalc(this, *this);
}

/**
 *  Generate image of each chunk.
 */
bool WorldManager::takePhotoOfChunk(const BW::string& chunkID,
...
bool WorldManager::snapPosition(Vector3& v)
{
    ...
    if (terrainSnapsEnabled())
        ...
        if (selectedItems_.size() > 1)
            startPosition =
              v - (Moo::rc().invView().applyToOrigin().length() * worldRay());
        bool    hitObstacle = false;
        Vector3 newV        = Snap::toObstacle(
          startPosition, worldRay(), false, getMaxFarPlane(), &hitObstacle);
        if (!hitObstacle) {
            v = origPosition;
            return false;
        } else {
...
void WorldManager::addReadOnlyBlock(const Matrix&                transform,
...
void WorldManager::showReadOnlyRegion(const BoundingBox& bbox,
...
void WorldManager::showFrozenRegion(const BoundingBox& bbox,
                                    const Matrix&      boxTransform)
{
    BW_GUARD;
...
bool WorldManager::isBoundingBoxInWriteableChunk(const BoundingBox& box,
                                                 const Vector3& offset) const
{
    BW_GUARD;

    return EditorChunk::outsideChunksWriteableInSpace(
      BoundingBox(box.minBounds() + offset, box.maxBounds() + offset));
}
...
void WorldManager::reloadAllChunks(bool askBeforeProceed)
{
    ...
    if (askBeforeProceed) {
        if (!canClose(LocaliseUTF8(
              L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/RELOAD"))) {
            return;
        }
    }
    ...
    currentSpace_ =
      LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/EMPTY");
                                  ...
                                  va_list              argPtr)
{
    ...
    if (DebugFilter::shouldAccept(messagePriority) &&
        ...
        if (isPythonError) {
            ...
            strncat(buf,
                    LocaliseUTF8(
                      L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/SEE_MSG_PANEL")
                      .c_str(),
...
void WorldManager::setTimeFromOptions()
{
    this->timeOfDay()->gameTime(
      float(Options::getOptionInt("graphics/timeofday",
...
void WorldManager::updateSelection(const BW::vector<ChunkItemPtr>& items)
{
    ...
    for (uint i = 0; i < items.size(); i++) {
        ...
        if (std::find(selectedItems_.begin(), selectedItems_.end(), item) ==
            selectedItems_.end()) {
    ...
    Script::call(PyObject_GetAttrString(scriptObject, "setSelection"),
                 Py_BuildValue("(Oi)", update, int(0)),
                 "WorldEditor");
}
    ...
    if (!rebuiltLodTexture && chunk != NULL) {
        for (int i = 0; i < ChunkCache::cacheNum(); ++i) {
            if (ChunkCache* cc = chunk->cache(i)) {
                cc->onChunkChanged(InvalidateFlags::FLAG_TERRAIN_LOD |
...
void WorldManager::chunkTexturesContextMenu(Chunk* chunk)
{
    ...
    if (PageTerrainTexture::instance() != NULL) {
        ...
        canEditProjections =
          PageTerrainTexture::instance()->canEditProjections();
    }
    ...
    menu.addItem(
      Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/TRACK_CURSOR",
               trackCursor),
      toggleTrackCursorCmd);
    menu.addSeparator();

    const int chunkTexturesCmd = 2000;
    if (chunk != NULL) {
        menu.addItem(
          Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/MANAGE_TEXTURE"),
          chunkTexturesCmd);
    }

    const int clearSelectedCmd = 3000;
    if (!PageChunkTexture::trackCursor() &&
        !PageChunkTexture::chunk().empty()) {
        menu.addItem(
          Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/DESELECT_CHUNK"),
          clearSelectedCmd);
    }
    menu.addSeparator();
    ...
    if (chunk != NULL) {
        ...
        size_t maxLayers =
          std::min((size_t)1000, layers.size()); // clip to 1000 texture layers
    ...
    if (chunk != NULL && canEditProjections) {
        menu.startSubmenu(Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/"
                                   L"SELECT_TEXTURE_AND_PROJ_FOR_MASK"));
        size_t maxLayers =
          std::min((size_t)1000, layers.size()); // clip to 1000 texture layers
        for (size_t i = 0; i < maxLayers; ++i) {
            // Do not include zero-strength layers.  Note we break instead of
            ...
            bw_utf8tow(
              BWResource::getFilename(layers[i].textureName_).to_string(),
              texName);
            menu.addItem(texName,
                         static_cast<int>(selTexAndProjForMaskCmd + i));
        }
        menu.endSubmenu();
    }
    ...
    if (result == toggleTrackCursorCmd) {
        ...
        if (chunk != NULL && PageTerrainTexture::instance() != NULL) {
            ...
            PageTerrainTexture::instance()->selectTextureAtPoint(cursorPos,
                                                                 layerIdx);
        }
        ...
        size_t layerIdx = layers[idx].layerIdx_;
        PageTerrainTexture::instance()->selectTextureMaskAtPoint(
          cursorPos, layerIdx, false, false);
    } else if (selTexAndProjForMaskCmd <= result &&
               result < selTexAndProjForMaskCmd + 1000) {
        size_t idx      = result - selTexAndProjForMaskCmd;
        size_t layerIdx = layers[idx].layerIdx_;
        PageTerrainTexture::instance()->selectTextureMaskAtPoint(
          cursorPos, layerIdx, true, false);
    } else if (editProjectionCmd <= result &&
               result < editProjectionCmd + 1000) {
        if (chunk != NULL && PageTerrainTexture::instance() != NULL) {
            size_t idx      = result - editProjectionCmd;
            size_t layerIdx = layers[idx].layerIdx_;
            PageTerrainTexture::instance()->editProjectionAtPoint(cursorPos,
                                                                  layerIdx);
        }
  ...
  const SceneBrowser::ItemList& selection)
{
    ...
    UndoRedo::instance().add(new SelectionOperation(selectedItems_, items));
    ...
    if (allow) {
        ...
        if (s_minimumAllowTimeSliceMS_ > 0) {
            ...
            if (delta < minimumAllowedStamps) {
                uint64 targetStamps =
                  timestamp() + (minimumAllowedStamps - delta);
                while (timestamp() < targetStamps &&
                       WorldManager::instance().numBgTasksLeft()) {
                    Sleep(0);
                }
            }
        }
        ...
        if (s_timeSlicePauseWarnThresholdMS_ > 0) {
            ...
            if (timeWaited > warningThreshold) {
                WARNING_MSG(
                  "WorldManager::allowChunkProcessorThreadsToRun: Waited "
 ...
 *
 */
/* virtual */ void WorldManager::setStaticThreadData(
  const BackgroundTaskPtr pTask)
{
...
bool WorldManager::isScriptDirty() const
{
    ...
    return ret &&
           ret.isTrue(ScriptErrorPrint("WorldManager isScriptDirty retval"));
};
...
bool WorldManager::isDirty() const
{
    ...
    return (changedTerrain || changedScenery || changedThumbnail ||
            changedEnvironment_ || isScriptDirty());
}
...
bool WorldManager::canClose(const BW::string& action)
{
    ...
    if (isDirty()) {
        if (!CStdMf::checkUnattended()) {
            ...
            BW::string::iterator lastChar = std::remove(
              actionNoShortcut.begin(), actionNoShortcut.end(), '&');
            ...
            MsgBox mb(
              Localise(
                L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/CHANGED_FILES_TITLE",
                actionNoShortcut),
              Localise(
              ...
              Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/WITHOUT_SAVE",
                       action),
            ...
            if (result == 3)
                ...
                GUI::ItemPtr quickSave =
                  GUI::Manager::instance()("/MainToolbar/File/QuickSave");
                quickSave->act();
                if (saveFailed_)
                    return false;
            } else if (result == 1) {
                GUI::ItemPtr save =
                  GUI::Manager::instance()("/MainToolbar/File/Save");
                save->act();
                if (saveFailed_)
                    return false;
            } else if (result == 2) {
...
PY_MODULE_STATIC_METHOD(WorldManager,
...
PyObject* WorldManager::py_repairTerrain(PyObject* args)
{
    ...
    for (int i = dirMap->minGridX(); i <= dirMap->maxGridX(); ++i) {
        for (int j = dirMap->minGridY(); j <= dirMap->maxGridY(); ++j) {
            BW::string prefix =
              dirMap->path() + dirMap->outsideChunkIdentifier(i, j);
            DataSectionPtr cs = BWResource::openSection(prefix + ".chunk");
            DataSectionPtr ds = BWResource::openSection(prefix + ".cdata");
            if (cs && ds && !cs->openSection("terrain") &&
                ds->openSection("terrain")) {
                cs->newSection("terrain")->writeString(
                  "resource",
 ...
 *
 *  @return If the farPlane parameter was not supplied, then this function
 * returns the currently set far plane distance, otherwise returns the new far
 * plane distance.
 */
...
PyObject* WorldManager::py_showEditorRenderables(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(args, "b", &showEditorRenderables)) {
        PyErr_SetString(PyExc_TypeError,
                        "WorldEditor.showEditorRenderables: "
                        "Argument parsing error: Expected a boolean");
        return NULL;
    }
...
SelectionOperation::SelectionOperation(const BW::vector<ChunkItemPtr>& before,
                                       const BW::vector<ChunkItemPtr>& after)
  ...
  , after_(after)
{
    ...
    for (BW::vector<ChunkItemPtr>::iterator iter = before_.begin();
         iter != before_.end();
         ++iter)
        addChunk((*iter)->chunk());
    for (BW::vector<ChunkItemPtr>::iterator iter = after_.begin();
         iter != after_.end();
         ++iter)
        addChunk((*iter)->chunk());
}
...
void SelectionOperation::undo()
{
    ...
    for (BW::vector<ChunkItemPtr>::const_iterator it = before_.begin();
        ...
        if (itemPtr->chunk() == NULL && itemPtr->edIsVLO()) {
            ChunkVLO* chunkVLO = dynamic_cast<ChunkVLO*>(itemPtr.get());
            if (chunkVLO != NULL && chunkVLO->object()) {
...
PyObject* WorldManager::py_revealSelection(PyObject* args)
{
    ...
    if (PyArg_ParseTuple(args, "O", &p)) {
        if (ChunkItemRevealer::Check(p)) {
            ...
            BW::vector<ChunkItemPtr> selectedItems =
              s_pInstance->selectedItems_;
            ...
            bool different =
              selectedItems.size() != s_pInstance->selectedItems_.size();
            if (!different) {
                std::sort(selectedItems.begin(), selectedItems.end());
                std::sort(s_pInstance->selectedItems_.begin(),
                          s_pInstance->selectedItems_.end());
                different = (selectedItems != s_pInstance->selectedItems_);
            }
            if (different) {
                UndoRedo::instance().add(new SelectionOperation(
                  selectedItems, s_pInstance->selectedItems_));
...
PyObject* WorldManager::py_isChunkSelected(PyObject* args)
{
    ...
    return Script::getData(
      WorldManager::instance().isChunkSelected(true /* force now */));
}
 ...
 *  @return Returns a group of the selected chunk items in the form of a
 * ChunkItemGroup object.
 */
 ...
 *
 *  @return Returns True (1) if the cursor is over the graphics window, False
 * (0) otherwise.
 *
 *  Code Example:
 *  @{
...
PyObject* WorldManager::py_rightClick(PyObject* args)
{
    ...
    if (PyArg_ParseTuple(args, "O", &p)) {
        if (ChunkItemRevealer::Check(p)) {
            ...
            if (ToolManager::instance().tool() &&
                ToolManager::instance().tool()->locator()) {
                cursorPos = ToolManager::instance()
                              .tool()
                              ->locator()
                              ->transform()
                              .applyToOrigin();
            ...
            if (items.size() == 1) {
                ...
                if (std::find(curSel.begin(), curSel.end(), item) !=
                    curSel.end()) {
                ...
                int numCmds =
                  PopupMenuHelper::buildCommandMenu(item, commandBaseId, menu);
                ...
                if (item->edCanAddSelection()) {
                    ...
                    menu.addItem(
                      Localise(
                        L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/RCLK_HIDE"),
                      idHide);

                    menu.addItem(Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/"
                                          L"BIG_BANG/RCLK_UNHIDE_ALL_FILTER"),
                                 idUnhideAllFilter);

                    menu.addItem(Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/"
                                          L"BIG_BANG/RCLK_UNHIDE_ALL"),
                                 idUnhideAll);

                    menu.addSeparator();

                    menu.addItem(Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/"
                                          L"BIG_BANG/RCLK_FREEZE"),
                                 idFreeze);

                    menu.addItem(Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/"
                                          L"BIG_BANG/RCLK_UNFREEZE_ALL_FILTER"),
                                 idUnfreezeAllFilter);

                    menu.addItem(Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/"
                                          L"BIG_BANG/RCLK_UNFREEZE_ALL"),
                                 idUnfreezeAll);

                    menu.addSeparator();

                    menu.addItem(Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/"
                                          L"BIG_BANG/SEE_IN_SCENE_BROWSER"),
                                 idSeeInSceneBrowser);

                    menu.addItem(
                      Localise(
                        L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/PROPERTIES"),
                      idProperties);
                }
            } else {
                multiSelectItems = &curSel;
            ...
            size_t numLightVisItems =
              LightContainerDebugger::instance()->numItems();
            if (!items.empty() || numLightVisItems > 0) {
                menu.addSeparator();
                if (!items.empty()) {
                    menu.addItem(Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/"
                                          L"BIG_BANG/VIS_LIGHT_CONTRIBUTION"),
                                 idVisLights);
                }
                ...
                if (numLightVisItems > 0) {
                    menu.addItem(
                      Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/"
                               L"CLEAR_LIGHT_CONTRIBUTION_VIS"),
                      idClearVisLights);
                }
            }
            ...
            if (!menu.empty()) {
                ...
                // Remove input focus so that keyup events get forced through
                // now.
                WorldEditorApp::instance().mfApp()->handleSetFocus(false);

                int result =
                  menu.doModal(WorldManager::instance().hwndGraphics());

                ShowCursor(FALSE);

                // Consume input, otherwise input given while the popup menu was
                // visible would be sent to the main window
                WorldEditorApp::instance().mfApp()->consumeInput();

                if (result) {
                    if (result == idProperties) {
                        WorldManager::instance().setSelection(
                          *multiSelectItems);
                        ...
                        ChunkItemPlacer::hideChunkItems(
                          *multiSelectItems, true, false);
                    } else if (result == idUnhideAllFilter ||
                               result == idUnhideAll) {
                        EditorChunkItem::updateSelectionMark();
                        ChunkItemRevealer::ChunkItems allItems;
                        ChunkItemRevealer::ChunkItems intermediateItems;
                        ...
                        if (result == idUnhideAll) {
                            ...
                            filterSetter.start(
                              SelectionFilter::SELECT_ANY, "", "");
                        }
                        allChunkItems(intermediateItems);
                        filterSelectableItems(
                          intermediateItems, true, true, allItems);
                        ChunkItemPlacer::hideChunkItems(allItems, false, true);
                    } else if (result == idFreeze) {
                        ChunkItemPlacer::freezeChunkItems(
                          *multiSelectItems, true, false);
                    } else if (result == idUnfreezeAllFilter ||
                               result == idUnfreezeAll) {
                        EditorChunkItem::updateSelectionMark();
                        ChunkItemRevealer::ChunkItems allItems;
                        ChunkItemRevealer::ChunkItems intermediateItems;
                        ScopedSelectionFilter         filterSetter;
 ...
 *  @components{ worldeditor }
 *
 *  This function gets the chunk identifier that the camera currently resides
 * in.
 *
 *  @return String  The identifier of the camera chunk, empty string if not
 * valid.
 */
 ...
 *  identifier string (as used by the chunk filenames). If the checkBounds
 * parameter is set to True, then this will return an empty string if the given
 * coordinates fall outside the space boundaries.
 *
 *  @param  Vector3     Position in the space to test against.
 *  @param  Boolean     Whether or not to check if the point falls within space
 * bounds.
 *
 *  @return String      The identifier of the outside chunk containing the given
 * point.
 */
...
void WEChunkFinder::operator()(ToolPtr tool, float buffer)
{
    ...
    if (tool->locator()) {
        ...
        EditorChunk::findOutsideChunks(BoundingBox(start, end),
                                       tool->relevantChunks());
...
bool WorldManager::changeSpace(const BW::string& space, bool reload)
{
    ...
    if (!reload) {
        ...
        spaceLock_ = CreateFile(wspace.c_str(),
        ...
        if (spaceLock_ == INVALID_HANDLE_VALUE) {
            ...
            MsgBox mb(
              Localise(
                L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/OPEN_SPACE_TITLE"),
              Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/"
                       L"UNABLE_TO_OPEN_SPACE",
                       space),
    ...
    if (!reload) {
        ...
        if (conn_.enabled()) {
            ...
            if (conn_.changeSpace(space))
                WaitDlg::overwriteTemp(
                  LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/"
                               L"CONNECT_TO_BWLOCKD_DONE",
                               conn_.host()),
                  500);
            else
                WaitDlg::overwriteTemp(
                  LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/"
                               L"CONNECT_TO_BWLOCKD_FAILED",
                               conn_.host()),
                  500);
        } else
    ...
    mapping_ =
      chunkSpace->addMapping(SpaceEntryID(), (float*)nonConstIdentity, space);
    if (!mapping_) {
        ChunkManager::instance().switchToSyncMode(false);
        renderDisabled_ = false;
        return false;
    ...
    DataSectionPtr localSpaceSettings =
      BWResource::openSection(space + "/" + SPACE_LOCAL_SETTING_FILE_NAME);
    if (!reload) {
        Vector3 dir(0.f, 0.f, 0.f), pos(0.f, 2.f, 0.f);
        if (localSpaceSettings) {
            pos = localSpaceSettings->readVector3("startPosition",
                                                  Vector3(0.f, 2.f, 0.f));
    ...
    AfxGetMainWnd()->SetWindowText(
      (wspace + Localise(L"WORLDEDITOR/APPLICATION_NAME")).c_str());
    ...
    if (terrainSettings == NULL ||
        (terrainSettings != NULL &&
         terrainSettings->readInt("version", 0) == 2)) {
        ...
        Terrain::TerrainSettingsPtr pTempSettings =
          new Terrain::TerrainSettings;
        pTempSettings->initDefaults(
          spaceSettings->readFloat("chunkSize", DEFAULT_GRID_RESOLUTION));
        ...
        if (terrainVersion == Terrain::TerrainSettings::TERRAIN2_VERSION) {
            ...
            spaceSettings->save();
        } else if (terrainVersion ==
                   Terrain::TerrainSettings::TERRAIN1_VERSION) {
            ERROR_MSG("Couldn't create space.settings/terrain section:"
                      "terrain version %d is no longer supported.\n",
                      terrainVersion);
        } else {
            ERROR_MSG("Couldn't create space.settings/terrain section: unknown "
                      "terrain version.\n");
        }
    }
    ...
    if (spaceSettings) {
        ...
        SpaceInformation spaceInfo(
          space, GridCoord(minX, minY), spaceWidth, spaceHeight);
    ...
    localSpaceSettings =
      BWResource::openSection(Options::getOptionString("space/mru0") + '/' +
...
bool WorldManager::handleGUIAction(GUI::ItemPtr item)
{
    ...
    if (action == "changeSpace")
        return changeSpace(item);
    else if (action == "newSpace")
        return newSpace(item);
    else if (action == "editSpace")
        return editSpace(item);
    else if (action == "recreateSpace")
        return recreateSpace(item);
    else if (action == "recentSpace")
        return recentSpace(item);
    else if (action == "clearUndoRedoHistory")
        return clearUndoRedoHistory(item);
    else if (action == "doExternalEditor")
        return doExternalEditor(item);
    else if (action == "doReloadAllTextures")
        return doReloadAllTextures(item);
    else if (action == "doReloadAllChunks")
        return doReloadAllChunks(item);
    else if (action == "doExit")
        return doExit(item);
    else if (action == "setLanguage")
        return setLanguage(item);
    else if (action == "recalcCurrentChunk")
        return recalcCurrentChunk(item);
        ...
        if (!foundSpace) {
            ...
            MsgBox mb(
              Localise(
                L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/OPEN_SPACE_TITLE"),
              Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/"
                       L"BAD_SPACE_VERSION_TEXT",
                       space),
              Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/OK"));

            MainFrame* mainFrame =
              static_cast<MainFrame*>(WorldEditorApp::instance().mainWnd());
            mb.doModal(mainFrame->m_hWnd);
...
bool WorldManager::editSpace(GUI::ItemPtr item)
{
    ...
    if (!canClose(
          LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/EDIT_SPACE")))
        return false;
...
bool WorldManager::recreateSpace(GUI::ItemPtr item)
{
    ...
    if (!canClose(LocaliseUTF8(
          L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/RECREATE_SPACE/ACTION")))
        return false;
...
void WorldManager::updateRecentFile()
{
    ...
    GUI::ItemPtr recentFiles =
      GUI::Manager::instance()("/MainMenu/File/RecentFiles");
    if (recentFiles) {
...
unsigned int WorldManager::handleGUIUpdate(GUI::ItemPtr item)
{
    ...
    if (updater == "updateRecreateSpace")
        return updateRecreateSpace(item);
    else if (updater == "updateUndo")
        return updateUndo(item);
    else if (updater == "updateRedo")
        return updateRedo(item);
    else if (updater == "updateExternalEditor")
        return updateExternalEditor(item);
...
{
    return selectedItems_.size() == 1 &&
           !selectedItems_[0]->edCommand("").empty();
}
 ...
 */
void WorldManager::drawMissingTextureLODs(bool complainIfNotDone,
                                          bool doAll,
                                          bool markDirty,
                                          bool progress)
{
...
void WorldManager::showBusyCursor()
{
    ...
    if (waitCursor_ || !loadedChunk) {
        WorldManager::instance().setCursor(
          loadedChunk ? ::LoadCursor(NULL, IDC_ARROW)
        ...
        if (pendingChangedChunks_.size()) {
            ...
            try {
                for (BW::list<PendingChangedChunk>::iterator it = tmp.begin();
                     ...
                     it++) {
                    changedChunk(
                      (*it).pChunk_, (*it).flags_, *(*it).pChangedItem_);
                }
            } catch (...) {
                ;
            }
...
bool WorldManager::createBlankChunkData(BW::string& retPath, bool withTerrain)
{
    ...
    Terrain::EditorTerrainBlock2* pTemplateTerrainBlock =
      new Terrain::EditorTerrainBlock2(pTerrainSetting);
    BW::string error;
    ...
    int32 idx = static_cast<int32>(
      pTemplateTerrainBlock->insertTextureLayer(blendSize, blendSize));
    if (idx == -1) {
        ERROR_MSG("Couldn't create default texture for new space.\n");
        return false;
    } else {
        Terrain::TerrainTextureLayer& layer =
          pTemplateTerrainBlock->textureLayer(idx);
 ...
 */
bool WorldManager::expandSpace(int  westCnt,
                               int  eastCnt,
                               int  northCnt,
                               int  southCnt,
                               bool chunkWithTerrain)
{
    ...
    if (curTerrainVer < Terrain::TerrainSettings::TERRAIN2_VERSION) {
        Localise(
          ...
          MessageBox(*WorldEditorApp::instance().mainWnd(),
                     Localise(L"WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK/"
                              L"EXPAND_SPACE_NOT_SUPPORTED"),
                     Localise(L"WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK/"
                              L"EXPAND_SPACE_NOT_SUPPORTED_TITLE"),
                     MB_ICONINFORMATION | MB_OK);
        return false;
    }

    if (westCnt < 0 || eastCnt < 0 || northCnt < 0 || southCnt < 0 ||
        (mapping_->maxGridX() - mapping_->minGridX() + westCnt + eastCnt + 1 >=
         MAX_CHUNK_SIZE) ||
        (mapping_->maxGridY() - mapping_->minGridY() + northCnt + southCnt +
           1 >=
         MAX_CHUNK_SIZE)) {
        this->addCommentaryMsg(
          LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK/"
                       L"EXPAND_SPACE_SIZE_ERROR"));
        return false;
    }
    ...
    if (!this->createBlankChunkData(cdataFilePath, chunkWithTerrain)) {
        this->addCommentaryMsg(
          LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK/"
                       L"EXPAND_SPACE_CDATA_ERROR"));
        return false;
    }

    WorldEditorApp::instance().pythonAdapter()->expandSpace(spacePath,
                                                            cdataFilePath,
                                                            westCnt,
                                                            eastCnt,
                                                            northCnt,
    ...
    this->addCommentaryMsg(LocaliseUTF8(
      L"WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK/SPACE_EXPANDED"));
...
void WorldManager::expandSpaceWithDialog()
{
    ...
    if (result) {
        ...
        this->expandSpace(info.west_,
                          info.east_,
                          info.north_,
                          info.south_,
                          info.includeTerrainInNewChunks_);
    }
}

/**
 * This function process messages in current message queue. But all mouse
 * events, keyboard events and menu events will be discarded. This is used for
 * preventing window from losing responding during some long time calculation
 */
