...
PlacementCtrlsDlg::PlacementCtrlsDlg(CWnd* pParent)
  ...
  , GUI::ActionMaker<PlacementCtrlsDlg>("newPlacementPreset",
                                        &PlacementCtrlsDlg::newPreset)
  , GUI::ActionMaker<PlacementCtrlsDlg, 1>("delPlacementPreset",
                                           &PlacementCtrlsDlg::delPreset)
  , GUI::UpdaterMaker<PlacementCtrlsDlg>(
      "presetEditableUpdater",
      &PlacementCtrlsDlg::presetEditableUpdater)
  , GUI::ActionMaker<PlacementCtrlsDlg, 2>("renamePlacementPreset",
                                           &PlacementCtrlsDlg::renamePreset)
  , updateEdits_(true)
  , linkIcon_(NULL)
  , resetIcon_(NULL)
{
...
void PlacementCtrlsDlg::savePreset()
{
    ...
    for (int j = 0; j < (int)sliders_.size(); ++j) {
        name.Format(L"min%s%c",
                    sliders_[j]->group_ == GROUP_ROTATION ? L"Rot" : L"Sca",
                    axis[j % 3]);
        preset->writeFloat(bw_wtoutf8(name.GetString()),
                           sliders_[j]->minEd_->GetValue());

        name.Format(L"max%s%c",
                    sliders_[j]->group_ == GROUP_ROTATION ? L"Rot" : L"Sca",
                    axis[j % 3]);
        preset->writeFloat(bw_wtoutf8(name.GetString()),
                           sliders_[j]->maxEd_->GetValue());
    }
...
bool PlacementCtrlsDlg::newPreset(GUI::ItemPtr item)
{
    ...
    DataSectionPtr presetsSection =
      PlacementPresets::instance()->presetsSection();
    if (!presetsSection)
        return false;
    ...
    preset->writeString("displayName",
                        bw_wtoutf8(newPlacementDlg_.newName_.GetString()));
...
bool PlacementCtrlsDlg::delPreset(GUI::ItemPtr item)
{
    ...
    if (MessageBox(
          Localise(L"WORLDEDITOR/GUI/PLACEMENT_CTRLS_DLG/DELETE_PRESET_TEXT"),
...
BOOL PlacementCtrlsDlg::OnInitDialog()
{
    ...
    sliders_.push_back(new SliderContainer(&sliderRandRotX_,
                                           &minRandRotX_,
                                           &maxRandRotX_,
                                           &resetRotX_,
                                           GROUP_ROTATION));
    sliders_.push_back(new SliderContainer(&sliderRandRotY_,
                                           &minRandRotY_,
                                           &maxRandRotY_,
                                           &resetRotY_,
                                           GROUP_ROTATION));
    sliders_.push_back(new SliderContainer(&sliderRandRotZ_,
                                           &minRandRotZ_,
                                           &maxRandRotZ_,
                                           &resetRotZ_,
                                           GROUP_ROTATION));

    sliders_.push_back(new SliderContainer(&sliderRandScaX_,
                                           &minRandScaX_,
                                           &maxRandScaX_,
                                           &resetScaX_,
                                           GROUP_SCALE));
    sliders_.push_back(new SliderContainer(&sliderRandScaY_,
                                           &minRandScaY_,
                                           &maxRandScaY_,
                                           &resetScaY_,
                                           GROUP_SCALE));
    sliders_.push_back(new SliderContainer(&sliderRandScaZ_,
                                           &minRandScaZ_,
                                           &maxRandScaZ_,
                                           &resetScaZ_,
                                           GROUP_SCALE));
    ...
    float scaMin = float(conf->readDouble("scaleRangeMin", 0.01) + 0.000000001);
    float scaMax = float(conf->readDouble("scaleRangeMax", 10.0) + 0.000000001);
    ...
    if (tbSection) {
        ...
        toolbar_.SetWindowPos(
          0, rect.right + 5, rect.top - 5, tbSize.cx, tbSize.cy, SWP_NOZORDER);
    } else
...
void PlacementCtrlsDlg::writeEditNum(controls::EditNumeric* edit,
                                     float                  num,
                                     int                    group)
{
    BW_GUARD;
...
void PlacementCtrlsDlg::syncBars(SliderContainerPtr slider)
{
    ...
    if ((slider->group_ == GROUP_ROTATION &&
        ...
        for (BW::vector<SliderContainerPtr>::iterator i = sliders_.begin();
             i != sliders_.end();
             ++i) {
            if ((*i)->group_ == slider->group_ && (*i) != slider) {
                if (max < min) {
                    std::swap(min, max);
                }
...
void PlacementCtrlsDlg::updateVisibility()
{
    ...
    linkRandSca_.EnableWindow(enable &&
...
void PlacementCtrlsDlg::OnSize(UINT nType, int cx, int cy)
{
    ...
    ScreenToClient(&ctrlRect);
    randRotGroup_.SetWindowPos(
      0, 0, 0, rect.Width(), ctrlRect.Height(), SWP_NOMOVE | SWP_NOZORDER);
    randScaGroup_.GetWindowRect(&ctrlRect);
    ScreenToClient(&ctrlRect);
    randScaGroup_.SetWindowPos(
      0, 0, 0, rect.Width(), ctrlRect.Height(), SWP_NOMOVE | SWP_NOZORDER);
    ...
    for (BW::vector<SliderContainerPtr>::iterator i = sliders_.begin();
        ...
        (*i)->maxEd_->SetWindowPos(
          0, maxEdPos, ctrlRect.top, 0, 0, SWP_NOSIZE | SWP_NOZORDER);

        (*i)->reset_->GetWindowRect(&ctrlRect);
        ScreenToClient(&ctrlRect);
        (*i)->reset_->SetWindowPos(0,
                                   rect.right - rightResetButDist_,
                                   ctrlRect.top,
                                   0,
                                   0,
...
afx_msg HBRUSH PlacementCtrlsDlg::OnCtlColor(CDC*  pDC,
                                             CWnd* pWnd,
                                             UINT  nCtlColor)
{
    BW_GUARD;
