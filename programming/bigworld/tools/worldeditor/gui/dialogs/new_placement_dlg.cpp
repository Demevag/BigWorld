...
void NewPlacementDlg::OnOK()
{
    ...
    if (!isValidName(str)) {
        MessageBox(
          Localise(L"WORLDEDITOR/GUI/NEW_PLACEMENT_DLG/BAD_PRESET_NAME_TEXT"),
          Localise(L"WORLDEDITOR/GUI/NEW_PLACEMENT_DLG/BAD_PRESET_NAME_TITLE"),
          MB_ICONERROR);
...
bool NewPlacementDlg::isValidName(CString const& str) const
{
    ...
    for (size_t i = 0; i < existingNames_.size(); ++i) {
        if (existingNames_[i] == s.GetBuffer() &&
