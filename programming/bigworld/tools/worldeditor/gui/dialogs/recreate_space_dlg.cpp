...
void RecreateSpaceDlg::setSpaceSettings(SpaceSettings* spaceSettings)
{
...
ON_CBN_SELCHANGE(IDC_SHADOWMAP_SIZE,
                 &RecreateSpaceDlg::OnCbnSelchangeShadowmapSize)
END_MESSAGE_MAP()

void RecreateSpaceDlg::initIntEdit(controls::EditNumeric& edit,
                                   int                    minVal,
                                   int                    maxVal,
                                   int                    val)
{
...
BOOL RecreateSpaceDlg::OnInitDialog()
{
    ...
    Terrain::TerrainSettingsPtr terrainSettings =
      hasSettings ? spaceSettings_->terrainSettings()
    ...
    if (hasSettings) {
        ...
        int gridMaxY;
        TerrainUtils::terrainSize(
          "", gridSize, gridMinX, gridMinY, gridMaxX, gridMaxY);
    ...
    space_.SetWindowText(
      bw_utf8tow(BWResource::getFilename(fullSpace).to_string()).c_str());
    spacePath_.SetWindowText(
      bw_utf8tow(BWResource::getFilePath(fullSpace)).c_str());
...
void RecreateSpaceDlg::OnBnClickedOk()
{
    ...
    SpaceRecreator recreator(
      bw_wtoutf8(spaceName), chunkSize, width, height, terrainInfo);
    ...
    if (!result) {
        MessageBox(bw_utf8tow(recreator.errorMessage()).c_str(),
...
void RecreateSpaceDlg::refreshWarnings()
{
    ...
    TerrainUtils::terrainSize(
      "", srcChunkSize, srcMinX, srcMinZ, srcMaxX, srcMaxZ);
    Terrain::TerrainSettingsPtr pSrcTerrain =
      WorldManager::instance().pTerrainSettings();
    float srcEditorHeightMapRes =
      pSrcTerrain->heightMapEditorSize() / srcChunkSize;
    ...
    if (editorHeightMapRes <= (srcEditorHeightMapRes / 2.f)) {
        warnings_.AddString(
          Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/RECREATE_SPACE/"
                   L"WARNING_HEIGHTMAP_RES_REDUCED"));
    }
    ...
    if (holeMapRes <= (srcHoleMapRes / 2.f)) {
        warnings_.AddString(
          Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/RECREATE_SPACE/"
                   L"WARNING_HOLEMAP_RES_REDUCED"));
    }

    if (blendMapRes <= (srcBlendMapRes / 2.f)) {
        warnings_.AddString(
          Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/RECREATE_SPACE/"
                   L"WARNING_BLENDMAP_RES_REDUCED"));
    }

    if (shadowMapRes <= (srcShadowMapRes / 2.f)) {
        warnings_.AddString(
          Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/RECREATE_SPACE/"
                   L"WARNING_SHADOWMAP_RES_REDUCED"));
    }
}
