...
class ChunkWatchControl : public CWnd
{
  public:
    enum DrawOptions
    {
        ...
        DRAW_ALL = DRAW_PROJECTVIEW | DRAW_CHUNKS | DRAW_GRID | DRAW_USERPOS |
                   DRAW_FRUSTUM | DRAW_UNLOADABLE | DRAW_WORKING
    };

    ChunkWatchControl();
    ~ChunkWatchControl();

    void onNewSpace(float chunkSize,
                    int32 minX,
                    int32 minZ,
                    int32 maxX,
                    int32 maxZ);
    ...
    bool getInfoText(CPoint const& pos,
                     BW::string&   chunkText,
