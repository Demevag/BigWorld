 ...
 */
void ChunkWatchControl::onNewSpace(float chunkSize,
                                   int32 minX,
                                   int32 minZ,
                                   int32 maxX,
                                   int32 maxZ)
...
/*afx_msg*/ void ChunkWatchControl::OnPaint()
{
    ...
    // Draw the details:
    if ((drawOptions_ & DRAW_PROJECTVIEW) != 0)
        drawProject(memDC_);
    if ((drawOptions_ & DRAW_CHUNKS) != 0)
        drawChunks(memDC_);
    if ((drawOptions_ & DRAW_UNLOADABLE) != 0)
        drawUnloadable(memDC_);
    if ((drawOptions_ & DRAW_GRID) != 0)
        drawGrid(memDC_);
    if ((drawOptions_ & DRAW_USERPOS) != 0)
        drawArrow(memDC_);
    if ((drawOptions_ & DRAW_FRUSTUM) != 0)
        drawFrustum(memDC_);

    // Draw the outsides:
    memDC_.Draw3dRect(extents_, // around the tiles itself
                      CLR_BORDER,
                      CLR_BORDER);
    memDC_.Draw3dRect(fullExtents,
...
/*afx_msg*/ void ChunkWatchControl::OnUpdateControl()
{
    ...
    if (lastUpdateTime_ != 0) {
        ...
        if ((now - lastUpdateTime_) / (double)stampsPerSecond() <
            UPDATE_DELTA) {
            return;
        }
    }
    ...
    view.invert();
    if (!similar(view, drawPos_, MATRICES_COMP_EPSILON)) {
        RedrawWindow();
    }
}
...
/*afx_msg*/ void ChunkWatchControl::OnLButtonUp(UINT /*flags*/, CPoint point)
{
    ...
    if (screenToWorld(point, &x, &z)) {
        ...
        float y =
          fheight + Options::getOptionInt("graphics/farclip", 500) / 10.0f;
 ...
 *  @param point	The coordinates of the mouse.
 */
/*afx_msg*/ void ChunkWatchControl::OnMouseMove(UINT /*flags*/,
                                                CPoint /*point*/)
{
    BW_GUARD;
 ...
 */
void ChunkWatchControl::enableDrawOptions(uint32 options,
                                          bool   update /*= true*/)
{
    BW_GUARD;
 ...
 *  @param update	If true then there is an immediate update.
 */
void ChunkWatchControl::disableDrawOptions(uint32 options,
                                           bool   update /*= true*/)
{
    BW_GUARD;
 ...
 */
bool ChunkWatchControl::getInfoText(CPoint const& pos,
                                    BW::string&   chunkText,
                                    BW::string&   posText) const
{
    BW_GUARD;
    ...
    if (screenToWorld(pos, &wx, &wz)) {
        ...
        if (chunk != NULL)
            ...
            chunkText = LocaliseUTF8(L"WORLDEDITOR/GUI/CONTROLS/"
                                     L"CHUNK_WATCH_CONTROL/NOT_LOADED_STRING");
        return true;
    } else {
        return false;
    }
...
void ChunkWatchControl::drawProject(CDC& dc)
{
    ...
    if (!projectDib_.isEmpty()) {
        if (!dibUpToDate || smallProjectDib_.width() != extents_.Width() ||
            ...
            projectDib_.draw(
              smallDC, 0, 0, extents_.Width(), extents_.Height());
...
void ChunkWatchControl::drawUnloadable(CDC& dc)
{
    ...
    for (uint32 z = 0; z < chunksHigh_; ++z) {
        ...
        int32 z1 = Math::lerp((int32)z,
                              (int32)0,
                              (int32)chunksHigh_,
                              extents_.bottom,
                              extents_.top);
        int32 z2 = Math::lerp((int32)z + 1,
                              (int32)0,
                              (int32)chunksHigh_,
                              extents_.bottom,
                              extents_.top);
        for (uint32 x = 0; x < chunksWide_; ++x) {
            ...
            int32 x1 = Math::lerp(
              x, (uint32)0, chunksWide_, extents_.left, extents_.right);
            int32 x2 = Math::lerp(
              x + 1, (uint32)0, chunksWide_, extents_.left, extents_.right);
...
void ChunkWatchControl::drawGrid(CDC& dc)
{
    ...
    // Draw a grid:
    if (chunksWide_ < TOO_MANY_GRID_LINES &&
        chunksHigh_ < TOO_MANY_GRID_LINES) {
        CPen  pen(PS_SOLID, 1, CLR_GRID);
        CPen* oldPen = dc.SelectObject(&pen);
        for (uint32 z = 1; z < chunksHigh_; ++z) {
            int sy = Math::lerp(
              z, (uint32)0, chunksHigh_, extents_.top, extents_.bottom);
 ...
 */
void ChunkWatchControl::worldToScreen(float* sx,
                                      float* sy,
                                      float  wx,
                                      float  wz) const
{
 ...
 */
bool ChunkWatchControl::screenToWorld(CPoint const& pos,
                                      float*        wx,
                                      float*        wz) const
{
    BW_GUARD;
...
bool ChunkWatchControl::updateProjectDib()
{
    ...
    if (lastUpdateProjectTime_ != 0) {
        uint64 now = timestamp();
        if ((now - lastUpdateProjectTime_) / (double)stampsPerSecond() <
            PROJECT_UPDATE_DELTA)
            return true;
    ...
    if (texture == NULL) {
        ...
        if (dirMap != NULL) {
            ...
            baseTexture        = Moo::TextureManager::instance()->get(
              mapName, true, false, true, "texture/chunk watcher");
