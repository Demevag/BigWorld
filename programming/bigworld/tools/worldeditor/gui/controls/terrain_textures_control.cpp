 ...
 */
BOOL TerrainTexturesControl::Create(DWORD        style,
                                    CRect const& extents,
                                    CWnd*        parent,
                                    UINT         id /*= 0*/
)
...
int TerrainTexturesControl::height() const
{
    ...
    return static_cast<int>(((textures_.size() + 1) / 2) *
                            (extent.Height() + IMAGE_OFFSET));
}
 ...
 */
bool TerrainTexturesControl::layerRectAtPoint(const CPoint& point,
                                              CRect&        rect) const
{
    BW_GUARD;
  ...
  DibSecVec const&                    oldTextures)
{
    ...
    if (!found) {
        ...
        // file ourselves:
            ...
            BW::string filename =
              BWResource::resolveFilename(layer.textureName());
    ...
    // Create the blend:
    Terrain::TerrainTextureLayer& nclayer =
      const_cast<Terrain::TerrainTextureLayer&>(layer);
    Terrain::TerrainTextureLayerHolder             holder(&nclayer, true);
    Terrain::TerrainTextureLayer::ImageType const& img = nclayer.image();
    controls::DibSection32Ptr                      blend =
      new controls::DibSection32(img.width(), img.height());
 ...
 */
CRect TerrainTexturesControl::textureExtent(size_t       idx,
 ...
 */
CRect TerrainTexturesControl::blendExtent(size_t       idx,
                                          CRect const* nclient /*= NULL*/
) const
{
    BW_GUARD;
...
/*afx_msg*/ void TerrainTexturesControl::OnLButtonDown(UINT flags, CPoint point)
{
    ...
    if (idx != -1) {
        bool toggleSelect =
          ((flags & MK_CONTROL) != 0) || ((flags & MK_SHIFT) != 0);
 ...
 *	@param point		The point that the button went down at.
 */
/*afx_msg*/ void TerrainTexturesControl::OnRButtonDown(UINT   nFlags,
                                                       CPoint point)
{
    BW_GUARD;
    ...
    if (idx != -1) {
        ...
        // Tell the parent
        parent->SendMessage(WM_TERRAIN_TEXTURE_RIGHT_CLICK,
 ...
 *	@param point		The point that the button went down at.
 */
/*afx_msg*/ void TerrainTexturesControl::OnLButtonDblClk(UINT   nFlags,
                                                         CPoint point)
{
    BW_GUARD;
    ...
    int idx = layerAtPoint(point);
    if (idx != -1) {
        parent->SendMessage(WM_TERRAIN_TEXTURE_SELECTED,
                            (WPARAM)textureNames_[idx].c_str(),
                            (LPARAM)idx);
    }
}
