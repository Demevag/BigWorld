...
{
  public:
    afx_msg void OnTvnEndlabeleditWeatherSystemsList(NMHDR*   pNMHDR,
                                                     LRESULT* pResult);
    afx_msg void OnNMDblclkWeatherSystemsList(NMHDR* pNMHDR, LRESULT* pResult);
    afx_msg void OnTvnKeydownWeatherSystemsList(NMHDR*   pNMHDR,
                                                LRESULT* pResult);
    afx_msg void OnLbnSelchangeWeatherSystemsList(NMHDR*   pNMHDR  = NULL,
 ...
 */
class PageOptionsWeather
  : public WeatherSettingsTable
  , public GUITABS::Content
{
    IMPLEMENT_BASIC_CONTENT(
      Localise(L"WORLDEDITOR/GUI/PAGE_OPTIONS_WEATHER/SHORT_NAME"),
      Localise(L"WORLDEDITOR/GUI/PAGE_OPTIONS_WEATHER/LONG_NAME"),
      290,
      500,
      NULL)
  ...
  private:
    ...
    static void PageOptionsWeather::saveWeatherUndoState(
      const BW::string& desc);
    PY_AUTO_MODULE_STATIC_METHOD_DECLARE(RETVOID,
                                         saveWeatherUndoState,
                                         ARG(BW::string, END))
