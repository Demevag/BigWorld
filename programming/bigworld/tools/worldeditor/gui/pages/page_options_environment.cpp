...
namespace {
    ...
    /*virtual*/
    bool EnvironmentUndo::iseq(UndoRedo::Operation const& other) const
    {
        return false;
    }
}
    ...
    if ((int)lpDrawItemStruct->itemID >= 0) {
        ...
        if (idx == partition) {
            ...
            if (!skyDomes.empty()) {
                ...
                dc.DrawText(bw_utf8tow(item).c_str(),
...
void PageOptionsEnvironment::reinitialise()
{
    ...
    skyFileEdit_.SetWindowText(
      bw_utf8tow(enviroMinder.skyGradientDomeFile()).c_str());
    todFileEdit_.SetWindowText(
      bw_utf8tow(enviroMinder.timeOfDayFile()).c_str());
...
void PageOptionsEnvironment::InitPage()
{
    ...
    skyCopyFileBtn_.setBitmapID(
      IDB_DUPLICATE, IDB_DUPLICATED, RGB(0, 128, 128));
    todBrowseFileBtn_.setBitmapID(IDB_OPEN, IDB_OPEND, RGB(255, 255, 255));
    todCopyFileBtn_.setBitmapID(
      IDB_DUPLICATE, IDB_DUPLICATED, RGB(0, 128, 128));
    ...
    if (!initialised_) {
        ...
        if (skyDomesTB_.GetSafeHwnd() == NULL) {
            ...
            skyDomesTB_.SetBarStyle(CBRS_ALIGN_TOP | CBRS_TOOLTIPS |
                                    CBRS_FLYBY);
...
/*afx_msg*/
LRESULT
PageOptionsEnvironment::OnUpdateControls(WPARAM /*wParam*/, LPARAM /*lParam*/
)
{
    BW_GUARD;
...
/*afx_msg*/
LRESULT
PageOptionsEnvironment::OnNewSpace(WPARAM /*wParam*/, LPARAM /*lParam*/
)
{
    BW_GUARD;
...
/*afx_msg*/
LRESULT
PageOptionsEnvironment::OnBeginSave(WPARAM /*wParam*/, LPARAM /*lParam*/
)
{
    BW_GUARD;
...
/*afx_msg*/
LRESULT
PageOptionsEnvironment::OnEndSave(WPARAM /*wParam*/, LPARAM /*lParam*/
)
{
    BW_GUARD;
...
/*afx_msg*/ void PageOptionsEnvironment::OnBrowseSkyFile()
{
    ...
    if (openDlg.showDialog()) {
        ...
        if (strcmpi(dissolvedFilename.c_str(), filename.c_str()) == 0) {
            ...
            saveUndoState(LocaliseUTF8(
              L"WORLDEDITOR/GUI/PAGE_OPTIONS_ENVIRONMENT/SET_SKY_FILE"));
            EnviroMinder& enviroMinder = getEnviroMinder();
    ...
    if (saveDlg.showDialog()) {
        ...
        if (strcmpi(dissolvedFilename.c_str(), newFilename.c_str()) == 0) {
            ...
            if (strcmpi(source.c_str(), dest.c_str()) == 0) {
                ...
                saveUndoState(LocaliseUTF8(
                  L"WORLDEDITOR/GUI/PAGE_OPTIONS_ENVIRONMENT/COPY_SKY_FILE"));
                SkyGradientDome* skd = enviroMinder.skyGradientDome();
                DataSectionPtr   section =
                  BWResource::openSection(newFilename, true);
                skd->save(section);
                section->save();
                enviroMinder.skyGradientDomeFile(dissolvedFilename);
                reinitialise();
...
/*afx_msg*/ void PageOptionsEnvironment::OnBrowseTODFile()
{
    ...
    if (openDlg.showDialog()) {
        ...
        if (strcmpi(dissolvedFilename.c_str(), filename.c_str()) == 0) {
            ...
            CWaitCursor waitCursor;
            saveUndoState(
              LocaliseUTF8(L"WORLDEDITOR/GUI/PAGE_OPTIONS_ENVIRONMENT/"
                           L"SET_TIME_OF_DAY_FILE"));
            EnviroMinder& enviroMinder = getEnviroMinder();
    ...
    if (openDlg.showDialog()) {
        ...
        if (strcmpi(dissolvedFilename.c_str(), filename.c_str()) == 0) {
            ...
            if (skyDome == NULL) {
                ...
                saveUndoState(LocaliseUTF8(
                  L"WORLDEDITOR/GUI/PAGE_OPTIONS_ENVIRONMENT/ADD_SKY_DOME"));
                EnviroMinder& enviroMinder = getEnviroMinder();
...
/*afx_msg*/ void PageOptionsEnvironment::OnClearSkyDomes()
{
    ...
    if (!enviroMinder.skyDomes().empty()) {
        saveUndoState(LocaliseUTF8(
          L"WORLDEDITOR/GUI/PAGE_OPTIONS_ENVIRONMENT/CLEAR_SKY_DOME"));
        enviroMinder.skyDomes().clear();
        skyDomesList_.ResetContent();
        skyDomesList_.AddString(Localise(
          L"WORLDEDITOR/GUI/PAGE_OPTIONS_ENVIRONMENT/WEATHER_PARTITION"));
...
/*afx_msg*/ void PageOptionsEnvironment::OnBrowseSkyGradBtn()
{
    ...
    if (openDlg.showDialog()) {
        ...
        if (strcmpi(dissolvedFilename.c_str(), newFilename.c_str()) == 0) {
            ...
            saveUndoState(
              LocaliseUTF8(L"WORLDEDITOR/GUI/PAGE_OPTIONS_ENVIRONMENT/"
                           L"LOAD_SKY_BOX_TEXTURE"));
            bool ok = skd->loadTexture(dissolvedFilename);
            if (ok) {
                skyBoxGradEdit_.SetWindowText(
...
/*afx_msg*/ void PageOptionsEnvironment::OnSkyboxUp()
{
    ...
    // Save current state:
    saveUndoState(LocaliseUTF8(
      L"WORLDEDITOR/GUI/PAGE_OPTIONS_ENVIRONMENT/MOVE_SKY_DOME_UP"));
...
/*afx_msg*/ void PageOptionsEnvironment::OnSkyboxDown()
{
    ...
    // Save current state:
    saveUndoState(LocaliseUTF8(
      L"WORLDEDITOR/GUI/PAGE_OPTIONS_ENVIRONMENT/MOVE_SKY_DOME_DOWN"));
...
/*afx_msg*/ void PageOptionsEnvironment::OnSkyboxDel()
{
    ...
    // Save current state:
    saveUndoState(LocaliseUTF8(
      L"WORLDEDITOR/GUI/PAGE_OPTIONS_ENVIRONMENT/DELETE_SKY_DOME"));
...
/*afx_msg*/ void PageOptionsEnvironment::OnHourLengthEdit()
{
    ...
    if (filterChange_ == 0) {
        ...
        if (WorldManager::instance().secondsPerHour() !=
            hourLength_.GetValue()) {
            saveUndoState(LocaliseUTF8(
              L"WORLDEDITOR/GUI/PAGE_OPTIONS_ENVIRONMENT/EDIT_HOUR_LENGTH"));
...
/*afx_msg*/ void PageOptionsEnvironment::OnStartTimeEdit()
{
    ...
    if (filterChange_ == 0) {
        ...
        if (tod.startTime() != startTime_.GetValue()) {
            saveUndoState(LocaliseUTF8(
              L"WORLDEDITOR/GUI/PAGE_OPTIONS_ENVIRONMENT/ON_EDIT_START_TIME"));
...
/*afx_msg*/
void PageOptionsEnvironment::OnHScroll(UINT        sbcode,
                                       UINT        pos,
...
PageOptionsEnvironment::OnCTSelTime(WPARAM /*wParam*/, LPARAM lParam)
{
    ...
    if (filterChange_ == NULL) {
        ...
        Options::setOptionInt(
          "graphics/timeofday",
          int(timeOfDaySlider_.GetPos() * WorldManager::TIME_OF_DAY_MULTIPLIER /
              SLIDER_PREC));
    ...
    if (filterChange_ == 0) {
        ...
        if (sms == SMS_STARTED) {
            ...
            if (initialValue_ != sunAngle) {
                ...
                saveUndoState(LocaliseUTF8(
                  L"WORLDEDITOR/GUI/PAGE_OPTIONS_ENVIRONMENT/SET_SUN_ANGLE"));
                WorldManager::instance().environmentChanged();
            }
        }
    ...
    if (filterChange_ == 0) {
        ...
        if (sms == SMS_STARTED) {
            ...
            if (initialValue_ != moonAngle) {
                ...
                saveUndoState(LocaliseUTF8(
                  L"WORLDEDITOR/GUI/PAGE_OPTIONS_ENVIRONMENT/SET_MOON_ANGLE"));
                WorldManager::instance().environmentChanged();
            }
        }
...
/*afx_msg*/ void PageOptionsEnvironment::OnResetBtn()
{
    ...
    TimeOfDay& tod = getTimeOfDay();
    saveUndoState(LocaliseUTF8(
      L"WORLDEDITOR/GUI/PAGE_OPTIONS_ENVIRONMENT/RESET_ENVIRONMENT_ANIMATION"));
    switch (mode_) {
        case MODE_SUN:
...
/*afx_msg*/ void PageOptionsEnvironment::OnDelClrBtn()
{
    ...
    if (colourTimeline_ != NULL) {
        ...
        if (ok) {
            saveUndoState(
              LocaliseUTF8(L"WORLDEDITOR/GUI/PAGE_OPTIONS_ENVIRONMENT/"
                           L"DELETE_ENVIRONMENT_ANIMATION_COLOR"));
            rebuildAnimation();
        }
    }
}
...
/*afx_msg*/
LRESULT
PageOptionsEnvironment::OnTimelineBegin(WPARAM wparam, LPARAM lparam)
{
    BW_GUARD;
...
/*afx_msg*/
LRESULT
PageOptionsEnvironment::OnTimelineMiddle(WPARAM /*wparam*/, LPARAM /*lparam*/
)
{
    BW_GUARD;
...
/*afx_msg*/
LRESULT
PageOptionsEnvironment::OnTimelineDone(WPARAM wparam, LPARAM lparam)
{
    BW_GUARD;
    ...
    rebuildAnimation();
    saveUndoState(LocaliseUTF8(
      L"WORLDEDITOR/GUI/PAGE_OPTIONS_ENVIRONMENT/EDIT_ENVIRONMENT_TIMELINE"));
...
/*afx_msg*/
LRESULT
PageOptionsEnvironment::OnTimelineAdd(WPARAM /*wparam*/, LPARAM /*lparam*/
)
{
    BW_GUARD;

    saveUndoState(LocaliseUTF8(
      L"WORLDEDITOR/GUI/PAGE_OPTIONS_ENVIRONMENT/ADD_ENVIRONMENT_COLOR"));
    timelineChanged();
    return TRUE;
}
...
/*afx_msg*/
LRESULT
PageOptionsEnvironment::OnTimelineNewSel(WPARAM /*wparam*/, LPARAM /*lparam*/
)
{
    BW_GUARD;
...
/*afx_msg*/
LRESULT
PageOptionsEnvironment::OnPickerDown(WPARAM /*wparam*/, LPARAM /*lparam*/
)
{
    BW_GUARD;
...
/*afx_msg*/
LRESULT
PageOptionsEnvironment::OnPickerMove(WPARAM /*wparam*/, LPARAM /*lparam*/
)
{
    BW_GUARD;
...
/*afx_msg*/
LRESULT
PageOptionsEnvironment::OnPickerUp(WPARAM /*wparam*/, LPARAM /*lparam*/
)
{
    BW_GUARD;
    ...
    if (colourTimeline_->itemSelected()) {
        ...
        rebuildAnimation();
        saveUndoState(LocaliseUTF8(
          L"WORLDEDITOR/GUI/PAGE_OPTIONS_ENVIRONMENT/EDIT_ENVIRONMENT_COLOR"));
    ...
    if (filterChange_ == 0) {
        ...
        if (skd != NULL) {
            ...
            if (changed) {
                ...
                vertexHeightEffectSlider_.setValue(
                  vertexHeightEffectEdit_.GetValue());
                sunHeightEffectSlider_.setValue(
                  sunHeightEffectEdit_.GetValue());
 ...
 *			background.
 */
/*afx_msg*/ HBRUSH PageOptionsEnvironment::OnCtlColor(CDC*  pDC,
                                                      CWnd* pWnd,
                                                      UINT  nCtlColor)
{
    BW_GUARD;
    ...
    sunAngleEdit_.SetBoundsColour(pDC,
                                  pWnd,
                                  sunAngleSlider_.getMinRangeLimit(),
                                  sunAngleSlider_.getMaxRangeLimit());
    moonAngleEdit_.SetBoundsColour(pDC,
                                   pWnd,
                                   moonAngleSlider_.getMinRangeLimit(),
                                   moonAngleSlider_.getMaxRangeLimit());
    ...
    mieEdit_.SetBoundsColour(
      pDC, pWnd, mieSlider_.getMinRangeLimit(), mieSlider_.getMaxRangeLimit());
    turbOffsEdit_.SetBoundsColour(pDC,
                                  pWnd,
                                  turbOffsSlider_.getMinRangeLimit(),
                                  turbOffsSlider_.getMaxRangeLimit());
    turbFactorEdit_.SetBoundsColour(pDC,
                                    pWnd,
                                    turbFactorSlider_.getMinRangeLimit(),
                                    turbFactorSlider_.getMaxRangeLimit());
    vertexHeightEffectEdit_.SetBoundsColour(
      pDC,
      pWnd,
      vertexHeightEffectSlider_.getMinRangeLimit(),
      vertexHeightEffectSlider_.getMaxRangeLimit());
    sunHeightEffectEdit_.SetBoundsColour(
      pDC,
      pWnd,
      sunHeightEffectSlider_.getMinRangeLimit(),
      sunHeightEffectSlider_.getMaxRangeLimit());
    powerEdit_.SetBoundsColour(pDC,
                               pWnd,
                               powerSlider_.getMinRangeLimit(),
                               powerSlider_.getMaxRangeLimit());
    return brush;
}
...
void PageOptionsEnvironment::onModeChanged()
{
    ...
    if (!animation->empty()) {
        ...
        for (LinearAnimation<Vector3>::const_iterator it = animation->begin();
             it != animation->end();
             ++it) {
...
void PageOptionsEnvironment::rebuildAnimation()
{
    ...
    TimeOfDay&                   tod = getTimeOfDay();
    controls::ColorScheduleItems schedule =
      colourTimeline_->colourScheduleItems();
    ...
    switch (mode_) {
        case MODE_SUN: {
            tod.clearSunAnimations();
            for (controls::ColorScheduleItems::const_iterator it =
                   schedule.begin();
                 it != schedule.end();
                 ++it) {
                controls::ColorScheduleItem const& item = *it;
                Vector3 color = Vector3(item.color_.x * 255.0f,
                                        item.color_.y * 255.0f,
                                        item.color_.z * 255.0f);
                tod.addSunAnimation(item.normalisedTime_ * 24.0f, color);
            }
        } break;
        case MODE_AMB: {
            tod.clearAmbientAnimations();
            for (controls::ColorScheduleItems::const_iterator it =
                ...
                Vector3 color = Vector3(item.color_.x * 255.0f,
                                        item.color_.y * 255.0f,
                                        item.color_.z * 255.0f);
                tod.addAmbientAnimation(item.normalisedTime_ * 24.0f, color);
            }
        } break;
    }
    WorldManager::instance().environmentChanged();
...
void PageOptionsEnvironment::timelineChanged()
{
    ...
    if (colourTimeline_ != NULL && colourPicker_ != NULL) {
        ...
        Vector4 colour = colourTimeline_->getColorScheduleItemSelectedColor();
...
void PageOptionsEnvironment::rebuildSkydomeList()
{
    ...
    for (size_t i = 0; i < skyDomes.size(); i++) {
        ...
        BW::string     file =
          BWResource::getFilename(skyDome->resourceID()).to_string();
        skyDomesList_.AddString(bw_utf8tow(file).c_str());
    }

    if (partition < skyDomes.size()) {
        skyDomesList_.InsertString(
