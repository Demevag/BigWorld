...
PanelManager::PanelManager()
  : mainFrame_(0)
  , ready_(false)
  , GUI::ActionMaker<PanelManager>("doDefaultPanelLayout",
                                   &PanelManager::loadDefaultPanels)
  , GUI::ActionMaker<PanelManager, 1>("doShowSidePanel",
                                      &PanelManager::showSidePanel)
  , GUI::ActionMaker<PanelManager, 2>("doHideSidePanel",
                                      &PanelManager::hideSidePanel)
  , GUI::ActionMaker<PanelManager, 3>("doLoadPanelLayout",
                                      &PanelManager::loadLastPanels)
  , GUI::UpdaterMaker<PanelManager>("updateSidePanel",
                                    &PanelManager::updateSidePanel)
  , GUI::UpdaterMaker<PanelManager, 1>("disableEnablePanels",
                                       &PanelManager::disableEnablePanels)
{
    BW_GUARD;
    ...
    panelNames_.insert(
      StrPair(L"TerrainTexture", PageTerrainTexture::contentID));
    ...
    panelNames_.insert(
      StrPair(L"Navigation", PageOptionsNavigation::contentID));
    panelNames_.insert(StrPair(L"Weather", PageOptionsWeather::contentID));
    panelNames_.insert(
      StrPair(L"Environment", PageOptionsEnvironment::contentID));
    panelNames_.insert(
      StrPair(L"HDR Lighting", PageOptionsHDRLighting::contentID));
    ...
    panelNames_.insert(
      StrPair(L"PostProcessing", PagePostProcessing::contentID));
    panelNames_.insert(
      StrPair(L"PageFloraSetting", PageFloraSetting::contentID));
    if (Options::getOptionBool("panels/chunkWatcher", false))
        panelNames_.insert(
          StrPair(L"ChunkWatcher", PageChunkWatcher::contentID));
}
...
/*static*/ bool PanelManager::init(CFrameWnd* mainFrame, CWnd* mainView)
{
    ...
    PlacementPresets::instance()
      ->readPresets(); // to fill all placement combo boxes
...
bool PanelManager::initPanels()
{
    ...
    UalManager::instance().setErrorCallback(
      new UalFunctor1<PanelManager, const BW::string&>(
        pInstance(), &PanelManager::addSimpleError));
...
bool PanelManager::allPanelsLoaded()
{
    ...
    for (BW::map<BW::wstring, BW::wstring>::iterator i = panelNames_.begin();
         res && i != panelNames_.end();
         ++i) {
        res = res && !!this->panels().getContent((*i).second);
    }
...
bool PanelManager::loadDefaultPanels(GUI::ItemPtr item)
{
    ...
    p =
      this->panels().insertPanel(PageChunkTexture::contentID, GUITABS::TAB, p);
    p = this->panels().insertPanel(
      PageOptionsGeneral::contentID, GUITABS::TAB, p);
    p = this->panels().insertPanel(
      PageOptionsNavigation::contentID, GUITABS::TAB, p);
    p = this->panels().insertPanel(
      PageOptionsWeather::contentID, GUITABS::TAB, p);
    p = this->panels().insertPanel(
      PageOptionsEnvironment::contentID, GUITABS::TAB, p);
    p = this->panels().insertPanel(
      PageOptionsHDRLighting::contentID, GUITABS::TAB, p);
    p = this->panels().insertPanel(PageDecals::contentID, GUITABS::TAB, p);
    p = this->panels().insertPanel(
      PageOptionsHistogram::contentID, GUITABS::TAB, p);
    ...
    p = this->panels().insertPanel(
      PagePostProcessing::contentID, GUITABS::TAB, p);
    this->panels().insertPanel(PageFloraSetting::contentID, GUITABS::TAB, p);
    if (Options::getOptionBool("panels/chunkWatcher", false))
        p = this->panels().insertPanel(
          PageChunkWatcher::contentID, GUITABS::TAB, p);
    ...
    this->panels().insertPanel(
      PageTerrainTexture::contentID, GUITABS::SUBCONTENT, p);
    this->panels().insertPanel(
      PageTerrainHeight::contentID, GUITABS::SUBCONTENT, p);
    this->panels().insertPanel(
      PageTerrainFilter::contentID, GUITABS::SUBCONTENT, p);
    this->panels().insertPanel(
      PageTerrainMesh::contentID, GUITABS::SUBCONTENT, p);
    this->panels().insertPanel(
      PageTerrainImport::contentID, GUITABS::SUBCONTENT, p);
    ...
    this->panels().insertPanel(
      PageTerrainRuler::contentID, GUITABS::SUBCONTENT, p);
...
const BW::wstring PanelManager::getContentID(const BW::wstring pyID)
{
    ...
    return pyID; // it they passed in the GUITABS contentID of the panel, then
                 // use it
}
...
const BW::wstring PanelManager::getPythonID(const BW::wstring contentID)
{
    ...
    return contentID; // it they passed in the GUITABS python ID of the panel,
                      // then use it
}
...
void PanelManager::setToolMode(const BW::wstring id, bool canUndo)
{
    ...
    this->panels().broadcastMessage(
      WM_ACTIVATE_TOOL, (WPARAM)contentID.c_str(), 0);
    ...
    if (canUndo) {
        ...
        UndoRedo::instance().add(new ToolChangeOperation(*this, oldTool, pyID));
...
void PanelManager::updateUIToolMode(const BW::wstring& pyID)
{
    ...
    if (pythonID.empty())
        ...
        currentTool_ = pythonID; // the pyID was actually a contentID, so
                                 // setting the obtained pythonID
                                     ...
                                     UalPopupMenuItems& menuItems)
{
    ...
    WorldEditorApp::instance().pythonAdapter()->contextMenuGetItems(
      ii->type(), ii->longText(), pyMenuItems);
...
void PanelManager::ualEndPopupMenu(UalItemInfo* ii, int result)
{
    ...
    WorldEditorApp::instance().pythonAdapter()->contextMenuHandleResult(
      ii->type(), ii->longText().c_str(), result);
}
...
void PanelManager::ualUpdateDrag(UalItemInfo* ii)
{
    ...
    bool onGraphicWindow = ::WindowFromPoint(CPoint(ii->x(), ii->y())) ==
                           WorldManager::instance().hwndGraphics();
    BW::wstring filename = ii->text();
    BW::wstring ext      = filename.rfind(L'.') == filename.npos
                             ? L""
                             : filename.substr(filename.rfind(L'.') + 1);
    bool        isIgnored =
      ignoredObjectTypes_.find(ext) != ignoredObjectTypes_.end() ||
    ...
    if (!ii->isFolder() &&
        CooperativeMoo::canUseMoo(&WorldEditorApp::instance(), true) &&
        onGraphicWindow)
...
void PanelManager::ualEndDrag(UalItemInfo* ii)
{
    ...
    if (ii->isFolder()) {
        ...
        if (::WindowFromPoint(CPoint(ii->x(), ii->y())) ==
              WorldManager::instance().hwndGraphics() &&
            currentTool_ != L"TerrainImpExp" && currentTool_ != L"Project") {
...
void PanelManager::ualAddItemToHistory(const BW::wstring& str,
...
bool PanelManager::showSidePanel(GUI::ItemPtr item)
{
    ...
    if (!isDockVisible) {
        ...
        //		GetMenu()->GetSubMenu( 2 )->CheckMenuItem(
        // ID_MENU_SHOWSIDEPANEL, MF_BYCOMMAND | MF_CHECKED );
    }
    return true;
}
...
bool PanelManager::hideSidePanel(GUI::ItemPtr item)
{
    ...
    if (isDockVisible) {
        ...
        this->panels().showFloaters(!isDockVisible);
        //		GetMenu()->GetSubMenu( 2 )->CheckMenuItem(
        // ID_MENU_SHOWSIDEPANEL, MF_BYCOMMAND | MF_UNCHECKED );
    }
    return true;
}
...
void PanelManager::onChangedChunkState(int x, int z)
{
    ...
    this->panels().broadcastMessage(
      WM_CHANGED_CHUNK_STATE, (LPARAM)x, (WPARAM)z);
}
...
void PanelManager::showItemInUal(const BW::wstring& vfolder,
