...
afx_msg void PageOptionsHistogram::OnSize(UINT nType, int cx, int cy)
{
    ...
    win    = GetDlgItem(IDC_RED);
    win->SetWindowPos(
      0, redBtnPos_, buttonsPos_ + dy, 0, 0, SWP_NOZORDER | SWP_NOSIZE);

    win = GetDlgItem(IDC_GREEN);
    win->SetWindowPos(
      0, greenBtnPos_, buttonsPos_ + dy, 0, 0, SWP_NOZORDER | SWP_NOSIZE);

    win = GetDlgItem(IDC_BLUE);
    win->SetWindowPos(
      0, blueBtnPos_, buttonsPos_ + dy, 0, 0, SWP_NOZORDER | SWP_NOSIZE);
...
afx_msg LRESULT PageOptionsHistogram::OnUpdateControls(WPARAM wParam,
                                                       LPARAM lParam)
{
    BW_GUARD;
    ...
    if (IsWindowVisible() && userAdded_) {
        ...
        if (pBmpBits_) {
            ...
            BITMAPINFOHEADER bmpInfo = { sizeof(bmpInfo),
                                         lumRect.right,
                                         lumRect.bottom,
                                         1,
                                         24,
            ...
            HistogramProvider::Histogram hist =
              HistogramProvider::instance().get(
                HistogramProvider::HT_LUMINANCE);
            unsigned int max = 0;
            for (unsigned int i = 0; i < HistogramProvider::HISTOGRAM_LEVEL;
                 ++i) {
            ...
            for (int i = 0; i < lumRect.right; ++i) {
                indices[i] =
                  i * HistogramProvider::HISTOGRAM_LEVEL / lumRect.right;
            }
        ...
        if (pBmpBits_) {
            ...
            BOOL R = mRed.GetCheck(), G = mGreen.GetCheck(),
                 B = mBlue.GetCheck();
            ...
            for (int i = 0; i < rgbRect.right; ++i) {
                indices[i] =
                  i * HistogramProvider::HISTOGRAM_LEVEL / rgbRect.right;
            }
...
void PageOptionsHistogram::OnHScroll(UINT        nSBCode,
                                     UINT        nPos,
...
void PageOptionsHistogram::OnDrawItem(int              nIDCtl,
...
void PageOptionsHistogram::OnNMCustomdrawRangeratioslider(NMHDR*   pNMHDR,
                                                          LRESULT* pResult)
{
    BW_GUARD;
