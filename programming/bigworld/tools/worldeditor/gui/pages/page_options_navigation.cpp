...
void PageOptionsNavigation::InitPage()
{
    ...
    locationList_.updateLocationsList(
      WorldManager::instance().getCurrentSpace());

    int maxCameraHeight =
      Options::getOptionInt("graphics/maxCameraHeight", 200);
    ...
    cameraHeightEdit_.SetValue(
      Options::getOptionFloat("graphics/cameraHeight", 2.f));
    cameraHeightSlider_.SetPos(
      (int)(HEIGHT_SLIDER_PREC *
            Options::getOptionFloat("graphics/cameraHeight", 2.f)));
...
ON_BN_CLICKED(IDC_OPTIONS_LOCATION_UPDATE,
              &PageOptionsNavigation::OnBnClickedOptionsLocationUpdate)
END_MESSAGE_MAP()
...
afx_msg LRESULT PageOptionsNavigation::OnUpdateControls(WPARAM wParam,
                                                        LPARAM lParam)
{
    BW_GUARD;
    ...
    isPlayerPreviewModeEnabled_.SetCheck(
      WorldManager::instance().isInPlayerPreviewMode());
    ...
    if ((posXEdit_.doUpdate()) || (posYEdit_.doUpdate()) ||
        (posZEdit_.doUpdate())) {
    ...
    if ((fabsf(pos.x - lastPos_.x) > 0.1f) ||
        ...
        Chunk* chunk =
          ChunkManager::instance().cameraSpace()->findChunkFromPoint(pos);
        if (chunk) {
            posChunkEdit_.SetWindowText(
              bw_utf8tow(chunk->identifier()).c_str());
        }
...
void PageOptionsNavigation::OnHScroll(UINT        nSBCode,
                                      UINT        nPos,
                                      CScrollBar* pScrollBar)
{
    ...
    Options::setOptionFloat("graphics/cameraHeight",
                            (float)cameraHeightSlider_.GetPos() /
                              HEIGHT_SLIDER_PREC);
...
void PageOptionsNavigation::OnEnChangeOptionsCameraHeightEdit()
{
    ...
    cameraHeightSlider_.SetPos(
      (int)(HEIGHT_SLIDER_PREC * cameraHeightEdit_.GetValue()));
    dontUpdateHeightEdit_ = false;

    Options::setOptionFloat("graphics/cameraHeight",
                            cameraHeightEdit_.GetValue());
}
...
void PageOptionsNavigation::OnBnClickedPlayerPreviewMode()
{
    ...
    WorldManager::instance().setPlayerPreviewMode(
      isPlayerPreviewModeEnabled_.GetCheck() == TRUE);
}
...
void PageOptionsNavigation::OnBnClickedOptionsChunkMove()
{
    ...
    if (*chunkName.rbegin() != 'o' && *chunkName.rbegin() != 'i') {
        ...
        if (BWResource::fileExists(mapping->path() + insideChunkName +
                                   ".chunk"))
    ...
    if (chunk->loaded()) {
        ...
        if (space) {
            ...
            if (chunk->isOutsideChunk()) {
                ...
                Vector3 extent =
                  camPos + (Vector3(0.f, -2.f * EXTENT_RANGE, 0.f));
...
HBRUSH PageOptionsNavigation::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
    ...
    cameraHeightEdit_.SetBoundsColour(pDC,
                                      pWnd,
                                      cameraHeightEdit_.GetMinimum(),
                                      cameraHeightEdit_.GetMaximum());

    return brush;
}

LRESULT PageOptionsNavigation::OnSearchFieldChanged(WPARAM wParam,
                                                    LPARAM lParam)
{
    BW_GUARD;
    ...
    std::transform(
      lowerSearch.begin(), lowerSearch.end(), lowerSearch.begin(), tolower);
...
ON_NOTIFY_REFLECT(TVN_ENDLABELEDIT,
                  &OptionsLocationsTree::OnTvnEndlabeleditOptionsLocationList)
ON_NOTIFY_REFLECT(NM_DBLCLK,
                  &OptionsLocationsTree::OnNMDblclkOptionsLocationList)
ON_NOTIFY_REFLECT(TVN_KEYDOWN,
                  &OptionsLocationsTree::OnTvnKeydownOptionsLocationList)
END_MESSAGE_MAP()

void OptionsLocationsTree::OnTvnEndlabeleditOptionsLocationList(
  NMHDR*   pNMHDR,
  LRESULT* pResult)
{
    BW_GUARD;
    ...
    if (locations_.find(newLocationName) != locations_.end()) {
        if (::MessageBox(AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
                         Localise(L"WORLDEDITOR/GUI/PAGE_OPTIONS_NAVIGATION/"
                                  L"OVERWRITTEN_LOCATION_MARK_TEXT",
                                  newLocationName),
                         Localise(L"WORLDEDITOR/GUI/PAGE_OPTIONS_NAVIGATION/"
                                  L"OVERWRITTEN_LOCATION_MARK_TITLE"),
                         MB_YESNO) == IDNO)
            return;
    ...
    while ((item) &&
           (bw_MW_strcmp(GetItemText(item), newLocationName.c_str()))) {
...
void OptionsLocationsTree::OnNMDblclkOptionsLocationList(NMHDR*   pNMHDR,
                                                         LRESULT* pResult)
{
    BW_GUARD;
...
void OptionsLocationsTree::OnTvnKeydownOptionsLocationList(NMHDR*   pNMHDR,
                                                           LRESULT* pResult)
{
    BW_GUARD;
...
void OptionsLocationsTree::updateLocationsList(const BW::string& spaceName)
{
    ...
    locationData_ = BWResource::openSection(spaceName + "/locations.xml", true);
    ...
    for (int i = 0; i < numChildren; ++i) {
        ...
        if (locationName == "bookmark")
            locations_.insert(
              locationData_->openChild(i)->readString("name", ""));
        else {
            DataSectionPtr newBookmark = locationData_->newSection("bookmark");
            newBookmark->writeString("name", locationName);
            newBookmark->writeMatrix34(
              "view",
              locationData_->readMatrix34(locationName, Matrix::identity));
...
void OptionsLocationsTree::redrawLocationsList()
{
    ...
    for (; it != end; ++it) {
        ...
        // convert it to lowercase
        std::transform(locationLower.begin(),
                       locationLower.end(),
                       locationLower.begin(),
                       tolower);
...
void OptionsLocationsTree::doAdd()
{
    ...
    BW::string newName =
      LocaliseUTF8(L"WORLDEDITOR/GUI/PAGE_OPTIONS_NAVIGATION/UNTITLED");
    int i = 2;
    while (locations_.find(newName) != locations_.end()) {
        newName = LocaliseUTF8(
          L"WORLDEDITOR/GUI/PAGE_OPTIONS_NAVIGATION/UNTITLED_D", i);
        ++i;
    }
