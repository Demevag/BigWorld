...
ON_WM_HSCROLL()
ON_WM_SIZE()
ON_EN_CHANGE(IDC_OPTIONS_DECAL_FADE_START_EDIT,
             &PageDecals::OnEnChangeOptionsDecalFadeStartEdit)
ON_EN_CHANGE(IDC_OPTIONS_DECAL_FADE_END_EDIT,
             &PageDecals::OnEnChangeOptionsDecalFadeEndEdit)
END_MESSAGE_MAP()
...
int PageDecals::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
    ...
    m_statistics_control =
      new StatisticsControl(this,
                            _T("Decal count"),
                            0,
                            Options::getOptionInt("statistics/decalCount", 200),
                            10,
                            10,
                            &DecalsManager::numVisibleDecals);
...
void PageDecals::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control(
      pDX, IDC_OPTIONS_DECAL_FADE_START_SLIDER, decalsFadeStartSlider_);
...
HBRUSH PageDecals::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
    ...
    decalsFadeStartEdit_.SetBoundsColour(pDC,
                                         pWnd,
                                         decalsFadeStartEdit_.GetMinimum(),
                                         decalsFadeStartEdit_.GetMaximum());
    decalsFadeEndEdit_.SetBoundsColour(pDC,
                                       pWnd,
                                       decalsFadeEndEdit_.GetMinimum(),
                                       decalsFadeEndEdit_.GetMaximum());
