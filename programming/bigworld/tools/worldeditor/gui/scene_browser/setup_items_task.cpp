...
void SetupItemsTask::execute()
{
    ...
    for (ItemInfoDB::ItemList::iterator it = items.begin(); it != items.end();
         ++it) {
 ...
 */
void SetupItemsTask::results(Items& retItems,
                             int&   retNumItems,
                             int&   retNumTris,
        ...
        if ((*it).second) {
            ...
            while (!pItem->isPropertiesEnd()) {
                ...
                if (!prop.second.empty() &&
                    ...
                    std::transform(propStrLwr.begin(),
                                   propStrLwr.end(),
                                   propStrLwr.begin(),
                                   tolower);
 ...
 *	@param pComparer	Comparer class to use, or NULL for default sorting.
 */
SetupItemsBackgroundTask::SetupItemsBackgroundTask(
  const BW::string&            search,
  const ListGroupStates&       groupStates,
 ...
 */
void SetupItemsBackgroundTask::results(SetupItemsTask::Items& retItems,
                                       int&                   retNumItems,
                                       int&                   retNumTris,
                                       int&                   retNumPrimitives)
{
