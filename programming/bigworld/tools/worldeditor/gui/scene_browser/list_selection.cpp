 ...
 *
 *	@param eraseMissingItems	If true, it remove items from the selection if
...
void ListSelection::restore(const BW::vector<ItemInfoDB::ItemPtr>& items)
{
    ...
    for (size_t item = 0; item < items.size(); ++item) {
        if (selectionHash_.find(items[item]->chunkItem()) !=
            selectionHash_.end()) {
            pList_->SetItemState(
              static_cast<int>(item), LVIS_SELECTED, LVIS_SELECTED);
        }
    }
}
                              ...
                              const BW::vector<ItemInfoDB::ItemPtr>& items)
{
    ...
    for (size_t idx = 0; idx < items.size(); ++idx) {
        ChunkItemHash::iterator itSel = tmpSel.find(items[idx]->chunkItem());

        if (itSel != tmpSel.end()) {
            pList_->SetItemState(
              static_cast<int>(idx), LVIS_SELECTED, LVIS_SELECTED);
            if (!cursorSet) {
                pList_->SetItemState(
                  static_cast<int>(idx), LVIS_FOCUSED, LVIS_FOCUSED);
                cursorSet = true;
            }
...
void ListSelection::deleteSelectedItems() const
{
    ...
    if (!selection_.empty()) {
        ...
        ChunkItemGroupPtr group = ChunkItemGroupPtr(
          new ChunkItemGroup(selItems), PyObjectPtr::STEAL_REFERENCE);
