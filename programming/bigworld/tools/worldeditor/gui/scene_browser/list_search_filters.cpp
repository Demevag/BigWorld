...
void ListSearchFilters::doModal(const CWnd& pParent, const CPoint& pt)
{
    ...
    menu.addItem(checkedPrefix(FILTER_VISIBLE) + descFromId(FILTER_VISIBLE),
                 FILTER_VISIBLE);

    menu.addItem(checkedPrefix(FILTER_ALL) + descFromId(FILTER_ALL),
                 FILTER_ALL);

    menu.addItem(checkedPrefix(FILTER_NAME) + descFromId(FILTER_NAME),
                 FILTER_NAME);

    menu.addItem(checkedPrefix(FILTER_TYPE) + descFromId(FILTER_TYPE),
                 FILTER_TYPE);

    menu.addItem(checkedPrefix(FILTER_CHUNK) + descFromId(FILTER_CHUNK),
                 FILTER_CHUNK);

    menu.addItem(checkedPrefix(FILTER_PATH) + descFromId(FILTER_PATH),
                 FILTER_PATH);
    ...
    if (result == FILTER_VISIBLE) {
        ...
        allowedTypes_.clear();
        allowedTypes_.insert(
          ItemInfoDB::Type::builtinType(ItemInfoDB::Type::TYPEID_ASSETNAME));
        currentFilter_ = result;
    } else if (result == FILTER_TYPE) {
        allowedTypes_.clear();
        allowedTypes_.insert(
          ItemInfoDB::Type::builtinType(ItemInfoDB::Type::TYPEID_ASSETTYPE));
        currentFilter_ = result;
    } else if (result == FILTER_CHUNK) {
        allowedTypes_.clear();
        allowedTypes_.insert(
          ItemInfoDB::Type::builtinType(ItemInfoDB::Type::TYPEID_CHUNKID));
        currentFilter_ = result;
    } else if (result == FILTER_PATH) {
        allowedTypes_.clear();
        allowedTypes_.insert(
          ItemInfoDB::Type::builtinType(ItemInfoDB::Type::TYPEID_FILEPATH));
        currentFilter_ = result;
    }
}
