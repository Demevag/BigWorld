...
bool SceneBrowserDlg::load(DataSectionPtr section)
{
    ...
    if (groupType.fromDataSection(pGroupDS)) {
        for (ListGroups::iterator it = groups_.begin(); it != groups_.end();
             ++it) {
 ...
 */
void SceneBrowserDlg::currentItems(BW::vector<ChunkItemPtr>& chunkItems) const
{
    BW_GUARD;
...
BOOL SceneBrowserDlg::OnInitDialog()
{
    ...
    search_.init(MAKEINTRESOURCE(IDB_SEARCHMAGNIFIER_CLICK),
                 ...
                 list_.searchFilters().filterDesc(),
    ...
    if (pDS) {
        ...
        if (pToolBarDS) {
            ...
            toolbar_.SetWindowPos(
              0, -100, -100, tbSize.cx, tbSize.cy, SWP_NOMOVE | SWP_NOZORDER);
        }
    }
    ...
    for (ListGroups::iterator it = groups_.begin(); it != groups_.end(); ++it) {
...
void SceneBrowserDlg::OnSize(UINT nType, int cx, int cy)
{
    ...
    workingAnim_.SetWindowPos(
      0, searchRect.right + 2, searchRect.top + 2, 16, 16, SWP_NOZORDER);
    ...
    if (clientRect.right <= groupByMinRight_) {
        ...
        actualGroupByLeft =
          searchRect.left + (groupByLeft_ - groupByLabelLeft_);
    ...
    groupByLabel_.SetWindowPos(0,
                               actualGroupByLabelLeft,
                               actualGroupByLabelTop,
                               0,
                               0,
                               SWP_NOZORDER | SWP_NOSIZE);

    groupBy_.SetWindowPos(
      0, actualGroupByLeft, actualGroupByTop, 0, 0, SWP_NOZORDER | SWP_NOSIZE);

    toolbar_.SetWindowPos(
      0, actualToolbarLeft, actualToolbarTop, 0, 0, SWP_NOZORDER | SWP_NOSIZE);

    list_.SetWindowPos(0,
                       listLeft_,
                       actualListTop,
                       clientRect.right - listLeft_ - listRight_,
                       statusTop - listBottom_ - actualListTop,
                       SWP_NOZORDER);

    statusBar_.SetWindowPos(0,
                            statusBarLeft_,
                            statusTop,
                            clientRect.right - statusBarLeft_ - statusBarRight_,
                            statusBarHeight_,
                            SWP_NOZORDER);
}
...
LRESULT SceneBrowserDlg::OnUpdateControls(WPARAM wParam, LPARAM lParam)
{
    ...
    if (forceListTick || list_.needsTick() ||
        timeSinceLastUpdate > CHECK_UPDATE_LIST_MILLIS) {
    ...
    if (list_.hasSelectionChanged()) {
        ...
        if (SceneBrowser::instance().callbackSelChange()) {
            CWaitCursor wait;
            (*SceneBrowser::instance().callbackSelChange())(list_.selection());
        }
    ...
    int multiplier =
      std::min(1 + ItemInfoDB::instance().numPending() / MAX_DB_MILLIS_STEP,
...
void SceneBrowserDlg::updateStatusBar()
{
    ...
    if (list_.numSelectedItems() == 0) {
        ...
        wchar_t* localiseTag = searching ? L"SCENEBROWSER/STATUS_SELECT_SEARCH"
                                         : L"SCENEBROWSER/STATUS_SELECT";

        statusBar_.SetWindowText(Localise(localiseTag,
                                          list_.numSelectedItems(),
                                          list_.numItems(),
                                          list_.numSelectedTris(),
                                          list_.numTris(),
                                          list_.numSelectedPrimitives(),
                                          list_.numPrimitives()));
    }
}
BW_END_NAMESPACE
