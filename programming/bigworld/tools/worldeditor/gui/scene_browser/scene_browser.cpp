...
void SceneBrowser::scrollTo(ChunkItemPtr pItem) const
{
    ...
    PanelManager::instance().panels().sendMessage(
      SceneBrowserDlg::contentID, WM_LIST_SCROLL_TO, 0, (LPARAM)pItem.get());
}
...
bool SceneBrowser::isFocused() const
{
    ...
    GUITABS::Content* pSB =
      PanelManager::instance().panels().getContent(SceneBrowserDlg::contentID);
 ...
 */
bool SceneBrowser::currentItems(BW::vector<ChunkItemPtr>& chunkItems) const
{
    BW_GUARD;

    GUITABS::Content* pSBwnd =
      PanelManager::instance().panels().getContent(SceneBrowserDlg::contentID);
    bool itemsWereCopied = false;
    if (pSBwnd->getCWnd() && isFocusedInternal(pSBwnd)) {
        SceneBrowserDlg* pSB = static_cast<SceneBrowserDlg*>(pSBwnd->getCWnd());
...
bool SceneBrowser::isFocusedInternal(GUITABS::Content* cnt) const
{
    ...
    if (cnt && cnt->getCWnd()) {
        ...
        focused =
          (cnt->getCWnd() == pCurFocus) || cnt->getCWnd()->IsChild(pCurFocus);
    }
