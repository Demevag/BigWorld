...
struct LinkGizmoColourPart : public ShapePart
{
    ...
    LinkGizmoColourPart(Moo::Colour c, Type type)
      : colour_(c)
      , type_(type)
    {
    }
 ...
 */
LinkGizmo::LinkGizmo(LinkProxyPtr   linkProxy,
 ...
 *                      gizmo.
 */
/*virtual*/ bool LinkGizmo::intersects(Vector3 const& origin,
                                       ...
                                       bool           force)
{
    ...
    if ((linkProxy_->linkType() & LinkProxy::LT_LINK) != 0) {
        ...
        LinkGizmoColourPart* linkIntersect =
          (LinkGizmoColourPart*)linkSelectionMesh_.intersects(
            originX, m.applyVector(direction), &lt);
    ...
    if ((linkProxy_->linkType() & LinkProxy::LT_ADD) != 0) {
        ...
        LinkGizmoColourPart* addIntersect =
          (LinkGizmoColourPart*)addSelectionMesh_.intersects(
            originX, m.applyVector(direction), &st);
...
void LinkGizmo::click(Vector3 const& origin, Vector3 const& direction)
{
    ...
    if (currentPart_->type_ == LinkGizmoColourPart::ADD_UPPER_TYPE ||
 ...
 *                      gizmo.
 */
void LinkGizmo::rollOver(Vector3 const& /*origin*/, Vector3 const& /*direction*/
)
{
    BW_GUARD;
...
void LinkGizmo::rebuildMeshes(bool force)
{
    ...
    linkSelectionMesh_.addSphere(
      ...
      linkRadius,
      linkColour,
      new LinkGizmoColourPart(Moo::Colour(linkColour),
    ...
    addSelectionMesh_.addBox(
      centerAddUL +
        Vector3(-0.5f * widthAdd, -0.5f * heightAdd, -0.5f * breadthAdd),
      centerAddUL +
        Vector3(+0.5f * widthAdd, +0.5f * heightAdd, +0.5f * breadthAdd),
      addColour,
      new LinkGizmoColourPart(Moo::Colour(addColour),
                              LinkGizmoColourPart::ADD_UPPER_TYPE));
    addSelectionMesh_.addBox(
      centerAddUL +
        Vector3(-0.5f * breadthAdd, -0.5f * heightAdd, -0.5f * widthAdd),
      centerAddUL +
        Vector3(+0.5f * breadthAdd, +0.5f * heightAdd, +0.5f * widthAdd),
      addColour,
      new LinkGizmoColourPart(Moo::Colour(addColour),
...
Matrix LinkGizmo::gizmoTransform() const
{
    ...
    float scale = (rc->invView()[2].dotProduct(pos) -
                   rc->invView()[2].dotProduct(rc->invView()[3]));
    if (scale > 0.05)
        scale /= 25.f;
    else
        scale = 0.05f / 25.f;
 ...
 *                      the lower.
 */
void LinkGizmo::clickAdd(Vector3 const& /*origin*/,
                         Vector3 const& direction,
                         bool /*upper*/
)
{
 ...
 *                      gizmo.
 */
void LinkGizmo::clickLink(Vector3 const& /*origin*/,
                          Vector3 const& /*direction*/,
                          bool /*left*/
)
{
    ...
    ToolPtr linkTool(
      new Tool(linkProxy_->createLocator(),
