...
namespace {
    /// The amount to offset items that are created or cloned relative to the
    /// camera position
 ...
 *
 *	This function snaps two chunks together, by finding their closest matching
 *portals.
 *
 *	@param chunk1	A ChunkItemRevealer object of the chunk that is currently
 *being edited.
 *	@param chunk2	A ChunkItemRevealer object to the chunk that needs to be
 *snapped to the chunk that is currently being edited.
 */
static PyObject* py_autoSnap(PyObject* args)
{
    ...
    // static PyObject* autoSnap( ChunkItemRevealer* chunkRevealer,
    // ChunkItemRevealer* snapToRevealer )
    //{
    ...
    if (autoSnapTransform != Matrix::identity) {
        ...
        UndoRedo::instance().add(new ChunkMatrixOperation(chunk, m));
        ...
        UndoRedo::instance().barrier(
          LocaliseUTF8(
            L"WORLDEDITOR/WORLDEDITOR/CHUNK/CHUNK_ITEM_PLACER/AUTO_SNAP"),
          false);
 ...
 *
 *	@param chunks	A ChunkItemRevealer object of the chunk that is currently
 *being rotate-snapped
 */
static PyObject* py_rotateSnap(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(args, "Of|O", &pPyRev1, &dz, &pPyRev2) ||
        ...
        PyErr_SetString(PyExc_ValueError,
                        "WorldEditor.rotateSnap() "
                        "expects one or two ChunkItemRevealer(s) and a float");
        return NULL;
    }

    ChunkItemRevealer* rotateRevealer =
      static_cast<ChunkItemRevealer*>(pPyRev1);
    ChunkItemRevealer* referenceRevealer =
      static_cast<ChunkItemRevealer*>(pPyRev2);
    ...
    for (unsigned int i = 0; i < snapChunks.size(); ++i) {
        ...
        Matrix          rotateSnapTransform =
          findRotateSnapTransform(chunks, dz > 0.f, referenceChunk);
...
void ChunkItemExistenceOperation::undo()
{
    ...
    if (pOldChunk_ != NULL) {
        ...
        FenceInfo*        fenceInfo =
          (model == nullptr) ? nullptr : model->getFenceInfo(false);
        ...
        if (sect != NULL && sect->sectionName() == "terrain") {
            EditorChunkTerrain* pEct = static_cast<EditorChunkTerrain*>(
              ChunkTerrainCache::instance(*pOldChunk_).pTerrain());
            if (pEct) {
        ...
        WorldManager::instance().collectOccupiedChunks(
          bb, pItem_->chunk(), chunks);
        WorldManager::instance().changedChunks(chunks, *pItem_);

        pItem_->edPreDelete();

        if (pItem_->chunk()) // a vlo reference could have been tossed
                             // out...checking
    ...
    if (modifiedTerrains.size() != 0) {
        for (BW::vector<EditorChunkTerrain*>::iterator i =
               modifiedTerrains.begin();
             i != modifiedTerrains.end();
             ++i) {
            (*i)->onEditHeights();
            // also change left & bottom chunks' heightmap
            EditorChunkTerrain* pEct =
              (*i)->neighbour(EditorChunkTerrain::WEST);
            if (pEct) {
                pEct->onEditHeights();
            }
...
void ChunkItemHideOperation::undo()
{
    ...
    UndoRedo::instance().add(new ChunkItemHideOperation(pItem_));
}
...
void ChunkItemFreezeOperation::undo()
{
    ...
    // Add the current state of this item to the undo/redo list. It's done at
    // the end to be consistent with the rest
    UndoRedo::instance().add(new ChunkItemFreezeOperation(pItem_));
}
...
class ChunkItemOperation : public ReferenceCount
{
  public:
    ...
    static ChunkItemOperation::Result processList(
      const ChunkItemRevealer::ChunkItems& items,
      ChunkItemOperationPtr                operation);
};
 ...
 */
/*static*/
ChunkItemOperation::Result ChunkItemOperation::processList(
  const ChunkItemRevealer::ChunkItems& items,
  ChunkItemOperationPtr                operation)
{
    BW_GUARD;
...
class ChunkItemHider : public ChunkItemOperation
{
  public:
    ...
    virtual void addUndo(ChunkItemPtr pItem)
    {
        ...
        UndoRedo::instance().add(new ChunkItemHideOperation(pItem));
    }
...
class ChunkItemFreezer : public ChunkItemOperation
{
  public:
    ...
    virtual void addUndo(ChunkItemPtr pItem)
    {
        ...
        UndoRedo::instance().add(new ChunkItemFreezeOperation(pItem));
    }
 ...
 */
static Vector3 calculateAverageOrigin(
  const ChunkItemRevealer::ChunkItems& items)
{
    BW_GUARD;
    ...
    for (ChunkItemRevealer::ChunkItems::const_iterator i = items.begin();
        ...
        averagePosition +=
          chunk->transform().applyPoint((*i)->edTransform().applyToOrigin());
    }
 ...
 *	@param locator	 A ToolLocator object which has the position of where the
 *copies should be placed in the space.
 *
 *	@return			 A ChunkItemGroup object of the cloned items.
 *
 *	Code Example:
 *	@{
 *	group = WorldEditor.cloneChunkItems( self.selection,
 *bd.itemTool.locator.subLocator ) # where bd is a BigBandDirector object
 *	@}
 */
PyObject* ChunkItemPlacer::py_cloneChunkItems(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(args, "OO", &pPyRev, &pPyLoc) ||
        !ChunkItemRevealer::Check(pPyRev) || !ToolLocator::Check(pPyLoc)) {
        PyErr_SetString(PyExc_ValueError,
    ...
    // Don't place when in terrain or object snap mode, unless the item to be
    // cloned is a shell model. We can't use the method
    // WorldManager::freeSnapsEnabled() because that returns false if a chunk is
    // selected
    if (!locator->positionValid() &&
        ...
        if (terrainLocator != NULL) {
            WorldManager::instance().addCommentaryMsg(
              "WorldEditor.cloneChunkItem() "
              "No valid terrain to clone object onto.",
              Commentary::WARNING);
            return Py_None;
        }
        ...
        if (obstacleLocator != NULL) {
            WorldManager::instance().addCommentaryMsg(
              "WorldEditor.cloneChunkItem() "
              "No valid obstacle to clone object onto.",
              Commentary::WARNING);
            return Py_None;
        }
        PyErr_SetString(
    ...
    for (i = items.begin(), j = itemMatrices.begin(); i != items.end();) {
        ...
        if (sect->sectionName() == "terrain") {
            ...
            item->edBounds(lbb);
            Chunk* pNewChunk =
              EditorChunk::findOutsideChunk((*j).applyPoint(lbb.centre()));
            if (pNewChunk) {
                ChunkItemPtr dest =
                  ChunkTerrainCache::instance(*pNewChunk).pTerrain();

                if (dest && dest != item) {
                    bool in = std::find(terrainItems.begin(),
                                        terrainItems.end(),
                                        dest) != terrainItems.end();

                    if (!in) {
                        in = std::find(i + 1, items.end(), dest) != items.end();
                    }
                ...
                ERROR_MSG("Failed to place terrain, destination point is "
                          "outside the space.\n");
            }
    ...
    while (!terrainItems.empty()) {
        for (i = terrainItems.begin(), j = terrainMatrices.begin();
             i != terrainItems.end();
             ++i, ++j) {
            ChunkItemPtr item = *i;
    ...
    for (i = items.begin(), j = itemMatrices.begin(); i != items.end();
        ...
        if (item->isShellModel()) {
            // Clone the chunk
            ChunkItemPtr pItem =
              ChunkPlacer::cloneChunk(pChunk, m, linkerCloneGuidMapping);
            if (!pItem)
                continue;
            ...
            if (!sect) {
                ...
                UndoRedo::instance().barrier(
                  LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/CHUNK/"
                               L"CHUNK_ITEM_PLACER/CLONED_ITEMS"),
                  false);
            ...
            Chunk* pNewChunk =
              isTerrain
                ? EditorChunk::findOutsideChunk(m.applyPoint(lbb.centre()))
                : pChunk->space()->findChunkFromPointExact(
                    m.applyPoint(lbb.centre()));

            if (pNewChunk == NULL ||
                !EditorChunkCache::instance(*pNewChunk).edIsWriteable()) {
                WorldManager::instance().addCommentaryMsg(
                  LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/CHUNK/"
                               L"CHUNK_ITEM_PLACER/TARGET_NOT_WRITABLE",
                               item->edDescription().c_str()),
                  Commentary::WARNING);
                continue;
            }
            ...
            DataSectionPtr chunkSection =
              EditorChunkCache::instance(*pNewChunk).pChunkSection();
            ...
            if (isTerrain) {
                ...
                if (pEct) {
                    DataSectionPtr terrainSection =
                      EditorChunkCache::instance(*pNewChunk)
                        .pCDataSection()
                        ->openSection(pEct->block().dataSectionName(), true);
                ...
                newSection->writeString(
                  "resource", pNewChunk->identifier() + ".cdata/terrain");
            } else {
                newSection = chunkSection->newSection(sect->sectionName());
            ...
            WorldManager::instance().linkerManager().updateLinkerGuid(
              newSection, linkerCloneGuidMapping);
            // create the new item
            ...
            // If it was terrain then fix terrain edges and regenerate the lod
            // texture
            if (isTerrain) {
                EditorChunkTerrain* pEct = static_cast<EditorChunkTerrain*>(
                  ChunkTerrainCache::instance(*pNewChunk).pTerrain());
            ...
            WorldManager::instance().collectOccupiedChunks(
              globalBB, pNewChunk, invalidateChunks);
    ...
    if (linkerCloneGuidMapping.size()) {
        WorldManager::instance().linkerManager().updateMappedLinkers(
          linkerCloneGuidMapping);
    }

    // Cloning is always done as part of a clone & move tool, don't set a
    // barrier here set a meaningful barrier name
    if (!newItems.empty()) {
        WorldManager::instance().setSelection(newItems);
        if (items.size() == 1)
            UndoRedo::instance().barrier(
              LocaliseUTF8(
                L"WORLDEDITOR/WORLDEDITOR/CHUNK/CHUNK_ITEM_PLACER/CLONED_ITEMS",
                newItems.front()->edDescription().c_str()),
              false);
        else
            UndoRedo::instance().barrier(
              LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/CHUNK/CHUNK_ITEM_PLACER/"
                           L"CLONED_ITEMS"),
              false);

        WorldManager::instance().addCommentaryMsg(LocaliseUTF8(
          L"WORLDEDITOR/WORLDEDITOR/CHUNK/CHUNK_ITEM_PLACER/CLONED_SELECTION"));
 ...
 *	chunk item at the position of the supplied ToolLocator object. It also
 *accepts an optional parameter which specifies whether the new item should be
 *moved to the ToolLocator's local coordinates or world coordinates.
 *
 ...
 *	@param useLocalPos	An optional int value which specifies whether the item
 *should be placed localy to the ToolLocator or in world position (default).
 *						useLocalPos = 2 for local transform, useLocalPos = 1 for
 *world transform.
 *
...
PyObject* ChunkItemPlacer::py_createChunkItem(PyObject* args)
{
    ...
    Chunk* pChunk =
      ChunkManager::instance().cameraSpace()->findChunkFromPoint(wpos);
    if (!pChunk) {
    ...
    if (!EditorChunkCache::instance(*pChunk).edIsWriteable()) {
        WorldManager::instance().addCommentaryMsg(
          LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/CHUNK/CHUNK_ITEM_PLACER/"
                       L"CANNOT_ADD_ASSET_ON_NON_LOCKED_CHUNK"));
        Py_RETURN_NONE;
    }
...
/*static */ PyObject* ChunkItemPlacer::createChunkItem(Chunk*         pChunk,
                                                       DataSectionPtr pSection,
                                                       ToolLocator*   pLoc,
                                                       const Matrix&  basePose,
                                                       int            useLocPos,
                                                       const Vector3& snaps)
{
    ...
    WorldManager::instance().changedChunk(pChunk, InvalidateFlags::FLAG_NONE);
    ...
    */
    ...
    if (useLocPos) {
        ...
        if (WorldManager::instance().terrainSnapsEnabled()) {
            Vector3 worldPos(
              pChunk->transform().applyPoint(newPose.applyToOrigin()));

            Snap::toGround(worldPos);
            newPose.translation(
              pChunk->transformInverse().applyPoint(worldPos));
        }
        ...
        if ((sectionName == "model" || sectionName == "speedtree") &&
            ...
            // rotation
            // yaw/pitch/roll rotation: X = yaw, Pitch = Y, Roll = Z, should
            // change it )
        ...
        if (sectionName == "speedtree") {
            Matrix work = Matrix::identity;
            work.setScale(PLACER_FEET_TO_METRES,
                          PLACER_FEET_TO_METRES,
                          PLACER_FEET_TO_METRES);
            newPose.preMultiply(work);
        }
        ...
        if ((sectionName == "model" || sectionName == "speedtree")) {
            ...
            float scaleX       = randInRange(min(largestScale, minScaX),
                                       min(largestScale, maxScaX));
            float scaleY       = randInRange(min(largestScale, minScaY),
                                       min(largestScale, maxScaY));
            float scaleZ       = randInRange(min(largestScale, minScaZ),
                                       min(largestScale, maxScaZ));
            if (largestEdge > 99.99) {
                ...
                WorldManager::instance().changedChunk(
                  pChunk, InvalidateFlags::FLAG_NONE);

                DataSectionPtr xmlSectionPtr =
                  EditorChunkModelVLORef::createDataSection(pSection);

                EditorChunkModelVLORef::ProxyChunkCreationHelper
                  proxyChunkCreationHelper;
                return ChunkItemPlacer::createChunkItem(
                  pChunk, xmlSectionPtr, pLoc, pose, useLocPos, snaps);
            }

            if (PlacementPresets::instance()->isCurrentDataUniform(
                  PlacementPresets::SCALE)) {
                scaleY = scaleX;
                scaleZ = scaleX;
            }
    ...
    if (minEdge < 0.001) {
        PyErr_SetString(PyExc_EnvironmentError,
                        "WorldEditor.createChunkItem() "
                        "the item is too small in at least one axis, please "
    ...
    // ok, everyone's happy then. so add an undo which deletes it
    UndoRedo::instance().add(new ChunkItemExistenceOperation(pItem, NULL));
 ...
 *	@components{ worldeditor }
 *
 *	This function deletes chunk items referenced by the supplied
 *ChunkItemRevealer object.
 *
...
PyObject* ChunkItemPlacer::py_deleteChunkItems(PyObject* args)
{
        ...
        if (pItem->isShellModel()) {
            ...
            if (!ChunkPlacer::deleteChunk(pChunk)) {
                ERROR_MSG(LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/CHUNK/"
                                       L"CHUNK_ITEM_PLACER/CANT_DELETE_CHUNK",
            ...
            if (!pItem->edCanDelete()) {
                WorldManager::instance().addError(
                  ...
                  pItem.get(),
                  LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/CHUNK/"
                               L"CHUNK_ITEM_PLACER/CANT_DELETE_ITEM",
                               pItem->edDescription().c_str())
                    .c_str());
                continue;
            }
    ...
    if (items.size() == 1)
        UndoRedo::instance().barrier(
          LocaliseUTF8(
            L"WORLDEDITOR/WORLDEDITOR/CHUNK/CHUNK_ITEM_PLACER/DELETE_BARRIER",
            items[0]->edDescription().c_str()),
          false);
    else
        UndoRedo::instance().barrier(
          LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/CHUNK/CHUNK_ITEM_PLACER/"
                       L"DELETE_ITEMS_BARRIER"),
          false);
 ...
 *	@components{ worldeditor }
 *
 *	This function hides chunk items referenced by the supplied ChunkItemRevealer
 *object.
 *
 *	@param revealer The ChunkItemRevealer object of the items that need
 *					to be hidden.
 ...
 *	@components{ worldeditor }
 *
 *	This function freezes chunk items referenced by the supplied
 *ChunkItemRevealer object. The optional bool parameter can be used to unfreeze
 *as well.
 *
 *	@param revealer The ChunkItemRevealer object of the items that need
 *					to be frozen.
...
/*static*/ bool ChunkItemPlacer::hideChunkItems(
  const BW::vector<ChunkItemPtr>& items,
  bool                            hide,
  bool                            keepSelection)
{
    BW_GUARD;
    ...
    if (items.size() > 0) {
        ...
        ChunkItemOperation::Result res =
          ChunkItemOperation::processList(items, new ChunkItemHider(hide));
        ...
        if (res.value_) {
            ...
            if (items.size() == 1) {
                UndoRedo::instance().barrier(
                  LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/CHUNK/"
                               L"CHUNK_ITEM_PLACER/HIDE_BARRIER",
                               items[0]->edDescription().c_str()),
                  false);
            } else {
                UndoRedo::instance().barrier(
                  LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/CHUNK/"
                               L"CHUNK_ITEM_PLACER/HIDE_ITEMS_BARRIER"),
                  false);
            }
        } else {
            BW::string error =
              "WorldEditor.hideChunkItems() " + res.errorString_;
...
/*static*/ bool ChunkItemPlacer::freezeChunkItems(
  const BW::vector<ChunkItemPtr>& items,
  bool                            freeze,
  bool                            keepSelection)
{
    ...
    if (items.size() > 0) {
        ...
        ChunkItemOperation::Result res =
          ChunkItemOperation::processList(items, new ChunkItemFreezer(freeze));
        if (res.value_) {
            // set a meaningful barrier name
            if (items.size() == 1) {
                UndoRedo::instance().barrier(
                  LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/CHUNK/"
                               L"CHUNK_ITEM_PLACER/FREEZE_BARRIER",
                               items[0]->edDescription().c_str()),
                  false);
            } else {
                UndoRedo::instance().barrier(
                  LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/CHUNK/"
                               L"CHUNK_ITEM_PLACER/FREEZE_ITEMS_BARRIER"),
                  false);
            }
        } else {
            BW::string error =
              "WorldEditor.freezeChunkItems() " + res.errorString_;
            PyErr_Format(PyExc_ValueError, error.c_str());
            return false;
        }
    }
