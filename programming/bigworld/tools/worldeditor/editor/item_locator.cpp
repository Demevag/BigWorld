        ...
        if (!lines_.empty()) {
            for (uint32 i = 0; i < lines_.size(); i++) {
                LineHelper::instance().drawLine(
                  lines_[i], lineEnds_[i], colours_[i]);
            }
            // save states
            DWORD oldLighting;
            DWORD oldZEnable;
...
{
    Py_Header(ChunkItemLocatorRevealer, ChunkItemRevealer) public
  ...
  private:
    virtual void reveal(BW::vector<ChunkItemPtr>& items)
    {
        ...
        if (ci && ci->chunk() &&
            !EditorChunkCache::instance(*ci->chunk()).edIsDeleted())
            items.push_back(ci);
    }
                           ...
                           float                    dist)
    {
        ...
        if (!Options::getOptionInt("render/terrain", 1)) {
            if ((triangle.flags() & TRIANGLE_TERRAIN))
                return COLLIDE_ALL;
            if (pItem->isShellModel() && SelectionFilter::getSelectMode() ==
                                           SelectionFilter::SELECT_NOSHELLS)
                return COLLIDE_ALL;
        }
        if ((triangle.flags() & onFlags_) != onFlags_ ||
            (triangle.flags() & offFlags_)) {
            // If the closest object is now the terrain then the previous
            // closest object cannot be selected.
        ...
        if (!(triangle.flags() & TRIANGLE_DOUBLESIDED)) {
            ...
            if (!frontFacing) {
                if (!pItem->isShellModel())
                    return COLLIDE_ALL;
                if (pItem->isShellModel() &&
                    !SelectionFilter::canSelect(&*pItem))
                    return COLLIDE_ALL;
            }
        }
        ...
        if (pItem->edIsVLO()) {
            ...
            if (!pItem->chunk()->isOutsideChunk()) {
                VeryLargeObject::ChunkItemList items =
                  vlo->object()->chunkItems();
                for (VeryLargeObject::ChunkItemList::iterator iter =
                       items.begin();
                     iter != items.end();
                     ++iter) {
...
namespace {
    ...
    float distToWorldRay(ChunkItemPtr item, Vector3 worldRay)
    {
        ...
        // We're only using this for comparisons, so we don't need the actual
        // length
...
class BestObstacleCatcher : public CollisionCallback
{
  ...
  private:
                           ...
                           float                    dist)
    {
        ...
        // We don't want terrain
        if (triangle.flags() & TRIANGLE_TERRAIN)
            return Options::getOptionInt("render/terrain", 1) ? COLLIDE_BEFORE
                                                              : COLLIDE_ALL;
        ...
        // We do this by checking the angle between the camerapos to item and
        // worldRay isn't too large
        Vector3 itemRay = vectorToItem(pItem);
        itemRay.normalise();

        if (acosf(Math::clamp(-1.0f, worldRay_.dotProduct(itemRay), +1.0f)) >
            DEG_TO_RAD(5.f))
            return COLLIDE_BEFORE;

        // Set the chunk item if this one is closer to the ray
        if (!chunkItem || distToWorldRay(pItem, worldRay_) <
...
void ChunkItemLocator::calculatePosition(const Vector3& worldRay, Tool& tool)
{
    ...
    if (enabled_) {
        ...
        distance = ChunkManager::instance().cameraSpace()->collide(
          Moo::rc().invView().applyToOrigin(), extent, coc);

        chunkItem_ = coc.chunkItem_;
        if (chunkItem_) {
            transform_.setTranslate(Moo::rc().invView().applyToOrigin() +
                                    worldRay * distance);
        }

        // ok, nothing is at the subLocator, try a fuzzy search aroung the
        // worldRay
        if (!chunkItem_) {
            const float SELECTION_SIZE = 0.5f;

            // The points for a square around the view position, on the camera
            // plane
            Vector3 tr = Moo::rc().invView().applyPoint(
              Vector3(SELECTION_SIZE, SELECTION_SIZE, 0.f));
            Vector3 br = Moo::rc().invView().applyPoint(
              Vector3(SELECTION_SIZE, -SELECTION_SIZE, 0.f));
            Vector3 bl = Moo::rc().invView().applyPoint(
              Vector3(-SELECTION_SIZE, -SELECTION_SIZE, 0.f));
            Vector3 tl = Moo::rc().invView().applyPoint(
              Vector3(-SELECTION_SIZE, SELECTION_SIZE, 0.f));
            ...
            // Get the best chunk for tri1, then get the best for tri2, the
            // result will be the best out of both of them
            BestObstacleCatcher boc(NULL, worldRay);
            ChunkManager::instance().cameraSpace()->collide(tri1, extent, boc);
            ChunkManager::instance().cameraSpace()->collide(tri2, extent, boc);
 ...
 *  If we can't intersect with x,y plane set the position to be the camera
 *position and set the direction internally so it can be retrieved with the
 *  direction() method.
...
void ItemToolLocator::calculatePosition(const Vector3& worldRay, Tool& tool)
{
    ...
    if (pos == rayStart) {
        /*
        The nicest solution we have thought so far is, when the tool locator is
        not colliding with anything,  to calculate the size of the object/shell
        being placed, and place it some distance away from the camera according
        to the object/shell's size (the bigger, the farther away, but take
        near/far planes into account!). I have found it makes more sense to use
        the worldRay direction which takes into account the x,y click
        coordinates
        */
