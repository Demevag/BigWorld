 ...
 */
/*virtual*/ bool ImportCodecBMP::canHandleFile(BW::string const& filename)
{
    BW_GUARD;
 ...
 */
/*virtual*/ ImportCodec::LoadResult ImportCodecBMP::load(
    ...
    try {
        ...
        if (infoHeader.bits == 8) {
            ...
            for (int i = 0; i < 256; ++i) {
                ...
                palette[i] =
                  (uint16)(257 * (R_TO_L * r + G_TO_L * g + B_TO_L * b));
            }
 ...
 */
/*virtual*/ bool ImportCodecBMP::save(BW::string const&  filename,
                                      ...
                                      float* maxVal)
{
    ...
    try {
        ...
        header.size = sizeof(BMPHeader) + sizeof(BMPInfoHeader) +
                      1024 // palette size
                      + image.height() * rowsize;
        header.reserved1 = header.reserved2 = 0;
        header.offset =
          sizeof(BMPHeader) + sizeof(BMPInfoHeader) + 1024; // palette size
