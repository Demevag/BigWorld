...
namespace {
    ...
    bool writeNoise(DataSectionPtr pDataSection, const SimplexNoise& noise)
    {
        ...
        for (size_t i = 0; i < octaves.size(); ++i) {
            ...
            DataSectionPtr octaveSec = pDataSection->newSection("octave");
...
TerrainPaintBrush::TerrainPaintBrush(PyTypeObject* pType /*= &s_type_*/)
  ...
  , importMaskAdd_(0.0f)
  , importMaskMul_(1.0f / std::numeric_limits<uint16>::max())
  , // full strength
  maxLayerLimit_(false)
  , maxLayers_(4)
{
    Terrain::TerrainTextureLayer::defaultUVProjections(paintUProj_,
                                                       paintVProj_);
...
bool TerrainPaintBrush::load(DataSectionPtr pDataSection)
{
    ...
    Terrain::TerrainTextureLayer::defaultUVProjections(defaultUProj,
                                                       defaultVProj);
    ...
    textureMaskIncludeProj_ =
      pDataSection->readBool("textureMask/includeProj", false);
    textureMaskIncludeBump_ =
      pDataSection->readBool("textureMask/includeBump", false);
    textureMaskUProj_ =
      pDataSection->readVector4("textureMask/uProjection", defaultUProj);
    textureMaskVProj_ =
      pDataSection->readVector4("textureMask/vProjection", defaultVProj);
    textureMaskTexture_ = pDataSection->readString("textureMask/texture");
    bumpTextureMaskTexture_ =
      pDataSection->readString("textureMask/bumpTexture");
...
bool TerrainPaintBrush::save(DataSectionPtr pDataSection) const
{
    ...
    if (textureMask_) {
        pDataSection->writeBool("textureMask/includeProj",
                                textureMaskIncludeProj_);
        pDataSection->writeBool("textureMask/includeBump",
                                textureMaskIncludeBump_);
        pDataSection->writeVector4("textureMask/uProjection",
                                   textureMaskUProj_);
        pDataSection->writeVector4("textureMask/vProjection",
                                   textureMaskVProj_);
        pDataSection->writeString("textureMask/texture", textureMaskTexture_);
        pDataSection->writeString("textureMask/bumpTexture",
                                  bumpTextureMaskTexture_);
 ...
 */
/*static*/ BW::string TerrainPaintBrush::texture(DataSectionPtr pDataSection)
{
    BW_GUARD;
...
/**
 *	This gets the name of the bump texture used by the brush defined in the
 *given DataSection.
 *
 *	@param pDataSection	The DataSection that defines the brush.
 *	@return				The name of the bump texture file used by the brush.
 *This can be the empty string if there is not texture or if the DataSection is
 *NULL.
 */
/*static*/ BW::string TerrainPaintBrush::bumpTexture(
  const DataSectionPtr& pDataSection)
{
    BW_GUARD;
...
bool TerrainPaintBrush::operator!=(const TerrainPaintBrush& other) const
{
    return paintTexture_ != other.paintTexture_ ||
           paintBumpTexture_ != other.paintBumpTexture_ ||
           strength_ != other.strength_ || size_ != other.size_ ||
           paintUProj_ != other.paintUProj_ ||
           paintVProj_ != other.paintVProj_ || uvLocked_ != other.uvLocked_ ||
           noise_ != other.noise_ || heightMask_ != other.heightMask_ ||
           h1_ != other.h1_ || h2_ != other.h2_ || h3_ != other.h3_ ||
           h4_ != other.h4_ || slopeMask_ != other.slopeMask_ ||
           s1_ != other.s1_ || s2_ != other.s2_ || s3_ != other.s3_ ||
           s4_ != other.s4_ || textureMask_ != other.textureMask_ ||
           ...
           noiseMask_ != other.noiseMask_ || importMask_ != other.importMask_ ||
 ...
 *						otherwise.
 */
bool TerrainPaintBrush::isMaskLayer(
  const Terrain::TerrainTextureLayer& layer) const
{
    BW_GUARD;

    return
      // The layer being painted?
      layer.sameTexture(paintTexture_,
                        paintBumpTexture_,
                        paintUProj_,
                        paintVProj_,
      ...
      // will do.
      (!textureMaskIncludeProj_ && layer.textureName() == textureMaskTexture_ &&
       layer.bumpTextureName() == (textureMaskIncludeBump_
                                     ? bumpTextureMaskTexture_
                                     : layer.bumpTextureName())) ||
      // If we do care about projections then compare the projects as well.
      layer.sameTexture(textureMaskTexture_,
                        textureMaskIncludeBump_ ? bumpTextureMaskTexture_
                                                : layer.bumpTextureName(),
                        textureMaskUProj_,
                        textureMaskVProj_,
                        TerrainTextureUtils::PROJECTION_COMPARISON_EPSILON);
