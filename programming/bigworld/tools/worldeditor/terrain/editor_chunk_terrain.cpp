  ...
  , brushTextureExist_(false)
{
    ...
    const float blockSize =
      WorldManager::instance().pTerrainSettings()->blockSize();
    ...
    transform_.setTranslate(
      0.5f * blockSize - 0.5f, 0.f, 0.5f * blockSize - 0.5f);
...
void EditorChunkTerrain::relativeElevations(
  Terrain::TerrainHeightMap::ImageType& relHeights) const
{
    BW_GUARD;
    ...
    Terrain::TerrainHeightMap& thm =
      const_cast<Terrain::TerrainHeightMap&>(cthm);

    Terrain::TerrainHeightMapHolder holder(&thm,
                                           true); // lock/unlock height map
 ...
 *					dominant blend.
 */
size_t EditorChunkTerrain::dominantBlend(float  x,
                                         float  z,
                                         uint8* strength) const
{
    BW_GUARD;
    ...
    for (size_t i = 0; i < b.numberTextureLayers(); ++i) {
        ...
        Terrain::TerrainTextureLayerHolder  holder(
          const_cast<Terrain::TerrainTextureLayer*>(&ttl), true);
...
bool EditorChunkTerrain::edTransform(const Matrix& m, bool transient)
{
    ...
    // We allow an exception for ourself to support tossing into our current
    // chunk We allow an exception for a selected item as it implies we're
    // moving a group of terrain items in a direction, and the current one
    // likely won't be there any longer.
    ...
    transform_     = Matrix::identity;
    transform_.setTranslate(
      0.5f * gridSize - 0.5f, 0.f, 0.5f * gridSize - 0.5f);
    ...
    float angle = acosf(
      Math::clamp(-1.0f, newPt.dotProduct(Vector3(1.f, 0.f, 0.f)), +1.0f));
    ...
    if (rotateAmount > 0) {
        ...
        Terrain::TerrainHeightMap& thm = block().heightMap();
        {
            ...
            for (int r = 0; r < rotateAmount; ++r) {
                ...
                MF_ASSERT(
                  0); // TODO: I'm not sure if the rotate direction is ok.
...
void EditorChunkTerrain::edBounds(BoundingBox& bbRet) const
{
    ...
    Vector3     minb =
      bb_.minBounds() +
      Vector3(-0.5f * gridSize + 0.5f, -0.25f, -0.5f * gridSize + 0.5f);
    Vector3 maxb =
      bb_.maxBounds() +
      Vector3(-0.5f * gridSize - 0.5f, 0.25f, -0.5f * gridSize - 0.5f);
...
bool EditorChunkTerrain::edSave(DataSectionPtr pSection)
{
    ...
    pSection->writeString("resource",
                          pChunk_->identifier() + ".cdata/" +
                            block().dataSectionName());
    return true;
}
...
void EditorChunkTerrain::draw(Moo::DrawContext& drawContext)
{
    ...
    // Draw using our transform, to show when the user is dragging the block
    // around
    Matrix m = transform_;
    // m.setTranslate( m[3] - Vector3( gridSize, 0.f, gridSize ) );
    m.setTranslate(
      m[3] - Vector3(0.5f * gridSize - 0.5f, 0.f, 0.5f * gridSize - 0.5f));
    ...
    if (!isWriteable && shadeReadOnlyBlocks && !projectModule) {
        ...
        if (OptionsTerrain::numLayersWarningVisible() &&
            ...
            if (maxNumLayers > 0 &&
                ...
                // add the "maxLayersWarning_" rendereable, which will be
                // rendered next frame, and will then remove itself from
                // WorldEditor's rendereables list.
 ...
 */
EditorChunkTerrain* EditorChunkTerrain::neighbour(
  Neighbour       n,
...
) const
{
    ...
    return static_cast<EditorChunkTerrain*>(
      ChunkTerrainCache::instance(*chunk).pTerrain());
}

/*static*/ Terrain::EditorBaseTerrainBlockPtr EditorChunkTerrain::loadBlock(
  ...
  BW::string*         errorString)
{
    ...
    if (ver == Terrain::TerrainSettings::TERRAIN2_VERSION) {
        ...
        if (prefabVer != currentTerrainVer) {
            ERROR_MSG(
              "EditorChunkTerrain::loadBlock: Failed to load %s, "
...
size_t EditorChunkTerrain::findChunksWithinBox(
            ...
            if (ChunkManager::instance().cameraSpace()) {
                ChunkSpace::Column* pColumn =
                  ChunkManager::instance().cameraSpace()->column(
                    Vector3(x * gridSize + 0.5f * gridSize,
                            bb.minBounds().y,
 ...
 */
bool EditorChunkTerrain::load(DataSectionPtr pSection,
                              Chunk*         pChunk,
                              BW::string*    errorString)
{
    BW_GUARD;
    ...
    if (!prefabBinKey.empty()) {
        ...
        if (!prefabBinarySection) {
            BW::string err = "Unable to load prefab. Section prefabBin found, "
                             "but no current prefab binary section";
            if (errorString) {
                *errorString = err;
            }
            ERROR_MSG("%s\n", err.c_str());
        ...
        if (!bp) {
            BW::string err = "Unable to load " + prefabBinKey +
                             "  from current prefab binary section";
            if (errorString) {
                *errorString = err;
            }
            ERROR_MSG("%s\n", err.c_str());
        ...
        if (!validatePrefabVersion(terrainRes)) {
            BW::string err = "Prefab's terrain version different than the "
                             "current terrain version "
                             "('" +
                             terrainRes + "')";
            if (errorString) {
                *errorString = err;
        ...
        if (pFile) {
            ...
            block_ = EditorChunkTerrain::loadBlock(
              terrainName, this, pChunk->transform().applyToOrigin());
        ...
        if (!block_) {
            BW::string err =
              "Could not load prefab's terrain block. "
        ...
        EditorChunkTerrain* currentTerrain =
          (EditorChunkTerrain*)ChunkTerrainCache::instance(*pChunk).pTerrain();
        ...
        if (currentTerrain) {
            ...
            if (!validatePrefabVersion(terrainRes)) {
                ...
                BW::string err = "Cloned terrain's version different than the "
                                 "current terrain version "
                                 "('" +
                                 terrainRes + "')";
                if (errorString) {
            ...
            block_ = EditorChunkTerrain::loadBlock(
              terrainName, this, Vector3(0, 0, 0));
            ...
            BW::string resName =
              pChunk->mapping()->path() + pSection->readString("resource");
            // Allocate the terrainblock.
            block_ = EditorChunkTerrain::loadBlock(
              resName, this, pChunk->transform().applyToOrigin(), errorString);

            if (!block_) {
                BW::string err =
                  "Could not load terrain block '" + resName + "': ";
    ...
    cast.ptr = this;
    static_cast<Terrain::EditorBaseTerrainBlock*>(block_.getObject())
      ->setSelectionKey(cast.key);
    calculateBB();
    return true;
}
 ...
 */
void EditorChunkTerrain::copyHeights(int32  chunkDx,
                                     int32  chunkDz,
                                     uint32 srcX,
                                     uint32 srcZ,
                                     uint32 srcW,
                                     ...
                                     uint32 dstZ)
{
    ...
    Chunk* chunk = ChunkManager::instance().findChunkByName(
      nchunkId, WorldManager::instance().geometryMapping());
    if (chunk == NULL || !chunk->loaded())
        return;
    ChunkTerrain* chunkTerrain = ChunkTerrainCache::instance(*chunk).pTerrain();
    if (chunkTerrain == NULL)
        return;
    Terrain::BaseTerrainBlockPtr other =
      static_cast<Terrain::EditorBaseTerrainBlock*>(
        chunkTerrain->block().getObject());
...
{
  public:
    TerrainItemMatrix(ChunkItemPtr pItem)
...
void EditorChunkTerrain::onTerrainChanged()
{
    ...
    for (Chunk::Items::const_iterator it = items.begin(); it != items.end();
         ++it) {
...
bool EditorChunkTerrain::setForcedLod(const int& forcedLod)
{
    ...
    if (terrainBlock != NULL) {
        ...
        WorldManager::instance().forcedLodMap().updateLODElement(
          chunk->x(), chunk->z(), forcedLod);
        return true;
    }
    return false;
}
