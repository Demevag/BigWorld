...
bool MouseDragHandler::isDragging(MouseKey key) const
{
    ...
    switch (key) {
        case KEY_LEFTMOUSE:
            mouseKey =
              updateDragging(InputDevices::isKeyDown(KeyCode::KEY_LEFTMOUSE),
                             draggingLeftMouse_);
            break;

        case KEY_MIDDLEMOUSE:
            mouseKey =
              updateDragging(InputDevices::isKeyDown(KeyCode::KEY_MIDDLEMOUSE),
                             draggingMiddleMouse_);
            break;

        case KEY_RIGHTMOUSE:
            mouseKey =
              updateDragging(InputDevices::isKeyDown(KeyCode::KEY_RIGHTMOUSE),
                             draggingRightMouse_);
            break;
    }
