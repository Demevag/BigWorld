  ...
  , currentStrength_(0.0f)
{
    ...
    if (strengths.load("resources/data/terrain_paint_strengths.xml") ==
        DataHandle::DHE_NoError) {
        DataSectionPtr pSection = strengths.getRootSection();
        if (pSection) {
            Vector2                    infVec(std::numeric_limits<float>::max(),
    ...
    if (strengthTable.size() == 0) {
        ...
        strengthTable.push_back(Vector2(
          99.0f, 4000.0f)); //  99% may not give full strength straight away
        strengthTable.push_back(
          Vector2(100.0f, 40000.0f)); // 100% is full strength in a single frame
    }
 ...
 */
/*virtual*/ bool TerrainTextureFunctor::handleKeyEvent(const KeyEvent& keyEvent,
                                                       Tool&           tool)
{
    BW_GUARD;
...
void TerrainTextureFunctor::update(float dTime, Tool& tool)
{
    ...
    bool isPainting =
      ...
      !InputDevices::isShiftDown() && !InputDevices::isCtrlDown() &&
                                    ...
                                    bool  allVerts)
{
    ...
    randomness_ = Math::lerp(tool.strength(),
                             0.0f,
                             LOW_STRENGTH_DITHER,
                             LOW_RANDOM_DITHER,
                             HIGH_RANDOM_DITHER);
    ...
    if (pChunk != NULL) {
        pCurrentTerrain_ = static_cast<EditorChunkTerrain*>(
          ChunkTerrainCache::instance(*pChunk).pTerrain());

        if (pCurrentTerrain_) {
            lastPaintedLayer_ =
              static_cast<int>(this->blendLevel(pCurrentTerrain_->block()));
  ...
  TerrainUtils::TerrainFormat& format) const
{
    ...
    const Terrain::EditorBaseTerrainBlock& block = chunkTerrain.block();
...
void TerrainTextureFunctor::onFirstApply(EditorChunkTerrain& chunkTerrain)
{
    ...
    if (level != Terrain::EditorBaseTerrainBlock::INVALID_LAYER) {
        ...
        if (pPaintBrush_->maxLayerLimit_ && pPaintBrush_->maxLayers_ != 0) {
            TerrainTextureUtils::limitLayers(
              &chunkTerrain, pPaintBrush_->maxLayers_, level);
        }
  ...
  int32                              maxz)
{
    ...
    if (!pPaintBrush_->maxLayerLimit_ &&
        (numLayers > MAX_TEXTURE_LAYERS ||
         !chunkTerrain.brushTextureExist_ &&
           (numLayers >= MAX_TEXTURE_LAYERS))) {
    ...
    if (!chunkTerrain.brushTextureExist_) {
        ...
        {
            MaskResult* pMaskResult = TextureMaskCache::instance().mask(&block);
            ...
            if (pMaskImage) {
                for (int32 z = minz; !needCreate && z <= maxz; z++) {
                    for (int32 x = minx; !needCreate && x <= maxx; x++) {
                        ...
                        needCreate =
                          pixel.fuzziness_ > 0 && pixel.maxValue_ > 0;
                    }
                }
            }
        ...
        if (pPaintBrush_->maxLayerLimit_ && pPaintBrush_->maxLayers_) {
            ...
            if (mlevel != Terrain::EditorBaseTerrainBlock::INVALID_LAYER) {
                TerrainTextureUtils::replaceTexture(
                  ...
                  pPaintBrush_->paintUProj_,
                  pPaintBrush_->paintVProj_,
                ...
                TerrainTextureUtils::limitLayers(
                  &chunkTerrain, pPaintBrush_->maxLayers_, mlevel);
                needCreate = false;
            } else if (pPaintBrush_->maxLayers_ ==
                       numLayers) // full, need remove some layer
            {
        ...
        if (needCreate) {
            ...
            if (level == Terrain::EditorBaseTerrainBlock::INVALID_LAYER) {
                // Adding the new layer failed, print an error message and add
                // it to the list of full blocks and exit
                fullBlocks_.insert(&block);
                ERROR_MSG("Can't paint chunk %s, it reached the maximum number "
                          "of textures (%d)\n",
    ...
    if (pPaintBrush_->textureMask_ == 0) {
        this->applyToSubBlockNoTexMask(chunkTerrain,
                                       toolOffset,
                                       chunkOffset,
                                       format,
                                       minx,
        ...
        this->applyToSubBlockTexMask(chunkTerrain,
                                     toolOffset,
                                     chunkOffset,
                                     format,
                                     minx,
    ...
    chunkTerrain.block().rebuildCombinedLayers(
      true, false); // don't generate materials

    // Let WorldEditor know about the change in textures, it may be interested:
    WorldManager::instance().chunkTexturesPainted(chunkTerrain.chunk(), false);
...
void TerrainTextureFunctor::onApplied(Tool& t)
{
    ...
    for (ChunkPtrVector::iterator it = t.relevantChunks().begin();
...
void TerrainTextureFunctor::onLastApply(EditorChunkTerrain& chunkTerrain)
{
    ...
    TerrainTextureUtils::deleteEmptyLayers(
      block, TerrainTextureUtils::REMOVE_LAYER_THRESHOLD);
    ...
    // Let WorldEditor know about the change in textures, it may be interested:
    WorldManager::instance().chunkTexturesPainted(chunkTerrain.chunk(),
                                                  rebuiltLodTexture);
  ...
  const Terrain::EditorBaseTerrainBlock& block) const
{
    ...
    return block.findLayer(pPaintBrush_->paintTexture_,
                           pPaintBrush_->paintBumpTexture_,
                           pPaintBrush_->paintUProj_,
                           pPaintBrush_->paintVProj_,
  ...
  const Terrain::EditorBaseTerrainBlock& block) const
{
    ...
    if (pPaintBrush_->textureMask_ && !pPaintBrush_->textureMaskInvert_) {
        for (size_t i = 0; i < block.numberTextureLayers(); ++i) {
            ...
            if (pPaintBrush_->textureMaskIncludeProj_ &&
                !layer.sameTexture(
                  pPaintBrush_->textureMaskTexture_,
                  pPaintBrush_->textureMaskIncludeBump_
                    ? pPaintBrush_->bumpTextureMaskTexture_
...
void TerrainTextureFunctor::updateMaskProjection(Tool& tool)
{
    ...
    for (ChunkRenderMap::iterator it = chunkRenderMap_.begin();
  ...
  Moo::Image<uint8>&    img) const
{
    ...
    TextureMaskCache::beginFuzziness(
      &(pChunkTerrain->block()), *pPaintBrush_, maskLayers);

    for (uint32 y = 0; y < img.height(); ++y) {
        uint32 sy =
          Math::lerp(y, (uint32)0, img.height(), (uint32)0, format.polesHeight);
        for (uint32 x = 0; x < img.width(); ++x) {
            uint32 sx = Math::lerp(
              x, (uint32)0, img.width(), (uint32)0, format.polesWidth);
            Vector3 absPos(
              sx * format.poleSpacingX, 0.f, sy * format.poleSpacingY);
            MaskPixel maskPixel =
              TextureMaskCache::fuzziness(&(pChunkTerrain->block()),
                                          terrainPos,
                                          absPos,
                                          *pPaintBrush_,
                                          maskLayers);
  ...
  int32                              maxz)
{
    ...
    for (int32 z = minz; z <= maxz; ++z) {
        ...
        for (int32 x = minx; x <= maxx; ++x) {
            ...
            Vector3 absPos(float(x) * format.poleSpacingX,
                           0.f,
                           float(z) * format.poleSpacingY);

            float rx   = absPos.x - toolOffset.x;
            float rz   = absPos.z - toolOffset.z;
            float rlen = ::sqrtf(rx * rx + rz * rz);
            float adjustedStrength =
              (1.0f - rlen * falloff_) * currentStrength_ / 255.0f;
            if (adjustedStrength > 0.0f) {
                ...
                if (adjustedStrength > 0.0f) {
                    ...
                    uint8  newValue =
                      (uint8)std::min((int)(startValue + adjustedStrength),
                                      (int)maskValue.maxValue_);
                    ...
                    *pPixel     = value;
                    ...
                    if (value == 255) {
                        ...
                        if (sumOther != 0) {
                            int ratio =
                              ((255 - value) << STRENGTH_EXP) / sumOther;
                            for (size_t i = 0; i < level; ++i) {
                                uint8* pPixel = pLayers[i];
                                *pPixel =
                                  (uint8)((*pPixel * ratio + HALF_STRENGTH) >>
                                          STRENGTH_EXP);
                            }
                            for (size_t i = level + 1; i < numLayers; ++i) {
                                uint8* pPixel = pLayers[i];
                    ...
                    *pPixel     = value;
                    ...
                    if (value == 255) {
                        ...
                        if (sumOther != 0) {
                            ...
                            int ratio =
                              ((255 - sumDestAndMask) << STRENGTH_EXP) /
                              sumOther;
                            for (size_t i = 0; i < numLayers; ++i) {
                                if (!layerMask[i] && i != level) {
                                    uint8* pPixel = pLayers[i];
                                    *pPixel       = (uint8)((*pPixel * ratio +
                                                       HALF_STRENGTH) >>
                                                      STRENGTH_EXP);
                                }
                            }
