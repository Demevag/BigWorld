...
class TerrainTextureFunctor : public TerrainFunctor
{
    Py_Header(TerrainTextureFunctor, TerrainFunctor)
      ...
      public : explicit TerrainTextureFunctor(PyTypeObject* pType = &s_type_);
    ...
    void applyToSubBlock(EditorChunkTerrain&                chunkTerrain,
    ...
    PY_RW_ACCESSOR_ATTRIBUTE_DECLARE(TerrainPaintBrushPtr,
                                     paintBrush,
                                     paintBrush)
  ...
  protected:
    ...
    void createOverlayImage(EditorChunkTerrainPtr terrain,
                            Moo::Image<uint8>&    img) const;

    void invalidateOverlays();

    void applyToSubBlockNoTexMask(EditorChunkTerrain& chunkTerrain,
                                  const Vector3&      toolOffset,
                                  const Vector3&      chunkOffset,
                                  const TerrainUtils::TerrainFormat& format,
                                  int32                              minx,
    ...
    void applyToSubBlockTexMask(EditorChunkTerrain&                chunkTerrain,
                                const Vector3&                     toolOffset,
                                const Vector3&                     chunkOffset,
                                const TerrainUtils::TerrainFormat& format,
                                int32                              minx,
