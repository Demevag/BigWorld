...
namespace {
    // Globals to maintain a powf lookup table, which stores the return values
    // of powf( X, useFalloff_ ), where X is between 0.0 and 1.0.
    ...
    if (!applying_) {
        ...
        if (s_lastUseFalloff != useFalloff_) {
            ...
            for (int i = 0; i < POW_LOOKUP_FACTOR + 1; ++i) {
                s_powLookup[i] =
                  1.0f -
                  powf(1.0f - i / float(POW_LOOKUP_FACTOR), (float)exponent);
            }
    ...
    if (useFalloff_ == AVERAGE_FALLOFF) {
        ...
        if (pChunk) {
            ...
            if (pTerrain) {
                ...
                float   toolHalfSize = std::min(
                  tool.size() / 2.0f,
                  50.0f); // max 100x100 pixels averaged for performance.

                for (float z = -toolHalfSize; z < toolHalfSize; z += stepZ) {
                    for (float x = -toolHalfSize; x < toolHalfSize;
                         x += stepX) {
                        avgHeight_ += pTerrain->block().heightAt(
 ...
 */
bool TerrainHeightFilterFunctor::handleKeyEvent(const KeyEvent& keyEvent,
                                                Tool&           t)
{
    BW_GUARD;
    ...
    if (keyEvent.isKeyDown()) {
        ...
        switch (keyEvent.key()) {
            ...
            case KeyCode::KEY_PERIOD:
                ...
                int numFilters =
                  static_cast<int>(MatrixFilter::instance().size());
                int numExcluded = 0;
                ...
                if (numExcluded != numFilters) {
                    ...
                    WorldManager::instance().addCommentaryMsg(
                      LocaliseUTF8(
                        L"WORLDEDITOR/TERRAIN/TERRAIN_HEIGHT_FILTER_FUNCTOR/"
                        L"SELECT_FILDER",
                        filterIndex_,
                        filter_.name_.c_str()),
                      0);
 ...
 */
void TerrainHeightFilterFunctor::onFirstApply(EditorChunkTerrain& chunkTerrain)
{
    BW_GUARD;
  ...
  const TerrainUtils::TerrainFormat& format)
{
    ...
    return filterHalfWidth > int(format.visOffsX) ||
 ...
 *  @param chunkTerrain	The new terrain touched.
 */
void TerrainHeightFilterFunctor::onBeginApply(EditorChunkTerrain& chunkTerrain)
{
    BW_GUARD;
    ...
    if (filter_.noise_) {
        if (singleHeightMapCache_.isEmpty()) {
            ...
            for (size_t y = 0; y < noise.height(); ++y) {
                Terrain::TerrainHeightMap::PixelType* pp =
                  noise.getRow(static_cast<int>(y));
            ...
            singleHeightMapCache_.resize(noise.width() * xratio,
                                         noise.height() * yratio);
            for (uint32 y = 0; y < singleHeightMapCache_.height(); ++y) {
                for (uint32 x = 0; x < singleHeightMapCache_.width(); ++x) {
                    singleHeightMapCache_.set(
                      ...
                      y,
                      constant *
                        noise.getBicubic(float(x) / xratio, float(y) / yratio));
                }
            }
        }
    } else {
        if (considerNeighbours(chunkTerrain, format)) {
            ...
            Vector3 blockPos(chunkTerrain.chunk()->transform().applyToOrigin());
            spaceHeightMap_->add(blockPos[0], blockPos[2], heightMap.image());
        }
    }
}
  ...
  int32                              maxz)
{
    ...
    if (!filter_.noise_) {
        ...
        if (useNeighbours) {
            ...
            // Optimisation: assuming the transform is only translation, so we
            // can apply it here and just increment the coords in the X and Z
            // axes.
            Vector3 minCoords(chunkTerrain.chunk()->transform().applyPoint(
              Vector3((minx - filterHalfWidth) * format.poleSpacingX,
                      0.0f,
                      (minz - filterHalfHeight) * format.poleSpacingY)));
            Vector3 maxCoords(chunkTerrain.chunk()->transform().applyPoint(
              Vector3((maxx + filterHalfWidth) * format.poleSpacingX,
                      0.0f,
                      (maxz + filterHalfHeight) * format.poleSpacingY)));
            bool    imgOk = spaceHeightMap_->getRect(minCoords[0],
                                                  minCoords[2],
                                                  maxCoords[0],
                                                  maxCoords[2],
        ...
        Vector3     spaceCoords(chunkTerrain.chunk()->transform().applyPoint(
          Vector3((minx + xVisOffs) * format.poleSpacingX,
                  0.0f,
        ...
        if (zIdx + zVisOffs >= 0 && zIdx + zVisOffs < image.height()) {
            ...
            for (int32 xIdx = minx; xIdx <= maxx; ++xIdx) {
                if (xIdx + xVisOffs >= 0 && xIdx + xVisOffs < image.width()) {
                    ...
                    float falloffFactor =
                      (1.0f - (std::sqrt(dx * dx + dz * dz) * falloff_));
                    if (falloffFactor > 0.0f) {
                        float newPix = 0.0f;

                        int powIdx =
                          int(falloffFactor * float(POW_LOOKUP_FACTOR));
                        ...
                        falloffFactor =
                          useFalloff_ ? s_powLookup[powIdx] : 1.0f;

                        if (useFalloff_ == AVERAGE_FALLOFF) {
                            newPix = (*pix) * (1.0f - strength_) +
                                     avgHeight_ * strength_;
                            if (fabs((*pix) - avgHeight_) <
                                fabs((*pix) - newPix)) {
                                newPix = avgHeight_;
                            }
                        } else {
                            ...
                            if (!filter_.noise_) {
                                BW::vector<float>::iterator kernelValue =
                                  filter_.kernel_.begin();
                                int xOffset;
                                int zOffset;
                                if (useNeighbours) {
                                ...
                                int startPadX =
                                  filterMinX < 0 ? -filterMinX : 0;
                                int startPadZ =
                                  filterMinZ < 0 ? -filterMinZ : 0;
                                int endPadX = filterMaxX >= mapWidth
                                                ? filterMaxX - mapWidth + 1
                                                : 0;
                                int endPadZ = filterMaxZ >= mapHeight
                                                ? filterMaxZ - mapHeight + 1
                                                : 0;

                                Terrain::TerrainHeightMap::PixelType* oldPix;

                                kernelValue +=
                                  startPadX + startPadZ * filter_.kernelWidth_;

                                for (int z = filterMinZ + startPadZ;
                                     z <= filterMaxZ - endPadZ;
                                     z++) {
                                    oldPix = singleHeightMapCache_.getRow(z) +
                                             filterMinX + startPadX;

                                    for (int x = filterMinX + startPadX;
                                         x <= filterMaxX - endPadX;
                                         x++) {
                                        newPix +=
                                          (*oldPix++) * (*kernelValue++);
                                    }
                                ...
                                float mx =
                                  fmod(noiseCoordX,
                                       singleHeightMapCache_.width() * 2.0f);
                                if (mx >= singleHeightMapCache_.width()) {
                                    mx = singleHeightMapCache_.width() * 2.0f -
                                         mx - 1.0f;
                                }
                                float my =
                                  fmod(noiseCoordY,
                                       singleHeightMapCache_.height() * 2.0f);
                                if (my >= singleHeightMapCache_.height()) {
                                    my = singleHeightMapCache_.height() * 2.0f -
                                         my - 1.0f;
                                }

                                // actual noise painting
                                newPix +=
                                  image.get(xIdx + xVisOffs, zIdx + zVisOffs);
                                newPix +=
                                  filter_.strengthRatio_ *
                                  singleHeightMapCache_.getBicubic(mx, my);
                            }
                        }
                        (*pix) = (*pix) * (1.0f - falloffFactor) +
                                 newPix * falloffFactor;
 ...
 *  @param chunkTerrain	The new terrain touched.
 */
void TerrainHeightFilterFunctor::onEndApply(EditorChunkTerrain& chunkTerrain)
{
    BW_GUARD;
 ...
 */
void TerrainHeightFilterFunctor::onLastApply(EditorChunkTerrain& chunkTerrain)
{
    BW_GUARD;
