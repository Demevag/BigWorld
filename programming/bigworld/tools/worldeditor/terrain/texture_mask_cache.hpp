...
struct MaskPixel
{
    ...
    uint8
      maxValue_; // maximum value that a pixel can have if painted using mask
};
#pragma pack()
...
class TextureMaskCache : public Singleton<TextureMaskCache>
{
  public:
    ...
    static void beginFuzziness(Terrain::EditorBaseTerrainBlockPtr terrain,
                               const TerrainPaintBrush&           brush,
                               BW::vector<bool>&                  maskLayers);

    static MaskPixel fuzziness(Terrain::EditorBaseTerrainBlockPtr terrain,
                               const Vector3&                     terrainPos,
                               const Vector3&                     pos,
                               const TerrainPaintBrush&           brush,
                               const BW::vector<bool>&            maskLayers);

    static void endFuzziness(Terrain::EditorBaseTerrainBlockPtr terrain,
                             const TerrainPaintBrush&           brush,
                             const BW::vector<bool>&            maskLayers);
