...
namespace {
    ...
    const static struct
    {
        ...
        { L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/RECREATE_SPACE/STAGE1",
          .01f },
        { L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/RECREATE_SPACE/STAGE2",
          .01f },
        { L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/RECREATE_SPACE/STAGE3",
          .35f },
        { L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/RECREATE_SPACE/STAGE4",
          .09f },
        { L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/RECREATE_SPACE/STAGE5",
          .09f },
        { L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/RECREATE_SPACE/STAGE6",
          .10f },
        { L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/RECREATE_SPACE/STAGE7",
          .35f }
    };

    static AutoConfigString s_spaceRecreatorBlankTexture(
      "system/emptyTerrainTextureBmp",
      "helpers/maps/aid_builder.tga");

    static inline bool intervalsIntersect(const float chunkMin,
                                          const float chunkMax,
                                          const float otherMin,
                                          const float otherMax,
                                          bool        includeStart,
                                          bool        includeEnd)
    {
        return (includeStart ? (chunkMin <= otherMax)
                             : (chunkMin < otherMax)) &&
               (includeEnd ? (otherMin <= chunkMax) : (otherMin < chunkMax));
    }

    // Do these two chunks overlap? We ignore the bottom and right edges of both
    // chunks
    static inline bool chunkTouchesChunk(const BoundingBox& chunkBB,
                                         const BoundingBox& otherBB)
    {
        ...
        return intervalsIntersect(chunkMin.x,
                                  chunkMax.x,
                                  otherMin.x,
                                  otherMax.x,
                                  false,
                                  false) &&
               intervalsIntersect(chunkMin.y,
                                  chunkMax.y,
                                  otherMin.y,
                                  otherMax.y,
                                  false,
                                  false) &&
               intervalsIntersect(
                 chunkMin.z, chunkMax.z, otherMin.z, otherMax.z, false, false);
    }

    // Does otherPos fall inside chunkBB? We ignore the bottom and right edges
    // in all chunks except the last row or column
    static inline bool fallsInChunk(const BoundingBox& chunkBB,
                                    const BoundingBox& otherBB,
                                    const bool         finalXChunk,
                                    const bool         finalZChunk)
    {
        const Vector3& chunkMin = chunkBB.minBounds();
        const Vector3& chunkMax = chunkBB.maxBounds();
        const Vector3& otherMin = otherBB.minBounds();
        ...
        return intervalsIntersect(chunkMin.x,
                                  ...
                                  finalXChunk) &&
               intervalsIntersect(
                 chunkMin.y, chunkMax.y, otherMin.y, otherMax.y, true, true) &&
               intervalsIntersect(chunkMin.z,
                                  chunkMax.z,
                                  otherMin.z,
                                  otherMax.z,
                                  true,
                                    ...
                                    const bool         finalZChunk)
    {
        ...
        return otherPos.x >= min.x && otherPos.y >= min.y &&
               otherPos.z >= min.z &&
...
class SpaceRecreator::Impl
{
  public:
    ...
    FixChunkLinkResult fixChunkLink(DataSectionPtr chunkLinkDS,
                                    bool           knownToBeChunkLinks = false);

    SpaceRecreator&                                    parent_;
    BW::unordered_map<BW::string, BW::set<BW::string>> chunkVloReferences;
};

SpaceRecreator::SpaceRecreator(const BW::string&  spaceName,
                               float              chunkSize,
                               int                width,
                               int                height,
  ...
  , pImpl(new Impl(*this))
{
    ...
    TerrainUtils::terrainSize(
      "", srcChunkSize, srcMinX, srcMinZ, srcMaxX, srcMaxZ);
    srcSpaceBB_ = BoundingBox(
      Vector3(srcMinX * srcChunkSize, MIN_CHUNK_HEIGHT, srcMinZ * srcChunkSize),
      Vector3((srcMaxX + 1) * srcChunkSize,
              MAX_CHUNK_HEIGHT,
              (srcMaxZ + 1) * srcChunkSize));
...
bool SpaceRecreator::createInternal()
{
    ...
    if (WorldManager::instance().connection().enabled() &&
        ...
        if (!WorldManager::instance().connection().changeSpace(
              BWResource::dissolveFilename(spaceName_)) ||
            !WorldManager::instance().connection().lock(
              GridRect(GridCoord(minX_ - 1, minZ_ - 1),
                       GridCoord(maxX_ + 1, maxZ_ + 1)),
              "create space")) {
...
bool SpaceRecreator::shouldAbort()
{
    ...
    if (ProgressBar::getCurrentProgressBar() &&
        ProgressBar::getCurrentProgressBar()->isCancelled()) {
...
void SpaceRecreator::calcBounds(int srcMinX,
                                int srcMinZ,
                                int srcMaxX,
                                int srcMaxZ,
                                int width,
...
const BW::string SpaceRecreator::createSpaceSettings()
{
    ...
    DataSectionPtr srcSpaceDS =
      BWResource::openSection(BWResource::dissolveFilename(srcSpaceName_));
    DataSectionPtr spaceDS =
      BWResource::openSection(BWResource::dissolveFilename(spaceName_));

    DataSectionPtr srcSpaceSettings =
      srcSpaceDS->openSection(SPACE_SETTING_FILE_NAME);
    DataSectionPtr spaceSettings =
      spaceDS->newSection(SPACE_SETTING_FILE_NAME, XMLSection::creator());
    if (srcSpaceSettings && spaceSettings) {
        ...
        DataSectionPtr srcTerrainSettings =
          srcSpaceSettings->openSection("terrain");

        spaceSettings->deleteSection("terrain");
        DataSectionPtr terrainSettingsDS_ =
          spaceSettings->newSection("terrain");
        ...
        pTerrainSettings_->heightMapEditorSize(
          terrainInfo_.heightMapEditorSize_);
        ...
        result = spaceSettings->save()
                   ? ""
                   : ("Unable to save " + SPACE_SETTING_FILE_NAME + "\n");
    } else {
    ...
    DataSectionPtr locations =
      spaceDS->newSection("locations.xml", XMLSection::creator());
...
bool SpaceRecreator::normaliseTerrainBlockBlends(
  Terrain::EditorBaseTerrainBlock& block)
{
    ...
    // Assumes all the layers are the same dimensions... But that assumption
    // appears everywhere Plus we created these texture layers earlier, so they
    // really should be.
    ...
    for (uint32 y = 0; y < height; ++y) {
        ...
        for (uint32 x = 0; x < width; ++x) {
            ...
            for (size_t i = 0; i < numLayers; ++i) {
                Terrain::TerrainTextureLayer::PixelType value =
                  pLayerRows[i][x];
                sum += value;
            }
            ...
            // So now we know our layer total is difference too low, or
            // -difference too high Spread the difference across all the
            // non-zero layers, with any leftover going to the highest weighted
            // layer (the lowest one we see)
            ...
            for (size_t i = 0; i < numLayers; ++i) {
                Terrain::TerrainTextureLayer::PixelType* pPixel =
                  pLayerRows[i] + x;
                if (*pPixel == 0)
                    continue;
                if (*pPixel > maxValue) {
                ...
                sum -= *pPixel;
                *pPixel = (Terrain::TerrainTextureLayer::PixelType)(
                  (*pPixel * ratio + HALF_STRENGTH) >> STRENGTH_EXP);
                sum += *pPixel;
            }
            MF_ASSERT(maxLayer !=
                      Terrain::EditorBaseTerrainBlock::INVALID_LAYER);
...
bool SpaceRecreator::createChunk(int x, int z)
{
    ...
    {
        // stop verifying tag name, this will disable unnecessary warning
        // message
        XMLSectionTagCheckingStopper tagCheckingStopper;
        chunkDS =
          spaceDS->openSection(name + ".chunk", true, XMLSection::creator());
        cdataDS =
          spaceDS->openSection(name + ".cdata", true, ZipSection::creator());
    }

    // Now for the actual work. We need to look at every source chunk that
    // overlaps this chunk and copy any relevant information in. Use a
    // BoundedChunkWalker for the source chunks this chunk overlaps? Overlaps
    // are: [ ( x * chunkSize, z * chunkSize ), ( x + 1 * chunkSize, z + 1 *
    // chunkSize ) )... Bounded works on grid, not chunk.
    const BoundingBox chunkBB = BoundingBox(
      Vector3(
        x * chunkSize_ + xOffset_, MIN_CHUNK_HEIGHT, z * chunkSize_ + zOffset_),
      Vector3(x * chunkSize_ + xOffset_ + chunkSize_,
              MAX_CHUNK_HEIGHT,
              z * chunkSize_ + zOffset_ + chunkSize_));

    Matrix chunkTransform;
    chunkTransform.setTranslate(
      x * chunkSize_ + xOffset_, 0.f, z * chunkSize_ + zOffset_);

    Matrix chunkTransformInv;
    chunkTransformInv.setTranslate(
      -x * chunkSize_ - xOffset_, 0.f, -z * chunkSize_ - zOffset_);

    uint16    srcGridWidth, srcGridHeight;
    GridCoord srcLocalToWorld(0, 0);
    float     srcGridSize;
    if (!TerrainUtils::spaceSettings(srcSpaceName_,
                                     srcGridSize,
                                     srcGridWidth,
                                     srcGridHeight,
                                     srcLocalToWorld)) {
    ...
    underLayerIndex =
      (uint32)pTerrainBlock->insertTextureLayer(blendSize, blendSize);
    ...
    { // Scope for the holders
        Terrain::TerrainTextureLayer& terrainTextureLayer =
          pTerrainBlock->textureLayer(underLayerIndex);
        terrainTextureLayer.textureName(s_spaceRecreatorBlankTexture.value());
        // See PageTerrainTexture::updateProjection... Kinda.
        terrainTextureLayer.uProjection(
          Vector4(1.f / chunkSize_, 0.f, 0.f, 0.f));
        terrainTextureLayer.vProjection(
          Vector4(0.f, 0.f, -1.f / chunkSize_, 0.f));
        Terrain::TerrainTextureLayerHolder terrainLayerHolder(
          &terrainTextureLayer, false);
        Terrain::TerrainTextureLayer::ImageType& textureLayerImage =
          terrainTextureLayer.image();
        // Fill in any terrain that's outside our srcSpaceBB
        const float spacingX =
          terrainTextureLayer.blockSize() / terrainTextureLayer.width();
        const float halfSpacingX = spacingX / 2.f;
        const float spacingZ =
          terrainTextureLayer.blockSize() / terrainTextureLayer.height();
        const float halfSpacingZ = spacingZ / 2.f;

        for (uint32 x = 0; x < terrainTextureLayer.width(); ++x) {
            const float xWorldPos =
              chunkBB.minBounds().x + (x * spacingX) + halfSpacingX;
            MF_ASSERT(xWorldPos <= chunkBB.maxBounds().x);
            for (uint32 z = 0; z < terrainTextureLayer.height(); ++z) {
                const float zWorldPos =
                  chunkBB.minBounds().z + (z * spacingZ) + halfSpacingZ;
                MF_ASSERT(zWorldPos <= chunkBB.maxBounds().z);
                if (!srcSpaceBB_.intersects(
                      Vector3(xWorldPos, 0.f, zWorldPos))) {
                    textureLayerImage.set(
                      x,
                      z,
                      std::numeric_limits<
                        Terrain::TerrainTextureLayer::PixelType>::max());
    ...
    // Walker's co-ords are in local grid space (i.e. [(0,0) -> (srcGridWidth,
    // srcGridHeight)) We need to expand one chunk in each direction because if
    // the old and new grid line up, then any badly-positioned objects (i.e.
    // with local transform X or Z < 0 or >= gridSize will be lost. Highlands
    // contains such objects at time of writing, for example: 00020000o:
    // fd_rock_sharp_medium.model has x >= 100.f 0002ffffo: beast_entrance.model
    // has x < 0
    BoundedChunkWalker walker(srcMinGridX - srcLocalToWorld.x - 1,
                              srcMinGridZ - srcLocalToWorld.y - 1,
                              srcMaxGridX - srcLocalToWorld.x + 1,
                              srcMaxGridZ - srcLocalToWorld.y + 1);
    walker.spaceInformation(SpaceInformation(
      srcSpaceName_, srcLocalToWorld, srcGridWidth, srcGridHeight));
    ...
    while (walker.nextTile(srcChunkName, srcGridX, srcGridZ)) {
        ...
        Chunk* srcChunk =
          ChunkManager::instance().findChunkByName(srcChunkName, srcMapping);
        MF_ASSERT(srcChunk);
        ...
        MF_ASSERT(!srcChunk->loading() && srcChunk->loaded() &&
                  srcChunk->isBound());
        // WorldManager::instance().workingChunk( srcChunk, false );

        const BoundingBox srcChunkBB =
          BoundingBox(Vector3(srcChunk->x() * srcGridSize,
                              MIN_CHUNK_HEIGHT,
                              srcChunk->z() * srcGridSize),
                      Vector3(srcChunk->x() * srcGridSize + srcGridSize,
                              MAX_CHUNK_HEIGHT,
                              srcChunk->z() * srcGridSize + srcGridSize));
        /* This is always being true, because Visual Studio hates me...
         * So leave it commented out for now.
         * Because of the way we process individual terrain blocks, we need the
         * two chunks to line up correctly, or things get weird. This also keeps
         * srcChunkBB consistent with the ChunkWalker's calculations
         */
#if 0
        ...
        /*
        DEBUG_MSG( "createChunk %s (%f, %f) => (%f,%f): considering source chunk
        %s (%f,%f) => (%f,%f)\n", name.c_str(), chunkBB.minBounds().x,
        chunkBB.minBounds().z, chunkBB.maxBounds().x, chunkBB.maxBounds().z,
            srcChunkName.c_str(),
            srcChunkBB.minBounds().x, srcChunkBB.minBounds().z,
            srcChunkBB.maxBounds().x, srcChunkBB.maxBounds().z );
        */

        // We need this to be able to skip it while iterating the static
        // objects.
        EditorChunkTerrain* srcChunkTerrain = dynamic_cast<EditorChunkTerrain*>(
          ChunkTerrainCache::instance(*srcChunk).pTerrain());
        MF_ASSERT(ChunkTerrainCache::instance(*srcChunk).pTerrain() == NULL ||
                  srcChunkTerrain != NULL);

        typedef BW::vector<ChunkItemPtr> Items;
        const Items&                     staticItems = srcChunk->staticItems();

        for (Items::const_iterator it = staticItems.begin();
             it != staticItems.end();
             ++it) {
            bool wrotePos = false;
            ...
            // EditorChunkLinks are created when their EditorChunkLinkables are
            // created, they are not stored as ChunkItems in the XML
            ...
            EditorChunkOverlapper* overlapper =
              dynamic_cast<EditorChunkOverlapper*>(item.getObject());
            EditorChunkAmbientLight* ambient =
              dynamic_cast<EditorChunkAmbientLight*>(item.getObject());
            if (overlapper != NULL) {
                // EditorChunkOverlappers need to appear in any chunk their
                // internal chunk overlaps
                const BoundingBox& overlapperBB =
                  overlapper->pOverlappingChunk()->boundingBox();
                if (!chunkBB.intersects(overlapperBB))
                    continue;

                const BW::string& overlapperID =
                  overlapper->pOverlappingChunk()->identifier();
                ...
                // We're spreading the ChunkAmbientLight to anywhere that
                // touches its original chunk, as they don't actually have a
                // real position.
                // TODO: Review this, perhaps we should be using chunk centre?
                if (!chunkTouchesChunk(chunkBB, srcChunkBB))
                    continue;

                // Need to avoid duplicate ChunkAmbientLights and log if we kill
                // one that doesn't match the one we did keep...
                if (seenAmbient) {
                    if (seenAmbientColour != ambient->colour() ||
                        seenAmbientMultiplier != ambient->multiplier())
                        WARNING_MSG(
                          "createChunk %s: ChunkAmbientLight read from %s has "
                          "colour %f,%f,%f and multipler %f, "
                          "ignoring in favour of previous ChunkAmbientLight "
                          "read from %s with colour %f,%f,%f and multipler "
                          "%f\n",
                          name.c_str(),
                          srcChunkName.c_str(),
                          ambient->colour().r * 255.f,
                          ambient->colour().g * 255.f,
                          ambient->colour().b * 255.f,
                          ambient->multiplier(),
                          seenAmbientSrcChunk.c_str(),
                          seenAmbientColour.r * 255.f,
                          seenAmbientColour.g * 255.f,
                          seenAmbientColour.b * 255.f,
                          seenAmbientMultiplier);
                    continue;
                }
                seenAmbient           = true;
                ...
                wrotePos            = true;
            } else if (!fallsInChunk(chunkBB,
                                     dstTransform.applyToOrigin(),
                                     x == maxX_,
                                     z == maxZ_)) {
                // For anything else, check if they lie within the destination
                // chunk dstTransform is in WorldSpace here
                continue;
            }
            ...
            DEBUG_MSG( "createChunk %s: (%f, %f, %f) <= %s (%f, %f, %f): %s\n",
                name.c_str(), dstTransform._41, dstTransform._42,
            dstTransform._43, srcChunkName.c_str(), srcTransform._41,
            srcTransform._42, srcTransform._43, itemClass.c_str());
            */

            if (item->isEditorChunkStationNode()) {
                EditorChunkStationNode* stationNode =
                  dynamic_cast<EditorChunkStationNode*>(item.getObject());
            ...
            if (item->isEditorEntity()) {
                ...
                guidChunks_.insert(
                  std::make_pair(guid, std::make_pair(srcChunkName, name)));
            ...
            DataSectionPtr dstItemDS =
              chunkDS->newSection(srcItemDS->sectionName());
            ...
            if (dstItemDS->openSection("position")) {
                wrotePos |= true;
                dstItemDS->writeVector3("position",
                                        dstTransform.applyToOrigin());
            }
            if (dstItemDS->openSection("direction"))
                dstItemDS->writeVector3("direction",
                                        dstTransform.applyToUnitAxisVector(2));

            if (!wrotePos)
                WARNING_MSG("createChunk %s: %s: No transform or position "
                            "attribute to update\n",
                            name.c_str(),
                            itemClass.c_str());

            // I'm not sure we need to do this, but there's no other way for the
            // ChunkItem to write out anything it wants to put into the cdata.
            // Mind you, every implementation of this is a no-op currently, and
            // there's no promise that any future implementation won't try to
            // access srcChunk or any other state which relates to the source
            // space rather than the new space.
            item->edChunkSaveCData(cdataDS);
        }

        // Process the terrain from this chunk that overlaps the new chunk
        // We don't want to process terrain from a chunk whose min border
        // happens to match the max border of our new chunk.
        ...
        if (copyTerrain) {
            ...
            if (srcChunkTerrain) {
                hasTerrain       = true;
                pSrcTerrainBlock = dynamic_cast<Terrain::EditorTerrainBlock2*>(
                  &srcChunkTerrain->block());
            } else {
                // Attempt to load the texture manually.
                // We have to do this because the height map module needs the
                // texture data even if there is not texture on the chunk.
                const BW::string filename =
                  BWResource::dissolveFilename(srcSpaceName_) + "/" +
                  srcChunkName + ".cdata/terrain";
                baseBlock = Terrain::BaseTerrainBlock::loadBlock(
                  filename,
                  Matrix::identity,
                  Vector3::zero(),
        ...
        if (copyTerrain) {
            ...
            // Copy maps: HeightMap
            { // Scope for the holders
                Terrain::TerrainHeightMap& srcTerrainHeightMap =
                  pSrcTerrainBlock->heightMap();

                Terrain::TerrainHeightMap& terrainHeightMap =
                  pTerrainBlock->heightMap();
                Terrain::TerrainHeightMapHolder terrainHeightMapHolder(
                  &terrainHeightMap, false);
                Terrain::TerrainHeightMap::ImageType& heightMapImage =
                  terrainHeightMap.image();

                // TerrainUtils::patchEdges actually fills the blocksWidth()'d
                // pole from the previous pole which means we don't need to
                // worry about overlapping if the chunk edges line up exactly on
                // that pole. This does mean the maxX and maxZ rows of triangles
                // in each space is flat
                for (uint32 x = 0; x < terrainHeightMap.blocksWidth(); ++x) {
                    const float xWorldPos =
                      chunkBB.minBounds().x + (x * terrainHeightMap.spacingX());
                    MF_ASSERT(xWorldPos <= chunkBB.maxBounds().x);
                    for (uint32 z = 0; z < terrainHeightMap.blocksHeight();
                         ++z) {
                        const float zWorldPos =
                          chunkBB.minBounds().z +
                          (z * terrainHeightMap.spacingZ());
                        MF_ASSERT(zWorldPos <= chunkBB.maxBounds().z);
                        BoundingBox heightPoleBox;
                        heightPoleBox.addBounds(
                          Vector3(xWorldPos, 0.f, zWorldPos));
                        heightPoleBox.expandSymmetrically(
                          terrainHeightMap.spacingX() / 2.f,
                          0.f,
                          terrainHeightMap.spacingZ() / 2.f);
                        if (!fallsInChunk(srcChunkBB,
                                          heightPoleBox,
                                          srcGridX == srcMapping->maxGridX(),
                                          srcGridZ == srcMapping->maxGridY()))
                            continue;
                        const float height = srcTerrainHeightMap.heightAt(
                          xWorldPos - srcChunkBB.minBounds().x,
                          zWorldPos - srcChunkBB.minBounds().z);
                        heightMapImage.set(
                          x + terrainHeightMap.xVisibleOffset(),
            ...
            // Holemap
            { // Scope for the holders
                Terrain::TerrainHoleMap& srcTerrainHoleMap =
                  pSrcTerrainBlock->holeMap();

                Terrain::TerrainHoleMap& terrainHoleMap =
                  pTerrainBlock->holeMap();
                Terrain::TerrainHoleMapHolder terrainHoleMapHolder(
                  &terrainHoleMap, false);
                Terrain::TerrainHoleMap::ImageType& holeMapImage =
                  terrainHoleMap.image();

                const float spacingX =
                  terrainHoleMap.blockSize() / terrainHoleMap.width();
                const float halfSpacingX = spacingX / 2.f;
                const float spacingZ =
                  terrainHoleMap.blockSize() / terrainHoleMap.height();
                const float halfSpacingZ = spacingZ / 2.f;
                ...
                for (uint32 x = 0; x < terrainHoleMap.width(); ++x) {
                    ...
                    for (uint32 z = 0; z < terrainHoleMap.height(); ++z) {
                        ...
                        holeBox.expandSymmetrically(
                          halfSpacingX, 0.f, halfSpacingZ);
                        if (!fallsInChunk(srcChunkBB,
                                          holeBox,
                                          srcGridX == srcMapping->maxGridX(),
                                          srcGridZ == srcMapping->maxGridY()))
            ...
            // Texture layers
            for (uint32 srcLayerIndex = 0;
                 srcLayerIndex < pSrcTerrainBlock->numberTextureLayers();
                 ++srcLayerIndex) { // Scope for the holders
                Terrain::TerrainTextureLayer& srcTerrainTextureLayer =
                  pSrcTerrainBlock->textureLayer(srcLayerIndex);
                Terrain::TerrainTextureLayerHolder srcTerrainTextureLayerHolder(
                  &srcTerrainTextureLayer, true);
                const Terrain::TerrainTextureLayer::ImageType&
                  srcTextureLayerImage = srcTerrainTextureLayer.image();

                const BW::string& textureName =
                  srcTerrainTextureLayer.textureName();
                const BW::string& bumpTextureName =
                  srcTerrainTextureLayer.bumpTextureName();
                ...
                if (layerIndex ==
                    Terrain::EditorBaseTerrainBlock::INVALID_LAYER) {
                    layerIndex =
                      pTerrainBlock->insertTextureLayer(blendSize, blendSize);
                    newLayer = true;
                    MF_ASSERT(layerIndex != underLayerIndex);
                }
                MF_ASSERT(layerIndex !=
                          Terrain::EditorBaseTerrainBlock::INVALID_LAYER);
                ...
                {
                    Terrain::TerrainTextureLayer& terrainTextureLayer =
                      pTerrainBlock->textureLayer(layerIndex);
                    Terrain::TerrainTextureLayerHolder
                      terrainTextureLayerHolder(&terrainTextureLayer, false);
                    Terrain::TerrainTextureLayer::ImageType& textureLayerImage =
                      terrainTextureLayer.image();
                    ...
                    const float spacingX = terrainTextureLayer.blockSize() /
                                           terrainTextureLayer.width();
                    const float halfSpacingX = spacingX / 2.f;
                    const float spacingZ     = terrainTextureLayer.blockSize() /
                                           terrainTextureLayer.height();
                    const float halfSpacingZ = spacingZ / 2.f;
                    ...
                    for (uint32 x = 0; x < terrainTextureLayer.width(); ++x) {
                        const float xWorldPos =
                          chunkBB.minBounds().x + (x * spacingX) + halfSpacingX;
                        MF_ASSERT(xWorldPos <= chunkBB.maxBounds().x);
                        const float srcX = Math::lerp<float, float>(
                          xWorldPos,
                          srcChunkBB.minBounds().x,
                          srcChunkBB.maxBounds().x,
                          0.f,
                          (float)srcTextureLayerImage.width());
                        if (srcX < 0.f)
                            continue;
                        if (srcX >= (float)srcTextureLayerImage.width())
                            break;
                        for (uint32 z = 0; z < terrainTextureLayer.height();
                             ++z) {
                            const float zWorldPos = chunkBB.minBounds().z +
                                                    (z * spacingZ) +
                                                    halfSpacingZ;
                            MF_ASSERT(zWorldPos <= chunkBB.maxBounds().z);
                            const float srcZ = Math::lerp<float, float>(
                              zWorldPos,
                              srcChunkBB.minBounds().z,
                              srcChunkBB.maxBounds().z,
                              0.f,
                              (float)srcTextureLayerImage.height());
                            if (srcZ < 0.f)
                                continue;
                            ...
                            blendBox.addBounds(
                              Vector3(xWorldPos, 0.f, zWorldPos));
                            blendBox.expandSymmetrically(
                              halfSpacingX, 0.f, halfSpacingZ);
                            IF_NOT_MF_ASSERT_DEV(
                              fallsInChunk(srcChunkBB,
                                           blendBox,
                                           srcGridX == srcMapping->maxGridX(),
                                           srcGridZ == srcMapping->maxGridY()))
                            continue;
                            ...
                            MF_ASSERT(
                              oldBlend <=
                              std::numeric_limits<Terrain::TerrainTextureLayer::
                                                    PixelType>::max());

                            uint32 blend = (uint32)floor(
                              srcTextureLayerImage.getBilinear(srcX, srcZ) +
                              0.5);
                            MF_ASSERT(
                              blend <=
                              std::numeric_limits<Terrain::TerrainTextureLayer::
                            ...
                            textureLayerImage.set(
                              x,
                              z,
                              (Terrain::TerrainTextureLayer::PixelType)(
                                blend + oldBlend));
        ...
        // TODO: Is there a better way than this? Can we tick the BigWorld
        // render loop?
    ...
    if (pTerrainBlock->numberTextureLayers() > 0) {
        ...
        TerrainTextureUtils::deleteEmptyLayers(
          *pTerrainBlock, TerrainTextureUtils::REMOVE_LAYER_THRESHOLD);
        pTerrainBlock->optimiseLayers(
          TerrainTextureUtils::PROJECTION_COMPARISON_EPSILON);
...
const BW::string SpaceRecreator::createChunks(bool withPurge)
{
    ...
    ProgressBarTask progressTask(LocaliseUTF8(STAGES[2].name),
                                 (float)(maxX_ - minX_ + 1) *
                                   (maxZ_ - minZ_ + 1),
                                 true);
    ...
    for (int x = minX_; x <= maxX_ && result.empty(); ++x) {
        ...
        for (int z = minZ_; z <= maxZ_ && result.empty(); ++z) {
            result += createChunk(x, z)
                        ? ""
                        : ("Unable to create chunk " + chunkName(x, z) + "\n");
            progressTask.step();
            if (shouldAbort())
...
bool copyVLODataSectionAndChangePosition(DataSectionPtr    srcDS,
                                         DataSectionPtr    dstDS,
...
const BW::string SpaceRecreator::copyAssets()
{
...

    {
        ProgressBarTask task(
          LocaliseUTF8(STAGES[3].name), (float)overlappers_.size(), true);

        BW::set<BW::string>::const_iterator it;
        for (it = overlappers_.begin();
             it != overlappers_.end() && result.empty();
             ++it) {
            const BW::string& name = *it;
            DataSectionPtr    srcChunkDS =
              srcSpaceDS->openSection(name + ".chunk");
            DataSectionPtr chunkDS =
              spaceDS->newSection(name + ".chunk", XMLSection::creator());
            if (srcChunkDS && chunkDS) {
                ...
                if (chunkDS->openSection(TRANSFORM_PROP_NAME)) {
                    Matrix origTransform =
                      chunkDS->readMatrix34(TRANSFORM_PROP_NAME);
                    Matrix offset(Matrix::identity);
                    offset.translation(
                      Vector3((float)-xOffset_, 0.0f, (float)-zOffset_));
                ...
                result += chunkDS->save()
                            ? ""
                            : ("Failed to save " + name + ".chunk\n");
            } else {
                result += "Failed to copy " + name + ".chunk\n";
            }

            DataSectionPtr srcCdataDS =
              srcSpaceDS->openSection(name + ".cdata");
            DataSectionPtr cdataDS =
              spaceDS->newSection(name + ".cdata", ZipSection::creator());
            if (srcCdataDS && cdataDS) {
                cdataDS->copy(srcCdataDS);
                result += cdataDS->save()
                            ? ""
                            : ("Failed to save " + name + ".cdata\n");
            } else {
                result += "Failed to copy " + name + ".cdata\n";
            }

            // Need to scan the overlapper for VLOs, StationGraph references and
            // EditorChunkItemLinkables
            Chunk* srcChunk =
              ChunkManager::instance().findChunkByName(name, srcMapping);
            MF_ASSERT(srcChunk);
            bool wasLoaded = srcChunk->loaded();
            if (!wasLoaded) {
                ChunkManager::instance().loadChunkNow(srcChunk);
            ...
            const Vector3 overlapperOwnerPosition =
              srcChunk->transform().applyToOrigin();
            const BW::string srcOutsideChunkName =
              srcMapping->outsideChunkIdentifier(overlapperOwnerPosition);
            // Open-coded GeometryMapping::outsideChunkIdentifier
            const int outsideChunkGridX =
              int(floorf((overlapperOwnerPosition.x - xOffset_) / chunkSize_));
            const int outsideChunkGridZ =
              int(floorf((overlapperOwnerPosition.z - zOffset_) / chunkSize_));
            const BW::string outsideChunkName =
              chunkName(outsideChunkGridX, outsideChunkGridZ);
            ...
            for (Items::const_iterator it = staticItems.begin();
                ...
                if (item->edIsVLO()) {
                    ChunkVLO* vlo = dynamic_cast<ChunkVLO*>(item.getObject());
                    MF_ASSERT(vlo);
                    // Doesn't need a bounds check, as indoor chunks are not
                    // changed.
                ...
                if (item->isEditorEntity()) {
                    ...
                    const BW::string guid = entity->chunkItemLinker()->guid();
                    MF_ASSERT(guidChunks_.count(guid) == 0);
                    guidChunks_.insert(std::make_pair(
                      guid,
                      std::make_pair(srcOutsideChunkName, outsideChunkName)));
                    itemsWithPossibleBrokenLinks.insert(item);
                }

                if (item->isEditorUserDataObject()) {
                    EditorChunkUserDataObject* udo =
                      dynamic_cast<EditorChunkUserDataObject*>(
                        item.getObject());
                    MF_ASSERT(udo);
                    // Need to fix up any references to this object later
                    const BW::string guid = udo->chunkItemLinker()->guid();
    ...
    {
        ProgressBarTask task(
          LocaliseUTF8(STAGES[4].name), (float)stationGraphs_.size(), true);
        BW::set<BW::string>::const_iterator it;
        for (it = stationGraphs_.begin();
             it != stationGraphs_.end() && result.empty();
             ++it) {
            const BW::string& name = *it;
            DataSectionPtr    srcGraphDS =
              srcSpaceDS->openSection(name + ".graph");
            DataSectionPtr graphDS =
              spaceDS->newSection(name + ".graph", XMLSection::creator());
            if (srcGraphDS && graphDS) {
                graphDS->copy(srcGraphDS);
                result += graphDS->save()
                            ? ""
                            : ("Failed to save " + name + ".graph\n");
            } else {
    ...
    {
        ProgressBarTask task(
          LocaliseUTF8(STAGES[5].name), (float)vlos_.size(), true);
        ...
        for (; it != vlos_end && result.empty(); ++it) {
            ...
            DataSectionPtr legacyVLODS =
              spaceDS->newSection(name + ".vlo", XMLSection::creator());
            if (legacyVLODS) {
                // LEGACY MODE:
                // Regardless of whether the VLO is loaded or not, we'll copy
                // over the old VLO DataSection and change its position values
                // directly.
                DataSectionPtr srcLegacyVLODS =
                  srcSpaceDS->openSection(name + ".vlo");
                if (!srcLegacyVLODS)
                    srcLegacyVLODS = pVLO->section();
                successfulSave =
                  copyVLODataSectionAndChangePosition(srcLegacyVLODS,
                                                      legacyVLODS,
                                                      vloData.type,
                                                      vloData.transform[3]);
                if (!successfulSave) {
            ...
            DataSectionPtr VLODS =
              spaceDS->newSection("_" + name + ".vlo", XMLSection::creator());
            if (VLODS && !successfulSave) {
                ...
                if (pVLO) {
                    EditorChunkModelVLO* chunkModelVLO =
                      dynamic_cast<EditorChunkModelVLO*>(pVLO.get());
                ...
                if (!successfulSave) {
                    ...
                    // in-place.
                    DataSectionPtr srcVLODS =
                      srcSpaceDS->openSection("_" + name + ".vlo");
                    successfulSave = copyVLODataSectionAndChangePosition(
                      srcVLODS, VLODS, vloData.type, vloData.transform[3]);
                    if (!successfulSave) {
            ...
            DataSectionPtr srcWaterLegacyTransparencyDS =
              srcSpaceDS->openSection(name + ".odata");
            if (srcWaterLegacyTransparencyDS) {
                DataSectionPtr waterLegacyTransparencyDS =
                  spaceDS->newSection(name + ".odata", BinSection::creator());
                if (waterLegacyTransparencyDS) {
                    waterLegacyTransparencyDS->copy(
                      srcWaterLegacyTransparencyDS);
                    result += waterLegacyTransparencyDS->save()
                                ? ""
                                : ("Failed to save " + name + ".odata\n");
                } else {
                    result += "Failed to copy " + name + ".odata\n";
                }
            }

            DataSectionPtr srcWaterTransparencyDS =
              srcSpaceDS->openSection("_" + name + ".odata");
            if (srcWaterTransparencyDS) {
                DataSectionPtr waterTransparencyDS = spaceDS->newSection(
                  "_" + name + ".odata", BinSection::creator());
                if (waterTransparencyDS) {
                    waterTransparencyDS->copy(srcWaterTransparencyDS);
                    result += waterTransparencyDS->save()
                                ? ""
                                : ("Failed to save _" + name + ".odata\n");
                } else {
...
void SpaceRecreator::fixChunkLinkArray(DataSectionPtr arrayDS,
                                       bool knownToBeChunkLinks /*= false */)
{
    BW_GUARD;
...
{
    BW_GUARD;
    for (DataSectionIterator it = chunkDS->begin(); it != chunkDS->end();
         ++it) {
        DataSectionPtr childDS = *it;
        // EditorChunkItemLinkable::loadBackLinks and
        // EditorChunkItemLinkable::saveBackLinks
        DataSectionPtr backLinksDS = childDS->openSection("backLinks");
        if (backLinksDS)
            fixChunkLinkArray(backLinksDS, true);
        // Brute-force properties search for ChunkLinks or arrays of ChunkLinks
        // (UDO_REFs) EditorChunkUserDataObject::edLoad and
        // EditorChunkEntity::edLoad
        DataSectionPtr propertiesDS = childDS->openSection("properties");
        if (propertiesDS)
            for (DataSectionIterator propIt = propertiesDS->begin();
                 propIt != propertiesDS->end();
                 ++propIt)
...
Terrain::EditorTerrainBlock2Ptr
SpaceRecreator::readTerrainBlock(DataSectionPtr spaceDS, int x, int z)
{
    BW_GUARD;
    ...
    Terrain::EditorTerrainBlock2Ptr result =
      new Terrain::EditorTerrainBlock2(pTerrainSettings_);

    BW::string error = "Failed to allocate Terrain::EditorTerrainBlock2";
    if (!result || !result->load(BWResource::dissolveFilename(spaceName_) +
                                   "/" + terrainPath,
                                 chunkTransform,
                                 chunkTransform.applyToOrigin(),
                                 &error)) {
...
bool SpaceRecreator::writeTerrainBlock(
  DataSectionPtr                  spaceDS,
  int                             x,
  int                             z,
  Terrain::EditorTerrainBlock2Ptr pTerrainBlock)
{
    ...
    BWResource::instance().purge(BWResource::dissolveFilename(spaceName_) +
                                   "/" + chunkName(x, z) + ".cdata/" +
                                   pTerrainBlock->dataSectionName(),
                                 true);
...
void SpaceRecreator::blitHeights(
  Terrain::TerrainHeightMap::ImageType& heightMapImage,
  ...
  uint32                                dstY)
{
    ...
    Terrain::TerrainHeightMap& neighbourHeightMap =
      pNeighbourBlock->heightMap();
    Terrain::TerrainHeightMapHolder neighbourHeightMapHolder(
      &neighbourHeightMap, true);
    Terrain::TerrainHeightMap::ImageType& neighbourHeightMapImage =
      neighbourHeightMap.image();
    heightMapImage.blit(
      neighbourHeightMapImage, srcX, srcY, srcW, srcH, dstX, dstY);
}
...
bool SpaceRecreator::patchHeightMapEdges(DataSectionPtr spaceDS, int x, int z)
{
    ...
    { // Scope for the Terrain::TerrainHeightMapHolder
        ...
        blitHeights(heightMapImage,
                    readTerrainBlock(spaceDS, x - 1, z - 1),
                    xSize,
                    zSize,
                    xOff,
                    zOff,
        ...
        // Fill the left area
        blitHeights(heightMapImage,
                    readTerrainBlock(spaceDS, x - 1, z),
                    xSize,
                    zOff,
                    xOff,
                    zSize,
        ...
        // Fill the top area
        blitHeights(heightMapImage,
                    readTerrainBlock(spaceDS, x, z - 1),
                    xOff,
                    zSize,
                    xSize,
                    zOff,
        ...
        // The only part of TerrainGetInfo patchEdges uses is the
        // EditorBaseTerrainBlockPtr
        ...
        TerrainUtils::patchEdges(
          x, z, maxX_ - minX_ + 1, maxZ_ - minZ_ + 1, info, heightMapImage);
    }
...
const BW::string SpaceRecreator::fixChunks(bool withPurge)
{
    ...
    ProgressBarTask progressTask(LocaliseUTF8(STAGES[6].name),
                                 (float)(maxX_ - minX_ + 1) *
                                   (maxZ_ - minZ_ + 1),
                                 true);
    ...
    // This one's a little nasty. Need to iterate all the chunks in the new
    // space, and manually edit the XML of each for anything that has a
    // 'backlinks' child section.
    int start = GetTickCount();
    ...
    for (int x = minX_; x <= maxX_ && result.empty(); ++x) {
        ...
        for (int z = minZ_; z <= maxZ_ && result.empty(); ++z) {
            ...
            result +=
              fixChunkLinksInChunk(chunkDS)
                ? ""
                : ("Failed to update chunk links in " + name + ".chunk\n");
            if (shouldAbort())
                return errorMessage_;
            result += patchHeightMapEdges(spaceDS, x, z)
                        ? ""
                        : ("Failed to update chunk height map overlaps in " +
                           name + ".chunk\n");
            progressTask.step();
            if (shouldAbort())
                return errorMessage_;
    ...
    for (BW::set<BW::string>::const_iterator it = overlappers_.begin();
        ...
        const BW::string& name    = *it;
        DataSectionPtr    chunkDS = spaceDS->openSection(name + ".chunk");
        result += fixChunkLinksInChunk(chunkDS)
                    ? ""
                    : ("Failed to update chunk links in " + name + ".chunk\n");
    }
    ...
    for (auto itr = itemsWithPossibleBrokenLinks.begin();
         itr != itemsWithPossibleBrokenLinks.end();
         ++itr) {
