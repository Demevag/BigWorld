...
namespace {
    ...
    bool ruleSectionCompare(const DataSectionPtr a, DataSectionPtr b)
    {
        ...
        if (aPathSize == bPathSize)
            return a->readString("filespec").size() >
                   b->readString("filespec").size();
        else
            return aPathSize > bPathSize;
    }

    Vector3 recursiveReadVector3(
      const BW::string&                          key,
      ...
      Vector3                                    def)
    {
        ...
        BW::map<BW::string, DataSectionPtr>::const_iterator i =
          sections.find(parent);
        if (i != sections.end())
...
VisualChecker::VisualChecker(const BW::string& visualName,
                             bool              cacheRules,
                             bool              snapVertices)
  ...
  , exportAs_("normal")
{
    ...
    if (!rulesSection) {
        if (!LogMsg::automatedTest()) {
            MessageBox(GetForegroundWindow(),
                       TEXT("VisualChecker::VisualChecker - Unable to find "
                            "visual_rules.xml file.\n")
                         TEXT("VisualChecker will be disabled temporarily\n"),
                       TEXT("Visual Exporter"),
                       MB_OK | MB_ICONEXCLAMATION);
        }
        addError("VisualChecker::VisualChecker - Unable to find "
                 "visual_rules.xml file.");
        return;
    }
    ...
    for (BW::vector<DataSectionPtr>::iterator i = rules.begin();
         i != rules.end();
         ++i) {
    ...
    std::stable_sort(
      matchingRules.begin(), matchingRules.end(), ruleSectionCompare);
    ...
    exportAs_ =
      recursiveReadString("exportAs", rulesById, ruleSection, exportAs_);
    if (!(exportAs_ == "normal" || exportAs_ == "static" ||
          exportAs_ == "static with nodes")) {
        char s[1024];
        bw_snprintf(
          s, sizeof(s), "Unknown value for exportAs [%s]\n", exportAs_.c_str());
        if (!LogMsg::automatedTest()) {
#ifdef UNICODE
            MessageBox(GetForegroundWindow(),
                       bw_utf8tow(s).c_str(),
    ...
    minSize_ =
      recursiveReadVector3("minSize", rulesById, ruleSection, minSize_);
    maxSize_ =
      recursiveReadVector3("maxSize", rulesById, ruleSection, maxSize_);
    minTriangles_ =
      recursiveReadInt("minTriangles", rulesById, ruleSection, minTriangles_);
    maxTriangles_ =
      recursiveReadInt("maxTriangles", rulesById, ruleSection, maxTriangles_);
    maxHierarchyDepth_ = recursiveReadInt(
      "maxHierarchyDepth", rulesById, ruleSection, maxHierarchyDepth_);
    portals_ = recursiveReadBool("portals", rulesById, ruleSection, portals_);
    portalSnap_ =
      recursiveReadVector3("portalSnap", rulesById, ruleSection, portalSnap_);
    portalDistance_ = recursiveReadFloat(
      "portalDistance", rulesById, ruleSection, portalDistance_);
    portalOffset_ =
      recursiveReadFloat("portalOffset", rulesById, ruleSection, portalOffset_);
    checkHardPoints_ = recursiveReadBool(
      "checkUnknownHardPoints", rulesById, ruleSection, checkHardPoints_);
...
namespace {
     ...
     */
    void checkDuplicateNodeNames(DataSectionPtr       nodeSection,
                                 ...
                                 BW::set<BW::string>& duplicates)
    {
        ...
        if (std::find(nodeNames.begin(), nodeNames.end(), id) ==
            nodeNames.end()) {
            nodeNames.insert(id);
        } else {
        ...
        for (BW::vector<DataSectionPtr>::iterator i = nodes.begin();
             i != nodes.end();
             ++i) {
     ...
     */
    void extractHardPoints(DataSectionPtr       nodeSection,
                           BW::set<BW::string>& hardPoints)
    {
        if (!nodeSection)
            return;
...
bool VisualChecker::check(DataSectionPtr    visualSection,
                          const BW::string& primResName)
{
    errors_.clear();
    ...
    if ((minSize_.x > 0.f && minSize_.x > size.x) ||
        ...
        good = false;
        addError("Too small, must be at least %f, %f, %f",
                 minSize_.x,
                 minSize_.y,
                 minSize_.z);
    }
    ...
    if ((maxSize_.x > 0.f && maxSize_.x < size.x) ||
        ...
        good = false;
        addError("Too big, must be no bigger than %f, %f, %f",
                 maxSize_.x,
                 maxSize_.y,
                 maxSize_.z);
    }
    ...
    if (maxHierarchyDepth_ != 0 && hierarchyDepth > maxHierarchyDepth_) {
        good = false;
        addError("Node hierarchy too deep, depth must be %d nodes or less",
                 maxHierarchyDepth_);
    }
    ...
    if (portals_) {
        ...
        for (i = portals.begin(); i != portals.end(); ++i) {
            ...
            basis.translation(boundary->readVector3("normal") *
                              boundary->readFloat("d"));
            ...
            // 9/10/04 PJ - visual exporter can now only export portals on
            boundaries. if (!((close(p.normal().x, 1.f) && close(fabsf(p.d()),
            fabsf(bbMin.x))) || (close(p.normal().x, -1.f) &&
            close(fabsf(p.d()), fabsf(bbMax.x))) || (close(p.normal().y, 1.f) &&
            close(fabsf(p.d()), fabsf(bbMin.y))) || (close(p.normal().y, -1.f)
            && close(fabsf(p.d()), fabsf(bbMax.y))) || (close(p.normal().z, 1.f)
            && close(fabsf(p.d()), fabsf(bbMin.z))) || (close(p.normal().z,
            -1.f) && close(fabsf(p.d()), fabsf(bbMax.z)))))
            {
                good = false;
            ...
            Vector3 minDiff = snapVector3(portalBB.minBounds(), portalSnap_) -
                              portalBB.minBounds();
            Vector3 maxDiff = snapVector3(portalBB.maxBounds(), portalSnap_) -
                              portalBB.maxBounds();
            ...
            if (portalDistance_ > 0.f) {
                if (!isMultipleOf(p.d(), portalDistance_, 1.0f / 1024.f)) {
                    good = false;
                    addError("Portal must be a multiple of %f away from the "
                             "origin (it's %f away)",
                             portalDistance_,
                             p.d());
                }
            ...
            if (offset != 0.f && portalOffset_ != 0.f) {
                if (!isMultipleOf(offset, portalOffset_)) {
                    good = false;
                    addError("Centre of portal must be a multiple of %f on the "
                             "portals plane (it's %f away)",
                             portalOffset_,
                             offset);
                }
            }
    ...
    checkDuplicateNodeNames(
      visualSection->openSection("node"), visualNodes, nodeDuplicates);
    for (BW::set<BW::string>::iterator it = nodeDuplicates.begin();
         it != nodeDuplicates.end();
         ++it) {
        good = false;
    ...
    for (BW::set<BW::string>::iterator i = hardPoints_.begin();
         i != hardPoints_.end();
         ++i) {
        ...
        for (uint32 g = 0; g < geometries.size(); g++) {
            ...
            for (uint32 pg = 0; pg < pgs.size(); pg++) {
                BW::string textureName =
                  pgs[pg]->readString("material/textureormfm");
                BW::string::size_type pos = textureName.find_last_of(" ");
                if (pos != BW::string::npos) {
                    good = false;
                    addError("Illegal texture name \"%s\", no spaces in name "
                             "or path allowed\n",
                             textureName.c_str());
                }
            }
...
bool VisualChecker::checkTriangleCount(DataSectionPtr spPrims)
{
    ...
    while (it != spPrims->end()) {
        ...
        if (endsWith(pSection->sectionName(), "indices")) {
            Moo::IndexHeader* ih =
              (Moo::IndexHeader*)pSection->asBinary()->data();
            count += ih->nIndices_ / 3;
        }
    }

    if (count > maxTriangles_ && maxTriangles_ != 0) {
        addError("Too many triangles (%d), must be less than %d",
                 count,
                 maxTriangles_);
        return false;
    }
    ...
    if (count < minTriangles_) {
        addError(
          "Too few triangles (%d), must be at least %d", count, minTriangles_);
        return false;
    }
