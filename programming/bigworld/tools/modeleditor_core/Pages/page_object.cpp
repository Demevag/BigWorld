...
void PageObject::DoDataExchange(CDataExchange* pDX)
{
    ...
    for (; it != pImpl_->materialKinds->end(); ++it) {
        ...
        pImpl_->kindItem[*materialID] = pImpl_->kind.InsertString(
          item, bw_utf8tow((*it)->sectionName()).c_str());
...
BOOL PageObject::OnInitDialog()
{
    ...
    UalManager::instance().dropManager().add(
      new UalDropFunctor<PageObject>(&(pImpl_->editorProxy),
                                     "model",
                                     this,
                                     &PageObject::changeEditorProxyDrop));
...
void PageObject::OnSize(UINT nType, int cx, int cy)
{
    ...
    PathSetDlgItemPath(
      this->GetSafeHwnd(), IDC_OBJECT_NAME, pImpl_->modelName.c_str());

    Utilities::centre(this, pImpl_->thumbnailRect, cx);

    Utilities::stretchToRight(this, pImpl_->editorProxy, cx, 40);
    PathSetDlgItemPath(
      this->GetSafeHwnd(), IDC_OBJECT_PROXY, pImpl_->editorProxyName.c_str());
...
afx_msg LRESULT PageObject::OnUpdateControls(WPARAM wParam, LPARAM lParam)
{
    ...
    if (pImpl_->updating) {
        ...
        bw_utf8tow(MeApp::instance().mutant()->editorProxyName(),
                   pImpl_->editorProxyName);
        PathSetDlgItemPath(this->GetSafeHwnd(),
                           IDC_OBJECT_PROXY,
                           pImpl_->editorProxyName.c_str());

        int item =
          pImpl_->kindItem[MeApp::instance().mutant()->modelMaterial()];
...
bool PageObject::changeEditorProxy(const BW::wstring& editorProxyFile)
{
    ...
    if (editorProxyFile != pImpl_->modelName) {
        MeApp::instance().mutant()->editorProxyName(
          bw_wtoutf8(editorProxyFile));
        PathSetDlgItemPath(
          this->GetSafeHwnd(), IDC_OBJECT_PROXY, editorProxyFile.c_str());
...
bool PageObject::changeEditorProxyDrop(UalItemInfo* ii)
{
    ...
    return changeEditorProxy(
      bw_utf8tow(BWResource::dissolveFilename(bw_wtoutf8(ii->longText()))));
}
