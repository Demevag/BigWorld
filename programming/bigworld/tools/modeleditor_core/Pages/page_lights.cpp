...
BOOL LightList::PreTranslateMessage(MSG* pMsg)
{
    ...
    if (pMsg->message == WM_KEYDOWN) {
        if (pMsg->wParam == VK_SPACE) {
            ...
            GeneralLight* light =
              (GeneralLight*)(this->GetItemData(GetSelectedItem()));
...
void PageLights::DoDataExchange(CDataExchange* pDX)
{
    ...
    setupsRect.bottom +=
      256; // Extend the dropdown box to show a reasonable selection
    ...
    pImpl_->ambient = pImpl_->lightList.InsertItem(
      Localise(L"MODELEDITOR/PAGES/PAGE_LIGHTS/AMBIENT"), NULL);
    ...
    for (int i = 0; i < MeApp::instance().lights()->numOmni(); i++) {
        pImpl_->omni.push_back(pImpl_->lightList.InsertItem(
          Localise(L"MODELEDITOR/PAGES/PAGE_LIGHTS/OMNI", i + 1), NULL));
    ...
    for (int i = 0; i < MeApp::instance().lights()->numDir(); i++) {
        pImpl_->dir.push_back(pImpl_->lightList.InsertItem(
          Localise(L"MODELEDITOR/PAGES/PAGE_LIGHTS/DIRECTIONAL", i + 1), NULL));
    ...
    for (int i = 0; i < MeApp::instance().lights()->numSpot(); i++) {
        pImpl_->spot.push_back(pImpl_->lightList.InsertItem(
          Localise(L"MODELEDITOR/PAGES/PAGE_LIGHTS/SPOT", i + 1), NULL));
    ...
    GUI::Manager::instance().add(
      new GUI::Toolbar("LightsToolbar", pImpl_->toolbar));
...
BOOL PageLights::OnInitDialog()
{
    ...
    UalManager::instance().dropManager().add(new UalDropFunctor<PageLights>(
      this, "mvl", this, &PageLights::applyLights));
...
void PageLights::redrawLightList(bool sel)
{
    ...
    for (unsigned i = 0; i < lights.size(); i++) {
        BW::string lightingName =
          BWResource::getFilename(lights[i]).to_string();
        pImpl_->lightSetups.InsertString(
          i, BWResource::removeExtensionW(lightingName).c_str());
    }
...
void PageLights::updateCheck(CButton&          button,
                             ...
                             int               test /* = true */)
{
    ...
    editorApp->pythonAdapter()->ActionScriptUpdate(
      actionName, enabled, checked);
...
void PageLights::updateChecks()
{
    ...
    pImpl_->lightList.SetCheck(pImpl_->ambient,
                               MeApp::instance().lights()->ambient()->enabled()
                                 ? BST_CHECKED
                                 : BST_UNCHECKED);
    for (int i = 0; i < MeApp::instance().lights()->numOmni(); i++) {
        pImpl_->lightList.SetCheck(
          pImpl_->omni[i],
          MeApp::instance().lights()->omni(i)->enabled() ? BST_CHECKED
                                                         : BST_UNCHECKED);
    }
    for (int i = 0; i < MeApp::instance().lights()->numDir(); i++) {
        pImpl_->lightList.SetCheck(pImpl_->dir[i],
                                   MeApp::instance().lights()->dir(i)->enabled()
                                     ? BST_CHECKED
                                     : BST_UNCHECKED);
    }
    for (int i = 0; i < MeApp::instance().lights()->numSpot(); i++) {
        pImpl_->lightList.SetCheck(
          pImpl_->spot[i],
          MeApp::instance().lights()->spot(i)->enabled() ? BST_CHECKED
                                                         : BST_UNCHECKED);
    }
...
void PageLights::lightsNew()
{
    ...
    if (!MeApp::instance().lights()->newSetup())
        return;
 ...
 *
 *	This function disables all custom lighting and allows for a new light's
 *display settings to be created.
 */
...
bool PageLights::openLightFile(const BW::wstring& lightFile)
{
    ...
    if (!lighting) {
        ::MessageBox(
          ...
          Localise(L"MODELEDITOR/PAGES/PAGE_LIGHTS/LIGHTS_FILE_MISSING",
                   lightFile),
          Localise(L"MODELEDITOR/PAGES/PAGE_LIGHTS/UNABLE_LIGHTS"),
          MB_OK | MB_ICONERROR);
    ...
    if (!MeApp::instance().lights()->open(bw_wtoutf8(lightFile), lighting))
        return false;
 ...
 *	@components{ modeleditor }
 *
 *	This function enables the Open File dialog, which allows a light to be
 *loaded.
 */
 ...
 *	@components{ modeleditor }
 *
 *	This function opens the Save dialog, for saving the current light's display
 *settings.
 */
        ...
        if (Options::getOptionInt("settings/useCustomLighting", 1) == 1) {
            if (PanelManager::instance().getEditorApp()->pythonAdapter()) {
                PanelManager::instance()
                  .getEditorApp()
                  ->pythonAdapter()
                  ->ActionScriptExecute("actUseCustomLighting");
                enableCustomLighting(false);
            }
        }
    ...
    if (filter_ == 0) {
        ...
        if (PanelManager::instance().getEditorApp()->pythonAdapter()) {
            PanelManager::instance()
              .getEditorApp()
              ->pythonAdapter()
              ->ActionScriptExecute("actShowLightModels");
        }
