...
// SliderContainer
SliderContainer::SliderContainer(controls::RangeSliderCtrl* slider,
...
void PageActions::initMatcherField(controls::EditNumeric& field,
                                   int                    dec /* = 0 */,
                                   float                  min /* = 0.f */,
                                   float                  max /* = 100.f */,
                                   bool                   allowNeg /* = true */)
  ...
  : TreeList(PageActions::IDD, mutant()->actTree(), "actions")
{
    ...
    pImpl_->sliders.push_back(new SliderContainer(&pImpl_->trigger_speed_sldr,
                                                  &pImpl_->trigger_speed_min,
                                                  &pImpl_->trigger_speed_max,
                                                  0));

    pImpl_->sliders.push_back(new SliderContainer(&pImpl_->trigger_aux1_sldr,
                                                  &pImpl_->trigger_aux1_min,
                                                  &pImpl_->trigger_aux1_max,
                                                  0));

    pImpl_->sliders.push_back(new SliderContainer(&pImpl_->trigger_yaw_sldr,
    ...
    pImpl_->sliders.push_back(new SliderContainer(&pImpl_->cancel_speed_sldr,
                                                  &pImpl_->cancel_speed_min,
                                                  &pImpl_->cancel_speed_max,
                                                  0));

    pImpl_->sliders.push_back(new SliderContainer(&pImpl_->cancel_aux1_sldr,
                                                  &pImpl_->cancel_aux1_min,
                                                  &pImpl_->cancel_aux1_max,
                                                  0));

    pImpl_->sliders.push_back(new SliderContainer(&pImpl_->cancel_yaw_sldr,
    ...
    DataSectionPtr caps =
      BWResource::openSection(EntityDef::Constants::entitiesCapabilitiesFile());
    if (caps.exists()) {
        caps->openSections("state", pImpl_->capsList);
    } else {
        WARNING_MSG(
          " Unable to locate the action matcher capabilities file \"%s\".\n",
...
void PageActions::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control(
      pDX, IDC_ACT_TRIGGER_CAPS_OFF_SEL, pImpl_->trigger_caps_off_sel);
    ...
    DDX_Control(
      pDX, IDC_ACT_SCALE_PLAYBACK_SPEED, pImpl_->scale_playback_speed);
    DDX_Control(
      pDX, IDC_ACT_FEET_FOLLOW_DIRECTION, pImpl_->feet_follow_direction);
    ...
    GUI::Manager::instance().add(
      new GUI::Toolbar("ActToolbar", pImpl_->toolbar));
...
void PageActions::writeEditNum(controls::EditNumeric* edit,
                               float                  num,
                               int                    group /* = 0 */)
{
    BW_GUARD;
...
void PageActions::updateCheck(CButton&          button,
...
void PageActions::updateMatchField(controls::EditNumeric& field,
                                   const StringPair&      actID,
                                   const BW::string&      typeName,
                                   const BW::string&      flagName,
                                   bool lockedParents /* = false */,
                                   bool canBeNeg /* = true */)
{
    BW_GUARD;
    ...
    field.ModifyStyle(lockedParents ? 0 : WS_DISABLED,
...
void PageActions::removeMRU(const StringPair& action)
{
    ...
    while (hItem != NULL) {
        BW::string itemDisplay =
          bw_wtoutf8(pImpl_->mru.GetItemText(hItem).GetString());
...
void PageActions::addMRU(const StringPair& action)
{
    ...
    while (hItem != NULL) {
        BW::string itemDisplay =
          bw_wtoutf8(pImpl_->mru.GetItemText(hItem).GetString());
        if (actionDisplay == itemDisplay) {
            BW::string* itemModelPath =
              (BW::string*)(pImpl_->mru.GetItemData(hItem));
            pImpl_->mru.DeleteItem(hItem);
...
afx_msg LRESULT PageActions::OnUpdateControls(WPARAM, LPARAM)
{
    ...
    // If it is, no action should be played.  Note that an empty string is
    // passed as the action name since an action may have the same name as the
    // model.
    static bool rootItemSelected = true;

    // Check whether the action (or model) selection has changed
    if (pImpl_->updating || (pImpl_->selectClicked && !sameClicked()) ||
        pImpl_->MRUClicked) {
        ...
        if (pImpl_->MRUClicked) {
            ...
            if (hItem) {
                ...
                BW::string actionDisplay =
                  bw_wtoutf8(pImpl_->mru.GetItemText(hItem).GetString());
                selection.first =
                  actionDisplay.substr(0, actionDisplay.rfind(" - "));
                selection.second =
                  *(BW::string*)(pImpl_->mru.GetItemData(hItem));
            }
        } else {
        ...
        if (hasParent && (mutant()->hasActs(selection.second))) {
            ...
            pImpl_->anim.ModifyStyle(lockedParents ? 0 : WS_DISABLED,
                                     lockedParents ? WS_DISABLED : 0);
            ...
            pImpl_->blendIn.ModifyStyle(lockedParents ? 0 : WS_DISABLED,
                                        lockedParents ? WS_DISABLED : 0);
            pImpl_->blendInSlider.SetPos((int)(100 * blendInVal));
            pImpl_->blendInSlider.ModifyStyle(lockedParents ? 0 : WS_DISABLED,
                                              lockedParents ? WS_DISABLED : 0);

            float blendOutVal =
              mutant()->actBlendTime(selection, "blendOutTime");
            pImpl_->blendOut.SetValue(blendOutVal);
            pImpl_->blendOut.SetReadOnly(lockedParents);
            pImpl_->blendOut.ModifyStyle(lockedParents ? 0 : WS_DISABLED,
                                         lockedParents ? WS_DISABLED : 0);
            pImpl_->blendOutSlider.SetPos((int)(100 * blendOutVal));
            pImpl_->blendOutSlider.ModifyStyle(lockedParents ? 0 : WS_DISABLED,
                                               lockedParents ? WS_DISABLED : 0);
            ...
            pImpl_->looped.ModifyStyle(lockedParents ? 0 : WS_DISABLED,
                                       lockedParents ? WS_DISABLED : 0);
            pImpl_->move.ModifyStyle(lockedParents ? 0 : WS_DISABLED,
                                     lockedParents ? WS_DISABLED : 0);
            pImpl_->impact.ModifyStyle(lockedParents ? 0 : WS_DISABLED,
                                       lockedParents ? WS_DISABLED : 0);
            pImpl_->coord.ModifyStyle(lockedParents ? 0 : WS_DISABLED,
                                      lockedParents ? WS_DISABLED : 0);
            ...
            pImpl_->track.ModifyStyle(
              ownTrack || lockedParents ? 0 : WS_DISABLED,
              ownTrack || lockedParents ? WS_DISABLED : 0);

            pImpl_->ownTrack.ModifyStyle(lockedParents ? 0 : WS_DISABLED,
                                         lockedParents ? WS_DISABLED : 0);

            pImpl_->trigger_falling.ModifyStyle(WS_DISABLED, 0);

            pImpl_->trigger_speed_sldr.ModifyStyle(
              lockedParents ? 0 : WS_DISABLED, lockedParents ? WS_DISABLED : 0);
            pImpl_->trigger_aux1_sldr.ModifyStyle(
              lockedParents ? 0 : WS_DISABLED, lockedParents ? WS_DISABLED : 0);
            pImpl_->trigger_yaw_sldr.ModifyStyle(
              lockedParents ? 0 : WS_DISABLED, lockedParents ? WS_DISABLED : 0);

            bool valSet;
            bool falling =
              mutant()->actMatchFloat(
                selection, "trigger", "minEntitySpeed", valSet) < 0.f;
            pImpl_->trigger_falling.SetCheck(falling ? BST_CHECKED
                                                     : BST_UNCHECKED);
            pImpl_->trigger_falling.ModifyStyle(
              lockedParents ? 0 : WS_DISABLED, lockedParents ? WS_DISABLED : 0);

            updateMatchField(pImpl_->trigger_speed_min,
                             selection,
                             "trigger",
                             "minEntitySpeed",
                             lockedParents,
                             false);
            updateMatchField(pImpl_->trigger_speed_max,
                             selection,
                             "trigger",
                             "maxEntitySpeed",
                             lockedParents,
                             false);
            updateMatchField(pImpl_->trigger_aux1_min,
                             selection,
                             "trigger",
                             "minEntityAux1",
                             lockedParents);
            updateMatchField(pImpl_->trigger_aux1_max,
                             selection,
                             "trigger",
                             "maxEntityAux1",
                             lockedParents);
            updateMatchField(pImpl_->trigger_yaw_min,
                             selection,
                             "trigger",
                             "minModelYaw",
                             lockedParents);
            updateMatchField(pImpl_->trigger_yaw_max,
                             selection,
                             "trigger",
                             "maxModelYaw",
                             lockedParents);
            ...
            pImpl_->trigger_caps_on.SetWindowText(
              getNamedCaps(
                mutant()->actMatchCaps(selection, "trigger", "capsOn"))
                .c_str());
            pImpl_->trigger_caps_off.SetWindowText(
              getNamedCaps(
                mutant()->actMatchCaps(selection, "trigger", "capsOff"))
                .c_str());

            pImpl_->trigger_caps_on_sel.ModifyStyle(
              lockedParents ? 0 : WS_DISABLED, lockedParents ? WS_DISABLED : 0);
            pImpl_->trigger_caps_off_sel.ModifyStyle(
              lockedParents ? 0 : WS_DISABLED, lockedParents ? WS_DISABLED : 0);

            pImpl_->cancel_falling.ModifyStyle(WS_DISABLED, 0);

            pImpl_->cancel_speed_sldr.ModifyStyle(
              lockedParents ? 0 : WS_DISABLED, lockedParents ? WS_DISABLED : 0);
            pImpl_->cancel_aux1_sldr.ModifyStyle(
              lockedParents ? 0 : WS_DISABLED, lockedParents ? WS_DISABLED : 0);
            pImpl_->cancel_yaw_sldr.ModifyStyle(
              lockedParents ? 0 : WS_DISABLED, lockedParents ? WS_DISABLED : 0);

            falling = mutant()->actMatchFloat(
                        selection, "cancel", "minEntitySpeed", valSet) < 0.f;
            pImpl_->cancel_falling.SetCheck(falling ? BST_CHECKED
                                                    : BST_UNCHECKED);
            pImpl_->cancel_falling.ModifyStyle(lockedParents ? 0 : WS_DISABLED,
                                               lockedParents ? WS_DISABLED : 0);

            updateMatchField(pImpl_->cancel_speed_min,
                             selection,
                             "cancel",
                             "minEntitySpeed",
                             lockedParents,
                             false);
            updateMatchField(pImpl_->cancel_speed_max,
                             selection,
                             "cancel",
                             "maxEntitySpeed",
                             lockedParents,
                             false);
            updateMatchField(pImpl_->cancel_aux1_min,
                             selection,
                             "cancel",
                             "minEntityAux1",
                             lockedParents);
            updateMatchField(pImpl_->cancel_aux1_max,
                             selection,
                             "cancel",
                             "maxEntityAux1",
                             lockedParents);
            updateMatchField(pImpl_->cancel_yaw_min,
                             selection,
                             "cancel",
                             "minModelYaw",
                             lockedParents);
            updateMatchField(pImpl_->cancel_yaw_max,
                             selection,
                             "cancel",
                             "maxModelYaw",
                             lockedParents);
            ...
            pImpl_->cancel_caps_on.SetWindowText(
              getNamedCaps(
                mutant()->actMatchCaps(selection, "cancel", "capsOn"))
                .c_str());
            pImpl_->cancel_caps_off.SetWindowText(
              getNamedCaps(
                mutant()->actMatchCaps(selection, "cancel", "capsOff"))
                .c_str());

            pImpl_->cancel_caps_on_sel.ModifyStyle(
              lockedParents ? 0 : WS_DISABLED, lockedParents ? WS_DISABLED : 0);
            pImpl_->cancel_caps_off_sel.ModifyStyle(
              lockedParents ? 0 : WS_DISABLED, lockedParents ? WS_DISABLED : 0);
            ...
            updateCheck(pImpl_->scale_playback_speed,
                        selection,
                        "match/scalePlaybackSpeed");
            updateCheck(pImpl_->feet_follow_direction,
                        selection,
                        "match/feetFollowDirection");

            pImpl_->one_shot.ModifyStyle(lockedParents ? 0 : WS_DISABLED,
                                         lockedParents ? WS_DISABLED : 0);
            updateScalePlaybackSpeed();
            pImpl_->feet_follow_direction.ModifyStyle(
              lockedParents ? 0 : WS_DISABLED, lockedParents ? WS_DISABLED : 0);
...
void PageActions::updateScalePlaybackSpeed()
{
    ...
    bool scalePlaybackSpeedEnabled =
      ((pImpl_->move.GetCheck() == BST_CHECKED) && !locked());
 ...
 *actions that belong to the LOD parents of the currently visible LOD level will
 *be locked from modifications.
 *
...
void PageActions::actNew()
{
    ...
    if (chooseAnimDlg.DoModal() == IDOK) {
        ...
        StringPair newAct =
          mutant()->createAct(actID, chooseAnimDlg.animName(), selID());
...
void PageActions::actPromote()
{
    ...
    BW::string actionName =
      bw_wtoutf8(tree().GetItemText(nextItem).GetString());
...
void PageActions::actDemote()
{
    ...
    BW::string actionName =
      bw_wtoutf8(tree().GetItemText(prevItem).GetString());
...
void PageActions::OnEnChangeActName()
{
    ...
    // action) This is not necessary since the field should be readonly (but
    // lets be safe)
...
void PageActions::updateName()
{
    ...
    if (new_name == "") {
        ...
        ::MessageBox(
          ...
          Localise(L"MODELEDITOR/PAGES/PAGE_ACTIONS/INVALID_ACTION_NAME"),
          MB_OK | MB_ICONERROR);
        ...
        ::MessageBox(
          ...
          Localise(L"MODELEDITOR/PAGES/PAGE_ACTIONS/ACTION_NAME_EXISTS"),
          MB_OK | MB_ICONERROR);
    }
}
...
void PageActions::OnChangeActAnim()
{
    ...
    if (chooseAnimDlg.DoModal() == IDOK) {
        ...
        pImpl_->anim.SetWindowText(
          bw_utf8tow(chooseAnimDlg.animName()).c_str());
    }
}
...
void PageActions::updateFlags()
{
    ...
    mutant()->actFlag(
      getAct(), "filler", pImpl_->looped.GetCheck() == BST_CHECKED);
    mutant()->actFlag(
      getAct(), "isMovement", pImpl_->move.GetCheck() == BST_CHECKED);
    mutant()->actFlag(
      getAct(), "isCoordinated", pImpl_->coord.GetCheck() == BST_CHECKED);
    mutant()->actFlag(
      getAct(), "isImpacting", pImpl_->impact.GetCheck() == BST_CHECKED);
                                     ...
                                     bool  reloadModel /*= false*/)
{
    ...
    hasChanged |= mutant()->actMatchVal(getAct(),
                                        "trigger",
                                        "minEntitySpeed",
                                        pImpl_->trigger_speed_min.isEmpty(),
                                        (falling ? -1.f : 1.f) *
                                          pImpl_->trigger_speed_min.GetValue(),
                                        shouldUndo);
    hasChanged |= mutant()->actMatchVal(getAct(),
                                        "trigger",
                                        "maxEntitySpeed",
                                        pImpl_->trigger_speed_max.isEmpty(),
                                        (falling ? -1.f : 1.f) *
                                          pImpl_->trigger_speed_max.GetValue(),
                                        shouldUndo);
    ...
    hasChanged |= mutant()->actMatchVal(getAct(),
                                        "trigger",
                                        "minEntityAux1",
                                        pImpl_->trigger_aux1_min.isEmpty(),
                                        pImpl_->trigger_aux1_min.GetValue(),
                                        shouldUndo);
    hasChanged |= mutant()->actMatchVal(getAct(),
                                        "trigger",
                                        "maxEntityAux1",
                                        pImpl_->trigger_aux1_max.isEmpty(),
                                        pImpl_->trigger_aux1_max.GetValue(),
                                        shouldUndo);
    ...
    hasChanged |= mutant()->actMatchVal(getAct(),
                                        "trigger",
                                        "minModelYaw",
                                        pImpl_->trigger_yaw_min.isEmpty(),
                                        pImpl_->trigger_yaw_min.GetValue(),
                                        shouldUndo);
    hasChanged |= mutant()->actMatchVal(getAct(),
                                        "trigger",
                                        "maxModelYaw",
                                        pImpl_->trigger_yaw_max.isEmpty(),
                                        pImpl_->trigger_yaw_max.GetValue(),
                                        shouldUndo);
                                    ...
                                    bool  reloadModel /*= false*/)
{
    ...
    hasChanged |= mutant()->actMatchVal(getAct(),
                                        "cancel",
                                        "minEntitySpeed",
                                        pImpl_->cancel_speed_min.isEmpty(),
                                        (falling ? -1.f : 1.f) *
                                          pImpl_->cancel_speed_min.GetValue(),
                                        shouldUndo);
    hasChanged |= mutant()->actMatchVal(getAct(),
                                        "cancel",
                                        "maxEntitySpeed",
                                        pImpl_->cancel_speed_max.isEmpty(),
                                        (falling ? -1.f : 1.f) *
                                          pImpl_->cancel_speed_max.GetValue(),
                                        shouldUndo);
    ...
    hasChanged |= mutant()->actMatchVal(getAct(),
                                        "cancel",
                                        "minEntityAux1",
                                        pImpl_->cancel_aux1_min.isEmpty(),
                                        pImpl_->cancel_aux1_min.GetValue(),
                                        shouldUndo);
    hasChanged |= mutant()->actMatchVal(getAct(),
                                        "cancel",
                                        "maxEntityAux1",
                                        pImpl_->cancel_aux1_max.isEmpty(),
                                        pImpl_->cancel_aux1_max.GetValue(),
                                        shouldUndo);

    // Clear edit field pairs if either is empty
    if (pImpl_->cancel_yaw_min.isEmpty() || pImpl_->cancel_yaw_max.isEmpty() ||
    ...
    hasChanged |= mutant()->actMatchVal(getAct(),
                                        "cancel",
                                        "minModelYaw",
                                        pImpl_->cancel_yaw_min.isEmpty(),
                                        pImpl_->cancel_yaw_min.GetValue(),
                                        shouldUndo);
    hasChanged |= mutant()->actMatchVal(getAct(),
                                        "cancel",
                                        "maxModelYaw",
                                        pImpl_->cancel_yaw_max.isEmpty(),
                                        pImpl_->cancel_yaw_max.GetValue(),
                                        shouldUndo);
...
void PageActions::updateMatchFlags()
{
    ...
    mutant()->actMatchFlag(
      getAct(), "oneShot", pImpl_->one_shot.GetCheck() == BST_CHECKED);
    mutant()->actMatchFlag(getAct(),
                           "scalePlaybackSpeed",
                           pImpl_->scale_playback_speed.GetCheck() ==
                             BST_CHECKED);
    mutant()->actMatchFlag(getAct(),
                           "feetFollowDirection",
                           pImpl_->feet_follow_direction.GetCheck() ==
                             BST_CHECKED);
...
void PageActions::OnBnClickedActTriggerCapsOnSel()
{
    ...
    CTriggerList triggerListDlg(
      "Action Triggers (On)", pImpl_->capsList, capsStr);
...
void PageActions::OnBnClickedActTriggerCapsOffSel()
{
    ...
    CTriggerList triggerListDlg(
      "Action Triggers (Off)", pImpl_->capsList, capsStr);
    if (triggerListDlg.DoModal() == IDOK) {
        BW::string caps = triggerListDlg.caps();
...
void PageActions::OnBnClickedActCancelCapsOnSel()
{
    ...
    CTriggerList triggerListDlg(
      "Action Cancel (On)", pImpl_->capsList, capsStr);
    if (triggerListDlg.DoModal() == IDOK) {
        BW::string caps = triggerListDlg.caps();
...
void PageActions::OnBnClickedActCancelCapsOffSel()
{
    ...
    CTriggerList triggerListDlg(
      "Action Cancel (Off)", pImpl_->capsList, capsStr);
    if (triggerListDlg.DoModal() == IDOK) {
        BW::string caps = triggerListDlg.caps();
...
void PageActions::OnBnClickedActTriggerFalling()
{
    ...
    initMatcherField(
      pImpl_->trigger_speed_min, 1, falling ? 0.1f : 0.f, 100.f, false);

    initMatcherField(
      pImpl_->trigger_speed_max, 1, falling ? 0.1f : 0.f, 100.f, false);

    initMatcherField(pImpl_->trigger_aux1_min, 0, -360.f, 360.f, true);

    initMatcherField(pImpl_->trigger_aux1_max, 0, -360.f, 360.f, true);

    initMatcherField(pImpl_->trigger_yaw_min, 0, -360.f, 360.f, true);

    initMatcherField(pImpl_->trigger_yaw_max, 0, -360.f, 360.f, true);

    pImpl_->trigger_speed_sldr.setRange(falling ? 0.1f : 0.f, 100.f, 1);

    pImpl_->trigger_aux1_sldr.setRange(-360.f, 360.f, 0);

    pImpl_->trigger_yaw_sldr.setRange(-360.f, 360.f, 0);

    if (falling) {
        if ((pImpl_->trigger_speed_min.isEmpty()) ||
            (pImpl_->trigger_speed_min.GetValue() == 0.f)) {
            pImpl_->trigger_speed_min.SetValue(0.1f);
        }
        if ((pImpl_->trigger_speed_max.isEmpty()) ||
            (pImpl_->trigger_speed_min.GetValue() == 0.f)) {
        ...
        if ((pImpl_->trigger_aux1_max.isEmpty()) ||
            (pImpl_->trigger_aux1_max.GetValue() == 0.f)) {
        ...
        if ((pImpl_->trigger_yaw_max.isEmpty()) ||
            (pImpl_->trigger_yaw_max.GetValue() == 0.f)) {
...
void PageActions::OnBnClickedActCancelFalling()
{
    ...
    initMatcherField(
      pImpl_->cancel_speed_min, 1, falling ? 0.1f : 0.f, 100.f, false);

    initMatcherField(
      pImpl_->cancel_speed_max, 1, falling ? 0.1f : 0.f, 100.f, false);

    initMatcherField(pImpl_->cancel_aux1_min, 0, -360.f, 360.f, true);

    initMatcherField(pImpl_->cancel_aux1_max, 0, -360.f, 360.f, true);

    initMatcherField(pImpl_->cancel_yaw_min, 0, -360.f, 360.f, true);

    initMatcherField(pImpl_->cancel_yaw_max, 0, -360.f, 360.f, true);

    pImpl_->cancel_speed_sldr.setRange(falling ? 0.1f : 0.f, 100.f, 1);

    pImpl_->cancel_aux1_sldr.setRange(-360.f, 360.f, 0);

    pImpl_->cancel_yaw_sldr.setRange(-360.f, 360.f, 0);

    if (falling) {
        if ((pImpl_->cancel_speed_min.isEmpty()) ||
            (pImpl_->cancel_speed_min.GetValue() == 0.f)) {
            pImpl_->cancel_speed_min.SetValue(0.1f);
        }
        if ((pImpl_->cancel_speed_max.isEmpty()) ||
            (pImpl_->cancel_speed_min.GetValue() == 0.f)) {
        ...
        if ((pImpl_->cancel_aux1_max.isEmpty()) ||
            (pImpl_->cancel_aux1_max.GetValue() == 0.f)) {
        ...
        if ((pImpl_->cancel_yaw_max.isEmpty()) ||
            (pImpl_->cancel_yaw_max.GetValue() == 0.f)) {
...
/*afx_msg*/ HBRUSH PageActions::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
    ...
    for (BW::vector<SliderContainerPtr>::iterator i = pImpl_->sliders.begin();
         i != pImpl_->sliders.end();
         ++i) {
        (*i)->minEd_->SetBoundsColour(
          pDC, pWnd, (*i)->minEd_->GetMinimum(), (*i)->maxEd_->GetMaximum());

        (*i)->maxEd_->SetBoundsColour(
          pDC, pWnd, (*i)->minEd_->GetMinimum(), (*i)->maxEd_->GetMaximum());
    }
