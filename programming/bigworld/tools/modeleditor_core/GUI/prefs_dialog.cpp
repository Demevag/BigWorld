...
BOOL CPrefsDlg::OnInitDialog()
{
    ...
    showSplashScreen_.SetCheck(
      Options::getOptionInt("startup/showSplashScreen", 1) ? BST_CHECKED
                                                           : BST_UNCHECKED);

    loadLastModel_.SetCheck(Options::getOptionInt("startup/loadLastModel", 1)
                              ? BST_CHECKED
                              : BST_UNCHECKED);

    loadLastLights_.SetCheck(Options::getOptionInt("startup/loadLastLights", 1)
                               ? BST_CHECKED
                               : BST_UNCHECKED);

    regenBBOnLoad_.SetCheck(Options::getOptionInt("settings/regenBBOnLoad", 1)
                              ? BST_CHECKED
                              : BST_UNCHECKED);

    zoomOnLoad_.SetCheck(Options::getOptionInt("settings/zoomOnLoad", 1)
                           ? BST_CHECKED
                           : BST_UNCHECKED);

    animateZoom_.SetCheck(Options::getOptionInt("settings/animateZoom", 1)
                            ? BST_CHECKED
                            : BST_UNCHECKED);

    lockLOD_.SetCheck(Options::getOptionInt("settings/lockLodParents", 0)
                        ? BST_CHECKED
                        : BST_UNCHECKED);

    invertMouse_.SetCheck(
      Options::getOptionInt("camera/invert", 0) ? BST_CHECKED : BST_UNCHECKED);
