...
void TreeList::OnSize(UINT nType, int cx, int cy)
{
    ...
    search_.SetWindowPos(0,
                         ...
                         rect.Height() - 7,
                         SWP_NOZORDER);
    search_button_.SetWindowPos(
      0, rect.left + 4, rect.top + 4, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
    search_cancel_.SetWindowPos(0,
                                rect.left + rect.Width() - 19,
                                rect.top + 4,
                                0,
                                0,
    ...
    tree_.SetWindowPos(
      0, rect.left, rect.top + 32, rect.Width(), rect2.Height(), SWP_NOZORDER);
    ...
    if (MeApp::instance().mutant()) {
        ...
        // "EnsureVisible"
        {
            ...
            for (unsigned m = 0; m < treeRoot_->size(); m++) {
                ...
                HTREEITEM model =
                  tree_.InsertItem(bw_utf8tow(modelName).c_str());
                ...
                if ((isParent) &&
                    (StringPair(modelName, *fileName) == currID)) {
                    selItem_ = model;
                    selID_   = currID;
                }
                ...
                if ((what_ == "MODELEDITOR/ANIMATIONS") &&
                    ...
                    for (unsigned a = 0; a < (*treeRoot_)[m].second.size();
                         a++) {
                        ...
                        if (nameLower.find(search_str_, 0) !=
                            BW::string::npos) {
                            HTREEITEM anim =
                              tree_.InsertItem(bw_utf8tow(name).c_str(), model);
                            if ((!isParent) &&
                                (StringPair(name, *fileName) == currID)) {
                                selItem_ = anim;
                                selID_   = currID;
                            }
                        }
...
void TreeList::OnNMClickSearchTree(NMHDR* pNMHDR, LRESULT* pResult)
{
    ...
    BW::string model =
      *(BW::string*)(tree_.GetItemData(parent ? parent : item));
