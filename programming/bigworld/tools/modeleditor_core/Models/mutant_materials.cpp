...
MaterialInfo::MaterialInfo(const BW::string&          cName,
...
TintInfo::TintInfo() {}
TintInfo::TintInfo(Moo::ComplexEffectMaterialPtr cEffect,
...
void Mutant::setDye(const BW::string&              matterName,
...
void Mutant::getMaterial(const BW::string&         materialName,
...
bool Mutant::setMaterialProperty(const BW::string& materialName,
                                 ...
                                 const BW::string& val)
{
    ...
    while (matIt != matEnd) {
        ...
        while (propsIt != propsEnd) {
            ...
            if (descName == prop->asString()) {
                ...
                if (textureFeed) {
                    textureFeed->writeString("default",
                                             BWResource::dissolveFilename(val));
                } else {
                    prop->writeString(propType,
                                      BWResource::dissolveFilename(val));
                }
    ...
    UndoRedo::instance().barrier(
      LocaliseUTF8(L"MODELEDITOR/MODELS/MUTANT_MATERIALS/CHANGE_TO", uiName),
      true);
...
void Mutant::instantiateMFM(DataSectionPtr data)
{
    ...
    if (mfmFile != "") {
        ...
        mfmData->writeInt("collisionFlags", data->readInt("collisionFlags", 0));

        mfmData->writeInt("materialKind", data->readInt("materialKind", 0));
...
void Mutant::overloadMFM(DataSectionPtr data, DataSectionPtr mfmData)
{
    ...
    data->writeInt("collisionFlags", mfmData->readInt("collisionFlags", 0));

    data->writeInt("materialKind", mfmData->readInt("materialKind", 0));
...
void Mutant::cleanMaterials()
{
    ...
    for (; it != end; ++it) {
        ...
        if (effect->baseMaterial()->pEffect()) {
            Moo::EffectMaterial::Properties& properties =
              effect->baseMaterial()->properties();
            Moo::EffectMaterial::Properties::iterator propIt =
              properties.begin();
            Moo::EffectMaterial::Properties::iterator propEnd =
              properties.end();

            for (; propIt != propEnd; ++propIt) {
                EditorEffectProperty* pProperty =
                  dynamic_cast<EditorEffectProperty*>(propIt->second.get());
                ...
                if (MaterialUtility::artistEditable(pProperty)) {
                    ...
                    BW::vector<DataSectionPtr>::iterator srcPropsIt =
                      srcProps.begin();
                    BW::vector<DataSectionPtr>::iterator srcPropsEnd =
                      srcProps.end();
                    for (; srcPropsIt != srcPropsEnd; ++srcPropsIt) {
                        if (descName == (*srcPropsIt)->asString()) {
                            // Skip over properties that we have already added.
                            // This can occur when using multi-layer effects -
                            // there will most likely be shared properties
                            // referenced by both effects.
                            BW::vector<BW::string>::iterator fit =
                              std::find(existingProps.begin(),
                                        existingProps.end(),
                                        descName);
                            if (fit == existingProps.end()) {
                                // For all the material sections add the
                                // property
                                for (unsigned i = 0; i < data.size(); i++) {
                                    data[i]
                                      ->newSection("property")
...
void Mutant::cleanTints()
{
    ...
    BW::map<BW::string, BW::map<BW::string, TintInfo>>::iterator dyeIt =
      tints_.begin();
    BW::map<BW::string, BW::map<BW::string, TintInfo>>::iterator dyeEnd =
      tints_.end();
...
void Mutant::cleanMaterialNames()
{
    ...
    if (materialNameDataToRemove_.empty())
        return;
...
bool Mutant::setTintProperty(const BW::string& matterName,
                             const BW::string& tintName,
                             const BW::string& descName,
                             const BW::string& uiName,
                             const BW::string& propType,
                             const BW::string& val)
{
    ...
    // Now do nearly the exact same thing for the parent's "property" values for
    // exposed properties. We do this to keep the python exposed value the same
    // as the materials.
    ...
    while (propsIt != propsEnd) {
        ...
        if (descName == prop->readString("name", "")) {
            prop->writeVector4(
              "default",
              getExposedVector4(matterName, tintName, descName, propType, val));
        }
    }
...
bool Mutant::materialName(const BW::string& materialName,
                          const BW::string& new_name)
{
    BW_GUARD;

    // Exit if we have already set this name
    if (new_name == materials_[materialName].name)
        return true;
    ...
    if (pNameData != NULL) {
        ...
        UndoRedo::instance().barrier(
          LocaliseUTF8(
            L"MODELEDITOR/MODELS/MUTANT_MATERIALS/CHANGING_MATERIAL_NAME"),
          true);
...
bool Mutant::matterName(const BW::string& matterName,
                        const BW::string& new_name,
                        bool              undoRedo)
{
    BW_GUARD;

    if (matterName == new_name)
        return true;

    if (undoRedo && dyes_.find(new_name) != dyes_.end())
        return false;
    ...
    if (undoRedo) {
        ...
        UndoRedo::instance().barrier(
          LocaliseUTF8(
            L"MODELEDITOR/MODELS/MUTANT_MATERIALS/CHANGING_DYE_NAME"),
          true);
    }
...
bool Mutant::tintName(const BW::string& matterName,
                      const BW::string& tintName,
                      const BW::string& new_name,
                      bool              undoRedo)
{
    ...
    if (undoRedo) {
        ...
        for (; tintIt != tintEnd; ++tintIt) {
            if (tintIt->second.find(new_name) != tintIt->second.end())
                return false;
        }
    }
    // If we are curently using this matter make sure to update the reference
    ...
    if (undoRedo) {
        UndoRedo::instance().add(
          new UndoRedoTintName(matterName, new_name, tintName));
        UndoRedo::instance().barrier(
          LocaliseUTF8(
            L"MODELEDITOR/MODELS/MUTANT_MATERIALS/CHANGING_TINT_NAME"),
          true);
    }
...
BW::string Mutant::newTint(const BW::string& materialName,
                           ...
                           const BW::string& mfmFile)
{
    ...
    if (matterName == "") {
        ...
        UndoRedo::instance().barrier(
          LocaliseUTF8(L"MODELEDITOR/MODELS/MUTANT_MATERIALS/ADDING_TINT"),
          true);
    ...
    if (fxFile != "") {
        ...
        {
            if ((tints_.find(matterName) != tints_.end()) &&
                (tints_[matterName].find(oldTintName) !=
                 tints_[matterName].end()) &&
                (tints_[matterName][oldTintName].data)) {
                DataSectionPtr materialData(
                  tints_[matterName][oldTintName].data->openSection(
                    "material"));
                if (materialData) {
...
bool Mutant::saveMFM(const BW::string& materialName,
                     const BW::string& matterName,
                     const BW::string& tintName,
                     const BW::string& mfmFile)
{
    ...
    // Even though this method could have dirtied the model (through the
    // instantiation of the mfm) we do not mark it as dirty since the user did
    // not expressly request this change. If any further changes are made then
    // this will be saved.
}

void Mutant::deleteTint(const BW::string& matterName,
                        const BW::string& tintName)
{
    BW_GUARD;
    ...
    UndoRedo::instance().add(new UndoRedoOp(0, currModel_, currModel_));
    UndoRedo::instance().barrier(
      LocaliseUTF8(L"MODELEDITOR/MODELS/MUTANT_MATERIALS/DELETING_TINT"), true);
...
bool Mutant::ensureShaderCorrect(const BW::string& fxFile,
                                 ...
                                 bool              dualUV)
{
    ...
    if (fxFile == "")
        return true; // No shader can apply to any format
    ...
    if (fxFile.find("hardskinned") != BW::string::npos) {
        if (softskinned) {
            ERROR_MSG("Unable to apply a hardskinned shader to a softskinned "
                      "object.\n");
            ::MessageBox(
              AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
              Localise(
                L"MODELEDITOR/MODELS/MUTANT_MATERIALS/SOFTSKINNED_ERROR_MSG"),
              Localise(L"MODELEDITOR/MODELS/MUTANT_MATERIALS/SHADER_ERROR"),
              MB_OK);
            return false;
        }
    } else if (fxFile.find("skinned") != BW::string::npos) {
        if (hardskinned) {
            ERROR_MSG("Unable to apply a softskinned shader to a hardskinned "
                      "object.\n");
            ::MessageBox(
              AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
              Localise(
                L"MODELEDITOR/MODELS/MUTANT_MATERIALS/HARDSKINNED_ERROR_MSG"),
              Localise(L"MODELEDITOR/MODELS/MUTANT_MATERIALS/SHADER_ERROR"),
              MB_OK);
            return false;
        } else if (!softskinned) {
            ERROR_MSG(
              "Unable to apply a softskinned shader to an unskinned object.\n");
            ::MessageBox(
              AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
              Localise(
                L"MODELEDITOR/MODELS/MUTANT_MATERIALS/UNSKINNED_ERROR_MSG"),
              Localise(L"MODELEDITOR/MODELS/MUTANT_MATERIALS/SHADER_ERROR"),
              MB_OK);
            return false;
        }
        ...
        if (softskinned) {
            WARNING_MSG(
              "Applying an unskinned shader to a softskinned object.\n");
            if (::MessageBox(
                  AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
                  Localise(L"MODELEDITOR/MODELS/MUTANT_MATERIALS/"
                           L"SOFTSKINNED_WARNING_MSG"),
                  Localise(
                    L"MODELEDITOR/MODELS/MUTANT_MATERIALS/SOFTSKINNED_WARNING"),
                  MB_OKCANCEL) == IDCANCEL) {
                return false;
            }
        } else if (hardskinned) {
            WARNING_MSG(
              "Applying an unskinned shader to a hardskinned object.\n");
            if (::MessageBox(
                  AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
                  Localise(L"MODELEDITOR/MODELS/MUTANT_MATERIALS/"
                           L"HARDSKINNED_WARNING_MSG"),
                  Localise(
                    L"MODELEDITOR/MODELS/MUTANT_MATERIALS/HARDSKINNED_WARNING"),
                  MB_OKCANCEL) == IDCANCEL) {
                return false;
            }
        }
...

    {
        ...
        if (material && material->initFromEffect(fxFile)) {
            if (material->dualUV() && !dualUV) {
                ERROR_MSG(
                  "Unable to apply a dual UV shader to a single UV object.\n");
                return false;
            } else if (!material->dualUV() && dualUV) {
                WARNING_MSG(
                  "Applying a single UV shader to a dual UV object.\n");
...
bool Mutant::doAnyEffectsHaveNormalMap()
{
    ...
    for (; it != end && !isNormalMap; ++it) {
        ...
        for (BW::set<Moo::ComplexEffectMaterialPtr>::iterator effectIt =
               (*it).second.effect.begin();
             effectIt != (*it).second.effect.end() && !isNormalMap;
             ++effectIt) {
...
bool Mutant::materialShader(const BW::string& materialName,
                            const BW::string& matterName,
                            const BW::string& tintName,
                            const BW::string& fxFile,
                            bool              undoable /* = true */)
{
    ...
    if ((matterName == "") || (tintName == "Default")) {
        ...
        if (!ensureShaderCorrect(fxFile,
                                 materials_[materialName].format,
                                 materials_[materialName].dualUV))
            return false;
        ...
        if (!ensureShaderCorrect(fxFile,
                                 tints_[matterName][tintName].format,
                                 tints_[matterName][tintName].dualUV))
            return false;
    ...
    if (undoable) {
        UndoRedo::instance().barrier(
          LocaliseUTF8(
            L"MODELEDITOR/MODELS/MUTANT_MATERIALS/CHANGING_MATERIAL_EFFECT"),
          true);
    }
...
BW::string Mutant::materialShader(const BW::string& materialName,
                                  const BW::string& matterName,
                                  const BW::string& tintName)
{
    BW_GUARD;
...
bool Mutant::materialMFM(const BW::string& materialName,
                         const BW::string& matterName,
                         const BW::string& tintName,
                         const BW::string& mfmFile,
                         BW::string*       fxFile)
{
    ...
    if ((matterName == "") || (tintName == "Default")) {
        ...
        // Hmm... Special case if we don't have a tint, we have to reload the
        // material by hand
        if (tintName == "") {
            BW::set<Moo::ComplexEffectMaterialPtr>::iterator matIt =
              materials_[materialName].effect.begin();
            BW::set<Moo::ComplexEffectMaterialPtr>::iterator matEnd =
              materials_[materialName].effect.end();
    ...
    UndoRedo::instance().barrier(
      LocaliseUTF8(
        L"MODELEDITOR/MODELS/MUTANT_MATERIALS/CHANGING_MATERIAL_MFM"),
      true);
...
void Mutant::tintFlag(const BW::string& matterName,
                      const BW::string& tintName,
                      ...
                      uint32            val)
{
    ...
    DataSectionPtr data =
      tints_[matterName][tintName].data->openSection("material");
    ...
    UndoRedo::instance().barrier(
      LocaliseUTF8(L"MODELEDITOR/MODELS/MUTANT_MATERIALS/CHANGING_TINT_FLAG"),
      true);
...
uint32 Mutant::tintFlag(const BW::string& matterName,
                        const BW::string& tintName,
                        const BW::string& flagName)
{
    BW_GUARD;
    ...
    // Now handle the case where the material is using an MFM which hasn't been
    // instanciated (legacy)
...
void Mutant::materialFlag(const BW::string& materialName,
                          const BW::string& flagName,
                          uint32            val)
{
    BW_GUARD;
    ...
    for (unsigned i = 0; i < materials_[materialName].data.size(); i++) {
        ...
        if (flagName == "materialKind") {
            ...
            for (ComplexEffectMaterialSet::iterator it = effMats.begin();
                 it != effMats.end();
                 it++)
    ...
    UndoRedo::instance().barrier(
      LocaliseUTF8(
        L"MODELEDITOR/MODELS/MUTANT_MATERIALS/CHANGING_MATERIAL_FLAG"),
      true);
...
uint32 Mutant::materialFlag(const BW::string& materialName,
                            const BW::string& flagName)
{
    BW_GUARD;
...
void Mutant::tintNames(const BW::string&       matterName,
...
void Mutant::modelMaterial(int id)
{
    ...
    UndoRedo::instance().barrier(
      LocaliseUTF8(
        L"MODELEDITOR/MODELS/MUTANT_MATERIALS/CHANGING_MATERIAL_KIND"),
      false);

    triggerUpdate("Object");
}

BW::string Mutant::materialTextureFeedName(const BW::string& materialName,
                                           const BW::string& matterName,
                                           const BW::string& tintName,
...
BW::string Mutant::materialPropertyVal(const BW::string& materialName,
                                       const BW::string& matterName,
                                       const BW::string& tintName,
                                       const BW::string& propName,
...
void Mutant::changeMaterialFeed(DataSectionPtr    data,
                                const BW::string& propName,
            ...
            if (prop->readString("TextureFeed", "") != "") {
                if (feedName != "") {
                    ...
                    BW::string textureName =
                      prop->readString("TextureFeed/default", "");
...
void Mutant::materialTextureFeedName(const BW::string& materialName,
                                     const BW::string& matterName,
                                     const BW::string& tintName,
                                     const BW::string& propName,
                                     const BW::string& feedName)
{
    ...
    UndoRedo::instance().barrier(
      LocaliseUTF8(
        L"MODELEDITOR/MODELS/MUTANT_MATERIALS/CHANGING_TEXTURE_FEED_NAME"),
      false);
...
BW::string Mutant::exposedToScriptName(const BW::string& matterName,
                                       const BW::string& tintName,
                                       const BW::string& propName)
{
    BW_GUARD;
...
void Mutant::toggleExposed(const BW::string& matterName,
                           const BW::string& tintName,
                           const BW::string& propName)
{
    BW_GUARD;
    ...
    while (propsIt != propsEnd) {
        ...
        if (propName == prop->readString("name", "")) {
            ...
            UndoRedo::instance().barrier(
              LocaliseUTF8(
                L"MODELEDITOR/MODELS/MUTANT_MATERIALS/DISABLING_PYTHON"),
              false);
    ...
    data->writeVector4(
      "default", getExposedVector4(matterName, tintName, propName, "", ""));

    UndoRedo::instance().barrier(
      LocaliseUTF8(L"MODELEDITOR/MODELS/MUTANT_MATERIALS/ENABLING_PYTHON"),
      false);
}

Vector4 Mutant::getExposedVector4(const BW::string& matterName,
                                  const BW::string& tintName,
                                  const BW::string& descName,
                                  const BW::string& propType,
                                  const BW::string& val)
{
    ...
    if (propType.empty() && val.empty()) {
        Moo::ComplexEffectMaterialPtr material =
          tints_[matterName][tintName].effect;
        if (material->baseMaterial()->pEffect()) {

            Moo::EffectMaterial::Properties& properties =
              material->baseMaterial()->properties();
        ...
        if (propType == "Bool")
            exposedVector4[0] =
              tempSection->readBool(propType, false) ? 1.f : 0.f;
...
uint32 Mutant::materialSectionTextureMemUsage(DataSectionPtr       data,
    ...
    for (unsigned i = 0; i < props.size(); i++) {
        ...
        if (name != "") {
            ...
            Moo::BaseTexturePtr baseTexture =
              Moo::TextureManager::instance()->get(name, true, false, false);
            if (baseTexture) {
            ...
            if (textureFeed != NULL) {
                ...
                if ((name == "") ||
                    (texturesDone.find(name) != texturesDone.end()))
                    continue;
                texturesDone.insert(name);
                Moo::BaseTexturePtr baseTexture =
...
Moo::EffectMaterialPtr Mutant::getEffectForTint(const BW::string& matterName,
                                                const BW::string& tintName,
                                  ...
                                  const BW::string& tintName)
{
    ...
    if (pTint) {
        ...
        if (tints_.find(matterName) != tints_.end()) {
            if (tints_[matterName].find(tintName) != tints_[matterName].end()) {
                MaterialFashionVector::iterator fit =
                  std::find(materialFashions_.begin(),
                            materialFashions_.end(),
                            tints_[matterName][tintName].dye.get());

                tints_[matterName][tintName].dye =
                  superModel_->getDye(matterName, tintName);
