...
EditorAmbientLight::EditorAmbientLight()
{
    ...
    ColourProperty* pColProp   = new ColourProperty(
      LocaliseUTF8("MODELEDITOR/MODELS/LIGHTS/COLOUR"), colourProxy);
...
OmniLight::OmniLight()
{
    ...
    // Colour
    ColourProxyPtr colourProxy =
      new MeLightColourWrapper<Moo::OmniLight>(light_);
    pProp = new ColourProperty(LocaliseUTF8("MODELEDITOR/MODELS/LIGHTS/COLOUR"),
                               colourProxy);
    pProp->UIDesc(LocaliseUTF8(L"MODELEDITOR/MODELS/LIGHTS/OMNI_COLOUR"));
    editor_->addProperty(pProp);

    // Position
    matrixProxy_ = new MeLightPosMatrixProxy<Moo::OmniLight>(light_, &matrix_);
    pProp        = new GenPositionProperty(
      LocaliseUTF8("MODELEDITOR/MODELS/LIGHTS/POSITION"), matrixProxy_);
    pProp->UIDesc(LocaliseUTF8(L"MODELEDITOR/MODELS/LIGHTS/OMNI_POSITION"));
    editor_->addProperty(pProp);

    // Inner Radius
    FloatProxyPtr minRadiusProxy =
      new MeLightFloatProxy<Moo::OmniLight>(light_,
                                            &(Moo::OmniLight::innerRadius),
                                            &(Moo::OmniLight::innerRadius),
                                            "full strength radius",
                                            10.f);
    pProp =
      new GenRadiusProperty(LocaliseUTF8("MODELEDITOR/MODELS/LIGHTS/RADIUS"),
                            minRadiusProxy,
                            matrixProxy_,
                            0xbf10ff10,
                            2.f);
    ...
    FloatProxyPtr maxRadiusProxy =
      new MeLightFloatProxy<Moo::OmniLight>(light_,
                                            ...
                                            20.f);
    pProp =
      new GenRadiusProperty(LocaliseUTF8("MODELEDITOR/MODELS/LIGHTS/FALLOFF"),
                            maxRadiusProxy,
                            matrixProxy_,
                            0xbf1010ff,
                            4.f);
    ...
    // Multiplier
    FloatProxyPtr multiplierProxy =
      new MeLightFloatProxy<Moo::OmniLight>(light_,
                                            &(Moo::OmniLight::multiplier),
                                            &(Moo::OmniLight::multiplier),
                                            "multiplier",
                                            1.f);
    pProp = new GenFloatProperty(
      LocaliseUTF8("MODELEDITOR/MODELS/LIGHTS/MULTIPLIER"), multiplierProxy);
...
DirLight::DirLight()
{
    ...
    // Colour
    ColourProxyPtr colourProxy =
      new MeLightColourWrapper<Moo::DirectionalLight>(light_);
    pProp = new ColourProperty(LocaliseUTF8("MODELEDITOR/MODELS/LIGHTS/COLOUR"),
                               colourProxy);
    pProp->UIDesc(
      LocaliseUTF8(L"MODELEDITOR/MODELS/LIGHTS/DIRECTIONAL_COLOUR"));

    editor_->addProperty(pProp);

    // Rotation
    matrixProxy_ =
      new MeLightDirMatrixProxy<Moo::DirectionalLight>(light_, &matrix_);
    pProp = new GenRotationProperty(
      LocaliseUTF8("MODELEDITOR/MODELS/LIGHTS/DIRECTION"), matrixProxy_);
    pProp->UIDesc(
      LocaliseUTF8(L"MODELEDITOR/MODELS/LIGHTS/DIRECTIONAL_DIRECTION"));
    editor_->addProperty(pProp);

    // Multiplier
    FloatProxyPtr multiplierProxy =
      new MeLightFloatProxy<Moo::DirectionalLight>(
        light_,
    ...
    pProp = new GenFloatProperty(
      LocaliseUTF8("MODELEDITOR/MODELS/LIGHTS/MULTIPLIER"), multiplierProxy);
    pProp->UIDesc(
      LocaliseUTF8(L"MODELEDITOR/MODELS/LIGHTS/DIRECTIONAL_INTENSITY"));
    editor_->addProperty(pProp);
...
SpotLight::SpotLight()
{
    ...
    // Colour
    ColourProxyPtr colourProxy =
      new MeLightColourWrapper<Moo::SpotLight>(light_);
    pProp = new ColourProperty(LocaliseUTF8("MODELEDITOR/MODELS/LIGHTS/COLOUR"),
                               colourProxy);
    ...
    // Position + Rotation
    matrixProxy_ =
      new MeSpotLightPosDirMatrixProxy<Moo::SpotLight>(light_, &matrix_);
    pProp = new GenPositionProperty(
      LocaliseUTF8("MODELEDITOR/MODELS/LIGHTS/POSITION"), matrixProxy_);
    ...
    // Inner Radius
    FloatProxyPtr minRadiusProxy =
      new MeLightFloatProxy<Moo::SpotLight>(light_,
    ...
    pProp =
      new AngleProperty(LocaliseUTF8("MODELEDITOR/MODELS/LIGHTS/CONE_ANGLE"),
                        coneAngleProxy,
                        matrixProxy_);
    ...
    if (dirty_) {
        int result =
          ::MessageBox(AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
                       ...
                       Localise(L"MODELEDITOR/APP/ME_APP/LIGHTS_CHANGED"),
        ...
        if (result == IDNO) {
            ME_WARNING_MSGW(
              Localise(L"MODELEDITOR/APP/ME_APP/LIGHTS_NOT_SAVED"));
        }
    }
...
bool Lights::open(const BW::string& name, DataSectionPtr file /* = NULL */)
{
    ...
    while (lightsIt != lightsEnd) {
        ...
        Moo::Colour colour(
          colourVector[2], colourVector[1], colourVector[0], 1.f);
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (omni->enabled()) {
            ...
            if (pChunk) {
                ...
                l->pLight() = omni->light();
                l->pLight()->position(
                  pChunk->transformInverse().applyPoint(pos));
                l->toss(pChunk);
            }
        }
    }
...
bool Lights::save(const BW::string& name /* = "" */)
{
    ...
    if (name == "") {
        if (currFile_ == NULL) {
            ...
            if (fileDlg.showDialog()) {
                currName_ = BWResource::dissolveFilename(
                  bw_wtoutf8(fileDlg.getFileName()));
    ...
    for (int i = 0; i < numSpot(); i++) {
        ...
        pLight->writeFloat(
          "Cone_Size",
          (float)(180.f * acos(light->light()->cosConeAngle()) / MATH_PI));
    }
