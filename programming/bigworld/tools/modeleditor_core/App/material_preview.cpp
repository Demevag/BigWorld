...
//==============================================================================
BinaryPtr MaterialPreview::generatePreview(const BW::string& materialName,
                                           ...
                                           const BW::string& tintName)
{
    ...
    BW::string textureString = MeApp::instance().mutant()->materialPropertyVal(
      materialName, matterName, tintName, "diffuseMap", "Texture");
    ...
    if (!textureString.empty()) {
        ...
        if (pLockableTexture.exists()) {
            ...
            HRESULT hr = D3DXSaveTextureToFileInMemory(
              &buffer, D3DXIFF_BMP, pLockableTexture->pTexture(), NULL);
    ...
    if (!conversionSuccess) {
        BW::string colourString =
          MeApp::instance().mutant()->materialPropertyVal(
            materialName, matterName, tintName, "Color", "Vector4");

        if (colourString == "") {
            colourString = MeApp::instance().mutant()->materialPropertyVal(
              materialName, matterName, tintName, "Colour", "Vector4");
        }
        ...
        Moo::Colour col;
        sscanf(
          colourString.c_str(), "%f %f %f %f", &col.r, &col.g, &col.b, &col.a);

        if (!rt_) {
            rt_ = new Moo::RenderTarget("Material Preview");
            if (!rt_ || !rt_->create(128, 128)) {
                WARNING_MSG(
                  "Could not create render target for material preview.\n");
                rt_ = NULL;
                return NULL;
            }
        }
        ...
        if (rt_->push()) {
            ...
            Geometrics::drawRect(Vector2(0, 0), Vector2(128, 128), col);

            Moo::rc().endScene();

            HRESULT hr = D3DXSaveTextureToFileInMemory(
              &buffer, D3DXIFF_BMP, rt_->pTexture(), NULL);

            if (FAILED(hr)) {
                WARNING_MSG("Could not create material preview file (D3D error "
                            "= 0x%x).\n",
                            hr);
                rt_->pop();
                return NULL;
