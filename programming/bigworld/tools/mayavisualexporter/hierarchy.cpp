...
{
    for (BW::map<BW::string, Hierarchy*>::iterator it = _children.begin();
         it != _children.end();
         ++it)
        delete it->second;
    ...
    for (uint32 i = 0; i < (int)mesh.meshes().length(); ++i) {
        bool bspNode =
          (toLower(mesh.meshes()[i].fullPathName().asChar()).find("_bsp") !=
           BW::string::npos);
        // dont add bsp nodes
        if ((bspNode == false) && (blendShapes.isBlendShapeTarget(
                                     mesh.meshes()[i].node()) == false)) {
            MStatus      stat;
            MFnTransform trans(mesh.meshes()[i].transform(), &stat);
            if (stat == MS::kSuccess) {
                addNode(trans.fullPathName().asChar(),
                        findDagPath(trans.fullPathName().asChar()));
            }
        }
    }
    fflush(stdout);
...
void Hierarchy::addNode(const BW::string& path,
                        MDagPath          dag,
                        BW::string        accumulatedPath)
{
    if (!parent_) {
        ...
        if (child.find("|") != BW::string::npos) {
            ...
            if (_children.find(child) == _children.end()) {
                ...
                _children[child]->_path =
                  findDagPath(accumulatedPath + "|" + child);
            }
...
BW::vector<BW::string> Hierarchy::children(bool orderChildren /*= false*/)
{
    ...
    if (orderChildren) {
        ...
        // Order the immediate children by their number of immediate decendences
        // in decending order.  This is required to make sure that the root bone
        // is selected as the itinerant bone inside model editor.
        //
        // 1) Populate a list
        for (BW::map<BW::string, Hierarchy*>::iterator it = _children.begin();
             it != _children.end();
        ...
        for (BW::list<std::pair<BW::string, Hierarchy*>>::iterator it =
               objectPairs.begin();
             it != objectPairs.end();
             ++it) {
...
Hierarchy& Hierarchy::find(const BW::string& path)
{
    ...
    while (it != _children.end()) {
        ...
        //~ return *it->second;
        if (BW::string(it->second->_path.fullPathName().asChar()) ==
              path.substr(0,
                          BW::string(it->second->_path.fullPathName().asChar())
                            .length()) &&
            ((BW::string(it->second->_path.fullPathName().asChar()).length() <
                path.length() &&
              path[BW::string(it->second->_path.fullPathName().asChar())
                     .length()] == '|') ||
             (BW::string(it->second->_path.fullPathName().asChar()).length() ==
              path.length())))
...
uint32 Hierarchy::numberOfFrames()
{
    ...
    // note the values in MTime correspond to frames if start and end are in
    // MTime::uiUnit() times
...
matrix4<float> Hierarchy::transform(double frame, bool relative)
{
    ...
    double numberOfFrames =
      end.as(MTime::uiUnit()) - start.as(MTime::uiUnit()) + 1;
    ...
    if (relative == false)
        ...
        MFloatMatrix parent(!parent_ ? _path.exclusiveMatrix().matrix
                                     : parent_->_path.inclusiveMatrix().matrix);
 ...
 */
bool numChildDescending(std::pair<BW::string, Hierarchy*> lhs,
