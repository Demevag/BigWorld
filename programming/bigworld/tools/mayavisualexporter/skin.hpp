...
class Skin
{
  public:
    ...
    // returns number of vertices for geometry object path (0 if not influenced
    // by skin)
    uint32 numberOfVertices(const BW::string& path);

    // returns number of vertex data for geometry object path (empty BW::vector
    // if not influenced by skin)
    ...
    // returns the initial transform for the bone boneName (where boneName is
    // the full bone name)
    matrix4<float> transform(const BW::string& boneName, bool relative = false);

    // this method removes skin bits that do not influence any of the meshes in
    // the mDagPathArray
  ...
  protected:
    ...
    // maps joint paths to bone indexes, refers to *current* initialised skin
    // only
    ...
    // initial bone transforms, to get the path of the object at an index, use
    // the same index into _bonePaths
