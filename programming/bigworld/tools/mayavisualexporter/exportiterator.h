...
class DependencyExportIterator : public ExportIteratorBase
{
  ...
  public:
    DependencyExportIterator(MFn::Type filter = MFn::kInvalid,
                             MStatus*  status = NULL)
    ...
    virtual MStatus next() { return mItDependencyNodes.next(); }
    virtual MStatus reset() { return mItDependencyNodes.reset(); }
...
class SelectionExportIterator : public ExportIteratorBase
{
  ...
  public:
    SelectionExportIterator(MFn::Type filter = MFn::kInvalid,
                            MStatus*  status = NULL)
    {
    ...
    ~SelectionExportIterator() { delete mItSelectionList; }
    virtual bool isDone(MStatus* status = NULL)
    {
        return mItSelectionList->isDone(status);
    }
    virtual MStatus next() { return mItSelectionList->next(); }
    virtual MStatus reset() { return mItSelectionList->reset(); }
    virtual MObject item(MStatus* status = NULL)
    {
        MObject result;
...
class ExportIterator : public ExportIteratorBase
{
  ...
  public:
    ...
    ~ExportIterator() { delete mExportIterator; }
    virtual bool isDone(MStatus* status = NULL)
    {
        return mExportIterator->isDone(status);
    }
    virtual MStatus next() { return mExportIterator->next(); }
    virtual MStatus reset() { return mExportIterator->reset(); }
    virtual MObject item(MStatus* status = NULL)
    {
