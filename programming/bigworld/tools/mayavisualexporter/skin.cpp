...
bool Skin::initialise(uint32 index)
{
    ...
    for (uint32 i = 0; i < numberOfGeometry; ++i) {
        ...
        BW::vector<BoneVertex>& vertices =
          _vertices[geometry.fullPathName().asChar()];
        ...
        for (uint32 h = 0; h < vertexList.length(); ++h) {
            ...
            if (ExportSettings::instance().useLegacyOrientation()) {
                // This legacy code came from the 1.7.2 release
                vertices.push_back(
                  BoneVertex(Point3(-static_cast<float>(vertexList[h].x),
        ...
        for (; it.isDone() == false; it.next()) {
            ...
            // NOTE for some reason sorting the ( index1, weight1 ), ( index2,
            // weight2 ) and ( index3, weight3 ) royally screws the mesh up?? -
            // even though they are sorted properly
            ...
            if (vertices[j].weight1 != 0) {
                ...
                // fix the index
                if (_boneSets[i].indexes.find(
                      _bonePaths[vertices[j].index1].fullPathName().asChar()) ==
                    _boneSets[i].indexes.end()) {
                    printf(
                      "Bone not found in Bone Set: %s\n:",
                      _bonePaths[vertices[j].index1].fullPathName().asChar());
                    fflush(stdout);
                }
                vertices[j].index1 =
...
{
    BW::map<BW::string, BW::vector<BoneVertex>>::iterator it =
      _vertices.find(path);
    if (it == _vertices.end())
        return 0;
    else
...
BW::vector<BW::string> Skin::paths()
{
    ...
    for (BW::map<BW::string, BW::vector<BoneVertex>>::iterator it =
           _vertices.begin();
         it != _vertices.end();
         ++it)
