...
{
  public:
    Color(float r = 1.0f, float g = 0.5f, float b = 0.25f)
      : r(r)
      , g(g)
      , b(b)
    {
...
class VisualMesh : public ReferenceCount
{
  ...
  public:
    ...
    struct Material
    {
        ...
        bool operator==(const Material& m) const
        {
            return identifier == m.identifier &&
                   ...
                   mapIdentifier == m.mapIdentifier && selfIllum == m.selfIllum;
        }

        //		void save( DataSectionPtr spGeometry, uint32 pgid, bool skinned
        //= false, int nodeCount = 0 ) const;
        enum SkinType
        {
            NO_SKIN,
            SOFT_SKIN
    ...
    MObject object; // Maya object for mesh, isNull() returns true except for
                    // blend shapes
  ...
  protected:
    ...
    void   addNormal(const vector3<float>& normal,
                     int                   realIndex,
                     int                   smoothingGroup,
                     int                   index);
