...
{
  public:
    BlendShapesDisabler() { BlendShapes::disable(); }

    ~BlendShapesDisabler() { BlendShapes::enable(); }
};
...
static float getWeight(MFnBlendShapeDeformer& blendShape,
                       uint32                 weightIndex,
                       uint32                 frame)
{
    MAnimControl control;
...
bool BlendShapes::initialise(uint32 index)
{
    ...
    for (uint32 i = 0; i < baseObjects.length(); ++i) {
        ...
        for (meshIndex = 0; meshIndex < dagPathArray.length(); ++meshIndex) {
            BW::string fullName =
              dagPathArray[meshIndex].fullPathName().asChar();
        ...
        for (uint32 n = 0; n < vertexList.length(); ++n) {
            ...
            if (ExportSettings::instance().useLegacyOrientation()) {
                ...
                _bases.back().vertices.push_back(
                  vector3<float>(-static_cast<float>(vertexList[n].x),
        ...
        for (uint32 j = 0; j < blendShape.numWeights(); ++j) {
            ...
            blendShape.getTargets(
              baseObjects[i], weightIndexList[j], targetObjects);
            ...
            for (uint32 k = 0; k < numTargets; ++k) {
                ...
                found = false;
                for (meshIndex = 0; meshIndex < dagPathArray.length();
                     ++meshIndex) {
                    BW::string fullName =
                      dagPathArray[meshIndex].fullPathName().asChar();
                    BW::string name =
                      fullName.substr(fullName.find_last_of("|") + 1);
                ...
                for (uint32 n = 0; n < numberOfFrames(); ++n) {
                    _targets[i][k + j * numTargets].weights.push_back(
                      getWeight(blendShape, weightIndexList[j], n));
                }
            }
        }
    }
...
{
    return _targets[base][target].vertices[index] -
           _bases[base].vertices[index];
}
