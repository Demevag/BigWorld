...
class ExporterDialogBase
{
  protected:
    ...
    ~ExporterDialogBase() { instance() = NULL; }

  public:
    int doModal(HINSTANCE hinst, HWND parent)
    {
        return (int)DialogBox(
          hinst, MAKEINTRESOURCE(DlgResource), parent, dialogProc);
    }
    virtual void prepareWindow()
    {
        ...
        int centerX =
          parentRect.left + (parentRect.right - parentRect.left) / 2;
    ...
    {
        SendMessage(GetDlgItem(hwnd_, child),
                    BM_SETCHECK,
...
class SettingDialog
  : public ExporterDialogBase<SettingDialog, IDD_SETTINGSDIALOG>
{
    int  boneCount_;
  ...
  public:
    SettingDialog()
    {
        ...
        disableVisualChecker_ =
          ExportSettings::instance().disableVisualChecker();
        useLegacyScaling_ = ExportSettings::instance().useLegacyScaling();
        useLegacyOrientation_ =
          ExportSettings::instance().useLegacyOrientation();
    ...
    {
        if (INT_PTR result =
              ExporterDialogBase<SettingDialog,
                                 IDD_SETTINGSDIALOG>::processMessage(msg,
                                                                     wParam,
                                                                     lParam))
            return result;
        if (msg == WM_COMMAND) {
            if (LOWORD(wParam) == IDOK) {
                ...
                disableVisualChecker_ =
                  ExportSettings::instance().disableVisualChecker();
                useLegacyScaling_ =
                  ExportSettings::instance().useLegacyScaling();
                useLegacyOrientation_ =
                  ExportSettings::instance().useLegacyOrientation();
                sceneRootAdded_ = ExportSettings::instance().sceneRootAdded();
                fixCylindrical_ = ExportSettings::instance().fixCylindrical();
            ...
            } else if (LOWORD(wParam) == IDC_VISUALCHECKER ||
                       LOWORD(wParam) == IDC_LEGACY_SCALING ||
                       LOWORD(wParam) == IDC_LEGACY_ORIENTATION ||
                       LOWORD(wParam) == IDC_CYLINDRICAL ||
                       LOWORD(wParam) == IDC_BONE_STEP &&
    ...
    void gatherSettings()
    {
        ...
        ExportSettings::instance().disableVisualChecker(
          checked(IDC_VISUALCHECKER));
        ExportSettings::instance().useLegacyScaling(
          checked(IDC_LEGACY_SCALING));
        ExportSettings::instance().useLegacyOrientation(
          checked(IDC_LEGACY_ORIENTATION));
        ExportSettings::instance().sceneRootAdded(
          checked(IDC_SCENE_ROOT_ADDED));
    ...
    virtual void setControlStatus()
    {
        ...
        SendMessage(GetDlgItem(hwnd_, IDC_BONE_STEP_SPIN),
                    UDM_SETRANGE,
                    0,
                    MAKELONG(85, 3));
        SetDlgItemInt(
          hwnd_, IDC_BONE_STEP, ExportSettings::instance().maxBones(), FALSE);
        check(IDC_VISUALCHECKER,
              ExportSettings::instance().disableVisualChecker());
        check(IDC_LEGACY_SCALING,
              ExportSettings::instance().useLegacyScaling());
        check(IDC_LEGACY_ORIENTATION,
              ExportSettings::instance().useLegacyOrientation());
        check(IDC_SCENE_ROOT_ADDED,
              ExportSettings::instance().sceneRootAdded());
        check(IDC_CYLINDRICAL, ExportSettings::instance().fixCylindrical());
    }
};

class VisualExporterDialog
  : public ExporterDialogBase<VisualExporterDialog, IDD_VISUALEXPORT_DLG>
{
  public:
    virtual INT_PTR processMessage(UINT msg, WPARAM wParam, LPARAM lParam)
    {
        if (INT_PTR result =
              ExporterDialogBase<VisualExporterDialog,
                                 IDD_VISUALEXPORT_DLG>::processMessage(msg,
                                                                       wParam,
                                                                       lParam))
            return result;
        if (msg == WM_COMMAND && HIWORD(wParam) == BN_CLICKED) {
            switch (LOWORD(wParam)) {
                ...
                case IDSETTINGS: {
                    SettingDialog sd;
                    sd.doModal(
                      (HINSTANCE)GetWindowLongPtr(hwnd_, GWLP_HINSTANCE),
                      hwnd_);
                } break;
                case IDC_COPY_TEXTURES_BROWSE: {
                    MString cmd =
                      "import maya.cmds as cmds; cmds.fileDialog2("
                    ...
                    if (MGlobal::executePythonCommand(cmd, result) ==
                        ...
                        if (result.length() > 0) {
                            ...
                            if (!BWUtil::isAbsolutePath(
                                ...
                                MessageBox(
                                  NULL,
                                  (bw_utf8tow(textureLocation) +
                                   L" is not a valid resource location")
                                    .c_str(),
    ...
    void gatherSettings()
    {
        ...
        ExportSettings::instance().transformToOrigin(
          checked(IDC_TRANSFORMTOORIGIN));
        ExportSettings::instance().allowScale(checked(IDC_ALLOWSCALE));
        ExportSettings::instance().bumpMapped(checked(IDC_BUMPMAP));
        ExportSettings::instance().keepExistingMaterials(
          checked(IDC_KEEP_EXISTING_MATERIALS));
        ExportSettings::instance().referenceNode(
          checked(IDC_USE_REFERENCE_NODE));
        ExportSettings::instance().snapVertices(checked(IDC_SNAP_VERTICES));
        ExportSettings::instance().stripRefPrefix(
          checked(IDC_STRIP_REF_PREFIX));

        ExportSettings::instance().copyExternalTextures(
          checked(IDC_COPY_TEXTURES_OPTION));
        ExportSettings::instance().copyTexturesTo(
          getText(IDC_COPY_TEXTURES_LOCATION));
    }
    virtual void setControlStatus()
    {
        SetDlgItemText(
          hwnd_,
          IDC_VISUAL_TYPE_ID,
          bw_utf8tow(("Checking against: " +
                      ExportSettings::instance().visualTypeIdentifier()))
            .c_str());
        if (ExportSettings::instance().exportAnim()) {
            ...
            enable(IDC_COPY_TEXTURES,
                   ExportSettings::instance().copyExternalTextures());
            enable(IDC_COPY_TEXTURES_BROWSE,
                   ExportSettings::instance().copyExternalTextures());
            enable(IDC_COPY_TEXTURES_LOCATION,
                   ExportSettings::instance().copyExternalTextures());
        }
        check(IDC_EXPORT_NORMAL, ExportSettings::instance().exportMode() == 0);
        check(IDC_EXPORT_STATIC, ExportSettings::instance().exportMode() == 1);
        check(IDC_EXPORT_STATIC_WITH_NODES,
              ExportSettings::instance().exportMode() == 2);
        check(IDC_EXPORT_MESH_PARTICLES,
              ExportSettings::instance().exportMode() == 3);

        check(IDC_TRANSFORMTOORIGIN,
              ExportSettings::instance().transformToOrigin());
        check(IDC_ALLOWSCALE, ExportSettings::instance().allowScale());
        check(IDC_BUMPMAP, ExportSettings::instance().bumpMapped());
        check(IDC_KEEP_EXISTING_MATERIALS,
              ExportSettings::instance().keepExistingMaterials());
        check(IDC_USE_REFERENCE_NODE,
              ExportSettings::instance().referenceNode());
        check(IDC_SNAP_VERTICES, ExportSettings::instance().snapVertices());
        check(IDC_STRIP_REF_PREFIX,
              ExportSettings::instance().stripRefPrefix());

        check(IDC_COPY_TEXTURES_OPTION,
              ExportSettings::instance().copyExternalTextures());
        setText(IDC_COPY_TEXTURES_LOCATION,
                ExportSettings::instance().copyTexturesTo());
    }
