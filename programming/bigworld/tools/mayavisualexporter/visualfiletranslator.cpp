...
namespace {
    ...
    // This is all handled in a single thread, so global/static access should be
    // ok. I didn't think that it needed to be a VisualFileTranslator member
    ...
    struct ScopedMELErrorVariableHandler
    {
        ...
        ~ScopedMELErrorVariableHandler()
        {
            BW::string mayaAssignErrorCmd =
              BW::string("$gBWVisualFileError = \"") + melErrorMessages.str() +
              "\"";
...
bool checkAnyRowHasZeroScale(const Vector3& row0,
                             const Vector3& row1,
                             const Vector3& row2)
{
...
void exportTree(DataSectionPtr pParentSection,
                Hierarchy&     hierarchy,
                BW::string     parent               = "",
                bool*          hasInvalidTransforms = 0)
{
    ...
    if (pThisSection) {
        ...
        if (hasInvalidTransforms && ((*hasInvalidTransforms) == false)) {
            if (!checkAnyRowHasZeroScale(row0, row1, row2)) {
                ...
                Matrix testMatrix =
                  pThisSection->readMatrix34("transform", Matrix::identity);
    ...
    for (int i = 0; i < (int)hierarchy.children().size(); i++) {
        exportTree(pThisSection,
                   hierarchy.child(hierarchy.children()[i]),
                   parent + "|" + hierarchy.name(),
                   hasInvalidTransforms);
    }
}

void exportAnimationXmlHeader(const BW::string& animDebugXmlFileName,
...
void exportAnimationXml(DataSectionPtr spParentNode,
                        Hierarchy&     hierarchy,
    ...
    if (hierarchy.name() != "") {
        ...
        for (int i = firstFrame; i <= lastFrame; i++) {
            ...
            positions.push_back(PositionKey(
              time, bt.translation() * ExportSettings::instance().unitScale()));
        ...
        BW::string nodeName =
          (stripRefPrefix)
            ? (stripReferencePrefix(hierarchy.path().substr(
                hierarchy.path().find_last_of("|") + 1)))
        ...
        for (rotIt = rotations.begin(); rotIt != rotations.end(); ++rotIt) {
            ...
            pRotation->writeVector4("Value",
                                    Vector4(rotIt->second.x,
                                            rotIt->second.y,
                                            rotIt->second.z,
                                            rotIt->second.w));
        }
...
void exportAnimation(BinaryFile& animFile,
                     Hierarchy&  hierarchy,
                     bool        stripRefPrefix = false)
{
    if (hierarchy.name() != "") {
        ...
        int lastFrame  = hierarchy.numberOfFrames() -
                        1; //~ExportSettings::instance().lastFrame();
...

        {
            ...
            if (stripRefPrefix)
                animFile << stripReferencePrefix(hierarchy.path().substr(
                  hierarchy.path().find_last_of("|") + 1));
            else
                animFile << hierarchy.path().substr(
                  hierarchy.path().find_last_of("|") + 1);
        }
...
void exportPortalsToBoundaries(DataSectionPtr               pVisualSection,
                               BW::vector<VisualPortalPtr>& portals)
{
    if (!portals.size())
        return;
    ...
    for (size_t i = 0; i < portals.size(); ++i) {
        ...
        if (bb.centre().dotProduct(portalPlane.normal()) - portalPlane.d() >
            0.f) {
    ...
    if (!eachOnBoundary) {
        if (!LogMsg::automatedTest()) {
            ::MessageBox(0,
                         L"One or more portals will not be exported since they "
                         L"are not on a boundary!",
                         L"Portal error!",
                         MB_ICONERROR);
        }
        melErrorMessages.addMultiLineString(
          "One or more portals will not be exported since they are not on a "
          "boundary!");
    }
}
 ...
 */
void findDuplicateNodes(Hierarchy&           hierarchy,
                        ...
                        BW::set<BW::string>& duplicates)
{
    ...
    for (int i = 0; i < (int)hierarchy.children().size(); i++) {
        findDuplicateNodes(
          hierarchy.child(hierarchy.children()[i]), names, duplicates);
    }
}
...
bool checkHierarchy(Hierarchy& hierarchy)
{
    ...
    if (duplicates.size()) {
        ...
        if (!LogMsg::automatedTest()) {
            ::MessageBoxA(0,
                          errorBuilder.string(),
                          "Bone hierarchy validity checking",
                          MB_ICONERROR);
        }
...
void warnDuplicateMeshNames(const BW::set<BW::string>& duplicates)
{
    ...
    ::MessageBoxA(
      0, errorBuilder.string(), "Model name validity checking", MB_ICONERROR);
    melErrorMessages.addMultiLineString(errorBuilder.string());
}
...
void invalidExportDlg(const BW::string& fileName)
{
    ...
    if (!LogMsg::automatedTest()) {
        MessageBox(NULL,
                   ...
                   L"Visual/Animation Exporter Error",
...
void VisualFileTranslator::getBoneCounts(BoneCountMap& boneCountMap)
{
    ...
    for (visualMeshVectorIt it = beginIt; it != endIt; ++it) {
        ...
        if (visEnv) {
            boneCountMap.insert(std::pair<BW::string, size_t>(
...
bool VisualFileTranslator::exportMesh(BW::string fileName)
{
    ...
    bool shell =
      isShell(resName); // boolean for checking if this file will be a shell
    ...
    if (pVisualSection) {
        ...
        Mesh mesh;
        if (ExportSettings::instance().exportMode() != ExportSettings::STATIC &&
        ...
        if (skin.count() > 0 && skin.initialise(0) &&
            ...
            for (uint32 j = 0; j < skin.count(); ++j) {
                ...
                for (uint32 i = 0; i < skin.numberOfBones(); ++i) {
                    hierarchy
                      .find(BW::string(skin.boneDAG(i).fullPathName().asChar()))
                      .addNode(
                        BW::string(skin.boneDAG(i).fullPathName().asChar()) +
                          "_BlendBone",
                        matrix4<
                          float>() /*skin.transform( i, false ).inverse()*/);
                }
            }
        }
        ...
        if (hasInvalidTransforms) {
            ...
            melErrorMessages.addMultiLineString(
              "The visual contains bones with invalid transforms "
        ...
        // save out each mesh
        size_t meshCount =
          visualMeshes.size() + bspMeshes.size() + hullMeshes.size();
        int count = 0;
        while (visualMeshes.size()) {
            if (ExportSettings::instance().exportMode() ==
                ExportSettings::NORMAL) {

                if (!visualMeshes.back()->save(pVisualSection,
                                               pExistingVisual,
                                               tempPrimFileName,
                                               true)) {
                    melErrorMessages.addMultiLineString(
                      visualMeshes.back()->errorText());
                    // return false;
                }
                ...
                if (stat == MS::kSuccess) {
                    Hierarchy* pNode =
                      &hierarchy.find(trans.fullPathName().asChar());
                    if (&hierarchy != pNode) {
                        matrix4<float> trans =
                          hierarchy.transform(0, false).inverse() *
                          pNode->transform(0, false);
                ...
                accMesh->dualUV(accMesh->dualUV() ||
                                visualMeshes.back()->dualUV());
                accMesh->vertexColours(accMesh->vertexColours() ||
                                       visualMeshes.back()->vertexColours());

                bool worldSpaceAcc = ExportSettings::instance().exportMode() ==
                                         ExportSettings::MESH_PARTICLES
                                       ? false
                                       : true;
        ...
        bb.setBounds(
          Vector3(bb.minBounds().x, bb.minBounds().y, bb.minBounds().z) *
            ExportSettings::instance().unitScale(),
          Vector3(bb.maxBounds().x, bb.maxBounds().y, bb.maxBounds().z) *
            ExportSettings::instance().unitScale());
    ...
    if (!ExportSettings::instance().disableVisualChecker()) {
        VisualChecker vc(
          tempFileName, false, ExportSettings::instance().snapVertices());
    ...
    if (ExportSettings::instance().exportMode() !=
        ...
        else if (ExportSettings::instance().exportMode() !=
                   ExportSettings::NORMAL &&
                 ExportSettings::instance().exportMode() !=
                   ExportSettings::NORMAL) {
...
class VisualFileTranslator::AutoCleanup
{
  public:
    ...
    ~AutoCleanup() { translator_.cleanup(); }
...
{
    return ((str.isInt() && (str.asInt() != 0)) || (str == "True"));
}
...
void parseOptionsString(const MString& optionsString, ExportSettings& settings)
{
    if (optionsString.length() > 0) {
        ...
        optionsString.split(';',
                            optionList); // break out all the optionsString.
        ...
        for (unsigned int i = 0; i < optionList.length(); ++i) {
            ...
            if (theOption.length() > 1) {
                if (theOption[0] == "exportAnimation") {
                    ...
                    ExportSettings::ExportMode exportMode =
                      parseMStringExportMode(theOption[1]);
                    ...
                    settings.keepExistingMaterials(
                    ...
                    settings.setExportEnvelopesAndBones(
                      parseMStringBool(theOption[1]));
                    ...
                    settings.setExportAnimations(
                      parseMStringBool(theOption[1]));
                } else if (theOption[0] == "useCharacterMode") {
                    settings.setUseCharacterMode(
                      parseMStringBool(theOption[1]));
                    ...
                    ExportSettings::NodeFilter nodeFilter =
                      parseMStringNodeFilter(theOption[1]);
                    ...
                    // std::cerr << "VisualFileTranslator export: unrecognised
                    // option '" << theOption[0] << "'." << std::endl;
                }
            }
        }
    }
...
MStatus VisualFileTranslator::writer(const MFileObject& file,
                                     ...
                                     FileAccessMode     mode)
{
    ...
    const BW::string output_filename =
      BWResource::removeExtension(bw_acptoutf8(file.fullName().asChar()))
        .to_string();
    ...
    // Override any of the previously defined settings from the values in
    // optionsString.
    ...
    if (settings.transformToOrigin()) {
        ...
        if (settings.exportMode() == ExportSettings::NORMAL ||
            ...
            for (uint32 i = 0; i < meshTemp.count(); ++i) {
                MDagPath transform;
                MDagPath::getAPathTo(meshTemp.meshes()[i].transform(),
                                     transform);
        ...
        for (uint32 i = 0; i < transforms.length(); ++i) {
            ...
            for (uint32 j = rootTransforms.length(); j > 0; --j) {
                ...
                if (strncmp(transformPath.asChar(),
                            rootTransformPath.asChar(),
                            std::min(transformPathLen, rootTransformPathLen)) !=
                    0) {
                    continue;
                }
    ...
    for (uint32 i = 0; i < mesh.count(); ++i) {
        ...
        bool bspNode =
          (toLower(mesh.meshes()[i].fullPathName().asChar()).find("_bsp") !=
           BW::string::npos);
        if (mesh.initialise(
              i,
              (settings.exportMode() == ExportSettings::NORMAL ||
               settings.exportMode() == ExportSettings::MESH_PARTICLES) &&
                !bspNode) == false)
            continue;

        if (mesh.name()
              .substr(mesh.name().find_last_of("|") + 1)
              .substr(0, 3) == "HP_")
            continue;
        ...
        bool hullNode =
          (toLower(mesh.meshes()[i].fullPathName().asChar()).find("_hull") !=
           BW::string::npos);
        if (hullNode && skipMesh == false) {
            // add the mesh to a hull node meshlist
            //~ printf( "Hull Mesh: %s\n",
            // mesh.meshes()[i].fullPathName().asChar() ); ~ fflush( stdout );
            HullMeshPtr spHullMesh = new HullMesh;
            spHullMesh->snapVertices(settings.exportMode() !=
                                       ExportSettings::NORMAL &&
        ...
        for (uint32 j = 0; settings.exportMode() == ExportSettings::NORMAL &&
                           skipMesh == false && j < skin.count();
             ++j) {
            skin.initialise(j);

            for (uint32 k = 0; skipMesh == false && k < skin.meshes().length();
                 ++k) {
                if (mesh.meshes()[i] == skin.meshes()[k]) {
                    mesh.initialise(i, false);
                    //~ printf( "Skinned: %s\n",
                    // mesh.meshes()[i].fullPathName().asChar() ); ~ fflush(
                    // stdout );
            ...
            if (skipMesh == true) {
                if (skinned == true) {
                    ...
                    if (spVisualEnvelope) {
                        ...
                        if (spVisualEnvelope->split(
                              ExportSettings::instance().maxBones(),
                              splitEnvelopes)) {
    ...
    if (!hasPortal) {
        ...
        for (HullMeshIt iter = hullMeshes.cbegin(); iter != hullMeshes.cend();
             ++iter) {
    ...
    if (!isShell(resName) && hasPortal) {
        if (!LogMsg::automatedTest()) {
            ...
            LogMsg::logToFile(
              "WARNING: You are exporting an object with portals "
              "into a non shell directory. This object will not be recognised "
              "as a shell ");
        }
    ...
    if (ExportSettings::instance().exportMode() !=
        ...
        !settings.exportAnim()) {
        BW::string resName =
          BWResolver::dissolveFilename(output_filename.c_str());
        ...
        if (pModelSection) {
            ...
            BW::string filename =
              BWResource::removeExtension(resName).to_string();
            if (ExportSettings::instance().exportMode() ==
                  ExportSettings::NORMAL ||
                ExportSettings::instance().exportMode() ==
    ...
    if (settings.exportAnim()) {
        ...
        if ((name2.length() == name.length()) ||
            ...
            errors = true;
            BW::string errorString;
            errorString = BW::string("The exported file \"") + name +
                          BW::string("\" is not in a valid game path.\n");
            errorString += "Need to export to a *subfolder* of:";
            uint32 pathIndex = 0;
            while (BWResource::getPath(pathIndex) != BW::string("")) {
        ...
        if ((cropIndex = animName.find_last_of("\\/")) > 0) {
            cropIndex++;
            animName =
              animName.substr(cropIndex, animName.length() - cropIndex);
        }

        {
            ...
            FILE* file2 = _wfopen(
              bw_utf8tow((BWResource::removeExtension(name) + ".animation"))
                .c_str(),
              L"wb");

#if DEBUG_ANIMATION_FILE
            // Delete the previous animation debug xml file if it exists
            BW::string animDebugXmlFileName =
              BWResource::removeExtension(name) + ".animation.xml";
            ...
            if (file2) {
                ...
                if (!referenceNodesFile.empty()) {
                    ...
                    StringHashMap<BW::string>::iterator it =
                      nodeParents_.begin();
                    StringHashMap<BW::string>::iterator end =
                      nodeParents_.end();
                    while (it != end) {
                ...
                animation << float(hierarchy.numberOfFrames() - 1) << animName
                          << animName;
                animation << nChannels;

#if DEBUG_ANIMATION_FILE
                exportAnimationXmlHeader(animDebugXmlFileName,
                                         animName,
                                         float(hierarchy.numberOfFrames() - 1),
                                         nChannels);
#endif
                ...
                if (skeletalAnimation) {
                    ...
                    // Special requirement for the root node.  The node must be
                    // named for it to be exported.
                    if (!ExportSettings::instance().sceneRootAdded()) {
                    ...
                    exportAnimation(
                      animation,
                      hierarchy,
                      ExportSettings::instance().stripRefPrefix());

#if DEBUG_ANIMATION_FILE
                    DataSectionPtr spFileRoot =
                      BWResource::openSection(animDebugXmlFileName);
                    if (spFileRoot) {
                        DataSectionPtr spFile =
                          spFileRoot->openSection(animName);
...
BW::string VisualFileTranslator::lookupBackupFilename(
