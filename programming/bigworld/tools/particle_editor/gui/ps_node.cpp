  ...
  , m_readOnly(false)
{
    ...
    BW::string label =
      BWResource::getFilename(BWResource::removeExtension(filename))
        .to_string();
...
void MetaNode::EnsureLoaded() const
{
    ...
    if (m_metaParticleSystem == NULL) {
        ...
        if (!ok) {
            ...
            BW::wstring fullPath =
              BWResource::resolveFilenameW(m_directory + GetFilename());
...
/*virtual*/ TreeNode* MetaNode::Serialise(DataSectionPtr data, bool load)
{
    BW_GUARD;
...
/*virtual*/ void MetaNode::OnExpand()
{
    ...
    BW::vector<ParticleSystemPtr>& partileSyss =
      m_metaParticleSystem->systemSet();
    for (size_t i = 0; i < partileSyss.size(); ++i) {
        ...
        tree->SetCheck(*psnode,
                       psnode->getParticleSystem()->enabled() ? BST_CHECKED
                                                              : BST_UNCHECKED);
        psnode->addChildren();
    }
    m_createdChildren = true;
...
void PSNode::addChildren()
{
    ...
    newNode = tree->AddNode(
      new ActionNode(NULL,
                     LocaliseUTF8(L"PARTICLEEDITOR/GUI/SYSTEM_PROP"),
                     ActionNode::AT_SYS_PROP),
      this);
    tree->SetItemState(*newNode, 0, TVIS_STATEIMAGEMASK);
    // Add the renderer properties nodes:
    newNode = tree->AddNode(
      new ActionNode(NULL,
                     LocaliseUTF8(L"PARTICLEEDITOR/GUI/RENDERER_PROP"),
                     ActionNode::AT_REND_PROP),
      this);
    tree->SetItemState(*newNode, 0, TVIS_STATEIMAGEMASK);
    // Add the action nodes:
    BW::vector<ParticleSystemActionPtr>& actions =
      m_particleSystem->actionSet();
    ...
    for (size_t j = 0; j < actions.size(); ++j) {
        ...
        if (actionNode != NULL) {
            tree->SetCheck(*actionNode,
                           actionNode->getAction()->enabled() ? BST_CHECKED
                                                              : BST_UNCHECKED);
        }
    }
}
...
ActionNode::ActionNode(ParticleSystemActionPtr action,
