...
bool PSTree::Load(BW::string const& dir)
{
    ...
    if (dataSection) {
        ...
        for (DataSectionIterator it = dataSection->begin();
             it != dataSection->end();
             ++it) {
            ...
            if (MetaNode::IsMetaParticleFile(filename)) {
                if (addedFiles.find(filename) == addedFiles.end()) {
                    MetaNode* mn =
                      (MetaNode*)AddNode(new MetaNode(dir, filename));
...
BOOL PSTree::OnSelection(NMHDR* nmhdr, LRESULT* result)
{
    ...
    if (!InLargeOperation()) {
        // Make sure that if we selected a meta-particle system that it is
        // loaded.
        ...
        while (node != NULL) {
            ...
            if (metanode != 0) {
                ...
                if (metanode->GetMetaParticleSystem() == NULL) {
                    AfxMessageBox(Localise(L"RCS_IDS_BADPARTICLEFILE",
