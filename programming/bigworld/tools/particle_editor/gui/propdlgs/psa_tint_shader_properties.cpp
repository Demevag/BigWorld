...
void PsaTintShaderProperties::OnHScroll(UINT        nSBCode,
                                        UINT        nPos,
    ...
    if (pScrollBar == (CScrollBar*)&fogAmountSlider_) {
        ...
        if (pos != s_prevSliderPos) {
            MainFrame::instance()->PotentiallyDirty(
              ...
              LocaliseUTF8(
                L"PARTICLEEDITOR/GUI/PSA_TINTSHADER_PROPERTIES/SET_FOG_BLEND"),
...
void PsaTintShaderProperties::OnInitialUpdate()
{
    ...
    colorTimelineWnd_.Create(
      WS_CHILD | WS_VISIBLE, theRect, this, colorSchedule);
...
void PsaTintShaderProperties::SetParameters(SetOperation task)
{
    ...
    if (task == SET_PSA) {
        ...
        if (::IsWindow(colorTimelineWnd_.GetSafeHwnd())) {
            ...
            ColorScheduleItem                    newItem;
            TintShaderPSA::Tints::const_iterator it;
            TintShaderPSA::Tints::const_iterator itEnd =
              action()->tintSet().end();
...
void PsaTintShaderProperties::OnBnLoop()
{
    ...
    MainFrame::instance()->PotentiallyDirty(
      ...
      UndoRedoOp::AK_PARAMETER,
      LocaliseUTF8(
        L"PARTICLEEDITOR/GUI/PSA_TINT_SHADER_PROPERTIES/TOGGLE_TINT_CYCLE"));
    UpdateData();
}
...
/*afx_msg*/ LRESULT PsaTintShaderProperties::OnColorPickerSelUp(WPARAM mParam,
                                                                LPARAM lParam)
{
    BW_GUARD;
    ...
    if (filterChange_ == 0) {
        ...
        if (colorTimelineWnd_.itemSelected()) {
            MainFrame::instance()->PotentiallyDirty(
              true,
              UndoRedoOp::AK_PARAMETER,
              LocaliseUTF8(L"PARTICLEEDITOR/GUI/PSA_TINT_SHADER_PROPERTIES/"
                           L"CHANGE_COLOUR_TINT"));
            UpdateData();
        }
        --filterChange_;
    }
...
/*afx_msg*/ LRESULT PsaTintShaderProperties::OnUpdateTints(WPARAM mParam,
                                                           LPARAM lParam)
{
    BW_GUARD;
...
/*afx_msg*/ LRESULT PsaTintShaderProperties::OnAddedTint(WPARAM mParam,
                                                         LPARAM lParam)
{
    BW_GUARD;
...
/*afx_msg*/
LRESULT
PsaTintShaderProperties::OnNewTimelineSel(WPARAM mParam, LPARAM lParam)
{
    BW_GUARD;
...
/*afx_msg*/ LRESULT PsaTintShaderProperties::OnEditClrText(WPARAM wParam,
                                                           LPARAM /*lParam*/
)
{
    BW_GUARD;
