...
afx_msg LRESULT PsaFlareProperties::OnUpdatePsRenderProperties(WPARAM mParam,
                                                               LPARAM lParam)
{
    BW_GUARD;
...
void PsaFlareProperties::SetParameters(SetOperation task)
{
    ...
    if (task == SET_CONTROL) {
        ...
        if (longFilename.IsEmpty()) {
            // nothing set.. give an appropriate directory by looking where the
            // sun flare is
            longFilename = bw_utf8tow(Options::getOptionString(
                                        "resourceGlue/environment/sunFlareXML"))
                             .c_str();
        ...
        Options::setOptionString("defaults/flareXML",
                                 bw_wtoutf8(longFilename.GetString()));
        ...
        PopulateComboBoxWithFilenames(
          flareNameSelection_, relativeDirectory, validFlareFilename);
        finishPopulatingFlareNames();
        flareNameDirectoryEdit_.SetWindowText(
        ...
        if (selected != -1) {
            ...
            BW::string nflareName =
              bw_wtoutf8((dirName + L"/" + texName).GetString());
...
void PsaFlareProperties::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control(
      pDX, IDC_PSA_FLARE_FLARENAME_DIRECTORY_BTN, flareNameDirectoryBtn_);
    DDX_Control(
      pDX, IDC_PSA_FLARE_FLARENAME_DIRECTORY_EDIT, flareNameDirectoryEdit_);
...
ON_BN_CLICKED(IDC_PSA_FLARE_FLARENAME_DIRECTORY_BTN,
              OnBnClickedPsaFlareFlarenameDirectory)
END_MESSAGE_MAP()
...
void PsaFlareProperties::OnBnClickedPsaFlareButton()
{
    ...
    if (flareNameSelection_.GetCurSel() != -1) {
        ...
        if (curSelection ==
            Localise(
              L"PARTICLEEDITOR/GUI/PSA_FLARE_PROPERTIES/NO_FLARES_FOUND")) {
...
void PsaFlareProperties::OnBnClickedPsaFlareFlarenameDirectory()
{
    ...
    if (dlg.doBrowse(AfxGetApp()->m_pMainWnd)) {
        ...
        BW::wstring relativeDirectory =
          BWResource::dissolveFilenameW(dlg.userSelectedDirectory_.GetString());
        flareNameDirectoryEdit_.SetWindowText(relativeDirectory.c_str());

        PopulateComboBoxWithFilenames(flareNameSelection_,
...
void PsaFlareProperties::finishPopulatingFlareNames()
{
    ...
    if (flareNameSelection_.GetCount() == 0) {
        flareNameSelection_.AddString(
          Localise(L"PARTICLEEDITOR/GUI/PSA_FLARE_PROPERTIES/NO_FLARES_FOUND"));
    }
}
