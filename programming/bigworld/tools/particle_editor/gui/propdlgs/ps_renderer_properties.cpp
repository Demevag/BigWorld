...
namespace {
    ...
    void AddBitmapDrop(CWnd*                                        control,
                       ...
                       UalDropFunctor<PsRendererProperties>::Method method)
    {
        ...
        UalManager::instance().dropManager().add(
          new UalDropFunctor<PsRendererProperties>(
            control, "bmp", dialog, method));
        UalManager::instance().dropManager().add(
          new UalDropFunctor<PsRendererProperties>(
            control, "dds", dialog, method));
        UalManager::instance().dropManager().add(
          new UalDropFunctor<PsRendererProperties>(
            control, "tga", dialog, method));
        UalManager::instance().dropManager().add(
          new UalDropFunctor<PsRendererProperties>(
            control, "texanim", dialog, method));
    }

    void AddMeshDrop(CWnd*                                         control,
                     PsRendererProperties*                         dialog,
                     UalDropFunctor<PsRendererProperties>::Method  method,
                     UalDropFunctor<PsRendererProperties>::Method2 method2)
    {
        ...
        UalManager::instance().dropManager().add(
          new UalDropFunctor<PsRendererProperties>(
            control, "visual", dialog, method, false, method2));
    }

    void AddMFMDrop(CWnd*                                        control,
                    PsRendererProperties*                        dialog,
                    UalDropFunctor<PsRendererProperties>::Method method)
    {
        BW_GUARD;
        ...
        UalManager::instance().dropManager().add(
          new UalDropFunctor<PsRendererProperties>(
            control, "mfm", dialog, method));
    }

    bool validMeshFilename(const BW::string& filename,
                           const BW::string& fullname)
    {
        BW_GUARD;
    ...
    void populateComboBoxWithFilenames(CComboBox&        theBox,
                                       ...
                                       TestFn            test)
    {
        ...
        BWResource::instance().fileSystem()->readDirectory(directory,
                                                           relativeDirectory);

        if (!directory.empty()) {
            theBox.InitStorage((int)directory.size(),
                               64); // approx 64 characters per filename

            for (IFileSystem::Directory::iterator it = directory.begin();
                 it != directory.end();
                 ++it) {
    ...
    std::pair<UINT, UINT> spriteFX[] = {
        std::make_pair<UINT, UINT>(IDS_ADDITIVE,
                                   SpriteParticleRenderer::FX_ADDITIVE),
        std::make_pair<UINT, UINT>(IDS_ADDITIVE_ALPHA,
                                   SpriteParticleRenderer::FX_ADDITIVE_ALPHA),
        std::make_pair<UINT, UINT>(IDS_BLENDED,
                                   SpriteParticleRenderer::FX_BLENDED),
        std::make_pair<UINT, UINT>(IDS_BLENDED_COLOUR,
                                   SpriteParticleRenderer::FX_BLENDED_COLOUR),
        std::make_pair<UINT, UINT>(
          IDS_BLENDED_INVERSE_COLOUR,
          SpriteParticleRenderer::FX_BLENDED_INVERSE_COLOUR),
        std::make_pair<UINT, UINT>(IDS_SOLID, SpriteParticleRenderer::FX_SOLID),
        std::make_pair<UINT, UINT>(IDS_SHIMMER,
                                   SpriteParticleRenderer::FX_SHIMMER),
        std::make_pair<UINT, UINT>(IDS_SOURCE_ALPHA,
                                   SpriteParticleRenderer::FX_SOURCE_ALPHA)
    };
    size_t spriteFXSz = sizeof(spriteFX) / sizeof(std::pair<UINT, UINT>);

    std::pair<UINT, UINT> meshMaterialFX[] = {
        std::make_pair<UINT, UINT>(IDS_ADDITIVE,
                                   MeshParticleRenderer::FX_ADDITIVE),
        std::make_pair<UINT, UINT>(IDS_BLENDED,
                                   MeshParticleRenderer::FX_BLENDED),
        std::make_pair<UINT, UINT>(IDS_SOLID, MeshParticleRenderer::FX_SOLID)
    };
    size_t meshMaterialFXSz =
      sizeof(meshMaterialFX) / sizeof(std::pair<UINT, UINT>);
    ...
    std::pair<UINT, UINT> meshSort[] = {
        ...
        std::make_pair<UINT, UINT>(IDS_ACCURATE,
                                   MeshParticleRenderer::SORT_ACCURATE),
    };
    ...
    {
        return static_cast<float>(slider.GetRangeMin()) /
               FLOAT_SLIDER_MULTIPLIER;
    }

    float getSliderMaxValue(CSliderCtrl& slider)
    {
        return static_cast<float>(slider.GetRangeMax()) /
               FLOAT_SLIDER_MULTIPLIER;
    }
...
ON_EN_CHANGE(IDC_PS_RENDERER_SPRITE_SOFT_FALLOFF_POWER_EDIT,
             OnSoftFalloffPowerEdit)
ON_EN_CHANGE(IDC_PS_RENDERER_SPRITE_SOFT_DEPTH_OFFSET_EDIT,
             OnSoftDepthOffsetEdit)
...
ON_EN_CHANGE(IDC_PS_RENDERER_SPRITE_NEAR_FADE_FALLOFF_POWER_EDIT,
             OnNearFadeFalloffPowerEdit)
...
ON_BN_CLICKED(IDC_PS_RENDERER_SPRITE_TEXTURENAME_DIRECTORY_BTN,
              OnSpriteTexturenameDirectoryBtn)

ON_BN_CLICKED(IDC_PS_RENDERER_MESH_VISUALNAME_DIRECTORY_BTN,
              OnMeshVisualnameDirectoryBtn)
...
ON_BN_CLICKED(IDC_PS_RENDERER_VISUAL_VISUALNAME_DIRECTORY_BTN,
              OnVisualVisualnameDirectoryBtn)
ON_CBN_SELCHANGE(IDC_PS_RENDERER_VISUAL_VISUALNAME, OnGenericBtn)

ON_BN_CLICKED(IDC_PS_RENDERER_AMP_TEXTURENAME_DIRECTORY_BTN,
              OnAmpTexturenameDirectoryBtn)
ON_CBN_SELCHANGE(IDC_PS_RENDERER_AMP_TEXTURENAME, OnGenericBtn)

ON_BN_CLICKED(IDC_PS_RENDERER_TRAIL_TEXTURENAME_DIRECTORY_BTN,
              OnTrailTexturenameDirectoryBtn)
ON_CBN_SELCHANGE(IDC_PS_RENDERER_TRAIL_TEXTURENAME, OnGenericBtn)

ON_BN_CLICKED(IDC_PS_RENDERER_BLUR_TEXTURENAME_DIRECTORY_BTN,
              OnBlurTexturenameDirectoryBtn)
...
void PsRendererProperties::OnInitialUpdate()
{
    ...
    if (renderer()->nameID() == SpriteParticleRenderer::nameID_) {
        ...
        explicitOrientation_.SetCheck(explicitOrientation !=
                                      Vector3(0.f, 0.f, 0.f));
    }
    ...
    AddBitmapDrop(
      &textureName_, this, &PsRendererProperties::DropSpriteTexture);
    AddBitmapDrop(&textureNameDirectoryEdit_,
                  this,
                  &PsRendererProperties::DropSpriteTexture);
    AddMeshDrop(&meshName_,
                this,
                &PsRendererProperties::DropMesh,
                &PsRendererProperties::CanDropMesh);
    AddMeshDrop(&meshNameDirectoryEdit_,
                this,
                &PsRendererProperties::DropMesh,
                &PsRendererProperties::CanDropMesh);
    AddMeshDrop(&visualName_, this, &PsRendererProperties::DropVisual, NULL);
    AddMeshDrop(
      &visualNameDirectoryEdit_, this, &PsRendererProperties::DropVisual, NULL);
    AddBitmapDrop(
      &ampTextureName_, this, &PsRendererProperties::DropAmpTexture);
    AddBitmapDrop(&ampTextureNameDirectoryEdit_,
                  this,
                  &PsRendererProperties::DropAmpTexture);
    AddBitmapDrop(
      &trailTextureName_, this, &PsRendererProperties::DropTrailTexture);
    AddBitmapDrop(&trailTextureNameDirectoryEdit_,
                  this,
                  &PsRendererProperties::DropTrailTexture);
    AddBitmapDrop(
      &blurTextureName_, this, &PsRendererProperties::DropBlurTexture);
    AddBitmapDrop(&blurTextureNameDirectoryEdit_,
                  this,
...
void PsRendererProperties::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control(pDX,
                IDC_PS_RENDERER_SPRITE_TEXTURENAME_DIRECTORY_BTN,
                textureNameDirectoryBtn_);
    DDX_Control(pDX,
                IDC_PS_RENDERER_SPRITE_TEXTURENAME_DIRECTORY_EDIT,
    ...
    DDX_Control(pDX,
                IDC_PS_RENDERER_SPRITE_SOFT_DEPTH_RANGE_STATIC,
                softDepthRangeStatic_);
    DDX_Control(
      pDX, IDC_PS_RENDERER_SPRITE_SOFT_DEPTH_RANGE_EDIT, softDepthRangeEdit_);
    DDX_Control(pDX,
                IDC_PS_RENDERER_SPRITE_SOFT_DEPTH_RANGE_SLIDER,
                softDepthRangeSlider_);
    DDX_Control(pDX,
                IDC_PS_RENDERER_SPRITE_SOFT_FALLOFF_POWER_STATIC,
                softFalloffPowerStatic_);
    DDX_Control(pDX,
                IDC_PS_RENDERER_SPRITE_SOFT_FALLOFF_POWER_EDIT,
                softFalloffPowerEdit_);
    DDX_Control(pDX,
                IDC_PS_RENDERER_SPRITE_SOFT_FALLOFF_POWER_SLIDER,
                softFalloffPowerSlider_);
    DDX_Control(pDX,
                IDC_PS_RENDERER_SPRITE_SOFT_DEPTH_OFFSET_STATIC,
                softDepthOffsetStatic_);
    DDX_Control(
      pDX, IDC_PS_RENDERER_SPRITE_SOFT_DEPTH_OFFSET_EDIT, softDepthOffsetEdit_);
    DDX_Control(pDX,
                IDC_PS_RENDERER_SPRITE_SOFT_DEPTH_OFFSET_SLIDER,
                softDepthOffsetSlider_);

    DDX_Control(pDX,
                IDC_PS_RENDERER_SPRITE_NEAR_FADE_CUTOFF_STATIC,
                nearFadeCutoffStatic_);
    DDX_Control(
      pDX, IDC_PS_RENDERER_SPRITE_NEAR_FADE_CUTOFF_EDIT, nearFadeCutoffEdit_);
    DDX_Control(pDX,
                IDC_PS_RENDERER_SPRITE_NEAR_FADE_CUTOFF_SLIDER,
                nearFadeCutoffSlider_);
    DDX_Control(
      pDX, IDC_PS_RENDERER_SPRITE_NEAR_FADE_START_STATIC, nearFadeStartStatic_);
    DDX_Control(
      pDX, IDC_PS_RENDERER_SPRITE_NEAR_FADE_START_EDIT, nearFadeStartEdit_);
    DDX_Control(
      pDX, IDC_PS_RENDERER_SPRITE_NEAR_FADE_START_SLIDER, nearFadeStartSlider_);
    DDX_Control(pDX,
                IDC_PS_RENDERER_SPRITE_NEAR_FADE_FALLOFF_POWER_STATIC,
                nearFadeFalloffPowerStatic_);
    DDX_Control(pDX,
                IDC_PS_RENDERER_SPRITE_NEAR_FADE_FALLOFF_POWER_EDIT,
                nearFadeFalloffPowerEdit_);
    DDX_Control(pDX,
                IDC_PS_RENDERER_SPRITE_NEAR_FADE_FALLOFF_POWER_SLIDER,
                nearFadeFalloffPowerSlider_);

    DDX_Control(pDX, IDC_PS_RENDERER_MESH_VISUALNAME, meshName_);
    DDX_Control(pDX,
                IDC_PS_RENDERER_MESH_VISUALNAME_DIRECTORY_BTN,
                meshNameDirectoryBtn_);
    DDX_Control(pDX,
                IDC_PS_RENDERER_MESH_VISUALNAME_DIRECTORY_EDIT,
    ...
    DDX_Control(pDX,
                IDC_PS_RENDERER_VISUAL_VISUALNAME_DIRECTORY_BTN,
                visualNameDirectoryBtn_);
    DDX_Control(pDX,
                IDC_PS_RENDERER_VISUAL_VISUALNAME_DIRECTORY_EDIT,
                visualNameDirectoryEdit_);

    DDX_Control(pDX, IDC_PS_RENDERER_AMP_TEXTURENAME, ampTextureName_);
    DDX_Control(pDX,
                IDC_PS_RENDERER_AMP_TEXTURENAME_DIRECTORY_BTN,
                ampTextureNameDirectoryBtn_);
    DDX_Control(pDX,
                IDC_PS_RENDERER_AMP_TEXTURENAME_DIRECTORY_EDIT,
    ...
    DDX_Control(pDX,
                IDC_PS_RENDERER_TRAIL_TEXTURENAME_DIRECTORY_BTN,
                trailTextureNameDirectoryBtn_);
    DDX_Control(pDX,
                IDC_PS_RENDERER_TRAIL_TEXTURENAME_DIRECTORY_EDIT,
    ...
    DDX_Control(pDX,
                IDC_PS_RENDERER_BLUR_TEXTURENAME_DIRECTORY_BTN,
                blurTextureNameDirectoryBtn_);
    DDX_Control(pDX,
                IDC_PS_RENDERER_BLUR_TEXTURENAME_DIRECTORY_EDIT,
...
afx_msg LRESULT PsRendererProperties::OnUpdatePsRenderProperties(WPARAM mParam,
                                                                 LPARAM lParam)
{
    BW_GUARD;
...
void PsRendererProperties::SetParameters(SetOperation task)
{
    ...
    if (task == SET_CONTROL) {
        ...
        if (renderer()->nameID() == SpriteParticleRenderer::nameID_ ||
            ...
            SpriteParticleRenderer::MaterialFX fxSelected =
              spriteRenderer->materialFX();
            for (size_t i = 0; i < spriteFXSz; ++i) {
            ...
            CString longFilename =
              bw_utf8tow(spriteRenderer->textureName()).c_str();

            // remember for next time
            Options::setOptionString("defaults/renderer/spriteTexture",
                                     bw_wtoutf8(longFilename.GetString()));
            ...
            BW::string relativeDirectory =
              BWResource::dissolveFilename(bw_wtoutf8(directory.GetString()));
            populateComboBoxWithFilenames(
              textureName_, relativeDirectory, validTextureFilename);
            textureNameDirectoryEdit_.SetWindowText(
            ...
            if (textureName_.SelectString(-1, filename) == -1) {
                ...
                //  *.dds file since this will be the texture loaded by Moo
                //  anyway.
            ...
            if (!isPointSprite &&
                explicitOrientation != Vector3(0.f, 0.f, 0.f)) {
                addPositionGizmo();
                Matrix mat;
            ...
            // Soft particles
            setupPairedNumericEditSlider(softDepthRangeEdit_,
                                         softDepthRangeSlider_,
                                         SPRITE_SOFT_DEPTH_RANGE_MIN,
                                         SPRITE_SOFT_DEPTH_RANGE_MAX,
                                         spriteRenderer->softDepthRange());

            setupPairedNumericEditSlider(softFalloffPowerEdit_,
                                         softFalloffPowerSlider_,
                                         SPRITE_SOFT_FALLOFF_POWER_MIN,
                                         SPRITE_SOFT_FALLOFF_POWER_MAX,
                                         spriteRenderer->softFalloffPower());

            setupPairedNumericEditSlider(softDepthOffsetEdit_,
                                         softDepthOffsetSlider_,
                                         SPRITE_SOFT_DEPTH_OFFSET_MIN,
                                         SPRITE_SOFT_DEPTH_OFFSET_MAX,
                                         spriteRenderer->softDepthOffset());

            // Near fade
            setupPairedNumericEditSlider(nearFadeCutoffEdit_,
                                         nearFadeCutoffSlider_,
                                         SPRITE_NEAR_FADE_CUTOFF_MIN,
                                         SPRITE_NEAR_FADE_CUTOFF_MAX,
                                         spriteRenderer->nearFadeCutoff());

            setupPairedNumericEditSlider(nearFadeStartEdit_,
                                         nearFadeStartSlider_,
                                         SPRITE_NEAR_FADE_START_MIN,
                                         SPRITE_NEAR_FADE_START_MAX,
                                         spriteRenderer->nearFadeStart());

            setupPairedNumericEditSlider(
              nearFadeFalloffPowerEdit_,
              nearFadeFalloffPowerSlider_,
              SPRITE_NEAR_FADE_FALLOFF_POWER_MIN,
              SPRITE_NEAR_FADE_FALLOFF_POWER_MAX,
            ...
            // Remember for next time:
            Options::setOptionString("defaults/renderer/meshVisual",
                                     bw_wtoutf8(longFilename.GetString()));
            ...
            BW::string relativeDirectory =
              BWResource::dissolveFilename(bw_wtoutf8(directory.GetString()));
            populateComboBoxWithFilenames(
              meshName_, relativeDirectory, validMeshFilename);
            meshNameDirectoryEdit_.SetWindowText(
              bw_utf8tow(relativeDirectory).c_str());
            ...
            // Remember for next time:
            Options::setOptionString("defaults/renderer/visualVisual",
                                     bw_wtoutf8(longFilename.GetString()));
            ...
            BW::string relativeDirectory =
              BWResource::dissolveFilename(bw_wtoutf8(directory.GetString()));
            populateComboBoxWithFilenames(
              visualName_, relativeDirectory, validVisualFilename);
            visualNameDirectoryEdit_.SetWindowText(
              bw_utf8tow(relativeDirectory).c_str());
            visualName_.SelectString(-1, filename);
        } else if (renderer()->nameID() == AmpParticleRenderer::nameID_) {
            OnAmpBtn();

            AmpParticleRenderer* ampRenderer =
              static_cast<AmpParticleRenderer*>(&*renderer());

            CString longFilename =
              bw_utf8tow(ampRenderer->textureName()).c_str();

            // remember the selection
            Options::setOptionString("defaults/renderer/ampTexture",
                                     bw_wtoutf8(longFilename.GetString()));
            ...
            BW::string relativeDirectory =
              BWResource::dissolveFilename(bw_wtoutf8(directory.GetString()));
            populateComboBoxWithFilenames(
              ampTextureName_, relativeDirectory, validTextureFilename);
            ampTextureNameDirectoryEdit_.SetWindowText(
              bw_utf8tow(relativeDirectory).c_str());
            ...
            circular_.SetCheck(ampRenderer->circular() ? BST_CHECKED
                                                       : BST_UNCHECKED);
        } else if (renderer()->nameID() == TrailParticleRenderer::nameID_) {
            OnTrailBtn();

            TrailParticleRenderer* trailRenderer =
              static_cast<TrailParticleRenderer*>(&*renderer());

            CString longFilename =
              bw_utf8tow(trailRenderer->textureName()).c_str();

            // remember the selection
            Options::setOptionString("defaults/renderer/trailTexture",
                                     bw_wtoutf8(longFilename.GetString()));
            ...
            BW::string relativeDirectory =
              BWResource::dissolveFilename(bw_wtoutf8(directory.GetString()));
            populateComboBoxWithFilenames(
              trailTextureName_, relativeDirectory, validTextureFilename);
            trailTextureNameDirectoryEdit_.SetWindowText(
              bw_utf8tow(relativeDirectory).c_str());
            ...
            BlurParticleRenderer* blurRenderer =
              static_cast<BlurParticleRenderer*>(&*renderer());

            CString longFilename =
              bw_utf8tow(blurRenderer->textureName()).c_str();

            // remember the selection
            Options::setOptionString("defaults/renderer/blurTexture",
                                     bw_wtoutf8(longFilename.GetString()));
            ...
            BW::string relativeDirectory =
              BWResource::dissolveFilename(bw_wtoutf8(directory.GetString()));
            populateComboBoxWithFilenames(
              blurTextureName_, relativeDirectory, validTextureFilename);
            blurTextureNameDirectoryEdit_.SetWindowText(
              bw_utf8tow(relativeDirectory).c_str());
        ...
        if (renderer()->nameID() == SpriteParticleRenderer::nameID_ ||
            ...
            if (selected1 != -1) {
                SpriteParticleRenderer::MaterialFX newFX =
                  (SpriteParticleRenderer::MaterialFX)spriteFX[selected1]
                    .second;
            ...
            if (selected != -1) {
                ...
                spriteRenderer->textureName(
                  bw_wtoutf8((dirName + texName).GetString()));
            ...
            Vector3 explicitOrientation(explicitOrientX_.GetValue(),
            ...
            if (selected != -1) {
                ...
                visualFile = bw_wtoutf8((dirName + visName).GetString());
                {
                    ...
                    if (!MeshParticleRenderer::isSuitableVisual(visualFile)) {
                        ERROR_MSG(
                          LocaliseUTF8(
                            L"PARTICLEEDITOR/GUI/PS_RENDERER_PROPERTIES/"
                            L"SELECT_VISUAL_FAIL",
                            visualFile.c_str())
                            .c_str());
                ...
                // remember for next time
                Options::setOptionString(
                  "defaults/renderer/meshVisual",
                  bw_wtoutf8((dirName + visName).GetString()));
            }
            mpr->visual(visualFile);
            MeshParticleRenderer::MaterialFX fx =
              (MeshParticleRenderer::MaterialFX)
                meshMaterialFX[meshMaterialFX_.GetCurSel()]
                  .second;
            mpr->materialFX(fx);
            ...
            if (selected != -1) {
                ...
                dirName.TrimRight(L"/");
                dirName += L"/";
                ampRenderer->textureName(
                  bw_wtoutf8((dirName + texName).GetString()));

                // remember the selection
                Options::setOptionString(
                  "defaults/renderer/ampTexture",
                  bw_wtoutf8((dirName + texName).GetString()));
            }
            ...
            if (selected != -1) {
                ...
                dirName.TrimRight(L"/");
                dirName += L"/";
                trailRenderer->textureName(
                  bw_wtoutf8((dirName + texName).GetString()));

                // remember the selection
                Options::setOptionString(
                  "defaults/renderer/trailTexture",
                  bw_wtoutf8((dirName + texName).GetString()));
            }
            ...
            if (selected != -1) {
                ...
                dirName.TrimRight(L"/");
                dirName += L"/";
                blurRenderer->textureName(
                  bw_wtoutf8((dirName + texName).GetString()));

                // remember the selection
                Options::setOptionString(
                  "defaults/renderer/blurTexture",
                  bw_wtoutf8((dirName + texName).GetString()));
            }
...
void PsRendererProperties::SetSpriteEnabledState(bool option)
{
    ...
    if (option)
        rendererSprite_.CheckRadioButton(IDC_PS_RENDERER_SPRITE,
                                         IDC_PS_RENDERER_TRAIL,
                                         IDC_PS_RENDERER_SPRITE);
    ...
    const bool useSoftParticles =
      option && SpriteParticleRenderer::isSoftSupported() && !isPointSprite;
...
void PsRendererProperties::copyRendererSettings(ParticleSystemRendererPtr src,
                                                ParticleSystemRenderer*   dst)
{
    BW_GUARD;
...
void PsRendererProperties::OnSpriteBtn()
{
    ...
    if ((!isPointSprite &&
        ...
        MainFrame::instance()->PotentiallyDirty(
          ...
          UndoRedoOp::AK_PARAMETER,
          LocaliseUTF8(
            L"PARTICLEEDITOR/GUI/PS_RENDERER_PROPERTIES/CHANGE_SPRITE"));

        // check to see haven't already specified a renderer
        BW::string defaultTexture = Options::getOptionString(
          "defaults/renderer/spriteTexture", s_notFoundTexture);

        // Make sure the default texture exists, if not use the AutoConfig
        // default
        ...
        CollidePSA* col = static_cast<CollidePSA*>(
          &*MainFrame::instance()->GetCurrentParticleSystem()->pAction(
            PSA_COLLIDE_TYPE_ID));
        if (col)
        ...
        if (selected != -1) {
            ...
            BW::string notFoundMesh =
              s_notFoundMeshModel.value().substr(
                0, s_notFoundMeshModel.value().length() - 6) +
              ".visual";
        ...
        if (selected != -1) {
            ...
            BW::string notFoundVisual =
              s_notFoundModel.value().substr(
                0, s_notFoundModel.value().length() - 6) +
              ".visual";
...
void PsRendererProperties::OnAmpBtn()
{
    ...
    if (renderer()->nameID() != AmpParticleRenderer::nameID_) {
        ...
        if (selected != -1) {
            ...
            BW::string defaultTexture = Options::getOptionString(
              "defaults/renderer/ampTexture", s_notFoundTexture);
...
void PsRendererProperties::OnTrailBtn()
{
    ...
    if (renderer()->nameID() != TrailParticleRenderer::nameID_) {
        ...
        if (selected != -1) {
            ...
            // get default parameters from the psa
            BW::string defaultTexture = Options::getOptionString(
              "defaults/renderer/trailTexture", s_notFoundTexture);
...
void PsRendererProperties::OnBlurBtn()
{
    ...
    if (renderer()->nameID() != BlurParticleRenderer::nameID_) {
        ...
        if (selected != -1) {
            ...
            // get default parameters from the psa
            BW::string defaultTexture = Options::getOptionString(
              "defaults/renderer/blurTexture", s_notFoundTexture);
...
void PsRendererProperties::OnSpriteTexturenameDirectoryBtn()
{
    ...
    if (startDir != c_defaultDirectoryText)
        dlg.startDirectory_ =
          BWResource::resolveFilenameW(startDir.GetString()).c_str();

    if (dlg.doBrowse(AfxGetApp()->m_pMainWnd)) {
        dlg.userSelectedDirectory_ += "/";
        BW::string relativeDirectory = BWResource::dissolveFilename(
          bw_wtoutf8(dlg.userSelectedDirectory_.GetString()));
        textureNameDirectoryEdit_.SetWindowText(
          bw_utf8tow(relativeDirectory).c_str());
...
void PsRendererProperties::OnExplicitOrientationBtn()
{
    ...
    if (renderer()->nameID() == SpriteParticleRenderer::nameID_) {
        ...
        spriteRenderer->explicitOrientation(
          Vector3(explicitOrientX_.GetValue(),
                  explicitOrientY_.GetValue(),
                  explicitOrientZ_.GetValue()));
    }
...
void PsRendererProperties::OnHScroll(UINT        nSBCode,
                                     UINT        nPos,
                                     CScrollBar* pScrollBar)
{
    ...
    if (renderer()->nameID() == SpriteParticleRenderer::nameID_) {
        ...
        if (target == softDepthRangeSlider_.GetSafeHwnd()) {
            ...
            softFalloffPowerEdit_.SetValue(
              getSliderValue(softFalloffPowerSlider_));
            ...
            softDepthOffsetEdit_.SetValue(
              getSliderValue(softDepthOffsetSlider_));
            ...
            nearFadeFalloffPowerEdit_.SetValue(
              getSliderValue(nearFadeFalloffPowerSlider_));
            spriteRenderer->nearFadeFalloffPower(
              nearFadeFalloffPowerEdit_.GetValue());
        }
    }
}
...
bool PsRendererProperties::DropAmpTexture(UalItemInfo* ii)
{
    ...
    BW::string file        = BWResource::getFilename(textureFile).to_string();
    populateComboBoxWithFilenames(
      ampTextureName_, reldir, validTextureFilename);
...
bool PsRendererProperties::DropTrailTexture(UalItemInfo* ii)
{
    ...
    BW::string file        = BWResource::getFilename(textureFile).to_string();
    populateComboBoxWithFilenames(
      trailTextureName_, reldir, validTextureFilename);
...
bool PsRendererProperties::DropBlurTexture(UalItemInfo* ii)
{
    ...
    BW::string file        = BWResource::getFilename(textureFile).to_string();
    populateComboBoxWithFilenames(
      blurTextureName_, reldir, validTextureFilename);
...
RectInt PsRendererProperties::CanDropMesh(UalItemInfo* ii)
{
    ...
    if (validMeshFilename(file, meshFile)) {
        return RectInt(
          -1, -1, -1, -1); // drop permitted, used default processing
    } else {
...
void PsRendererProperties::addPositionGizmo()
{
    ...
    positionMatrixProxy_ = new VectorGeneratorMatrixProxy<PsRendererProperties>(
      this, &PsRendererProperties::position, &PsRendererProperties::position);
    editor->addProperty(
      new GenPositionProperty("vector", positionMatrixProxy_));
    positionGizmo_ = new VectorGizmo(
      MODIFIER_ALT, positionMatrixProxy_, 0xFFFFFF00, 0.015f, NULL, 0.1f);
