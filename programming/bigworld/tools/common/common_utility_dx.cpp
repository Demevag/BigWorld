 ...
 */
void __fastcall CommonUtility::loadDDSImage(AnsiString         filename,
                                            ...
                                            Graphics::TBitmap* alphaDest)
{
    ...
    HRESULT            hr = D3DXCreateTextureFromFileEx(Moo::rc().device(),
                                             filename.c_str(),
                                             rgbDest->Width,
                                             rgbDest->Height,
                                             1,
    ...
    if (FAILED(hr)) {
        CRITICAL_MSG("CommonUtility::loadDDSImage - Could not create texture "
                     "from file %s\n",
                     filename.c_str());
    } else {
        ...
        if (SUCCEEDED(hr)) {
            ...
            CRITICAL_MSG("CommonUtility::loadDDSImage - Could not lock texture "
                         "resource %s\n",
                         filename.c_str());
        }
