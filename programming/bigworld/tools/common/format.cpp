...
bool details::sformatNextObj(BW::string&       result,
                             BW::string const& str,
                             size_t&           pos,
                             size_t&           idx)
{
...
bool details::wformatNextObj(BW::wstring&       result,
                             ...
                             size_t&            idx)
{
    ...
    while (pos < str.length()) {
        ...
        // double "{", print a single "{":
        if (str[pos] == L'{' && pos + 1 < str.length() &&
            str[pos + 1] == L'{') {
            result += L'{';
            pos += 2;
        }
        // double "}", print a single "}":
        else if (str[pos] == L'}' && pos + 1 < str.length() &&
                 str[pos + 1] == L'}') {
            result += L'}';
            pos += 2;
        }
