...
namespace StringUtils {
    ...
    T vectorToStringT(const BW::vector<T>& vec, char separator = ';')
    {
        ...
        T ret;
        for (BW::vector<T>::const_iterator i = vec.begin(); i != vec.end();
             ++i) {
            if (ret.length() > 0)
                ret += separator;
            ret += (*i);
    ...
    bool matchExtensionT(const T1& fname, const T2& extensions)
    {
        ...
        for (T2::const_iterator i = extensions.begin(); i != extensions.end();
             ++i)
    ...
    bool matchSpecCaseSensitiveT(const T* fname, const T* spec)
    {
        ...
        while (*fname && *spec) {
            ...
            if (*spec == T('*')) {
                ...
                bool     match       = false;
                while (*fnameSubstr) {
                    // if the spec substring is shorter than the fname, no
                    // match!
    ...
    //------------------------------------------------------------------------------
    /**
     *	This function tests to see if a string 'fname' matches at least one of
     *the DOS-style wildcard strings in the 'specs' vector. In addition, if a
     *spec starts with '!', the test for it is done case-sensitive.
     */
    template <typename T1, typename T2>
    ...
    bool findInVectorT(const T1& str, const T2& vec)
    {
        ...
        for (BW::vector<BW::string>::const_iterator i = vec.begin();
             i != vec.end();
             ++i)
    ...
    void replace(T& str, const T& from, const T& to)
    {
        ...
        if (!from.empty()) {
            T newStr;
            while (const T::value_type* p =
                     strstrT(str.c_str(), from.c_str())) {
                newStr.insert(newStr.end(), str.c_str(), p);
                newStr += to;
                str.erase(str.begin(),
                          str.begin() + (p - str.c_str()) + from.size());
            }
            str = newStr + str;
        }
    }
    ...
    template <>
    inline UINT ClipboardFormatGetter<char>::get()
    {
        return CF_TEXT;
    }
    template <>
    inline UINT ClipboardFormatGetter<wchar_t>::get()
    {
    ...
    bool copyToClipboardT(const T& str)
    {
        ...
        if (::OpenClipboard(NULL)) {
            HGLOBAL data = ::GlobalAlloc(
              GMEM_MOVEABLE, (str.length() + 1) * sizeof(T::value_type));
            if (data != NULL && ::EmptyClipboard() != FALSE) {
                ...
                ::GlobalUnlock(data);
                ::SetClipboardData(ClipboardFormatGetter<T::value_type>::get(),
                                   data);
                ok = true;
            }
            ::CloseClipboard();
    ...
    bool canCopyFromClipboardT()
    {
        ...
        bool ok = false;
        if (::OpenClipboard(NULL)) {
            ok = ::IsClipboardFormatAvailable(
                   ClipboardFormatGetter<T::value_type>::get()) != FALSE;
            ::CloseClipboard();
        }
        return ok;
    }
    ...
    bool copyFromClipboardT(T& str)
    {
        ...
        if (::OpenClipboard(NULL)) {
            HGLOBAL data =
              ::GetClipboardData(ClipboardFormatGetter<T::value_type>::get());
            if (data != NULL) {
