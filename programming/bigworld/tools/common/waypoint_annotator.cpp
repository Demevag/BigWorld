 ...
 *	Constructor
 */
WaypointAnnotator::WaypointAnnotator(IWaypointView* pView, ChunkSpace* pSpace)
  : pView_(pView)
  , pSpace_(pSpace)
{
    BW_GUARD;
    ...
    Moo::rc().device()->CreateOffscreenPlainSurface(
      64, 64, D3DFMT_A8R8G8B8, D3DPOOL_SYSTEMMEM, &pSurface_, NULL);
    ...
    if (!pTarget_->create(64, 64)) {
        ...
        if (!pTarget_->push()) {
            ...
            if (!pRTSurface_.hasComObject()) {
                ERROR_MSG(
                  "WaypointAnnotator: Could not access render target.\n");
            }
        }
    }
}
 ...
 */
static int drawTarget(Moo::RenderTargetPtr pTarget,
                      int                  p,
                      int                  v,
                      int                  a,
                      uint32               flags)
{
    ...
    if (a == 0) {
        Moo::rc().device()->Clear(
          0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, 0x000000aa, 1, 0);
        ...
        if (xfont && FontManager::instance().begin(*xfont)) {
            ...
            xfont->drawConsoleString("Escape to not wait for space", 1, 27);
            xfont->drawConsoleString(
 ...
 *	input vertices.
 */
void unproject(const Matrix& view, uint num, void* array, uint stride)
{
    BW_GUARD;
    ...
    for (uint i = 0; i < num; ++i) {
        ...
        //		dprintf( "v (%f,%f,%f,1), tv (%f,%f,%f,%f), yoff %f, v + yoff
        //%f\n", 			v.x, v.y, v.z, tv.x, tv.y, tv.z, tv.w, yoff, v.y +
        //yoff );
...
float vertexHeight(ChunkSpace* pSpace, const Vector2& pt, float top)
{
    ...
    top += 0.5f;
    float drop = pSpace->collide(Vector3(pt.x, top, pt.y),
...
int WaypointAnnotator::annotate(int p, int v, bool slowly)
{
    ...
    for (int sp = 0; sp < pView_->getPolygonCount(); ++sp) {
        ...
        for (uint sv = 0; sv < spoly.size(); ++sv) {
            ...
            if (p1Sweep.isInFrontOf(bsvp) != ainp1s) {
                if (p1Sweep.intersect(sedge) <
                    1.414f) { // 1.414 is the dist to the end pt, i.e. p1
                    shore.push_back(sp);
                    break;
                }
            }
            if (p2Sweep.isInFrontOf(bsvp) != ainp2s) {
                if (p2Sweep.intersect(sedge) >
                    0.f) { // 0 is the start of the line, i.e. p2
                    shore.push_back(sp);
                    break;
                }
            }
    ...
    for (a = 0; a < 6; ++a) {
        ...
        Moo::rc().beginScene();
        Moo::rc().device()->Clear(
          0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, 0x00000000, 1, 0);
        Moo::rc().nextFrame();
        ...
        if ((a >> 1) == 0) {
            ...
            mview.lookAt(Vector3((frustrum[0].x + frustrum[1].x) * 0.5f,
                                 (blevel + tlevel) * 0.5f,
                                 (frustrum[0].z + frustrum[1].z) * 0.5f) -
                           frForward * 1.f / frForward.length(),
                         frForward,
                         Vector3(0.f, 1.f, 0.f));
            Moo::rc().device()->SetTransform(D3DTS_VIEW, &mview);

            mproj.orthogonalProjection(
              frRight.length(), tlevel - blevel, 1.f, 1.f + frForward.length());
            ...
            float frForwardLen =
              frForwardDir.dotProduct(Vector3(frustrum[2] - frustrum[0]));

            LineEq  frToBackLLine(Vector2(frustrum[0].x, frustrum[0].z),
                                 Vector2(frustrum[2].x, frustrum[2].z));
            LineEq  frToBackRLine(Vector2(frustrum[1].x, frustrum[1].z),
                                 Vector2(frustrum[3].x, frustrum[3].z));
            Vector2 viewPt2 =
              frToBackLLine.param(frToBackLLine.intersect(frToBackRLine));
            ...
            Vector3 frRightDir = frRightF;
            frRightDir.normalise();
            ...
            // mview[0][2], mview[0][3] ); dprintf( "        (%f,%f,%f,%f)\n",
            // mview[1][0], mview[1][1], mview[1][2], mview[1][3] ); dprintf( "
            // (%f,%f,%f,%f)\n", mview[2][0], mview[2][1], mview[2][2],
            // mview[2][3] ); dprintf( "        (%f,%f,%f,%f)\n", mview[3][0],
            // mview[3][1], mview[3][2], mview[3][3] );
        ...
        if (!renver.empty()) {
            ...
            if ((a >> 1) != 0)
                unproject(
                  mview, (uint)num, &renver.front(), sizeof(renver.front()));
        ...
        if (!triac.tris.empty()) {
            ...
            if ((a >> 1) != 0) {
                ...
                MF_ASSERT(num <= UINT_MAX);
                unproject(mview, (uint)num, &triac.tris[0], (uint)stride);
            }
        ...
        HRESULT hr =
          Moo::rc().device()->GetRenderTargetData(&*pRTSurface_, &*pSurface_);
        if (hr != D3D_OK) {
        ...
        hr = pSurface_->LockRect(
          &lockedRect, &copyRect, D3DLOCK_NOSYSLOCK | D3DLOCK_READONLY);
        if (hr != D3D_OK) {
