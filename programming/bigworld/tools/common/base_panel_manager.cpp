...
namespace {
    bool openHelpFile(const char* name, const wchar_t* file)
    {
        ...
        LocaliseUTF8(helpFile.container(),
                     file,
        ...
        intptr result = (intptr)ShellExecute(AfxGetMainWnd()->GetSafeHwnd(),
                                             L"open",
                                             helpFile->c_str(),
                                             NULL,
                                             NULL,
        ...
        if (result < 32) {
            Commentary::instance().addMsg(
              Localise(L"HELP/HELP_NOT_FOUND", helpFile->c_str(), name),
              Commentary::WARNING);
        }
...
PanelManagerFunctor::PanelManagerFunctor()
  : GUI::ActionMaker<PanelManagerFunctor>(
      "doToolsReferenceGuide",
      &PanelManagerFunctor::OnToolsReferenceGuide)
  , GUI::ActionMaker<PanelManagerFunctor, 1>(
      "doContentCreation",
      &PanelManagerFunctor::OnContentCreation)
{
}
...
{
    return openHelpFile("toolsReferenceGuide",
                        L"HELP/CONTENT_TOOLS_GUIDE_PATH");
}

//==============================================================================
bool PanelManagerFunctor::OnContentCreation(GUI::ItemPtr item)
{
    return openHelpFile("contentCreationManual",
                        L"HELP/CONTENT_CREATION_GUIDE_PATH");
}
