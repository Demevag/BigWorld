...
bool LightingPlan::VirtualLight::load(DataSectionPtr spLightSection)
{
    ...
    Vector3 orientation =
      spLightSection->readVector3("Orientation", Vector3(0, 0, 1));
...
void LightingPlan::VirtualLight::drawOmni()
{
    ...
    Geometrics::instance().wireSphere(
      transform_.applyToOrigin(), inner_, colour);

    Geometrics::instance().wireSphere(
      transform_.applyToOrigin(), outer_, colour);
}
...
bool LightingPlan::save(const BW::string& filename)
{
    ...
    DataSectionPtr spSection =
      BWResource::instance().rootSection()->openSection(filename, true);
    if (spSection) {
...
bool LightingPlan::VirtualLight::save(DataSectionPtr spLight)
{
    ...
    if (spSection) {
        ...
        spSection->writeVector3("Orientation",
                                transform_.applyToUnitAxisVector(2));
 ...
 *	TODO : remove this horrible
 *making-explicit-of-an-already-existing-implicit-thing
 */
    ...
    while (it != end) {
        ...
        if (l.enabled_) {
            switch (l.type_) {
                ...
                case LIGHT_OMNI: {
                    Moo::OmniLightPtr o =
                      new Moo::OmniLight(l.colour_,
                                         l.transform_.applyToOrigin(),
                                         l.inner_,
                                         l.outer_);
                    plc->addOmni(o);
                } break;
                case LIGHT_DIRECTIONAL: {
                    ...
                    Moo::DirectionalLightPtr d =
                      new Moo::DirectionalLight(l.colour_, dir);
                ...
                case LIGHT_SPOT: {
                    ...
                    Moo::SpotLightPtr s =
                      new Moo::SpotLight(l.colour_,
                                         l.transform_.applyToOrigin(),
                                         dir,
                                         l.inner_,
                                         l.outer_,
                                         cosf(DEG_TO_RAD(l.angle_)));
                    plc->addSpot(s);
                } break;
...
{
    // TODO
    MF_ASSERT("LightContainerLightingPlan::from( Moo::LightContainerPtr plc ) "
