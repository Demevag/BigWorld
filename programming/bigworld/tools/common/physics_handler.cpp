 ...
 */
void PhysicsHandler::preventCollision(const Vector3& oldPos,
                                      Vector3&       newPos,
                                      float          hopMax,
                                      float          modelWidth,
                                      float          modelHeight,
                                      float          modelDepth)
{
    ...
    // off.
    const Vector3 widthVector(
      modelWidth * flatDir.z, 0.f, -modelWidth * flatDir.x);
    ...
    WorldTriangle sweepTriangle(bottomLeft,
    ...
    if (debug_) {
        DEBUG_MSG(
          "preventCollision: maxAdvance %f, clear %f, flatDir(%f,%f,%f)\n",
          maxAdvance,
          clear,
          flatDir.x,
          flatDir.y,
...
void PhysicsHandler::applyGravity(Vector3& newPos,
                                  float    hopMax,
                                  float    modelWidth,
                                  float    modelDepth)
{
    ...
    Vector3 corners[4];
    corners[0] =
      newPos - flatDir * modelDepth * 0.5f - perpFlatDir * modelWidth * 0.5f;
    corners[1] =
      newPos - flatDir * modelDepth * 0.5f + perpFlatDir * modelWidth * 0.5f;
    corners[2] =
      newPos + flatDir * modelDepth * 0.5f - perpFlatDir * modelWidth * 0.5f;
...
void PhysicsHandler::adjustMove(const Vector3& src,
                                const Vector3& dst,
                                Vector3&       dst2)
{
    BW_GUARD;
    ...
    Vector3 newPos = src;
    while (Vector2(newPos.x - dst.x, newPos.z - dst.z).lengthSquared() >
           0.001f) {
        Vector3 halfPos = newPos;
        newPos          = dst;
        this->preventCollision(halfPos,
                               newPos,
                               scrambleHeight_,
                               modelWidth_,
                               modelHeight_,
        ...
        this->applyGravity(newPos, scrambleHeight_, modelWidth_, modelDepth_);
    ...
    dst2 = newPos;
    float y;
    if (!findDropPoint(dst2, y)) { // bottomless point, raise it to drop again
        if (!findDropPoint(Vector3(dst2.x, dst2.y + modelHeight_, dst2.z),
                           y)) { // cannot find bottom, mark it as invalid
            dst2.y = -10000.f;
            return;
        }
