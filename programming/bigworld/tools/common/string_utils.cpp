        ...
        if (str.substr(0, prefix.size()) == prefix) {
            ...
            if (pos < str.length()) {
                ...
                if (str.substr(pos, suffix.size()) == suffix) {
                    BW::string remainder =
                      str.substr(pos + suffix.size(), BW::string::npos);
                    str = LocaliseUTF8(
                      L"COMMON/STRING_UTILS/COPY_OF_N", num, remainder);
                    return;
                }
            }
        }
                             ...
                             StringUtils::IncrementStyle incrementStyle)
{
    ...
    if (incrementStyle == IS_EXPLORER) {
        ...
        if (str.substr(0, prefix.size()) == prefix) {
            BW::wstring remainder =
              str.substr(prefix.size(), BW::wstring::npos);
        ...
        if (str.substr(0, prefix.size()) == prefix) {
            ...
            if (pos < str.length()) {
                ...
                if (str.substr(pos, suffix.size()) == suffix) {
                    BW::wstring remainder =
                      str.substr(pos + suffix.size(), BW::wstring::npos);
                    str = Localise(
                      L"COMMON/STRING_UTILS/COPY_OF_N", num, remainder);
                    return;
                }
...
//------------------------------------------------------------------------------
bool StringUtils::makeValidFilenameT(BW::string& str,
                                     char        replaceChar,
                                     bool        allowSpaces)
{
    BW_GUARD;
...
//------------------------------------------------------------------------------
bool StringUtils::makeValidFilenameT(BW::wstring& str,
                                     wchar_t      replaceChar,
                                     bool         allowSpaces)
{
    BW_GUARD;
