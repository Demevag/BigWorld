...
//---------------------------------------------------------------------------
float __fastcall CommonUtility::strToFloatDef(AnsiString& str,
                                              float       defaultValue)
{
    float retVal;
...
TColor __fastcall CommonUtility::vectorToColor(Vector3 color)
{
    ...
    (unsigned long)tColor =
      (((unsigned long)(color.x) | ((unsigned long)(color.y) << 8) |
        ((unsigned long)(color.z) << 16)));
...
TColor __fastcall CommonUtility::vectorToTColor(Vector3 color)
{
    ...
    (unsigned long)tColor =
      (((unsigned long)(color.z) | ((unsigned long)(color.y) << 8) |
        ((unsigned long)(color.x) << 16)));
...
{
    return filename
      .SubString(filename.LastDelimiter("\\/") + 1, filename.Length())
      .LowerCase();
}
//---------------------------------------------------------------------------
                                                ...
                                                BW::vector<BW::string>& results)
{
    ...
    while (dotIndex > slashIndex && dotIndex > backSlashIndex && dotIndex > 0) {
        int length = f.Length();
        results.push_back(ANSI_TO_STD(
          f.SubString(dotIndex, length - dotIndex + 1).LowerCase()));
...
//---------------------------------------------------------------------------
AnsiString __fastcall CommonUtility::vectorToString(Vector3    v,
                                                    AnsiString seperator)
{
    AnsiString s;
...
//---------------------------------------------------------------------------
AnsiString __fastcall CommonUtility::vectorToString(Vector4    v,
                                                    AnsiString seperator,
                                                    bool       asFloats)
{
...
bool __fastcall CommonUtility::stringToVector(AnsiString s, Vector3& v)
{
    ...
    for (int i = 1; i <= s.Length(); i++) {
        // only allow valid float characters ( no scientific format support - (e
        // or E ) )
    ...
    while ((tokens < 3) && getToken(t, start, ",", token)) {
        if (token != "") {
            // convert the token to a float and put it in the right spot in the
            // vector
            try {
...
//---------------------------------------------------------------------------
bool __fastcall CommonUtility::getToken(AnsiString  s,
                                        int         start,
...
void __fastcall CommonUtility::doubleBufferControl(TWinControl* control)
{
    ...
    for (int i = 0; i < control->ControlCount; i++) {
        ...
        if (ctrl->ClassNameIs("TPanel") || ctrl->ClassNameIs("TToolbar") ||
        ...
        if (ctrl->ClassNameIs("TEdit") || ctrl->ClassNameIs("TUpDown") ||
...
//---------------------------------------------------------------------------
bool __fastcall CommonUtility::isMiddle(bool         bUseMiddle,
                                        TShiftState  Shift,
                                        TMouseButton Button)
{
    if (bUseMiddle)
...
// controls
/*void __fastcall CommonUtility::loadTGAImage( AnsiString filename,
Graphics::TBitmap* rgbDest, Graphics::TBitmap* alphaDest )
{
#pragma pack ( push, 1 )
    struct TGAHEADER {
...
#pragma pack ( pop )
    ...
    if ( ( f = bw_fopen( filename.c_str(), "rb" ) ) != NULL )
    {
        ...
        if ( strncmp( tgaFooter.signature, TGA_SIGNATURE, 16 ) == 0 )
        {
            ...
            // is it a valid file type - no color map, and image is uncompressed
true-color if ( tgaHeader.idLength == 0 && tgaHeader.colorMapType == 0 &&
                ...
                if ( tgaHeader.imgDepth == 24 || tgaHeader.imgDepth == 32 )
                {
                    ...
                    if ( pixels )
                    {
                        for( uint32 y = 0; y < tgaHeader.imgHeight; y++ )
                        {
                            ...
                            //now write the current scanline out to the image
control
                            //remembering to reverse the image in the
y-direction

                            //RGB
                            if ( is32Bit )
                            {
                                uint32 *rgbScanline = (uint32
*)rgbDest->ScanLine[tgaHeader.imgHeight-1-y]; memcpy( rgbScanline, pixels,
tgaHeader.imgWidth * 4 );
                            }
                            else
                            {
                                uint32 *rgbScanline = (uint32
*)rgbDest->ScanLine[tgaHeader.imgHeight-1-y]; memcpy( rgbScanline, pixels,
tgaHeader.imgWidth * 3 );
                            }

                            //Alpha
                            if ( is32Bit && alphaDest )
                            {
                                uint32 *alphaScanline = (uint32
*)alphaDest->ScanLine[tgaHeader.imgHeight-1-y];

                                for( uint32 x = 0; x < tgaHeader.imgWidth; x++)
                                {
                                    Byte a = (Byte)((pixels[x] & 0xff000000) >>
24); uint32 a32 = (a << 16) | (a << 8) | a; *alphaScanline++ = a32;
                                }
                            }
                        }
...
//---------------------------------------------------------------------------
int __fastcall CommonUtility::countFiles(AnsiString root,
                                         int        attr,
            ...
            if (pass) {
                if (sr.Name != "." && sr.Name != "..") {
                    ...
                    if (includeSubDirectories)
                        count += countFiles(
                          root + "/" + sr.Name, attr, includeSubDirectories);
                }
            }
        } while (FindNext(sr) == 0);
...
//---------------------------------------------------------------------------
void __fastcall CommonUtility::zoomExtents(float        fov,
                                           Matrix34&    camera,
                                           BoundingBox& bbox)
{
    float radius;
    ...
    camera.lookAt(
      relativePosition, objectCenter - relativePosition, Vector3(0, 1, 0));
}
//---------------------------------------------------------------------------
//
...
//---------------------------------------------------------------------------
AnsiString __fastcall CommonUtility::bestFitLabel(TCanvas*   Canvas,
                                                  AnsiString caption,
                                                  int        width)
{
    ...
    if (Canvas->TextWidth(label) > width) {
        for (int i = label.Length(); i; i--) {
            if (caption[i] == '\\' || caption[i] == '/') {
                ...
                AnsiString test =
                  "..." + caption.SubString(i, caption.Length());
...
//---------------------------------------------------------------------------
AnsiString __fastcall CommonUtility::convertPosixToWindows(
  const AnsiString& posixDirectory)
{
...
//---------------------------------------------------------------------------
AnsiString __fastcall CommonUtility::convertWindowsToPosix(
  const AnsiString& winDirectory)
{
 ...
 */
void __fastcall CommonUtility::loadAnyImage(AnsiString         filename,
                                            Graphics::TBitmap* rgbDest,
                                            Graphics::TBitmap* alphaDest)
{
    if (filename == "")
    ...
    // Removed this debug message since it was flooding the message window in
    // Modelviewer
    ...
    try {
        ...
        pTexture = Moo::rc().createTextureFromFileEx(filename.c_str(),
                                                     rgbDest->Width,
                                                     rgbDest->Height,
                                                     1,
                                                     0,
    ...
    if (!pTexture) {
        WARNING_MSG("CommonUtility::loadDDSImage - Could not create texture "
                    "from file %s\n",
                    filename.c_str());
    } else {
        try {
            // DEBUG_MSG( "CommonUtility::loadDDSImage - lockRect" );
            // now, lock the texture and copy the pixels to the destination
            // images.
            ...
            if (SUCCEEDED(hr)) {
                ...
                // Removed this debug message since it was flooding the message
                // window in Modelviewer
                ...
                WARNING_MSG("CommonUtility::loadDDSImage - Could not lock "
                            "texture resource %s\n",
                            filename.c_str());
            }
            ...
            WARNING_MSG("CommonUtility::loadAnyImage - Unknown error copying "
                        "texture memory\n");
            return;
        }
    }
