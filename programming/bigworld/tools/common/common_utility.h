...
class CommonUtility
{
  public:
    ...
    static AnsiString __fastcall vectorToString(Vector3    v,
                                                AnsiString seperator);
    static AnsiString __fastcall vectorToString(Vector4    v,
                                                AnsiString seperator,
                                                bool       asFloats = true);
    static bool __fastcall stringToVector(AnsiString s, Vector3& v);
    static bool __fastcall stringToVector(AnsiString s, Vector4& v);
    static bool __fastcall getToken(AnsiString  s,
                                    int         start,
    ...
    static bool __fastcall isMiddle(bool         bUseMiddle,
                                    TShiftState  Shift,
                                    TMouseButton Button);
    //	static void __fastcall loadTGAImage( AnsiString filename,
    //Graphics::TBitmap* rgbDest, Graphics::TBitmap* alphaDest );
    //    static void __fastcall loadDDSImage( AnsiString filename,
    //    Graphics::TBitmap* rgbDest, Graphics::TBitmap* alphaDest );
    static void __fastcall loadAnyImage(AnsiString         filename,
                                        Graphics::TBitmap* rgbDest,
                                        Graphics::TBitmap* alphaDest);
    // static void __fastcall loadTGAImage( char *filename, TImage &rgbDest,
    // TImage &alphaDest );
    static void __fastcall cameraLookAt(Matrix34& matrix, Vector3& location);
    static int __fastcall countFiles(AnsiString root,
                                     int        attr,
                                     bool       includeSubDirectories);
    static void __fastcall zoomExtents(float        fov,
                                       Matrix&      camera,
                                       BoundingBox& bbox);
    static AnsiString __fastcall memorySizeToStr(uint32 memorySizeInBytes);
    static AnsiString _fastcall bestFitLabel(TCanvas*   Canvas,
                                             AnsiString caption,
                                             int        width);
    static TMouseButton __fastcall shiftToMouse(TShiftState shift);
    static AnsiString __fastcall convertPosixToWindows(
      const AnsiString& posixDirectory);
    static AnsiString __fastcall convertWindowsToPosix(
