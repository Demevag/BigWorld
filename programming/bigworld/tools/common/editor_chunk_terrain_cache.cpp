    ...
    if (spread) {
        ...
        for (BW::set<Chunk*>::const_iterator it = chunkList.cbegin();
            ...
            manager->spreadInvalidate(
              *it, 1, 1, instance.index(), pChangedItem);
        }
                                     ...
                                     UnsavedList&           unsavedList)
{
    ...
    if (pTerrain()) {
        TerrainShadowProcessorPtr task =
          new TerrainShadowProcessor(this, manager, unsavedList);
...
bool EditorChunkTerrainCache::calcDone(ChunkProcessor*           processor,
                                       ChunkProcessorManager&    manager,
                                       bool                      result,
                                       const HorizonShadowImage& image,
                                       UnsavedList&              unsavedList)
{
    BW_GUARD;
    if (currentProcessor_ == processor) {
        if (pTerrain() && result) {
            ...
            for (int32 z = 0; z < (int32)hsm.height(); ++z) {
                const Terrain::HorizonShadowMap::PixelType* src =
                  image.getRow(z);
                Terrain::HorizonShadowMap::PixelType* dst =
                  hsm.image().getRow(z);
                Terrain::HorizonShadowMap::PixelType* end =
                  dst + hsm.image().width();
            ...
            SpaceEditor::instance().changedChunk(
              pTerrain()->chunk(), InvalidateFlags::FLAG_THUMBNAIL);
            manager.updateChunkDirtyStatus(pTerrain()->chunk());
...
bool EditorChunkTerrainCache::loadChunkForCalculate(
  ChunkProcessorManager* manager)
{
    BW_GUARD;
    BW::set<Chunk*> chunkList;
                               ...
                               bool             chunkDependencies)
{
    ...
    const uint32  numChunks =
      std::max((uint32)(std::ceil(distance / gridSize)), 1u);
    ...
    for (uint32 i = 0; i < numChunks; ++i, curPos += adjustPos) {
        ...
        BW::string chunkName = pMapping->outsideChunkIdentifier(gridX, gridZ);
        ...
        Chunk* pCurChunk =
          ChunkManager::instance().findChunkByName(chunkName, pMapping, true);
