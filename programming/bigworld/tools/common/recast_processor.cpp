...
RecastProcessor::RecastProcessor(EditorChunkNavmeshCacheBase* navmeshCache,
                                 Chunk*                       chunk,
...
bool RecastProcessor::processInBackground(ChunkProcessorManager& manager)
{
    ...
    for (size_t i = 0; i < girths_.size(); ++i) {
        ...
        RecastConfig config(visibilityBB,
                            girth.maxSlope(),
                            girth.maxClimb(),
                            girth.height(),
                            girth.radius(),
                            chunk_->space()->scaleNavigation(),
                            chunk_->space()->gridSize());

        if (!generator_.generate(collisionSceneProvider_,
                                 config,
                                 gridX,
                                 gridZ,
