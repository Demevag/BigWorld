  ...
  , maxZoomOut_(std::numeric_limits<float>::max())
{
    ...
    cameraMatrixFree_.lookAt(
      initialPosition, initialDirection, Vector3(0.f, 1.f, 0.f));
...
void ToolsCamera::update(float dTime /* = 0.f */,
                         bool  activeInputHandler /* = true */)
{
    BW_GUARD;
    ...
    if (zoomTime_ > 0.f) {
        ...
        Vector3 pos =
          (zoomDuration_ - zoomTime_) * centreFinish_ / zoomDuration_ +
...
void ToolsCamera::boundingBox(const BoundingBox& bb,
                              bool               updateCentre /* = true */)
{
    ...
    bb_ = bb;
    if (updateCentre) {
        Vector3 centre((bb_.maxBounds().x + bb_.minBounds().x) / 2.f,
...
void ToolsCamera::zoomToExtents(
  bool               animate,
  const BoundingBox& box /* = BoundingBox::s_insideOut_ */)
{
    BW_GUARD;

    if (box != BoundingBox::s_insideOut_)
        bb_ = box;
    ...
    centreFinish_ = Vector3((bb_.maxBounds().x + bb_.minBounds().x) / 2.f,
                            (bb_.maxBounds().y + bb_.minBounds().y) / 2.f,
                            (bb_.maxBounds().z + bb_.minBounds().z) / 2.f);
