...
/*static*/ void Utilities::moveAndSize(CWnd& widget,
                                       int   x,
                                       int   y,
                                       int   width,
                                       int   height)
...
/*static*/ int Utilities::moveAndStretchWidth(CWnd* parent,
                                              CWnd& widget,
                                              int   x,
                                              int   y,
                                              int   border)
...
/*static*/ void Utilities::moveAndStretch(CWnd* parent,
                                          CWnd& widget,
                                          int   x,
                                          int   y,
                                          int   border)
...
/*static*/ void Utilities::stretchToRight(CWnd* parent,
                                          CWnd& widget,
                                          int   pageWidth,
                                          int   border)
{
...
/*static*/ void Utilities::stretchToBottomRight(CWnd* parent,
                                                CWnd& widget,
                                                int   pageWidth,
                                                int   bx,
                                                int   pageHeight,
                                                int   by)
{
    ...
    widget.SetWindowPos(0,
                        rect.left,
                        rect.top,
                        pageWidth - rect.left - bx,
                        pageHeight - rect.top - by,
                        SWP_NOZORDER);
}

/*static*/ void Utilities::moveToRight(CWnd* parent,
                                       CWnd& widget,
                                       int   pageWidth,
                                       int   border)
{
    ...
    widget.SetWindowPos(
      0, pageWidth - width - border, rect.top, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
}

/*static*/ void Utilities::moveToBottom(CWnd* parent,
                                        CWnd& widget,
                                        int   pageHeight,
                                        int   border)
{
    ...
    widget.SetWindowPos(0,
                        rect.left,
                        pageHeight - height - border,
                        0,
                        0,
                        SWP_NOSIZE | SWP_NOZORDER);
}

/*static*/ void Utilities::moveToTopRight(CWnd* parent,
                                          CWnd& widget,
                                          int   pageWidth,
                                          int   bx,
                                          int   py)
...
/*static*/ void Utilities::moveToBottomLeft(CWnd* parent,
                                            CWnd& widget,
                                            int   pageHeight,
                                            int   by,
                                            int   px)
{
    ...
    int height = rect.bottom - rect.top;
    widget.SetWindowPos(
      0, px, pageHeight - height - by, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
}
...
/*static*/ void Utilities::centre(CWnd* parent, CWnd& widget, int pageWidth)
{
    ...
    int width = rect.right - rect.left;
    widget.SetWindowPos(
      0, pageWidth / 2 - width / 2, rect.top, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
}

/*static*/ void Utilities::fieldEnabledState(CEdit&             field,
                                             bool               enable,
...
/*static*/ BW::wstring Utilities::memorySizeToStr(uint32 memorySizeInBytes)
{
    ...
    switch (suffixIndex) {
        case 0:
            result = Localise(L"COMMON/UTILITIES/BYTES",
                              Formatter(memorySize, L"%.1f"));
            break;
        case 1:
            result = Localise(L"COMMON/UTILITIES/KILOBYTES",
                              Formatter(memorySize, L"%.1f"));
            break;
        case 2:
            result = Localise(L"COMMON/UTILITIES/MEGABYTES",
                              Formatter(memorySize, L"%.1f"));
            break;
        case 3:
            result = Localise(L"COMMON/UTILITIES/GIGABYTES",
                              Formatter(memorySize, L"%.1f"));
            break;
        case 4:
            result = Localise(L"COMMON/UTILITIES/TERABYTES",
                              Formatter(memorySize, L"%.1f"));
            break;
    }
 ...
 *  @param loadNeighIfNotReady    Attempt to load a chunk if any chunk is not
 *bound
 *  @return         a bool indicates whether the chunk is ready to process or
 *not
 */
bool Utilities::isOutdoorChunkReadyToProcess(Chunk* chunk,
                                             int    xNeighs,
                                             int    zNeighs,
                                             bool   loadChunkIfNotReady)
{
    ...
    if (chunk->completed()) {
        MF_VERIFY(
          mapping->gridFromChunkName(chunk->identifier(), gridX, gridZ));
        ...
        for (int z = gridZ - zNeighs; z <= gridZ + zNeighs; ++z) {
            for (int x = gridX - xNeighs; x <= gridX + xNeighs; ++x) {
                ...
                if (chunkName.empty()) { // the neighbour chunk is out of bound
                    continue;
                }

                if (Chunk* c = ChunkManager::instance().findChunkByName(
                      chunkName, mapping)) {
                    if (!c->completed()) {
                        return false;
                    }
 ...
 *not
 */
bool Utilities::isShellReadyToProcess(Chunk* chunk,
                                      int    portalDepth,
                                      bool   loadChunkIfNotReady)
{
    MF_ASSERT(!chunk->isOutsideChunk());
    ...
    while (!open.empty()) {
        ...
        for (ChunkBoundaries::iterator bit = chunk->joints().begin();
            ...
            for (ChunkBoundary::Portals::iterator ppit =
                ...
                if (pit->hasChunk()) { // it has a chunk but unbound
                    return false;
                }
            }
        }
 ...
 *not
 */
bool Utilities::isChunkReadyToProcess(Chunk* chunk,
                                      int    depth,
                                      bool   loadChunkIfNotReady)
{
    return chunk->isOutsideChunk()
             ? isOutdoorChunkReadyToProcess(
                 chunk, depth, depth, loadChunkIfNotReady)
 ...
 */
void Utilities::compose(Matrix&        m,
                        Vector3 const& scale,
                        Vector3 const& trans,
                        Vector3 const& rot)
{
 ...
 *  @param rot      Rotation (in degrees)
 */
void Utilities::compose(Vector4*       axis0,
                        Vector4*       axis1,
                        Vector4*       axis2,
                        Vector3 const& scale,
                        Vector3 const& trans,
 ...
 */
void Utilities::decompose(Matrix const& m,
                          Vector3&      scale,
                          Vector3&      trans,
                          Vector3&      rot)
{
 ...
 *  @param rot      Receives the resulting rotation angles (in degrees)
 */
void Utilities::decompose(Vector4 const& axis0,
                          Vector4 const& axis1,
                          Vector3&       scale,
                          Vector3&       trans,
                          Vector3&       rot)
