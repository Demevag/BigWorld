                    ...
                    if (vex->GetVertexType() == RIGID_NON_BLENDED_TYPE) {
                        ...
                        if (n == NULL) {
                            ...
                            MessageBox(GetForegroundWindow(),
                                       L"Physique could not retrieve a node "
                                       L"for one of the vertices.\n"
                                       L"The animation may not appear as it "
                                       L"should inside the BigWorld engine.",
                                       L"Animation Exporter",
                                       MB_OK | MB_ICONEXCLAMATION);

#else
                            MessageBox(GetForegroundWindow(),
                                       "Physique could not retrieve a node for "
                                       "one of the vertices.\n"
                                       "The animation may not appear as it "
                                       "should inside the BigWorld engine.",
                                       "Animation Exporter",
                                       MB_OK | MB_ICONEXCLAMATION);
#endif
                            return false;
        ...
        if (pSkin) {
            ...
            if (pContext) {
                ...
                for (int i = 0; i < nVertices; i++) {
                    ...
                    // If we have no nodes, the vertex is skinned to the parent
                    // node and we need the animation for this node as well
                    if (nNodes == 0) {
                        boneIndex(node);
                    }
                }
...
void VisualEnvelope::save(DataSectionPtr    spVisualSection,
...
#else
                    spNode->setString(
                      trimWhitespaces(boneNodes_[i]->GetName()));
#endif
                }
                //				spRenderSet->addString( "node", trimWhitespaces(
                // boneNodes_[i]->GetName() ) );
            }
