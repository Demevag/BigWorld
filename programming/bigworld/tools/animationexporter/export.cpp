...
void writeMFXChunkHeader(ChunkID identifier,
                         int     totalSize,
                         int     size,
                         FILE*   stream)
{
...
void MFExp::ExportGlobalInfo()
{
    ...
    // GetString(IDS_VERSIONSTRING), VERSION / 100.0f, today);

    /*	fprintf(pStream, "%s {\n", ID_SCENE);
        fprintf(pStream, "\t%s \"%s\"\n", ID_FILENAME,
       FixupName(ip->GetCurFileName())); fprintf(pStream, "\t%s %d\n",
       ID_FIRSTFRAME, range.Start() / GetTicksPerFrame()); fprintf(pStream,
...
void MFExp::ExportGeomObject(INode* node, int indentLevel)
{
    ...
    /*	fprintf(pStream, "%s\t%s %d\n", indent.data(), ID_PROP_MOTIONBLUR,
       node->MotBlur()); fprintf(pStream, "%s\t%s %d\n", indent.data(),
       ID_PROP_CASTSHADOW, node->CastShadows()); fprintf(pStream, "%s\t%s %d\n",
...
void MFExp::ExportShapeObject(INode* node, int indentLevel)
{
    ...
    //	fprintf(pStream,"%s\t%s %d\n", indent.data(), ID_SHAPE_LINECOUNT,
    // numLines);

    for (int poly = 0; poly < numLines; poly++) {
        //		fprintf(pStream,"%s\t%s %d {\n", indent.data(), ID_SHAPE_LINE,
        // poly);
...
void MFExp::DumpPoly(PolyLine* line, Matrix3 tm, int indentLevel)
{
    ...
    //	fprintf(pStream,"%s\t\t%s %d\n", indent.data(), ID_SHAPE_VERTEXCOUNT,
    // numVerts);
    ...
    for (int i = 0; i < numVerts; i++) {
        ...
        if (pt->flags & POLYPT_KNOT) {
            /*			fprintf(pStream,"%s\t\t%s\t%d\t%s\n", indent.data(),
               ID_SHAPE_VERTEX_KNOT, i, Format(tm * pt->p));*/
        } else {
            /*			fprintf(pStream,"%s\t\t%s\t%d\t%s\n", indent.data(),
               ID_SHAPE_VERTEX_INTERP, i, Format(tm * pt->p));*/
        }
    }
}
...
void MFExp::mfxCreateBones(INode* node)
{
    ...
    if ((identifier.substr(0, bonePre_.size()) == bonePre_) &&
        (!node->IsHidden())) {
        ObjectState os = node->EvalWorldState(GetStaticFrame());
        if (os.obj && (os.obj->SuperClassID() == GEOMOBJECT_CLASS_ID)) {
            BOOL       needDel;
            TriObject* tri =
              GetTriObjectFromNode(node, GetStaticFrame(), needDel);
            if (tri) {
                Mesh* mesh = &tri->mesh;
                if (mesh->getNumFaces()) {
                    BoneContainer bc;
                    ...
                    for (int i = 0; i < mesh->getNumFaces(); i++) {
                        ...
                        /*						bf.p1_.x = 0; bf.p1_.y = 0;
                           bf.p1_.z = 0; bf.p2_.x = 0; bf.p2_.y = 0;	bf.p2_.z
                           = 1; bf.p3_.x = 1; bf.p3_.y = 0;	bf.p3_.z = 0;*/
...
typedef struct EnvelopeContainer_
{
    ...
    intVec
      tIndices_; // these triangles have vertices that belong to different bones
} EnvelopeContainer;
...
void MFExp::mfxCreateEnvelopes(INode* node)
{
    ...
    if ((identifier.substr(0, envPre_.size()) == envPre_) &&
        (!node->IsHidden())) {
        ObjectState os = node->EvalWorldState(GetStaticFrame());
        if (os.obj && (os.obj->SuperClassID() == GEOMOBJECT_CLASS_ID)) {
            BOOL       needDel;
            ...
            if (triObj) {
                ...
                if (mesh->getNumVerts() && mesh->getNumFaces()) {
                    ...
                    for (int i = 0; i < mesh->getNumFaces(); i++) {
                        for (int j = 0; j < 3; j++) {
                            VertexContainer    vc(mesh->faces[i].v[j],
                                               hasTCoords ? mesh->tvFace[i].t[j]
                                                             : 0);
                            VCVector::iterator itVC =
                              std::find(vertices.begin(), vertices.end(), vc);
                    ...
                    for (i = 0; i < boneList_.size(); i++) {
                        boneList_[i].vIndices_.erase(
                          boneList_[i].vIndices_.begin(),
                          boneList_[i].vIndices_.end());
                    }
                    ...
                    for (i = 0; i < vertices.size(); i++) {
                        ...
                        float  distance =
                          getDistanceToBone(boneList_.front(), p);
                    ...
                    for (i = 0; i < boneList_.size(); i++) {
                        ...
                        if (bc.vIndices_.size() > 0) {
                            ...
                            for (intVec::iterator iit = bc.vIndices_.begin();
                                 iit != bc.vIndices_.end();
                                 iit++) {
                    ...
                    // since we have to transform the envelope to max-worldspace
                    // we have to determine if the polygons will be counter
                    // clockwise or clockwise, in max-space.
                    int iV1, iV2, iV3;
                    if (TMNegParity(m)) {
                        iV1 = 0;
                        iV2 = 1;
                    ...
                    for (i = 0; i < mesh->getNumFaces(); i++) {
                        VertexContainer vc(mesh->faces[i].v[iV1],
                                           hasTCoords ? mesh->tvFace[i].t[iV1]
                                                      : 0);
                        VertexContainer vc2(mesh->faces[i].v[iV2],
                                            hasTCoords ? mesh->tvFace[i].t[iV2]
                                                       : 0);
                        VertexContainer vc3(mesh->faces[i].v[iV3],
                                            hasTCoords ? mesh->tvFace[i].t[iV3]
                                                       : 0);
                        Triangle        tri;
                    ...
                    // add triangle indices to bones, or to the envelope,
                    ...
                    for (i = 0; i < env.triangles_.size(); i++) {
                        ...
                        for (BW::vector<EnvelopeBone>::iterator bIt =
                               env.bones_.begin();
                             bIt != env.bones_.end();
                             bIt++) {
                    ...
                    int verticesSize = CHUNK_HEADER_SIZE +
                                       (nVertices * 3 * 4); // size of vertices;
                    int tVerticesSize =
                      hasTCoords ? (CHUNK_HEADER_SIZE + (nVertices * 2 * 4))
                                 : 0; // size of texture coordinates;
                    int trianglesSize = CHUNK_HEADER_SIZE +
                                        (nFaces * 3 * 4); // size of triangles;

                    int totalSize =
                      size + verticesSize + tVerticesSize + trianglesSize;

                    writeMFXChunkHeader(
                      CHUNKID_ENVELOPE, totalSize, size, pStream);
                    ...
                    writeMFXChunkHeader(
                      CHUNKID_VERTEXLIST, verticesSize, verticesSize, pStream);
                    ...
                    if (hasTCoords) {
                        writeMFXChunkHeader(CHUNKID_TEXCOORDLIST,
                                            tVerticesSize,
                                            tVerticesSize,
                                            pStream);
                        for (i = 0; i < nVertices; i++) {
                    ...
                    writeMFXChunkHeader(CHUNKID_TRIANGLELIST,
                                        trianglesSize,
                                        trianglesSize,
                                        pStream);
                    for (i = 0; i < nFaces; i++) {
                    ...
                    for (BW::vector<EnvelopeBone>::iterator bIt =
                        ...
                        if (GetUseCharacterMode()) {
                            no.transform_ = Inverse(normaliseMatrix(
                              bIt->boneParent_->GetObjTMAfterWSM(
                                GetStaticFrame())));
                        } else {
                            no.transform_ =
                              Inverse(bIt->boneParent_->GetObjTMAfterWSM(
                                GetStaticFrame()));
                        }
...
float MFExp::getDistanceToBone(BoneContainer& bc, Point3& p)
{
    ...
    for (BW::vector<BoneFace>::iterator iBF = bc.faces_.begin();
         iBF != bc.faces_.end();
         iBF++) {
        // is our point within our three lines, if so, calculate the distance to
        // the plane
        if ((DotProd(iBF->l1N_, p) < iBF->l1D_) &&
            (DotProd(iBF->l2N_, p) < iBF->l2D_) &&
            (DotProd(iBF->l3N_, p) < iBF->l3D_)) {
            float len = fabs(DotProd(p, iBF->peqN_) - iBF->peqD_);

            if (ret > len)
                ret = len;
        } else {
            // is our point within the first line of the triangle, if it is,
            // calculate the distance to the line if not calculate the distance
            // to the first point in the line
            ...
            // is our point within the second line of the triangle, if it is,
            // calculate the distance to the line if not calculate the distance
            // to the first point in the line
            v  = iBF->p3_ - iBF->p2_;
            v2 = p - iBF->p2_;
            ...
            // is our point within the third line of the triangle, if it is,
            // calculate the distance to the line if not calculate the distance
            // to the first point in the line
            v  = iBF->p1_ - iBF->p3_;
            v2 = p - iBF->p3_;
...
void MFExp::mfxAnimationNodeEnum(INode* node)
{
    ...
    for (int i = 0; i < time; i++) {
        ...
        if (parent) {
            ...
            if (GetUseCharacterMode()) {
                parentMatrix =
                  Inverse(normaliseMatrix(parent->GetObjectTM(cf)));
            } else {
...
void MFExp::ExportLightObject(INode* node, int indentLevel)
{
    ...
    //	fprintf(pStream,"%s\t%s %s\n", indent.data(), ID_LIGHT_USELIGHT,
    // Format(light->GetUseLight()));

    /*	fprintf(pStream,"%s\t%s %s\n", indent.data(), ID_LIGHT_SPOTSHAPE,
            light->GetSpotShape() == RECT_LIGHT ? ID_LIGHT_SHAPE_RECT :
       ID_LIGHT_SHAPE_CIRC);*/

    /*	fprintf(pStream,"%s\t%s %s\n", indent.data(), ID_LIGHT_USEGLOBAL,
       Format(light->GetUseGlobal())); fprintf(pStream,"%s\t%s %s\n",
       indent.data(), ID_LIGHT_ABSMAPBIAS, Format(light->GetAbsMapBias()));
        fprintf(pStream,"%s\t%s %s\n", indent.data(), ID_LIGHT_OVERSHOOT,
       Format(light->GetOvershoot()));*/

    NameTab* el = light->GetExclList();
    if (el->Count()) {
        /*		fprintf(pStream,"%s\t%s {\n", indent.data(),
           ID_LIGHT_EXCLUSIONLIST); fprintf(pStream,"%s\t\t%s %s\n",
           indent.data(), ID_LIGHT_NUMEXCLUDED, Format(el->Count()));
                fprintf(pStream,"%s\t\t%s %s\n", indent.data(),
           ID_LIGHT_EXCLINCLUDE, Format(el->TestFlag(NT_INCLUDE)));
                fprintf(pStream,"%s\t\t%s %s\n", indent.data(),
           ID_LIGHT_EXCL_AFFECT_ILLUM, Format(el->TestFlag(NT_AFFECT_ILLUM)));
                fprintf(pStream,"%s\t\t%s %s\n", indent.data(),
           ID_LIGHT_EXCL_AFFECT_SHAD,
           Format(el->TestFlag(NT_AFFECT_SHADOWCAST)));*/
        for (int nameid = 0; nameid < el->Count(); nameid++) {
            //			fprintf(pStream,"%s\t\t%s \"%s\"\n", indent.data(),
            // ID_LIGHT_EXCLUDED, (*el)[nameid]);
        }
        //		fprintf(pStream,"%s\t}\n", indent.data());
    }
    ...
    if (!valid.InInterval(animRange) && GetIncludeCamLightAnim()) {
        ...
        while (1) {
            valid = FOREVER; // Extend the validity interval so the camera can
                             // shrink it.
            light->EvalLightState(t, valid, &ls);

            t = valid.Start() < animRange.Start() ? animRange.Start()
                                                  : valid.Start();
            ...
            if (valid.End() >= animRange.End()) {
                ...
                t = (valid.End() / GetTicksPerFrame() + GetMeshFrameStep()) *
                    GetTicksPerFrame();
            }
        }
...
void MFExp::ExportLightSettings(LightState* ls,
                                GenLight*   light,
                                TimeValue   t,
                                int         indentLevel)
{
    ...
    /*	fprintf(pStream, "%s\t\t%s %d\n",indent.data(), ID_TIMEVALUE, t);
        ...
        fprintf(pStream,"%s\t\t%s %s\n", indent.data(), ID_LIGHT_COLOR,
       Format(ls->color)); fprintf(pStream,"%s\t\t%s %s\n", indent.data(),
       ID_LIGHT_INTENS, Format(ls->intens)); fprintf(pStream,"%s\t\t%s %s\n",
       indent.data(), ID_LIGHT_ASPECT, Format(ls->aspect));*/

    if (ls->type != OMNI_LIGHT) {
        /*		fprintf(pStream,"%s\t\t%s %s\n", indent.data(),
           ID_LIGHT_HOTSPOT, Format(ls->hotsize)); fprintf(pStream,"%s\t\t%s
           %s\n", indent.data(), ID_LIGHT_FALLOFF, Format(ls->fallsize));*/
    }
    if (ls->type != DIR_LIGHT && ls->useAtten) {
        /*		fprintf(pStream,"%s\t\t%s %s\n", indent.data(),
           ID_LIGHT_ATTNSTART, Format(ls->attenStart));
                fprintf(pStream,"%s\t\t%s %s\n", indent.data(),
           ID_LIGHT_ATTNEND,	Format(ls->attenEnd));*/
    }

    /*	fprintf(pStream,"%s\t\t%s %s\n", indent.data(), ID_LIGHT_TDIST,
       Format(light->GetTDist(t, FOREVER))); fprintf(pStream,"%s\t\t%s %s\n",
       indent.data(), ID_LIGHT_MAPBIAS, Format(light->GetMapBias(t, FOREVER)));
        fprintf(pStream,"%s\t\t%s %s\n", indent.data(), ID_LIGHT_MAPRANGE,
       Format(light->GetMapRange(t, FOREVER))); fprintf(pStream,"%s\t\t%s %s\n",
       indent.data(), ID_LIGHT_MAPSIZE, Format(light->GetMapSize(t, FOREVER)));
        fprintf(pStream,"%s\t\t%s %s\n", indent.data(), ID_LIGHT_RAYBIAS,
       Format(light->GetRayBias(t, FOREVER)));
...
void MFExp::ExportCameraObject(INode* node, int indentLevel)
{
    ...
    INode* target = node->GetTarget();
    if (target) {
        //		fprintf(pStream,"%s\t%s %s\n", indent.data(), ID_CAMERA_TYPE,
        // ID_CAMERATYPE_TARGET);
    } else {
        //		fprintf(pStream,"%s\t%s %s\n", indent.data(), ID_CAMERA_TYPE,
        // ID_CAMERATYPE_FREE);
    }
...
void MFExp::ExportCameraSettings(CameraState*  cs,
                                 CameraObject* cam,
                                 TimeValue     t,
                                 int           indentLevel)
{
    ...
    if (cs->manualClip) {
        /*		fprintf(pStream,"%s\t\t%s %s\n", indent.data(),
           ID_CAMERA_HITHER, Format(cs->hither)); fprintf(pStream,"%s\t\t%s
           %s\n", indent.data(), ID_CAMERA_YON, Format(cs->yon));*/
    }

    /*	fprintf(pStream,"%s\t\t%s %s\n", indent.data(), ID_CAMERA_NEAR,
       Format(cs->nearRange)); fprintf(pStream,"%s\t\t%s %s\n", indent.data(),
       ID_CAMERA_FAR, Format(cs->farRange)); fprintf(pStream,"%s\t\t%s %s\n",
       indent.data(), ID_CAMERA_FOV, Format(cs->fov)); fprintf(pStream,"%s\t\t%s
...
void MFExp::ExportHelperObject(INode* node, int indentLevel)
{
    ...
    if (helperObj) {
        ...
        /*		fprintf(pStream,"%s\t%s %s\n", indent.data(),
           ID_BOUNDINGBOX_MIN, Format(bbox.pmin)); fprintf(pStream,"%s\t%s
...
void MFExp::ExportNodeHeader(INode* node, TCHAR* type, int indentLevel)
{
    ...
    if (parent && !parent->IsRootNode()) {
        //		fprintf(pStream,"%s\t%s \"%s\"\n", indent.data(),
        // ID_NODE_PARENT, FixupName(parent->GetName()));
    }
}
...
// for every specified frame. This can result in, dare I say, rather large
// files.
//
...
// user (or developer) to make sure that the data conforms with the target
// system.
...
void MFExp::ExportAnimMesh(INode* node, int indentLevel)
{
    ...
    if (!objRange.InInterval(animRange)) {
        ...
        while (1) {
            ...
            t        = objRange.Start() < animRange.Start() ? animRange.Start()
                                                            : objRange.Start();
            ...
            if (objRange.End() >= animRange.End()) {
                ...
                t = (objRange.End() / GetTicksPerFrame() + GetMeshFrameStep()) *
                    GetTicksPerFrame();
            }
        }
        //		fprintf(pStream,"%s\t}\n", indent.data());
...
void MFExp::mfxNodeExport(INode* node)
{
    ...
    if (parent) {
        ...
        if (GetUseCharacterMode()) {
            parentMatrix =
              Inverse(normaliseMatrix(parent->GetObjectTM(GetStaticFrame())));
        } else {
            parentMatrix = Inverse(parent->GetObjectTM(GetStaticFrame()));
        }

        nodeMatrix = nodeMatrix * parentMatrix;
    } else if (overrideRoot_) {
    ...
    size +=
      getMFXStrLen(nodeId) + getMFXStrLen(nodeId) + getMFXStrLen(parentId);
...
void MFExp::writeMFXNodeOverrides(void)
{
    for (int i = 0; i < nodeOverrides_.size(); i++) {
        ...
        size += getMFXStrLen(nodeOverrides_[i].nodeId_) +
                getMFXStrLen(nodeOverrides_[i].nodeId_) +
                getMFXStrLen(nodeOverrides_[i].parentId_);
...
void MFExp::writeMFXRootNodeOverrides(void)
{
    ...
    if (overrideRoot_) {
        ...
        size += getMFXStrLen(overrideRootIds_.back()) +
                getMFXStrLen(overrideRootIds_.back()) + getMFXStrLen(parent);
        ...
        for (int i = (overrideRootIds_.size() - 1); i > 0; i--) {
            ...
            size += getMFXStrLen(overrideRootIds_[i - 1]) +
                    getMFXStrLen(overrideRootIds_[i - 1]) +
                    getMFXStrLen(overrideRootIds_[i]);
...
void MFExp::ExportMesh(INode* node, TimeValue t, int indentLevel)
{
    ...
    BOOL hasTCoords = FALSE;
    if (GetIncludeTextureCoords() &&
        !CheckForAndExportFaceMap(nodeMtl, mesh, indentLevel + 1)) {
        int numTVx = mesh->getNumTVerts();
        if (numTVx && mesh->tvFace) {
            hasTCoords = TRUE;
        }
    ...
    if (!node->IsHidden() &&
        (GetIncludeEnvelopes()
           ? (meshId.substr(0, envPre_.size()) != envPre_) &&
               (meshId.substr(0, bonePre_.size()) != bonePre_)
           : TRUE)) {
        VCVector vcV;
        ...
        for (i = 0; i < nRFaces; i++) {
            ...
            if (Length(mesh->verts[mesh->faces[i].v[0]] -
                       mesh->verts[mesh->faces[i].v[1]]) == 0)
                continue;
            if (Length(mesh->verts[mesh->faces[i].v[1]] -
                       mesh->verts[mesh->faces[i].v[2]]) == 0)
                continue;
            if (Length(mesh->verts[mesh->faces[i].v[2]] -
                       mesh->verts[mesh->faces[i].v[0]]) == 0)
                continue;
            ...
            for (int j = 0; j < 3; j++) {
                VertexContainer    vc(mesh->faces[i].v[j],
                                   hasTCoords ? mesh->tvFace[i].t[j] : 0);
        ...
        if ((nVertices >= 3) && (nFaces >= 1)) {
            ...
            int totalSize = size;
            int verticesSize =
              CHUNK_HEADER_SIZE + (nVertices * 3 * 4); // size of vertices;
            int tVerticesSize = hasTCoords
                                  ? (CHUNK_HEADER_SIZE + (nVertices * 2 * 4))
                                  : 0; // size of texture coordinates;
            int trianglesSize =
              CHUNK_HEADER_SIZE + (nFaces * 3 * 4); // size of triangles;
            ...
            if (hasTCoords) {
                writeMFXChunkHeader(
                  CHUNKID_TEXCOORDLIST, tVerticesSize, tVerticesSize, pStream);
                for (i = 0; i < nVertices; i++) {
            ...
            writeMFXChunkHeader(
              CHUNKID_TRIANGLELIST, trianglesSize, trianglesSize, pStream);
            for (i = 0; i < nRFaces; i++) {
                if (Length(mesh->verts[mesh->faces[i].v[0]] -
                           mesh->verts[mesh->faces[i].v[1]]) == 0)
                    continue;
                ...
                for (int j = 0; j < 3; j++) {
                    ...
                    VertexContainer vc(mesh->faces[i].v[vi],
                                       hasTCoords ? (mesh->tvFace[i].t[vi])
                                                  : 0);
    ...
    /*	fprintf(pStream, "%s%s {\n",indent.data(),  ID_MESH);
        fprintf(pStream, "%s\t%s %d\n",indent.data(), ID_TIMEVALUE, t);
        fprintf(pStream, "%s\t%s %d\n",indent.data(), ID_MESH_NUMVERTEX,
       mesh->getNumVerts()); fprintf(pStream, "%s\t%s %d\n",indent.data(),
       ID_MESH_NUMFACES, mesh->getNumFaces());*/
    ...
    for (i = 0; i < mesh->getNumFaces(); i++) {
        /*		fprintf(pStream,"%s\t\t%s %4d:    A: %4d B: %4d C: %4d AB: %4d
           BC: %4d CA: %4d", indent.data(), ID_MESH_FACE, i,
                    mesh->faces[i].v[vx1],
                    mesh->faces[i].v[vx2],
                    mesh->faces[i].v[vx3],
        ...
        for (int j = 0; j < 32; j++) {
            if (mesh->faces[i].smGroup & (1 << j)) {
                if (mesh->faces[i].smGroup >> (j + 1)) {
                    //					fprintf(pStream,"%d,",j+1); // Add extra
                    // comma
                } else {
        ...
        // the material ID.
        /*		fprintf(pStream,"\t%s %d", ID_MESH_MTLID,
    ...
    if (GetIncludeTextureCoords() &&
        ...
        if (numTVx) {
            ...
            //			fprintf(pStream, "%s\t%s %d\n",indent.data(),
            // ID_MESH_NUMTVFACES, mesh->getNumFaces());
    ...
    if (GetIncludeVertexColors()) {
        ...
        if (numCVx) {
            ...
            /*			fprintf(pStream,"%s\t}\n",indent.data());
                        ...
                        fprintf(pStream, "%s\t%s %d\n",indent.data(),
               ID_MESH_NUMCVFACES, mesh->getNumFaces());
    ...
    if (GetIncludeNormals()) {
        ...
        // In MAX a vertex can have more than one normal (but doesn't always
        // have it). This is depending on the face you are accessing the vertex
        // through. To get all information we need to export all three vertex
        // normals for every face.
        for (i = 0; i < mesh->getNumFaces(); i++) {
            f  = &mesh->faces[i];
            fn = mesh->getFaceNormal(i);
            //			fprintf(pStream,"%s\t\t%s %d\t%s\n", indent.data(),
            // ID_MESH_FACENORMAL, i, Format(fn));

            vert = f->getVert(vx1);
            vn   = GetVertexNormal(mesh, i, mesh->getRVertPtr(vert));
            //			fprintf(pStream,"%s\t\t\t%s %d\t%s\n",indent.data(),
            // ID_MESH_VERTEXNORMAL, vert, Format(vn));
...
Point3 MFExp::GetVertexNormal(Mesh* mesh, int faceNo, RVertex* rv)
{
    ...
    // to a smoothing group
        ...
        if (numNormals == 1) {
            ...
            // and find the vertex with the same smoothing group as the current
            // face. You will find multiple normals in the ern member.
...
void MFExp::DumpJointParams(JointParams* joint, int indentLevel)
{
    ...
    /*	fprintf(pStream,"%s\t%s %s\n", indent.data(), ID_IKTYPE,   joint->Type()
       == JNT_POS ? ID_IKTYPEPOS : ID_IKTYPEROT); fprintf(pStream,"%s\t%s %d\n",
       indent.data(), ID_IKDOF,    joint->dofs);

        fprintf(pStream,"%s\t%s %d\n", indent.data(), ID_IKXACTIVE, joint->flags
       & JNT_XACTIVE  ? 1 : 0); fprintf(pStream,"%s\t%s %d\n", indent.data(),
       ID_IKYACTIVE,  joint->flags & JNT_YACTIVE  ? 1 : 0);
        fprintf(pStream,"%s\t%s %d\n", indent.data(), ID_IKZACTIVE, joint->flags
       & JNT_ZACTIVE  ? 1 : 0);

        fprintf(pStream,"%s\t%s %d\n", indent.data(), ID_IKXLIMITED,
       joint->flags & JNT_XLIMITED ? 1 : 0); fprintf(pStream,"%s\t%s %d\n",
       indent.data(), ID_IKYLIMITED, joint->flags & JNT_YLIMITED ? 1 : 0);
        fprintf(pStream,"%s\t%s %d\n", indent.data(), ID_IKZLIMITED,
       joint->flags & JNT_ZLIMITED ? 1 : 0);

        fprintf(pStream,"%s\t%s %d\n", indent.data(), ID_IKXEASE, joint->flags &
       JNT_XEASE    ? 1 : 0); fprintf(pStream,"%s\t%s %d\n", indent.data(),
       ID_IKYEASE,    joint->flags & JNT_YEASE    ? 1 : 0);
        fprintf(pStream,"%s\t%s %d\n", indent.data(), ID_IKZEASE, joint->flags &
       JNT_ZEASE    ? 1 : 0);

        fprintf(pStream,"%s\t%s %d\n", indent.data(), ID_IKLIMITEXACT,
       joint->flags & JNT_LIMITEXACT ? 1 : 0);*/

    for (int i = 0; i < joint->dofs; i++) {
        //		fprintf(pStream,"%s\t%s %d %s %s %s\n", indent.data(),
        // ID_IKJOINTINFO, i, Format(joint->min[i]), Format(joint->max[i]),
        // Format(joint->damping[i]));
    }
}
...
void MFExp::ExportMaterial(INode* node, int indentLevel)
{
    ...
    if (mtl) {
        ...
        if (mtlID >= 0) {
            //			fprintf(pStream,"%s\t%s %d\n", indent.data(),
            // ID_MATERIAL_REF, mtlID);
        }
    } else {
        DWORD c = node->GetWireColor();
        /*		fprintf(pStream,"%s\t%s %s\n", indent.data(), ID_WIRECOLOR,
                    Format(Color(GetRValue(c)/255.0f, GetGValue(c)/255.0f,
           GetBValue(c)/255.0f)));*/
    }
}
...
void MFExp::DumpMaterial(Mtl* mtl, int mtlID, int subNo, int indentLevel)
{
    ...
    if (subNo == -1) {
        // Top level material
        //		fprintf(pStream,"%s%s %d {\n",indent.data(), ID_MATERIAL,
        // mtlID);
    } else {
        //		fprintf(pStream,"%s%s %d {\n",indent.data(), ID_SUBMATERIAL,
        // subNo);
    }
    /*	fprintf(pStream,"%s\t%s \"%s\"\n",indent.data(), ID_MATNAME,
       FixupName(mtl->GetName())); fprintf(pStream,"%s\t%s
    ...
    if (mtl->ClassID() == Class_ID(DMTL_CLASS_ID, 0)) {
        ...
        for (i = 0; i < mtl->NumSubTexmaps(); i++) {
            ...
            float   amt    = 1.0f;
            if (subTex) {
                // If it is a standard material we can see if the map is
                // enabled.
                if (mtl->ClassID() == Class_ID(DMTL_CLASS_ID, 0)) {
        ...
        if (nTextures) {
            writeMFXChunkHeader(
              CHUNKID_TEXTURE, textureSize, textureSize, pStream);
            writeMFXStr(texName, pStream);
        }

        /*		fprintf(pStream,"%s\t%s %s\n",indent.data(), ID_AMBIENT,
           Format(std->GetAmbient(t))); fprintf(pStream,"%s\t%s
           %s\n",indent.data(), ID_DIFFUSE, Format(std->GetDiffuse(t)));
                fprintf(pStream,"%s\t%s %s\n",indent.data(), ID_SPECULAR,
           Format(std->GetSpecular(t))); fprintf(pStream,"%s\t%s
           %s\n",indent.data(), ID_SHINE, Format(std->GetShininess(t)));
                fprintf(pStream,"%s\t%s %s\n",indent.data(), ID_SHINE_STRENGTH,
           Format(std->GetShinStr(t))); fprintf(pStream,"%s\t%s
           %s\n",indent.data(), ID_TRANSPARENCY, Format(std->GetXParency(t)));
                fprintf(pStream,"%s\t%s %s\n",indent.data(), ID_WIRESIZE,
        ...
        /*		fprintf(pStream,"%s\t%s %s\n", indent.data(), ID_XP_FALLOFF,
           Format(std->GetOpacFalloff(t))); fprintf(pStream,"%s\t%s %s\n",
        ...
        //		fprintf(pStream,"%s\t%s %s\n", indent.data(), ID_FALLOFF,
        // std->GetFalloffOut() ? ID_FALLOFF_OUT : ID_FALLOFF_IN);
        ...
        // Since the materials are procedural there is no real "diffuse" color
        // for a MAX material but we can at least take the interactive color.

        /*		fprintf(pStream,"%s\t%s %s\n",indent.data(), ID_AMBIENT,
           Format(mtl->GetAmbient())); fprintf(pStream,"%s\t%s
           %s\n",indent.data(), ID_DIFFUSE, Format(mtl->GetDiffuse()));
                fprintf(pStream,"%s\t%s %s\n",indent.data(), ID_SPECULAR,
           Format(mtl->GetSpecular())); fprintf(pStream,"%s\t%s
           %s\n",indent.data(), ID_SHINE, Format(mtl->GetShininess()));
                fprintf(pStream,"%s\t%s %s\n",indent.data(), ID_SHINE_STRENGTH,
           Format(mtl->GetShinStr())); fprintf(pStream,"%s\t%s
...
TCHAR* MFExp::GetMapID(Class_ID cid, int subNo)
{
    ...
    if (cid == Class_ID(0, 0)) {
        ...
        switch (subNo) {
            case ID_AM:
                strcpy(buf, ID_MAP_AMBIENT);
                break;
            case ID_DI:
                strcpy(buf, ID_MAP_DIFFUSE);
                break;
            case ID_SP:
                strcpy(buf, ID_MAP_SPECULAR);
                break;
            case ID_SH:
                strcpy(buf, ID_MAP_SHINE);
                break;
            case ID_SS:
                strcpy(buf, ID_MAP_SHINESTRENGTH);
                break;
            case ID_SI:
                strcpy(buf, ID_MAP_SELFILLUM);
                break;
            case ID_OP:
                strcpy(buf, ID_MAP_OPACITY);
                break;
            case ID_FI:
                strcpy(buf, ID_MAP_FILTERCOLOR);
                break;
            case ID_BU:
                strcpy(buf, ID_MAP_BUMP);
                break;
            case ID_RL:
                strcpy(buf, ID_MAP_REFLECT);
                break;
            case ID_RR:
                strcpy(buf, ID_MAP_REFRACT);
                break;
        }
    } else {
...
void MFExp::DumpTexture(Texmap*  tex,
                        Class_ID cid,
                        int      subNo,
                        float    amt,
                        int      indentLevel)
{
    ...
    /*	fprintf(pStream,"%s%s {\n", indent.data(), GetMapID(cid, subNo));
        ...
        fprintf(pStream,"%s\t%s \"%s\"\n", indent.data(), ID_TEXNAME,
       FixupName(tex->GetName())); fprintf(pStream,"%s\t%s \"%s\"\n",
    ...
    for (int i = 0; i < tex->NumSubTexmaps(); i++) {
        DumpTexture(
          tex->GetSubTexmap(i), tex->ClassID(), i, 1.0f, indentLevel + 1);
    }
...
void MFExp::DumpUVGen(StdUVGen* uvGen, int indentLevel)
{
    ...
    /*	fprintf(pStream,"%s%s %s\n", indent.data(), ID_U_OFFSET,
       Format(uvGen->GetUOffs(t))); fprintf(pStream,"%s%s %s\n", indent.data(),
       ID_V_OFFSET, Format(uvGen->GetVOffs(t))); fprintf(pStream,"%s%s %s\n",
       indent.data(), ID_U_TILING, Format(uvGen->GetUScl(t)));
        fprintf(pStream,"%s%s %s\n", indent.data(), ID_V_TILING,
       Format(uvGen->GetVScl(t))); fprintf(pStream,"%s%s %s\n", indent.data(),
       ID_ANGLE, Format(uvGen->GetAng(t))); fprintf(pStream,"%s%s %s\n",
       indent.data(), ID_BLUR, Format(uvGen->GetBlur(t))); fprintf(pStream,"%s%s
       %s\n", indent.data(), ID_BLUR_OFFSET, Format(uvGen->GetBlurOffs(t)));
        fprintf(pStream,"%s%s %s\n", indent.data(), ID_NOISE_AMT,
       Format(uvGen->GetNoiseAmt(t))); fprintf(pStream,"%s%s %s\n",
       indent.data(), ID_NOISE_SIZE, Format(uvGen->GetNoiseSize(t)));
        fprintf(pStream,"%s%s %d\n", indent.data(), ID_NOISE_LEVEL,
       uvGen->GetNoiseLev(t)); fprintf(pStream,"%s%s %s\n", indent.data(),
       ID_NOISE_PHASE, Format(uvGen->GetNoisePhs(t)));*/
}
...
BOOL MFExp::CheckForAndExportFaceMap(Mtl* mtl, Mesh* mesh, int indentLevel)
{
    ...
    for (int i = 0; i < mesh->getNumFaces(); i++) {
        ...
        make_face_uv(f, tv);
        /*		fprintf(pStream, "%s\t%s %d {\n", indent.data(),
           ID_MESH_FACEMAP, i); fprintf(pStream, "%s\t\t%s\t%d\t%d\t%d\n",
           indent.data(), ID_MESH_FACEVERT, (int)tv[0].x, (int)tv[0].y,
           (int)tv[0].z); fprintf(pStream, "%s\t\t%s\t%d\t%d\t%d\n",
           indent.data(), ID_MESH_FACEVERT, (int)tv[1].x, (int)tv[1].y,
           (int)tv[1].z); fprintf(pStream, "%s\t\t%s\t%d\t%d\t%d\n",
           indent.data(), ID_MESH_FACEVERT, (int)tv[2].x, (int)tv[2].y,
           (int)tv[2].z); fprintf(pStream, "%s\t}\n", indent.data());*/
    }
    //	fprintf(pStream, "%s}\n", indent.data());
...
// factor so when calculating the normal we should take the vertices counter
// clockwise. If we don't compensate for this the objects will be 'inverted'.
...
void MFExp::DumpMatrix3(Matrix3* m, int indentLevel)
{
    ...
    /*	fprintf(pStream,"%s%s %s\n", indent.data(), ID_TM_POS, Format(ap.t));
        fprintf(pStream,"%s%s %s\n", indent.data(), ID_TM_ROTAXIS,
       Format(rotAxis)); fprintf(pStream,"%s%s %s\n", indent.data(),
       ID_TM_ROTANGLE, Format(rotAngle)); fprintf(pStream,"%s%s %s\n",
       indent.data(), ID_TM_SCALE, Format(ap.k)); fprintf(pStream,"%s%s %s\n",
       indent.data(), ID_TM_SCALEAXIS, Format(scaleAxis)); fprintf(pStream,"%s%s
       %s\n", indent.data(), ID_TM_SCALEAXISANG, Format(scaleAxAngle));*/
}

// From the SDK
// How to calculate UV's for face mapped materials.
static Point3 basic_tva[3] = { Point3(0.0, 0.0, 0.0),
                               Point3(1.0, 0.0, 0.0),
                               Point3(1.0, 1.0, 0.0) };
static Point3 basic_tvb[3] = { Point3(1.0, 1.0, 0.0),
                               Point3(0.0, 1.0, 0.0),
                               Point3(0.0, 0.0, 0.0) };
