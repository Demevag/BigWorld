...
namespace OfflineProcessor {
    ...
    void CommandUpgradeBSP::showDetailedHelp() const
    {
        ...
        std::cout
          << "Loads and saves bsp data upgrading it to the latest format:"
          << std::endl;
        std::cout << "[file] name can be a folder name" << std::endl;

        std::cout << "Options:" << std::endl;
        std::cout << "\t-recursive\t\tProcess all files recursively starting "
    ...
    void CommandUpgradeBSP::showShortHelp() const
    {
        std::cout << strCommand();
        std::cout
          << "\t\tUpgrade bsp in .primitives and .bsp2 to the latest format.";
    }

    /**
     *	Upgrade bsp in .primitives file
     */
    bool CommandUpgradeBSP::doFormatUpgradeBSPInPrimitives(
      const BW::string& primName)
    {
        DataSectionPtr primDS = BWResource::openSection(primName);
        if (!primDS) {
            std::cerr << "ERROR: Unable to open primitives " << primName
                      << std::endl;
            return false;
        }
        ...
        if (!bp) {
            // primitive might not contain bsp section, this just means there is
            // no bsp in there return true as this is not an error
            return true;
        }
        ...
        if (!bRes) {
            std::cerr << "ERROR: Failed ot write regenerated bsp for "
                      << primName << std::endl;
            return false;
        }
        bRes = primDS->save(primName);
        if (!bRes) {
            std::cerr << "ERROR: Failed ot save regenerated bsp for "
                      << primName << std::endl;
            return false;
        }
     ...
     */
    bool CommandUpgradeBSP::doFormatUpgradeStandaloneBSPFile(
      const BW::string& bspName)
    {
        DataSectionPtr bspSect = BWResource::openSection(bspName);
        if (!bspSect.exists()) {
            std::cout << "ERROR: Could not load BSP section from " << bspName
                      << std::endl;
            return false;
        }
        ...
        if (!bp.exists()) {
            std::cout << "ERROR: Could not load BSP data from file " << bspName
                      << std::endl;
            return false;
        }
     ...
     */
    BatchCommand::FileProcessResult CommandUpgradeBSP::processFile(
      ...
      const BW::string& outFileName)
    {
        ...
        if (ext == "primitives") {
            return doFormatUpgradeBSPInPrimitives(disFileName) ? RESULT_SUCCESS
                                                               : RESULT_FAILURE;
        } else if (ext == "bsp2") {
            return doFormatUpgradeStandaloneBSPFile(disFileName)
                     ? RESULT_SUCCESS
                     : RESULT_FAILURE;
        }
