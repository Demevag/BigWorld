...
//	(main one for offline_processor, for the actual main entry point see
...
namespace OfflineProcessor {
        ...
        if (messagePriority <= MESSAGE_PRIORITY_TRACE) {
            if (verbose_) {
                ...
                if (pCategory &&
                    ...
                    fprintf(
                      DebugFilter::consoleOutputFile(), "%s: ", pPriorityName);
                    fprintf(
                      DebugFilter::consoleOutputFile(), "[%s] ", pCategory);
                ...
                // swallow the message, as we don't want this message spamming
                // up
    ...
    void CommandManager::showAllSupportedCommands() const
    {
        ...
        std::cout << "Usage: offline_processor [command] [options]" << std::endl
                  << std::endl;
    ...
    /**
     *	Run command based on command line arguments or display help if there are
     *no arguments
     */
    bool CommandManager::run()
    {
        ...
        const char* unattendedCmd =
          applicationCommandLine_.getFullParam("unattended");
        if (unattendedCmd) {
            CStdMf::checkUnattended();
        }

        // tell the commandmanager & converttexturetask messages to go to the
        // console
        this->debugCallback_.verbose_ =
          applicationCommandLine_.hasParam("verbose");

        // make sure all messages get sent through
        DebugMessagePriority previousThreshold =
        ...
        if (pCommand) {
            ...
            if (cmd != NULL && strlen(cmd) > 0) {
                // unknown command
                ERROR_MSG(
                  "[CommandManager::run()] ERROR: Cannot recognise command "
                  "%s\n",
                  cmd);
            }
            this->showAllSupportedCommands();
