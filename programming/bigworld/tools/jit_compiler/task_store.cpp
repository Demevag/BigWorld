...
void TaskStore::addOutputToTask(ConversionTask*      task,
                                const BW::StringRef& output)
{
    auto taskInfo = findTaskInfo(task);
    MF_ASSERT(taskInfo != nullptr);
...
Connection TaskStore::registerRequestedTaskCallback(
...
Connection TaskStore::registerCompletedTaskCallback(
  TaskCallbackFunction callback)
{
