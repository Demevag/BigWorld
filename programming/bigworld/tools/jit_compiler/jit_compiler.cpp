...
void JITCompiler::stop()
{
    ...
    // tell the asset compiler to terminate. Will allow it to finish up any
                                       ...
                                       ConversionTask&   conversionTask)
{
    ...
    ForwardDependencies& forwardDependencies =
      forwardDependencyMap_[&conversionTask];
                                       ...
                                       ConversionTask&   conversionTask)
{
    BW::string path = bw_format("%s>%s>%s>%s",
                                ...
                                regex ? "1" : "0",
                                recursive ? "1" : "0");
    if (std::find(directoryDependencies_.begin(),
                  directoryDependencies_.end(),
                  path) == directoryDependencies_.end()) {
...
void JITCompiler::collectReverseDependencies(
  ...
  BW::vector<ConversionTask*>& conversionTasks)
{
    ...
    for (size_t i = 0; i < reverseDependencies.size();) {
        ...
        // requeue this task and on completion of the task the reverse
        // dependencies will be populated again.
        ConversionTask*      task                = reverseDependencies[i].first;
        ForwardDependencies& forwardDependencies = forwardDependencyMap_[task];
        for (ForwardDependencies::iterator forwardIt =
               forwardDependencies.begin();
             forwardIt != forwardDependencies.end();
             ++forwardIt) {
            ReverseDependencies& tasks =
              reverseDependencyMap_[forwardIt->to_string()];
...
void JITCompiler::purgeResource(const StringRef& resourceId)
{
    ...
    for (BW::vector<ResourceCallbacks*>::iterator it =
           resourceCallbacks_.begin();
         it != resourceCallbacks_.end();
         ++it) {
...
void JITCompiler::onAssetRequested(const StringRef& asset)
{
    ...
    // Try to push the task to the start of the queue
    {
        ...
        if (task.status_ == ConversionTask::QUEUED) {
            ...
            if (it != taskQueue_.end()) {
                taskQueue_.erase(it);
                // Set the status of this task back to new as it is no longer in
                // the queue.
    ...
    if (task.status_ >= ConversionTask::DONE) {
        BW::string relativeSourceFile =
          BWResolver::dissolveFilename(sourceFile);
        if (!BWResource::instance().hasPendingModification(
              relativeSourceFile)) {
    ...
    BW::string request = asset.to_string();
    {
        ...
        BW::vector<std::pair<ConversionTask*, BW::string>>::iterator requestIt =
          std::find(
            requests_.begin(), requests_.end(), std::make_pair(&task, request));
                                     ...
                                     Action           modType)
{
    ...
    for (BW::vector<ConversionTask*>::iterator it = tasks.begin();
         it != tasks.end();
         ++it) {
...
void JITCompiler::onTaskCompleted(ConversionTask& conversionTask)
{
...

    {
        ...
        for (BW::vector<std::pair<ConversionTask*, BW::string>>::iterator it =
               requests_.begin();
             it != requests_.end();) {
...

    {
        ...
        const BW::vector<DependencyList::Input>& primaryInputs =
          depList.primaryInputs();
        for (BW::vector<DependencyList::Input>::const_iterator it =
               primaryInputs.begin();
             it != primaryInputs.end();
             ++it) {
            addReverseDependency(*it->first, conversionTask);
        }

        const BW::vector<DependencyList::Input>& secondaryInputs =
          depList.secondaryInputs();
        for (BW::vector<DependencyList::Input>::const_iterator it =
               secondaryInputs.begin();
             it != secondaryInputs.end();
             ++it) {
            addReverseDependency(*it->first, conversionTask);
        }
        ...
        if (conversionTask.status_ != ConversionTask::FAILED) {
            ...
            for (BW::vector<DependencyList::Output>::const_iterator it =
                   intermediateOutputs.begin();
                 it != intermediateOutputs.end();
                 ++it) {
            ...
            const BW::vector<DependencyList::Output>& outputs =
              depList.outputs();
            for (BW::vector<DependencyList::Output>::const_iterator it =
                   outputs.begin();
                 it != outputs.end();
...
{
    store_.setCurrentTaskState(&conversionTask,
                               TaskInfoState::GENERATING_DEPENDENCIES);
                                ...
                                va_list              argPtr)
{
    ...
    if (st_currentTask_ != nullptr) {
        ...
        _vsnprintf(buffer.get(),
                   count - 1,
                   pFormat,
                   argPtr); // Shouldn't truncate but don't care if it does
        buffer.get()[count - 1] = '\0';

        BW::wstring message;
        bw_utf8tow(buffer.get(), message);
        if (store_.handleTaskMessage(
              st_currentTask_, messagePriority, pCategory, message)) {
            handled = true;
        }
    }
