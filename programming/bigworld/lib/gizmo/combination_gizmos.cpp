...
VectorGizmo::VectorGizmo(int            disablerModifiers,
                         MatrixProxyPtr matrix,
                         Moo::Colour    arrowColor,
                         float          arrowRadius,
                         MatrixProxyPtr originMatrixProxy,
                         float          radius)
  ...
  , originMatrixProxy_(originMatrixProxy)
{
    ...
    mesh_.addCylinder(Vector3(0, 0, 0),
                      arrowRadius,
                      shaftLength,
                      true,
                      false,
                      arrowColor_,
                      NULL);
    mesh_.addCone(Vector3(0, 0, shaftLength),
                  arrowRadius * 2.f,
                  arrowLength,
                  true,
                  arrowColor_,
...
bool VectorGizmo::draw(Moo::DrawContext& drawContext, bool force)
{
    ...
    if ((worldPosition.x == 0) && (worldPosition.y == 0))
        rightAngleVector =
          Vector3(worldPosition.x, -worldPosition.z, worldPosition.y);
    else
        rightAngleVector =
          Vector3(worldPosition.y, -worldPosition.x, worldPosition.z);
    ...
    Vector4 row1 = m.row(1);
    row1.normalise();
    m.row(1, row1);
    m.row(0,
          Vector4(rightAngleVector.crossProduct(worldPositionNormalised), 0.f));
...
BoxGizmo::BoxGizmo(MatrixProxyPtr matrix1,
                   MatrixProxyPtr matrix2,
                   Moo::Colour    boxColor,
                   bool           drawVectors,
                   MatrixProxyPtr originMatrixProxy,
  ...
  , originMatrixProxy_(originMatrixProxy)
{
    ...
    if (drawVectors) {
        cornerGizmo1_ = new VectorGizmo(disablerModifiers,
                                        matrix1_,
                                        boxColor,
                                        0.015f,
                                        originMatrixProxy,
                                        gizmo_axis_radius);
        cornerGizmo2_ = new VectorGizmo(disablerModifiers,
                                        matrix2_,
                                        boxColor,
                                        0.015f,
                                        originMatrixProxy,
        ...
        cornerGizmo1_ = new PositionGizmo(
          disablerModifiers, matrix1_, originMatrixProxy, gizmo_axis_radius);
        cornerGizmo2_ = new PositionGizmo(
          disablerModifiers, matrix2_, originMatrixProxy, gizmo_axis_radius);
    }
}
...
bool BoxGizmo::intersects(const Vector3& origin,
...
LineGizmo::LineGizmo(MatrixProxyPtr matrix1,
                     MatrixProxyPtr matrix2,
                     Moo::Colour    lineColor,
                     bool           drawVectors,
                     MatrixProxyPtr originMatrixProxy,
                     int            disablerModifiers)
  : matrix1_(matrix1)
  ...
  , originMatrixProxy_(originMatrixProxy)
{
    ...
    if (drawVectors) {
        pointGizmo1_ = new VectorGizmo(disablerModifiers,
                                       matrix1_,
                                       lineColor,
                                       0.015f,
                                       originMatrixProxy,
                                       gizmo_axis_radius);
        pointGizmo2_ = new VectorGizmo(disablerModifiers,
                                       matrix2_,
                                       lineColor,
                                       0.015f,
                                       originMatrixProxy,
        ...
        pointGizmo1_ = new PositionGizmo(
          disablerModifiers, matrix1_, originMatrixProxy, gizmo_axis_radius);
        pointGizmo2_ = new PositionGizmo(
          disablerModifiers, matrix2_, originMatrixProxy, gizmo_axis_radius);
    }
}
...
bool LineGizmo::intersects(const Vector3& origin,
                           const Vector3& direction,
                           float&         t,
                           bool           force)
{
...
CylinderGizmo::CylinderGizmo(MatrixProxyPtr matrix1,
                             MatrixProxyPtr matrix2,
                             FloatProxyPtr  radius,
                             Moo::Colour    cylinderColor,
                             bool           drawVectors,
                             MatrixProxyPtr originMatrixProxy,
  ...
  , pointGizmo2_(NULL)
{
    ...
    if (drawVectors) {
        ...
        pointGizmo1_ = new PositionGizmo(disablerModifiers,
                                         matrix1_,
                                         originMatrixProxy,
                                         gizmo_axis_radius,
                                         isPlanar);
        ...
        if (matrix2_)
            pointGizmo2_ = new PositionGizmo(disablerModifiers,
                                             matrix2_,
                                             originMatrixProxy,
                                             gizmo_axis_radius,
                                             isPlanar);
    }
    radiusGizmo_ = new RadiusGizmo(radius,
                                   matrix1_,
                                   "radius",
                                   cylinderColor,
                                   radiusGizmoRadius,
...
bool CylinderGizmo::draw(Moo::DrawContext& drawContext, bool force)
{
    ...
    if (start != end) {
        ...
        Vector3 referenceVector =
          Vector3(planeNormal.y, -planeNormal.x, planeNormal.z);
        ...
        radiusGizmoMatrix_.row(
          1, Vector4(referenceVector.crossProduct(radiusGizmoMatrix_[2]), 0.f));
        Vector4 row1 = radiusGizmoMatrix_.row(1);
        row1.normalise();
        radiusGizmoMatrix_.row(1, row1);
        radiusGizmoMatrix_.row(
          0,
          Vector4(radiusGizmoMatrix_[1].crossProduct(radiusGizmoMatrix_[2]),
                  0.f));
        Vector4 row0 = radiusGizmoMatrix_.row(0);
        row0.normalise();
    ...
    Geometrics::instance().wireCylinder(
      start, end, radius_->get(), cylinderColor_);
...
SphereGizmo::SphereGizmo(MatrixProxyPtr matrix1,
                         FloatProxyPtr  radius1,
                         FloatProxyPtr  radius2,
                         Moo::Colour    color,
                         bool           drawVectors,
                         MatrixProxyPtr originMatrixProxy,
                         int            disablerModifiers)
  ...
  , pointGizmo1_(NULL)
{
    ...
    radiusGizmo1_ = new RadiusGizmo(radius1_,
                                    matrix1_,
                                    "min radius",
    ...
    if (radius2_)
        radiusGizmo2_ = new RadiusGizmo(radius2_,
                                        matrix1_,
                                        "max radius",
...
bool SphereGizmo::intersects(const Vector3& origin,
                             const Vector3& direction,
                             float&         t,
                             bool           force)
{
