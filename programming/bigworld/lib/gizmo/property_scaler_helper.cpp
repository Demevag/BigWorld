...
void PropertyScalerHelper::init(const Matrix& gizmoTransform)
{
    ...
    for (BW::vector<GenScaleProperty*>::iterator i = props.begin();
         i != props.end();
         ++i) {
        PropInfo pi;
    ...
    // property list is not in the scale property list, add it to our position
    // properties list
    ...
    for (BW::vector<GenPositionProperty*>::iterator i = positionProps.begin();
        ...
        // Check if our matrixproxy is shared with one of the scale properties,
        // if not, we hold on to the position property so that we can modify the
        // position in case of world space scale being applied Note: this
        // assumes that the scale and position properties share the same matrix
        // proxy
    ...
    if (CoordModeProvider::ins()->getCoordMode() !=
        ...
        for (; posPropIt != positionOnlyProps_.end(); ++posPropIt) {
            ...
            newMatrix.translation(
              scaleMatrix.applyPoint(ppi.initialMatrix_.applyToOrigin()));
 ...
 */
bool PropertyScalerHelper::PropFinder::operator()(
  PropertyScalerHelper::PropInfo& prop)
{
    BW_GUARD;
