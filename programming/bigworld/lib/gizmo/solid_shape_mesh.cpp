 ...
 */
SpherePicker::SpherePicker(const Vector3& position,
                           float          radius,
 ...
 * ray intersects the outside of the sphere in the positive direction of the
 * ray.
 *
 ...
 */
bool SpherePicker::intersects(const Vector3& origin,
                              ...
                              Vector3*       intersection) const
{
    ...
    // find the distance from lo along the ray ld that the position of the
    // sphere is 90degrees on the line
    ...
    if (t > 0) {
        ...
        if (lengthSq <= radiusSquared_) {
            float distanceFromEdge =
              lengthSq < radiusSquared_ ? sqrtf(radiusSquared_ - lengthSq) : 0;
            float distanceFromOrigin = t - distanceFromEdge;
            if (distanceFromOrigin > 0) {
                ret = true;
                if (intersection) {
                    *intersection =
                      transform_.applyPoint(lo + (ld * distanceFromOrigin));
                }
            }
        }
    }
 ...
 */
BoxPicker::BoxPicker(const Vector3& minValues,
                     const Vector3& maxValues,
                     const Matrix&  transform)
  : min_(minValues)
  , max_(maxValues)
  , transform_(transform)
 ...
 */
bool BoxPicker::intersects(const Vector3& origin,
                           const Vector3& direction,
                           Vector3*       intersection) const
{
    BW_GUARD;
 ...
 */
CylinderPicker::CylinderPicker(const Vector3& position,
                               float          radius,
                               float          length,
                               bool           capFront,
                               bool           capBack,
 ...
 */
bool CylinderPicker::intersectsDisc(const Vector3& discPosition,
                                    float          discSign,
                                    ...
                                    Vector3*       intersection) const
{
    ...
    if (discSign * direction.z < 0 && dsOrigin.z * discSign > 0) {
 ...
 * ray intersects the outside of the cylinder in the positive direction of the
 * ray.
 *
 * @param origin origin of the ray
 * @param direction direction of the ray
 ...
 */
bool CylinderPicker::intersects(const Vector3& origin,
                                const Vector3& direction,
                                Vector3*       intersection) const
{
    BW_GUARD;
    ...
    if (ld.z != 0) {
        ...
        if (capBack_ && ret == false) {
            if (intersectsDisc(
                  position + Vector3(0, 0, 1), 1, lo, ld, &result)) {
                ret = true;
            }
        }
    }
    ...
    if ((ld.x != 0 || ld.y != 0) && ret == false) {
        ...
        if (t > 0) {
            ...
            if (lengthSq <= radiusSquared_) {
                float distanceFromEdge   = lengthSq < radiusSquared_
                                             ? sqrtf(radiusSquared_ - lengthSq)
                                             : 0;
                float distanceFromOrigin = t - distanceFromEdge;
                if (distanceFromOrigin > 0) {
 ...
 */
ConePicker::ConePicker(const Vector3& position,
                       float          radius,
                       float          length,
                       bool           cap,
                       const Matrix&  transform)
 ...
 */
bool ConePicker::intersects(const Vector3& origin,
                            const Vector3& direction,
                            Vector3*       intersection) const
{
    BW_GUARD;
    ...
    if (!ret) {
        ...
        float cosConeAngle =
          fabsf(length_) / (sqrtf((length_ * length_) + (radius_ * radius_)));
        ...
        if (dir.dotProduct(pointInCone) < cosConeAngle) {
            ...
            // Solve the quadratic.  Keep only those X for which Dot(A,X-V) &gt;
            // 0.
 ...
 */
bool ConePicker::intersectsDisc(const Vector3& discPosition,
                                float          discSign,
                                const Vector3& origin,
                                const Vector3& direction,
                                Vector3*       intersection) const
 ...
 */
DiscPicker::DiscPicker(const Vector3& position,
                       float          innerRadius,
                       float          outerRadius,
                       const Matrix&  transform)
  : position_(position)
 ...
 * @return whether the ray intersected with the cone or not
 */
bool DiscPicker::intersects(const Vector3& origin,
                            const Vector3& direction,
                            Vector3*       intersection) const
{
    BW_GUARD;
...
struct MiniEdge
{
    ...
    {
        return (me.a == a && me.b == b) || (me.a == b && me.b == a);
    }
    int a, b;
};
...
/**
 * This method adds a spehere to the mesh and transforms it by the current
 * transform
 ...
 */
void SolidShapeMesh::addSphere(const Vector3& pos,
                               float          radius,
                               uint32         colour,
                               ShapePartPtr   callback)
{
    BW_GUARD;
...
/**
 * This method adds a cone to the mesh and transforms it by the current
 * transform
 ...
 */
void SolidShapeMesh::addCone(const Vector3& pos,
                             float          radius,
                             float          length,
                             bool           cap,
                             uint32         colour,
...
/**
 * This method adds a cylinder to the mesh and transforms it by the current
 * transform
 ...
 */
void SolidShapeMesh::addCylinder(const Vector3& pos,
                                 float          radius,
                                 float          length,
                                 bool           capFront,
                                 bool           capBack,
                                 ...
                                 ShapePartPtr   callback)
{
    ...
    pickers_.push_back(
      new CylinderPicker(pos, radius, length, capFront, capBack, transform_));
    callbacks_.push_back(callback);
 ...
 */
void SolidShapeMesh::addBox(const Vector3& minValues,
                            const Vector3& maxValues,
                            uint32         colour,
                            ShapePartPtr   callback)
{
...
/**
 * This method adds a disc to the mesh and transforms it by the current
 * transform
 ...
 */
void SolidShapeMesh::addDisc(const Vector3& pos,
                             float          innerRadius,
                             float          outerRadius,
                             uint32         colour,
                             ShapePartPtr   callback)
{
    ...
    pickers_.push_back(
      new DiscPicker(pos, innerRadius, outerRadius, transform_));
    callbacks_.push_back(callback);
...
/**
 * This method intersects a ray with the objects in the mesh. It only returns
 * true if the ray intersects the outside of any of the objects, and it also
 * calls the collision callback for the closest object it intersects with.
 *
 * @param origin origin of the ray
 * @param direction direction of the ray
 * @param distance pass in the maximum distance of the direction returns the
 * distance to the closest object
 * @return the ShapePartPtr of the intersection
 */
ShapePart* SolidShapeMesh::intersects(const Vector3& origin,
                                      const Vector3& direction,
                                      float*         distance)
{
    BW_GUARD;
...
void SolidShapeMesh::draw(Moo::RenderContext& rc)
{
    ...
    Moo::IndicesReference ind = dib.lock(static_cast<uint32>(indices().size()));
    if (ind.size()) {
        ...
        if (verts) {
            ...
            rc.drawIndexedPrimitive(D3DPT_TRIANGLELIST,
                                    ...
                                    ibLockIndex,
                                    static_cast<UINT>(indices_.size() / 3));
        }
    }
}
