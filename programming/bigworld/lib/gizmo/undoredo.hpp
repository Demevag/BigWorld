  ...
  public:
    ...
    class Operation
    {
      public:
        ...
        {
            if (kind_ != oth.kind_ || !kind_)
                return false;
            return this->iseq(oth);
        }

        bool operator!=(const Operation& oth) const { return !(*this == oth); }

        virtual bool iseq(const Operation& oth) const = 0;
        void         record() { env_ = Environment::record(); }
        void         replay() { Environment::replay(env_); }
        void         addChunk(Chunk* chunk) { affectedChunks_.insert(chunk); }
        void         markChunks();
