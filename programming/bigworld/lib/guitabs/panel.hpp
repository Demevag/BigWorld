...
namespace GUITABS {
    ...
    /*
     *  This simple struct is used to save a panel's relative position to
     *another destination panel ( destPanel ). A list of relative positions is
     *kept for the last docked positions as well as for the last floating
     *positions. When the user double-clicks on a panel's title bar, the panel
     *will toggle between the last floating position and the last docked
     *position using this struct. Each PanelPos entry contains info about this
     *panels position relative to others, like "Left to the Asset Browser", "Top
     *of the Properties", "As a tab in the Object panel" and so on.
     *
    ...
    {
        PanelPos()
          : insertAt(UNDEFINED_INSERTAT)
          , destPanel(0)
        {
        }
        PanelPos(InsertAt ins, CWnd* pan)
          : insertAt(ins)
          , destPanel(pan)
        {
        }
     ...
     *  responsible for drawing all the panel controls, including the tabs,
     * close button, clone button, titlebar, etc.
     */
    class Panel
      : public CWnd
      , public TabCtrlEventHandler
      , public ReferenceCount
    {
      ...
      private:
        ...
        bool                               isExpanded_;
        int                                expandedSize_;
        // Lists of preferred saved docking locations, both for when in a
        // floating window and when docked in the mainframe.
