...
namespace GUITABS {
     ...
     *  @return true if the factory was successfully registered, false
     * otherwise.
     *  @see ContentFactory
     */
     ...
     *
     *  @param mainFrame the Main Frame window of the application. Must inherit
     * from CFrameWnd.
     *  @param mainFrame the Main View window (ie. the 3D window in
     * WorldEditor). Must inherit from CWnd.
     *  @return true if the windows were successfully registered, false
     * otherwise.
     */
     ...
     *  and will set the Main Frame window as the parent of the Main View
     * window.
     */
     ...
     *  @param destPanel destination panel to insert the panel into. If
     * ommitted, the manager will layout the panel automatically.
     *  @return a PanelHandle to the panel that was created, 0 otherwise.
     *  @see ContentFactory, Content, Manager::isValid(), Manager::load(),
     * Manager::save()
     */
    PanelHandle Manager::insertPanel(const BW::wstring& contentID,
                                     InsertAt           insertAt,
                                     PanelHandle        destPanel)
    {
        ...
        // this code should be improved so in the rare case of having two
        // contents with the same contentID in one panel, it returns the last
        // inserted one. (probably using the getContent method that takes an
        // 'index' param?)
     ...
     *  @param LPARAM additional message-dependent information.
     */
    void Manager::sendMessage(const BW::wstring& contentID,
                              UINT               msg,
                              WPARAM             wParam,
                              LPARAM             lParam)
    {
     ...
     *  with their corresponding insert position, floating state, visibility,
     * etc. This function is tipically called on app's startup, after creating
     * the mainframe window and the main 3D view window.
     *
    ...
    ContentPtr Manager::createContent(const BW::wstring& contentID)
    {
        ...
        for (ContentFactoryItr i = factoryList_.begin();
             i != factoryList_.end();
             ++i) {
