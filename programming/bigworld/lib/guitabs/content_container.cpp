...
namespace GUITABS {
    ...
    const BW::wstring ContentContainer::contentID =
      L"GUITABS::ContentContainer";
     ...
     *by contentID.
     *
     *	@param subcontentID	ContentID for the subcontent to add to this
     *container.
     */
    ...
    /**
     *	This method makes the specified subcontent the currently active
     *subcontent in this container panel.
     *
    ...
    void ContentContainer::currentContent(ContentPtr content)
    {
        ...
        CRect rect;
        GetClientRect(&rect);
        currentContent_->getCWnd()->SetWindowPos(
          0, 0, 0, rect.Width(), rect.Height(), SWP_NOZORDER);
        currentContent_->getCWnd()->ShowWindow(SW_SHOW);
        if (oldContent)
            oldContent->ShowWindow(SW_HIDE);
        CWnd* parent = GetParent();
        if (parent)
            parent->RedrawWindow(0,
                                 0,
     ...
     *	@return		True of the subcontent is part of this container, false if
     *not.
     */
    ...
    /**
     *	This method returns the first occurrence of a contentID in this
     *container.
     *
     *	@param subcontentID	ContentID of the desired subcontent.
     ...
     *
     *	@param subcontentID	ContentID of the desired subcontent.
     *	@return		The occurrence number "index" of a contentID in this
     *container.
     */
    ContentPtr ContentContainer::getContent(const BW::wstring subcontentID,
                                            int&              index)
     ...
     */
    void ContentContainer::broadcastMessage(UINT   msg,
                                            WPARAM wParam,
                                            LPARAM lParam)
    {
        BW_GUARD;
    ...
    bool ContentContainer::load(DataSectionPtr section)
    {
        ...
        for (BW::vector<DataSectionPtr>::iterator s = sections.begin();
             ...
             ++s) {
            ContentPtr content =
              Manager::instance().createContent((*s)->asWideString());
            if (!content)
                continue;
    ...
    /**
     *	This method forwards the call to the active subcontainer so its tab
     *caption text gets displayed.
     *
    ...
    /**
     *	This method forwards the call to the active subcontainer so its icon
     *gets used.
     *
    ...
    /**
     *	This method calculates the preferred size by getting the maximum of all
     *the subcontainers preferred sizes.
     *
    ...
    /**
     *	This method is called when the user right-clicks on the non-client area
     *of this container panel, and it simply forwards that call to the active
     *	subcontent.
     *
