...
namespace GUITABS {
    ...
    void Panel::addTab(const BW::wstring contentID)
    {
        ...
        tabBar_->insertItem(
          newTab->getTabDisplayString(), newTab->getIcon(), newTab.getObject());
        newTab->show(true);
        setActiveTab(newTab);
    }
    ...
    void Panel::addTab(TabPtr tab)
    {
        ...
        if (tab->isVisible()) {
            tabBar_->insertItem(
              tab->getTabDisplayString(), tab->getIcon(), tab.getObject());
            setActiveTab(tab);
        }
    }
    ...
    void Panel::detachTab(const BW::wstring contentID)
    {
        ...
        for (TabItr i = tabList_.begin(); i != tabList_.end();) {
            if (contentID.compare((*i)->getContent()->getContentID()) == 0) {
                // restart iterator since detachTab will remove the tab from the
                // list
                TabItr old = i;
                ++i;
                detachTab(*old);
    ...
    /**
     *	This method loads the panel's position and layout info from the layout
     *xml file, and creates and loads its tabs.
     *
    ...
    bool Panel::load(DataSectionPtr section)
    {
        ...
        SetWindowPos(0,
                     0,
                     0,
                     section->readInt("lastWidth", w),
                     section->readInt("lastHeight", h),
        ...
        activeTab_      = 0;
        TabPtr firstTab = 0;
        for (BW::vector<DataSectionPtr>::iterator i = tabs.begin();
             i != tabs.end();
             ++i) {
    ...
    bool Panel::save(DataSectionPtr section)
    {
        ...
        // save tabs
        for (BW::vector<TabPtr>::iterator i = tabOrder.begin();
             i != tabOrder.end();
             ++i) {
            ...
            tabSec->writeWideString("contentID",
                                    (*i)->getContent()->getContentID());
    ...
    /**
     *	This method sets whether a panel is expanded (true) or rolled up
     *(false).
     *
     *	@param expanded		Sets the panel to expanded (true) or rolled up
     *(false).
     */
    ...
    /**
     *	This method returns whether the panel is floating (true) or docked
     *(false).
     *
     ...
     *
     *	@param floating		Whether the panel is floating (true) or docked
     *(false).
     */
    ...
    /**
     *	This method returns the panels preferred default size, which is the max
     *of its tabs preferred sizes.
     *
    ...
    /**
     *	This method returns the height of its tabs control, which can be
     *multiline.
     *
     ...
     *
     *	@param docked	Whether or not the panel is docked (true) or not
     *(false).
     */
     ...
     *	positions.
     *	This list is used for when, for example, double clicking on a docked
     *panel, which results in the panel toggling from docked to floating.  This
     *list stores information on how the panel was inserted in the dock tree so,
     *if it has to be docked again, it makes the best guess.  Each PanelPos
     *entry contains info about this panels position relative to others, like
     *"Left to the Asset Browser", "Top of the Properties", "As a tab in the
     *Object panel" and so on.
     *
     *	@param docked	Whether or not the panel is docked (true) or not
     *(false).
     */
     ...
     *	@return		True if a position was returned in pos, false if no more
     *info.
     */
    bool Panel::getNextPos(bool docked, PanelPos& pos)
    {
        ...
        if (docked) {
            ...
            if (floatingPosItr_ == floatingPosList_.end() ||
                floatingPosList_.empty())
                return false;
    ...
    /**
     *	This method returns whether or not a specified tab contains a Content.
     *If the tab is a content container, the we ask the content container if it
     ...
     *
     *	@param t	Tab we want to test.
     *	@param contentID	Content we want to know whether or not is in tab
     *"t".
     ...
     *
     *	@param content	Content we want to know whether or not it's in the
     *panel.
     ...
     *	This method returns whether or not this panel contains a Content.
     *
     *	@param contentID	Content we want to know whether or not is in the
     *panel.
    ...
    /**
     *	This method returns the "index" occurrence of the Content specified by
     *the contentID contained in this panel, or NULL if the content is not
     *contained or there are less than "index" occurrences of the content.
     *
     *	@param contentID	Content ID of the desired Content.
        ...
        for (TabItr i = tabList_.begin(); i != tabList_.end(); ++i) {
            ...
            if (content->getContentID() == ContentContainer::contentID)
                ((ContentContainer*)content)
                  ->broadcastMessage(msg, wParam, lParam);
    ...
    void Panel::clickedTab(void* itemData, int x, int y)
    {
        ...
        if (activeTab_) {
            ...
            Manager::instance().dragManager()->startDrag(
              rect.left + x, rect.top + y, this, activeTab_);
        }
    }

    /**
     *	This method handles a double-click on a tab event sent from the tab
     *control which results in activating or floating the tab.
     *
     *	@param itemData	Callback specific data, the Tab pointer in this case.
     *	@param x	Mouse X position.
     *	@param y	Mouse Y position.
    ...
    /**
     *	This method handles a right-click on a tab event sent from the tab
     *control, which simply forwards the event to the Tab's Content.
     *
     *	@param itemData	Callback specific data, the Tab pointer in this case.
     *	@param x	Mouse X position.
     *	@param y	Mouse Y position.
    ...
    void Panel::paintCaptionBarOnly()
    {
        ...
        dc.DrawText(CString(text.c_str()),
                    &tRect,
                    DT_SINGLELINE | DT_LEFT | DT_END_ELLIPSIS);

        if (!isActive_ && tabBar_->itemCount() > 1) {
            CPen  btnBottomLine(PS_SOLID,
                               1,
                               isExpanded_ ? ::GetSysColor(COLOR_BTNSHADOW)
     ...
     *	@return		Button being hit, or if no button is hit, whether the point
     *is in the caption or client areas.
     */
    UINT Panel::hitTest(const CPoint point)
    {
        ...
        closeRect.DeflateRect(winRect.Width() - CAPTION_HEIGHT,
                              0,
                              0,
                              winRect.Height() - CAPTION_HEIGHT);
        numBut++;
        ...
        if (closeRect.PtInRect(point))
            ...
            ret = BUT_ROLLUP;
        else if (activeTab_ && activeTab_->isClonable() &&
                 cloneRect.PtInRect(point))
            ret = BUT_CLONE;
    ...
    /**
     *	This method recalculates the size and position of the tab bar control
     *and the current tab, to match the current panel size.
     */
    ...
    /**
     *	This method recalculates the size and position of the tab bar control
     *and the current tab, to the specified width and height.
     *
     *	@param w	New width.
     *	@param h	New height.
     */
    void Panel::recalcSize(int w, int h)
    {
        ...
        if (tabBar_->GetSafeHwnd()) {
            ...
            tabBar_->recalcHeight(); // force a recalc of the height/number of
                                     // lines, based on the width
            if (tabBar_->getAlignment() == TabCtrl::TOP)
                tabBar_->SetWindowPos(
                  0, 0, 0, w, tabBar_->getHeight(), SWP_NOZORDER);
            else
                tabBar_->SetWindowPos(0,
                                      0,
                                      h - tabBar_->getHeight(),
                                      w,
                                      tabBar_->getHeight(),
        ...
        if (activeTab_) {
            if (tabBar_->getAlignment() == TabCtrl::TOP)
                activeTab_->getCWnd()->SetWindowPos(
                  0, 0, tabBarHeight, w, h - tabBarHeight, SWP_NOZORDER);
            else
                activeTab_->getCWnd()->SetWindowPos(
                  0, 0, 0, w, h - tabBarHeight, SWP_NOZORDER);
        }
    }
     ...
     *	the user moves the dragged temporary tab over the tab control of the
     *panel.
     *
     *	@param x	Mouse X position.
     *	@param y	Mouse Y position.
    ...
    void Panel::updateTempTab(int x, int y)
    {
        ...
        CRect rect;
        tabBar_->GetWindowRect(&rect);
        tabBar_->updateItemPosition(
          tempTab_.getObject(), x - rect.left, y - rect.top);
    }
     ...
     *
     *	@param contentID	Content ID of the content that will be shown or
     *hidden.
    ...
    /**
     *	This method clones a tab after the user presses the "clone" button on
     *the panel's caption bar.
     *
    ...
    ContentPtr Panel::cloneTab(ContentPtr content, int x, int y)
    {
        ...
        CRect rect;
        GetWindowRect(&rect);
        Manager::instance().dock()->dockTab(
          this, tab, 0, FLOATING, rect.left, rect.top, x, y);
    ...
    void Panel::updateTabPosition(int x, int y)
    {
        ...
        tabBar_->GetWindowRect(&rect);
        tabBar_->updateItemPosition(
          activeTab_.getObject(), x - rect.left, y - rect.top);
    }
    ...
    /**
     *	This method mvoes the active tab to a new position in the tab control
     *list.
     *
    ...
    /**
     *	This method returns the position of the active tab in the tab control
     *list.
     *
    ...
    bool Panel::onTabClose(TabPtr tab)
    {
        ...
        int cnt = Manager::instance().dock()->getContentCount(
          tab->getContent()->getContentID());
        ASSERT(cnt != 0);

        Content::OnCloseAction response =
          tab->getContent()->onClose((cnt == 1));
    ...
    /**
     *	This method handles a click on the "close" caption bar button of the
     *panel.
     *
    ...
    bool Panel::onClose()
    {
        ...
        FloaterPtr floater =
          Manager::instance().dock()->getFloaterByWnd(getCWnd());

        if (floater) {
            // Hack: To avoid that the CFrameWnd floater still points to a
            // deleted panel as its view, which later results in assertion
            // failed/crash.
        ...
        if (isFloating_ &&
            (!floater || floater->getRootNode()->getCWnd() == getCWnd())) {
            doClose = true;

            for (TabItr i = tabList_.begin(); i != tabList_.end();) {
                // save iterator since onTabClose can potentially remove the tab
                // from the list
                TabItr old = i;
                ++i;
                if (!onTabClose((*old)))
    ...
    /**
     *	This method returns the panel's index in the dock's panel list, useful
     *for when saving.
     *
    ...
    /**
     *	This MFC method is called by Windows paint non-client area (a.k.a
     *caption bar).
     */
    ...
    void Panel::OnPaint()
    {
        ...
        for (TabItr i = tabList_.begin(); i != tabList_.end(); ++i)
            tabBar_->updateItemData(
              (*i).getObject(), (*i)->getTabDisplayString(), (*i)->getIcon());
        recalcSize();
        CWnd::OnPaint();
    }
     ...
     *	@return		Which button the mouse is over, or whether it's over the
     *client area or the caption bar (see hitTest).
     */
        ...
        switch (nHitTest) {
            ...
            case BUT_CLONE:
                ...
                if (activeTab_) {
                    CRect rect;
                    GetWindowRect(&rect);
                    // position is hand-hacked, but should work well in all
                    // cases
                    cloneTab(activeTab_->getContent(),
                             (rect.left + 10) %
                               (GetSystemMetrics(SM_CXMAXIMIZED) - 64),
                             (rect.top) %
                               (GetSystemMetrics(SM_CYMAXIMIZED) - 64));
                }
                break;
        }
    }
    ...
    /**
     *	This MFC method is called by Windows when the user double-clicks the
     *left mouse button on the non-client area (a.k.a caption bar).
     *
     *	@param nHitTest	Which area, result of OnNcHitTest (see OnNcHitTest).
     *	@param point	Mouse position.
     */
    ...
    /**
     *	This MFC method is called by Windows when the user presses down the
     *right mouse button on the non-client area (a.k.a caption bar).
     *
     *	@param nHitTest	Which area, result of OnNcHitTest (see OnNcHitTest).
     *	@param point	Mouse position.
     */
    ...
    /**
     *	This MFC method is called by Windows when the user moves the mouse over
     *the non-client area (a.k.a caption bar).
     *
     *	@param nHitTest	Which area, result of OnNcHitTest (see OnNcHitTest).
     *	@param point	Mouse position.
     */
    ...
    /**
     *	This MFC method is called by Windows when the user activates the panel
    ...
    int Panel::OnMouseActivate(CWnd* pDesktopWnd, UINT nHitTest, UINT message)
    {
        ...
        if (nHitTest == HTCAPTION || (!isActive_ && nHitTest == HTCLIENT)) {
            ...
            activate();
            if (Manager::instance().dock()->getMainFrame()) {
                // Hack: needed to avoid that the MainFrame thinks a FormView
                // tab contained in the panel is the active view, which results
                // in windows no longer receiving messages properly for some
                // reason.
                Manager::instance().dock()->getMainFrame()->SetActiveView(
                  (CView*)Manager::instance().dock()->getMainView());
            }
