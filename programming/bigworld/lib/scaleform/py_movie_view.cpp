...
#define PY_ATTR_SCOPE PyMovieView::
    ...
    PY_RW_ACCESSOR_ATTRIBUTE_DECLARE(PyMovieView::eScaleModeType,
                                     scaleMode,
                                     scaleMode);
     ...
     *
     *	Call this method to pass a key event into the underlying flash movie. If
     *the movie handles the event, this method will return True.
     *
     ...
     *
     *	Call this method to pass a mouse button event into the underlying flash
     *movie. If the movie handles the event, this method will return True.
     *
     *	@param	event	A PyKeyEvent object.
     *
      ...
      , pMovieView_(*pMovieView)
    {
        ...
        pMovieView_->SetExternalInterface(
          Ptr<GFx::ExternalInterface>(gameDelegate_));
    ...
    void PyMovieView::setExternalInterfaceCallback(PyObjectPtr args)
    {
        ...
        if (args != Py_None) {
            externalInterface_ =
              new PyExternalInterfaceHandler(args.getObject());
            gameDelegate_->RegisterFallbackHandler(
              externalInterface_, &PyExternalInterfaceHandler::handleCallback);
        }
    }
    ...
    PyObject* PyMovieView::invoke(PyObjectPtr spArgs)
    {
        ...
        if (numArgs == 0) {
            PyErr_SetString(PyExc_Exception,
                            "PyMovieView.invoke expected atleast 1 argument.");
            return NULL;
        } else if (numArgs > 0) {
            ...
            if (!PyString_CheckExact(arg0)) {
                PyErr_SetString(
                  PyExc_Exception,
                  "PyMovieView.invoke - The first argument must be a String.");
                return NULL;
            }
        ...
        if (numArgs <= 1) {
            ...
            if (!this->pMovieView_->Invoke(methodName, &gfxResult, NULL, 0)) {
                PyErr_SetString(
                  PyExc_Exception,
                  "PyMovieView.invoke - Failed to invoke method.");
                return NULL;
            }
        }
        ...
        // Arguments stored in a list (2nd argument of the tuple).
            ...
            // Make the call.
            if (!this->pMovieView_->Invoke(methodName,
                                           &gfxResult,
                                           &gfxArgs[0],
                                           static_cast<UInt>(numArgs))) {
                PyErr_SetString(
                  PyExc_Exception,
                  "PyMovieView.invoke - Failed to invoke method.");
                return NULL;
    ...
    {
        BW_GUARD;
        return (PyMovieView::eScaleModeType)this->pMovieView_
          ->GetViewScaleMode();
    }
    ...
    {
        BW_GUARD;
        return this->pMovieView_->GotoLabeledFrame(plabel.c_str(),
                                                   (SInt)offset);
    }
    ...
    bool PyMovieView::handleMouseEvent(const MouseEvent& event)
    {
        ...
        NotifyMouseStateFn msFn(mousePos.x,
                                mousePos.y,
                                buttons,
                                (Float)((event.dz() / WHEEL_DELTA) * 3));
        msFn(this);
    ...
    bool PyMovieView::handleKeyEvent(const KeyEvent& event)
    {
        ...
        if (event.isMouseButton()) {
            ...
            NotifyMouseStateFn msFn(
              (float)mousePos.x, (float)mousePos.y, buttons, 0.f);
            msFn(this);
            handled = msFn.handled();
        } else if (!(event.isAltDown() &&
                     (event.key() == KeyCode::KEY_RETURN ||
                      event.key() ==
                        KeyCode::KEY_NUMPADENTER))) // SCALEFORM - skip
                                                    // alt+enter
        {
            ...
            if (gfxevent.KeyCode != SF::Key::None &&
                ...
                !(containsChar && gfxevent.Modifiers.IsAltPressed() &&
                  gfxevent.Modifiers.IsCtrlPressed())) {
                HandleEventFn kyFn(gfxevent);
                kyFn(this);
                handled = kyFn.handled();
            }

            if (containsChar && !(event.isCtrlDown() &&
                                  event.key() == KeyCode::KEY_BACKSPACE)) {
                GFx::CharEvent gfxevent(*(const UInt32*)(event.utf16Char()));
                handled |= (pMovieView()->HandleEvent(gfxevent) ==
                            GFx::Movie::HE_Completed);
            }
        }
                                     ...
                                     const ScriptObject& value)
    {
        ...
        GFx::Value gfxValue = PyGFxValue::convertValueToGFx(value.get(), this);
        if (pMovieView_->SetVariable(
              attrObj.c_str(), gfxValue, GFx::Movie::SV_Normal)) {
            res = true;
        }
