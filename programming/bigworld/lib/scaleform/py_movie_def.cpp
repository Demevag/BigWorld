...
namespace ScaleformBW {
    ...
    PyObject* PyMovieDef::createInstance(bool initFirstFrame)
    {
        ...
        // movieView->SetViewport( (SInt)Moo::rc().backBufferDesc().Width,
        // (SInt)Moo::rc().backBufferDesc().Height , (SInt)pViewport.X,
        // (SInt)pViewport.Y, (SInt)Moo::rc().screenWidth(),
        // (SInt)Moo::rc().screenHeight() );
    ...
    {
        BW_GUARD;
        Ptr<GFx::DrawTextManager> pdm =
          *new GFx::DrawTextManager(this->pMovieDef_);
    ...
    PyObject* PyMovieDef::pyNew(PyObject* args)
    {
        ...
        GFx::MovieDef* movieDef = Manager::instance().pLoader()->CreateMovie(
          fileName, GFx::Loader::LoadWaitCompletion);
        if (!movieDef) {
            // File not found exception
