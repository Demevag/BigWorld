...
namespace ScaleformBW {
    ...
    /*
     * Derived from PyGFxValue, exposes Scaleform DisplayObject API methods to
     * Python scripts.
     */
    class PyGFxDisplayObject : public PyGFxValue
    {
      ...
      public:
        ...
        PY_RW_ACCESSOR_ATTRIBUTE_DECLARE(BW::wstring,
                                         textHTMLUnicode,
                                         textHTMLUnicode);
    ...
    class PyGFxDisplayInfo : public PyObjectPlus
    {
      ...
      public:
        ...
        Vector2 position()
        {
            return Vector2((float)info_.GetX(), (float)info_.GetY());
        }
        ...
        {
            return Vector3(
              (float)info_.GetX(), (float)info_.GetY(), (float)info_.GetZ());
        }
        void position3D(const Vector3& pos)
        {
            info_.SetX(pos.x);
            info_.SetY(pos.y);
            info_.SetZ(pos.z);
        ...
        Vector2 scale()
        {
            return Vector2((float)info_.GetXScale(), (float)info_.GetYScale());
        }
        ...
        Vector3 scale3D()
        {
            return Vector3((float)info_.GetXScale(),
                           (float)info_.GetYScale(),
                           (float)info_.GetZScale());
        }
        void scale3D(const Vector3& sc)
        {
            info_.SetXScale(sc.x);
            info_.SetYScale(sc.y);
            info_.SetZScale(sc.z);
        }
        ...
        PY_RW_ACCESSOR_ATTRIBUTE_DECLARE(Matrix,
                                         projectionMatrix3D,
                                         projectionMatrix3D);
        ...
        void edgeAAMode(uint val)
        {
            info_.SetEdgeAAMode((SF::Render::EdgeAAMode)val);
        }
        PY_RW_ACCESSOR_ATTRIBUTE_DECLARE(uint, edgeAAMode, edgeAAMode);

        // Gfx::Value::DisplayInfo has 16 bit alignment requirements, thus need
        // to do an aligned alloc
