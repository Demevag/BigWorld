 ...
 * Constructor.
 */
PythonMapping::PythonMapping(const Namer&      namer,
  ...
  : StringLikeMapping(namer, propName, INDEX_TYPE_NONE, length)
{
    ...
    if (defaultValue_.empty()) {
        defaultValue_ =
          PythonMapping::getPickler().pickle(ScriptObject::none());
 ...
 */
void PythonMapping::pickleExpression(BW::string& output, const BW::string& expr)
{
