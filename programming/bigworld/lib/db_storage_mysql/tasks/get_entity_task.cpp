...
void GetEntityTask::performBackgroundTask(MySql& conn)
{
    ...
    if (pStream_ != NULL) {
        isOkay = entityTypeMapping_.getStreamByID(
          conn, entityKey_.dbID, threadStream_);
    }

    if (isOkay && shouldGetBaseEntityLocation_) {
        // Try to get base mailbox
        hasBaseLocation_ = entityTypeMapping_.getLogOnRecord(
          conn, entityKey_.dbID, baseEntityLocation_);
    }
...
void GetEntityTask::performEntityMainThreadTask(bool succeeded)
{
    ...
    handler_.onGetEntityComplete(
      succeeded, entityKey_, hasBaseLocation_ ? &baseEntityLocation_ : NULL);
}
...
{
    handler_.onGetEntityComplete(/*succeeded:*/ false,
                                 entityKey_,
