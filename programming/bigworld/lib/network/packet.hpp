...
namespace Mercury {
    ...
    /**
     *  This type is used to track regions of data on a packet.  The packet type
     * is fairly opaque (i.e. just a chunk of bytes in the data array) and
     * without these things it's kinda hard to extract stuff out of the packet
     * data after it has been streamed on.
     */
    struct Field
    {
        char*  beg_;
    ...
    class Packet : public ReferenceCount
    {
      ...
      private:
        /// The maximum payload size for a packet.  This is actually defined in
        /// the cpp to allow changing packet size without needing to edit hpps.
        /// This is private to force the use of freeSpace(), thereby respecting
        /// footerSize_ and extraFilterSize_.
      ...
      public:
        ...
        /// packet.  This is done so that the bundle logic can always assume
        /// that these footers will fit and not have to worry about
        /// pre-allocating them. This is currently 27 bytes, roughly 1.5% of the
        /// capacity of a packet, so there's not too much wastage.
      ...
      public:
        ...
        int freeSpace() const
        {
            return MAX_SIZE - RESERVED_FOOTER_SIZE - msgEndOffset_ -
                   footerSize_ - extraFilterSize_;
        }
        ...
        /**
         *  This method strips a footer off the back of this packet.  If the use
         * of a switch is a performance issue, this can be reimplemented as a
         * macro. It returns true if there was enough data on the packet for the
         * footer.
         */
        template <class TYPE>
        bool stripFooter(TYPE& value)
        {
            ...
            switch (sizeof(TYPE)) {
                case sizeof(uint8):
                    value = TYPE(*(TYPE*)this->back());
                    break;

                case sizeof(uint16):
                    value = TYPE(BW_NTOHS(*(TYPE*)this->back()));
                    break;

                case sizeof(uint32):
                    value = TYPE(BW_NTOHL(*(TYPE*)this->back()));
                    break;
        ...
        /**
         *  This method writes a footer to the back of this packet.  It should
         * only be called from NetworkInterface::send() and assumes that size_
         * has been artificially increased so that it points to the end of the
         * footers, the idea being that we work back towards the real body end.
         */
        template <class TYPE>
        void packFooter(TYPE value)
        {
            ...
            switch (sizeof(TYPE)) {
                case sizeof(uint8):
                    *(TYPE*)this->back() = value;
                    break;

                case sizeof(uint16):
                    *(TYPE*)this->back() = BW_HTONS(value);
                    break;

                case sizeof(uint32):
                    *(TYPE*)this->back() = BW_HTONL(value);
                    break;
        ...
        static void addToStream(BinaryOStream& data,
        ...
        /**
         *  This method returns the maximum possible body size for a single
         * packet. It does not take into account packetFilter reservations or
         * footers, so should only be used as a rough guide for payload size.
         */
      ...
      private:
        ...
        /// temporarily incorrect in two situations: when sending, it is
        /// incorrect in NetworkInterface::send() whilst footers are being
        /// written, and when receiving, it  is incorrect until
        /// processOrderedPacket() strips the fragment footers.
        int msgEndOffset_;
        ...
        /// Fragment begin and end sequence numbers, or Channel::SEQ_NULL if
        /// this packet isn't a fragment.
        SeqNum fragBegin_;
        SeqNum fragEnd_;
