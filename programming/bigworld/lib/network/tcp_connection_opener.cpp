...
namespace Mercury {
      ...
      , pUserData_(pUserData)
    {
        ...
        int connectResult =
          pEndpoint_->connect(connectAddress.port, connectAddress.ip);
        ...
        this->dispatcher().registerWriteFileDescriptor(
          pEndpoint_->fileno(), this, "TCPConnectionOpener");
    ...
    int TCPConnectionOpener::handleInputNotification(int fd)
    {
        ...
        // Linux will inform of both a connection success and error with a
        // POLLOUT notification. Shouldn't hurt to check for SO_ERROR on Windows
        // as well, but Emscripten does it via a POLLHUP event, which is caught
        // in handleErrorNotification.

#if !defined(EMSCRIPTEN)
        int socketError = 0;
        if (-1 ==
            pEndpoint_->getSocketOption(SOL_SOCKET, SO_ERROR, socketError)) {
        ...
        Mercury::TCPChannel* pChannel =
          new TCPChannel(networkInterface_, *pEndpoint, /* isServer */ false);
...
#else
        // Linux won't notify of connect error via POLLERR or POLLHUP, but
        // Windows will via POLLERR.
        int socketError = 0;
        if (-1 ==
            pEndpoint_->getSocketOption(SOL_SOCKET, SO_ERROR, socketError)) {
            ERROR_MSG("TCPConnectionOpener::handleErrorNotification( %s ): "
    ...
    void TCPConnectionOpener::handleSocketError(int error)
    {
        ...
        switch (error) {
            ...
            case WSAECONNREFUSED:
                this->cancel();
                listener_.onTCPConnectFailure(*this,
                                              Mercury::REASON_NO_SUCH_PORT);
                break;

            case WSAETIMEDOUT:
                this->cancel();
                listener_.onTCPConnectFailure(*this,
                                              Mercury::REASON_TIMER_EXPIRED);
                break;
            default:
                this->cancel();
                listener_.onTCPConnectFailure(*this,
     ...
     */
    void TCPConnectionOpener::handleTimeout(TimerHandle handle, void* pUser)
    {
