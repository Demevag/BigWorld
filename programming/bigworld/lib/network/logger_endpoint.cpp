 ...
 */
class LoggerEndpointMemberHandler : public Mercury::InputNotificationHandler
{
  ...
  public:
    ...
    {
        return endpoint_.loggerState_ ==
                 LoggerEndpoint::LOGGER_ENDPOINT_ERROR ||
...
// -----------------------------------------------------------------------------
class LoggerEndpointNeedWriteTimeout : public TimerHandler
{
  public:
 ...
 */
void LoggerEndpointNeedWriteTimeout::handleTimeout(TimerHandle handle,
                                                   void*       arg)
{
    ...
    if (loggerEndpoint_.loggerState_ ==
        ...
        loggerEndpoint_.dispatcher_.registerWriteFileDescriptor(
          ...
          loggerEndpoint_.pTcpWriteHandler_,
          "LoggerEndpointTcpWrite");
    }
}
...
class BufferedLoggerPacket
{
  public:
     ...
     *	malloc/free methods
     */
    void operator delete(void* ptr) { bw_free(ptr); }
     ...
     */
    BufferedLoggerPacket* pNext() { return pNext_; }
     ...
     */
    char* data() const { return ((char*)this) + sizeof(*this); }
     ...
     */
    void seek(int offset) { offset_ += offset; }
     ...
     */
    int len() const { return sizeof(int32) + *(int*)(this->data()); }

    /**
     * Create a new BufferedLoggerPacket
     */
    static BufferedLoggerPacket* create(MemoryOStream& ostream)
    {
 ...
 *	@see init
 */
LoggerEndpoint::LoggerEndpoint(Mercury::EventDispatcher& dispatcher,
  ...
  , isSendingExceededBufferedMessage_(false)
{
    ...
    pTcpConnectHandler_ =
      new LoggerEndpointMemberHandler(*this, &LoggerEndpoint::handleConnectTCP);
    pTcpReadHandler_ =
      new LoggerEndpointMemberHandler(*this, &LoggerEndpoint::handleReadTCP);
    pTcpWriteHandler_ =
      new LoggerEndpointMemberHandler(*this, &LoggerEndpoint::handleWriteTCP);

    pUdpReadHandler_ =
      new LoggerEndpointMemberHandler(*this, &LoggerEndpoint::handleReadUDP);
}
...
bool LoggerEndpoint::init(uint16 majorVersion, uint16 minorVersion)
{
    ...
    registerWriteTimer_ = dispatcher_.addTimer(
      registerWriteTimeout, pWriteTimeoutHandler_, NULL, "LoggerEndpointWrite");
...
bool LoggerEndpoint::enableUDP()
{
    ...
    dispatcher_.registerFileDescriptor(
      udpEndpoint_.fileno(), pUdpReadHandler_, "LoggerEndpointUdpRead");
...
void LoggerEndpoint::sendDelete()
{
    ...
    udpEndpoint_.sendto(buf,
                        sizeof(wdm) + pathSize + valueLen,
                        remoteAddress_.port,
                        remoteAddress_.ip);
}
...
void LoggerEndpoint::sendImmediate(MemoryOStream& buffer)
{
    ...
    if (sentVal == size) {
        ...
        PROFILER_LEVEL_COUNTER_DEC(senderBufferLevel, sentVal + sizeof(int32));
        return;
    }
...
bool LoggerEndpoint::reconnect()
{
    ...
    dispatcher_.registerWriteFileDescriptor(
      tcpEndpoint_.fileno(), pTcpConnectHandler_, "LoggerEndpointTcpConnect");
...
bool LoggerEndpoint::send(MemoryOStream& buffer)
{
    ...
    if (!isTCP_) {
        ...
        udpEndpoint_.sendto(
          buffer.data(), buffer.size(), remoteAddress_.port, remoteAddress_.ip);
    ...
    PROFILER_LEVEL_COUNTER_INC(senderBufferLevel,
                               buffer.size() + sizeof(int32));
 ...
 */
bool LoggerEndpoint::handlePacket(char* packet, int packetLength)
{
    ...
    int       pathLen    = 0;
    while (pathLen < (packetLength - pathOffset) && path[pathLen] != 0) {
        ++pathLen;
    }
...
{
    return MessageLogger::versionSupportsMetaData(majorVersion_, minorVersion_);
}
...
int LoggerEndpoint::handleConnectTCP(int fd)
{
    ...
    */
    ...
    int       err         = getsockopt(tcpEndpoint_.fileno(),
                         SOL_SOCKET,
                         SO_ERROR,
                         (char*)&errcode,
                         &errcodesize);
...
WatcherPtr LoggerEndpoint::pWatcher()
{
    ...
    pWatcher->addChild(
      "address", &Mercury::Address::watcher(), &pNull->remoteAddress_);
...
{
    return lhs->addr().ip == rhs.ip && lhs->addr().port == rhs.port;
}
