...
namespace Mercury {
    ...
    When you make a request you put it on the bundle with a 'startRequest'
    message. This means the bundle takes note of it and puts extra information
    (a reply id) in the message's header.
    ...
    */
    ...
    {
        // We check isReliable() because that indicates whether or not a
        // sequence number will be streamed onto this bundle during
        // NetworkInterface::send().
        bool hasData = numMessages_ > 0 || this->hasMultipleDataUnits() ||
                       this->isReliable() || this->hasDataFooters() ||
                       ack_ != SEQ_NULL;
                                 ...
                                 ReliableType            reliable)
    {
        ...
        // NetworkInterface::send().
        ReplyID* pReplyID =
          (ReplyID*)this->newMessage(sizeof(ReplyID) + sizeof(Packet::Offset));

        Packet::Offset messageStart = Packet::Offset(
          pCurrentPacket_->msgEndOffset() -
          (ie.headerSize() + sizeof(ReplyID) + sizeof(Packet::Offset)));
        Packet::Offset nextRequestLink = Packet::Offset(
          pCurrentPacket_->msgEndOffset() - sizeof(Packet::Offset));
    ...
    /**
     *  This method returns true if this UDPBundle is owned by an external
     * channel.
     */
    ...
    void UDPBundle::doFinalise()
    {
        ...
        if (msgBeg_ == NULL &&
            pCurrentPacket_->msgEndOffset() != msgChunkOffset_) {
     ...
     *	@param seqNumAllocator 	The network interface's sequence number
     *allocator, used for off-channel sending.
                                      ...
                                      bool             shouldUseChecksums)
    {
        ...
        for (Packet* pPacket = this->pFirstPacket(); pPacket;
            ...
            // At this point, pPacket->back() is positioned just after the
            // message data, so we advance it to the end of where the footers
            // end, then write backwards towards the message data. We check that
            // we finish up back at the message data as a sanity check.
            ...
            if (pPacket->hasFlags(Packet::FLAG_IS_RELIABLE)) {
                if (pChannel) {
                    ...
                    if (!pChannel->addResendTimer(
                          pPacket->seq(), pPacket, roBeg, roEnd)) {
    ...
    /**
     *  Set up the flags on a packet in accordance with the bundle's
     * information.
     */
    void UDPBundle::writeFlags(Packet* p) const
    {
        // msgIsReliable_ is only set here if there are no msgs (only footers)
        // on the bundle, but the setter wants to indicate that it should still
        // be reliable

        if (reliableOrders_.size() || msgIsReliable_ ||
            numReliableMessages_ > 0) {
    ...
    /**
     * 	This message begins a new message, with the given number of extra bytes
     * in the header. These extra bytes are normally used for request
     * information.
     *
    ...
    void UDPBundle::addReliableOrder()
    {
        ...
        // actual message header.  Otherwise, we have to settle for the part of
        // the message that's on this packet.
        if (msgBeg_ == begInCurWithHeader) {
            begInCur = begInCurWithHeader;
        }

        ReliableOrder reliableOrder = { begInCur,
                                        uint16((uint8*)pCurrentPacket_->back() -
                                               begInCur),
                                        msgIsRequest_ };
                                   ...
                                   const ReliableOrder*& roEnd)
    {
        if (!reliableOrders_.empty()) {
            ...
            if (pFirstPacket_ == pCurrentPacket_) {
                ...
                size_t reliableOrdersExtracted =
                  (roEnd + 1) - &reliableOrders_.front();
     ...
     */
    bool UDPBundle::piggyback(SeqNum                seq,
                              const ReliableVector& reliableOrders,
                              Packet*               p)
    {
        Packet::Flags flags = Packet::FLAG_HAS_SEQUENCE_NUMBER |
        ...
        // We also need to figure out if the dropped packet had piggybacks on
        // it. If so, we need to preserve these on the outgoing packet.  Yes
        // this means the piggyback has piggybacks on it.  Wheeeeee.
        ...
        // It fits, so tag packet with piggyback and reliable flags, because we
        // are about to discard the original packet and therefore can't afford
        // to lose this packet too.
        pCurrentPacket_->enableFlags(Packet::FLAG_HAS_PIGGYBACKS |
                                     Packet::FLAG_IS_RELIABLE |
                                     Packet::FLAG_HAS_SEQUENCE_NUMBER);

        // Don't include the size suffix in the packet length
        BundlePiggyback* pPiggy =
          new BundlePiggyback(p,
                              flags,
                              seq,
        ...
        // Reserve enough footer space for the piggyback.  It's OK to do this
        // late since we've already worked out that this fits on the current
        // packet.
