...
class TestChannelVersionLocalHandler : public CommonHandler
{
  ...
  protected:
    ...
    {
        INFO_MSG(
          "on_msg1: args.seq = %u, args.data = %u\n", args.seq, args.data);
        ++count_;
...
class TestChannelVersionFixture
{
  public:
    TestChannelVersionFixture()
      ...
      , packetReceiver_(networkInterface_.socket(), networkInterface_)
      , endpoint_()
      , pChannel_(NULL)
    ...
    void setupChannel()
    {
        ...
        packetReceiver_.processPacket(
          endpoint_.getLocalAddress(), pPacket.get(), NULL);
    }
  ...
  protected:
    Mercury::SeqNum version() const { return version_; }

    Mercury::SeqNum nextVersion() { return ++version_; }

    Mercury::SeqNum nextSeqNum() { return nextSeqNum_++; }

    void resetSequenceNumber() { nextSeqNum_ = 0; }
...
TEST_F(TestChannelVersionFixture, ChannelVersion)
{
    ...
    args.seq  = 0;
    args.data = 0;
    generators[0].addFixedLengthMessage(
      CommonInterface::msg1.id(), &args, sizeof(args));
    ...
    args.data = 1;
    generators[1].addFixedLengthMessage(
      CommonInterface::msg1.id(), &args, sizeof(args));
    ...
    args.seq  = 0;
    args.data = 0;
    generators[2].addFixedLengthMessage(
      CommonInterface::msg1.id(), &args, sizeof(args));
