...
class TestTimeout : public TimerHandler
{
  public:
    ...
    {
        timerHandle_ =
          dispatcher.addOnceOffTimer(int64(timeoutSeconds * 1000000), this);
    }

    /**
     *	Destructor.
     */
    virtual ~TestTimeout() { this->cancel(); }
    ...
    void cancel() { timerHandle_.cancel(); }
 ...
 */
class WebSocketStreamFilterFactory
...
class ConnectionHelper : public Mercury::TCPConnectionOpenerListener
{
  public:
        ...
        if (useWebSockets_) {
            ...
            Mercury::StreamFilterPtr pWebSockets(
              new Mercury::WebSocketStreamFilter(
                *pChannelStream, "host.example.com", "/"));
     ...
     *	established.
     */
    Mercury::TCPChannel* pChannel() { return pChannel_.get(); }

    /**
     *	This method returns the status of a completed request.
     */
    Mercury::Reason reason() const { return reason_; }
  ...
  , public Singleton<TCPChannelsServerApp>
{
  public:
    ...
    TCPChannelsServerApp(Mercury::EventDispatcher& dispatcher,
                         bool                      useWebSockets,
                         TEST_ARGS_PROTO)
      : NetworkApp(dispatcher, Mercury::NETWORK_INTERFACE_EXTERNAL, TEST_ARGS)
     ...
     */
    void setCipher(Mercury::BlockCipherPtr pCipher) { pCipher_ = pCipher; }
  ...
  private:
    ...
    WebSocketStreamFilterFactory webSocketStreamFilterFactory_;
...
{
    for (Clients::iterator iter = clients_.begin(); iter != clients_.end();
         ++iter) {
 ...
 */
void TCPChannelsServerApp::msg1(
...
{
    TCPChannelsServerInterface::registerWithInterface(this->networkInterface());
...
{
    TCPChannelsClientInterface::registerWithInterface(this->networkInterface());

    ConnectionHelper helper(this->dispatcher(), useWebSockets_);

    Mercury::TCPConnectionOpener opener(
      helper, this->networkInterface(), serverAddress_);
    ...
    pChannel_->bundle().startRequest(TCPChannelsServerInterface::msg2, this);
 ...
 */
void TCPChannelsClientApp::msg1(
  const Mercury::UnpackedMessageHeader&       header,
  const TCPChannelsClientInterface::msg1Args& args)
{
...
TEST(TCPChannelsBasicTesting)
{
    ...
    TCPChannelsClientApp clientApp(
      eventDispatcher, serverApp.tcpServer().address(), false, TEST_ARGS);
...
TEST(CombineBlocks)
{
    ...
    Mercury::BlockCipherPtr pCipher = Mercury::NullCipher::create(BLOCK_SIZE);
...
TEST(TCPChannelsWebSockets)
{
    ...
    TCPChannelsClientApp clientApp(
      eventDispatcher, serverApp.tcpServer().address(), true, TEST_ARGS);
