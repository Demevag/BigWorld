...
TEST(HTTPHeaders)
{
    ...
    BW::string headerBytes = "Host: server.example.com\r\n\
Upgrade: websocket\r\n\
Connection: Upgrade\r\n\
...
Sec-WebSocket-Version: 13\r\n\r\n";
    ...
    CHECK_EQUAL("chat,superchat", headers.valueFor("sec-websocket-protocol"));
}
...
TEST(HTTPRequest)
{
    ...
    BW::string requestBytes = "GET /chat HTTP/1.1\r\n\
...
TEST(HTTPResponse)
{
    ...
    BW::string responseBytes = "HTTP/1.1 101 Switching Protocols\r\n\
Upgrade: websocket\r\n\
Connection: Upgrade\r\n\
Sec-WebSocket-Accept: s3pPLMBiTxaQ9kYGzzhZRbK+xOo=\r\n\r\n";
    ...
    CHECK_EQUAL(responseBytes.data() + responseBytes.size(), responseByteData);
}
  ...
  , public Mercury::WebSocketHandshakeHandler
{
  public:
    ...
    virtual void shutDown() { isConnected_ = false; }
    ...
    BinaryIStream& readBuffer() { return readBuffer_; }

    BinaryOStream& writeBuffer() { return writeBuffer_; }
...
Origin: www.example.com\r\n\r\n";
    ...
    const char*           writeData =
      reinterpret_cast<const char*>(writeBuffer.retrieve(writeBufferLength));
    ...
    BW::string expectedAcceptDigestBase64 = Base64::encode(
      reinterpret_cast<const char*>(expectedAcceptDigest), SHA_DIGEST_LENGTH);
...
TEST(WebSocketStreamFilterClientHandshakeRequest)
{
    ...
    const char*          requestBytes =
      reinterpret_cast<const char*>(writeBuffer.retrieve(requestSize));
    ...
    CHECK(
      request.headers().valueCaseInsensitivelyEquals("upgrade", "websocket"));

    CHECK(
      request.headers().valueCaseInsensitivelyEquals("connection", "upgrade"));
...
TEST(WebSocketStreamFilterConnection)
{
    ...
    TestStreamPtr pTestStreamServer =
      TestStream::create(clientToServerBuffer, serverToClientBuffer);
    TestStreamPtr pTestStreamClient =
      TestStream::create(serverToClientBuffer, clientToServerBuffer);

    Mercury::StreamFilterPtr pServerFilter = new Mercury::WebSocketStreamFilter(
    ...
    Mercury::StreamFilterPtr pClientFilter =
      new Mercury::WebSocketStreamFilter(*pTestStreamClient,
                                         "host.example.com",
                                         "/example/uri",
                                         "origin.example.com");
    ...
    const uint8* maskingKeyBytes =
      reinterpret_cast<const uint8*>(testDataStream.retrieve(sizeof(uint32)));
