...
namespace Mercury {
    ...
    class SymmetricBlockCipher::Impl
    {
      public:
        ...
        bool encryptBlock(const uint8* pInputBuffer, uint8* pOutputBuffer);
        bool decryptBlock(const uint8* pInputBuffer, uint8* pOutputBuffer);
      ...
      , bfKey_()
    {
        ...
        // block size being 64-bit (since it uses uint64's for the XOR
        // operation). If a larger block size is used, the implementation of the
        // XOR operation needs to be modified and this assertion can be changed.
    ...
    {
        BWOpenSSL::BF_set_key(
          &bfKey_,
          static_cast<int>(keyString_.size()),
                                                  ...
                                                  uint8*       pOutputBuffer)
    {
        ...
        BWOpenSSL::BF_ecb_encrypt(
          pInputBuffer, pOutputBuffer, &bfKey_, BF_ENCRYPT);
        return true;
    }
                                                  ...
                                                  uint8*       pOutputBuffer)
    {
        ...
        BWOpenSSL::BF_ecb_encrypt(
          pInputBuffer, pOutputBuffer, &bfKey_, BF_DECRYPT);
        return true;
    }

    /**
     *	This method is a factory method for creating a symmetric block cipher
     *with a generated key.
     *
     *	@param keySize 	The size of the key to generate, which must be between
     *					MIN_KEY_SIZE and MAX_KEY_SIZE bytes.
     *
     *	@return 		NULL if the key size is invalid, otherwise a new
    ...
    {
        if (keyString.size() < MIN_KEY_SIZE ||
            keyString.size() > MAX_KEY_SIZE) {
            ERROR_MSG("SymmetricBlockCipher::create: "
     ...
     *	Constructor with a generated key.
     */
    SymmetricBlockCipher::SymmetricBlockCipher(size_t keySize)
     ...
     *	Override from BlockCipher.
     */
    bool SymmetricBlockCipher::encryptBlock(const uint8* pInputBuffer,
                                            uint8*       pOutputBuffer)
    {
     ...
     *	Override from BlockCipher.
     */
    bool SymmetricBlockCipher::decryptBlock(const uint8* pInputBuffer,
                                            uint8*       pOutputBuffer)
    {
