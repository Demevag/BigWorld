...
namespace // anonymous
{
     ...
     *	writing, upon which the channel buffered data will be attempted to be
     *sent.
     */
    class SendWaiter : public Mercury::InputNotificationHandler
    {
      public:
         ...
         *	Constructor.
         *
         *	@param channel 	The channel to notify when the socket is available
         *for writing.
         */
...
#pragma warning(disable : 4355)
      ...
      , pSendWaiter_(new SendWaiter(*this))
    {
        ...
        this->dispatcher().registerFileDescriptor(
          pEndpoint_->fileno(), this, "TCPChannel");
...
#pragma warning(pop)
    ...
    void TCPChannel::handleInput()
    {
        ...
        while (potentiallyMoreFrames) {
            if (headerState_ == HEADER_STATE_SERVER_WAIT_FOR_HEADER) {
                // For WebSockets, the header would look like the first line of
                // the HTTP handshake - which we can then reject here
                // immediately.
                ...
                // remote side is sending WebSockets frames and the local side
                // is expecting raw TCP frames, because otherwise the
    ...
    /**
     *	This method reads the connection header. This header identifies whether
     *we are looking at a raw TCP connection or a WebSockets connection.
     *
    ...
    bool TCPChannel::readHeader()
    {
        ...
        const char* receivedHeaderData =
          reinterpret_cast<const char*>(pFrameData_->retrieve(0));

        if (0 !=
            memcmp(receivedHeaderData, CLIENT_HEADER, CLIENT_HEADER_SIZE)) {
    ...
    void TCPChannel::processReceivedFrame()
    {
        ...
        Reason             reason = processor.dispatchMessages(
          *this, pNetworkInterface_->interfaceTable());
    ...
    void TCPChannel::cleanUpAfterFrameReceive()
    {
        ...
        // Copy any leftover data to be processed as the start of the next
        // frame.
    ...
    void TCPChannel::doSend(Bundle& bundleUncast)
    {
        ...
        if (pStreamFilter_) {
            pStreamFilter_->writeFrom(
              frameData, Mercury::NetworkStream::WRITE_SHOULD_NOT_CORK);
        } else {
    ...
    bool TCPChannel::hasUnsentData() const
    {
        ...
        return ((pStreamFilter_.get() != NULL) &&
                pStreamFilter_->hasUnsentData());
    }
    ...
    int TCPChannel::maxSegmentSize() const
    {
        ...
        if (-1 ==
            getsockopt(
              pEndpoint_->fileno(), IPPROTO_TCP, TCP_INFO, &info, &infoSize)) {
    ...
    const char* TCPChannel::c_str() const
    {
        ...
        if (pStreamFilter_.exists()) {
            ...
            bw_snprintf(buffer, BUFFER_SIZE, "%s:%s", "tcp", addr_.c_str());
        }
     ...
     *	Override from Channel.
     *
     *	This works by adding an encryption stream filter on top of any
     pre-existing *	filter. Make sure to set lower-level stream filters on the
     channel (e.g. *	WebSocketsStreamFilter) before you set encryption.
     *
    ...
    void TCPChannel::shutDown()
    {
        if ((pStreamFilter_.get() != NULL) &&
            ...
            // shutdown gracefully. When that's done, they'll call shutDown()
            // again and eventually all filters in the stack will return false
            // in didFinishShuttingDown().
            return;
        }
    ...
    void TCPChannel::doShutDown()
    {
        ...
        // TODO: Need to remove this and use shutdown() when it's supported. In
        // its current state, close() is actually a no-op anyway. :(
        pEndpoint_->close();
#elif defined(_WIN32)
        ...
        // And in response, the peer should shutdown their side, and we'll get
        // that in the form of a 0-length read, and can close the endpoint and
        // destroy the channel.
    }
    ...
    /**
     * 	This method reads any available data from the socket's read buffer into
    ...
    bool TCPChannel::writeFrom(BinaryIStream& input, bool shouldCork)
    {
        ...
        int sendResult =
          this->basicSend(input.retrieve(0), numToSend, shouldCork);
        ...
        if (sendResult < numToSend) {
            ...
            const uint8* leftOverData = reinterpret_cast<const uint8*>(
              input.retrieve(numToSend - sendResult));

            if (!pSendBuffer_) {
                pSendBuffer_ = new MemoryOStream(this->maxSegmentSize());

                this->dispatcher().registerWriteFileDescriptor(
                  pEndpoint_->fileno(), pSendWaiter_, "TCPChannel");
            }
     ...
     *successfully (can be zero or less than the given size if there is
    ...
    void TCPChannel::handlePeerDisconnect()
    {
        if (isShuttingDown_) {
            TRACE_MSG(
              "TCPChannel::handlePeerDisconnect( %s ): Shutdown cleanly\n",
              this->c_str());
        } else {
