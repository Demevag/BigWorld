...
namespace Mercury { namespace MachineDaemon {
     ...
     *  message.
     */
    bool ProcessMessageHandler::onProcessMessage(ProcessMessage& pm,
                                                 uint32          addr)
    {
        hasResponded_ = true;
        return false;
                                ...
                                bool              isRegister)
    {
...
#ifdef MF_SERVER
        // Do not call blocking reply handler after registering with bwmachined
        // as other processes can now find us and send other messages.
        ...
        pm.param_ =
          (isRegister ? pm.REGISTER : pm.DEREGISTER) | pm.PARAM_IS_MSGTYPE;
    ...
    Reason deregisterWithMachined(const Address&    srcAddr,
                                  ...
                                  int               id)
    {
        return name.empty() ? REASON_SUCCESS
                            : registerWithMachined(
                                srcAddr, name, id, /*isRegister:*/ false);
    }
    ...
    /**
     *  This method registers a callback with machined to be called when a
     *certain type of process is started.
     *
    ...
    /**
     *  This method registers a callback with machined to be called when a
     *certain type of process stops unexpectedly.
     *
     *	@note This needs to be fixed up if rebind is called on this nub.
     */
     ...
     *	@param ie		The interface element of the callback message. The
     *message must be callable with one parameter of type Mercury::Address.
     ...
     */
    bool IFindInterfaceHandler::onProcessStatsMessage(ProcessStatsMessage& psm,
                                                      uint32               ip)
    {
        if (psm.pid_ == 0) {
            return true;
        ...
        if (!psm.username_.empty() && (psm.username_ != getUsername())) {
    ...
    class FindFirstInterfaceHandler : public IFindInterfaceHandler
    {
      public:
        ...
        /* Override from IFindInterfaceHandler */
        Address result() const /* override */ { return addr_; }
     ...
     *	@param pHandler		Handler to process ProcessStatsMessages responses.
     *If NULL, the default FindFirstInterfaceHandler is used.
     *
     ...
     */
    Reason findInterface(const char*            name,
                         int                    id,
                         Address&               addr,
                         int                    retries,
                         bool                   verboseRetry,
    ...
    bool queryForInternalInterface(u_int32_t& addr)
    {
        ...
        if (ep.getInterfaceAddress("lo", ifaddr) != 0) {
            WARNING_MSG(
              "MachineDaemon::queryForInternalInterface: "
