...
namespace Mercury {
    ...
    int UDPChannel::s_sendWindowWarnThresholds_[] = { INTERNAL_CHANNEL_SIZE / 4,
    ...
    namespace {
        ...
        {
          public:
            UDPChannelWatcherIniter() { UDPChannel::staticInit(); }
        };
     ...
     *
     * 	@param networkInterface	The interface on which to send and receive
     *messages
    ...
    UDPChannel::UDPChannel(NetworkInterface& networkInterface,
      ...
      , roundTripTime_((traits == INTERNAL) ? stampsPerSecond() / 10
                                            : stampsPerSecond())
    ...
    {
        // TODO: Update the thresholds from bw.xml, probably in a global
        // <network> section.
     ...
     *  anonymous channel to the specified address, and if found will mark it as
     * no longer being anonymous and return it.  If not found, the regular
    ...
    /**
     *	This method clears out any objects that this UDPChannel is responsible
     *for.
     */
    void UDPChannel::clearState(bool warnOnDiscard /*=false*/)
    {
        ...
        // Increment the version, since we're not going to be talking to the
        // same channel on the other side anymore.
        if (this->isIndexed()) {
    ...
    /**
     *	This method schedules this channel for deletion once all of its packets
     *have been acked.
     */
    void UDPChannel::condemn()
    {
        ...
        if (this->hasUnsentData()) {
            if (this->isEstablished()) {
                ...
                WARNING_MSG(
                  "UDPChannel::condemn( %s ): "
                  "Unsent data was lost because channel not established\n",
                  this->c_str());
            }
        }

        // Since you aren't going to be actively sending on this channel
        // anymore, it must be marked as irregular.
    ...
    /**
     *	This method checks for overflow errors, checking how large the overflow
     *has become. It warns if the overflow is starting to get large, and assert
     *if it has exceeded MAX_OVERFLOW_PACKETS.
     */
     ...
     *	the source channel.
     */
    void UDPChannel::initFromStream(BinaryIStream& data, const Address& addr)
    {
        ...
        highestAck_ = (oldestUnackedSeq_ != SEQ_NULL)
                        ? seqMask(oldestUnackedSeq_ - 1)
                        : seqMask(smallOutSeqAt_ - 1);
    ...
    /**
     *  This method writes this channel's state to the provided stream so that
     * it can be reconstructed with initFromStream().
     */
    ...
    {
        // Unreliable messages written by the bundle primer are not counted
        // here.
        ...
        return (bundle.numMessages() > primeMessages) ||
               bundle.hasDataFooters() || bundle.isReliable() ||
               !acksToSend_.empty();
    }
    ...
    void UDPChannel::doSend(Bundle& bundleUncast)
    {
        ...
        if (hasRemoteFailed_) {
            ...
            this->networkInterface().cancelRequestsFor(
              *pBundle, Mercury::REASON_CHANNEL_LOST);
        ...
        if (!isSendingOwnBundle) {
            ...
            // channels, it's probably OK to just enable the flag here instead
            // of asserting.  Can't see why we would need to interleave bundles
            // on an indexed channel like that though. Unless we are just
            // pushing acks which will use an empty bundle.
            MF_ASSERT(!this->isIndexed() || pBundle->numMessages() == 0);

            // If this channel uses a bundle primer, then the external bundle
            // won't have been set up correctly.  We don't support sending
            // external bundles on channels with bundle primers yet. Unless we
            // are just pushing acks.
        ...
        IF_NOT_MF_ASSERT_DEV(addr_.ip != 0)
        {
            WARNING_MSG("UDPChannel::doSend(%s): "
                        ...
                        this->c_str());
            this->networkInterface().cancelRequestsFor(
              *pBundle, Mercury::REASON_GENERAL_NETWORK);
        ...
        // calls.
        MF_ASSERT(this->isExternal() || pBundle->numMessages() == 0 ||
                  pBundle->isReliable());

        // If we're sending the channel's bundle and it's empty, just don't do
        // it. It's important to do this after the call to checkResendTimers()
        // so that channels that are marked as regular but don't have any actual
        // data to send will still check their resends when they call this
        // method.
    ...
    /**
     *	This method calls send on this channel if it has not sent for a while
     *and is getting close to causing resends.
     */
    ...
    /**
     *	This method records a packet that may need to be resent later if it is
     *not acknowledged. It is called when a packet is sent on our behalf.
     *
     *	@return false if the window size was exceeded.
     */
    bool UDPChannel::addResendTimer(SeqNum               seq,
                                    Packet*              p,
    ...
    /**
     *	This method removes a packet from the collection of packets that have
     *been sent but not acknowledged. It is called when an acknowledgement to a
     *packet on this channel is received.
     *
    ...
    bool UDPChannel::handleAck(SeqNum seq)
    {
        ...
        if (seq == oldestUnackedSeq_) {
            oldestUnackedSeq_ = SEQ_NULL;
            for (uint i = seqMask(seq + 1); i != largeOutSeqAt_;
                 i      = seqMask(i + 1)) {
                if (unackedPackets_[i]) {
                    oldestUnackedSeq_ = i;
                    break;
        ...
        // If the incoming seq is after the last ack, then it is the new last
        // ack
    ...
    /**
     *	This method resends any unacked packets as appropriate. This can be
     *because of time since last sent, receiving later acks before earlier ones.
     */
    void UDPChannel::checkResendTimers(UDPBundle& bundle)
    {
        ...
        if (hasRemoteFailed_) {
            if (this->networkInterface().isDebugVerbose()) {
                WARNING_MSG(
                  "UDPChannel::checkResendTimers( %s ): "
        ...
        // If we have unacked packets that are getting a bit old, then resend
        // the ones that are older than we'd like.  Anything that has taken more
        // than twice the RTT on the channel to come back is considered to be
        // too old.
        uint64 now = timestamp();
        uint64 resendPeriod =
        ...
        // The idea is to throttle the resends a little in extreme situations.
        // We want to send enough so that no (or not too many) packets are lost
        // but still be able to send more when the RTT is large.
        ...
        for (SeqNum seq = oldestUnackedSeq_;
            ...
            if (pUnacked != NULL) {
                ...
                const bool shouldResend =
                  hasNewerAck ||
                  (isIrregular &&
                   (now - pUnacked->lastSentTime_ > resendPeriod));
    ...
    bool UDPChannel::resend(SeqNum seq, UDPBundle& bundle)
    {
            ...
            (unackedPackets_[smallOutSeqAt_] == NULL)) // Not going to overflow
        {
            if (bundle.piggyback(
                ...
                unacked.wasResent_ =
                  true; // Don't count this for RTT calculations
                this->handleAck(seq);
                return true;
            }
        }

        // If there are any acks on this packet, then they will be resent too,
        // but it does no harm.
    ...
    void UDPChannel::sendUnacked(UnackedPacket& unacked)
    {
        ...
        pNetworkInterface_->sendPacket(addr_,
                                       unacked.pPacket_.get(),
                                       this,
                                       /* isResend: */ true);
      ...
      PacketReceiverStats& stats)
    {
        ...
        if (!p->isPiggyback()) {
            ...
            // Always add the ACKs for others even though a cumulative ack may
            // mean it is not sent. This still shows that the channel is not
            // empty.
        ...
        if (seqLessThan(seq, inSeqAt_)) {
            if (isDebugVerbose) {
                DEBUG_MSG(
                  "UDPChannel::addToReceiveWindow( %s ): "
    ...
    /**
     *	This method returns a string representation of this channel which is
     *useful in output messages.
     *
    ...
    const char* UDPChannel::c_str() const
    {
        ...
        if (this->isIndexed()) {
            length +=
              bw_snprintf(buffer + length, bufferSize - length, "/%d", id_);
        }

        // Annotate condemned channels with an exclamation mark.
        if (isCondemned_) {
            length += bw_snprintf(buffer + length, bufferSize - length, "!");
        }
    ...
    {
        return new UDPBundle((pFilter_ ? pFilter_->maxSpareSize() : 0), this);
    }

    /**
     *  This method will write the flags on a packet fitting for one that will
     * ride on this channel. It will also reserve enough space for the footer.
     */
    void UDPChannel::writeFlags(Packet* p)
    {
        ...
        // Add a cumulative ACK. This indicates that all packets BEFORE a given
        // seq have been received.
        ...
        if (p->hasFlags(Packet::FLAG_IS_RELIABLE) && this->isInternal() &&
            (SeqNum(largeOutSeqAt_) == 0) && (numReliablePacketsSent_ == 0)) {
            p->enableFlags(Packet::FLAG_CREATE_CHANNEL);
        }
    }

    /**
     *  This method will write the appropriate flags on a packet to indicate
     * that it is on this channel. It must be called after writeFlags.
     */
    void UDPChannel::writeFooter(Packet* p)
    {
        ...
        if (p->hasFlags(Packet::FLAG_HAS_ACKS)) {
            // Note: Technically we should start at inSeqAt_ since sequence
            // numbers wrap around but this is rare enough not to worry about
            // (since it still works but is less efficient).
    ...
    /**
     *	This method resets this channel to be as if it had just been
     *constructed. It will deregister the channel (but does not clear the
     *index).
     */
    void UDPChannel::reset(const Address& newAddr, bool warnOnDiscard)
    {
        ...
        // with FLAG_CREATE_CHANNEL, since we don't want to accept any packets
    ...
    void UDPChannel::configureFrom(const UDPChannel& other)
    {
        ...
        // We don't support setting this fields post-construction, so for now,
    ...
    /**
     *	This method returns the number of packets that have not been
     *acknowledge. It includes the packets that have not been sent.
     */
    ...
    /**
     *	This method sets whether this channel sends irregularly and indicates
     *that its resends are managed globally.
     */
    ...
    /**
     *	This method sets whether the remote side of this channel sends
     *regularly. If the remote side sends regularly, resends can be done via
     *NACKS instead of being timer based.
     */
    ...
    SeqNum UDPChannel::useNextSequenceID()
    {
        ...
        if (this->isInternal()) {
            ...
            if (this->isIndexed() && (s_pSendWindowCallback_ != NULL) &&
    ...
    void UDPChannel::setRemoteFailed()
    {
        ...
        // If this channel is anonymous, then no-one else is going to clean it
        // up.
        if (isAnonymous_) {
            INFO_MSG("UDPChannel::setRemoteFailed: "
...
#define ADD_WATCHER(PATH, MEMBER)                                              \
    pWatcher->addChild(#PATH, makeWatcher(&UDPChannel::MEMBER));
            ...
            ADD_WATCHER(numReliableDataUnitsResent, numReliablePacketsSent_);
...
#undef ADD_WATCHER
    ...
    /**
     *	This static method sets the callback associated with the send window
     *usage for an internal, indexed channel exceeding the
     *sendWindowCallbackThreshold.
     */
    ...
    /**
     *	This static method returns the threshold for when the send-window
     *callback is called.
     */
