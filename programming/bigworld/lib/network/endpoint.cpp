...
{
    static struct if_nameindex staticIfList[3] = { { 1, "eth0" },
                                                   { 2, "lo" },
                                                   { 0, 0 } };
                                  ...
                                  uint32&           info)
{
    ...
    for (ctlHeader = CMSG_FIRSTHDR(&errHeader); ctlHeader != NULL;
...
int Endpoint::getBufferSize(int optname) const
{
    ...
    int       rberr =
      getsockopt(socket_, SOL_SOCKET, optname, (char*)&recvbuf, &rbargsize);
...
Mercury::Address Endpoint::getLocalAddress() const
{
    ...
    if (this->getlocaladdress((u_int16_t*)&addr.port, (u_int32_t*)&addr.ip) ==
        -1) {
...
Mercury::Address Endpoint::getRemoteAddress() const
{
    ...
    if (this->getremoteaddress((u_int16_t*)&addr.port, (u_int32_t*)&addr.ip) ==
        -1) {
...
void initNetwork()
{
...
#if defined(_WIN32)
#if defined(USE_OPENSSL)
    if (!BWOpenSSL::CRYPTO_set_mem_functions(OpenSSLMemHooks::malloc,
...
#if !defined(_RELEASE)
    {
        ...
        BWOpenSSL::CRYPTO_get_mem_functions(
          &currentMalloc, &currentRealloc, &currentFree);
