...
std::ostream& operator<<(std::ostream& stream, const Direction3D& t)
{
    char buf[128];
    bw_snprintf(
      buf, sizeof(buf), "(%1.1f, %1.1f, %1.1f)", t.roll, t.pitch, t.yaw);
    stream << buf;
...
namespace Mercury {
    ...
    bool watcherStringToValue(const char* valueStr, Address& value)
    {
        ...
        if (sscanf(valueStr, "%d.%d.%d.%d:%d", &a1, &a2, &a3, &a4, &a5) != 5) {
     ...
     *  These values need to be in the same byte order on both systems so we
     * just use the raw methods.
     */
