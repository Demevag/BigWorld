        ...
        switch (state) {
            ...
            case DP_S_CONV:
                switch (ch) {
                    ...
                    case 'i':
                        handler.onToken(
                          'd', cflags, min, max, flags, 10, vflags);
                        break;
                    case 'o':
                        flags |= DP_F_UNSIGNED;
                        handler.onToken(
                          'o', cflags, min, max, flags, 8, vflags);
                        break;
                    case 'u':
                        flags |= DP_F_UNSIGNED;
                        handler.onToken(
                          'u', cflags, min, max, flags, 10, vflags);
                        break;
                    case 'X':
                        flags |= DP_F_UP;
                    case 'x':
                        flags |= DP_F_UNSIGNED;
                        handler.onToken(
                          'x', cflags, min, max, flags, 16, vflags);
                        break;
                    case 'f':
                        /* um, floating point? */
                        handler.onToken(
                          'f', cflags, min, max, flags, 0, vflags);
                        break;
                    case 'E':
                        flags |= DP_F_UP;
                    case 'e':
                        handler.onToken(
                          'e', cflags, min, max, flags, 0, vflags);
                        break;
                    case 'G':
                        flags |= DP_F_UP;
                    case 'g':
                        handler.onToken(
                          'g', cflags, min, max, flags, 0, vflags);
                        break;
                    case 'c':
                        handler.onToken(
                          'c', cflags, min, max, flags, 0, vflags);
                        break;
                    case 's':
                        handler.onToken(
                          's', cflags, min, max, flags, 0, vflags);
                        break;
                    case 'p':
                        handler.onToken(
                          'p', cflags, min, max, flags, 16, vflags);
                        break;
