...
// 6 bytes to write a diff + 2 bytes to write an output header + 1 bytes saving
// = 9 bytes before we use some diff'd data
static const unsigned int BS = 9;

static weak_checksum_type checksumBytes(const Bytes& buf,
                                        unsigned     begin,
                                        unsigned     length)
{
