...
void XmlSpecialChars::reduce(char* buf)
{
    ...
    for (int i = 0; i < len; ++i) {
        if (buf[i] == '&') {
            ...
            if (specialLen >= 31) {
                WARNING_MSG("XML escape sequence too long (no trailing "
                            "semicolon found) in \'%s\'\n",
                            buf);
                continue;
            }
            ...
            if (strcmp(special, "amp") == 0)
                ...
                if (val < 1 || val > 255)
                    WARNING_MSG(
                      "Invalid char code \'&%s;\' in \'%s\'. Only char codes "
                      "between 1 and 255 are currently supported in \'&#000;\' "
                      "and \'&#x00;\' style escape sequences.\n",
                      special,
                      buf);
                replace = (char)val;
            } else {
            ...
            buf[i] = replace;
            // and shift the remaining of the string, removing the escape
            // sequence
            specialLen--;
...
BW::string XmlSpecialChars::expand(const char* buf)
{
    ...
    for (int i = 0; i < len; ++i) {
        ...
        if (buf[i] == '&')
            ...
            char str[20];
            bw_snprintf(
              str, sizeof(str), "&#%d;", (int)((unsigned char)buf[i]));
            replace = str;
        }
