...
namespace {
    ...
    const uint32 COMPRESSED_HEADER_SZ =
      3; // two magic uint32s plus a uint32 for size
}
 ...
 *	@param pOwner		A smart pointer to the owner of this data. If not NULL,
 *a reference is kept to the owner, otherwise a copy of the data is taken.
 */
BinaryBlock::BinaryBlock(const void* data,
                         size_t      len,
 ...
 *	@param len			Size of the data block
 *	@param allocator	A string describing the allocator of this object.
 *	@param owned		A flag specifying whether or not this class should copy
 *and own the data.
 */
BinaryBlock::BinaryBlock(const void* data,
                         size_t      len,
 ...
 */
BinaryBlock::BinaryBlock(std::istream&   stream,
                         std::streamsize len,
                         const char*     allocator)
  : data_(NULL)
  , len_(len)
...
BinaryPtr BinaryBlock::compress(int level /*= DEFAULT_COMPRESSION*/) const
{
    ...
    if (!largeBuffer) {
        ...
        int result = compress2(
          largeBuffer, &compressedSz, (const Byte*)data_, (uint32)len_, level);
...
BinaryPtr BinaryBlock::decompress() const
{
    ...
    uLongf uncompressSz =
      *((uint32*)data_ + COMPRESSED_HEADER_SZ - 1); // size uncompressed
    ...
    if (!uncompressBuffer) {
        ...
        int    result =
          uncompress(uncompressBuffer, &uncompressSz, rawData, rawDataSz);
        MF_ASSERT(result == Z_OK);
#if ENABLE_RESOURCE_COUNTERS
...
bool BinaryBlock::isCompressed() const
{
    if (len_ <= int(COMPRESSED_HEADER_SZ * sizeof(uint32))) {
        ...
        return *((uint32*)data_) == COMPRESSED_MAGIC1 &&
