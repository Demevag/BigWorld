...
namespace {
    ...
    void checkIsUTF8(const BW::string& filename, uint16 mask)
    {
        ...
        if (!filename.empty()) {
            ...
            // stop when first hit a '/' because we only want to check the
            // leaves
            ...
            // If general purpose(entry.mask) bit 11 is set, the filename and
            // comment must support The Unicode Standard
            if (nonAscii && ((mask & 0x800) == 0)) // not ascii and not utf-8
            {
                WARNING_MSG("ZipFileSystem::openZip the file name (%s) is "
    ...
    /**
     * This returns a string with all slashes normalised to single forward
     * slashes
     */
    ...
    inline typename MapType::iterator fileMapLookup(
      MapType&             fileMap,
      const BW::StringRef& filename)
    {
        ...
        // because it relies on the Windows Shell interface to find the right
        // file
        for (fileIt = fileMap.begin(); fileIt != fileMap.end(); fileIt++) {
            ...
            if (candidateName.equals_lower(mapKey)) {
                ...
                WARNING_MSG(
                  "Case in filename %s does not match case in zip file %s\n",
                  mapKey.c_str(),
                  fileIt->first.to_string().c_str());
                return fileIt;
            }
        }
        return fileMap.end();
     ...
     */
    template <class MapType>
    inline typename MapType::const_iterator fileMapLookupConst(
      const MapType&       fileMap,
      const BW::StringRef& filename)
    {
        // Don't handle bad-case lookups for now
        BW::string mapKey = filename.to_string();
 ...
 *								so we can't rely on offset_ to judge if
 *internalChildNode().
 *
 */
void ZipFileSystem::tag(const BW::StringRef& zipSectionTag,
                        bool                 amInternalChildNode)
{

    // if we're an internal zip file, iterate up to find the root of current
    // zip.
 ...
 *	@param childrenAlreadyIntrospected	if child zipSections are already
 *created, we don't need load the file,like space data
 */
ZipFileSystem::ZipFileSystem(const BW::StringRef& zipFile,
 ...
 *ZipFileSystem::tag_, if it's a root of a zip
 *	@param childrenAlreadyIntrospected	if child zipSections are already
 *created, we don't need load the file,like space data
 */
  ...
  , size_(0)
{
    ...
    bool amInternalChildNode =
      childrenAlreadyIntrospected || parentZip->dirTest(zipSectionTag);
    ...
    if (!amInternalChildNode) // a zip file and need load the file
    {
        ...
        // because we read from the parent zip, we need mutex from the parent
        // zip
        SimpleMutexHolder mtxParent(this->residedZipRoot()->mutex_);

        // we also write to our own temp file, so we need our own file mutex as
        // well
 ...
 */
void ZipFileSystem::getFileOffset(const BW::StringRef& path,
                                  uint32&              fileOffset,
                                  uint32&              fileSize)
{
    if (internalChildNode()) {
        return parentZip_->getFileOffset(
          tag_ + "/" + path, fileOffset, fileSize);
    }
...
void ZipFileSystem::clear(const BW::StringRef& dirPath)
{
    ...
    for (; it != dir.end(); it++) {
        ...
        if (checkDuplicate(filename)) {
            ...
            FileDuplicatesMap::iterator it =
              fileMapLookup(duplicates_, decoded);
...
void ZipFileSystem::beginReadLocalFile(LocalFile& localFile)
{
    ...
    if (localFile.entry_.localHeaderOffset >= 0) {
        MF_VERIFY(fseek(pFile_, localFile.entry_.localHeaderOffset, SEEK_SET) ==
                  0);
...
bool ZipFileSystem::openZip()
{
    ...
    if (fseek(pFile_, size_ - (int)sizeof(footer), SEEK_SET) != 0) {
        ERROR_MSG(
          "ZipFileSystem::openZip Failed to seek to footer (opening %s)\n",
          path_.c_str());
        this->closeZip();
        return false;
    }
    ...
    if (footer.signature != DIR_FOOTER_SIGNATURE) {
        ERROR_MSG(
          "ZipFileSystem::openZip Invalid footer signature (opening %s)\n",
          path_.c_str());
        this->closeZip();
        return false;
    }
    ...
    if (fseek(pFile_, footer.dirOffset, SEEK_SET) != 0) {
        ERROR_MSG("ZipFileSystem::openZip Failed to seek to directory start "
                  "(opening %s)\n",
                  path_.c_str());
        this->closeZip();
        return false;
    ...
    if (footer.dirSize) {
        ...
        dirBlock =
          new BinaryBlock(NULL, footer.dirSize, "BinaryBlock/ZipDirectory");

        if (fread(dirBlock->cdata(), dirBlock->len(), 1, pFile_) != 1) {
            ERROR_MSG("ZipFileSystem::openZip Failed to read directory block "
                      "(opening %s)\n",
                      path_.c_str());
            this->closeZip();
            return false;
    ...
    for (i = 0; i < footer.totalDirEntries; i++) {
        ...
        if (entry.signature != DIR_ENTRY_SIGNATURE) {
            ERROR_MSG("ZipFileSystem::openZip Invalid directory signature "
                      "(opening %s)\n",
                      path_.c_str());
            this->closeZip();
            return false;
        ...
        if (entry.filenameLength > MAX_FIELD_LENGTH) {
            ERROR_MSG(
              "ZipFileSystem::openZip Filename is too long (opening %s)\n",
              path_.c_str());
            this->closeZip();
            return false;
        }
        ...
        if (entry.extraFieldLength > MAX_FIELD_LENGTH) {
            ERROR_MSG(
              "ZipFileSystem::openZip Extra field is too long (opening %s)\n",
              path_.c_str());
            this->closeZip();
            return false;
        }
        ...
        if (entry.fileCommentLength > MAX_FIELD_LENGTH) {
            ERROR_MSG(
              "ZipFileSystem::openZip File comment is too long (opening %s)\n",
              path_.c_str());
            this->closeZip();
            return false;
        }
        ...
        if (checkDuplicate(filename)) {
            ...
            // keep the name.. and add the decoded version to the duplicates
            // list.
        ...
        if (dirIter == dirMap_.end()) {
            ...
            // TODO: should this be stored on the full path not the file
            // component?
            if (checkDuplicate(fileComponent)) {
                pair.first.push_back(fileComponent);
                pair.second.push_back(
                  decodeDuplicate(fileComponent).to_string());
            } else {
                pair.first.push_back(fileComponent);
            ...
            if (checkDuplicate(fileComponent)) {
                ...
                dirIter->second.second.push_back(
                  decodeDuplicate(fileComponent).to_string());
            } else {
                dirIter->second.first.push_back(fileComponent);
 ...
 */
BW::StringRef ZipFileSystem::decodeDuplicate(
  const BW::StringRef& filename) const
{
 ...
 */
BW::string ZipFileSystem::encodeDuplicate(const BW::StringRef& filename,
                                          int                  count) const
{
...
BinaryPtr ZipFileSystem::createZip()
{
    ...
    while (it != centralDirectory_.end()) {
        if ((it)->isValid()) {
            ...
            ++it;
        } else // TODO: note that modifying the central dir means the indices in
               // file map are wrong...
            it =
              centralDirectory_.erase(it); // remove invalid/unitialised files
    }
    BinaryPtr pBinary =
      new BinaryBlock(NULL, totalFileSize, "BinaryBlock/ZipCreate");
    ...
    DirFooter footer;
    memset(&footer, 0, sizeof(DirFooter));
...
{
    FileDuplicatesMap::const_iterator dupe =
      fileMapLookupConst(duplicates_, path);
...
{
    // ToDo: maybe this function should be removed since we made the tag_ and
    // path_ consistant now.
    cleanSlashes(path);
    ...
    if (bExtraChecks) {
        ...
        if (off != BW::string::npos) {
            path = path.substr(MF_MIN(off + dirPath.size() + 1, path.size()),
                               path.size());
        }
...
BinaryPtr ZipFileSystem::readFile(const BW::StringRef& dirPath, uint index)
{
    ...
    BWResource::checkAccessFromCallingThread(dirPath,
                                             "ZipFileSystem::readFile");
    // our own file mutex
    SimpleMutexHolder mtx(mutex_);
 ...
 *	@return A BinaryBlock object containing the file data.
 */
BinaryPtr ZipFileSystem::readFileInternal(const BW::StringRef& inPath,
                                          bool                 uncompressData)
{
    ...
    if (file.entry_.localHeaderOffset <
        0) { // doesnt exist in the file (dummy entry for directory struct)
        return static_cast<BinaryBlock*>(NULL);
    }
    ...
    if (fseek(pFile_, file.dataOffset(), SEEK_SET) != 0) {
        ERROR_MSG("ZipFileSystem::readFile Failed to seek past local header "
                  "(%s in %s)\n",
                  path.c_str(),
                  path_.c_str());
        return static_cast<BinaryBlock*>(NULL);
    ...
    if (file.entry_.compressionMethod != METHOD_STORE &&
        ...
        ERROR_MSG("ZipFileSystem::readFile Compression method %d not yet "
                  "supported (%s in %s)\n",
                  file.entry_.compressionMethod,
                  path.c_str(),
                  path_.c_str());
        return static_cast<BinaryBlock*>(NULL);
    }

    BinaryPtr pCompressedBin =
      new BinaryBlock(NULL, file.entry_.compressedSize, "BinaryBlock/ZipRead");
    pCompressed = (char*)pCompressedBin->data();
    if (file.entry_.compressedSize) {
        if (!pCompressed) {
            ERROR_MSG("ZipFileSystem::readFile Failed to alloc data buffer (%s "
                      "in %s)\n",
                      path.c_str(),
                      path_.c_str());
            return static_cast<BinaryBlock*>(NULL);
...

        {
            ...
            if (fread(pCompressed, 1, file.entry_.compressedSize, pFile_) !=
                file.entry_.compressedSize) {
                ERROR_MSG(
                  "ZipFileSystem::readFile Data read error (%s in %s)\n",
                  path.c_str(),
                  path_.c_str());
                return static_cast<BinaryBlock*>(NULL);
            }
    ...
    if (uncompressData && file.entry_.compressedSize &&
        file.entry_.compressionMethod == METHOD_DEFLATE) {
        ...
        BinaryPtr pUncompressedBin = new BinaryBlock(
          NULL, file.entry_.uncompressedSize, "BinaryBlock/ZipRead");
        ...
        if (inflateInit2(&zs, -MAX_WBITS) != Z_OK) {
            ERROR_MSG(
              "ZipFileSystem::readFile inflateInit2 failed (%s in %s)\n",
              path.c_str(),
              path_.c_str());
            return static_cast<BinaryBlock*>(NULL);
        }
        ...
        if ((r = inflate(&zs, Z_FINISH)) != Z_STREAM_END) {
            ERROR_MSG(
              "ZipFileSystem::readFile Decompression error %d (%s in %s)\n",
              r,
              path.c_str(),
              path_.c_str());
            inflateEnd(&zs);
                                                 ...
                                                 FileInfo*            pFI)
{
    ...
    if (pFI != NULL) {
        ...
        uint64 alltime =
          uint64(_ft.dwHighDateTime) << 32 | uint64(_ft.dwLowDateTime);
#else
        uint32 alltime =
                                                   ...
                                                   FileInfo*            pFI)
{
    ...
    if (file.pData_) {
        if (hdr.compressionMethod == METHOD_DEFLATE) {
            ...
            zs.zfree  = NULL;
            if (inflateInit2(&zs, -MAX_WBITS) != Z_OK) {
                ERROR_MSG("ZipFileSystem::getFileTypeEx: inflateInit2 failed "
                          "(%s in %s)\n",
            ...
            inflateEnd(&zs);
            if (result != Z_OK) {
                ERROR_MSG("ZipFileSystem::getFileTypeEx: Decompression error "
                          "%d (%s in %s)\n",
                          result,
                          path.to_string().c_str(),
                          path_.c_str());
        ...
        if (hdr.uncompressedSize == 0) {
            ...
            while ((readRecords = fread(&input, sizeof(input), 1, pFile_)) ==
                     1 &&
                ...
                if (result != Z_OK) {
                    inflateEnd(&zs);
                    ERROR_MSG("ZipFileSystem::getFileTypeEx: Decompression "
                              "error %d (%s in %s)\n",
                              result,
                              path.to_string().c_str(),
                              path_.c_str());
                ...
                // We decompressed successfully, but still have not gotten
                // enough data
                MF_ASSERT(zs.total_out < sizeof(magic));
                // Inflate is happily advancing and reducing the pointers and
                // length. That's good for the output, but we're reading into
...
{
    return static_cast<uint32>(sizeof(LocalHeader) + diskName().length() * 2 +
                               pData_->len() + sizeof(DirEntry));
}
...
bool ZipFileSystem::LocalFile::writeDirEntry(BinaryPtr dst, uint32& offset)
{
    ...
    entry_.filenameLength =
      (uint16)diskName().length(); // max = MAX_FIELD_LENGTH
...
bool ZipFileSystem::LocalFile::writeDirEntry(FILE* pFile, uint32& offset)
{
    ...
    offset +=
      (uint32)fwrite(diskName().c_str(), 1, entry_.filenameLength, pFile);
    return true;
}
 ...
 *
 */
/*static*/ BinaryPtr ZipFileSystem::LocalFile::compressData(
  const BinaryPtr pData)
{
    int      r;
    z_stream zs;
    memset(&zs, 0, sizeof(zs));

    BinaryPtr pCompressed = new BinaryBlock(
      NULL, pData->len() + 1, "BinaryBlock/ZipUncompressedData");
    ...
    BinaryPtr pBlock = new BinaryBlock(
      pCompressed->cdata(), zs.total_out, "BinaryBlock/ZipCompressedData");
    (void)deflateEnd(&zs);
    return pBlock;
}
                                            ...
                                            bool updateModifiedTime /*= false*/)
{
    ...
    if (!bCompressed_ && pData_) {
        header_.crc32 =
          crc32(header_.crc32, (unsigned char*)pData_->data(), pData_->len());
        bCompressed_ = true;
...
void ZipFileSystem::updateFile(const BW::StringRef& name, BinaryPtr data)
{
    ...
    if (it == fileMap_.end()) // not found (add new)
    {
        ...
        fileMap_[realName].first =
          static_cast<uint32>(centralDirectory_.size() - 1);
    ...
    if (dirIter == dirMap_.end()) // no directory listing found
    {
        ...
        // add the directory index
        fileMap_[realName].second =
          static_cast<uint32>(dirIter->second.first.size());
                              ...
                              bool                 binary)
{
    ...
    // we could have collected all data (via createZip()) then write in one go
    // into pFile_, but in case the file size is too huge and createZip()
    // consumes too much memory, so better use a temp file, and write unit by
    // unit to the temp file, then rename it to path_
    ...
    // writeFile
    for (CentralDir::iterator fileIter = centralDirectory_.begin();
         fileIter != centralDirectory_.end();
         fileIter++) {
        ...
        {
            fileIter->pData_ =
              this->readFileInternal(fileIter->filename_, false);
    ...
    footer.dirSize = (offset - footer.dirOffset);
    offset +=
      static_cast<uint32>(fwrite(&footer, 1, sizeof(footer), pTempFile));
 ...
 *	@return	True if successful
 */
bool ZipFileSystem::writeToStructure(const BW::StringRef& path, BinaryPtr pData)
{
    if (!pData)
        return true;
...
bool ZipFileSystem::eraseFileOrDirectory(const BW::StringRef& filePath)
{
    ...
    if (dirPair.first.size() !=
        dirIndex) // not the last place in the list.. rebuild
    {
...
/**
 *	This method must be only called by nested zip root. It first extracts the
 *file from the Zip file, and then saves it to a temporary file
 *
 *	@param path		Path relative to the base of the filesystem.
 *	@param mode		The mode that the file should be opened in.
 *	@return	the file pointer if successful, otherwise NULL
 */
FILE* ZipFileSystem::openFileFromCurrentZipRoot(const BW::StringRef& path,
                                                const char*          mode)
{
    ...
    if (!pFile) {
        ERROR_MSG("ZipFileSystem::openFileFromCurrentZipRoot: Unable to create "
                  "temp file (%s in %s)\n",
                  path.to_string().c_str(),
                  path_.c_str());
        return NULL;
    ...
    if (fwrite(bin->cdata(), bin->len(), 1, pFile) != 1) {
        ERROR_MSG("ZipFileSystem::openFileFromCurrentZipRoot: temp file write "
                  "failed\n");
        fclose(pFile);
        return NULL;
    }
 ...
 */
FILE* ZipFileSystem::posixFileOpen(const BW::StringRef& path, const char* mode)
{
    if (parentSystem_)
...
/**
 *	This class implements the IFileStreamer interface for streaming from a file
 *in a zip archive
 */
class ZipFileStreamer : public IFileStreamer
{
  public:
    ZipFileStreamer(FILE*          f,
                    size_t         baseOffset,
                    size_t         size,
    ...
    size_t read(size_t nBytes, void* buffer)
    {
        ...
        if ((currentOffset_ + nBytes) <= size_) {
            ...
            ERROR_MSG(
              "ZipFileStreamer::read - Trying to read outside the file\n");
    ...
    bool skip(int nBytes)
    {
        ...
        if (newOffset < 0 || newOffset > int32(size_)) {
    ...
    bool setOffset(size_t offset)
    {
        MF_ASSERT(file_);
        if (offset > size_) {
            ERROR_MSG("ZipFileStreamer::setOffset - Trying to set the offset "
        ...
        if (fileMapLookup(dirMap_, path) == dirMap_.end()) // not a folder
        {
            ...
            if (file.entry_.compressionMethod == METHOD_STORE) {
                if (pFile_ || openZip()) {
                    this->beginReadLocalFile(file);
                    pStreamer =
                      new ZipFileStreamer(pFile_,
                                          file.dataOffset(),
                                          file.entry_.uncompressedSize,
                                          this,
                                          mutex_);
                }
            } else {
                ERROR_MSG(
                  "ZipFileSystem::streamFile - trying to stream compressed "
                  "file %s from zip archive %s, only "
                  "uncompressed files in zip archives can be streamed\n",
                  inPath.to_string().c_str(),
                  path_.c_str());
            }
        } else {
            ERROR_MSG("ZipFileSystem::streamFile - trying to stream a "
                      "directory %s from zip archive %s\n",
                      inPath.to_string().c_str(),
                      path_.c_str());
        }
    }
