...
class ZipFileSystem : public IFileSystem
{
  ...
  private:
    ...
    {
      public:
        FileHandleHolder(ZipFileSystem* zfs)
          : valid_(false)
          , zfs_(zfs)
        {
        ...
        {
#ifdef EDITOR_ENABLED
            // we close the file every time if the editor is enabled to make
            // sure we don't hold the cdata file (preventing the work of bwlockd
            // - committing to the source control)
    ...
    mutable SimpleMutex
      mutex_; // to make sure only one thread access the zlib stream
    ...
    bool              openZip();
    void              closeZip();
    virtual BinaryPtr readFileInternal(const BW::StringRef& path,
                                       bool uncompressData = true);
    void              updateFile(const BW::StringRef& name, BinaryPtr data);
    // return if it's a childNode who is not the root of a nested zip, so
    // basically it's a folder node
    bool internalChildNode() const { return (childNode() && offset_ == 0); }
    ...
    void getFileOffset(const BW::StringRef& path, uint32& offset, uint32& size);
    ...
    FILE*          openFileFromCurrentZipRoot(const BW::StringRef& path,
                                              const char*          mode);
