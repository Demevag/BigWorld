  ...
  FileInfo*            pFI)
{
    BWResource::checkAccessFromCallingThread(
      path, "UnixFileSystem::getAbsoluteFileType");
...
BW::string UnixFileSystem::eventTimeToString(uint64 eventTime)
{
    ...
    /*	sprintf( buf, "%s %s %02d %02d:%02d:%02d %d",
            g_daysOfWeek[etm.tm_wday], g_monthsOfYear[etm.tm_mon+1],
       etm.tm_mday, etm.tm_hour, etm.tm_min, etm.tm_sec, 1900 + etm.tm_year );
     */
    asctime_r(&etm, buf);
    if (buf[24] == '\n')
        buf[24] = 0;
...
BinaryPtr UnixFileSystem::readFile(const BW::StringRef& path)
{
    ...
    BWResource::checkAccessFromCallingThread(path, "UnixFileSystem::readFile");
 ...
 */
FILE* UnixFileSystem::posixFileOpen(const BW::StringRef& path, const char* mode)
{
                                         ...
                                         const BW::StringRef& newPath)
{
    ...
    return rename((basePath_ + oldPath).c_str(),
                  (basePath_ + newPath).c_str()) == 0;
}
