 ...
 *	@return 			None
 */
DirSection::DirSection(const BW::StringRef& fullPath, IFileSystem* pFileSystem)
        ...
        case IFileSystem::FT_DIRECTORY: {
            {
                ...
                DirSection* pDirSection =
                  new DirSection(fullName, pFileSystem_);
        ...
        case IFileSystem::FT_ARCHIVE: {
            {
                ...
                // Disabling this creator because generally we don't want to
                // load a zip as a binary... using the creator would force this
                // at the moment... needs evaluation/fixing

                // ZipSection load via creator is not currently implemented:
                if (creator && creator != ZipSection::creator()) {
                    pSection = creator->load(
                      this, tag, pFileSystem_->readFile(fullName));
                } else {
                    // ZipSections avoid loading the entire file.
                    ZipSection* pZipSection =
                      new ZipSection(fullName, pFileSystem_);
                    pZipSection->tag(tag);
        ...
        case IFileSystem::FT_FILE: {
            {
                ...
                if (pBinary) {
                    ...
                    if (checkRootTag) {
                        // TODO:UNICODE: Make sure this still works with unicode
                        // (utf8) strings as expected.

                        // The following chunk of code is being used to check if
                        // the root is conforming to our standard
                        bool isXML =
                          pBinary->len() > 0 && *pBinary->cdata() == '<';
                        BW::string                         s;
                        static const BW::string::size_type MAX_TAG_NAME_SIZE =
                          128;
                        int offset = 1;
...
bool DirSection::saveChildIfExists(DataSectionPtr pChild, bool isBinary)
{
    ...
    return pFileSystem_->writeFileIfExists(
      fullName, pChild->asBinary(), isBinary);
}
