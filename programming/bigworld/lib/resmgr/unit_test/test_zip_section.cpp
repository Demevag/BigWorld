...
namespace {
     ...
     */
    bool compMemBlocks(void const* block1,
                       size_t      block1Sz,
                       void const* block2,
                       size_t      block2Sz)
    {
...
TEST_F(ResMgrUnitTestHarness, ZipSection_NonExistent)
{
    ...
    DataSectionPtr emptySection = BWResource::openSection("flying_cats");
    CHECK(!emptySection);
}
...
TEST_F(ResMgrUnitTestHarness, ZipSection_ReadNumbers)
{
    ...
    CHECK(
      compMemBlocks(&data[0], sizeof(data), binData->data(), binData->len()));
}
...
TEST_F(ResMgrUnitTestHarness, ZipSection_LotsOfFiles)
{
    ...
    for (size_t i = 0; i < NUM_LOTS_OF_FILES; ++i) {
        BW::string filename =
          "lots_of_files/test_file_" + toString((int)i) + ".zip";
        DataSectionPtr zipSection =
          BWResource::openSection("test", true, ZipSection::creator());
        CHECK(zipSection);
        DataSectionPtr child =
          zipSection->openSection("child", true, BinSection::creator());
        CHECK(child);
    ...
    for (size_t i = 0; i < NUM_LOTS_OF_FILES; ++i) {
        ...
        DataSectionPtr numSection =
          zipSection->openSection("child", false, BinSection::creator());
        ...
        CHECK(compMemBlocks(
          binData->data(), binData->len(), data[i]->data(), data[i]->len()));
    }
...
{
    return compMemBlocks(bin1->data(), bin1->len(), bin2->data(), bin2->len());
}
...
TEST_F(ResMgrUnitTestHarness, ZipSection_TestCreate)
{
    ...
    {
        DataSectionPtr zipSection =
          BWResource::openSection(filename, true, ZipSection::creator());
        CHECK(zipSection);
        ...
        DataSectionPtr testXML =
          xmlChild->openSection(xmlSectionName, true, XMLSection::creator());
        ...
        zipChild =
          zipChild->openSection("zipChild2", true, ZipSection::creator());
        CHECK(zipChild);

        zipChild =
          zipChild->openSection("zipChild3", true, ZipSection::creator());
        CHECK(zipChild);

        // Add some sections with duplicate names
        for (size_t i = 0; i < duplicateCount; i++) {
            DataSectionPtr nestedBin =
              zipChild->newSection("nestedBin", BinSection::creator());
            CHECK(nestedBin);
    ...
    BWResource::instance().purgeAll(); // Clear any cached values
    {
        // Read in the written files and compare the on-disk values with the
        // values in memory.
        ...
        // DataSectionPtr binChild = zipSection->openSection( "binChild", false,
        // BinSection::creator() );
        ...
        // TODO: is creation order meant to be guaranteed with sections of the
        // same name?
        for (size_t i = 0; i < duplicateCount; i++) {
...
TEST_F(ResMgrUnitTestHarness, ZipSection_TestDelete)
{
    ...
    fileSystem->eraseFileOrDirectory(filename);
    {
        ...
        DataSectionPtr child1 =
          zipChild->newSection("child1", BinSection::creator());
        CHECK(child1);
        BinaryPtr bin = randomBlock(random);
        child1->setBinary(bin);
        data.push_back(bin);

        DataSectionPtr child2 =
          zipChild->newSection("child2", BinSection::creator());
        CHECK(child2);
        bin = randomBlock(random);
        child2->setBinary(bin);
        data.push_back(bin);

        DataSectionPtr child3 =
          zipChild->newSection("child3", BinSection::creator());
        CHECK(child3);
        bin = randomBlock(random);
    ...
    BWResource::instance().purgeAll(); // Clear any cached values
    {
        ...
        DataSectionPtr pChild =
          zipChild->newSection("pChild", BinSection::creator());
        CHECK(pChild);
        BinaryPtr bin = randomBlock(random);
            ...
            {
                {
                    ...
                    CHECK(((uint32)blockDS->countChildren()) ==
                          (childCount - 1));
                    CHECK(!checkChildExists(blockDS, "heights"));
                    {
                        DataSectionPtr pSection =
                          blockDS->newSection("heights", BinSection::creator());
                        CHECK(pSection);
                        BinaryPtr bin = randomBlock(random);
...

                {
                    ...
                    {
                        DataSectionPtr pSection =
                          blockDS->newSection("normals", BinSection::creator());
                        CHECK(pSection);
                        BinaryPtr bin = randomBlock(random);
                        pSection->setBinary(bin);
                    }
    ...
    BWResource::instance().purgeAll(); // Clear any cached values
    {
        DataSectionPtr cDataDS =
          BWResource::openSection("testA.cdata", true, ZipSection::creator());
        CHECK(cDataDS);
        // if ( cDataDS )
        {
            DataSectionPtr blockDS =
              cDataDS->newSection("terrain2", ZipSection::creator());
            CHECK(blockDS);
            uint childCount = blockDS->countChildren();
            CHECK(childCount == 0);
            // if ( blockDS )
        ...
        DataSectionPtr pSection =
          cDataDS->newSection("bin1", BinSection::creator());
        CHECK(pSection);
        BinaryPtr bin = randomBlock(random);
        pSection->setBinary(bin);
    ...
    {
        DataSectionPtr cDataDS =
          BWResource::openSection("delete_and_save_result.cdata");
        CHECK(cDataDS);
        // if ( cDataDS )
        {
            DataSectionPtr blockDS =
              BWResource::openSection("delete_and_save_result.cdata/terrain2");
            CHECK(blockDS);
            // if ( blockDS )
            {
                CHECK(checkChildExists(blockDS, "heights"));
...
TEST_F(ResMgrUnitTestHarness, ZipSection_TestConversions)
{
    ...
    BWResource::instance().purgeAll(); // Clear any cached values
    {
        ...
        DataSectionPtr binSection =
          BWResource::openSection("test_bin.bin", true, BinSection::creator());
        CHECK(binSection);

        DataSectionPtr pTest =
          binSection->newSection("test", BinSection::creator());
        CHECK(pTest);
    ...
    BWResource::instance().purgeAll(); // Clear any cached values
    {
        ...
        DataSectionPtr binSection =
          BWResource::openSection("test_binary", true, ZipSection::creator());
        CHECK(binSection);
    }
...
TEST_F(ResMgrUnitTestHarness, ZipSection_TestNestedDirectory)
{
    ...
    BWResource::instance().purgeAll(); // Clear any cached values
    {
        // Load, modify and save a zip file with directories
        DataSectionPtr zipSection =
          BWResource::openSection("test_path/test_dir.zip");
        CHECK(zipSection);
        ...
        DataSectionPtr testXML2 = nestedXMLChild->openSection(
          xmlSectionName, true, XMLSection::creator());
        CHECK(testXML2);
    ...
    BWResource::instance().purgeAll(); // Clear any cached values
    {
        ...
        DataSectionPtr nestedXMLChild =
          zipSection->openSection("zipChild/xmlChild");
        CHECK(nestedXMLChild);
...
/**
 *	This tests creating one zip section, adding a multiple childs and reading
 *them. In addition it testes using multiple res paths Was used to debug and
 *understand performance issues when using multiple res paths (some zipped)
 */
TEST_F(ResMgrUnitTestHarness, ZipSection_TestCreate_And_Read_Specific_Case)
{
    ...
    // Test Create
    {
        ...
        for (size_t i = 0; i < childCount; i++) {
            ...
            DataSectionPtr nestedBin =
              zipChild->newSection(buf, BinSection::creator());
            CHECK(nestedBin);
...
TEST_F(ResMgrUnitTestHarness, ZipSection_TestEmptySubFolder)
{
    ...
    DataSectionPtr zipSection =
      BWResource::openSection(saveAsFilename, true, ZipSection::creator());
    saveAsFilename = saveAsFilename + "_tmp";
    zipSection->save(saveAsFilename);

    CHECK(BWResource::resolveToAbsolutePath(saveAsFilename) !=
    ...
    DataSectionPtr savedSection =
      BWResource::openSection("empty_subfolder.zip_tmp/test_xml_section.xml");
    CHECK(checkBlocks(orgSection->asBinary(), savedSection->asBinary()));
 ...
 *	causing a folder in the contained zip file becomes an empty binary after
 *save.
 */
/* This test fails on server site, I disabled it for the server guys to
investigate further.
...
TEST_F( ResMgrUnitTestHarness, ZipSection_TestZipInZip )
{
    ...
    CHECK( BWResource::resolveToAbsolutePath( saveAsFilename ) !=
IFileSystem::FT_NOT_FOUND); ZipFileSystemPtr fileSystem = new ZipFileSystem(
saveAsFilename );
    ...
    fileSystem = new ZipFileSystem( fileSystem,
"folder1/folder1_1/a_zip_file.zip" );

    IFileSystem::FileType ft1 = fileSystem->getFileTypeEx( "not_empty_folder" );
    //todo: This test failed with empty folder, bug BWT-22513 is created for
this issue.
...
//	fileSystem->writeFile( "zip_in_zip.zip/folder1/folder1_1/a_zip_file.zip",
zipFileSection->asBinary(), true );
}
*/
BW_END_NAMESPACE
