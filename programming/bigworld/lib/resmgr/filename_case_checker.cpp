 ...
 *						new file).
 */
bool FilenameCaseChecker::check(BW::StringRef const& path,
...
)
{
    ...
    if (!wasInCache) {
        if (warnIfNotFound && !cachedEntry.exists_) {
            WARNING_MSG("Filename %s does not exist on disk\n",
                        path.to_string().c_str());
            return true;
        } else if (warnIfNotCorrect && !cachedEntry.matchedCase_) {
            WARNING_MSG("Case in filename %s does not match case on disk %s\n",
                        path.to_string().c_str(),
 ...
 */
void FilenameCaseChecker::filenameOnDisk(BW::StringRef const& path,
                                         BW::string&          out)
{
    SimpleMutexHolder holder(mutex_);
 ...
 */
bool FilenameCaseChecker::caseCorrectFilenameOnDisk(const BW::StringRef& path,
                                                    BW::string&          out,
                                                    bool* isCorrectCase)
{
 ...
 */
const FilenameCaseChecker::FileNameCacheEntry& FilenameCaseChecker::cachePath(
  ...
  bool*                pWasInCache)
{
    ...
    if (!wasInCache) {
        ...
        cacheEntry.exists_ = caseCorrectFilenameOnDisk(
          spath, cacheEntry.resolvedName_, &cacheEntry.matchedCase_);
 ...
 */
bool FilenameCaseChecker::compare(BW::StringRef const& path1,
