...
#define AUTO_SCOPED_ENTITY_PROFILE(ENTITY_PTR)                                 \
    EntityProfiler::AutoScopedHelper<BaseOrEntity> _autoEntityProfile(         \
      ENTITY_PTR)

#define AUTO_SCOPED_THIS_ENTITY_PROFILE AUTO_SCOPED_ENTITY_PROFILE(this)
...
class EntityProfiler
{
  public:
    ...
    {
      public:
        AutoScopedHelper(const ENTITY* pEntity)
          : pEntity_()
        {
            if (pEntity) {
                pEntity_ = pEntity;
