...
bool ManagerAppGateway::init(const char* interfaceName,
                             int         numRetries,
                             float       maxMgrRegisterStagger)
{
    if (!almostZero(maxMgrRegisterStagger)) {
        ...
        // Spread starting time of processes within a tick to avoid possible
        // network peaks during startup
        BWRandom rand(mf_getpid());
        uint32   delay = static_cast<uint>(rand(0.0f, maxMgrRegisterStagger) *
                                         MICROSECONDS_IN_SECOND);

        if (delay > 0) {
            DEBUG_MSG("ManagerAppGateway::init: "
                      "Manager Registration Stagger mode is active: "
                      "maxMgrRegisterStagger: %f s. "
                      "Delaying process start for %f s.\n",
                      maxMgrRegisterStagger,
                      delay / MICROSECONDS_IN_SECOND);
    ...
    Mercury::Reason reason =
      Mercury::MachineDaemon::findInterface(interfaceName, 0, addr, numRetries);
