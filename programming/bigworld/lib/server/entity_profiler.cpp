...
WatcherPtr EntityProfiler::pWatcher()
{
    ...
    if (pWatcher == NULL) {
        ...
        pWatcher->addChild("load", makeWatcher(&EntityProfiler::load));

        pWatcher->addChild("rawLoad", makeWatcher(&EntityProfiler::rawLoad));
                          ...
                          EntityTypeProfiler& typeProfiler)
{
    ...
    currSmoothedLoad_ =
      smoothingFactor * rawLoad + (1.f - smoothingFactor) * currSmoothedLoad_;
...
BinaryIStream& operator>>(BinaryIStream& data, EntityProfiler& profiler)
{
...
BinaryOStream& operator<<(BinaryOStream& data, const EntityProfiler& profiler)
{
    data << profiler.currSmoothedLoad_ << profiler.maxRawLoad_;
