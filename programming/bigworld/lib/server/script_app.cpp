...
class DelayedEvent : public PyObjectPlus
{
    ...
    void addTo(ScriptObject pAddCallbacks)
    {
        ...
        ScriptObject thisScript =
          ScriptObject(this, ScriptObject::FROM_BORROWED_REFERENCE);
        ScriptObject pCallback =
          thisScript.getAttribute("callback", ScriptErrorPrint());
        ScriptObject pErrback =
          thisScript.getAttribute("errback", ScriptErrorPrint());
    ...
    void increment() { ++outstandingDelays_; }
    ...
    void callback(ScriptObject value) { this->decrement(); }
...
bool ScriptApp::triggerDelayableEvent(const char* eventName, PyObject* pArgs)
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        ScriptObject addCallbacks =
          currResult.getAttribute("addCallbacks", ScriptErrorClear());
...
{
    return this->triggerDelayableEvent(
      "onInit", PyTuple_Pack(1, isReload ? Py_True : Py_False));
}
