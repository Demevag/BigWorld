...
namespace ServerUtil {
     ...
     */
    inline Vector2 projectXZ(const Vector3& pos)
    {
    ...
    inline float distSqrBetween(const Vector2& v1, const Vector2& v2)
    {
     ...
     *	positions.
     */
    inline float distSqrBetween(const Vector3& v1, const Vector3& v2)
    {
     ...
     */
    inline int bpsToPacketSize(int bitsPerSecond, int updateHertz)
    {
     ...
     *	@return Byte count
     */
    inline int bpsToBytesPerTick(int bitsPerSecond, int updateHertz)
    {
        return (bitsPerSecond / (NETWORK_BITS_PER_BYTE * updateHertz));
    }
     ...
     *	@return The packet size required to achieve the input data rate.
     */
    inline int packetSizeToBPS(int packetSize, int updateHertz)
    {
    ...
    /**
     *	This function returns the executables full path. This function only
     *works on Linux. On other flavours of Unix, the structure of the /proc
     *filesystem is a bit different.
     */
    inline BW::string exePath()
    {
        pid_t pid = getpid();
