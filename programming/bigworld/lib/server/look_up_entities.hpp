...
class LookUpEntitiesCriteria
{
  public:
    ...
    LookUpEntitiesFilter filter() const { return filter_; }

    void filter(LookUpEntitiesFilter value) { filter_ = value; }

    bool hasSortProperty() const { return !sortProperty_.empty(); }

    const BW::string& sortProperty() const { return sortProperty_; }
    ...
    bool isAscending() const { return isAscending_; }

    void isAscending(bool value) { isAscending_ = value; }

    int limit() const { return limit_; }

    void limit(int value) { limit_ = value; }

    int offset() const { return offset_; }

    void offset(int value) { offset_ = value; }

    const PropertyQueries& propertyQueries() const { return propertyQueries_; }
...
inline BinaryOStream& operator<<(BinaryOStream&                os,
                                 const LookUpEntitiesCriteria& criteria)
{
    return criteria.addToStream(os);
}

inline BinaryIStream& operator>>(BinaryIStream&          is,
