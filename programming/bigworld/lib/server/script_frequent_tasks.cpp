...
class ScriptFrequentTask : public Updatable
{
  public:
    ...
    virtual void update()
    {
        PyObject* pResult = Script::ask(pCallback_.newRef(), PyTuple_New(0));
...
FrequentTaskFini g_frequentTaskFini;
     ...
     *	This method is exposed to scripting. It is used to register a frequent
     task.
     */
    bool addFrequentTask(PyObjectPtr pCallback, int level)
    {
        ...
        ScriptFrequentTask* pTask =
          new ScriptFrequentTask(ScriptObject(pCallback));
     ...
     *
     *	@param callback Specifies the callable to be used. This is a function
     *that takes no parameters.
     *	@param level Optional argument for the level to run the task at. This
     *higher the level, the later the task is executed. Currently the only valid
     *levels are 0 and 1.
     */
    PY_AUTO_MODULE_FUNCTION_WITH_DOC(
      RETOK,
      addFrequentTask,
      ARG(PyObjectPtr, OPTARG(int, 0, END)),
      BigWorld,
      "The function addFrequentTask registers a callback function to be \n"
      "called periodically (at game tick frequency). If the callback ever \n"
      "returns True, it is deregistered and will not be called again.")
