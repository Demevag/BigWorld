...
bool ForwardingCollector::start()
{
    ...
    int forwardingDataSize = pathRequest_.getValueStream()->remainingLength();
...
void ForwardingCollector::handleMessage(const Mercury::Address&         addr,
...
// Overridden from Mercury::ReplyMessageHandler
void ForwardingCollector::handleException(const Mercury::NubException& ne,
                                          void*                        arg)
{
 ...
 * @returns true on success, false on error.
 */
bool ForwardingCollector::ResponseDecoder::packResult(BinaryIStream& stream)
{
    ...
    watcherValueToStream(tempStream, component_.second, Watcher::WT_READ_ONLY);
...
bool ForwardingCollector::ResponseDecoder::stringHandler(BinaryIStream& stream,
                                                         Watcher::Mode  mode)
{
...
// Overriden from WatcherProtocolDecoder
bool ForwardingCollector::ResponseDecoder::tupleHandler(BinaryIStream& stream,
                                                        Watcher::Mode  mode)
{
