...
// calculated num constants in vshader for a hard-skinned mesh (no texture
// transforms):
//  4		WVP
//  2		fog
...
const int PARTICLE_MAX_MESHES = 15; // limited by number of constants in
                                    // vshader.

const float PARTICLE_MESH_MAX_SPIN = 20.f; // since the spin value is clamped
                                           //  0..1, this multiplier allows
                                           //  greater spin speeds.
...
class Particle
{
  public:
    ...
    {
        Vector3
          position_; ///< Position, units in metres. [12 bytes]
        uint32
          colour_; ///< Shade/Tint with R,G,B and Alpha. [4 bytes]
    };
    ...
    Particle(const Vector3& position,
             ...
             float          startingAge   = 0.0f); // non mesh particles only
    ...
    Particle(const Vector3& position,
             ...
             float          startingAge   = 0.0f); // mesh particles only
  ...
  private:
    ...
    uint16 pitchYaw_; ///< Used for storing rotational data. [2 bytes]
                      ///( when a particle bounces, it can have one of 256
                      ///directions in pitch and yaw. )
    ExtraData extraData_; ///< idx, distance for non mesh, packed angle info for
                          ///< meshes [6 bytes]
};
