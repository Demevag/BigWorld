                                  ...
                                  const BoundingBox&  inbb)
{
    ...
    while (it != end) {
        ...
        if (!pMeshTint_) {
            pMeshTint_ =
              Moo::rc().effectVisualContext().getMapping("ParticleMeshTint");
        }
        s_particleMeshTintConstant->tint(
...
void VisualParticleRenderer::serialise(const Serialiser& s) const
{
    ...
    BW_PARITCLE_SERIALISE_NAMED(
      s, VisualParticleRenderer, visual, "visualName_");
}
...
void VisualParticleRenderer::loadInternal(DataSectionPtr pSect)
{
    BW_GUARD;
    this->serialise(
      BW::LoadFromDataSection<VisualParticleRenderer>(pSect, this));
}

void VisualParticleRenderer::saveInternal(DataSectionPtr pSect) const
{
    BW_GUARD;
    this->serialise(BW::SaveToDataSection<VisualParticleRenderer>(pSect, this));
}
 ...
 *	Factory function to create and return a new PyVisualParticleRenderer object.
 *A VisualParticleRenderer is a ParticleSystemRenderer which renders each
 *particle as a visual. Note that it is quicker to use MeshParticle objects
 *exported specifically for use with the MeshParticleRenderer, as they can be
 *drawn 15-at-a-time; however the MeshParticle objects have more limited
 *material functionality.  The VisualParticleRenderer can render any material.
 ...
 *	Constructor.
 */
PyVisualParticleRenderer::PyVisualParticleRenderer(VisualParticleRendererPtr pR,
                                                   PyTypeObject* pType)
...
PyObject* PyVisualParticleRenderer::pyNew(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(args, "|s", &nameFromArgs)) {
        PyErr_SetString(PyExc_TypeError,
                        "VisualParticleRenderer() expects "
