...
void ChunkParticles::tick(float dTime)
{
    ...
    // Update umbra object
    if (!pUmbraDrawItem_) {
        // Set up our unit boundingbox for the attachment, we use a unit
        // bounding box and scale it using transforms since this is a dynamic
        // object.
        UmbraChunkItem* pUmbraChunkItem = new UmbraChunkItem;
        pUmbraChunkItem->init(this,
                              BoundingBox(Vector3(0, 0, 0), Vector3(1, 1, 1)),
                              Matrix::identity,
                              NULL);
    ...
    if (pChunk_ != NULL) {
        if (updated) {
            ...
            if (scale.x != 0.f && scale.y != 0.f && scale.z != 0.f &&
                scale.x < TOO_BIG_FOR_UMBRA && scale.y < TOO_BIG_FOR_UMBRA &&
                scale.z < TOO_BIG_FOR_UMBRA) {
                // Set up our transform, the transform includes the scale of the
                // bounding box
                Matrix m2;
                m2.setTranslate(
 ...
 *	The parameters passed in are ignored, the details coming directly from the
 *ChunkParticle.
 */
void ChunkParticles::drawBoundingBoxes(const BoundingBox& bb,
 ...
 */
void ChunkParticles::worldVisibilityBoundingBox(BoundingBox& bb,
                                                bool clipChunkSize) const
{
    BW_GUARD;
    ...
    if (clipChunkSize) {
        ...
        float scale = max(max(bb.width() / gridSize, bb.depth() / gridSize),
...
bool ChunkParticles::load(DataSectionPtr pSection)
{
    ...
    // Particles used to not have a path, support that by defaulting to
    // particles/
...
/*virtual*/ void ChunkParticles::draw(Moo::DrawContext& drawContext)
{
    ...
    float distance =
      (worldTransform_.applyToOrigin() - Moo::rc().invView().applyToOrigin())
        .length();
 ...
 */
ChunkItemFactory::Result ChunkParticles::create(Chunk*         pChunk,
...
ChunkItemFactory ChunkParticles::factory_("particles",
                                          0,
                                          ChunkParticles::create);
