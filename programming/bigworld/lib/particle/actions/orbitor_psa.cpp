...
void OrbitorPSA::lateExecute(ParticleSystem& particleSystem, float dTime)
{
    ...
    if (particleSystem.particles()->capacity() > indexedOrbits_.size()) {
        ...
        indexedOrbits_.resize(particleSystem.particles()->capacity(),
                              blueprint);
    }
 ...
 *	Factory function to create and return a new PyOrbitorPSA object. OrbitorPSA
 *is a ParticleSystemAction that causes particles to orbit around a point.
...
PyObject* PyOrbitorPSA::pyNew(PyObject* args)
{
    ...
    if (PyTuple_Size(args) > 0) {
        ...
        if (PyTuple_Size(args) > 1) {
            if (Script::setData(PyTuple_GetItem(args, 1),
                                velocity,
