...
void JitterPSA::saveInternal(DataSectionPtr pSect) const
{
    ...
    if (pPositionSrc_) {
        DataSectionPtr pVGSect =
          pSect->newSection(JitterPSAPositionSourceString);
        pPositionSrc_->save(pVGSect);
    }

    if (pVelocitySrc_) {
        DataSectionPtr pVGSect =
          pSect->newSection(JitterPSAVelocitySourceString);
 ...
 *	Factory function to create and return a new PyJitterPSA object. JitterPSA is
 *a ParticleSystemAction that adds random motion to the particles.
...
PyObject* PyJitterPSA::py_setPositionSource(PyObject* args)
{
    ...
    if (PyArg_ParseTuple(args, "O", &positionDesc)) {
        VectorGenerator* vGen = VectorGenerator::parseFromPython(positionDesc);
        if (vGen != NULL) {
