 ...
 */
PyParticleSystem::PyParticleSystem(ParticleSystemPtr pSystem,
                                   PyTypeObject*     pType)
  : PyAttachment(pType)
  , pSystem_(pSystem)
  , lod_(0.0f)
  , actionsHolder_(pSystem->actionSet(), this, true)
{
    ...
    {
        MF_EXIT(
          "PyParticleSystem: failed to allocate memory for particle system.");
    }
}
 ...
 */
void PyParticleSystem::updateAnimations(const Matrix& worldTransform, float lod)
{
    BW_GUARD;
    lod_ = lod;
}
 ...
 *	This accumulates our bounding box into the given variable
 */
void PyParticleSystem::worldBoundingBox(BoundingBox&  bb,
                                        const Matrix& world,
                                        bool          skinny)
{
    BW_GUARD;
 ...
 *	This accumulates our visibility bounding box into the given variable
 */
void PyParticleSystem::worldVisibilityBox(BoundingBox&  wvbb,
                                          const Matrix& world,
                                          bool          skinny)
{
    BW_GUARD;
 ...
 *	this indicates the desired frame rate.  If &lt;=0 then the current frame
 *rate is used.
 *	@type Float
 */
...
PyObject* PyParticleSystem::py_update(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(args, "f", &dTime)) {
        PyErr_SetString(PyExc_TypeError,
                        "PyParticleSystem::py_update expects a dTime");
        return NULL;
    }
...
PyObject* PyParticleSystem::py_render(PyObject* args)
{
    ...
    // temporary solution is to create temp draw context, draw to it and flush
    // immediately
...
PyObject* PyParticleSystem::py_load(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(args, "s", &name)) {
        PyErr_SetString(PyExc_TypeError,
                        "PyParticleSystem::py_load expects a filename");
        return NULL;
    }
...
PyObject* PyParticleSystem::py_save(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(args, "s", &name)) {
        PyErr_SetString(PyExc_TypeError,
                        "PyParticleSystem::py_save expects a filename");
        return NULL;
    }
...
PyObject* PyParticleSystem::py_addAction(PyObject* args)
{
    ...
    if (PyArg_ParseTuple(args, "O", &pNewAction)) {
        if (PyParticleSystemAction::Check(pNewAction)) {
            pSystem_->addAction(
              (static_cast<PyParticleSystemAction*>(pNewAction))->pAction());
            Py_RETURN_NONE;
        }
    }

    PyErr_SetString(PyExc_TypeError,
                    "PyParticleSystem.addAction: "
...
PyObject* PyParticleSystem::py_removeAction(PyObject* args)
{
    ...
    if (PyTuple_Size(args) == 1) {
        ...
        if (PyParticleSystemAction::Check(pFirstItem)) {
            pSystem_->removeAction(
              (static_cast<PyParticleSystemAction*>(pFirstItem))->pAction());
            Py_RETURN_NONE;
        } else {
            int actionTypeID;
