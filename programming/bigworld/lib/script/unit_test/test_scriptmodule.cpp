...
namespace BW {
    ...
    TEST_F(ScriptUnitTestHarness, ScriptModule_ImportMathModule)
    {
        ...
        ScriptObject res =
          pCos.callFunction(ScriptArgs::create(0.0f),
    ...
    // create a module and add attributes to it, then see if they are still
    // there
    TEST_F(ScriptUnitTestHarness, ScriptModule_CreateAModule)
    {
        {
            ScriptModule module =
              ScriptModule::getOrCreate("test_module", ScriptErrorPrint());
            CHECK(module);

            CHECK(module.setAttribute(
              "test", ScriptString::create("test"), ScriptErrorPrint()));
        }
    ...
    {
        ScriptModule module =
          ScriptModule::import("test_script", ScriptErrorPrint());
        CHECK(module);
    ...
    TEST_F(ScriptUnitTestHarness, ScriptModule_AddModule)
    {
        ScriptModule module =
          ScriptModule::getOrCreate("test_module", ScriptErrorPrint());
        CHECK(module);
        CHECK(module.addObject(
          "test_obj", ScriptString::create("test"), ScriptErrorPrint()));
    ...
    TEST_F(ScriptUnitTestHarness, ScriptModule_AddIntConstant)
    {
        ...
        CHECK(module);
        CHECK(
          module.addIntConstant("test_const", testConst, ScriptErrorPrint()));
