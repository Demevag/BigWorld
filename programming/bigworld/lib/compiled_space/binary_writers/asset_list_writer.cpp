...
namespace BW { namespace CompiledSpace {
    ...
    bool AssetListWriter::write(BinaryFormatWriter& writer)
    {
        BinaryFormatWriter::Stream* stream = writer.appendSection(
          AssetListTypes::FORMAT_MAGIC, AssetListTypes::FORMAT_VERSION);
      ...
      StringTableWriter&        stringTable)
    {
        ...
        {
          public:
            Finder(StringTableTypes::Index idx)
              : idx_(idx)
            {
            }
        ...
        auto it =
          std::find_if(assets_.begin(), assets_.end(), Finder(tableIdx));
        if (it == assets_.end()) {
            AssetListTypes::Entry entry = { type, tableIdx };
