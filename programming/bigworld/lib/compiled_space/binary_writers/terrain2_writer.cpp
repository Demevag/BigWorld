...
namespace BW { namespace CompiledSpace {
    ...
    bool Terrain2Writer::write(BinaryFormatWriter& writer)
    {
        ...
        BinaryFormatWriter::Stream* stream = writer.appendSection(
          Terrain2Types::FORMAT_MAGIC, Terrain2Types::FORMAT_VERSION);
    ...
    {
        supportedVersion_ =
          pSpaceSettings->readInt("terrain/version", 0) == 200;
        header_.blockSize_ =
    ...
    // ----------------------------------------------------------------------------
    void Terrain2Writer::addFromChunk(int                   gridX,
                                      int                   gridZ,
                                      ...
                                      const Matrix&         chunkTransform)
    {
        ...
        const AABB blockBB =
          terrainBlockBounds(header_.blockSize_, chunkTransform, pCDataSection);
    ...
    void Terrain2Writer::postProcess()
    {
        ...
        int32 minX = Terrain2Types::pointToGrid(minBounds.x + halfBlockSize,
                                                header_.blockSize_);
        int32 minZ = Terrain2Types::pointToGrid(minBounds.z + halfBlockSize,
                                                header_.blockSize_);
        int32 maxX = Terrain2Types::pointToGrid(maxBounds.x - halfBlockSize,
                                                header_.blockSize_);
        int32 maxZ = Terrain2Types::pointToGrid(maxBounds.z - halfBlockSize,
                                                header_.blockSize_);
        ...
        blockGridLookup_.resize(numGridEntries,
                                        ...
                                        NodeContentsTable&  sceneOctreeContents)
    {
        ...
        float       maxBlocksInADimension =
          max(numBlocksInWidth, max(numBlocksInHeight, numBlocksInDepth));
        ...
        terrainOctree.initialise(
          terrainBounds_.centre(), terrainBounds_.width(), maxOctreeDepth);
