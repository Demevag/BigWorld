...
namespace BW { namespace CompiledSpace {
    ...
    namespace {
        ...
        bool stringEndsWith(const BW::string& value, const BW::string& endsWith)
        {
        ...
        void extractVisualPrimitiveReferences(
          ...
          BW::vector<BW::string>& primitiveReferences)
        {
            ...
            // Load our primitives
            const BW::string primitivesFileName =
              loader.getPrimitivesResID() + '/';
        ...
        void extractModelPrimitiveReferences(
          const BW::string&       filename,
          BW::vector<BW::string>& primitiveReferences)
        {
        ...
        void extractPrimitiveReferences(
          const BW::string&       filename,
          BW::vector<BW::string>& primitiveReferences)
        {
        ...
        const size_t MAX_BUFFER_SIZE =
          (1 << 16) * 64; // 2 ^ 16 - max value indexable by short
                            ...
                            size_t                    bufferSize)
        {
            ...
            // Iterate over all existing buffers looking for a buffer with
            // enough space to fit this one:
            size_t index = 0;
        ...
        bool extractStream(Moo::VertexBuffer                 vb,
                           ...
                           BW::vector<MergedBuffer>*         buffers)
        {
            ...
            memcpy((char*)buffer.buffer_ + buffer.usedBuffer_,
                   vertexLock,
                   bufferSize);
        ...
        void rollbackStream(StaticGeometryTypes::StreamEntry& streamEntry,
                            BW::vector<MergedBuffer>*         buffers)
        {
          ...
          StringTableWriter*                            stringTable)
        {
            ...
            Moo::VerticesPtr pVerts =
              Moo::VerticesManager::instance()->get(primitives, 0);
            if (!pVerts) {
                WARNING_MSG(
                  "Failed to load primitives file '%s'. This data will"
            ...
            entry.originalVertexFormat_ =
              stringTable->addString(pVerts->sourceFormat().c_str());
        ...
        void generateEntries(
          const BW::vector<BW::string>&                 primitiveReferences,
          BW::vector<StaticGeometryTypes::Entry>*       entries,
          BW::vector<StaticGeometryTypes::StreamEntry>* streams,
          BW::vector<Moo::VerticesPtr>*                 loadedData,
          BW::vector<MergedBuffer>*                     buffers,
          StringTableWriter*                            stringTable)
        {
            for (size_t index = 0; index < primitiveReferences.size();
                 ++index) {
                const BW::string& primitiveName = primitiveReferences[index];
                generateEntry(primitiveName,
                              entries,
                              streams,
                              loadedData,
                              buffers,
    ...
    bool StaticGeometryWriter::write(BinaryFormatWriter& writer)
    {
        ...
        BW::vector<StaticGeometryTypes::StreamEntry> streams;
        BW::vector<MergedBuffer>                     buffers;
        generateEntries(primitiveReferences_,
                        &entries,
                        &streams,
                        &loadedData,
                        &buffers,
        ...
        BinaryFormatWriter::Stream* pStream =
          writer.appendSection(StaticGeometryTypes::FORMAT_MAGIC,
