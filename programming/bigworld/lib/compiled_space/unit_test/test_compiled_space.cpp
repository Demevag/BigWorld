...
TEST_F(CompiledSpaceUnitTestHarness, BinaryContainerFormat_WithData)
{
    ...
    BinaryFormatWriter::Stream* pStream1 =
      writer.appendSection(TEST_MAGIC1, TEST_VERSION1);
    CHECK(pStream1 != NULL);
    ...
    BinaryFormatWriter::Stream* pStream2 =
      writer.appendSection(TEST_MAGIC2, TEST_VERSION2);
    CHECK(pStream2 != NULL);
    ...
    BinaryFormatWriter::Stream* pStream3 =
      writer.appendSection(TEST_MAGIC3, TEST_VERSION3);
    CHECK(pStream3 != NULL);
    ...
    BinaryFormatWriter::Stream* pStream4 =
      writer.appendSection(TEST_MAGIC4, TEST_VERSION4);
    CHECK(pStream4 != NULL);
    ...
    BinaryFormatWriter::Stream* pStream5 =
      writer.appendSection(TEST_MAGIC5, TEST_VERSION5);
    CHECK(pStream5 != NULL);
...
TEST_F(CompiledSpaceUnitTestHarness, StringTable)
{
    ...
    CHECK(inputTable.addString("A quick brown fox jumped over the lazy dog.") ==
          3);
...
TEST_F(CompiledSpaceUnitTestHarness, AssetList_Test)
{
    ...
    assetListWriter.addAsset(AssetListTypes::ASSET_TYPE_MODEL,
                             "system/helpers/unit_cube.model",
                             stringTableWriter);

    CHECK(assetListWriter.size() == 1);

    assetListWriter.addAsset(
      AssetListTypes::ASSET_TYPE_TEXTURE, "a/b/blah.tga", stringTableWriter);

    CHECK(assetListWriter.size() == 2);

    assetListWriter.addAsset(AssetListTypes::ASSET_TYPE_EFFECT,
                             "shaders/std_effects/light_only.fx",
                             stringTableWriter);
    ...
    for (size_t i = 0; i < assetList.size(); ++i) {
        ...
        BW::string srcResID =
          stringTableWriter.entry(assetListWriter.assetNameIndex(i));
