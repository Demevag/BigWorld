...
namespace BW { namespace CompiledSpace {
                                           ...
                                           size_t               maxObjects)
    {
        ...
        size_t sectionIdx =
          binFile.findSection(StaticSceneSpeedTreeTypes::FORMAT_MAGIC);
    ...
    // ----------------------------------------------------------------------------
    void StaticSceneSpeedTreeHandler::loadTreeInstances(
      const StringTable& strings,
      SceneObject*       pLoadedObjects,
      size_t             maxObjects)
    {
      ...
      Instance&                             instance)
    {
        ...
        instance.pSpeedTree_->load(
          strings.entry(def.resourceID_), def.seed_, def.worldTransform_);
    ...
    // ----------------------------------------------------------------------------
    // ----------------------------------------------------------------------------
    class StaticSceneSpeedTreeHandler::DrawHandler
    ...
    // ----------------------------------------------------------------------------
    // ----------------------------------------------------------------------------
    class StaticSceneSpeedTreeHandler::CollisionHandler
      : public ICollisionTypeHandler
    {
      ...
      private:
                               ...
                               CollisionState&    cs) const
        {
            ...
            if (pBSP &&
                this->doCollideInternal(
                  sceneObject, pInstance, *pBSP, source, extent, sp, cs)) {
                return true;
            }
    ...
    void StaticSceneSpeedTreeHandler::registerHandlers(Scene& scene)
    {
        ...
        pTextureStreamingOp
          ->addHandler<StaticSceneSpeedTreeHandler::Instance,
