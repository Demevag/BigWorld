...
namespace BW { namespace CompiledSpace {
      ...
      const SceneTypeSystem::RuntimeTypeID& viewTypeID)
    {
        ...
        exposeView<Moo::ISemiDynamicShadowSceneViewProvider>(
      ...
      AABB&                  bb)
    {
        ...
        // on the y=0 plane. (we are going to cull against the column
        // represented)
        Vector3 bottomLeft(static_cast<float>(divisionCoords.x),
                           0.f,
                           static_cast<float>(divisionCoords.y));
        Vector3 topRight(static_cast<float>(divisionCoords.x + 1),
                         0.f,
        ...
        const IntersectSceneView* cullView =
          scene()->getView<IntersectSceneView>();
        MF_ASSERT(cullView);
        ...
        pSpatialQuery->accumulateWorldVisibilityBoundingBoxes(objects, &bb);
    }
