...
void ChunkWaypointStatePath::removeDuplicates()
{
    ...
    while (pathIndex != reversePath_.size()) {
        ...
        if (pathIndex != reversePath_.size() - 1) {
            const Vector3& next = reversePath_[pathIndex + 1].navLoc().point();
  ...
  const ChunkWaypointState&                                  dst)
{
    ...
    while (pNextState != NULL) {
        ...
        std::pair<EdgeIndex, EdgeIndex> edgeVertices =
          astar.searchData().getConnection(pSearchState->navLoc().pSet(),
                                           pSearchState->navLoc().waypoint(),
                                           pNextState->navLoc().pSet(),
    ...
    for (uint32 pathIndex = 0; pathIndex != pathPoints.size(); ++pathIndex) {
        ...
        uint32 reversePathIndex = static_cast<uint32>(reversePath_.size()) -
                                  pathPoints[pathIndex].first - 2;

        // Find the maxY of the segment
        while (prevReverseIndex > reversePathIndex) {
            maxY =
              std::max(reversePath_[prevReverseIndex].navLoc().point().y, maxY);
            --prevReverseIndex;
        }
        ...
        Vector3        newPoint(
          pathPoint.x, std::max(maxY, navLoc.point().y), pathPoint.y);
...
bool ChunkWaypointStatePath::processCorridorPoint(
  const BW::vector<Vector2>& corridorPoints,
  uint32&                    root,
  uint32&                    funnelLeft,
  uint32&                    funnelRight,
  uint32&                    current)
{
    ...
    if (almostEqual(corridorPoints[root], corridorPoints[funnelLeft])) {
        root = funnelLeft;
        return true;
    }

    if (almostEqual(corridorPoints[root], corridorPoints[funnelRight])) {
        root = funnelRight;
        return true;
    }
    ...
    if (almostEqual(corridorPoints[current], corridorPoints[current - 2]) &&
        current != corridorPoints.size() - 1) {
        return true;
    }

    RelativePosition relativeToFunnel =
      ChunkWaypointStatePath::isPointInsideFunnel(corridorPoints[root],
                                                  corridorPoints[funnelLeft],
                                                  corridorPoints[funnelRight],
                                                  corridorPoints[current]);
    ...
    switch (relativeToFunnel) {
        ...
        // The funnel has a zero angle, so use the closer of the two ends as the
        // new root.
        case ON_LINE: {
            Vector2 leftVector =
              corridorPoints[funnelLeft] - corridorPoints[root];
            Vector2 rightVector =
              corridorPoints[funnelRight] - corridorPoints[root];
 ...
 */
bool ChunkWaypointStatePath::funnel(
  ...
  BW::vector<std::pair<uint32, Vector2>>& pathPoints)
{
    ...
    for (uint32 corridorIndex = 0; corridorIndex != corridor.size();
         ++corridorIndex) {
        corridorPoints[corridorIndex * 2 + 1] = corridor[corridorIndex].first;
        corridorPoints[corridorIndex * 2 + 2] = corridor[corridorIndex].second;
    }
    ...
    while (prevRoot != root) {
        ...
        if (root != 0) {
            ...
            if (!pathPoints.empty()) {
                ...
                if (almostEqual(corridorPoints[root], prevPathPoint.second)) {
                    shouldAddRoot = false;
                }
            }
            ...
            if (shouldAddRoot) {
                pathPoints.push_back(
                  std::make_pair(corridorIndex, corridorPoints[root]));
            }
        }
        ...
        for (uint32 current = funnelRight + 1;
             ...
             ++current) {
            if (!processCorridorPoint(
                  corridorPoints, root, funnelLeft, funnelRight, current)) {
                return false;
            }
        }
    }
                                            ...
                                            NavLoc*        outNavLoc) const
{
    ...
    for (BW::vector<ChunkWaypointState>::const_reverse_iterator iter =
 ...
 */
void ChunkWaypointStatePath::moveStateAlongPath(const ChunkWaypointState& state)
{
    const ChunkWaypointState* nextState = this->pNext();
    ...
    BW::vector<ChunkWaypointState>::const_reverse_iterator nextOnFullPath =
      (stateIterOnFullPath_ + 1);
