 ...
 */
void ChunkNavigator::tryGrid(GirthGridList*              gridList,
                             ...
                             NavigatorFindResult&        res) const
{
    ...
    if (uint(x) < GIRTH_GRID_SIZE && uint(z) < GIRTH_GRID_SIZE) {
        gridList[x + z * GIRTH_GRID_SIZE].findNonVisited(
          &chunk_, point, bestDistanceSquared, res);
    }
}
 ...
 */
bool ChunkNavigator::find(const WorldSpaceVector3& wpoint,
                          float                    girth,
                          ...
                          bool                     ignoreHeight) const
{
    ...
    if (git != girthGrids_.end()) {
        ...
        for (iter = ChunkWaypoint::s_visitedWaypoints_.begin();
             iter != ChunkWaypoint::s_visitedWaypoints_.end();
             ++iter) {
            (*iter)->visited_ = 0;
        }
...
bool ChunkNavigator::findExact(const WorldSpaceVector3& wpoint,
                               float                    girth,
                               NavigatorFindResult&     res) const
{
...
void ChunkNavigator::add(ChunkWaypointSet* pSet)
{
    ...
    if (git == girthGrids_.end()) {
        ...
        gridList             = girthGrid.grid =
          new GirthGridList[GIRTH_GRID_SIZE * GIRTH_GRID_SIZE];
    }
...
{
    ChunkWaypointSets::iterator found =
      std::find(wpSets_.begin(), wpSets_.end(), pSet);
