...
struct Fixture
{
    ...
    ~Fixture() { bw_safe_delete(triangle_); }
...
TEST_F(Fixture, WorldTri_Equality)
{
    ...
    WorldTriangle wt2(
      Vector3::zero(), Vector3(0.0f, 0.0f, 1.0f), Vector3(0.0f, 1.0f, 0.0f));
...
TEST_F(Fixture, WorldTri_Intersects)
{
    ...
    CHECK_EQUAL(true,
                triangleA.intersects(triangleB, Vector3(0, -0.91617918, 0)));
    ...
    CHECK_EQUAL(true,
                triangleA.intersects(
                  triangleB, Vector3(0.00000000, -0.78094482, 0.00000000)));

    // Test case for when the triangle prism intersects exactly with one of the
    // points in the collision triangle
    triangleA =
      WorldTriangle(Vector3(2, 0, 3), Vector3(1, 0, 1), Vector3(3, 0, 1));

    triangleB =
      WorldTriangle(Vector3(0, 7, 4), Vector3(0, 7, 0), Vector3(4, -7, 2));

    CHECK_EQUAL(true,
                triangleA.intersects(
                  triangleB, Vector3(0.00000000, -0.78094482, 0.00000000)));

    // Test case for when the start of the volume intersects with the plane of
    // the test triangle but the test triangle does not intersect the test
    // volume The 2d projection that is used for the collision does intersect
    triangleA =
      WorldTriangle(Vector3(0, 0, 0), Vector3(1, 0, 1), Vector3(1, 0, 0));
    ...
    CHECK_EQUAL(
      false,
      triangleA.intersects(triangleB, Vector3(0.00000000, 0.6f, 0.00000000)));
