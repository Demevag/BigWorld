...
{
  public:
    LineV3Gen(const Vector3& start, const Vector3& end);
...
{
  public:
    SphereV3Gen(const Vector3& centre, float maxRadius, float minRadius = 0.f);
    virtual void generate(Vector3& vec) const;
...
{
  public:
    BoxV3Gen(const Vector3& min, const Vector3& max);
    virtual void generate(Vector3& vec) const;
