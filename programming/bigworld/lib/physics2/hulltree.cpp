 ...
 */
void HullTree::addPlane(const PlaneEq& plane,
                        Portal3D&      outline,
                        HullTree*&     firstMarked)
{
    ...
    if (!(anySide[0] | anySide[1])) {
        ...
        // dprintf( "Plane (%f,%f,%f) > %f is on top of (%f,%f,%f) > %f, sameDir
        // %d\n", 	plane.normal().x, plane.normal().y, plane.normal().z,
        //plane.d(), 	divider_.normal().x, divider_.normal().y,
        //divider_.normal().z, divider_.d(), 	sameDir );
    ...
    if (anySide[0] && anySide[1]) {
        ...
        // [ out, in ] [ max, min ]
        float fineVals[2][2] = { { -1000000.f, 1000000.f } , { -1000000.f,
        1000000.f } }; int fineIdxs[2][2] = { { -1, -1 }, { -1, -1 } };
        ...
        */
        ...
        bool pointingSameWay =
          divider_.normal().dotProduct(plane.normal()) > 0.f;
...
const HullContents* HullTree::testPoint(const Vector3& point) const
{
    ...
    int outBack;
    for (const HullTree* pLook = this; pLook != NULL;
...
void HullTree::print(int depth, bool back) const
{
    ...
    dprintf("%c Plane (%f,%f,%f) > %f",
            ...
            divider_.d());
    if (!tagFront_.empty())
        dprintf(" Ftag %" PRIzu "", tagFront_.size());
    if (!tagBack_.empty())
        dprintf(" Btag %" PRIzu "", tagBack_.size());
    if (marked_ & 1)
        dprintf(" BMARK");
    if (marked_ & 2)
        dprintf(" FMARK");
    ...
    dprintf("\n");
    if (pFront_ != NULL)
        pFront_->print(depth + 1, false);
    if (pBack_ != NULL)
        pBack_->print(depth + 1, true);
}
