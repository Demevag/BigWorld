 ...
 *	code.
 */
const char* ServerConnection::transportToCString(ConnectionTransport transport)
{
    switch (transport) {
 ...
 */
ServerConnection::ServerConnection(LoginChallengeFactories& challengeFactories,
  ...
  FIRST_AVATAR_UPDATE_MESSAGE(ClientInterface::avatarUpdateNoAliasDetailed.id())
  , LAST_AVATAR_UPDATE_MESSAGE(
      ClientInterface::avatarUpdateAliasNoPosNoDir.id())
  , isEarly_(false)
{
    this->pInterface(new Mercury::NetworkInterface(
      &dispatcher_, Mercury::NETWORK_INTERFACE_EXTERNAL)),
      // Timer for updating statistics
      timerHandle_ = this->dispatcher().addTimer(
        static_cast<int>(UPDATE_STATS_PERIOD * 1000000),
        this,
        NULL,
        "ServerConnection");
...
{
    NOTICE_MSG(
      "ServerConnection::logOn: This method has been deprecated as "
    ...
    LoginHandlerPtr pLoginHandler =
      this->logOnBegin(serverName, username, password, port);
 ...
 *						ConnectionTransport.
 */
LoginHandlerPtr ServerConnection::logOnBegin(const char*         serverName,
                                             const char*         username,
                                             const char*         password,
                                             uint16              port,
                                             ConnectionTransport transport)
{
    ...
    LogOnParamsPtr pParams =
      new LogOnParams(username, password, pBlockCipher_->key());
    ...
    TRACE_MSG("ServerConnection::logOnBegin: "
              ...
              serverName,
              this->transportToCString(transport),
 ...
 */
LogOnStatus ServerConnection::logOnComplete(LoginHandlerPtr       pLoginHandler,
                                            ServerMessageHandler* pHandler)
{
    ...
    if ((status == LogOnStatus::LOGGED_ON) && !this->isOnline()) {
        ...
        if (result.serverAddr != this->addr()) {
            ...
            WARNING_MSG(
              "ServerConnection::logOnComplete: "
 ...
 */
void ServerConnection::addMove(EntityID          id,
                               SpaceID           spaceID,
                               EntityID          vehicleID,
                               ...
                               const Position3D& globalPos)
{
    ...
    if (id == id_) {
        ...
        // TODO: When on a vehicle, the reference number is not used and so does
        // not need to be sent (and remembered).
...
bool ServerConnection::processInput()
{
    ...
    if (gotAnyPackets) {
        ...
        if (lastReceiveTime_ != 0) {
            uint64 delta = (currTimeStamp - lastReceiveTime_) * uint64(1000) /
                           stampsPerSecond();
 ...
 */
void ServerConnection::entityMethod(
 ...
 */
void ServerConnection::entityProperty(
  const Mercury::Address&               srcAddr,
  const Mercury::UnpackedMessageHeader& header,
  BinaryIStream&                        stream)
{
 ...
 */
int ServerConnection::entityMethodGetStreamSize(Mercury::MessageID msgID) const
{
    MF_ASSERT(pHandler_);
    ...
    return pHandler_->getEntityMethodStreamSize(
      selectedEntityID_, exposedMethodID, isEarly_);
}
  ...
  Mercury::MessageID msgID) const
{
    ...
    return pHandler_->getEntityPropertyStreamSize(
      selectedEntityID_, exposedPropertyID, isEarly_);
}
  ...
  const ClientInterface::relativePositionReferenceArgs& args)
{
    referencePosition_ = ::BW_NAMESPACE calculateReferencePosition(
      sentPositions_[args.sequenceNumber]);
}
 ...
 */
void ServerConnection::setVehicle(const ClientInterface::setVehicleArgs& args)
{
  ...
  const ClientInterface::avatarUpdateNoAliasDetailedArgs& args)
{
    ...
    if (pHandler_ != NULL) {
        pHandler_->onEntityMoveWithError(id,
                                         spaceID_,
                                         vehicleID,
                                         args.position,
                                         Vector3::zero(),
...
#define AVATAR_UPDATE_GET_POS_FullPos                                          \
    ...
    args.position.getXYZError(                                                 \
      posError.x, posError.y, posError.z, packedXZScale_);
...
#define AVATAR_UPDATE_GET_POS_ORIGIN                                           \
    const Position3D& originPos =                                              \
      (vehicleID == NULL_ENTITY_ID) ? referencePosition_ : Position3D::zero();
  ...
  const ClientInterface::detailedPositionArgs& args)
{
    ...
    if (pHandler_ != NULL) {
        pHandler_->onEntityMoveWithError(selectedEntityID_,
                                         spaceID_,
                                         vehicleID,
                                         args.position,
                                         Vector3::zero(),
  ...
  const ClientInterface::forcedPositionArgs& args)
{
    ...
    if (args.id == id_) {
        if ((spaceID_ != NULL_SPACE_ID) && (spaceID_ != args.spaceID) &&
            (pHandler_ != NULL)) {
    ...
    if (pHandler_ != NULL) {
        pHandler_->onEntityMoveWithError(args.id,
                                         args.spaceID,
                                         args.vehicleID,
                                         args.position,
                                         Vector3::zero(),
                                                ...
                                                const Position3D& position)
{
    ...
    if ((id == id_) && (vehicleID == NULL_ENTITY_ID)) {
        referencePosition_ =
          ::BW_NAMESPACE calculateReferencePosition(position);
    }
#endif /* !VOLATILE_POSITIONS_ARE_ABSOLUTE */
}
...
void ServerConnection::bandwidthFromServer(int bandwidth)
{
    if (s_bandwidthFromServerMutator == NULL) {
        ERROR_MSG(
          "ServerConnection::bandwidthFromServer: Cannot comply "
...
{
    return pInterface_ ? pInterface_->receivingStats().bitsPerSecond() : 0.0;
}
...
{
    return pInterface_ ? pInterface_->sendingStats().bitsPerSecond() : 0.0;
}
...
{
    return pInterface_ ? pInterface_->receivingStats().packetsPerSecond() : 0.0;
}
...
{
    return pInterface_ ? pInterface_->sendingStats().packetsPerSecond() : 0.0;
}
...
{
    return pInterface_ ? pInterface_->receivingStats().messagesPerSecond()
                       : 0.0;
}
...
{
    return pInterface_ ? pInterface_->sendingStats().messagesPerSecond() : 0.0;
}
...
{
    double total = numMovementBytes_.getRateOfChange() +
...
#if !VOLATILE_POSITIONS_ARE_ABSOLUTE
    newMovementBytes += ifTable[ClientInterface::relativePositionReference.id()]
                          .numBytesReceived();
    newMovementBytes +=
      ifTable[ClientInterface::relativePosition.id()].numBytesReceived();
#endif /* VOLATILE_POSITIONS_ARE_ABSOLUTE */

    newMovementBytes +=
      ifTable[ClientInterface::setVehicle.id()].numBytesReceived();
    newMovementBytes +=
      ifTable[ClientInterface::forcedPosition.id()].numBytesReceived();

    for (int i = FIRST_AVATAR_UPDATE_MESSAGE; i <= LAST_AVATAR_UPDATE_MESSAGE;
         i++) {
 ...
 */
void ServerConnection::setArtificialLoss(float lossRatio,
                                         float minLatency,
                                         float maxLatency)
{
                                                   ...
                                                   double   currentTime)
{
    ...
    timeAtSequenceStart_ =
      currentTime - double(tickByte_) / pConnection_->updateFrequency();
}
 ...
 */
double ServerConnection::ServerTimeHandler::serverTime(double clientTime) const
{
    if (!hasReceivedGameTime_) {
        return 0.0;
    }
    ...
    double srvTime =
      (gameTimeAtSequenceStart_ / pConnection_->updateFrequency()) +
...
WatcherPtr ServerConnection::pWatcher()
{
    ...
    if (!pWatcher.hasObject()) {
        ...
        pWatcher->addChild("bps in", makeWatcher(&ServerConnection::bpsIn));
        pWatcher->addChild("bps out", makeWatcher(&ServerConnection::bpsOut));
        ...
        pWatcher->addChild(
          "Messages out", makeWatcher(&ServerConnection::messagesPerSecondOut));

        pWatcher->addChild("Expected Freq",
                           makeWatcher(&ServerConnection::updateFrequency));

        pWatcher->addChild(
          "Game Time", makeWatcher(&ServerConnection::gameTimeOfLastMessage));

        pWatcher->addChild(
          "Movement pct", makeWatcher(&ServerConnection::movementBytesPercent));
        pWatcher->addChild(
          "Non-move pct",
          makeWatcher(&ServerConnection::nonMovementBytesPercent));
        pWatcher->addChild(
          "Overhead pct", makeWatcher(&ServerConnection::overheadBytesPercent));
        ...
        pWatcher->addChild("Latency", makeWatcher(&ServerConnection::latency));
        ...
        pWatcher->addChild(
          "Interface", interfaceWatcher, &(pNull->pInterface_));
    }
 ...
 */
void ServerConnection::tickSync(const ClientInterface::tickSyncArgs& args)
{
 ...
 */
void ServerConnection::setGameTime(const ClientInterface::setGameTimeArgs& args)
{
...
void ServerConnection::createBasePlayer(BinaryIStream& stream)
{
    ...
    if (pHandler_) { // just get base data here
        pHandler_->onBasePlayerCreate(id_, playerType, stream);
    }
}
...
void ServerConnection::createCellPlayer(BinaryIStream& stream)
{
    ...
    if (pHandler_) {
        ...
        pHandler_->onCellPlayerCreate(
          id_, spaceID_, vehicleID, pos, dir.yaw, dir.pitch, dir.roll, stream);
    }
...
void ServerConnection::spaceData(BinaryIStream& stream)
{
    ...
    TRACE_MSG(
      "ServerConnection::spaceData(%d): space %u key %hu\n", id_, spaceID, key);
 ...
 */
void ServerConnection::enterAoI(EntityID id,
                                IDAlias  idAlias,
                                EntityID vehicleID)
{
...
void ServerConnection::createEntity(BinaryIStream& rawStream)
{
    ...
    if (pHandler_) {
        pHandler_->onEntityCreate(
          id, type, spaceID_, vehicleID, pos, yaw, pitch, roll, stream);
    }
 ...
 *	BaseApps.
 */
void ServerConnection::switchBaseApp(
  const Mercury::Address&                   srcAddr,
  const Mercury::UnpackedMessageHeader&     header,
  const ClientInterface::switchBaseAppArgs& args)
{
    ...
    LoginHandlerPtr pLoginHandler =
      new LoginHandler(this, LogOnStatus::NOT_SET);
...
void ServerConnection::resourceFragment(BinaryIStream& stream)
{
    ...
    DownloadSegment* pSegment =
      new DownloadSegment((char*)stream.retrieve(length), length, args.seq);
