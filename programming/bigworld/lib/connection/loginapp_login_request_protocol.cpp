 ...
 *	Override from LoginRequestProtocol.
 */
void LoginAppLoginRequestProtocol::onAttemptsExhausted(
 ...
 *	Override from LoginRequestProtocol.
 */
void LoginAppLoginRequestProtocol::sendRequest(LoginRequestTransport& transport,
                                               LoginRequest&          request,
                                               float timeout) const
{
    BW_GUARD;
    ...
    if (request.loginHandler().pChallengeData()) {
        ...
        bundle.addBlob(pResponse->retrieve(0), pResponse->remainingLength());
    }

    bundle.startRequest(LoginInterface::login,
                        &request,
                        NULL,
                        int(timeout * 1000000),
                        Mercury::RELIABLE_NO);
                                           ...
                                           BinaryIStream& data) const
{
    ...
    if (status != LogOnStatus::LOGGED_ON) {
        ...
        INFO_MSG("LoginAppLoginRequestProtocol::onReply: "
                 ...
                 request.attemptNum(),
                 status,
                 serverMessage.empty() ? "none" : serverMessage.c_str());

        if (serverMessage.empty()) // this really shouldn't happen
        {
            serverMessage = (status == LogOnStatus::LOGIN_CUSTOM_DEFINED_ERROR)
                              ? "Unspecified error"
                              : "Unelaborated error";
        }
    ...
    if (pServerConnection->pBlockCipher().exists()) {
        ...
        Mercury::EncryptionFilterPtr pFilter =
          Mercury::EncryptionFilter::create(pServerConnection->pBlockCipher());
