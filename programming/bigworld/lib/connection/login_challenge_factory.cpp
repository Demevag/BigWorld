...
class DelayLoginChallengeFactory : public LoginChallengeFactory
{
  ...
  public:
    ...
    {
        duration_ = static_cast<float>(config.getDouble("duration", duration_));
...
void LoginChallengeFactories::registerDefaultFactories()
{
    ...
    this->registerFactory("cuckoo_cycle", new CuckooCycleLoginChallengeFactory);
}
...
void LoginChallengeFactories::addWatchers(WatcherPtr pWatcherRoot)
{
    ...
    while (iter != map_.end()) {
        ...
        pWatcherRoot->addChild(
          iter->first.c_str(), pWatcher, (void*)iter->second.get());
