...
uint8        flags;
END_STRUCT_MESSAGE();
MERCURY_ISTREAM(avatarUpdateExplicit, x.vehicleID >> x.pos >> x.dir >> x.flags);
MERCURY_OSTREAM(avatarUpdateExplicit, x.vehicleID << x.pos << x.dir << x.flags);
...
MERCURY_ISTREAM(avatarUpdateWardExplicit,
                x.ward >> x.spaceID >> x.vehicleID >> x.pos >> x.dir >>
                  x.flags);
