...
// Matches colour tags for colourFormatting mode. Returns the number of
// characters to skip to get passed the tag, otherwise 0 if there was no match.
int FontMetrics::matchColourTag(const BW::wstring& str,
                                size_t             offset,
                                uint32&            outColour)
{
    BW_GUARD;
    ...
    static size_t         colourTagLength =
      colourTagPrefixLen + wcslen(L"FFFFFFFF") + colourTagPostfixLen;
    ...
    outColour =
      ((outColour >> 8) & 0xFFFFFF) | ((outColour << 24) & 0xFF000000);
 ...
 *	@param	wordBreak					The chars that could be used as
 *seperator of word.
 *  @param	punctuation					The chars work as punctuations.
 *  @param	minHyphenWidth				If the width of the rest of line is <=
 *minHyphenWidth, we will use hyphen to break word
 *
 *	@return BW::vector<BW::string>	Broken string lines.
 */
BW::vector<BW::wstring> FontMetrics::breakString(BW::wstring       wstr,
                                                 /* IN OUT */ int& w,
                                                 int&              h,
                                                 int minHyphenWidth,
                                                 ...
                                                 const BW::wstring& punctuation)
{
    ...
    while (wstr.find('\r') != wstr.npos || wstr.find('\n') != wstr.npos) {
        offset1 = wstr.find('\r');
        offset2 = wstr.find('\n');
        if (offset1 == wstr.npos ||
            (offset2 != wstr.npos && offset2 < offset1)) {
            offset1 = offset2;
        }
        int                     W    = w, H;
        BW::vector<BW::wstring> temp = breakString(wstr.substr(0, offset1),
                                                   W,
                                                   H,
                                                   minHyphenWidth,
                                                   wordBreak,
    ...
    while (!wstr.empty()) {
        ...
        while (!wstr.empty() &&
               (punctuation.find(wstr[0]) != punctuation.npos ||
                wordBreak.find(wstr[0]) != wordBreak.npos)) {
    ...
    while (!words.empty()) {
        BW::wstring suffix =
          (line.empty() || wordBreak.empty() ? L"" : wordBreak.substr(0, 1)) +
          words[0];
        if (stringWidth(line + suffix, false) <= (uint)w) {
            line += suffix;
            words.erase(words.begin());
        } else {
            if (stringWidth(line + (line.empty() || wordBreak.empty()
                                      ? L""
                                      : (wordBreak.substr(0, 1) + L"-")),
                            false) < (uint)minHyphenWidth ||
                line.empty()) {
                if (!wordBreak.empty() && !line.empty())
                    line += wordBreak[0];
                while (!words[0].empty() &&
                       stringWidth(line + words[0].substr(0, 1) + L'-', false) <
                         (uint)w)
 ...
 */
uint FontMetrics::stringWidth(const BW::string& str,
                              bool              multiline,
 ...
 *	@return uint	The width of the string, in texels.
 */
uint FontMetrics::stringWidth(const BW::wstring& str,
                              bool               multiline,
                              bool               colourFormatting)
{
    BW_GUARD;
                                           ...
                                           int&               h)
{
    ...
    // be one pixel too small, meaning any centering calculations will still be
    // ok. 2) for single words in the gui, if wrapping the word in a border,
    // then one would expect a margin to be applied anyway, in which case the
    // small inaccuracy won't matter. 3) *most importantly* the string width is
    // used to lay out text fields, and is used incrementally ( i.e. calculate
    // the incremental width of a sentence ) in this case, we don't want to add
    // the effects margin on at all, because if we did the accumulated addition
    // of effects margins ( that won't be drawn ) would end up reported a far
    // greater width than is correct.
    //
    // Therefore, we choose the lowest probable error and don't add on the final
    // margin.
    //
    // TODO : maybe we could just have two functions, one for incremental calcs,
    // and one for single use calcs.
    ...
    while (i < s) {
        ...
        if (multiline && str[i] == '\n') {
            ...
            linew +=
              (int)(uvWidth * cache_->mapWidth() - cache_->effectsMargin());
        }
...
void FontMetrics::stringDimensionsInternal(const BW::string& s,
                                           bool              multiline,
                                           bool              colourFormatting,
                                           int&              w,
                                           int&              h)
 ...
 */
void FontMetrics::stringDimensions(const BW::string& str,
                                   int&              w,
                                   int&              h,
                                   bool              multiline,
                                   bool              colourFormatting)
 ...
 *	@param	bool	Whether or not colour formatting tags should be parsed.
 */
void FontMetrics::stringDimensions(const BW::wstring& str,
                                   int&               w,
                                   int&               h,
                                   bool               multiline,
                                   bool               colourFormatting)
