...
static BasicAutoConfig<uint8>  s_ecmNumFaceUpdatesPerFrame(
  "environment/cubeMap/numFaceUpdatesPerFrame",
  1);
...
class EnvironmentCubeMapSetter : public Moo::EffectConstantValue
{
  public:
    ...
    ~EnvironmentCubeMapSetter() { value_ = NULL; }

    void cubeTexture(Moo::BaseTexturePtr pTexture) { value_ = pTexture; }
  ...
  , numberOfFacesPerFrame_(numFacesPerFrame)
{
    ...
    if (numFacesPerFrame == 0) {
        numberOfFacesPerFrame_ =
          Math::clamp((uint8)1, s_ecmNumFaceUpdatesPerFrame.value(), (uint8)6);
    }
...
//------------------------------------------------------------------------------
/**
 * This method draws the environment How many faces to draw on this draw call.
 * Max 6, min 0. If 1, it will update each face in turn, one per frame. If -1,
 * it will draw the N faces, where N is configurable below.
 */
void EnvironmentCubeMap::update(float  dTime,
                                bool   defaultNumFaces,
                                uint8  numberOfFaces,
                                uint32 drawSelection)
{
    BW_GUARD;
    ...
    if (drawSelection <= EnviroMinder::DRAW_ALL) {
        ...
        ds = EnviroMinder::DRAW_SUN_AND_MOON | EnviroMinder::DRAW_SKY_GRADIENT |
    ...
    for (int i = 0; i < numberOfFaces; ++i) {
        D3DCUBEMAP_FACES face =
          static_cast<D3DCUBEMAP_FACES>(environmentCubeMapFace_);
