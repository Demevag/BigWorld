...
namespace { // anonymous
    ...
    // Helper functions
    void tickLogger();
    void logFrame(double                gameTime,
                  double                frameTime,
                  const LogEntryVector& logline);
...
void FrameLogger::init()
{
    BW_GUARD;
    MF_WATCH(
      "Debug/Frame Logger/Frame time threshold",
      f_logFrameThreshold,
    ...
    MainLoopTasks::root().add(
      f_frameLoogerInstance.get(), "Debug/Frame Logger", ">App", NULL);
}
...
namespace { // anonymous
    ...
    {
        BW_GUARD;
        DogWatchManager::iterator dwmBegin =
          DogWatchManager::instance().begin();

        double fAcc         = 0;
        double secsPerStamp = 1.0 / stampsPerSecondD();
        f_slowestFrame2Sec  = 0;
        for (int period = 1; period < DogWatchManager::NUM_SLICES && fAcc < 2;
             period++) {
        ...
        if (frameTime > f_logFrameThreshold) {
            ...
            DogWatchManager::iterator dwmEnd =
              DogWatchManager::instance().end();
        ...
        while (entryIt != entryEnd) {
            ...
            if (f_useCSVFormat) {
                ...
                if (percent >= f_percentThreshold) {
                    ...
                    (*f_logFile)
                      << BW::string(entryIt->ident * 2, ' ') + entryIt->name;
                    (*f_logFile) << std::endl;
                }
            }
            ++entryIt;
    ...
    {
        BW_GUARD;
        int frameTimeIdx = int(s_histogram.size() *
    ...
    void renderHistogram()
    {
        ...
        while (it != end) {
            ...
            if (FontManager::instance().begin(*font)) {
                int interval = int((s_maxFrameTimeMs - s_minFrameTimeMs) /
                                   s_histogram.size());
                ...
                minstr << min;
                float minwidth =
                  font->metrics().stringWidth(minstr.str()) / screenWidth;
                font->drawStringInClip(minstr.str(),
                                       p1 + Vector3(-minwidth, 0, 0));

                BW::stringstream maxstr;
                maxstr << max;
                float maxwidth =
                  font->metrics().stringWidth(maxstr.str()) / screenWidth;
                font->drawStringInClip(maxstr.str(),
                                       p1 + Vector3(-maxwidth, -height, 0));

                BW::stringstream valuestr;
                valuestr << value;
                float valuewidth =
                  font->metrics().stringWidth(valuestr.str()) / screenWidth;
                font->drawStringInClip(valuestr.str(),
                                       p2 + Vector3(-valuewidth, +height, 0));
        ...
        float totalwidth =
          font->metrics().stringWidth(totalstr.str()) / screenWidth;
