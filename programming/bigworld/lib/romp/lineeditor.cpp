...
namespace { // anonymous
    ...
    const BW::wstring LINEEDITOR_SEPARATORS =
      L" `~!@#$%^&*()_+-=[]{}\\|;:'\",./<>?";
 ...
 */
LineEditor::ProcessState LineEditor::processKeyEvent(KeyEvent     event,
                                                     BW::wstring& resultString)
{
    BW_GUARD;
    ...
    if (event.isKeyDown()) {
        ...
        if (!isHandled && keyChar != 0) {
            ...
            if (event.isKeyDown() && eventKey == KeyCode::KEY_U) {
                BW::wstring::size_type assignmentLocation =
                  editString_.find_last_of(L"=");
...
bool LineEditor::processAdvanceEditKeys(KeyEvent event)
{
    ...
    if (event.isCtrlDown()) {
        ...
        switch (event.key()) {
            ...
            case KeyCode::KEY_K:
                clipBoard_ =
                  cutText(cx_, static_cast<int>(editString_.length()));
                break;
...
const LineEditor::StringVector LineEditor::history() const
{
    ...
    std::reverse_copy(
      this->history_.begin(), this->history_.end(), result.begin());
    std::transform(
      result.begin(), result.end(), result.begin(), &encodeSpaces::doit);
    return result;
}
...
void LineEditor::setHistory(const StringVector& history)
{
    ...
    std::transform(this->history_.begin(),
                   this->history_.end(),
                   this->history_.begin(),
                   &decodeSpaces::doit);
...
void LineEditor::deleteChar(int pos)
{
    ...
    if (pos == editString_.length())
        ...
        BW::wstring postStr = editString_.substr(pos + 1, editString_.length());
...
int LineEditor::curWordEnd(int pos) const
{
    BW_GUARD;
    // go to end of word
    while (uint(pos) < editString_.length() && !isSeparator(editString_[pos])) {
        ++pos;
    }
...
BW::wstring LineEditor::cutText(int start, int end)
{
    ...
    editString_      = editString_.substr(0, start) +
...
{
    BW_GUARD;
    editString_ = editString_.substr(0, pos) + text +
...
namespace { // anonymous
    ...
    {
        BW_GUARD;
        return std::find(LINEEDITOR_SEPARATORS.begin(),
