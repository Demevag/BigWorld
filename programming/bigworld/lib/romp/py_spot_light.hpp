...
namespace BW {
     ...
     *  and/or specular light source for models and shells. This is not the
     *class used for environmental lights or lights which have been placed in
     *the editor.
     *
     ...
     *  the PySpotLight. If the distance is less than innerRadius, then the
     *light is applied at full strength (regardless of outerRadius). The
     *strength of the light falls off linearly from full to 0 between
     *innerRadius and outerRadius, and is 0 from outerRadius onwards.
     *
     *  Code Example:
     *  @{
     ...
     */
    /**
     * The intention of the PySpotLight class is to wrap both chunk-based
     * scripted lights as well as any other dynamic/scripted light.  The
     * encapsulation is defined by the ILightEmbodiment, for which the concrete
     * SpaceFactory type is used to create the correct concrete light embodiment
     * type.
     */
    class PySpotLight
      ...
      , public ISpotLight
    {
        Py_Header(PySpotLight, PyObjectPlusWithVD)
          ...
          public : PySpotLight(PyTypeObject* pType = &s_type_);
        virtual ~PySpotLight();
        ...
        // calling points for derived classes in case they need to
        // increase/decrease the references.
    ...
    {
        Py_Header(PyChunkSpotLight, PySpotLight) public
