...
/// This token should be referenced to link in the TextureFeeds script
/// functions.
...
class TextureFeeds : public Singleton<TextureFeeds>
{
  ...
  public:
    static void addTextureFeed(const BW::string&    identifier,
                               PyTextureProviderPtr texture);
    PY_AUTO_MODULE_STATIC_METHOD_DECLARE(RETVOID,
                                         addTextureFeed,
                                         ARG(BW::string,
                                             ARG(PyTextureProviderPtr, END)))
...
{
  public:
    TextureFeed(const BW::string&   resourceID,
