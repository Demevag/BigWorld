...
class SkyLightMapSetter : public Moo::EffectConstantValue
{
  public:
    ...
    ~SkyLightMapSetter() { value_ = NULL; }
    void reset() { value_ = NULL; }
    void active(bool newValue) { active_ = newValue; }
...
class SkyLightMapTransformSetter : public Moo::EffectConstantValue
{
    ...
    void calculate()
    {
        ...
        transform_[0].set(
          farPlane, 0.f, 0.f, 0.5f - texCoordOffset_.x + sunAngleOffset_);
  ...
  , sunAngleOffset_()
{
    ...
    if (s_cloudMapDarkness < 0.f) {
        ...
        MF_WATCH("Client Settings/Clouds/max sky light map darkness",
                 ...
                 "Maximum sky light map darkness.  Sky light map values are "
                 "clamped to "
                 "this alpha value.");
    }
...
void SkyLightMap::update(float sunAngle, const Vector2& strataMovement)
{
    ...
    float       xOffset =
      Math::clamp(-FADE_UPPER, cloudStrataHeight * tanf(sunAngle), FADE_UPPER);
    this->sunAngleOffset(xOffset);

    // set the new tex coord offset
    float x = texCoordOffset().x;
    float y = texCoordOffset().y;
    // NOTE : setting texCoordOffset here implicitly recalculates the light map
    // transform every frame.  Even if the texCoordOffset doesn't change, we may
    // as well calculate the transform per-frame just in case the far plane has
    // changed too.  That way we don't need to explicitly keep track of camera
    // far plane changes.
    this->texCoordOffset(Vector2(x + strataMovement.x / CLOUD_EXTENT,
                                 y + strataMovement.y / CLOUD_EXTENT));
    ...
    if (!material_.get() || !material_->pEffect() ||
        !material_->pEffect()->pEffect())
        return;

    if (this->beginRender()) {
        if (SkyLightMapSettings::isEnabled()) {
            // we are going to recreate the light map, so reset the tex coord
            // offset thing
...
{
    BW_GUARD;
    uint32 nTextures = Moo::rc()
                         .deviceInfo(Moo::rc().deviceIndex())
                         .caps_.MaxSimultaneousTextures;
    ...
    if (!pRT_ || !pRT_->push()) {
        ...
        Moo::rc().device()->Clear(
          0, NULL, D3DCLEAR_ZBUFFER | D3DCLEAR_TARGET, 0x00000000, 1, 0);
...
void SkyLightMap::texCoordOffset(const Vector2& value)
{
    ...
    SkyLightMapTransformSetter* slmts =
      static_cast<SkyLightMapTransformSetter*>(ecv);
 ...
 */
void SkyLightMapSettings::configureKeywordSetting(
  Moo::EffectMacroSetting& setting)
{
    BW_GUARD;
...
Moo::EffectMacroSetting::EffectMacroSettingPtr
  SkyLightMapSettings::s_skyLightMapSetting;
