  ...
  , tTime_(0.0)
{
    ...
    if (!s_createdWatchers) {
        MF_WATCH("Client Settings/Flora/Noise Factor",
                 ...
                 "Multiplier for flora noise animation grid.  Affects overall "
                 "flora movement.");
...
{
    BW_GUARD;
    if (*Moo::rc().effectVisualContext().getMapping("FloraAnimationGrid") ==
        pAnimGridSetter_.get()) {
        *Moo::rc().effectVisualContext().getMapping("FloraAnimationGrid") =
          NULL;
    }
...
void FloraAnimationGrid::set(int constantBase)
{
    ...
    Moo::rc().device()->SetVertexShaderConstantF(
      constantBase, Vector4(0.f, 0.f, 255.f, 0.f), 1);
    Moo::rc().device()->SetVertexShaderConstantF(
      constantBase + 1, (float*)constants_, nConstants_);
}
    ...
    *Moo::rc().effectVisualContext().getMapping("FloraAnimationGrid") =
    ...
    for (int z = 0; z < height_; z++) {
        ...
        for (int x = 0; x < width_; x++) {
            ...
            noise_[idx] =
              perlin_.noise3(Vector3((float)x, (float)z, animationTime));
...
void FloraAnimationLocal::update(float dTime, const EnviroMinder& enviro)
{
    ...
    Vector2 vWind =
      enviro.weather()->wind() * enviro.weather()->floraWindFactor();
    float fWind = logf(vWind.length() + 1.0f) * m_fScale;

    for (int i = 0; i < m_iWidth; i++) {
        Vector2& vNoise = m_pGrid[i];
        vNoise.x =
          (m_noise.noise1(m_fTotalTime + i + 1024) - 0.5f) * 2.0f * fWind;
...
void FloraAnimationGlobal::update(float dTime, const class EnviroMinder& enviro)
{
    ...
    Vector2 vWind =
      enviro.weather()->wind() * enviro.weather()->floraWindFactor() * m_fScale;
    m_vDirection = vWind;
