 ...
 */
bool LogConsole::handleMessage(DebugMessagePriority messagePriority,
                               ...
                               va_list              formatArgs)
{
    ...
    MF_VERIFY(DebugFilter::instance().shouldAccept(messagePriority, pCategory));
    ...
    bool shouldResetDisplayBase =
      (messageBuffer_.empty() ||
       (messageBuffer_.full() &&
    ...
    rNewTail.iCategory =
      categorySet_.insert((pCategory) ? pCategory : BW::string()).first;
...
void LogConsole::update()
{
    ...
    if (isDirty_) {
        ...
        for (iMessage = iDisplayBase_; iMessage != messageBuffer_.rend();
             ++iMessage) {
 ...
 */
bool LogConsole::renderLine(const MessageBufferEntry& rEntry, int& rBaseLine)
{
    BW_GUARD_MEMTRACKER(LogConsole);
    ...
    if (isWrapping_) {
        ...
        for (int i = 0; i < lines; ++i) {
            this->line(
              rBaseLine + i + 1, outputLine.c_str() + i * wrapWidth, wrapWidth);
...
void LogConsole::drawStatusLine()
{
    ...
    */
    ...
    uint32 currStatusOnColour =
      ((statusOnColour & 0xffffff) | uint32((statusOnColour >> 24) * mainAlpha)
                                       << 24);
    uint32 currStatusOffColour =
      ((statusOffColour & 0xffffff) |
       uint32((statusOffColour >> 24) * mainAlpha) << 24);

    if (isMonitoring_) {
        column +=
          this->drawStatusWString(BW::wstring(L"Monitor mode: Ctrl-Z to exit"),
                                  column,
                                  currStatusOnColour);
        column++;
    } else {
    ...
    column += this->drawStatusWString(
      BW::wstring(L"Priorities:"), column, currConsoleColour);
    column++;
    ...
    column += this->drawStatusWString(BW::wstring(L"Category"),
                                      column,
                                      ((selectedCategory_ != categorySet_.end())
                                         ? currStatusOnColour
                                         : currStatusOffColour));
    ...
    column += this->drawStatusWString(
      BW::wstring(L"Filter:"), column, currConsoleColour);
    column++;

    column +=
      this->drawStatusWString(stringMatchFilter_, column, currStatusOnColour);
 ...
 */
int LogConsole::drawStatusWString(const BW::wstring& rString,
                                  int                column,
                                  uint32             colour)
{
    BW_GUARD_MEMTRACKER(LogConsole);
    ...
    if (rString.length() <= availableSpace) {
        font_->drawWConsoleString(
          rString, column, row, int(offset_.x), int(offset_.y));
        return static_cast<int>(rString.length());
    } else {
        const BW::wstring buffer(rString, 0, availableSpace - 1);
        font_->drawWConsoleString(
          buffer, column, row, int(offset_.x), int(offset_.y));
        font_->colour(this->consoleColour());
        font_->drawWConsoleString(
          BW::wstring(L">"), visWidth - 1, row, int(offset_.x), int(offset_.y));
 ...
 *	otherwise false.
 */
bool LogConsole::shouldFilterMessage(const MessageBufferEntry& rMessage) const
{
    BW_GUARD_MEMTRACKER(LogConsole);
    ...
    if (shouldCategoryFilter && rMessage.iCategory != selectedCategory_) {
        return true;
    }
    ...
    if (shouldStringMatchFilter &&
        rMessage.message.find(stringMatchFilter_) == rMessage.message.npos) {
        return true;
    }
