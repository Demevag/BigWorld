...
void ResourceStatistics::displayResourceStatistics(XConsole& console)
{
    ...
    if (granularityMode_ == ResourceCounters::GM_BREAKDOWN_RESOURCE_TYPE) {
        // NOTE: This is done in here as GPU info comes from Moo, and
        // ResourceCounters
        //       is in cstdmf.
...
{
    BW_GUARD;
    granularityMode_ = static_cast<ResourceCounters::GranularityMode>(
      (granularityMode_ + 1) % ResourceCounters::GM_MAX);
}

/**
 *	Prints the current granularity to the passed console using the passed
 *hanging.
 *
...
void ResourceStatistics::printUsageStatistics(XConsole& console)
{
    BW_GUARD;
    console.print(
      ResourceCounters::instance().getUsageDescription(granularityMode_));
}
...
void ResourceStatistics::printResourceTypeBreakdown(XConsole& console)
{
    ...
    // NOTE: Dedicated & Shared are roughly equivelent to System, and since
    // System is
    //       what we could consistently track for GPU memory, this is what will
    //       be used in the breakdown. This is still not 100% as the driver
    //       internally moves stuff around, but it gives us a good idea of how
    //       much GPU System memory is being consumed.
    Moo::GpuInfo::MemInfo memInfo;
    Moo::rc().getGpuMemoryInfo(&memInfo);

    // Grab current main (private bytes) usage & GPU system mem usage, calculate
    // unaccounted from that. Unaccounted memory is memory which is used, but
    // ResourceCounters has not been told about with a call to 'add'.
    ResourceCounters::MemoryEntry usedMemory(memInfo.privateUsage_,
                                             memInfo.systemMemUsed_);
    ...
    const BW::string entryNames[] = {
        "- Texture", "- Index Buffer", "- Vertex Buffer", "- Other"
    };

    BW_STATIC_ASSERT(ARRAY_SIZE(entryNames) == ResourceCounters::RT_MAX,
                     resource_type_size_and_entrynames_mismatch);

    // Grab entries for each resource type.
    for (int resourceTypeIdx = 0; resourceTypeIdx < ResourceCounters::RT_MAX;
         ++resourceTypeIdx) {
        ResourceCounters::Entry& entry(entries[resourceTypeIdx]);
        ResourceCounters::instance().getResourceTypeMemoryUsage(
          (ResourceCounters::ResourceType)resourceTypeIdx, &entry);

        unaccountedMemory -= entry.size_;
    }

    // Clamp unaccounted memory above zero. Sometimes the GPU system usage may
    // not actually use as much as we think. D3D can move data to/from GPU
    // system memory as it pleases.
    unaccountedMemory.clampAboveZero();

    // Main (private set) memory usage.
    ResourceCounters::instance().addEntry(ss,
                                          "Virtual Address Space Total",
    ...
    ResourceCounters::instance().addEntry(ss,
                                          ...
                                          0,
                                          0);
    ResourceCounters::instance().addEntry(
      ss, "Main (Private Bytes) Used", usedMemory.main_, 0, 0, 0, 0, 0, 0);
    ResourceCounters::instance().addEntry(
      ss, "- Unaccounted", unaccountedMemory.main_, 0, 0, 0, 0, 0, 0);
    for (int resourceTypeIdx = 0; resourceTypeIdx < ResourceCounters::RT_MAX;
         ++resourceTypeIdx) {
        ResourceCounters::Entry entry(entries[resourceTypeIdx]);
        ResourceCounters::instance().addEntry(ss,
                                              entryNames[resourceTypeIdx],
                                              entry.size_.main_,
    ...
    ResourceCounters::instance().addEntry(ss,
                                          ...
                                          0);
    ResourceCounters::instance().addEntry(ss,
                                          "GPU (Dedicated Memory) Committed",
                                          ...
                                          0);
    ResourceCounters::instance().addEntry(
      ss, "GPU (System Memory) Used", usedMemory.gpu_, 0, 0, 0, 0, 0, 0);
    ResourceCounters::instance().addEntry(
      ss, "- Unaccounted", unaccountedMemory.gpu_, 0, 0, 0, 0, 0, 0);
    for (int resourceTypeIdx = 0; resourceTypeIdx < ResourceCounters::RT_MAX;
         ++resourceTypeIdx) {
        ResourceCounters::Entry entry(entries[resourceTypeIdx]);
        ResourceCounters::instance().addEntry(ss,
