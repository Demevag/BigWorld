...
{
    BW_GUARD;
    //-- choose format of reflection scene based on the current rendering
    //pipeline.
 ...
 */
bool WaterScene::getVisibleChunks(BW::vector<Chunk*>&  visibleChunks,
                                  bool&                outsideVisible,
 ...
 *  Constructor.
 */
WaterSceneRenderer::WaterSceneRenderer(WaterScene* scene,
                                       bool        reflect,
                                       D3DFORMAT   format)
  : TextureRenderer(512, 512, format)
  , // default sizes
  scene_(scene)
  , reflection_(reflect)
...
bool WaterSceneRenderer::recreate()
{
    ...
    success =
      pRT_->create(uint(float(Moo::rc().screenWidth()) * s_textureSize_),
...
bool WaterSceneRenderer::shouldReflect(const Vector3& pos, PyModel* model)
{
    ...
    // TODO: this will still draw all instances of the player model..fix!!!
        ...
        shouldReflect =
          ((reflectionCount_ < s_maxReflections_) && s_drawDynamics_);
    }
...
void WaterSceneRenderer::renderSelf(float dTime)
{
    ...
    if (flipScene) {
        ...
        if (eyeUnderWater_) {
            clipPlaneOffset =
              D3DXPLANE(0.f,
                        ...
                        0.f,
                        (waterHeight() +
                         reflectionFudge)); // including error fudgery factor.

            clipPlaneActual = D3DXPLANE(
              0.f,
              -1.f,
              0.f,
              (waterHeight() +
               reflectionActualFudge)); // including error fudgery factor.
        } else {
            clipPlaneOffset =
              D3DXPLANE(0.f,
                        1.f,
        ...
        Matrix refMatrix;
        refMatrix.setTranslate(
          0.f, this->waterHeight() * 2.f, 0.f); // water height*2
        ...
        if (eyeUnderWater_) {
            clipPlaneOffset = D3DXPLANE(
              0.f,
              1.f,
              0.f,
              -waterHeight() +
                REFRACTION_FUDGE_FACTOR); // including error fudgery factor.
            clipPlaneActual = D3DXPLANE(
              0.f,
              1.f,
              0.f,
              -waterHeight() + 0.1f); // including error fudgery factor.
        } else {
            clipPlaneOffset = D3DXPLANE(
              0.f,
              -1.f,
    ...
    if (s_useClipPlane_ && !s_simpleScene_) {
        ...
        D3DXMatrixInverse(
          (D3DXMATRIX*)&worldViewProjectionInverseTransposeMatrix,
          NULL,
        ...
        D3DXPlaneTransform(
          &transformedClipPlaneOffset,
          &clipPlaneOffset,
          (D3DXMATRIX*)&worldViewProjectionInverseTransposeMatrix);
        D3DXPlaneTransform(
          &transformedClipPlaneActual,
          &clipPlaneActual,
          (D3DXMATRIX*)&worldViewProjectionInverseTransposeMatrix);
    ...
    Moo::rc().device()->Clear(
      0, NULL, D3DCLEAR_ZBUFFER | D3DCLEAR_TARGET, 0x0, 1.f, 0);
#else
    Moo::rc().device()->Clear(0, NULL, D3DCLEAR_ZBUFFER, 0x0, 1.f, 0);
#endif
    ...
    if (pChunkSpace) {
        ...
        Chunk*  cc = NULL;
        Vector3 pp =
          invView.applyToOrigin() + Moo::rc().camera().nearPlane() *
    ...
    if (!s_simpleScene_) {
        if (pChunkSpace) {
            ...
            if (scene_->getVisibleChunks(
                  visibleChunks, outsideVisible, verts)) {
                ...
                ChunkManager::instance().drawReflection(
                  *waterDrawContext_, visibleChunks, outsideVisible, nearPoint);
            }
        } else {
            SceneIntersectContext cullingContext(scene);
            IntersectionSet       visibleObjects;
            DrawHelpers::cull(cullingContext,
                              scene,
                              Moo::rc().viewProjection(),
                              visibleObjects);
            DrawHelpers::drawSceneVisibilitySet(
              drawContext, scene, visibleObjects);
        }
    }
...
void WaterSceneRenderer::createRenderTarget()
{
    ...
    PyTextureProviderPtr feed =
      PyTextureProviderPtr(new PyTextureProvider(NULL, pRT_), true);
