    ...
    class Emitter
    {
      public:
        Emitter()
          : position_(0, 0, 0)
          , colour_(0xffff8080)
          , // gross pink, just so you know it's unitialised
          maxMultiplier_(1.f)
        ...
        float innerRadius(void) const { return innerRadius_; }
        ...
        float outerRadiusSquared(void) const { return outerRadiusSquared_; }

        float innerRadiusSquared(void) const { return innerRadiusSquared_; }

        float falloffRange(void) const { return falloffRange_; }

        bool localised(void) const { return localised_; }

        void localised(bool state) { localised_ = state; }
