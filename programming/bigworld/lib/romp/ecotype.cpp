...
void Ecotype::init(DataSectionPtr allEcotypesSection, uint8 id)
{
    ...
    li->loadingTask_ =
      new CStyleBackgroundTask("Load Ecotype task",
                               &Ecotype::backgroundInit,
                               li,
                               &Ecotype::onBackgroundInitComplete,
                               li);
    li->ecotype_     = this;
    this->loadInfo_  = li;
...
void Ecotype::onBackgroundInitComplete(void* bkLoadInfo)
{
    ...
    if (li->ecotype_ && li->pSection_) {
        ...
        if (pSection) {
            ...
            // pSection->sectionName().c_str(), pTex ?
            // pTex->resourceID().c_str() : "None" );
        }
    }
...
void Ecotype::incRef()
{
    ...
    if (refCount_ == 1) {
        ...
        // pTexture_ ? pTexture_->resourceID().c_str() : "None" );
        grabTexture();
    }
}
 ...
 *	the indexPath list
 */
bool Ecotype::highLight(FloraVertexContainer* pVerts,
 ...
 */
uint32 Ecotype::generate(class FloraVertexContainer* pVerts,
                         uint32                      idx,
                         uint32                      numVerts,
                         ...
                         BoundingBox&                bb)
{
    ...
    if (generator_)
        return generator_->generate(
          uvOffset_, pVerts, idx, numVerts, objectToWorld, objectToChunk, bb);
    else {
