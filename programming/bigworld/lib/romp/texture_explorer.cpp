  ...
  , alpha_(false)
{
    ...
    ts.colourOperation(Moo::TextureStage::SELECTARG1,
                       Moo::TextureStage::TEXTURE_ALPHA,
...
{
    const Moo::TextureManager::TextureList& texList =
      Moo::TextureManager::instance()->textureList();
    ...
    if (pTexture_.hasObject()) {
        if (index_ != currentIndex_) {
            ...
            if (lastSlash < int(resID.length()) && lastSlash > 0) {
...
void TextureExplorer::draw()
{
    if (enabled_ && pTexture_.hasObject()) {
        ...
        Vector2 bottomRight(Moo::rc().screenWidth() * 0.95f,
                            Moo::rc().screenHeight() * 0.95f);
        ...
        if (fitToScreen_ && preserveAspect_) {
            ...
            topLeft.set(bottomRight.x - float(width_) * aspect,
                        bottomRight.y - float(height_) * aspect);
        } else if (fitToScreen_) {
            topLeft.set(Moo::rc().screenWidth() * 0.05f,
                        Moo::rc().screenHeight() * 0.05f);
        }
        ...
        Moo::rc().drawVerticesUP(
          D3DPT_TRIANGLESTRIP, 4, verts, sizeof(Moo::VertexTUV));
    }
}
