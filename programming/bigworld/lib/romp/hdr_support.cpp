...
namespace {
    ...
    //-- the minimal size of the bloom calculation chain. Affects overall
    //blooming quality. Lower
    //-- values make bloom more smooth.
    const float g_minBloomTargetSize = 25.0f;

    //-- Warning: this feature used only for debugging because it kills
    //asynchronous behavior between
    //--          CPU and GPU.
    ...
    //----------------------------------------------------------------------------------------------
    void doGaussianBlur(Moo::RenderTarget*   inoutRT,
                        Moo::RenderTarget*   inRT,
                        ...
                        DX::Viewport&        vp)
    {
        ...
        Vector4 screen   = Vector4(static_cast<float>(vp.Width),
    ...
    {
      public:
        HDRParamsConstant(const HDRSupport& hdr)
          : m_hdr(hdr)
        {
        }
        ...
        bool operator()(ID3DXEffect* pEffect, D3DXHANDLE constantHandle)
        {
            ...
            if (m_hdr.enable()) {
                const HDRSettings::Environment& cfg =
                  m_hdr.settings().m_environment;

                params.set(cfg.m_skyLumMultiplier,
    ...
    {
      public:
        GammaCorrectionConstant(const HDRSupport& hdr)
          : m_hdr(hdr)
        {
        }
        ...
        bool operator()(ID3DXEffect* pEffect, D3DXHANDLE constantHandle)
        {
            ...
            if (m_hdr.enable() &&
                m_hdr.settings().m_gammaCorrection.m_enabled) {
                const HDRSettings::GammaCorrection& cfg =
                  m_hdr.settings().m_gammaCorrection;
  ...
  , m_update(true)
{
    ...
    MF_WATCH("Render/HDR/readback avg lum",
             g_readbackAvgLumToCPU,
             Watcher::WT_READ_WRITE,
             "enable/disable readback of avg luminance from GPU (slow).");

    MF_WATCH("Render/HDR/average luminance update interval",
             g_cfg.m_avgLumUpdateInterval,
             Watcher::WT_READ_WRITE,
             "How often average luminance will be calculated.");

    MF_WATCH("Render/HDR/use the log of luminance",
             g_cfg.m_useLogAvgLuminance,
             Watcher::WT_READ_WRITE,
             "Use log for average luminance.");

    MF_WATCH("Render/HDR/accommodation speed",
             g_cfg.m_adaptationSpeed,
             Watcher::WT_READ_WRITE,
             "Accommodation speed 10 - 1000.");

    MF_WATCH("Render/HDR/tone mapping/middle gray",
             g_cfg.m_tonemapping.m_middleGray,
             Watcher::WT_READ_WRITE,
             "Middle gray value to which overall HDR luminance aims.");

    MF_WATCH("Render/HDR/tone mapping/white point",
             g_cfg.m_tonemapping.m_whitePoint,
             Watcher::WT_READ_WRITE,
             "White point luminance value.");

    MF_WATCH("Render/HDR/tone mapping/eye dark limit",
             g_cfg.m_tonemapping.m_eyeDarkLimit,
             Watcher::WT_READ_WRITE,
             "Minimal pixel luminance value before it will be clamped.");

    MF_WATCH("Render/HDR/tone mapping/eye light limit",
             g_cfg.m_tonemapping.m_eyeLightLimit,
             Watcher::WT_READ_WRITE,
             "Maximal pixel luminance value after it will be clamped.");

    MF_WATCH("Render/HDR/environment/ambient luminance multiplier",
             g_cfg.m_environment.m_ambientLumMultiplier,
             Watcher::WT_READ_WRITE,
             "0.0 - 20.0f");

    MF_WATCH("Render/HDR/environment/sky luminance multiplier",
             g_cfg.m_environment.m_skyLumMultiplier,
             Watcher::WT_READ_WRITE,
             "0.0 - 20.0f");

    MF_WATCH("Render/HDR/environment/sun luminance multiplier",
             g_cfg.m_environment.m_sunLumMultiplier,
             Watcher::WT_READ_WRITE,
             "0.0 - 20.0f");

    MF_WATCH("Render/HDR/environment/fog luminance multiplier",
             g_cfg.m_environment.m_fogLumMultiplier,
             Watcher::WT_READ_WRITE,
             "0.0 - 20.0f");

    MF_WATCH("Render/HDR/bloom/enabled",
             g_cfg.m_bloom.m_enabled,
             Watcher::WT_READ_WRITE,
             "Enable/disable bloom.");

    MF_WATCH("Render/HDR/bloom/factor",
             g_cfg.m_bloom.m_factor,
             Watcher::WT_READ_WRITE,
             "Bloom factor 0.0 - 5.0.");

    MF_WATCH("Render/HDR/bloom/bright threshold",
             g_cfg.m_bloom.m_brightThreshold,
             Watcher::WT_READ_WRITE,
             "0.0 - 5.0.");

    MF_WATCH("Render/HDR/bloom/bright offset",
             g_cfg.m_bloom.m_brightOffset,
             Watcher::WT_READ_WRITE,
             "0.0 - 5.0.");

    MF_WATCH("Render/HDR/linear space lighting/enable",
             g_cfg.m_gammaCorrection.m_enabled,
             Watcher::WT_READ_WRITE,
             "Use linear space lighting calculation.");

    MF_WATCH("Render/HDR/linear space lighting/gamma",
             g_cfg.m_gammaCorrection.m_gamma,
             Watcher::WT_READ_WRITE,
             "Gamma.");
}
...
void HDRSupport::tick(float dt)
{
    ...
    //-- ToDo (b_sviglo): Ugly solution for 8.0 version reconsider later.
    {
        const Moo::GraphicsSetting::GraphicsSettingPtr& ppCfg =
          Moo::GraphicsSetting::getFromLabel("POST_PROCESSING_QUALITY");
...
void HDRSupport::intercept()
{
    ...
    //-- 1. set all textures to NULL, just in case the render target was set on
    //any of the stages before.
    uint32 nTextures =
      Moo::rc().currentDeviceInfo().caps_.MaxSimultaneousTextures;
    ...
    Moo::rc().device()->Clear(
      0, NULL, D3DCLEAR_TARGET, Moo::Colour(0, 0, 0, 0), 1, 0);
}
...
void HDRSupport::resolve()
{
    ...
    if (m_successed) {
        ...
        //-- Warning: for debug purposes we can read back average luminance
        //value calculated on the
        //--          GPU but it significantly decreases frame rate due
        //CPU<->GPU synchronization.
        if (g_readbackAvgLumToCPU) {
            g_avgLum = readAvgLum();
        }

        //-- now we know average luminance for current frame use it to decide
        //which pixels are
...
void HDRSupport::calcBloom()
{
    ...
    if (m_bloomMaps[0].first->push()) {
        ...
        Vector4             screen(static_cast<float>(vp0.Width),
                       static_cast<float>(vp0.Height),
                       1.0f / vp0.Width,
                       1.0f / vp0.Height);
        ...
        {
            const Vector4 params(g_cfg.m_bloom.m_brightThreshold,
                                 g_cfg.m_bloom.m_brightOffset,
                                 g_cfg.m_tonemapping.m_middleGray,
                                 0);
    ...
    for (uint i = 1; i < m_bloomMaps.size(); ++i) {
        if (m_bloomMaps[i].first->push()) {
            ...
            ID3DXEffect* effect = m_bloomMat->pEffect()->pEffect();
            {
                effect->SetTexture("g_srcMap",
                                   m_bloomMaps[i - 1].first->pTexture());
    ...
    //-- 3.1. process the largest (last) mip level.
    {
        ...
        //-- blur mip.
        doGaussianBlur(mapPair.first.get(),
                       mapPair.second.get(),
                       *m_gaussianBlurMat.get(),
                       vp);
    }
...
void HDRSupport::calcAvgLum()
{
    ...
    if (downSampledHDRRT->push()) {
        ...
        ID3DXEffect* effect = m_averageLuminanceMat->pEffect()->pEffect();
        {
            ...
            Moo::rc().fsQuad().draw(*m_averageLuminanceMat.get(),
                                    m_viewPorts[1].Width,
                                    m_viewPorts[1].Height);
        }

        downSampledHDRRT->pop();
    }

    //-- 2. make down sampled average luminance map based on down sampled copy
    //of HDR map.
    m_averageLuminanceMat->hTechnique(
      g_cfg.m_useLogAvgLuminance ? "LOG_AVERAGE_LUM" : "AVERAGE_LUM");
    if (m_avgLumRTs[0]->push()) {
        const DX::Viewport& vp = m_viewPorts[1];
        Vector4             screen(static_cast<float>(vp.Width),
                       static_cast<float>(vp.Height),
                       1.0f / vp.Width,
                       1.0f / vp.Height);
        ...
        ID3DXEffect* effect = m_averageLuminanceMat->pEffect()->pEffect();
        {
            effect->SetTexture("g_previousMipMap",
                               downSampledHDRRT->pTexture());
            effect->SetVector("g_previousMipMapSize", &screen);
            m_averageLuminanceMat->commitChanges();

            Moo::rc().fsQuad().draw(*m_averageLuminanceMat.get(),
                                    m_viewPorts[2].Width,
                                    m_viewPorts[2].Height);
        }
    ...
    for (uint i = 1; i < m_avgLumRTs.size(); ++i) {
        if (m_avgLumRTs[i]->push()) {
            ...
            {
                effect->SetTexture("g_previousMipMap",
                                   m_avgLumRTs[i - 1]->pTexture());
                m_averageLuminanceMat->commitChanges();

                Moo::rc().fsQuad().draw(*m_averageLuminanceMat.get(),
                                        m_viewPorts[i + 2].Width,
                                        m_viewPorts[i + 2].Height);
            }
    ...
    //-- swap 1x1 mip with another 1x1 mip for accessing this texture from
    //shader.
...
void HDRSupport::calcAvgLumOnGPU()
{
    ...
    if (m_finalAvgLumRT->push()) {
        ...
        //-- setup constants.
        m_averageLuminanceMat->hTechnique(g_cfg.m_useLogAvgLuminance
                                            ? "FINAL_AVERAGE_LOG_LUM"
                                            : "FINAL_AVERAGE_LUM");
        ID3DXEffect* effect = m_averageLuminanceMat->pEffect()->pEffect();
        {
            const Vector4 params(g_cfg.m_tonemapping.m_eyeDarkLimit,
                                 g_cfg.m_tonemapping.m_eyeLightLimit,
                                 m_curTime,
                                 g_cfg.m_adaptationSpeed);
...
void HDRSupport::doTonemapping()
{
    ...
    {
        const Vector4 bloomParams(g_cfg.m_bloom.m_enabled &&
                                    !g_forceDisableBloom,
                                  g_cfg.m_bloom.m_factor,
                                  0.0f,
                                  0.0f);
        const Vector4 params(g_cfg.m_tonemapping.m_middleGray,
                             g_cfg.m_tonemapping.m_whitePoint,
                             0,
                             0);
        effect->SetTexture("g_avgLumMap", m_finalAvgLumRT->pTexture());
        ...
        //-- draw full screen quad into the main back-buffer and resolve HDR
        //buffer.
...
void HDRSupport::createUnmanagedObjects()
{
    ...
    bool success = createRenderTarget(m_hdrRT,
                                      width,
                                      height,
    ...
    for (uint i = 0; i < mipArray.size(); ++i, ++bloomMapsCount) {
        if (mipArray[i].first < g_minBloomTargetSize &&
            mipArray[i].first < g_minBloomTargetSize)
            break;
    }
    ...
    for (uint i = 0; i < bloomMapsCount; ++i) {
        ...
        success &= createRenderTarget(m_bloomMaps[i].first,
                                      size.first,
                                      size.second,
                                      D3DFMT_A16B16G16R16F,
                                      rtName1.c_str(),
        ...
        success &= createRenderTarget(m_bloomMaps[i].second,
                                      size.first,
                                      size.second,
                                      D3DFMT_A16B16G16R16F,
                                      rtName2.c_str(),
    ...
    for (uint i = 1; i < mipArray.size(); ++i) {
        ...
        success &= createRenderTarget(m_avgLumRTs[i - 1],
                                      size.first,
                                      size.second,
                                      D3DFMT_R32F,
                                      rtName.c_str(),
    ...
    success &= createRenderTarget(m_avgLumRT,
                                  1,
                                  1,
                                  D3DFMT_R32F,
                                  "texture/HDR average luminance RT",
                                  true,
                                  true);

    success &= createRenderTarget(m_finalAvgLumRT,
                                  1,
                                  1,
                                  D3DFMT_R32F,
                                  "texture/HDR final average luminance RT",
                                  true,
                                  true);
...
void HDRSupport::createManagedObjects()
{
    ...
    success &=
      createEffect(m_averageLuminanceMat, "shaders/hdr/average_luminance.fx");
    success &= createEffect(m_bloomMat, "shaders/hdr/hdr_bloom.fx");
    success &=
      createEffect(m_gaussianBlurMat, "shaders/hdr/fast_gaussian_blur.fx");
