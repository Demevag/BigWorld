...
static AutoConfigString s_fontMFM("system/fontMFM",
                                  "system/materials/font.mfm");
...
bool FontManager::UVDivisorSetter::operator()(ID3DXEffect* pEffect,
...
bool FontManager::FontMapSetter::operator()(ID3DXEffect* pEffect,
                                            D3DXHANDLE   constantHandle)
{
    BW_GUARD;
...
bool FontManager::FontColourSetter::operator()(ID3DXEffect* pEffect,
                                               D3DXHANDLE   constantHandle)
{
    BW_GUARD;
...
bool FontManager::FontPixelSnapSetter::operator()(ID3DXEffect* pEffect,
                                                  D3DXHANDLE   constantHandle)
{
    BW_GUARD;
...
bool FontManager::doInit()
{
    ...
    if (!material_->pEffect()) {
        CRITICAL_MSG(
          "Could not load the font material or fx file %s, exiting\n",
          mfmName.c_str());
        exit(EXIT_FAILURE);
    }
    blendedTechnique_ =
      material_->pEffect()->pEffect()->GetTechniqueByName("FX_BLEND");
    ...
    *Moo::rc().effectVisualContext().getMapping("FontUVDivisor") =
      uvDivisorSetter_;
    *Moo::rc().effectVisualContext().getMapping("FontColour") =
      fontColourSetter_;
    *Moo::rc().effectVisualContext().getMapping("FontPixelSnap") =
      pixelSnapSetter_;
 ...
 *							explicitly release them via
 *cachedFont->releaseGlyphRefs()
 *
...
FontPtr FontManager::get(const BW::string& resourceName, bool cached)
{
    ...
    Moo::ScopedResourceLoadContext resLoadCtx(
      BWResource::getFilename(resourceName));
    ...
    if (res != NULL) {
        return (cached ? new CachedFont(*res) : new Font(*res));
    }
 ...
 */
void FontManager::prepareMaterial(Font&      font,
                                  uint32     colour,
                                  bool       pixelSnap,
...
static void saveFontCacheMap(const BW::string& fontName)
{
    ...
    if (pFont) {
        ...
        DataSectionPtr pFile =
          BWResource::instance().rootSection()->openSection(
            metricsFilename, true, ZipSection::creator());
        if (pFile) {
            pFile->delChildren();
        ...
        PyErr_Format(PyExc_ValueError,
                     "BigWorld.saveFontCacheMap() "
                     "font resource '%s' does not exist",
                     fontName.c_str());
    }
}
