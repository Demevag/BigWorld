...
ProgressTask::ProgressTask(ProgressDisplay*  pOwner,
...
ProgressDisplay::ProgressDisplay(FontPtr          pFont,
...
{
    BW_GUARD;
    const Vector2 screenDim(float(Moo::rc().screenWidth()),
...
void ProgressDisplay::drawBar(int row, float fraction)
{
    ...
    for (uint i = 0; i < 8; i++) {
        ...
        verts[i].pos_.w = 1;
        verts[i].colour_ =
          ((i < 4) ? fillAlpha : frameAlpha) | (colour_ & 0x00ffffff);
    }

    for (uint i = 0; i < 8; i += 4) {
        verts[0 + i].pos_.x = -0.01f * halfWidth + halfWidth;
        verts[0 + i].pos_.y =
          -(1.f - rowHeight_ * row) * halfHeight + halfHeight;

        verts[1 + i].pos_.x =
          (1.f * (i ? 1 : fraction) - 0.01f) * halfWidth + halfWidth;
        verts[1 + i].pos_.y =
          -(1.f - rowHeight_ * row) * halfHeight + halfHeight;

        verts[2 + i].pos_.x = -0.01f * halfWidth + halfWidth;
        verts[2 + i].pos_.y =
          -(1.f - rowHeight_ * (row + 1) + rowSep_) * halfHeight + halfHeight;
    ...
    uint32 lockIndex = 0;
    if (vb.lockAndLoad(&verts[0], 8, lockIndex) && SUCCEEDED(vb.set(0))) {
        ...
        if (ind.valid()) {
            ...
            if (SUCCEEDED(dynamicIndexBuffer.indexBuffer().set())) {
                ...
                Moo::rc().drawIndexedPrimitive(
                  D3DPT_LINELIST, lockIndex + 4, 0, 4, startIndex + 6, 4);
            }
        }
    }
}
    ...
    for (uint i = 0; i < tasks_.size(); i++) {
        if (tasks_[i].task == &task) {
            ...
            for (uint j = 0; j < tasks_.size(); j++) {
                if (tasks_[j].task != NULL && tasks_[j].level >= depth) {
                    deepestNode_ = j;
