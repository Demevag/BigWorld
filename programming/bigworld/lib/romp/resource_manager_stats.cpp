...
void ResourceManagerStats::displayStatistics(XConsole& console)
{
    ...
    for (uint32 i = 0; i < D3DRTYPECOUNT; i++) {
        ...
        _snprintf(statString,
                  STR_SIZE - 1,
                  "Thrashing  : %s\tBytes Downloaded : %d   \n",
                  stats.bThrashing ? "Yes" : "No",
                  stats.ApproxBytesDownloaded);
        console.print(statString);
        _snprintf(statString,
                  STR_SIZE - 1,
                  "Evicts : %d\tCreates : %d\t\n",
...
void ResourceManagerStats::getData()
{
    ...
    HRESULT hr = pEventQuery_->GetData(
      &results_[frame_], sizeof(D3DRESOURCESTATS) * D3DRTYPECOUNT, 0);
...
void ResourceManagerStats::createUnmanagedObjects()
{
    BW_GUARD;
    HRESULT hr =
      Moo::rc().device()->CreateQuery(D3DQUERYTYPE_RESOURCEMANAGER, NULL);
    ...
    if (available_) {
        Moo::rc().device()->CreateQuery(D3DQUERYTYPE_RESOURCEMANAGER,
                                        &pEventQuery_);
        memset(
          &results_, 0, sizeof(D3DRESOURCESTATS) * D3DRTYPECOUNT * NUM_FRAMES);
    }
}
