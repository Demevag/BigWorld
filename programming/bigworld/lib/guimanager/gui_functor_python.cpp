...
PyObject* PythonFunctor::call(BW::string function, ItemPtr item)
{
    ...
    PyObject* func =
      PyObject_GetAttrString(modules_[module], (char*)(function.c_str()));
    if (func != NULL) {
        ...
        if (obj) {
            result =
              Script::ask(func, obj, (module + "." + function + ':').c_str());
            if (!result)
                PyErr_Clear();
        } else
            PyErr_Clear();
...
bool PythonFunctor::text(const BW::string& textor,
                         ItemPtr           item,
                         BW::string&       result)
{
    BW_GUARD;
...
bool PythonFunctor::update(const BW::string& updater,
                           ItemPtr           item,
                           unsigned int&     result)
{
    BW_GUARD;
