...
static const UINT WM_CGUITOOLBAR_REGISTER =
  RegisterWindowMessage(L"WM_CGUITOOLBAR_REGISTER_APP");
static const UINT WM_CGUITOOLBAR_UNREGISTER =
  RegisterWindowMessage(L"WM_CGUITOOLBAR_UNREGISTER");
static const UINT WM_CGUITOOLBAR_USE_WRAP =
  RegisterWindowMessage(L"WM_CGUITOOLBAR_USE_WRAP");
...
BOOL CGUIToolBar::OnWndMsg(UINT     message,
                           WPARAM   wParam,
                           LPARAM   lParam,
                           LRESULT* pResult)
{
...
CSize CGUIToolBar::CalcDynamicLayout(int nLength, DWORD nMode)
{
    ...
    if (guiToolbar_)
        guiToolbar_
          ->restoreText(); // for some reason the toolbar name gets cleared
...
Toolbar::Toolbar(const BW::string& root,
                 HWND              toolbar,
  ...
  , forceChanged_(false)
{
    ...
    sendMessage(TB_SETEXTENDEDSTYLE,
                0,
                sendMessage(TB_GETEXTENDEDSTYLE, 0, 0) |
                  TBSTYLE_EX_MIXEDBUTTONS);
    SetWindowLong(
      toolbar, GWL_STYLE, GetWindowLong(toolbar, GWL_STYLE) | TBSTYLE_LIST);

    disabledImageList_ =
      ImageList_Create(iconSize, iconSize, ILC_COLOR32, 1, 256);
    ...
    HIMAGELIST imglist;
    imglist =
      (HIMAGELIST)sendMessage(TB_SETIMAGELIST, 0, (LPARAM)normalImageList_);
    if (imglist)
        ImageList_Destroy(imglist);
    imglist =
      (HIMAGELIST)sendMessage(TB_SETHOTIMAGELIST, 0, (LPARAM)hotImageList_);
    if (imglist)
        ImageList_Destroy(imglist);
    imglist = (HIMAGELIST)sendMessage(
      TB_SETDISABLEDIMAGELIST, 0, (LPARAM)disabledImageList_);
    if (imglist)
        ImageList_Destroy(imglist);
...
int Toolbar::getImageIndex(ItemPtr item)
{
    ...
    if (imageIndices_.find(item.getObject()) == imageIndices_.end()) {
        ...
        BitmapPtr bitmap = Manager::instance().bitmaps().get(
          (*item)["imageNormal"], transparency);
        result_ = ImageList_Add(normalImageList_, (*bitmap), NULL);
        if (item->exist("imageHot"))
            ImageList_Add(hotImageList_,
                          *Manager::instance().bitmaps().get(
                            (*item)["imageHot"], transparency),
                          NULL);
        else
            ImageList_Add(hotImageList_,
                          *Manager::instance().bitmaps().get(
                            (*item)["imageNormal"], transparency, "HOVER"),
                          NULL);

        if (item->exist("imageDisabled"))
            ImageList_Add(disabledImageList_,
                          *Manager::instance().bitmaps().get(
                            (*item)["imageDisabled"], transparency),
                          NULL);
        else
            ImageList_Add(disabledImageList_,
                          *Manager::instance().bitmaps().get(
                            (*item)["imageNormal"], transparency, "DISABLED"),
                          NULL);
...
void Toolbar::changed(ItemPtr item)
{
    ...
    // normally a toolbar only has several buttons ( less than 30 ), always do a
    // full refresh
...
void Toolbar::updateSeparator(unsigned int& index,
                              ItemPtr       item,
                              TBBUTTONINFO& info)
{
    BW_GUARD;

    int width = item->exist("width") ? atoi((*item)["width"].c_str()) : 6;
        ...
        || info.iImage != width)       // width, will be configurable in future
    {
        ...
        button.iBitmap =
          width; // width of the separatar, could be configurable in future
        button.idCommand = 0;
        button.fsState   = 0;
...
void Toolbar::updateAction(unsigned int& index,
                           ItemPtr       item,
                           TBBUTTONINFO& info)
{
    BW_GUARD;
    ...
    unsigned int mask =
      BTNS_SEP | BTNS_CHECK | BTNS_GROUP | BTNS_CHECKGROUP | BTNS_DROPDOWN;
    ...
    TBBUTTONINFO buttonInfo = { sizeof(buttonInfo),
                                TBIF_TEXT | TBIF_BYINDEX | TBIF_STATE };
    BW::wstring  tooltip;
    ...
    if (forceChanged_ || info.pszText != tooltip ||
        info.fsState != buttonInfo.fsState)
        sendMessage(TB_SETBUTTONINFO, index, (LPARAM)&buttonInfo);
}

void Toolbar::updateToggle(unsigned int& index,
                           ItemPtr       item,
                           TBBUTTONINFO& info)
{
    BW_GUARD;
...
void Toolbar::updateChoice(unsigned int& index,
                           ItemPtr       item,
                           TBBUTTONINFO& info)
{
    BW_GUARD;
    ...
    while (index < (unsigned int)sendMessage(TB_BUTTONCOUNT, 0, 0) &&
        ...
        TBBUTTONINFO info = { sizeof(info),
                              TBIF_BYINDEX | TBIF_COMMAND | TBIF_IMAGE |
                                TBIF_LPARAM | TBIF_SIZE | TBIF_STATE |
                                TBIF_STYLE | TBIF_TEXT };
        info.pszText      = txtBuf;
        info.cchText      = MAX_MENU_TEXT;
        ...
        if (subItem->shortcutKey().size() || item->shortcutKey().size()) {
            if (subItem->shortcutKey().size() && item->shortcutKey().size()) {
                Localise(L"GUIMANAGER/GUI_TOOLBAR/TOOLTIP_FORM0",
                         tooltip,
                         subItem->description(),
                         subItem->shortcutKey(),
                         item->shortcutKey());
            } else if (subItem->shortcutKey().size()) {
                Localise(L"GUIMANAGER/GUI_TOOLBAR/TOOLTIP_FORM1",
                         tooltip,
                         subItem->description(),
                         subItem->shortcutKey());
            } else if (item->shortcutKey().size()) {
                Localise(L"GUIMANAGER/GUI_TOOLBAR/TOOLTIP_FORM2",
                         tooltip,
                         subItem->description(),
                         item->shortcutKey());
            }
...
void Toolbar::updateExpandedChoice(unsigned int& index,
                                   ItemPtr       item,
                                   TBBUTTONINFO& info)
{
    BW_GUARD;
    ...
    {
        int width = item->exist("width") ? atoi((*item)["width"].c_str()) : 150;
        combobox  = CreateWindow(L"COMBOBOX",
                                L"",
                                CBS_AUTOHSCROLL | CBS_DROPDOWNLIST | WS_CHILD |
                                  WS_VISIBLE,
                                0,
                                0,
                                width,
                                ...
                                NULL);
        SendMessage(
          combobox, WM_SETFONT, SendMessage(toolbar_, WM_GETFONT, 0, 0), FALSE);
        TBBUTTON button;
        memset(&button, 0, sizeof(button));
        button.iBitmap =
          width; // width of the combobox, MUST be configurable in future
        button.idCommand = 0;
        button.fsState   = 0;
        button.fsStyle   = BTNS_SEP;
    ...
    while (comboIndex < SendMessage(combobox, CB_GETCOUNT, 0, 0) &&
        ...
        size_t wbufferSize =
          SendMessage(combobox, CB_GETLBTEXTLEN, comboIndex, 0) + 1;
        wchar_t* wbuffer = (wchar_t*)_alloca(wbufferSize * sizeof(wchar_t));
        SendMessage(combobox, CB_GETLBTEXT, comboIndex, (LPARAM)wbuffer);
        BW::string buffer;
        bw_wtoutf8(wbuffer, buffer);

        if (SendMessage(combobox, CB_GETITEMDATA, comboIndex, 0) !=
              subitem->commandID() ||
            ...
            SendMessage(
              combobox, CB_SETITEMDATA, comboIndex, subitem->commandID());
        }
        if (subitem->update() &&
            !SendMessage(combobox, CB_GETDROPPEDSTATE, 0, 0))
    ...
    while (subitemIndex < item->num()) {
        ...
        bw_utf8tow(PatchDisplayName(subitem->displayName()).c_str(),
                   itemString);
        SendMessage(
          combobox, CB_INSERTSTRING, comboIndex, (LPARAM)itemString.c_str());
...
LRESULT Toolbar::wndProc(UINT msg, WPARAM wparam, LPARAM lparam)
{
    ...
    if (msg == WM_COMMAND && HIWORD(wparam) == CBN_SELCHANGE) {
        ...
        LRESULT data     = SendMessage(combobox,
                                   CB_GETITEMDATA,
...
/*static*/
bool Toolbar::createToolbars(const BW::string  appTbsSection,
                             const HwndVector& appTbsData,
                             int               iconSize /*=16*/)
{
    BW_GUARD;
    ...
    for (int i = 0; i < (int)appTbsItem->num() && hwndIt != appTbsData.end();
         ++i, ++hwndIt) {
...
void Toolbar::onSysColorChange()
{
    ...
    // the following line is used to hack the toolbar background color for
    // tradeshow
    return;
    int iconSize;
