...
namespace {
    ...
    /**
     *	This function returns the file information of the given file via
     *parameter pFileInfo. If the given pathname is not a file, it will return
     *false.
     */
    bool getFileInfo(const BW::string&      pathName,
                     IFileSystem::FileInfo* pFileInfo)
    {
        BW_GUARD;

        IFileSystem::FileType ft = BWResource::getFileType(pathName, pFileInfo);

        return ft == IFileSystem::FT_FILE || ft == IFileSystem::FT_ARCHIVE;
    }
...
void CleanChunkList::save() const
{
    ...
    for (BW::map<BW::string, ChunkInfo>::const_iterator iter =
           checkedChunks_.begin();
         iter != checkedChunks_.end();
         ++iter) {
        DataSectionPtr ds = dsDL->newSection("chunk");

        ds->writeUInt64("cdataCreationTime", iter->second.cdataCreationTime_);
        ds->writeUInt64("cdataModificationTime",
                        iter->second.cdataModificationTime_);
        ds->writeUInt64("chunkCreationTime", iter->second.chunkCreationTime_);
        ds->writeUInt64("chunkModificationTime",
                        iter->second.chunkModificationTime_);
        ds->writeString("name", iter->first);
    }

    for (BW::map<BW::string, ChunkInfo>::const_iterator iter =
           uncheckedChunks_.begin();
         iter != uncheckedChunks_.end();
         ++iter) {
        DataSectionPtr ds = dsDL->newSection("chunk");
    ...
    for (BW::set<BW::string>::iterator iter = chunks.begin();
        ...
        if (uncheckedChunks_.find(*iter) != uncheckedChunks_.end()) {
            ...
            if (getFileInfo(chunkName, &fiChunk) &&
                ...
                if (uncheckedChunks_[*iter].cdataCreationTime_ ==
                      fiCdata.created &&
                    uncheckedChunks_[*iter].cdataModificationTime_ ==
                      fiCdata.modified &&
                    uncheckedChunks_[*iter].chunkCreationTime_ ==
                      fiChunk.created &&
                    uncheckedChunks_[*iter].chunkModificationTime_ ==
                      fiChunk.modified) {
...
void CleanChunkList::update(Chunk* pChunk)
{
    ...
    if (uncheckedChunks_.find(pChunk->identifier()) != uncheckedChunks_.end()) {
    ...
    if (checkedChunks_.find(pChunk->identifier()) != checkedChunks_.end()) {
...
void CleanChunkList::markAsClean(const BW::string& chunk)
{
    ...
    if (getFileInfo(chunkName, &fiChunk) && getFileInfo(cdataName, &fiCdata)) {
        ci.cdataCreationTime_     = fiCdata.created;
        ci.cdataModificationTime_ = fiCdata.modified;
        ci.chunkCreationTime_     = fiChunk.created;
        ci.chunkModificationTime_ = fiChunk.modified;
...
DataSectionPtr CleanChunkList::getDataSection() const
{
    ...
    return BWResource::openSection(dirMapping_.path() + "space.cleanlist",
                                   true);
}
