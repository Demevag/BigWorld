...
void ClientChunkSpace::mappingSettings(DataSectionPtr pSS)
{
    ...
    if (Renderer::pInstance()->pipelineType() !=
        ...
        DecalsManager::instance().setFadeRange(
          pSS->readFloat("decals/fadeStart", 100.0f),
          pSS->readFloat("decals/fadeEnd", 120.0f));

    sunLight_ = new Moo::DirectionalLight(Moo::Colour(0.8f, 0.5f, 0.1f, 1.f),
                                          Vector3(0, 1, 0));
...
void ClientChunkSpace::focus(const Vector3& point)
{
    ...
    int         cx       = int(point.x / gridSize);
    if (point.x < 0.f)
        cx--;
    int cz = int(point.z / gridSize);
    if (point.z < 0.f)
        cz--;
    ...
    if (currentFocus_.originX() == cx && currentFocus_.originZ() == cz &&
        ...
        for (int z = cz - ColumnGrid::SPANH;
             z <= cz + ColumnGrid::SPANH && !changed;
             z++) {
            for (int x = cx - ColumnGrid::SPANH;
                 x <= cx + ColumnGrid::SPANH && !changed;
                 x++) {
                if (Column* pCol = currentFocus_(x, z)) {
                    bool soft = x <= cx - (focusRange - 1) ||
                                x >= cx + (focusRange - 1) ||
                                z <= cz - (focusRange - 1) ||
                                z >= cz + (focusRange - 1);
    ...
    for (uint i = 0; i < blurred_.size(); i++) {
        ...
        if (-focusRange <= nx && nx <= focusRange && -focusRange <= nz &&
            nz <= focusRange) {
            // skip this one if we're outta time
            if (hitTimeLimit &&
                (nx <= -(focusRange - 1) || nx >= (focusRange - 1) ||
                 nz <= -(focusRange - 1) || nz >= (focusRange - 1)))
                continue;
...
void ClientChunkSpace::tick(float dTime)
{
...

    {
        ...
        for (BW::list<ChunkItemPtr>::iterator it = pendingTickItems_.begin();
...
void ClientChunkSpace::updateAnimations()
{
...

    {
        ...
        if (tmpVisibilityUpdatesCount) {
            tmpVisibilityUpdates =
              (Chunk**)bw_alloca(sizeof(Chunk*) * tmpVisibilityUpdatesCount);
            memcpy(tmpVisibilityUpdates,
                   &visibilityUpdates_[0],
                   sizeof(Chunk*) * tmpVisibilityUpdatesCount);
...
void ClientChunkSpace::updateHeavenlyLighting()
{
    ...
    if (pOutLight_ != NULL && sunLight_) {
        ...
        Vector3 lightPos =
          pOutLight_->mainLightTransform().applyPoint(Vector3(0, 0, -1));
        Vector3 dir = Vector3(0, 0, 0) - lightPos;
        dir.normalise();
        sunLight_->direction(dir);
        sunLight_->worldTransform(
          Matrix::identity); // a hack to set worldDirection_ as direction_
...
void ClientChunkSpace::delUpdateItem(ChunkItem* pItem)
{
    ...
    if (found != updateItems_.end()) {
        ...
        BW::list<ChunkItem*>::iterator pendingFound = std::find(
          pendingUpdateItems_.begin(), pendingUpdateItems_.end(), pItem);
...
void ClientChunkSpace::delHomelessItem(ChunkItemPtr pItem)
{
    ...
    if (found != homeless_.end())
        homeless_.erase(found);
}
 ...
 */
void ClientChunkSpace::getVisibleOutsideChunks(const Matrix& viewProjection,
...
void ClientChunkSpace::removeOutsideChunkFromQuadTree(Chunk* pChunk)
{
    ...
    if (pChunk->isOutsideChunk()) {
...

        {
            ...
            // If the chunk is currently in the visibility update list, remove
            // it
            BW::vector<Chunk*>::iterator it = std::find(
              visibilityUpdates_.begin(), visibilityUpdates_.end(), pChunk);
