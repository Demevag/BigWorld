                            ...
                            const BW::string&                  name)
{
    ...
    info.vb_.create(uintBufferSize,
                    ...
                    Moo::VertexXYZL::fvf(),
                    D3DPOOL_MANAGED);
    Moo::VertexLock<Moo::VertexXYZL>(info.vb_).fill(&verts[0], uintBufferSize);
                                   ...
                                   bool                setStates)
{
    ...
    rc.drawPrimitive(D3DPT_TRIANGLELIST, 0, iter->second.primitiveCount_);
}
...
void ChunkBspHolder::getRandomBspColour(Moo::Colour& colour) const
{
    ...
    colour           = Moo::Colour(
      random(0.0f, 1.0f), random(0.0f, 1.0f), random(0.0f, 1.0f), 1.0f);
}
...
/*static*/ void ChunkBspHolder::readSettings(BspStatics& statics)
{
    ...
    if (rc.frameTimestamp() != statics.settingsMark_) {
        statics.drawBsp_ = Options::getOptionInt("render/drawBSP", 0) != 0;
