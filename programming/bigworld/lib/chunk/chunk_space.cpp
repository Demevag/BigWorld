 ...
 */
template <class X>
bool traverseObstacleTree(const SweepShape<X>&      start,
                          const Vector3&            end,
                          ...
                          const ObstacleTree&       tree)
{
    ...
    while ((pObstacle = static_cast<const ChunkObstacle*>(htt.next())) !=
           NULL) {
        ...
        if (SweepHelpers::collideIntoObject(start,
                                            sp,
                                            *pObstacle,
                                            pObstacle->bb_,
                                            pObstacle->transformInverse_,
                                            cs)) {
            return true;
        }
    }
 ...
 */
template <class X>
float ChunkSpace_collide(const ChunkSpace::ColumnGrid& currentFocus,
                         ...
                         CollisionCallback&            cc)
{
    ...
    SpaceGridTraversal sgt(
      sp.bsource(), sp.bextent(), sp.shapeRange(), gridSize);
    ...
    do {
      ...
      (!cs.onlyLess_ || (cs.dist_ + sp.shapeRadius() > sgt.cellETravel)) &&
      sgt.next());
 ...
 */
float ChunkSpace::collide(const Vector3&     start,
                          const Vector3&     end,
                          CollisionCallback& cc) const
{
    ...
    // TODO : remove for trade shows.  add back in during development (bug
    // 22332)
    MF_ASSERT_DEV(-100000.f < start.x && start.x < 100000.f &&
                  -100000.f < start.z && start.z < 100000.f);
    MF_ASSERT_DEV(-100000.f < end.x && end.x < 100000.f && -100000.f < end.z &&
                  end.z < 100000.f);
                                        ...
                                        DataSectionPtr    pSettings)
{
    ...
    if (!terrainSettingsData) // settings without terrain section
    {
        ...
        for (int i = pMapping->minLGridX(); i <= pMapping->maxLGridX(); ++i) {
            for (int j = pMapping->minLGridY(); j <= pMapping->maxLGridY();
                 ++j) {
    ...
    if (terrainSettings_->serverHeightMapLod() > 0) {
        NOTICE_MSG("ChunkSpace::addMapping: "
                   ...
                   terrainSettings_->serverHeightMapLod(),
                   path.c_str());
    }
#endif // MF_SERVER

#if !defined(MF_SERVER) && !defined(EDITOR_ENABLED)
    isNavigationEnabled_ =
      pSettings->readBool("clientNavigation/enable", false);
    ...
    for (it = currentChunks_.begin(); it != currentChunks_.end(); it++) {
        for (uint i = 0; i < it->second.size(); i++) {
            ...
            if (pChunk->mapping() != pMapping) {
                ...
                // we'll get back to you later. When loaded, this will be
                // deleted because the mapping is condemned.
            } else {
...
void ChunkSpace::dumpDebug() const
{
    ...
    for (int z = zBegin; z < zEnd; z++) {
        for (int x = xBegin; x < xEnd; x++) {
            ...
            int obstacleSize = pCol ? pCol->obstacles().size() : 0;
            total += obstacleSize;
...
{
    return BoundingBox(Vector3(gridToPoint(minGridX()),
                               MIN_CHUNK_HEIGHT,
                               gridToPoint(minGridY())),
                       Vector3(gridToPoint(maxGridX() + 1),
                               MAX_CHUNK_HEIGHT,
...
{
    return BoundingBox(Vector3(gridToPoint(minSubGridX()),
                               MIN_CHUNK_HEIGHT,
                               gridToPoint(minSubGridY())),
                       Vector3(gridToPoint(maxSubGridX() + 1),
                               MAX_CHUNK_HEIGHT,
    ...
    */
    ...
    for (GeometryMappings::iterator it = mappings_.begin();
        ...
        for (int gridx = gridmx - grido; gridx <= gridmx + grido; gridx++)
            for (int gridz = gridmz - grido; gridz <= gridmz + grido;
                 gridz++) { // start of unindented for loop (a noop unless
                            // looking inside)

                // build the chunk identifier
                BW::string gridChunkIdentifier =
                  pMapping->outsideChunkIdentifier(lpoint);
                if (gridChunkIdentifier.empty())
                    continue;
                /*
                ...
                sizeof(chunkIdentifierCStr), "%01xxxx%01xxxx/", int(gridxs >>
                12), int(gridzs >> 12) ); gridChunkIdentifier =
                chunkIdentifierCStr;
                }
                if ((gridxs + 256) >= 512 || (gridzs + 256) >= 512)
                {
                    bw_snprintf( chunkIdentifierCStr,
                sizeof(chunkIdentifierCStr), "%02xxx%02xxx/", int(gridxs >> 8),
                int(gridzs >> 8) ); gridChunkIdentifier += chunkIdentifierCStr;
                }
                if ((gridxs + 16) >= 32 || (gridzs + 16) >= 32)
                {
                    bw_snprintf( chunkIdentifierCStr,
                sizeof(chunkIdentifierCStr), "%03xx%03xx/", int(gridxs >> 4),
                int(gridzs >> 4) ); gridChunkIdentifier += chunkIdentifierCStr;
                }
                bw_snprintf( chunkIdentifierCStr, sizeof(chunkIdentifierCStr),
                "%04x%04xo", int(gridxs), int(gridzs) ); gridChunkIdentifier +=
                chunkIdentifierCStr;
                */
                ...
                if (!pOutsideDS) {
                    ...
                    pOutsideDS = pDir->openSection( gridChunkIdentifier +
                    ".chunk" ); if (!pOutsideDS) continue;
                ...
                // TODO: Don't bother opening the section if we're not looking
                // inside (ResMgr does not yet have such an interface though)

                // this could be the chunk we want if we're in the middle grid
                // square
                if (gridx == gridmx && gridz == gridmz &&
                    chunkIdentifier.empty())
                    chunkIdentifier = gridChunkIdentifier;

                // if we want to look inside, then see if there's any
                // overlappers that might do a better job
                if (lookInside) {
                    BW::vector<BW::string> overlappers;
                    pOutsideDS->readStrings("overlapper", overlappers);
                    for (uint i = 0; i < overlappers.size(); i++) {
                        DataSectionPtr pBBSect = pDir->openSection(
                          overlappers[i] + ".chunk/boundingBox");
                        if (!pBBSect)
                            continue;
                        ...
                        // DEBUG_MSG( "Considering overlapper %s
                        // (%f,%f,%f)-(%f,%f,%f)\n", 	overlappers[i].c_str(),
                        //	bb.minBounds().x, bb.minBounds().y,
                        // bb.minBounds().z, 	bb.maxBounds().x,
                        // bb.maxBounds().y, bb.maxBounds().z );
        ...
        if (pBestMapping == NULL && !chunkIdentifier.empty()) {
            ...
            if (!lookInside)
                break; // only inside chunks can beat this
        }
    }
...
void ChunkSpace::recalcGridBounds()
{
    ...
    if (mappings_.empty()) {
        ...
        for (GeometryMappings::iterator it = mappings_.begin();
            ...
            minSubGridX_ = std::min(minSubGridX_, it->second->minSubGridX());
            minSubGridY_ = std::min(minSubGridY_, it->second->minSubGridY());
            maxSubGridX_ = std::max(maxSubGridX_, it->second->maxSubGridX());
            maxSubGridY_ = std::max(maxSubGridY_, it->second->maxSubGridY());
        }
    }
...
void ChunkSpace::unloadChunks() const
{
    ...
    for (ChunkMap::const_iterator mapItr = currentChunks_.begin();
         mapItr != currentChunks_.end();
         ++mapItr) {
        const BW::vector<Chunk*>& chunkVector = mapItr->second;

        for (BW::vector<Chunk*>::const_iterator chunkItr = chunkVector.begin();
             chunkItr != chunkVector.end();
             ++chunkItr) {
    ...
    for (int x = nx - 1; x <= nx + 1; x++)
        for (int z = nz - 1; z <= nz + 1; z++) {
            if (currentFocus_.inSpan(x, z)) {
                ...
                if (rpCol != NULL) {
                    // pCol->stale();
                    //  Note: We actually delete these columns instead of
                    //  setting them as stale. This is not to save time in
                    //  focus! It is to make sure we never find anything about
                    //  this chunk in the focus grid, because that would be very
                    //  bad. (e.g. if findChunkFromPoint returned it, and we
                    //  added an item to it)
                    //
                                       ...
                                       WorldTriangle::Flags collisionFlags)
{
    ...
    pPortal->setCollisionState(pChunk, isPermissive, collisionFlags);
    ...
    pMatchingPortal->setCollisionState(
      pPortal->pChunk, isPermissive, collisionFlags);
