...
bool ServerChunkWater::load(DataSectionPtr pSection, Chunk* pChunk)
{
    ...
    // Two triangle quad
    tris_.push_back(WorldTriangle(Vector3(-0.5f, 0.f, -0.5f),
...
{
    ChunkVLOObstacle::instance(*item->chunk())
      .addModel(pModel_, transform_, item);
}
 ...
 */
bool ServerChunkWater::create(Chunk*            pChunk,
