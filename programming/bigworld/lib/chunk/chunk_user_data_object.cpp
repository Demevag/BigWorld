...
ChunkUserDataObject::ChunkUserDataObject()
  : ChunkItem(WANTS_NOTHING)
  , // TODO: what wants flags do i need?
  pType_(NULL)
 ...
 */
ChunkItemFactory::Result ChunkUserDataObject::create(Chunk*         pChunk,
...
ChunkItemFactory ChunkUserDataObject::factory_("UserDataObject",
                                               0,
                                               ChunkUserDataObject::create);

/**
 *	Load method
 */
bool ChunkUserDataObject::load(DataSectionPtr pSection,
                               Chunk*         pChunk,
                               BW::string*    pErrorString)
{
    ...
    if (pType_ == NULL) {
        if (pErrorString) {
            *pErrorString =
              "ChunkUserDataObject::load: No such UserDataObject type '" +
              type + "'";
        }
        return false;
    }
    ...
    if (idStr.empty()) {
        if (pErrorString) {
            *pErrorString =
              "ChunkUserDataObject::load: UserDataObject has no GUID";
        }
        return false;
    }
    ...
    if (pTransform) {
        ...
        initData_.direction =
          Direction3D(Vector3(m.roll(), m.pitch(), m.yaw()));
    } else {
        if (pErrorString) {
            *pErrorString =
              "ChunkUserDataObject::load: UserDataObject (of type '" + type +
              "') has no position";
        }
        return false;
    }
    ...
    if (!initData_.propertiesDS) {
        if (pErrorString) {
            *pErrorString = "ChunkUserDataObject::load: UserDataObject has no "
                            "'properties' section";
        }
        return false;
    }
...
void ChunkUserDataObjectCache::del(ChunkUserDataObject* pUserDataObject)
{
    ...
    ChunkUserDataObjects::iterator found = std::find(
      userDataObjects_.begin(), userDataObjects_.end(), pUserDataObject);
    if (found != userDataObjects_.end())
        userDataObjects_.erase(found);
}

/// Static instance object initialiser
ChunkCache::Instance<ChunkUserDataObjectCache>
  ChunkUserDataObjectCache::instance;
