...
class FocusGrid
{
  public:
    ...
    {
        return x >= cx_ - SPANH && x <= cx_ + SPANH && z >= cz_ - SPANH &&
               z <= cz_ + SPANH;
    }
...
class ClientChunkSpace : public BaseChunkSpace
{
  public:
    ...
    void heavenlyLightSource(const OutsideLighting* pOL) { pOutLight_ = pOL; }
    ...
    void getVisibleOutsideChunks(const Matrix&       viewProjection,
