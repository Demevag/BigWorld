...
class ChunkUmbra
{
  public:
    ...
    bool castShadowCullingEnabled() const
    {
        return castShadowCullingEnabled_ && umbraEnabled_;
    }
    void castShadowCullingEnabled(bool b)
    {
        cullMask_ =
          (b) ? cullMask_ | SHADOW_OBJECT : cullMask_ & ~SHADOW_OBJECT;
    ...
    void wireFrameTerrain(bool wireFrameTerrain)
    {
        wireFrameTerrain_ = wireFrameTerrain;
    }
    ...
    float shadowDistanceVisible() const
    {
        return shadowDistanceVisible_ + shadowDistanceDelta();
    }
