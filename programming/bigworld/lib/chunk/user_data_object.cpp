 ...
 *  @components{ cell }
 *  This is the unique identifier for the UserDataObject. It is common across
 * client, cell, and base applications.
 ...
 *  @components{ cell }
 *  The position attribute is the current location of the UserDataObject, in
 * world coordinates.
 ...
 *  of the current facing direction of the UserDataObject, in radians and in
 * world space. This can be set via the direction attribute.
 ...
 *  roll of the current facing direction of the UserDataObject, in radians and
 * in world space. This can be set via the direction attribute.
 *  @type Read-only Float
 */
PY_ATTRIBUTE(roll)
...
/*static*/ UserDataObjectPtr UserDataObject::findOrLoad(
  const UserDataObjectInitData& initData,
  UserDataObjectTypePtr         pType)
{
    ...
    if (!pUDO) {
        ...
        pUDO = UserDataObjectPtr(pType->newUserDataObject(initData.guid),
                          ...
                          UserDataObjectTypePtr         pType)
{
    ...
    pUserDataObjectType_->description().addToDictionary(initData.propertiesDS,
                                                        pDict);
...
/*static*/ bool UserDataObject::createRefType()
{
    ...
    PyModule_AddObject(
      module, "UnresolvedUDORefException", s_udoRefException.getObject());

    // Initialise the base user data object reference.
    PyObjectPtr pModule(PyImport_ImportModule("UserDataObjectRef"),
...
void UserDataObject::callScriptInit()
{
    ...
    PyObject* pFunction =
      PyObject_GetAttrString(this, const_cast<char*>("__init__"));

    if (pFunction != NULL) {
        PyObject* pResult =
          PyObject_CallFunction(pFunction, const_cast<char*>("()"));
        PY_ERROR_CHECK()
        Py_XDECREF(pResult);
        Py_DECREF(pFunction);
    } else {
...
void UserDataObject::callScriptDel()
{
    ...
    // Call the __init__ method of the object, if it has one.
    PyObject* pFunction =
      PyObject_GetAttrString(this, const_cast<char*>("__del__"));
...
ScriptObject UserDataObject::pyGetAttribute(const ScriptString& attrObj)
{
    ...
    if (attr[0] != '_' && strcmp(attr, "guid") != 0 && !this->isLoaded()) {
        IF_NOT_MF_ASSERT_DEV(s_udoRefException != NULL)
        {
            PyErr_Format(PyExc_AttributeError,
                         "Cannot access attribute '%s' in UserDataObject '%s', "
                         "not loaded and no exception set.",
                         attr,
                         guid_.toString().c_str());
            return ScriptObject();
        }
        ...
        BW::string excstr = "Cannot access attribute '" + BW::string(attr) +
                            "' in UserDataObject " + guid_.toString() +
                            ", it has not been loaded yet.";
        PyErr_SetString(s_udoRefException.getObject(), excstr.c_str());
        return ScriptObject();
    }
 ...
 *	this object.
 *  Therefore in this method we search to see if we have a description available
 *for the property, if not we allow it to be changed. This allows python scripts
 *to have temporary scratchpad variables. However they will not be retained if
 *the chunk is unloaded or the server is shut down.
 */
