 ...
 */
/*static*/ void StationGraph::mergeGraphs(const UniqueID&  graph1Id,
                                          const UniqueID&  graph2Id,
...
void StationGraph::loadAllChunks(GeometryMapping* pMapping)
{
    ...
    for (BW::map<UniqueID, StationGraph::Node>::iterator it = nodes_.begin();
         it != nodes_.end();
         ++it) {
        Node&      node      = it->second;
        ...
        Vector3    localPos  = Vector3(wgx * gridSize + gridSize / 2.0f,
                                   0.0f,
...
bool StationGraph::save()
{
    ...
    if (pSect_ && isReady_) {
        if (nodes_.empty()) {
            ...
            // Delete any children of pSect_ whose names do not match an id of
            // the graph.  These deleted data section nodes correspond to
            // deleted station graph nodes.  Note that we go through nodes_, not
            // edNodes_, this is because nodes_ exist, even  if nodes aren't
            // loaded.
            ...
            BW::map<UniqueID, ChunkStationNode*>::iterator it =
              edNodes_.begin();
            ...
            while (it != end) {
                ...
                if (edNode->chunk()) {
                    DataSectionPtr pNodeSect = pSect_->openSection(
                      buildNodeSectionName(edNode->id()), true);
                    ...
                    ChunkStationNode::LinkInfoConstIter nit =
                      edNode->beginLinks();
                    ChunkStationNode::LinkInfoConstIter ned =
                      edNode->endLinks();
                    while (nit != ned) {
...
/*virtual*/ bool StationGraph::isValid(BW::string& failureMsg) const
{
    ...
    if (edNodes_.size() != nodes_.size()) {
        failureMsg =
          LocaliseUTF8(L"CHUNK/INHABITANTS/STATION_GRAPH/NODE_SIZE_UNEQUAL");
        return false;
    }

    for (BW::map<UniqueID, ChunkStationNode*>::const_iterator it =
           edNodes_.begin();
         it != edNodes_.end();
         ++it) {
 ...
 */
bool StationGraph::updateNodeIds(const UniqueID&             id,
...
void StationGraph::constructFilename(const BW::string& spacePath)
{
    ...
    filename_ += ".graph";
    std::transform(
      filename_.begin(), filename_.end(), filename_.begin(), tolower);
}
