...
BSPTreeCollisionSceneProvider::BSPTreeCollisionSceneProvider(
  const BSPTree*          bsp,
...
void BSPTreeCollisionSceneProvider::feedPortals(
  CollisionSceneConsumer* consumer) const
{
    static const int LOOP_TIME_BEFORE_CHECK = 1024;
    int              loopTimes              = 0;

    for (RealWTriangleSet::const_iterator it = portalTriangles_.begin();
         it != portalTriangles_.end();
         ++it) {
        ++loopTimes;
...
{
    for (Providers::const_iterator iter = providers_.begin();
         iter != providers_.end() && !consumer->stopped();
         ++iter) {
        (*iter)->feed(consumer);
    }
...
bool CollisionSceneProviders::feedPortals(
  CollisionSceneConsumer* consumer) const
{
    for (Providers::const_iterator iter = providers_.begin();
         iter != providers_.end() && !consumer->stopped();
