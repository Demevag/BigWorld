...
PyEventReverb::PyEventReverb(SoundManager::EventReverb* pEventReverb,
                             PyTypeObject*              pType)
  : PyObjectPlus(pType)
...
void PyEventReverb::update3DAttributes()
{
    ...
    if (eventReverb_) {
        FMOD_RESULT result = eventReverb_->set3DAttributes(
          (FMOD_VECTOR*)&position_, minDistance_, maxDistance_);
        SoundManager::FMOD_ErrCheck(result,
                                    "PyEventReverb::update3DAttributes");
    }
}
...
PyObject* PyEventReverb::pyNew(PyObject* args)
{
    ...
    PyObject* reverb = SoundManager::instance().pyEventReverb(presetName);
