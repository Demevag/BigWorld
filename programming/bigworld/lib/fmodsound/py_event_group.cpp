...
PyEventGroup::PyEventGroup(SoundManager::EventGroup* pEventGroup,
                           PyTypeObject*             pType)
  : PyObjectPlus(pType)
...
#if 1
        FMOD_RESULT result = eventGroup_->getMemoryInfo(
...
bool PyEventGroup::isLoaded()
{
    ...
    if (eventGroup_ != NULL) {
        ...
        if (state & FMOD_EVENT_STATE_ERROR) {
            ERROR_MSG("PyEventGroup::isLoaded: An error has ocurred while "
                      "loaded event group.");
        }
    } else {
        PyErr_Format(PyExc_RuntimeError,
    ...
    return ((state & FMOD_EVENT_STATE_LOADING) ==
            0); // should FMOD_EVENT_STATE_READY be needed?
}
...
PyObject* PyEventGroup::pyNew(PyObject* args)
{
    ...
    PyObject* group = SoundManager::pInstance()->pyEventGroup(groupPath);
