...
class PySound : public PyObjectPlus
{
  ...
  public:
    ...
    bool    update(const Vector3& position,
                   const Vector3& orientation = Vector3::zero(),
  ...
  protected:
    ...
    static FMOD_RESULT F_CALLBACK eventCallback(FMOD_EVENT*             event,
  ...
  protected:
    ...
    {
        STATUS_PLAYING,
        STATUS_STOLEN, // When there are more PySounds than avaliable event
                       // handles,some will be stolen.
        STATUS_STOPPED
    };
    ...
    Vector3* pPosition_;
    // This is the previous position of the event, if it is 3D this will
    // determine the velocity so FMOD can calculate doppler effect.
