...
#if defined(BW_EXPORTER) || defined(__linux__) || defined(__ANDROID__) ||      \
...
inline void markAlloc(void* __restrict ptr,
                      size_t size,
                      size_t allocSize,
                      int    fill,
                      int    pad)
...
namespace BW {
...
#if ENABLE_MEMORY_DEBUG
        if (s_memoryDebug)
            s_memoryDebug->setSystemStage(stage);
#endif // ENABLE_MEMORY_DEBUG
    }
    ...
    void* Allocator::allocate(size_t size)
    {
        ...
        if (s_memoryDebug) {
            ...
            s_memoryDebug->allocate(
              ptr, info.allocSize_, size, info.allocFlags_);
        }
#endif
    ...
    void* Allocator::heapAllocateAligned(size_t       size,
                                         size_t       alignment,
                                         unsigned int heapAllocFlags)
    {
        init();
    ...
    void Allocator::heapDeallocateAligned(void*        ptr,
                                          unsigned int heapAllocFlags)
    {
        MF_ASSERT_NOALLOC(!(heapAllocFlags & IF_POOL_ALLOC));
        ...
        if (s_memoryDebug) {
            ...
            if (s_memoryDebug->shouldReportCacheGrind()) {
                ...
                if (BWUtil::getExecutablePath(exePath, ARRAY_SIZE(exePath))) {
                    ...
                    strncpy(
                      callgrindPath, "callgrind.bw", ARRAY_SIZE(callgrindPath));
                }
    ...
    void Allocator::init()
    {
        if (!s_initialised) {
            ...
            // check to see if user has specified a BW_GUARD to generate
            // complete callstacks for note : we cannot use sscanf here as that
            // internally allocates memory so we need to
            ...
            s_fixedSizedAllocator = new (s_fixedSizedAllocatorMem)
              BW::FixedSizedAllocator(sizes, numSizes, poolSize, "System");
            ...
            BW::Memhook::AllocFuncs allocFuncs = {
                ...
                ::bw_new /* nothrow */,
                ::bw_new_array,
                ::bw_new_array /* nothrow */,
                ::bw_delete,
                ::bw_delete /* nothrow */,
                ::bw_delete_array,
                ::bw_delete_array /* nothrow */,
                ::bw_malloc,
                ::bw_free,
                ::bw_malloc_aligned,
                ::bw_free_aligned,
            ...
            if (!disabled) {
                s_memoryDebug =
                  new (s_memoryDebugMem) BW::MemoryDebug(callstackMode,
                                                         reportHotspots,
                                                         reportIgnoredLeaks,
                                                         reportCacheGrind);
                if (debugGuard) {
    ...
    void* Allocator::heapReallocate(void*        ptr,
                                    size_t       size,
                                    unsigned int heapAllocFlags)
    {
#if ENABLE_MEMORY_DEBUG
        ...
        if (s_memoryDebug && !(heapAllocFlags & IF_NOTRACK_ALLOC)) {
            s_memoryDebug->allocate(
              ptr, SystemAllocator::memorySize(ptr), size, heapAllocFlags);
        }
#endif
...
void BW::Allocator::trackSmartPointerAssignment(const void* smartPointer,
                                                const void* object)
{
#if ENABLE_MEMORY_DEBUG
    if (s_memoryDebug) {
