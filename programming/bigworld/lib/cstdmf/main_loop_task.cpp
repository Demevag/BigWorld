...
void MainLoopTasks::updateAnimations(float dGameTime) const
{
    ...
    for (OrderList::const_iterator it = order_.begin(); it != order_.end();
         ++it) {
...
void MainLoopTasks::add(MainLoopTask* pTask, const char* name, ...)
{
    ...
    if (found != pGroup->tasks_.end()) {
        ...
        for (size_t index = found - pGroup->tasks_.begin(); index > 0;
             index--) {
            while (*rit++ != NULL)
                ; // scan
        }
        ...
        pGroup->rules_.insert(
          pGroup->rules_.begin() + rindex, newRules.begin(), newRules.end());
    } else {
        if (pTask == NULL) {
        ...
        pGroup->rules_.insert(
          pGroup->rules_.end(), newRules.begin(), newRules.end());
...
void MainLoopTasks::del(MainLoopTask* pTask, const char* name)
{
    ...
    while ((slash = strchr(rname, '/')) != NULL) {
        ...
        if (found == pGroup->tasks_.end()) { // break if there's no such group
...
class AlphaOrderPredicate
{
  public:
    ...
    {
        return strcmp((smap_.begin() + a)->first, (smap_.begin() + b)->first) <
               0;
    }
...
void MainLoopTasks::sort()
{
    ...
    if (!finished_) {
        ...
        for (RulesList::iterator it = rules_.begin(); it != rules_.end();
             it++) {
            while (*it != NULL) {
                const char* arule = *it;
        ...
        for (uint t = 0; t < tasks_.size(); t++) {
            ...
            for (ai = 0; ai < tasks_.size(); ai++)
                if (prereqs[index = alphaOrder[ai]].empty())
                    break;
            if (ai == tasks_.size()) {
