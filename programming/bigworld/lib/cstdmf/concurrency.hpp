...
inline LONG InterlockedCompareExchangeChecked(LONG volatile* dest,
                                              LONG           newval,
                                              LONG           oldval)
{
...
#define BW_ATOMIC32_INC_AND_FETCH(dest) InterlockedIncrementChecked((dest))
#define BW_ATOMIC32_DEC_AND_FETCH(dest) InterlockedDecrementChecked((dest))
...
#define BW_ATOMIC32_DEC_AND_FETCH(dest) InterlockedDecrement((dest))
#define BW_ATOMIC32_FETCH_AND_ADD(dest, value)                                 \
...
static inline bw_atomic32_t __sync_val_compare_and_swap_checked(
  bw_atomic32_t* dest,
  bw_atomic32_t  oldval,
  bw_atomic32_t  newval)
{
    MF_DEBUG_ASSERT_ALIGNED(dest, 4);
    return __sync_val_compare_and_swap(dest, oldval, newval);
}

static inline bw_atomic32_t __sync_add_and_fetch_checked(bw_atomic32_t* dest,
                                                         bw_atomic32_t  value)
{
    MF_DEBUG_ASSERT_ALIGNED(dest, 4);
...
static inline bw_atomic32_t __sync_sub_and_fetch_checked(bw_atomic32_t* dest,
                                                         bw_atomic32_t  value)
{
    MF_DEBUG_ASSERT_ALIGNED(dest, 4);
...
static inline bw_atomic32_t __sync_fetch_and_add_checked(bw_atomic32_t* dest,
                                                         bw_atomic32_t  value)
{
    MF_DEBUG_ASSERT_ALIGNED(dest, 4);
...
static inline bw_atomic32_t __sync_fetch_and_sub_checked(bw_atomic32_t* dest,
                                                         bw_atomic32_t  value)
{
    MF_DEBUG_ASSERT_ALIGNED(dest, 4);
...
#define BW_ATOMIC32_COMPARE_AND_SWAP(dest, newval, oldval)                     \
    __sync_val_compare_and_swap_checked((dest), (oldval), (newval))
...
#define BW_ATOMIC32_FETCH_AND_ADD(dest, value)                                 \
    __sync_fetch_and_add_checked((dest), (value))
#define BW_ATOMIC32_FETCH_AND_SUB(dest, value)                                 \
    __sync_fetch_and_sub_checked((dest), (value))
...
{
    ThreadTrampolineInfo(SimpleThreadFunc func, void* arg, SimpleThread* thread)
    {
        this->func    = func;
        this->arg     = arg;
        this->thread_ = thread;
 ...
 *					recorded - all threads will be frozen after the call, even
 *if there were more threads then maxSize.
 *
 * @return actual amount of threads recorded into array (zero if threads ==
 *NULL)
 */
...
{
  public:
    SimpleEvent(bool manReset = true, bool initVal = false, LPCTSTR name = NULL)
    {
        event_ = CreateEvent(0, manReset, initVal, name);
        MF_ASSERT(event_ != NULL);
    }
    ~SimpleEvent() { CloseHandle(event_); }
...
{
  public:
    SimpleEventCounter(long    initialValue,
                       bool    manReset = true,
                       bool    initVal  = false,
                       LPCTSTR name     = NULL)
    ...
    void increment()
    {
        if (InterlockedIncrement(&counter_) == 1) {
            this->reset(); // first time counter is non zero, signal the event
                           // to wait if asked
        }
    }
    ...
    bool wait() { return SimpleEvent::wait(); }
...
class SimpleSemaphore
{
  public:
    SimpleSemaphore()
      : sema_(CreateSemaphore(NULL, 0, 32767, NULL))
    {
    }
    ...
    bool pullTry(unsigned milliseconds)
    {
        return WaitForSingleObject(sema_, milliseconds) == WAIT_OBJECT_0;
    }
...
class SimpleThread
{
  public:
    ...
    CSTDMF_DLL SimpleThread(SimpleThreadFunc  threadfunc,
                            void*             arg,
    ...
    CSTDMF_DLL void init(SimpleThreadFunc  threadfunc,
                         void*             arg,
                         const BW::string& threadName);
  ...
  private:
    ...
    /*
     * this trampoline function is present so that we can hide the fact that
     * windows and linux expect different function signatures for thread
     * functions
     */
...
class ThreadLocal
{
  public:
    ...
    T operator++(int) { return this->get()++; }

    const T& operator--() { return --this->get(); }

    T operator--(int) { return this->get()--; }

    const T& operator+=(const T& inc) { return this->get() += inc; }

    const T& operator-=(const T& dec) { return this->get() -= dec; }
...
{
  public:
    SimpleSemaphore() { sem_init(&sem_, /* pshared */ 0, /* value */ 0); }
    ~SimpleSemaphore() { sem_destroy(&sem_); }
    ...
    bool pullTry() { return sem_trywait(&sem_) == 0; }
...
class ReadWriteLock
{
  ...
  public:
    ReadWriteLock() { MF_VERIFY(pthread_rwlock_init(&lock_, NULL) == 0); }

    ~ReadWriteLock() { MF_VERIFY(pthread_rwlock_destroy(&lock_) == 0); }

    void beginRead() { MF_VERIFY(pthread_rwlock_rdlock(&lock_) == 0); }

    void endRead() { MF_VERIFY(pthread_rwlock_unlock(&lock_) == 0); }

    void beginWrite() { MF_VERIFY(pthread_rwlock_wrlock(&lock_) == 0); }

    void endWrite() { MF_VERIFY(pthread_rwlock_unlock(&lock_) == 0); }
...
class SimpleThread
{
  public:
    ...
    SimpleThread(SimpleThreadFunc  threadfunc,
                 void*             arg,
                 const BW::string& threadName);
...
{
  public:
    SimpleMutexHolder(SimpleMutex& sm)
      : sm_(sm)
    {
        sm_.grab();
    }
...
class RecursiveMutexHolder
{
  public:
    ...
    int recursiveIndex() const { return index_; }

    bool isOwner() const { return index_ == 1; }
...
namespace BWConcurrency {
    ...
    inline void setMainThreadIdleFunctions(void (*pStartFunc)(),
                                           void (*pEndFunc)())
    {
