...
TEST(Allocator_testHeapReallocNew)
{
    char* string = NULL;
    string =
      reinterpret_cast<char*>(BW::Allocator::heapReallocate(string, 128));
    CHECK(string);
    BW::Allocator::heapDeallocate(string);
}
...
void checkString(const char* string,
                 const char* var,
                 size_t      len,
                 TestResult& result_,
                 const char* m_name)
...
void runTestFull1(const size_t size, TestResult& result_, const char* m_name);
...
void runTestFull1(const size_t size, TestResult& result_, const char* m_name)
{
    ...
    stringManager.setLargeAllocationHooks(&BW::Allocator::heapAllocate,
    ...
    const size_t initialNumFree(stringManager.getFirstPoolNumFreeForSize(size));
    ...
    {
        const size_t numUsed(stringManager.getNumPoolItemsForSize(size) -
    ...
    CHECK_EQUAL(initialNumFree, stringManager.getFirstPoolNumFreeForSize(size));
...
void runTestFull2(const size_t size, TestResult& result_, const char* m_name);
...
void runTestFull2(const size_t size, TestResult& result_, const char* m_name)
{
    ...
    secondPoolPointers[secondPoolAllocCount] = stringManager.allocate(size);
