...
FILE* const BinaryFile::FILE_INTERNAL_BUFFER = reinterpret_cast<FILE*>(-1);
...
size_t BinaryFile::bytesAvailable()
{
    ...
    if (ptr_) {
        return reinterpret_cast<unsigned char*>(data_) + bytesCached_ -
               reinterpret_cast<unsigned char*>(ptr_);
    }
...
size_t BinaryFile::write(const void* buffer, size_t nbytes)
{
    ...
    MF_ASSERT((file_ == NULL) || (file_ == FILE_INTERNAL_BUFFER) ||
              (ptr_ == NULL));
    ...
    if (ptr_) {
        ...
        if ((bytesAvail < nbytes) && (file_ == FILE_INTERNAL_BUFFER) &&
            ...
            const size_t growBufferSize = std::min(
              MAX_MANAGED_BUFFER_SIZE, static_cast<size_t>(bytesCached_) * 2);
