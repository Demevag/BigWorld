 ...
 *	This function prints a debug message
 */
void dprintf(DebugMessagePriority messagePriority,
             const char*          category,
             const char*          format,
             ...)
{
...
size_t convertTime(char* buf, size_t size, const timeval* time)
{
    ...
    if (result != 0) {
        result += bw_snprintf(
          buf + result, size - result, ".%03ld", time->tv_usec / 1000);
    }
    return result;
}
...
/**
 *	This function prints a debug message, with a specified priorityname, & a
 *specified output io file It is called by the other vdprintf functions.
 */
void vdprintf(const char* format,
              va_list     argPtr,
              const char* pPriorityName,
 ...
 *		Since we have no debugmessagepriority, we send the output to the
 *DebugFilter::consoleOutputFIle() which by default is stderr.
 */
void vdprintf(const char* format,
              va_list     argPtr,
              const char* pPriorityName,
...
/**
 *	This function prints a debug message using the debugMessagePriority name &
 *sends it to the io defined by the debugMessagePriority.
 */
void vdprintf(const char*          format,
              va_list              argPtr,
              DebugMessagePriority messagePriority,
