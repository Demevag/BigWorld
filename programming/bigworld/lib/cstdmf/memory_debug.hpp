...
namespace BW {
    ...
    namespace detail {
        ...
        class DebugStlAllocator
        {
          public:
            ...
            {
                return (typename std::allocator<T>::pointer)baseAllocator_
                  ->allocate(n * sizeof(T), Allocator::IF_DEBUG_ALLOC);
            }
    ...
    class MemoryDebug
    {
      public:
        ...
        {
            AF_ALLOCATION_PREMAIN =
              1,                    ///< Allocation occured before main() called
            AF_ALLOCATION_POOL = 2, ///< Allocation came from a fixed sized pool
            AF_ALLOCATION_INTERNAL =
              4, ///< Allocation is an internal heap allocation (e.g. fixed
                 ///< sized pool memory)
            AF_ALLOCATION_IGNORELEAK =
              8, ///< Allocation has been flagged as allowed to leak (should
                 ///< only be used for thirdparty allocations)
        };
        ...
        struct CallstackHeader
        {
            ...
            static const CallstackAddressType* callstack(
              const CallstackHeader* hdr);
            static StackTracker::StackItem* guardstack(CallstackHeader* hdr);
            static const StackTracker::StackItem* guardstack(
              const CallstackHeader* hdr);
        };

        struct CallstackStats
        {
            int32 allocCount; ///< number of allocations that came from this
                              ///< callstack
#ifdef MF_SERVER
      ...
      public:
        /// constructor
        MemoryDebug(CallstackMode csMode,
                    bool          reportHotspots,
        ...
        /// \param sizeUsed The size used by the allocation as returned by
        /// memsize \param sizeRequested The size requested for the allocation
        /// \param flags Allocator::InfoFlags describing the allocation
        void allocate(const void* ptr,
                      size_t      sizeUsed,
        ...
        /// Saves high-level allocation stats to file (slot based stats in CSV
        /// format)
        ...
        /// Begin flagging all allocations by the current thread as
        /// AF_ALLOCATION_IGNORELEAK
        void allocTrackingIgnoreBegin();

        /// Stop flagging allocations by the current thread as
        /// AF_ALLOCATION_IGNORELEAK
        void allocTrackingIgnoreEnd();

        /// Record the assignment of a smart pointer to a given value
        void trackSmartPointerAssignment(const void* smartPointer,
                                         const void* object);
      ...
      private:
        ...
        struct HashCallstackHeaderPtr
          : public std::unary_function<const CallstackHeader*, std::size_t>
        {
            std::size_t operator()(const CallstackHeader*) const;
        };

        struct EqualsCallstackHeaderPtr
          : public std::binary_function<const CallstackHeader*,
                                        const CallstackHeader*,
                                        bool>
        {
        ...
        typedef BW::unordered_map<
          const void*,
          SmartPointerInfo,
          BW::hash<const void*>,
          std::equal_to<const void*>,
          detail::DebugStlAllocator<
            std::pair<const void* const, SmartPointerInfo>>>
          SmartPointerMap;
        typedef BW::unordered_map<
          const CallstackHeader*,
          CallstackStats,
        ...
        typedef BW::unordered_map<
          ...
          std::equal_to<const void*>,
          detail::DebugStlAllocator<
            std::pair<const void* const, AllocationInfo>>>
          LiveAllocationMap;
        typedef BW::unordered_map<
          const void*,
          AddressLookupCacheEntry,
          ...
          std::equal_to<const void*>,
          detail::DebugStlAllocator<
            std::pair<const void* const, AddressLookupCacheEntry>>>
          LookupCacheMap;
        typedef BW::unordered_set<const char*,
                                  HashCStr,
                                  EqualsCStr,
                                  detail::DebugStlAllocator<const char*>>
          HashStringMap;
        ...
        /// callstack generated when we are initialised, used to decided whether
        /// we need to do a more complete stack trace
        ...
        // static memory areas to allocate internal structures to avoid hitting
        // the heap
        ...
        uint recordGuardCallstack(StackTracker::StackItem* pBuffer,
                                  uint                     length,
        ...
        void        formatShortGuardStack(const StackTracker::StackItem* items,
                                          size_t                         numItems,
                                          BW::StringBuilder&             builder);
        void        formatLongGuardStack(const StackTracker::StackItem* items,
                                         size_t                         numItems,
                                         BW::StringBuilder&             builder);
        ...
        bool cachedResolve(const CallstackAddressType address,
                           ...
                           const char**               ppFilename = NULL,
                           int*                       ppLineno   = NULL);
        void formatShortCallstack(const CallstackAddressType* items,
                                  size_t                      numItems,
                                  BW::StringBuilder&          builder);
        void formatLongCallstack(const CallstackAddressType* callstack,
                                 int                         depth,
                                 BW::StringBuilder&          builder);
#endif

        friend bool sortCallstacksByAllocCount(const CallstackHeader* lhs_,
                                               const CallstackHeader* rhs_);
    };
