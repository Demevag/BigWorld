...
THREADLOCAL(StackTracker::AnnotationEntry)
StackTracker::annotationEntries_[MAX_ANNOTATION_STACK_ENTRIES];
THREADLOCAL(char*) StackTracker::nextAnnotationPtr_ = NULL;
THREADLOCAL(StackTracker::AnnotationEntry*)
StackTracker::nextAnnotationEntryPtr_                      = NULL;
...
#if defined(_DEBUG)
uint StackTracker::getMaxStackPos()
{
    return maxStackPos_;
}
#endif
...
/**
 *  This function pushes string annotation to the current callstack entry
 * recorded by BW_GUARD. Annotation string is dynamic and will be copied in the
 * thread local annotation buffer along with the current stack index
 *  collectCallstack functions will embed annotation data into the generated
 * callstacks
 */
void StackTracker::pushAnnotation(const char* annotation)
{
    if (numAnnotationsToSkipPop_ > 0) {
        // early out if we are out memory and already skipped at least one
        // annotation
    ...
    if (dst < dstEnd && nextAnnotationEntryPtr_ !=
                          annotationEntries_ + ARRAY_SIZE(annotationEntries_)) {
