...
uint64& DogWatchManager::grabSliceMissedCache(int id)
{
    ...
    TablePtr  pTable =
      (pte.tableid != -1) ? tables_[pte.tableid] : this->newTable(pte);
    ...
    uint64* pSlice =
      &(*((cte.statid != -1) ? stats_[cte.statid]
                             : this->newManifestation(cte, id)))[slice_];
...
void DogWatchManager::tick()
{
    ...
    while (stack_.size() > 1) {
        DogWatch* pWatch = watches_[stats_[stack_.back()->statid]->watchid];
        pWatch->stop();
