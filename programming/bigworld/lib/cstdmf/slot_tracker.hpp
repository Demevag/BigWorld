...
#define MEMTRACKER_END() BW::ThreadSlotTracker::instance().end()

#define MEMTRACKER_SCOPED(id)                                                  \
    BW::ScopedSlotTracker g_scopedSlotTracker_##id(g_trackerSlot_##id)

#define MEMTRACKER_REFERENCE(id) extern ScopedSlot g_trackerSlot_##id;

#define MEMTRACKER_BREAK_ON_ALLOC(slotId, allocId)                             \
    ScopedSlotBreak break_##slotTrackerId_##allocId(g_trackerSlot_##slotId,    \
                                                    allocId)
...
namespace BW {
    ...
    {
      public:
        ScopedSlotTracker(int id) { ThreadSlotTracker::instance().begin(id); }
        ~ScopedSlotTracker() { ThreadSlotTracker::instance().end(); }
    };
    ...
    class ScopedSlot
    {
      ...
      public:
        ...
        ~ScopedSlot() { ThreadSlotTracker::instance().popSlot(id_); }
        operator uint() { return id_; }
    };
    ...
    class ScopedSlotBreak
    {
      ...
      public:
        ...
        ~ScopedSlotBreak() { ThreadSlotTracker::instance().popBreak(id_); }
        operator uint() { return id_; }
    };
