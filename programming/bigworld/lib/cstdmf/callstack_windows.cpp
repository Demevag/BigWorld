...
typedef USHORT(__stdcall* RtlCaptureStackBackTraceFuncType)(
  ULONG  FramesToSkip,
...
typedef BOOL(__stdcall* SymInitializeFuncType)(HANDLE hProcess,
                                               PSTR   UserSearchPath,
                                               BOOL   fInvadeProcess);
typedef DWORD(__stdcall* SymGetOptionsFuncType)(VOID);
typedef DWORD(__stdcall* SymSetOptionsFuncType)(DWORD SymOptions);
typedef BOOL(__stdcall* SymGetSearchPathFuncType)(HANDLE hProcess,
                                                  PSTR   SearchPath,
                                                  DWORD  SearchPathLength);
typedef DWORD64(__stdcall* SymLoadModule64FuncType)(HANDLE  hProcess,
                                                    HANDLE  hFile,
                                                    PSTR    ImageName,
                                                    PSTR    ModuleName,
                                                    DWORD64 BaseOfDll,
                                                    DWORD   SizeOfDll);
typedef BOOL(__stdcall* StackWalk64FuncType)(
  DWORD                            MachineType,
  HANDLE                           hProcess,
  HANDLE                           hThread,
...
typedef BOOL(__stdcall* SymGetSymFromAddr64FuncType)(HANDLE   hProcess,
                                                     DWORD64  qwAddr,
                                                     PDWORD64 pdwDisplacement,
                                                     PIMAGEHLP_SYMBOL64 Symbol);
typedef BOOL(__stdcall* SymGetLineFromAddr64FuncType)(HANDLE  hProcess,
                                                      DWORD64 qwAddr,
                                                      PDWORD  pdwDisplacement,
                                                      PIMAGEHLP_LINE64 Line64);
typedef BOOL(__stdcall* SymCleanupFuncType)(HANDLE hProcess);
typedef BOOL(__stdcall* SymFromAddrFuncType)(HANDLE       hProcess,
                                             DWORD64      Address,
                                             PDWORD64     Displacement,
                                             PSYMBOL_INFO Symbol);
typedef PVOID(__stdcall* SymFunctionTableAccess64FuncType)(HANDLE  hProcess,
                                                           DWORD64 AddrBase);
typedef DWORD64(__stdcall* SymGetModuleBase64FuncType)(HANDLE  hProcess,
                                                       DWORD64 AddrBase);
...
namespace BW {
    ...
    void initStacktraces()
    {
        ...
        if (init) {
            ...
            MF_ASSERT(kernel32);
            if (dbghelp == NULL) {
                dprintf("Failed to initialise symbol lookups : Cannot load "
                        "dbghelp.dll\n");
                return;
            }
            ...
            // build PDB path that should be the same as executable path
            {
                ...
                // check that the pdb actually exists and is accessible, if it
                // doesn't then SymInitialize will raise an obscure error dialog
                ...
                *pend      = 0;
                ...
                if (f == NULL) {
                    dprintf("Failed to initialise symbol lookups : %s does not "
            ...
            SymSetOptionsFunc(SYMOPT_LOAD_LINES | SYMOPT_DEFERRED_LOADS |
                              SYMOPT_UNDNAME);
            if (SymInitializeFunc(
                  ::GetCurrentProcess(), (PSTR)builder.string(), TRUE)) {
    ...
    int stacktrace(CallstackAddressType* trace,
                   size_t                depth,
                   size_t                skipFrames /*= 0*/)
    {
        MF_ASSERT(depth <= ULONG_MAX);
        initStacktraces();
        size_t r = RtlCaptureStackBackTraceFunc(
          1 + (ULONG)skipFrames, (ULONG)depth, trace, NULL);
        return (int)r;
    }
...
#if (_MSC_VER == 1500)
// RtlCaptureContextFunc crashes in hybrid on Visual Studio 2008 if
// optimisations are enabled for this function
#pragma optimize("", off)
#pragma warning(push)
                           ...
                           int                   threadID /*= 0*/)
    {
        ...
        if (threadID != 0) {
            const DWORD permissions =
              THREAD_GET_CONTEXT | READ_CONTROL | THREAD_QUERY_INFORMATION;
            ...
            // From MSDN: "Calling the CloseHandle function with this handle has
            // no effect."
...
#pragma warning(pop)
                                  ...
                                  int*                 line)
    {
        ...
        const int       SymbolBufferSize =
          sizeof(SYMBOL_INFO) + (MaxSymbolNameLength * sizeof(TCHAR)) - 1;
        ...
        // lookup file and line info for this address, but only if user wants
        // the information
        if ((fileBuf || line) &&
            SymGetLineFromAddr64Func(
              process, (DWORD64)address, &displacement, &source_info)) {
            if (fileBuf && source_info.FileName != 0) {
                strncpy_s(
                  fileBuf, fileBufSize, source_info.FileName, _TRUNCATE);
            }
        ...
        if (nameBuf &&
            SymFromAddrFunc(
              process, (DWORD64)address, &displacement64, function_info)) {
            ...
            if (VirtualQuery(address, &mbi, sizeof(mbi)) &&
                GetModuleFileNameA(
                  (HMODULE)mbi.AllocationBase, fullPath, sizeof(fullPath))) {
                // Get base name of DLL
                char* filename = strrchr(fullPath, '\\');
                strncpy_s(nameBuf,
                          nameBufSize,
                          filename == NULL ? fullPath : (filename + 1),
                          _TRUNCATE);
            } else {
