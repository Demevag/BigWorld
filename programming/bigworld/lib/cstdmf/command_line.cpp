 ...
 *
 */
const char* CommandLine::getFullParam(const char* paramToFind,
                                      int         paramIndex) const
{
    if (paramIndex > 0) {
        ...
        for (Params::const_iterator it = params_.begin(); it != params_.end();
             ++it) {
                                  ...
                                  const char* commandSwitch) const
{
    ...
    for (size_t i = 0; i < switchNum && !found; ++i) {
        for (Params::const_iterator iter = params_.begin();
            ...
            if (str[0] == commandSwitch[i] && strcmp(str + 1, param) == 0) {
                found = true;
                ++iter;
