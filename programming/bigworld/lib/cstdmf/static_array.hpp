 ...
 * It provides a fixed size array to work with, and asserts if you trying to
 * grow
 *
...
class StaticStoragePolicy
{
  public:
    enum
    {
        ARRAY_SIZE = INITIAL_COUNT
    };

    /**
     * This method returns fixed array size
     */
    inline size_t capacity() const { return ARRAY_SIZE; }
  ...
  protected:
     ...
     */
    inline void grow(size_t requestedSize) { MF_ASSERT(0); }
 ...
 * It behaves the same way as ExpandableEmbeddedStoragePolicy, the only
 * difference is it displays warning on each heap allocation This is to help
 * tweak INITIAL_COUNT values for ExpandableEmbeddedStoragePolicy to avoid
 * unnecessary heap allocations in runtime. Heap allocation suppose to be
 * non-frequent operation, just to keep program going instead of crashing hard.
 * This is especially important for the server.
 *
 */
template <typename TYPE, size_t INITIAL_COUNT>
  ...
  public:
    ...
    inline void grow(size_t requestedSize)
    {
       ...
       // display warning on grow
            WARNING_MSG(
              "Expanding embedded array with the initial COUNT %" PRIzu
              " to %" PRIzu " in\n\"%s\"\n"
 ...
 * and provide an extra benefit of completely avoiding runtime memory
 *allocations.
 *
...
class ArrayBase : public StoragePolicy<TYPE, COUNT>
{
  public:
     ...
     */
    inline TYPE* data() { return this->data_; }

    /**
     * Iterator methods
     */
    inline iterator       begin() { return this->data_; }
    inline const_iterator begin() const { return this->data_; }

    inline typename ArrayBase<TYPE, COUNT, StoragePolicy>::iterator end()
    {
        return this->data_ + this->size_;
    }

    inline typename ArrayBase<TYPE, COUNT, StoragePolicy>::const_iterator end()
      const
    {
        return this->data_ + this->size_;
    }
...
class StaticArray : public ArrayBase<TYPE, COUNT, StaticStoragePolicy>
{
  public:
    /**
     * Due to template issues in gcc all constructors have to be empty,
     * initDefault does constructor work
     */
    StaticArray(size_t initialSize = 0) { this->initDefault(initialSize); }
};
 ...
 * Default constructor behavior is identical to BW::vector
 *  i.e. construct an empty container, with no elements and a size of zero
 * DynamicArray( size_t n ) constructor initialises array with the size n
 * elements Note that n can't be bigger than template COUNT parameter
 *
 ...
 * This is to help tweak INITIAL_COUNT values for
 * ExpandableEmbeddedStoragePolicy to avoid unnecessary heap allocations in
 * runtime.
 *
 * Default constructor behavior is identical to BW::vector
 *  i.e. construct an empty container, with no elements and a size of zero
 ...
 */
template <typename TYPE>
class DynamicArray : public ArrayBase<TYPE, 0, DynamicStoragePolicy>
{
  public:
    /**
     * Due to template issues in gcc all constructors have to be empty,
     * initDefault does constructor work
     */
    DynamicArray(size_t initialSize = 0) { this->initDefault(initialSize); }
};
 ...
 */
template <typename TYPE>
class ExternalArray : public ArrayBase<TYPE, 0, ExternalStoragePolicy>
{
  public:
    ExternalArray() { this->init(NULL, 0, 0); }
    ...
    void reset() { this->init(NULL, 0, 0); }

    void assignExternalStorage(TYPE* p, size_t capacity, size_t size)
    {
        this->init(p, capacity, size);
    }

    bool valid() const { return this->data_ != NULL; }
};

template <typename IndexType>
class IndexSpan
{
    IndexType first_, last_;
