...
namespace // (anonymous)
{
    ...
    /**
     *	This CheckOStream::Impl subclass is used where the underlying stream is
     *a MemoryOStream. No extra buffer is needed in this case, and the checksum
    ...
    class UnbufferedChecksumOStreamImpl : public ChecksumOStream::Impl
    {
      public:
                              ...
                              BinaryOStream*    pChecksum)
        {
            ...
            // take advantage of rewind to reset the stream to where
            // checksumming was started.

            int payloadLength = wrappedStream_.size() - startOffset_;

            char* pCurrentRead = (char*)wrappedStream_.retrieve(0);
            int   readProgress =
              (int)(pCurrentRead - (char*)wrappedStream_.data());

            wrappedStream_.retrieve(startOffset_ - readProgress);

            pScheme->readBlob((char*)wrappedStream_.retrieve(0), payloadLength);
    ...
    /**
     *	This CheckOStream::Impl subclass is used where the underlying stream is
     *not known to be a MemoryOStream. Stream data is stored in a buffer, to be
    ...
    class BufferedChecksumOStreamImpl : public ChecksumOStream::Impl
    {
      public:
        ...
        virtual void finalise(ChecksumSchemePtr pScheme,
                              BinaryOStream*    pChecksum)
        {
            // Everything given to the stream since checksumming started is in
            // the buffer, add a checksum onto the end of the buffer and then
            // transfer data to the wrapped stream.
...
{
    size_t streamSize =
      (pChecksumScheme_.get() ? pChecksumScheme_->streamSize() : 0U);
