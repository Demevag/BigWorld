...
bool ArgParser::parse(int argc, char* argv[])
{
    for (int i = 1; i < argc; ++i) {
        ...
        if (strncmp(argv[i], "--", 2) == 0) {
            ...
            BW::string name =
              pSeperator ? BW::string(argv[i] + 2, pSeperator - argv[i] - 2)
                         : BW::string(argv[i] + 2);
    ...
    while (presenceIt != args_.end()) {
        if (!presenceIt->second->isOptional && !presenceIt->second->isPresent) {
            break;
        }
        ++presenceIt;
    }
 ...
 * 	@return	Arguement value if found, default value otherwise.
 */
unsigned int ArgParser::get(const char* name, const unsigned int& defaultValue)
{
    char* value = this->value(name);
