...
template <size_t indexBits,
          size_t generationBits = (32 - indexBits),
...
struct Handle
{
    ...
    operator baseType() { return index_ | (generation_ << GENERATION_SHIFT); }
...
class HandleTable
{
  public:
    ...
    HandleType create()
    {
        ...
        return HandleType((typename HandleType::base)entryIndex,
...
template <typename ObjectType,
          typename HandleType   = DefaultHandle,
...
class PackedObjectPool
{
  public:
    ...
    size_t capacity() const { return innerLookup_.capacity(); }

    size_t size() const { return innerLookup_.size(); }
    ...
    ObjectType& operator[](const HandleType& handle) { return lookup(handle); }
    ...
    iterator begin() { return innerLookup_.begin(); }

    iterator end() { return innerLookup_.end(); }

    const_iterator begin() const { return innerLookup_.begin(); }

    const_iterator end() const { return innerLookup_.end(); }

    ObjectContainer& container() { return innerLookup_; }
