...
namespace BW {
      ...
      public:
        ...
        class Hooks
        {
          public:
            ...
            typedef ProfilerStringHandle (*CreateStringHandleHook)(
              const char* string);
            typedef ProfilerFrameHandle (*EnterFrameHook)(
              ProfilerFrameFlags   flags,
    ...
    class ScopedProfilerFrame
    {
      public:
        ...
        ~ScopedProfilerFrame() { ExternalProfiler::leaveFrame(frameHandle_); }
    };
...
#define PROFILER_UNIQUE_VAR_NAME(prefix, token)                                \
    PROFILER_CONCAT_HELPER(prefix, token)
#define PROFILER_SCOPE_NAME(token) PROFILER_UNIQUE_VAR_NAME(prof_, token)
#define PROFILER_STRINGHANDLE_NAME(token)                                      \
    PROFILER_UNIQUE_VAR_NAME(prof_stringHandle_, token)

#define PROFILER_SCOPED_INTERNAL(token, name)                                  \
    static BW::ExternalProfiler::ProfilerStringHandle                          \
      PROFILER_STRINGHANDLE_NAME(token) =                                      \
...
#define PROFILER_SCOPED_DYNAMIC_STRING_INTERNAL(token, name)                   \
    BW::ScopedProfilerFrame PROFILER_SCOPE_NAME(token)(                        \
      BW::ExternalProfiler::enterFrame(                                        \
        0, BW::ExternalProfiler::createStringHandle(name)))

#define PROFILER_SCOPED(name)                                                  \
    PROFILER_SCOPED_INTERNAL(PROFILER_TOKEN_GEN, #name)

#define PROFILER_COUNTER(name, value) // Do nothing
