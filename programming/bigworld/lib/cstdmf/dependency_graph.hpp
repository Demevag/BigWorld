 ...
 */
template <typename NodeKey>
void DependencyGraph<NodeKey>::addNode(const NodeKey& key)
{
    BW_GUARD;
 ...
 */
template <typename NodeKey>
void DependencyGraph<NodeKey>::addDependency(const NodeKey& depending,
                                             const NodeKey& dependsOn)
{
    BW_GUARD;
  ...
  BW::vector<NodeKey>& dependencyOrder) const
{
    ...
    while (!noDependencyKeys.empty()) {
        ...
        for (typename BW::unordered_set<NodeKey>::const_iterator depItr =
             ...
             ++depItr) {
            typename GraphNodeMap::iterator depNodeItr =
              sortNodes.find(*depItr);
        ...
        for (typename BW::vector<
               typename GraphNodeMap::iterator>::const_iterator itr =
               toRemove.begin();
             itr != toRemove.end();
             ++itr) {
