...
class MsgBox
{
  public:
    ...
    CSTDMF_DLL MsgBox(const BW::wstring& caption,
    ...
    CSTDMF_DLL unsigned int doModal(HWND         parent       = NULL,
                                    bool         topmost      = false,
                                    unsigned int timeOutTicks = INFINITE,
                                    TickFunction tickFunction = NULL);
    CSTDMF_DLL void         doModalless(HWND         parent       = NULL,
                                        unsigned int timeOutTicks = INFINITE,
                                        TickFunction tickFunction = NULL);
    CSTDMF_DLL unsigned int getResult() const;
    CSTDMF_DLL bool         stillActive() const;

    CSTDMF_DLL static HWND getDefaultParent() { return defaultParent_; }
  ...
  private:
    ...
    static INT_PTR CALLBACK staticDialogProc(HWND   hwnd,
                                             UINT   msg,
                                             WPARAM w,
                                             LPARAM l);
