 ...
 */
bool ResourceCounters::DescriptionPool::operator<(
  const DescriptionPool& Other) const
{
    return (description_ < Other.description_) ||
           (!(Other.description_ < description_) &&
            memoryPool_ < Other.memoryPool_) ||
           (!(Other.description_ < description_) &&
            !(Other.memoryPool_ < memoryPool_) &&
...
ResourceCounters::MemoryEntry& ResourceCounters::MemoryEntry::operator=(
  const ResourceCounters::MemoryEntry& other)
{
    main_ = other.main_;
    gpu_  = other.gpu_;
    return *this;
...
ResourceCounters::MemoryEntry ResourceCounters::MemoryEntry::operator+(
  const ResourceCounters::MemoryEntry& other) const
{
 ...
 */
ResourceCounters::MemoryEntry ResourceCounters::add(
  const DescriptionPool& descriptionPool,
  size_t                 amount,
  size_t                 gpuAmount)
{
    ...
    if (amount != 0) {
        ...
        ++entry.instances_;
        entry.instances_ > entry.peakInstances_
          ? entry.peakInstances_ = entry.instances_
          : 0;
 ...
 */
ResourceCounters::MemoryEntry ResourceCounters::subtract(
  const DescriptionPool& descriptionPool,
  size_t                 amount,
  size_t                 gpuAmount)
{
 ...
 *  @returns				The current allocation for the given resource.
 */
ResourceCounters::MemoryEntry ResourceCounters::operator[](
  const DescriptionPool& descriptionPool) const
{
    ResourceCounters* myself = const_cast<ResourceCounters*>(this);
    if (myself->resourceCounts_.find(descriptionPool) ==
        myself->resourceCounts_.end()) {
 ...
 *
 *	@param granularityMode	The granularity mode to be used to categorise the
 *resource usage.
 *  @returns				String describing the current memory usage.
 */
BW::string ResourceCounters::getUsageDescription(
  GranularityMode granularityMode)
{
    ...
    addTotalUsage(ss,
                  descriptionsMap,
                  MPM_SYSTEM_MASK | MPM_DEFAULT_MASK | MPM_MANAGED_MASK |
                    MPM_MISC_MASK);
    ...
    switch (granularityMode) {
        case GM_MEMORY: {
            addMemorySubtree(ss,
                             descriptionsMap,
                             MPM_SYSTEM_MASK | MPM_DEFAULT_MASK |
                               MPM_MANAGED_MASK | MPM_MISC_MASK);
        } break;
        case GM_SYSTEM_VIDEO_MISC: {
            ...
            ss << std::endl;
            addTotalUsage(
              ss, descriptionsMap, MPM_DEFAULT_MASK | MPM_MANAGED_MASK);
            addMemorySubtree(
              ss, descriptionsMap, MPM_DEFAULT_MASK | MPM_MANAGED_MASK);
            ss << std::endl;
                                     ...
                                     uint                      pool)
{
    ...
    while (it != resourceCounts_.end()) {
        ...
        if (((pool & MPM_SYSTEM_MASK) && descPool.memoryPool_ == MP_SYSTEM) ||
            ...
            ((pool & MPM_MISC_MASK) && (descPool.memoryPool_ != MP_SYSTEM) &&
    ...
    BW::string desc;
    if ((pool & MPM_DEFAULT_MASK) && (pool & MPM_MANAGED_MASK) &&
        (pool & MPM_SYSTEM_MASK) && (pool & MPM_MISC_MASK))
        desc = "Total usage";
    else if ((pool & MPM_DEFAULT_MASK) && (pool & MPM_MANAGED_MASK))
        desc = "   Video";
    ...
    ss << formatNumber(size.main_, 14) << ' '
       << formatNumber(peakSize.main_, 14) << ' ' << formatNumber(instances, 7)
       << ' ' << formatNumber(peakInstances, 7) << ' '
       << formatNumber(numberAdds, 8) << ' ' << formatNumber(numberSubs, 8)
...
/**
 *	Adds the usage subtree for the passed memory pool to the passed
 *stringstream.
 *
 *  @param ss			The stream to write to.
 *  @param descriptions	The map of descriptions.
                                        ...
                                        uint                      pool)
{
    ...
    while (it != descriptions.end()) {
            ...
            while (true) {
                ...
                if (delimiter != BW::string::npos &&
                    ...
                    if (desc.substr(0, delimiter) ==
                        ...
                        previousDesc = previousDesc.substr(delimiter + 1,
                                                           previousDesc.size());
                        depth++;
                    } else {
                        break;
        ...
        while (res_it != resourceCounts_.end()) {
            ...
            if ((res_it->first.description_.find(desc) == 0 &&
                 delimiter != BW::string::npos) ||
                (res_it->first.description_.find(desc) == 0 &&
                 delimiter == BW::string::npos &&
                 res_it->first.description_.size() == it->second.size())) {
                if (res_it->first.description_ != desc)
                    hasChildren = true;

                if (((pool & MPM_SYSTEM_MASK) &&
                     descPool.memoryPool_ == MP_SYSTEM) ||
                    ((pool & MPM_DEFAULT_MASK) &&
                     descPool.memoryPool_ == MP_DEFAULT) ||
                    ((pool & MPM_MANAGED_MASK) &&
                     descPool.memoryPool_ == MP_MANAGED) ||
                    ((pool & MPM_SCRATCH_MASK) &&
                     descPool.memoryPool_ == MP_SCRATCH) ||
                    ((pool & MPM_MISC_MASK) &&
                     (descPool.memoryPool_ != MP_SYSTEM) &&
                     (descPool.memoryPool_ != MP_DEFAULT) &&
        ...
        // if (size)  // Commented out to prevent the list jumping
        {
            ...
            // Output statistics
            BW::string name =
              setIndentation(pool, depth) + parseDescription(desc, depth);
 ...
 */
void ResourceCounters::getResourceTypeMemoryUsage(ResourceType resourceType,
                                                  Entry*       entry)
{
    assert(entry != NULL);
 ...
 *	@param numberAdds Number of instances added to this resource total. (confirm
 *this)
 *	@param numberSubs Number of instances removed from this resource total.
 *(confirm this)
                                ...
                                uint64            average)
{
    ...
    ss << formatNumber(size, 14) << ' ' << formatNumber(peakSize, 14) << ' '
       << formatNumber(instances, 7) << ' ' << formatNumber(peakInstances, 7)
       << ' ' << formatNumber(numberAdds, 8) << ' '
       << formatNumber(numberSubs, 8) << ' ' << formatNumber(average, 12)
       << std::endl;
...
bool ResourceCounters::toCSV(const BW::string& filename)
{
    ...
    for (ResourceCountMap::iterator it = resourceCounts_.begin();
 ...
 */
ResourceCounters::Handle ResourceCounters::newHandle(
