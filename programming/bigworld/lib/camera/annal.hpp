...
{
  public:
    AnnalBase() { AnnalVault::instance().add(this); }

    ~AnnalBase() { AnnalVault::instance().del(this); }
...
class AnnalSynchronised : public AnnalBase
{
  public:
    ...
    void push(const T& event) { history_.push_back(event); }
    ...
    void pop(T& event)
    {
        ...
        Tween::tween(history_[fat - 1], history_[fat], s_tween_, event);
    }
 ...
 */
template <class T>
class AnnalIrregular : public AnnalBase
{
  public:
    ...
    bool pop(T& event)
    {
        if (nextPop_ >= int(history_.size()) ||
            history_[nextPop_].first >= s_atFrame_)
            return false;
    ...
    int left() const
    {
        int i;
        for (i = nextPop_;
             nextPop_ < history_.size() && history_[i].first < s_atFrame_;
             i++)
            ; // scan
        return i - nextPop_;
    }
 ...
 */
template <class T>
class AnnalAbundant : public AnnalIrregular<T>
{
