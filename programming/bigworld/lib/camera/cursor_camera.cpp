...
void CursorCamera::update(float deltaTime)
{
    ...
    if (pTarget_ && pSource_) {
        // pWorldDirectionCursor_ the transformation matrix from local space to
        // world space.
        ...
        */
        ...
        if (pivotPosition_ != targetPivotPosition_) {
            pivotPosition_ =
              Vector3(Math::decay(pivotPosition_.x,
                                  targetPivotPosition_.x,
                                  this->movementHalfLife() * 2.0f,
                                  deltaTime),
                      Math::decay(pivotPosition_.y,
                                  targetPivotPosition_.y,
                                  this->movementHalfLife() * 2.0f,
                                  deltaTime),
                      Math::decay(pivotPosition_.z,
                                  targetPivotPosition_.z,
                                  this->movementHalfLife() * 2.0f,
                                  deltaTime));
        }

        // Move camera distance towards the target camera distance.
        if (!MF_FLOAT_EQUAL(maxDistanceFromPivot_,
                            targetMaxDistanceFromPivot_)) {
            maxDistanceFromPivot_ = Math::decay(maxDistanceFromPivot_,
                                                targetMaxDistanceFromPivot_,
                                                maxDistHalfLife_,
                                                deltaTime);
        }
        ...
        */
        ...
        this->yaw(Angle::decay(
          this->yaw(), targetYaw, this->turningHalfLife(), deltaTime));

        this->pitch(Angle::decay(
          this->pitch(), targetPitch, this->turningHalfLife(), deltaTime));

        this->roll(Angle::decay(
          this->roll(), targetRoll, this->turningHalfLife(), deltaTime));
        ...
        if (!this->firstPersonPerspective()) {
            ...
            collidedWithScene = BaseCamera::sceneCheck(idealPosInWS,
                                                       positionOnEntity,
                                                       cameraDirInWS_,
                                                       uprightDirection_);
        }
        ...
        if ((!collidedWithScene && !inPosition_) || oldDist < newDist ||
            limitVelocity_) {
            ...
            if (!limitVelocity_) {
                ...
                if (range != 0.0f) {
                    float ratio =
                      (oldDist - this->minDistanceFromPivot()) / range;
                    float div = ((1.0f - ratio) + 1.0f) * 2.0f;
                    dist      = Math::decay(oldDist,
                                       newDist,
                                       this->movementHalfLife() / div,
                                       deltaTime);
                } else {
                    dist = Math::decay(
                      oldDist, newDist, this->movementHalfLife(), deltaTime);
                }
        ...
        // check if on top of the water surface before any snapping of the
        // camera can be done
        ...
        if (overWater) {
            ...
            // snap the camera's position up or down depending on which is
            // closer
 ...
 */
bool CursorCamera::intersects(WorldTriangle* triangles,
                              int            numTriangles,
