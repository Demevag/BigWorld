 ...
 *	pivotMaxDistance behind the target offset by pivotPosition, which is a
 *Vector3 which gets interpreted in the source-space.  After the desired
 *position has been offset by pivotPosition, it is then clipped to the geometry,
 *and moved forwards along the source z-axis if necessary to give it a clear
 *line of site to the target position.
 *
...
class CursorCamera : public BaseCamera
{
    ...
    //	PY_READABLE_ATTRIBUTE_GET( this->targetMaxDistanceFromPivot(),
    //targetMaxDist )
    ...
    PY_RW_ACCESSOR_ATTRIBUTE_DECLARE(Vector3,
                                     uprightDirection,
                                     uprightDirection)
  ...
  protected:
    ...
    //@{
    bool intersects(WorldTriangle* triangles,
                    int            numTriangles,
