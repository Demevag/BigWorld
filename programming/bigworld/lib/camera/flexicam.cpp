...
void FlexiCam::update(float deltaTime)
{
    ...
    while (remainingTime > 0.f) {
        ...
        Vector3 currDesiredPos =
          finalDesiredPos - remainingTime * deltaPosPerSecond;
        Vector3 currDesiredDir =
          finalDesiredDir - remainingTime * deltaDirPerSecond;

        // Move the camera towards the preferred position.

        actualPos_ =
          currDesiredPos + powf(0.5f, stepSize / positionAcceleration_) *
                             (actualPos_ - currDesiredPos);

        // Turn the camera towards the preferred direction.
        actualDir_ =
          currDesiredDir + powf(0.5f, stepSize / trackingAcceleration_) *
    ...
    if (pCamera) {
        ...
        BaseCamera::sceneCheck(finalPos, actualDir_, direction, uprightDir_);
    }
