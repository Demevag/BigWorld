  ...
  : public HasScriptOrTagDistributionDecider
{
  public:
    ...
    bool canBeOnClient(const BW::string&         name,
  ...
  : public HasScriptOrTagDistributionDecider
{
  public:
    ...
    bool canBeOnCell(const BW::string&         name,
                     const EntityDistribution& distr) const
    {
        return false;
    }

    bool canBeOnBase(const BW::string&         name,
                     const EntityDistribution& distr) const
    {
        ...
        return this->fileExists(EntityDef::Constants::entitiesServicePath(),
                                name);
#else
        return true;
#endif
 ...
 */
bool EntityDescriptionMap::parse(
  ...
  const ExposedMethodMessageRange*   pCellMessageRange)
{
    ...
    int capacity = (pServicesSection ? pServicesSection->countChildren() : 0);
    ...
    if (pClientServerEntities) {
        ...
        if (pServerOnlyEntities) {
            isOkay &= this->parseInternal(
              pServerOnlyEntities, EntityDescriptionHasClientScript(false));
        } else {
        ...
        NOTICE_MSG(
          "EntityDescriptionMap::parse: scripts/entities.xml does not "
    ...
    this->setExposedMessageIDs(
      pClientMessageRange, pBaseMessageRange, pCellMessageRange);
    ...
    if (isOkay) {
        ...
        if (pClientPropertyMessageRange) {
            isOkay &=
              this->checkCount("client top-level property",
                               &EntityDescription::clientServerPropertyCount,
                               pClientPropertyMessageRange->numSlots(),
                               256);
        }
  ...
  const ExposedMethodMessageRange* pCellMessageRange)
{
    ...
    while (iter != vector_.end()) {
        maxClientServerPropertyCount_ = std::max(
          maxClientServerPropertyCount_, iter->clientServerPropertyCount());

        maxExposedClientMethodCount_ =
          std::max(maxExposedClientMethodCount_, iter->client().exposedSize());

        maxExposedBaseMethodCount_ =
          std::max(maxExposedBaseMethodCount_, iter->base().exposedSize());

        maxExposedCellMethodCount_ =
          std::max(maxExposedCellMethodCount_, iter->cell().exposedSize());
    ...
    while (iter != vector_.end()) {
        iter->setExposedMsgIDs(maxExposedClientMethodCount_,
                               pClientMessageRange,
                               maxExposedBaseMethodCount_,
                               pBaseMessageRange,
                               maxExposedCellMethodCount_,
 ...
 */
bool EntityDescriptionMap::parseInternal(
  DataSectionPtr                    pSection,
  ...
  DescriptionVector*                pServerOnlyEntities)
{
    ...
    while (iter != pSection->end()) {
        ...
        if (desc.parse(
              typeName, /*componentName*/ BW::string(), &hasScriptDecider)) {
                                         ...
                                         EntityTypeID   initialTypeID)
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        if (!desc.parseService(
              serviceName, /*componentName*/ BW::string(), &hasScriptDecider)) {
        ...
        BW::string activeOnServerModes =
          pSubSection->readString("activeOnServerModes");
...
bool EntityDescriptionMap::checkCount(const char* description,
                                      unsigned int (EntityDescription::*fn)()
                                        const,
                                      int maxEfficient,
