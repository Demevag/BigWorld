...
ScriptObject PyEntityMailBox::pyGetAttribute(const ScriptString& attrObj)
{
    ...
    if (pDescription) {
        return ScriptObject(new RemoteEntityMethod(this, pDescription),
...
typedef BW::map<EntityMailBoxRef::Component, PyEntityMailBox::FactoryFn>
  Fabricators;
typedef BW::vector<
  std::pair<PyEntityMailBox::CheckFn, PyEntityMailBox::ExtractFn>>
                                  Interpreters;
 ...
 */
PyObject* PyEntityMailBox::callMethod(const ScriptString& methodName,
                                      const ScriptTuple&  arguments)
{
    const MethodDescription* pDescription =
      this->findMethod(methodName.c_str());
    if (!pDescription) {
        PyErr_Format(
          PyExc_TypeError, "Unable to find method %s", methodName.c_str());
        return NULL;
    }
  ...
  const ScriptTuple&       pArgs)
{
    ...
    BinaryOStream* pBOS = this->getStream(
      *pMethodDescription,
    ...
    if (pMethodDescription->component() == MethodDescription::CLIENT) {
        ...
        if (!pMethodDescription->addToServerStream(
              source, *pBOS, sourceEntityID)) {
            return NULL;
        }
    }
...
PyObject* PyEntityMailBox::constructFromRef(const EntityMailBoxRef& ref)
{
    ...
    if (pResult) {
        ...
        WARNING_MSG(
          "PyEntityMailBox::constructFromRef: "
...
void PyEntityMailBox::registerMailBoxComponentFactory(
  EntityMailBoxRef::Component c,
  FactoryFn                   fn,
  PyTypeObject*               pType)
{
    if (s_pRefReg == NULL)
        s_pRefReg = new MailBoxRefRegistry();
...
static PyObject* PyEntityMailBox_pyPickleResolve(const BW::string& str)
{
    if (str.size() != sizeof(EntityMailBoxRef)) {
        PyErr_SetString(PyExc_ValueError,
                        "PyEntityMailBox_pyPickleResolve: "
...
PY_AUTO_UNPICKLING_FUNCTION(RETOWN,
                            PyEntityMailBox_pyPickleResolve,
                            ARG(BW::string, END),
                            MailBox)
...
PyObject* PyEntityMailBox::pyRepr()
{
    ...
    const char* location =
      ...
      : (embr.component() == EntityMailBoxRef::CLIENT_VIA_CELL)
        ? "ClientViaCell"
      : (embr.component() == EntityMailBoxRef::CELL_VIA_BASE) ? "CellViaBase"
      : (embr.component() == EntityMailBoxRef::CLIENT_VIA_BASE)
        ? "ClientViaBase"
...
PyObject* PyEntityMailBox::pyGet_address()
{
    ...
    return Py_BuildValue("(sH)", address.ipAsString(), ntohs(address.port));
}
  ...
  RecordingOption& recordingOption)
{
    ...
    KeywordParser::ParseResult parseResult =
    ...
    recordingOption = (shouldRecordOnly ? RECORDING_OPTION_RECORD_ONLY
 ...
 *	setData function
 */
int Script::setData(PyObject* pObj, EntityMailBoxRef& mbr, const char* varName)
{
    if (!PyEntityMailBox::reduceToRef(pObj, &mbr)) {
        PyErr_Format(PyExc_TypeError,
                     "%s must be set to "
                     "a type reducible to an EntityMailBox",
                     varName);
        return -1;
    }
