 ...
 * @param IEntityDelegate * pEntityDelegate - is optional, could contain empty
 * object to skip Delegate properties
  ...
  int                      dataDomains)
{
    ...
    {
      public:
        AddToDictFromPropertiesVisitor(const char*         entityName,
                                       const ScriptObject& entity,
                                       IEntityDelegate*    pEntityDelegate,
        ...
        ScriptDict dict() const { return dict_; }
        ...
        virtual bool visit(const DataDescription& propDesc)
        {
            ...
            if (!propDesc.isComponentised()) {
                ...
                resultObj = PyComponent::getAttribute(
                  *pEntityDelegate_, propDesc, isPersistentOnly_);
    ...
    AddToDictFromPropertiesVisitor visitor(
...
bool populateDelegateWithDict(const EntityDescription& entityDescription,
                              IEntityDelegate*         pEntityDelegate,
                              ...
                              int                      dataDomains)
{
    class AddToDelegateFromDictVisitor : public IDataDescriptionVisitor
    {
      public:
        ...
        virtual bool visit(const DataDescription& propDesc)
        {
            ...
            if (pObject) {
                if (!PyComponent::setAttribute(
                      *pDelegate_, propDesc, pObject, isPersistentOnly_)) {
                    Script::printError();
                    return false;
                }
            }
    ...
    AddToDelegateFromDictVisitor visitor(
      pEntityDelegate,
      dict,
      (dataDomains & EntityDescription::ONLY_PERSISTENT_DATA) != 0);
