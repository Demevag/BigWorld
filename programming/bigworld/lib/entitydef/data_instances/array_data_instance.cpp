...
void PyArrayDataInstance::setInitialSequence(ScriptObject pSeq)
{
    ...
    IF_NOT_MF_ASSERT_DEV(pDataType_->getSize() == 0 ||
                         pDataType_->getSize() == sz)
    {
        return;
    }
 ...
 */
ScriptObject PyArrayDataInstance::setOwnedProperty(int ref, BinaryIStream& data)
{
 ...
 */
ScriptObject PyArrayDataInstance::setOwnedSlice(int            startIndex,
                                                int            endIndex,
                                                BinaryIStream& data)
{
    ...
    if (startIndex < endIndex) // this behaviour is the same as PyList's
    {
        for (Py_ssize_t i = startIndex; i < endIndex; ++i) {
            ...
            if (ppOldValues) {
                Py_INCREF(pOldValue);
                PyTuple_SET_ITEM(
                  (*ppOldValues).get(), i - startIndex, pOldValue);
            }

            elementType.detach(
              ScriptObject(pOldValue, ScriptObject::FROM_BORROWED_REFERENCE));
        }
        values_.erase(values_.begin() + startIndex, values_.begin() + endIndex);
    }
 ...
 */
bool PyArrayDataInstance::getValuesFromSequence(
  PyObject*                 pSeq,
  BW::vector<ScriptObject>& values)
{
    ...
    for (size_t i = 0; i < seqSize; ++i) {
        ...
        if (!dataType.isSameType(
              ScriptObject(pVal, ScriptObject::FROM_BORROWED_REFERENCE))) {
            PyErr_Format(PyExc_TypeError,
                         "Array elements must be set to type %s",
                         dataType.typeName().c_str());
            return false;
 ...
 */
bool PyArrayDataInstance::setSlice(Py_ssize_t                      startIndex,
                                   Py_ssize_t                      endIndex,
                                   ...
                                   ScriptObject*                   ppOldValues)
{
    ...
    if ((pDataType_->getSize() != 0) &&
        (endIndex - startIndex != numToInsert)) {
    ...
    SlicePropertyChange change(
      startIndex, endIndex, values_.size(), newValues, dataType);
...
{
    int count = 0, cur;
    for (uint i = 0; (cur = this->findFrom(i, pObject.get())) >= 0;
         i      = cur + 1) {
        ++count;
    }
    return count;
...
int PyArrayDataInstance::pyCompare(PyObject* a, PyObject* b)
{
    ...
    MF_ASSERT(PyArrayDataInstance::Check(a) && PyArrayDataInstance::Check(b));
    ...
    return (iterA != aValues.end()) ? 1 : (iterB != bValues.end()) ? -1 : 0;
}
