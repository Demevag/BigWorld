 ...
 */
PyFixedDictDataInstance::PyFixedDictDataInstance(FixedDictDataType* pDataType)
...
void PyFixedDictDataInstance::initFieldValue(int index, ScriptObject val)
{
    ...
    if (!fieldValues_[index]) {
        PyObjectPtr pStr(PyObject_Str(val.get()), PyObjectPtr::STEAL_REFERENCE);
                                         ...
                                         const FixedDictDataType& dataType)
{
    return PyFixedDictDataInstance::Check(pValue.get()) &&
           (&static_cast<PyFixedDictDataInstance*>(pValue.get())->dataType() ==
            &dataType);
}
 ...
 */
PropertyOwnerBase* PyFixedDictDataInstance::getChildPropertyOwner(int ref) const
{
  ...
  const ScriptString& attrObj)
{
    ...
    if (!pResult) {
        PyErr_Clear();
        pResult = ScriptObject(this->getFieldByKey(attr),
 ...
 */
int PyFixedDictDataInstance::pySetFieldByKey(PyObject* self,
                                             PyObject* key,
                                             PyObject* value)
{
    return static_cast<PyFixedDictDataInstance*>(self)->setFieldByKey(key,
                                                                      value);
}
                                           ...
                                           PyObject*   value)
{
    if (value == NULL) {
        PyErr_Format(
          PyExc_TypeError, "Cannot delete entry %s from FIXED_DICT", keyString);
        return -1;
    }
...
PyObject* PyFixedDictDataInstance::py_values(PyObject* /*args*/)
{
    ...
    if (pyValueList) {
        for (FieldValues::iterator i = fieldValues_.begin();
            ...
            PyList_SET_ITEM(
              pyValueList, i - fieldValues_.begin(), pyFieldValue);
        }
    }
    return pyValueList;
}
...
PyObject* PyFixedDictDataInstance::py_items(PyObject* /*args*/)
{
    ...
    if (pyItemList) {
        for (FixedDictDataType::Fields::size_type i = 0; i < fields.size();
             ++i) {
            PyObject* pyTuple = Py_BuildValue(
              "(sO)", fields[i].name_.c_str(), fieldValues_[i].get());
...
PyObject* PyFixedDictDataInstance::pyPickleReduce()
{
    ...
    for (FixedDictDataType::Fields::size_type i = 0; i < fields.size(); ++i) {
        PyDict_SetItemString(
          pDict, fields[i].name_.c_str(), fieldValues_[i].get());
    }
 ...
 */
/* static */
int PyFixedDictDataInstance::pyCompareKeys(PyFixedDictDataInstance* pFixedDictA,
...
int PyFixedDictDataInstance::pyCompare(PyObject* a, PyObject* b)
{
    ...
    int compareResult =
      PyFixedDictDataInstance::pyCompareKeys(pFixedDictA, pFixedDictB);
