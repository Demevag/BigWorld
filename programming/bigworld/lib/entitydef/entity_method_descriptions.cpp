                                    ...
                                    unsigned int* pNumLatestEventMembers)
{
    ...
    while (iter != pMethods->end()) {
        ...
        if (!methodDescription.parse(
              interfaceName, *iter, component, pNumLatestEventMembers)) {
  ...
  const char* interfaceName)
{
    ...
    for (uint eindex = 0; eindex < exposedMethods_.size(); eindex++) {
        ...
        const char* const szMethodType =
          mdesc.component() == MethodDescription::CLIENT ? "a client"
                                                         : "an Exposed";
 ...
 */
void EntityMethodDescriptions::setExposedMsgIDs(
  int                              maxExposedMethodCount,
  const ExposedMethodMessageRange* pRange)
{
    maxExposedMethodCount_ = maxExposedMethodCount;

    std::stable_sort(exposedMethods_.begin(),
                     exposedMethods_.end(),
 ...
 *	name and component.
 */
MethodDescription* EntityMethodDescriptions::find(const char* name) const
{
    MF_ASSERT_DEV(name);
