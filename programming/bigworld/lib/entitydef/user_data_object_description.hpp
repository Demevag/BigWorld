 ...
 *properties of the data object. It is normally created on startup when the
 *user_data_objects.xml file is parsed.
 */
class UserDataObjectDescription : public BaseUserDataObjectDescription
{
  public:
    ...
    /* A user data object only lives on one of the base, cell or client.
       Accordingly any DATA_OBJECT links on entities should have the appropriate
       flag set so that it can be read in the appropriate domain. */
