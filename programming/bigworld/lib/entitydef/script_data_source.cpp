...
namespace BW {
    ...
    namespace // Anonymous namespace for static functions
    {
        ...
        /**
         *	This function populates the given reference from the given object
         *reference.
         *
         ...
         *
         *	@return			true unless the object doesn't, or the value could
         *not be coerced from the ScriptObject.
         */
                                         ...
                                         inputT&             value)
        {
            if (!object.exists()) {
                ERROR_MSG(
                  "getFromObject<>: Tried to get value from unset object\n");
                return false;
            }
        ...
        inline static bool getFromObject(const ScriptObject& object,
                                         Vector2&            value)
        {
            // Needed because VectorDataType accepts any ScriptSequence of the
            // right length, but ScriptObject::convertTo (via Script::setData)
            // only accepts ScriptTuples of the right length.
...
#if defined(SCRIPT_PYTHON)
        /**
         *	This function converts the given object into the given stream using
         *the custom class of the given FixedDictDataType.
         *	@param	object				The object to read from
         *	@param	type				The FixedDictDataType with the custom
         *class
         *	@param	stream				The stream to write the output to
         *	@param	isPersistentOnly	Whether the data stream is to contain
         *persistent data only.
         */
        inline static bool getStreamForObject(const ScriptObject       object,
                                              const FixedDictDataType& type,
                                              ...
                                              bool isPersistentOnly)
        {
            ...
            if (!isPersistentOnly && type.hasCustomAddToStream()) {
                ...
                // XXX: No way to tell if it succeeded.
                type.addInstanceToStream(
                  instance.get(), stream, isPersistentOnly);
            }
        ...
        /**
         *	This function converts the given object into the given stream using
         *the custom class of the given UserDataType.
         *	@param	object				The object to read from
     ...
     *	@param	isNone	This reference is set to true if the current node is
     *None, and set to false otherwise.
     *	@return			true unless we cannot tell if we have None.
     */
    bool ScriptDataSource::readNone(bool& isNone)
    {
        if (!this->currentObject().exists()) {
            ERROR_MSG("ScriptDataSource::readNone: Tried to get value from "
                      "unset object\n");
            return false;
        }
    ...
    bool ScriptDataSource::beginSequence(size_t& count)
    {
        if (readingType_ != None) {
            ERROR_MSG(
              "ScriptDataSource::beginSequence: Tried to read a sequence "
              "when already reading a collection.\n");
            return false;
        }
        ...
        if (!this->currentObject().exists()) {
            ERROR_MSG(
              "ScriptDataSource::beginSequence: Tried to read a non-existent "
              "object.\n");
            return false;
        }
    ...
    bool ScriptDataSource::enterItem(size_t item)
    {
        ...
        if (!sequence || readingType_ != Sequence) {
            ERROR_MSG(
              "ScriptDataSource::enterItem: Tried to enter an item of a "
              "non-sequence.\n");
            return false;
        }
    ...
    bool ScriptDataSource::leaveItem()
    {
        if (!this->hasParent()) {
            ERROR_MSG(
              "ScriptDataSource::leaveItem: Tried to leave an item of a "
              "non-sequence.\n");
            return false;
        }
    ...
    bool ScriptDataSource::beginClass()
    {
        ...
        if (!this->currentObject().exists()) {
            ERROR_MSG(
              "ScriptDataSource::beginClass: Tried to read a non-existent "
              "object.\n");
            return false;
        }
#if defined(SCRIPT_PYTHON)
        // If we're reading a class, and it's not a PyClassDataInstance, then if
        // it happens to be a mapping, we read the fields instead of the
        // attributes.
    ...
    bool ScriptDataSource::beginDictionary(const FixedDictDataType* pType)
    {
        if (readingType_ != None) {
            ERROR_MSG(
              "ScriptDataSource::beginDictionary: Tried to read a dictionary "
              "when already reading a collection.\n");
            return false;
        }
        ...
        if (!this->currentObject().exists()) {
            ERROR_MSG(
              "ScriptDataSource::beginDictionary: Tried to read a non-existent "
              "object.\n");
            return false;
        }
        ...
        if (pType != NULL) {
            ...
            FixedDictDataType& type = *const_cast<FixedDictDataType*>(pType);

            type.initCustomClassImplOnDemand();

            if (type.hasCustomClass()) {
                // Replace currentObject() (The custom class) with a
                // PyFixedDictDataInstance representation of its data to allow
    ...
    /**
     *	This method makes the current node an element of the current node, if it
     *is a map or object.
    ...
    bool ScriptDataSource::enterField(const BW::string& name)
    {
        if (readingType_ != Class && readingType_ != Dictionary) {
            ERROR_MSG(
              "ScriptDataSource::enterField: Tried to enter a field when "
        ...
        if (!leaf.exists()) {
            ERROR_MSG(
              "ScriptDataSource::enterField: Tried to enter a field of a "
              "non-existent object.\n");
            return false;
        }
    ...
    bool ScriptDataSource::leaveField()
    {
        if (!this->hasParent()) {
            ERROR_MSG(
              "ScriptDataSource::leaveField: Tried to leave an item of a "
              "non-sequence.\n");
            return false;
        }
                                          ...
                                          bool isPersistentOnly)
    {
        ...
        if (!this->currentObject().exists()) {
            ERROR_MSG(
              "ScriptDataSource::beginDictionaryStream: Tried to read a "
              "non-existent object.\n");
            return false;
        }
        ...
        if (type.hasCustomClass()) {
            return getStreamForObject(
              this->currentObject(), type, stream, isPersistentOnly);
        }
        ERROR_MSG(
          "ScriptDataSink::readCustomType: Tried to read a "
    ...
    /**
     *	This method returns a reference to the parent CollectionType of the
     *current node
     */
    ...
    /**
     *	This method indicates if a parent collection exists for the current
     *node.
     */
    ...
    ScriptDataSource::VectorOfNodesHolder::operator ScriptDataSource::
      VectorOfNodes&()
    {
        return *data_;
    }
