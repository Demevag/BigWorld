...
namespace BW {
    ...
    namespace // Anonymous namespace for static functions
    {
        ...
        /**
         *	This function populates the given object reference from the given
         *reference.
         *
         *	@param	object	A ScriptObject reference to populate. Must not
         *exist.
         *	@param	value	A value to write into the ScriptObject.
         *
         *	@return			true unless the object already existed, or the value
         *could not be coerced into a ScriptObject.
         */
        ...
        inline static bool setObject(ScriptObject& object, const inputT& value)
        {
            if (object.exists()) {
                ERROR_MSG(
                  "setObject<>: Tried to set value for already-set object\n");
                return false;
            }
                                     ...
                                     const UniqueID& value)
        {
            ...
            if (value == UniqueID::zero()) {
                ...
                // UserDataObjectLinkDataType::createFromStream and
                // UserDataObjectLinkDataType::createFromSection already handle
                // this.
...
#if defined(SCRIPT_PYTHON)
        /**
         *	This function populates the given object using the custom class of
         *the given FixedDictDataType with the given stream.
        ...
        /**
         *	This function populates the given object using the custom class of
         *the given UserDataType with the given stream.
         *	@param	object				The object to populate
        ...
        inline static bool setObjectFromStream(ScriptObject&       object,
                                               ...
                                               bool isPersistentOnly)
        {
            return type.createObjectFromStream(
              object, stream, isPersistentOnly);
        }

        /**
         *	This function populates the given object using the custom class of
         *the given FixedDictDataType with the given DataSection.
         *	@param	object				The object to populate
         *	@param	type				The FixedDictDataType with the custom
         *class
        ...
        /**
         *	This function populates the given object using the custom class of
         *the given UserDataType with the given DataSection.
         *	@param	object				The object to populate
         *	@param	type				The UserDataType with the custom class
         *	@param	section				The DataSection to read from
         */
        inline static bool setObjectFromSection(ScriptObject&       object,
                                                ...
                                                DataSection&        section)
        {
            return type.createObjectFromSection(object,
    ...
    bool ScriptDataSink::beginTuple(const TupleDataType* pType, size_t count)
    {
        ...
        if (pType != NULL) {
            for (size_t i = 0; i < count; i++) {
                ...
                MF_VERIFY(pType->getElemType().getDefaultValue(defaultSink));
    ...
    {
        if (this->parentType() != Array && this->parentType() != Tuple) {
                ...
                if (PyArrayDataInstance::Check(parent.get())) {
                    ...
                    if (parent.exists() && ScriptList::check(parent)) {
                        ...
                        if (!list.setItem(item, leaf)) {
                            ERROR_MSG(
                              "ScriptDataSink::leaveItem: Failed to store "
                              "new value.\n");
                            return false;
                        }
                    } else {
    ...
    bool ScriptDataSink::beginClass(const ClassDataType* pType)
    {
        ...
        if (pType == NULL) {
            ...
            // We'd need the equivalent of `type( "Typeless", (object,), {} )()`
            // to create the instance. NULL pType is not defined of the
            // Source/Sink API anyway, so this isn't a problem yet.
    ...
    /**
     *	This method creates a ScriptDict or PyFixedDictDataInstance at the
     *current empty node.
     *	@param	pType				The TupleDataType being written
     *	@return						true unless the tuple could not be written
     */
    bool ScriptDataSink::beginDictionary(const FixedDictDataType* pType)
    {
        ...
        if (pType != NULL) {
            ...
            if (pType->hasCustomClass()) {
                ...
                ERROR_MSG(
                  "Not yet supporting dict-based custom FixedDict writing\n");
                return false;
            } else {
    ...
    /**
     *	This method puts the current node into its place in the class or
     *dictionary we're currently writing.
     *	@return	true unless we aren't writing a class or dictionary, or the node
     *is empty.
     */
    bool ScriptDataSink::leaveField()
    {
        if (this->parentType() != Class && this->parentType() != Dictionary &&
            this->parentType() != Custom) {
        ...
        if (buildingType_ == Custom) {
            ...
            {
                ERROR_MSG( "ScriptDataSink::leaveField: Error finalising a
            custom " "type.\n" ); return false;
            }
            */
        }
        ...
        switch (this->parentType()) {
            case Class: {
                ...
                int index =
                  pInst->dataType()->fieldIndexFromName(field.c_str());
            ...
            case Dictionary: {
                ...
                if (PyFixedDictDataInstancePtr::check(parent)) {
                    ...
                    if (parent.exists() && ScriptDict::check(parent)) {
                        ...
                        if (!dict.setItem(
                              field.c_str(), leaf, ScriptErrorPrint())) {
                            ERROR_MSG(
                              "ScriptDataSink::leaveField: Failed to store "
                              "new value.\n");
                            return false;
                        }
                        break;
                                         ...
                                         DataSection&             section)
    {
        if (buildingType_ != None) {
            ERROR_MSG(
              "ScriptDataSink::writeCustomType: Tried to write a "
    ...
    /**
     *	This method returns a reference to the sequence index of the current
     *node in the parent sequence.
     */
    ...
    ScriptDataSink::VectorOfNodesHolder::operator ScriptDataSink::
      VectorOfNodes&()
    {
        return *data_;
    }
