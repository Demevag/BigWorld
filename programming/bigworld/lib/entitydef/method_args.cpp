 ...
 */
bool MethodArgs::addToStream(DataSource& source, BinaryOStream& stream) const
{
    ...
    for (size_t i = 0; i < numArgs; i++) {
        if (i >= numArgsInSource) {
            ...
            args_[i].second->addToStream(source,
                                         stream,
        ...
        if (!source.enterItem(i) && isOkay) {
            ERROR_MSG(
              "MethodArgs::addToStream: enterItem failed for arg %" PRIzu ".\n",
              i);
            isOkay = false;
        }

        if (!args_[i].second->addToStream(source,
                                          stream,
                                          /* isPersistentOnly */ false) &&
            isOkay) {
            ERROR_MSG(
              "MethodArgs::addToStream: addToStream failed for arg %" PRIzu
              ".\n",
              i);
            isOkay = false;
        }
        ...
        if (!source.leaveItem() && isOkay) {
            ERROR_MSG(
              "MethodArgs::addToStream: leaveItem failed for arg %" PRIzu ".\n",
              i);
            isOkay = false;
        }
    }
                                  ...
                                  EntityID*         pImplicitSource) const
{
    ...
    if (hasImplicitSourceID) {
        if (!sink.enterItem(argAt++) || !sink.write(*pImplicitSource) ||
            !sink.leaveItem()) {
    ...
    for (int i = 0; i < numArgs; i++) {
        ...
        if (!pType->createFromStream(data,
                                     sink,
                                     /* isPersistentOnly */ false) ||
            data.error()) {
            ERROR_MSG("MethodArgs::createFromStream: "
                                            ...
                                            ScriptDict  kwargs) const
{
    ...
    while (i < size) {
        ScriptObject pArg =
          kwargs.getItem(args_[i].first.c_str(), ScriptErrorClear());
...
{
    if (!ScriptTuple::check(args) || (args.size() != int(args_.size()))) {
#if defined(SCRIPT_PYTHON)
        PyErr_Format(PyExc_TypeError,
    ...
    for (int i = 0; i < numArgs; ++i) {
        pDict.setItem(args_[i].first.c_str(),
                      args.getItem(i),
