...
class MethodArgs
{
  public:
    ...
    bool checkValid(ScriptTuple args,
                    const char* name,
                    int         firstOrdinaryArg = 0) const;

    BWENTITY_API
    bool addToStream(DataSource& source, BinaryOStream& stream) const;
    BWENTITY_API bool createFromStream(BinaryIStream&    data,
                                       DataSink&         sink,
                                       const BW::string& name,
                                       EntityID* pImplicitSource = NULL) const;

    void addToMD5(MD5& md5) const;

    ScriptObject convertKeywordArgs(ScriptTuple args, ScriptDict kwargs) const;
    ...
    friend BWENTITY_API bool operator==(const MethodArgs& left,
                                        const MethodArgs& right);
