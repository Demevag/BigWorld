...
namespace BW {
    ...
    namespace {
         ...
         */
        static DataSectionPtr getDataSectionForXML(
          const BW::string& dataSectionStr)
        {
            BW::stringstream dataSectionStream;
            dataSectionStream << dataSectionStr;
            XMLSectionPtr xml =
              XMLSection::createFromStream("", dataSectionStream);
            return xml;
        }

        /**
         *	Utility method to hide the difference between ScriptList and
         *PyArrayDataInstance
         */
                                          ...
                                          const BW::string& field)
        {
            ...
            int index =
              pPyInstance->dataType()->fieldIndexFromName(field.c_str());
            if (index == -1) {
                return ScriptObject();
            }
    ...
    TEST_F(EntityDefUnitTestHarness, Script_ScriptDataSink_UDO_REF)
    {
        ...
        UserDataObject* pResultUDO =
          static_cast<UserDataObject*>(resultObject.get());
    ...
    TEST_F(EntityDefUnitTestHarness,
           Script_ScriptDataSink_Array_of_Tuple_of_int8)
    {
        DataTypePtr pType = DataType::buildDataType(
    ...
    TEST_F(EntityDefUnitTestHarness,
           Script_ScriptDataSink_Tuple_of_Array_of_int8)
    {
        DataTypePtr pType = DataType::buildDataType(
    ...
    TEST_F(EntityDefUnitTestHarness, Script_ScriptDataSink_Class)
    {
        ...
        ScriptObject secondObject =
          getClassField(resultObject, "str_TestString");
        CHECK(secondObject.exists());
        RETURN_ON_FAIL_CHECK(ScriptString::check(secondObject));
        ScriptString secondString(secondObject);
        BW::string   secondStr;
    ...
    TEST_F(EntityDefUnitTestHarness, Script_ScriptDataSink_ScriptDict)
    {
        ...
        ScriptObject firstObject =
          resultDict.getItem("int8_5", ScriptErrorPrint());
        CHECK(firstObject.exists());
        RETURN_ON_FAIL_CHECK(ScriptInt::check(firstObject));
        ScriptInt firstInt(firstObject);
        CHECK(firstInt.exists());
        ...
        ScriptObject secondObject =
          resultDict.getItem("str_TestString", ScriptErrorPrint());
        CHECK(secondObject.exists());
        RETURN_ON_FAIL_CHECK(ScriptString::check(secondObject));
        ScriptString secondString(secondObject);
        BW::string   secondStr;
    ...
    TEST_F(EntityDefUnitTestHarness, Script_ScriptDataSink_FixedDict)
    {
        ...
        ScriptObject secondObject =
          getDictionaryField(resultObject, "str_TestString");
        CHECK(secondObject.exists());
        RETURN_ON_FAIL_CHECK(ScriptString::check(secondObject));
        ScriptString secondString(secondObject);
        BW::string   secondStr;
    ...
    TEST_F(EntityDefUnitTestHarness,
           Script_ScriptDataSink_PyFixedDict_as_stream_default)
    {
        DataTypePtr pType = DataType::buildDataType(
          "<type> FIXED_DICT "
          "<implementedBy> FixedDictWrappers.noStream </implementedBy> "
        ...
        ScriptDataSink sink;
        CHECK(sink.writeCustomType(*pFixedDictType,
                                   stream,
    ...
    TEST_F(EntityDefUnitTestHarness,
           Script_ScriptDataSink_PyFixedDict_as_stream_custom)
    {
        DataTypePtr pType = DataType::buildDataType(
          "<type> FIXED_DICT "
    ...
    TEST_F(EntityDefUnitTestHarness,
           Script_ScriptDataSink_PyFixedDict_as_section)
    {
        DataTypePtr pType = DataType::buildDataType(
          "<type> FIXED_DICT "
          "<implementedBy> FixedDictWrappers.noStream </implementedBy> "
        ...
        DataSectionPtr pInputSection =
          getDataSectionForXML("<root> "
                               " <int8_5> 5 </int8_5> "
    ...
    TEST_F(EntityDefUnitTestHarness, Script_ScriptDataSink_PyUserType_as_stream)
    {
        ...
        ScriptDataSink sink;
        CHECK(sink.writeCustomType(*pUserType,
                                   stream,
                                   /* isPersistentOnly */ false));
        CHECK_EQUAL(0, stream.remainingLength());
        CHECK(!stream.error());
    ...
    TEST_F(EntityDefUnitTestHarness,
           Script_ScriptDataSink_PyUserType_as_section)
    {
        DataTypePtr pType = DataType::buildDataType(
          "<type> USER_TYPE "
          "<implementedBy> UserTypes.intStr </implementedBy> </type>");
