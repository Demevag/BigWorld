  ...
  const IEntityDistributionDecider* pDistributionDecider)
{
    ...
    if (pDistributionDecider) {
        ...
        if (!canBeOnClient_) {
            ...
            NOTICE_MSG("Support for <ClientName> is deprecated. It may be "
                       ...
                       "support@bigworldtech.com if you would like this "
                       "feature maintained\n");
        }
    }

    bool result = this->parseInterface(pSection, name_.c_str(), componentName);
    ...
    result &= initCompressionTypes(pSection->findChild("NetworkCompression"),
                                   internalNetworkCompressionType_,
                                   externalNetworkCompressionType_);
 ...
 *
 */
bool EntityDescription::parseService(
  const BW::string&                 name,
  const BW::string&                 componentName,
  const IEntityDistributionDecider* pDistributionDecider)
{
                                       ...
                                       const BW::string& componentName)
{
    ...
    forceExplicitDBID_ =
      pSection->readBool("ExplicitDatabaseID", forceExplicitDBID_);
    ...
    result &= this->parseTempProperties(
      pSection->openSection("TempProperties"), interfaceName, componentName);

    result &=
      this->parseComponents(pSection->openSection("Components"), interfaceName);
                                        ...
                                        const char*    interfaceName)
{
    ...
    for (DataSectionIterator itComponent = pComponents->begin();
        ...
        if (componentSections.end() == itComponentDef) {
            ...
            itComponentDef =
              componentSections
                .insert(std::make_pair(componentType, pComponentFile))
                .first;
        }
        ...
        if (!this->parseComponent(
              itComponentDef->second, interfaceName, componentName.c_str())) {
            ERROR_MSG("EntityDescription::parseComponents: "
                                     ...
                                     const BW::string& componentName)
{
    ...
    if (!isService_) {
        result &= this->parseCellMethods(
          pSection->openSection("CellMethods"), interfaceName, componentName);

        result &= this->parseClientMethods(
          pSection->openSection("ClientMethods"), interfaceName, componentName);
    }
                                        ...
                                        const BW::string& componentName)
{
    ...
    for (DataSectionIterator iter = pProperties->begin();
        ...
        if (!dataDescription.parse(
              *iter, name_, componentName, &numLatestChangeOnlyMembers_)) {
        ...
        if (propIter != propertyMap.end()) {
            ...
            if (dataDescription.isClientServerData()) {
                clientServerIndex = properties_[index].clientServerFullIndex();
            }
        }
        ...
        if (dataDescription.isClientServerData()) {
            if (clientServerIndex != -1) {
                ...
                clientServerProperties_.push_back(dataDescription.index());
            }
                                         ...
                                         const BW::string& componentName)
{
    ...
    return cell_.init(
      pMethods, MethodDescription::CELL, interfaceName, componentName);
}
                                         ...
                                         const BW::string& componentName)
{
    ...
    return base_.init(
      pMethods, MethodDescription::BASE, interfaceName, componentName);
}
...
{
    return isService_ ? EntityDef::Constants::servicesDefsPath()
 ...
 */
/*static*/ int EntityDescription::getDataDomains(StreamContentType contentType)
{
    int dataDomains = 0;
 ...
 *	on the current pass.
 */
/*static*/ bool EntityDescription::shouldConsiderData(
  int                    pass,
...
class AddToStreamVisitor
{
  public:
    ...
    virtual bool isErrorOnNULL() const { return true; }
                                              ...
                                              int numDataSizes) const
{
    ...
    return this->addToStream(
      visitor, stream, dataDomains, pDataSizes, numDataSizes);
}
    ...
    for (int pass = 0; pass < NUM_PASSES; pass++) {
        if (!EntityDescription::shouldSkipPass(pass, dataDomains)) {
            ...
            for (uint i = 0; i < this->propertyCount(); i++) {
                ...
                if (EntityDescription::shouldConsiderData(
                      pass, pDD, dataDomains)) {
                    // TRACE_MSG( "EntityDescription::addToStream: "
                    //			"Adding property = %s\n", pDD->name().c_str() );

                    if (!visitor.addToStream(
                          *pDD,
                          stream,
                        ...
                        ERROR_MSG(
                          "EntityDescription::addToStream(%s): "
                                         ...
                                         const ScriptMapping& dict) const
{
    class Visitor : public IDataDescriptionVisitor
    {
      public:
        ...
        bool visit(const DataDescription& dataDesc)
        {
            ...
            ScriptDataSink sink;
            bool           result =
              dataDesc.createFromStream(stream_, sink, onlyPersistent_);
    ...
    Visitor visitor(stream, dict, ((dataDomains & ONLY_PERSISTENT_DATA) != 0));
 ...
 */
bool EntityDescription::readTaggedClientStreamToDict(
  BinaryIStream&       stream,
  const ScriptMapping& dict,
  bool                 allowOwnClientData) const
{
    uint8 size;
    stream >> size;
    ...
    for (uint8 i = 0; i < size; i++) {
        ...
        if (pDD != NULL && (pDD->isOtherClientData() ||
                            (allowOwnClientData && pDD->isOwnClientData()))) {
            ScriptDataSink sink;
            bool           result = pDD->createFromStream(stream,
                                                sink,
                                            ...
                                            DataSectionPtr pSection) const
{
    class Visitor : public IDataDescriptionVisitor
    {
      ...
      public:
        ...
        bool visit(const DataDescription& dataDesc)
        {
            DataSectionPtr pCurr =
              pSection_->openSection(dataDesc.fullName(), true);

            MF_ASSERT_DEV(pCurr);

            if (pCurr) {
                if (!dataDesc.fromStreamToSection(
                      stream_, pCurr, onlyPersistent_)) {
    ...
    Visitor visitor(
      *this, stream, pSection, ((dataDomains & ONLY_PERSISTENT_DATA) != 0));
    return this->visit(dataDomains, visitor);
}
 ...
 */
const DataDescription* EntityDescription::clientServerProperty(
...
WatcherPtr EntityDescription::pWatcher()
{
    ...
    if (watchMe == NULL) {
        ...
        watchMe->addChild(
          "cellMethods", EntityMethodDescriptions::pWatcher(), &pNull->cell_);
        watchMe->addChild(
          "baseMethods", EntityMethodDescriptions::pWatcher(), &pNull->base_);
