...
namespace {
    ...
    {
      public:
        ClassStartStreamElement(const ClassDataType& type, bool& isNone)
  ...
  , clientSafety_(CLIENT_SAFE)
{
    ...
    int i = 0;
    for (Fields::iterator it = fields_.begin(); it != fields_.end();
         ++it, ++i) {
...
{
    if (pValue.get() == Py_None && allowNone_)
        return true;
    ...
    if (PyClassDataInstance::Check(pValue.get()) &&
        ...
        bool ok = pInst->setToForeign(pValue.get(),
                                      PyMapping_Check(pValue.get())
                                        ? pInst->FOREIGN_MAPPING
                                        : pInst->FOREIGN_ATTRS);
                                 ...
                                 DataSectionPtr pSection) const
{
    ...
    for (Fields_iterator it = fields_.begin(); it != fields_.end(); ++it) {
        if (!source.enterField(it->name_) ||
            !it->type_->addToSection(source, pSection->newSection(it->name_)) ||
            !source.leaveField()) {
            return false;
        }
    }
                                   ...
                                   int                ownerRef)
{
    ...
    if (pInst.exists() && pInst->dataType() == this) {
        if (pInst->hasOwner()) { // note: up to caller to check that prop isn't
            ...
            pInst                           = PyClassDataInstancePtr(
              new PyClassDataInstance(this),
    ...
    // otherwise just see if it has the same keys/attrs
        ...
        pInst =
          PyClassDataInstancePtr(new PyClassDataInstance(this),
                                 PyClassDataInstancePtr::FROM_NEW_REFERENCE);
...
{
    if (!PyClassDataInstance::Check(pObject.get()))
...
DataType::StreamElementPtr ClassDataType::getStreamElement(
  size_t  index,
  size_t& size,
  bool&   isNone,
  bool    isPersistentOnly) const
{
    ...
    if (index == 0) {
        ...
        return StreamElementPtr(new ClassStartStreamElement(*this, isNone));
    } else if (index < (size * 2) + 1) {
        int fieldIndex = static_cast<int>(index - 1) / 2;
        if (index % 2 == 1) {
            return StreamElementPtr(new ClassEnterFieldStreamElement(
              *this,
              *this->getFields()[fieldIndex].type_,
              this->getFields()[fieldIndex].name_));
        } else {
            return StreamElementPtr(new ClassLeaveFieldStreamElement(*this));
        }
    } else if (index == (size * 2) + 1) {
...
bool ClassDataType::Field::createDefaultValue(DataSectionPtr pDefaultSection,
                                              DataSink&      sink) const
{
    DataSectionPtr pChildSection =
      pDefaultSection ? pDefaultSection->openSection(name_) : NULL;
