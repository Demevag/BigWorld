                                     ...
                                     bool /* isPersistentOnly */) const
{
    ...
    ScriptObject result = converter.callMethod(
      "addToStream", ScriptArgs::create(object), ScriptErrorPrint());
                                          ...
                                          bool /* isPersistentOnly */) const
{
    ...
    object = converter.callMethod(
      "createFromStream", ScriptArgs::create(data), ScriptErrorPrint());
                                      ...
                                      DataSectionPtr      pSection) const
{
    ...
    ScriptObject result = converter.callMethod(
      "addToSection", ScriptArgs::create(object, section), ScriptErrorPrint());
                                           ...
                                           DataSectionPtr pSection) const
{
    ...
    object = converter.callMethod(
      "createFromSection", ScriptArgs::create(section), ScriptErrorPrint());
...
bool UserDataType::getDefaultValue(DataSink& output) const
{
    ...
    ScriptObject pResult(PyObject_CallMethod(pObject.get(), "defaultValue", ""),
                                       ...
                                       bool           isPersistentOnly) const
{
    ...
    if (!pMethod) {
        return this->DataType::fromStreamToSection(
                                       ...
                                       bool           isPersistentOnly) const
{
    ...
    if (!pMethod) {
        return this->DataType::fromSectionToStream(
    ...
    PyObject* pResult = Script::ask(pMethod.newRef(),
  ...
  bool isPersistentOnly) const
{
    size = 1;
    if (index == 0) {
        return StreamElementPtr(new UserStreamElement(*this, isPersistentOnly));
    }
...
bool UserDataType::operator<(const DataType& other) const
{
    ...
    int           diff      = (moduleName_ + instanceName_)
                 .compare(otherUser.moduleName_ + otherUser.instanceName_);
    if (diff < 0)
        return true;
    if (diff > 0)
        return false;
...
void UserDataType::init()
{
    ...
    if (pModule) {
        pObject_ = PyObject_GetAttrString(
          pModule.get(), const_cast<char*>(instanceName_.c_str()));
        if (!pObject_) {
...
inline ScriptObject UserDataType::method(const char* name) const
{
    ...
    ScriptObject pMethod(
      PyObject_GetAttrString(pObject.get(), const_cast<char*>(name)),
      ScriptObject::STEAL_REFERENCE);
