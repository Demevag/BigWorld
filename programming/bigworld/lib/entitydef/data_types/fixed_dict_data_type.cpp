...
namespace {
    ...
    class FixedDictStreamedStreamElement : public DataType::StreamElement
    {
      public:
        ...
        */
        ...
        bool isCustomStreamed() const { return true; }
        ...
        bool fromSourceToStream(DataSource& source, BinaryOStream& stream) const
        {
            ...
            return source.readCustomType(
              fixedDictType, stream, isPersistentOnly_);
        }
    ...
    {
      public:
        FixedDictStartStreamElement(const FixedDictDataType& type, bool& isNone)
          : DataType::StreamElement(type)
          , isNone_(isNone)
        {
#if defined SCRIPT_PYTHON
...
class ScriptDictPropertyOwner : public PropertyOwnerBase
{
  public:
    ...
    virtual PropertyOwnerBase* getChildPropertyOwner(int childIndex) const
    {
        ...
        ScriptObject childProperty =
          dict_.getItem(name.c_str(), ScriptErrorClear());

        if (!childProperty) {
            return NULL;
        }

        return pFixedDictDataType_->getFieldDataType(childIndex)
          .asOwner(childProperty);
    }
    ...
    virtual ScriptObject setOwnedProperty(int childIndex, BinaryIStream& data)
    {
        ...
        BW::string propertyName = pFixedDictDataType_->getFieldName(childIndex);

        ScriptObject oldValue =
          dict_.getItem(propertyName.c_str(), ScriptErrorClear());
 ...
 *	Constructor.
 */
FixedDictDataType::FixedDictDataType(MetaDataType* pMeta,
                                     Fields&       fields,
                                     bool          allowNone)
  ...
  , clientSafety_(CLIENT_SAFE)
{
    ...
    int i = 0;
    for (Fields::const_iterator it = fields_.begin(); it != fields_.end();
         ++it, ++i) {
...
void FixedDictDataType::setCustomClassFunctions(bool ignoreFailure)
{
    ...
    if (pModule) {
        ...
        if (pImplementor_) {
            pGetDictFromObjFn_ = PyObjectPtr(
              PyObject_GetAttrString(pImplementor_.get(), "getDictFromObj"),
              PyObjectPtr::STEAL_REFERENCE);
            ...
            pCreateObjFromDictFn_ = PyObjectPtr(
              PyObject_GetAttrString(pImplementor_.get(), "createObjFromDict"),
              PyObjectPtr::STEAL_REFERENCE);
            ...
            pIsSameTypeFn_ = PyObjectPtr(
              PyObject_GetAttrString(pImplementor_.get(), "isSameType"),
              PyObjectPtr::STEAL_REFERENCE);
            if (!pIsSameTypeFn_) {
                PyErr_Clear();
            ...
            pAddToStreamFn_ = PyObjectPtr(
              PyObject_GetAttrString(pImplementor_.get(), "addToStream"),
              PyObjectPtr::STEAL_REFERENCE);
            if (!pAddToStreamFn_)
                PyErr_Clear();

            pCreateFromStreamFn_ = PyObjectPtr(
              PyObject_GetAttrString(pImplementor_.get(), "createFromStream"),
              PyObjectPtr::STEAL_REFERENCE);
            if (!pCreateFromStreamFn_)
                PyErr_Clear();
...
bool FixedDictDataType::isSameType(ScriptObject pValue)
{
    ...
    if (allowNone_ && pValue.isNone()) {
        ...
        if (this->hasCustomIsSameType()) {
            ...
            // getDictFromObj() function. If it succeeds, we'll assume that it
    ...
    // check if an instance of our special Python class
        ...
        for (Fields::const_iterator i = fields_.begin();
             (i != fields_.end()) && isSame;
             ++i) {
            ScriptObject pKeyedValue =
              dict.getItem(i->name_.c_str(), ScriptErrorClear());

            isSame = pKeyedValue ? i->type_->isSameType(pKeyedValue) : false;
        }
    } else {
        isSame = false;
    }
...
void FixedDictDataType::addToMD5(MD5& md5) const
{
    ...
    if (!isCustomClassImplInited_ || this->hasCustomClass()) {
        ...
        md5.append(instanceName_.c_str(),
                   static_cast<int>(instanceName_.size()));
    }
#endif // SCRIPT_PYTHON
...
bool FixedDictDataType::operator<(const DataType& other) const
{
    ...
    if (customTypeName_ < otherCl.customTypeName_)
        return true;
    if (otherCl.customTypeName_ < customTypeName_)
        return false;

    if (moduleName_ < otherCl.moduleName_)
        return true;
    if (otherCl.moduleName_ < moduleName_)
        return false;

    if (instanceName_ < otherCl.instanceName_)
        return true;
    if (otherCl.instanceName_ < instanceName_)
        return false;

    if (allowNone_ != otherCl.allowNone_)
        return !allowNone_; // we are less if our allowNone is false

    if (fields_.size() < otherCl.fields_.size())
        return true;
    if (otherCl.fields_.size() < fields_.size())
        return false;
    ...
    for (uint i = 0; i < fields_.size(); ++i) {
        ...
        if ((*fields_[i].type_) < (*otherCl.fields_[i].type_))
            return true;
        if ((*otherCl.fields_[i].type_) < (*fields_[i].type_))
            return false;
    }
...
DataType::StreamElementPtr FixedDictDataType::getStreamElement(
  size_t  index,
  size_t& size,
  bool&   isNone,
...
{
    size = isNone ? 0
                  : (isPersistentOnly ? this->getNumPersistentFields()
                                      : this->getNumFields());
    ...
    if (hasCustomClass()) {
        ...
        if (index == 0) {
            return StreamElementPtr(new FixedDictStreamedStreamElement(
              *this, isPersistentOnly, isNone));
        }
    ...
    if (index == 0) {
        ...
        return StreamElementPtr(new FixedDictStartStreamElement(*this, isNone));
        ...
        if (index % 2 == 1) {
            return StreamElementPtr(new FixedDictEnterFieldStreamElement(
              *this,
                                             ...
                                             DataSectionPtr pSection) const
{
    ...
    for (Fields::const_iterator it = fields_.begin(); it != fields_.end();
        ...
        ScriptDataSource source(pValue);
        if (!it->type_->addToSection(source, pSection->newSection(it->name_))) {
            return false;
        }
    }
    return true;
...
bool FixedDictDataType::isSameTypeCustom(ScriptObject pValue) const
{
    ScriptObject pResult(
      PyObject_CallFunctionObjArgs(pIsSameTypeFn_.get(), pValue.get(), NULL),
    ...
    if (pGetDictFromObjFn_) {
        ScriptObject pResult(
          PyObject_CallFunctionObjArgs(
            pGetDictFromObjFn_.get(), pCustomClass.get(), NULL),
          ScriptObject::STEAL_REFERENCE);
        if (pResult) {
  ...
  PyFixedDictDataInstance* pInst) const
{
    ScriptObject pResult(
      PyObject_CallFunctionObjArgs(
        pCreateObjFromDictFn_.get(), static_cast<PyObject*>(pInst), NULL),
...
{
    ScriptObject pResult(
      PyObject_CallFunctionObjArgs(pAddToStreamFn_.get(), pValue.get(), NULL),
      ScriptObject::FROM_NEW_REFERENCE);
    if (pResult) {
  ...
  bool           isPersistentOnly) const
{
    ...
    ScriptObject pResult(
      PyObject_CallFunction(
        pCreateFromStreamFn_.get(), "s#", data.data(), data.length()),
      ScriptObject::STEAL_REFERENCE);
    if (!pResult) {
        ERROR_MSG("FixedDictDataType::createFromStreamCustom: "
 ...
 */
FixedDictDataType::Fields::size_type FixedDictDataType::getNumPersistentFields()
  const
{
