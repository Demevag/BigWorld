 ...
 */
int Script::setData(PyObject* pObject, VolatileInfo& rInfo, const char* varName)
{
    if (!PySequence_Check(pObject) || PyObject_Length(pObject) != 4) {
        PyErr_Format(
          PyExc_TypeError, "%s must be a sequence of length 4", varName);
        return -1;
    }
    ...
    VolatileInfo newInfo(
      positionPriority, yawPriority, pitchPriority, rollPriority);
    ...
    PyErr_Format(PyExc_TypeError,
                 ...
                 "(The last 3 values must be descending)",
                 varName);
    return -1;
}
...
PyObject* Script::getData(const VolatileInfo& info)
{
    ...
    PyTuple_SetItem(
      pTuple, 0, PyVolatileInfo::pyObjectFromPriority(info.positionPriority()));
    PyTuple_SetItem(
      pTuple, 1, PyVolatileInfo::pyObjectFromPriority(info.yawPriority()));
    PyTuple_SetItem(
      pTuple, 2, PyVolatileInfo::pyObjectFromPriority(info.pitchPriority()));
    PyTuple_SetItem(
      pTuple, 3, PyVolatileInfo::pyObjectFromPriority(info.rollPriority()));
