...
namespace URL {
    ...
    void Manager::registerRequest(Request& request)
    {
        ...
        CURLMcode ret =
          curl_multi_add_handle(curlMultiState_, request.handle());
                           ...
                           Request**         ppOutRequest /*= NULL*/)
    {
        ...
        pRequest->fetchURL(
          url, pResponse, pHeaders, method, connectTimeoutInSeconds, pPostData);
    ...
    void Manager::processEvents()
    {
        ...
        CURLMsg* msg             = NULL;
        while ((msg = curl_multi_info_read(curlMultiState_,
                                           &messagesInQueue)) != NULL) {
    ...
    BW::string escape(const BW::string& in)
    {
        ...
        char* outCString = curl_easy_escape(
          request.handle(), in.c_str(), static_cast<int>(in.size()));
