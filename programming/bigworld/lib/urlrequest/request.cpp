...
namespace URL {
    ...
    namespace // (anonymous)
    {
         ...
         */
        size_t curl_write_function(void*  ptr,
                                   size_t size,
                                   size_t nmemb,
                                   void*  userdata)
        {
            ...
            // From CURL's point of view, it is writing data out somewhere. From
            // the application point of view, we are reading data from a URL.
            // URLRequest abstracts the curl crazy away, but this is why there
            // is a function being called here named read inside a function
            // named write.
            return pRequest->onRead(reinterpret_cast<const char*>(ptr),
                                    size * nmemb);
        }

#if defined(BW_CURL_ENABLE_DEBUG)
        int curl_debug_function(CURL*         handle,
                                curl_infotype type,
                                char*         data,
                                size_t        size,
                                void*         userptr)
    ...
    size_t Request::onRead(const char* data, size_t len)
    {
        ...
        if (pResponse_) {
            if (responseCode_ == 0 &&
                this->getCurlInfo(CURLINFO_RESPONSE_CODE, &responseCode_) ==
                  CURLE_OK) {
            ...
            if (!haveReadContentType_) {
                ...
                if (CURLE_OK ==
                    this->getCurlInfo(CURLINFO_CONTENT_TYPE, &contentType)) {
                    if (!pResponse_->onContentTypeHeader(BW::string(
                           ...
                           PostData*         pPostData)
    {
        ...
        long connectTimeoutInMilliseconds =
          long(connectTimeoutInSeconds * 1000);
        ret = this->setCurlOption(CURLOPT_CONNECTTIMEOUT_MS,
                                  connectTimeoutInMilliseconds);

        if (ret != CURLE_OK) {
            ERROR_MSG(
              "URL::Request::fetchURL: could not set connect timeout: %s\n",
              this->curlEasyErrorToString(ret).c_str());
            return false;
        }

        ret =
          this->setCurlOption(CURLOPT_TIMEOUT_MS, connectTimeoutInMilliseconds);
