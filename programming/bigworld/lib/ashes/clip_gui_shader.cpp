 ...
 */
bool ClipGUIShader::processComponent(SimpleGUIComponent& component, float dTime)
{
    BW_GUARD;
    int        nVerts;
    ...
    if (slant) {
        if (nVerts == 4)
            ...
            WARNING_MSG(
              "ClipGUIShader::processComponent - slant only implemented for "
              "gui components with 4 vertices ... ignoring slant\n");
    }
 ...
 *	@param dTime		The delta time for the current frame of the application
 */
bool ClipGUIShader::processComponentWithSlant(SimpleGUIComponent& component,
                                              float               dTime)
{
    BW_GUARD;
    int        nVerts;
