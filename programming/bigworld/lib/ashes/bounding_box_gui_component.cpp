 ...
 *  are thus directly extracted from it. In addition, when this attribute is
 *true but no source matrix provider is set, the component's own vertices are
 ...
 *	@components{ client, tools }
 *	Determines the rules for the subspace that the bounding box's children are
 *in. The lowest two bits of this value control whether or not the x and y axes
 *are scaled to the size of the bounding box, or left at the size of the screen.
 ...
 *	@components{ client, tools }
 *	By default, when the bounding box is not on the screen, the children will
 *not be drawn. Set this to true to ensure the children will be drawn when the
 *BoundingBoxGUIComponent is off-screen.  Defaults to 0 (false).
 *	@type	boolean (0 or 1)
 */
 ...
 *	Constructor.
 */
BoundingBoxGUIComponent::BoundingBoxGUIComponent(const BW::string& textureName,
                                                 PyTypeObject*     pType)
                                     ...
                                     float relParentHeight)
{
    ...
    // this is valid because CameraApp::tick sets an appropriate
    // viewProjectionMatrix, which occurs well before GUI::tick.
 ...
 */
void BoundingBoxGUIComponent::preDraw(float relParentWidth,
                                      float relParentHeight)
{
    BW_GUARD;
    ...
    if (clipSpaceSource_) {
        ...
        // calculate the position of the corner of the bounding box, in clip
        // coords Transform the local bounding box into clip space Then divide
        // by w ( if we can ) Finally, work out the min/max extents in
        // post-perspective homogenous clip coordinates
        Matrix objectToCamera = bbMatrix;
        objectToCamera.postMultiply(savedView_);

        if (!calculatePostDivideMinMax(
              min, max, bb_, objectToCamera, savedProjection_)) {
        ...
        // constrain corners, so that they don't get closer than one width /
 ...
 *	@param dTime	Delta time for this frame of the app
 */
void BoundingBoxGUIComponent::applyShaders(float dTime)
{
    BW_GUARD;
    return;
}
 ...
 */
void BoundingBoxGUIComponent::draw(Moo::DrawContext& drawContext,
                                   bool              reallyDraw,
                                   bool              overlay)
{
    BW_GUARD;
    // figure out where to draw stuff (like update but needs rc)
    this->preDraw(SimpleGUI::instance().screenWidth(),
    ...
    if (onScreen_) {
        ...
        if (clipToBox_) {
            Vector2 halfDim(SimpleGUI::instance().halfScreenWidth(),
            ...
            newViewport.X =
              max(int(oldViewport.X), int((1 + minInClip_.x) * halfDim.x));
            newViewport.Y =
              max(int(oldViewport.Y), int((1 - maxInClip_.y) * halfDim.y));
            newViewport.Width = min(int(oldViewport.X + oldViewport.Width),
                                    int((1 + maxInClip_.x) * halfDim.x)) -
                                int(newViewport.X);
            newViewport.Height = min(int(oldViewport.Y + oldViewport.Height),
                                     int((1 - minInClip_.y) * halfDim.y)) -
                                 int(newViewport.Y);

            if (int(newViewport.Height) <= 0)
                return;
            if (int(newViewport.Width) <= 0)
                return;
 ...
 */
bool BoundingBoxGUIComponent::calculatePostDivideMinMax(
  Vector3&      min,
  Vector3&      max,
  BoundingBox&  _bb,
                                   ...
                                   LoadBindings&     bindings)
{
    ...
    alwaysDisplayChildren_ =
      pSect->readBool("alwaysDisplayChildren", alwaysDisplayChildren_);
 ...
 *	Save
 */
void BoundingBoxGUIComponent::save(DataSectionPtr pSect, SaveBindings& bindings)
{
    BW_GUARD;
