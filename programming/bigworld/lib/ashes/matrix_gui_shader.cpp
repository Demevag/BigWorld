...
class WorldToClipMP : public MatrixProvider
{
    Py_Header(WorldToClipMP, MatrixProvider)
      ...
      public
      : WorldToClipMP(PyTypeObject* pType = &s_type_)
 ...
 *	@components{ client, tools }
 *
 *	Used primarily with the MatrixGUIShader, this returns a new WorldToClip
 *MatrixProider object.
 */
                                       ...
                                       float               dTime)
{
    ...
    // easy!
    return false; // don't need to update children - runTimeTransform already
                  // persists
}
