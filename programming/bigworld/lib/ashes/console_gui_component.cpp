...
PY_RW_ACCESSOR_ATTRIBUTE_DECLARE(int,
                                 pConsole_->le().lineLength,
                                 editLineLength)
 ...
 *	@components{ client, tools }
 *
 *	Creates and returns a new ConsoleGUIComponent, which displays an editable
...
PyObject* ConsoleGUIComponent::py_prints(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(args, "s", &str)) {
        PyErr_SetString(PyExc_TypeError, "Console.prints: Expected a string");
        return NULL;
    }
...
{
    BW_GUARD;
    Vector2 pos(static_cast<float>(pConsole_->cursorX()),
 ...
 */
void ConsoleGUIComponent::draw(Moo::DrawContext& drawContext,
                               bool              reallyDraw,
                               bool              overlay)
{
    BW_GUARD;
 ...
 *	Load
 */
bool ConsoleGUIComponent::load(DataSectionPtr    pSect,
                               ...
                               LoadBindings&     bindings)
{
    ...
    if (pLines) {
        ...
        for (it = pLines->begin(); it != pLines->end(); ++it) {
            ...
            if (sname == "colour") {
                pConsole_->lineColourOverride(
                  i, Colour::getUint32((*it)->asVector4()));
            } else if (sname == "line") {
...
void ConsoleGUIComponent::save(DataSectionPtr pSect, SaveBindings& bindings)
{
    ...
    for (int i = 0; i < MAX_CONSOLE_HEIGHT; i++) {
        ...
        if (pConsole_->lineColourRetrieve(i, lc))
            pSect->newSection("colour")->setVector4(Colour::getVector4(lc));
