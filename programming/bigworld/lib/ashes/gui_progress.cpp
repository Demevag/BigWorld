...
void GUIProgressDisplay::add(const BW::string& str)
{
    ...
    if (this->gui_->script().exists()) {
        fn = PyObject_GetAttrString(this->gui_->script().getObject(),
                                    "addMessage");
    }

    if (fn) {
        Script::call(
          &*fn, Py_BuildValue("(s)", str.c_str()), "GUIProgress add: ");
    }
...
void GUIProgressDisplay::append(const BW::string& str)
{
    ...
    if (fn) {
        Script::call(
          &*fn, Py_BuildValue("(s)", str.c_str()), "GUIProgress append: ");
    }
...
bool GUIProgressDisplay::draw(bool force)
{
    ...
    if (percentDone - s_lastPercent > MIN_PROGRESS_STEP || force) {
        ...
        if (fn) {
            Script::call(
              &*fn, Py_BuildValue("(f)", percentDone), "GUIProgress draw: ");
        } else {
            SimpleGUIComponentPtr bar = gui_->child("bar");
            if (bar) {
                GUIShaderPtr shader = bar->shader("clipper");
                if (shader) {
                    ClipGUIShader* clipper =
                      static_cast<ClipGUIShader*>(&*shader);
        ...
        float currentTime =
          lastTime_ < 0.f ? currentTime = 0.f : currentTime = lastTime_;
