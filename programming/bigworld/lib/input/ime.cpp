...
namespace {
    IME::State immGetIMEState()
    {
        switch (ImeUi_GetState()) {
            case IMEUI_STATE_ENGLISH:
                return IME::STATE_ENGLISH;
            case IMEUI_STATE_ON:
                return IME::STATE_ON;
            case IMEUI_STATE_OFF:
                return IME::STATE_OFF;
            default:
                return IME::STATE_OFF;
        }
    }
            ...
            case LANG_CHINESE:
                switch (SUBLANGID(ImeUi_GetLanguage())) {
                    case SUBLANG_CHINESE_SIMPLIFIED:
                        return IME::LANGUAGE_CHS;
                    case SUBLANG_CHINESE_TRADITIONAL:
                        return IME::LANGUAGE_CHT;
                    default:
                        return IME::LANGUAGE_NON_IME;
                }
            case LANG_CHS:
                return IME::LANGUAGE_CHS;
            case LANG_CHT:
                return IME::LANGUAGE_CHT;
            case LANG_JAPANESE:
                return IME::LANGUAGE_JAPANESE;
            case LANG_KOREAN:
                return IME::LANGUAGE_KOREAN;
            default:
                return IME::LANGUAGE_NON_IME;
        }
    }
...
bool IME::handleWindowsMessage(HWND     hWnd,
                               UINT     msg,
                               WPARAM&  wParam,
                               LPARAM&  lParam,
                               LRESULT& result)
{
    ...
    bool    handled = false;
    LRESULT imeResult =
      ImeUi_ProcessMessage(hWnd, msg, wParam, lParam, &handled);
    if (handled) {
        result = imeResult;
    }
    return handled;
...
void IME::onInputLangChange()
{
    if (hwnd_) {
        ...
        // any pause directly after language change it seems to screw up the
        // refresh.
...
void IME::update()
{
    ...
    bool compAttrChanged = (compositionAttr_.size() != composition_.size());
