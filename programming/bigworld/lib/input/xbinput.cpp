...
class XBInput
{
  public:
    ...
    bool rumbleEnabled() const { return rumbleEnabled_; }
...
{
    MF_WATCH("Client Settings/rumble",
             *this,
...
void XBInput::update()
{
    ...
    if (port_ != -1 && ERROR_SUCCESS == XInputGetState(padHandle_, &state_)) {
        leftStick_.set(stickValue(state_.Gamepad.sThumbLX),
                       stickValue(state_.Gamepad.sThumbLY));
        rightStick_.set(stickValue(state_.Gamepad.sThumbRX),
                        stickValue(state_.Gamepad.sThumbRY));
    } else {
...
bool Joystick::updateFromJoystickDevice()
{
    ...
    this->getAxis(AxisEvent::AXIS_LX)
      .value(scaleFromDIToUnit(padState.Gamepad.sThumbLX));
    this->getAxis(AxisEvent::AXIS_LY)
      .value(scaleFromDIToUnit(padState.Gamepad.sThumbLY));

    this->getAxis(AxisEvent::AXIS_RX)
      .value(scaleFromDIToUnit(padState.Gamepad.sThumbRX));
    this->getAxis(AxisEvent::AXIS_RY)
      .value(scaleFromDIToUnit(padState.Gamepad.sThumbRY));
