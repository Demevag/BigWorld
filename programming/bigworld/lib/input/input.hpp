...
class KeyEvent
{
  public:
    ...
    static KeyEvent make(KeyCode::Key   key,
                         int32          state,
                         uint32         mods,
     ...
     *	this key event did not generate a character, false is returned and
    ...
    ///	event occurred.
    bool isShiftDown() const { return !!(modifiers_ & MODIFIER_SHIFT); }
    /// This method returns whether or not either Ctrl key was down when this
    ///	event occurred.
    bool isCtrlDown() const { return !!(modifiers_ & MODIFIER_CTRL); }
    /// This method returns whether or not either Alt key was down when this
    ///	event occurred.
    bool isAltDown() const { return !!(modifiers_ & MODIFIER_ALT); }
    ...
    BW::string toString() const
    {
        ...
        oss << "Key " << KeyCode::keyToString(key_) << " state " << state_
...
class Joystick
{
  public:
    ...
    class Axis
    {
      public:
        ...
        /// This method returns whether or not a zero position event has been
        /// sent.
    ...
    bool init(IDirectInput8* pDirectInput, void* hWnd);
  ...
  private:
    ...
    void generateKeyEvent(bool           isDown,
                          int            key,
                          InputHandler&  handler,
                          KeyStates&     keyStates,
                          const Vector2& cursorPosition);

    void generateCommonEvents(InputHandler&  handler,
                              KeyStates&     keyStates,
                              const Vector2& cursorPosition);
...
class InputDevices : public Singleton<InputDevices>
{
  public:
    ...
    void forceKeyUp(KeyCode::Key key, uint32 mods, const Vector2& cursorPos);
