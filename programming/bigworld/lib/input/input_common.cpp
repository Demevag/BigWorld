 ...
 */
void Joystick::generateKeyEvent(bool           isDown,
                                int            key,
                                ...
                                const Vector2& cursorPosition)
{
    ...
    KeyEvent event;
    event.fill(
      KeyCode::Key(key), isDown, InputDevices::modifiers(), cursorPosition);
...
{
    BW_GUARD;
    if (joy_x * joy_x + joy_y * joy_y < box * box)
        return 4;
 ...
 *	internal data.
 */
void Joystick::generateCommonEvents(InputHandler&  handler,
                                    KeyStates&     keyStates,
                                    const Vector2& cursorPosition)
{
    BW_GUARD;
    ...
    for (int i = 0; i < 2; i++) {
        ...
        quantJoyDir_[i] = joystickDirection(
          axis_[horA].value(), axis_[verA].value(), JOY_SEL_AMT);
    }
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if ((nDir = quantJoyDir_[i] % 3) != (oDir = oldJoyDir[i] % 3)) {
            if (oDir != 1)
                this->generateKeyEvent(false,
                                       keyEvBase + (oDir ? 3 : 2),
                                       handler,
                                       keyStates,
                                       cursorPosition);
            if (nDir != 1)
                this->generateKeyEvent(true,
                                       keyEvBase + (nDir ? 3 : 2),
                                       handler,
                                       keyStates,
                                       cursorPosition);
