...
namespace controls {
    ...
    /**
     *	This gives sensible values to the column/row sizing information used in
     *a GridSizer.
     */
    ...
    /**
     *	This constructs a GridSizer that also sizes a control such as a
     *group-box.
     *
     *  @param columns		The number of columns.
     *  @param rows			The number of rows.
     ...
     */
    GridSizer::GridSizer(uint32 columns, uint32 rows, CWnd* container)
      : Sizer()
      , wnd_(container)
      , id_(0)
      , leftEdgeGap_(DEFAULT_EDGE_GAP)
     ...
     */
    GridSizer::GridSizer(uint32 columns,
                         uint32 rows,
                         CWnd*  parent,
                         uint32 containerID)
      : Sizer()
     ...
     */
    void GridSizer::setChild(uint32 column, uint32 row, SizerPtr sizer)
    {
        BW_GUARD;
    ...
    /**
     *	This is called when the sizer should be resized to fit inside a
     *rectangle.
     *
        ...
        for (size_t row = 0; row < rows_; ++row) {
            ...
            for (size_t col = 0; col < columns_; ++col) {
                ...
                if (colInfoSizes_[col].units_ == PIXELS)
                    ...
                    colSize =
                      colInfoSizes_[col].size_ * weightXSize / sumXWeights;
        ...
        for (uint32 col = 0; col < columns_; ++col) {
            ...
            for (uint32 row = 0; row < rows_; ++row) {
                ...
                if (sizer != NULL) {
                    ...
                    minXThisCol =
                      std::max((uint32)thisSizerMinSz.cx, minXThisCol);
                }
            }
            cx += minXThisCol;
        }
        ...
        for (uint32 row = 0; row < rows_; ++row) {
            ...
            for (uint32 col = 0; col < columns_; ++col) {
                ...
                if (sizer != NULL) {
                    CSize thisSizerMinSz = sizer->minimumSize();
                    minYThisRow =
                      std::max((uint32)thisSizerMinSz.cy, minYThisRow);
                }
            }
            cy += minYThisRow;
        }
