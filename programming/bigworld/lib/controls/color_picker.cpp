...
namespace controls {
      ...
      , alphaSelection_(true)
    {
        ...
        currentAlpha_ = 1.f;
        RGBtoHLS(
          currentColor_, currentHue_, currentLuminance_, currentSaturation_);
    }
    ...
    void ColorPicker::OnLButtonDown(UINT nFlags, CPoint point)
    {
        ...
        if (!slGraphDirty_) {
            ...
            // since the user can select the movement handle and still be
    ...
    void ColorPicker::OnMouseMove(UINT nFlags, CPoint point)
    {
        ...
        if (!slGraphDirty_ && (nFlags & MK_LBUTTON)) {
            if (mouseMoveMode_ == MMM_HUE) {
                ...
                currentHue_ = (float)(point.y - hueBarRect_.top) /
                              (float)hueBarRect_.Height();
                currentColor_ =
                  HLStoRGB(currentHue_, currentLuminance_, currentSaturation_);
                slGraphDirty_ = true;
                ...
                currentAlpha_ = (float)(point.x - alphaBarRect_.left) /
                                (float)alphaBarRect_.Width();
                slGraphDirty_ = true;
                ...
                currentLuminance_ = (float)(point.x - slGraphRect_.left) /
                                    (float)slGraphRect_.Width();
                currentSaturation_ = (float)(point.y - slGraphRect_.top) /
                                     (float)slGraphRect_.Height();
                currentColor_ =
                  HLStoRGB(currentHue_, currentLuminance_, currentSaturation_);
    ...
    void ColorPicker::OnPaint()
    {
        ...
        sz.cy = 6;
        CPoint pt(slGraphRect_.Width() + c_spacer_ + hueBarRect_.Width() / 2 +
                    c_borderSpace_,
        ...
        if (alphaSelection_) {
            ...
            pt    = CPoint((int)(currentAlpha_ * alphaBarRect_.Width()) +
                          c_borderSpace_,
                        (int)(slGraphRect_.Height() + c_spacer_ +
                              alphaBarRect_.Height() / 2) +
                          c_borderSpace_);
            drawCrossAt(pt, dc, sz);
        }
    ...
    void ColorPicker::generatePicker()
    {
        ...
        if (createPickerDC_) {
            ...
            pickerBmp.CreateCompatibleBitmap(
              GetDC(), slGraphRect_.Width(), slGraphRect_.Height());
    ...
    void ColorPicker::generateHueBar()
    {
        ...
        if (createHueBarDC_) {
            // Create bitmap:
            CBitmap hueBmp;
            hueBmp.CreateCompatibleBitmap(
              GetDC(), hueBarRect_.Width(), hueBarRect_.Height());
    ...
    void ColorPicker::generateAlphaBar()
    {
        ...
        if (createAlphaBarDC_) {
            ...
            alphaBmp.CreateCompatibleBitmap(
              GetDC(), alphaBarRect_.Width(), alphaBarRect_.Height());
        ...
        if (alphaSelection_) {
            ...
            for (int i = 0; i < alphaBarRect_.Width(); ++i) {
                ...
                COLORREF col      = HLStoRGB(
                  0.64f, theAlpha, 0.12f); // just pick a color to blend
                alphaBarDC_.FillSolidRect(i, 0, 1, alphaBarRect_.Height(), col);
            }
        } else {
            // Treat as a color previewer:
            alphaBarDC_.FillSolidRect(0,
                                      0,
                                      alphaBarRect_.Width(),
                                      alphaBarRect_.Height(),
                                      currentColor_);
