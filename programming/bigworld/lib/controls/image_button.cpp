...
namespace controls {
     ...
     */
    void ImageButton::setBitmapID(UINT     defID,
                                  UINT     disabledID,
     ...
     *
     *	@return					True if the button is a toggle button, false if
     ...
     *
     *	@param toggled			If true then the button is in the down state and
     *if false then it is in the up state.
     */
     ...
     *
     *	@returns				True if the button is in the down state, false
     *if is in the up state.
     */
    ...
    /*virtual*/ void ImageButton::DrawItem(LPDRAWITEMSTRUCT drawItemStruct)
    {
        ...
        bool disable =
          ((drawItemStruct->itemState & ODS_DISABLED) == ODS_DISABLED);
     ...
     *  @param clrMask      The transparent colour.
     */
    void ImageButton::transparentBlit(CDC*     dc,
                                      int      left,
                                      int      top,
                                      int      width,
                                      int      height,
                                      HBITMAP  bitmap,
                                      COLORREF clrMask) const
    {
        BW_GUARD;
        ...
        dc->TransparentBlt(
          left, top, width, height, &srcDC, 0, 0, width, height, clrMask);
