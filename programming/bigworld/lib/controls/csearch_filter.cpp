...
namespace controls {
     ...
     */
    BOOL CSearchFilter::Create(DWORD       style,
                               RECT const& extents,
                               CWnd*       parent,
                               UINT        id,
                               UINT        sbmpID,
                               UINT        cbmpID)
    {
        ...
        // Create this control:
        BOOL result =
          CWnd::Create(className.c_str(), NULL, style, extents, parent, id);

        // Load the bitmaps:
        searchBmp_ = (HBITMAP)::LoadImage(AfxGetInstanceHandle(),
                                          MAKEINTRESOURCE(sbmpID),
                                          IMAGE_BITMAP,
                                          0,
                                          0,
                                          LR_DEFAULTCOLOR | LR_LOADTRANSPARENT);
        clearBmp_  = (HBITMAP)::LoadImage(AfxGetInstanceHandle(),
    ...
    /**
     *  This functions returns whether the text displayed when the edit control
     * is empty is currently displayed.
     *
     ...
     */
    BOOL CSearchFilter::OnSetCursor(CWnd* /*wnd*/,
                                    UINT /*hitTest*/,
                                    UINT /*message*/
    )
    {
    ...
    /**
     *  Called when the left mouse button is pressed down.  We test to see if
     * the user has clicked within the clear area, and if so clear the text.
     *
    ...
    void CSearchFilter::OnLButtonDown(UINT flags, CPoint point)
    {
        ...
        if (isClearVis) {
            ...
            if (clearRect.PtInRect(point)) {
                edit_->SetWindowText(
                  CString()); // hiding etc is done in OnEditText
            }
        }
     ...
     */
    void CSearchFilter::getRects(CRect const& client,
                                 CRect&       searchRect,
                                 CRect&       editRect,
                                 CRect&       clearRect,
                                 bool         clearVis) const
    ...
    void CSearchFilter::updatePositions()
    {
        ...
        if (IsWindow(searchImg_.GetSafeHwnd()) &&
            ...
            CRect searchRect, editRect, clearRect;
            getRects(client, searchRect, editRect, clearRect, isClearVis);
            searchImg_.SetWindowPos(NULL,
                                    searchRect.left,
                                    searchRect.top,
                                    searchRect.Width(),
                                    searchRect.Height(),
                                    SWP_NOZORDER);
            edit_->SetWindowPos(NULL,
                                editRect.left,
                                editRect.top,
                                editRect.Width(),
                                editRect.Height(),
