...
// Callback function called by SHBrowseForFolder's browse control after
// initialisation and when selection changes
/*static*/ int __stdcall DirDialog::browseCtrlCallback(HWND   hwnd,
                                                       UINT   uMsg,
                                                       LPARAM lParam,
                                                       LPARAM lpData)
{
    ...
    if (uMsg == BFFM_INITIALIZED) {
        if (!pDirDialogObj->startDirectory_.IsEmpty()) {
            SendMessage(hwnd,
                        BFFM_SETSELECTION,
                        TRUE,
                        (LPARAM)(LPCTSTR)(pDirDialogObj->startDirectory_));
        ...
        if (inter != NULL) {
            ...
            if (SUCCEEDED(inter->QueryInterface(IID_IFolderFilterSite,
                                                (void**)&folderFilterSite))) {
                folderFilterSite->SetFilter(
                  (IFolderFilter*)pDirDialogObj->folderFilter_);
            }
        }
    }
...
DirDialog::DirDialog()
{
    ...
    if (s_basePath_.Compare(L"") ==
        0) // If the base path has not yet been set, set it...
    {
        ...
        while (test.Compare(L"") !=
               0) // If there is a test string to comapre against
        {
            if (test.CompareNoCase(appPath) !=
                0) // Make sure we don't use the application path
            {
                // Save this on the path list for later folder filtering
                ...
                int i = 0;
                while (defaultPath[i] && test[i] &&
                       (toupper(defaultPath[i]) == toupper(test[i]))) {
                    i++;
                }
...
DirDialog::~DirDialog()
{
    // This won't let me delete this for some reason without causing seg faults
    // later (I blame Windows). Yes, this is a very (very) slow leak.
...
BOOL DirDialog::doBrowse(CWnd* pwndParent)
{
    ...
    if (!startDirectory_.IsEmpty()) {
        ...
        if (startDirectory_.Right(1) == "\\")
            startDirectory_ =
              startDirectory_.Left(startDirectory_.GetLength() - 1);
    }
    ...
    if (!fakeRootDirectory_.IsEmpty()) {
        ...
        if (SUCCEEDED(SHGetDesktopFolder(&pDesktopFolder))) {
            ...
            hr = pDesktopFolder->ParseDisplayName(
              NULL,
              NULL,
              fakeRootDirectory_.GetBuffer(MAX_PATH),
              NULL,
              &pidl,
              NULL);
        ...
        // If the base root was not on the same drive then use the "Drives"
        // virtual folder.
    ...
    bInfo.ulFlags =
      BIF_RETURNFSANCESTORS | BIF_RETURNONLYFSDIRS | BIF_NEWDIALOGSTYLE;
    ...
    if (SHGetPathFromIDList(pidl, userSelectedDirectory_.GetBuffer(MAX_PATH)) ==
        FALSE) {
        pMalloc->Free(pidl);
        pMalloc->Release();
        return FALSE;
