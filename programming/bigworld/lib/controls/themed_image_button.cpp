...
namespace controls {
    ...
    /**
     *	Sets the button's image icon. If the button is a push-like checkbox,
     *this icon will be used when the button is unchecked. NOTES:
     *	  - This is NOT a virtual method in the base class, careful with
     *pointers
     ...
     */
    void ThemedImageButton::OnNotifyCustomDraw(NMHDR*   pNotifyStruct,
                                               LRESULT* result)
    {
        BW_GUARD;
        ...
        if (ii.hbmColor != NULL) {
            ...
            GetDIBits(
              hDC, ii.hbmColor, 0, 0, NULL, (LPBITMAPINFO)&bi, DIB_RGB_COLORS);
            cx = bi.bmiHeader.biWidth;
            cy = bi.bmiHeader.biHeight;
        } else {
            // icon has single mask bitmap which is twice as high as icon
            GetDIBits(
              hDC, ii.hbmMask, 0, 0, NULL, (LPBITMAPINFO)&bi, DIB_RGB_COLORS);
            cx = bi.bmiHeader.biWidth;
        ...
        DrawState(hDC,
                  ...
                  cx,
                  cy,
                  (style & WS_DISABLED) != 0 ? (DST_ICON | DSS_DISABLED)
                                             : (DST_ICON | DSS_NORMAL));
