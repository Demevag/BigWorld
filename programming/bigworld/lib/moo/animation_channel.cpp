...
namespace Moo {
    ...
    {
        BW_GUARD;
        if (s_channelTypes_ == NULL)
            s_channelTypes_ = new ChannelTypes();
        (*s_channelTypes_)[type] = cons;
        INFO_MSG("AnimationChannel: registered type %d\n", type);
    }

    /**
     *	This interface doesn't have an operator== and neither do most
     *subclasses. Subclasses that do implement one can use this function to
     *check base-class members for equivalence.
     *
