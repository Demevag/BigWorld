...
namespace Moo {
...
#endif
    {
        ...
        this->qualitySettings_ =
          Moo::makeCallbackGraphicsSetting("TEXTURE_QUALITY",
                                           "Texture Quality",
                                           *this,
        ...
        this->compressionSettings_ = Moo::makeCallbackGraphicsSetting(
          "TEXTURE_COMPRESSION",
          "Texture Compression",
          *this,
        ...
        this->filterSettings_ =
          Moo::makeCallbackGraphicsSetting("TEXTURE_FILTERING",
                                           "Texture Filtering",
                                           *this,
        ...
        this->filterSettings_->addOption("ANISOTROPIC_16X",
                                         "Anisotropic (16x)",
                                         Moo::rc().maxAnisotropy() >= 16);
        this->filterSettings_->addOption(
          "ANISOTROPIC_8X", "Anisotropic (8x)", Moo::rc().maxAnisotropy() >= 8);
        this->filterSettings_->addOption(
          "ANISOTROPIC_4X", "Anisotropic (4x)", Moo::rc().maxAnisotropy() >= 4);
        this->filterSettings_->addOption(
          "ANISOTROPIC_2X", "Anisotropic (2x)", Moo::rc().maxAnisotropy() >= 2);
    ...
    TextureManager::~TextureManager()
    {
        ...
        while (!textures_.empty()) {
            INFO_MSG(
              "[TextureManager] -- Texture not properly deleted on exit: %s\n",
    ...
    /**
     *	Static method to return the default texture used as a fallback for
     *missing textures.
     */
    ...
    uint32 TextureManager::textureMemoryUsed() const
    {
        ...
        while (it != end) {
            ...
            if (id.length() < animated.length() ||
                id.substr(id.length() - animated.length(), animated.length()) !=
                  animated) {
     ...
     *
     *	@param texture the DX::BaseTexture to save out (important has to be
     *32bit colour depth.
...
#define DEFAULT_BMP_FORMAT D3DFMT_DXT1
    ...
    void TextureManager::reloadAllTextures(ProgressListener* pCallBack)
    {
        ...
        while (it != end) {
            ...
            bw_str_copy(
              resourceName, ARRAY_SIZE(resourceName), pTex->resourceID());
            if (BWResource::getExtension(resourceName) == "dds") {
                BWResource::instance().purge(pTex->resourceID());
                prepareResource(
                  resourceName, resourceName, ARRAY_SIZE(resourceName), true);
            }
            it++;
            finishedCount++;
            if (pCallBack) {
                pCallBack->onTextureManagerProgress(
                  ProgressListener::PURGE,
        ...
        while (it != end) {
            ...
            if (pTex->resourceID() == s_notFoundBmp.value()) {
                ...
                pTex->reload(prepareResource(
                  it->first, tempBuffer, ARRAY_SIZE(tempBuffer)));
            } else {
                pTex->reload();
            }
            it++;
     ...
     *  @param mustExist		Attempt to load the not-found texture on
     *failure.
                                       ...
                                       const char*          allocator)
    {
        ...
        if (name.empty()) {
            ...
            ASSET_MSG(
              "Moo::TextureManager::get: empty texture path was requested!\n");
#endif
            return NULL;
        }
...
#define DISABLE_TEXTURING 0
        ...
        if (name.length() > 3) {
            if (allowAnimation) {
                char tempBuffer[BW_MAX_PATH];
                res = this->find(TextureHelpers::copyBaseNameAndExtension(
                  baseName, "texanim", tempBuffer, ARRAY_SIZE(tempBuffer)));
                if (res)
                    return res;
            }
        }
            ...
            const BW::StringRef ddsName =
              name.empty()
                ...
                : TextureHelpers::copyBaseNameAndExtension(
                    baseName, "dds", ddsNameBuffer, ARRAY_SIZE(ddsNameBuffer));
        ...
        result                = this->getUnique(
          name, resourceName, allowAnimation, mustExist, allocator);
     ...
     *failure.
     *  @param loadIfMissing	Load the texture if missing?
     *  @param refresh			Refresh the texture from the data section even
                                       ...
                                       const char*          allocator)
    {
        ...
        // cache.  If in the cache and not refreshing then return the cached
        // value:
        res = find(name);
        if (res != NULL && refresh) {
        ...
        ManagedTexture* manTex = new ManagedTexture(
          data,
          name,
    ...
    void TextureManager::getAnimationSet(const BW::string&     pathWithPrefix,
                                         Moo::BaseTexturePtr** dst,
                                         uint32                number,
                                         ...
                                         bool loadIfMissing /*= true*/)
    {
        ...
        for (uint32 i = 0; i < number; ++i) {
            ...
            name += ".dds";
            textures[i] =
              this->get(name, allowAnimation, mustExist, loadIfMissing);
        }
        (*dst) = textures;
    }
    /**
      ...
      const char*          allocator)
    {
        ...
        // load the texture
        TextureDetailLevelPtr lod =
          this->pDetailLevels_->detailLevel(sanitisedID);

        if (allowAnimation) {
            char                tempBuffer[BW_MAX_PATH];
            const BW::StringRef animTextureName =
              TextureHelpers::copyBaseNameAndExtension(
                removeTextureExtension(sanitisedID),
                "texanim",
                tempBuffer,
                ARRAY_SIZE(tempBuffer));
            DataSectionPtr animSect =
              BWResource::instance().openSection(animTextureName);
            if (animSect.exists()) {
        ...
        if (!res.exists()) {
            ...
            res = streamingManager()->createStreamingTexture(
              lod.get(),
              sanitisedID,
              mustExist ? BaseTexture::FAIL_ATTEMPT_LOAD_PLACEHOLDER
                        : BaseTexture::FAIL_RETURN_NULL,
            ...
            const BW::StringRef internalID =
              resourceID.length() ? TextureHelpers::copyBaseNameAndExtension(
                                      removeTextureExtension(resourceID),
                                      "dds",
                                      internalIDBuffer,
     ...
     *  @param checkExists  if set to false, faile does not need to exist to be
     *a texture file.
     *
     *	@return				True if the resource is a texture, false otherwise.
     */
    bool TextureManager::isTextureFile(const BW::StringRef& resourceID,
      ...
      const BW::StringRef& resourceID)
    {
        ...
        // load it
        char resourceName[BW_MAX_PATH];
        bw_str_copy(resourceName, ARRAY_SIZE(resourceName), resourceID);
        std::for_each(
          resourceName, resourceName + bw_str_len(resourceName), toLower);

        prepareResource(resourceName, resourceName, ARRAY_SIZE(resourceName));

        char systemName[BW_MAX_PATH];
        TextureHelpers::copyBaseNameAndExtension(
          resourceName, "_system", systemName, ARRAY_SIZE(systemName));
        ...
        SmartPointer<SysMemTexture> sysTexture =
          new SysMemTexture(resourceName);
    ...
    void TextureManager::prepareResource(
      const BW::StringRef& resourceName) const
    {
      ...
      bool                 forceConvert /*= false*/) const
    {
        ...
        resource = TextureHelpers::copyBaseNameAndExtension(
          ...
          isCompressed(pDetailLevel) ? "c.dds" : "dds",
          outPtr,
          maxSize);
      ...
      size_t               maxSize) const
    {
        ...
        for (uint32 i = 0; TextureHelpers::textureExts[i] != NULL; ++i) {
            BW::StringRef fileName = TextureHelpers::copyBaseNameAndExtension(
              baseName, TextureHelpers::textureExts[i], outPtr, maxSize);
#if ENABLE_FILE_CASE_CHECKING
        ...
        if (result.rfind("dds") == result.size() - 3)
            return result;
        TextureDetailLevelPtr pDetailLevel =
          this->pDetailLevels_->detailLevel(result);
    ...
    void TextureManager::checkLoadedResourceName(
      const BW::StringRef& requested,
                                     ...
                                     const BW::StringRef& resourceID)
    {
        ...
        const BW::StringRef lowercaseResourceID =
          TextureHelpers::copyNormalisedTextureName(
            ...
            resourceBuffer,
            ARRAY_SIZE(resourceBuffer));

        // transform buffer to lower case
        const size_t len = lowercaseResourceID.size();
        std::transform(
          resourceBuffer, resourceBuffer + len, resourceBuffer, tolower);
    ...
    BaseTexturePtr TextureManager::find(const BW::StringRef& resourceID)
    {
        ...
        const size_t size =
          std::min(resourceID.size(), ARRAY_SIZE(lowercaseResourceID) - 1);
        std::transform(resourceID.begin(),
                       resourceID.begin() + size,
                       lowercaseResourceID,
                       tolower);
        // null terminate
        ...
        TextureMap::iterator it =
          textures_.find(BW::StringRef(lowercaseResourceID, size));
        if (it == textures_.end())
            return NULL;
     ...
     *
     *	Texture quality can vary from 0 (highest) to 2 (lowest). Lowering
     *texture quality is done by skipping the topmost mipmap levels (normally
     *two in the lowest quality setting, one in the medium and none in the
     *highest).
     *
     *	Artists can tweak how each texture responds to the quality setting using
     *the <lodMode> field in the ".texformat" file or global
     *texture_detail_level.xml. The table bellow shows the number of lods
     *skipped according to the texture's lodMode and the current quality
     *setting.
     *
    ...
    void TextureManager::setQualityOption(int optionIndex)
    {
        ...
        while (it != end) {
            BaseTexture*          pTex = it->second;
            TextureDetailLevelPtr lod =
              this->pDetailLevels_->detailLevel(pTex->resourceID());
            int lodMode    = lod->lodMode();
        ...
        int pendingOption = 0;
        if (!Moo::GraphicsSetting::isPending(this->compressionSettings_,
                                             pendingOption)) {
    ...
    void TextureManager::setCompressionOption(int optionIndex)
    {
        ...
        while (it != end) {
            ...
            TextureDetailLevelPtr lod =
              this->pDetailLevels_->detailLevel(resourceName);
            if (lod->isCompressed() &&
                BWResource::getExtension(resourceName) == "dds") {
                char preparedName[BW_MAX_PATH];
                prepareResource(
                  resourceName, preparedName, ARRAY_SIZE(preparedName));
                this->dirtyTextures_.push_back(
                  std::make_pair(resourceName, // old name
        ...
        int pendingOption = 0;
        if (!Moo::GraphicsSetting::isPending(this->qualitySettings_,
                                             pendingOption)) {
            this->reloadDirtyTextures();
        }
    }
    ...
    int TextureManager::configurableMipFilter() const
    {
        ...
        static D3DTEXTUREFILTERTYPE s_mipFilters[] = {
            D3DTEXF_LINEAR, D3DTEXF_LINEAR, D3DTEXF_LINEAR, D3DTEXF_LINEAR,
    ...
    void TextureManager::onResourceModified(
      ...
      ResourceModificationListener::Action modType)
    {
        ...
        if (ext.equals_lower("texanim")) {
            ...
            if (NULL != texture) {
                queueReloadTask(
                  TextureReloadContext(texture), basePath, resourceID);
            }
        } else if (ext.equals_lower("dds")) {
            char           ddsName[BW_MAX_PATH];
            BaseTexturePtr texture =
              this->find(TextureHelpers::copyBaseNameAndExtension(
                baseName, "dds", ddsName, ARRAY_SIZE(ddsName)));
            char preparedName[BW_MAX_PATH];
            if (NULL != texture &&
                resourceID == prepareResource(baseName,
                                              preparedName,
            ...
            queueReloadTask(TextureDetailReloadContext(), basePath, resourceID);
        }
    }
