...
namespace Moo {
    ...
    /**
     * @internal
     *	Used internally to register settings that work by redefining effect
     *files compile time macros. This settings require the client application to
     *be restarted form them to come into effect.
     *
     *	Note: EffectMacroSetting instances need to be static to make sure it
     *gets instantiated before the EffectManager is initialised). For an example
     *on how to use EffectMacroSetting, see romp/sky_light_map.cpp.
     */
    class EffectMacroSetting : public GraphicsSetting
    {
      ...
      public:
        ...
        EffectMacroSetting(const BW::string& label,
                           const BW::string& desc,
                           const BW::string& Macro,
                           SetupFunc         setupFunc,
                           bool              advanced = false);

        void addOption(const BW::string& label,
                       const BW::string& desc,
                       bool              isSupported,
