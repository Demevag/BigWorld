...
namespace Moo {
    ...
    inline bool validateUnitLengthNormal(const Vector3& norm)
    {
        ...
        if (almostZero(lengthSquared)) {
            WARNING_MSG(
              ...
              "(%.2f, %.2f, %.2f) length=%.2f, which will be normalised when "
              "packing.\n",
              norm.x,
              norm.y,
              norm.z,
            ...
            WARNING_MSG("Moo::validateUnitLengthNormal: potential data loss on "
                        "non-unit length normal data: "
                        "(%.2f, %.2f, %.2f) length=%.2f, which will be "
                        "normalised when packing.\n",
                        norm.x,
                        norm.y,
                        norm.z,
                        sqrtf(lengthSquared));
    ...
    //-- 1 sign bit, 4 bit for the int part, and 11 bits for the fractional
    //part.
    ...
    inline bool validatePackingRangeFloatToInt16(float src)
    {
        ...
        if (fabs(src) > 16.0f) {
            WARNING_MSG(
              "Moo::validatePackingRangeFloatToInt16: potential data loss of "
              "texcoord data. "
    ...
    inline Vector3 unpackUByte4Normal888toFloat3(uint32 packed)
    {
        ...
        return Vector3(
          (x - 127) / 127.0f, (y - 127) / 127.0f, (z - 127) / 127.0f);
    }
    ...
    //-- convert normals packing scheme for uint32 from (8, 8, 8, X) to (11, 11,
    //10)
    ...
    //-- convert normals packing scheme for uint32 from (11, 11, 10) to (8, 8,
    //8, X)
    ...
    {
      public:
        inline PackedVector3NormalUByte4_8_8_8() { m_data = 0; }
        ...
        inline PackedVector3NormalUByte4_8_8_8(float* data)
        {
            MF_ASSERT(data != 0);
            m_data =
              packFloat3toUByte4Normal888(Vector3(data[0], data[1], data[2]));
        }
    ...
    class PackedVector2TexcoordInt16_X2
    {
      public:
        ...
        inline PackedVector2TexcoordInt16_X2(const Vector2& in) { *this = in; }
