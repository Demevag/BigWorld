...
namespace Moo {
      ...
      const ViewpointData&            viewpoint)
    {
        ...
        for (size_t index = 0; index < numUsages; ++index) {
            ...
            uint32 numPixels = static_cast<uint32>(
              MATH_PI * usage.bounds_.radius() * usage.bounds_.radius() *
              pow((viewpoint.resolution /
                   (closestDistance * tan(viewpoint.fov / 2) * 2)),
                  2));

            // Calculate how far away from the center of the frustum the object
            // is
            ...
            if ((distToObj - usage.bounds_.radius()) <= 0.0f) {
                ...
                angleToFrustumRadians -=
                  asin(usage.bounds_.radius() / distToObj);
    ...
    void BaseTextureStreamingSceneViewProvider::drawDebugUsages(uint32 color)
    {
        ...
        for (size_t index = 0; index < numUsages; ++index) {
            ...
            Geometrics::instance().wireSphere(
              usage.bounds_.center(), usage.bounds_.radius(), color);
#endif
        }
      ...
      , public BaseTextureStreamingSceneViewProvider
    {
      ...
      private:
        ...
        ModelTextureUsageGroup::Data& fetchUsageForObject(
      ...
      const SceneTypeSystem::RuntimeTypeID& viewTypeID)
    {
        ...
        exposeView<ITextureStreamingSceneViewProvider>(
      ...
      Scene* pNewScene)
    {
        if (pOldScene) {
            ...
            pChangeView->objectsAdded()
              .remove<RuntimeProvider, &RuntimeProvider::onObjectsAdded>(this);
            pChangeView->objectsRemoved()
              .remove<RuntimeProvider, &RuntimeProvider::onObjectsRemoved>(
                this);
            pChangeView->objectsChanged()
              .remove<RuntimeProvider, &RuntimeProvider::onObjectsChanged>(
                this);
        ...
        if (pNewScene) {
            ...
            pChangeView->objectsAdded()
              .add<RuntimeProvider, &RuntimeProvider::onObjectsAdded>(this);
            pChangeView->objectsRemoved()
              .add<RuntimeProvider, &RuntimeProvider::onObjectsRemoved>(this);
            pChangeView->objectsChanged()
              .add<RuntimeProvider, &RuntimeProvider::onObjectsChanged>(this);
        }
    }
      ...
      const ObjectChangeSceneView::ObjectsAddedEventArgs& args)
    {
        ...
        for (size_t index = 0; index < args.numObjects_; ++index) {
            ...
            ModelTextureUsageGroup::Data& usageData =
              fetchUsageForObject(object);
            ModelTextureUsageGroup usageGroup(usageData, usageContext_);

            if (!pTextureStreamingOp_->registerTextureUsage(object,
                                                            usageGroup)) {
    ...
    void TextureStreamingSceneView::onSetScene(Scene* pOldScene,
                                               Scene* pNewScene)
    {
        if (pOldScene) {
    ...
    void TextureStreamingSceneView::unignoreObjectsFrom(
      SceneProvider* pProvider)
    {
