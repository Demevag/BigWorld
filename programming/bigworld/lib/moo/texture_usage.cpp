...
namespace Moo {
    ...
    ModelTextureUsageContext::Handle ModelTextureUsageContext::registerUsage(
    ...
    void ModelTextureUsageContext::unregisterUsage(Handle handle)
    {
        // TODO: MF_ASSERT( textureUsageLock_.isHeldByCurrentThread() );
        textureUsage_.release(handle);
    }

    ModelTextureUsage* ModelTextureUsageContext::getUsage(Handle handle)
    {
    ...
    void ModelTextureUsageContext::getUsages(const ModelTextureUsage** pUsages,
                                             size_t*                   numUsage)
    {
    ...
    //-----------------------------------------------------------------------------
    ModelTextureUsageGroup::ModelTextureUsageGroup(Data&    data,
                                                   Context& provider)
      : data_(data)
      , provider_(provider)
    ...
    void ModelTextureUsageGroup::setWorldScale_FromTransform(
      const Matrix& xform)
    {
                                                 ...
                                                 float        uvDensity)
    {
        ...
        ModelTextureUsageMap::iterator findResult =
          data_.usages_.find(pTexture);
    ...
    float ModelTextureUsageGroup::calcMaxScale(const Matrix& xform)
    {
        ...
        maxSqrLength =
          max(maxSqrLength, xform.applyToUnitAxisVector(1).lengthSquared());
        maxSqrLength =
          max(maxSqrLength, xform.applyToUnitAxisVector(2).lengthSquared());
