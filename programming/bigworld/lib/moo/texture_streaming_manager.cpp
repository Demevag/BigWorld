...
#define TS_WATCHER_GROUP "TextureStreaming/"
        ...
        // Configuration
        MF_WATCH(TS_WATCHER_GROUP "Config_StreamingMode",
                 *this,
        ...
        MF_WATCH(TS_WATCHER_GROUP "Config_PrioritisationMode",
                 *this,
        ...
        MF_WATCH(TS_WATCHER_GROUP "Config_DebugMode",
                 *this,
        ...
        MF_WATCH(
          TS_WATCHER_GROUP "Query_Data_OnDisk",
          *this,
        ...
        MF_WATCH(
          TS_WATCHER_GROUP "Query_MemUsage_StreamingTextures",
          *this,
          &TextureStreamingManager::memoryUsageStreaming,
          "Amount of streaming texture data currently loaded into memory");

        MF_WATCH(TS_WATCHER_GROUP "Query_MemUsage_ManagedTextures",
                 *this,
...
#undef TS_WATCHER_GROUP
        ...
        // until we have some common startup code library available to put it
        // in.
    ...
    TextureStreamingManager::~TextureStreamingManager()
    {
        ...
        SpaceManager& spaceManager = SpaceManager::instance();
        spaceManager.spaceCreated()
          .remove<TextureStreamingManager,
                  &TextureStreamingManager::onSpaceCreated>(this);
        spaceManager.spaceDestroyed()
          .remove<TextureStreamingManager,
                  &TextureStreamingManager::onSpaceDestroyed>(this);
    ...
    {
        return streamingMode_ != MODE_OFF && streamingMode_ != MODE_DISABLE;
    }
    ...
    {
        if (detail->streamable() && canStreamResource(resourceID)) {
            SmartPointer<StreamingTexture> res =
              new StreamingTexture(detail,
                                   resourceID,
                                   notFoundPolicy,
                                   format,
                                   mipSkip,
    ...
    bool TextureStreamingManager::registerTexture(
      ...
      const TextureDetailLevel* detail)
    {
        ...
        bool success =
          entry.currentMip != INVALID_MIP && entry.format != D3DFMT_UNKNOWN;
      ...
      const StreamingTexture* pTex) const
    {
        ...
        return std::max<uint32>(
          pEntry->width >> (pEntry->maxAvailableMip - pEntry->currentMip), 1);
    }
      ...
      const StreamingTexture* pTex) const
    {
        ...
        return std::max<uint32>(
          pEntry->height >> (pEntry->maxAvailableMip - pEntry->currentMip), 1);
    }
    ...
    void TextureStreamingManager::updateTextureMipSkip(StreamingTexture* pTex,
                                                       uint32 mipSkip)
    {
        ...
        pEntry->maxAvailableStreamingMip = max(
          pEntry->maxAvailableStreamingMip, pEntry->minAvailableStreamingMip);
    }
    ...
    void TextureStreamingManager::calculateIdealMips()
    {
        ...
        for (StreamingTextureEntryVector::iterator
               iter = streamingTextures_.begin(),
               end  = streamingTextures_.end();
             iter != end;
             ++iter) {
        ...
        // Iterate over all scene views and calculate ideal mips
        {
            ...
            for (uint32 index = 0; index < activeViewpoints.size(); ++index) {
                ...
                for (auto iter = sceneViews_.begin(); iter != sceneViews_.end();
                     ++iter) {
                    // Iterate through all usages of the textures and update
                    // idealMap entry
    ...
    void TextureStreamingManager::calculateTargetMips()
    {
        ...
        if (isOversubscribed && (streamingMode_ == MODE_CONSERVATIVE ||
                                 streamingMode_ == MODE_ON_DEMAND)) {
            executeOverSubscriptionPolicy(proposedMemoryUsage);
        }

        if (!isOversubscribed && streamingMode_ == MODE_ON_DEMAND) {
      ...
      size_t proposedMemoryUsage)
    {
        ...
        for (uint32 index = 0; index < numTextures; ++index) {
            ...
            if (!canSacrifice) {
                // Don't attempt to prioritise textures that MUST be loaded to
                // max
                continue;
            }
        ...
        while (proposedMemoryUsage > memoryBudget_ && !priorityHeap.empty()) {
            ...
            // proposed mip
            size_t usage = calculateTextureMemoryUsage(entry.width,
                                                       entry.height,
      ...
      const StreamingTextureEntry& entry)
    {
        ...
        size_t topMipMemoryUsage =
          calculateTextureMemoryUsage(entry.width,
                                      entry.height,
        ...
        float cost = static_cast<float>(
          BW::log2ceil(static_cast<uint32>(topMipMemoryUsage)) >> 1);

        float benefit =
          static_cast<float>(entry.idealAvailableMip - entry.targetMip + 1) *
      ...
      const StreamingTextureEntry& entry)
    {
        ...
        if (entry.streamingPriority != 0 &&
            (prioritisationMode_ &
             PM_ENABLE_STREAMING_PRIORITY_DISABLE_SACRIFICE) != 0) {
            return false;
        }
    ...
    void TextureStreamingManager::generateStreamingRequests()
    {
        ...
        for (StreamingTextureEntryVector::iterator
            ...
            if (entry.idealAvailableMip > entry.targetMip) {
                entry.sacrificedDetail =
                  entry.idealAvailableMip - entry.targetMip;
            } else {
    ...
    void TextureStreamingManager::unregisterViewpoint(Viewpoint* pViewpoint)
    {
        ...
        ViewpointList::iterator findResult =
          std::find(viewpoints_.begin(), viewpoints_.end(), pViewpoint);
    ...
    void TextureStreamingManager::forceTextureSkipMips(StreamingTexture* pTex,
                                                       uint32 skipMips)
    {
        SimpleMutexHolder smh(textureLock_);
        ...
        if (skipMips == INVALID_MIP) {
            ...
            pEntry->forcedMip = Math::clamp(pEntry->minAvailableMip,
    ...
    bool TextureStreamingManager::getStreamingRequest(
      StreamingTexture*        pTex,
    ...
    uint32 TextureStreamingManager::calculateMaxMips(uint32 width,
                                                     uint32 height)
    {
      ...
      BaseTexture*              pTex)
    {
        ...
        if (pEntry) {
            ...
            return debugTextures_.getDebugTexture(
              debugSet, pEntry->width, pEntry->height, mipSkip);
        } else {
            // Texture is not a streaming texture, technically shouldn't be
            // here, but if it is, then grab the information from the object
            // itself
    ...
    void TextureStreamingManager::debugMode(uint32 mode)
    {
        if (debugMode_ == mode) {
            return;
        }
    ...
    uint32 TextureStreamingManager::streamingTextureUsageCount() const
    {
        ...
        for (auto iter = sceneViews_.begin(); iter != sceneViews_.end();
             ++iter) {
            // Iterate through all usages of the textures and update idealMap
            // entry
            TextureStreamingSceneView* pView = *iter;
    ...
    size_t TextureStreamingManager::memoryUsageAllTextures() const
    {
        ...
        ResourceCounters::instance().getResourceTypeMemoryUsage(
          ResourceCounters::RT_TEXTURE, &entry);
    ...
    void TextureStreamingManager::tick()
    {
        ...
        if (nextEvaluationTime_ == 0.0 || currentTime >= nextEvaluationTime_) {
    ...
    void TextureStreamingManager::updatePerformanceMetrics()
    {
        ...
        calculateTotalTextureMemoryUsage(&textureDataInMemory_,
                                         &textureDataOnDisk_);
        textureDataPercentInMemory_ = uint32(
          float(textureDataInMemory_) / float(textureDataOnDisk_) * 100.0f);
    }

    void TextureStreamingManager::calculateTotalTextureMemoryUsage(
      size_t* pDataInMemory,
      size_t* pDataOnDisk) const
    {
        ...
        for (StreamingTextureEntryVector::const_iterator
               iter = streamingTextures_.begin(),
               end  = streamingTextures_.end();
             iter != end;
             ++iter) {
            ...
            onDisk += calculateTextureMemoryUsage(entry.width,
                                                  entry.height,
                                                  0,
                                                  entry.maxAvailableMip -
      ...
      TextureFormat format)
    {
        ...
        float topMipMemoryUsage =
          static_cast<float>(DX::surfaceSize(width, height, format));
      ...
      bool                   immediate)
    {
        ...
        // Clamp the new request to the maximum number of mips away from the
        // current
        if (newRequestedMip > entry.currentMip ||
            ...
            if (effectiveCurrentMip == INVALID_MIP) {
                effectiveCurrentMip =
                  max(entry.minAvailableStreamingMip, minMipLevel_);
            }

            newRequestedMip =
              min(newRequestedMip, effectiveCurrentMip + maxMipsPerRequest_);
        }
        ...
        // With any luck any existing streaming tasks will use this new value if
      ...
      const StreamingTextureRequest& requestEntry)
    {
        ...
        if (requestEntry.currentMip == INVALID_MIP) {
            ...
            // Max available streaming mip may have changed due to a texture
            // detail setting change whilst streaming. An example of this being
            // the user changing texture detail level in WorldEditor.
      ...
      const BW::StringRef& filenamePrefix)
    {
        ...
        BW::string filename =
          filenamePrefix.to_string() + "_streamingReport.csv";
        FILE* f = fopen(filename.c_str(), "wb+");

        if (f) {

            fprintf(f,
                    "TotalTextures memory (MB): %" PRIzu "\n",
                    memoryUsageAllTextures() / 1024 / 1024);
            fprintf(f,
                    "ManagedTextures memory (MB): %" PRIzu "\n",
                    memoryUsageManagedTextures() / 1024 / 1024);
            fprintf(f,
                    "StreamingTextures memory (MB): %" PRIzu "\n",
                    memoryUsageStreaming() / 1024 / 1024);
            fprintf(f,
                    "CachedTextures memory (MB): %" PRIzu "\n",
                    memoryUsageGPUCache() / 1024 / 1024);

            fprintf(
              f, "Num Textures in cache: %" PRIzu "\n", numTexturesInCache());
            fprintf(f, "\n\n");
            ...
            for (TextureManager::TextureMap::const_iterator it =
                   textures.begin();
                 it != textures.end();
                 ++it) {
            ...
            fprintf(f, "\nStreaming Textures:\n");
            fprintf(f,
                    "Name;Width;Height;MaxAvMip;MinAvMip;CurrentMip;TargetMip;"
    ...
    // ============================
    TextureStreamingManager::TextureStreamingUsageCollector::
      TextureStreamingUsageCollector(TextureStreamingManager* pManager)
      : pManager_(pManager)
    {
    }

    void TextureStreamingManager::TextureStreamingUsageCollector::
      submitTextureRequirements(BaseTexture* pTexture,
                                uint32       requiredMip,
                                ...
                                float        angleToFrustumRadians)
    {
        ...
        entry.minAngleToFrustum =
          min(entry.minAngleToFrustum, angleToFrustumRadians);
    }
    ...
    UvDensityTriGenFunctor::UvDensityTriGenFunctor(const VertexFormat& format,
                                                   const void*         buffer,
                                                   size_t bufferSize,
    ...
    bool UvDensityTriGenFunctor::operator()(uint32 index1,
                                            uint32 index2,
                                            uint32 index3)
    {
        // index range check
        if (index1 >= posData_.size() || index2 >= posData_.size() ||
            index3 >= posData_.size() || index1 >= uvData_.size() ||
            index2 >= uvData_.size() || index3 >= uvData_.size()) {
            return false;
        }
    ...
    TextureStreamingManager::StreamingTextureLoadTask::
      ...
      QueueType currentQueue)
    {
        ...
        switch (state_) {
            ...
            case STREAM: {
                ...
                MF_ASSERT(requestEntry_.currentMip <
    ...
    void TextureStreamingManager::StreamingTextureLoadTask::doComplete()
    {
        ...
        if (!isImmediateTask) {
            ...
            pManager_->reportStreamingCompleted(
              pTexture_.get(), pTargetTexture_.pComObject(), requestEntry_);
        }
    }

    size_t
    TextureStreamingManager::StreamingTextureLoadTask::doEstimateBandwidth()
    {
    ...
    bool
    TextureStreamingManager::StreamingTextureLoadTask::createTargetTexture()
    {
        StreamingTextureLoader::HeaderInfo header;

        if (requestEntry_.currentMip == INVALID_MIP) {
            // We need to load all the relevant header information out of the
            // file.
            FileStreamerPtr pStream =
            ...
            if (DDSHelper::isDXTFormat(requestEntry_.format)) {
                // Our minimum mip to ever drop back to will be the lowest
                // multiple of 4. But the other mips are allowed to back it up,
                // just we'll never strip away these due to limitations inside
                // copyMipChain
                while ((requestEntry_.width >>
                          (requestEntry_.maxAvailableMip -
                           requestEntry_.minAvailableStreamingMip) < 4 ||
                        requestEntry_.height >>
                          (requestEntry_.maxAvailableMip -
                           requestEntry_.minAvailableStreamingMip) < 4) &&
                       requestEntry_.maxAvailableMip >
                         requestEntry_.minAvailableStreamingMip) {
            ...
            header.valid_  = true;
            header.numMipLevels_ =
              requestEntry_.maxAvailableMip - requestEntry_.minAvailableMip + 1;
        ...
        uint32 totalNumMips =
          targetMipLevel - requestEntry_.minAvailableMip + 1;
    ...
    uint32 TextureStreamingManager::StreamingTextureLoadTask::copySharedMips()
    {
        ...
        if (!origTexture || requestEntry_.currentMip == INVALID_MIP) {
            return 0;
        }
        ...
        if (requestEntry_.requestedMip > requestEntry_.currentMip ||
            ...
            numToCopy =
              requestEntry_.requestedMip - requestEntry_.minAvailableMip + 1;
        }
    ...
    uint32 TextureStreamingManager::StreamingTextureLoadTask::numMipsToLoad()
      const
    {
        uint32 newLoadedMips =
          requestEntry_.requestedMip - requestEntry_.minAvailableMip + 1;
        if (requestEntry_.currentMip != INVALID_MIP) {
            newLoadedMips =
              requestEntry_.requestedMip - requestEntry_.currentMip;
        }
    ...
    bool TextureStreamingManager::StreamingTextureLoadTask::lockSurfaces()
    {
        ...
        for (uint32 index = 0; index < numMipsToLock; ++index) {
            ...
            if (SUCCEEDED(hr)) {
                ...
                // We couldn't lock all the surfaces, so clean up the incomplete
                // operation.
                unlockSurfaces();
                WARNING_MSG("TextureStreamingManager::StreamingTextureLoadTask:"
                            ":lockSurfaces: %s\n",
    ...
    void
    TextureStreamingManager::StreamingTextureEvaluationTask::doBackgroundTask(
      TaskManager& mgr)
    {
      ...
      BaseTexture*              pTex)
    {
        return getDebugTexture(
          debugSet, pTex->width(), pTex->height(), pTex->mipSkip());
    }
      ...
      const DebugTextureSet_Key& debugRequest)
    {
        ...
        static const uint32 size_colorArray[] = {
            0xFF808080, 0xFF000000, 0xFFFFFFFF, 0xFF0000FF, 0xFF00FFFF,
        ...
        static const uint32 size_colorArrayLen = ARRAY_SIZE(size_colorArray);
        ...
        switch (debugRequest.debugSet) {
            case DEBUG_TS_CURRENTWORKINGSET: {
                width =
                  std::max(debugRequest.width >> debugRequest.skipLevels, 1u);
                height =
                  std::max(debugRequest.height >> debugRequest.skipLevels, 1u);
            } break;
            ...
            case DEBUG_TS_MISSINGAVAILABLEDETAIL: {
                ...
                if (debugRequest.skipLevels != 0) {
                    width = std::max(
                      (debugRequest.width << 1) >> debugRequest.skipLevels, 1u);
                    height = std::max((debugRequest.height << 1) >>
            ...
            case DEBUG_TS_TEXTURE_SIZE: {
                ...
                maxMipLevel =
                  Math::clamp(minValue, maxMipLevel, maxValue) - minValue;
                uint32 skipColorIndex =
                  min(maxMipLevel, size_colorArrayLen - 1);
            ...
            case DEBUG_TS_CURRENT_TEXTURE_SIZE: {
                ...
                uint32 maxMipLevel =
                  BW::log2ceil(debugRequest.width) - debugRequest.skipLevels;
                maxMipLevel =
                  Math::clamp(minValue, maxMipLevel, maxValue) - minValue;
                uint32 skipColorIndex =
                  min(maxMipLevel, size_colorArrayLen - 1);
