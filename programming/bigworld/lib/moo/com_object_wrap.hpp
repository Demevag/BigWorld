...
/**
 *	COM object wrapper class template used for safe access, automatic resource
 *release and memory accounting.
 */
...
struct ComObjectWrap
{
    ...
    ComObjectWrap& operator=(ComObjectWrap<OTHER> const& other)
    {
        ComObjectWrap obj;
        if (SUCCEEDED(
              other->QueryInterface(__uuidof(COMOBJECTTYPE), (LPVOID*)&obj)))
    ...
    operator void*() const { return comObject_; }
    bool operator!() const { return !comObject_; }
