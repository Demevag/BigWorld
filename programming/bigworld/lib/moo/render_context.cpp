...
{
    return ((dm1.Width == dm2.Width) && (dm1.Height == dm2.Height) &&
            (dm1.Format == dm2.Format) && (dm1.RefreshRate == dm2.RefreshRate));
}
...
namespace Moo {
        ...
        if (device_) {
            ...
            for (uint32 i = 0; i < 256; i++) {
                ...
                ramp.red[i] = ramp.green[i] = ramp.blue[i] =
                  WORD((/*1.f -*/ powf(f, 1.f / gammaCorrection_)) * 65535.f);
            }
    ...
    namespace {
        ...
        IDirect3D9Ex* createD3DEx()
        {
            ...
            if (hDll) {
                ...
                LPDIRECT3DCREATE9EX func = (LPDIRECT3DCREATE9EX)GetProcAddress(
                  hDll, "Direct3DCreate9Ex");
    ...
    {
        BW_GUARD;
        IF_NOT_MF_ASSERT_DEV(isValid_ == false &&
                             "RenderContext already initialised")
        {
            return true;
        }
        ...
        deviceExSettings_ =
          Moo::makeCallbackGraphicsSetting("DEVICE_EX",
                                           "Direct3D 9Ex Device",
                                           *this,
        ...
        FXAASettings_ =
          Moo::makeCallbackGraphicsSetting("FXAA_PROCESSING",
                                           "FXAA Processing",
                                           *this,
        ...
        // Check option
        // pass this in from BWMain since GraphicsSettings aren't init until
        // later
        ...
        if (d3d_) {
            // Reduce the reference count, because d3ds_.pComobject( ) increases
            // it.

            // Iterate through the adapters.
            for (uint32 adapterIndex = 0;
                 adapterIndex < d3d_->GetAdapterCount();
                 adapterIndex++) {
                DeviceInfo deviceInfo;
                deviceInfo.windowed_  = false;
                deviceInfo.adapterID_ = adapterIndex;
                if (D3D_OK == d3d_->GetAdapterIdentifier(
                                adapterIndex, 0, &deviceInfo.identifier_)) {
                    if (D3D_OK == d3d_->GetDeviceCaps(adapterIndex,
                                                      D3DDEVTYPE_HAL,
                                                      &deviceInfo.caps_)) {
                        const uint32 VENDOR_ID_ATI    = 0x1002;
                        const uint32 VENDOR_ID_NVIDIA = 0x10de;

                        // set up compatibilty flags for special rendering
                        // codepaths
                        deviceInfo.compatibilityFlags_ = 0;
                        if (strstr(deviceInfo.identifier_.Description,
                                   "GeForce4 MX") != NULL) {
                            // this card does not support D3DLOCK_NOOVERWRITE in
                            // the way flora uses it
                            deviceInfo.compatibilityFlags_ |=
                              COMPATIBILITYFLAG_NOOVERWRITE;
                            NOTICE_MSG("GeForce4 MX compatibility selected\n");
                        }

                        if (deviceInfo.identifier_.VendorId ==
                            VENDOR_ID_NVIDIA) {
                            // this card does not support D3DLOCK_NOOVERWRITE in
                            // the way flora uses it
                            deviceInfo.compatibilityFlags_ |=
                              COMPATIBILITYFLAG_NVIDIA;
                        ...
                        //-- compatibility for deferred shading pipeline.
                        {
                            ...
                            supported &=
                              (deviceInfo.caps_.VertexShaderVersion >= 0x300);
                            supported &=
                              (deviceInfo.caps_.PixelShaderVersion >= 0x300);
                            supported &=
                              (deviceInfo.caps_.NumSimultaneousRTs >= 3);
                            supported &=
                              (deviceInfo.caps_.PrimitiveMiscCaps &
                               D3DPMISCCAPS_INDEPENDENTWRITEMASKS) != 0;
                            // supported &=
                            // SUCCEEDED(d3d_->CheckDeviceFormat(deviceInfo.adapterID_,
                            // D3DDEVTYPE_HAL, D3DFMT_A8R8G8B8, 0,
                            // D3DRTYPE_TEXTURE, D3DFMT_A16B16G16R16F));

                            if (supported) {
                                deviceInfo.compatibilityFlags_ |=
                                  COMPATIBILITYFLAG_DEFERRED_SHADING;
                                NOTICE_MSG(
                                  "Deferred shading pipeline compatibile\n");
                            }
                        }

                        D3DFORMAT formats[] = {
                            D3DFMT_X8R8G8B8,    D3DFMT_A8R8G8B8,
                            D3DFMT_A2B10G10R10, D3DFMT_X1R5G5B5,
                        ...
                        for (uint32 fi = 0; fi < nFormats; fi++) {
                            ...
                            for (uint32 displayModeIndex = 0;
                                 displayModeIndex < d3d_->GetAdapterModeCount(
                                                      adapterIndex, format);
                                 displayModeIndex++) {
                                D3DDISPLAYMODE mode;
                                if (D3D_OK ==
                                    d3d_->EnumAdapterModes(adapterIndex,
                                                           format,
                                                           displayModeIndex,
                                                           &mode)) {
                                    mode.RefreshRate = 0;
                                    if (std::find(
                                          deviceInfo.displayModes_.begin(),
                                          deviceInfo.displayModes_.end(),
                                          mode) ==
                                        deviceInfo.displayModes_.end()) {
                                        if (mode.Width >= 640 &&
                                            mode.Height >= 480) {
                                            deviceInfo.displayModes_.push_back(
                                              mode);
                                        }
                                    }
                                }
                        ...
                        deviceInfo.windowed_ =
                          true; //( deviceInfo.caps_.Caps2 &
                                // D3DCAPS2_CANRENDERWINDOWED ) != 0;
                    }

                    if (deviceInfo.displayModes_.size() != 0 ||
                        deviceInfo.windowed_ == true) {

                        if (deviceInfo.windowed_) {
                            d3d_->GetAdapterDisplayMode(
                              deviceInfo.adapterID_,
                              &deviceInfo.windowedDisplayMode_);
                        }

                        std::sort(deviceInfo.displayModes_.begin(),
                                  deviceInfo.displayModes_.end());
            ...
            if (devices_.size()) {
                ...
                ERROR_MSG("Moo::RenderContext::RenderContext: No hardware "
                          "rasterisation devices found.\n");
            }
        } else {
            ERROR_MSG("Moo::RenderContext::RenderContext: Unable to create "
                      "Directx interface. Is Directx9c installed?\n");
        }
    ...
    void RenderContext::createSecondSurface()
    {
        ...
        secondRenderTargetTexture_ =
          this->createTexture((UINT)this->screenWidth(),
        ...
        if (secondRenderTargetTexture_.hasComObject()) {
            HRESULT hr = (secondRenderTargetTexture_->GetSurfaceLevel(
              0, &renderTarget_[1]));
            if (FAILED(hr)) {
     ...
     *  @param windowed tells the rendercontext whether to use windowed mode or
     * not.
     *  @param testCooperative if true, tests the device's cooperative level
     * before changing the mode. The mode is not changed if the device is lost.
     *  @param backBufferWidthOverride  Width to set the back buffer to. 0
     *                                  indicates same width as the visible
     * display.
     *
     *
     *  @return true if the operation was successful, false otherwise.
     */
    bool RenderContext::changeMode(uint32 modeIndex,
                                   bool   windowed,
                                   ...
                                   uint32 backBufferWidthOverride)
    {
        ...
        // we don't want to get the current window size if we are currently
        // fullscreen
        //  and are going to windowed mode, as all we will be getting will be
        //  the rectangle of the fullscreen window
        ...
        bool result = this->changeModePriv(
          modeIndex, windowed, testCooperative, backBufferWidthOverride);

        if (result && goingWindowMode) {
            // If going back to windowed mode, resize it.
            SetWindowPos(windowHandle_,
                         HWND_NOTOPMOST,
                         windowedRect_.left,
                         windowedRect_.top,
    ...
    /**
     *  This method resets the device to the exact same current modeIndex and
     * windowed mode. Use it whenever the underlying window is resized to adjust
    ...
    bool RenderContext::resetDevice()
    {
        ...
        //  store the current window size, so it can be used to check to see if
        //  the window has changed in rendercontext::checkdevice() to do a reset
        //  there but only if we are in windowed mode, not fullscreen.
        if (windowed_ == true)
            GetWindowRect(windowHandle_, &windowedRect_);
     ...
     *  mode on the current device. It differs from changeMode by not reseting
     * the
     *
     *  @param modeIndex is the index of the displaymode to choose.
     *  @param windowed tells the rendercontext whether to use windowed mode or
     ...
     */
    bool RenderContext::changeModePriv(uint32 modeIndex,
                                       bool   windowed,
                                       bool   testCooperative,
                                       uint32 backBufferWidthOverride)
    {
        ...
        if (!usingD3DDeviceEx_) {
            ...
            while (hr == D3DERR_DEVICELOST) {
                ...
                if (FAILED(hr) && hr != D3DERR_DEVICELOST) {
                    printErrorMsg_(
                      hr,
                      "Moo::RenderContext::changeMode: Unable to reset device");
                    return false;
                }
                Sleep(100);
            }
            ...
            if (windowed) {
                ...
                dispModeDesc.RefreshRate =
                  presentParameters_.FullScreen_RefreshRateInHz;
        ...
        INFO_MSG("RenderContext::changeMode - available texture memory after "
                 "reset: %dkb\n",
                 availTexMem / 1024);
        ...
        if (windowed_) {
            ...
            this->windowedSize_ =
              Vector2(float(clientRect.right - clientRect.left),
    ...
    void RenderContext::updateDeviceInfo()
    {
        ...
        HRESULT hr;
        if (SUCCEEDED(
              hr = device_->CreateOffscreenPlainSurface(backBufferDesc_.Width,
                                                        backBufferDesc_.Height,
                                                        D3DFMT_A8R8G8B8,
                                                        D3DPOOL_SCRATCH,
                                                        &surf,
                                                        NULL))) {
        ...
        DEBUG_MSG("Moo: Changed mode: %d %d\n",
                  backBufferDesc_.Width,
                  backBufferDesc_.Height);
    }

    /*
     *  This method sets up the presentation parameters, for a reset or a
     * createdevice call.
     */
    void RenderContext::fillPresentationParameters(void)
    {
        ...
        if (windowed_) {
            ...
            presentParameters_.BackBufferHeight = static_cast<uint32>(
              backBufferWidthOverride_ * (screenHeight() / screenWidth()));
            presentParameters_.BackBufferFormat =
              devices_[deviceIndex_].windowedDisplayMode_.Format;
            presentParameters_.PresentationInterval =
              waitForVBL_ ? D3DPRESENT_INTERVAL_ONE
                          : D3DPRESENT_INTERVAL_IMMEDIATE;
        } else {
            D3DDISPLAYMODE mode =
              devices_[deviceIndex_].displayModes_[modeIndex_];
            ...
            presentParameters_.PresentationInterval =
              waitForVBL_ ? D3DPRESENT_INTERVAL_DEFAULT
                          : D3DPRESENT_INTERVAL_IMMEDIATE;
        }

        if (presentParameters_.BackBufferFormat == D3DFMT_X8R8G8B8) {
            if (SUCCEEDED(d3d_->CheckDeviceType(D3DADAPTER_DEFAULT,
                                                D3DDEVTYPE_HAL,
                                                D3DFMT_X8R8G8B8,
                                                D3DFMT_A8R8G8B8,
                                                windowed_))) {
                ...
                INFO_MSG(
                  "This adapter does not support A8R8G8B8 back buffers\n");
            }
        }
                                                      ...
                                                      bool& stencilAvailable)
    {
        ...
        if (colourFormat == D3DFMT_R8G8B8 || colourFormat == D3DFMT_X8R8G8B8 ||
            colourFormat == D3DFMT_A8R8G8B8) {
            if (!stencilWanted) {
                if (SUCCEEDED(d3d_->CheckDeviceFormat(adapter,
                                                      deviceType_,
                                                      format,
                                                      D3DUSAGE_DEPTHSTENCIL,
                                                      D3DRTYPE_SURFACE,
                                                      D3DFMT_D32)) &&
                    SUCCEEDED(d3d_->CheckDepthStencilMatch(
                      adapter, deviceType_, format, colourFormat, D3DFMT_D32)))
                    return D3DFMT_D32;
                if (SUCCEEDED(d3d_->CheckDeviceFormat(adapter,
                                                      deviceType_,
                                                      format,
                                                      ...
                                                      D3DRTYPE_SURFACE,
                                                      D3DFMT_D24X8)) &&
                    SUCCEEDED(d3d_->CheckDepthStencilMatch(adapter,
                                                           deviceType_,
                                                           format,
                                                           colourFormat,
                                                           D3DFMT_D24X8)))
        ...
        if (!stencilWanted) {
            if (SUCCEEDED(d3d_->CheckDeviceFormat(adapter,
                                                  ...
                                                  D3DFMT_D16)) &&
                SUCCEEDED(d3d_->CheckDepthStencilMatch(
                  adapter, deviceType_, format, colourFormat, D3DFMT_D16)))
                return D3DFMT_D16;
            if (SUCCEEDED(d3d_->CheckDeviceFormat(adapter,
                                                  deviceType_,
                                                  format,
                                                  ...
                                                  D3DFMT_D15S1)) &&
                SUCCEEDED(d3d_->CheckDepthStencilMatch(
                  adapter, deviceType_, format, colourFormat, D3DFMT_D15S1)))
                return D3DFMT_D15S1;
            if (SUCCEEDED(d3d_->CheckDeviceFormat(adapter,
                                                  deviceType_,
                                                  format,
                                                  ...
                                                  D3DFMT_D24X8)) &&
                SUCCEEDED(d3d_->CheckDepthStencilMatch(
                  adapter, deviceType_, format, colourFormat, D3DFMT_D24X8)))
                return D3DFMT_D24X8;
            if (SUCCEEDED(d3d_->CheckDeviceFormat(adapter,
                                                  deviceType_,
                                                  format,
            ...
            if (SUCCEEDED(d3d_->CheckDeviceFormat(adapter,
                                                  ...
                                                  D3DFMT_D24S8)) &&
                SUCCEEDED(d3d_->CheckDepthStencilMatch(
                  adapter, deviceType_, format, colourFormat, D3DFMT_D24S8)))
                return D3DFMT_D24S8;
            if (SUCCEEDED(d3d_->CheckDeviceFormat(adapter,
                                                  deviceType_,
                                                  format,
                                                  ...
                                                  D3DFMT_D24X4S4)) &&
                SUCCEEDED(d3d_->CheckDepthStencilMatch(
                  adapter, deviceType_, format, colourFormat, D3DFMT_D24X4S4)))
                return D3DFMT_D24X4S4;
        } else {
            if (SUCCEEDED(d3d_->CheckDeviceFormat(adapter,
                                                  deviceType_,
    ...
    void RenderContext::updateProjectionMatrix(bool detectAspectRatio)
    {
        ...
        if (detectAspectRatio && !renderTargetStack_.nStackItems()) {
            if (!windowed_) {
                ...
                camera_.aspectRatio(float(backBufferDesc_.Width) /
                                    float(backBufferDesc_.Height));
            }
        }

        zoomFactor_ = camera_.fov() / DEG_TO_RAD(60);

        projection_.perspectiveProjection(camera_.fov(),
                                          camera_.aspectRatio(),
                                          camera_.nearPlane(),
                                          camera_.farPlane());
    }
    ...
    bool RenderContext::checkDevice(bool* reset /*=NULL*/)
    {
        ...
        switch (hr) {
            ...
            default:
                INFO_MSG(
                  "RenderContext::checkDevice - UNHANDLED D3D RESULT : %ld\n",
                  hr);
                break;
        }
        s_lost = !result;
    ...
    bool RenderContext::checkDeviceYield(bool* reset /*= NULL*/)
    {
        ...
        while (!(result = this->checkDevice(reset))) {
            ...
            if (::PeekMessage(&msg, 0, 0, 0, PM_REMOVE)) {
                if (!::IsWindow(windowHandle_) || (msg.message == WM_QUIT) ||
                    (msg.message == WM_KEYDOWN && msg.wParam == VK_F4 &&
                     ::GetKeyState(VK_MENU))) {
                    result = false;
                    break;
                }
     ...
     *  @return true if the operation was successful.
     */
    bool RenderContext::createDevice(HWND           hWnd,
                                     uint32         deviceIndex,
                                     uint32         modeIndex,
                                     bool           windowed,
                                     bool           requireStencil,
    ...
    )
    {
        ...
        if (deviceIndex > devices_.size()) {
            CRITICAL_MSG("Moo::RenderContext::createDevice: Tried to select a "
        ...
        if (device_) {
            // If the device is the same as the current device, just change the
            // display mode, else release the device.
        ...
        if (devices_[deviceIndex].caps_.DevCaps &
            D3DDEVCAPS_HWTRANSFORMANDLIGHT) {
            // Treat hardware with no pixel or vertex shader support as fixed
            // function cards.
            if (uint16(devices_[deviceIndex].caps_.VertexShaderVersion) >
                  0x100 &&
                uint16(devices_[deviceIndex].caps_.PixelShaderVersion) >
                  0x100) {
        ...
        while (retries > 0) {
            if (usingD3DDeviceEx_) {
                ...
                if (windowed) {
                    ...
                    dispModeDesc.RefreshRate =
                      presentParameters_.FullScreen_RefreshRateInHz;
                    dispModeDesc.ScanLineOrdering =
                      D3DSCANLINEORDERING_PROGRESSIVE;
                }

                hr = d3dEx_->CreateDeviceEx(
                  devices_[deviceIndex_].adapterID_,
                  devType,
                  windowHandle_,
            ...
            if (SUCCEEDED(hr)) {
                ...
                WARNING_MSG("Moo::RenderContext::createDevice: Unable to "
                            "create device, retrying (%d retries left)\n",
                            retries);
                // Wait a couple of seconds before reattempting, to give a
                // chance to other D3D apps to free there resources and/or quit
                // properly
                Sleep(2000);
                retries--;
            }
        ...
        if (FAILED(hr)) {
            printErrorMsg_(
              hr, "Moo::RenderContext::createDevice: Unable to create device");
            return false;
        }
        ...
        maxSimTextures_ =
          uint16(devices_[deviceIndex_].caps_.MaxSimultaneousTextures);
        ...
        mrtSupported_ = (psVersion_ >= 0x300) &&
                        ...
                        (devices_[deviceIndex_].caps_.PrimitiveMiscCaps &
                         D3DPMISCCAPS_INDEPENDENTWRITEMASKS);

        setGammaCorrection();
        UINT availTexMem = device_->GetAvailableTextureMem();
        INFO_MSG("RenderContext::createDevice - available texture memory after "
                 "create: %dkb\n",
                 availTexMem / 1024);
        ...
        TextureManager::instance()->getDetailsManager()->setAssetClient(
          pAssetClient);
#endif
        ...
        if (!assetProcessingOnly_) {
            ...
            MF_WATCH(
              ...
              *this,
              ...
              "Resize the back buffer. Only works in windowed mode. 0 matches "
              "window size. Aspect ratio is maintained.");

            MF_WATCH("Render/numPreloadResources",
                     s_numPreloadResources,
                     Watcher::WT_READ_ONLY,
                     "The number of managed pool resources not yet preloaded");

            MF_WATCH("Render/Draw Calls",
                     lastFrameProfilingData_.nDrawcalls_,
                     Watcher::WT_READ_ONLY,
                     "DrawCalls of the previous frame.");
            MF_WATCH("Render/Primitives",
                     lastFrameProfilingData_.nPrimitives_,
                     Watcher::WT_READ_ONLY,
                     "Primitives drawn in the previous frame.");

            MF_WATCH("Render/numTexturesInReuseList",
                     textureReuseCache(),
                     &TextureReuseCache::size,
                     "How many textures currently cached for future re-use.");

            // If we're not using a d3dex device and we are in fullscreen, we do
            // an extra reset here since the device gets lost because of display
            // uniqueness change
    ...
    /**
     *   This method initialises all the render states and TS states in the
     * cache.
     */
    ...
    /**
     * This function overrides the dimensions of the back buffer when in
     * windowed mode using a given width.
     *
     * Given values are first clamped to the maximum width supported by the
     * device. The height is set so as to maintain the aspect ratio of the front
     * buffer. A value of 0 means that this feature is disabled and the back
     * buffer will have the same dimensions as the front. This functionality can
     * be used to produce screenshots at resolutions greater than that that can
    ...
    void RenderContext::backBufferWidthOverride(uint32 backBufferWidthOverride)
    {
        ...
        if (backBufferWidthOverride != this->backBufferWidthOverride_)
            this->changeMode(this->modeIndex(),
                             this->windowed(),
                             true,
     ...
     * Takes a screenshot
     *
     * @param fileExt - the extension to save the screenshot as, this can be
     * "bmp", "jpg", "tga", "png" or "dds".
     *
     * @param fileName - the root name of the file to save out
     *
     * @param autoNumber - is this is true then try to append a unique
     *                     identifying number to the shot name(e.g.
     * shot_666.bmp)
     */
      ...
      bool              autoNumber /*= true*/)
    {
        ...
        D3DXIMAGE_FILEFORMAT d3dFromat =
          TextureCompressor::getD3DImageFileFormat(fileName + "." + ext,
                                                   D3DXIFF_DDS);
        ...
        if (autoNumber) {
            ...
            // go through filenames until we find one that has not been created
            // yet.
            bool foundEmptyFile = false;
            while (!foundEmptyFile) {
                // Create the filename
                BW::ostringstream findName;
                findName << fileName << "_" << std::setfill('0') << std::setw(3)
        ...
        if (FAILED(D3DXLoadSurfaceFromSurface(systemMemorySurface.pComObject(),
                                              NULL,
                                              NULL,
        ...
        if (FAILED(D3DXSaveSurfaceToFile(wname.c_str(),
                                         d3dFromat,
    ...
    /**
     * This method copies the current back buffer into and internal surface,
     * then returns a pointer to the internal surface.
     */
    DX::Surface* RenderContext::getScreenCopy()
    {
        ...
        if (screenCopySurface_.hasComObject()) {
            ComObjectWrap<DX::Surface> backBuffer;
            if (SUCCEEDED(device_->GetBackBuffer(
                  0, 0, D3DBACKBUFFER_TYPE_MONO, &backBuffer))) {
                if (SUCCEEDED(D3DXLoadSurfaceFromSurface(
                      screenCopySurface_.pComObject(),
                      NULL,
                      NULL,
                      backBuffer.pComObject(),
                      NULL,
                    ...
                    ERROR_MSG(
                      "RenderContext::getScreenCopy - unable to get copy\n");
                }
            } else {
                ERROR_MSG(
                  "RenderContext::getScreenCopy - unable to get backbuffer\n");
            }
        } else {
            ERROR_MSG("RenderContext::getScreenCopy - no screencopy surface "
                      "available\n");
        }
        return ret;
    }
    ...
    bool RenderContext::supportsTextureFormat(D3DFORMAT fmt) const
    {
        ...
        if (device_) {
            if (SUCCEEDED(
                  d3d_->CheckDeviceFormat(devices_[deviceIndex_].adapterID_,
                                          deviceType_,
                                          adapterFormat(),
                                          0,
                                          D3DRTYPE_TEXTURE,
    ...
    {
        return false; //(multisamplingType_ != D3DMULTISAMPLE_NONE &&
                      // multisamplingQuality_ >= 0);
    }
    ...
    bool RenderContext::RenderTargetStack::push(RenderContext* rc)
    {
        ...
        if (!ts.hasComObject()) {
            CRITICAL_MSG("Moo::RenderContext::RenderTargetStack::push: "
                         "couldn't get current render target\n");
            return false;
        }
        ...
        if (FAILED(hr)) {
            CRITICAL_MSG("Moo::RenderContext::RenderTargetStack::push: "
                         "couldn't get current depth/stencil buffer\n");
            return false;
        }
    ...
    bool RenderContext::RenderTargetStack::pop(RenderContext* rc)
    {
        ...
        if (!stackItems_.size()) {
            CRITICAL_MSG("Tried to pop the render target when no render target "
                         "was there.\n");
            return false;
        }
        ...
        if (FAILED(hr) || FAILED(hr2)) {
            CRITICAL_MSG("Moo::RenderContext::RenderTargetStack::pop: couldn't "
                         "set rendertarget/depth buffer\n");
            return false;
        }
    ...
    /**
     *  This method returns the rectangle that is the intersection of the
     * near-plane with the view frustum.
     *
     *  @param corner       The position of the bottom-left corner of the
     * rectangle.
     *  @param xAxis        The length of the horizontal edges.
     *  @param yAxis        The length of the vertical edges.
     *
     *  @note   The invView matrix must be correct before this method is called.
     * You may need to call updateViewTransforms.
     *
     *  @see invView
    ...
    /**
     *  Restores the windows cursor state, hidding or showing it depending on
     * the value passed.
     *
    ...
    static BW::vector<std::pair<D3DRENDERSTATETYPE, uint32>>
      s_renderStateStackDX;
    static BW::vector<std::pair<D3DRENDERSTATETYPE, uint32>>
      s_renderStateStackMain;

    void RenderContext::pushRenderState(D3DRENDERSTATETYPE state)
    {
        s_renderStateStackMain.push_back(
          std::make_pair(state, rsCache_[state].currentValue));
    }
    ...
    {
        setRenderState(s_renderStateStackMain.back().first,
                       s_renderStateStackMain.back().second);
     ...
     *  It is D3D recommended behaviour to unbind all resource states after
     * every frame.
     *
     */
    void RenderContext::clearBindings()
    {
        ...
        setIndices(NULL);
        for (uint32 i = 0;
             (i < devices_[deviceIndex_].caps_.MaxStreams) || (i < 1);
             i++) {
        ...
        for (uint32 i = 0;
             i < devices_[deviceIndex_].caps_.MaxSimultaneousTextures;
             i++) {
            setTexture(i, NULL);
        }
 ...
 *//*-------------------------------------------------------------------*/
    ...
    OcclusionQuery* RenderContext::createOcclusionQuery()
    {
        ...
        HRESULT          res =
          device_->CreateQuery(D3DQUERYTYPE_OCCLUSION, &pOcclusionQuery);
     ...
     */
    bool RenderContext::getQueryResult(int&            visiblePixels,
                                       ...
                                       bool            wait)
    {
        ...
        if (wait) {
            ...
            while (S_FALSE ==
                   (hres = pOcclusionQuery->GetData(
                      &visiblePixels, sizeof(DWORD), D3DGETDATA_FLUSH))) {
                if (Moo::rc().device()->TestCooperativeLevel() ==
                    D3DERR_DEVICELOST) {
                    visiblePixels = 1;
                    DEBUG_MSG("Device lost when waiting for occlusion data, "
                              "default visiblePixels is used\n");
                    return true;
                }
            }
            return S_OK == hres;
        } else {
            return S_OK == pOcclusionQuery->GetData(
                             &visiblePixels, sizeof(DWORD), D3DGETDATA_FLUSH);
        }
    }

    namespace {
        char* poolStrings[] = { "D3DPOOL_DEFAULT",
                                "D3DPOOL_MANAGED",
                                "D3DPOOL_SYSTEMMEM",
                                "D3DPOOL_SCRATCH",
                                "INVALID_POOL" };
         ...
         * Output a string error message for some DirectX errors.
         */
        void printCreateTextureErrorMsg(HRESULT              hr,
                                        const BW::string&    msg,
                                        UINT                 Width,
                                        UINT                 Height,
                                        UINT                 Levels,
                                        ...
                                        const BW::StringRef& allocator)
        {
            ...
            switch (hr) {
                case D3DERR_DRIVERINTERNALERROR:
                    CRITICAL_MSG(
                      "%s size %dx%dx%d format %s in pool %s for %s %s\n",
                      msg.c_str(),
                      Width,
                      Height,
                      Levels,
     ...
     */
    ComObjectWrap<DX::Texture> RenderContext::createTexture(
      UINT                 Width,
      UINT                 Height,
      UINT                 Levels,
      DWORD                Usage,
      ...
      const BW::StringRef& allocator)
    {
        ...
        // calculate number of mip levels for reuse call if "Levels" argument is
        // 0
        ...
        tex = this->getTextureFromReuseList(
          Width, Height, reuseNumLevels, Usage, Format, Pool);
        if (tex.hasComObject()) {
            return tex;
        }
        // nothing in reuse list, have to create a new texture
        HRESULT hr = device_->CreateTexture(
          Width, Height, Levels, Usage, Format, Pool, &tex, NULL);
        if (SUCCEEDED(hr)) {
            ...
            if (hr == E_OUTOFMEMORY && Pool != D3DPOOL_DEFAULT) {
                ...
                // Try to remove managed resources from video memory and
            ...
            printCreateTextureErrorMsg(
              hr,
              "RenderContext::createTexture - could not create texture map",
              Width,
              Height,
              Levels,
              Format,
     ...
     */
    ComObjectWrap<DX::CubeTexture> RenderContext::createCubeTexture(
      UINT                 edgeLength,
      UINT                 levels,
      DWORD                usage,
      D3DFORMAT            format,
      ...
      const BW::StringRef& allocator)
    {
        ...
        HRESULT hr = device_->CreateCubeTexture(
          edgeLength, levels, usage, format, pool, &tex, NULL);
        if (SUCCEEDED(hr)) {
#if ENABLE_RESOURCE_COUNTERS
            tex.addAlloc(allocator);
#endif
            ...
            printCreateTextureErrorMsg(
              hr,
              "RenderContext::createCubeTexture - could not create texture map",
              edgeLength,
              edgeLength,
              levels,
              format,
      ...
      const BW::StringRef& allocator)
    {
        ...
        HRESULT hr = D3DXCreateTextureFromFileInMemoryEx(device_,
                                                         pSrcData,
                                                         SrcDataSize,
                                                         Width,
                                                         Height,
        ...
        if (SUCCEEDED(hr)) {
            ...
            printCreateTextureErrorMsg(
              hr,
              "RenderContext::createTextureFromFileInMemoryEx - could not "
              "create texture map",
              Width,
              Height,
              MipLevels,
     ...
     */
    ComObjectWrap<DX::Texture> RenderContext::createTextureFromFileEx(
      LPCTSTR              pSrcFile,
      UINT                 Width,
      UINT                 Height,
      UINT                 MipLevels,
      ...
      const BW::StringRef& allocator)
    {
        ...
        if (SUCCEEDED(hr)) {
            ...
            printCreateTextureErrorMsg(hr,
                                       "RenderContext::createTextureFromFileEx "
                                       "- could not create texture map",
                                       Width,
                                       Height,
                                       MipLevels,
                                       D3DFMT_UNKNOWN,
     ...
     */
    ComObjectWrap<DX::CubeTexture>
    RenderContext::createCubeTextureFromFileInMemoryEx(
      LPCVOID              pSrcData,
      UINT                 SrcDataSize,
      UINT                 Size,
      UINT                 MipLevels,
      ...
      const BW::StringRef& allocator)
    {
        ...
        HRESULT hr = D3DXCreateCubeTextureFromFileInMemoryEx(device_,
                                                             pSrcData,
                                                             SrcDataSize,
                                                             Size,
                                                             MipLevels,
        ...
        if (SUCCEEDED(hr)) {
            ...
            printCreateTextureErrorMsg(
              hr,
              "RenderContext::createCubeTextureFromFileInMemoryEx - could not "
              "create texture map",
              Size,
              Size,
              MipLevels,
      ...
      const BW::StringRef& allocator)
    {
        ...
        HRESULT hr = device_->CreateOffscreenPlainSurface(
          Width, Height, Format, Pool, &surface, NULL);
        if (SUCCEEDED(hr)) {
#if ENABLE_RESOURCE_COUNTERS
            ...
            WARNING_MSG("RenderContext::createOffscreenPlainSurface - could "
                        "not create memory surface "
                        "size %dx%d.\n ( error %lx:%s )\n",
                        Width,
                        Height,
                        hr,
    ...
    RenderContext& rc()
    {
        ...
        MF_ASSERT_DEBUG(g_RC && g_RC->isValid() &&
                        "RenderContext not initialised.");
        return *g_RC;
    }
    ...
    {
        BW_GUARD;
        if (!preloadResourceList_.empty() && preloadResourceMutex_.grabTry()) {
            ...
            // has been reached or the list is empty, continue, leave any
            // resource that has not been preloaded in the list.
            while ((timestamp() - beginStamp) < timeLimit &&
                ...
                // only remaining reference to the resource and we can release
                // it) Since AddRef and Release are the only ways of getting the
                // reference count of an IUnknown object we use these methods.
                pResource->AddRef();
    ...
    void RenderContext::addPreloadResource(IDirect3DResource9* preloadResource)
    {
        ...
        if (enablePreloadResources_ && !usingD3DDeviceEx_) {
            ...
            if (preloadResourceList_.size() > 2000) {
                WARNING_MSG(
                  "RenderContext::addPreloadResource: Too many preload "
                  "resources, "
                  "you may want to call Moo::rc().preloadDeviceResources to "
                  "flush them.\n");
            }
        }
    }
     ...
     */
    void RenderContext::putTextureToReuseList(
      ...
      D3DPOOL   pool)
    {
        ...
        return textureReuseCache_.requestTexture(
          width, height, nLevels, usage, fmt, pool);
    }
