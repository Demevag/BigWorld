...
class DynamicDecalManager : public Singleton<DynamicDecalManager>
{
    struct DecalGroup
    {
        float      m_lifeTime;
        BW::string m_name;
        DecalGroup(const BW::string& name, float lifeTime)
          : m_name(name)
          , m_lifeTime(lifeTime)
        {
        }
    ...
    int32 cur_idx;
    float decalObjectImpactSizeCoefficient; // size coefficient of decal on
                                            // impact into complex geometry
                                            // objects to reduce artifacts
  public:
    DynamicDecalManager();
    ...
    static void addDecal(float             lifeTime,
                         const Vector3&    start,
                         const Vector3&    end,
                         const Vector2&    size,
                         float             yaw,
