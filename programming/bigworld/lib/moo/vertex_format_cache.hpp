...
namespace Moo {
    ...
    class VertexFormatCache
    {
      public:
         ...
         *	@return	Valid pointer if found or loaded successfully, else NULL
         *ptr.
         */
         ...
         *	@return	Valid pointer if formatName added successfully, else NULL
         *ptr.
         */
        static const VertexFormat* add(const BW::StringRef& formatName,
                                       const VertexFormat&  format);

        /// This function returns the filepath for the corresponding vertex
        /// format
         ...
         * return TF (loading it if necessary). Otherwise, if no target format
         *is specified for the target, or the target format failed to load, will
        ...
        /// Get the targeted vertex format object corresponding to a source
        /// vertex type
        template <class SourceVertexType>
        static const VertexFormat* getTarget(const BW::StringRef& targetKey,
                                             bool fallbackToSource = true);

        /// Get the targeted vertex format name corresponding to a source vertex
        /// type Return empty StringRef if no target found
        template <class SourceVertexType>
        static const BW::StringRef getTargetName(const BW::StringRef& targetKey,
                                                 bool fallbackToSource = true);
