...
//--------------------------------------------------------------------------------------------------
template <typename T>
typename BSPTree2D<T>::Node* BSPTree2D<T>::Node::insert(uint width,
                                                        uint height,
                                                        T*   value)
{
    ...
    if (m_childs[0] && m_childs[1]) {
        ...
        //-- if we're just right, accept because image fits perfectly in node's
        //rect.
        if (roomWidth == width && roomHeight == height) {
        ...
        if (dw > dh) {
            m_childs[0]->m_rect = Rect2D(m_rect.m_left,
                                         m_rect.m_top,
                                         m_rect.m_left + width - 1,
                                         m_rect.m_bottom);

            m_childs[1]->m_rect = Rect2D(m_rect.m_left + width,
                                         m_rect.m_top,
                                         m_rect.m_right,
                                         m_rect.m_bottom);
        } else {
            m_childs[0]->m_rect = Rect2D(m_rect.m_left,
                                         m_rect.m_top,
                                         m_rect.m_right,
                                         m_rect.m_top + height - 1);

            m_childs[1]->m_rect = Rect2D(m_rect.m_left,
                                         m_rect.m_top + height,
                                         m_rect.m_right,
                                         m_rect.m_bottom);
        }
