...
MEMTRACKER_DECLARE(StreamedAnimation_CacheRecord,
                   "StreamedAnimation_CacheRecord",
                   0);
...
namespace Moo {
      ...
      : blocks_(NULL)
    {
        ...
        RESOURCE_COUNTER_ADD(
          ResourceCounters::DescriptionPool("Animation/Streamed Animations",
    ...
    void StreamedAnimation::add(StreamedAnimationChannel* pChannel)
    {
        ...
        for (uint i = 0; i < numBlocks_; i++) {
            ...
            BinaryPtr newData =
              new BinaryBlock(NULL,
                              sb.size_ + newsize,
                              "Animation/Streamed Animation Add",
                              NULL);
     ...
     */
    void StreamedAnimation::load(BinaryFile&        bf,
                                 float              totalTime,
                                 StreamedDataCache* pSource,
                                 uint32             streamedOffset)
    {
        BW_GUARD;
        // store our source
        pSource_ = pSource;
        ...
        numBlocks_ =
          (uint32(ceilf(totalTime + 1.f)) + (STREAM_BLOCK_SIZE - 1)) /
          STREAM_BLOCK_SIZE;
    ...
    uint32 StreamedAnimation::dam(uint32 streamedOffset)
    {
        ...
        for (uint i = 0; i < numBlocks_; i++) {
            ...
            if (sb.data_ != NULL)
                pSource_->immsave(
                  sb.offset_, sb.size_, sb.data_->pData_->data());
    ...
    void StreamedAnimation::touch(float time)
    {
        ...
        if (sb.reader_ == NULL) { // not reading in
     ...
     *	Constructor
     */
    StreamedAnimation::CacheRecord::CacheRecord(StreamedAnimation* owner,
                                                StreamedBlock*     block)
      : owner_(owner)
      , next_(NULL)
      , prev_(NULL)
    {
        ...
        pData_ = new BinaryBlock(
          NULL, block->size_, "Animation/Streamed Animation Cache", NULL);
    ...
    StreamedAnimation::CacheRecord::~CacheRecord()
    {
        ...
        if (owner_ != NULL) {
            ...
            for (; i < owner_->numBlocks_; i++)
                if ((block = &owner_->blocks_[i])->data_ == this)
                    break;
    ...
    /**
     *	The maximum number bytes that will be used for streamed animation
     *blocks. Accessible through the watcher
     *'Memory/StreamedBlocks_CacheMaxSize' Setting this value too low can cause
     *the animation system to be starved of data resulting in animation glitches
     *and excessive disk usage.
     */
    size_t StreamedAnimation::s_cacheMaxSize_ = 1 << 20;

    THREADLOCAL(StreamedDataCache*)
    StreamedAnimation::s_loadingAnimCache_ = NULL;
    THREADLOCAL(StreamedDataCache*)
...
#define calcNumWords_PART(kname, kchar, ktype)                                 \
    ...
    if (!(kchar##Header_.rosterOffset_ & 0x80))                                \
        kname##KeyData++;                                                      \
     ...
     */
    void StreamedAnimationChannel::result(float           ftime,
                                          BlendTransform& out) const
    {
        BW_GUARD;
...
#define SAC_result_PART(kname, kheader, ktype, interpolator)                   \
    ...
    kname##BotKey =                                                            \
      (!(durHere & 0x80)) ? kname##KeyData - 1 : &kname##Fallback_;            \
                                                                               \
    ...
    kname##TopKey = (!(durHere & 0x80)) ? kname##KeyData : &kname##Fallback_;  \
                                                                               \
    ktype kname##Out;                                                          \
    interpolator(&kname##Out, kname##BotKey, kname##TopKey, param / fdur)
            ...
            SAC_result_PART(
              rotation, cb.rHeader_, Quaternion, XPQuaternionSlerp);
    ...
    {
        ERROR_MSG(
          "StreamedAnimationChanne::postCombine - method not supported");
    }

    namespace {
        /*
         *	This class helps with creating the blocks used by the streamed
         *animations
         */
        ...
        class BlockHelper
        {
          public:
             ...
             */
            BlockHelper(const KeyFrames& frames,
                        uint32           numBlocks,
              ...
              , lastKeyFallback_()
            {
                ...
                // and that the keys that are there cover the full range of the
                // anim
                uint32 totalTime = STREAM_BLOCK_SIZE * numBlocks;

                // If there are actual keyframes, the first frame is used as the
                // fallback Otherwise we add the fallback key as the first frame
                if (frames_.size()) {
                ...
                // If there is no key at the start of the animation we extend
                // the first key to the start
                if (frames_.front().first > 0.f) {
                    frames_.insert(frames_.begin(),
                                   std::make_pair(0.f, frames_.front().second));
                }

                // If there is no key at the end of the animation we extend the
                // first last to the start
                if (frames_.back().first < float(totalTime)) {
                    frames_.push_back(
                      std::make_pair(float(totalTime), frames_.back().second));
                }
            }
                          ...
                          const KeyType& endKey)
            {
                ...
                // If the lastOffset does not fit in a uint8, we use the
                // supplied endkey for the block as our last key
                ...
                // If the firstOffset does not fit in a uint8, we use the
                // supplied beginkey for the block as our first key
                ...
                // If the first key is the same as the fallback set up the
                // roster to use the default key for the first frame Otherwise
                // add the key at the front of the values list
            ...
            uint8 lastKeyFallback()
            {
                return uint8(lastKeyFallback_ ? 0x80 : 0);
            }
            ...
            uint32 valuesSize()
            {
                return static_cast<uint32>(sizeof(KeyType) * values_.size());
            }
     ...
     */
    void StreamedAnimationChannel::mirror(
      const InterpolatedAnimationChannel& iac)
    {
        ...
        BlockHelper<Vector3> scaleHelper(
          iac.scaleKeys_, numBlocks_, Vector3(1.f, 1.f, 1.f));
        BlockHelper<Vector3> positionHelper(
          iac.positionKeys_, numBlocks_, Vector3(0.f, 0.f, 0.f));
        BlockHelper<Quaternion> rotationHelper(
          iac.rotationKeys_, numBlocks_, Quaternion(0.f, 0.f, 0.f, 1.f));
        ...
        for (uint i = 0; i < numBlocks_; i++) {
            ...
            positionHelper.setBlock(
              i, btBefore.translation(), btAfter.translation());
            rotationHelper.setBlock(i, btBefore.rotation(), btAfter.rotation());

            // Calculate the roster size padded out to 4 bytes
            uint32 rostersSize =
              (scaleHelper.rosterSize() + positionHelper.rosterSize() +
               rotationHelper.rosterSize() + 3) &
              ~3L;
            ...
            // The block is the ChannelBlockHeader plus the rosters plus the
            // data
            ChannelBlock* pcb = (ChannelBlock*)bw_malloc(
              sizeof(ChannelBlock) + rostersSize + valuesSize);
            ...
            pcb->pHeader_.rosterOffset_ =
              uint8(pRoster - pData) | positionHelper.lastKeyFallback();
            pRoster = positionHelper.copyRoster(pRoster);

            pcb->rHeader_.rosterOffset_ =
              uint8(pRoster - pData) | rotationHelper.lastKeyFallback();
            ...
            pcb->pHeader_.keyDataOffset_ =
              uint8(pcb->sHeader_.keyDataOffset_ +
                    scaleHelper.valuesSize() / sizeof(uint32));
            pcb->rHeader_.keyDataOffset_ =
              uint8(pcb->pHeader_.keyDataOffset_ +
    ...
    StreamedAnimationChannel::TypeRegisterer StreamedAnimationChannel::s_rego_(
      STREAMED_ANIMATION_CHANNEL,
      New);
