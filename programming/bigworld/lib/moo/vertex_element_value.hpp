...
namespace Moo { namespace VertexElement {
    // -----------------------------------------------------------------------------
    // Declare some stuff from VertexConversions, so we do not need to include
    // it
    // -----------------------------------------------------------------------------
    ...
    /**
     * This class represents the storage and access of a vertex element value,
     * and the components each vertex element consists of (e.g. x,y,z as floats,
     * or r,g,b,a as color). It is essentially a fixed array with type and size
     * known at compile time.
     *
     * However, it provides component-wise named and array access (both compile
     * time, and run time), and has various functions for casting between
     * different Component types and sizes, for supporting conversion between
     * different formats and storage/packing representations.
     */
    /// Value type that provides a component-based writeable view of arbitrary
    /// data.
    template <class ComponentType, unsigned int ComponentCount>
    struct ElementValue
      : public MultiComponentValue<ComponentType, ComponentCount>
    {
        ...
        /// Copies bytes from passed in objects to itself, clamping the copy
        /// range to not exceed the size of target component data
        template <class OtherType>
        void copyBytesFrom(const OtherType& srcData)
        {
            BW_STATIC_ASSERT(component_count > 0 && component_size > 0,
                             ElementValue_zero_sized_instantiation_is_invalid);
            unsigned int bytesToCopy =
              std::min(size_bytes, (unsigned int)sizeof(srcData));
        ...
        value_type& get()
        {
            BW_STATIC_ASSERT(ComponentIndex < component_count,
                             ElementValue_index_out_of_bounds);
        ...
        void fillEndComponents(unsigned int count, int fillVal)
        {
            ...
            memset(&components_[component_count - count],
                   fillVal,
                           ...
                           SourceElementVal::value_type>)
    {
        ...
        unsigned int componentsToCopy =
          std::min(std::min(DestElementVal::component_count,
                            SourceElementVal::component_count),
        ...
        for (unsigned int i = 0; i < componentsToCopy; ++i) {
            ...
            componentConversionFunc(
              &(*dst)[i], &(*src)[i], DestElementVal::component_size);
        }
    ...
    template <template <class, unsigned int> class ElementVal,
              class ElementType,
              unsigned int DstComponentCount,
              unsigned int SrcComponentCount>
    static void copyComponents(
      ElementVal<ElementType, DstComponentCount>*       dst,
      const ElementVal<ElementType, SrcComponentCount>* src,
      size_t                                            copyCount)
    {
        ...
        unsigned int componentsToCopy =
          std::min(std::min(DstComponentCount, SrcComponentCount),
                   static_cast<unsigned int>(copyCount));
    ...
    static bool validateComponents(const SourceElementVal* src,
