...
#define READREQMAX 32
    ...
    void __cdecl read_start(void* arg)
    {
        ...
        while (1) {
            ...
            size_t numRead =
              fread(rr.pData->cdata(), 1, rr.pData->len(), rr.file);
      ...
      , deleteOnClose_(false)
    {
        ...
        file_ =
          BWResource::instance().fileSystem()->posixFileOpen(fileName_, "r+b");
        if (createIfAbsent) {
            // if it didn't work, try to create the file and open it
            if (!file_)
                file_ = BWResource::instance().fileSystem()->posixFileOpen(
                  fileName_, "w+b");
        }
      ...
      uint64            minModified)
    {
        ...
        if (found != directory_.end()) {
            if (minVersion == 0 || minVersion <= found->second.version_) {
                if (minModified == 0 ||
                    minModified <= found->second.modifiedTime_) {
                    return &found->second;
                }
            }
        }
                                           ...
                                           uint64            modified)
    {
        ...
        uint32 dataEnd =
          (lastEntry_ != NULL)
            ? lastEntry_->fileOffset_ +
                ((lastEntry_->preloadSize_ + lastEntry_->streamSize_ + 3) & ~3L)
            : 0;
        ...
        if (ei == NULL) {
            ...
            lastEntry_ =
              &directory_.insert(std::make_pair(name, entryInfo)).first->second;
        } else {
            ...
            while (dataOffset < dataEnd) {
                uint32 thisLen =
                  std::min(dataEnd - dataOffset, SHUFFLE_BUFFER_SIZE);
            ...
            for (EntryInfos::iterator it = directory_.begin();
                ...
                if (aei->fileOffset_ > ei->fileOffset_)
                    aei->fileOffset_ -= oldLen;
            }
     ...
     */
    bool StreamedDataCache::immsave(uint32      offset,
                                    uint32      size,
                                    const void* data)
    {
        BW_GUARD;
     ...
     */
    StreamedDataCache::TrackerPtr StreamedDataCache::save(uint32      offset,
                                                          uint32      size,
                                                          const void* data)
    {
        BW_GUARD;
          ...
          public:
              ...
              , fileDataOffset_(0)
            {
                ...
                if (fileSize_ > sizeof(uint32)) {
                    // Check if there is enough room in the file for the
                    // directory
                    if (pCache->preload(fileSize - sizeof(uint32),
                                        sizeof(uint32),
                                        &directorySize_) &&
                        directorySize_ < (fileSize - sizeof(uint32))) {
                        // load up the directory
                        directoryStart_ =
                          fileSize - sizeof(uint32) - directorySize_;
                        pData_ = new char[directorySize_];
                        if (pCache->preload(
                              directoryStart_, directorySize_, pData_)) {
                            valid_ = true;
                        }
                    }
                }
            ...
            /*
             *	This method loads up the entryinfo for the next entry in the
             *file
            ...
            bool nextEntry()
            {
                ...
                if (load(entryDataLen) && load(entryPreloadLen) &&
                    load(entryVersion) && load(entryModified) &&
            ...
            bool load(KeyType& data)
            {
                ...
                if (readOffset_ + KEY_SIZE <= directorySize_) {
                    ...
                    ERROR_MSG(
                      "EntryInfoLoader::load - Failed to read %d bytes\n",
                      KEY_SIZE);
                }
                return ret;
            }
            ...
            bool loadString(BW::string& data)
            {
                ...
                if (!ret) {
                    ERROR_MSG(
                      "EntryInfoLoader::load - Failed to read string\n");
                }
                return ret;
            }
    ...
    void StreamedDataCache::loadSelf()
    {
        ...
        while (loader.nextEntry()) {
            EntryInfos::iterator it =
              directory_.insert(std::make_pair(loader.name(), loader.info()))
                .first;
            lastEntry_ = &it->second;
        }

        // If the entry data is not valid, either the data
        if (!loader.valid() || !validateEntryData(loader.dataSize())) {
            directory_.clear();
            lastEntry_ = NULL;
        }
    }
    ...
    void StreamedDataCache::saveSelf()
    {
        ...
        for (it = revEntries.begin(); it != revEntries.end(); it = nit) {
            nit = it;
            nit++;
            ...
            dirFile.append((char*)&zero,
                           (0 - entryNameLen) & (sizeof(zero) - 1));
        }
        ...
        this->immsave(
          dataEnd, static_cast<uint32>(dirFile.length()), dirFile.c_str());
        fflush(file_);

        // Make sure we are truncating the file to the size of the data +
        // directory Seek to the right position
        MF_VERIFY(fseek(file_,
                        static_cast<long>(dataEnd + dirFile.length()),
                        SEEK_SET) == 0);

        // Truncate the file, we use win32 functions as there is no posix
        // equivalent
    ...
    namespace {
        ...
        class ValidateSingleEntryData
        {
          public:
            ...
            {
                BW_GUARD;
                // A big number that signals that there is something wrong in
                // the animation
                ...
                if (pCache->preload(
                      info.fileOffset_, info.preloadSize_, pData_)) {
                    ...
                    uint32     ncb = 0;
                    if (read(totalTime) && readString(identifier) &&
                        readString(internalIdentifier) && read(ncb)) {
                        if (ncb < MAX_ANIM_CHANNELS && !_isnan(totalTime) &&
                            totalTime > 0.f) {
                            valid_ = true;
                        }
                    }
    ...
    bool StreamedDataCache::validateEntryData(uint32 headerStart)
    {
        ...
        while (it != directory_.end()) {
            ...
            uint32           dataEnd =
              ((ei.fileOffset_ + ei.preloadSize_ + ei.streamSize_ + 3) & ~3L);

            // If the data goes past the header report an error
            if (dataEnd > headerStart) {
                ERROR_MSG(
                  "StreamedDataCache::validateEntryData: "
        ...
        if (headerStart != 0 && !hitEnd) {
            ERROR_MSG(
              "StreamedDataCache::validateEntryData - data in %s "
