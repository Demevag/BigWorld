...
namespace Moo {
     ...
     *	Constructor
     */
    PulseLight::PulseLight(const Colour&                 colour,
      ...
      , colourAnimFrame_(0)
    {
        ...
        pAnimation_ =
          Moo::AnimationManager::instance().get(animationName, pNode);
    ...
    void PulseLight::tick(float dTime)
    {
        if (pAnimation_) {
            ...
            for (uint32 i = 0; i < pAnimation_->nChannelBinders(); i++) {
                const Moo::AnimationChannelPtr pChannel =
        ...
        colourAnimFrame_ =
          fmodf(colourAnimFrame_, colourAnimation_.getTotalTime());
