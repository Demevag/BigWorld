...
namespace Moo {
    ...
    //-- Fog is divided into two part. First depends on camera and the distance
    //to the camera,
    //-- second one describes fog which hides space bounds by smoothly change
    //it's density from the
    //-- inner box to the outer box.
    //-- watch the structure memory alignment because we send this structure as
    //is to
