  ...
  bool  readCompressionFactors)
{
    ...
    InterpolatedAnimationChannelPtr result(
      new Moo::InterpolatedAnimationChannel(false, readCompressionFactors));
    ...
    for (int key = 3; key < numKeys; ++key) {
        ...
        switch (key % 3) {
            ...
            case 2:
                result->addRotationKey(time,
                                       Quaternion(vec3, random(0.0f, 1.0f)));
                break;
        }
    }

    MF_ASSERT(numKeys == (result->nScaleKeys() + result->nPositionKeys() +
                          result->nRotationKeys()));
    MF_ASSERT(result->valid());
 ...
 */
bool animationChannelsEquivalent(const InterpolatedAnimationChannelPtr& lhs,
...
Moo::NodePtr createRandomNode(int seed, const BW::string& identifier)
{
    ...
    result->blend(random(0, 10) * 16, random(0.0f, 1.0f));
 ...
 *		parents).
 */
bool compareNodeHierarchies(const Moo::Node& lhs,
                            const Moo::Node& rhs,
                            bool             expectRootNodes)
{
    BW_GUARD;
    ...
    for (uint32 lhsChildIndex = 0; lhsChildIndex < lhs.nChildren();
         ++lhsChildIndex) {
        ...
        for (uint32 rhsChildIndex = 0; rhsChildIndex < rhs.nChildren();
             ++rhsChildIndex) {
...
bool compareNodeHierarchies(const Moo::NodePtr& lhs,
                            const Moo::NodePtr& rhs,
                            bool                expectRootNodes)
{
    BW_GUARD;
