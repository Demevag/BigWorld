...
TEST_F(AnimTestsFixture, Interpolated_Animation_Channel_IO)
{
    ...
    Moo::InterpolatedAnimationChannel::inhibitCompression(!USE_COMPRESSION);
    ...
    for (unsigned int seed = 0; seed < NUM_TEST_SETS; ++seed) {
        ...
        const int   numKeys      = 3 + random(0, MAX_ANIM_KEY_FRAMES - 3);
        const float animDuration = random(0.0f, MAX_ANIM_DURATION);
        ...
        MF_ASSERT(Moo::InterpolatedAnimationChannel::s_saveConverter_ == NULL);
        ...
        Moo::InterpolatedAnimationChannel restoredChannel(false,
                                                          !USE_COMPRESSION);
        testFile.rewind();
