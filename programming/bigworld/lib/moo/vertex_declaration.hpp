...
namespace Moo {
    ...
    /**
     * This class handles the vertex declarations used by moo. Vertex
     * declarations encapsulate the way VertexFormats are sent to the graphics
     * device. Vertex formats are stored on disk (in res\\shaders\\formats ), a
     * subset of which can be supported by this class as VertexDeclarations,
     * depending on the graphics target device.
     *
    ...
    {
      public:
        DX::VertexDeclaration* declaration() const
        {
            return pDecl_.pComObject();
        }

        const BW::string&   name() const { return name_; }
        const VertexFormat& format() const { return format_; }

        static VertexDeclaration* combine(VertexDeclaration* orig,
                                          VertexDeclaration* extra);
