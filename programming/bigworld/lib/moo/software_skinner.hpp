...
namespace Moo {
    ...
    class RigidSkinVertex
    {
      public:
        ...
        void output(VertexXYZNUV&        vertex,
        ...
        void output(VertexXYZNUVTBPC&    vertex,
                    const NodePtrVector& nodes,
                    const Vector3&       positionOverride)
        {
            const Vector3 v3Zero(0, 0, 0);
        ...
        void output(float&         depth,
                    const Vector4* partialWorldView,
                    const Vector3& positionOverride)
        {
            const Vector4& wTrans = partialWorldView[index];
            depth =
              (((Vector3*)&wTrans)->dotProduct(positionOverride) + wTrans.w);
        }
    ...
    class SoftSkinVertex
    {
      public:
        ...
        void output(float& depth, const Vector4* partialWorldView)
        {
            ...
            depth = (((Vector3*)&wTrans1)->dotProduct(position) + wTrans1.w) *
                      weight1 +
                    (((Vector3*)&wTrans2)->dotProduct(position) + wTrans2.w) *
                      weight2 +
                    (((Vector3*)&wTrans3)->dotProduct(position) + wTrans3.w) *
                      weight3;
        }
        void output(float&         depth,
                    const Vector4* partialWorldView,
                    const Vector3& positionOverride)
        {
            ...
            const Vector4& wTrans3 = partialWorldView[index3];
            depth =
              (((Vector3*)&wTrans1)->dotProduct(positionOverride) + wTrans1.w) *
                weight1 +
              (((Vector3*)&wTrans2)->dotProduct(positionOverride) + wTrans2.w) *
                weight2 +
              (((Vector3*)&wTrans3)->dotProduct(positionOverride) + wTrans3.w) *
                weight3;
        }
    ...
    class BaseSoftwareSkinner : public ReferenceCount
    {
      public:
        ...
        virtual void transformVertices(
          VertexXYZNUV*        pDestVertices,
          int                  startVertex,
          int                  nVertices,
          const NodePtrVector& nodes,
          const Vector3*       pPositionOverrides = NULL) = 0;
        virtual void transformVertices(
          VertexXYZNUVTBPC*    pDestVertices,
          int                  startVertex,
          int                  nVertices,
          const NodePtrVector& nodes,
          const Vector3*       pPositionOverrides = NULL) = 0;
    ...
    class SoftwareSkinner : public BaseSoftwareSkinner
    {
      public:
        ...
        void transformVertices(VertexXYZNUV*        pDestVertices,
                               int                  startVertex,
                               int                  nVertices,
                               const NodePtrVector& nodes,
                               const Vector3*       pPositionOverrides)
        {
            IF_NOT_MF_ASSERT_DEV(unsigned(startVertex + nVertices) <=
                                 vertices_.size())
            {
                return;
            }
        ...
        void transformVertices(VertexXYZNUVTBPC*    pDestVertices,
                               int                  startVertex,
                               int                  nVertices,
                               const NodePtrVector& nodes,
                               const Vector3*       pPositionOverrides)
                               ...
                               const Vector3* pPositionOverrides)
        {
            ...
            if (pPositionOverrides) {
                while (it != end) {
                    (it++)->output(
                      *(pVert++), pTransforms, *(pPositionOverrides++));
                }
            } else {
                while (it != end) {
                                  ...
                                  const Vector3* pPositionOverrides = NULL)
        {
            ...
            if (pPositionOverrides) {
                while (it != end) {
                    (it++)->output(
                      *(pDepth++), partialWVP, *(pPositionOverrides++));
                }
            } else {
                while (it != end) {
