...
//-- Texture atlas may collect power of two textures in much bigger texture to
//prevent memory
//-- fragmentation with manual DX API creation of these textures and to minimize
//state changes during
//-- rendering. You may dynamically add and delete sub-textures to\from the
//texture atlas. The atlas
//-- texture located in the un-managed pool so it doesn't waste any additional
//system memory but
//-- TextureAtlas has to take a look at each loaded textures and reload them and
//recreate atlas every
...
class TextureAtlas : public Moo::BaseTexture
{
  ...
  public:
    ...
    {
        SubTextureData(const BW::string&          name,
                       const Moo::BaseTexturePtr& source,
                       const SubTexturePtr&       subTexture)
          : m_name(name)
          , m_source(source)
  ...
  public:
    ...
    //-- Note:		returns an existing sub-texture and automatically increment
    //refs counter or a
    //--			new one if desired sub-texture doesn't exist.
    //-- Note:		If <source> is a valid pointer then it's used as a source
    //for uploading into
    //--			the atlas. Operation performs in the caller thread and async
    //flag is ignored.
    //-- Warning:	Be sure that pair <name, source> always unique because if
    //not you will have
    //--			the same texture in the atlas but with different names.
    SubTexturePtr subTexture(const BW::string&          name,
                             const Moo::BaseTexturePtr& source = NULL,
                             bool                       async  = false);
    ...
    //-- have to be called by the owner whenever create/destroy unmanaged
    //resources needed.
    void createUnmanagedObjects();
    void deleteUnmanagedObjects();
    bool recreateForD3DExDevice() const;

    //-- copy desired texture data into the atlas. Copying is performed on the
    //main thread.
    bool copySubTexture(const SubTexturePtr&       subTexture,
