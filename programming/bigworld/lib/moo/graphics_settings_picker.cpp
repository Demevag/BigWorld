...
static AutoConfigString s_graphicsSettingsConfigName(
  "system/graphicsSettingsPresets");
static AutoConfigString s_autoConfigSettingsFileName(
  "system/autoConfigSettings");
...
class VendorDeviceIDPair
{
  public:
    ...
    {
        BW_GUARD;
        return other.vendorID_ == vendorID_ && other.deviceID_ == deviceID_;
    }
...
class DeviceDescriptionMatcher
{
  public:
    /*
     *	Constructor loads up a number of substrings from the provided
     *datasection
     */
...
namespace Moo {
    ...
    class SettingsGroup : public ReferenceCount
    {
      public:
        ...
        bool init(DataSectionPtr pSection)
        {
            ...
            pSection->openSections("VendorDeviceIDMatch",
                                   vendorDeviceIDMatchSections);
            for (uint32 i = 0; i < vendorDeviceIDMatchSections.size(); ++i) {
                vendorDeviceIDMatches_.push_back(
                  VendorDeviceIDPair(vendorDeviceIDMatchSections[i]));
            }

            // Read our device description matches
            BW::vector<DataSectionPtr> deviceDescriptionMatchSections;
            pSection->openSections("DeviceDescriptionMatch",
                                   deviceDescriptionMatchSections);

            for (uint32 i = 0; i < deviceDescriptionMatchSections.size(); ++i) {
                deviceDescriptionMatches_.push_back(
                  DeviceDescriptionMatcher(deviceDescriptionMatchSections[i]));
            }
            ...
            return (!guidMatches_.empty() || !vendorDeviceIDMatches_.empty() ||
                    !deviceDescriptionMatches_.empty() || isDefault_) &&
                   !settings_.empty();
        }
         ...
         *		if the device description matches the return value is 2, if
         *there is no match but this the default settings group, the return
         *value is one, otherwise the return value is 0
         */
        uint32 check(const D3DADAPTER_IDENTIFIER9& adapterID) const
        {
            BW_GUARD;
            // First check if we have a GUID match, the GUID match is highest
            // priority match
            const UniqueID& deviceId =
              (const UniqueID&)adapterID.DeviceIdentifier;
            ...
            VendorDeviceIDPair currentVendorDeviceID(adapterID.VendorId,
                                                     adapterID.DeviceId);
            ...
            // Check if the device description matches, this is weakest actual
            // match We make sure the description is lower case before checking
            BW::string deviceDescription = adapterID.Description;
            std::transform(deviceDescription.begin(),
    ...
    /**
     *	This method inits the graphics settings picker, it loads the graphics
     *settings config from the .xml file so that we can match it with the
    ...
    {
        BW_GUARD;
        DataSectionPtr pConfig = BWResource::instance().openSection(
          s_graphicsSettingsConfigName.value());
        if (pConfig.exists()) {
     ...
     *	@return true if settings for the current device were found, false
            if not
     */
    bool GraphicsSettingsPicker::getSettings(
      const D3DADAPTER_IDENTIFIER9& adapterID,
    ...
    void GraphicsSettingsAutoDetector::initConfig()
    {
        ...
        DataSectionPtr pConfig = BWResource::instance().openSection(
          s_autoConfigSettingsFileName.value());
        if (pConfig.exists() && !s_autoConfigSettingsFileName.value().empty()) {
            ...
            memcpy_s(&bounds,
                     sizeof(PerfBounds) * PERF_LEVEL_COUNT,
                     &defaultBounds,
                     sizeof(PerfBounds) * PERF_LEVEL_COUNT);
    ...
    void GraphicsSettingsAutoDetector::readVendorSetting(
      const DataSectionPtr& vendorSection,
      SystemPerformance     level,
    ...
    void GraphicsSettingsAutoDetector::readConfigSetting(
      const DataSectionPtr& pConfig,
      const char*           name,
      SystemPerformance     level)
    {
        ...
        if (it != sections.end()) {
            bounds[level].CPUFrequency =
              (*it)->readInt("CPUFrequency", defaultBounds[level].CPUFrequency);
            bounds[level].GPUMemory =
              (*it)->readInt("GPUMemory", defaultBounds[level].GPUMemory);
            bounds[level].RAM = (*it)->readInt("RAM", defaultBounds[level].RAM);
            bounds[level].AvailableVirtualMemory =
              (*it)->readInt("AvailableVirtualMemory",
                             defaultBounds[level].AvailableVirtualMemory);
            bounds[level].RAMFreq =
              (*it)->readInt("RAMFrequency", defaultBounds[level].RAMFreq);
            bounds[level].PixelShaderVersion = (*it)->readFloat(
              "PixelShaderVersion", defaultBounds[level].PixelShaderVersion);
            bounds[level].VertexShaderVersion = (*it)->readFloat(
              "VertexShaderVersion", defaultBounds[level].VertexShaderVersion);
            bounds[level].NotebookCompartible = (*it)->readBool(
              "NotebookCompartible", defaultBounds[level].NotebookCompartible);
            ...
            if (vendorSection.exists()) {
                ...
                if (bounds[level]
                      .deviceInfo[ATI]
                      .need_check) // sign known prefixes
                {
                ...
                if (bounds[level]
                      .deviceInfo[NVIDIA]
                      .need_check) // sign newer xxx series in order to be more
                                   // than xxxx older series
                {
                    if (bounds[level].deviceInfo[NVIDIA].number >= 100 &&
                        bounds[level].deviceInfo[NVIDIA].number < 1000)
    ...
    void GraphicsSettingsAutoDetector::addSettingsGroup(
    ...
    bool GraphicsSettingsAutoDetector::getSettings(SettingsVector& out,
                                                   int overrideSettingsGroup)
    {
        BW_GUARD;
...
#define MINOR_SHADER_VER(quality, ShaderType)                                  \
    ((BYTE)((bounds[quality].ShaderType##ShaderVersion -                       \
             (BYTE)bounds[quality].ShaderType##ShaderVersion) *                \
            10.0f))
#define MAJOR_SHADER_VER(quality, ShaderType)                                  \
    ((BYTE)bounds[quality].ShaderType##ShaderVersion)
#define PS_VERSION(quality)                                                    \
    D3DPS_VERSION(MAJOR_SHADER_VER(quality, Pixel),                            \
                  MINOR_SHADER_VER(quality, Pixel))
#define VS_VERSION(quality)                                                    \
    D3DVS_VERSION(MAJOR_SHADER_VER(quality, Vertex),                           \
                  MINOR_SHADER_VER(quality, Vertex))

#define CHECK_PARAMETER(param)                                                 \
    if (param < bounds[MIN].param) {                                           \
        INFO_MSG("Warning! %s amount %" PRI64                                  \
                 " is too low! Must be at least %u\n",                         \
                 #param,                                                       \
                 param,                                                        \
                 bounds[MIN].param);                                           \
        quality = MIN;                                                         \
    ...
    GraphicsSettingsAutoDetector::SystemPerformance
    GraphicsSettingsAutoDetector::detectBestSettings(bool forceNewDetection)
    {
        static const char* messages[] = {
            "Checking Notebook", "GPU Memory: ",       "RAM: ",
            "Virtual Memory: ",  "RAM Frequency:	", "CPU Frequency:	",
            "Vertex Shader Ver", "Pixel Shader Ver",   "Check VideoAdapter"
        };
        ...
        GraphicsSettingsAutoDetector::SystemPerformance
                qualitiesAfterEachParameter[9] = { MAX };
        ...
        if (pD3D9) {
            ...
            if (SUCCEEDED(hr)) {
                if (Caps.PixelShaderVersion < PS_VERSION(MIN) ||
                    Caps.VertexShaderVersion < VS_VERSION(MIN)) {
                    INFO_MSG(
                      "Warning! Shader version is too low! Must be at least %f "
                      "for vertex and %f for pixel shader version.\n",
                      bounds[MIN].VertexShaderVersion,
                      bounds[MIN].PixelShaderVersion);
                    quality = MIN;
        ...
        if (di.identifier_.VendorId == 0x10DE) // NVIDIA
            quality =
              max(quality, CheckNvidiaGPUId(di.identifier_.Description));
        ...
        if (bPrintSettings) {
            ...
            for (int i = 0; i < 9; ++i) {
                ...
                out << ";	Quality: " << qualitiesAfterEachParameter[i]
                    << std::endl;
            }
            out.close();
        }
    ...
    Moo::GraphicsSettingsAutoDetector::SystemPerformance
    Moo::GraphicsSettingsAutoDetector::getQualityForNotebooks()
    {
        GraphicsSettingsAutoDetector::SystemPerformance quality = MAX;
        if (IsNotebook()) {
            bool flag = false;
    ...
    int64 Moo::GraphicsSettingsAutoDetector::getVideoMemory()
    {
        ...
        if (pD3D9) {
            ...
            const HRESULT hr = GetVideoMemoryViaDXGI(hMonitor,
        ...
        *pSharedSystemMemory    = 0;
        ...
        if (hDXGI) {
            ...
            for (int index = 0;; ++index) {
                ...
                if (FAILED(hr)) // DXGIERR_NOT_FOUND is expected when the end of
                                // the list is hit
                    break;
                ...
                for (int iOutput = 0;; ++iOutput) {
                    ...
                    if (FAILED(hr)) // DXGIERR_NOT_FOUND is expected when the
     ...
     *		as the CPU throttles. On older CPUs it might measure clock cycles,
     *but on some such older systems it's not consistent between cores/sockets.
     */
...
#define MEMORY_TO_ALLOC 10000000
    ...
    bool GraphicsSettingsAutoDetector::GetDeviceEnumerator(
      IWbemServices*         pSvc,
      IWbemLocator*          pLoc,
      BSTR                   wmiQuery,
    ...
    void GraphicsSettingsAutoDetector::CheckEnclosureType(INT&    result,
                                                          VARIANT vtProp)
    {
    ...
    void GraphicsSettingsAutoDetector::CheckBatteryType(INT&    result,
                                                        VARIANT vtProp)
    {
    ...
    void GraphicsSettingsAutoDetector::CheckPhysMemoryType(INT&    result,
                                                           VARIANT vtProp)
    {
    ...
    INT GraphicsSettingsAutoDetector::GetDeviceObjectsInfo(
      IEnumWbemClassObject* pEnumerator,
      wchar_t*              nameOfTheInfo,
      CHECK_FUNCTION        myComparator)
    {
        ...
        while (pEnumerator) {
            ...
            if (uReturn > 0) {
                VARIANT vtProp;
                if (SUCCEEDED(
                      pclsObj->Get(nameOfTheInfo, 0L, &vtProp, NULL, NULL))) {
    ...
    bool GraphicsSettingsAutoDetector::IsNotebook()
    {
        ...
        try {
            ...
            if (SUCCEEDED(hres)) {
                ...
                bool res    = false;
                INT  result = 0;
                if (GetDeviceObjectsInfo(
                      pEnumerator, L"ChassisTypes", CheckEnclosureType) > 0)
                    result += 1;

                pEnumerator->Release(), pEnumerator = NULL;
                if (GetDeviceEnumerator(pSvc,
                                        pLoc,
                                        bstr_t("SELECT * FROM Win32_Battery"),
                                        &pEnumerator)) {
                    if (GetDeviceObjectsInfo(
                          pEnumerator, L"Name", CheckBatteryType) > 0)
                        result += 1;
                    pEnumerator->Release(), pEnumerator = NULL;
                }
                ...
                if (GetDeviceEnumerator(
                      ...
                      &pEnumerator)) {
                    if (GetDeviceObjectsInfo(
                          pEnumerator, L"FormFactor", CheckPhysMemoryType) > 0)
                        result += 1;
                    pEnumerator->Release(), pEnumerator = NULL;
                }
                if (result >= 1)
    ...
    HRESULT GraphicsSettingsAutoDetector::InitWMI(IWbemServices** pSvc,
                                                  IWbemLocator**  pLoc)
    {
        HRESULT hres;
        try {
            ...
            // We need COM initialization. Second parameter should be
            // COINIT_APARTMENTTHREADED because of calling that method in
            // ImeUi.cpp
    ...
    GraphicsSettingsAutoDetector::SystemPerformance
    GraphicsSettingsAutoDetector::CheckNvidiaGPUId(
      const char*       deviceId,
      SystemPerformance curPerformance)
    {
        if (!deviceId)
        ...
        if (!bounds[curPerformance].deviceInfo[NVIDIA].need_check) {
            if (curPerformance == MAX) {
                ...
                return CheckNvidiaGPUId(
                  deviceId, (SystemPerformance)(curPerformance - 1));
            }
        }
        ...
        if (number != 0 && number != LONG_MAX && number != LONG_MIN) {
            ...
            if (bounds[curPerformance].deviceInfo[NVIDIA].number < number)
                return (curPerformance == MAX
                          ? MAX
                          : CheckNvidiaGPUId(
                              device, (SystemPerformance)(curPerformance - 1)));
        }

        return (curPerformance < MIN) ? (SystemPerformance)(curPerformance + 1)
                                      : (curPerformance);
    }

    GraphicsSettingsAutoDetector::SystemPerformance
    GraphicsSettingsAutoDetector::CheckAtiGPUId(
      const char*       deviceId,
      SystemPerformance curPerformance /*= MIN*/)
    {
        if (!deviceId)
            return curPerformance;
        ...
        if (!bounds[curPerformance].deviceInfo[ATI].need_check) {
            if (curPerformance == MAX) {
                ...
                return CheckAtiGPUId(deviceId,
                                     (SystemPerformance)(curPerformance - 1));
            }
        }
    ...
    GraphicsSettingsAutoDetector::SystemPerformance
    GraphicsSettingsAutoDetector::CheckIntelGPUId(
      const char*       deviceId,
      SystemPerformance curPerformance /*= MIN*/)
    {
        if (!deviceId)
        ...
        if (!bounds[curPerformance].deviceInfo[INTEL].need_check) {
            if (curPerformance == MAX) {
                ...
                return CheckIntelGPUId(deviceId,
                                       (SystemPerformance)(curPerformance - 1));
            }
        }
    ...
    GraphicsSettingsAutoDetector::SystemPerformance
      GraphicsSettingsAutoDetector::lastDetection =
        GraphicsSettingsAutoDetector::PERF_LEVEL_COUNT;
