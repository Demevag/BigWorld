...
namespace Moo {
    ...
    struct TextureAggregatorPimpl : DeviceCallback
    {
        ...
        int registerTile(BaseTexturePtr tex,
                         const Vector2& min,
                         const Vector2& max,
                         int            width,
                         int            height,
        ...
        void renderTile(BaseTexturePtr tex,
                        const Vector2& min,
                        const Vector2& max,
                        int            width,
                        int            height,
     ...
     *	Constructor.
     *
     *	@param	notifyFunc	pointer to reset notification function. This
     *function will be called back whenever the aggregator is repacked.
     */
     ...
     */
    int TextureAggregator::addTile(BaseTexturePtr tex,
                                   const Vector2& min,
                                   const Vector2& max)
    {
        BW_GUARD;
     ...
     *	@param	min		(out) will hold the coords' upper-left corner (in
     *texels).
     *	@param	max		(out) will hold the coords' lower-right corner (in
     *texels).
     */
    void TextureAggregator::getTileCoords(int      id,
                                          Vector2& min,
                                          Vector2& max) const
    {
        BW_GUARD;
...
#define DEBUG_DEL_TILE 0
#if DEBUG_DEL_TILE
            Vector2 dstMin(float(slot->x), float(slot->y));
            Vector2 dstMax(float(slot->x + slot->width),
    ...
    /**
     *	Returns true if the tiles have been reset due to repacking since the
     *last tile this method was called, returning false henceforth. If called
     *every frame, this can be used as an alternative to the repack callback.
     *
     ...
     *	bigger size. Newly created slots will be placed on the freeSlots
     *container.
     *
     *	@param	log2	slot's size's base-2 log.
     *
    ...
    {
        BW_GUARD;
        return !this->texture.hasComObject() ||
     ...
     */
    TextureAggregatorPimpl::SlotVector TextureAggregatorPimpl::growTexture(
      int log2Hint)
    {
        BW_GUARD;
      ...
      int height)
    {
        ...
        if (width <= this->maxSize && height <= this->maxSize) {
            ...
            texture   = rc().createTexture(width,
                                         height,
                                         mipLevels,
                ...
                for (int i = 0; i < mipLevels; ++i) {
                    ...
                    if (SUCCEEDED(hr)) {
                        ...
                        if (SUCCEEDED(hr)) {
                            ...
                            ERROR_MSG("TextureAggregator::createTexture: "
     ...
     */
    void TextureAggregatorPimpl::copyWholeTexture(DXTexturePtr src,
                                                  DXTexturePtr dst)
    {
        BW_GUARD;
     ...
     */
    int TextureAggregatorPimpl::registerTile(BaseTexturePtr tex,
                                             const Vector2& min,
                                             const Vector2& max,
                                             int            width,
                                             int            height,
     ...
     *	@param	slot	tile's destination slot.
     */
    void TextureAggregatorPimpl::renderTile(BaseTexturePtr tex,
                                            const Vector2& min,
                                            const Vector2& max,
                                            int            width,
                                            int            height,
                                            SlotPtr        slot)
    {
        ...
        if (this->texture.hasComObject()) {
            Vector2 dstMin(float(slot->x), float(slot->y));
            Vector2 dstMax(float(slot->x + width), float(slot->y + height));
    ...
    void TextureAggregatorPimpl::recycleSlot(SlotPtr slot)
    {
        ...
        if (this->areNeightboursFree(slot, slots)) {
            ...
            this->recycleSlot(new Slot(baseSlot->x,
                                       baseSlot->y,
                                       baseSlot->width * 2,
                                       baseSlot->width * 2));
        }
     ...
     *
     */
    bool TextureAggregatorPimpl::areNeightboursFree(SlotPtr     slot,
                                                    SlotVector& slots)
    {
        BW_GUARD;
    ...
    float TextureAggregatorPimpl::textureUsage() const
    {
        ...
        if (this->texture.hasComObject()) {
            ...
            while (tileIt != tileEnd) {
                usedArea +=
                  tileIt->second.slot->width * tileIt->second.slot->height;
                ++tileIt;
            }
    ...
    void TextureAggregatorPimpl::repack()
    {
        ...
        while (tileIt != tileEnd) {
            ...
            if (newSlot == NULL) {
                ERROR_MSG("TextureAggregatorPimpl::repack: getSlot(%d) failed. "
            ...
            renderTile(tileIt->second.tex,
                       tileIt->second.min,
                       tileIt->second.max,
                       tileIt->second.width,
                       tileIt->second.height,
                       tileIt->second.slot);
            ++tileIt;
        }
    ...
    namespace // anonymous
    {
        ...
        /**
         *	Subdivides given slot into it's four smaller constituents quarter
         *slots.
         *
         ...
         *	@param	cropBorders	true if borders should be cropperd to avoid
         *bleeding.
         */
        void copyTile(int              srcWidth,
                      int              srcHeight,
                      ...
                      bool             cropBorders)
        {
            ...
            if (!rc().checkDevice()) {
                ERROR_MSG(
                  "TextureAggregator: device lost when trying to copy tile.\n");
                return;
            }
            ...
            int texMipLevels =
              std::min(srcLevelCount, int(dst->GetLevelCount()));
            for (int i = 0; i < texMipLevels; ++i) {
                float tileBorder =
                  cropBorders ? float(c_TileBorder << i) : 0.0f;
                ...
                if (SUCCEEDED(hr)) {
                    ...
                    if (SUCCEEDED(hr)) {
                        ...
                        rc().setSamplerState(
                          0, D3DSAMP_MIPFILTER, D3DTEXF_POINT);

                        // set transforms
                        rc().device()->SetTransform(D3DTS_VIEW,
                                                    &Matrix::identity);
                        rc().device()->SetTransform(D3DTS_WORLD,
                                                    &Matrix::identity);
                        rc().device()->SetTransform(D3DTS_TEXTURE0,
                                                    &Matrix::identity);
                        ...
                        rc().device()->SetTransform(D3DTS_PROJECTION,
                                                    &projection);
                        ...
                        // may contain some left-overs)
                        {
                            ...
                            v[0].pos_ =
                              Vector3(pDstMin.x - w0d2, h0d2 - pDstMax.y, 0);
                            v[0].colour_ = 0;
                            v[1].pos_ =
                              Vector3(pDstMin.x - w0d2, h0d2 - pDstMin.y, 0);
                            v[1].colour_ = 0;
                            v[2].pos_ =
                              Vector3(pDstMax.x - w0d2, h0d2 - pDstMin.y, 0);
                            v[2].colour_ = 0;
                            v[3].pos_ =
                              Vector3(pDstMax.x - w0d2, h0d2 - pDstMax.y, 0);
                            v[3].colour_ = 0;
                            ...
                            rc().drawPrimitiveUP(
                              D3DPT_TRIANGLEFAN, 2, v, sizeof(VertexType));
                        }
                        ...
                        // set texture
                        {
                            ...
                            v[0].pos_ =
                              Vector3(dstMin.x - w0d2, h0d2 - dstMax.y, 0);
                            v[0].uv_ =
                              Vector2(srcMin.x + offsetX, (srcMax.y + offsetY));
                            v[0].colour_ = 0;
                            v[1].pos_ =
                              Vector3(dstMin.x - w0d2, h0d2 - dstMin.y, 0);
                            v[1].uv_ =
                              Vector2(srcMin.x + offsetX, (srcMin.y + offsetY));
                            v[1].colour_ = 0;
                            v[2].pos_ =
                              Vector3(dstMax.x - w0d2, h0d2 - dstMin.y, 0);
                            v[2].uv_ =
                              Vector2(srcMax.x + offsetX, (srcMin.y + offsetY));
                            v[2].colour_ = 0;
                            v[3].pos_ =
                              Vector3(dstMax.x - w0d2, h0d2 - dstMax.y, 0);
                            v[3].uv_ =
                              Vector2(srcMax.x + offsetX, (srcMax.y + offsetY));
                            v[3].colour_ = 0;
                            ...
                            rc().setTextureStageState(
                              0,
                              D3DTSS_COLORARG1,
                              src != NULL ? D3DTA_TEXTURE : D3DTA_DIFFUSE);
                        ...
                        WARNING_MSG("TextureAggregator::renderTile : "
                                    "Unable to set render target on device\n");
                    }
                    surface = NULL;
                    rc().popRenderTarget();
