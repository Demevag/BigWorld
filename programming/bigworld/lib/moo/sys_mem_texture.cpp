...
namespace Moo {
     ...
     *  Constructor.
     */
    SysMemTexture::SysMemTexture(const BW::StringRef& resourceID,
                                 D3DFORMAT            format)
      : width_(0)
      , height_(0)
      , format_(format)
    ...
    SysMemTexture::~SysMemTexture()
    {
        ...
        // DISABLED reuse for system memory textures till we get some stats on
        // it
    ...
    void SysMemTexture::load(BinaryPtr data)
    {
        ...
        if (data == NULL)
            data = BWResource::instance().rootSection()->readBinary(
              qualifiedResourceID_);
