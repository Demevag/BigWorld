...
namespace {
    ...
    // the client's PyModule configuration). This causes the TreadLocal<T> class
    // to use it's "operator T &", instead of thinking that ".requester_" is a
    // member of TreadLocal<T>.
...
namespace Moo {
    ...
    /*static*/ void ResourceLoadContext::push(const BW::StringRef& requester)
    {
        ...
        if (s_stackPos < MAX_STACK_DEPTH) {
            size_t length =
              std::min<size_t>(requester.size(), MAX_REQUESTER_SIZE);
