  ...
  const WaypointGenerator::PointDef& p) const
{
    ...
    return angle == p.angle && fabs(offset - p.offset) < 1.5 &&
           fabs(t - p.t) < 1.5;
}
...
bool WaypointGenerator::PolygonDef::ptNearEnough(const Vector3& pt) const
{
    ...
    for (uint bi = 0; bi < vertices.size(); ai = bi++) {
        ...
        xd += 1.f;
        zd -= 2.f; // before first loop will change to both -1
        for (int j = 0; j < 9; ++j) {
            if (j % 3 != 0) {
                xd += 1.f;
            } else {
...
bool WaypointGenerator::readTGA(const char* filename)
{
    ...
    if (hdr.extraLength != sizeof(Vector3) + sizeof(float) ||
        hdr.colourMapType != 1 || hdr.imageType != 1 ||
        hdr.colourMapStart != 0 || hdr.colourMapLength != 256 ||
        hdr.colourMapDepth != 24 || hdr.x != 0 || hdr.y != 0 || hdr.bpp != 8 ||
    ...
    for (int g = 0; g < 16; ++g) {
        if (fread(
              adjGrids_[g], gridX_ * gridZ_ * sizeof(AdjGridElt), 1, pFile) !=
            1) {
    ...
    for (int g = 0; g < 16; ++g) {
        if (fread(hgtGrids_[g], gridX_ * gridZ_ * sizeof(float), 1, pFile) !=
            1) {
            ERROR_MSG("WaypointGenerator: Read error.\n");
            fclose(pFile);
            return false;
 ...
 */
bool WaypointGenerator::init(int            width,
                             int            height,
                             const Vector3& gridMin,
...
bool WaypointGenerator::nodesSame(const int index1, const int index2)
{
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        if (res1) {
            if (!(p1a.nearlySame(
                  p2a))) // operator== includes float_fudge factor.
            {
                return false;
            }
 ...
 *	tries to split the node along edges, between passable and impassable
 *regions. It also tries to find a split close to the centre of the region, in
 *order to create a balanced tree.
 *
...
void WaypointGenerator::processNode(BW::vector<int>& indexStack)
{
    ...
    for (int x = x1; x <= x2; ++x) {
        for (int z = z1; z <= z2; ++z) {
            ...
            // make sure there is only one point at these coordinates in the
            // node
            ng = -1;
            for (g = 0; g < 16; ++g) {
    ...
    // if there was a problem, bsp_ and indexStack are untouched.
        ...
        // INFO_MSG( "Problem vertical splitting, using best horizontal split
        // instead\n" );
        doBestHorizontalSplit(index, indexStack);
    }
}

void WaypointGenerator::doBestHorizontalSplit(int              index,
                                              BW::vector<int>& indexStack)
{
    BW_GUARD;
    ...
    float cx =
      (float)floor(bsp_[index].centre.x + (-bsp_[index].borderOffset[2]));
    float cz =
      (float)floor(bsp_[index].centre.y + (-bsp_[index].borderOffset[0]));
    ...
    if (this->splitNode(index, split, override)) {
        ...
        std::pair<float, int> hgt1 =
          this->averageHeight(bsp_[bsp_[index].front]);
        std::pair<float, int> hgt2 =
          this->averageHeight(bsp_[bsp_[index].back]);
    ...
    for (int a = 1; a < 4; ++a) {
        ...
        if (this->splitNode(
              index,
              split,
              override)) // override already calculated front/backs.
        {
            override = true;
    ...
    // else
    // split didn't work! quit. not waypoints, and don't add to stack to
    // recurse.
...
void WaypointGenerator::calcSplitValue(const BSPNode& node, SplitDef& split)
{
    ...
    value = (node.centre - Vector2(split.position[0], split.position[1]))
              .lengthSquared();
 ...
 */
bool WaypointGenerator::splitNode(int             index,
                                  ...
                                  bool            replace)
{
    ...
    if (split.normal == SplitDef::VERTICAL_SPLIT) {
        if (this->findDispoints(front)) {
            //			INFO_MSG( "splitNode: Vertical split at %f (collapsed to
            //%f) leads " 				"to points disconnected by split. Trying range %f to
            //%f.\n", 				d1, front.maxHeight, split.position[0], split.position[1]
            //);

            float h;
            for (h = split.position[0] + 0.1f; h < split.position[1] - 0.1f;
                 h += 0.1f) {
                front.maxHeight = h;
                ...
                this->reduceNodeHeight(front);
                if (!this->findDispoints(front)) {
                    //					INFO_MSG( "splitNode: Found clean
                    //vertical split " 						"at %f (collapsed to %f)\n", h,
                    //front.maxHeight );
                    d2 = h;
                    break;
                }
            ...
            if (h >= split.position[1] - 0.1f) {
                //				INFO_MSG( "splitNode: Could not find any clean
                //vertical split "
                //					"(caller will deal)\n" );
                return false; // problem with vertical split. bsp_[index]
                              // unchanged.
            }
        }
    }
...
void WaypointGenerator::refineNodeEdges(BSPNode* node) const
{
    ...
    float offsets[4] = { node->borderOffset[1] / HALF_ROOT_2,
    ...
    for (x = x1; x <= x2; x++) {
        for (z = z1; z <= z2; z++) {
            ...
            for (int g = 15; g >= 0; --g) {
                ...
                if (adjGrids_[g][x + gridX_ * z].all == 0)
                    continue;
 ...
 *	returned as PointDefs. This basically defines the equation of a line, and
 *the distance along that line from a known starting point. It is done this way
 ...
 */
bool WaypointGenerator::calcEdge(int       index,
                                 int       edge,
                                 PointDef& p1,
                                 PointDef& p2) const
{
...
void WaypointGenerator::generatePolygons()
{
    ...
    for (i = 0; i < bsp_.size(); ++i) {
        ...
        if (bsp_[i].waypoint) {
            ...
            if (bsp_[i].borderOffset[0] == -bsp_[i].borderOffset[4])
                continue;
            if (bsp_[i].borderOffset[1] == -bsp_[i].borderOffset[5])
                continue;
            if (bsp_[i].borderOffset[2] == -bsp_[i].borderOffset[6])
                continue;
            if (bsp_[i].borderOffset[3] == -bsp_[i].borderOffset[7])
                continue;
            ...
            for (a = 0; a < 4; ++a) {
                if (this->calcEdge(i, a, p1, p2)) {
                    ...
                    for (pointIter = points_.lower_bound(p2);
                        ...
                        if (pointIter->maxHeight < p1.minHeight)
                            continue;
                        if (pointIter->minHeight > p1.maxHeight)
                            continue;
...
//					bsp_[i].waypointIndex, i );
                ...
                if (polygons_.size() == 473) {
                    DEBUG_MSG("polygon 473 min, max height: %f %f\n",
                              polygons_[473].minHeight,
                              polygons_[473].maxHeight);
                    DEBUG_MSG("polygon 468 min, max height: %f %f\n",
                              polygons_[468].minHeight,
                              polygons_[468].maxHeight);
                }
#endif
            }
        }
    ...
    for (p = 0; p < polygons_.size(); ++p) {
        ...
        if (p == 473 || p == 468) {
            ...
            for (int ii = 0; ii < (int)polygon.vertices.size(); ++ii) {
                DEBUG_MSG("%f %f \n",
                          polygon.vertices[ii].pos[0],
                          polygon.vertices[ii].pos[1]);
            }
            DEBUG_MSG("--\n");
        }
#endif
        ...
        for (v = 0; v < vertexCount; ++v) {
            ...
            for (edgeIter = edges_.lower_bound(edge);
                ...
                if (maybeAdj.maxHeight < polygon.minHeight)
                    continue;
                if (maybeAdj.minHeight > polygon.maxHeight)
                    continue;
...
int WaypointGenerator::getBSPNode(int cursor, int depth, BSPNodeInfo& bni) const
{
    ...
    while (depth > 0) {
        int ni = (!(cursor & (1 << (depth - 1)))) ? bsp_[bspIndex].front
                                                  : bsp_[bspIndex].back;
        if (ni == -1)
            break;
    ...
    Vector2  v1, v2;
    for (int a = 0; a < 8; a++) {
        if (!this->calcEdge(bspIndex, a, p1, p2))
            continue;
        if (a >= 4) {
            this->invertPoint(p1);
            this->invertPoint(p2);
    ...
    for (int v = 0; v < bs; v++) {
        ...
        float ta = (na - pa > MATH_PI) ? (na + MATH_PI * 2.f + pa) / 2.f
                                       : (na + pa) / 2.f;
 ...
 */
void WaypointGenerator::getVertex(int      polygon,
                                  int      vertex,
                                  Vector2& v,
                                  int&     adjacency,
...
bool WaypointGenerator::gridPoll(int cursor, Vector3& pt, Vector4& ahgts) const
{
    ...
    if (z != gridZ_ - 1 && x != gridX_ - 1 &&
        (og = adjGrids_[g][index].each.ur))
...
bool WaypointGenerator::joinPolygon(int polyIndex)
{
    ...
    do {
        ...
        while (p1.vertices[edgeStart1].angles == p1.vertices[edgeEnd1].angles) {
            if (p1.vertices[edgeStart1].adjNavPoly !=
                p1.vertices[edgeEnd1].adjNavPoly)
        ...
        if (singleAdjacency) {
            ...
            if (!(size2 > 0)) {
                ERROR_MSG("Polygon %d from %d (%f,%f) to %d (%f,%f) links to "
                          ...
                          i1,
                          edgeStart1,
                          p1.vertices[edgeStart1].pos.x,
                          p1.vertices[edgeStart1].pos.y,
                          edgeEnd1,
                          p1.vertices[edgeEnd1].pos.x,
                          p1.vertices[edgeEnd1].pos.y,
                          i2);
                ...
                if (isConvexJoint(p1.vertices[edgePrev1].angles,
                                  p1.vertices[edgeStart1].angles,
                                  p2.vertices[edgeEnd2].angles) &&
                    isConvexJoint(p2.vertices[edgePrev2].angles,
                                  p2.vertices[edgeStart2].angles,
                                  p1.vertices[edgeEnd1].angles) &&
                    ...
                    for (i = edgeEnd2; i != edgeStart2; i = (i + 1) % size2) {
                        ...
                        if (p2.vertices[i].adjNavPoly > 0) {
                            //					dprintf( "Joining: Fixing up
                            //adjs on %d to point to %d instead of %d\n",
                            //						p2.vertices[i].adjNavPoly -
                            //1, i1, i2 );
                            this->fixupAdjacencies(
                              p2.vertices[i].adjNavPoly - 1, i2 + 1, i1 + 1);
                        }
                    }
 ...
 */
BW::vector<int> WaypointGenerator::findWaypoints(const Vector3& v,
                                                 float /*girth*/) const
{
    BW_GUARD;
...
Vector3 WaypointGenerator::toVector3(const Vector2&    v,
                                     const PolygonDef& polygon,
                                     Chunk*            pChunk) const
{
    BW_GUARD;
    ...
    for (g = 0; g < 16; g++) {
        ...
        if (p.y >= polygon.minHeight && p.y <= polygon.maxHeight)
            break;
    }
    if (g == 16)
        p.y = polygon.maxHeight + 0.01f;
...
void WaypointGenerator::extendThroughUnboundPortals(Chunk* pChunk)
{
    ...
    for (unsigned int p = 0; p < polygons_.size(); ++p) {
        ...
        for (unsigned int v = 0; v < vertexCount; ++v) {
            ...
            VertexDef& nVertex =
              polygon.vertices[(v + 1) % vertexCount]; // next vertex.
            ...
            BW::vector<ChunkBoundary::Portal*>::iterator pIter =
              ubPortals.begin();
            for (; pIter != ubPortals.end(); ++pIter) {
                ...
                if (((*pIter)->plane.distanceTo(cTv) < 0.45) &&
                    ...
                    Vector2 extendDir(-(nVertex.pos[1] - cVertex.pos[1]),
                                      (nVertex.pos[0] - cVertex.pos[0]));
                    extendDir.normalise();

                    // remember to change.
                    extendPoints.push_back(
                      std::pair<int, Vector2>(v, extendDir));
                }
            }
        }
        ...
        for (int i = 0; i < (int)extendPoints.size(); ++i) {
            ...
            for (int j = 0; j < (int)extendPoints.size(); ++j) {
                if (extendPoints[j].first ==
                    (extendPoints[i].first + 1) % vertexCount) {
            ...
            if (!found) {
                extraExtendPoints.push_back(std::pair<int, Vector2>(
                  (extendPoints[i].first + 1) % vertexCount,
        ...
        for (int i = 0; i < (int)extendPoints.size(); ++i) {
            ...
            for (int j = 0; j < (int)extendPoints.size(); ++j) {
                if (extendPoints[j].first ==
                    (extraExtendPoints[0].first - i - 1 + vertexCount) %
                      vertexCount) {
        ...
        // remaining edges adjacent to existing polygon, and left hand vertical
        // edge.
        for (int i = 0; i < (int)orderedExtendPoints.size(); ++i) {
            ...
            if (i < (int)orderedExtendPoints.size() - 1) {
                ...
                polygon.vertices[orderedExtendPoints[i + 1].first].adjNavPoly =
                  static_cast<int>(polygons_.size() + newPolys.size() + 1);
                polygon.vertices[orderedExtendPoints[i + 1].first]
                  .adjToAnotherChunk = false;
            }
 ...
 */
void WaypointGenerator::determineEdgesAdjacentToOtherChunks(Chunk*    pChunk,
                                                            IPhysics* physics)
{
    BW_GUARD;
    ...
    for (unsigned int p = 0; p < polygons_.size(); ++p) {
        ...
        for (unsigned int v = 0; v < vertexCount; v++) {
            Vector3 p1 =
              this->toVector3(polygon.vertices[v].pos, polygon, pChunk);
            if (p1.x < b.minBounds().x && p1.z < b.minBounds().z)
                outsides |= 1;
            if (p1.x < b.minBounds().x && p1.z > b.maxBounds().z)
                outsides |= 2;
            if (p1.x > b.maxBounds().x && p1.z < b.minBounds().z)
                outsides |= 4;
            if (p1.x > b.maxBounds().x && p1.z > b.maxBounds().z)
                outsides |= 8;
        }
        if (outsides == 1 + 2 + 4 + 8) {
            for (unsigned int v = 0; v < vertexCount; v++)
        ...
        for (unsigned int v = 0; v < vertexCount; ++v) {
            if (polygon.vertices[v].adjNavPoly >
                0) { // we have connected to another nav poly, skip
                continue;
            }
    ...
    for (int p = 0; p < (int)polygons_.size(); ++p) {
        if (polygonPortalParity[p] % 2 == 1) {
            WARNING_MSG(
              "WaypointGenerator::determineEdgesAdjacentToOtherChunks: "
              "polygon %d crosses portal boundaries an odd number (%d) of "
              "times\n",
              p,
              polygonPortalParity[p]);
        }
    }
}
...
void WaypointGenerator::streamline()
{
    ...
    for (uint p = 0; p < polygons_.size(); p++) {
        ...
        if (polygon.vertices.empty()) {
            ...
            for (uint v = 0; v < polygon.vertices.size(); v++) {
                ...
                Vector2 nextPt =
                  polygon.vertices[(v + 1) % polygon.vertices.size()].pos;
                if (nextPt == thisPt) {
                    polygon.vertices.erase(polygon.vertices.begin() + v);
                    v--;
                    pointEdges++;
    ...
    if (lostEdges & 1) {
        ERROR_MSG("There should have been an even number of edges removed "
                  "after streamlining.");
        ERROR_MSG("\tConsider to use custom BSP if the BSP in this chunk is "
                  "too complex.");
    }
}
  ...
  const BW::string&          chunkName) // chunkName used for debugging message.
{
    ...
    for (fit = frontierPts.begin(); fit != frontierPts.end(); ++fit) {
        localFPs.push_back(Vector3((fit->x - gridMin_.x) / gridResolution_,
                                   fit->y,
    ...
    for (uint p = 0; p < polygons_.size(); p++) {
        ...
        for (uint v = 0; v < polygon.vertices.size();
             v++) { // is it frontier 'coz it leads to another chunk?
        ...
        if (!frontier) { // is it frontier 'coz a frontier pt is in it?
        ...
        if (!frontier) {
            if (setNext > 0 || p < polygons_.size() - 1)
                continue;
                ...
                if (aidx > 0) {
                    if (polygons_[aidx - 1].set != 0) {
                        ...
                        if (polygons_[aidx - 1].set != setNext) {
                            ...
                            CRITICAL_MSG(
                              "Polygon %d in new set %d has "
                              "unreciprocated connection to p %d in set %d\n",
                              idx + 1,
                              setNext,
                              aidx,
                              polygons_[aidx - 1].set);
                            ...
                            DEBUG_MSG("473 height: %f %f\n",
                                      spoly.minHeight,
                                      spoly.maxHeight);
                            for (int ii = 0; ii < (int)spoly.vertices.size();
                                 ++ii) {
                                DEBUG_MSG("spoly: %d %f %f\n",
                                          ii,
                                          spoly.vertices[ii].pos[0],
                                          spoly.vertices[ii].pos[1]);
                            }

                            DEBUG_MSG("---\n\n468 height: %f %f\n",
                                      polygons_[aidx - 1].minHeight,
                                      polygons_[aidx - 1].maxHeight);
                            for (int ii = 0; ii < (int)spoly.vertices.size();
                                 ++ii) {
                                DEBUG_MSG(
                                  "spoly: %d %f %f\n",
                                  ii,
                                  polygons_[aidx - 1].vertices[ii].pos[0],
