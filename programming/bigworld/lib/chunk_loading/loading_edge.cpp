...
bool LoadingEdge::tick(EdgeGeometryMapping* pMapping,
                       const LoadingEdge&   lineLo,
                       const LoadingEdge&   lineHi,
                       ...
                       bool&                rAnyColumnsLoaded)
{
    ...
    if (currPos_ * loadMoveDir < destPos_ * loadMoveDir) {
        ...
        if (low_.pos_ > (lineLo.currPos_ - 1)) {
            ...
            // load cur column
            if (low_.stepLoad(
                  pMapping, perpPos, this->isVertical(), rAnyColumnsLoaded)) {
                low_.pos_--;
    ...
    } else if (currPos_ * loadMoveDir >
               destPos_ * loadMoveDir) { // (comment the if above to test
                                         // unloading+loading)
        ...
        if (low_.pos_ + 1 >= high_.pos_) {
            ...
            if (low_.state_ == UNLOADED && high_.state_ == UNLOADED) {
                allUnloaded = true;
            }
        }
        if (!allUnloaded) {
        ...
        if (allUnloaded) {
            ...
            low_.state_  = LOADED;
            high_.state_ = LOADED;
            if (low_.pos_ > high_.pos_) { // missing both
                low_.pos_--;
                high_.pos_++;
                low_.state_  = UNLOADED;
                high_.state_ = UNLOADED;
            } else if (low_.pos_ == high_.pos_) { // missing one
