...
namespace PostProcessing {
     ...
     *	@param	debug	optional debugging object to records each phase's
     *results.
     */
    ...
    bool Effect::load(DataSectionPtr pDS)
    {
        ...
        for (; it != en; ++it) {
            ...
            // sections returns a different pointer hear than from its
            // findChild).
    ...
    bool Effect::save(DataSectionPtr pDS)
    {
        ...
        if (bypass_.hasObject()) {
            ...
            // becomes a static value representing the value it was at save
            // time.
            Vector4 b;
            bypass_->output(b);
