...
PyObject* DelegateEntityMethod::pyCall(PyObject* args, PyObject* kwargs)
{
    ...
    if (pMethodDescription_->component() == MethodDescription::CLIENT) {
        ...
        if (!pMethodDescription_->addToServerStream(
              source, stream, sourceEntityID)) {
            return NULL;
        }
    }

    if (!delegate_->handleMethodCall(
          *pMethodDescription_, stream, sourceEntityID)) {
        PyErr_SetString(PyExc_ValueError, "Failed to call delegate method");
        return NULL;
    }
