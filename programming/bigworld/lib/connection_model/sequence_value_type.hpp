...
class SequenceValueType
{
  public:
    ...
    value_type& operator[](size_type index) { return sequence_[index]; }

    const_iterator begin() const { return sequence_.begin(); }

    const_iterator end() const { return sequence_.end(); }

    const_reference front() const { return sequence_.front(); }

    const_reference back() const { return sequence_.back(); }
    ...
    friend BinaryIStream& operator>>
      <>(BinaryIStream& stream, SequenceValueType& sequence);
...
template <typename T>
BinaryIStream& operator>>(BinaryIStream& stream, SequenceValueType<T>& sequence)
{
