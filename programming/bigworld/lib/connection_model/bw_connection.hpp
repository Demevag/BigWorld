...
class BWConnection
{
  public:
    ...
    BWENTITY_API void addLocalMove(EntityID           entityID,
                                   SpaceID            spaceID,
                                   EntityID           vehicleID,
    ...
    BWENTITY_API BinaryOStream* addServerMessage(EntityID entityID,
                                                 int      methodID,
    ...
    /**
     *	This method returns the last time a message was received from the
     *server.
     */
    ...
    BWENTITY_API void registerStreamDataHandler(BWStreamDataHandler& handler,
                                                uint16               streamID);
    BWENTITY_API void deregisterStreamDataHandler(BWStreamDataHandler& handler,
                                                  uint16 streamID);
  ...
  private:
     ...
     */
    virtual void enqueueLocalMove(EntityID           entityID,
                                  SpaceID            spaceID,
                                  EntityID           vehicleID,
                                  const Position3D&  localPosition,
                                  const Direction3D& localDirection,
                                  bool               isOnGround,
                                  const Position3D& worldReferencePosition) = 0;
    /**
     ...
     *			output parameter shouldDrop should be consulted. If shouldDrop
     *is true after this call, then the call has been deemed by the connection
     *to be silently dropped. If shouldDrop is false after this call, a stream
     *could not be set up for this call, and the calling code should treat this
     *as an error.
     */
    virtual BinaryOStream* startServerMessage(EntityID entityID,
                                              int      methodID,
                                              bool     isForBaseEntity,
                                              bool&    shouldDrop) = 0;
    /**
