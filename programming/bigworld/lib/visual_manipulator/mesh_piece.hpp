...
namespace VisualManipulator {
    ...
    /*
     *	This class implements a piece of a mesh that can be saved to a visual
     *file
     */
    class MeshPiece : public ReferenceCount
    {
      public:
        ...
        void   addTriangle(const BloatVertex& bv1,
        ...
        void save(DataSectionPtr     pVisualSection,
                  DataSectionPtr     pPrimSection,
                  bool               skinned,
                  const MaterialMap& materials);
        void saveBSP(DataSectionPtr    pPrimSection,
                     const BW::string& modelFilename) const;
      ...
      private:
        void writeIndices(DataSectionPtr    pPrimSection,
                          const BW::string& sectionName);
        void writeVertices(DataSectionPtr    pPrimSection,
                           const BW::string& sectionName,
                           bool              skinned);

        void writeUVStream(DataSectionPtr    pPrimSection,
                           const BW::string& sectionName) const;
        void writeColourStream(DataSectionPtr    pPrimSection,
                               const BW::string& sectionName) const;
