...
Hull::BiIndex::BiIndex()
  : i1(0u)
  , i2(0u)
{
}

Hull::BiIndex::BiIndex(uint i1, uint i2)
  : i1(i1)
  , i2(i2)
{
}
...
Hull::TriIndex::TriIndex(uint i1, uint i2, uint i3)
  : i1(i1)
  , i2(i2)
  , i3(i3)
{
...
void Hull::save(DataSectionPtr pSection)
{
    ...
    if (!this->makeConvex(pBaseMesh_->vertices(), faces)) {
        /*		::MessageBox(	0,
                                "Unable to create custom hull.  Custom hull most
           contain at least four " "non-colinear and non-coplanar vertices!",
                                "Custom hull error!",
                                MB_ICONERROR );*/
        return;
    }
    ...
    /*	if (portalsAdded != 0)
            ::MessageBox(	0,
                            "One or more portals defined using hull materials
       will not be exported.\n" "The hull was converted from concave to convex
       at that location!", "Portal error!", MB_ICONERROR );*/
}
 ...
 */
void Hull::extractPlanesAndPortals(VertexVector&         vv,
                                   BW::vector<TriIndex>& faces)
{
    planeEqs_.clear();
    portals_.clear();
    ...
    for (face_it = faces.begin(); face_it != faces.end(); face_it++) {
        ...
        PlaneEq peq(Vector3(vv[face_it->i3].pos),
    ...
    while (it != end) {
        Triangle& tri = *it++;
        PlaneEq   peq(Vector3(vv[tri.a].pos),
...
/**
 *	This method search for a face that matches the passed parameter in the
 *triangles_ vector.
 *
 *	@param face			The face being searched for.
 *	@returns			A pointer to the original triangle if its found, NULL
 *otherwise.
 */
Triangle* Hull::findOriginalTriangle(TriIndex& face)
{
    TriangleVector::iterator it;
    for (it = pBaseMesh_->triangles().begin();
         it != pBaseMesh_->triangles().end();
         it++) {
        for (int i = 0; i < 3; i++) {
            if (it->a == face[i] && it->b == face[i + 1] &&
                it->c == face[i + 2])
                return &*it;
        }
    }
...
/**
 *	This method creates a convex hull from a vector of vertices.  It will
 *determine if the vertices describe a convex or a concave hull.  If they
 *describe a concave hull, a convex hull that enclose this cancave hull is
...
bool Hull::makeConvex(VertexVector& vertices, BW::vector<TriIndex>& faces) const
{
    ...
    /*	if ( isConcave )
            ::MessageBox(	0,
                            "The custom hull is concave or has colinear/coplanar
       vertices.  The exported hull may have changed shape to make it convex!",
                            "Custom hull warning!",
                            MB_ICONWARNING );*/
...
/**
 *	This method creates the Maximal Volume Tetrahedron (MVT).  The MVT is the
 *maximum volume enclosed by any four points of the vertices point cloud.
 *
 ...
 *	@returns			MVT created
 */
bool Hull::mcCreateMaxTetrahedron(const VertexVector&   vertices,
                                  ...
                                  const float           epsilon) const
{
    ...
    uint minX_index, minY_index, minZ_index, maxX_index, maxY_index, maxZ_index;
    BW::vector<uint>::iterator it;
    for (it = verts.begin(); it != verts.end(); it++) {
        if (vertices[*it].pos[0] < minX) {
            minX       = vertices[*it].pos[0];
            minX_index = *it;
        }
        if (vertices[*it].pos[1] < minY) {
            minY       = vertices[*it].pos[1];
            minY_index = *it;
        }
        if (vertices[*it].pos[2] < minZ) {
            minZ       = vertices[*it].pos[2];
            minZ_index = *it;
        }
        if (vertices[*it].pos[0] > maxX) {
            maxX       = vertices[*it].pos[0];
            maxX_index = *it;
        }
        if (vertices[*it].pos[1] > maxY) {
            maxY       = vertices[*it].pos[1];
            maxY_index = *it;
        }
        if (vertices[*it].pos[2] > maxZ) {
            maxZ       = vertices[*it].pos[2];
            maxZ_index = *it;
        }
    }
    ...
    // Must be at least one vert left and must have a point that is not colinear
    // to line
...
/**
 *	This method removes a vertex from the verts list as well as all vertices
 *with the same position within the epsilon threshold.
 *
 *	@param vertIndex	The index of the vertex to remove from verts.
 *	@param vertices		The vertices to be enclosed in the convex hull.
 *	@param verts		The verts outside the convex hull.
 *	@param epsilon		The threshold determining if two vertices are
 *coincident.
 */
void Hull::mcRemoveVertex(const uint          vertIndex,
 ...
 */
bool Hull::mcRemoveInsideVertices(const VertexVector&         vertices,
                                  BW::vector<uint>&           verts,
...
/**
 *	This method returns the index of the next vertex to be add to the convex
 *hull.
 *
 *	@param vertices		The vertices to be enclosed in the convex hull.
 *	@param verts		The verts outside the convex hull.
 ...
 */
uint Hull::mcFindNextVert(const VertexVector&         vertices,
...
/**
 *	This method determines the edge horizon for vertIndex and expands the convex
 *hull from the edge horizon to vertIndex.
 *
 ...
 *	@param faces		The faces of the convex hull.
 */
void Hull::mcExpandHull(const uint            vertIndex,
                        const VertexVector&   vertices,
                        BW::vector<uint>&     verts,
                        BW::vector<TriIndex>& faces) const
{
    ...
    if (frontFacing.size() == 1) {
        ...
        faces.erase(faces.begin() + frontFacing[0],
                    faces.begin() + frontFacing[0] + 1);
        this->mcRemoveVertex(vertIndex, vertices, verts);
        this->mcAddFaces(vertIndex, horizonVerts, faces);
        return;
    }

    // We need to be able to handle the general case of many front facing
    // triangle, some of which do not touch the edge horizon, and others that
    // touch the edge horizon with one or more vertices

    // Any vertices that appears in both the front facing and back facing lists
    // must lie on the horizon.
        ...
        for (size_t j = 0; j < backFacing.size(); j++) {
            ...
            for (int k = 0; k < 3; k++) {
                if (faces[frontFacing[i]][k] == faces[backFacing[j]].i1 ||
                    ...
                    result = std::find(horizonVerts.begin(),
                                       horizonVerts.end(),
                                       faces[frontFacing[i]][k]);
    ...
    // Order the vertices that lie on the horizon in a clockwise order when
    // viewed from the vertIndex's position

    // Find a front facing triangle with two or more vertices on the edge
    // horizon and with a unique vert that no other front facing triangle is
    // indexing
    ...
    // This assert ensures that at least two of the face vertices are on the
    // horizon
    MF_ASSERT(count > 1 &&
              "Hull::mcExpandHull - Failed to find triangle on horizon edge")
    ...
    // Find the front facing triangles with two (or more) verts on the horizon,
    // one of which is sortedHorizonVerts.back() and the next that is not
    // already in sortedHorizonVerts.  Also Make sure this edge is and external
    // edge of the front facing triangles
            ...
            for (int k = 0; k < 3; k++) {
                ...
                if (faces[frontFacing[j]][k] == sortedHorizonVerts.back()) {
                    ...
                    for (size_t l = 0; l < sortedHorizonVerts.size(); l++) {
                        // Check if equal
                        if (faces[frontFacing[j]][k + 1] ==
                            sortedHorizonVerts[l]) {
                            inSortedList = true;
                            break;
                        }
                    }
                    ...
                    if (!inSortedList) {
                        ...
                        if (!this->mcIsInternalEdge(
                              faces,
                              frontFacing,
                              sortedHorizonVerts.back(),
                              faces[frontFacing[j]][k + 1])) {
                            sortedHorizonVerts.push_back(
                              faces[frontFacing[j]][k + 1]);
                            found = true;
                            break;
                        }
    ...
    for (size_t i = 0; i < frontFacing.size(); i++) {
        faces.erase(faces.begin() + frontFacing[i],
                    faces.begin() + frontFacing[i] + 1);
    }
...
bool Hull::mcIsInternalEdge(BW::vector<TriIndex>&   faces,
                            ...
                            const uint              vertIndex2) const
{
    ...
    for (size_t i = 0; i < frontFacing.size(); i++) {
        for (int j = 0; j < 3; j++) {
            if ((vertIndex1 == faces[frontFacing[i]][j] &&
                 vertIndex2 == faces[frontFacing[i]][j + 1]) ||
                (vertIndex2 == faces[frontFacing[i]][j] &&
                 vertIndex1 == faces[frontFacing[i]][j + 1]))
                edgeUseCount++;
        }
    }
 ...
 *	@param faces		The faces of the convex hull.
 */
void Hull::mcAddFaces(const uint              vertIndex,
                      const BW::vector<uint>& horizonVerts,
                      BW::vector<TriIndex>&   faces) const
{
    for (size_t i = 0; i < horizonVerts.size(); i++) {
        faces.push_back(TriIndex(horizonVerts[i],
                                 horizonVerts[(i + 1) % horizonVerts.size()],
                                 vertIndex));
    }
}

void Hull::generateHullFromBB(DataSectionPtr     pVisualSection,
...
namespace {
    ...
    {
        // remember, portal normals face the opposite direction to boundary
        // normals.
                                     ...
                                     BW::vector<PortalPtr>& portals)
{
    ...
    /*	// Warn the user that one or more of the exported portals were culled in
       some way if (portalCulled)
            ::MessageBox(	0,
                            "One or more portals were clipped because they
       extend past the hull/bounding box!\n" "This may lead to snapping issues
       inside WorldEditor.\n" "Hint: A custom hull can be used to resolve this
       issue.", "Portal clipping warning!", MB_ICONWARNING );

        // Warn the user that one or more portals is not being exported
        if (!eachOnBoundary)
            ::MessageBox(	0,
                            "One or more portals will not be exported since they
       are not on a boundary!", "Portal error!", MB_ICONERROR );*/
}
