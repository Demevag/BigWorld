...
namespace BW {
    ...
    bool ConvexHull::intersects(const Vector3& center,
                                float          radius,
                                uint16&        planeEqFlags) const
    {
        uint32 curPlaneBit = 1;
        for (const PlaneEq* p = planes_; p < (planes_ + numPlanes_); ++p) {
    ...
    namespace Despair { namespace Math {
    ...
    bool ConvexHull::intersects(const AABB& bb, uint16& planeEqFlags) const
    {
        ...
        while (curPlaneBit <= inFlags) {
            if (inFlags & curPlaneBit) {
                Vector3 neg;
                neg.x =
                  Despair::Math::SelectFloatGE0(p->normal().x, min.x, max.x);
                neg.y =
                  Despair::Math::SelectFloatGE0(p->normal().y, min.y, max.y);
                neg.z =
                  Despair::Math::SelectFloatGE0(p->normal().z, min.z, max.z);
                ...
                Vector3 pos;
                pos.x =
                  Despair::Math::SelectFloatGE0(p->normal().x, max.x, min.x);
                pos.y =
                  Despair::Math::SelectFloatGE0(p->normal().y, max.y, min.y);
                pos.z =
                  Despair::Math::SelectFloatGE0(p->normal().z, max.z, min.z);
