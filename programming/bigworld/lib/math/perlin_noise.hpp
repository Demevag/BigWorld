 ...
 *	number generator) which is just several Interpolated Noise functions at
 *different scales/frequencies added together.
 */
class PerlinNoise
{
  public:
    ...
    float sumHarmonics2(const Vector2& vec,
                        float          persistence,
                        float          frequency,
                        float          nIterations);
