...
namespace TestMatrix {
    ...
    bool testYawPitchRollConversion(const float* mats, size_t numFloats)
    {
        ...
        for (size_t i = 0; i < numMats; ++i) {
            ...
            if (!almostEqual(len0, 1.f) && !almostEqual(len1, 1.f) &&
                !almostEqual(len2, 1.f)) {
                // TRACE_MSG("Not normalised %f %f %f\n", len0, len1, len2 );
                // continue;
            }

            // It is possible that there is scale in the matrices which would
            // have been lost by converted to roll, pitch and yaw. Reapply the
    ...
    TEST(Matrix_testYawPitchRollConversion)
    {
        ...
        const size_t numHighlands =
          sizeof(matrixValuesHighlands) / sizeof(float);
        ...
        const size_t numAllModels =
          sizeof(matrixValuesAllModels) / sizeof(float);
