      ...
      public:
        ...
        TEST_SUB_FUNCTOR_OPERATOR
        {
            ...
            for (int pass = 0; pass < 3; ++pass) {
                CHECK_CLOSE(angle_, testA.getUnitVectorAngle(testB), EPS);
                CHECK_CLOSE(angle_, testA.getAngle(testB), EPS);
...
TEST(Vector3_angle)
{
    ...
    TEST_SUB_FUNCTOR(CheckAngles(Vector3::I, Vector3::I, 0.0f));
    TEST_SUB_FUNCTOR(CheckAngles(Vector3::I, Vector3::J, MATH_PI * 0.5f));
    TEST_SUB_FUNCTOR(CheckAngles(Vector3::J, Vector3::K, MATH_PI * 0.5f));
    TEST_SUB_FUNCTOR(CheckAngles(Vector3::J, -Vector3::J, MATH_PI));
...
TEST(Vector3_distance)
{
    ...
    TEST_SUB_FUNCTOR(CheckDistance(Vector3::I, Vector3::J, sqrt(2.0f)));
    TEST_SUB_FUNCTOR(CheckDistance(Vector3::I, Vector3::K, sqrt(2.0f)));
    TEST_SUB_FUNCTOR(CheckDistance(Vector3::J, Vector3::K, sqrt(2.0f)));
    TEST_SUB_FUNCTOR(CheckDistance(Vector3::J, Vector3::ZERO, 1.0f));

    TEST_SUB_FUNCTOR(CheckDistance(Vector3::ZERO, Vector3::ZERO, 0.0f));
    TEST_SUB_FUNCTOR(CheckDistance(Vector3::I, Vector3::I, 0.0f));
    TEST_SUB_FUNCTOR(CheckDistance(Vector3::J, Vector3::J, 0.0f));
    TEST_SUB_FUNCTOR(CheckDistance(Vector3::J, -Vector3::J, 2.0f));
    TEST_SUB_FUNCTOR(CheckDistance(Vector3::K, Vector3::K, 0.0f));
    TEST_SUB_FUNCTOR(CheckDistance(-Vector3::K, Vector3::K, 2.0f));

    const Vector3 testDistVect(10.0f, 5.0f, 100.0f);
    TEST_SUB_FUNCTOR(CheckDistance(Vector3::ZERO,
                                   testDistVect,
                                   sqrt(Math::squared(testDistVect[0]) +
...
TEST(Vector3_print)
{
    ...
    const Vector3 testVect(-std::numeric_limits<float>::max(),
    ...
    // Stream insertion
    {
        ...
        RETURN_ON_FAIL_CHECK(bw_snprintf(compareStr,
                                         sizeof(compareStr) - 1,
                                         "(%1.1f, %1.1f, %1.1f)",
                                         testVect.x,
                                         testVect.y,
