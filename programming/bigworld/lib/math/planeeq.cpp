...
LineEq PlaneEq::intersect(const PlaneEq& slice) const
{
    ...
    if ((ir - intersectPoint3D).length() > 1.f) {
        ...
        dprintf("  intersectPoint2D (%f,%f)\n",
                ...
                intersectPoint2D.y);
        dprintf(
          "  Reconstituted intersect point (%f,%f,%f)\n", ir.x, ir.y, ir.z);
        Vector3 xdir, ydir;
...
void PlaneEq::basis(Vector3& xdir, Vector3& ydir) const
{
    ...
    float fabses[3] = { float(fabs(normal_.x)),
                        float(fabs(normal_.y)),
                        float(fabs(normal_.z)) };
    int   bi        = fabses[0] > fabses[1] ? (fabses[0] > fabses[2] ? 0 : 2)
...
Vector2 PlaneEq::project(const Vector3& point) const
{
    ...
    return Vector2(xdir.dotProduct(offset), ydir.dotProduct(offset));
}
