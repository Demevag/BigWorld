...
class PlaneEq
{
  public:
    ...
    INLINE void init(const Vector3& normal, const float d);
    ...
    INLINE float intersectRayHalf(const Vector3& source,
                                  float          normalDotDir) const;
    INLINE float intersectRayHalfNoCheck(const Vector3& source,
