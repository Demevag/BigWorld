...
namespace BW {
    ...
    class DynamicSceneProvider::DebugDraw
    {
        ...
        static void draw(DynamicSceneProvider& provider)
        {
            if (s_debug_drawCollisionBBs_) {
                drawBoundingBoxes(provider.objectHandles_,
                                  provider.objectCollisionBBs_,
                                  COLLISION_BB_COLOUR);
            }
            if (s_debug_drawVisibilityBBs_) {
                drawBoundingBoxes(provider.objectHandles_,
                                  provider.objectVisibilityBBs_,
                                  VISIBILITY_BB_COLOUR);
            }
            if (s_debug_drawCollisionTreeLevel_) {
                drawOctree(provider.collisionTree_,
                           s_debug_drawCollisionTreeLevel_ - 1,
                           COLLISION_TREE_BB_COLOUR);
            }
            if (s_debug_drawVisibilityTreeLevel_) {
                drawOctree(provider.visibilityTree_,
                           s_debug_drawVisibilityTreeLevel_ - 1,
                           VISIBILITY_TREE_BB_COLOUR);
            }
        }
      ...
      private:
        ...
        static void drawBoundingBoxes(
          const ObjectHandleCollection& handles,
          const BoundingBoxCollection&  boxCollection,
          uint32                        colour)
        {
            for (ObjectHandleCollection::const_iterator iter = handles.begin();
                ...
                SpaceDebugDraw::drawBoundingBox(boxCollection[handle.index_],
                                                colour);
            }
        }
    ...
    size_t DynamicSceneProvider::DebugDraw::s_debug_drawVisibilityTreeLevel_ =
      0;
    ...
    void DynamicSceneProvider::removeObject(const DynamicObjectHandle& handle)
    {
        ...
        pObjectChangeView_->notifyObjectsRemoved(
          this, &objects_[objectLookup_[handle.index_]], 1);
    ...
    class DynamicSceneProvider::Detail
    {
      public:
        ...
        static void insertObjectNode(DynamicObjectHandle    handle,
                                     ...
                                     Octree&                tree,
                                     OctreeContents&        contents,
                                     Octree::NodeIndex*     newIndex,
                                     bool                   updateHeirachy)
        {
            Octree::NodeIndex nodeIndex =
              tree.insert(bbCollection[handle.index_]);
          ...
          Octree::RuntimeNodeDataReferences& nodeReferences)
        {
            ...
            for (Octree::RuntimeNodeDataReferences::iterator iter =
                   nodeReferences.begin();
                 iter != nodeReferences.end();
                 ++iter) {
                ...
                for (OctreeNodeContents::iterator itemIter = contents.begin();
                     itemIter != contents.end();
                     ++itemIter) {
                    const DynamicObjectHandle& objHandle = *itemIter;
                    const SceneObject&         object =
                      provider
                        .objects_[provider.objectLookup_[objHandle.index_]];
    ...
    void DynamicSceneProvider::onPostTick()
    {
        ...
        for (ObjectHandleCollection::iterator iter = objectHandles_.begin();
            ...
            if (objectDirtyFlags_[handle.index_]) {
                ...
                updatedObjects.push_back(
                  objects_[objectLookup_[handle.index_]]);
            }
        }

        // Now insert all dynamic objects into these trees again and update
        // their tree info
        for (ObjectHandleCollection::iterator iter = objectHandles_.begin();
             iter != objectHandles_.end();
             ++iter) {
            ...
            DynamicObjectStorageInfo& treeInfo =
              objectStorageInfo_[handle.index_];
            Detail::insertObjectNode(handle,
                                     objectCollisionBBs_,
                                     collisionTree_,
                                     collisionTreeContents_,
                                     &treeInfo.collisionNode_,
                                     false);
        }
        ...
        for (ObjectHandleCollection::iterator iter = objectHandles_.begin();
            ...
            DynamicObjectStorageInfo& treeInfo =
              objectStorageInfo_[handle.index_];
            Detail::insertObjectNode(handle,
                                     objectVisibilityBBs_,
                                     visibilityTree_,
                                     visibilityTreeContents_,
                                     &treeInfo.visibilityNode_,
                                     false);
        }
        ...
        pObjectChangeView_->notifyObjectsChanged(
          this, &updatedObjects.front(), updatedObjects.size());
        numDirtyObjects_ = 0;
    }
                                       ...
                                       CollisionState&    cs) const
    {
        ...
        return Detail::collideReferences(
          *this, source, extent, sp, cs, nodeReferences);
    }
                                           ...
                                           IntersectionSet& intersection) const
    {
        ...
        for (Octree::RuntimeNodeDataReferences::iterator iter =
            ...
            OctreeNodeContents contents = visibilityTreeContents_[nodeRef];
            for (OctreeNodeContents::iterator itemIter = contents.begin();
                 itemIter != contents.end();
                 ++itemIter) {
                const DynamicObjectHandle& objHandle = *itemIter;
    ...
    void DynamicSceneProvider::resizeScene(const AABB& sceneBB)
    {
        ...
        float maxDimension = std::max(
          sceneBB.width(), std::max(sceneBB.height(), sceneBB.depth()));
        ...
        for (OctreeContents::iterator iter = oldCollisionContents.begin();
             iter != oldCollisionContents.end();
             ++iter) {
            OctreeContents::ValueType& nodeContents = *iter;
            for (OctreeContents::ValueType::iterator objIter =
                   nodeContents.begin();
                 objIter != nodeContents.end();
                 ++objIter) {
        ...
        for (OctreeContents::iterator iter = oldVisibilityContents.begin();
             iter != oldVisibilityContents.end();
             ++iter) {
            OctreeContents::ValueType& nodeContents = *iter;
            for (OctreeContents::ValueType::iterator objIter =
                   nodeContents.begin();
    ...
    DynamicObjectHandle DynamicSceneProvider::insertObject_internal(
      const SceneObject& object,
      const Matrix&      worldXform,
      const AABB&        collisionBB,
      const AABB&        visibilityBB)
      ...
      const DynamicObjectHandle& handle)
    {
        ...
        Detail::removeObjectNode(handle,
                                 treeInfo.collisionNode_,
                                 collisionTree_,
                                 collisionTreeContents_);
        Detail::removeObjectNode(handle,
        ...
        // Leave the data in the slots behind since they'll be used for
        // something else soon.
