...
{
    for (SpaceMap::iterator it = spaces_.begin(); it != spaces_.end(); ++it) {
        it->second->tick(dTime);
    }

    for (SpaceMap::iterator it = localSpaces_.begin(); it != localSpaces_.end();
         ++it) {
        it->second->tick(dTime);
    }
}
...
void SpaceManager::addSpace(ClientSpace* pSpace)
{
    ...
    SpaceMap& map = pSpace->id() >= LOCAL_SPACE_START ? localSpaces_ : spaces_;
...
SpaceManager::SpaceCreatedEvent::EventDelegateList& SpaceManager::spaceCreated()
{
