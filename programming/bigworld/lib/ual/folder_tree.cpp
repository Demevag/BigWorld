...
int FolderTree::addBitmap(HBITMAP image)
{
    ...
    if (!unusedImages_.empty()) {
        ...
        mask.ReleaseDC();
        imgList_.Replace(
          i, CBitmap::FromHandle(image), CBitmap::FromHandle(mask));
        return i;
    } else
...
void FolderTree::removeImage(int index)
{
    ...
    if (index >= 0 && index >= firstImageIndex_ &&
        index < imgList_.GetImageCount())
 ...
 */
void FolderTree::getGroupIcons(int  group,
                               int& icon,
                               int& iconSel,
                               bool expandable)
{
 ...
 */
void FolderTree::setExtensionsIcons(BW::wstring extensions,
                                    HICON       icon,
                                    HICON       iconSel)
{
    BW_GUARD;
 ...
 *	@param iconSel	Returns the index of the icon for when it is selected.
 */
void FolderTree::getExtensionIcons(const BW::wstring& name,
                                   int&               icon,
                                   int&               iconSel)
{
    BW_GUARD;
 ...
 */
BW::wstring FolderTree::getVFolderOrder(const BW::wstring orderStr,
                                        HTREEITEM         item)
{
    BW_GUARD;
 ...
 */
static int CALLBACK FolderTreeOrderStrFunc(LPARAM lParam1,
                                           LPARAM lParam2,
                                           LPARAM lParamSort)
{
    BW_GUARD;
...
void FolderTree::moveVFolder(VFolderPtr vf1, VFolderPtr vf2)
{
    ...
    if (!vf2)
        ...
        bool added = false;
        for (BW::vector<BW::wstring>::iterator i = oi.begin(); i != oi.end();
             ++i)
            if ((*i) == vf2->getName()) {
...
void FolderTree::refreshVFolder(VFolderPtr vfolder)
{
    ...
    while (sel) {
        if (sel == item)
            break; // found the vfolder, so the vfolder's subtree includes the
                   // selected item
    ...
    if (!sel)
        lastSelected
          .clear(); // selected item not in the vfolder's subtree, so don't care
    ...
    if (!lastSelected.empty()) {
        // the selected item was inside the vfolder's tree before
        // reconstruction, so reselect
        child                 = item;
        HTREEITEM deepestNull = item;
        for (BW::vector<BW::wstring>::reverse_iterator i =
               lastSelected.rbegin();
             i != lastSelected.rend();
             ++i) {
        ...
        if (deepestNull)
            SelectItem(
              deepestNull); // select the deepest level of the tree found (
                            // hopefully, the old selected item )
    }
 ...
 */
VFolderPtr FolderTree::getVFolder(const BW::wstring& name,
                                  bool               strict,
                                  HTREEITEM          item)
{
    BW_GUARD;
 ...
 *	@return			Desired virtual folder, or NULL of none was found.
 */
VFolderPtr FolderTree::getVFolderCustom(ItemTestCB test,
                                        void*      testData,
                                        bool       strict,
                                        HTREEITEM  item)
{
 ...
 *			lParam1 > lParam2, zero if they are equal.
 */
static int CALLBACK FolderTreeCompareFunc(LPARAM lParam1,
                                          LPARAM lParam2,
                                          LPARAM lParamSort)
{
    BW_GUARD;
    ...
    return wcsicmp(data1->assetInfo().text().c_str(),
                   data2->assetInfo().text().c_str());
}
...
void FolderTree::buildTree(HTREEITEM parentItem, VFolderProviderPtr provider)
{
    ...
    while (data = provider->getNextChild(*thumbnailManager_, img)) {
        ...
        if (!img.IsNull()) {
            ...
            getGroupIcons(
              data->getGroup(), icon, iconSel, data->getExpandable());
        ...
        if (!dontAdd) {
            HTREEITEM item = InsertItem(
              data->assetInfo().text().c_str(), icon, iconSel, parentItem);
            setItemData(item, data);
            if (getVFolder(data.getObject())->getSortSubFolders())
                sortSubTree(parentItem);
            if (data->getExpandable())
                InsertItem(L"***dummychild***",
                           0,
                           0,
                           item); // later removed when expanded.
        }
    }
    if (parentData && parentData->isVFolder()) {
        ...
        if (items) {
            ...
            for (XmlItemVec::iterator i = items->begin(); i != items->end();
                 ++i) {
                int icon    = itemIcon_;
                ...
                thumbnailManager_->create((*i).assetInfo().thumbnail(),
                                          ...
                                          true);
                data =
                  new VFolderItemData(provider, (*i).assetInfo(), 0, false);
 ...
 */
VFolderPtr FolderTree::addVFolder(const BW::wstring& displayName,
                                  ...
                                  bool               subVFolders)
{
    ...
    if (show) {
        ...
        if (expandable)
            InsertItem(
              L"***dummychild***", 0, 0, item); // later removed when expanded.
    }

    VFolderPtr newVFolder = new VFolder(parent,
                                        displayName,
                                        item,
                                        provider,
                                        expandable,
 ...
 */
void FolderTree::removeVFolder(const BW::wstring& displayName,
                               HTREEITEM          curItem)
{
    BW_GUARD;
...
void FolderTree::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    ...
    if (nChar == VK_DELETE && eventHandler_) {
        ...
        if (item)
            eventHandler_->folderTreeItemDelete(
              (VFolderItemData*)GetItemData(item));
        return;
    }
...
void FolderTree::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    UINT hitflags;
    if (eventHandler_ && item && item == GetSelectedItem() &&
        item == HitTest(point, &hitflags))
...
void FolderTree::OnSelChanged(NMHDR* pNotifyStruct, LRESULT* result)
{
    ...
    if (eventHandler_) {
        HTREEITEM item = GetSelectedItem();
        if (item)
            eventHandler_->folderTreeSelect(
              (VFolderItemData*)GetItemData(item));
    }
}
...
void FolderTree::OnBeginDrag(NMHDR* pNMHDR, LRESULT* pResult)
{
    ...
    *pResult = 0;
    ...
    bw_safe_delete(dragImage_);
    dragImgList_ =
      CreateDragImage(item.hItem); // get the image list for dragging
    ...
    dragging_ = true;
    eventHandler_->folderTreeStartDrag(
      (VFolderItemData*)GetItemData(item.hItem));
}
...
void FolderTree::OnLButtonDblClk(UINT nFlags, CPoint point)
{
    ...
    if (item && (hitflags & TVHT_ONITEM))
        eventHandler_->folderTreeDoubleClick(
          (VFolderItemData*)GetItemData(item));
}
...
bool FolderTree::updateItem(const AssetInfo& assetInfo, HTREEITEM item)
{
    ...
    if (item != TVI_ROOT) {
        ...
        if (data) {
            ...
            if (data &&
                (assetInfo.text() == L"" ||
                 data->assetInfo().text() == assetInfo.text()) &&
                wcsicmp(infLongText.c_str(), assetInfoLongText.c_str()) == 0) {
                // Found. Just refresh the it's thumbnail
                if (img.IsNull())
                    data->getProvider()->getThumbnail(
                      *thumbnailManager_, data, img);
                int icon;
                int iconSel;
                if (!img.IsNull()) {
                    icon    = addBitmap((HBITMAP)img);
