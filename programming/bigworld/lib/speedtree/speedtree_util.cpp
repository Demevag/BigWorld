...
namespace speedtree {
     ...
     */
    void createTreeDefName(const BW::StringRef& filename,
                           uint                 seed,
                           char*                dst,
                           size_t               dstSize)
    {
    ...
    //----------------------------------------------------------------------------------------------
    float LineInBox(const Vector3& vMin,
                    const Vector3& vMax,
                    ...
                    const Vector3& vEnd)
    {
        ...
        for (int iBound = 0; iBound < 2; iBound++) {
            for (int iAxis = 0; iAxis < 3; iAxis++) {
                if (fabsf(vDir[iAxis]) < 0.001f)
                    continue;
                float t =
                  (vBounds[iBound][iAxis] - vStart[iAxis]) / vDir[iAxis];

                if (t > 0 && t < 1) {
                    Vector3 v         = vStart + t * vDir;
                    int     iNextAxis = (iAxis + 1) % 3;
                    int     iPrevAxis = (iAxis + 2) % 3;
                    if (v[iNextAxis] > vBounds[0][iNextAxis] &&
                        v[iNextAxis] < vBounds[1][iNextAxis] &&
                        v[iPrevAxis] > vBounds[0][iPrevAxis] &&
                        v[iPrevAxis] < vBounds[1][iPrevAxis]) {
                        tMin  = min(tMin, t);
                        tMax  = max(tMax, t);
                        bClip = true;
                    }
    ...
    //----------------------------------------------------------------------------------------------
    float LineInEllipsoid(const Vector3& vSize,
                          const Vector3& vStart,
                          const Vector3& vEnd)
    {
        // vStart - v0	vDir - D
    ...
    //----------------------------------------------------------------------------------------------
    void decomposeMatrix(const Matrix& iMat,
                         Quaternion&   oRotQuat,
