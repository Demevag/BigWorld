...
namespace {
    ...
    struct SemitransparentTree
    {
        ...
        SemitransparentTree(TSpeedTreeType*           type,
                            TSpeedTreeType::DrawData* data)
          : m_type(type)
          , m_data(data)
        {
        }
    ...
    class SpeedTreeMaterialsMapConstant : public Moo::EffectConstantValue
    {
      public:
        ...
        {
            m_texture = Moo::rc().createTexture(2,
                                                c_maxTreeMaterialTypes * 4,
                                                1,
                                                0,
                                                D3DFMT_A8R8G8B8,
                                                D3DPOOL_MANAGED,
                                                "Speedtree/MaterialsMap");
        ...
        void update()
        {
            ...
            //--    Note: Here we make assumption that 32 unique types per space
            // is more than enough.
            uint8 materials[c_maxTreeMaterialTypes * 4][8];
            {
                //-- Note: make the type coefficient black to see errors (if we
                // exceed available
                //--       materials count) as short as possible.
                memset(
                  materials, 0, c_maxTreeMaterialTypes * 4 * 8 * sizeof(uint8));

                for (TSpeedTreeType::TreeTypesMap::const_iterator it =
                       TSpeedTreeType::s_typesMap_.begin();
                    ...
                    if (idx >= c_maxTreeMaterialTypes) {
                        WARNING_MSG("Too many unique tree types. Current count "
                                    "of unique types %d",
                                    TSpeedTreeType::s_typesMap_.size());
                        idx = (c_maxTreeMaterialTypes - 1);
                    }

                    const float* const sptMats[] = { spt.GetBranchMaterial(),
                    ...
                    for (uint i = 0; i < 4; ++i) {
                        //-- diffuse.
                        materials[idx * 4 + i][0] =
                          static_cast<uint8>(sptMats[i][0] * 255);
                        materials[idx * 4 + i][1] =
                          static_cast<uint8>(sptMats[i][1] * 255);
                        materials[idx * 4 + i][2] =
                          static_cast<uint8>(sptMats[i][2] * 255);
                        materials[idx * 4 + i][3] =
                          (i > 1) ? static_cast<uint8>(lightAdj * 255) : 0;

                        //-- ambient.
                        materials[idx * 4 + i][4] =
                          static_cast<uint8>(sptMats[i][3] * 255);
                        materials[idx * 4 + i][5] =
                          static_cast<uint8>(sptMats[i][4] * 255);
                        materials[idx * 4 + i][6] =
                          static_cast<uint8>(sptMats[i][5] * 255);
            ...
            //-- 2. upload regenerated data into texture.
            {
                ...
                if (Moo::rc().usingD3DDeviceEx()) {
                    ...
                    pLockTex =
                      Moo::rc().createTexture(sd.Width,
                                              sd.Height,
                                              m_texture->GetLevelCount(),
                                              0,
                                              sd.Format,
                                              D3DPOOL_SYSTEMMEM);
                } else
                ...
                if (Moo::rc().usingD3DDeviceEx())
                    Moo::rc().device()->UpdateTexture(pLockTex.pComObject(),
        ...
        {
            HRESULT hr =
              pEffect->SetTexture(constantHandle, m_texture.pComObject());
            return SUCCEEDED(hr);
        }
    ...
    //-- looks slightly ugly because we lose auto life-time control. But it's
    // lesser of two evils.
    //-- Initialization of the smart pointer performed in
    // SpeedTreeRenderer::init and reseting in fini().
...
namespace speedtree {
    ...
    Moo::ERenderingPassType SpeedTreeRenderer::s_curRenderingPass_ =
    ...
    class MFSpeedTreeAllocator : public CSpeedTreeAllocator
    {
        ...
        {
            char* pAllocated =
              new char[BlockSize + sizeof(Header) + Alignment - 1];
            char* p = pAllocated;
            p += sizeof(Header);

            void* pAligned =
              (char*)(((uintptr_t)p + Alignment - 1) & ~(Alignment - 1));
      ...
      , windOffset_(0.0f)
    {
        ...
        if (!s_isWatcherAdded) {
            ...
            MF_WATCH("Visibility/Speed_Tree",
                     isVisible,
                     Watcher::WT_READ_WRITE,
                     "Is speed tree visible.");
        }
#endif
    }
    ...
    void SpeedTreeRenderer::init()
    {
        ...
        DataSectionPtr section =
          BWResource::instance().openSection(s_speedTreeXML);
        ...
        //-- ToDo: currently BillboardOptimiser::init depends on
        // SpeedTreeRendererCommon. Fix this.
#if ENABLE_BB_OPTIMISER
        ...
        MF_WATCH("SpeedTree/Optimise billboards",
                 TSpeedTreeType::s_optimiseBillboards_,
                 Watcher::WT_READ_WRITE,
                 "Toggles billboards optimisations on/off (requires batching "
                 "to be on)");
#endif
        ...
        //-- register shared auto-constant.
        {
            ...
            Moo::rc().effectVisualContext().registerAutoConstant(
              ...
              "SpeedTreeMaterials",
              g_speedTreeMaterialsConstant);
        }

        MF_WATCH("SpeedTree/Envirominder Lighting",
                 TSpeedTreeType::s_enviroMinderLight_,
                 Watcher::WT_READ_WRITE,
        ...
        MF_WATCH("SpeedTree/Play animation",
                 TSpeedTreeType::s_playAnimation_,
                 Watcher::WT_READ_WRITE,
                 "Toggles wind animation on/off");

        TSpeedTreeType::s_leafRockFar_ =
          section->readFloat("leafRockFar", TSpeedTreeType::s_leafRockFar_);
        ...
        TSpeedTreeType::s_lodMode_ =
          section->readFloat("lodMode", TSpeedTreeType::s_lodMode_);
        MF_WATCH("SpeedTree/LOD Mode",
                 TSpeedTreeType::s_lodMode_,
                 Watcher::WT_READ_WRITE,
        ...
        TSpeedTreeType::s_lodNear_ =
          section->readFloat("lodNear", TSpeedTreeType::s_lodNear_);
        MF_WATCH("SpeedTree/LOD near",
                 TSpeedTreeType::s_lodNear_,
                 Watcher::WT_READ_WRITE,
                 "Global LOD near value (distance for maximum level of detail");

        TSpeedTreeType::s_lodFar_ =
          section->readFloat("lodFar", TSpeedTreeType::s_lodFar_);
        ...
        TSpeedTreeType::s_lod0Yardstick_ =
          section->readFloat("lod0Yardstick", TSpeedTreeType::s_lod0Yardstick_);
        MF_WATCH("SpeedTree/LOD-0 Yardstick",
                 TSpeedTreeType::s_lod0Yardstick_,
                 Watcher::WT_READ_WRITE,
                 "Trees taller than the yardstick will LOD to billboard "
                 "farther than LodFar. "
                 "Trees shorter than the yardstick will LOD to billboard "
                 "closer than LodFar.");

        TSpeedTreeType::s_lod0Variance_ =
          section->readFloat("lod0Variance", TSpeedTreeType::s_lod0Variance_);
        MF_WATCH(
          "SpeedTree/LOD-0 Variance",
          TSpeedTreeType::s_lod0Variance_,
          Watcher::WT_READ_WRITE,
          "How much will the lod level vary depending on the size of the tree");

        MF_WATCH("SpeedTree/Counters/Unique",
                 TSpeedTreeType::s_uniqueCount_,
        ...
        TSpeedTreeType::s_speedWindFile_ =
          section->readString("speedWindINI", TSpeedTreeType::s_speedWindFile_);

        MF_WATCH("Render/Performance/DrawPrim SpeedTreeRenderer",
                 s_enableDrawPrim,
                 Watcher::WT_READ_WRITE,
                 "Allow SpeedTreeRenderer to call drawIndexedPrimitive().");

        MF_WATCH("SpeedTree/BB mipmap bias",
                 BillboardOptimiser::s_mipBias_,
                 Watcher::WT_READ_WRITE,
    ...
    void SpeedTreeRenderer::loadSpaceCfg(const DataSectionPtr& spaceCfg)
    {
        ...
        TSpeedTreeType::s_lodNear_ =
          cfg->readFloat("lodNear", TSpeedTreeType::s_lodNear_);
        TSpeedTreeType::s_lodFar_ =
          cfg->readFloat("lodFar", TSpeedTreeType::s_lodFar_);
    }
     ...
     *
     *  @note               will throw std::runtime_error if file can't be
     * loaded.
     */
    void SpeedTreeRenderer::load(const char*   filename,
                                 uint          seed,
                                 const Matrix& world)
    {
        BW_GUARD;
    ...
    void SpeedTreeRenderer::tick(float dTime)
    {
        BW_GUARD;
#ifdef EDITOR_ENABLED
        // Checking for 0.f time elapsed from previous frame. (means game is
        // paused) To generate the same animation consistently for automated
        // testing screenshots.
        s_paused = dTime == 0.f;
#endif
     ...
     *
     */
    void SpeedTreeRenderer::beginFrame(EnviroMinder*           envMinder,
                                       Moo::ERenderingPassType type,
                                       const Matrix&           lodCamera)
    {
        BW_GUARD;
        ...
        //-- check ready status. BigWorld shader system requires that even if
        // shader compilation and
        //-- loading is performed in background final step must be done in the
        // main thread. So before
    ...
    void TSpeedTreeType::drawSemitransparentTrees(bool skipDrawing)
    {
        ...
        for (TreeTypesMap::iterator it = s_typesMap_.begin();
             it != s_typesMap_.end();
             ++it) {
            TSpeedTreeType& rt = *it->second;
            DrawDataVector& dd = rt.drawDataCollector_.m_semitransparentTrees;

            //-- iterate over the whole set of instances of the current tree
            // type.
        ...
        for (SemitransparentTrees::iterator it = trees.begin();
             it != trees.end();
             ++it) {
            ...
            for (uint i = 0; i < SPECIAL_DRAWING_STATE_COUNT; ++i) {
                ESpecialDrawingState state =
                  static_cast<ESpecialDrawingState>(i);

                rt.customDrawLeaf(g_commonRenderData->specialDrawingState(
                                    state, TREE_PART_LEAF),
                                  dd);
                rt.customDrawFrond(g_commonRenderData->specialDrawingState(
                                     state, TREE_PART_FROND),
                                   dd);
                rt.customDrawBranch(g_commonRenderData->specialDrawingState(
                                      state, TREE_PART_BRANCH),
                                    dd);
                rt.customDrawBillBoard(g_commonRenderData->specialDrawingState(
                                         state, TREE_PART_BILLBOARD),
                                       dd);
            }
        }
    ...
    bool SpeedTreeRenderer::tryToUpdateBBOptimizer(const Matrix& worldTransform)
    {
        ...
        if (TSpeedTreeType::s_optimiseBillboards_ &&
            treeType_->bbTreeType_ != -1 && bbOptimiser_.exists()) {
            ...
            if (drawData_.lod_.billboardDraw_) {
                ...
                if (treeID_ != -1) {
                    ...
                    if (bbOptimiser_->isFull()) {
                        bbOptimiser_ = BillboardOptimiser::retrieve(
                          worldTransform.applyToOrigin());
                    }

                    //-- If tree have not yet been registered with the bb
                    // optimizer (and the
                    //-- billboard vertices have already been calculated), do it
                    // now.
                    treeID_ = bbOptimiser_->addTree(
                      treeType_->bbTreeType_, worldTransform, alphaValue);
                }
    ...
    void SpeedTreeRenderer::draw(const Matrix& worldTransform)
    {
        ...
        //-- ToDo: Think about using for real rendering list of only visible
        // tree types. Means tree types
        //--       for which at least one instance is visible from the current
        // view point. For the real big
        //--       world this may be good optimization but for relatively small
        // it may be not, because
        //--       almost every time we will see almost all the tree types.

        //-- skip tree rendering in case if we don't start speedtree frame. This
        // may happen either
        //-- we didn't call beginFrame() method or some preparation in that
    ...
    world.preMultiply(rot);
        ...
        if (TSpeedTreeType::s_drawBoundingBox_) {
            ...
            Geometrics::wireBox(treeType_->treeData_.boundingBox_,
                                Moo::Colour(1, 0, 0, 0));
            Moo::rc().pop();
        }
        ...
        //-- collect tree's rendering data.
        {
            ...
            bool semitransparent =
              (s_curRenderingPass_ == Moo::RENDERING_PASS_COLOR)
                ? TSpeedTreeType::isTreeSemiTransparent(drawData_)
                : false;
            bool useBBOptimizer =
              semitransparent ? false : tryToUpdateBBOptimizer(world);

            treeType_->collectTreeDrawData(
              drawData_, !useBBOptimizer, semitransparent);
        }
    }

    void SpeedTreeRenderer::generateTextureUsage(
    ...
    /**
     *  Notifies this tree that it's position has been updated. Calling this
     * every time the transform of a tree changes is necessary because some
     * optimizations assume the tree to be static most of the time. Calling this
     * allows the tree to refresh all cached data that becomes obsolete whenever
     * a change occours.
     *
     *  @param  updateBB    true if the tree should update the bb optimizer.
     *                      Default is true.
     */
    void SpeedTreeRenderer::resetTransform(const Matrix& world,
                                           bool          updateBB /*=true*/)
    {
        BW_GUARD;
     ...
     *
     *  @param  newValue    true if the environminder should be used as source
     * of lighing. false is Moo's light containers should be used.
     *
        ...
        //--
        {
            ...
            while (renderIt != TSpeedTreeType::s_typesMap_.end()) {
                if (renderIt->second->refCount() == 0) {
                    s_materialHandles_.getBack(
                      renderIt->second->materialHandle_);
     ...
     *  the rendering thread (e.g., registering a new tree type into the
     * billboard optimiser).
     *
     ...
     *  moving them to the tree-types-map, where they will be ready to be
     * rendered.
     *
    ...
    bool TSpeedTreeType::doSyncInit()
    {
        ...
        if (initIt != initEnd) {
            ...
            createTreeDefName((*initIt)->filename_,
                              (*initIt)->seed_,
                              treeDefName,
    ...
    /**
     *  Deletes all tree type objects in the pending initialisation list
     * (static).
     */
    void TSpeedTreeType::clearDXResources(bool forceDelete)
    {
        ...
        if (forceDelete) {
            ...
            for (TreeTypesMap::const_iterator it = s_typesMap_.begin();
                 it != s_typesMap_.end();
                 ++it) {
     ...
     */
    void TSpeedTreeType::computeDrawData(const Matrix& world,
                                         float         windOffset,
                                         DrawData&     oDrawData)
    {
        BW_GUARD;
        ...
        //-- recalculate draw data if needed.
        {
...
#endif
            {
                ...
                oDrawData.m_instData.m_uniformScale =
                  0.333f * (scale.x + scale.y + scale.z);
        ...
        float fTreeHeight = treeData_.boundingBox_.height();
        {
            ...
            //-- ToDo (b_sviglo): Try to optimize. Currently it takes about 1.5
            // ms when shadows is enabled.
            ...
            //-- dynamic LOD calculation based on global lod parameters. Means
            // it's the same for all trees.
            else if (s_lodMode_ == -2.0f) {
                //-- calculate lod distance.
                Vector3 distVector = treePos - eyePos;
                fDistance =
                  g_commonRenderData->applyZoomFactor(distVector.length());
                float lodNear = s_lodNear_;
                float lodFar  = s_lodFar_;
                g_commonRenderData->applyLodBias(lodNear, lodFar);
                float lodLevel =
                  1.0f - (fDistance - lodNear) / (lodFar - lodNear);

                float lodVari =
                  s_lod0Variance_ *
                  (-0.5f + (scaleZ * treeData_.boundingBox_.height()) /
                             s_lod0Yardstick_);
                lodLevel = Math::clamp(0.0f,
                                       lodLevel + lodVari,
                                       max(Moo::rc().lodZoomFactor(), 0.25f));

                //-- ToDo: reconsider.
                // Manually rounding very small lod values to avoid problems
                // in the speedtree lod calculation.(avoiding problem, not
                // fixing)
                if (lodLevel < 0.0001f) {
                    lodLevel = 0.0f;
                }
    ...
    speedTree->SetLodLevel(lodLevel);
        ...
        //-- now calculate transparency of trees but do that only for the main
        // color pass.
        if (SpeedTreeRenderer::s_curRenderingPass_ ==
            Moo::RENDERING_PASS_COLOR) {
            ...
            if (SpeedTreeRenderer::s_hideClosest &&
                sqrDistToEye < SpeedTreeRenderer::s_sqrHideRadius) {
                float        fDist      = sqrtf(sqrDistToEye);
                const float& hideRadius = SpeedTreeRenderer::s_hideRadius;
                const float& hideAlphaRadius =
                  SpeedTreeRenderer::s_hideRadiusAlpha;

                alpha =
                  (fDist - hideAlphaRadius) / (hideRadius - hideAlphaRadius);
            ...
            //-- if tree ...
            if (SpeedTreeRenderer::s_pTarget &&
                SpeedTreeRenderer::s_enableTransparency) {
                ...
                if (!treeData_.isBush && oDrawData.lod_.model3dDraw_ &&
                    fTreeHeight * scaleY > 6.0f &&
                    (fDistance - fDiag) <
                      SpeedTreeRenderer::s_fToTargetDistance) {
                    Matrix wInv = world;
                    wInv.preTranslateBy(
                      Vector3(0.0f, fTreeHeight * 0.5f, 0.0f));
                    wInv.invert(wInv);

                    Vector3 vCamInTree =
                      wInv.applyPoint(SpeedTreeRenderer::s_vCamPos);
                    Vector3 vTargetInTree =
                      wInv.applyPoint(SpeedTreeRenderer::s_vTargetPos);

                    Vector3 vSize = vDiag * 0.5f;
                    float   tBig  = LineInEllipsoid(
                      vSize * ((vSize.length() + 1.0f) / vSize.length()),
                      vCamInTree,
                      vTargetInTree);

                    if (tBig > 0) {
                        float tSmall =
                          LineInEllipsoid(vSize, vCamInTree, vTargetInTree);

                        oDrawData.leavesAlpha_ =
                          max(1.0f - tSmall / tBig,
                              SpeedTreeRenderer::s_hiddenLeavesAlpha);
                        oDrawData.branchAlpha_ =
                          max(1.0f - tSmall / tBig,
                              SpeedTreeRenderer::s_hiddenBranchAlpha);
                    }
                }
            }
        }
    ...
    //--------------------------------------------------------------------------------------------------
    void TSpeedTreeType::uploadInstanceConstants(Moo::EffectMaterial& effect,
                                                 const ConstantsMap&  cm,
                                                 const DrawData::Instance& inst)
    {
        BW_GUARD;

        //-- that is all what we need send to the shader. Only 4 float4 values
        // and more over we send
        ...
        effect.pEffect()->pEffect()->SetValue(
          cm.handle(ConstantsMap::CONST_INSTANCE),
          reinterpret_cast<const void*>(&inst),
          sizeof(DrawData::Instance));
    }

    //--------------------------------------------------------------------------------------------------
    uint TSpeedTreeType::uploadInstancingVB(Moo::VertexBuffer&        vb,
                                            uint32                    numInst,
                                            const DrawData::Instance* instances,
                                            uint& offsetInBytes)
    {
        BW_GUARD;
        ...
        //-- if current instancing buffer doesn't have enough memory to store
        // rendering data, then
        //-- start recycling process.
        if ((offsetInBytes + sizeInBytes) >
            SpeedTreeRendererCommon::instance().maxInstBufferSizeInBytes()) {
            offsetInBytes = 0;
        }
        ...
        //-- ToDo: reconsider. May be better to change vertex lock type to void*
        // and use memcpy for memory
        //--       copying.
        ...
        Moo::VertexLock<DrawData::PackedGPU> vl(
          vb, offsetInBytes, sizeInBytes, lockFlag);
        if (vl) {
    ...
    void TSpeedTreeType::computeLodData(LodData& oLod)
    {
        ...
        oLod.branchDraw_ =
          (sLod.m_nBranchActiveLod >= 0) && treeData_.branches_.checkVerts();
        oLod.branchAlpha_ = sLod.m_fBranchAlphaTestValue;

        //-- frond.
        oLod.frondLod_ = sLod.m_nFrondActiveLod;
        oLod.frondDraw_ =
          (sLod.m_nFrondActiveLod >= 0) && treeData_.fronds_.checkVerts();
        ...
        oLod.leafLodCount_       = (sLod.m_anLeafActiveLods[0] >= 0 ? 1 : 0) +
                             (sLod.m_anLeafActiveLods[1] >= 0 ? 1 : 0);
        oLod.leafDraw_ =
          oLod.leafLodCount_ > 0 && treeData_.leaves_.checkVerts();

        //-- billboards.
        oLod.billboardFadeValue_ = sLod.m_fBillboardFadeOut;
        oLod.billboardDraw_ =
          !treeData_.billboards_.lod_.empty() && (sLod.m_fBillboardFadeOut > 0);

        //-- recognize need we draw 3D model like branch, frond or leaf or not.
        oLod.model3dDraw_ =
          oLod.branchDraw_ || oLod.frondDraw_ || oLod.leafDraw_;
    }
    ...
    void TSpeedTreeType::drawTrees(Moo::ERenderingPassType pass)
    {
        ...
        //-- 1. first update wind animations.
        {
            ...
            for (TreeTypesMap::const_iterator it = s_typesMap_.begin();
                 it != s_typesMap_.end();
                 ++it) {
                //-- ToDo: check is this speedtree type need to be rendered. For
        ...
        if (pass == Moo::RENDERING_PASS_COLOR && s_enableZPrePass_) {
            g_commonRenderData->pass(Moo::RENDERING_PASS_DEPTH,
                                     s_enableInstaning_);
            ...
            drawBranches(g_commonRenderData->drawingState(TREE_PART_BRANCH),
                         false);
            drawBillboards(
              g_commonRenderData->drawingState(TREE_PART_BILLBOARD), false);
            rp().restoreColorWritingMask();

            g_commonRenderData->pass(Moo::RENDERING_PASS_COLOR,
                                     s_enableInstaning_);
        }
    ...
    void TSpeedTreeType::drawOptBillboards(Moo::ERenderingPassType pass)
    {
        ...
        if (s_optimiseBillboards_ && s_drawBillboards_) {
            ...
            BillboardOptimiser::FrameData frameData(viewProj,
                                                    s_invView_,
                                                    s_sunDirection_,
                                                    s_sunDiffuse_,
                                                    s_sunAmbient_,
            ...
            //-- do actual drawing and mark speedtree pixels in the stencil
            // buffer.
            rp().setupSystemStencil(IRendererPipeline::STENCIL_USAGE_SPEEDTREE);
    ...
    namespace TextureStreamingHelper {
        ...
        void generateTextureUsage(
          ...
          float (*calculateUVSpaceDensity_Callback)(TSpeedTreeType&,
                                                    const Moo::VertexFormat&))
        {
            ...
            if (pTexture) {
                usageGroup.setTextureUsage(pTexture,
                                           renderData.uvSpaceDensity_);
            }
#if SPT_ENABLE_NORMAL_MAPS
     ...
     */
    void TSpeedTreeType::generateTextureUsage(
      Moo::ModelTextureUsageGroup& usageGroup)
    {
     ...
     */
    void TSpeedTreeType::setBranchRenderStates(Moo::EffectMaterial& effect,
                                               const ConstantsMap&  cm) const
    {
        BW_GUARD;
        ...
        pEffect->SetBool(cm.handle(ConstantsMap::CONST_USE_Z_PRE_PASS),
                         s_enableZPrePass_);
        pEffect->SetBool(cm.handle(ConstantsMap::CONST_ALPHA_TEST_ENABLED),
                         false);
        pEffect->SetVectorArray(cm.handle(ConstantsMap::CONST_MATERIAL),
                                (const D3DXVECTOR4*)&material[0],
                                2);
        pEffect->SetBool(cm.handle(ConstantsMap::CONST_TEXTURED_TREES),
                         TSpeedTreeType::s_texturedTrees_);

        if (rd.diffuseMap_.exists()) {
            pEffect->SetTexture(cm.handle(ConstantsMap::CONST_DIFFUSE_MAP),
                                rd.diffuseMap_->pTexture());
        }

#if SPT_ENABLE_NORMAL_MAPS
        bool bUseNormals = rd.normalMap_.exists();
        if (bUseNormals) {
            pEffect->SetTexture(cm.handle(ConstantsMap::CONST_NORMAL_MAP),
                                rd.normalMap_->pTexture());
        }
        pEffect->SetBool(cm.handle(ConstantsMap::CONST_USE_NORMAL_MAP),
                         bUseNormals);
#endif // SPT_ENABLE_NORMAL_MAPS
        ...
        if (pWind_) {
            pEffect->SetMatrixArray(
              cm.handle(ConstantsMap::CONST_WIND_MATRICES),
              pWind_->matrices(),
              pWind_->matrixCount());
        }
    }
     ...
     */
    void TSpeedTreeType::setFrondRenderStates(Moo::EffectMaterial& effect,
                                              const ConstantsMap&  cm) const
    {
        BW_GUARD;
        ...
        pEffect->SetVectorArray(
          cm.handle(ConstantsMap::CONST_MATERIAL), (D3DXVECTOR4*)material, 2);
        pEffect->SetBool(cm.handle(ConstantsMap::CONST_TEXTURED_TREES),
                         TSpeedTreeType::s_texturedTrees_);
     ...
     */
    void TSpeedTreeType::setLeafRenderStates(Moo::EffectMaterial& effect,
                                             const ConstantsMap&  cm) const
    {
        BW_GUARD;
        ...
        pEffect->SetFloat(cm.handle(ConstantsMap::CONST_LEAF_LIGHT_ADJUST),
                          adjustment);
        ...
        if (pWind_) {
            ...
            pEffect->SetVectorArray(
              ...
              reinterpret_cast<const D3DXVECTOR4*>(pWind_->leafAnglesTable()),
              pWind_->leafAnglesCount());
        }

        Vector4 leafAngleScalars(
          treeData_.leafRockScalar_, treeData_.leafRustleScalar_, 0, 0);
        pEffect->SetVector(cm.handle(ConstantsMap::CONST_LEAF_ANGLE_SCALARS),
                           (D3DXVECTOR4*)&leafAngleScalars);
        pEffect->SetFloat(cm.handle(ConstantsMap::CONST_LEAF_ROCK_FAR),
                          TSpeedTreeType::s_leafRockFar_);
    }
     ...
     */
    void TSpeedTreeType::setBillboardRenderStates(Moo::EffectMaterial& effect,
                                                  const ConstantsMap&  cm) const
    {
        BW_GUARD;
      ...
      uint        seed)
    {
        ...
        START_TIMER(searchExisting);
        {
            char treeDefName[BW_MAX_PATH];
            createTreeDefName(
              filename, seed, treeDefName, ARRAY_SIZE(treeDefName));

            SimpleMutexHolder   mutexHolder(TSpeedTreeType::s_syncInitLock_);
            const TreeTypesMap& typesMap = TSpeedTreeType::s_typesMap_;
            TreeTypesMap::const_iterator rendererIt =
              typesMap.find(treeDefName);
        ...
        if (!result.exists()) {
            ...
            if (!success) {
                BW::string runtimeError =
                  BW::string(filename) + ": " + errorMsg;
            ...
            bw_str_copy(treeDataName,
                        ARRAY_SIZE(treeDataName),
            ...
            if (!speedTree->Compute(NULL, seed)) {
                BW::string runtimeError =
                  BW::string(filename) + ": Could not compute tree";
                throw std::runtime_error(runtimeError.c_str());
            }

            // speed wind
            BW::string datapath = BWResource::getFilePath(filename);
            renderer->pWind_    = setupSpeedWind(
              *speedTree, datapath, TSpeedTreeType::s_speedWindFile_);

            if (BWResource::isFileOlder(treeDataName, filename) ||
                !renderer->treeData_.load(treeDataName)) {
                int windAnglesCount = renderer->pWind_->matrixCount();

                renderer->treeData_ = TSpeedTreeType::generateTreeData(
                  *speedTree,
                  *renderer->pWind_->speedWind(),
                  filename,
                  seed,
                  windAnglesCount);
            ...
            START_TIMER(bspTree);
            renderer->bspTree_ =
              createBSPTree(renderer->speedTree_.get(),
     ...
     *  @param  filename        name of the spt file from where the tree was
     * loaded.
      ...
      int               windAnglesCount)
    {
        ...
        result.branches_ =
          createPartRenderData<BranchTraits>(speedTree, speedWind, datapath);
        STOP_TIMER(localBranches, "");

        START_TIMER(localFronds);
        result.fronds_ =
          createPartRenderData<FrondTraits>(speedTree, speedWind, datapath);
        STOP_TIMER(localFronds, "");

        START_TIMER(localLeaves);
        result.leaves_ = createLeafRenderData(speedTree,
                                              speedWind,
                                              filename,
                                              datapath,
                                              windAnglesCount,
                                              result.leafRockScalar_,
                                              result.leafRustleScalar_);
        STOP_TIMER(localLeaves, "");

        START_TIMER(localBillboards);
        result.billboards_ = createBillboardRenderData(speedTree, datapath);
    ...
    bool TSpeedTreeType::TreeData::save(const BW::string& filename)
    {
        ...
        *(float*)data = this->leafRustleScalar_;
        ...
        const BW::string& diff4 =
          this->billboards_.diffuseMap_.exists()
        ...
        {
            SimpleMutexHolder mutexHolder(TSpeedTreeType::s_vertexListLock_);
            data = this->branches_.write(
              data, CommonTraits::s_vertexList_, CommonTraits::s_indexList_);
            data = this->fronds_.write(
              data, CommonTraits::s_vertexList_, CommonTraits::s_indexList_);
            data = this->leaves_.write(
              data, LeafTraits::s_vertexList_, LeafTraits::s_indexList_);
            data = this->billboards_.write(data,
                                           BillboardTraits::s_vertexList_,
                                           BillboardTraits::s_indexList_);
        }
    ...
    bool TSpeedTreeType::TreeData::load(const BW::string& filename)
    {
        ...
        {
            SimpleMutexHolder mutexHolder(TSpeedTreeType::s_vertexListLock_);
            data = this->branches_.read(
              data, CommonTraits::s_vertexList_, CommonTraits::s_indexList_);
            data = this->fronds_.read(
              data, CommonTraits::s_vertexList_, CommonTraits::s_indexList_);
            data = this->leaves_.read(
              data, LeafTraits::s_vertexList_, LeafTraits::s_indexList_);
            data = this->billboards_.read(data,
                                          BillboardTraits::s_vertexList_,
                                          BillboardTraits::s_indexList_);
        }
    ...
    void TreePartRenderData<VertexType>::unload(STVector<VertexType>& verts,
    ...
    {
        SimpleMutexHolder mutexHolder(TSpeedTreeType::s_vertexListLock_);
        branches_.unload(CommonTraits::s_vertexList_,
                         CommonTraits::s_indexList_);
        ...
        billboards_.unload(BillboardTraits::s_vertexList_,
                           BillboardTraits::s_indexList_);
    }
    ...
    int SpeedTreeRenderer::numTris() const
    {
        ...
        if (this->treeType_) {
            ...
            if (!treeData.billboards_.lod_.empty() &&
                treeData.billboards_.lod_[0].index_) {
    ...
    //--------------------------------------------------------------------------------------------------
    void SpeedTreeRenderer::getTexturesAndVertices(
      BW::map<void*, int>& textures,
      BW::map<void*, int>& vertices)
    {
        ...
        if (this->treeType_) {
            ...
            TSpeedTreeType::TreeTypesMap::const_iterator it =
              TSpeedTreeType::s_typesMap_.find(uid);
            if (it != TSpeedTreeType::s_typesMap_.end()) {
                ...
                vertMemory += treeData.billboards_.size();
                vertices.insert(
                  std::make_pair((void*)it->first.data(), vertMemory));
            }
        }
    }
