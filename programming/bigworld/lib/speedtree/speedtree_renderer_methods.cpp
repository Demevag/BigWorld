...
namespace {
    ...
    //-- Note: To minimize redundant stage changes and with knowledge that
    // unique tree's types count
    //--	   is around 8-12 we can combine all geometry data including the
    // lod's data for each
    //--	   individual tree part in one big buffer. But the bigworld's engine
    // is dynamic by the
    //--	   nature i.e. it actively uses data streaming so we need to care
    // about dynamic inserting
    //--	   and removing desired tree types from these index and vertex
    // buffers. To make it works
    //--	   each tree type has information about its location in these big
    // buffers. This information
    //--	   is stored in the BufferSlot class.
    //--	   Because we have dynamic streaming in a background thread we have
    // to use some sync
    ...
    void bindModelData()
    {
        ...
        //-- copy the data.
        {
            ...
            if (!BASE::s_indexList_.empty() && BASE::s_indexList_.dirty()) {
                if (BASE::s_indexBuffer_->reset(
                      int(BASE::s_indexList_.size()))) {
                    //--TODO: 32-bit indices....
        ...
        if (BASE::s_vertexBuffer_ == NULL) {
            BASE::s_vertexBuffer_ =
              new VertexBufferWrapper<typename BASE::VertexType>;
        }
        ...
        //-- copy the data.
        {
            ...
            if (!BASE::s_vertexList_.empty() && BASE::s_vertexList_.dirty()) {
                if (BASE::s_vertexBuffer_->reset(
                      int(BASE::s_vertexList_.size()))) {
    ...
    //-- Note: Here we are using templated version of speedtree drawing. The
    // main method called
    //--	   drawTreePart is a template and uses *DrawingTraits structures as
    // a template argument.
    //--	   This approach widely known as a "template strategy pattern".
    // There are a lot of examples
    //--	   of using this pattern, one of them is Traits structure used in
    // the std::string class to
    //--	   describe some special properties of char type
    //(char_traits<char>).
    ...
    struct BranchDrawingCommonTraits
    {
        ...
        static void setRenderStates(TreeType&           rt,
                                    Effect&             eff,
                                    const ConstantsMap& cm)
        {
            rt.setBranchRenderStates(eff, cm);
        }
        static RenderData& getRenderData(TreeType& rt)
        {
            return rt.treeData_.branches_;
        }
        static D3DPRIMITIVETYPE getPrimitiveType()
        {
            return D3DPT_TRIANGLESTRIP;
        }
        static uint getPrimitiveCount(const RenderData& rd, uint lod)
        {
            return rd.lod_[lod].index_->count() - 2;
        }
    };
    ...
    struct FrondDrawingCommonTraits
    {
        ...
        static void setRenderStates(TreeType&           rt,
                                    ...
                                    const ConstantsMap& cm)
        {
            rt.setFrondRenderStates(eff, cm);
        }
        static RenderData& getRenderData(TreeType& rt)
        {
            return rt.treeData_.fronds_;
        }
        static D3DPRIMITIVETYPE getPrimitiveType()
        {
            return D3DPT_TRIANGLESTRIP;
    ...
    struct LeafDrawingCommonTraits
    {
        ...
        static void setRenderStates(TreeType&           rt,
                                    ...
                                    const ConstantsMap& cm)
        {
            rt.setLeafRenderStates(eff, cm);
        }
        static RenderData& getRenderData(TreeType& rt)
        {
            return rt.treeData_.leaves_;
        }
        static D3DPRIMITIVETYPE getPrimitiveType()
        {
        ...
        static uint getPrimitiveCount(const RenderData& rd, uint lod)
        {
            return rd.lod_[lod].index_->count() / 3;
        }
    };
    ...
    struct BillboardDrawingCommonTraits
    {
        ...
        static void setRenderStates(TreeType&           rt,
                                    ...
                                    const ConstantsMap& cm)
        {
            rt.setBillboardRenderStates(eff, cm);
        }
        static RenderData& getRenderData(TreeType& rt)
        {
            return rt.treeData_.billboards_;
        }
        static D3DPRIMITIVETYPE getPrimitiveType()
        {
            return D3DPT_TRIANGLELIST;
    ...
    struct BranchDrawingTraits : public BranchDrawingCommonTraits
    {
        ...
        static Instances& getInstances(TreeType& rt, uint lod)
        {
            return rt.drawDataCollector_.m_branches[lod];
        }
    };
    ...
    struct FrondDrawingTraits : public FrondDrawingCommonTraits
    {
        ...
        static Instances& getInstances(TreeType& rt, uint lod)
        {
            return rt.drawDataCollector_.m_fronds[lod];
        }
    };
    ...
    struct LeafDrawingTraits : public LeafDrawingCommonTraits
    {
        ...
        static Instances& getInstances(TreeType& rt, uint lod)
        {
            return rt.drawDataCollector_.m_leaves[lod];
        }
    };
    ...
    struct BillboardDrawingTraits : public BillboardDrawingCommonTraits
    {
        ...
        static Instances& getInstances(TreeType& rt, uint lod)
        {
            return rt.drawDataCollector_.m_billboards;
        }
    };

    //-- Note: This method assumes that most expensive driver operations are
    // changing shader, vertex
    //--	   declaration and shader's constants. This assumption based on the
    // MSDN's paper
    ...
    void drawTreesPart(DrawingState& ds, bool clear)
    {
        ...
        if (ds.m_material->begin()) {
            //-- Note: assume that we always have one pass in each technique.
            // That is true for almost
            //--	   all shaders in the engine and is definitely true for
            // SpeedTree's shaders.
            if (ds.m_material->beginPass(0)) {
                //-- set properties actual for current tree part. I.e. vertex
                // declaration, instancing
                //-- vertex buffer and so on.
                Moo::rc().setVertexDeclaration(
                  ds.m_vertexFormat->declaration());
                bindModelData<typename DrawingTraits::TreePartTraits>();

                if (instanced) {
                    Moo::rc().device()->SetStreamSourceFreq(
                      Moo::InstancingStream::STREAM_NUMBER,
                      (D3DSTREAMSOURCE_INSTANCEDATA | 1));
                }

                //-- 2. then iterate over the whole set of currently active tree
                // types.
                ...
                for (; it != TSpeedTreeType::s_typesMap_.end(); ++it) {
                    ...
                    const typename DrawingTraits::RenderData& rd =
                      DrawingTraits::getRenderData(rt);

                    //-- set properties actual for current tree type.
                    DrawingTraits::setRenderStates(
                      rt, *ds.m_material, *ds.m_constantsMap);

                    if (instanced) {
                        ds.m_material->commitChanges();
                    }

                    //-- 3. then iterate over the whole set of LOD levels for
                    // each individual tree type.
                    for (uint lod = 0; lod < DrawingTraits::getLodCount();
                         ++lod) {
                        //-- skip lod if it's empty.
                        TSpeedTreeType::Instances& instances =
                          DrawingTraits::getInstances(rt, lod);
                        if (instances.empty())
                            continue;
                        ...
                        int  primCount =
                          DrawingTraits::getPrimitiveCount(rd, lod);
                        int primStart = rd.lod_[lod].index_->start();

                        if (instanced) {
                            //-- 4. fill instancing buffer with new values use
                            // DRAW_NO_OVERWRITE flag for that.
                            uint prevOffset =
                              rt.uploadInstancingVB(*ds.m_instBuffer,
                                                    instCount,
                                                    &instances[0],
                                                    ds.m_offsetInBytes);

                            //-- set up source stream frequency and install
                            // instancing buffer with
                            //-- desired offset.
                            Moo::rc().device()->SetStreamSourceFreq(
                            ...
                            ds.m_instBuffer->set(
                              Moo::InstancingStream::STREAM_NUMBER,
                              prevOffset,
                              sizeof(TSpeedTreeType::DrawData::PackedGPU));
                            ...
                            //-- iterate over the whole set of instances and
                            // draw them independently.
                            for (uint j = 0; j < instCount; ++j) {
                                rt.uploadInstanceConstants(*ds.m_material,
                                                           *ds.m_constantsMap,
                                                           instances[j]);
                ...
                if (instanced) {
                    ...
                    Moo::rc().device()->SetStreamSourceFreq(
                      Moo::InstancingStream::STREAM_NUMBER, 1);
                    Moo::VertexBuffer::reset(
                      Moo::InstancingStream::STREAM_NUMBER);
                }
    ...
    {
        static bool needToDraw(const LodData& lodData)
        {
            return TSpeedTreeType::s_drawBranches_ && lodData.branchDraw_;
        }
        static uint getLodCount(const LodData& lodData) { return 1; }
        static uint getLod(const LodData& lodData, uint idx)
        {
            return lodData.branchLod_;
        }
        static float getAlphaRef(const LodData& lodData, uint idx)
        {
            return lodData.branchAlpha_ / 255.0f;
        }
    };
    ...
    struct FrondCustomDrawingTraits : public FrondDrawingCommonTraits
    {
        static bool needToDraw(const LodData& lodData)
        {
            return TSpeedTreeType::s_drawFronds_ && lodData.frondDraw_;
        }
        static uint getLodCount(const LodData& lodData) { return 1; }
        static uint getLod(const LodData& lodData, uint idx)
        {
            return lodData.frondLod_;
        }
        static float getAlphaRef(const LodData& lodData, uint idx)
        {
            return lodData.frondAlpha_ / 255.0f;
        }
    };
    ...
    struct LeafCustomDrawingTraits : public LeafDrawingCommonTraits
    {
        static bool needToDraw(const LodData& lodData)
        {
            return TSpeedTreeType::s_drawLeaves_ && lodData.leafDraw_ &&
                   lodData.leafLodCount_ > 0;
        }
        static uint getLodCount(const LodData& lodData)
        {
            return lodData.leafLodCount_;
        }
        static uint getLod(const LodData& lodData, uint idx)
        {
            return lodData.leafLods_[idx];
        }
        static float getAlphaRef(const LodData& lodData, uint idx)
        {
            return lodData.leafAlphaValues_[idx] / 255.0f;
        }
    };
    ...
    struct BillboardCustomDrawingTraits : public BillboardDrawingCommonTraits
    {
        static bool needToDraw(const LodData& lodData)
        {
            return TSpeedTreeType::s_drawBillboards_ && lodData.billboardDraw_;
        }
        static uint  getLodCount(const LodData& lodData) { return 1; }
    ...
    void drawTreePart(TSpeedTreeType&           rt,
                      Moo::EffectMaterial&      effect,
                      const ConstantsMap&       constsMap,
                      Moo::VertexDeclaration*   vertexFormat,
                      TSpeedTreeType::DrawData& drawData)
    {
        ...
        typename DrawingTraits::RenderData& rd =
          DrawingTraits::getRenderData(rt);
        ...
        if (DrawingTraits::needToDraw(lod) && !rd.lod_.empty()) {
            ...
            if (effect.begin()) {
                for (uint i = 0; i < DrawingTraits::getLodCount(lod); ++i) {
                    ...
                    int  vertBase  = rd.verts_->start();
                    int  primCount =
                      DrawingTraits::getPrimitiveCount(rd, lodIdx);
                                  ...
                                  const Moo::VertexFormat& vertFormat)
    {
        ...
        for (uint lod = 0; lod < rd.lod_.size(); ++lod) {
            ...
            const uint32* indexData = &baseIndexData[primStart];

            float geoMinUVDensity =
              calculateGeometryUVSpaceDensity<DrawingTraits>(vertFormat,
                                                             vertData,
                                                             vertDataSize,
                                                             indexData,
                                                             primCount,
      ...
      Moo::PrimitiveHelper::PrimitiveType primType)
    {
        ...
        if (!densityFunct.isValid()) {
            WARNING_MSG(
              "TSpeedTreeType::calculateUVSpaceDensity<DrawingTraits>:"
        ...
        Moo::PrimitiveHelper::generateTrianglesFromIndices(
          indexData, 0, primCount, primType, densityFunct);
    ...
    float calculateGeometryUVSpaceDensity(
      ...
      Moo::PrimitiveHelper::PrimitiveType                       primType)
    {
        return calculateStandardGeometryUVSpaceDensity(
          vertFormat, vertData, vertDataSize, indexData, primCount, primType);
    }
      ...
      Moo::PrimitiveHelper::PrimitiveType primType)
    {
        ...
        LeafDrawingTraits::TreePartTraits::VertexType* pModifiedVerts =
          (LeafDrawingTraits::TreePartTraits::VertexType*)bw_malloc(
            vertDataSize);
        memcpy(pModifiedVerts, vertData, vertDataSize);

        const Vector3 leafUnitSquare[5] = { Vector3(+0.5f, +0.5f, 0.0f),
        ...
        for (uint32 index = 0; index < numVerts; ++index) {
            ...
            corner.xyz	    *= inst.m_uniformScale * saturate(g_cameraPos.w
            * 7.0f);
            //-- adjust by pivot point so rotation occurs around the correct
            point corner.xyz	    *= i.rotGeomInfo.zwz;
            */
            ...
            cornerPos += Vector3(
              vertData[index].pivotInfo_.x, vertData[index].pivotInfo_.y, 0.0f);
    ...
    float calculateGeometryUVSpaceDensity<BillboardDrawingTraits>(
      const Moo::VertexFormat& vertFormat,
      const typename BillboardDrawingTraits::TreePartTraits::VertexType*
                                          vertData,
      uint                                vertDataSize,
      const uint32*                       indexData,
      int                                 primCount,
      Moo::PrimitiveHelper::PrimitiveType primType)
    {
        ...
        BillboardDrawingTraits::TreePartTraits::VertexType* pModifiedVerts =
          (BillboardDrawingTraits::TreePartTraits::VertexType*)bw_malloc(
            vertDataSize);
        memcpy(pModifiedVerts, vertData, vertDataSize);
        ...
        for (uint32 index = 0; index < numVerts; ++index) {
            ...
            pModifiedVerts[index].alphaNormal_ = Vector3(
              pModifiedVerts[index].tc_.x, pModifiedVerts[index].tc_.y, 0.0f);
        }
...
namespace speedtree {
    ...
    void TSpeedTreeType::drawBranches(DrawingState& ds, bool clear)
    {
        BW_GUARD;
        if (ds.m_instanced)
            drawTreesPart<BranchDrawingTraits, true>(ds, clear);
        else
            drawTreesPart<BranchDrawingTraits, false>(ds, clear);
    }
    ...
    void TSpeedTreeType::drawFronds(DrawingState& ds, bool clear)
    {
        BW_GUARD;
        if (ds.m_instanced)
            drawTreesPart<FrondDrawingTraits, true>(ds, clear);
        else
            drawTreesPart<FrondDrawingTraits, false>(ds, clear);
    }
    ...
    void TSpeedTreeType::drawLeaves(DrawingState& ds, bool clear)
    {
        BW_GUARD;
        if (ds.m_instanced)
            drawTreesPart<LeafDrawingTraits, true>(ds, clear);
        else
            drawTreesPart<LeafDrawingTraits, false>(ds, clear);
    }
    ...
    void TSpeedTreeType::drawBillboards(DrawingState& ds, bool clear)
    {
        BW_GUARD;
        if (ds.m_instanced)
            drawTreesPart<BillboardDrawingTraits, true>(ds, clear);
        else
            drawTreesPart<BillboardDrawingTraits, false>(ds, clear);
    }
    ...
    void TSpeedTreeType::customDrawBranch(DrawingState& ds, DrawData& drawData)
    {
        ...
        drawData.m_instData.m_materialID =
          static_cast<float>(materialHandle_ * 4 + 0);
        drawTreePart<BranchCustomDrawingTraits>(*this,
                                                *ds.m_material,
                                                *ds.m_constantsMap,
                                                ds.m_vertexFormat,
                                                drawData);
    ...
    void TSpeedTreeType::customDrawFrond(DrawingState& ds, DrawData& drawData)
    {
        ...
        drawData.m_instData.m_blendFactor = drawData.branchAlpha_;
        drawData.m_instData.m_materialID =
          static_cast<float>(materialHandle_ * 4 + 1);
        drawTreePart<FrondCustomDrawingTraits>(*this,
                                               *ds.m_material,
                                               *ds.m_constantsMap,
                                               ds.m_vertexFormat,
                                               drawData);
    ...
    void TSpeedTreeType::customDrawLeaf(DrawingState& ds, DrawData& drawData)
    {
        ...
        drawData.m_instData.m_materialID =
          static_cast<float>(materialHandle_ * 4 + 2);
        drawTreePart<LeafCustomDrawingTraits>(*this,
                                              *ds.m_material,
                                              *ds.m_constantsMap,
                                              ds.m_vertexFormat,
                                              drawData);
    ...
    //----------------------------------------------------------------------------------------------
    void TSpeedTreeType::customDrawBillBoard(DrawingState& ds,
                                             DrawData&     drawData)
    {
        BW_GUARD;
        drawData.m_instData.m_blendFactor = 1.0f;
        drawData.m_instData.m_materialID =
          static_cast<float>(materialHandle_ * 4 + 3);
        drawTreePart<BillboardCustomDrawingTraits>(*this,
                                                   *ds.m_material,
                                                   *ds.m_constantsMap,
                                                   ds.m_vertexFormat,
                                                   drawData);
    ...
    //----------------------------------------------------------------------------------------------
    float TSpeedTreeType::calculateUVSpaceDensityLeaves(
      TSpeedTreeType&          rt,
      const Moo::VertexFormat& format)
    {
        return calculateUVSpaceDensity<LeafDrawingTraits>(rt, format);
    }

    //----------------------------------------------------------------------------------------------
    float TSpeedTreeType::calculateUVSpaceDensityFronds(
      TSpeedTreeType&          rt,
      const Moo::VertexFormat& format)
    {
    ...
    //----------------------------------------------------------------------------------------------
    float TSpeedTreeType::calculateUVSpaceDensityBranches(
      TSpeedTreeType&          rt,
      const Moo::VertexFormat& format)
    {
    ...
    //----------------------------------------------------------------------------------------------
    float TSpeedTreeType::calculateUVSpaceDensityBillboards(
      TSpeedTreeType&          rt,
      const Moo::VertexFormat& format)
    {
