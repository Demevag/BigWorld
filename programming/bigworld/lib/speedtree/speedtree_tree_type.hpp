...
namespace speedtree {
    ...
    //in-memory as an instance
    //-- of the SpeedTreeRenderer class, all copies of a unique speedtree
    //(loaded from a unique
    //-- ".spt" file or generated form a unique seed number) will be represented
    //by a single
    ...
    class TSpeedTreeType
    {
      public:
        ...
        //-- Holds all data that can be cached to disk to save time when
        //generating the geometry of a
        //-- tree (vertex and index buffers plus bounding-boxses and leaf
        //animation coefficients. Also
        ...
        //-- Holds LODing values and informations for all render parts of a
        //tree.
        //------------------------------------------------------------------------------------------
        struct LodData
        {
        ...
        //--  Stores all the information required to draw a unique instance of a
        //tree.
        //------------------------------------------------------------------------------------------
        class DrawData
        {
...
#pragma pack(push, 1)
            //-- Tightly packed data for instancing. Total memory consumption is
            //40 bytes.
            struct PackedGPU
            {
                Vector4 m_rotation;
...
#pragma pack(push, 4)
            //-- data of each individual instance. Total memory consumption is
            //64 bytes.
            struct Instance
            {
                Vector4 m_rotation;
                ...
                //-- Used fixed point arithmetic. Layout [(1-fracBits) bits -
                //(fracBits) bits]
                template <typename T>
                ...
                inline PackedGPU packForInstancing() const
                {
                    ...
                    packed.m_materialID =
                      max<int16>(static_cast<int16>(m_materialID), 0);
                    packed.m_alphaRef = pack<uint8>(m_alphaRef, 8);
                    packed.m_windOffset =
                      min<uint8>(static_cast<uint8>(m_windOffset), 255);
...
#pragma pack(pop)
        ...
        //-- Notice that here we use aligned versions of BW::vector container so
        //our data has to be
        //-- 4 byte aligned.
        ...
        struct DrawDataCollector
        {
            ...
            //-- Note: billboards may be drawn with billboard_optimizer.*pp
            //stuff.
        };
      ...
      public:
        ...
        void collectTreeDrawData(DrawData& drawData,
                                 bool      collectBillboards,
                                 bool      semiTransparent);

        void        computeDrawData(const Matrix& world,
                                    float         windOffset,
                                    DrawData&     oDrawData);
        void        uploadInstanceConstants(Moo::EffectMaterial&      effect,
                                            const ConstantsMap&       constsMap,
                                            const DrawData::Instance& inst);
        uint        uploadInstancingVB(Moo::VertexBuffer&        vb,
                                       uint32                    numInst,
                                       const DrawData::Instance* instances,
                                       uint&                     offsetInBytes);
        void        computeLodData(LodData& oLod);
        static bool isTreeSemiTransparent(DrawData& data)
        {
            return (data.leavesAlpha_ != 1.0f) || (data.branchAlpha_ != 1.0f);
        }
        ...
        //-- drawing method for desired custom material. For example for
        //semi-transparent trees.
        //-- Note: These method don't take advantage of some internal
        //optimizations like instancing,
        //--	   material sorting, render state changes reduction mechanism
        //cos' often these objects
        //--	   are really very few. So this mechanism of speedtree
        //customization rendering works well
        //--	   only with a small amount of trees. If you need custom
        //rendering of relatively big amount
        //--	   of trees take a look at ComplexEffectMaterial and reconsider
        //possibility to add a new
        //--	   stage to the pipeline among the other like
        //RENDERING_PASS_REFLECTION and so on.
        ...
        void        setBranchRenderStates(Moo::EffectMaterial& effect,
                                          const ConstantsMap&  constsMap) const;
        void        setFrondRenderStates(Moo::EffectMaterial& effect,
                                         const ConstantsMap&  constsMap) const;
        void        setLeafRenderStates(Moo::EffectMaterial& effect,
                                        const ConstantsMap&  constsMap) const;
        void        setBillboardRenderStates(Moo::EffectMaterial& effect,
                                             const ConstantsMap&  constsMap) const;

        // -- texture streaming data collection
        static float calculateUVSpaceDensityLeaves(
          TSpeedTreeType&          rt,
          const Moo::VertexFormat& format);
        static float calculateUVSpaceDensityFronds(
          TSpeedTreeType&          rt,
          const Moo::VertexFormat& format);
        static float calculateUVSpaceDensityBranches(
          TSpeedTreeType&          rt,
          const Moo::VertexFormat& format);
        static float calculateUVSpaceDensityBillboards(
          TSpeedTreeType&          rt,
          const Moo::VertexFormat& format);
        ...
        static TreeData    generateTreeData(CSpeedTreeRT&     speedTree,
        ...
        //-- Represents unique wind parameters for each unique type of tree.
        //This class also helps
        //-- to set these parameters on rendering device.
        //-- If a particular speedtree type doesn't want to have its own wind
        //parameters it
        ...
        //-- default wind animation as a fall-back and instance of wind
        //animation as an per-type wind
        //-- parameters.
        WindAnimation*       pWind_;
        static WindAnimation s_defaultWind_;

        static WindAnimation* setupSpeedWind(CSpeedTreeRT&     speedTree,
                                             const BW::string& datapath,
                                             const BW::string& defaultWindIni);
        ...
        static Matrix s_lodCamera_; //-- this is invView matrix for the camera
                                    //at wich lod calculates.
