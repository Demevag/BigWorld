...
namespace speedtree {
    namespace { // anonymous
        ...
        struct AlphaBillboardVertex
        {
            ...
            static DWORD fvf()
            {
                return D3DFVF_XYZ | D3DFVF_NORMAL | D3DFVF_TEXCOORDSIZE3(0) |
                       D3DFVF_TEXCOORDSIZE3(1) | D3DFVF_TEXCOORDSIZE1(2) |
                       D3DFVF_TEXCOORDSIZE4(3) | D3DFVF_TEXCOORDSIZE3(4) |
                       D3DFVF_TEXCOORDSIZE3(5) | D3DFVF_TEXCOORDSIZE4(6) |
                       D3DFVF_TEXCOORDSIZE3(7) | D3DFVF_TEX8;
            }
        };
     ...
     */
    class BillboardsVBuffer : public ReferenceCount
    {
      public:
        ...
        static void drawVisible(const FrameData& frameData,
                                DX::Texture*     texture,
#if SPT_ENABLE_NORMAL_MAPS
        ...
        void touch();
        void draw(const Moo::EffectMaterialPtr& material,
                  bool                          clearAlphas = true);
        bool isCompiled() const;
        void compile();

        void bind(BillboardOptimiser* owner,
...
#define GROUP_SIZE 600.f
    ...
    {
        BW_GUARD;
        return ((uint16)this->trees_.size() >=
                  (BillboardOptimiser::s_maxInstanceCount_ - 1) &&
     ...
     *
     *  @param  pos     world position which to retrieve the optimiser instance
     * for.
     *
     *  @return         the requested optimiser instance.
     */
    BillboardOptimiser::BillboardOptimiserPtr BillboardOptimiser::retrieve(
      const Vector3& pos)
    {
        BW_GUARD;
        ...
        if (it != s_chunkBBMap_.end()) {
            // Has already been added so it's guaranteed to have at least one
            // group already..
    ...
    void BillboardOptimiser::init(DataSectionPtr section)
    {
        ...
        if (optSection.exists()) {
            ...
            if (fxFile.empty()) {
                CRITICAL_MSG("Effect for billboards optimiser "
                             "not defined in config file.\n");
            }

            BillboardOptimiser::s_mipBias_ =
              optSection->readFloat("mipBias", BillboardOptimiser::s_mipBias_);
        ...
        for (int i = 0; i < sizeof(aggreg) / sizeof(Moo::TextureAggregator*);
             ++i) {
            if (aggreg[i] != NULL) {
...
#undef SAFEDELETE
     ...
     *  @param  vertices        (in/out) vertice data. Will vertices with
     * texture coordinates modified to use the texture atlas.
     ...
     */
    int BillboardOptimiser::addTreeType(Moo::BaseTexturePtr texture,
#if SPT_ENABLE_NORMAL_MAPS
                                        ...
                                        uint              materialID)
    {
        ...
        treeType.diffuseNAdjust_ =
          Vector4(material[0], material[1], material[2], leafLightAdj);
        ...
        const BillboardVertex* vertices =
          &BillboardTraits::s_vertexList_[verticesSlot.start()];
        ...
        for (int i = 0; i < quadCount; ++i) {
            ...
            // Go through the texure coordinate for this quad and get the
            // min/max values for putting into the texture aggregator. skip the
            // first one because it's assigned above.
            for (uint c = 1; c < 6; c++) {
                int   index  = i * 6 + c;
            ...
            if (normAggreg != NULL) {
                normId = normAggreg->addTile(
                  normalMap.exists() ? normalMap : texture, min, max);
        ...
        if (success) {
            ...
            saveTextureAtlas(BillboardOptimiser::s_saveTextureAtlas_,
                             diffAggreg.texture());
        } else {
     ...
     */
    int BillboardOptimiser::addTree(int           treeTypeId,
                                    const Matrix& world,
                                    float         alphaValue)
    {
        BW_GUARD;
                                     ...
                                     bool                    useZPrePass)
    {
        ...
        // BillboardsVBuffer::s_shaderCap_ =
        // Moo::EffectManager::instance().PSVersionCap();
        ...
        const Matrix& texXForm =
          BillboardsVBuffer::s_diffuseMapAggregator_->transform();

        //-- use optional z-pre-pass rendering.
        if (useZPrePass && pass == Moo::RENDERING_PASS_COLOR) {
            rp().setupColorWritingMask(false);
            BillboardsVBuffer::drawVisible(frameData,
                                           texture,
#if SPT_ENABLE_NORMAL_MAPS
                                           normalMap,
#endif // SPT_ENABLE_NORMAL_MAPS
                                           texXForm,
                                           Moo::RENDERING_PASS_DEPTH,
                                           false);
     ...
     *  Constructor (private).
     *
     *  @param  pos     world position which this optimiser instance is related
     * to.
     */
    ...
    // (64*4): linked to the size of the g_bbAlphaRefs array in
    // billboards_opt.fx
     ...
     *  @param  section XML section containing all initialisation settings for
     *                  the optimiser engine (usually read from speedtree.xml).
     *  @param  fxFile  name of the FX file to use when rendering the
     * billboards.
     */
    void BillboardsVBuffer::init(DataSectionPtr    section,
                                 const BW::string& fxFile)
    {
        ...
        MF_WATCH("SpeedTree/BB Optimiser/Frames to recycle",
                 BillboardsVBuffer::s_framesToRecycle_,
                 Watcher::WT_READ_WRITE,
        ...
        MF_WATCH("SpeedTree/BB Optimiser/Frames to delete",
                 BillboardsVBuffer::s_framesToDelete_,
                 Watcher::WT_READ_WRITE,
                 "Number of frames a billboard optimiser buffer "
    ...
    void BillboardsVBuffer::initFXFiles()
    {
        ...
        if (!effect->initFromEffect(BillboardsVBuffer::s_fxFile_)) {
            CRITICAL_MSG("Could not load speedtree effect file: %s\n",
        ...
        s_diffuseMapAggregator_->setMinSize(
          BillboardOptimiser::s_atlasMinSize_);
        s_diffuseMapAggregator_->setMaxSize(
          BillboardOptimiser::s_atlasMaxSize_);
        s_diffuseMapAggregator_->setMipLevels(
          BillboardOptimiser::s_atlasMipLevels_);
        ...
        // if ( BillboardsVBuffer::s_effect_->bumpMapped() )
        {
            ...
            s_normalMapAggregator_->setMinSize(
              BillboardOptimiser::s_atlasMinSize_);
            s_normalMapAggregator_->setMaxSize(
              BillboardOptimiser::s_atlasMaxSize_);
            s_normalMapAggregator_->setMipLevels(
              BillboardOptimiser::s_atlasMipLevels_);
        }
#endif // SPT_ENABLE_NORMAL_MAPS
    }
            ...
            // may be a good candidate to be deleted
                ...
                if ((*buffersIt)->vbuffer_.isValid()) {
                    ...
                    ++activeCounter;
                           ...
                           (*buffersIt)->activeStamp_ <
                             curFrameStamp - framesToDelete) {
                                        ...
                                        bool                    clearAlphas)
    {
        ...
        //-- ToDo: reconsider. And Try to move to the class
        // SpeedTreeRendererCommon.
        if (!BillboardsVBuffer::s_effect_->baseMaterial()->pEffect()) {
            return;
        }

        const Moo::EffectMaterialPtr& material =
          BillboardsVBuffer::s_effect_->pass(type);
        const ComObjectWrap<ID3DXEffect>& pEffect =
        ...
        //-- ToDo: reconsider. Currently we're using shared pool of fx
        // constants. So this is redundant
        //--       operations.
        /*
     ...
     *  Draws buffer.
     */
    void BillboardsVBuffer::draw(const Moo::EffectMaterialPtr& material,
                                 bool                          clearAlphas)
    {
        BW_GUARD;
        if (this->trees_ == NULL || this->owner_ == NULL) {
            // Workaround for crash in WE when ProcessAll, to let the artists
            // work.
            // TODO: Find out why when called from photographThumbnail, "this"
            // has NULL trees_ and owner_.
            // ERROR_MSG( "BillboardsVBuffer::draw: Tried to render billboardVB
            // that has no trees or owner\n" );

            // This situation usually occurs when old trees are left around and
            // a 'tick' has yet to be called (which would clear out these old
            // trees)...so it should be safe to return without alerting the
            // user.... this should be tested thoroughly however.
            return;
        }
        ...
        if (this->vbuffer_.isValid()) {
            ...
            if (!alphas_->empty()) {
                if (this->alphas_->size() >
                    BillboardOptimiser::s_maxInstanceCount_) {
                    WARNING_MSG("BillboardsVBuffer::draw(): Too many trees in "
                ...
                // Can only upload 256 instances for anything below shader model
                // 2
                // TODO: enable selection of maximum instances (requires
                // rebuilding of the vertex buffers). if ( s_shaderCap_ < 2 )
                //{
    ...
    void BillboardsVBuffer::compile()
    {
        ...
        if (vbuffer_.reset(verticesCount) && vbuffer_.lock()) {
            ...
            while (bbIt != bbEnd) {
                ...
                if (typeId != -1) // skip removed trees
                {
                    TreeTypeMap::const_iterator treeTypeIt =
                      treeTypes.find(typeId);

                    const BufferSlot& slot = *treeTypeIt->second.verticesSlot_;

                    // Lock for access to BillboardTraits::s_vertexList_
                    SimpleMutexHolder mutexHolder(
                      TSpeedTreeType::s_vertexListLock_);
                    const BillboardVertex* vertices =
                      &BillboardTraits::s_vertexList_[slot.start()];
                    ...
                    while (vertexIdx < slot.count()) {
                        ...
                        // PCWJ - until we have normal mapped lighting on
                        // billboards, change the lighting normals for all
                        // billboard vertices so they point roughly in an
                        // upwards arc.  Do this by taking the vector between an
                        // arbitrary point n metres below the origin of the
                        // tree.
                        ...
                        avert.lightNormal_ =
                          bbIt->world_.applyVector(mungedNormal);
                        avert.lightNormal_.normalise();

                        avert.alphaNormal_ =
                          bbIt->world_.applyVector(vertex.alphaNormal_);
...
#if SPT_ENABLE_NORMAL_MAPS
                        avert.binormal_ =
                          bbIt->world_.applyVector(vertex.binormal_);
                        avert.binormal_.normalise();

                        avert.tangent_ =
                          bbIt->world_.applyVector(vertex.tangent_);
                        avert.tangent_.normalise();
#endif // SPT_ENABLE_NORMAL_MAPS

                        avert.diffuseNAdjust_ =
                          treeTypeIt->second.diffuseNAdjust_;
                        avert.ambient_ = treeTypeIt->second.ambient_;
                        avert.texCoordsMatID_[2] =
                          static_cast<float>(treeTypeIt->second.materialID_);
                        ...
                        switch (vertexIdx % 6) {
                            case 0: {
                                ...
                                // texture coordinates from the texture
                                // aggregator.
                                int id =
                                  treeTypeIt->second.tileIds_[vertexIdx / 6];
                                BillboardsVBuffer::s_diffuseMapAggregator_
                                  ->getTileCoords(id, min, max);
            ...
            ERROR_MSG("BillboardsVBuffer::compile: "
     ...
     */
    void BillboardsVBuffer::bind(BillboardOptimiser* owner,
                                 TreeInstanceVector* billboards,
                                 AlphaValueVector*   alphas)
    {
        BW_GUARD;
