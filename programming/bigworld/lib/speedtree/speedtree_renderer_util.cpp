        ...
        if (leafCount > 0) {
            ...
            // build the actual leaf mesh data
            {
                ...
                for (int leaf = 0; leaf < leafCount; ++leaf) {
                    ...
                    Vector3 cardCenter(+data.m_pCenterCoords[leaf * 3 + 0],
                                       +data.m_pCenterCoords[leaf * 3 + 1],
                                       -data.m_pCenterCoords[leaf * 3 + 2]);

                    Vector3 cardNormal =
                      Vector3(-data.m_pBinormals[leaf * 12 + 0],
                              -data.m_pBinormals[leaf * 12 + 1],
                              +data.m_pBinormals[leaf * 12 + 2]);

                    Vector3 cardTangent =
                      Vector3(-data.m_pTangents[leaf * 12 + 0],
                              -data.m_pTangents[leaf * 12 + 1],
                              +data.m_pTangents[leaf * 12 + 2]);

                    Vector3 cardBinormal =
                      Vector3(+data.m_pNormals[leaf * 12 + 0],
                    ...
                    const size_t baseIndex =
                      LeafTraits::s_vertexList_.size() - result.verts_->start();
                    ...
                    if (leafMesh != NULL) {
                        ...
                        for (int v = 0; v < leafMesh->m_nNumVertices; ++v) {
                            ...
                            // wind info
                            float fWindMatrixIndex1 = float(
                              int(data.m_pWindMatrixIndices[0][leaf] * 10.0f /
                                  speedWind.GetNumWindMatrices()));

                            float fWindMatrixWeight1 =
                              c_fWindWeightCompressionRange *
                              data.m_pWindWeights[0][leaf];
                            float fWindMatrixIndex2 = float(
                              int(data.m_pWindMatrixIndices[1][leaf] * 10.0f /
                                  speedWind.GetNumWindMatrices()));
                            ...
                            vert.tcWindInfo_.z =
                              fWindMatrixIndex1 + fWindMatrixWeight1;
                            vert.tcWindInfo_.w =
                              fWindMatrixIndex2 + fWindMatrixWeight2;

                            // texture coords
                            vert.tcWindInfo_.x =
                              leafMesh->m_pTexCoords[v * 2 + 0];
                            vert.tcWindInfo_.y =
                              leafMesh->m_pTexCoords[v * 2 + 1];

                            // rock and rustle info
                            vert.rotGeomInfo_.x =
                              DEG_TO_RAD(leafCard.m_afAngleOffsets[0]);
                            vert.rotGeomInfo_.y =
                              DEG_TO_RAD(leafCard.m_afAngleOffsets[1]);
                        ...
                        for (int v = 0; v < leafMesh->m_nNumIndices; ++v) {
                            size_t index =
                              baseIndex +
                              leafMesh
                                ->m_pIndices[v] /*+ result.verts_->start()*/;
                        ...
                        for (int v = 0; v < c_vertPerCard; ++v) {
                            ...
                            vert.normal_ =
                              Vector3(+data.m_pNormals[leaf * 12 + v * 3 + 0],
                                      +data.m_pNormals[leaf * 12 + v * 3 + 1],
                                      -data.m_pNormals[leaf * 12 + v * 3 + 2]);

#if SPT_ENABLE_NORMAL_MAPS
                            vert.tangent_ =
                              Vector3(+data.m_pTangents[leaf * 12 + v * 3 + 0],
                            ...
                            // texture coords
                            vert.tcWindInfo_.x =
                              leafCard.m_pTexCoords[v * 2 + 0];
                            vert.tcWindInfo_.y =
                              leafCard.m_pTexCoords[v * 2 + 1];
                            ...
                            // pivot into
                            vert.pivotInfo_.x =
                              leafCard.m_afPivotPoint[0] - 0.5f;
                            vert.pivotInfo_.y =
                              -(leafCard.m_afPivotPoint[1] - 0.5f);
...
speedtree::createBillboardRenderData(CSpeedTreeRT&     speedTree,
                                     const BW::string& datapath)
{
    BW_GUARD;
    ...
    if (data360.m_nNumImages < 0) {
        ...
        ERROR_MSG(
          "createBillboardRenderData: tree '%s' does not have billboards. "
    ...
    static const int vertexCount = 6;
    {
        ...
        result.verts_ =
          BillboardTraits::s_vertexList_.newSlot(bbCount * vertexCount);
        lodData.index_ =
          BillboardTraits::s_indexList_.newSlot(bbCount * vertexCount);
        ...
        for (int bb = 0; bb < bbCount; ++bb) {
            ...
            bbSpace.setRotateY(-bb * (2 * MATH_PI / data360.m_nNumImages) -
                               (MATH_PI / 2));
            ...
            if (bb < (bbCount - (drawHorizontalBB ? 1 : 0))) {
                ...
                for (int v = 0; v < vertexCount; ++v) {
                    ...
                    vertex.pos_.x =
                      0.5f * tangVector.x * xSignal[vIdx[v]] * data360.m_fWidth;
                    vertex.pos_.y = ySignal[vIdx[v]] * data360.m_fHeight;
                    vertex.pos_.z =
                      0.5f * tangVector.z * xSignal[vIdx[v]] * data360.m_fWidth;
