 ...
 */
bool EditorTerrainBlock2::create(DataSectionPtr settings,
                                 BW::string*    error /*= NULL*/)
{
    ...
    pDetailHeightMapResource_ =
      new HeightMapResource(blockSize(), fileName_, 0);
...
bool EditorTerrainBlock2::load(BW::string const& filename,
...
bool EditorTerrainBlock2::saveLodTexture(DataSectionPtr dataSection) const
{
    ...
    if (pLodMap_) {
        ok =
          pLodMap_->save(dataSection, TerrainBlock2::LOD_TEXTURE_SECTION_NAME);
    }
...
/*virtual*/ bool EditorTerrainBlock2::save(DataSectionPtr dataSection) const
{
    ...
    ok &= this->shadowMap().save(
      dataSection->newSection("horizonShadows", BinSection::creator()));

    dataSection->deleteSections("holes");
    if (!this->holeMap().noHoles()) {
        ok &= this->holeMap().save(
          dataSection->newSection("holes", BinSection::creator()));
    }

    dataSection->deleteSections(LAYER_SECTION_NAME);
    for (size_t i = 0; i < numberTextureLayers(); ++i) {
        ok &= this->textureLayer(i).save(
          dataSection->newSection(LAYER_SECTION_NAME, BinSection::creator()));
    }

    dataSection->deleteSections("dominantTextures");
    if (this->dominantTextureMap()) {
        ok &= this->dominantTextureMap()->save(
          dataSection->newSection("dominantTextures", BinSection::creator()));
    }
    ...
    DataSectionPtr terrainBlockMetaData = dataSection->openSection(
      TERRAIN_BLOCK_META_DATA_SECTION_NAME, true, XMLSection::creator());
    if (terrainBlockMetaData != NULL) {
        terrainBlockMetaData->writeInt(FORCED_LOD_ATTRIBUTE_NAME,
                                       getForcedLod());
    }
...
/*virtual*/ bool EditorTerrainBlock2::saveToCData(
  DataSectionPtr pCDataSection) const
{
    BW_GUARD;
    bool ret = false;
    if (pCDataSection) {
        DataSectionPtr dataSection = pCDataSection->openSection(
          dataSectionName(), true, ZipSection::creator());
...
/*virtual*/ TerrainTextureLayer const& EditorTerrainBlock2::textureLayer(
  size_t idx) const
{
    BW_GUARD;
    ensureBlendsLoaded();
...
/*virtual*/ bool EditorTerrainBlock2::draw(
  ...
  bool invalidateLodMap /*= true */)
{
    ...
    pBlendsResource_->rebuild(
      compressTextures, generateDominantTextureMap ? &tempDominantMap : NULL);
        ...
        if (curDominantMap) {
            for (uint y = 0; y < curDominantMap->image().height(); ++y)
                for (uint x = 0; x < curDominantMap->image().width(); ++x)
                    tempDominantMap->setGroundType(
                      x, y, curDominantMap->getGroundType(x, y));
        }
...
bool EditorTerrainBlock2::importAOMap(const Moo::Image<uint8>& srcImage,
                                      uint                     top,
                                      uint                     left)
{
    BW_GUARD;
...
{
    BW_GUARD;
    pNormalMap_->generate(heightMap2(),
                          normalMapQuality,
                                            ...
                                            uint32        newSize /*= 0*/)
{
    ...
    // Enable constant max vertex lod, and disable texture lod ( because thats
    ...
    // now take photograph
    TerrainPhotographer& photographer =
      TerrainRenderer2::instance()->photographer();

    bool oldUseHoleMap = TerrainRenderer2::instance()->enableHoleMap(false);

    // HACK - disable special mode rendering
    bool oldUseSpecialMode =
      TerrainRenderer2::instance()->isSpecialModeEnabled_;
    ...
    //-- if size of the loaded lod texture and desired size is not equal, or the
    //lod texture doesn't
    ...
    if (photographer.init(newSize) &&
        ...
        if (!texture.hasComObject()) {
            ...
            texture     = Moo::rc().createTexture(
              newSize, newSize, 0, usage, D3DFMT_DXT5, D3DPOOL_MANAGED);
        }

        if (photographer.output(texture, D3DFMT_DXT5)) {
            if (Moo::rc().device()->TestCooperativeLevel() == D3D_OK) {
                // If we didn't lose device during photo (eg from a
                // CTRL+ALT+DEL) then mark as ok.
                ok = true;
                ...
                pLodMap_ = NULL;
                ERROR_MSG(
                  "EditorTerrainBlock2::rebuildLodTexture: "
...
{
    BW_GUARD;
    if ((lodRenderInfo_.renderTextureMask_ & TerrainRenderer2::RTM_DrawBlend) !=
          0 ||
        (lodRenderInfo_.renderTextureMask_ &
         TerrainRenderer2::RTM_PreLoadBlend) != 0) {
        ensureBlendsLoaded();
    }
    ...
    if (pBlendsResource_->getObject() != NULL) {
        ...
        if ((!pLodMap_ ||
             (lodRenderInfo_.renderTextureMask_ &
              TerrainRenderer2::RTM_DrawBlend) != 0 ||
            ...
            uint64 blendBuildTimeout =
              s_lastBlendBuildTime +
        ...
        // If we are drawing blends (or forced into drawing them) and not
        // drawing the lod texture then get rid of the blend texture to save
        // precious memory.  If we need it again then we regenerate it in the
        // above if statement.
        if (pLodMap_ && (lodRenderInfo_.renderTextureMask_ &
                         TerrainRenderer2::RTM_DrawBlend) == 0) {
                         ...
                         uint32                              lodLevel)
{
    ...
    for (uint32 x = 0; x < dimg.width(); x++) {
        for (uint32 y = 0; y < dimg.height(); y++) {
            ...
            uint8 grey =
              Math::lerp(val, minHeight, maxHeight, (uint8)0, (uint8)255);
            *p++ = grey;
        }
    }
    ...
    char filename[64];
    bw_snprintf(filename, 64, "e:/temp/%d_heights_%d.png", fileCount, lodLevel);
    FILE* file =
...
bool EditorTerrainBlock2::saveHeightmap(DataSectionPtr dataSection) const
{
    ...
    bool ok = heightMap().save(
      dataSection->newSection("heights", BinSection::creator()));
    ...
    if (ok) {
        ...
        for (uint32 i = 1; i < settings_->numVertexLods(); i++) {
            ...
            ok = dhm.save(dataSection->openSection(
              sectionName, true, BinSection::creator()));

#ifdef DEBUG_HEIGHT_MAPS
            if (i == 1) {
                // write source map only once as lod 0
                WriteHeightImagePNG(simg,
                                    heightMap2()->minHeight(),
                                    heightMap2()->maxHeight(),
                                    fileCount,
                                    0);
            }
            // write lod height map
            WriteHeightImagePNG(dimg,
                                heightMap2()->minHeight(),
                                heightMap2()->maxHeight(),
                                fileCount,
                                i);
