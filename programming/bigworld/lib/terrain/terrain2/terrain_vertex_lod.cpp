  ...
  TerrainBlock2::LodRenderInfo&      lodRenderInfo) const
{
    ...
    TerrainVertexLodConsts* consts =
      Manager::instance().vertexLodConsts(blockSize_);
    ...
    if (TerrainSettings::constantLod() ||
        (lodLevelDistance >
         (distanceInfo.minDistanceToCamera_ + consts->minNeighbourDistance))) {
    ...
    if (xzDistance(consts->blockNegativeX,
                   blockSize_,
                   distanceInfo.relativeCameraPos_) > lodLevelDistance)
        lodRenderInfo.neighbourMasks_[0] |=
          TerrainIndexBuffer::DIRECTION_NEGATIVEX;
    if (xzDistance(consts->blockPositiveX,
                   blockSize_,
                   distanceInfo.relativeCameraPos_) > lodLevelDistance)
        lodRenderInfo.neighbourMasks_[0] |=
          TerrainIndexBuffer::DIRECTION_POSITIVEX;
    if (xzDistance(consts->blockNegativeZ,
                   blockSize_,
                   distanceInfo.relativeCameraPos_) > lodLevelDistance)
        lodRenderInfo.neighbourMasks_[0] |=
          TerrainIndexBuffer::DIRECTION_NEGATIVEZ;
    if (xzDistance(consts->blockPositiveZ,
                   blockSize_,
                   distanceInfo.relativeCameraPos_) > lodLevelDistance)
        lodRenderInfo.neighbourMasks_[0] |=
          TerrainIndexBuffer::DIRECTION_POSITIVEZ;
    ...
    if (distanceInfo.currentVertexLod_ < 2 &&
        ...
        lodRenderInfo.morphRanges_.subblock_ =
          calcMorphRanges(distanceInfo.currentVertexLod_ + 1);
        ...
        const float subBlockDistances[4] = {
            xzDistance(consts->subBlockOffset0,
                       subBlockSize,
                       distanceInfo.relativeCameraPos_),
            xzDistance(consts->subBlockOffset1,
                       subBlockSize,
                       distanceInfo.relativeCameraPos_),
            xzDistance(consts->subBlockOffset2,
                       subBlockSize,
                       distanceInfo.relativeCameraPos_),
            xzDistance(consts->subBlockOffset3,
                       subBlockSize,
                       distanceInfo.relativeCameraPos_)
        };
                                             ...
                                             uint8           subBlockMask) const
{
    ...
    if (myValue != value1) {
        if (myValue) {
            // set me
            neighbourMasks[myindex + 1] |=
              TerrainIndexBuffer::DIRECTION_POSITIVEX;
        } else {
            // set neighbour
            neighbourMasks[index1 + 1] |=
              TerrainIndexBuffer::DIRECTION_NEGATIVEX;
        }
    }
    if (myValue != value2) {
...
void TerrainVertexLod::externalSubBlockTests(
  uint32          mainBlockLod,
  uint8           subBlockMask,
  ...
  NeighbourMasks& neighbourMasks) const
{
    ...
    for (uint32 i = 0; i < ARRAY_SIZE(consts->testData); i++) {
        ...
        // if we're a higher lod than our neighbour, then flag that direction
        // for degenerates
        float distance = xzDistance(
          consts->testData[i].sbOffset, subBlockSize, cameraPosition);

        if (distance > ourDistance) {
            neighbourMasks[1 + consts->testData[i].sbIndex] |=
              consts->testData[i].mask;
        }
    }
}
                                   ...
                                   const Vector3& cameraPosition)
{
    ...
    Vector2 v(cameraPosition.x - blockCorner.x,
              cameraPosition.z - blockCorner.z);
 ...
 *
 */
uint32 TerrainVertexLod::calculateLodLevel(
  float                           blockDistance,
  const Terrain::TerrainSettings* settings) const
{
    //-- apply zoom factor.
...
void TerrainVertexLod::getDistanceForLod(uint32 lodLevel,
                                         float& o_MinDistance,
                                         float& o_MaxDistance) const
{
    const uint32 nLods = static_cast<uint32>(size());
...
void TerrainVertexLod::TestTerrainVertexLod()
{
    ...
    // Test 3 - sub-sub test with first one set only
    {
        ...
        MF_ASSERT("Test failed!" &&
                  (nm[1] & TerrainIndexBuffer::DIRECTION_POSITIVEX));
        MF_ASSERT("Test failed!" &&
                  (nm[1] & TerrainIndexBuffer::DIRECTION_POSITIVEZ));
    ...
    // Test 4 - sub-sub test with first two set only
    {
        ...
        MF_ASSERT("Test failed!" && (nm[0] == 0));
        MF_ASSERT("Test failed!" &&
                  ((nm[1] ^ TerrainIndexBuffer::DIRECTION_POSITIVEZ) == 0));
        MF_ASSERT("Test failed!" &&
                  ((nm[2] ^ TerrainIndexBuffer::DIRECTION_POSITIVEZ) == 0));
        MF_ASSERT("Test failed!" && (nm[3] == 0));
        MF_ASSERT("Test failed!" && (nm[4] == 0));
    }
    ...
    // Test 5 - sub-sub test with first three set only
    {
        ...
        MF_ASSERT("Test failed!" &&
                  ((nm[2] ^ TerrainIndexBuffer::DIRECTION_POSITIVEZ) == 0));
        MF_ASSERT("Test failed!" &&
                  ((nm[3] ^ TerrainIndexBuffer::DIRECTION_POSITIVEX) == 0));
        MF_ASSERT("Test failed!" && (nm[4] == 0));
    }
    ...
    // Test 6 - sub-sub test with last one set only
    {
        ...
        MF_ASSERT("Test failed!" && (nm[3] == 0));
        MF_ASSERT("Test failed!" &&
                  (nm[4] & TerrainIndexBuffer::DIRECTION_NEGATIVEX));
        MF_ASSERT("Test failed!" &&
                  (nm[4] & TerrainIndexBuffer::DIRECTION_NEGATIVEZ));
    }
}
#endif // TESTS_ENABLED
