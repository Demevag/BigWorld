    ...
    class HoleHelper
    {
      public:
          ...
          : pBlock_(pHolesData)
        {
            ...
            srcStride_ =
              (pHeader_->width_ / 8) + ((pHeader_->width_ & 3) ? 1 : 0);
...
#ifndef MF_SERVER
        bool updateTexture(ComObjectWrap<DX::Texture>& texture,
                           uint32                      w,
                           uint32                      h)
        {
            BW_GUARD;
...
/*virtual*/ bool TerrainHoleMap2::lock(bool readOnly)
{
    ...
    MF_ASSERT(lockCount_ == 0 || (lockCount_ != 0 && (readOnly_ == readOnly ||
                                                      readOnly_ == false)));
        ...
        if (!readOnly_) {
            ...
            if (!allHoles_ && !noHoles_) {
                if (texture_.hasComObject() || createTexture()) {
                    ...
                    ERROR_MSG("TerrainHoleMap2::unlock - unable to create hole "
                              "texture\n");
                    ret = false;
                }
            }
...
/*virtual*/ bool TerrainHoleMap2::save(DataSectionPtr pSection) const
{
    ...
    if (myself->lock(true)) {
        ...
        BinaryPtr binaryBlock = new BinaryBlock(
          &data.front(), data.size(), "BinaryBlock/TerrainHoleMap2");
 ...
 */
/*virtual*/ bool TerrainHoleMap2::load(DataSectionPtr dataSection,
...
#endif
            {
                image_ = Moo::Image<bool>(helper.header().width_,
                                          helper.header().height_);
                uint32 sz =
                  std::min(helper.stride(), (uint32)image_.rawRowSize());
...
bool TerrainHoleMap2::createTexture()
{
    ...
    texture_ = Moo::rc().createTexture(holesMapSize_,
                                       holesMapSize_,
                                       1,
                                       0,
                                       D3DFMT_DXT1,
                                       D3DPOOL_MANAGED,
                                       "Terrain/HoleMap2/Texture");
...
void TerrainHoleMap2::updateTexture()
{
    ...
    if (texture_ != NULL && !image_.isEmpty()) {
        HoleHelper helper(image_.width(),
                          image_.height(),
                          image_.rawRowSize(),
                          image_.getRawRow(0));
