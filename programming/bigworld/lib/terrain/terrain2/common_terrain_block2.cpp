...
namespace Terrain {
    ...
    CommonTerrainBlock2::CommonTerrainBlock2(TerrainSettingsPtr pSettings)
      : CommonTerrainBlock2Base(pSettings == NULL ? DEFAULT_GRID_RESOLUTION
                                                  : pSettings->blockSize())
      , settings_(pSettings)
    ...
    {
        // Note, we explicitly dereference pointers so ensuing destruction can
        // be profiled.
     ...
     *  @param worldTransform   The transform matrix to position the terrain
     *block in world co-ordinates.
     *	@param cameraPosition	The position of the camera (to work out LoD
     *level).
     ...
     */
    /*virtual*/ bool CommonTerrainBlock2::load(BW::string const& filename,
     ...
     *
     *  @param filename         A file to load the terrain block from.
     *  @param terrainDS        DataSection corresponding to the file
     *'filename'.
     *  @param worldTransform   The transform matrix to position the terrain
     *block in world co-ordinates.
     *	@param cameraPosition	The position of the camera (to work out LoD
     ...
     */
    /*virtual*/ bool CommonTerrainBlock2::internalLoad(
      ...
      BW::string*       error /* = NULL*/)
    {
        ...
        if (!pHeights) {
            if (error)
                *error =
                  "Can't open heights section '" + heightMapSection + "'";
            return false;
        }

        // Load height map
        TerrainHeightMap2Ptr pDHM =
          new TerrainHeightMap2(blockSize(), 0, heightMapLod);

        if (!pDHM->load(pHeights, error)) {
            if (error)
                *error = "Can't load height map.";
            return false;
        }
        pHeightMap_ = pDHM;
        ...
        if (settings_->loadDominantTextureMaps()) {
            DataSectionPtr pDominantTexures =
              terrainDS->openSection("dominantTextures");
            if (pDominantTexures) {
                pDominantTextureMap_ = new DominantTextureMap2(blockSize());
                BW::string dominantTextureMapError;
                if (!pDominantTextureMap_->load(pDominantTexures,
                                                &dominantTextureMapError)) {
                    // doesn't matter, keep loading without the dominant texture
                    // map.
                    pDominantTextureMap_ = NULL;
                    WARNING_MSG(
                      "Failed to load dominant texture map for %s: %s\n",
                      filename.c_str(),
                      dominantTextureMapError.c_str());
                }
            }
        }
     ...
     *  2.
     *
     *  @returns				The size of the hole map raised to the next
     *power of 2.
     */
     ...
     */
    /*virtual*/ bool CommonTerrainBlock2::collide(
      Vector3 const&            start,
      Vector3 const&            end,
      ...
      uint32         heightMapLodToPreserve)
    {
        BW::string heightMapSectionToPreserve =
          TerrainHeightMap2::getHeightSectionName(BASE_HEIGHT_SECTION_NAME,
                                                  heightMapLodToPreserve);
    ...
    /**
     *	This function allows access to the dominant texture map.  This can be
     ...
     *
     *  @param dominantTextureMap         The terrain dominant texture map to
     * use.
     */
    void CommonTerrainBlock2::dominantTextureMap2(
      DominantTextureMap2Ptr dominantTextureMap)
    {
