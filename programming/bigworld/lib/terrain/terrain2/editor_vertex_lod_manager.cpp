...
EditorVertexLodManager::EditorVertexLodManager(TerrainBlock2& owner,
                                               uint32         numLods)
                                  ...
                                  TerrainHeightMap2Ptr pSourceHeightMap)
{
    ...
    DataSectionPtr pTerrainSection =
      BWResource::openSection(terrainSectionPath);
    if (!pTerrainSection) {
        WARNING_MSG("Can't save to terrain section %s",
                    terrainSectionPath.c_str());
        return false;
    }
                                  ...
                                  TerrainHeightMap2Ptr pSourceHeightMap)
{
    ...
    for (uint32 i = 0; i < numVertexLods; i++) {
        // Create an aliased height map based on the current lod level's
        // heightmap
...
bool EditorVertexLodManager::load()
{
    ...
    if (isDirty_) {
        ...
        // of the working set.
        for (VertexLodArray::iterator it = object_->begin();
             it != object_->end();
             ++it) {
