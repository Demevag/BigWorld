...
namespace Terrain {
    ...
    TerrainVertexBuffer::TerrainVertexBuffer()
    {
        if (!s_terrainVBWatchesAdded) {
            ...
            // which causes small bright pixels to appear on the edges of the
            // blocks with different LOD. It raises edges of the block looking
            // towards lower LOD blocks but we can't see that since the camera
            // always looks from the block with the highest LOD.
            MF_WATCH(
              "Render/Terrain/Terrain2/Geomorphing Fix",
              s_lodGeomorphingFix,
     ...
     *	@param vertices      is filled with two heights per vertex, the height
     *of the vertex in the current LOD and the height of the vertex in the next
     *lower LOD.
     */
                                       ...
                                       BW::vector<Vector2>&    vertices)
    {
        ...
        // Create the heights for the vertex buffer, the heights are calculated
        // so that the vertex contains the current height and the corresponding
        ...
        for (uint32 z = 0; z < resolutionZ; z += 2) {
            ...
            for (uint32 x = 0; x < resolutionX; x += 2, ++lodX) {
                (it + x)->set(hm->height(x, z), lodHM->height(lodX, lodZ));
            }
        ...
        for (uint32 z = 1; z < resolutionZ; z += 2) {
            ...
            // These indices are for the odd and even rows and columns for
            // creating the approximate height value of the next LOD down, since
            // we are using crossed indices we want to alternate the angle of
            // the diagonals the height is calculated from.
     ...
     *	@param usage         D3D usage options that identify how resources are
     to be used.
                                   ...
                                   uint32         usage)
    {
        ...
        if (Moo::rc().device()) {
            ...
            if (SUCCEEDED(pVertexBuffer_.create(bufferSize,
                                                usage,
                                                0,
                                                D3DPOOL_MANAGED,
                ...
                if (vl) {
                    ...
                    res          = true;
                    pGridBuffer_ = GridVertexBuffer::get(uint16(resolutionX),
                                                         uint16(resolutionZ));
