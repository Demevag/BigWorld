...
namespace Terrain {
     ...
     */
    bool TerrainLodTexture::init(uint32    textureSize,
                                 uint32    lodSize,
                                 D3DFORMAT textureFormat)
    {
        BW_GUARD;
        textureSize_ = std::min((uint32)Moo::rc()
                                  .deviceInfo(Moo::rc().deviceIndex())
                                  .caps_.MaxTextureWidth,
                                textureSize);
    ...
    {
        BW_GUARD;
        pTexture_ = Moo::rc().createTexture(textureSize_,
                                            textureSize_,
                                            1,
                                            D3DUSAGE_DYNAMIC,
                                            textureFormat_,
      ...
      uint32       vTile)
    {
        ...
        if (SUCCEEDED(hr) && SUCCEEDED(hr2)) {
            ...
            ERROR_MSG(
              "TerrainLodTexture::addTextureTile - unable to get surface "
