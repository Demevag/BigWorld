...
namespace Terrain {
    ...
    class TerrainGraphicsOptions
    {
      public:
        ...
        float lodModifier() const
        {
            MF_ASSERT(currentOption_);
            return currentOption_->m_vertexLodScale;
        }
        float lodTextureModifier() const
        {
            MF_ASSERT(currentOption_);
            return currentOption_->m_textureLodScale;
        }
        float lodNormalModifier() const
        {
            MF_ASSERT(currentOption_);
            return currentOption_->m_normalLodScale;
        }
        float lodBumpModifier() const
        {
            MF_ASSERT(currentOption_);
            return currentOption_->m_bumpLodScale;
        }
        bool bumpMapping() const
        {
            MF_ASSERT(currentOption_);
            return currentOption_->m_useBumpMapping;
        }
      ...
      private:
        ...
        {
            GraphicsOption(bool  useBumpMapping,
                           uint  meshLoadBias,
                           float vertexLodScale,
                           float lodTextureScale,
                           float lodNormalScale,
                           float lodBumpScale)
              : m_useBumpMapping(useBumpMapping)
              , m_meshLodBias(meshLoadBias)
              , m_vertexLodScale(vertexLodScale)
              , m_textureLodScale(lodTextureScale)
              , m_normalLodScale(lodNormalScale)
              , m_bumpLodScale(lodBumpScale)
            {
            }
