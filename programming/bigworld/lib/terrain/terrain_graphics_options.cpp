...
void TerrainGraphicsOptions::initOptions(DataSectionPtr pOptions)
{
    ...
    pQualitySettings_ = Moo::makeCallbackGraphicsSetting(
      ...
      "Terrain Quality",
      *this,
      &TerrainGraphicsOptions::setQualityOption,
      -1,
      true,
      false);

    //--
    pQualitySettings_->addOption(
      "MAX", "Max", Moo::rc().psVersion() >= 0x300, true);
    pQualitySettings_->addOption(
      "HIGH", "High", Moo::rc().psVersion() >= 0x300, true);
    pQualitySettings_->addOption(
      "MEDIUM", "Medium", Moo::rc().psVersion() >= 0x300, true);
    pQualitySettings_->addOption("LOW", "Low", true);
    pQualitySettings_->addOption("MIN", "Min", true);

    //--
    graphicsOptions_.push_back(
      GraphicsOption(true, 0, 1.0f, 1.0f, 1.0f, 1.0f)); //-- MAX
    graphicsOptions_.push_back(
      GraphicsOption(true, 0, 1.0f, 0.8f, 0.8f, 0.6f)); //-- HIGH
    graphicsOptions_.push_back(
      GraphicsOption(true, 0, 1.0f, 0.6f, 0.6f, 0.2f)); //-- MEDIUM
    graphicsOptions_.push_back(
      GraphicsOption(false, 0, 1.0f, 0.4f, 0.4f, 1.0f)); //-- LOW
    graphicsOptions_.push_back(
      GraphicsOption(false, 1, 1.0f, 0.2f, 0.2f, 1.0f)); //-- MIN
...
void TerrainGraphicsOptions::setQualityOption(int optionIdx)
{
    ...
    //-- 2. apply bump mapping.
    {
        for (uint32 i = 0; i < settings_.size(); i++) {
            ...
            settings_[i]->applyBumpMappingModifier(
              currentOption_->m_bumpLodScale);
        }
    }
    ...
    //-- 3. apply lod modifiers.
    {
        for (uint32 i = 0; i < settings_.size(); i++) {
            ...
            settings_[i]->applyLodTextureModifier(
              currentOption_->m_textureLodScale);
            settings_[i]->applyLodNormalModifier(
              currentOption_->m_normalLodScale);
        }
    }
}
