...
namespace Terrain {
      ...
      TerrainSettingsPtr pSettings)
    {
        ...
        BaseTerrainBlockPtr pNewBlock = BaseTerrainBlock::loadBlock(
          resourceID, Matrix::identity, Vector3::zero(), pSettings);
    ...
    /**
     *	This method attempts to find an already loaded BaseTerrainBlock and
     *return it. If the BaseTerrainBlock has not yet been loaded, NULL is
     *returned.
     */
     ...
     */
    void TerrainBlockCache::incRefCacheEntry(
      const TerrainBlockCacheEntry& entry)
    {
        SimpleMutexHolder smh(lock_);
     ...
     *	ReferenceCountedCacheEntry interface function
     */
    void TerrainBlockCache::decRefCacheEntry(
      const TerrainBlockCacheEntry& entry)
    {
        SimpleMutexHolder smh(lock_);
