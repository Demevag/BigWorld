...
size_t EditorBaseTerrainBlock::findLayer(BW::string const& texture,
...
) const
{
    ...
    for (size_t i = startIdx; i < numberTextureLayers(); ++i) {
        ...
        if (layer.sameTexture(
              texture, bumpTexture, uProjection, vProjection, epsilon))
            return i;
    }
    return INVALID_LAYER;
}

size_t EditorBaseTerrainBlock::findLayer(TerrainTextureLayer const& layer,
                                         float                      epsilon,
                                         size_t startIdx /*= 0*/
) const
{
...
size_t EditorBaseTerrainBlock::findWeakestLayer() const
{
    ...
    for (uint32 i = 0; i < numberTextureLayers(); ++i) {
        ...
        TerrainTextureLayerHolder holder(const_cast<TerrainTextureLayer*>(&ttl),
                                         true);
            ...
            if (srcLayerIdx != INVALID_LAYER) {
                { // Force a scope for dstHolder and srcHolder
                    ...
                    if (src.width() == dst.width() &&
                        ...
                        // Actual copy, which adds together the blends of both
                        // layers
                ...
                } // dstHolder and srcHolders release their lock on the layers
                  // here
