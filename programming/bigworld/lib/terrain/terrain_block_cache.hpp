...
namespace Terrain {
     ...
     * 	reference count of the BaseTerrainBlock separate from the reference
     *count of the cache entry. This is to minimise mutex locking operations
     *since every time the cache entry is incRef/decRef, we need to hold lock on
     *the terrain cache. Keeping the count separate means that
     *BaseTerrainBlockPtr can be used freely without incurring overhead of
     *locking.
     */
     ...
     *
     * 	Currently used by the server to save memory when running multiple
     *instances of the same space geometry.
     */
