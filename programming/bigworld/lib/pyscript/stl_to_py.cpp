...
PyObject* PySTLSequence::pySeq_item(Py_ssize_t index)
{
    ...
    // if (index < 0) index += sz;
    if (index < 0 || index >= sz) {
        PyErr_SetString(PyExc_IndexError, "PySTLSequence index out of range");
        return NULL;
    }
 ...
 */
int PySTLSequence::pySeq_ass_slice(Py_ssize_t indexA,
                                   Py_ssize_t indexB,
                                   PyObject*  pOther)
{
    // check that the holder is writable
    ...
    if (indexA < indexB)
        holder_.erase((int)indexA, (int)indexB);
...
int Script::setData(PyObject*                pObject,
                    PySTLSequenceHolderBase& holder,
                    const char*              varName)
{
    // check that the holder is writable
    if (!holder.writable()) {
        PyErr_Format(PyExc_TypeError, "%s is a read-only sequence", varName);
        return -1;
    }

    // make sure we're setting it to a sequence
    if (!PySequence_Check(pObject)) {
        PyErr_Format(PyExc_TypeError, "%s must be set to a sequence", varName);
        return -1;
    }
...
{
    return new PySTLSequence(const_cast<PySTLSequenceHolderBase&>(holder),
                             holder.pOwner());
}
