                                              ...
                                              const char*       funcName)
{
    ...
    PyObjectPtr pFunc = PyObjectPtr(PyObject_GetAttrString(pModule, funcName),
    ...
    if (!PyCallable_Check(pFunc.getObject())) {
        PyErr_Format(PyExc_TypeError,
                     "Macro expansion function '%s' is not callable",
                     funcName);
        PyErr_Print();
        return "";
    }

    PyObjectPtr pExpansion =
      PyObjectPtr(PyObject_CallFunction(pFunc.getObject(), "s", line.c_str()),
                  PyObjectPtr::STEAL_REFERENCE);
