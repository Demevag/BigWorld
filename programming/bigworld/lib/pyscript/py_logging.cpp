...
namespace PyLogging {
    ...
    PyObject* py_logCommon(PyObject*            args,
                           DebugMessagePriority priority,
                           const char*          name)
    {
        if (!PyTuple_Check(args)) {
            PyErr_Format(
              PyExc_TypeError, "Invalid arguments provided to py_log%s.", name);
            return NULL;
        }
        ...
        if (PyTuple_Size(args) != 3) {
            PyErr_Format(
              PyExc_TypeError,
              "py_log%s argument tuple has invalid number of elements.",
              name);
            return NULL;
        }
        ...
        if (!PyString_Check(objCategoryString) ||
            ...
            PyErr_Format(
              PyExc_TypeError, "py_log%s has invalid element types.", name);
            return NULL;
        }
        ...
        if (pMessageString == NULL) {
            PyErr_Format(
              PyExc_ValueError, "py_log%s has invalid message string.", name);
            return NULL;
        }

        if ((objLogMetaData != Py_None) &&
            (PyString_Size(objLogMetaData) > 0)) {
...
#define __DOC__logTrace "Log a message with severity 'TRACE'."
#define __DOC__logDebug "Log a message with severity 'DEBUG'."
#define __DOC__logInfo "Log a message with severity 'INFO'."
#define __DOC__logNotice "Log a message with severity 'NOTICE'."
#define __DOC__logWarning "Log a message with severity 'WARNING'."
#define __DOC__logError "Log a message with severity 'ERROR'."
#define __DOC__logCritical "Log a message with severity 'CRITICAL'."
#define __DOC__logHack "Log a message with severity 'HACK'."
