...
namespace Personality {
    ...
    class PersonalityFiniTimeJob : public Script::FiniTimeJob
    {
      ...
      private:
        ...
        {
            if (s_pInstance_) {
                s_pInstance_.callMethod("onFini",
                                        ScriptErrorPrint("onFini"),
     ...
     *	a default ScriptModule() is returned and no Python exception state is
     *set.
     */
    ScriptModule import(const BW::string& name)
    {
        ...
        s_pInstance_ = ScriptModule::import(
          name.c_str(), ScriptErrorPrint(importError.c_str()));
    ...
    bool callOnInit(bool isReload)
    {
        if (s_pInstance_) {
            ...
            s_pInstance_.callMethod("onInit",
                                    ScriptArgs::create(isReload),
                                    ScriptErrorPrint("onInit"),
                                    /* allowNullMethod */ true);
        }
     ...
     *	could be found, a default ScriptObject() is returned and no Python
    ...
    ScriptObject getMember(const char* name)
    {
        ...
        ScriptObject member =
          s_pInstance_.getAttribute(name, ScriptErrorClear());
