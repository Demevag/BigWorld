...
class SwitchedModel : public Model
{
  public:
    ...
    SwitchedModel(const BW::StringRef& resourceID, DataSectionPtr pFile);
  ...
  protected:
    bool wireSwitch(DataSectionPtr    pFile,
                    BulkLoader        loader,
 ...
 */
template <class BULK>
SwitchedModel<BULK>::SwitchedModel(const BW::StringRef& resourceID,
 ...
 */
template <class BULK>
bool SwitchedModel<BULK>::wireSwitch(DataSectionPtr    pFile,
                                     BulkLoader        loader,
                                     const BW::string& mainLabel,
                                     const BW::string& frameLabel)
{
    ...
    SwitchedModelAnimation<BULK>* pDefAnim =
      new SwitchedModelAnimation<BULK>(*this, "", 1.f);
    ...
    for (uint i = 0; i < vAnimSects.size(); i++) {
        ...
        if (!frameLabel.empty()) {
            ...
            for (int j = 0; j < frameCount; j++) {
                ...
                ss << j << std::ends;
                frameNames.push_back(bulkName + "." + animName + "." +
                                     ss.str());
            }
        }
        if (frameNames.empty()) {
        ...
        SwitchedModelAnimation<BULK>* pAnim =
          new SwitchedModelAnimation<BULK>(*this, animName, frameRate);
 ...
 */
template <class BULK>
void SwitchedModel<BULK>::dressSwitchedModel()
{
    BW_GUARD;
