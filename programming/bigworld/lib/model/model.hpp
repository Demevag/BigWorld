...
{
  public:
    StringOrderProxy(const BW::string* pString)
      : pString_(pString)
    {
    }
...
class Model
  ...
  ,
     ...
     *	in onReloaderPriorReloaded which happen right before the visual
     *reloaded.
     *
     *	Model is also always listening if it's parent_ has
     *	been reloaded, if you are pulling info out from the parent_
     *	then please update it again in onReloaderReloaded which is
     *	called when the parent model is reloaded. and you might want to do
     *something in onReloaderPriorReloaded which happen before the parent_
     *reloaded.
     */
  ...
  ,
     ...
     *pModel->beginRead() and pModel->endRead() to avoid concurrency issues.
     */
    public Reloader
{
