...
namespace {
    ...
    class PyURLResponse : public PyObjectPlus
    {
        Py_Header(PyURLResponse, PyObjectPlus)
          ...
          public
          : PyURLResponse(const BW::string& contentType,
                          const BW::string& body,
                          int               responseCode)
    ...
    void Response::onDone(int response, const char* error)
    {
        ...
        PyObject* pPyResponse =
          new PyURLResponse(contentType_, body_.str(), response);

        PyObject* pResult =
          PyObject_CallFunctionObjArgs(pCallback_.get(), pPyResponse, NULL);
     ...
     *	This function fetches a URL asynchronously. The result is returned to
     *the callback.
     *
     *	Keyword arguments are supported for this function.
     *
     *	@param url	The URL to retrieve.
     *	@param callback	The callable object that receives the response object.
     *The response object has <i>body</i> and <i>responseCode</i> properties.
     *	@param headers An optional sequence of strings to be added to the
     *request's HTTP headers.
     *	@param timeout Optional argument for the number of seconds before this
     ...
     *			POST/PUT/PATCH request. If it is a Python dict, then this method
     *will encode the data to MULTIPART/FORM-DATA format. Otherwise, if it is a
     *Python string, the data will be posted to the HTTP server directly. In
     *this case, the user of this method needs to make sure that the data is
     *encoded in the right format(i.e. JSON/XML).
     */
    PyObject* py_fetchURL(PyObject* args, PyObject* kwargs)
    {
        ...
        const char* keywords[] = { "url",    "callback", "headers", "timeout",
                                   "method", "postdata", NULL };
        ...
        if (methodStr) {
            if (strcmp(methodStr, "GET") == 0) {
                ...
                PyErr_Format(PyExc_ValueError,
                             "Invalid method %s. Should be 'GET', 'PUT', "
        ...
        URL::Manager::instance()->fetchURL(
          url, new Response(pCallback), pHeaders, method, timeout, pPostData);
