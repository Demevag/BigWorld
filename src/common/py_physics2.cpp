...
namespace // anonymous
{
    ...
    {
        const MaterialKinds::Map& kinds =
          MaterialKinds::instance().materialKinds();
     ...
     *	point to another through the collision scene. If the line intersects
     ...
     *that was hit (specified by its three points, as Vector3s), and the
     *material type of the poly that was hit (an integer between 0 and 255).
     *This material type corresponds to the material or object Kind specified in
     *	ModelViewer (see Object->Properties and Material->Material Properties
     *panes).
     *
     *	For example:
     *	@{
     ...
     *	below it. It intersects with a triangle at the origin, where the
     *triangles three points are (0,0,0), (4,0,0) and (4,0,4). The material is
     *of type 0.
     *
     ...
     *	@param triangleFlags	Optional triangle filter. Only collisions with
     *these triangles are tested, e.g TRIANGLE_WATER for water triangles, and
     *TRIANGLE_TERRAIN for terrain triangles.
     *
     *	@return A 3-tuple of the hit point, the triangle it hit, and the
     *material type or None if nothing was hit.
     */
    /**
     *	Collides the given line with the collision scene
     *
     *	@return If detailedResults is true, a 3-tuple of the hit point, the
     *triangle it hit, and the material type or None if nothing was hit. If
     *detailedResults is false, the hit point is returned or None if nothing was
     *hit.
     */
    ...
    PY_AUTO_MODULE_FUNCTION(
      ...
      collide,
      ARG(SpaceID,
          ARG(Vector3, ARG(Vector3, OPTARG(WorldTriangle::Flags, 0, END)))),
      BigWorld)
