...
void DetailsDialog::createDialog(CWindow          parentWindow,
                                 TaskInfoPtr      task,
...
{
    // Gets called from the message loop once all messages have been processed
    // for this window Hence we should destroy the object here as the dialog is
    // already closed
    delete this;
}

DetailsDialog::DetailsDialog(TaskInfoPtr      info,
                             MainMessageLoop& messageLoop,
                             TaskStore&       store)
  : store_(store)
...
void DetailsDialog::onLogDetailSelChange(UINT    notifyCode,
                                         int     id,
                                         CWindow handle)
{
    updateControls();
