    ...
    class ClipboardHelper
    {
      ...
      private:
        HGLOBAL copyTextIntoGlobalBuffer(const BW::wstring& text)
        {
            ...
            HGLOBAL globalHandle = ::GlobalAlloc(
              GMEM_MOVEABLE, (text.length() + 1) * sizeof(char_type));
            if (globalHandle != nullptr) {
                auto buffer =
                  static_cast<char_type*>(::GlobalLock(globalHandle));
                if (buffer != nullptr) {
                    ::CopyMemory(
                      buffer, text.data(), text.length() * sizeof(char_type));
...
LargeTaskListBox::LargeTaskListBox(TaskStore&       store,
                                   MainMessageLoop& messageLoop)
...
{
    // Fixes up the item height because we've already missed the WM_MEASUREITEM
    // message as we're being used by SubClassWindow.
...
void LargeTaskListBox::addTask(TaskInfoPtr task)
{
    ...
    if (isTaskVisible(task)) {
        if (sortMode_ == SORT_NONE) {
            ...
            auto iter = std::lower_bound(std::begin(currentTasks_),
                                         std::end(currentTasks_),
                                         task,
                                         predicate);
...
void LargeTaskListBox::removeTask(TaskInfoPtr task)
{
    ...
    if (iter != std::end(allTasks_)) {
        ...
        auto iter =
          std::find(std::begin(currentTasks_), std::end(currentTasks_), task);
...
void LargeTaskListBox::filterIntoTaskList(TaskList& list)
{
    ...
    auto predicate = [this](TaskInfoPtr taskInfo) {
        return isTaskVisible(taskInfo);
    };
    std::copy_if(std::begin(allTasks_),
                 std::end(allTasks_),
...
void LargeTaskListBox::onContextMenu(CWindow handle, ::CPoint screenPoint)
{
    ...
    UINT             id        = popupMenu.TrackPopupMenu(
      TPM_NONOTIFY | TPM_RETURNCMD, screenPoint.x, screenPoint.y, GetParent());
    switch (id) {
...
void LargeTaskListBox::onDrawItem(int id, LPDRAWITEMSTRUCT drawStruct)
{
    ...
    if (drawStruct->itemAction != ODA_DRAWENTIRE &&
        drawStruct->itemAction != ODA_SELECT) {
        return;
    }
