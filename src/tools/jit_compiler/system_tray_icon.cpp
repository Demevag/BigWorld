...
bool SystemTrayIcon::init(CWindow& window)
{
    ...
    iconNormal_.LoadIconW(
      MAKEINTRESOURCE(IDI_JIT_COMPILER), iconSizeX, iconSizeY, 0);
    ...
    iconWarnings_.LoadIconW(
      MAKEINTRESOURCE(IDI_JIT_WARNING), iconSizeX, iconSizeY, 0);
    ...
    iconErrors_.LoadIconW(
      MAKEINTRESOURCE(IDI_JIT_ERROR), iconSizeX, iconSizeY, 0);
...
void SystemTrayIcon::showBalloonPopup(const BW::WStringRef& title,
                                      const BW::WStringRef& text,
                                      BalloonType           type)
{
    NOTIFYICONDATA data;
    ...
    switch (lParam) {
        case WM_CONTEXTMENU: {
            ...
            popupMenu.TrackPopupMenu(
              0, popupPoint.x, popupPoint.y, ownerWindow_);
        } break;
