...
bool MainWindow::init()
{
    ...
    iconNormal_.LoadIconW(
      MAKEINTRESOURCE(IDI_JIT_COMPILER), 0, 0, LR_DEFAULTSIZE);
    ...
    iconWarnings_.LoadIconW(
      MAKEINTRESOURCE(IDI_JIT_WARNING), 0, 0, LR_DEFAULTSIZE);
    ...
    SendMessage(
      WM_SETICON, ICON_BIG, reinterpret_cast<LPARAM>(iconNormal_.m_hIcon));
    ...
    completedList_
      .updateItemHeight(); // Fix for WM_MEASUREITEM message being lost
    ...
    WTL::CToolInfo toolinfo(TTF_SUBCLASS,
                            normalTasksToggle_,
                            0,
                            nullptr,
                            L"Show/Hide successfully completed tasks.");
    tooltip_.AddTool(toolinfo);
    toolinfo.Init(TTF_SUBCLASS,
                  warningTasksToggle_,
                  0,
                  nullptr,
                  L"Show/Hide completed tasks with warnings.");
    tooltip_.AddTool(toolinfo);
    toolinfo.Init(TTF_SUBCLASS,
                  errorTasksToggle_,
                  0,
                  nullptr,
                  L"Show/Hide completed tasks with errors.");
    tooltip_.AddTool(toolinfo);
    ...
    store_.setStatusCallbacks(
      std::bind(
        &MainWindow::setStatusText, this, BW::wstring(L"Scanning Disk...")),
      std::bind(
        &MainWindow::setStatusText, this, BW::wstring(L"Processing...")));

    connections_.push_back(store_.registerRequestedTaskCallback(
      std::bind(&MainWindow::changeTaskList,
                this,
                std::ref(requestedList_),
                std::_1,
                std::_2)));
    connections_.push_back(
      store_.registerCurrentTaskCallback(std::bind(&MainWindow::changeTaskList,
                                                   this,
                                                   std::ref(currentList_),
                                                   std::_1,
                                                   std::_2)));
    connections_.push_back(store_.registerCompletedTaskCallback(
      std::bind(&MainWindow::changeTaskList,
                this,
                std::ref(completedList_),
                std::_1,
                std::_2)));
...
void MainWindow::updateStatus()
{
    ...
    if (store_.failuresExist()) {
        trayIcon_.setError();
        SendMessage(
          WM_SETICON, ICON_BIG, reinterpret_cast<LPARAM>(iconErrors_.m_hIcon));
        showBalloonPopup();
    } else if (store_.warningsExist()) {
        trayIcon_.setWarning();
        SendMessage(WM_SETICON,
                    ICON_BIG,
                    reinterpret_cast<LPARAM>(iconWarnings_.m_hIcon));
        showBalloonPopup();
    } else {
        trayIcon_.setNormal();
        SendMessage(
...
{
    messageLoop_.addAction(
      [this, text]() { statusText_.SetWindowTextW(text.c_str()); });
}

void MainWindow::changeTaskList(TaskListBoxBase& list,
                                TaskInfoPtr      task,
