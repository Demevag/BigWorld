...
class PackerHelper
{
  public:
    ...
    // Inits BWResource and AutoConfig strings to the paths of the "file"
    // parameters
    ...
    static bool copyFile(const BW::string& src,
                         const BW::string& dst,
                         bool              silent = false);
    ...
    // Helper method to check if a file1 is newer than a file2
    // (platform-independent)
    static bool isFileNewer(const BW::string& file1, const BW::string& file2);

    // Helper class to delete files on destruction. Ideal for temp files
    class FileDeleter
    {
      public:
        FileDeleter(const BW::string& file)
          : file_(file){};
  ...
  private:
    ...
    // AutoConfig and method to get the cursor textures, so we can set them to
    // the appropriate format. It has to be done this way to avoid the overkill
    // of having to modify the MouseCursor class and including ashes, pyscript,
    ...
    // AutoConfig and method need to be checked and changed if necesary to
    // reflect any changes in the tag names and/or the required texture format.
