...
{
  public:
    PackerFactory(BasePacker*    packer,
                  unsigned short priority = Packers::HIGHEST_PRIORITY)
    {
        Packers::instance().add(packer, priority);
    }
};

#define DECLARE_PACKER() static PackerFactory s_packer_factory_;
#define IMPLEMENT_PACKER(P) PackerFactory P::s_packer_factory_(new P());
