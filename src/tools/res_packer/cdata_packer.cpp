    ...
    class DummyChunkTerrain : public ChunkItem
    {
      ...
      public:
        bool load(DataSectionPtr pSection, BW::string* errorString)
        {
            ...
            resName =
              s_terrainChunkCDataFilename + resName.substr(cDataPathEnd);
            MF_ASSERT(s_terrainBlock == NULL);
            s_terrainBlock = BaseTerrainBlock::loadBlock(resName,
                                                         Matrix::identity,
                                                         Vector3::zero(),
                                                         s_terrainSettings,
                                                         errorString);
      ...
      public:
        InitTerrainDependencies(DataSectionPtr terrainSettingsData)
        {
            ...
            DataSectionPtr terrainSettings =
              ChunkSpace::getTerrainSettingsDataSection(terrainSettingsData);

            DummyChunkTerrain::s_terrainSettings->init(gridSize,
                                                       terrainSettings);
        ...
        ~InitTerrainDependencies()
        {
            ...
            /*
            // mslater : work around a bug that exists in
            BgTaskManager::numBgTasksLeft. This has a race condition
            //           between tasks being added to the active list and
            removed from the task list that makes
            //			 the loop in the prepare function below exit early and
            the function to exit before all
            //           background tasks are complete. This causes an assertion
            in Terrain::Manager as it has been
            //           destroyed by this object on destruction. Below
            preserves existing behaviour until a correct
            //           fix can be introduced.
            BgTaskManager::instance().stopAll();
            BgTaskManager::instance().startThreads("Background Thread:
            res_packer", 1);
            */
        }
...
bool CDataPacker::prepare(const BW::string& src, const BW::string& dst)
{
    ...
    if (!ds) {
        WARNING_MSG(
          "Couldn't find the space.settings file associated with '%s'."
          " Ignoring this .cdata file",
          src.c_str());
        return false;
    }
    ...
    if (terrainVersion >= 200) {
        ...
        DataSectionPtr chunkSection =
          BWResource::openSection(dissolvedChunkFilename, false);
        ...
        Chunk::loadInclude(chunkSection,
                           flattenMatrix,
                           &errorString,
                           NULL,
        ...
        stripQualityNormals_    = false;
        heightMapLodToPreserve_ = -1;
        heightMapLodToPreserve_ =
          DummyChunkTerrain::s_terrainBlock->getForcedLod();
        ...
        uint8 renderTextureMask = TerrainRenderer2::getLoadFlag(
          ...
          DummyChunkTerrain::s_terrainSettings
            ->absoluteNormalPreloadDistance());

        renderTextureMask =
          TerrainRenderer2::getDrawFlag(renderTextureMask,
                                        minDistance,
                                        maxDistance,
                                        DummyChunkTerrain::s_terrainSettings);
...
bool CDataPacker::pack()
{
    ...
    DataSectionPtr root =
      BWResource::openSection(BWResolver::dissolveFilename(dst_));
    if (!root) {
...
#ifndef MF_SERVER
    DataSectionPtr dataSectionPtr =
      root->openSection(terrainDataSectionName_, false);
    ...
    if (dataSectionPtr != NULL) {
        ...
        CommonTerrainBlock2::stripUnusedHeightSections(dataSectionPtr,
                                                       heightMapLodToPreserve_);
    }
#endif // MF_SERVER
