...
bool PackerHelper::initResources()
{
    ...
    SetCurrentDirectory(
      bw_utf8tow(BWResource::getFilePath(bw_wtoutf8(buffer))).c_str());
#endif // _WIN32
...
void PackerHelper::initTextureFormats()
{
    ...
    for (int i = 0; i < ds->countChildren(); ++i) {
        Moo::TextureManager::instance()->getDetailsManager()->setFormat(
          ds->openChild(i)->readString("texture"), D3DFMT_A8R8G8B8);
    }
}
#endif // MF_SERVER
...
bool PackerHelper::initMoo()
{
    ...
    WNDCLASS wc = { 0,    WndProc, 0,    0,    NULL,
                    NULL, NULL,    NULL, NULL, L"packer_helper" };
    if (!RegisterClass(&wc)) {
    ...
    HWND hWnd = CreateWindow(L"packer_helper",
                             L"packer_helper",
                             WS_OVERLAPPEDWINDOW,
                             CW_USEDEFAULT,
                             CW_USEDEFAULT,
    ...
    if (!Moo::rc().createDevice(
          hWnd, 0, 0, true, false, Vector2(0, 0), true, forceRef)) {
...
bool PackerHelper::copyFile(const BW::string& src,
                            const BW::string& dst,
                            bool              silent)
{
...
bool PackerHelper::isFileNewer(const BW::string& file1, const BW::string& file2)
{
    ...
    if (!GetFileAttributesEx(
          bw_utf8tow(file1).c_str(), GetFileExInfoStandard, &data1) ||
        !GetFileAttributesEx(
          bw_utf8tow(file2).c_str(), GetFileExInfoStandard, &data2) ||
