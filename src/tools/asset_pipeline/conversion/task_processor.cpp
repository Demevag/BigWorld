...
class ConverterGuard
{
  public:
    ...
    {
        threadSafe_ =
          ((converterInfo.flags_ & ConverterInfo::THREAD_SAFE) != 0);
...
TaskProcessor::DependencyContext::DependencyContext(
  TaskProcessor&  taskProcessor,
  ConversionTask& conversionTask)
  : depList_(taskProcessor.compiler_)
{
    ...
    MF_VERIFY(
      taskProcessor.compiler_.resolveIntermediatePath(depListFileName_));
...
bool TaskProcessor::DependencyContext::loadFromCache(bool secondaryDependencies,
                                                     Compiler& compiler)
{
    const BW::string file =
      BWResource::getFilename(depListFileName_).to_string();
    ...
    if (ContentAddressableCache::readFromCache(
          depListFileName_, hash, compiler)) {
        ...
        if (depListRoot_ != NULL) {
            ...
            BW::vector<DependencyList::Input>& primaryDeps =
              depList_.primaryInputs_;
            for (BW::vector<DependencyList::Input>::iterator it =
                   primaryDeps.begin();
                 it != primaryDeps.end() && depListValid;
                 ++it) {
            ...
            BW::vector<DependencyList::Input>& secondaryDeps =
              depList_.secondaryInputs_;
            for (BW::vector<DependencyList::Input>::iterator it =
                   secondaryDeps.begin();
                 it != secondaryDeps.end() && depListValid;
                 ++it) {
                // The hashes of all our secondary dependencies should be zero
                uint64 hash =
                  secondaryDependencies ? compiler.getHash(*it->first) : 0;
                depListValid &= (it->second == hash);
            }
        } else {
            depListValid = false;
        ...
        if (!depListValid) {
            WARNING_MSG(
              "Corrupted dependency list retrieved from the cache: %s.%IX\n",
              file.c_str(),
              hash);
...
void TaskProcessor::processTask(ConversionTask& conversionTask)
{
    ...
    // Sanity check that the task being processed is actually in a processing
    // stage
    ...
    if (conversionTask.status_ == ConversionTask::NEEDS_PRIMARY_DEPS) {
        if (!processPrimaryDependencies(
              taskContext, dependencyContext, conversionContext)) {
            MF_ASSERT(conversionTask.status_ == ConversionTask::FAILED);
            return;
        }
        MF_ASSERT(conversionTask.status_ ==
                  ConversionTask::NEEDS_SECONDARY_DEPS);
    }

    if (conversionTask.status_ == ConversionTask::NEEDS_SECONDARY_DEPS) {
        if (!processSecondaryDependencies(
              taskContext, dependencyContext, conversionContext)) {
            MF_ASSERT(conversionTask.status_ ==
                        ConversionTask::NEEDS_CONVERSION ||
                      conversionTask.status_ == ConversionTask::FAILED);
            return;
        }
...
bool TaskProcessor::processNewTask(TaskContext& taskContext)
{
    ...
    BW::string resolvedSource =
      BWResource::resolveFilename(taskContext.relativeSource_);
    if (taskContext.conversionTask_.source_ != resolvedSource) {
        ERROR_MSG("FAILED: Found %s while trying to compile %s. This asset "
                  "will be ignored.\n",
  ...
  ConversionContext& conversionContext)
{
    ...
    if (!needsDeps) {
        ...
        needsDeps =
          !checkPrimaryDependenciesUpToDate(taskContext, dependencyContext);
    }

    if (needsDeps) {
        // Setup the primary dependencies for this task.
        // This is the same for all assets so we don't need to ask the converter
        // to do it for us.
        dependencyContext.depList_.initialise(
          taskContext.relativeSource_,
    ...
    if (needsDeps) {
        ...
        // One of our primary dependencies have changed or previously did not
        // exist on disk. Thus we can't trust the dependency list we loaded and
        // must ask the converter to generate a new one for us.
        ...
        // While in the converter any assert will throw an exception rather than
        // aborting the application. We use this to catch errors and fail the
        // task.
        try {
            INFO_MSG("Creating dependencies...\n");
            ConverterGuard converterGuard(conversionContext.converterInfo_);
            res = conversionContext.converter_->createDependencies(
              taskContext.conversionTask_.source_,
              compiler_,
              dependencyContext.depList_);
        } catch (...) {
            res = false;
        }
        ...
        if (!res || compiler_.hasError()) {
            ...
            // The converter returned false, an assert was triggered or someone
            // used the ERROR_MSG macro
            taskContext.conversionTask_.status_ = ConversionTask::FAILED;
            return false;
        }
        ...
        if (!compiler_.hasWarning() && dependencyContext.depListRoot_ != NULL) {
            dependencyContext.depList_.serialiseOut(
              dependencyContext.depListRoot_);
            dependencyContext.depListResource_.save();

            // Write our generated dependency list to the cache without the
            // secondary dependencies
            if ((conversionContext.converterInfo_.flags_ &
                 ConverterInfo::CACHE_DEPENDENCIES) != 0) {
                ContentAddressableCache::writeToCache(
                  dependencyContext.depListFileName_,
                  dependencyContext.depList_.getInputHash(false),
                  compiler_);
            }
        }
    } else {
        INFO_MSG("  Primary dependencies up to date\n");
        // All our primary dependencies match what is the dependency list on
        // disk so all our secondary dependencies should already be in the
        // dependency list. Thus we don't need to ask the converter to generate
        // our dependencies.
    }
  ...
  ConversionContext& conversionContext)
{
    ...
    // We now need to go through all our secondary dependencies and make sure
    // they are up to date before we can convert our task.
    bool blocked      = false;
    bool subTaskError = false;

    BW::vector<DependencyList::Input>& secondaryDeps =
      dependencyContext.depList_.secondaryInputs_;
    ...
    for (it = secondaryDeps.begin(); it != secondaryDeps.end(); ++it) {
        ...
        if (secondaryTask != NULL) {
            if (secondaryTask->status_ == ConversionTask::FAILED ||
                ...
                // If the secondary dependency is a task and it has failed, or
                // if it has been blocked by the current thread, then mark it as
                // up to date. We can't do anything else with it.
                upToDate = true;
            } else {
                // If the secondary dependency is a task and it's still
                // processing then it actually hasn't failed and is just not up
                // to date yet.
                failed = false;
            }

            // Add this secondary task to our sub tasks
            taskContext.conversionTask_.subTasks_.push_back(
              std::make_pair(secondaryTask, it->first->isCritical()));
        }
    ...
    // If we are blocked then return from being processed. This will put our
    // task back into queue but we will be guaranteed that all our secondary
    // dependencies will be up to date the next time we process this task.
                                      ...
                                      ConversionContext& conversionContext)
{
    ...
    if (!needsConversion) {
        // we need to convert if any of secondary dependencies are not up to
        // date
        needsConversion =
          !checkSecondaryDependenciesUpToDate(dependencyContext);
    }
    ...
    if (needsConversion) {
        ...
        // we don't need to convert if we can retrieve an up to date dependency
        // list from the cache
        needsConversion = !retrieveSecondaryDependencyListFromCache(
          dependencyContext, conversionContext);
    }
    ...
    if (!needsConversion) {
        ...
        // and we can't retrieve them from the cache
        needsConversion =
          !checkOutputsUpToDate(dependencyContext, conversionContext);
    }
    ...
    if (needsConversion) {
        ...
        if ((conversionContext.converterInfo_.flags_ &
             ConverterInfo::UPGRADE_CONVERSION) != 0) {
        ...
        try {
            ...
            ConverterGuard converterGuard(conversionContext.converterInfo_);
            res = conversionContext.converter_->convert(
              taskContext.conversionTask_.source_,
              compiler_,
              intermediateFiles,
              outputFiles);
        ...
        updateIntermediateOutputs(
          intermediateFiles, taskContext, dependencyContext, conversionContext);
        // update the stored hashes of our outputs
        // and push them to the cache
        updateOutputs(
          outputFiles, taskContext, dependencyContext, conversionContext);
        ...
        if (!compiler_.hasWarning() && dependencyContext.depListRoot_ != NULL) {
            ...
            // Write our generated dependency list to the cache with the
            // secondary dependencies
            if ((conversionContext.converterInfo_.flags_ &
                 ConverterInfo::CACHE_CONVERSION) != 0) {
                ContentAddressableCache::writeToCache(
                  dependencyContext.depListFileName_,
                  dependencyContext.depList_.getInputHash(true),
                  compiler_);
            }
        }
        ...
        TRACE_MSG("Current output files were built with existing input files. "
                  "Skipping conversion\n");
...
bool TaskProcessor::checkPrimaryDependenciesUpToDate(
  TaskContext&       taskContext,
  DependencyContext& dependencyContext)
{
    BW::vector<DependencyList::Input>& primaryDeps =
      dependencyContext.depList_.primaryInputs_;
        ...
        switch (i) {
            case 0: {
                SourceFileDependency* sourcefileDependency =
                  dynamic_cast<SourceFileDependency*>(dependency.first);
                if (sourcefileDependency == NULL ||
                    sourcefileDependency->getFileName() !=
                      taskContext.relativeSource_) {
                    // The first of the primary dependencies should always be a
            ...
            case 1: {
                ConverterDependency* converterDependency =
                  dynamic_cast<ConverterDependency*>(dependency.first);
                if (converterDependency == NULL ||
                    converterDependency->getConverterId() !=
                      taskContext.conversionTask_.converterId_ ||
                    converterDependency->getConverterVersion() !=
                      taskContext.conversionTask_.converterVersion_) {
                    // The secondary of the primary dependencies should always
                    // be a converter dependency.
                    return false;
                }
            } break;
            ...
            case 2: {
                ConverterParamsDependency* converterParamsDependency =
                  dynamic_cast<ConverterParamsDependency*>(dependency.first);
                if (converterParamsDependency == NULL ||
                    converterParamsDependency->getConverterParams() !=
                      taskContext.conversionTask_.converterParams_) {
                    // The last of the primary dependencies should always be a
                    // converter params dependency.
                    return false;
                }
            } break;
...
bool TaskProcessor::retrievePrimaryDependencyListFromCache(
  DependencyContext& dependencyContext,
  ConversionContext& conversionContext)
{
    // Check if we can pull the deplist from the cache
    if (!forceRebuild_ && (conversionContext.converterInfo_.flags_ &
                           ConverterInfo::CACHE_DEPENDENCIES) != 0) {
        if (dependencyContext.loadFromCache(false, compiler_)) {
            INFO_MSG("  Dependency list retrieved from cache\n");
            // We have found a deplist in the cache and copied to our local
            // drive. Need to reload the deplist from disk.
            return true;
        }
    }
...
{
    bool subTaskError = false;
    for (ConversionTask::SubTaskList::const_iterator it =
           taskContext.conversionTask_.subTasks_.begin();
         it != taskContext.conversionTask_.subTasks_.end();
         ++it) {
        ...
        if (it->second == true &&
            it->first->status_ == ConversionTask::FAILED) {
            // A critical secondary dependency has failed.
            // We cannot convert this task without this dependency so fail.
            ERROR_MSG("Errors processing critical dependent task %s\n",
...
bool TaskProcessor::checkSecondaryDependenciesUpToDate(
  DependencyContext& dependencyContext)
{
    BW::vector<DependencyList::Input>& secondaryDeps =
      dependencyContext.depList_.secondaryInputs_;
    for (BW::vector<DependencyList::Input>::iterator it = secondaryDeps.begin();
         it != secondaryDeps.end();
         ++it) {
        // If the hash of any of our secondary dependencies do not match what is
        // stored in our dependency list then we need to convert this task
...
bool TaskProcessor::retrieveSecondaryDependencyListFromCache(
  DependencyContext& dependencyContext,
  ConversionContext& conversionContext)
{
...
bool TaskProcessor::checkIntermediateOutputsUpToDate(
  DependencyContext& dependencyContext,
  ConversionContext& conversionContext)
{
    BW::vector<DependencyList::Output>& intermediateOutputs =
      dependencyContext.depList_.intermediateOutputs_;

    BW::vector<DependencyList::Output>::const_iterator it;
    for (it = intermediateOutputs.begin(); it != intermediateOutputs.end();
         ++it) {
        ...
        if (it->second != hash) {
            if ((conversionContext.converterInfo_.flags_ &
                 ConverterInfo::CACHE_CONVERSION) != 0 &&
                ContentAddressableCache::readFromCache(
                  output, it->second, compiler_)) {
                if (it->second != compiler_.getFileHash(output, true)) {
                    WARNING_MSG(
                      "Corrupted intermediate output retrieved from the cache: "
                      "%s.%IX\n",
                      BWResource::getFilename(output).to_string().c_str(),
                      hash);
                    return false;
                }
...
bool TaskProcessor::checkOutputsUpToDate(DependencyContext& dependencyContext,
                                         ConversionContext& conversionContext)
{
    BW::vector<DependencyList::Output>& outputs =
      dependencyContext.depList_.outputs_;
    ...
    for (it = outputs.begin(); it != outputs.end(); ++it) {
        ...
        if (it->second != hash) {
            if ((conversionContext.converterInfo_.flags_ &
                ...
                if (it->second != compiler_.getFileHash(output, true)) {
                    WARNING_MSG(
                      "Corrupted output retrieved from the cache: %s.%IX\n",
                      BWResource::getFilename(output).to_string().c_str(),
                      hash);
                    return false;
                }
...
void TaskProcessor::updateSecondaryDependencies(
  DependencyContext& dependencyContext)
{
    BW::vector<DependencyList::Input>& secondaryDeps =
      dependencyContext.depList_.secondaryInputs_;
...
void TaskProcessor::updateIntermediateOutputs(
  BW::vector<BW::string>& intermediateFiles,
  TaskContext&            taskContext,
  DependencyContext&      dependencyContext,
  ConversionContext&      conversionContext)
{
    ...
    for (BW::vector<BW::string>::const_iterator it = intermediateFiles.begin();
         it != intermediateFiles.end();
         ++it) {
        // Convert all the intermediate files to relative paths and push them
        // into our intermediate output list
        BW::string output         = *it;
        ...
        INFO_MSG("  %s ->\n", taskContext.conversionTask_.source_.c_str());
        INFO_MSG("      %s\n", output.c_str());
        // Update the hash of all our output files. Make sure we specify force
        // update to ensure the hash gets recalculated.
...
{
    // This function is called by multiple threads simultaneously and needs to
    // be thread safe.
        ...
        if (compiler_.hasTasks()) {
            if (numThreads > numTasks) {
                for (int i = 0; i < numThreads - numTasks; ++i) {
                    // For every available thread create a background task to
                    // process tasks.
                    BgTaskManager::instance().addBackgroundTask(
                      new TaskProcessorTask(*this));
                }
            } else if (numThreads < numTasks) {
                InterlockedExchange(&threadKillCount_, numTasks - numThreads);
            }
        } else if (numTasks == 0) {
            // If the compiler has no tasks left and all our background tasks
            // have completed then we are done.
            break;
        }

        // Sleep for a while and then try to queue some more background tasks.
        // One of our background tasks may have finished prematurely or a
        // background task we were not managing may have completed, freeing up
        // another thread.
        Sleep(100);
    }
}
