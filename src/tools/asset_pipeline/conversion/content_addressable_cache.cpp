...
bool ContentAddressableCache::readFromCache(const BW::string& filename,
                                            uint64            hash,
                                            Compiler&         compiler)
{
    ...
    sprintf_s(cacheFilename,
              MAX_PATH,
              "%s%hhX/%s.%llX",
              cachePath_.c_str(),
              static_cast<uint8>(hash),
              file.c_str(),
              hash);
    ...
    if (!BWResource::instance().fileSystem()->moveFileOrDirectory(tempFilename,
                                                                  filename)) {
        // Erase the temporary file
        BWResource::instance().fileSystem()->eraseFileOrDirectory(tempFilename);
        compiler.onCacheReadMiss(filename);
...
bool ContentAddressableCache::writeToCache(const BW::string& filename,
                                           uint64            hash,
                                           Compiler&         compiler)
{
    ...
    if (!GetTempFileNameA(
          cachePath_.c_str(), file.c_str(), 0, tempCacheFileName)) {
    ...
    if (!BWResource::instance().fileSystem()->moveFileOrDirectory(
