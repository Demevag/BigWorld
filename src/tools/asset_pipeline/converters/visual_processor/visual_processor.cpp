...
const uint64 VisualProcessor::s_TypeId =
  Hash64::compute(VisualProcessor::getTypeName());
                                         ...
                                         DependencyList&   dependencies)
{
    ...
    BW::string baseName = sourcefile.substr(0, sourcefile.find_last_of('.'));
    ...
    dependencies.addSecondaryOutputFileDependency(primitivesProcessedResource,
                                                  false);
                              ...
                              BW::vector<BW::string>& outputFiles)
{
    ...
    while (rsit != rsend) {
        ...
        while (geit != geend) {
            ...
            Moo::VerticesPtr vertices = Moo::VerticesManager::instance()->get(
              verticesName, numNodesValidate);

            if (vertices->nVertices() == 0) {
                ERROR_MSG(
                  "No vertex information in \"%s\".\nUnable to load \"%s\".\n",
                  verticesName.c_str(),
                  sourcefile.c_str());
                return false;
            }
            ...
            while (prit != prend) {
                ...
                primitiveGroupSection->writeVector3(
                  "groupOrigin", primitives->origin(groupIndex));
            }
        }
    }
