...
const uint64 HierarchicalConfigConverter::s_TypeId =
  Hash64::compute(HierarchicalConfigConverter::getTypeName());
...
bool HierarchicalConfigConverter::createDependencies(
  ...
  DependencyList&   dependencies)
{
    ...
    dependencies.addSecondaryDirectoryDependency(
      configDirectory_.to_string(), configFile_.to_string(), false, true, true);
    return true;
}

bool HierarchicalConfigConverter::convert(
  const BW::string&       sourcefile,
  const Compiler&         compiler,
