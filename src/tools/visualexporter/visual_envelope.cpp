...
void VisualEnvelope::initialPoseVertices()
{
    ...
    while (it != end) {
        ...
        bloatV.pos =
          /*initialTransforms_[ boneV.boneIndex_ ] **/ boneV.position_;
    }
}
...
bool VisualEnvelope::init(INode* node, MFXNode* root)
{
    ...
    if (pPhyMod) {
        IPhysiqueExport* phyExport =
          (IPhysiqueExport*)pPhyMod->GetInterface(I_PHYINTERFACE);
        if (phyExport) {
            if (MATRIX_RETURNED !=
                phyExport->GetInitNodeTM(node, initialObjectTransform_)) {
                initialObjectTransform_ = node->GetNodeTM(timeNow());
            }

            // Disable the physique modifier, and grab the original vertex
            // positions so that we only need to grab the weights for each
            // vertex, should also work better with edit mesh modifier etc.
            pPhyMod->DisableMod();
            ...
            if (contextExport) {
                ...
                for (int i = 0; i < nVertices; i++) {
                    ...
                    IPhyVertexExport* vex =
                      contextExport->GetVertexInterface(i);
                    if (vex->GetVertexType() == RIGID_NON_BLENDED_TYPE) {
                        ...
                        IPhyBlendedRigidVertex* rbv =
                          (IPhyBlendedRigidVertex*)vex;
                        ...
                        MessageBox(GetForegroundWindow(),
                                   L"VisualEnvelope::init - unknown vertex "
                                   L"type in physiqued object\n",
                                   L"Visual Exporter",
                                   MB_OK | MB_ICONEXCLAMATION);
#else
                        MessageBox(GetForegroundWindow(),
                                   "VisualEnvelope::init - unknown vertex type "
                                   "in physiqued object\n",
                                   "Visual Exporter",
                                   MB_OK | MB_ICONEXCLAMATION);
#endif
...
#ifdef _UNICODE
                    MessageBox(GetForegroundWindow(),
                               L"Unable to collect initial transform in "
                               L"physiqued object\n"
                               L"The visual may not appear as it should inside "
                               L"the BigWorld engine.",
                               L"Visual Exporter",
                               MB_OK | MB_ICONEXCLAMATION);
#else
                    MessageBox(GetForegroundWindow(),
                               "Unable to collect initial transform in "
                               "physiqued object\n"
                               "The visual may not appear as it should inside "
                               "the BigWorld engine.",
                               "Visual Exporter",
                               MB_OK | MB_ICONEXCLAMATION);
#endif
        ...
        for (int i = 0; i < boneNodes_.size(); i++) {
            ...
            BW::string id = trimWhitespaces(x->getIdentifier()) +
                            BW::string("BlendBone") + identifier_;
            ...
            if (!xx) {
                ...
                xx->setTransform(
                  Inverse(initialTransforms_[boneIndex(x->getMaxNode())]));
                x->addChild(xx);
            }
            x = xx;
            ...
            if (pContext) {
                ...
                for (int i = 0; i < nVertices; i++) {
                    ...
                    for (int j = 0; j < nNodes; j++) {
                        Matrix3 transform;
                        INode*  n =
                          pSkin->GetBone(pContext->GetAssignedBone(i, j));
                        ...
                        indexWeights.insert(
                          std::make_pair(weight, boneIndex(n)));
                    }

                    if (nNodes == 0) {
                        indexWeights.insert(
                          std::make_pair(1.f, boneIndex(node)));
                    }
                    ...
                    boneVertices_.push_back(BoneVertex(p,
                                                       index,
                                                       index2,
                                                       index3,
                                                       weight / sumWeights,
...
#ifdef _UNICODE
                    pNode->setString(
                      trimWhitespaces(bw_wtoutf8(boneNodes_[i]->GetName())) +
                      BW::string("BlendBone") + originalIdentifier_);
#else
...
int VisualEnvelope::boneIndex(INode* node)
{
    NodeVector::iterator it =
      std::find(boneNodes_.begin(), boneNodes_.end(), node);
