...
void MFExp::ExportAnimKeys(INode* node, int indentLevel)
{
    ...
    if (!GetAlwaysSample()) {
        ...
        if (IsKnownController(pC) && IsKnownController(rC) &&
            IsKnownController(sC)) {
            bDoKeys = TRUE;
        }
    }
    ...
    if (bDoKeys) {
        ...
        if (node->GetTMController()->GetPositionController()->NumKeys() ||
            ...
            /*			fprintf(pStream,"%s\t%s {\n", indent.data(),
               ID_TM_ANIMATION); fprintf(pStream,"%s\t\t%s \"%s\"\n",
        ...
        /*		fprintf(pStream,"%s\t%s {\n", indent.data(), ID_TM_ANIMATION);
           ...
           ID_NODE_NAME, FixupName(node->GetName()));*/
...
// but if you turn on follow and banking, it will also affect the rotation
// component. If we want to, we can examine the position, rotation and scale
// controllers and if they all are Linear, Hybrid (bezier) or TCB, then we could
// export the actual keys. This is not at all difficult, but the importer has to
// know the exact interpolation algorithm in order to use it. The source code to
// the interpolation routines are available to ADN members.
//
// For an example of how to export actual keys, look at DumpPoint3Keys() below.
// This method will check the actual controller to determine if the controller
// is known. If we know how to work this controller, its actual keys will be
// exported, otherwise the controller will be sampled using the user specified
// sampling frequency.
...
void MFExp::DumpPoint3Keys(Control* cont, int indentLevel)
{
    ...
    // If the user wants us to always sample, we will ignore the
    // KeyControlInterface
    if (!GetAlwaysSample())
    ...
    if (ikc && cont->ClassID() == Class_ID(TCBINTERP_POINT3_CLASS_ID, 0)) {
        ...
        for (i = 0; i < ikc->GetNumKeys(); i++) {
            ...
            // Format(key.tens), Format(key.cont), Format(key.bias),
            // Format(key.easeIn), Format(key.easeOut));
        }
        //		fprintf(pStream,"%s\t\t}\n", indent.data());
    }
    // Bezier point3
    ...
    // Bezier color
        ...
        /*		fprintf(pStream,"%s\t\t%s \"%s\" {\n", indent.data(),
           ID_CONTROL_POINT3_SAMPLE, FixupName(name));*/
        ...
        if (cont->IsAnimated()) {
            ...
            while (animRange.InInterval(t)) {
                ...
                if (range.End() > cont->GetTimeRange(TIMERANGE_ALL).End()) {
                    ...
                    t = (range.End() / GetTicksPerFrame() + GetKeyFrameStep()) *
                        GetTicksPerFrame();
                }
            }
        }
...
void MFExp::DumpFloatKeys(Control* cont, int indentLevel)
{
    ...
    // Bezier float
        ...
        cont->GetClassName(name);
        /*		fprintf(pStream,"%s\t\t%s \"%s\" {\n", indent.data(),
           ID_CONTROL_FLOAT_SAMPLE, FixupName(name));*/
    ...
    if (ikc && cont->ClassID() == Class_ID(TCBINTERP_ROTATION_CLASS_ID, 0)) {
        ...
        if (numKeys = ikc->GetNumKeys()) {
            ...
            for (i = 0; i < numKeys; i++) {
                ...
                /*				fprintf(pStream, "%s\t\t\t%s %d\t%s\n",
                                    indent.data(),
                                     // Quaternions are converted to AngAxis
                   when written to file ID_ROT_KEY, key.time,
                                    Format(key.val));*/
