...
bool ExportSettings::displayDialog(HWND hWnd)
{
    if (!DialogBoxParam(hInstance,
                        MAKEINTRESOURCE(IDD_VISUALEXPORT_DLG),
                        hWnd,
                        dialogProc,
                        (LPARAM)this)) {
        return false;
...
INT_PTR CALLBACK ExportSettings::dialogProc(HWND   hWnd,
                                            UINT   msg,
                                            WPARAM wParam,
                                            LPARAM lParam)
{
    ExportSettings* settings =
      (ExportSettings*)GetWindowLongPtr(hWnd, GWLP_USERDATA);
    ...
    switch (msg) {
        case WM_INITDIALOG: {
            ...
            CheckDlgButton(hWnd,
                           IDC_KEEP_EXISTING_MATERIALS,
                           settings->keepExistingMaterials_);
            CheckDlgButton(hWnd, IDC_SNAP_VERTICES, settings->snapVertices_);

#ifdef UNICODE
            BW::wstring typeDesc = L"Checking against: " +
                                   bw_acptow(settings->visualTypeIdentifier_);
#else
            BW::string typeDesc =
              "Checking against: " + settings->visualTypeIdentifier_;
#endif // UNICODE
            ...
            SendDlgItemMessage(hWnd,
                               ...
                               WM_SETTEXT,
                               0,
                               (LPARAM)typeDesc.c_str());
        } break;
        case WM_COMMAND:
            switch (LOWORD(wParam)) {
                case IDOK:
                    settings->allowScale_ =
                      IsDlgButtonChecked(hWnd, IDC_ALLOWSCALE) == TRUE;
                    settings->bumpMapped_ =
                      IsDlgButtonChecked(hWnd, IDC_BUMPMAP) == TRUE;
                    settings->keepExistingMaterials_ =
                      IsDlgButtonChecked(hWnd, IDC_KEEP_EXISTING_MATERIALS) ==
                      TRUE;
                    settings->snapVertices_ =
                      IsDlgButtonChecked(hWnd, IDC_SNAP_VERTICES) == TRUE;
                    ...
                    if (IsDlgButtonChecked(hWnd, IDC_EXPORT_NORMAL)) {
                        ...
                        settings->exportMode_ = STATIC;
                                 ...
                                 hWnd, IDC_EXPORT_STATIC_WITH_NODES)) {
...
INT_PTR CALLBACK ExportSettings::settingsDialogProc(HWND   hWnd,
                                                    UINT   msg,
                                                    WPARAM wParam,
                                                    LPARAM lParam)
{
    ...
    switch (msg) {
        case WM_INITDIALOG: {
            ...
            CheckDlgButton(
              hWnd, IDC_VISUALCHECKER, !settings->visualCheckerEnable());

            CheckDlgButton(hWnd, IDC_CYLINDRICAL, settings->fixCylindrical());

            CheckDlgButton(
              hWnd, IDC_LEGACY_ORIENTATION, settings->useLegacyOrientation());
        } break;
        case WM_COMMAND:
            switch (LOWORD(wParam)) {
                case IDOK: {
                    ...
                    settings->visualCheckerEnable(
                      IsDlgButtonChecked(hWnd, IDC_VISUALCHECKER) != TRUE);

                    settings->fixCylindrical(
                      IsDlgButtonChecked(hWnd, IDC_CYLINDRICAL) == TRUE);

                    settings->useLegacyOrientation(
                      IsDlgButtonChecked(hWnd, IDC_LEGACY_ORIENTATION) == TRUE);
