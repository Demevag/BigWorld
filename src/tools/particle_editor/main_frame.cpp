...
static UINT indicators[] = {
    ...
    ID_PERFORMANCE_PANE, ID_INDICATOR_CAPS, ID_INDICATOR_NUM, ID_INDICATOR_SCRL,
};
...
int MainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
    ...
    particleDirectory_ =
      bw_utf8tow(
        Options::getOptionString("defaults/startDirectory",
                                 bw_wtoutf8(particleDirectory_.GetString())))
        .c_str();
    ...
    if (!wndStatusBar_.Create(this) ||
        !wndStatusBar_.SetIndicators(indicators,
...
/*virtual*/ LRESULT MainFrame::WindowProc(UINT   message,
                                          WPARAM wParam,
                                          LPARAM lParam)
{
    BW_GUARD;
...
void MainFrame::SetPerformancePaneText(CString text)
{
    ...
    wndStatusBar_.SetPaneInfo(
      index, ID_PERFORMANCE_PANE, SBPS_NOBORDERS, size.cx);
...
void MainFrame::ChangeToActionPropertyWindow(int                     index,
...
void MainFrame::SaveUndoState(int               actionKind,
 ...
 *
 *  @param  option - whether the particle system is now potentially dirty or
 * not.
 *  @param  actionKind - integer specifying the type of operation, for gui
 * refresh
 *  @param  changeDesc - string description of the operation, for undo/redo
 *  @param  wait       - signifies a batched interactive operation, for
 * undo/redo
 *  @param  addBarrier - add an undo/redo barrier (last call of a batched
 * operation)
 */
void MainFrame::PotentiallyDirty(bool                   option,
                                 ...
                                 bool                   addBarrier)
{
    ...
    if (option) {
        if (!waitForLButtonUp) {
            ...
            if ((changeDesc != pendingLBtnUpDesc_) ||
...
ActionSelection* MainFrame::GetActionSelection() const
{
    ...
    return (ActionSelection*)PanelManager::instance().panels().getContent(
...
int MainFrame::PromptSave(UINT type, bool clearUndoStack /*= false*/)
{
    ...
    if (potentiallyDirty_) {
        ...
        if (actionSelection->isSelectionReadOnly()) {
            ...
            result = ::MessageBox(AfxGetMainWnd()->GetSafeHwnd(),
...
void MainFrame::ParticlesDirectory(CString directory)
{
    ...
    Options::setOptionString("defaults/startDirectory",
                             bw_wtoutf8(particleDirectory_.GetString()));
}
...
BOOL MainFrame::CursorOverGraphicsWnd() const
{
    ...
    if (fore != PeShell::instance().hWndApp() &&
...
BOOL MainFrame::Create(LPCTSTR         lpszClassName,
                       ...
                       CCreateContext* pContext)
{
    ...
    dwStyle |= WS_MAXIMIZE;
    BOOL ans = CFrameWnd::Create(lpszClassName,
                                 lpszWindowName,
                                 dwStyle,
                                 rect,
                                 pParentWnd,
...
void MainFrame::OnBackgroundColor()
{
    ...
    if (!colorDialogThread_) {
        colorDialogThread_ = (ColorPickerDialogThread*)(AfxBeginThread(
          RUNTIME_CLASS(ColorPickerDialogThread)));
    }
}
...
void MainFrame::UpdateBackgroundColor()
{
    ...
    MainFrame::instance()->BgColour(
      Moo::Colour(newColor.x, newColor.y, newColor.z, newColor.w));
}
...
{
    getStatusBar().SetPaneText(getStatusBar().CommandToIndex(id), text, TRUE);
}
