  ...
  bool                   drawIt)
{
    ...
    if (vectorGenerator->nameID() == PointVectorGenerator::nameID_) {
        ...
        if (drawVectors) {
            newGizmo = new VectorGizmo(MODIFIER_SHIFT | MODIFIER_ALT,
                                       matrixProxy,
                                       wireColor,
                                       0.015f,
                                       originMatrixProxy,
            ...
            newGizmo = new PositionGizmo(
              MODIFIER_SHIFT | MODIFIER_ALT, matrixProxy, NULL, 0.1f);
        }
        ...
        matrixProxy->recordState(); // record initial state (otherwise will
                                    // forget it on first edit)
        ...
        newGizmo = new LineGizmo(matrixProxy1,
                                 matrixProxy2,
                                 wireColor,
                                 drawVectors,
                                 originMatrixProxy);
        ...
        newGizmo = new CylinderGizmo(matrixProxy1,
                                     matrixProxy2,
                                     radiusProxy,
                                     wireColor,
                                     drawVectors,
        ...
        newGizmo = new SphereGizmo(matrixProxy,
                                   radiusProxy1,
                                   radiusProxy2,
                                   wireColor,
                                   drawVectors,
        ...
        // Bug 5075 fix: added a sensable maximum radius check of 365 km
        // (maximum possible distance in an space).
        if (setDefaultRadius || (radiusProxy1->get() > 365000.f) ||
        ...
        newGizmo = new BoxGizmo(matrixProxy1,
                                matrixProxy2,
                                wireColor,
                                drawVectors,
                                originMatrixProxy);
    ...
    if (keepCurrentEditors) {
        ...
        for (GeneralEditor::Editors::const_iterator it = oldEds.begin();
             it != oldEds.end();
             it++) {
