...
TreeNode* TreeNode::Serialise(DataSectionPtr data, bool load)
{
    BW_GUARD;
    ...
    if (load) {
        ...
        for (DataSectionIterator it = data->begin(); it != data->end(); ++it) {
            ...
            if (ds->sectionName() == TREE_NODE_STR) {
                ...
                if (nodeidx != children_.size()) {
                    TreeNode* selChild =
                      children_[nodeidx]->Serialise(ds, load);
        ...
        if (selected)
            result = this;
        for (size_t i = 0; i < children_.size(); ++i) {
            DataSectionPtr childSection = data->newSection(TREE_NODE_STR);
...
TreeNode* TreeNode::CopyExpandState(TreeNode const* other)
{
    ...
    if (tree_ != NULL && htreeitem_ != NULL) {
        tree_->Expand(htreeitem_,
                      other->IsExpanded() ? TVE_EXPAND : TVE_COLLAPSE);
    }
...
/*static*/ BW::string const& TreeNode::DefaultSerializeName()
{
...
/*virtual*/ void TreeNode::OnHide()
{
    ...
    if (htreeitem_ != NULL) {
        expanded_ = (tree_->GetItemState(htreeitem_, TVIS_EXPANDED) &
...
TreeNode* TreeControl::AddNode(TreeNode* newNode,
...
)
{
    ...
    if (prevSibling == AtEnd()) {
        ...
        if (idx != parent->children_.size()) {
            parent->children_.insert(parent->children_.begin() + idx + 1,
                                     newNode);
        } else {
    ...
    if (IsShown(parent)) {
        ...
        if (prevSibling != AtStart() && prevSibling != AtEnd() &&
            prevSibling->htreeitem_ != NULL) {
            hsibling = prevSibling->htreeitem_;
        }
        HTREEITEM hitem =
          InsertItem(bw_utf8tow(newNode->label_).c_str(), hparent, hsibling);
    ...
    if (node == NULL) {
        if (show) {
            ...
            for (size_t i = 0; i < root_->children_.size(); ++i) {
                lastHItem =
                  CreateHTreeItems(root_->children_[i], TVI_ROOT, lastHItem);
            }
            root_->OnShow();
        } else {
            DeleteAllItems();
        ...
        if (show) {
            ...
            TreeNode* parent  = node->parent_;
            HTREEITEM hparent = parent == root_ ? TVI_ROOT : parent->htreeitem_;
...
HTREEITEM
TreeControl::CreateHTreeItems(TreeNode* node,
                              HTREEITEM parent,
                              HTREEITEM prevSibling)
{
    ...
    HTREEITEM newItem =
      InsertItem(bw_utf8tow(node->label_).c_str(), parent, prevSibling);
    ...
    for (size_t i = 0; i < node->children_.size(); ++i) {
        subPrevSibling =
          CreateHTreeItems(node->children_[i], newItem, subPrevSibling);
    }
    if (node->expanded_)
...
void TreeControl::OnMouseMove(UINT flags, CPoint point)
{
    ...
    if (dragItem_ != NULL && dragItem_->CanDragDrop() != DROPEFFECT_NONE &&
        (flags & MK_LBUTTON) != 0) {
