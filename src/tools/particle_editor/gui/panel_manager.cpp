...
/*static*/ bool PanelManager::init(CFrameWnd*     mainFrame,
                                   CWnd*          mainView,
                                   const wchar_t* layoutFilename)
{
    BW_GUARD;
...
bool PanelManager::initPanels(const wchar_t* layoutFilename)
{
    ...
    ActionSelection* actionSelection =
      (ActionSelection*)this->panels().getContent(ActionSelection::contentID);
...
void PanelManager::ualAddItemToHistory(BW::string filePath)
{
    ...
    UalManager::instance().history().add(
      AssetInfo(L"FILE", bw_utf8tow(fname), bw_utf8tow(longText)));
}
...
bool PanelManager::loadDefaultPanels(GUI::ItemPtr item)
{
    ...
    GUITABS::PanelHandle bottomPanel =
      this->panels().insertPanel(UalDialog::contentID, GUITABS::BOTTOM);
    this->panels().insertPanel(
      PageMessages::contentID, GUITABS::TAB, bottomPanel);
                                  ...
                                  const wchar_t* defaultLayoutFilename)
{
    ...
    CWaitCursor wait;
    if (MessageBox(m_mainFrame->GetSafeHwnd(),
...
PanelManager::PanelManager()
  : GUI::ActionMaker<PanelManager>("doShowSidePanel",
                                   &PanelManager::showSidePanel)
  , GUI::ActionMaker<PanelManager, 1>("doHideSidePanel",
                                      &PanelManager::hideSidePanel)
  , GUI::ActionMaker<PanelManager, 2>("setLanguage", &PanelManager::setLanguage)
  , GUI::UpdaterMaker<PanelManager>("updateSidePanel",
                                    &PanelManager::updateSidePanel)
  , GUI::UpdaterMaker<PanelManager, 1>("updateUalPanel",
                                       &PanelManager::updateUalPanel)
  , GUI::UpdaterMaker<PanelManager, 2>("updateMsgsPanel",
                                       &PanelManager::updateMsgsPanel)
  , GUI::UpdaterMaker<PanelManager, 3>("updateLanguage",
...
void PanelManager::finishLoad()
{
    if (this->panels().getContent(PageMessages::contentID) == NULL) {
        // If upgrading to the new PE with page messages, make sure the panel is
        // there
        GUITABS::Content* ualPanel =
          this->panels().getContent(UalDialog::contentID);
        this->panels().insertPanel(
          PageMessages::contentID, GUITABS::TAB, ualPanel);
    }
...
void PanelManager::ualUpdateDrag(UalItemInfo* ii)
{
    ...
    bool inView =
      ::WindowFromPoint(CPoint(ii->x(), ii->y())) == activeView->m_hWnd;
