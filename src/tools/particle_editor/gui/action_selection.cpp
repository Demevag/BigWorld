...
namespace {
    std::pair<UINT, UINT> actionDescriptions[] = {
        ...
        std::make_pair<UINT, UINT>(IDS_MATRIXSWARM_DESC,
                                   PSA_MATRIX_SWARM_TYPE_ID),
    ...
    void actionDescriptionTTCB(unsigned int item,
                               BW::string&  tooltip,
                               void* /*data*/
    )
    {
...
/*virtual*/ BOOL ActionSelection::OnInitDialog()
{
    ...
    searchFilter_.Create(WS_CHILD | WS_VISIBLE,
                         extents,
                         this,
                         SEARCH_FILTER_ID,
                         IDB_SEARCHFILTER,
    ...
    UalManager::instance().dropManager().add(
      new UalDropFunctor<ActionSelection>(ParticleEditorView::instance(),
                                          "xml",
                                          this,
...
bool ActionSelection::save()
{
    ...
    if (mn != NULL) {
        ...
        if (mps != NULL) // may not yet have been loaded
        {
            ...
            if (!dirFile.empty()) {
                HANDLE mailSlot =
                  CreateFile(L"\\\\.\\mailslot\\WorldEditorUpdate",
                             GENERIC_WRITE,
                             FILE_SHARE_READ,
                             NULL,
                             OPEN_EXISTING,
...
bool ActionSelection::reload()
{
    ...
    selection_ = NULL;
    BW::string directory =
      bw_wtoutf8(MainFrame::instance()->ParticlesDirectory().GetString());
 ...
 *                      take longer.
 */
void ActionSelection::reserialise(DataSectionPtr data,
                                  bool           load,
                                  bool           transient,
                                  bool           undoing /*=true*/)
{
    ...
    if (load) {
        ...
        sel                    = NULL;
        MetaNode*      newNode = addMetaParticleSystem(mps,
                                                  mnode,
                                                  false,
                                                  false,
                                                  mnode->GetLabel(),
...
void ActionSelection::setSubDlg(CView* dlg)
{
    ...
    if (!IsWindow(dlg->GetSafeHwnd())) {
        ...
        BOOL ok = dlg->Create(
          classname.c_str(), L"properties", WS_CHILD, dlgRect, this, 0, 0);
    }
...
void ActionSelection::addSystemOffsetGizmo()
{
    ...
    VectorGeneratorMatrixProxy<ParticleSystem>* matrixProxy =
      new VectorGeneratorMatrixProxy<ParticleSystem>(
        &*system, &ParticleSystem::localOffset, &ParticleSystem::localOffset);
    generalEditor->addProperty(
      new GenPositionProperty("system origin", matrixProxy));
    sysOffsGizmo_ =
      new PositionGizmo(MODIFIER_SHIFT | MODIFIER_ALT, matrixProxy, NULL, 0.1f);
...
void ActionSelection::cleanupAppendPS()
{
    ...
    for (int i = (int)appendedPS_.size() - 1; i >= 0; --i) {
        ...
        MetaParticleSystem::ParticleSystems& systems      = mps->systemSet();
        for (MetaParticleSystem::ParticleSystems::iterator it = systems.begin();
    ...
    if (dispInfo->item.pszText != NULL) {
        ...
        switch (renameResult) {
            ...
            case NAME_INVALID_CHAR_INC_SPACE: {
                BW::wstring msg =
                  Localise(controls::loadStringW(IDS_INVALIDFILENAME).c_str());
            ...
            case NAME_INVALID_CHAR: {
                BW::wstring msg =
                  Localise(controls::loadStringW(IDS_INVALIDNAME).c_str());
                AfxMessageBox(msg.c_str(), MB_OK);
                particleTree_.EditLabel(dispInfo->item.hItem);
                *result = FALSE;
            } break;
            case NAME_NOT_UNIQUE: {
                BW::wstring text =
                  Localise(controls::loadStringW(IDS_NOTUNIQUENAME).c_str());
...
void ActionSelection::OnNMClickParticletree(NMHDR* nmhdr, LRESULT* result)
{
    ...
    if (node && (htFlags & TVHT_ONITEMSTATEICON)) {
        ...
        if (metaNode != NULL) {
            // Make sure the children have been created so we can pass the
            // message down.
            metaNode->OnExpand();
            for (size_t i = 0; i < particleTree_.NumberChildren(metaNode);
                 i++) {
                PSNode* child =
                  dynamic_cast<PSNode*>(particleTree_.GetChild(metaNode, i));
                MF_ASSERT(child != NULL);
                child->getParticleSystem()->enabled(boxState);
                particleTree_.SetCheck(*child,
                                       boxState ? BST_CHECKED : BST_UNCHECKED);
            }
        }
        ...
        if (psNode != NULL) {
            ...
            if ((GetAsyncKeyState(VK_LCONTROL) < 0) ||
                ...
                // first check to see if any other nodes are enabled. We do this
                // to see if we should be enabling this node and disabling
                // others or vice versa
                bool anyEnabled = false;
                for (size_t i = 0; i < particleTree_.NumberChildren(parent);
                     i++) {
                    PSNode* child =
                      dynamic_cast<PSNode*>(particleTree_.GetChild(parent, i));
                    if (psNode != child) {
                ...
                for (size_t i = 0; i < particleTree_.NumberChildren(parent);
                    ...
                    particleTree_.SetCheck(
                      *child, anyEnabled ? BST_UNCHECKED : BST_CHECKED);
                }
            } else
                ps->enabled(boxState);

            // If we have all particle systems disabled then we might as well
            // disable the metaPS box and if all of them are enabled then we
            // enable the metaPS box
        ...
        ActionNode* actionNode = dynamic_cast<ActionNode*>(node);
        if (actionNode != NULL) {
            // Special Case: Make sure you cannot turn off System Prop or Render
            // Prop checkbox
...
void ActionSelection::OnSelectComponent(TreeNode* node)
{
    ...
    if (actionNode == NULL) {
        ...
        MainFrame::instance()->ChangeToActionPropertyWindow(
          metaNode ? ActionNode::AT_META_PS : ActionNode::AT_PS, NULL);
    } else {
        // System or renderer properties case:
        if (actionNode->getActionType() != ActionNode::AT_ACTION) {
            MainFrame::instance()->ChangeToActionPropertyWindow(
              (int)actionNode->getActionType(), NULL);
        }
        // An actual component:
        else {
    ...
    if (metanode != NULL) {
        ...
        if (id != IDCANCEL) {
            ...
            if (oldsystem != NULL) {
                ...
                MetaData::updateCreationInfo(newsystem->metaData());
                addMetaParticleSystem(newsystem, NULL, true, false, label);
                done = true;
            }
        }
        if (id == IDNO) {
...
void ActionSelection::OnAddAction()
{
    ...
    ParticleSystemActionPtr action =
      ParticleSystemAction::createActionOfType(id);
...
void ActionSelection::OnAddEnabled(CCmdUI* cmdui)
{
    ...
    if (anode != NULL) {
        ...
        if (actionType == ActionNode::AT_SYS_PROP ||
            ...
            (actionType == ActionNode::AT_ACTION &&
             anode->getAction() != NULL)) {
...
void ActionSelection::OnSize(UINT type, int cx, int cy)
{
    ...
    if (subDlg_ != NULL) {
        ...
        subDlg_->SetWindowPos(
          0, rect.left, rect.top, rect.Width(), rect.Height(), SWP_NOZORDER);
    }
}
 ...
 */
BW::string ActionSelection::generateUniquePSName(MetaParticleSystemPtr ms,
                                                 ...
                                                 bool nameIsNew) const
{
    ...
    while (true) {
        ...
        StringUtils::incrementT(
          name, nameIsNew ? StringUtils::IS_END : StringUtils::IS_EXPLORER);
    }
 ...
 */
BW::string ActionSelection::generateUniqueMetaSystemName(
...
) const
{
    ...
    while (true) {
        ...
        // Generate another potential name:
        StringUtils::incrementT(
          result, nameisNew ? StringUtils::IS_END : StringUtils::IS_EXPLORER);
...
ParticleSystemPtr ActionSelection::generateNewParticleSystem() const
{
    ...
    BW::string        defaultTexture = Options::getOptionString(
      "defaults/renderer/spriteTexture", Moo::TextureManager::notFoundBmp());
...
/*virtual*/
DROPEFFECT
ActionSelection::OnDragEnter(CWnd*           window,
...
/*virtual*/
DROPEFFECT
ActionSelection::OnDragOver(CWnd*           window,
                            COleDataObject* dataObject,
                            DWORD           keyState,
                            CPoint          point)
{
 ...
 */
DROPEFFECT
ActionSelection::OnDragTest(CWnd*           window,
                            COleDataObject* dataObject,
                            DWORD           keyState,
                            CPoint          point)
{
 ...
 */
DROPEFFECT
ActionSelection::OnDragTestAction(CWnd*           window,
                                  COleDataObject* dataObject,
                                  DWORD           keyState,
                                  CPoint          point)
{
    ...
    if (an != NULL || pn != NULL) {
        ...
        if (canInsertAction(
              pn, actionDescriptions[componentList_.GetCurSel()].second))
 ...
 */
DROPEFFECT
ActionSelection::OnDragTestTree(CWnd*           window,
                                COleDataObject* dataObject,
                                DWORD           keyState,
                                CPoint          point)
{
    ...
    if (dragANode != NULL) {
        ...
        if (dragANode->getAction() != NULL) {
            ...
            if (hitANode != NULL || hitPSNode != NULL) {
                ...
                if (canInsertAction(hitPSNode,
                                    dragANode->getAction()->typeID()))
                    return possibleFx;
            }
        }
        // Was it system or render properties?
        else {
            if (hitANode != dragANode &&
                ((hitANode != NULL &&
 ...
 */
BOOL ActionSelection::OnDrop(CWnd*           window,
                             COleDataObject* dataObject,
 ...
 */
/*virtual*/
BOOL ActionSelection::OnDropAction(CWnd*           window,
                                   COleDataObject* dataObject,
                                   DROPEFFECT      dropEffect,
                                   CPoint          point)
{
        ...
        if (anode != NULL) {
            ...
            if (canInsertAction(psnode, id)) {
                MainFrame::instance()->PotentiallyDirty(
                  ...
                  UndoRedoOp::AK_NPARAMETER,
                  LocaliseUTF8(
                    L"PARTICLEEDITOR/GUI/ACTION_SELECTION/ADD_ACTION"));
                fixupAction(psa, psnode->getParticleSystem());
 ...
 */
/*virtual*/
BOOL ActionSelection::OnDropTree(CWnd*           window,
                                 COleDataObject* dataObject,
                                 DROPEFFECT      dropEffect,
                                 CPoint          point)
{
    ...
    if (dragANode != NULL) {
        ...
        if (dragAction != NULL) {
            ...
            PSNode* sourcePNode = dynamic_cast<PSNode*>(dragANode->GetParent());
            ...
            if (sourcePNode != NULL && destPNode != NULL &&
                sourcePNode != destPNode) {
                ParticleSystemPtr sourcePS =
                  sourcePNode->getParticleSystem()->clone();
    ...
    // Dragged a PSNode?
        ...
        if (hitPSNode != NULL) {
            ...
            newNode  = addParticleSystem(
              dropPS, hitMNode, hitPSNode, false, false, label);
            doneSomething = true;
        ...
        // Dropped onto a MetaNode?
            ...
            if (dropMPS) {
                ...
                hitMNode->OnExpand();
                newNode = addParticleSystem(
                  dropPS, hitMNode, NULL, false, false, label);
                doneSomething = true;
    ...
    // Dragged a MetaNode?
        ...
        if (hitMNode != NULL || (flags & TVHT_NOWHERE) == TVHT_NOWHERE) {
            newNode =
              addMetaParticleSystem(dropMPS, hitMNode, false, false, label);
            doneSomething = true;
            sameParent    = true;
        }
    }
 ...
 */
void ActionSelection::copySystemProperties(ParticleSystemPtr source,
 ...
 *  @param dest          The destination ParticleSystem.
 */
void ActionSelection::copyRendererProperties(ParticleSystemPtr source,
                                             ParticleSystemPtr dest)
{
    BW_GUARD;
 ...
 */
MetaNode* ActionSelection::addMetaParticleSystem(
...
)
{
    ...
    MetaNode* mn = (MetaNode*)particleTree_.AddNode(
      new MetaNode(directory(), newname), NULL, prevSibling);
    ...
    if (isNew || !keepName)
        mps->save(filename,
                  fullpath.substr(0, fullpath.find(name)),
                  true); // new node, so need only transient
    mn->onSave();
 ...
 *  @returns             The new node.
 */
PSNode* ActionSelection::addParticleSystem(
...
)
{
    ...
    PSNode* psnode =
      (PSNode*)particleTree_.AddNode(new PSNode(ps), parent, prevSibling);
 ...
 *  @returns             The new node.
 */
ActionNode* ActionSelection::addAction(ParticleSystemActionPtr psa,
                                       PSNode*                 parent,
...
)
{
    ...
    // Update the tree control:
    ActionNode* an = (ActionNode*)particleTree_.AddNode(
      new ActionNode(psa, actionname), parent, prevSibling);
...
bool ActionSelection::canInsertAction(PSNode const* node, int actionID) const
{
    ...
    if (actionID == PSA_COLLIDE_TYPE_ID) {
        ...
        for (ParticleSystem::Actions::iterator it = actions.begin();
             it != actions.end();
             ++it) {
 ...
 */
ActionSelection::RenameResult ActionSelection::checkUniqueName(
  TreeNode const*   node,
  BW::string const& label) const
{
    ...
    if (metanode != NULL &&
        !StringUtils::makeValidFilenameT(validLabel, '_', false))
        return NAME_INVALID_CHAR_INC_SPACE;
    if (metanode == NULL &&
        !StringUtils::makeValidFilenameT(validLabel, '_', true))
        ...
        if (parent != NULL) {
            for (size_t i = 0; i < parent->NumberChildren(); ++i) {
                ...
                if (thispsnode != NULL && thispsnode != psnode) {
                    if (strcmpi(thispsnode->GetLabel().c_str(),
                                label.c_str()) == 0) {
                        return NAME_NOT_UNIQUE;
                    }
                }
 ...
 */
/*static*/
void ActionSelection::fixupAction(ParticleSystemActionPtr newAction,
