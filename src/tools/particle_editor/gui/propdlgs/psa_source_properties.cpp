...
void PsaSourceProperties::OnInitialUpdate()
{
    ...
    positionGeneratorCustodian_.setVectorGeneratorSetFn(
      &SourcePSA::setPositionSource);
    positionGeneratorCustodian_.setVectorGeneratorGetFn(
      &SourcePSA::getPositionSource);
    positionGeneratorCustodian_.setVectorGeneratorOwner(action());
    positionGeneratorCustodian_.setGizmoProperties(0xFFFF0000);
    positionGeneratorCustodian_.setPositionControls(&positionGeneratorX_,
                                                    &positionGeneratorY_,
                                                    &positionGeneratorZ_,
                                                    &positionGeneratorX2_,
                                                    &positionGeneratorY2_,
                                                    &positionGeneratorZ2_);

    velocityGeneratorCustodian_.setComboBox(&velocityGenerator_);
    velocityGeneratorCustodian_.setVectorGeneratorSetFn(
      &SourcePSA::setVelocitySource);
    velocityGeneratorCustodian_.setVectorGeneratorGetFn(
      &SourcePSA::getVelocitySource);
    ...
    velocityGeneratorCustodian_.setPositionControls(&velocityGeneratorX_,
                                                    &velocityGeneratorY_,
                                                    &velocityGeneratorZ_,
                                                    &velocityGeneratorX2_,
                                                    &velocityGeneratorY2_,
                                                    &velocityGeneratorZ2_);

    // initialise dependencies
    positionGeneratorCustodian_.setParentVectorGeneratorCustodian(NULL);
    positionGeneratorCustodian_.setChildVectorGeneratorCustodian(
      &velocityGeneratorCustodian_);
    velocityGeneratorCustodian_.setParentVectorGeneratorCustodian(
      &positionGeneratorCustodian_);
    ...
    bool Randomise =
      mesh && (initialOrientRandomise_.GetCheck() == BST_CHECKED);
    ...
    bool sprite =
      (pSystem->pRenderer()->nameID() == SpriteParticleRenderer::nameID_);
...
void PsaSourceProperties::SetParameters(SetOperation task)
{
    ...
    if (task == SET_CONTROL) {
        ...
        if (sprite) {
            ...
            initialOrientRandomise_.SetCheck(
              (randRotation != Vector2::zero()) ? BST_CHECKED : BST_UNCHECKED);
        }
    } else // set the psa
    {
        // generators

        // Bug 5105 Fix: Added checks for the "show" checkbox for drawing of
        // gizmo.
        positionGeneratorCustodian_.updateOwner(
          positionGeneratorShow_.GetCheck() & BST_CHECKED);
        velocityGeneratorCustodian_.updateOwner(
          velocityGeneratorShow_.GetCheck() & BST_CHECKED);
        ...
        if (sprite) {
            ...
            Vector2 randRotation =
              Vector2(spriteSpinRand_.GetValue(),
            ...
            SpriteParticleRenderer* sRenderer =
              static_cast<SpriteParticleRenderer*>(
                &*MainFrame::instance()
                    ->GetCurrentParticleSystem()
                    ->pRenderer());
            // Bug 5147 fix: the rotated flag didn't take into account the
            // initial rotation
            ...
            if (initialOrientRandomise_.GetState() & 0x0003) {
                randRotation =
                  Vector2(DEG_TO_RAD(initialOrientRandomiseX_.GetValue()),
...
void PsaSourceProperties::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control(
      pDX, IDC_PSA_SOURCE_VELOCITY_GENERATOR_SHOW, velocityGeneratorShow_);
    DDX_Control(
      pDX, IDC_PSA_SOURCE_POSITION_GENERATOR_SHOW, positionGeneratorShow_);
    ...
    DDX_Control(
      pDX, IDC_PSA_SOURCE_INITIALORIENT_RANDOMISE, initialOrientRandomise_);
    DDX_Control(pDX,
                IDC_PSA_SOURCE_INITIALORIENT_RANDOMISE_MIN,
                initialOrientRandomiseX_);
    DDX_Control(pDX,
                IDC_PSA_SOURCE_INITIALORIENT_RANDOMISE_MAX,
                initialOrientRandomiseY_);
    ...
    DDX_Control(
      pDX, IDC_PSA_SOURCE_SPRITE_INITORIENT_RAND, spriteInitialOrientRand_);
    ...
    DDX_Control(
      pDX, IDC_PSA_SOURCE_POSITION_GENERATOR_X2, positionGeneratorX2_);
    DDX_Control(
      pDX, IDC_PSA_SOURCE_POSITION_GENERATOR_Y2, positionGeneratorY2_);
    DDX_Control(
      pDX, IDC_PSA_SOURCE_POSITION_GENERATOR_Z2, positionGeneratorZ2_);
    ...
    DDX_Control(
      pDX, IDC_PSA_SOURCE_VELOCITY_GENERATOR_X2, velocityGeneratorX2_);
    DDX_Control(
      pDX, IDC_PSA_SOURCE_VELOCITY_GENERATOR_Y2, velocityGeneratorY2_);
    DDX_Control(
      pDX, IDC_PSA_SOURCE_VELOCITY_GENERATOR_Z2, velocityGeneratorZ2_);
...
ON_BN_CLICKED(IDC_PSA_SOURCE_POSITION_GENERATOR_SHOW,
              OnBnClickedPsaSourcePositionGeneratorShow)
ON_BN_CLICKED(IDC_PSA_SOURCE_VELOCITY_GENERATOR_SHOW,
              OnBnClickedPsaSourceVelocityGeneratorShow)
ON_BN_CLICKED(IDC_PSA_SOURCE_RANDOM_SPIN, OnBnClickedPsaSourceButton)
ON_BN_CLICKED(IDC_PSA_SOURCE_INITIALORIENT_RANDOMISE,
              OnBnClickedPsaSourceButton)
...
ON_EN_CHANGE(IDC_PSA_SOURCE_SPRITE_SPINRATE_RAND,
             OnEnChangePsaSourceSpriteSpinrateRand)
END_MESSAGE_MAP()
...
void PsaSourceProperties::OnEnChangePsaSourceSpriteSpinrateRand()
{
    ...
    // PsaProperties::OnInitDialog() function and call
