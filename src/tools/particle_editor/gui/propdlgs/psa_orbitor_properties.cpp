...
void PsaOrbitorProperties::addPositionGizmo()
{
    ...
    positionMatrixProxy_ = new VectorGeneratorMatrixProxy<PsaOrbitorProperties>(
      this, &PsaOrbitorProperties::position, &PsaOrbitorProperties::position);
    generalsDaughter->addProperty(
      new GenPositionProperty(Name("vector"), positionMatrixProxy_));
    positionGizmo_ =
      new PositionGizmo(MODIFIER_ALT, positionMatrixProxy_, NULL, 0.1f, true);
