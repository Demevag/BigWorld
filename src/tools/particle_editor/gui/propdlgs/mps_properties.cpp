...
ON_BN_CLICKED(IDC_CHECKBOX_DRAW_HELPER_MODEL,
              &MpsProperties::OnCbClickedDrawHelperModel)
ON_BN_CLICKED(IDC_BTN_CHOOSE_HELPER_MODEL,
              &MpsProperties::OnBnClickedChooseHelperModel)
ON_CBN_SELCHANGE(IDC_COMBO_CHOOSE_HELPER_MODEL_HARDPOINT,
                 OnCbnSelchangeCenterOnHardpoint)
END_MESSAGE_MAP()
...
void MpsProperties::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control(pDX,
                IDC_COMBO_CHOOSE_HELPER_MODEL_HARDPOINT,
                ctlComboHelperModelHardpoint_);
}
...
void MpsProperties::OnInitialUpdate()
{
    ...
    if (!PeModule::instance().helperModelName().empty()) {
        ...
        BW::vector<BW::string>& hardPointNames =
          PeModule::instance().getHelperModelHardPointNames();

        for (uint i = 0; i < hardPointNames.size(); i++) {
            ctlComboHelperModelHardpoint_.AddString(
              bw_utf8tow(hardPointNames[i]).c_str());
        }

        ctlComboHelperModelHardpoint_.SetCurSel(
          PeModule::instance().helperModelCenterOnHardPoint() + 1);
        ctlEditHelperModelName_.SetWindowText(
          bw_utf8tow(PeModule::instance().helperModelName()).c_str());
        ctlCheckDrawHelperModel_.SetCheck(
          PeModule::instance().drawHelperModel() ? BST_CHECKED : BST_UNCHECKED);
...
afx_msg LRESULT MpsProperties::OnChangePropertyItem(WPARAM wParam,
                                                    LPARAM lParam)
{
    BW_GUARD;
...
void MpsProperties::OnCbClickedDrawHelperModel()
{
    ...
    PeModule::instance().drawHelperModel(ctlCheckDrawHelperModel_.GetCheck() ==
                                         BST_CHECKED);
}
...
void MpsProperties::OnBnClickedChooseHelperModel()
{
    ...
    if (!PeModule::instance().helperModelName(path)) {
        Commentary::instance().addMsg("Failed to load model from file: " + path,
                                      Commentary::ERROR_LEVEL);
        return;
    }
