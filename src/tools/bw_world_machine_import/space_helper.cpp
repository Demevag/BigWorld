 ...
 *	@param spaceSettingsSpath the full path including the filename of the
 *space.settings file
...
bool SpaceHelper::init(const BW::string& spaceSettingsPath)
{
    DataSectionPtr pSpaceSettings =
      BW::XMLSection::createFromFile(spaceSettingsPath.c_str());
...
BW::DataSectionPtr SpaceHelper::getCDataForChunk(int x, int z)
{
    ...
    if (!cDataName.empty() && pFileSystem_.exists() &&
