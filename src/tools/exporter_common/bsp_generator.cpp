  ...
  public:
    ...
    bool load(const BW::string& resourceID)
    {
        ...
        if (pData_) {
            ...
            std::transform(std::begin(sourceFormat),
                           std::end(sourceFormat),
                           std::begin(sourceFormat),
                           tolower);
    ...
    {
        return *reinterpret_cast<const Vector3*>(pVertices_ +
                                                 vertexStride_ * index);
    }
...
{
    WorldTriDegenerateCuller(RealWTriangleSet&    ws,
                             const Matrix&        m,
 ...
 */
void populateWorldTriangles(RealWTriangleSet&      ws,
                            const Matrix&          m,
                            const Vertices&        vertices,
                            ...
                            const PrimitiveGroups& primitiveGroups)
{
    ...
    if (primitivesType == Primitive::PT_TRIANGLE_LIST) {
        ...
        while (iter != primitiveGroups.end()) {
            ...
            if (flags != (WorldTriangle::Flags)-1) {
                ...
                Moo::PrimitiveHelper::generateTrianglesFromIndices(
                  ...
                  pg.startIndex_,
                  pg.nPrimitives_,
                  Moo::PrimitiveHelper::TRIANGLE_LIST,
                  culler,
                  primitives.nIndices());
        ...
        if (bspTriangleCulled) {
            INFO_MSG("One or more BSP triangles have been culled.\n"
                     "Creating a simplified BSP would fix the problem and "
                     "improve performance inside BigWorld.");
        }
    }
}
                 ...
                 BW::vector<BW::string>& materialIDs)
{
    BW::string primitivesName =
      visualName.substr(0, visualName.find_last_of('.')) + ".primitives";
        ...
        if ((*iter)->sectionName() == "renderSet") {
            ...
            while (nodeIter != pRenderSetSection->end()) {
                ...
                if (pNS->sectionName() == "node") {
                    Moo::NodePtr pNode = rootNode.find(pNS->asString());
                    MF_ASSERT(pNode);
            ...
            while (geomIter != pRenderSetSection->end()) {
                ...
                if (pGeomSection->sectionName() == "geometry") {
                    ...
                    BW::string verticesName =
                      pGeomSection->readString("vertices");
                    ...
                    BW::string indicesName =
                      pGeomSection->readString("primitive");
                    ...
                    DataSection::iterator primitiveGroupIter =
                      pGeomSection->begin();

                    while (primitiveGroupIter != pGeomSection->end()) {
                        DataSectionPtr primitiveGroupSection =
                          *primitiveGroupIter;
                        if (primitiveGroupSection->sectionName() ==
                            "primitiveGroup") {
                            ...
                            // these are interpreted at runtime based on
                            // material settings that may have changed in model
                            // editor etc.
                            materialIDs.push_back(
                              getMaterialIdentifier(primitiveGroupSection));

                            primitiveGroup.materialFlags_ =
                              (WorldTriangle::Flags)(materialIDs.size() - 1);
