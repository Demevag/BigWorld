...
namespace {
     ...
     *	need to remove it.
     */
    LRESULT WINAPI viewControlsWndProcHook(int    nCode,
                                           WPARAM wParam,
                                           LPARAM lParam)
    {
        BW_GUARD;
        ...
        if (nCode == HC_ACTION && pInfo->message == WM_KILLFOCUS) {
            // Check to see if the window that lost the focus is one of our
            // views
            ...
            if (itKilled != s_viewControlsHookWindows.end()) {
                ...
                if (itFocused == s_viewControlsHookWindows.end() ||
                    ...
                    // The new focused window is not one of our views, or it is
                    // but it's from a different property list, so deselect
                    // ourselves from our property list.
...
void PropertyItem::setGroup(const Name& group)
{
    ...
    if (group_.empty())
        ...
        groupDepth_ = (int)std::count(group_.begin(), group_.end(), '/') + 1;
    }
}
...
{
    return this->getType() == Type_Group || this->getType() == Type_Colour ||
...
/*virtual*/ BW::wstring PropertyItem::UIDescExtra() const
{
    ...
    if (!canExposeToScript_)
        return L"";

    if (exposedToScriptName_.empty()) {
        return Localise(L"COMMON/PROPERTY_LIST/EXPOSE_PYTHON");
    } else {
        return Localise(L"COMMON/PROPERTY_LIST/PYTHON_EXPOSED",
                        bw_utf8tow(exposedToScriptName_.c_str()));
    }
}
...
ColourPropertyItem::ColourPropertyItem(const Name&    name,
                                       const CString& init,
                                       int            depth,
                                       bool           colour)
...
void ColourPropertyItem::create(PropertyList* parent)
{
    ...
    if (colour_ && button_.find(parent_) == button_.end()) {
        ...
        button_[parent_]->Create(
          L"...", style, rect, parent, IDC_PROPERTYLIST_BROWSE);
...
void ColourPropertyItem::select(CRect rect, bool showDropDown /* = true */)
{
    ...
    if (!parent_)
        return;
    if ((edit_.find(parent_) == edit_.end()) &&
        (button_.find(parent_) == button_.end()))
        return;
    ...
    if (colour_) {
        ...
        button_[parent_]->SetWindowPos(
          &CWnd::wndTop, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE);
...
void ColourPropertyItem::onBrowse()
{
    ...
    if (!colour_)
        return; // Don't show a colour picker for vectors
    if (!parent_)
        return;
    if ((edit_.find(parent_) == edit_.end()))
        return;
...
ColourScalarPropertyItem::ColourScalarPropertyItem(const Name&    name,
                                                   const CString& init,
                                                   int            depth)
  : GroupPropertyItem(name, depth)
{
...
void ColourScalarPropertyItem::select(CRect rect,
...
void ColourScalarPropertyItem::onBrowse()
{
    ...
    edit_[parent_]->GetWindowText(color);
    int r, g, b;
    ////COLOURSCALAR figure out how to get this alpha to set it back in the
    /// string value
    float a;
...
StringPropertyItem::StringPropertyItem(const Name&    name,
                                       const CString& currentValue,
                                       bool           readOnly)
  : PropertyItem(name)
  , readOnly_(readOnly)
  , defaultDir_(L"")
...
void StringPropertyItem::select(CRect rect, bool showDropDown /* = true */)
{
    ...
    const int BUTTON_WIDTH =
      ((fileFilter_.size() == 0 && !isHexColor() && !isVectColor()) ||
       readOnly_)
        ? 0
        : 20;
...
void StringPropertyItem::onBrowse()
{
    ...
    if (isHexColor()) {
        ...
        struct SwapRB
        {
            ...
            {
                return ((ref / 65536) & 0xff) + ((ref / 256) & 0xff) * 256 +
                       (ref & 0xff) * 65536;
            }
        } SwapRB;
        CString color;
        ...
        if (colorDlg.DoModal() == IDOK) {
            CString s;
            s.Format(_T( "#%06x" ), (SwapRB(colorDlg.GetColor()) & 0xffffff));
            stringValue_ = s;
            edit_[parent_]->SetWindowText(s);
            parent_->SendMessage(
              WM_CHANGE_PROPERTYITEM, 0, (LPARAM)changeBuddy_);
        }
    } else if (isVectColor()) {
        CString color;
        edit_[parent_]->GetWindowText(color);
        ...
        BW::wstring initDir =
          stringValue_ != L"" ? BW::wstring(stringValue_) : defaultDir_;
        BW::string ninitDir;
        ...
        if (fileDialog.showDialog()) {
            ...
            BW::string dissolvedFileName =
              BWResource::dissolveFilename(npathName);
...
void IDPropertyItem::create(PropertyList* parent)
{
    ...
    if (edit_.find(parent_) == edit_.end()) {
        ...
        CRect rect(10, 10, 10, 10);
        DWORD style =
          ES_LEFT | ES_AUTOHSCROLL | WS_CHILD | WS_BORDER | EM_SETREADONLY;
        edit_[parent_]->Create(style, rect, parent, IDC_PROPERTYLIST_STRING);
  ...
  , possibleValues_(possibleValues)
{
    ...
    //	MF_ASSERT( std::find( possibleValues_.begin(), possibleValues_.end(),
    // currValue ) != possibleValues_.end() );
    stringValue_ = currentValue;
}
...
void ComboPropertyItem::select(CRect rect, bool showDropDown /* = true */)
{
    ...
    rect.top -= 2;
    rect.bottom +=
      256; // extend the bottom to fit a reasonable number of choices
...
void ComboPropertyItem::set(const BW::wstring& value)
{
    ...
    BW::vector<BW::wstring>::iterator it =
      std::find(possibleValues_.begin(), possibleValues_.end(), value);
    // validate string
    ...
    if (parent_ && parent_->selectedItem() == this &&
        comboBox_.find(parent_) != comboBox_.end()) {
        comboBox_[parent_]->SelectString(-1, stringValue_);
    }
...
void ComboPropertyItem::comboChange()
{
    ...
    if ((comboBox_.find(parent_) == comboBox_.end()))
        return;
    comboBox_[parent_]->GetLBText(comboBox_[parent_]->GetCurSel(),
                                  stringValue_);
    parent_->SendMessage(WM_CHANGE_PROPERTYITEM, 0, (LPARAM)changeBuddy_);
}
...
/*static*/ BW::map<CWnd*, controls::EditNumeric*>
                                             FloatPropertyItem::editNumeric_;
...
void FloatPropertyItem::create(PropertyList* parent)
{
    ...
    if (editNumeric_.find(parent_) == editNumeric_.end()) {
        ...
        CRect rect(10, 10, 10, 10);
        editNumeric_[parent_]->Create(ES_LEFT | ES_AUTOHSCROLL | WS_CHILD |
                                        WS_BORDER,
                                      rect,
                                      parent,
                                      IDC_PROPERTYLIST_FLOAT);
    ...
    if (slider_.find(parent_) == slider_.end()) {
        ...
        CRect rect(10, 10, 10, 10);
        slider_[parent_]->Create(TBS_HORZ | TBS_NOTICKS | WS_CHILD |
                                   WS_VISIBLE | WS_TABSTOP,
                                 rect,
                                 parent,
                                 IDC_PROPERTYLIST_SLIDER);
    }
    ...
    if (button_.find(parent_) == button_.end()) {
        ...
        DWORD style = BS_PUSHBUTTON | WS_CHILD | WS_BORDER;
        button_[parent_]->Create(
          L"*", style, rect, parent, IDC_PROPERTYLIST_DEFAULT);
...
void FloatPropertyItem::select(CRect rect, bool showDropDown /* = true */)
{
    ...
    if (!parent_)
        return;
    if ((editNumeric_.find(parent_) == editNumeric_.end()) &&
        (slider_.find(parent_) == slider_.end()) &&
        (button_.find(parent_) == button_.end()))
        return;
    // display edit box
    rect.bottom -= 1; // keep it inside the selection
    ...
    const int SLIDER_WIDTH = (ranged_ == 0 || rect.Width() - BUTTON_WIDTH <
                                                EDIT_WIDTH + MIN_SLIDER_WIDTH)
                               ? 0
                               : rect.Width() - BUTTON_WIDTH - EDIT_WIDTH;
    ...
    if (SLIDER_WIDTH != 0) {
        ...
        slider_[parent_]->SetRange((int)(min_ * pow(10.f, digits_)),
                                   (int)(max_ * pow(10.f, digits_)));
...
float FloatPropertyItem::get() const
{
    ...
    if (parent_ && parent_->selectedItem() == this &&
        editNumeric_.find(parent_) != editNumeric_.end())
...
void FloatPropertyItem::sliderChange(int value, bool transient)
{
    ...
    if (!changing_) {
        ...
        parent_->SendMessage(
          WM_CHANGE_PROPERTYITEM, (WPARAM)transient, (LPARAM)changeBuddy_);
        changing_ = false;
    }
}
...
void FloatPropertyItem::onDefault()
{
    ...
    if (!changing_) {
        ...
        parent_->SendMessage(WM_CHANGE_PROPERTYITEM, 0, (LPARAM)changeBuddy_);
        if (ranged_)
            slider_[parent_]->SetPos(
              (int)(editNumeric_[parent_]->GetValue() * pow(10.f, digits_)));
        changing_ = false;
    }
}
...
void IntPropertyItem::create(PropertyList* parent)
{
    ...
    if (editNumeric_.find(parent_) == editNumeric_.end()) {
        ...
        editNumeric_[parent_]->SetNumericType(
          controls::EditNumeric::ENT_INTEGER);
...
void IntPropertyItem::select(CRect rect, bool showDropDown /* = true */)
{
    ...
    const bool rangeTooBig =
      (((float)max_ - (float)min_) > 65535.0f) ? true : false;
    const int SLIDER_WIDTH = (ranged_ == 0 || rangeTooBig ||
                              rect.Width() < EDIT_WIDTH + MIN_SLIDER_WIDTH)
                               ? 0
                               : rect.Width() - EDIT_WIDTH;
 ...
 */
ArrayPropertyItem::ArrayPropertyItem(const Name&    name,
                                     const Name&    group,
                                     const CString& str,
                                     ArrayProxyPtr  proxy)
...
void ArrayPropertyItem::select(CRect rect, bool showDropDown /* = true */)
{
    ...
    if (addButton_.find(parent_) == addButton_.end()) {
        ...
        addButton->Create(
          _T( "+" ), style, rect, parent_, IDC_PROPERTYLIST_CUSTOM_MIN);
        ...
        delButton->Create(
          _T( "-" ), style, rect, parent_, IDC_PROPERTYLIST_CUSTOM_MIN + 1);
...
/**
 *	Called when the property item is deselected by the user clicking something
 *else.
 */
...
ON_COMMAND_RANGE(IDC_PROPERTYLIST_CUSTOM_MIN,
                 IDC_PROPERTYLIST_CUSTOM_MAX,
                 OnCustom)
...
void PropertyList::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
    ...
    if (delayRedraw_ || lpDrawItemStruct->itemID == -1 ||
...
void PropertyList::OnLButtonUp(UINT nFlags, CPoint point)
{
    ...
    if (dividerMove_) {
        ...
        // Do this to ensure that the value field(s) are moved to the new
        // divider position
        selChange(false);
...
void PropertyList::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    if (item) {
        if (item->getType() == PropertyItem::Type_Group) {
            ...
            int xBoundLower = item->getGroupDepth() * childIndent;
            int xBoundUpper = xBoundLower + childIndent;
            // COLOURSCALAR - derive from ColourPropertyItem to get rid of this
            // malarky
            if (item->getType() == PropertyItem::Type_ColourScalar) {
                if (point.x >= xBoundLower && point.x <= xBoundUpper) {
                    ColourScalarPropertyItem* cItem =
...
void PropertyList::OnMouseMove(UINT nFlags, CPoint point)
{
    ...
    if (dividerMove_) {
        ...
        // set the cursor to a sizing cursor if the cursor is over the row
        // divider
        ::SetCursor(cursorSize_);
    } else {
...
void PropertyList::establishGroup(PropertyItem* item)
{
    ...
    while (!stackTooBig && !stackTooSmall) {
        BW::string name =
          group.substr(startIndex, endIndex == -1 ? -1 : endIndex - startIndex);
...
void PropertyList::makeSubGroup(const BW::string& subGroup, PropertyItem* item)
{
    ...
    while (endIndex != BW::string::npos) {
        addGroupToStack(
          Name(subGroup.substr(startIndex, endIndex - startIndex)));
        startIndex = endIndex + 1;
...
void PropertyList::setArrayProperty(PropertyItem* item)
{
    ...
    if (!arrayCallback_.empty()) {
        item->arrayData(arrayIndex_.top(),
                        arrayCallback_.top().getObject(),
                        arrayReadOnly_.top());
        ++arrayIndex_.top();
    }
}
...
int PropertyList::AddPropItem(PropertyItem* item, int forceIndex)
{
    ...
    if (!arrayCallback_.empty() && arrayPropertItemIndex_.top() != -1 &&
        forceIndex == -1) {
    ...
    if (selected_ != -1) {
        ...
        if ((item) && (item->getSelectable())) {
            ...
            GetParent()->SendMessage(
              WM_SELECT_PROPERTYITEM, 0, (LPARAM)(item->getChangeBuddy()));

            if (item->arrayCallback() != NULL && !item->arrayReadOnly()) {
                CRect butRect(0, 0, 1, 1);
                if (!s_arrayDeleteButton_.GetSafeHwnd()) {
                    s_arrayDeleteButton_.Create(L"-",
                ...
                s_arrayDeleteButton_.SetWindowPos(
                  &CWnd::wndTop, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE);
...
PropertyItem* PropertyList::getHighlightedItem()
{
    ...
    if (pItem == reinterpret_cast<void*>(-1))
        return NULL;
...
/*afx_msg*/ HBRUSH PropertyList::OnCtlColor(CDC*  pDC,
                                            CWnd* pWnd,
                                            UINT  nCtlColor)
{
    BW_GUARD;
    ...
    if (item) {
        ...
        if (edit) {
            if (edit->isRanged()) {
                edit->SetBoundsColour(
                  pDC, pWnd, edit->GetMinimum(), edit->GetMaximum());
            }
        }
    }
...
void PropertyList::collapseGroup(GroupPropertyItem* gItem,
                                 int                index,
                                 bool               reselect)
{
    BW_GUARD;
...
void PropertyList::expandGroup(GroupPropertyItem* gItem, int index)
{
    ...
    for (PropertyItemVector::iterator it = children.begin();
        ...
        if (this->isSorted_ && gItem->getType() != PropertyItem::Type_Colour &&
 ...
 *	added for the array, so other non-array property items can be added
 *properly.
 */
...
CRect PropertyList::dropTest(CPoint point, const BW::wstring& fileName)
{
    ...
    if (item && (item->getType() == PropertyItem::Type_String)) {
        ...
        if ((!stringItem->fileFilter().empty()) &&
            (bw_utf8tow(stringItem->fileFilter().c_str()).find(ext) !=
             BW::wstring::npos)) {
            return rect;
        }
    }
...
INT_PTR PropertyList::OnToolHitTest(CPoint point, TOOLINFO* pTI) const
{
    ...
    return pTI->uId; // By returning a unique value per listItem,
                     // we ensure that when the mouse moves over another list
                     // item, the tooltip will change
}
...
BOOL PropertyList::OnToolTipText(UINT id, NMHDR* pNMHDR, LRESULT* pResult)
{
    ...
    CToolTipCtrl* pToolTip =
      AfxGetModuleState()->m_thread.GetDataNA()->m_pToolTip;
    ...
    // Ignore messages from the built in tooltip, we are processing them
    // internally
