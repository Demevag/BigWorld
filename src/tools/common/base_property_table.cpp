    ...
    bool isSameView(BaseView* view1, BaseView* view2)
    {
        ...
        if (view1->cachedName() == view2->cachedName()) {
            ...
            for (size_t i = 0; i < view1->propertyItems().size(); ++i) {
                if (view1->propertyItems()[i]->getType() !=
                    view2->propertyItems()[i]->getType())
                    return false;
            }
            return true;
        }
...
void BasePropertyTable::update(int interleaveStep /*= 0*/,
                               int maxTimeMS /*= 0*/)
{
    BW_GUARD;
    ...
    if (!pImpl_->viewList.empty()) {
        if (interleaveStep > 1) {
            ...
            while (--iterations) {
                int idx = 0;
                for (BW::list<BaseView*>::iterator it =
                       pImpl_->viewList.begin();
                     it != pImpl_->viewList.end();
                     ++it) {
                    if ((idx % interleaveStep) ==
                        (guiInterleave_ % interleaveStep)) {
                        (*it)->updateGUI();
                    }
                    ++idx;
                }
                ++guiInterleave_;
                if (((timestamp() - startTime) * 1000 / stampsPerSecond()) >
                    maxTimeMS) {
                    break;
                }
            }
...
int BasePropertyTable::addView(BaseView* view)
{
    ...
    if (view) {
        ...
        return GeneralEditor::settingMultipleEditors() ? -1
                                                       : addItemsForView(view);
    } else {
        return GeneralEditor::settingMultipleEditors() &&
                   GeneralProperty::View::pLastElected()
                 ? addItemsForViews()
                 : -1;
    }
}
...
int BasePropertyTable::addItemsForView(BaseView* view)
{
    ...
    for (size_t i = 0; i < view->propertyCount(); ++i) {
        int newIndex = pImpl_->propertyList.AddPropItem(
          view->propertyItems()[i], static_cast<int>(firstIndex + i));
        if (firstIndex == -1)
            firstIndex = newIndex;
    }
    return firstIndex;
...
int BasePropertyTable::addItemsForViews()
{
    ...
    for (BW::list<BaseView*>::iterator i = pImpl_->viewList.begin();
        ...
        if (item->isCommonView()) {
            ...
            list.insert(
              list.end(), commItem->views().begin(), commItem->views().end());
    ...
    for (BW::vector<BaseView*>::iterator i = list.begin(); i != list.end();
         ++i) {
        (*i)->cacheName();
    }
