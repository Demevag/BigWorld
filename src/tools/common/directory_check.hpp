...
class DirectoryCheck
{
  public:
    DirectoryCheck(const BW::wstring& appName)
    {
        ...
        directorPath =
          directorPath + L"resources\\scripts\\" + appName + L"Director.py";

        if (GetFileAttributes(directorPath.c_str()) ==
            INVALID_FILE_ATTRIBUTES) { // we are not running in the proper
                                       // directory
            ...
            directorPath = curDir;
            directorPath = directorPath + L"\\resources\\scripts\\" + appName +
                           L"Director.py";
            ...
            if (GetFileAttributes(directorPath.c_str()) !=
                ...
                GetCurrentDirectory(MAX_PATH_SIZE, curDir);
                MessageBox(NULL,
                           Localise(L"COMMON/DIRECTORY_CHECK/DIR_CHECK",
                                    appName,
                                    curDir,
                                    appName),
                           appName.c_str(),
