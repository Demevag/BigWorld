 ...
 */
bool EditorChunkModelBase::load(DataSectionPtr pSection,
                                Chunk*         pChunk,
...
BW::vector<BW::string> EditorChunkModelBase::extractVisualNames() const
{
    ...
    for (uint i = 0; i < models.size(); i++) {
        ...
        Moo::VisualPtr visual =
          Moo::VisualManager::instance()->get(fullVisualName);
...
void EditorChunkModelBase::tossWithExtra(Chunk* pChunk, SuperModel* extraModel)
{
    ...
    if (firstToss_) {
        // If the .visual file is newer than the .cdata file, then we need
        // update the navMesh, shadow map, static lighting and thumbnail of that
        // chunk
