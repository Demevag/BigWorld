...
class MaterialPropertiesUser
{
  public:
    ...
    typedef SmartPointer<BWBaseFunctor2R<int&, int&, bool>>
      RangeFnInt; // min, max

    // floatProxy typedefs
    typedef SmartPointer<BWBaseFunctor0R<float>> GetFnFloat;
    typedef SmartPointer<BWBaseFunctor1<float>>  SetFnFloat;
    typedef SmartPointer<BWBaseFunctor3R<float&, float&, int&, bool>>
      RangeFnFloat; // min, max, digits
    ...
    virtual BW::string exposedToScriptName(
      const BW::string& descName) const = 0;
...
class MaterialProperties
{
  public:
    ...
    static bool addMaterialProperties(Moo::EffectMaterialPtr material,
