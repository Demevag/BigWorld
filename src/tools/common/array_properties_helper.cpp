 ...
 */
void ArrayPropertiesHelper::init(EditorChunkItem*     pItem,
...
bool ArrayPropertiesHelper::addItem()
{
    ...
    return PySequence_InPlaceConcat(pSeq_, newItem) != NULL;
}
...
PropertyIndex ArrayPropertiesHelper::propGetIdx(const BW::string& name) const
{
    ...
    BW::string index =
      name.substr(openBracket + 1, closeBracket - openBracket - 1);
