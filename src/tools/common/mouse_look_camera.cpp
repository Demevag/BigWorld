...
bool MouseLookCamera::handleMouseEvent(const MouseEvent& event)
{
    ...
    if (InputDevices::isKeyDown(KeyCode::KEY_RIGHTMOUSE)) {
        float speedFactor =
          0.01f * Options::getOptionFloat("camera/rotateSpeedMultiplier", 1.0f);
...
void MouseLookCamera::handleInput(float dTime)
{
    ...
    if (viewChanged) {
        ...
        view_._41 =
          Math::clamp(limit().minBounds().x, view_._41, limit().maxBounds().x);
        view_._42 =
          Math::clamp(limit().minBounds().y, view_._42, limit().maxBounds().y);
        view_._43 =
          Math::clamp(limit().minBounds().z, view_._43, limit().maxBounds().z);
    }
...
void MouseLookCamera::polarToView()
{
    ...
    if (!almostEqual(pos.x, lastPos_.x) || !almostEqual(pos.y, lastPos_.y) ||
        !almostEqual(pos.z, lastPos_.z) || !almostEqual(yaw_, lastYaw_) ||
        !almostEqual(pitch_, lastPitch_)) {
        lastPos_   = pos;
        lastYaw_   = yaw_;
        lastPitch_ = pitch_;
