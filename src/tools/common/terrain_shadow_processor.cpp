...
TerrainShadowProcessor::TerrainShadowProcessor(EditorChunkTerrainCache* cache,
  ...
  , processInBackground_(true)
{
    ...
    for (int32 z = 0; z < static_cast<int32>(height_); ++z) {
        for (int32 x = width_ - 1; x >= 0; --x) {
            float chunkX = blockSize * static_cast<float>(x) /
                           static_cast<float>(width_ - 1);
            float chunkZ = blockSize * static_cast<float>(z) /
...
bool TerrainShadowProcessor::processShadow()
{
    return cache_->pTerrain()->calculateShadows(
      image_, width_, height_, heights_);
}

bool TerrainShadowProcessor::processInBackground(ChunkProcessorManager& manager)
{
    BW_GUARD;
...
bool TerrainShadowProcessor::processInMainThread(ChunkProcessorManager& manager,
                                                 bool backgroundTaskResult)
{
    ...
    return cache_->calcDone(
      this, manager, backgroundTaskResult, image_, unsavedList_);
}
