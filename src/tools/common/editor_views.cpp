    ...
    if (PyUnicode_Check(pValue)) {
        ...
        if (pUO) {
            ...
            PyUnicodeObject* unicodeObj =
              reinterpret_cast<PyUnicodeObject*>(pUO);
...
CommonView::CommonView(BaseView* view)
{
    ...
    propertyItems_.assign(view->propertyItems().begin(),
                          view->propertyItems().end());
    for (PropertyItems::iterator i = propertyItems_.begin();
         i != propertyItems_.end();
         ++i) {
...
void CommonView::onChangeInternal(bool skipFirst, bool transient)
{
    ...
    if (!skipFirst) {
        (*it)->onChange(transient,
                        (*it) ==
                          pLastView /*create undo barrier if it's the last*/);
    }
    while (++it != views_.end()) {
...
void CommonView::updateDisplayItems()
{
    ...
    for (size_t i = 0; i < propertyItems_.size(); ++i) {
        ...
        for (size_t j = 1; j < views_.size(); ++j) {
            if (!isSameValue(curPropertyStr,
                             views_[j]->propertyItems()[i]->value())) {
                propertyItems_[i]->multipleValue(true);
                break;
            }
        }
        for (size_t j = 1; j < views_.size(); ++j) {
            views_[j]->propertyItems()[i]->multipleValue(
              propertyItems_[i]->multipleValue());
        }
    }
}
...
void CommonView::initGroups()
{
    ...
    BW::string group =
      LocaliseUTF8(L"COMMON/EDITOR_VIEWS/OBJECT_SELECTED", views_.size());
    for (size_t i = 0; i < propertyItems_.size(); ++i) {
...
void TextView::elect()
{
    ...
    StringPropertyItem* newItem =
      new StringPropertyItem(property_.name(), oldValue_.c_str());
...
void StaticTextView::elect()
{
    ...
    PropertyItem* newItem =
      new StringPropertyItem(property_.name(), oldValue_.c_str(), true);
    newItem->setGroup(property_.getGroup());
    newItem->setChangeBuddy(this);
...
void TextLabelView::elect()
{
    ...
    PropertyItem* newItem =
      new LabelPropertyItem(property_.name(), property_.highlight());
    newItem->setGroup(property_.getGroup());
    newItem->setChangeBuddy(this);
...
void IDView::elect()
{
    ...
    PropertyItem* newItem =
      new IDPropertyItem(property_.name(), oldValue_.c_str());
    newItem->setGroup(property_.getGroup());
    newItem->setChangeBuddy(this);
...
void ListTextView::elect()
{
    ...
    PropertyItem* newItem = new ComboPropertyItem(
      property_.name(), oldValue_.c_str(), property_.possibleValues());
    newItem->setGroup(property_.getGroup());
    newItem->setChangeBuddy(this);
...
void ChoiceView::elect()
{
    ...
    for (DataSectionIterator iter = choices->begin(); iter != choices->end();
         iter++) {
    ...
    // make sure the old string value is valid, otherwise select the first valid
    // value
    bool setDefault = oldStringValue.empty();
    if (setDefault) {
        oldStringValue = possibleValues.front();
        // INFO_MSG( Localise(L"COMMON/EDITOR_VIEWS/CHOICEVIEW_ELECT",
        // oldStringValue, property_.name() ) );
    }

    PropertyItem* newItem = new ComboPropertyItem(
      property_.name(), oldStringValue.c_str(), possibleValues);
    newItem->setGroup(property_.getGroup());
    newItem->setChangeBuddy(this);
...
void GenFloatView::elect()
{
    ...
    FloatPropertyItem* newItem =
      new FloatPropertyItem(property_.name(), property_.pFloat()->get());
    newItem->setGroup(property_.getGroup());
    newItem->setChangeBuddy(this);
...
void GenIntView::elect()
{
    ...
    PropertyItem* newItem =
      new IntPropertyItem(property_.name(), property_.pInt()->get());
    newItem->setGroup(property_.getGroup());
    newItem->setChangeBuddy(this);
...
void GenUIntView::elect()
{
    ...
    PropertyItem* newItem =
      new UIntPropertyItem(property_.name(), property_.pUInt()->get());
    newItem->setGroup(property_.getGroup());
    newItem->setChangeBuddy(this);
...
void GenPositionView::elect()
{
    ...
    FloatPropertyItem* newItem =
      new FloatPropertyItem(property_.name().c_str() + s_x, oldValue_.x);
    newItem->setGroup(property_.getGroup());
    newItem->setChangeBuddy(this);
    ...
    propertyItems_.push_back(newItem);
    newItem =
      new FloatPropertyItem(property_.name().c_str() + s_y, oldValue_.y);
    newItem->setGroup(property_.getGroup());
    newItem->setChangeBuddy(this);
    ...
    propertyItems_.push_back(newItem);
    newItem =
      new FloatPropertyItem(property_.name().c_str() + s_z, oldValue_.z);
    newItem->setGroup(property_.getGroup());
    newItem->setChangeBuddy(this);
...
void GenRotationView::elect()
{
    ...
    FloatPropertyItem* newItem =
      new FloatPropertyItem(property_.name().c_str() + s_yaw, oldValue_.y);
    newItem->setGroup(property_.getGroup());
    newItem->setDigits(1);
    newItem->setChangeBuddy(this);
    ...
    newItem =
      new FloatPropertyItem(property_.name().c_str() + s_pitch, oldValue_.x);
    newItem->setGroup(property_.getGroup());
    newItem->setDigits(1);
    newItem->setChangeBuddy(this);
    ...
    newItem =
      new FloatPropertyItem(property_.name().c_str() + s_roll, oldValue_.z);
    newItem->setGroup(property_.getGroup());
    newItem->setDigits(1);
    newItem->setChangeBuddy(this);
...
void ColourView::elect()
{
    ...
    bw_snwprintf(buf,
                 ...
                 (int)(c.b),
                 (int)(c.a));
    ColourPropertyItem* newItem =
      new ColourPropertyItem(property_.name(), buf, 1);
    newItem->setGroup(property_.getGroup());
    ...
    // we need to add the view here otherwise the children will be added in the
    // view as well ( we don't want that. Trust me )
    ...
    propTable_->propertyList()->collapseGroup(newItem, listLocation, false);
}
...
void ColourScalarView::elect()
{
    ...
    bw_snwprintf(buf,
                 ...
                 (int)(c.b),
                 c.a);
    ColourScalarPropertyItem* newItem =
      new ColourScalarPropertyItem(property_.name(), buf, 1);
    newItem->setGroup(property_.getGroup());
    newItem->setGroupDepth(newItem->getGroupDepth() + 1);
    newItem->setChangeBuddy(this);
    ...
    FloatPropertyItem* newAlphaItem =
      new FloatPropertyItem(property_.scalarName(), c.a);
...
void Vector2View::elect()
{
    ...
    wchar_t buf[256];
    bw_snwprintf(
      buf, ARRAY_SIZE(buf), L"%.2f , %.2f", (float)(c.x), (float)(c.y));
    // Vector2PropertyItem* newItem = new Vector2PropertyItem(property_.name(),
    // buf, 1, false /*property_.getGroupDepth() + 1*/ );
    ColourPropertyItem* newItem =
      new ColourPropertyItem(property_.name(), buf, 1, false);
    newItem->setGroup(property_.getGroup());
    newItem->setGroupDepth(newItem->getGroupDepth() /* + 1*/);
    newItem->setChangeBuddy(this);
...
void Vector3View::elect()
{
    ...
    bw_snwprintf(buf,
                 ...
                 (float)(c.y),
                 (float)(c.z));
    ColourPropertyItem* newItem = new ColourPropertyItem(
      property_.name(), buf, 1, false /*property_.getGroupDepth() + 1*/);
    newItem->setGroup(property_.getGroup());
    newItem->setGroupDepth(newItem->getGroupDepth() + 1);
    newItem->setChangeBuddy(this);
...
void Vector3View::onChange(bool transient, bool addBarrier)
{
    ...
    if (c2 != oldValue_) {
        ...
        wchar_t buf[256];
        bw_snwprintf(
          buf, ARRAY_SIZE(buf), L"%.2f , %.2f , %.2f", c2.x, c2.y, c2.z);
        item()->set(buf);
...
bool Vector3View::updateGUI()
{
    ...
    if (v != oldValue_) {
        ...
        wchar_t buf[256];
        bw_snwprintf(
          buf, ARRAY_SIZE(buf), L"%.2f , %.2f , %.2f", v.x, v.y, v.z);
        item()->set(buf);
        changed = true;
    }
...
void MatrixView::elect()
{
    ...
    PropertyItem* newItem = new StringPropertyItem(
      property_.name(), NumVecToStr((float*)&oldValue_, 16).c_str());
    newItem->setGroup(property_.getGroup());
    newItem->setChangeBuddy(this);
...
void ArrayView::elect()
{
    ...
    label += " (array)";
    ArrayPropertyItem* newItem = new ArrayPropertyItem(
      Name(label), property_.name(), L"", property_.proxy());
    ...
    propTable_->propertyList()->startArray(
      new BWFunctor1<ArrayView, int>(this, &ArrayView::delItem),
