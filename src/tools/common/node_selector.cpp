...
{
    // node->treeDeselect( );
    SceneNodeVector::iterator it =
      std::find(selectionList_.begin(), selectionList_.end(), node);
...
{
    if (std::find(selectionList_.begin(), selectionList_.end(), node) ==
        selectionList_.end())
        return false;
...
{
    if (std::find(highlightList_.begin(), highlightList_.end(), node) ==
        highlightList_.end())
        return false;
