...
bool MsgHandler::addAssetErrorMessage(BW::string  msg,
                                      Chunk*      chunk,
                                      ChunkItem*  item,
                                      const char* stackTrace)
{
    ...
    messages_.push_back(new BWMessageInfo(messagePriority,
                                          dateStr,
                                          timeStr,
                                          priorityStr,
                                          msg.c_str(),
 ...
 */
bool MsgHandler::handleMessage(DebugMessagePriority messagePriority,
                               ...
                               va_list     argPtr)
{
    ...
    // Anything that is not an error, warning or notice should be filtered with
    // info
    ...
    {
      public:
        TempString(int size) { buffer_ = new char[size + 1]; }
        ~TempString() { bw_safe_delete_array(buffer_); }
        char* buffer_;
    ...
    if (filterLevel != MESSAGE_PRIORITY_INFO || MAX_INFO_MESSAGES > 0) {
        if (filterLevel == MESSAGE_PRIORITY_INFO) {
            for (BW::vector<BWMessageInfoPtr>::iterator it = messages_.begin();
                 it != messages_.end() &&
                 numInfoMessages_ >= MAX_INFO_MESSAGES;) {
        ...
        messages_.push_back(new BWMessageInfo(
          filterLevel, dateStr, timeStr, priorityStr, tempString.buffer_));
        newMessages_ = true;
    }

    if (messagePriority == MESSAGE_PRIORITY_ERROR) {
        Commentary::instance().addMsg(tempString.buffer_,
...
int MsgHandler::numAssetMsgs(const BW::string& msg)
{
    ...
    return (assetErrorList_.find(msg) == assetErrorList_.end())
             ? 0
             : assetErrorList_[msg];
}
