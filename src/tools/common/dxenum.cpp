...
DXEnum::EnumEntry::EnumEntry(DataSectionPtr dataSection)
{
    ...
    for (BW::vector<DataSectionPtr>::iterator iter = entries.begin();
         iter != entries.end();
         ++iter) {
        BW::string       name  = (*iter)->readString("NAME");
        DXEnum::EnumType value = (DXEnum::EnumType)(*iter)->readInt("VALUE");
        if (name.size() &&
            nameToValueMap_.find(name) == nameToValueMap_.end() &&
...
const BW::string& DXEnum::EnumEntry::entry(
  DXEnum::EnumEntry::size_type index) const
{
    BW_GUARD;

    // no check for index, since no exceptions are allowed here
    BW::map<BW::string, EnumType>::const_iterator iter =
      nameToValueMap_.begin();
...
DXEnum::DXEnum(const BW::string& resFileName)
{
    ...
    if (enumDef) {
        for (DataSectionIterator iter = enumDef->begin();
             iter != enumDef->end();
             ++iter) {
        ...
        while (changed) {
            changed = false;
            for (BW::map<BW::string, BW::string>::iterator iter =
                   alias_.begin();
                 iter != alias_.end();
                 ++iter) {
                if (enums_.find(iter->first) !=
                    enums_.end()) { // enum and alias has same name
                    alias_.erase(iter);
                    changed = true;
                    break;
                }
                ...
                if (enums_.find(s) ==
                    enums_.end()) { // alias has no correspond enum
                    alias_.erase(iter);
                    changed = true;
                    break;
                }
...
const BW::string& DXEnum::entry(const BW::string& typeName,
                                DXEnum::size_type index) const
{
    BW_GUARD;
...
DXEnum::EnumType DXEnum::value(const BW::string& typeName,
...
const BW::string& DXEnum::name(const BW::string& typeName,
                               DXEnum::EnumType  value) const
{
    BW_GUARD;
