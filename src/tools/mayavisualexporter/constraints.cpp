                                        ...
                                        DataSectionPtr pConstraintsSection)
{
    ...
    if (getConnectedObject(orientConstraintFn.findPlug("constraintRotateX"),
                           true,
                           targetNodeObj)) {
        MFnDependencyNode targetNode(targetNodeObj);
        pOrientConstraintSection->writeString("target",
                                              targetNode.name().asChar());
    }

    const Vector3 offsetAngle =
      vector3FromPlug(orientConstraintFn.findPlug("offset"));
    pOrientConstraintSection->writeVector3(
      "offset", convertAngleOffsetToBigWorldUnits(offsetAngle));
    ...
    for (uint i = 0; i < arrayPlug.numElements(); ++i) {
        ...
        for (uint j = 0; j < sourcePlug.numChildren(); ++j) {
            ...
            if (propertyAttribute.name() == "targetRotate") {
                MObject destNodeObj;
                if (getConnectedObject(propertyPlug, false, destNodeObj)) {
                    MFnDependencyNode destNode(destNodeObj);
                    pSourceSection->writeString("identifier",
                                                destNode.name().asChar());
                }
            }
                                       ...
                                       DataSectionPtr pConstraintsSection)
{
    ...
    if (getConnectedObject(pointConstraintFn.findPlug("constraintTranslateX"),
                           true,
                           targetNodeObj)) {
        ...
        if (targetNodeObj.apiType() == MFn::kUnitConversion) {
            ...
            if (getConnectedObject(unitConversionFn.findPlug("output"),
                                   true,
        ...
        MFnDependencyNode targetNode(targetNodeObj);
        pPointConstraintSection->writeString("target",
                                             targetNode.name().asChar());
    }
                                        ...
                                        DataSectionPtr pConstraintsSection)
{
    ...
    if (getConnectedObject(parentConstraintFn.findPlug("constraintRotateX"),
                           true,
                           targetNodeObj)) {
        MFnDependencyNode targetNode(targetNodeObj);
        pParentConstraintSection->writeString("target",
                                              targetNode.name().asChar());
    }
    ...
    for (uint i = 0; i < arrayPlug.numElements(); ++i) {
        ...
        for (uint j = 0; j < sourcePlug.numChildren(); ++j) {
            ...
            if (propertyName == "targetTranslate") {
                ...
                if (getConnectedObject(propertyPlug, false, destNodeObj)) {
                    MFnDependencyNode destNode(destNodeObj);
                    pTargetSection->writeString("identifier",
                                                destNode.name().asChar());
                }
            }
                                     ...
                                     DataSectionPtr pConstraintsSection)
{
    ...
    if (getConnectedObject(
        ...
        pAimConstraintSection->writeString("target", srcNode.name().asChar());
    }

    const Vector3 offset = vector3FromPlug(aimConstraintFn.findPlug("offset"));
    pAimConstraintSection->writeVector3(
      "offset", convertAngleOffsetToBigWorldUnits(offset));

    Vector3 aimVector = vector3FromPlug(aimConstraintFn.findPlug("aimVector"));
    pAimConstraintSection->writeVector3(
      "aimVector", convertVectorToBigWorldUnits(aimVector));
    ...
    pAimConstraintSection->writeVector3(
      "worldUpVector", convertVectorToBigWorldUnits(worldUpVector));
                                ...
                                DataSectionPtr pConstraintsSection)
{
    ...
    if (getConnectedObject(
        ...
        pIKHandleSection->writeString("source/node",
                                      translateNode.name().asChar());
    } else {
        Vector3 translate = vector3FromPlug(ikHandleFn.findPlug("translate"));
    ...
    if (getConnectedObject(
        ...
        if (poleVectorSource.apiType() == MFn::kUnitConversion) {
            ...
            if (getConnectedObject(unitConversionFn.findPlug("input"),
                                   false,
        ...
        pIKHandleSection->writeString("poleVector/node",
                                      poleVectorNode.name().asChar());
    } else {
        Vector3 poleVector = vector3FromPlug(ikHandleFn.findPlug("poleVector"));
        pIKHandleSection->writeVector3(
          "poleVector/vector", convertVectorToBigWorldUnits(poleVector));
    }
}
