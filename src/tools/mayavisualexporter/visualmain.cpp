...
MLL_EXPORT MStatus initializePlugin(MObject obj)
{
    ...
    stat =
      plugin.registerFileTranslator("BigWorldAsset",
                                    "none",
    ...
    BW::wstring    exporterPath =
      BW_NAMESPACE bw_acptow(plugin.loadPath().asChar());
    ...
    if (BW_NAMESPACE g_exporterErrorMsg) {
        MessageBoxA(GetForegroundWindow(),
                    BW_NAMESPACE g_exporterErrorMsg,
                    "Visual Exporter",
                    MB_OK | MB_ICONEXCLAMATION);
        return MS::kFailure;
    }

    BW_NAMESPACE BWResource::instance().fileSystem()->addBaseFileSystem(
      BW_NAMESPACE NativeFileSystem::create(""));
    ...
    BW_NAMESPACE ExportSettings::instance().setSettingFileName(
      bw_wtoutf8(exporterPath) + "\\exporter.xml");
    BW_NAMESPACE ExportSettings::instance().readSettings();

    SetCurrentDirectory(currentDir);

    // Don't write out xml attributes in the exporter as we want to remain
    // compatible with older versions of BW
