...
HullMesh::BiIndex::BiIndex()
  : i1(0u)
  , i2(0u)
{
}

HullMesh::BiIndex::BiIndex(uint i1, uint i2)
  : i1(i1)
  , i2(i2)
{
}
...
HullMesh::TriIndex::TriIndex(uint i1, uint i2, uint i3)
  : i1(i1)
  , i2(i2)
  , i3(i3)
{
...
bool HullMesh::exportHull(DataSectionPtr pSection)
{
    ...
    if (!this->makeConvex(vv, faces)) {
        if (!LogMsg::automatedTest()) {
            ::MessageBox(0,
                         L"Unable to create custom hull.  Custom hull most "
                         L"contain at least four "
                         L"non-colinear and non-coplanar vertices!",
                         L"Custom hull error!",
                         MB_ICONERROR);
        }
        addError("Unable to create custom hull. "
                 "Custom hull most contain at least four non-colinear and "
    ...
    if (portalsAdded != 0) {
        if (!LogMsg::automatedTest()) {
            ::MessageBox(0,
                         L"One or more portals defined using hull materials "
                         L"will not be exported.\n"
                         L"The hull was converted from concave to convex at "
                         L"that location!",
                         L"Portal error!",
                         MB_ICONERROR);
        }
        addError(
          "One or more portals defined using hull materials will not be "
          "exported. \n"
 ...
 */
void HullMesh::extractPlanesAndPortals(VertexVector&         vv,
                                       BW::vector<TriIndex>& faces)
{
    planeEqs_.clear();
    portals_.clear();
    ...
    for (face_it = faces.begin(); face_it != faces.end(); face_it++) {
        ...
        PlaneEq peq(Vector3(vv[face_it->i3].pos_),
    ...
    while (it != end) {
        Triangle& tri = *it++;
        PlaneEq   peq(Vector3(vv[tri.index[0]].pos_),
        ...
        if (isPortal(tri.materialIndex)) {
            if (portals_.find(planeIndex) == portals_.end()) {
                ...
                const char* dataName =
                  vpp->propDataFromPropName(matName.c_str());
                if (dataName)
                    vpp->name(dataName);
            }
...
VisualMesh::Triangle* HullMesh::findOriginalTriangle(TriIndex& face)
{
    ...
    for (it = triangles_.begin(); it != triangles_.end(); it++) {
        for (int i = 0; i < 3; i++) {
            if (it->index[2] == face[i] && it->index[1] == face[i + 1] &&
                it->index[0] ==
                  face[i + 2]) // Note that the winding orders are different
                return &*it;
        }
    }
 ...
 */
bool HullMesh::makeConvex(VertexVector&         vertices,
                          BW::vector<TriIndex>& faces) const
{
    ...
    if (isConcave) {
        if (!LogMsg::automatedTest()) {
            ::MessageBox(
              0,
              L"The custom hull is concave or has colinear/coplanar vertices. "
              L"The exported hull may have changed shape to make it convex!",
              L"Custom hull warning!",
              MB_ICONWARNING);
        } else {
            LogMsg::logToFile(
              "WARNING: The custom hull is concave or has colinear/coplanar "
              "vertices. "
              "The exported hull may have changed shape to make it convex!");
        }
        addError(
          "The custom hull is concave or has colinear/coplanar vertices. "
          "The exported hull may have changed shape to make it convex!");
    }
 ...
 *	@returns			MVT created
 */
bool HullMesh::mcCreateMaxTetrahedron(const VertexVector&   vertices,
                                      ...
                                      const float           epsilon) const
{
    ...
    for (it = verts.begin(); it != verts.end(); it++) {
        if (vertices[*it].pos_[0] < minX) {
            minX       = vertices[*it].pos_[0];
            minX_index = *it;
        }
        if (vertices[*it].pos_[1] < minY) {
            minY       = vertices[*it].pos_[1];
            minY_index = *it;
        }
        if (vertices[*it].pos_[2] < minZ) {
            minZ       = vertices[*it].pos_[2];
            minZ_index = *it;
        }
        if (vertices[*it].pos_[0] > maxX) {
            maxX       = vertices[*it].pos_[0];
            maxX_index = *it;
        }
        if (vertices[*it].pos_[1] > maxY) {
            maxY       = vertices[*it].pos_[1];
            maxY_index = *it;
        }
        if (vertices[*it].pos_[2] > maxZ) {
            maxZ       = vertices[*it].pos_[2];
            maxZ_index = *it;
        }
    }
 ...
 *coincident.
 */
void HullMesh::mcRemoveVertex(const uint          vertIndex,
 ...
 */
bool HullMesh::mcRemoveInsideVertices(const VertexVector&         vertices,
                                      BW::vector<uint>&           verts,
 ...
 */
uint HullMesh::mcFindNextVert(const VertexVector&         vertices,
 ...
 *	@param faces		The faces of the convex hull.
 */
void HullMesh::mcExpandHull(const uint            vertIndex,
                            const VertexVector&   vertices,
                            BW::vector<uint>&     verts,
                            BW::vector<TriIndex>& faces) const
{
    ...
    MF_ASSERT(
      count > 1 &&
      "HullMesh::mcExpandHull - Failed to find triangle on horizon edge")
...
bool HullMesh::mcIsInternalEdge(BW::vector<TriIndex>&   faces,
 ...
 *	@param faces		The faces of the convex hull.
 */
void HullMesh::mcAddFaces(const uint              vertIndex,
