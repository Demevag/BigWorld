...
MStatus ea::doIt(const MArgList& args)
{
    ...
    // skin.meshes()[0].fullPathName().asChar() ).size() );
    ...
    for (uint32 i = 0; i < mesh.faces().size(); ++i)
        printf("%2i %2i %2i | %2i %2i %2i | %2i %2i %2i | %i\n",
               mesh.faces()[i].positionIndex[0],
               mesh.faces()[i].positionIndex[1],
               mesh.faces()[i].positionIndex[2],
               mesh.faces()[i].normalIndex[0],
               mesh.faces()[i].normalIndex[1],
               mesh.faces()[i].normalIndex[2],
               mesh.faces()[i].uvIndex[0],
               mesh.faces()[i].uvIndex[1],
               mesh.faces()[i].uvIndex[2],
    ...
    for (uint32 i = 0; i < mesh.materials().size(); ++i)
        printf("%s: %s\n",
               ...
               mesh.materials()[i].texture.c_str());
