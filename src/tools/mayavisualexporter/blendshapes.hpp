...
class BlendShapes
{
  public:
    ...
    // implementation is inefficient, may need to be optimised (initialises each
    // blend shape in turn) NOTE: leaves the blend shape object in a finalised
    // state
    ...
    // initialise blend shape for MObject, returns false if no Blendshape on
    // object
