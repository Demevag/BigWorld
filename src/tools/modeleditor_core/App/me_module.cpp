...
static int s_chunkTokenSet =
  ...
  ChunkWater_token | ChunkTree_token | ChunkParticles_token;
...
namespace {
    ...
    static void renderThumbnail()
    {
        ...
        MeApp::instance().mutant()->render(
          thumbnailDrawContext, 0.f, SHOW_MODEL);
...
class WireframeMaterialOverride : public Moo::DrawContext::OverrideBlock
{
  public:
    WireframeMaterialOverride()
      : Moo::DrawContext::OverrideBlock(
          s_light_only.value().substr(0, s_light_only.value().rfind(".")),
          true)
    {
    }
};
...
void MeModule::onStart()
{
    ...
    cc_ =
      new ClosedCaptions(Options::getOptionInt("consoles/numMessageLines", 5));
...
bool MeModule::updateState(float dTime)
{
    ...
    if (dTime != 0.f) {
        dTime = dTime / NUM_FRAMES_AVERAGE +
                (NUM_FRAMES_AVERAGE - 1.f) * s_lastDTime / NUM_FRAMES_AVERAGE;
        s_lastDTime = dTime;
    }
    ...
    // update the camera.  this interprets the view direction from the mouse
    // input
    ...
    Vector3 worldRay =
      mainFrame_->getWorldRay((int)cursorPos.x, (int)cursorPos.y);
...
bool MeModule::renderThumbnail(const BW::string& fileName)
{
    ...
    BW::wstring screenShotName =
      bw_utf8tow(BWResource::removeExtension(modelName).to_string());
    ...
    if (!rt_) {
        ...
        if (!rt_ || !rt_->create(128, 128)) {
            WARNING_MSG(
              "Could not create render target for model thumbnail.\n");
            rt_ = NULL;
            return false;
        }
    }
    ...
    if (rt_->push()) {
        ...
        HRESULT hr = D3DXSaveTextureToFile(
          screenShotName.c_str(), D3DXIFF_JPG, rt_->pTexture(), NULL);

        rt_->pop();

        if (FAILED(hr)) {
            WARNING_MSG(
              "Could not create model thumbnail \"%s\" (D3D error 0x%x).\n",
...
bool setShadowGraphicsSetting(int shadowQualityOptionIndex, bool enableShadows)
{
...
void MeModule::render(float dTime)
{
    ...
    bool showAxes =
      !checkForSparkles && !!Options::getOptionInt("settings/showAxes", 0);
    bool showBoundingBox =
      !checkForSparkles &&
      !!Options::getOptionInt("settings/showBoundingBox", 0);
    bool useTerrain =
      !checkForSparkles && !!Options::getOptionInt("settings/useTerrain", 1);
    bool useFloor = !checkForSparkles && !useTerrain &&
                    !!Options::getOptionInt("settings/useFloor", 1);
    bool showBsp =
      !checkForSparkles && !!Options::getOptionInt("settings/showBsp", 0);
    int useHDR =
      checkForSparkles ? 0 : Options::getOptionInt("settings/enableHDR", 0);
    int  useSSAO = Options::getOptionInt("settings/enableSSAO", 1);
    bool special = checkForSparkles || showBsp;
    bool showModel =
      special || !!Options::getOptionInt("settings/showModel", 1);
    bool showWireframe =
      !special && !!Options::getOptionInt("settings/showWireframe", 0);
    bool showSkeleton =
      !checkForSparkles && !!Options::getOptionInt("settings/showSkeleton", 0);
    bool showPortals =
      !special && !!Options::getOptionInt("settings/showPortals", 0);
    ...
    bool showOriginalAnim =
      !special && !!Options::getOptionInt("settings/showOriginalAnim", 0);
    ...
    bool showEditorProxy =
      !special && !!Options::getOptionInt("settings/showEditorProxy", 0);
    bool showActions = !special && showModel &&
                       !!Options::getOptionInt("settings/showActions", 0);
    Vector3 bkgColour = Options::getOptionVector3(
                          "settings/bkgColour", Vector3(255.f, 255.f, 255.f)) /
                        255.f;
    ...
    //-- update model
    // TODO_DRAW_CONTEXT: a_radchenko: investigate why update function draws a
    // model
    ...
    //-- Turn off the light in the checkForSparkles mode for filling the model
    // into
    //-- black color.
    Moo::SunLight sun;
    if (!checkForSparkles) {
    ...
    // Update Shadows
    {
        ...
        if (lastShadowsQuality_ != shadowQualityOptionIndex ||
            ...
            lastShadowsOn_      = showShadowing;
            if (!setShadowGraphicsSetting(shadowQualityOptionIndex,
                                          showShadowing)) {
                WARNING_MSG(
                  "MeModule::render: "
        ...
        shadowRenderItem_.update(
          MeApp::instance().mutant(), dTime, renderState);
    ...
    speedtree::SpeedTreeRenderer::beginFrame(
      &enviroMinder, Moo::RENDERING_PASS_COLOR, rc.lodView());
#endif
    ...
    if (!renderingThumbnail_ && showOriginalAnim && !materialPreviewMode_ &&
    ...
    if (dTime != 0.f) {
        fps = Localise(L"MODELEDITOR/APP/ME_MODULE/POSITIVE_FPS",
                       Formatter(1.f / dTime, L"%.1f"));
    } else {
...
//-- This is old render function (before introducing of deferred render
// pipeline).
...
void MeModule::renderChunks(Moo::DrawContext& drawContext)
{
    ...
    Moo::rc().setRenderState(
      D3DRS_FILLMODE,
      Options::getOptionInt("render/scenery/wireFrame", 0) ? D3DFILL_WIREFRAME
                                                           : D3DFILL_SOLID);
...
void MeModule::renderTerrain(float dTime /* = 0.f */,
...
void MeModule::renderOpaque(Moo::DrawContext& drawContext, float dTime)
{
    ...
    bool showPortals =
      !special && !!Options::getOptionInt("settings/showPortals", 0);
    // bool showShadowing = !isDeffered && showModel && !showWireframe &&
    // !showBsp &&
    ...
    if ((showWireframe) && (!materialPreviewMode_)) {
        rc.setRenderState(D3DRS_FILLMODE, D3DFILL_WIREFRAME);
        // Use a lightonly renderer in case the material will render nothing
        // (e.g. some alpha shaders)
        drawContext.pushOverrideBlock(wireframeRenderer_);
    } else {
        rc.setRenderState(D3DRS_FILLMODE, D3DFILL_SOLID);
    }
    ...
    if (materialPreviewMode_) {
        if (PageMaterials::currPage()->previewObject()) {
            ...
            if (showBoundingBox) {
                Geometrics::wireBox(
                  PageMaterials::currPage()->previewObject()->boundingBox(),
                  0x000000ff);
            }
        }
    } else if (MeApp::instance().mutant()) {
        ...
        if (!showBsp) {
            ...
            renderState |= showModel && !MeApp::instance().mutant()->isSkyBox()
                             ? SHOW_MODEL
                             : 0;
            renderState |= showEditorProxy ? SHOW_EDITOR_PROXY : 0;
        }
        ...
        if (numTris != s_lastNumTris) {
            mainFrame_->setStatusText(
              ...
              Localise(L"MODELEDITOR/APP/ME_MODULE/TRIANGLES", numTris));
            s_lastNumTris = numTris;
        }

        // if (showShadowing)
        //{
        ////TODO: Move the following line so that the watcher can work
        /// correctly. /Set the shadow quailty
        // pShadowCommon_->shadowQuality( Options::getOptionInt(
        // "renderer/shadows/quality", 2 )  );

        //// Render to the shadow caster
        // pCaster_->begin( MeApp::instance().mutant()->shadowVisibililtyBox(),
        // lightTrans ); MeApp::instance().mutant()->drawModel( false, modelDist
        // ); pCaster_->end();
        //}
    }
...
void MeModule::renderFixedFunction(Moo::DrawContext& drawContext, float dTime)
{
    ...
    Vector3           bkgColourV     = Options::getOptionVector3(
                           "settings/bkgColour", Vector3(255.f, 255.f, 255.f)) /
                         255.f;
    Moo::Colour backgroundColour(
      bkgColourV[0], bkgColourV[1], bkgColourV[2], 0.0f);
    ...
    if (showBackground) {
        ...
        rc.setRenderState(D3DRS_ZENABLE, D3DZB_TRUE);
        {
            ...
            {
                Geometrics::drawRect(
                  topLeft, bottomRight, rectColour, rectDepth);
            }
            rc.popRenderState();
        }
        rc.popRenderState();
...
void MeModule::updateModel(Moo::DrawContext& drawContext, float dTime)
{
    ...
    if (MeApp::instance().mutant()->modelName() != "") {
        ...
        if (!materialPreviewMode_) {
            ...
            if ((s_last_groundModel != groundModel) ||
    ...
    if (InputDevices::isKeyDown(KeyCode::KEY_SPACE) && (event.dz() != 0)) {
        ...
        float speed =
          newSpeed == "Medium"
            ? 8.f
            : (newSpeed == "Fast" ? 24.f
                                  : (newSpeed == "SuperFast" ? 48.f : 1.f));
        MeApp::instance().camera()->speed(
          Options::getOptionFloat("camera/speed/" + newSpeed, speed));
        MeApp::instance().camera()->turboSpeed(Options::getOptionFloat(
...
PyObject* MeModule::py_onEditorReady(PyObject* args)
{
    ...
    ChunkManager::instance().startTimeLoading(&MeModule::chunksLoadedCallback,
                                              true);
