...
void MeApp::initCamera()
{
    ...
    if (!!Options::getOptionInt("startup/loadLastModel", 1)) {
        camera_->origin(
          Options::getOptionVector3("startup/lastOrigin", camera_->origin()));
    }
...
void MeApp::saveModel()
{
    ...
    ME_INFO_MSGW(
      Localise(L"MODELEDITOR/APP/ME_APP/SAVING", mutant_->modelName()));
    ...
    if (mutant_->dirty()) {
        int result =
          ::MessageBox(AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
                       ...
                       Localise(L"MODELEDITOR/APP/ME_APP/MODEL_CHANGED"),
        ...
        if (result == IDNO) {
            ME_WARNING_MSGW(
              Localise(L"MODELEDITOR/APP/ME_APP/MODEL_NOT_SAVED"));
        }
    }
 ...
 *
 *	@return Returns True (1) if a model is currently loaded, False (0)
 *otherwise.
 */
 ...
 *	@components{ modeleditor }
 *
 *	This function reverts the model to the last saved model. Any modifications
 *made to the model that have not been saved will be lost.
 */
static PyObject* py_revertModel(PyObject* args)
{
    ...
    int result =
      ::MessageBox(AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
                   ...
                   Localise(L"MODELEDITOR/APP/ME_APP/REVERT_MODEL"),
...
static PyObject* py_zoomToExtents(PyObject* args)
{
    ...
    MeApp::instance().camera()->zoomToExtents(
      true, MeApp::instance().mutant()->zoomBoundingBox());
