...
void PageMaterials::DoDataExchange(CDataExchange* pDX)
{
    ...
    GUI::Manager::instance().add(
      new GUI::Toolbar("MaterialsToolbar", pImpl_->toolbar));
...
BOOL PageMaterials::OnInitDialog()
{
    ...
    UalManager::instance().dropManager().add(new UalDropFunctor<PageMaterials>(
      this, "fx", this, &PageMaterials::changeShaderDrop));
    UalManager::instance().dropManager().add(new UalDropFunctor<PageMaterials>(
      this, "mfm", this, &PageMaterials::changeMFMDrop));

    UalManager::instance().dropManager().add(
      new UalDropFunctor<PageMaterials>(propertyList(),
                                        "bmp",
                                        this,
                                        &PageMaterials::doDrop,
                                        false,
                                        &PageMaterials::dropTest));
    UalManager::instance().dropManager().add(
      new UalDropFunctor<PageMaterials>(propertyList(),
                                        "tga",
                                        this,
    ...
    MeApp::instance().mutant()->registerModelChangeCallback(
      new ModelChangeFunctor<PageMaterials>(this,
                                            &PageMaterials::clearCurrMaterial));
...
ON_NOTIFY(TVN_ITEMEXPANDING,
          IDC_MATERIALS_TREE,
          OnTvnItemexpandingMaterialsTree)
...
void PageMaterials::drawMaterialsList()
{
    ...
    // Need to scope the DelayRedraw since it interfers with EnsureVisible
    {
        ...
        BW::vector<StringPair*>::iterator matterDataIt =
          pImpl_->matterData.begin();
        BW::vector<StringPair*>::iterator matterDataEnd =
          pImpl_->matterData.end();
        ...
        for (unsigned m = 0; m < treeRoot->size(); m++) {
            BW::string materialDisplayName =
              MeApp::instance().mutant()->materialDisplayName(
                (*treeRoot)[m].first.first);

            // TODO:UNICODE: Where is this getting deleted?
            StringPair* matterData = new StringPair(
              (*treeRoot)[m].first.first, (*treeRoot)[m].first.second);
            ...
            HTREEITEM material =
              pImpl_->materialTree.InsertItem(bw_utf8tow(displayName).c_str());
            ...
            if ((*treeRoot)[m].second.size() == 0) {
                ...
                pImpl_->materialTree.SetItemState(material,
                                                  TVIS_BOLD | TVIS_EXPANDED,
                                                  TVIS_BOLD | TVIS_EXPANDED);
            } else {
                pImpl_->materialTree.SetItemState(
                  material, TVIS_EXPANDED, TVIS_EXPANDED);
            }
            ...
            for (unsigned a = 0; a < (*treeRoot)[m].second.size(); a++) {
                if (a == 0) {
                    HTREEITEM item =
                      pImpl_->materialTree.InsertItem(L"Default", material);
                    if ((pImpl_->matterName == wmatterName) &&
                        (pImpl_->tintName == L"Default")) {
                        pImpl_->selItem = item;
                    }
                    if (MeApp::instance().mutant()->getTintName(matterName) ==
                        "Default") {
                        pImpl_->materialTree.SetItemState(
                          item,
                          TVIS_BOLD | TVIS_EXPANDED,
                ...
                HTREEITEM   item =
                  pImpl_->materialTree.InsertItem(tintName.c_str(), material);
                if ((pImpl_->matterName == wmatterName) &&
                    (pImpl_->tintName == tintName)) {
                    pImpl_->selItem = item;
                }
                if (MeApp::instance().mutant()->getTintName(matterName) ==
                    (*treeRoot)[m].second[a]) {
                    pImpl_->materialTree.SetItemState(item,
                                                      TVIS_BOLD | TVIS_EXPANDED,
                                                      TVIS_BOLD |
                                                        TVIS_EXPANDED);
                }
...
afx_msg LRESULT PageMaterials::OnUpdateControls(WPARAM wParam, LPARAM lParam)
{
    ...
    pImpl_->updating = pImpl_->updateCount !=
                       MeApp::instance().mutant()->updateCount("Materials");
    ...
    if (previewMode != s_lastPreviewMode) {
        pImpl_->previewCheck.SetCheck(previewMode ? BST_CHECKED
                                                  : BST_UNCHECKED);
...
void PageMaterials::OnUpdateMaterialPreview()
{
    ...
    BW::BinaryBlockPtr pImage =
      MaterialPreview::instance().generatePreview(bw_wtoutf8(materialName()),
    ...
    GlobalFree(hGlobal);
    UalManager::instance().thumbnailManager().stretchImage(
      *(pImpl_->materialPreview), 128, 128, true);
    pImpl_->materialPreviewRect.SetBitmap(
      (HBITMAP)(*(pImpl_->materialPreview)));
...
afx_msg LRESULT PageMaterials::OnChangePropertyItem(WPARAM wParam,
                                                    LPARAM lParam)
{
    BW_GUARD;
...
afx_msg LRESULT PageMaterials::OnDblClkPropertyItem(WPARAM wParam,
                                                    LPARAM lParam)
{
    BW_GUARD;
...
afx_msg LRESULT PageMaterials::OnRightClkPropertyItem(WPARAM wParam,
                                                      LPARAM lParam)
{
    BW_GUARD;
    ...
    if (lParam) {
        ...
        if (menuOptionsList != L"") {
            ...
            if (((menuOptions[0] == L"FeedOff") &&
                 (sel == 1)) || // Enable a texture feed name
            ...
            } else if ((menuOptions[0] == L"FeedOn") &&
                       (sel == 3)) // Remove the texture feed name
            {
                MeApp::instance().mutant()->materialTextureFeedName(
                  bw_wtoutf8(pImpl_->materialName),
                  bw_wtoutf8(pImpl_->matterName),
...
void PageMaterials::OnTvnItemexpandingMaterialsTree(NMHDR*   pNMHDR,
                                                    LRESULT* pResult)
{
    *pResult = 1; // Ignore any expand or collapse requests
}

void PageMaterials::OnTvnSelchangedMaterialsTree(NMHDR*   pNMHDR,
                                                 LRESULT* pResult)
{
    BW_GUARD;
    ...
    // This makes sure that if we select the material entry then the current
    // tint is selected
    pImpl_->selItem = pImpl_->materialTree.GetSelectedItem();
    HTREEITEM item  = pImpl_->materialTree.GetChildItem(pImpl_->selItem);
    if (item != NULL) {
        StringPair matterData =
          *(StringPair*)(pImpl_->materialTree.GetItemData(pImpl_->selItem));
        BW::wstring tintName = bw_utf8tow(
          MeApp::instance().mutant()->getTintName(matterData.second));
        while (item != NULL) {
            if (pImpl_->materialTree.GetItemText(item).GetString() ==
                tintName) {
    ...
    if ((item != NULL) && (pImpl_->selItem != pImpl_->selParent)) {
        pImpl_->materialTree.SetItemState(pImpl_->selItem,
                                          TVIS_BOLD | TVIS_EXPANDED,
                                          TVIS_BOLD | TVIS_EXPANDED);
    } else {
        pImpl_->materialTree.SetItemState(
          item, TVIS_BOLD | TVIS_EXPANDED, TVIS_BOLD | TVIS_EXPANDED);
    }

    BW::set<Moo::ComplexEffectMaterialPtr> effects;

    StringPair matterData =
      *(StringPair*)(pImpl_->materialTree.GetItemData(pImpl_->selParent));

    pImpl_->materialDisplayName = bw_utf8tow(
      MeApp::instance().mutant()->materialDisplayName(matterData.first));
    ...
    Utilities::fieldEnabledState(
      pImpl_->material, true, pImpl_->materialDisplayName);
    ...
    if (pImpl_->matterName != L"") {
        pImpl_->tintName =
          parent ? pImpl_->materialTree.GetItemText(pImpl_->selItem).GetString()
                 : L"Default";
        Moo::ComplexEffectMaterialPtr effect;
        MeApp::instance().mutant()->setDye(
          bw_wtoutf8(pImpl_->matterName), bw_wtoutf8(pImpl_->tintName), effect);
        effects.insert(effect);
        fxFile = MeApp::instance().mutant()->materialShader(
          bw_wtoutf8(pImpl_->materialName),
          bw_wtoutf8(pImpl_->matterName),
          bw_wtoutf8(pImpl_->tintName));
        ...
        Utilities::fieldEnabledState(
          pImpl_->tint, pImpl_->tintName != L"Default", pImpl_->tintName);
    } else {
        pImpl_->tintName = L"";
        MeApp::instance().mutant()->getMaterial(
          bw_wtoutf8(pImpl_->materialName), effects);
        fxFile = MeApp::instance().mutant()->materialShader(
          bw_wtoutf8(pImpl_->materialName),
          bw_wtoutf8(pImpl_->matterName),
          bw_wtoutf8(pImpl_->tintName));
    ...
    if (fxFile != "") {
        ...
        pImpl_->fxList.SelectString(
          -1, Localise(L"MODELEDITOR/PAGES/PAGE_MATERIALS/DONT_RENDER"));
    }
    ...
    BW::set<Moo::ComplexEffectMaterialPtr>::iterator materialIt =
      effects.begin();
    BW::set<Moo::ComplexEffectMaterialPtr>::iterator materialEnd =
      effects.end();
    ...
    SmartPointer<MeMaterialFlagProxy> cfp =
      new MeMaterialFlagProxy(L"collisionFlags",
                              pImpl_->materialName,
                              &(pImpl_->matterName),
                              &(pImpl_->tintName));

    pProp = new ChoiceProperty(
      LocaliseUTF8("MODELEDITOR/PAGES/PAGE_MATERIALS/COLLISION"),
      cfp.get(),
      collisionFlags,
      true);
    pProp->UIDesc(
      LocaliseUTF8(L"MODELEDITOR/PAGES/PAGE_MATERIALS/COLLISION_DESC"));
    pImpl_->editor->addProperty(pProp);

    // Now lets add the material type for the material

    SmartPointer<MeMaterialFlagProxy> mkp =
      new MeMaterialFlagProxy(L"materialKind",
                              pImpl_->materialName,
                              &(pImpl_->matterName),
                              &(pImpl_->tintName));

    pProp =
      new ChoiceProperty(LocaliseUTF8("MODELEDITOR/PAGES/PAGE_MATERIALS/KIND"),
                         mkp.get(),
                         materialKinds);
    ...
    if (pImpl_->previewObject) {
        ...
        if (count > 0 && materialIt != materialEnd) {
            Moo::ComplexEffectMaterialPtr mat = mats[0];
            pImpl_->previewObject->overrideMaterial(mat->identifier(),
                                                    *(pImpl_->currMaterial));
        }
    }
    ...
    while (materialIt != materialEnd) {
        ...
        MaterialProperties::addMaterialProperties((*materialIt)->baseMaterial(),
                                                  pImpl_->editor.get(),
                                                  canExposeToScript,
                                                  true,
                                                  this);
...
void PageMaterials::tintNew()
{
    ...
    MeApp::instance().mutant()->tintNames(bw_wtoutf8(pImpl_->matterName),
                                          tintNames);
    CNewTint newTintDlg(tintNames);
    if (newTintDlg.DoModal() == IDOK) {
        BW::string newMatterName =
          MeApp::instance().mutant()->newTint(bw_wtoutf8(pImpl_->materialName),
                                              bw_wtoutf8(pImpl_->matterName),
                                              bw_wtoutf8(pImpl_->tintName),
                                              newTintDlg.tintName(),
                                              newTintDlg.fxFile(),
 ...
 *	@components{ modeleditor }
 *
 *	This function enables the Open File dialog, which allows an MFM to be
 *loaded.
 */
...
void PageMaterials::mfmSave()
{
    ...
    if (fileDlg.showDialog()) {
        ...
        if (BWResource::validPathW(mfmName)) {
            ...
            if (MeApp::instance().mutant()->saveMFM(
                  bw_wtoutf8(pImpl_->materialName),
                  bw_wtoutf8(pImpl_->matterName),
                  bw_wtoutf8(pImpl_->tintName),
                  bw_wtoutf8(mfmName))) {
...
void PageMaterials::tintDelete()
{
    ...
    MeApp::instance().mutant()->deleteTint(bw_wtoutf8(pImpl_->matterName),
                                           bw_wtoutf8(pImpl_->tintName));
}
...
void PageMaterials::OnEnKillfocusMaterialsMaterial()
{
    ...
    if (first != BW::string::npos) {
        ...
        pImpl_->materialTree.SetItemText(pImpl_->selParent,
                                         displayName.c_str());
        ...
        ::MessageBox(
          ...
          Localise(L"MODELEDITOR/PAGES/PAGE_MATERIALS/INVALID_MATERIAL_NAME"),
          MB_OK | MB_ICONERROR);

        return;
    }

    if (MeApp::instance().mutant()->materialName(
          bw_wtoutf8(pImpl_->materialName), bw_wtoutf8(new_name))) {
        ...
        ::MessageBox(
          ...
          Localise(L"MODELEDITOR/PAGES/PAGE_MATERIALS/MATERIAL_NAME_EXISTS"),
          MB_OK | MB_ICONERROR);
    }
}
...
void PageMaterials::OnEnChangeMaterialsMatter()
{
    ...
    BW::wstring displayName =
      pImpl_->materialDisplayName + L" (" + bw_utf8tow(new_name) + L")";
    pImpl_->materialTree.SetItemText(pImpl_->selParent, displayName.c_str());
...
void PageMaterials::OnEnKillfocusMaterialsMatter()
{
    ...
    if (new_name == L"") {
        BW::wstring displayName =
          pImpl_->materialDisplayName + L" (" + pImpl_->matterName + L")";
        ...
        ::MessageBox(
          ...
          Localise(L"MODELEDITOR/PAGES/PAGE_MATERIALS/INVALID_DYE_NAME"),
          MB_OK | MB_ICONERROR);
    } else if (MeApp::instance().mutant()->matterName(
                 bw_wtoutf8(pImpl_->matterName), bw_wtoutf8(new_name))) {
        StringPair* matterData =
          (StringPair*)(pImpl_->materialTree.GetItemData(pImpl_->selParent));
        ...
        ::MessageBox(
          ...
          Localise(L"MODELEDITOR/PAGES/PAGE_MATERIALS/DYE_NAME_EXISTS"),
          MB_OK | MB_ICONERROR);
    }
}
...
void PageMaterials::OnEnKillfocusMaterialsTint()
{
    ...
    if (first != BW::wstring::npos) {
        ...
        pImpl_->materialTree.SetItemText(pImpl_->selItem,
                                         pImpl_->tintName.c_str());
        pImpl_->tint.SetWindowText(pImpl_->tintName.c_str());

        ::MessageBox(
          AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
          Localise(L"MODELEDITOR/PAGES/PAGE_MATERIALS/NO_RENAME_TINT"),
          Localise(L"MODELEDITOR/PAGES/PAGE_MATERIALS/INVALID_DYE_TINT"),
          MB_OK | MB_ICONERROR);

        return;
    }

    if (MeApp::instance().mutant()->tintName(bw_wtoutf8(pImpl_->matterName),
                                             bw_wtoutf8(pImpl_->tintName),
                                             bw_wtoutf8(new_name))) {
        ...
        ::MessageBox(
          ...
          Localise(L"MODELEDITOR/PAGES/PAGE_MATERIALS/TINT_NAME_EXISTS"),
          MB_OK | MB_ICONERROR);
    }
}

void PageMaterials::redrawList(CComboBox&        list,
...
void PageMaterials::OnCbnSelchangeMaterialsFxList()
{
    ...
    if (pImpl_->fxList.GetCurSel() ==
        pImpl_->fxList.GetCount() - 1) // (Other...)
    {
        redrawList(pImpl_->fxList, "fx", true);
        OnBnClickedMaterialsFxSel();
        return;
    } else if (pImpl_->fxList.GetCurSel() ==
               pImpl_->fxList.GetCount() - 2) // (Don't Render)
    {
        MeApp::instance().mutant()->materialShader(
          bw_wtoutf8(pImpl_->materialName),
          bw_wtoutf8(pImpl_->matterName),
          bw_wtoutf8(pImpl_->tintName),
          "");
...
bool PageMaterials::changeShader(const BW::wstring& fxFile)
{
    ...
    if (MeApp::instance().mutant()->materialShader(
          bw_wtoutf8(pImpl_->materialName),
          bw_wtoutf8(pImpl_->matterName),
          bw_wtoutf8(pImpl_->tintName),
          bw_wtoutf8(fxFile))) {
    ...
    BW::string oldFxFile = MeApp::instance().mutant()->materialShader(
      bw_wtoutf8(pImpl_->materialName),
      bw_wtoutf8(pImpl_->matterName),
      bw_wtoutf8(pImpl_->tintName));
    if (oldFxFile != "") {
...
bool PageMaterials::changeMFM(const BW::wstring& mfmFile)
{
    ...
    if (MeApp::instance().mutant()->materialMFM(
          bw_wtoutf8(pImpl_->materialName),
          bw_wtoutf8(pImpl_->matterName),
          bw_wtoutf8(pImpl_->tintName),
          bw_wtoutf8(mfmFile),
...
RectInt PageMaterials::dropTest(UalItemInfo* ii)
{
    ...
    CRect rect = propertyList()->dropTest(
      CPoint(ii->x(), ii->y()), BWResource::dissolveFilenameW(ii->longText()));
...
void PageMaterials::OnCbnSelchangeMaterialsPreviewList()
{
    ...
    if (count > 0) {
        ...
        if (pImpl_->currMaterial)
            pImpl_->previewObject->overrideMaterial(
              mat->baseMaterial()->identifier(), *(pImpl_->currMaterial));
    }
...
void PageMaterials::OnBnClickedMaterialsPreview()
{
    ...
    // Make sure we can only go into preview mode if there is a valid preview
    // object
...
BW::string PageMaterials::exposedToScriptName(const BW::string& descName) const
{
    ...
    return MeApp::instance().mutant()->exposedToScriptName(
      bw_wtoutf8(pImpl_->matterName), bw_wtoutf8(pImpl_->tintName), descName);
}
