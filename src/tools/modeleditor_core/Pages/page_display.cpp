  ...
  : CFormView(PageDisplay::IDD)
{
    ...
    pImpl_->floraSettings =
      Moo::GraphicsSetting::getFromLabel(FloraSettings::getFloraSettingId());
    ...
    if (pImpl_->floraSettings) {
        ...
        for (unsigned i = 0; i < pImpl_->numFloraOptions; i++) {
            ...
            if (i != pImpl_->numFloraOptions - 1) {
                ...
                label +=
                  Localise(L"MODELEDITOR/PAGES/PAGE_DISPLAY/FLORA_DENSITY");
            } else {
                label =
                  Localise(L"MODELEDITOR/PAGES/PAGE_DISPLAY/DISABLE_FLORA");
            }

            pImpl_->flora.InsertString(0, label.c_str());
        }

        pImpl_->floraSettings->selectOption(Options::getOptionInt(
          "settings/floraDensity", pImpl_->numFloraOptions));
    } else {
        // Disable the flora tab if we can't get settings
        pImpl_->flora.InsertString(
          0, Localise(L"MODELEDITOR/PAGES/PAGE_DISPLAY/DISABLE_FLORA"));
        pImpl_->flora.SetCurSel(0);
        pImpl_->flora.ModifyStyle(0, WS_DISABLED);
        pImpl_->flora.RedrawWindow();
    }

    UalManager::instance().dropManager().add(new UalDropFunctor<PageDisplay>(
      &(pImpl_->floorTexture), "bmp", this, &PageDisplay::floorTextureDrop));

    UalManager::instance().dropManager().add(new UalDropFunctor<PageDisplay>(
      &(pImpl_->floorTexture), "tga", this, &PageDisplay::floorTextureDrop));

    UalManager::instance().dropManager().add(new UalDropFunctor<PageDisplay>(
      &(pImpl_->floorTexture), "jpg", this, &PageDisplay::floorTextureDrop));

    UalManager::instance().dropManager().add(new UalDropFunctor<PageDisplay>(
      &(pImpl_->floorTexture), "png", this, &PageDisplay::floorTextureDrop));

    UalManager::instance().dropManager().add(new UalDropFunctor<PageDisplay>(
      &(pImpl_->floorTexture), "dds", this, &PageDisplay::floorTextureDrop));

    UalManager::instance().dropManager().add(
      new UalDropFunctor<PageDisplay>(&(pImpl_->floorTexture),
                                      "texanim",
                                      this,
                                      &PageDisplay::floorTextureDrop));
...
ON_BN_CLICKED(IDC_DISPLAY_CHOOSE_FLOOR_TEXTURE,
              OnBnClickedDisplayChooseFloorTexture)
...
afx_msg LRESULT PageDisplay::OnUpdateControls(WPARAM wParam, LPARAM lParam)
{
    ...
    if (PanelManager::instance().getEditorApp()->pythonAdapter()) {
        ...
        bool success;
        success = PanelManager::instance()
                    .getEditorApp()
                    ->pythonAdapter()
                    ->ActionScriptUpdate("actGetShadowIndex", enabled, index);
        ...
        success = PanelManager::instance()
                    .getEditorApp()
                    ->pythonAdapter()
                    ->ActionScriptUpdate("actGetBkgIndex", enabled, index);
    ...
    if (pImpl_->numFloraOptions) {
        index =
          pImpl_->numFloraOptions - pImpl_->floraSettings->activeOption() - 1;
    ...
    if (time != pImpl_->last_time) {
        ...
        pImpl_->timeOfDaySlider.SetPos(
          (int)(pImpl_->timeOfDaySlider.GetRangeMin() +
...
void PageDisplay::OnBnClickedShowAxes()
{
    ...
    if (PanelManager::instance().getEditorApp()->pythonAdapter()) {
        PanelManager::instance()
          .getEditorApp()
          ->pythonAdapter()
          ->ActionScriptExecute("actShowAxes");
    }
}
...
void PageDisplay::OnBnClickedCheckForSparkles()
{
    ...
    if (PanelManager::instance().getEditorApp()->pythonAdapter()) {
        PanelManager::instance()
          .getEditorApp()
          ->pythonAdapter()
          ->ActionScriptExecute("actCheckForSparkles");
    }
}
...
void PageDisplay::OnBnClickedEnableFog()
{
    ...
    if (PanelManager::instance().getEditorApp()->pythonAdapter()) {
        PanelManager::instance()
          .getEditorApp()
          ->pythonAdapter()
          ->ActionScriptExecute("actEnableFog");
    }
}
...
void PageDisplay::OnBnClickedEnableHDR()
{
    ...
    if (PanelManager::instance().getEditorApp()->pythonAdapter()) {
        PanelManager::instance()
          .getEditorApp()
          ->pythonAdapter()
          ->ActionScriptExecute("actEnableHDR");
    }
}
...
void PageDisplay::OnBnClickedEnableSSAO()
{
    ...
    if (PanelManager::instance().getEditorApp()->pythonAdapter()) {
        PanelManager::instance()
          .getEditorApp()
          ->pythonAdapter()
          ->ActionScriptExecute("actEnableSSAO");
    }
}
...
void PageDisplay::OnBnClickedShowModel()
{
    ...
    if (PanelManager::instance().getEditorApp()->pythonAdapter()) {
        PanelManager::instance()
          .getEditorApp()
          ->pythonAdapter()
          ->ActionScriptExecute("actShowModel");
    }
}
...
void PageDisplay::OnBnClickedShowWireframe()
{
    ...
    if (PanelManager::instance().getEditorApp()->pythonAdapter()) {
        PanelManager::instance()
          .getEditorApp()
          ->pythonAdapter()
          ->ActionScriptExecute("actShowWireframe");
    }
}
...
void PageDisplay::OnBnClickedShowSkeleton()
{
    ...
    if (PanelManager::instance().getEditorApp()->pythonAdapter()) {
        PanelManager::instance()
          .getEditorApp()
          ->pythonAdapter()
          ->ActionScriptExecute("actShowSkeleton");
    }
}
...
void PageDisplay::OnBnClickedShowBsp()
{
    ...
    if (PanelManager::instance().getEditorApp()->pythonAdapter()) {
        PanelManager::instance()
          .getEditorApp()
          ->pythonAdapter()
          ->ActionScriptExecute("actShowBsp");
    }
}
...
void PageDisplay::OnBnClickedShowBoundingBox()
{
    ...
    if (PanelManager::instance().getEditorApp()->pythonAdapter()) {
        PanelManager::instance()
          .getEditorApp()
          ->pythonAdapter()
          ->ActionScriptExecute("actShowBoundingBox");
        GUI::Manager::instance()
          .update(); // Need this since it will cause a toolbar update
    }
}
...
void PageDisplay::OnBnClickedShowPortals()
{
    ...
    if (PanelManager::instance().getEditorApp()->pythonAdapter()) {
        PanelManager::instance()
          .getEditorApp()
          ->pythonAdapter()
          ->ActionScriptExecute("actShowPortals");
    }
}
...
void PageDisplay::OnBnClickedShowVertexNormals()
{
    ...
    if (PanelManager::instance().getEditorApp()->pythonAdapter()) {
        PanelManager::instance()
          .getEditorApp()
          ->pythonAdapter()
          ->ActionScriptExecute("actShowNormals");
    }
}
...
void PageDisplay::OnBnClickedShowVertexBinormals()
{
    ...
    if (PanelManager::instance().getEditorApp()->pythonAdapter()) {
        PanelManager::instance()
          .getEditorApp()
          ->pythonAdapter()
          ->ActionScriptExecute("actShowBinormals");
    }
}
...
void PageDisplay::OnBnClickedShowCustomHull()
{
    ...
    if (PanelManager::instance().getEditorApp()->pythonAdapter()) {
        PanelManager::instance()
          .getEditorApp()
          ->pythonAdapter()
          ->ActionScriptExecute("actShowCustomHull");
    }
}
...
void PageDisplay::OnBnClickedShowHardPoints()
{
    ...
    if (PanelManager::instance().getEditorApp()->pythonAdapter()) {
        PanelManager::instance()
          .getEditorApp()
          ->pythonAdapter()
          ->ActionScriptExecute("actShowHardPoints");
    }
}
...
void PageDisplay::OnBnClickedGroundModel()
{
    ...
    if (PanelManager::instance().getEditorApp()->pythonAdapter()) {
        PanelManager::instance()
          .getEditorApp()
          ->pythonAdapter()
          ->ActionScriptExecute("actGroundModel");
    }
}
...
void PageDisplay::OnBnClickedCentreModel()
{
    ...
    if (PanelManager::instance().getEditorApp()->pythonAdapter()) {
        PanelManager::instance()
          .getEditorApp()
          ->pythonAdapter()
          ->ActionScriptExecute("actCentreModel");
    }
}
...
void PageDisplay::OnBnClickedShowEditorProxy()
{
    ...
    if (PanelManager::instance().getEditorApp()->pythonAdapter()) {
        PanelManager::instance()
          .getEditorApp()
          ->pythonAdapter()
          ->ActionScriptExecute("actShowEditorProxy");
    }
}
        ...
        switch (pImpl_->shadowing.GetCurSel()) {
            ...
            default:
                PanelManager::instance()
                  .getEditorApp()
                  ->pythonAdapter()
                  ->ActionScriptExecute("actShadowMaxQuality");
                break;
            case 1:
                PanelManager::instance()
                  .getEditorApp()
                  ->pythonAdapter()
                  ->ActionScriptExecute("actShadowHighQuality");
                break;
            case 2:
                PanelManager::instance()
                  .getEditorApp()
                  ->pythonAdapter()
                  ->ActionScriptExecute("actShadowMedQuality");
                break;
            case 3:
                PanelManager::instance()
                  .getEditorApp()
                  ->pythonAdapter()
                  ->ActionScriptExecute("actShadowLowQuality");
                break;
            case 4:
                PanelManager::instance()
                  .getEditorApp()
                  ->pythonAdapter()
                  ->ActionScriptExecute("actShadowOff");
                break;
        };
    }
}
        ...
        switch (pImpl_->bkg.GetCurSel()) {
            case 0:
                if (PanelManager::instance()
                      .getEditorApp()
                      ->pythonAdapter()
                      ->ActionScriptExecute("actBkgNone")) {
                    pImpl_->flora.ModifyStyle(0, WS_DISABLED);
            ...
            case 1:
                if (PanelManager::instance()
                      .getEditorApp()
                      ->pythonAdapter()
                      ->ActionScriptExecute("actBkgFloor")) {
                    pImpl_->flora.ModifyStyle(0, WS_DISABLED);
                    pImpl_->bkgColour.ModifyStyle(WS_DISABLED, 0);
            ...
            default:
                if (PanelManager::instance()
                      .getEditorApp()
                      ->pythonAdapter()
                      ->ActionScriptExecute("actBkgTerrain")) {
...
void PageDisplay::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
    ...
    if (PanelManager::instance().getEditorApp()->pythonAdapter()) {
        PanelManager::instance()
          .getEditorApp()
          ->pythonAdapter()
          ->onSliderAdjust("slrCurrentTime",
                           pImpl_->timeOfDaySlider.GetPos(),
                           pImpl_->timeOfDaySlider.GetRangeMin(),
                           pImpl_->timeOfDaySlider.GetRangeMax());

        BW::string currentTime =
          MeShell::instance().timeOfDay()->getTimeOfDayAsString();
...
void PageDisplay::OnBnClickedDisplayChooseBkgColour()
{
    ...
    CColorDialog colorDlg(RGB(bkgColour[0], bkgColour[1], bkgColour[2]),
                          CC_FULLOPEN);
    if (colorDlg.DoModal() == IDOK) {
        ...
        Options::setOptionVector3("settings/bkgColour",
                                  Vector3((float)r, (float)g, (float)b));
    }
}
...
void PageDisplay::OnBnClickedDisplayChooseFloorTexture()
{
    ...
    static wchar_t szFilter[] =
      L"Texture "
      L"files(*.bmp;*.tga;*.jpg;*.png;*.dds;*.texanim)|*.bmp;*.tga;*.jpg;*.png;"
      L"*.dds;*.texanim|"
    ...
    BW::string floorTextureDir =
      BWResource::resolveFilename(MeApp::instance().floor()->getTextureName());
