...
void PageAnimations::calculateCompressionErrorFactors(float& scale,
                                                      float& rotation,
                                                      float& position)
{
    BW_GUARD;
    ...
    position =
      0.00000000001f * powf(1.02f, pImpl_->compPosSldr.GetPos() + 300.f);
    rotation = 0.00001f * powf(1.03f, (float)pImpl_->compRotSldr.GetPos());
    scale =
      0.00000000001f * powf(1.02f, pImpl_->compScaleSldr.GetPos() + 300.f);
}
...
void PageAnimations::setSlidersCompressionErrorFactors(float scale,
                                                       float rotation,
                                                       float position)
{
    BW_GUARD;

    pImpl_->compPosSldr.SetPos(
      int(logf(1.02f, position / 0.00000000001f) - 300.f));
    pImpl_->compRotSldr.SetPos(int(logf(1.03f, rotation / 0.00001f)));
    pImpl_->compScaleSldr.SetPos(
      int(logf(1.02f, scale / 0.00000000001f) - 300.f));
}
...
void PageAnimations::updateCompression()
{
    ...
    Moo::AnimationPtr anim =
      MeApp::instance().mutant()->restoreChannels(selID());
    ...
    for (uint i = 0; i < anim->nChannelBinders(); i++) {
        Moo::AnimationChannelPtr channel = anim->channelBinder(i).channel();
        Moo::NodePtr             node =
          Moo::NodeCatalogue::instance().find(channel->identifier().c_str());
    ...
    if (osk > 0) {
        bw_snwprintf(buf,
                     ARRAY_SIZE(buf),
                     L"%.0f %%",
                     100.f * (1.f - ((float)tsk / (float)osk)));
    } else {
        buf[0] = 0;
    }
    pImpl_->compScale.SetWindowText(buf);

    pImpl_->compTotal.SetWindowText(Localise(
      L"MODELEDITOR/PAGES/PAGE_ANIMATIONS_COMP/COMPRESSED_DATA",
      Formatter(origSize / 1024.f, L"%0.f"),
      Formatter(compressedSize / 1024.f, L"%0.f"),
      Formatter(100.f * (1.f - ((float)compressedSize / (float)origSize)),
                L"%0.f")));
}
 ...
 *
 *	This function checks whether it is possible to view the uncompressed
 *animation.
 *
 *	@return Returns True (1) if it is possible to view the uncompressed
 *animation, False (0) otherwise.
 */
static PyObject* py_canViewAnimComp(PyObject* args)
{
    ...
    if (PageAnimations::currPage())
        return PyInt_FromLong(
          PageAnimations::currPage()->canViewAnimCompression());
 ...
 *	@components{ modeleditor }
 *
 *	This function checks whether it is possible to save the compressed
 *animation.
 *
 *	@return Returns True (1) if it is possible to save the compressed animation,
 *False (0) otherwise.
 */
static PyObject* py_canSaveAnimComp(PyObject* args)
{
    ...
    if (PageAnimations::currPage())
        return PyInt_FromLong(
          PageAnimations::currPage()->canSaveAnimCompression());
    return PyInt_FromLong(0);
}
...
void PageAnimations::saveAnimCompression()
{
    ...
    if (MeApp::instance().mutant()->animName(selID()) == "")
        return;
