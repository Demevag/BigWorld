...
void CLodBar::setWidth(int w)
{
    ...
    GetWindowRect(&rect);
    ScreenToClient(&rect);
    parent_->SetWindowPos(
      0, 0, 0, rect.left + w, rect.Height(), SWP_NOMOVE | SWP_NOZORDER);
    SetWindowPos(
      0, 0, 0, rect.left + w, rect.Height(), SWP_NOMOVE | SWP_NOZORDER);
    width_ = w;
    redraw();
...
void CLodBar::OnMouseMove(UINT nFlags, CPoint point)
{
    ...
    if (mover_ == -1) {
        SetCursor(
          LoadCursor(NULL, (mover(point.x) != -1) ? IDC_SIZEWE : IDC_ARROW));
    }
...
void CLodBar::OnLButtonUp(UINT nFlags, CPoint point)
{
    ...
    if (mover_ != -1) {
        SetCursor(
          LoadCursor(NULL, (mover(point.x) != -1) ? IDC_SIZEWE : IDC_ARROW));
        MeApp::instance().mutant()->lodList(
          lodList_); // Commit the new LOD list
        UndoRedo::instance().barrier(
          LocaliseUTF8(L"MODELEDITOR/GUI/LOD_BAR/CHANGING_MODEL_EXTENT"),
          false);
        mover_ = -1;
    }
}
