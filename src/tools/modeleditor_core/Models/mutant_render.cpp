  ...
  bool                                                   toCompressedChannel)
{
    ...
    for (; animIt != animEnd; ++animIt) {
        ...
        SuperModelAnimationPtr superModelAnim =
        ...
        if (!toCompressedChannel) {
            animations_[animIt->second].uncompressAnim(
              anim, oldChannels[animIt->first]);
        } else {
            animations_[animIt->second].restoreAnim(anim,
                                                    oldChannels[animIt->first]);
        }
    }
}
...
void Mutant::drawOriginalModel(Moo::DrawContext& drawContext)
{
    ...
    // Now draw the model
    superModel_->draw(drawContext, this->transform(false), &materialFashions_);
        ...
        if (model != NULL) {
            ...
            while (it != end) {
                ...
                if ((nodeName.substr(0, 3) != BW::string("HP_")) &&
                    (nodeName.find("BlendBone", 0) == nodeName.npos)) {
                    Vector3 fromPos = it->pParentTransform->applyToOrigin();

                    const Matrix& nodeTrans =
                      it->pData->pNode->worldTransform();
                    ...
                    float   length =
                      0.1f * vDir.length(); // Make the arrowhead proportional
                                            // to the length of the bone.
                    vDir.normalise();
                    Vector3 ortog = vDir.crossProduct(nodeTrans[1]);
                    ortog.normalise();
                    Geometrics::drawLine(toPos,
                                         toPos - vDir * length + ortog * length,
                                         0x00ff0000);
                    Geometrics::drawLine(toPos,
                                         toPos - vDir * length - ortog * length,
                                         0x00ff0000);
...
void Mutant::drawPortals()
{
    ...
    while (opit != opend) {
        ...
        Moo::rc().device()->SetTransform(
          D3DTS_WORLD, &transform(groundModel_, centreModel_));
    ...
    while (bit != bend) {
        ...
        while (pit != pend) {
            ...
            while (ptit != ptend) {
                ...
                points.push_back(
                  Vector3(uAxis * pt[0] + vAxis * pt[1] + origin));
            }
...
bool Mutant::drawBspInternal()
{
    ...
    // Draw the BSP
    this->drawBsp(Moo::rc(), transform(groundModel_, centreModel_), false);
    return true;
...
void Mutant::drawNormals(bool showNormals, bool showBinormals)
{
    ...
    // Determine a scale to use for the axes based on the total size of the
    // model
    float scale = ((float)normalsLength_ / 50) *
                  (modelBB_.maxBounds() - modelBB_.minBounds()).length() / 30.f;
    ...
    for (int i = 0; i < superModel_->nModels(); ++i) {
        Model* model = superModel_->curModel(i);
        if (model != NULL) {
            const Moo::Visual::RenderSetVector& set =
              model->getVisual()->renderSets();
            ...
            for (; it != set.end(); it++) {
                ...
                Moo::Visual::GeometryVector::const_iterator g_it =
                  (*it).geometry_.begin();
                ...
                Moo::BaseSoftwareSkinner* pSkinner =
                  geometry.vertices_->softwareSkinner();
                ...
                if (pSkinner) {
                    ...
                    VertexType* pVerts =
                      vb.lock<VertexType>(geometry.vertices_->nVertices());
                    pSkinner->transformVertices(pVerts,
                                                0,
                                                geometry.vertices_->nVertices(),
                                                (*it).transformNodes_);
                    vb.unlock();
                    for (uint32 i = 0; i < geometry.vertices_->nVertices();
                         i++) {
                        points.push_back(pVerts[i].pos_);
                        tangentspace.push_back(
                          Moo::packNormal(pVerts[i].normal_));
                        tangentspace.push_back(
                          Moo::packNormal(pVerts[i].tangent_));
                        tangentspace.push_back(
                          Moo::packNormal(pVerts[i].binormal_));
                    }
                ...
                Moo::Vertices::VertexNormals::const_iterator n_it =
                  tangentspace.begin();
                Moo::Vertices::VertexPositions::const_iterator n2_it =
                  normals.begin();
                Moo::Vertices::VertexNormals::const_iterator n3_it =
                  packedNormals.begin();
                ...
                Moo::Vertices::VertexPositions::const_iterator p_it =
                  points.begin();
                ...
                for (; p_it != points.end(); p_it++) {
                    ...
                    if (drawTS && !showBinormals) {
                        // We need to do this to so that on the next pass we get
                        // the correct normal
                        n_it++;
                        n_it++;
                    }
...
void Mutant::drawHardPoints(Moo::DrawContext& drawContext)
{
    ...
    drawContext.drawUserItem(
      nodesIDs, Moo::DrawContext::TRANSPARENT_CHANNEL_MASK, 0.0f);
    Moo::rc().pop();
}
...
void Mutant::calcCustomHull()
{
    ...
    // STEP 2:
    // for each plane we need to store all of its intersections with other
    // planes O(P^2)
    ...
    for (size_t i = 0; i < planeLines.size(); i++) {
        ...
        // STEP 4:
        // since the lines on the planes have infinite length there will be alot
        // of bogus points calculated. What we need to do is clean up so that we
        // only store the points that we are interested in. O(L^2*P^2)
        ...
        for (size_t j = 0; j < planePoints.at(i).size(); j++) {
            ...
            for (size_t k = 0; k < planes.size(); k++) {
                ...
                if (i == k)
                    continue;
                if (almostEqual(
                      planes.at(k).distanceTo(planePoints.at(i).at(j)), 0.f))
                    continue;
    ...
    // STEP 5:
    // Reorder the points so we can easily connect the dots without any criss
    // crossing uses same algorithm as in drawPortals.
    for (size_t i = 0; i < planes.size(); i++) {
...
void Mutant::drawCustomHull()
{
    ...
    for (size_t i = 0; i < customHullPoints_.size(); i++) {
        ...
        for (; j < customHullPoints_.at(i).size() - 1; j++)
            Geometrics::drawLine(customHullPoints_.at(i).at(j),
                                 customHullPoints_.at(i).at(j + 1),
                                 0x00ffff00);
        Geometrics::drawLine(customHullPoints_.at(i).at(j),
                             customHullPoints_.at(i).at(0),
                             0x00ffff00);
    }
}

void Mutant::render(Moo::DrawContext& drawContext,
                    float             dTime,
                    int               renderStates)
{
    BW_GUARD;
    ...
    if ((renderStates & SHOW_BSP) || ChunkBspHolder::getDrawBsp()) {
        ...
        if ((renderStates & SHOW_EDITOR_PROXY) && (editorProxySuperModel_)) {
            editorProxySuperModel_->draw(
              drawContext, this->transform(false), NULL);
        }
    }
