...
/**
 * This is a helper function which strip two strings down to their differing
 roots, e.g.
...
void Mutant::clipToDiffRoot(BW::string& strA, BW::string& strB)
{
    ...
    while ((posA != BW::string::npos) && (posB != BW::string::npos) &&
...
/**
 * This method checks whether a texture could be an animated texture and fixes
 *the animtex if needed.
 *
 *  @param	texRoot			The texture to test
 *	@param	oldRoot			The old root to replace
 *	@param	newRoot			The new root to replace with
 */
void Mutant::fixTexAnim(const BW::string& texRoot,
                        const BW::string& oldRoot,
                        const BW::string& newRoot)
{
    BW_GUARD;

    BW::string texAnimName =
      texRoot.substr(0, texRoot.find_last_of(".")) + ".texanim";
        ...
        for (unsigned i = 0; i < textures.size(); i++) {
            ...
            if (textureName != "") {
                ...
                if (pos != BW::string::npos) {
                    textureName =
                      textureName.replace(pos, oldRoot.length(), newRoot);
 ...
 */
bool Mutant::fixTextures(DataSectionPtr    mat,
                         const BW::string& oldRoot,
                         const BW::string& newRoot)
{
    BW_GUARD;
    ...
    for (unsigned i = 0; i < effects.size(); i++) {
        ...
        if (newFx != "") {
            if (BWResource::fileExists(newFx))
                continue;
    ...
    for (unsigned i = 0; i < props.size(); i++) {
        ...
        if (newTexture != "") {
            if (BWResource::fileExists(newTexture))
                continue;
        ...
        } else // Make sure to handle texture feeds as well
        {
            ...
            if (newTexture != "") {
                ...
                if (pos != BW::string::npos) {
                    newTexture =
                      newTexture.replace(pos, oldRoot.length(), newRoot);
 ...
 *  @param	ext				The extension of the file type to find (e.g.
 "model", "visual", etc)
 *  @param	what			What operation is being exectuted (e.g. "load",
 "add", etc)
 *  @param	criticalMsg		A string for whether the model can load without
 ...
 */
bool Mutant::locateFile(BW::string&       fileName,
                        BW::string        modelName,
                        const BW::string& ext,
                        ...
                        const BW::string& criticalMsg /* = "" */)
{
    ...
    BW::vector<BW::string>::iterator entry =
      std::find(s_missingFiles_.begin(), s_missingFiles_.end(), fileName);

    if (entry != s_missingFiles_.end())
        return false;
    ...
    int response = ::MessageBox(
      ...
      Localise(L"MODELEDITOR/MODELS/MUTANT_VALIDATION/CANT_LOCATE_Q",
               ext,
               fileName,
               ext,
               criticalMsg),
      Localise(L"MODELEDITOR/MODELS/MUTANT_VALIDATION/UNABLE_LOCATE", ext),
      MB_ICONERROR | MB_YESNO);

    if (response == IDYES) {
        BW::string oldFileName =
          fileName.substr(fileName.find_last_of("/") + 1) + "." + ext;

        BW::string   fileFilter = ext + " (*." + ext + ")|*." + ext + "||";
        BWFileDialog fileDlg(true,
                             L"",
                             bw_utf8tow(oldFileName).c_str(),
                             BWFileDialog::FD_WRITABLE_AND_EXISTS,
                             bw_utf8tow(fileFilter).c_str());
...
/**
 * This method tests to see if the given file is read only, and display a
 *message box.
 *
...
/**
 * This method tests to see if any of the files associated with the model are
 *read only. Checks .model, .visual, .primitives. .animation files can be
 *checked separately. Displays a message box if a one of the files is read only.
 *
 ...
 *	@param	primitivesName	The path of the primitives file to test for read
 *only
 *
 *	@return Whether any of the files associated with the model are read only.
 */
bool Mutant::testReadOnly(const BW::string& modelName,
 ...
 *	@param	readOnly		Returns true if any of the model's associated files
 are read only
 *
 *	@return Whether the file was valid.
 */
bool Mutant::ensureModelValid(const BW::string& name,
                              const BW::string& what,
                              DataSectionPtr*   model /* = NULL */,
                              DataSectionPtr*   visual /* = NULL */,
                              BW::string*       visualName /* = NULL */,
                              BW::string*       primitivesName /* = NULL */,
                              bool*             readOnly /* = NULL */)
{
    BW_GUARD;
    ...
    if (testModel) {
        ...
        if (testRoot != "") {
            ...
            if (!testVisual) {
                ...
                if (!locateFile(
                      ...
                      "visual",
                      what,
                      LocaliseUTF8(L"MODELEDITOR/MODELS/MUTANT_VALIDATION/"
                                   L"UNABLE_LOAD_OTHERWISE")))
                    return false;

                newRoot = testRoot;
                testVisual =
                  BWResource::openSection(testRoot + ".visual", false);
                ...
                for (unsigned i = 0; i < anims.size(); i++) {
                    ...
                    if (BWResource::fileExists(animRoot + ".animation"))
                        continue;
                    BW::string::size_type pos = animRoot.find(oldRoot);
                    if (pos != BW::string::npos) {
                        animRoot =
                          animRoot.replace(pos, oldRoot.length(), newRoot);
                ...
                for (unsigned i = 0; i < dyes.size(); i++) {
                    ...
                    for (unsigned j = 0; j < tints.size(); j++) {
                        fixTextures(
                          tints[j]->openSection("material"), oldRoot, newRoot);
                    }
                }
                ...
                if (testRoot != "") {
                    ...
                    if (!testVisual) {
                        ...
                        if (!locateFile(
                              ...
                              what,
                              LocaliseUTF8(
                                L"MODELEDITOR/MODELS/MUTANT_VALIDATION/"
                                L"UNABLE_LOAD_OTHERWISE")))
                            return false;
                    ...
                    ERROR_MSG(
                      "Cannot determine the visual type of the model\"%s\".\n"
                      "Unable to %s model.",
                      name.c_str(),
                      what.c_str());
                    return false;
        ...
        if (newRoot != oldRoot) {
            ...
            for (unsigned i = 0; i < renderSets.size(); i++) {
                ...
                for (unsigned j = 0; j < geometries.size(); j++) {
                    ...
                    BW::string            newVerts =
                      geometries[j]->readString("vertices", "");
                    pos = newVerts.find(oldRoot);
                    if (pos != BW::string::npos) {
                        newVerts =
                          newVerts.replace(pos, oldRoot.length(), newRoot);
                        geometries[j]->writeString("vertices", newVerts);
                        visualChange = true;
                    }

                    BW::string newStream =
                      geometries[j]->readString("stream", "");
                    pos = newStream.find(oldRoot);
                    if (pos != BW::string::npos) {
                        newStream =
                          newStream.replace(pos, oldRoot.length(), newRoot);
                        geometries[j]->writeString("stream", newStream);
                        visualChange = true;
                    }

                    BW::string newPrims =
                      geometries[j]->readString("primitive", "");
                    pos = newPrims.find(oldRoot);
                    if (pos != BW::string::npos) {
                        newPrims =
                          newPrims.replace(pos, oldRoot.length(), newRoot);
                    ...
                    for (unsigned k = 0; k < primGroups.size(); k++) {
                        ...
                        for (unsigned l = 0; l < materials.size(); l++) {
                            visualChange =
                              fixTextures(materials[l], oldRoot, newRoot) ||
                              visualChange;
                        }
                    }
                }
        ...
        if (oldParentRoot != "") {
            DataSectionPtr testParentModel =
              BWResource::openSection(oldParentRoot + ".model", false);
            ...
            if (!ensureModelValid(parentRoot + ".model",
                                  what,
                                  NULL,
                                  NULL,
                                  NULL,
    ...
    if (readOnly)
        *readOnly |=
          testReadOnly(name, testRoot + ".visual", definedPrimitivesName);

    return true;
}

/**
 *	This method checks to see whether the model being load is using a
 *depreciated vertex format.
 *
 *	@param visual A DataSectionPtr to the visual of this model.
 *	@param primitivesName The name of the primitives file.
 *
 *	@return Returns true if the vertex format used by the model is depreciated,
 *false otherwise.
 */
bool Mutant::isFormatDepreciated(DataSectionPtr    visual,
