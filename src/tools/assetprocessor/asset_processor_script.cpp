...
{
    BW::string baseName = resourceID.substr(0, resourceID.find_last_of('.'));
...
namespace AssetProcessorScript {
    ...
    void init()
    {
        ...
        // Create moo, we don't care if render context init fails because we
        ...
        DataSectionPtr ptr =
          BWResource::instance().openSection("shaders/formats");
        if (ptr) {
     ...
     *
     *  @param materialIDs  vector of materialIds.  Each primitive group will
     * have a corresponding materialID.  The index of the material in the list
     * is used as the material kind, and will later be mapped to materials in
     * the visual.
     *
                               ...
                               BW::vector<BW::string>& materialIDs)
    {
        ...
        if ((verts->sourceFormat() != "xyznuv" &&
             verts->sourceFormat() != "xyznuvtb"))
            return 0;

        const Moo::Vertices::VertexPositions& vertices =
          verts->vertexPositions();
        ...
        if (pt == D3DPT_TRIANGLELIST) {
            for (uint32 i = 0; i < prims->nPrimGroups(); i++) {
                ...
                nIndices =
                  max(uint32(pg.startIndex_ + (pg.nPrimitives_ * 3)), nIndices);
            }
        } else if (pt == D3DPT_TRIANGLESTRIP) {
            for (uint32 i = 0; i < prims->nPrimGroups(); i++) {
                const Moo::PrimitiveGroup& pg = prims->primitiveGroup(i);
                nIndices =
                  max(uint32(pg.startIndex_ + (pg.nPrimitives_) + 2), nIndices);
            }
        }
        ...
        if (pt == D3DPT_TRIANGLELIST) {
            for (uint32 i = 0; i < geometry.primitiveGroups_.size(); i++) {
                // flags stored in a generated BSP is the index of the material
                // ID, until we remap them to material kind / collision flags
                // later. we search backwards because the most recently added
                // materialIDs will correspond to the primitive groups in this
                // geometry.
        ...
        while (rsit != rsend) {
            ...
            if (nodes.size()) {
                ...
                while (it != end) {
                    ...
                    if (!node) {
                        ret =
                          "Couldn't find node " + (*it) + " in " + resourceID;
                        return ret;
                    }
            ...
            while (geit != geend) {
                ...
                geometry.vertices_ =
                  Moo::VerticesManager::instance()->get(verticesName);
                ...
                geometry.primitives_ =
                  Moo::PrimitiveManager::instance()->get(indicesName);
                ...
                while (prit != prend) {
                    ...
                    materialIDs.push_back(
                      getMaterialIdentifier(primitiveGroupSection));

                    Moo::ComplexEffectMaterial* pMat =
                      new Moo::ComplexEffectMaterial();
                    primitiveGroup.material_ = pMat;
                    // No need to load the material at this point.
                    // all we need is the identifier
                    // pMat->load( primitiveGroupSection->openSection(
                    // "material" ) );
                ...
                nDegenerateTris += populateWorldTriangles(
                  geometry,
                  tris,
                  renderSet.firstNodeStaticWorldTransform_,
                  materialIDs);
        ...
        if (bp) {
            ...
            if (p) {
                ...
                DataSectionPtr materialIDsSection =
                  BWResource::openSection("temp_bsp_materials.xml", true);
...
static BW::string updateBSPVersion(const BW::string& visualName)
{
    ...
    // First check whether this visual already has bsp2 data.  If so, we are
    // done.
    ...
    // We will check the number of triangles is the same as in the visual.  If
    // it's not, then it's may be a custom BSP.  However since meshes may have
    // degenerate triangles, the bsp triangle count and the visual's triangle
    // count may be different also.  So we need to construct a BSP anyway, and
    // check that the number of post-degenerate triangles matches the number of
    // tris in the visual.
    Moo::BSPProxyPtr pGeneratedBSP   = NULL;
    uint32           nVisualTris     = 0;
    uint32           nDegenerateTris = 0;
    ret                              = AssetProcessorScript::generateBSP2(
      visualName, pGeneratedBSP, bspMaterialIDs, nVisualTris, nDegenerateTris);
    ...
    if (pGeneratedBSP) {
        // Note about triangle count check.  If the generated BSP size is the
        // same as the existing BSP size, then that's a pretty good check that
        // we are not using a custom BSP.  For further certainty, we also check
        // that the number of triangles in the existing BSP plus the number of
        // degenerate triangles equals the number of triangles in the visual.
        //
        // And if there was no existing BSP, then just create one.
        int  nGeneratedBSPTriangles = pGeneratedBSP->pTree()->numTriangles();
        bool triCountEqual =
          (nGeneratedBSPTriangles == nFoundBSPTriangles &&
        ...
        if (triCountEqual || (nFoundBSPTriangles == 0)) {
            ...
            AssetProcessorScript::replaceBSPData(
              visualName, pGeneratedBSP, bspMaterialIDs);
            ret = "";
        } else {
            ret = visualName;
        ...
        // generating the BSP from the visual will return NULL if the visual
        // only contains skinned / mesh particle vertices, or only contained
        // degenerate triangles.  This is ok.
        if (originalFileHadBSP) {
            // Can't create a new BSP from the vertices but the old file had a
            // BSP.
            ret = visualName;

            if (nFoundBSPTriangles > 0) {
                // This suggests that the object is skinned but somebody created
                // a custom BSP.
                ret +=
                  " has a custom BSP.  Please re-export this visual manually.";
                return ret;
            } else {
                // Can't create a new BSP from the vertices but the old file had
                // a BSP, which was 0 length.  This is a weird case, and means
                // it's most likely all degenerate triangles.
            ...
            // uses a vertex format that does not produced BSPs (object was
            // skinned)
            return "";
        }
    }
 ...
 */
static bool upgradeHardskinnedVertices(const BW::string& resourceID,
                                       BW::string&       ret)
{
    // open the resource
    DataSectionPtr root =
    ...
    while (rsit != rsend) {
        ...
        while (geit != geend) {
            ...
            if (geometry.vertices_->sourceFormat() == "xyznuvi") {
                ret = verticesName + " has hard-skinned, non bumped vertices.  "
                                     "Changed to xyznuviiiww format.";
                geometry.vertices_->resaveHardskinnedVertices();
                changed = true;
            } else if (geometry.vertices_->sourceFormat() == "xyznuvitb") {
                ret = verticesName + " has hard-skinned, bumped vertices.  "
                                     "Changed to xyznuviiiwwtb format.";
                geometry.vertices_->resaveHardskinnedVertices();
                changed = true;
            }
        }
 ...
 *  This function generates a UniqueID of the form
 ...
 *
 *  This method finds the identifier of a chunk given a position in world
 * coordinates. This method was copied from chunk_format.hpp.
 *
 *  @param position     The position in world space coordinates to find the
 * chunk identifier of.
 *
 *  @return The outside chunk identifier.
 */
static BW::string outsideChunkIdentifier(
  const Vector3& position,
  float          chunkSize = DEFAULT_GRID_RESOLUTION)
{
    ...
    if (uint16(gridxs + 4096) >= 8192 || uint16(gridzs + 4096) >= 8192) {
        bw_snprintf(chunkIdentifierCStr,
                    sizeof(chunkIdentifierCStr),
                    "%01xxxx%01xxxx/sep/",
                    int(gridxs >> 12),
                    int(gridzs >> 12));
...
PY_AUTO_MODULE_FUNCTION(RETDATA,
                        ...
                        ARG(Vector3,
                            OPTARG(float, DEFAULT_GRID_RESOLUTION, END)),
                        _AssetProcessor)

static PyObject* compileShader(const BW::string& resourceID,
                               PyObjectPtr       macroCombination,
                               bool              force)
{
    using namespace Moo;
    ...
    if (!PyTuple_Check(macroCombination.get())) {
        PyErr_SetString(PyExc_TypeError,
                        "compileShader arg must be a tuple of integers.");
        Py_RETURN_NONE;
    }
    ...
    if (comboLen != macroSettings.size()) {
        PyErr_SetString(PyExc_TypeError,
                        "compileShader macroCombination tuple is the wrong "
                        "length. Must be len(getShaderMacros).");
        Py_RETURN_NONE;
    }
    ...
    for (size_t i = 0; i < comboLen; i++) {
        ...
        long                                      settingIdx =
          PyInt_AsLong(PyTuple_GetItem(macroCombination.get(), i));
...
PY_AUTO_MODULE_FUNCTION(RETOWN,
                        compileShader,
                        ARG(BW::string, ARG(PyObjectPtr, ARG(bool, END))),
                        _AssetProcessor)
...
static PyObject* getShaderMacros()
{
    ...
    for (size_t i = 0; i < macroSettings.size(); i++) {
        ...
        PyObject*                                 tuple   = Py_BuildValue(
          "(si)", setting->macroName().c_str(), setting->numMacroOptions());
...
static PyObject* shaderNeedsRecompile(const BW::string& resourceID,
                                      PyObjectPtr       macroCombination)
{
    using namespace Moo;
    ...
    if (!PyTuple_Check(macroCombination.get())) {
        PyErr_SetString(
          PyExc_TypeError,
          "shaderNeedsRecompile arg must be a tuple of integers.");
        Py_RETURN_NONE;
    }
    ...
    if (comboLen != macroSettings.size()) {
        PyErr_SetString(PyExc_TypeError,
                        "shaderNeedsRecompile macroCombination tuple is the "
                        "wrong length. Must be len(getShaderMacros).");
        Py_RETURN_NONE;
    }
...
PY_AUTO_MODULE_FUNCTION(RETOWN,
                        shaderNeedsRecompile,
                        ARG(BW::string, ARG(PyObjectPtr, END)),
                        _AssetProcessor)

static bool convertVisual(const BW::string& visualFile,
                          const BW::string& colladaFile)
{
    PyErr_SetString(PyExc_NotImplementedError,
                    " Collada is no longer a supported format.");
    return false;
}

PY_AUTO_MODULE_FUNCTION(RETDATA,
                        convertVisual,
                        ARG(BW::string, OPTARG(BW::string, BW::string(), END)),
                        _AssetProcessor)
...
#ifdef _WIN32
        bw_snwprintf(
          buf, sizeof(buf) / sizeof(wchar_t), L"%.256S", val.c_str());
#else
...
bool convertBase64DataSection(PyObjectPtr pyObj)
{
    if (PyDataSection::Check(pyObj.get())) {
        DataSectionPtr pSection =
          static_cast<PyDataSection*>(pyObj.get())->pSection();
