...
/**
 *	Displays a message box with standard message box flags and returns the
 *result. Uses a dummy parent window to hide the task bar button (given the
 *launcher is a windowless app, it puts in a default taskbar icon which looks
 *ugly).
 */
int displayMessage(const tstring& msg, int mbFlags)
{
    ...
    HWND wnd = ::CreateWindow(wcex.lpszClassName,
                              _T("BWLauncher"),
                              WS_ICONIC | WS_DISABLED,
                              -1000,
                              -1000,
                              1,
                              1,
    ...
    int result =
      ::MessageBox(wnd, msg.c_str(), _T("BigWorld Launcher"), mbFlags);
 ...
 */
size_t splitCommandArgs(const tstring& str,
                        StringVector&  out,
                        const tstring& delim,
                        const tstring& bind,
                        wchar_t        escape)
...
/**
 *	Helper function to determine the index of the given argument in the vecotr
 *of strings. Returns -1 if not found.
 */
...
/**
 *	Helper function to determine if the given string is in the given vector of
 *strings. It does case insensitive comparisons.
 */
...
tstring getAppFullPath()
{
    ...
    while (GetModuleFileName(NULL, &path[0], (DWORD)path.size()) ==
           path.size()) {
...
/*
 *	Helper function that wraps SHGetFolderPath. Returns "" if the path can't be
 *found.
 */
...
/*
 *	Gets the full path and filename of the launcher executable's correct
 *installed location.
 */
...
/*
 *	Restarts the process with the appropriate installation switch, using
 *elevated administrator priviliges.
 */
...
/*
 *	Helper function that extracts a value out of the registry and compares it
 *against a reference value. Returns true if the value in the registry matches
 *the reference.
 */
...
/*
 *	Helper function that writs the given key/value pair into the Window
 *registry. Raises UnprivilegedException if we couldn't write due to a
 *permissions problem.
 */
 ...
 */
bool getRegistryValue(HKEY     baseKey,
                      LPCTSTR  key,
                      LPCTSTR  name,
                      tstring& outValue)
{
    ...
    if (lResult == ERROR_SUCCESS) {
        do {
            ...
            lResult = RegEnumKeyEx(
              hKey, 0, szName, &dwSize, NULL, NULL, NULL, &ftWrite);
...
/**
 *	Split the given input string into a list of strings, similar to Python
 *split().
 */
