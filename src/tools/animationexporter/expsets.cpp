...
bool ExportSettings::displayDialog(HWND hWnd)
{
    if (!DialogBoxParam(hInstance,
                        MAKEINTRESOURCE(IDD_MFEXPORT_DLG),
                        hWnd,
                        dialogProc,
                        (LPARAM)this)) {
        return false;
...
#ifdef _UNICODE
    SetDlgItemText(hWnd,
                   IDC_REFERENCEHIERARCHYFILE,
                   bw_utf8tow(this->referenceNodesFile_).c_str());
    SetDlgItemText(
      hWnd, IDC_REFERENCEHIERARCHY_FILENAME, bw_utf8tow(displayName).c_str());
#else
    SetDlgItemText(
      hWnd, IDC_REFERENCEHIERARCHYFILE, this->referenceNodesFile_.c_str());
                                            ...
                                            LPARAM lParam)
{
    ...
    switch (msg) {
        case WM_INITDIALOG:
            ...
            CheckDlgButton(
              hWnd, IDC_EXPORTNODEANIMATION, settings->exportNodeAnimation_);

            CheckDlgButton(hWnd,
                           IDC_REFERENCEHIERARCHY,
            ...
            CheckDlgButton(
              hWnd, IDC_LEGACY_ORIENTATION, settings->useLegacyOrientation_);
            break;
        case WM_COMMAND:
            switch (LOWORD(wParam)) {
                case IDC_REFERENCEHIERARCHY:
                    if (IsDlgButtonChecked(hWnd, IDC_REFERENCEHIERARCHY) !=
                        TRUE) {
                        settings->referenceNodesFile_ = "";
                    } else {
                        settings->referenceNodesFile_ =
                          settings->getReferenceFile(hWnd);
                    }
                ...
                case IDOK:
                    ...
                    settings->exportNodeAnimation_ =
                      IsDlgButtonChecked(hWnd, IDC_EXPORTNODEANIMATION) == TRUE;

                    // store whether we should export cue tracks or not
                    settings->exportCueTrack_ =
                      IsDlgButtonChecked(hWnd, IDC_CUETRACK) == BST_CHECKED;

                    settings->useLegacyOrientation_ =
