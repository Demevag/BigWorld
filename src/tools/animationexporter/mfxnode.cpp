    ...
    if (node && ExportSettings::instance().exportCueTrack()) {
        for (int i = 0; i < node->NumNoteTracks(); ++i) {
            ...
            for (int j = 0; j < note->keys.Count(); ++j) {
                ...
                CueTrack::addCue(
                  note->keys[j]->time,
                  bw_wtoutf8(note->keys[j]->note.data()).c_str());
#else
...
{
    // Is there a max node attached to this mfxnode, if there is, return its
    // world transform
    if (node_) {
        if (normalise)
...
Matrix3 MFXNode::getRelativeTransform(TimeValue t,
                                      bool      normalise,
                                      MFXNode*  idealParent)
{
    ...
    if (node_) {
        ...
        if (parent_ && !idealParent)
            idealParent = parent_;

        // If there is a parent get the relative transform, from the parent,
        // else return this nodes transform.
        if (idealParent) {
...
{
    BW::vector<MFXNode*>::iterator it =
      std::find(children_.begin(), children_.end(), n);
    ;
...
void MFXNode::exportTree(DataSectionPtr pParentSection, MFXNode* idealParent)
{
    ...
    if (this->include_) {
        ...
        if (pThisSection) {
            ...
            Matrix3 m = rearrangeMatrix(getRelativeTransform(
              0, !ExportSettings::instance().allowScale(), idealParent));
            m.SetRow(3, m.GetRow(3) * ExportSettings::instance().unitScale());

            pThisSection->writeVector3("transform/row0",
                                       reinterpret_cast<Vector3&>(m.GetRow(0)));
            pThisSection->writeVector3("transform/row1",
                                       reinterpret_cast<Vector3&>(m.GetRow(1)));
            pThisSection->writeVector3("transform/row2",
                                       reinterpret_cast<Vector3&>(m.GetRow(2)));
            pThisSection->writeVector3("transform/row3",
                                       reinterpret_cast<Vector3&>(m.GetRow(3)));
        }
        parent = this;
    }
...
void MFXNode::exportAnimation(BinaryFile&            animFile,
                              ...
                              MFXNode*               idealParent)
{
    ...
    if (include_) {
        if (trailingLeadingWhitespaces(this->getIdentifier())) {
            ...
            bw_snprintf(s,
                        sizeof(s),
                        "Node \"%s\" has an illegal name. Remove any spaces at "
        ...
        for (int i = firstFrame; i <= lastFrame; i++) {
            Matrix3 theMatrix = this->getRelativeTransform(
              i * GetTicksPerFrame(), normalise, idealParent);
            ...
            /*			debugText << "\tRotation: " << bt.rotation() << " Scale:
               " << bt.scaling() << " Translation: " << bt.translation() <<
               '\n';
