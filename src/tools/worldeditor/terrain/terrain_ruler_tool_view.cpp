  ...
  , verticalMode_(false)
{
    ...
    if (effect_ != NULL) {
        ...
        hWVPParam_ =
          effect_->pEffect()->GetParameterByName(NULL, "worldViewProj");
    }
}
...
void TerrainRulerToolView::render(Moo::DrawContext& drawContext,
                                  const Tool&       tool)
{
    BW_GUARD;
    ...
    Moo::DynamicIndexBufferBase& indBuf =
      rc.dynamicIndexBufferInterface().get(D3DFMT_INDEX16);
    Moo::IndicesReference indLock =
      indBuf.lock(static_cast<uint32>(indices_.size()));
    if (!indLock.size()) {
        indBuf.unlock();
        return;
    }
...
void TerrainRulerToolView::updateVertices()
{
    ...
    while (remainingDistance > 0) {
        if (iLine > 0) {
            ...
            float   lineLength =
              (iLine % 10 == 0) ? innerLine10MLength : innerLine1MLength;
...
void TerrainRulerToolView::addLine(const Vector3& from,
                                   const Vector3& to,
                                   float          width)
{
    Vector3 c0, c1, c2, c3;
...
void TerrainRulerToolView::addQuad(const Vector3& c0,
                                   const Vector3& c1,
                                   const Vector3& c2,
                                   const Vector3& c3)
{
...
bool TerrainRulerToolView::calcQuadCorners(const Vector3& a,
                                           const Vector3& b,
                                           float          width,
                                           Vector3&       c0,
                                           Vector3&       c1,
