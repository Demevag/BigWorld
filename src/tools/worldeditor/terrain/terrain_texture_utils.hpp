...
namespace TerrainTextureUtils {
    ...
    FindError findNeighboursWithTexture(LayerInfo const& layer,
                                        LayerSet&        layers);

    FindError findNeighboursWithTexture(EditorChunkTerrainPtr seed,
    ...
    void setProjections(LayerSet const& layers,
                        Vector4 const&  uProj,
                        Vector4 const&  vProj,
                        bool            temporary);

    bool dominantTexture(Vector3 const&       position,
                         size_t*              idx,
                         EditorChunkTerrain** ect,
                         Chunk**              chunk,
                         uint8*               strength = NULL);

    bool layerInfo(Vector3 const& position, LayerVec& info);

    bool canReplaceTexture(size_t                idx,
    ...
    bool replaceTexture(size_t                idx,
                        EditorChunkTerrainPtr ect,
                        BW::string const&     textureName,
                        BW::string const&     bumpTextureName,
                        Vector4 const&        uProjection,
    ...
    void deleteEmptyLayers(Terrain::EditorBaseTerrainBlock& block,
                           uint8                            threshold);

    int mostPopulousLayer(Terrain::EditorBaseTerrainBlock const& block,
                          int                                    ignoreLayer);

    int mostPopulousLayer(Terrain::EditorBaseTerrainBlock const& block,
                          BW::vector<size_t> const&              ignoreLayers);

    int leastPopulousLayer(Terrain::EditorBaseTerrainBlock const& block,
                           int                                    ignoreLayer);

    int leastPopulousLayer(Terrain::EditorBaseTerrainBlock const& block,
                           BW::vector<size_t> const&              ignoreLayers);

    void layersDistributions(Terrain::EditorBaseTerrainBlock const& block,
                             BW::vector<uint64>& distribution);

    void limitLayers(EditorChunkTerrainPtr ect,
                     size_t                maxLayers,
                     size_t                ignoreLayer = (size_t)-1);

    void mergeLayers(EditorChunkTerrainPtr     ect,
