 ...
 *							second.
 */
bool TerrainTextureUtils::operator<(LayerInfo const layerInfo1,
 ...
 *						unloaded area.
 */
TerrainTextureUtils::FindError TerrainTextureUtils::findNeighboursWithTexture(
  ...
  LayerSet&        layers)
{
    ...
    return findNeighboursWithTexture(
      layer.terrain_,
      ttl.textureName(),
      ttl.bumpTextureName(),
  ...
  LayerSet&             layers)
{
    ...
    size_t layerIdx = seed->block().findLayer(
      texture, bumpTexture, uProj, vProj, PROJECTION_COMPARISON_EPSILON);
    if (layerIdx != (size_t)-1) {
        ...
        if (layers.find(layerInfo) == layers.end()) {
            ...
            for (uint32 i = EditorChunkTerrain::FIRST_NEIGHBOUR;
                 i != EditorChunkTerrain::LAST_NEIGHBOUR;
                 ++i) {
                EditorChunkTerrain::NeighbourError error =
                  EditorChunkTerrain::NEIGHBOUR_OK;
                EditorChunkTerrain* neighbour = seed->neighbour(
                  (EditorChunkTerrain::Neighbour)i, false, true, &error);
                if (neighbour == NULL &&
                    error != EditorChunkTerrain::NEIGHBOUR_OK) {
                    if (error == EditorChunkTerrain::NEIGHBOUR_NOTLOADED)
                        result =
                          (FindError)(result | FIND_EXTENDS_INTO_UNLOADED);
                    if (error == EditorChunkTerrain::NEIGHBOUR_LOCKED)
                        result = (FindError)(result | FIND_EXTENDS_INTO_LOCKED);
                }
                if (neighbour != NULL) {
                    result = (FindError)(result |
                                         findNeighboursWithTexture(neighbour,
                                                                   texture,
                                                                   bumpTexture,
                                                                   uProj,
                                                                   vProj,
...
void TerrainTextureUtils::printErrorMessage(FindError findError)
{
    ...
    if ((findError & TerrainTextureUtils::FIND_EXTENDS_INTO_LOCKED) != 0)
        ERROR_MSG(LocaliseUTF8(
                    L"WORLDEDITOR/GUI/PAGE_TERRAIN_TEXTURE/LAYER_NOT_WRITABLE")
                    .c_str());
    if ((findError & TerrainTextureUtils::FIND_EXTENDS_INTO_UNLOADED) != 0)
        ERROR_MSG(
          LocaliseUTF8(L"WORLDEDITOR/GUI/PAGE_TERRAIN_TEXTURE/LAYER_NOT_LOADED")
            .c_str());
}
 ...
 *							recalcuate
 */
void TerrainTextureUtils::setProjections(LayerSet const& layers,
                                         Vector4 const&  uProj,
                                         Vector4 const&  vProj,
                                         bool            temporary)
{
    ...
    for (LayerSetConstIter it = layers.begin(); it != layers.end(); ++it) {
        ...
        Terrain::TerrainTextureLayer& layer =
          ect->block().textureLayer(layerIdx);
        ...
        if (layer.hasUVProjections()) {
            ...
            if (!temporary)
                WorldManager::instance().chunkTexturesPainted(
                  ect->chunk(), false); // let WorldEditor do the LOD textures
 ...
 */
bool TerrainTextureUtils::dominantTexture(Vector3 const&       position,
                                          size_t*              idx,
                                          ...
                                          uint8*               strength)
{
    ...
    *chunk =
    ...
    if (chunk != NULL) {
        *ect = static_cast<EditorChunkTerrain*>(
 ...
 *	given position.
 */
bool TerrainTextureUtils::layerInfo(Vector3 const& position, LayerVec& infoVec)
{
    BW_GUARD;
 ...
 */
bool TerrainTextureUtils::canReplaceTexture(size_t                idx,
 ...
 */
bool TerrainTextureUtils::replaceTexture(size_t                idx,
                                         EditorChunkTerrainPtr ect,
                                         BW::string const&     textureName,
...
)
{
    ...
    if (canReplaceTexture(
          idx, ect, textureName, bumpTextureName, uProjection, vProjection)) {
        ...
        if (doFloodFill) {
            findError =
              TerrainTextureUtils::findNeighboursWithTexture(ect,
                                                             oldTextureName,
                                                             oldBumpTextureName,
                                                             uProj,
                                                             vProj,
        ...
        for (BW::set<TerrainTextureUtils::LayerInfo>::iterator it =
               neighbours.begin();
             it != neighbours.end();
             ++it) {
            ...
            if (undoable) {
                UndoRedo::instance().add(
                  new TerrainTextureLayerUndo(&thisBlock, thisECT->chunk()));
            }
            ...
            // the blends back:
            {
                ...
                Terrain::TerrainTextureLayer::ImageType oldBlends =
                  thisLayer.image();
                thisLayer.textureName(
                  textureName); // this resets the blends to 0
            ...
            WorldManager::instance().changedTerrainBlock(thisECT->chunk(),
                                                         false);
            WorldManager::instance().chunkTexturesPainted(thisECT->chunk(),
                                                          rebuiltTextureLOD);
            changedOne = true;
        }
        if (changedOne) {
  ...
  uint8                            threshold)
{
    ...
    if (firstNonEmptyLayer >= 0 && numEmptyLayers > 0) {
        // If we are going to remove layers, normalise the sum for each column
        // of pixels by adding the removed values to the first non-zero layer.
        ...
        for (int y = 0; y < height; ++y) {
            for (size_t i = 0; i < numEmptyLayers; ++i) {
                pEmptyLayers[i] =
                  block.textureLayer(emptyIdxs[i]).image().getRow(y);
            }
            pNonEmptyLayer =
              block.textureLayer(firstNonEmptyLayer).image().getRow(y);
    ...
    for (BW::vector<size_t>::reverse_iterator it = emptyIdxs.rbegin();
         it != emptyIdxs.rend();
         ++it) {
 ...
 */
void TerrainTextureUtils::limitLayers(EditorChunkTerrainPtr ect,
                                      size_t                maxLayers,
 ...
 */
void TerrainTextureUtils::mergeLayers(EditorChunkTerrainPtr     ect,
