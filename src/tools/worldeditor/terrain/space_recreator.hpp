...
class SpaceRecreator
{
  public:
    ...
    SpaceRecreator(const BW::string&  spaceName,
                   float              chunkSize,
                   int                width,
                   int                height,
  ...
  private:
    ...
    void fixChunkLinkArray(DataSectionPtr arrayDS,
                           bool           knownToBeChunkLinks = false);

    bool fixChunkLinksInChunk(DataSectionPtr chunkDS);

    Terrain::EditorTerrainBlock2Ptr readTerrainBlock(DataSectionPtr spaceDS,
                                                     int            x,
                                                     int            z);

    bool writeTerrainBlock(DataSectionPtr                  spaceDS,
                           int                             x,
                           int                             z,
