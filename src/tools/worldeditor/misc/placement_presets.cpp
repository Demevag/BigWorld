...
PlacementPresets::PlacementPresets()
  : GUI::ActionMaker<PlacementPresets>("placementPresetChanged",
                                       &PlacementPresets::presetChanged)
  , GUI::UpdaterMaker<PlacementPresets>("placementPresetUpdate",
                                        &PlacementPresets::presetUpdate)
  , GUI::ActionMaker<PlacementPresets, 1>("showPlacementDialog",
                                          &PlacementPresets::showDialog)
  , dialog_(0)
...
void PlacementPresets::init(const BW::string& placementXML)
{
    ...
    currentPreset_ =
      Options::getOptionString(PLACECTRL_OPTIONS_STR, PLACECTRL_NORMAL_STR);
    readPresets();
}
...
void PlacementPresets::readPresets()
{
    ...
    presetsComboNames.push_back(
      dialogCombo); // Internal toolbar presets combo name
    presetsComboNames.push_back(
      "/UalToolbar/PlacementPresets"); // UAL's presets combo name

    for (BW::vector<BW::string>::iterator i = presetsComboNames.begin();
         i != presetsComboNames.end();
         ++i) {
        ...
        if ((*i) != dialogCombo) {
            item =
              new GUI::Item("CHILD",
                            ...
                            section_->readString(
                              "editPresetsString",
                              LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/TOOL/"
                                           L"PLACEMENT_PRESETS/EDIT_PRESET")),
                            "",
                            "",
                            "",
                            "showPlacementDialog",
                            "placementPresetUpdate",
                            "");
            presets->add(item);
        }
        ...
        item =
          new GUI::Item("CHILD",
                        ...
                        section_->readString(
                          "normalPlacementString",
                          LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/TOOL/"
                                       L"PLACEMENT_PRESETS/NORMAL_PLACEMENT")),
                        "",
                        "",
                        "",
                        "placementPresetChanged",
                        "placementPresetUpdate",
                        "");
        presets->add(item);
        ...
        for (int i = 0; i < presetsSection_->countChildren(); ++i) {
            ...
            if (itemData) {
                item = new GUI::Item("CHILD",
                                     itemData->readString("name"),
                                     itemData->readString("displayName"),
                                     "",
                                     "",
                                     "",
                                     "placementPresetChanged",
    ...
    for (BW::vector<CComboBox*>::iterator c = comboBoxes_.begin();
        ...
        idx = (*c)->AddString(
          section_
            ->readWideString("normalPlacementString",
                             Localise(L"WORLDEDITOR/WORLDEDITOR/TOOL/"
                                      L"PLACEMENT_PRESETS/NORMAL_PLACEMENT"))
            .c_str());
        ...
        for (int i = 0; i < presetsSection_->countChildren(); ++i) {
            ...
            if (itemData) {
                idx = (*c)->AddString(
                  itemData->readWideString("displayName", L"").c_str());
                (*c)->SetItemData(idx, i);
            }
        }
    }
...
float PlacementPresets::getCurrentPresetData(DataType type,
                                             Axis     axis,
                                             Value    value)
{
    BW_GUARD;
...
void PlacementPresets::removeComboBox(CComboBox* cbox)
{
    ...
    if (!cbox)
        return;
    BW::vector<CComboBox*>::iterator i =
      std::find(comboBoxes_.begin(), comboBoxes_.end(), cbox);
...
void PlacementPresets::currentPreset(CComboBox* cbox)
{
    ...
    if (idx == PLACECTRL_EDIT_INT) {
        ...
        currentPreset_ = PLACECTRL_NORMAL_STR;
             ...
             idx < static_cast<DWORD_PTR>(presetsSection_->countChildren())) {
...
bool PlacementPresets::currentPresetStock()
{
    ...
    if (PLACECTRL_NORMAL_STR != currentPreset_ && preset &&
        preset->readBool("stockPreset"))
        return true;
    else
        return false;
}
...
bool PlacementPresets::showDialog(GUI::ItemPtr item)
{
    ...
    dlg.DoModal();
    */
    GUITABS::PanelHandle panel = PanelManager::instance().panels().getContent(
      PlacementCtrlsDlg::contentID);
    if (!panel)
        PanelManager::instance().panels().insertPanel(
          PlacementCtrlsDlg::contentID, GUITABS::FLOATING);
