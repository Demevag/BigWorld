...
ON_CBN_SELCHANGE(IDC_HEIGHTMAP_EDITOR_SIZE,
                 &NewSpaceDlg::OnCbnSelchangeHeightmapEditorSize)
...
void NewSpaceDlg::initIntEdit(controls::EditNumeric& edit,
                              int                    minVal,
                              int                    maxVal,
                              int                    val)
{
...
BOOL NewSpaceDlg::OnInitDialog()
{
    ...
    initIntEdit(
      chunkSize_, 10, (int)MAX_SPACE_WIDTH, (int)DEFAULT_GRID_RESOLUTION);
    initIntEdit(width_, 1, NEWSPACEDLG_MAX_CHUNKS, 10);
    initIntEdit(height_, 1, NEWSPACEDLG_MAX_CHUNKS, 10);

    initIntEdit(floraVB_,
                DEFAULT_FLORA_VB_SIZE / 2,
                DEFAULT_FLORA_VB_SIZE * 2,
                DEFAULT_FLORA_VB_SIZE);
    initIntEdit(floraTextureWidth_,
                DEFAULT_FLORA_TEXTURE_WIDTH / 4,
                DEFAULT_FLORA_TEXTURE_WIDTH * 2,
                DEFAULT_FLORA_TEXTURE_WIDTH);
    initIntEdit(floraTextureHeight_,
                DEFAULT_FLORA_TEXTURE_HEIGHT / 4,
                DEFAULT_FLORA_TEXTURE_HEIGHT * 2,
                DEFAULT_FLORA_TEXTURE_HEIGHT);
    ...
    int heightMapSize = Options::getOptionInt(
      "terrain2/defaults/heightMapSize",
      Terrain::TerrainSettings::defaults()->heightMapSize());
    int heightMapEditorSize =
      max(heightMapSize,
    ...
    _itow(Options::getOptionInt(
            "terrain2/defaults/normalMapSize",
            Terrain::TerrainSettings::defaults()->normalMapSize()),
          buffer,
          10);
    normalMapSize_.SelectString(-1, buffer);

    _itow(Options::getOptionInt(
            "terrain2/defaults/holeMapSize",
            Terrain::TerrainSettings::defaults()->holeMapSize()),
          buffer,
          10);
    holeMapSize_.SelectString(-1, buffer);
    _itow(Options::getOptionInt(
            "terrain2/defaults/shadowMapSize",
            Terrain::TerrainSettings::defaults()->shadowMapSize()),
          buffer,
          10);
    ...
    spacePath_.SetWindowText(
      bw_utf8tow(BWResource::resolveFilename(defaultSpacePath_)).c_str());

    defaultTexture_.SetWindowText(
      bw_utf8tow(Options::getOptionString(LAST_DEFAULT_TEXTURE_TAG)).c_str());
...
class SpaceMaker
{
  public:
    ...
    SpaceMaker(const BW::string&  spaceName,
               float              chunkSize,
               int                width,
               int                height,
               int                minHeight,
               int                maxHeight,
               int                floraVBSize,
               int                floraTextureWidth,
               int                floraTextureHeight,
    ...
    bool create()
    {
        ...
        progress_ = new ProgressBarTask(
          LocaliseUTF8(
            L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/CREATE_SPACE/STAGE1"),
          (float)(maxX_ - minX_ + 1) * (maxZ_ - minZ_ + 1),
          true);
  ...
  private:
    ...
    bool createFloraSettings()
    {
        ...
        DataSectionPtr settings =
          BWResource::openSection("system/data/flora.xml");
        ...
        settings->save(BWResource::dissolveFilename(spaceName_) + "/" +
                       SPACE_FLORA_SETTING_FILE_NAME);
    ...
    bool createTemplateTerrainBlock()
    {
        ...
        pTerrainSettings->init(
          chunkSize_, terrainSettingsDS, Terrain::DEFAULT_BLOCK_SIZE);
        ...
        bool       deleteTemplate = false;
        {
            ...
            templateTname =
              BWResource::resolveFilename(s_spaceDlgBlankCDataFname);
            ...
            DataSectionPtr cdata =
              new BinSection(s_spaceDlgBlankCDataFname, NULL);
            cdata = cdata->convertToZip(s_spaceDlgBlankCDataFname);
            if (cdata == NULL)
                return false;

            DataSectionPtr pTerrain =
              cdata->openSection(block->dataSectionName(), true);
        ...
        if (!defaultTexture_.empty()) {
            ...
            Terrain::EditorBaseTerrainBlockPtr block =
              static_cast<Terrain::EditorBaseTerrainBlock*>(
                Terrain::EditorBaseTerrainBlock::loadBlock(terrainName,
                                                           Matrix::identity,
                                                           Vector3(0, 0, 0),
                                                           pTerrainSettings,
                                                           NULL)
            ...
            if (block) {
                ...
                int32 idx = static_cast<int32>(
                  block->insertTextureLayer(blendSize, blendSize));
                if (idx == -1) {
                    ERROR_MSG(
                      "Couldn't create default texture for new space.\n");
                } else {
                    ...
                    layer.textureName(defaultTexture_);
                    {
                        ...
                        Terrain::TerrainTextureLayerHolder holder(&layer,
                                                                  false);
                        layer.image().fill(
                          std::numeric_limits<
                    ...
                    if (cDataDS) {
                        DataSectionPtr blockDS =
                          cDataDS->openSection(block->dataSectionName());
                        if (blockDS) {
                            if (block->save(blockDS)) {
                                cDataDS->save();
                                // set 'templateTname' to the temp file instead
                                // of the blank 's_ndBlankCDataFname' file.
                                templateTname  = fname;
                                deleteTemplate = true;
                            }
                        }
        ...
        HANDLE file           = CreateFile(wtemplateTname.c_str(),
                                 GENERIC_READ,
                                 FILE_SHARE_READ,
                                 NULL,
                                 OPEN_EXISTING,
        ...
        if (file != INVALID_HANDLE_VALUE) {
            ...
            size =
              (size + bytesPerSector - 1) / bytesPerSector * bytesPerSector;
            if (terrainCache_ != NULL) {
                VirtualFree(terrainCache_, 0, MEM_RELEASE);
                terrainCache_ = NULL;
            }
        ...
        memoryCache_ = (char*)VirtualAlloc(
          NULL, memoryCacheSize_ /*big enough*/, MEM_COMMIT, PAGE_READWRITE);
    ...
    void createTerrain(int x, int z, char*& buffer)
    {
        ...
        BW::string cDataName   = name.substr(0, name.size() - 1) + "o.cdata";
        ...
        HANDLE     file   = CreateFile(wfname.c_str(),
        ...
        if (file != INVALID_HANDLE_VALUE) {
            ...
            if (bytesPerSector == 0) {
                ...
                GetDiskFreeSpace(
                  diskRoot, &dummy, &bytesPerSector, &dummy, &dummy);
            }
            DWORD size = (terrainSize_ + bytesPerSector - 1) / bytesPerSector *
                         bytesPerSector;
            WriteFile(file, terrainCache_, size, &dummy, NULL);
            CloseHandle(file);
            file = CreateFile(bw_utf8tow(fname).c_str(),
                              GENERIC_READ | GENERIC_WRITE,
                              0,
                              NULL,
                              OPEN_EXISTING,
            ...
            if (block) {
                ...
                if (block->rebuildLodTexture(m)) {
                    ...
                    if (cdataDS) {
                        ...
                        ERROR_MSG(
                          "Couldn't open terrain block %s .cdata file\n",
                          fname.c_str());
                    }
                } else {
                ...
                ERROR_MSG("Couldn't load terrain block %s to create its "
                          "terrain lod texture\n",
                          fname.c_str());
            }
        }
    ...
    bool createChunk(int x, int z)
    {
        ...
        HANDLE      file  = CreateFile(fname.c_str(),
                                 GENERIC_READ | GENERIC_WRITE,
                                 0,
                                 NULL,
                                 CREATE_ALWAYS,
        ...
        if (file != INVALID_HANDLE_VALUE) {
            ...
            size_t size = (buffer - memoryCache_ + bytesPerSector - 1) /
                          bytesPerSector * bytesPerSector;
            WriteFile(
              file, memoryCache_, static_cast<DWORD>(size), &dummy, NULL);
            CloseHandle(file);
...
void NewSpaceDlg::OnBnClickedOk()
{
    ...
    if (p.IsEmpty() ||
        BWResource::dissolveFilename(bw_wtoutf8(p.GetString())) == "") {
        BW::wstring msg =
          L"The space cannot be located in the root directory."
          L"\n\nWould you like to create it in \"" +
          bw_utf8tow(BWResource::resolveFilename(defaultSpacePath_)) + L"/" +
          s.GetString() + L"\"?";
        if (MessageBox(msg.c_str(),
                       L"No space folder given",
                       MB_ICONINFORMATION | MB_OKCANCEL) == IDCANCEL) {
            spacePath_.SetSel(0, -1); // Select the space path field
            spacePath_.SetFocus();
            return;
        }
        spaceName = bw_utf8tow(BWResource::resolveFilename(defaultSpacePath_)) +
                    L"/" + s.GetString();
    } else if (BWResource::fileExists(bw_wtoutf8(spaceName) + "/" +
                                      SPACE_SETTING_FILE_NAME)) {
        BW::wstring msg =
          BW::wstring(L"The folder \"") + spaceName +
          L"\" already contains a space."
          L"\nPlease use a different space name or select a different folder.";
        MessageBox(
          msg.c_str(), L"Folder already contains a space", MB_ICONERROR);
        spacePath_.SetSel(0, -1); // Select the space path field
        spacePath_.SetFocus();
        return;
    } else if (bw_MW_strcmp(
                 BWResource::dissolveFilename(bw_wtoutf8(spaceName)).c_str(),
                 spaceName.c_str()) == 0) {
        BW::wstring msg = BW::wstring(L"The folder \"") + p.GetString() +
                          L"\" is not in a BigWorld path."
                          L"\nPlease correct the path, or select the desired "
                          L"folder using the '...' button";
        MessageBox(
          msg.c_str(), L"Folder not inside a BigWorld path.", MB_ICONERROR);
        spacePath_.SetSel(0, -1); // Select the space path field
        spacePath_.SetFocus();
        return;
    }
    ...
    SpaceMaker::TerrainInfo terrainInfo;
    {
        ...
        heightMapEditorSize_.GetLBText(heightMapEditorSize_.GetCurSel(),
                                       buffer);
        ...
        Options::setOptionInt("terrain2/defaults/heightMapSize",
                              terrainInfo.heightMapSize_);
        Options::setOptionInt("terrain2/defaults/heightMapEditorSize",
                              terrainInfo.heightMapEditorSize_);
        Options::setOptionInt("terrain2/defaults/normalMapSize",
                              terrainInfo.normalMapSize_);
        Options::setOptionInt("terrain2/defaults/holeMapSize",
                              terrainInfo.holeMapSize_);
        Options::setOptionInt("terrain2/defaults/shadowMapSize",
                              terrainInfo.shadowMapSize_);
        Options::setOptionInt("terrain2/defaults/blendMapSize",
                              terrainInfo.blendMapSize_);
    }
    ...
    if (!result) {
        MessageBox(bw_utf8tow(maker.errorMessage()).c_str(),
                   L"Unable to create space",
                   MB_ICONERROR);
    } else {
        createdSpace_ =
          bw_utf8tow(BWResource::dissolveFilename(bw_wtoutf8(spaceName)))
            .c_str();
        ...
        if (!textureImage_.image().isEmpty()) {
            ...
            BW::string dissolvedFileName =
              BWResource::dissolveFilename(bw_wtoutf8(dt.GetString()));
            Options::setOptionString(LAST_DEFAULT_TEXTURE_TAG,
                                     dissolvedFileName);
        }
        OnOK();
    }
...
bool NewSpaceDlg::validateDefaultTexture()
{
    ...
    if (defaultTexture_.GetWindowTextLength() > 0 &&
        textureImage_.image().isEmpty()) {
        CString dt;
        defaultTexture_.GetWindowText(dt);
        BW::wstring resolvedFName =
          bw_utf8tow(BWResource::resolveFilename(bw_wtoutf8(dt.GetString())));
        BW::wstring msg =
          L"The Default Terrain Texture '" + resolvedFName +
          L"' cannot be used because it's missing or it's not in a valid "
          L"resource path.\n" +
          L"To create the space, please enter a valid texture file name, or "
          L"clear the Default Terrain Texture field.";
        MessageBox(
          msg.c_str(), L"Invalid Default Terrain Texture", MB_ICONERROR);
        return false;
    }
...
void NewSpaceDlg::OnBnClickedNewspaceBrowse()
{
    ...
    if (BWResource::instance().fileSystem()->getFileType(
          bw_wtoutf8(curpath.GetString())) == IFileSystem::FT_DIRECTORY)
        dlg.startDirectory_ =
          BWResource::resolveFilename(bw_wtoutf8(curpath.GetString())).c_str();
    else
...
void NewSpaceDlg::OnBnTextureBrowse()
{
    ...
    if (!dt.IsEmpty()) {
        lastDefTex = bw_utf8tow(BWResource::getFilePath(
          BWResource::resolveFilename(bw_wtoutf8(dt.GetString()))));
    ...
    if (fileDialog.showDialog()) {
        bool       failed = true;
        BW::string dissolvedFileName =
          BWResource::dissolveFilename(bw_wtoutf8(fileDialog.getFileName()));
        if (dissolvedFileName[1] != ':' && dissolvedFileName.size()) {
            defaultTexture_.SetWindowText(
              bw_utf8tow(dissolvedFileName).c_str());
            OnEnTextureChange();
            failed = false;
        }
        ...
        if (textureImage_.image().isEmpty() || failed) {
            BW::string msg = "The file '" + dissolvedFileName +
                             "' cannot be used as a texture because it's "
                             "missing or is not in the resource paths.";
            MessageBox(bw_utf8tow(msg).c_str(),
                       L"Browse for Default Terrain Texture",
                       MB_ICONERROR);
        }
    }
}
