...
void TakePhotoDlg::setSpaceInfo(const BW::string& spaceName,
                                float             minPixelsperMetre,
                                float             maxPixelsperMetre,
                                int               blockSize,
                                int               gridCountX,
                                int               gridCountY)
{
    BW_GUARD;
    if (minPixelsperMetre >= maxPixelsperMetre || minPixelsperMetre <= 0 ||
        maxPixelsperMetre <= 0) {
        minPixelsperMetre_ = MIN_PIXELS_PER_METRE;
        maxPixelsperMetre_ = MAX_PIXELS_PER_METRE;
    } else {
...
ON_WM_HSCROLL()
ON_BN_CLICKED(IDC_RADIO_CREATE_LEVEL_MAP,
              &TakePhotoDlg::OnBnClickedRadioCreateLevelMap)
ON_BN_CLICKED(IDC_RADIO_CREATE_SINGLE_MAP,
              &TakePhotoDlg::OnBnClickedRadioCreateSingleMap)
...
BOOL TakePhotoDlg::OnInitDialog()
{
    ...
    int maxScrollRange =
      (int)(maxPixelsperMetre_ / PIXELS_PER_METRE_SCROLL_STEP);
...
void TakePhotoDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
    ...
    if (pScrollBar->GetDlgCtrlID() == IDC_SLIDER_PIXELS_PER_METRE) {
        ...
        pixelsPerMetre_ =
          minPixelsperMetre_ + (float)pos * PIXELS_PER_METRE_SCROLL_STEP;
...
TakePhotoDlg::PhotoInfo TakePhotoDlg::getPhotoInfo()
{
    ...
    if (createSingleMap_) {
        ...
        photoInfo.pixelsPerMetre =
          photoInfo.spaceMapWidth * 1.0f / (blockSize_ * gridCountY_);
