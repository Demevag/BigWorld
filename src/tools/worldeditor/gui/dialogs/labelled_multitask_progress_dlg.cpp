...
void LabelledMultiTaskProgressDlg::playFinishingAnimation()
{
    if (!cancelled_ && curTask_ < taskCnt_) {
        for (int i = curTask_ + 1; i <= taskCnt_; i++) {
            ...
            BW::string str = "";
            bw_wtoutf8(Localise(L"WORLDEDITOR/WORLDEDITOR/BIGBANG/"
                                L"BIG_BANG_PROGRESS_BAR/FINISHING"),
                       str);
...
void LabelledMultiTaskProgressDlg::recalcProgress()
{
    ...
    if (!freezeEntireProgressMovement_) {
        ...
        entireProgress_.SetPos(ENTIRE_PROGRESS_HIGH_RANGE * entireProgress /
                               entireLength);
    }
...
{
    // ProgressBar in Windows 7 is crappy, have to do this to show a full
...
void LabelledMultiTaskProgressDlg::setCurrentTaskInfo(const BW::string& sMsg,
                                                      float             length,
                                                      bool escapable)
{
    BW_GUARD;
    ...
    if (cancelled_ == true) {
        labelledMessage_.SetWindowTextW(Localise(
          L"WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG_PROGRESS_BAR/CANCELLING"));
    } else {
...
void LabelledMultiTaskProgressDlg::setCurrentTask(int task)
{
    ...
    if (curTask_ != task && task <= taskCnt_) {
        ...
        curProgress_                  = 0;
        freezeEntireProgressMovement_ = false;
        entireProgress_.SetPos(ENTIRE_PROGRESS_HIGH_RANGE * curTask_ /
                               taskCnt_);
    }
}
