...
void ExpandSpaceControl::init()
{
    ...
    maxToExpandX_ =
      MAX_CHUNK_SIZE - (srcMapping->maxGridX() - srcMapping->minGridX() + 1);
    maxToExpandY_ =
      MAX_CHUNK_SIZE - (srcMapping->maxGridY() - srcMapping->minGridY() + 1);
...
ON_BN_CLICKED(IDC_INCLUDE_TERRAIN,
              &ExpandSpaceControl::OnBnClickedCheckEmptyChunk)
END_MESSAGE_MAP()
...
void ExpandSpaceControl::OnBnClickedExpandSpaceBtnOk()
{
    ...
    if (eastCnt + westCnt > maxToExpandX_ ||
        ...
        // todo message box
        MessageBox(
          Localise(L"WORLDEDITOR/GUI/EXPAND_SPACE_DLG/EXCEEDED_MAX_CHUNK_SIZE"),
          NULL,
          MB_ICONERROR);
        return;
    }
...
void ExpandSpaceControl::OnEnChangeEdit()
{
    ...
    if (validationChangeHandler_) {
        bool validates =
          !(westCnt <= 0 && eastCnt <= 0 && northCnt <= 0 && southCnt <= 0);
...
{
    expandInfo_.includeTerrainInNewChunks_ =
      (includeTerrain_.GetCheck() == BST_CHECKED);
}

IMPLEMENT_DYNAMIC(ExpandSpaceDlg, CDialog)

BEGIN_MESSAGE_MAP(ExpandSpaceDlg, CDialog)
ON_BN_CLICKED(IDC_EXPAND_SPACE_BTN_OK,
              &ExpandSpaceDlg::OnBnClickedExpandSpaceBtnOk)
ON_BN_CLICKED(IDC_EXPAND_SPACE_BTN_CANCEL,
              &ExpandSpaceDlg::OnBnClickedExpandSpaceBtnCancel)
END_MESSAGE_MAP()
