        ...
        if (clicked->item() && clicked->item()->chunk()) {
            ...
            if (WorldEditorApp::instance().pythonAdapter()) {
                WorldEditorApp::instance()
                  .pythonAdapter()
                  ->onPageControlTabSelect("pgc", "Object");
            }
            ...
            if (WorldEditorApp::instance().pythonAdapter()) {
                WorldEditorApp::instance()
                  ...
                  ->onPageControlTabSelect("pgc", "Object");
                   ...
                   strcmp(clicked->stackTrace(), "") != 0) {
            ::MessageBox(
              AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
              bw_utf8tow(clicked->stackTrace()).c_str(),
              Localise(L"WORLDEDITOR/GUI/PAGE_MESSAGE_IMPL/SCRIPT_ERROR"),
              MB_OK | MB_TASKMODAL);
        } else if (clicked->priority() == MESSAGE_PRIORITY_ASSET) {
            msgs_->expanded_[clicked->msgStr()] =
              !msgs_->expanded_[clicked->msgStr()];
