...
PageFloraSetting::PageFloraSetting()
  : GUI::ActionMaker<PageFloraSetting>(
      "ActionRefresh|ActionNewEcotypy|"
...
int PageFloraSetting::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
    ...
    secondaryWnd_.Create(_T("STATIC"),
                         NULL,
                         WS_CHILD | WS_VISIBLE,
                         CRect(0, 0, 500, 500),
                         this,
                         IDC_FLORA_SETTING_RIGHT_CHILD_WND);
    return ret;
}
  ...
  bool      saveToDisk /*=true*/)
{
    ...
    bw_snprintf(
      ecotypesPath, sizeof(ecotypesPath), "%s/ecotypes", floraXML_.c_str());
    ...
    if (hHighLightItem != NULL) {
        ...
        floraSettingTree_.findChildItemsBySection(
          hHighLightItem, pVisuaSection, visualItems, false);
        // include itself.
...
bool PageFloraSetting::actionRefresh(GUI::ItemPtr item)
{
    BW_GUARD;
    DataSectionPtr pFloraXMLDataSect =
      this->refreshFloraXMLDataSection(NULL, true);
    // Refresh flora generation
    uint32         version = Terrain::TerrainSettings::TERRAIN2_VERSION;
    DataSectionPtr pSpaceSettings =
      ChunkManager::instance().cameraSpace()->enviro().pData();
    if (pSpaceSettings) {
        version = pSpaceSettings->readInt(
          "terrain/version", Terrain::TerrainSettings::TERRAIN2_VERSION);
    }
...
void PageFloraSetting::deleteSelectedExistingResource()
{
    ...
    if (pSection) {
        ...
        this->fillSelectedItemsIntoAssetsInfo(this->existingResourceList(),
                                              assets);
...
bool PageFloraSetting::actionTurnOn3DView(GUI::ItemPtr item)
{
    ...
    if (changed_) {
        MsgBox mb(L"",
                  Localise(L"WORLDEDITOR/GUI/PAGE_FLORA_SETTING/SAVE_PROMPT"),
...
void PageFloraSetting::layOut()
{
    ...
    // Calculate client area, so it scrolls rather than resize if less than the
    // minimum.
    ...
    Utilities::moveAndSize(splitterBar_,
                           WND_BORDER_SPACE,
 ...
 *	@return			TRUE to stop further handling, FALSE to continue handling
 *it.
 */
