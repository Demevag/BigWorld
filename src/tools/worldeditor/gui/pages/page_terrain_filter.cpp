...
BOOL PageTerrainFilter::OnInitDialog()
{
    ...
    sizeSlider_.setRangeLimit(
      Options::getOptionFloat("terrain/filter/minsizelimit", 1),
      Options::getOptionFloat("terrain/filter/maxsizelimit", 4000));
    sizeSlider_.setRange(
      Options::getOptionFloat("terrain/filter/minsize", 1),
    ...
    for (size_t i = 0; i < MatrixFilter::instance().size(); ++i) {
        if (MatrixFilter::instance().filter(i).included_) {
            int idx = filtersList_.AddString(
              bw_utf8tow(MatrixFilter::instance().filter(i).name_).c_str());
...
afx_msg void PageTerrainFilter::OnSize(UINT nType, int cx, int cy)
{
    ...
    mSizeRange.SetWindowPos(NULL,
                            rect.Width() - MARGIN - buttonRect.Width(),
                            buttonRect.top,
                            0,
                            0,
    ...
    sizeSlider_.SetWindowPos(NULL,
                             0,
                             0,
                             newWidth - buttonRect.Width() - MARGIN -
                               sliderRect.left,
                             sliderRect.Height(),
    ...
    ScreenToClient(&sliderRect);
    filtersList_.SetWindowPos(NULL,
                              0,
                              0,
                              newWidth,
                              cy - sliderRect.top - MARGIN,
                              SWP_NOZORDER | SWP_NOACTIVATE | SWP_NOMOVE);
}

void PageTerrainFilter::OnHScroll(UINT        nSBCode,
                                  UINT        nPos,
...
afx_msg LRESULT PageTerrainFilter::OnUpdateControls(WPARAM wParam,
                                                    LPARAM lParam)
{
    BW_GUARD;
    ...
    if (lastFilterIndex_ != Options::getOptionInt("terrain/filter/index", 0)) {
        ...
        for (int i = 0; i < filtersList_.GetCount(); ++i) {
            if (filtersList_.GetItemData(i) ==
                Options::getOptionInt("terrain/filter/index", 0)) {
        ...
        if (!found && filtersList_.GetCount() > 0) {
            ...
            Options::setOptionInt("terrain/filter/index", (uint)itemData);
        }
    }

    updateSliderEdits();

    Options::setOptionFloat("terrain/filter/minsize",
                            sizeSlider_.getMinRange());
    Options::setOptionFloat("terrain/filter/maxsize",
