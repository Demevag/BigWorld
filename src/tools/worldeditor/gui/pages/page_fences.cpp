...
ON_BN_CLICKED(IDC_BUTTON_FENCE_FILLMODELNAME,
              &PageFences::OnBnClickedSelectionToFenceModelName)
ON_BN_CLICKED(IDC_BUTTON_FENCE_STARTNEW,
              &PageFences::OnBnClickedStartNewSequence)
ON_BN_CLICKED(IDC_CHECKBOX_FENCE_SELECTWHOLE,
              &PageFences::OnCbClickedSelectWholeFences)
ON_BN_CLICKED(IDC_CHECKBOX_FENCE_ALIGNNORMAL,
              &PageFences::OnCbClickedAlignToTerrainNormal)
...
LRESULT PageFences::OnActivateTool(WPARAM wParam, LPARAM lParam)
{
        ...
        if (pFirstItem != NULL && pFirstItem->edIsSelected()) {
            ...
            if (itemTypeId == ChunkModelTypeId) {
                ...
                if (modelName.size() > 0) {
                    ...
                    toolView->continueSequenceFrom(
                      (EditorChunkModel*)pFirstItem);
                }
            }
        }
    }
    ...
    ctlCbSelectWholeFences.SetCheck(EditorChunkModel::s_autoSelectFenceSections
                                      ? BST_CHECKED
                                      : BST_UNCHECKED);
...
void PageFences::OnBnClickedSelectionToFenceModelName()
{
    ...
    if (dlg != NULL) {
        ...
        if (selAssetPaths.size() > 0) {
            ...
            FencesToolView* toolView =
              WorldManager::instance().getFenceToolView();
            if (toolView != NULL) {
...
void PageFences::OnBnClickedStartNewSequence()
{
    ...
    if (toolView != NULL) {
        ...
        if (modelName.size() == 0) {
            ...
            wchar_t szFilters[] = L"Model Files (*.model)|*.model";

            // Create an Open dialog; the default file name extension is
            // ".prefab".
            ...
            BWFileDialog fileDlg(
              true, L"model", L"*.model", flags, szFilters, this);
            ...
            if (fileDlg.showDialog()) {
                ...
                if (pathName.length() != 0) {
                    // BW::wstring modelNameW = bw_utf8tow( pathName.c_str()
                    // ).c_str();
...
void PageFences::OnCbClickedAlignToTerrainNormal()
{
    ...
    FencesToolView* toolView = WorldManager::instance().getFenceToolView();
    if (toolView != NULL)
        toolView->setAlignToTerrainNormal(
          ctlCbAlignToTerrainNormal.GetCheck() == BST_CHECKED);
}
