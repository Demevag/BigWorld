...
BOOL PageTerrainHeight::OnInitDialog()
{
    ...
    sizeSlider_.setRangeLimit(
      Options::getOptionFloat("terrain/height/minsizelimit", 0.8f),
      Options::getOptionFloat("terrain/height/maxsizelimit", 4000));
    sizeSlider_.setRange(
      Options::getOptionFloat("terrain/height/minsize", 1),
    ...
    strengthSlider_.setRangeLimit(
      Options::getOptionFloat("terrain/height/minstrengthlimit", 1),
      Options::getOptionFloat("terrain/height/maxstrengthlimit", 4000));
    strengthSlider_.setRange(
      Options::getOptionFloat("terrain/height/minstrength", 1),
      Options::getOptionFloat("terrain/height/maxstrength", 800));
    strengthSlider_.setValue(
      Options::getOptionFloat("terrain/height/strength", 1));
    ...
    heightSlider_.setRangeLimit(
      Options::getOptionFloat("terrain/height/minheightlimit", 1),
      Options::getOptionFloat("terrain/height/maxheightlimit", 4000));
    heightSlider_.setRange(
      Options::getOptionFloat("terrain/height/minheight", 1),
...
afx_msg void PageTerrainHeight::OnSize(UINT nType, int cx, int cy)
{
    ...
    sizeSlider_.SetWindowPos(NULL,
                             0,
                             0,
                             newWidth - buttonRect.Width() - sliderRect.left,
                             sliderRect.Height(),
    ...
    mStrengthRange.SetWindowPos(NULL,
                                rect.Width() - buttonRect.Width(),
                                buttonRect.top,
                                0,
                                0,
    ...
    strengthSlider_.SetWindowPos(NULL,
                                 0,
                                 0,
                                 newWidth - buttonRect.Width() -
                                   sliderRect.left,
                                 sliderRect.Height(),
                                 SWP_NOZORDER | SWP_NOACTIVATE | SWP_NOMOVE);
    ...
    mHeightRange.SetWindowPos(NULL,
                              rect.Width() - buttonRect.Width(),
                              buttonRect.top,
                              0,
                              0,
    ...
    heightSlider_.SetWindowPos(NULL,
                               0,
                               0,
                               newWidth - buttonRect.Width() - sliderRect.left,
                               sliderRect.Height(),
...
void PageTerrainHeight::OnHScroll(UINT        nSBCode,
                                  UINT        nPos,
                                  CScrollBar* pScrollBar)
{
    ...
    Options::setOptionFloat("terrain/height/strength",
                            strengthSlider_.getValue());
...
afx_msg LRESULT PageTerrainHeight::OnUpdateControls(WPARAM wParam,
                                                    LPARAM lParam)
{
    BW_GUARD;
    ...
    EditorChunkTerrain* pEct = static_cast<EditorChunkTerrain*>(
      ChunkTerrainCache::instance(*ChunkManager::instance().cameraChunk())
        .pTerrain());
    if (!pEct)
        return 0;
    ...
    Options::setOptionFloat("terrain/height/minsize",
                            sizeSlider_.getMinRange());
    Options::setOptionFloat("terrain/height/maxsize",
                            sizeSlider_.getMaxRange());

    Options::setOptionFloat("terrain/height/minstrength",
                            strengthSlider_.getMinRange());
    Options::setOptionFloat("terrain/height/maxstrength",
                            strengthSlider_.getMaxRange());

    Options::setOptionFloat("terrain/height/minheight",
                            heightSlider_.getMinRange());
    Options::setOptionFloat("terrain/height/maxheight",
                            heightSlider_.getMaxRange());

    ++filterControls_;

    falloffFlat_.SetCheck(
      Options::getOptionInt("terrain/height/brushFalloff") ==
      TerrainHeightFilterFunctor::FLAT_FALLOFF);
    falloffLinear_.SetCheck(
      Options::getOptionInt("terrain/height/brushFalloff") ==
      TerrainHeightFilterFunctor::LINEAR_FALLOFF);
    falloffCurve_.SetCheck(
      Options::getOptionInt("terrain/height/brushFalloff") ==
      TerrainHeightFilterFunctor::CURVE_FALLOFF);
    falloffAverage_.SetCheck(
      Options::getOptionInt("terrain/height/brushFalloff") ==
      TerrainHeightFilterFunctor::AVERAGE_FALLOFF);
    ...
    if (PanelManager::instance().isCurrentTool(PageTerrainHeight::contentID)) {
        if (InputDevices::isAltDown() && ToolManager::instance().tool() &&
...
void PageTerrainHeight::OnBnClickedTerrainFalloffLinear()
{
    ...
    Options::setOptionInt("terrain/height/brushFalloff",
                          TerrainHeightFilterFunctor::LINEAR_FALLOFF);
}
...
afx_msg LRESULT PageTerrainHeight::OnChangeEditNumeric(WPARAM wParam,
                                                       LPARAM lParam)
{
    BW_GUARD;
    ...
    if (filterControls_ == 0) {
        ++filterControls_;
        heightSlider_.setValue(
          static_cast<float>(heightEdit_.GetIntegerValue()));
        Options::setOptionFloat("terrain/height/height",
                                heightSlider_.getValue());
        --filterControls_;
    }
 ...
 *			background.
 */
afx_msg HBRUSH PageTerrainHeight::OnCtlColor(CDC*  pDC,
                                             CWnd* pWnd,
                                             UINT  nCtlColor)
{
    BW_GUARD;
    ...
    heightEdit_.SetBoundsColour(pDC,
                                pWnd,
...
void PageTerrainHeight::OnEnChangeTerrainStrengthEdit()
{
    ...
    if (filterControls_ == 0) {
        ++filterControls_;
        strengthSlider_.setValue(
          static_cast<float>(strengthEdit_.GetIntegerValue()));
        Options::setOptionFloat("terrain/height/strength",
                                strengthSlider_.getValue());
        --filterControls_;
    }
