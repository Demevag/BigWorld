...
namespace {
     ...
     */
    void callPython(PyObject*        pModule,
                    ...
                    PyObject*        pArgs)
    {
        ...
        if (pModule) {
            PyObject* pFunction =
              PyObject_GetAttrString(pModule, funcName.c_str());

            if (pFunction != NULL) {
                Script::call(
                  pFunction,
                  pArgs,
                  ("PostProcessingChains::" + funcName + ": ").c_str());
                argsNeedsDecref = false;
            } else {
                ERROR_MSG(
                  "PostProcessingChains::%s - script function not found.\n",
                  funcName.c_str());
                PyErr_Print();
            }
        }
     ...
     */
    PyObject* askPython(PyObject*        pModule,
                        const BW::string funcName,
                        PyObject*        pArgs)
    {
        BW_GUARD;
...
void PostProcessingChains::update()
{
    ...
    BOOL enableSave =
      (WorldManager::instance().userEditingPostProcessing() ? TRUE : FALSE);
    GetDlgItem(IDC_PP_SAVE_CHAIN)->EnableWindow(enableSave);

    BOOL enableDiscard =
      ((WorldManager::instance().userEditingPostProcessing() ||
        enableListItemOps)
         ? TRUE
         : FALSE);
...
BOOL PostProcessingChains::OnInitDialog()
{
    ...
    chainsFolder_ = Options::getOptionString("post_processing/chainsFolder",
                                             "system/post_processing/chains");
    ...
    callPython(
      pScriptObject_, "init", Py_BuildValue("(s)", chainsFolder_.c_str()));
...
void PostProcessingChains::OnSize(UINT nType, int cx, int cy)
{
    ...
    if (list_.GetSafeHwnd()) {
        ...
        list_.SetWindowPos(NULL,
                           0,
                           0,
                           cx - btnRect.Width() - BUTTON_XMARGIN - LIST_XMARGIN,
                           cy - LIST_YMARGIN - listRect.top,
                           SWP_NOZORDER | SWP_NOMOVE);

        GetDlgItem(IDC_PP_NEW_CHAIN)
          ->SetWindowPos(NULL, btnX, btnY, 0, 0, SWP_NOZORDER | SWP_NOSIZE);
        btnY += btnYStep;
        GetDlgItem(IDC_PP_RENAME_CHAIN)
          ->SetWindowPos(NULL, btnX, btnY, 0, 0, SWP_NOZORDER | SWP_NOSIZE);
        btnY += btnYStep;
        GetDlgItem(IDC_PP_DUPLICATE_CHAIN)
          ->SetWindowPos(NULL, btnX, btnY, 0, 0, SWP_NOZORDER | SWP_NOSIZE);
        btnY += btnYStep;
        GetDlgItem(IDC_PP_DELETE_CHAIN)
          ->SetWindowPos(NULL, btnX, btnY, 0, 0, SWP_NOZORDER | SWP_NOSIZE);
        btnY += btnYStep;
        GetDlgItem(IDC_PP_SAVE_CHAIN)
          ->SetWindowPos(NULL, btnX, btnY, 0, 0, SWP_NOZORDER | SWP_NOSIZE);
        btnY += btnYStep;
        GetDlgItem(IDC_PP_DISCARD_CHANGES)
          ->SetWindowPos(NULL, btnX, btnY, 0, 0, SWP_NOZORDER | SWP_NOSIZE);
 ...
 */
void PostProcessingChains::OnChangeSelectedChain(NMHDR*   pNotifyStruct,
                                                 LRESULT* result)
{
    BW_GUARD;
...
void PostProcessingChains::OnListBeginEditName(NMHDR* pNMHDR, LRESULT* pResult)
{
    ...
    *pResult = (list_.GetSelectedItem() != defaultItem_) ? 0 /*allow edit*/
                                                         : 1 /*cancel edit*/;
}
...
void PostProcessingChains::OnListEndEditName(NMHDR* pNMHDR, LRESULT* pResult)
{
    ...
    if (pTVDispInfo->item.pszText) {
        ...
        callPython(
          pScriptObject_, "renameChain", Py_BuildValue("(s)", newName.c_str()));

        // Refresh the list in case a new chain was created, but keep the
        // selection.
        listNeedsUpdate_ = true;
    }
...
void PostProcessingChains::getPythonChain(BW::string& ret)
{
    ...
    PyObject* pChainName =
      askPython(pScriptObject_, "getCurrentChain", PyTuple_New(0));
    if (pChainName && PyString_Check(pChainName)) {
        Script::setData(pChainName, ret);
        ret =
          BWResource::removeExtension(BWResource::getFilename(ret)).to_string();
    }
...
void PostProcessingChains::changeSelectedChain()
{
    ...
    callPython(
      pScriptObject_, "setChain", Py_BuildValue("(s)", chainPath.c_str()));
    ...
    if (chainName == pyChainName) {
        ...
        if (WorldManager::instance().changedPostProcessing()) {
            // The chain was set. Reset this flag because it should be set when
            // the chain is changed outside the post-processing panel, not now.
            WorldManager::instance().changedPostProcessing(false);

            // We still need to tell the panel that it needs to update the
            // graph.
            if (handler_) {
        ...
        // change failed (probably there were unsaved changes that the user
        // didn't want to lose).
        select(pyChainName);
    }
}
    ...
    if (pChainsDir) {
        for (int i = 0; i < pChainsDir->countChildren(); ++i) {
            ...
            if (BWResource::getExtension(chainFile) == "ppchain") {
                BW::string chainName =
                  BWResource::removeExtension(chainFile).to_string();

                HTREEITEM hItem =
                  list_.InsertItem(bw_utf8tow(chainName).c_str());
                listMap_.insert(std::make_pair(chainName, hItem));
            }
        }
    } else {
        ERROR_MSG(
          "PostProcessingChains - Could not open the chains directory %s\n",
          chainFolder.c_str());
    }

    list_.SortChildren(TVI_ROOT);

    defaultItem_ = list_.InsertItem(
      Localise(L"WORLDEDITOR/GUI/PAGE_POST_PROCESSING/DEFAULT_CHAIN_NAME"),
      TVI_ROOT,
      TVI_FIRST); // Default Chain
