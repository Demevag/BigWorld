...
namespace {
    ...
    const char* OPTION_SHOW_NUM_LAYER_WARNING =
      "chunkTexture/numLayerWarningShow";
}
...
PageChunkTextureView::PageChunkTextureViewPtr PageChunkTextureView::view_ =
  NULL;
 ...
 */
/*static*/ void PageChunkTexture::chunk(const BW::string& chunk,
                                        bool              show /*=false*/)
{
    BW_GUARD;
    ...
    if (!panel) {
        if (show) {
            panel = static_cast<PageChunkTexture*>(
              PanelManager::instance().panels().insertPanel(
                PageChunkTexture::contentID, GUITABS::FLOATING));
            if (!panel)
                return;
        } else {
            return;
...
void PageChunkTexture::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control(
      pDX, IDC_PAGE_TERRAIN_TEX_MANAGE_SEPARATOR, textureManagementSeparator_);
    DDX_Control(
      pDX, IDC_PAGE_TERRAIN_TEXTURES_CHUNK_SEPARATOR, texturesChunkSeparator_);
}
...
void PageChunkTexture::InitPage()
{
    ...
    latch_.SetCheck(Options::getOptionInt(OPTION_LATCH) ? BST_CHECKED
                                                        : BST_UNCHECKED);

    chunkID_.EnableWindow(FALSE);

    chunkTrackButtonImg_ =
    ...
    warningShow_.SetCheck(
      Options::getOptionInt(OPTION_SHOW_NUM_LAYER_WARNING, 1) ? BST_CHECKED
                                                              : BST_UNCHECKED);
    warningShow_.SetIcon(chunkShowButtonImg_);
    warningShow_.SetCheckedIcon(chunkShowChkButtonImg_);

    texturesControl_.Create(
      WS_CHILD | WS_VISIBLE, textureControlExtents(), this);

    for (const char** ext = Moo::TextureManager::validTextureExtensions();
         *ext != NULL;
         ++ext) {
...
void PageChunkTexture::setChunk(const BW::string& chunk)
{
    ...
    Chunk* pChunk = ChunkManager::instance().findChunkByName(
      chunk, WorldManager::instance().geometryMapping());
    if (!pChunk) {
    ...
    if (!pEct) {
        ERROR_MSG(
          "PageChunkTexture::chunk: No EditorChunkTerrain for chunk %s.\n",
          chunk.c_str());
        chunkID_.SetWindowText(L"");
        PageChunkTextureView::chunk(NULL);
        setTextureControl(NULL);
 ...
 */
CRect PageChunkTexture::textureControlExtents(int cx /*=0*/,
                                              int cy /*=0*/) const
{
    BW_GUARD;
    ...
    CRect sepExtents =
      controls::childExtents(*this, IDC_PAGE_TERRAIN_TEXTURES_CHUNK_SEPARATOR);
    return CRect(clientRect.left + CONTROL_OFFSET,
...
void PageChunkTexture::editLayerInfo(int idx)
{
    ...
    layerInfo.bumpTextureName_ =
      terrain->block().textureLayer(idx).bumpTextureName();
...
void PageChunkTexture::deleteBumpMap(int idx)
{
    ...
    UndoRedo::instance().barrier(
      "delete bump map " + block->textureLayer(idx).bumpTextureName(), true);
...
void PageChunkTexture::deleteLayers(BW::vector<size_t> const& selection)
{
    ...
    UndoRedo::instance().add(new TerrainTextureLayerUndo(block, pChunk));
    UndoRedo::instance().barrier(
      LocaliseUTF8(L"WORLDEDITOR/GUI/PAGE_CHUNK_TEXTURE/LAYER_DELETE"), true);
...
void PageChunkTexture::mergeLayers(BW::vector<size_t> const& selection)
{
    ...
    UndoRedo::instance().add(new TerrainTextureLayerUndo(block, pChunk));
    UndoRedo::instance().barrier(
      LocaliseUTF8(L"WORLDEDITOR/GUI/PAGE_CHUNK_TEXTURE/LAYERS_MERGED"), true);
...
LRESULT PageChunkTexture::OnUpdateControls(WPARAM wParam, LPARAM lParam)
{
    ...
    if (enabled_ && chunkTrack_.GetCheck() == BST_CHECKED &&
        ToolManager::instance().tool() &&
        ToolManager::instance().tool()->currentChunk()) {
        BW::string chunk =
          ToolManager::instance().tool()->currentChunk()->identifier();
        if (chunk != chunkName()) {
            setChunk(chunk);
        }
    }
...
LRESULT PageChunkTexture::OnTerrainTextureSelected(WPARAM wParam, LPARAM lParam)
{
    ...
    if (PageTerrainTexture::instance() != NULL) {
        const TerrainPaintBrushPtr original =
          PageTerrainTexture::instance()->getTerrainBrush();
 ...
 */
LRESULT PageChunkTexture::OnTerrainTextureRightClick(WPARAM wParam,
                                                     LPARAM lParam)
{
    BW_GUARD;
    ...
    menu.addItem(
      Localise(L"WORLDEDITOR/GUI/PAGE_CHUNK_TEXTURE/MENU_USE_TEXTURE"),
      selectCmd);

    Terrain::EditorBaseTerrainBlock* block = texturesControl_.terrainBlock();
    if (block && block->textureLayer((int)lParam).hasUVProjections()) {
        menu.addItem(Localise(L"WORLDEDITOR/GUI/PAGE_CHUNK_TEXTURE/"
                              L"MENU_EDIT_PROJECTION_AND_SCALE"),
                     editCmd);
    }

    if (block && block->numberTextureLayers() != 0) {
        menu.addItem(
          Localise(L"WORLDEDITOR/GUI/PAGE_CHUNK_TEXTURE/MENU_SET_TEXTURE_MASK"),
          maskCmd);
    }

    bool addedSeparator = false;

    if (block && selection.size() > 0 &&
        selection.size() != block->numberTextureLayers()) {
        if (!addedSeparator) {
            addedSeparator = true;
            menu.addSeparator();
        }
        ...
        if (selection.size() == 1)
            menu.addItem(
              Localise(
                L"WORLDEDITOR/GUI/PAGE_CHUNK_TEXTURE/MENU_REMOVE_TEXTURE"),
              deleteCmd);
        else
            menu.addItem(
              Localise(
                L"WORLDEDITOR/GUI/PAGE_CHUNK_TEXTURE/MENU_REMOVE_TEXTURES"),
              deleteCmd);
    }
    ...
    if (block && selection.size() > 1) {
        ...
        menu.addItem(
          Localise(L"WORLDEDITOR/GUI/PAGE_CHUNK_TEXTURE/MENU_MERGE_TEXTURES"),
          mergeCmd);
    }

    menu.addItem(
      Localise(L"WORLDEDITOR/GUI/PAGE_CHUNK_TEXTURE/MENU_ADD_BUMP_TEXTURE"),
      addBumpMapCmd);
    menu.addItem(
      Localise(L"WORLDEDITOR/GUI/PAGE_CHUNK_TEXTURE/MENU_DELETE_BUMP_TEXTURE"),
      deleteBumpMapCmd);
...
void PageChunkTexture::OnSize(UINT type, int cx, int cy)
{
    ...
    controls::childResize(textureManagementSeparator_,
                          SEP_OFFSET,
                          controls::NO_RESIZE,
                          cx - SEP_OFFSET,
                          controls::NO_RESIZE);

    controls::childResize(texturesChunkSeparator_,
                          SEP_OFFSET,
                          controls::NO_RESIZE,
                          cx - SEP_OFFSET,
                          controls::NO_RESIZE);
...
void PageChunkTexture::OnBnClickedLatch()
{
    ...
    Options::setOptionInt(OPTION_LATCH,
...
void PageChunkTexture::OnBnClickedShow()
{
    ...
    Chunk*     pChunk = NULL;
    if (!chunk.empty()) {
        pChunk = ChunkManager::instance().findChunkByName(
          chunk, WorldManager::instance().geometryMapping());
    }
...
void PageChunkTexture::OnBnClickedWarning()
{
    ...
    Options::setOptionInt(OPTION_SHOW_NUM_LAYER_WARNING,
...
/*virtual*/ BOOL PageChunkTexture::PreTranslateMessage(MSG* msg)
{
    ...
    if (msg->message == WM_KEYDOWN && focusWnd != NULL &&
        focusWnd->GetSafeHwnd() == texturesControl_.GetSafeHwnd()) {
        UINT repCnt =
          msg->lParam &
          0x0f; // From http://msdn2.microsoft.com/en-us/library/ms646280.aspx
...
bool PageChunkTexture::onDropTexture(UalItemInfo* ii)
{
    ...
    return TerrainTextureUtils::replaceTexture(
      (size_t)layer,
      terrain,
...
RectInt PageChunkTexture::onDropTextureTest(UalItemInfo* ii)
{
    ...
    if (ii != NULL) {
        ...
        if (texturesControl_.layerRectAtPoint(pt, layerRect)) {
            result = RectInt(
              layerRect.left, layerRect.bottom, layerRect.right, layerRect.top);
        }
    }
