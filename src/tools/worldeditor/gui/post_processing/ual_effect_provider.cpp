    ...
    class Effects : public BW::vector<Effect>
    {
      public:
        ...
        void reset()
        {
            ...
            if (pPP) {
                PyObject* pEffectsAttr =
                  PyObject_GetAttrString(pPP, "getEffectNames");
                ...
                if (pEffects && PySequence_Check(pEffects)) {
                    for (int i = 0; i < PySequence_Size(pEffects); ++i) {
                        PyObjectPtr pEffectInfo(PySequence_GetItem(pEffects, i),
                                                PyObjectPtr::STEAL_REFERENCE);
                        if (pEffectInfo) {
                            if (PySequence_Check(pEffectInfo.get())) {
                                PyObjectPtr pEffectName(
                                  PySequence_GetItem(pEffectInfo.get(), 0),
                                  PyObjectPtr::STEAL_REFERENCE);
                                PyObjectPtr pDescription(
                                  PySequence_GetItem(pEffectInfo.get(), 1),
                                  PyObjectPtr::STEAL_REFERENCE);
                                this->push_back(Effect(
                                  bw_utf8tow(
                                    PyString_AsString(pEffectName.get())),
                                  bw_utf8tow(
                                    PyString_AsString(pDescription.get()))));
                                ok = true;
                            } else {
                                this->push_back(Effect(
                                  bw_utf8tow(
                                    PyString_AsString(pEffectInfo.get())),
                                  bw_utf8tow(
                                    PyString_AsString(pEffectInfo.get()))));
                                ok = true;
                            }
            ...
            if (!ok) {
                INFO_MSG("UalEffectProvider: Could not find any "
        ...
        AssetInfo assetInfo(int idx)
        {
            ...
            return AssetInfo(L"PostProcessingEffect",
                             (*this)[idx],
                             L"effects:" + (*this)[idx],
                             L"",
...
class EffectThumbProv : public ThumbnailProvider
{
  public:
    ...
    bool isValid(const ThumbnailManager& manager, const BW::wstring& file)
    {
        ...
        return std::find(s_effects.begin(), s_effects.end(), file) !=
               s_effects.end();
    }
 ...
 *	@param img		Returns the thumbnail for the item, if available.
 */
void EffectVFolderProvider::getThumbnail(ThumbnailManager&  manager,
 ...
 */
const BW::wstring EffectVFolderProvider::getDescriptiveText(
  VFolderItemDataPtr data,
  int                numItems,
  bool               finished)
{
    ...
    if (data->isVFolder()) {
        ...
        return Localise(
          L"WORLDEDITOR/GUI/PAGE_POST_PROCESSING/EFFECTS_PROVIDER_INFO",
          s_effects.size());
    } else {
 ...
 */
void EffectListProvider::getThumbnail(ThumbnailManager& manager,
                                      int               index,
                                      CImage&           img,
                                      int               w,
                                      int               h,
 ...
 */
VFolderPtr UalEffectVFolderLoader::load(UalDialog*     dlg,
