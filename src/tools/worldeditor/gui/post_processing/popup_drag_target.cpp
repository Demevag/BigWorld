 ...
 */
bool PopupDragTarget::open(const CPoint&   pt,
...
BW::string PopupDragTarget::update(int alpha)
{
    ...
    pDC->RoundRect(CRect(0,
                         arrowUp_ ? ARROW_SIZE : 0,
                         rect.Width(),
                         arrowUp_ ? rect.Height() : rect.Height() - ARROW_SIZE),
                   BK_ROUNDNESS);

    // Draw the tip, pointing either up or down.
    CPoint arrowPts[3];
    arrowPts[0] = CPoint(rect.Width() / 2 - ARROW_SIZE,
                         arrowUp_ ? ARROW_SIZE : rect.Height() - ARROW_SIZE);
    arrowPts[1] = CPoint(rect.Width() / 2, arrowUp_ ? 0 : rect.Height());
    arrowPts[2] = CPoint(rect.Width() / 2 + ARROW_SIZE,
                         arrowUp_ ? ARROW_SIZE : rect.Height() - ARROW_SIZE);
    ...
    CRect curItem(HORZ_MARGIN,
                  VERT_MARGIN,
                  HORZ_MARGIN + ITEM_WIDTH,
                  VERT_MARGIN + ITEM_HEIGHT);
    if (arrowUp_) {
    ...
    for (ItemList::const_iterator it = items_.begin(); it != items_.end();
         ++it) {
        ...
        pDC->DrawText(bw_utf8tow(*it).c_str(),
                      (int)len,
                      curItem,
