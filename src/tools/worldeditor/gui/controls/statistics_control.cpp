...
StatisticsControl::StatisticsControl(CWnd*             p_parent,
                                     LPCTSTR           caption,
                                     int               minV,
                                     int               maxV,
                                     int               x,
                                     int               y,
                                     GetStatisticsFunc func,
                                     bool              reverce)
  : mp_parent(p_parent)
  , m_minValue(minV)
  , m_maxValue(maxV)
...
void StatisticsControl::Init()
{
    ...
    m_captionStatic.Create(m_caption,
                           WS_CHILD | WS_VISIBLE,
                           CRect(m_x, m_y, m_x + 120, m_y + TEXT_HEIGHT),
                           mp_parent,
                           s_ctrlID++);
    ...
    m_curValueStatic.Create(
      buf,
      WS_CHILD | WS_VISIBLE,
      CRect(m_x + 122, m_y, m_x + rect.right - rect.left, m_y + TEXT_HEIGHT),
      mp_parent,
      s_ctrlID++);
    m_curValueStatic.SetFont(pFont);

    _stprintf_s(buf, 20, _T("%d"), m_minValue);
    m_minValueStatic.Create(
      buf,
      WS_CHILD | WS_VISIBLE,
      CRect(m_x, m_y + 25, m_x + 30, m_y + 25 + TEXT_HEIGHT),
      mp_parent,
      s_ctrlID++);
...
void StatisticsControl::Update()
{
    ...
    COLORREF clr = ((value > 1.0f || value < 0.0f)
                      ? RGB(255, 0, 0)
                      : RGB(min(220, (int)(220.0f * value)), 220, 0));
