...
LimitSliderDlg::LimitSliderDlg(CWnd* pParent /*=NULL*/)
  : CDialog(LimitSliderDlg::IDD, pParent)
  , mChanging(false)
  , mLimitSlider(0)
{
}
...
CRect LimitSliderDlg::getBestRect(CSize size, CPoint p)
{
    ...
    CPoint pos =
      controls::validateDlgPos(size, p, controls::LEFT, controls::TOP);

    return CRect(pos, pos + size);
}

void LimitSliderDlg::internalShow(float        minLimit,
                                  float        maxLimit,
                                  float        min,
                                  float        max,
                                  unsigned int digits,
...
void LimitSliderDlg::show(float        minLimit,
                          float        maxLimit,
                          float        min,
                          float        max,
                          unsigned int digits,
                          LimitSlider* limitSlider)
{
    ...
    limitSliderDlg->internalShow(
      minLimit, maxLimit, min, max, digits, limitSlider);
}
...
afx_msg HBRUSH LimitSliderDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
    ...
    if (mLimitSlider) {
        mMin.SetBoundsColour(pDC,
                             pWnd,
                             mLimitSlider->getMinRangeLimit(),
                             mLimitSlider->getMaxRangeLimit());
        mMax.SetBoundsColour(pDC,
                             pWnd,
                             mLimitSlider->getMinRangeLimit(),
                             mLimitSlider->getMaxRangeLimit());
    }
...
void LimitSlider::setRangeLimit(float min, float max)
{
    ...
    if (mMin < min || mMax < max) {
        setRange(mMin < min ? min : mMin, mMax > max ? max : mMax);
    }
}
