...
namespace {
    const float LOD_EPSILON =
      0.1f; // minimum precision required for a texture lod change
    ...
    TerrainLodUndo::TerrainLodUndo(
      TerrainTextureLodControl* terrainTextureLodDlg)
      : UndoRedo::Operation((size_t)(typeid(TerrainLodUndo).name()))
      , lodTextureStart_(
          terrainTextureLodDlg->terrainSettings()->lodTextureStart())
      , lodTextureDistance_(
          terrainTextureLodDlg->terrainSettings()->lodTextureDistance())
      , blendPreloadDistance_(
          terrainTextureLodDlg->terrainSettings()->blendPreloadDistance())
    ...
    /*virtual*/ bool TerrainLodUndo::iseq(
...
/*afx_msg */ void TerrainTextureLodControl::OnHScroll(UINT        sbcode,
                                                      UINT        pos,
 ...
 *			background.
 */
/*afx_msg*/ HBRUSH TerrainTextureLodControl::OnCtlColor(CDC*  pDC,
                                                        CWnd* pWnd,
                                                        UINT  nCtlColor)
{
    BW_GUARD;
    ...
    HBRUSH brush =
      controls::EmbeddableCDialog::OnCtlColor(pDC, pWnd, nCtlColor);

    texLodStartEdit_.SetBoundsColour(pDC,
                                     pWnd,
                                     texLodStartSlider_.getMinRangeLimit(),
                                     texLodStartSlider_.getMaxRangeLimit());
    texLodDistEdit_.SetBoundsColour(pDC,
                                    pWnd,
                                    texLodDistSlider_.getMinRangeLimit(),
                                    texLodDistSlider_.getMaxRangeLimit());
    texLodPreloadEdit_.SetBoundsColour(pDC,
                                       pWnd,
                                       texLodPreloadSlider_.getMinRangeLimit(),
                                       texLodPreloadSlider_.getMaxRangeLimit());
...
{
    if (IsWindow(*this) == false || disableInitialize_) {
        return;
    }
...
void TerrainTextureLodControl::OnTexLodEdit()
{
    ...
    if (filterChange_ == 0) {
        ...
        if (!almostEqual(texLodStart, oldTexLodStart, LOD_EPSILON) ||
            ...
            terrainSettings()->blendPreloadDistance(
              texLodPreloadSlider_.getValue());
