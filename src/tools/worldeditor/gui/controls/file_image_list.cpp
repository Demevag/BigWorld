...
void FileImageList::initialise()
{
    ...
    imageList_.Create(
      cx, cy, ILC_COLOR24, initialImageCount, maximumImageCount);
...
int FileImageList::addImage(const CString& fullFilenameOriginal)
{
    ...
    if (BWResource::fileExists(
          bw_wtoutf8((fullFilename + THUMB_NAIL_SUFFIX).GetString()))) {
        HANDLE img      = CreateFile(fullFilename,
                                GENERIC_READ,
                                FILE_SHARE_READ,
                                NULL,
                                OPEN_EXISTING,
                                0,
                                NULL);
        HANDLE imgThumb = CreateFile(fullFilename + THUMB_NAIL_SUFFIX,
                                     GENERIC_READ,
                                     FILE_SHARE_READ,
                                     NULL,
                                     OPEN_EXISTING,
    ...
    if (imageIndex == -1) {
        ...
        pTexture = Moo::rc().createTextureFromFileEx(fullFilename,
                                                     64,
                                                     64,
                                                     1,
                                                     0,
        ...
        if (pTexture) {
            ...
            HRESULT hr =
              textureLock.lockRect(0, &lockedRect, NULL, D3DLOCK_READONLY);
            if (SUCCEEDED(hr)) {
            ...
            if (saveThumbnail)
                D3DXSaveTextureToFile(fullFilename + THUMB_NAIL_SUFFIX,
                                      D3DXIFF_BMP,
                                      pTexture.pComObject(),
                                      NULL);
        }
...
int FileImageList::findImageIndex(const CString& fullFilename)
{
    ...
    for (StringStringMap::iterator i = fileImageRepMap_.begin();
        ...
        if (fullFilename.Right(extLength) == ext) {
            fullImageFilename =
              fullFilename.Left(fullFilename.GetLength() - extLength);
    ...
    for (BW::vector<CString>::iterator it = fileMask_.begin();
        ...
        while (moreFiles) {
            ...
            if (!finder.IsDirectory()) {
                ...
                // since we want to hide the dds when a bmp or a tga with the
                // same name is available currently we haven't a general
                // override mechanism and it is not economical to add one
                BW::string nfullFilename;
                bw_wtoutf8(fullFilename, nfullFilename);
                if (BWResource::getExtension(nfullFilename)
                      .equals_lower("dds")) {
                    if (BWResource::fileExists(
                ...
                if (fullFilename.Right(static_cast<int>(
                      wcslen(THUMB_NAIL_SUFFIX))) == THUMB_NAIL_SUFFIX)
                    continue;
                ...
                if (!callbackFn_ ||
                    (callbackFn_)(bw_wtoutf8(fullFilename.GetBuffer()))) {
                    if (ignoreExtension)
                        InsertItem(
                          LVIF_TEXT | LVIF_IMAGE,
                          i++,
                          bw_utf8tow(BWResource::removeExtension(
                                       bw_wtoutf8(filename.GetString()))
                                       .to_string())
                            .c_str(),
                          0,
                          0,
                                    ...
                                    const CString& imageExtension)
{
    ...
    fileImageRepMap_.insert(
      std::pair<CString, CString>(fileExtension, imageExtension));
}
