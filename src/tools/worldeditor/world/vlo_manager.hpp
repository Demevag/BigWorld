...
class VLOManager
{
  public:
    ...
    void columnsTouchedByBounds(const BoundingBox&   bb,
                                const Matrix&        matrix,
  ...
  private:
    ...
    void updateReferencesInChunk(const VeryLargeObjectPtr        pVLO,
                                 Chunk*                          pChunk,
                                 BW::set<Chunk*>&                removedChunks,
                                 VeryLargeObject::ChunkItemList& movableItems,
                                 const EditorChunkVLO*           pSelectedItem);

    void updateReferencesInternal(
      const ChunkVLO*                 pVLORef,
      const BW::vector<ChunkItemPtr>* selectedItems);
    void removeVloReferenceFromUnloadedChunk(const BW::string& id,
                                             const BW::string& dir,
