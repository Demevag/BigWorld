...
namespace {
    ...
    bool adjustRadius(const Vector3& posn,
                      float&         radius,
 ...
 */
template <class LT>
class LightColourWrapper : public UndoableDataProxy<ColourProxy>
{
  public:
    ...
    virtual BW::string opName()
    {
        ...
        return LocaliseUTF8(
          L"WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_LIGHT/SET_COLOUR",
...
bool EditorChunkDirectionalLight::edSave(DataSectionPtr pSection)
{
    ...
    pSection->writeVector3("colour", Vector3(vcol.r, vcol.g, vcol.b) * 255.f);
...
bool EditorChunkDirectionalLight::edEdit(class GeneralEditor& editor)
{
    ...
    STATIC_LOCALISE_NAME(
      s_colour, "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_LIGHT/COLOUR");
    STATIC_LOCALISE_NAME(
      s_direction,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_LIGHT/DIRECTION");
    STATIC_LOCALISE_NAME(
      s_multiplier,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_LIGHT/MULTIPLIER");

    editor.addProperty(new ColourProperty(
      s_colour, new LightColourWrapper<EditorChunkDirectionalLight>(this)));
...
const Matrix& EditorChunkDirectionalLight::edTransform()
{
    ...
    if (pChunk_ != NULL) {
        ...
        transform_.translation(pChunk_->transformInverse().applyPoint(
 ...
 */
bool EditorChunkDirectionalLight::edTransform(const Matrix& m, bool transient)
{
    BW_GUARD;
...
template <class LT>
class LightRadiusOperation : public UndoRedo::Operation
{
  public:
    LightRadiusOperation(SmartPointer<LT> pItem,
  ...
  private:
    virtual void undo()
    {
        ...
        UndoRedo::instance().add(
          new LightRadiusOperation<LT>(pItem_,
                                       pItem_->pLight()->innerRadius(),
                                       pItem_->pLight()->outerRadius()));
    ...
    {
        return pItem_ == static_cast<const LightRadiusOperation&>(oth).pItem_;
    }
 ...
 * Registering undo-operation of light's radius in
 * ChunkItemMatrix::commitState().
 */
template <class LT>
class ChunkLightMatrix : public ChunkItemMatrix
{
  public:
    ...
    bool commitState(bool revertToRecord, bool addUndoBarrier)
    {
        ...
        if (!revertToRecord) {
            UndoRedo::instance().add(new LightRadiusOperation<LT>(
              pItem_, originInnerRadius_, originOuterRadius_));
        }
 ...
 */
template <class LT>
class LightRadiusWrapper : public UndoableDataProxy<FloatProxy>
{
  public:
    ...
    virtual BW::string opName()
    {
        ...
        if (isOuter_) {
            return LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/CHUNK/"
                                L"EDITOR_CHUNK_LIGHT/SET_OUTER_RADIUS",
                                pItem_->edDescription().c_str());
        }
        return LocaliseUTF8(
          L"WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_LIGHT/SET_INNER_RADIUS",
          pItem_->edDescription().c_str());
    }
...
bool EditorChunkOmniLight::edEdit(class GeneralEditor& editor)
{
    ...
    STATIC_LOCALISE_NAME(
      s_colour, "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_LIGHT/COLOUR");
    STATIC_LOCALISE_NAME(
      s_position, "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_LIGHT/POSITION");
    STATIC_LOCALISE_NAME(
      s_innerRadius,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_LIGHT/INNER_RADIUS");
    STATIC_LOCALISE_NAME(
      s_outerRadius,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_LIGHT/OUTER_RADIUS");
    STATIC_LOCALISE_NAME(
      s_multiplier,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_LIGHT/MULTIPLIER");
    STATIC_LOCALISE_NAME(
      s_priority, "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_LIGHT/PRIORITY");

    editor.addProperty(new ColourProperty(
      s_colour, new LightColourWrapper<EditorChunkOmniLight>(this)));
    ...
    editor.addProperty(new GenRadiusProperty(
      s_innerRadius,
      new LightRadiusWrapper<EditorChunkOmniLight>(this, false),
      pMP,
      GIZMO_INNER_COLOUR,
      GIZMO_INNER_RADIUS));
    editor.addProperty(new GenRadiusProperty(
      s_outerRadius,
      new LightRadiusWrapper<EditorChunkOmniLight>(this, true),
      pMP,
      GIZMO_OUTER_COLOUR,
      GIZMO_OUTER_RADIUS));
...
bool influencesReadOnlyChunk( Chunk* srcChunk, Vector3 atPosition, LightType
light, BW::set<Chunk*>& searchedChunks = BW::set<Chunk*>(), int currentDepth = 0
)
{
    ...
    for (Chunk::piterator pit = srcChunk->pbegin(); pit != srcChunk->pend();
pit++)
    {
        ...
        if ( !pit->pChunk->boundingBox().intersects( atPosition,
light->worldOuterRadius() ) ) continue;

        if ( influencesReadOnlyChunk( pit->pChunk, atPosition, light,
searchedChunks, currentDepth + 1 ) ) return true;
    }
...
bool EditorChunkOmniLight::edTransform(const Matrix& m, bool transient)
{
    ...
    // Ok, harder question: if we influence any non locked chunks in our old or
    // new positions, we can't move (+ do the same thing for edEdit, and use
    // readonly props in the same case)

    /*
    if (influencesReadOnlyChunk( pOldChunk, pLight_->worldPosition(), pLight_ ))
        return false;
    if (influencesReadOnlyChunk( pNewChunk, pNewChunk->transform().applyPoint(
    pLight_->position() ), pLight_ )) return false;
    */
...
class SLAngleWrapper : public UndoableDataProxy<FloatProxy>
{
  public:
    ...
    virtual BW::string opName()
    {
        ...
        return LocaliseUTF8(
          L"WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_LIGHT/SET_CONE_ANGLE",
          pItem_->edDescription().c_str());
    }
...
bool EditorChunkSpotLight::edEdit(class GeneralEditor& editor)
{
    ...
    STATIC_LOCALISE_NAME(
      s_coneAngle,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_LIGHT/CONE_ANGLE");
    STATIC_LOCALISE_NAME(
      s_multiplier,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_LIGHT/MULTIPLIER");
    STATIC_LOCALISE_NAME(
    ...
    editor.addProperty(new ColourProperty(
      s_colour, new LightColourWrapper<EditorChunkSpotLight>(this)));
    ...
    editor.addProperty(new GenRadiusProperty(
      s_innerRadius,
      new LightRadiusWrapper<EditorChunkSpotLight>(this, false),
      pMP,
      GIZMO_INNER_COLOUR,
      GIZMO_INNER_RADIUS));
    editor.addProperty(new GenRadiusProperty(
      s_outerRadius,
      new LightRadiusWrapper<EditorChunkSpotLight>(this, true),
      pMP,
      GIZMO_OUTER_COLOUR,
      GIZMO_OUTER_RADIUS));

    editor.addProperty(
      new AngleProperty(s_coneAngle, new SLAngleWrapper(this), pMP));
...
class ChunkPulseLightFrameOperation : public UndoRedo::Operation
{
  ...
  private:
    virtual void undo()
    {
        ...
        UndoRedo::instance().add(
          new ChunkPulseLightFrameOperation(pulseLight_));
...
class ChunkPulseLightFrameProxy : public Vector2Proxy
{
  ...
  public:
    ...
    virtual Vector2 get() const { return transient_; }
            ...
            if (addBarrier) {
                UndoRedo::instance().barrier(
                  LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/CHUNK/"
                               L"EDITOR_CHUNK_LIGHT/MODIFY_PULSE_ANIMATION"),
                  false);
            }
  ...
  public:
    ...
    virtual void elect(GeneralProperty* parent)
    {
        ...
        for (EditorChunkPulseLight::Frames::iterator iter = frames.begin();
            ...
            properties_.push_back(new Vector2Property(
              Name(propName),
    ...
    virtual bool addItem()
    {
        ...
        UndoRedo::instance().barrier(
          LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_LIGHT/"
                       L"MODIFY_PULSE_ANIMATION"),
          false);
 ...
 */
class PulseColourWrapper : public UndoableDataProxy<ColourProxy>
{
  public:
...
bool EditorChunkPulseLight::load(DataSectionPtr pSection)
{
    ...
    if (!pSection->openSection("timeScale")) { // load the legacy animations
    ...
    if (pSection) {
        ...
        animation_ = pSection->readString(
          "animation", "system/animation/lightnoise.animation");
...
bool EditorChunkPulseLight::edEdit(class GeneralEditor& editor)
{
    ...
    STATIC_LOCALISE_NAME(
      s_timeScale,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_LIGHT/TIMESCALE");
    STATIC_LOCALISE_NAME(
      s_duration, "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_LIGHT/DURATION");
    STATIC_LOCALISE_NAME(
      s_animation,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_LIGHT/ANIMATION");

    editor.addProperty(
      new ColourProperty(s_colour, new PulseColourWrapper(this)));
    ...
    editor.addProperty(new GenRadiusProperty(
      s_innerRadius,
      new LightRadiusWrapper<EditorChunkPulseLight>(this, false),
      pMP,
      GIZMO_INNER_COLOUR,
      GIZMO_INNER_RADIUS));
    editor.addProperty(new GenRadiusProperty(
      s_outerRadius,
      new LightRadiusWrapper<EditorChunkPulseLight>(this, true),
      pMP,
      GIZMO_OUTER_COLOUR,
      GIZMO_OUTER_RADIUS));
    ...
    editor.addProperty(
      new ArrayProperty(Name("frames"), new ChunkPulseLightFramesProxy(*this)));
...
void EditorChunkPulseLight::onFrameChanged()
{
    ...
    if (colourAnimation_.getTotalTime() != 0.f) {
        colourAnimation_.loop(true,
                              duration_ > 0.f
                                ? duration_ * timeScale_
                                : colourAnimation_.getTotalTime());
    }
...
ChunkItemFactory EditorChunkAmbientLight::factory_(
  "ambientLight",
  1,
  EditorChunkAmbientLight::create);

ChunkItemFactory::Result EditorChunkAmbientLight::create(
  Chunk*         pChunk,
  DataSectionPtr pSection)
{
...

    {
        ...
        if (visitor.item_) {
            // Check to see for loading errors (Two ambient lights in one chunk
            // file)
            if (!visitor.loaded_) {
            ...
            return ChunkItemFactory::Result(
              visitor.item_, "The old ambient light was removed", true);
        }
    }
...
bool EditorChunkAmbientLight::edEdit(class GeneralEditor& editor)
{
    ...
    editor.addProperty(new ColourProperty(
      s_colour, new LightColourWrapper<EditorChunkAmbientLight>(this)));
                ...
                struct Visitor
                {
                    ...
                    bool operator()(const ChunkItemPtr& item)
                    {
                        if (item->isShellModel()) {
                            BoundingBox bb;
                            item->edBounds(bb);
                            m_.translation((bb.maxBounds() + bb.minBounds()) /
                                           2.f);
                            return true;
                        }
                        return false;
 ...
 *	Change our transform, temporarily or permanently
 */
bool EditorChunkAmbientLight::edTransform(const Matrix& m, bool transient)
{
    BW_GUARD;
...
{
    BW_GUARD;
    SpaceEditor::instance().changedChunk(this->pChunk_,
