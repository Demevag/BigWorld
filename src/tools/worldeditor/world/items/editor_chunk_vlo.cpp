...
void EditorChunkVLO::edSelectedBox(BoundingBox& bbRet) const
{
    ...
    if (WorldManager::instance().cursorOverGraphicsWnd() &&
        ...
        ToolManager::instance().tool()->locator() && pObject_) {
 ...
 */
bool EditorChunkVLO::load(DataSectionPtr pSection,
                          Chunk*         pChunk,
...
bool EditorChunkVLO::legacyLoad(DataSectionPtr pSection,
                                Chunk*         pChunk,
                                BW::string&    type)
{
    BW_GUARD;
...
void EditorChunkVLO::edPreDelete()
{
    ...
    if (pObject_) {
        UndoRedo::instance().add(new VLOExistenceOperation(this, chunk()));
...
void EditorChunkSubstance<ChunkVLO>::addAsObstacle()
{
    ...
    if (model) {
        ...
        ChunkVLOObstacle::instance(*pChunk_).addModel(model, world, this);
    }
}
...
class VLOCloneNotifier : CloneNotifier
{
  ...
  public:
    ...
    bool in(UniqueID id) const { return idMap_.find(id) != idMap_.end(); }
    void add(DataSectionPtr sect) { sects_.insert(sect); }
} VLOCloneNotifier;

/**
 * get the DataSection for clone
 */
void EditorChunkVLO::edCloneSection(Chunk*         destChunk,
                                    ...
                                    DataSectionPtr destDS)
{
    ...
    if (type_ == "water") {
        if (!VLOCloneNotifier.in(uid_)) {
            ...
            DataSectionPtr newDS = BWResource::openSection(
              chunk()->mapping()->path() + '/' + uid + ".vlo", false);

            if (!newDS) {
                newDS = BWResource::openSection(
                  chunk()->mapping()->path() + "/_" + uid + ".vlo", true);
            }
 ...
 */
bool EditorChunkVLO::edPreChunkClone(Chunk*         srcChunk,
...
bool EditorChunkVLO::edBelongToChunk()
{
    ...
    return chunkBB.intersects(bb.minBounds()) &&
           chunkBB.intersects(bb.maxBounds());
}
...
bool EditorChunkVLO::edTransform(const Matrix& m, bool transient)
{
    ...
    BoundingBox vloBB;
    edBounds(vloBB);
    return VeryLargeObjectBehaviour::edTransform(*object(),
                                                 pChunk_,
                                                 vloBB,
                                                 this,
                                                 transform_,
...
void EditorChunkVLO::updateTransform(Chunk* pChunk)
{
    ...
    if (pObject_) {
        localPos_ = Vector3(50, 0, 50); // TODO: un-hardcode the chunk size
                                        // stuff
        vloTransform_ = Matrix::identity;
        vloTransform_.translation(-localPos_);
        vloTransform_.postMultiply(pChunk->transformInverse());
...
bool EditorChunkVLO::edShouldDraw()
{
    ...
    if (pOwnSect()->readString("type", "") == "water") {
        // This is a hack for water, Since EditorChunkWater is not really a
        // ChunkItem
        bool showOutside =
          !s_hideAllOutside_ ||          // not hidding outside objects, or...
          (!chunk()->isOutsideChunk() && // hidding outside objects, but we are
                                         // in a shell, and...
           object() &&
           object()->visibleInside()); // the VLO is visible inside shells.

        return OptionsScenery::waterVisible() && showOutside &&
               !this->edHidden();
    }
...
ChunkItemFactory::Result EditorChunkVLO::create(Chunk*         pChunk,
                                                DataSectionPtr pSection)
{
    ...
    bool       isDeleted =
      VLOManager::instance()->isDeleted(pSection->readString("uid"));
    bool itemAlreadyExists =
      VLOManager::instance()->contains(pChunk, pSection->readString("uid"));
