...
class MarkerModels
{
  ...
  private:
    MarkerModels()
    {
        ...
        DataSectionPtr pMarkerSection =
          BWResource::openSection(EntityDef::Constants::markerCategoriesFile());
        if (pMarkerSection) {
            ...
            while (it != end) {
                ...
                if (markerName.length()) {
                    DEBUG_MSG("MarkerModels::MarkerModels - Finding model for "
                              "marker %s\n",
                              pDS->sectionName().c_str());
                    if (BWResource::fileExists(markerName)) {
                        models_[pDS->sectionName()] = markerName;
                    } else {
                        ERROR_MSG("MarkerModels::MarkerModels - Unable to find "
                                  "model %s, using default\n",
                                  markerName.c_str());
...
void EditorChunkMarker::draw(Moo::DrawContext& drawContext)
{
    ...
    const EditorChunkEntity::BindingProperties& bProps =
      edEntity_->getBindingProps();
    for (EditorChunkEntity::BindingProperties::const_iterator bit =
           bProps.begin();
         bit != bProps.end();
         bit++) {
        ...
        BW::string toID = "";
        PyObject*  val =
          PyObject_GetAttrString(action, const_cast<char*>(argName.c_str()));
        if (val) {
        ...
        Vector3              to = markerTo->chunk()->transform().applyPoint(
                       markerTo->edTransform().applyToOrigin()) +
                     Vector3(0.f, 0.15f, 0.f);
...
bool EditorChunkMarker::loadEntityDescription()
{
    ...
    if (pOwnSect()->openSection("type", false)) {
        ...
        DataSectionPtr sect =
          BWResource::openSection(EntityDef::Constants::markerCategoriesFile());
        if (sect) {
            DataSectionPtr categoryLevel =
              sect->openSection(getCategory() + "/level");
            if (categoryLevel) {
                DataSectionPtr markerXMLs = BWResource::openSection(
                  EntityDef::Constants::markerEntitiesPath());

                if (markerXMLs && categoryLevel->countChildren() > 0) {
                    DataSectionPtr pFirst = categoryLevel->openChild(0);
                    DataSectionPtr mEntity =
                      markerXMLs->openSection(pFirst->sectionName() + ".xml");
...
bool EditorChunkMarker::edEdit(class GeneralEditor& editor)
{
    ...
    STATIC_LOCALISE_NAME(
      s_position, "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_MARK/POSITION");
    STATIC_LOCALISE_NAME(
      s_rotation, "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_MARK/ROTATION");
    STATIC_LOCALISE_NAME(
      s_category, "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_MARK/CATEGORY");
    ...
    DataSectionPtr edEntitySect =
      BWResource::openSection(EntityDef::Constants::entitiesEditorPath());
    if (edEntitySect) {
    ...
    // a marker is a representation of an entity, find the first underlying
    // entity if changed, reload its properties (may have changed)
    if (loadEntityDescription())
        findCommonProperties();
    ...
    if (!edEntity_->edEditProperties(editor, pMP)) {
        ERROR_MSG(
          "EditorChunkMarker::edEdit - failed to load entity properties\n");
        return false;
    }
...
void EditorChunkMarker::findCommonProperties()
{
    ...
    if (sect) {
        ...
        for (DataSectionIterator it = category->begin(); it != category->end();
             it++) {
            for (DataSectionIterator itLevel = category->begin();
                 itLevel != category->end();
                 itLevel++) {
    ...
    DataSectionPtr         markerXMLs =
      BWResource::openSection(EntityDef::Constants::markerEntitiesPath());
    for (BW::vector<BW::string>::iterator it = markerEntityTypes.begin();
         it != markerEntityTypes.end();
         it++) {
        ...
        if (std::find(entityTypes.begin(), entityTypes.end(), eType) !=
            entityTypes.end())
            continue;
        ...
        if (mProps) {
            for (DataSectionIterator it = mProps->begin(); it != mProps->end();
                 it++) {
                DataSectionPtr pDS = *it;
    ...
    for (BW::vector<BW::string>::iterator itName = overriddenPropNames.begin();
         itName != overriddenPropNames.end();
         ++itName) {
 ...
 *
 *	This function adjusts the cluster, joins or removes the cluster association
 *between what is currently selected and the what the mouse is current over.
 *
 *	@param selected		A ChunkItemRevealer object which references what is
 *currently selected.
 *	@param mouseOver	A ChunkItemRevealer object which references what the
...
static PyObject* py_adjustMarkerCluster(PyObject* args)
{
    ...
    STATIC_LOCALISE_NAME(
      s_mark, "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_MARK/ED_DESCRIPTION");
    STATIC_LOCALISE_NAME(
      s_markCluster,
      "WORLDEDITOR/WORLDEDITOR/CHUNK/EDITOR_CHUNK_MARK_CLUSTER/ED_DESCRIPTION");
    ...
    if (items2.front()->edDescription() == s_mark) {
        ...
        ChunkItemTreeNode* itemTo =
          static_cast<ChunkItemTreeNode*>(&*(items2.front()));
        ...
        return Py_BuildValue(
          "s", "Markers can not be a parent node. Binging coming soon.");
    ...
    // at this point:
    // highlighted is the parent, selectedList will become or cease to become
    // children

    // if parent not fully loaded, don't do anything (can't then check for
    // circular links)
    if (!highlighted->allChildrenLoaded())
        return Py_BuildValue(
          "s", "The parent has children yet to load, cannot link.");
    ...
    for (BW::vector<ChunkItemTreeNodePtr>::iterator it = selectedList.begin();
        ...
        if (highlighted->getParent() == (*it)) {
            ...
            UndoRedo::instance().add(
              new ChunkItemTreeNodeOperation(highlighted, true));
        ...
        if ((*it)->getParent()) {
            // Add an undo op
            UndoRedo::instance().add(
              new ChunkItemTreeNodeOperation((*it), true));
            (*it)->setParent(NULL);
        }

        if (!allLinkedToSameParent ||
            (allLinkedToSameParent &&
             (!commonParent || (commonParent != highlighted)))) {
            // add the desired link
            (*it)->setParent(highlighted);
            // Add an undo op
            UndoRedo::instance().add(
              new ChunkItemTreeNodeOperation((*it), false));
        }
        // else, leave unlinked
    }
