...
{
  public:
    virtual GeneralProperty* createProperty(BasePropertiesHelper* props,
                                            int                   propIndex,
                                            const Name&           name,
                                            ...
                                            MatrixProxy*          pMP)
    {
        ...
        EntityPropertyTypeParserPtr parser =
          EntityPropertyTypeParser::create(name, dataType);
        if (parser)
                                            ...
                                            MatrixProxy*          pMP)
    {
        ...
        if (pyEnum_ != NULL) {
            ...
            if (PySequence_Check(pyEnum_)) {
                ...
                for (int j = 0; j < (int)nItems; j++) {
                    ...
                    if (PyTuple_Check(item)) {
                        ...
                        if (valueObj && nameObj && parser->checkVal(valueObj) &&
            ...
            if (failed) {
                WorldManager::instance().addError(
                  ...
                  LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/CHUNK/"
                               L"EDITOR_ENTITY_PROXY/BAD_ENUM",
                ...
                return parser->enumProperty(
                  props, propIndex, name, dataType, choices);
            }
        } else {
            ...
            for (BW::vector<DataSectionPtr>::iterator it = sections.begin();
                ...
                choices->newSection((*it)->readString("display"))
                  ->setInt(index);
            }
      ...
      public:
                                       ...
                                       DataSectionPtr widget)
        {
            ...
            if (pyClass) {
                ...
                PyObject*  fnPtr  = PyObject_GetAttrString(
                  pyClass, const_cast<char*>(fnName.c_str()));
                if (fnPtr) {
                    pyEnum = Script::ask(
                      fnPtr, PyTuple_New(0), "EditorChunkEntity::edEdit: ");
                } else {
                    PyErr_Clear();
                }
            }
...
class EntityRadiusWidget : public EntityPropertyParser
{
  public:
                                            ...
                                            MatrixProxy*          pMP)
    {
        ...
        uint32  radiusColour = (((uint8)colour[0]) << 16) |
                              (((uint8)colour[1]) << 8) | (((uint8)colour[2])) |
                              (((uint8)colour[3]) << 24);
