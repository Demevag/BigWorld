                                                ...
                                                bool              dblClick)
{
    ...
    if (!itemName.empty()) {
        ...
        selectedFile =
          BWResource::getFilePath(BWResource::dissolveFilename(itemName)) +
    ...
    if (pFunction != NULL) {
        Script::call(
          pFunction,
          Py_BuildValue("(si)", selectedFile.c_str(), dblClick ? 1 : 0),
          "PythonAdapter::onBrowserObjectItemSelect: ");
    } else {
        PyErr_Clear();
        Script::call(
          PyObject_GetAttrString(pScriptObject_, "brwObjectItemSelect"),
          Py_BuildValue("(ss)", theTabName.c_str(), selectedFile.c_str()),
          "PythonAdapter::onBrowserObjectItemSelect: ");
    }

    // update the selection criteria
    BW::string tabName = "tabObject" + theTabName;
    Script::call(PyObject_GetAttrString(pScriptObject_, "pgcObjectsTabSelect"),
                 Py_BuildValue("(s)", tabName.c_str()),
                 "PythonAdapter::onBrowserObjectItemSelect: ");
}
...
void WEPythonAdapter::onBrowserObjectItemAdd()
{
    ...
    PyObject* pFunction =
      PyObject_GetAttrString(pScriptObject_, "brwObjectItemAdd");
...
void WEPythonAdapter::onPageControlTabSelect(const BW::string& fnPrefix,
                                             const BW::string& theTabName)
{
    ...
    if (pFunction != NULL) {
        ...
        PyErr_Clear();
        Script::call(
          PyObject_GetAttrString(pScriptObject_, "pgcAllToolsTabSelect"),
          Py_BuildValue("(s)", tabName.c_str()),
          "PythonAdapter::onPageControlTabSelect: ");
    }
}
...
void WEPythonAdapter::onBrowserTextureItemSelect(const BW::string& itemName)
{
    ...
    if (pFunction != NULL) {
        Script::call(pFunction,
                     Py_BuildValue("(s)", selectedFile.c_str()),
...
void WEPythonAdapter::setTerrainPaintMode(int mode)
{
    ...
    PyObject* pFunction =
      PyObject_GetAttrString(pScriptObject_, fnName.c_str());
...
void WEPythonAdapter::onLimitSliderAdjust(const BW::string& name,
                                          float             pos,
                                          float             min,
                                          float             max)
{
...
void WEPythonAdapter::limitSliderUpdate(LimitSlider*      control,
                                        const BW::string& controlName)
{
    BW_GUARD;
...
void WEPythonAdapter::selectFilterUpdate(CComboBox* comboList)
{
    ...
    if (pFunction != NULL) {
        pResult = Script::ask(
          pFunction, PyTuple_New(0), "PythonAdapter::selectFilterUpdate: ");
    } else {
        PyErr_Clear();
    }
...
void WEPythonAdapter::fillFilterKeys(CComboBox* comboList)
{
    ...
    if (pFunction != NULL) {
        pResult = Script::ask(
          pFunction, PyTuple_New(0), "PythonAdapter::selectFilterKeys: ");
    } else {
        PyErr_Clear();
    }
...
void WEPythonAdapter::coordFilterUpdate(CComboBox* comboList)
{
    ...
    if (pFunction != NULL) {
        pResult = Script::ask(
          pFunction, PyTuple_New(0), "PythonAdapter::coordFilterUpdate: ");
    } else {
        PyErr_Clear();
    }
...
void WEPythonAdapter::fillCoordFilterKeys(CComboBox* comboList)
{
    ...
    if (pFunction != NULL) {
        pResult = Script::ask(
          pFunction, PyTuple_New(0), "PythonAdapter::coordFilterKeys: ");
    } else {
        PyErr_Clear();
    }
...
void WEPythonAdapter::projectLock(const BW::string& commitMessage)
{
    ...
    Script::call(pFunction,
                 Py_BuildValue("(s)", commitMessage.c_str()),
                 "PageProject::OnBnClickedProjectSelectionLock: ");
}

void WEPythonAdapter::commitChanges(const BW::string& commitMessage,
                                    bool              keepLocks)
{
    BW_GUARD;
    ...
    Script::call(pFunction,
                 Py_BuildValue("(si)", commitMessage.c_str(), keepLocks),
                 "PageProject::OnBnClickedProjectCommitAll: ");
}

void WEPythonAdapter::discardChanges(const BW::string& commitMessage,
                                     bool              keepLocks)
{
    BW_GUARD;
...
void WEPythonAdapter::updateSpace()
{
    ...
    Script::call(
      pFunction, PyTuple_New(0), "PageProject::OnBnClickedProjectUpdate: ");
}
...
void WEPythonAdapter::calculateMap()
{
    ...
    if (pFunction != NULL) {
        Script::call(
          pFunction, PyTuple_New(0), "PythonAdapter::calculateMap: ");
    } else {
        PyErr_Clear();
    }
}
...
void WEPythonAdapter::exportMap()
{
    ...
    if (pFunction != NULL) {
        Script::call(pFunction, PyTuple_New(0), "PythonAdapter::exportMap: ");
    } else {
        PyErr_Clear();
    }
}
    ...
    if (pScriptObject_) {
        ...
        if (pFunction != NULL) {
            PyObject* pResult = Script::ask(
              pFunction, Py_BuildValue("()"), "PythonAdapter::canSavePrefab: ");

            if (pResult != NULL) {
                if (PySizeT_Check(pResult)) {
                    return (int)PySizeT_AsLong(pResult) != 0;
                } else {
                    ERROR_MSG("PythonAdapter::canSavePrefab - canSavePrefab "
...
void WEPythonAdapter::saveSelectionPrefab(BW::string fileName)
{
    ...
    if (pScriptObject_) {
        ...
        if (pFunction != NULL) {
            Script::call(pFunction,
                         Py_BuildValue("(s)", fileName.c_str()),
                         "PythonAdapter::saveSelectionPrefab: ");
        }
    }
}

void WEPythonAdapter::expandSpace(const BW::string& spacePath,
...
PyObjectPtr WEPythonAdapter::addChunkItem(const BW::string& resourceID)
{
    ...
    if (pScriptObject_) {
        ...
        if (pFunc != NULL) {
            ...
            PyObjectPtr ret =
              Script::ask(pFunc, pArgs, "PythonAdapter::addChunkItem: ");
...
void WEPythonAdapter::selectAll()
{
    ...
    if (pScriptObject_) {
        ...
        if (pFunc != NULL) {
            Script::call(pFunc, PyTuple_New(0), "PythonAdapter::selectAll: ");
        }
    }
}

bool WEPythonAdapter::createSpaceLevelImage(const BW::string& path,
                                            int               imgSize,
                                            int               ChunkSizeX,
                                            int               ChunkSizeY,
...
bool WEPythonAdapter::createSpaceSingleMap(const BW::string& path,
                                           int               imgSize,
                                           int               ChunkSizeX,
                                           int               ChunkSizeY,
                                           int               outputWidth,
