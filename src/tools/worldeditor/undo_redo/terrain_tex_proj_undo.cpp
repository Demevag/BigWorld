...
TerrainTexProjUndo::TerrainTexProjUndo(EditorChunkTerrainPtr terrain,
                                       size_t                layerIdx)
...
/*virtual*/ void TerrainTexProjUndo::undo()
{
    ...
    UndoRedo::instance().add(new TerrainTexProjUndo(terrain_, layerIdx_));
    ...
    if (layer.hasUVProjections()) {
        ...
        WorldManager::instance().chunkTexturesPainted(
          terrain_->chunk(), false); // let WorldEditor do the LOD textures
    }
}
...
/*virtual*/ bool TerrainTexProjUndo::iseq(const UndoRedo::Operation& oth) const
{
    ...
    return other->terrain_ == terrain_ && other->layerIdx_ == layerIdx_ &&
           other->uProj_ == uProj_ && other->vProj_ == vProj_;
}
BW_END_NAMESPACE
