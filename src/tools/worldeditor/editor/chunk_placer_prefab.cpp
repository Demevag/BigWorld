 ...
 */
static Vector3 placerPrefabCalculateAverageOrigin(
...
static void calculateSnaps(const ChunkItemRevealer::ChunkItems& items,
...
static void removeRefVLOs(Chunk*                             pChunk,
                          const DataSectionPtr&              ds,
                          BW::map<BW::string, ChunkItemPtr>& vlo)
{
    BW_GUARD;
    ...
    while ((vloSec = ds->findChild("vlo")) != NULL) {
        ...
        VeryLargeObjectPtr pExistingVlo =
          VeryLargeObject::getObject(UniqueID(id));
...
static bool savePrefab(const BW::string&              prefabName,
                       ChunkItemRevealer::ChunkItems& items)
{
    ...
    BW::string binFileName = BWResource::changeExtension(prefabName, ".pdata");
    DataSectionPtr binDS =
      new BinSection(BWResource::getFilename(binFileName),
    ...
    for (ChunkItemRevealer::ChunkItems::iterator i = dupItems.begin();
        ...
        if (item->isShellModel()) {
            ...
            bw_snprintf(
              chunkNumberStr, sizeof(chunkNumberStr), "%d", chunkNumber);
            BW::string     chunkName("chunk" + BW::string(chunkNumberStr) +
                                 ".chunk");
            ...
            if (chunkNumber >= 1000000) {
                // too many
                ERROR_MSG(
                  "savePrefab(): Too many chunks in prefab, max is 1000000\n");
                return false;
            }
    ...
    for (BW::map<BW::string, ChunkItemPtr>::iterator i = refVLOs.begin();
         i != refVLOs.end();
         ++i) {
    ...
    for (ChunkItemRevealer::ChunkItems::iterator i = dupItems.begin();
        ...
        if (!item->isShellModel()) {
            ...
            if (origDS->sectionName() == "vlo") {
                pDS->delChild("uid");
                EditorChunkVLO* vloChunkItem =
                  reinterpret_cast<EditorChunkVLO*>(item.getObject());
                if (vloChunkItem) {
            ...
            if (bp) {
                ...
                bw_snprintf(itemNumberStr,
                            sizeof(itemNumberStr),
                            "item%02d",
                            itemNumber++);
    ...
    binDS->setParent(
      BWResource::openSection(BWResource::getFilePath(binFileName)));
 ...
 */
static BW::string filterString(const BW::string& s,
                               const BW::string& validCharacters)
{
    BW_GUARD;
 ...
 *
 *	@return Returns a message if the save was successful, returns NULL
 *otherwise.
 */
PyObject* ChunkPlacer::py_saveChunkPrefab(PyObject* args)
{
    ...
    /*
    BW::string prefabFilename = filterString( name,
    ...
    BW::string prefabName = "";
    {
        ...
        DataSectionPtr pChunkSection = EditorChunkCache::instance( *pChunk
    ).pChunkSection(); BW::string modelName = pItem->pOwnSect()->readString(
    "resource" ); MF_ASSERT( !modelName.empty() );
        ...
        if (items.size() == 1)
        {
            ...
            prefabName += modelName.substr(postShellIndex, extensionIndex -
    postShellIndex); prefabName += "_";
        }
        else
        {
            prefabName += modelName.substr(postShellIndex, modelName.find( "/",
    postShellIndex ) + 1 - postShellIndex); prefabName += "groups/";
        }
    ...
    */
    ...
    BW::string returnMessage = LocaliseUTF8(
      L"WORLDEDITOR/WORLDEDITOR/CHUNK/CHUNK_PLACER_PREFAB/SAVED_PREFAB_MSG",
      prefabName);
 ...
 */
static void rewriteLinks(DataSectionPtr item,
                         ...
                         IdMapping&     stationGraphIDmap)
{
    ...
    if ((item->sectionName() == "marker") ||
        (item->sectionName() == "marker_cluster")) {
        BW::string myIdString = item->readString("id", "");
        MF_ASSERT(myIdString.length() > 0);
        item->writeString(
          "id", clusterMarkerIDmap.find(UniqueID(myIdString), true).toString());

        if (item->openSection("parentID", false)) {
            BW::string parentIdString = item->readString("parentID", "");
            item->writeString(
              "parentID",
              clusterMarkerIDmap.find(UniqueID(parentIdString), false)
                .toString());
        }
    }
    ...
    if (item->sectionName() == "station") {
        ...
        MF_ASSERT(myIdString.length() > 0);
        item->writeString(
          "id", stationIDmap.find(UniqueID(myIdString), true).toString());

        BW::string graphIdString = item->readString("graph", "");
        MF_ASSERT(graphIdString.length() > 0);
        item->writeString(
          "graph",
          stationGraphIDmap.find(UniqueID(graphIdString), false).toString());

        DataSectionIterator iChildLinks;
        for (iChildLinks = item->begin(); iChildLinks != item->end();
             iChildLinks++) {
            ...
            pLinkDS->writeString(
              "to", stationIDmap.find(UniqueID(toIdString), false).toString());
        }
    }
}
...
static Vector3 findBoundingBoxOffset(const BoundingBox& bb,
...
PyObject* ChunkPlacer::py_loadChunkPrefab(PyObject* args)
{
    ...
    BoundingBox prefabBB =
      BoundingBox(prefabSect->readVector3("boundingBox/min",
                                          Vector3(FLT_MAX, FLT_MAX, FLT_MAX)),
                  prefabSect->readVector3(
                    "boundingBox/max", Vector3(-FLT_MAX, -FLT_MAX, -FLT_MAX)));
    ...
    DataSectionPtr terrainTranslation =
      prefabItemSect->openSection("terrain/prefabTransform/row3");
    if (terrainTranslation) {
        // Using the grid lines nearest to the prefab's centre, to match the
        // "save".
    ...
    if (!(prefabBB == BoundingBox::s_insideOut_)) {
        BoundingBox bb = BoundingBox(prefabBB.minBounds() + locatorPosition,
                                     prefabBB.maxBounds() + locatorPosition);
    ...
    if (!WorldManager::instance().isBoundingBoxInWriteableChunk(
          prefabBB, locatorPosition)) {
        ERROR_MSG(
          "There's non-writable chunk inside the prefab's bounding box.\n");
        Py_RETURN_NONE;
    }
    ...
    for (it = prefabChunkSect->begin(); it != prefabChunkSect->end(); ++it) {
        ...
        Chunk*     pNearbyChunk = ChunkManager::instance()
                                .cameraSpace()
                                ->column(locatorPosition)
                                ->pOutsideChunk();
        ...
        DataSectionPtr pDS =
          utilCreateInsideChunkDataSection(pNearbyChunk, newChunkName);
    ...
    for (i = newChunkSections.begin(); i != newChunkSections.end(); ++i) {
        ...
        for (iChild = pDS->begin(); iChild != pDS->end(); ++iChild) {
            ...
            WorldManager::instance().linkerManager().updateLinkerGuid(
              pChildDS, linkerGuidMapping);

            if (keepLinks)
                rewriteLinks(pChildDS,
                             clusterMarkerIDmap,
                             stationIDmap,
                             stationGraphIDmap);
        }
    ...
    for (i = newItemSections.begin(); i != newItemSections.end(); ++i) {
        ...
        if (keepLinks) {
            rewriteLinks(
              *i, clusterMarkerIDmap, stationIDmap, stationGraphIDmap);
        }
    }
    ...
    if (keepLinks) {
        ...
        for (i = newChunkSections.begin(); i != newChunkSections.end(); ++i) {
            ...
            for (BW::vector<DataSectionPtr>::iterator ei =
                   sectionsToErase.begin();
                 ei != sectionsToErase.end();
                 ei++) {
                pDS->delChild(*ei);
            }
        }
        ...
        for (BW::vector<DataSectionPtr>::iterator ei = sectionsToErase.begin();
             ei != sectionsToErase.end();
             ei++) {
            newItemSections.erase(
              std::find(newItemSections.begin(), newItemSections.end(), *ei));
        }
    }
    ...
    for (i = newChunkSections.begin(), ni = newChunkSectionNames.begin();
        ...
        ChunkSpace::Column* col =
          ChunkManager::instance().cameraSpace()->column(pChunk->centre(),
                                                         false);
        if (!col) {
    ...
    if (success) {
        ...
        for (i = newItemSections.begin(); i != newItemSections.end(); i++) {
            ...
            Matrix itemTransform =
              itemPrefabSection->readMatrix34("prefabTransform");
            ...
            if (!isTerrain) {
                pChunk =
                  ChunkManager::instance()
                    .cameraSpace()
                    ->findChunkFromPointExact(itemTransform.applyToOrigin());
            } else {
                pChunk =
                  EditorChunk::findOutsideChunk(itemTransform.applyToOrigin());
            }
            ...
            DataSectionPtr newSection =
              chunkSection->newSection(itemPrefabSection->sectionName());
            ...
            if (newSection->openSection("position"))
                newSection->writeVector3("position",
                                         localTransform.applyToOrigin());
    ...
    if (!success) {
        UndoRedo::instance().barrier(
          LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/CHUNK/CHUNK_PLACER_PREFAB/"
                       L"LOAD_PREFAB_BARRIER"),
          false);
