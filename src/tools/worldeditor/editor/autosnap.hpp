...
class SnappedChunkSet
{
  ...
  public:
    ...
    ChunkBoundary::Portal* portal(unsigned int index) const
    {
        return snapPortals_.at(index);
    }
    const Matrix& transform(unsigned int index) const
    {
        return portalTransforms_.at(index);
    }
};
...
class SnappedChunkSetSet
{
  ...
  public:
    ...
    const SnappedChunkSet& item(unsigned int index) const
    {
        return chunkSets_.at(index);
    }
};
