...
void gatherUnboundPortals(Portals* portals,
                          Chunk*   chunk,
                          Chunk*   boundChunk = NULL)
{
    BW_GUARD;
 ...
 * in excludeChunks
 */
void gatherBoundPortals(Portals*           portals,
                        Chunk*             chunk,
    ...
    for (ChunkBoundaries::iterator bit = chunk->joints().begin();
        ...
        for (uint i = 0; i < (*bit)->boundPortals_.size(); i++) {
            ...
            if (p.hasChunk() && !p.pChunk->isOutsideChunk())
                ...
                if (std::find(excludeChunks.begin(),
                              excludeChunks.end(),
                              p.pChunk) == excludeChunks.end())
                    portals->push_back(&p);
        }
    }
}
 ...
 */
static bool matchPortal(Portal*  p1,
                        int      startIndex1,
                        Portal*  p2,
                        int      startIndex2,
                        bool     reverse,
                        ...
                        Vector3* n2     = NULL)
{
    ...
    for (unsigned int i = 0; i < pointNum; ++i) {
        ...
        p1->objectSpacePoint((startIndex1 + i + pointNum) % pointNum,
                             points1[0]);
        p1->objectSpacePoint((startIndex1 + i + 1 + pointNum) % pointNum,
                             points1[1]);
        p1->objectSpacePoint((startIndex1 + i + 2 + pointNum) % pointNum,
                             points1[2]);

        if (reverse) {
            p2->objectSpacePoint((startIndex2 - i + 2 * pointNum) % pointNum,
                                 points2[0]);
            p2->objectSpacePoint(
              (startIndex2 - i - 1 + 2 * pointNum) % pointNum, points2[1]);
            p2->objectSpacePoint(
              (startIndex2 - i - 2 + 2 * pointNum) % pointNum, points2[2]);
        } else {
            p2->objectSpacePoint((startIndex2 + i + 2 * pointNum) % pointNum,
                                 points2[0]);
            p2->objectSpacePoint(
...
Matrix getRotatedTransform(Portal* source, Portal* target, Chunk* targetChunk)
{
    ...
    Vector3 normal2 =
      targetChunk->transform().applyVector(-target->plane.normal());
    ...
    if (!almostEqual(fabs(normal1.dotProduct(normal2)), 1)) {
        ...
        float angle =
          acosf(Math::clamp(-1.0f, normal1.dotProduct(normal2), +1.0f));
    ...
    for (unsigned int startIndex2 = 0; startIndex2 < pointNum; ++startIndex2) {
        ...
        if (matchPortal(
              source, 0, target, startIndex2, true, &normal, &n1, &n2)) {
            ...
            if (almostEqual(normal, bestNormal)) {
                ...
                if (n1.dotProduct(n2) > normal1.dotProduct(normal2)) {
                    ...
                    portalOffset = source->objectSpacePoint(0) -
                                   targetChunk->transform().applyPoint(
                                     target->objectSpacePoint(startIndex2));
                }
...
Portal* getOtherSidePortal(Chunk* chunk, Portal* portal)
{
    ...
    if (!portal || !portal->hasChunk() || portal->isInvasive())
        return NULL;
    for (ChunkBoundaries::iterator bit = portal->pChunk->joints().begin();
         bit != portal->pChunk->joints().end();
         ++bit) {
        for (uint i = 0; i < (*bit)->boundPortals_.size(); ++i) {
            ...
            if (otherPortal->hasChunk() && !otherPortal->isInvasive() &&
                ...
                Vector3 p2 =
                  portal->pChunk->transform().applyPoint(otherPortal->lcentre);
...
std::pair<Portal*, Portal*> getSnappedPortals(Chunk*             snapChunk,
                                              BW::vector<Chunk*> snapToChunks)
{
    BW_GUARD;

    for (BW::vector<Chunk*>::iterator iter = snapToChunks.begin();
         iter != snapToChunks.end();
         ++iter) {
        for (ChunkBoundaries::iterator bit = (*iter)->joints().begin();
             bit != (*iter)->joints().end();
             ++bit) {
            for (uint i = 0; i < (*bit)->boundPortals_.size(); ++i) {
                ...
                if (portal->hasChunk() && !portal->isInvasive() &&
                    ...
                    return std::make_pair(getOtherSidePortal(*iter, portal),
                                          portal);
                }
            }
        }
...
bool findAutoSnapTransform(Chunk*             snapChunk,
                           BW::vector<Chunk*> snapToChunks,
                           Portals*           snapPortals,
                           Matrix*            m)
{
    ...
    std::pair<Portal*, Portal*> snappedPortals =
      getSnappedPortals(snapChunk, snapToChunks);
    Portal*                      currentSnappingPortal = snappedPortals.second;
    Vector3                      currentOffset(0.f, 0.f, 0.f);
    SnapHistory::difference_type minIndex =
      (SnapHistory::difference_type)s_snapHistory.size();
    ...
    while (getItemType != GET_ITEM_NUM) {
        ...
        for (Portals::iterator si = snapPortals->begin();
             si != snapPortals->end();
             ++si) {
            // for each snapToChunk
            for (BW::vector<Chunk*>::iterator i = snapToChunks.begin();
                 i != snapToChunks.end();
                 ++i) {
                Chunk* snapToChunk = *i;
                ...
                for (Portals::iterator sti = snapToPortals.begin();
                     sti != snapToPortals.end();
                     ++sti) {
                    // if they match
                    unsigned int targetIndex;
                    if (portalsMatch(
                          *si, *sti, snapChunk, snapToChunk, targetIndex)) {
                        Vector3 portalOffset =
                          snapChunk->transform().applyPoint(
                            (*si)->objectSpacePoint(0)) -
                          snapToChunk->transform().applyPoint(
                            (*sti)->objectSpacePoint(targetIndex));

                        SnapHistory::iterator iter =
                          std::find(s_snapHistory.begin(),
                                    s_snapHistory.end(),
                        ...
                        if (almostZero(m.pitch()) && almostZero(m.yaw()) &&
                            almostZero(m.roll()) &&
                        ...
                        if (getItemType == GET_ITEM_NOT_IN_HISTORY) {
                            if (iter == s_snapHistory.end()) {
                                if (bestSnapToChunk == NULL ||
                                    (!almostEqual(pitchAndRoll,
                                                  minPitchAndRoll) &&
                                     pitchAndRoll < minPitchAndRoll) ||
                                    (almostEqual(pitchAndRoll,
                                                 minPitchAndRoll) &&
                                     (*sti == currentSnappingPortal ||
                                      portalOffset.lengthSquared() <
                                        currentOffset.lengthSquared()))) {
                            ...
                            if (iter != s_snapHistory.end() &&
                                (bestSnapToChunk == NULL ||
                                 std::distance(s_snapHistory.begin(), iter) <
                                   minIndex)) {
                                // set them to the best match
                                minIndex =
                                  std::distance(s_snapHistory.begin(), iter);
...
Matrix findRotateSnapTransform(const SnappedChunkSet& snapChunk,
                               bool                   clockWise,
        ...
        for (unsigned int i = 0; i < snapChunk.portalSize(); ++i) {
            if (snapChunk.portal(i)->pChunk) {
                if (snapChunk.portal(i)->pChunk == referenceChunk) {
                    ...
                    transforms.insert(transforms.begin(),
                                      snapChunk.transform(i));
                } else {
    ...
    }
    {
        for (unsigned int i = 0; i < portals.size(); ++i) {
            ...
            for (unsigned int startIndex2 = clockWise ? 1 : -1;
                ...
                portal->objectSpacePoint((startIndex2 + pointNum) % pointNum,
                                         target);
