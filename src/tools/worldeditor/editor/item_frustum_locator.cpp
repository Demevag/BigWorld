...
{
    Py_Header(ChunkItemFrustumLocatorRevealer, ChunkItemRevealer) public
      : ChunkItemFrustumLocatorRevealer(
          SmartPointer<ChunkItemFrustumLocator> pLoc,
...
ChunkItemFrustumLocator::ChunkItemFrustumLocator(ToolLocatorPtr pSub,
                                                 PyTypeObject*  pType)
  : ToolLocator(pType)
  , subLocator_(pSub)
  , enabled_(true)
...
void ChunkItemFrustumLocator::leaveSelectionMode()
{
    ...
    if (D3DXSaveTextureToFile(screenShotName.c_str(),
                              D3DXIFF_BMP,
...
void ChunkItemFrustumLocator::calculatePosition(const Vector3& worldRay,
                                                Tool&          tool)
{
    BW_GUARD;
    ...
    if (enabled_) {
        ...
        if (currentPosition_.x == startPosition_.x ||
            currentPosition_.y == startPosition_.y)
            return;
        ...
        if (SUCCEEDED(texture->GetSurfaceLevel(0, &rtSurface)) &&
            ...
            !!(inmemTexture =
                 Moo::rc().createTexture(desc.Width,
                                         desc.Height,
                                         0,
                                         0,
                                         desc.Format,
            ...
            SUCCEEDED(Moo::rc().device()->GetRenderTargetData(
              rtSurface.pComObject(), inmemSurface.pComObject())) &&
            ...
            uint32 minx = min(startPosition_.x, currentPosition_.x) *
                          renderTarget()->width() /
                          uint32(Moo::rc().screenWidth());
            uint32 maxx = max(startPosition_.x, currentPosition_.x) *
                          renderTarget()->width() /
                          uint32(Moo::rc().screenWidth());
            uint32 miny = min(startPosition_.y, currentPosition_.y) *
                          renderTarget()->height() /
                          uint32(Moo::rc().screenHeight());
            uint32 maxy = max(startPosition_.y, currentPosition_.y) *
                          renderTarget()->height() /
                          uint32(Moo::rc().screenHeight());

            for (uint32 y = miny; y <= maxy; ++y) {
                DWORD* pixel = (DWORD*)((unsigned char*)lockRect.pBits +
                                        lockRect.Pitch * y) +
                               minx;
                ...
                for (; pixel < end; ++pixel) {
                    if (*pixel != 0 && *pixel != lastItemPixel) {
                        lastItemPixel = *pixel;
                        ChunkItem* item =
                          WorldManager::instance().getDrawSelectionItem(*pixel);
                        if (item) {
                            // The pixel corresponds to an actual selectable
                            // chunk item.
                            if (fastItems.find(item) == fastItems.end() &&
                                SelectionFilter::canSelect(item)) {
                                if (item->edIsVLO() &&
                                    !item->edCheckMark(
                                      EditorChunkItem::selectionMark()))
                                    continue;
...
DragBoxView::DragBoxView(ChunkItemFrustumLocatorPtr locator,
                         Moo::Colour                colour,
                         PyTypeObject*              pType)
  : ToolView(pType)
  , locator_(locator)
...
void DragBoxView::render(Moo::DrawContext& drawContext, const Tool& tool)
{
    ...
    Geometrics::drawRect(Vector2((float)minx, (float)miny),
