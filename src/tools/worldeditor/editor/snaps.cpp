...
namespace Snap {
    ...
    Vector3 toGround(const Vector3& v)
    {
        ...
        Vector3 groundPos = toGround(
          pos, Vector3(0.f, -1.f, 0.f), MAX_CHUNK_HEIGHT - MIN_CHUNK_HEIGHT);
        if (groundPos != pos)
            return groundPos;
    ...
    Vector3 toGround(const Vector3& startPosition,
                     ...
                     bool           ignoreHoles)
    {
        ...
        if (space) {
            space->collide(startPosition, extent, terrainCallback);
        }
        s_collideTerrainHoles = false;

        if (terrainCallback.collided()) {
            // return the ground position
            return startPosition + (direction * terrainCallback.dist());
        }
                       ...
                       bool*          snapped)
    {
        ...
        ChunkSpacePtr space = ChunkManager::instance().cameraSpace();
        distance            = space->collide(startPosition, extent, *pCallback);
    ...
    Vector3 toObstacleNormal(const Vector3& startPosition,
                             const Vector3& direction,
                             float          distance)
    {
        BW_GUARD;
    ...
    float satisfy(float a, float b)
    {
        if (a > 0.f) {
            if (b > 0.f) {
                // We convert to integers because this can't be done right with
                // an arbitary amount of decimal places, we allow units as small
                // as 0.001, which is enough granularity for user snaps.
