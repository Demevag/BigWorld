...
ON_COMMAND(ID_PROPERTIES_LISTITEM_POPUP_REMOVEITEM,
           OnPopupPropertyListItemRemoveItem)
...
static UINT indicators[] = {
    ...
    ID_INDICATOR_MEMORYLOAD, ID_INDICATOR_POSITION,  ID_INDICATOR_SNAPS,
    ID_INDICATOR_TRIANGLES,  ID_INDICATOR_FRAMERATE, ID_INDICATOR_CHUNKS,
    //	ID_INDICATOR_CAPS,
    //	ID_INDICATOR_NUM,
    //	ID_INDICATOR_SCRL,
};
...
MainFrame::MainFrame()
  ...
  , triangles_("")
  , initialised_(false)
  , GUI::ActionMaker<MainFrame>("doSaveSelectionAsPrefab",
                                &MainFrame::saveSelectionAsPrefab)
  ...
  , GUI::UpdaterMaker<MainFrame, 1>("updateStatusBar",
                                    &MainFrame::updateStatusBar)
  , GUI::ActionMaker<MainFrame, 5>("doShowPlayerPreview",
                                   &MainFrame::showPlayerPreview)
  , GUI::ActionMaker<MainFrame, 6>("doHidePlayerPreview",
                                   &MainFrame::hidePlayerPreview)
  , GUI::UpdaterMaker<MainFrame, 2>("updatePlayerPreview",
                                    &MainFrame::updatePlayerPreview)
  , GUI::UpdaterMaker<MainFrame, 3>("updateToolMode",
                                    &MainFrame::updateToolMode)
{
...
int MainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
    ...
    //							ID_INDICATOR_MESSAGE, SBPS_NORMAL,
    // messageWidth);
    m_wndStatusBar.SetPaneInfo(
      ...
      trianglesWidth);
    m_wndStatusBar.SetPaneInfo(
      m_wndStatusBar.CommandToIndex(ID_INDICATOR_SNAPS),
      ID_INDICATOR_SNAPS,
      SBPS_NORMAL,
      snapsWidth);
    m_wndStatusBar.SetPaneInfo(
      m_wndStatusBar.CommandToIndex(ID_INDICATOR_MEMORYLOAD),
      ID_INDICATOR_MEMORYLOAD,
      SBPS_NORMAL,
      memoryLoadWidth);
    m_wndStatusBar.SetPaneInfo(
      m_wndStatusBar.CommandToIndex(ID_INDICATOR_POSITION),
      ID_INDICATOR_POSITION,
      SBPS_NORMAL,
      positionWidth);
    m_wndStatusBar.SetPaneInfo(
      m_wndStatusBar.CommandToIndex(ID_INDICATOR_CHUNKS),
      ID_INDICATOR_CHUNKS,
      SBPS_NORMAL,
      chunksWidth);
    m_wndStatusBar.SetPaneInfo(
...
void MainFrame::updateStatusBar()
{
    ...
    if (m_wndStatusBar.GetSafeHwnd()) {
        ...
        m_wndStatusBar.SetPaneText(
          m_wndStatusBar.CommandToIndex(ID_INDICATOR_MEMORYLOAD),
          bw_utf8tow(WorldManager::instance().getStatusMessage(0)).c_str(),
          TRUE);
        m_wndStatusBar.SetPaneText(
          m_wndStatusBar.CommandToIndex(ID_INDICATOR_TRIANGLES),
          bw_utf8tow(WorldManager::instance().getStatusMessage(1)).c_str(),
          TRUE);
        m_wndStatusBar.SetPaneText(
          m_wndStatusBar.CommandToIndex(ID_INDICATOR_SNAPS),
          bw_utf8tow(WorldManager::instance().getStatusMessage(2)).c_str(),
          TRUE);
        m_wndStatusBar.SetPaneText(
          m_wndStatusBar.CommandToIndex(ID_INDICATOR_POSITION),
          bw_utf8tow(WorldManager::instance().getStatusMessage(3)).c_str(),
          TRUE);
        m_wndStatusBar.SetPaneText(
          m_wndStatusBar.CommandToIndex(ID_INDICATOR_FRAMERATE),
          bw_utf8tow(WorldManager::instance().getStatusMessage(4)).c_str(),
          TRUE);
        m_wndStatusBar.SetPaneText(
          m_wndStatusBar.CommandToIndex(ID_INDICATOR_CHUNKS),
          bw_utf8tow(WorldManager::instance().getStatusMessage(5)).c_str(),
          TRUE);
    }
...
void MainFrame::enableCloseButton(bool enable)
{
    ...
    EnableMenuItem(::GetSystemMenu(m_hWnd, FALSE), SC_CLOSE, uEnable);
}
...
bool MainFrame::saveSelectionAsPrefab(GUI::ItemPtr item)
{
    ...
    if (!WorldEditorApp::instance().pythonAdapter()->canSavePrefab()) {
        WorldManager::instance().addCommentaryMsg(
          LocaliseUTF8(L"WORLDEDITOR/GUI/MAINFRAME/PREFAB_WARNING"));
        return false;
    }
    ...
    BWFileDialog::FDFlags flags = (BWFileDialog::FDFlags)(
      BWFileDialog::FD_OVERWRITEPROMPT | BWFileDialog::FD_HIDEREADONLY |
      BWFileDialog::FD_PATHMUSTEXIST);
    BWFileDialog fileDlg(false, L"prefab", L"*.prefab", flags, szFilters, this);

    CString     path(L"object/prefabs/");
    BW::wstring initialDir = bw_utf8tow(
      Options::getOptionString(bw_wtoutf8((path + L"directory").GetString())));
    if (initialDir.empty())
    ...
    WorldEditorApp::instance().pythonAdapter()->saveSelectionPrefab(
      bw_wtoutf8(pathName));
    return true;
}
...
unsigned int MainFrame::updateToolMode(GUI::ItemPtr item)
{
    ...
    return (PanelManager::instance().currentTool() ==
            bw_utf8tow((*item)["toolMode"]))
             ? 1
             : 0;
}
...
void MainFrame::OnClose()
{
    ...
    if (WorldManager::instance().canClose(
          LocaliseUTF8(L"WORLDEDITOR/GUI/MAINFRAME/CAN_CLOSE_EXIT"))) {
...
void MainFrame::OnToolbarHotItemChange(UINT     id,
                                       NMHDR*   pNotifyStruct,
                                       LRESULT* result)
{
    BW_GUARD;
...
{
    m_wndStatusBar.SetPaneText(m_wndStatusBar.CommandToIndex(id), text, TRUE);
}
