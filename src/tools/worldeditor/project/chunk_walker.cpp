...
bool LinearChunkWalker::nextTile(BW::string& retChunkName,
                                 int16&      gridX,
                                 int16&      gridZ)
{
    BW_GUARD;
 ...
 */
BoundedChunkWalker::BoundedChunkWalker(int16 left,
                                       int16 bottom,
                                       int16 right,
                                       int16 top)
  : x_(left)
...
{
    if (left_ >= info_.gridWidth_ || right_ <= 0 ||
        bottom_ >= info_.gridHeight_ || top_ <= 0) {
...
bool BoundedChunkWalker::nextTile(BW::string& retChunkName,
                                  int16&      gridX,
                                  int16&      gridZ)
{
    BW_GUARD;
...
bool CacheChunkWalker::nextTile(BW::string& chunkName,
                                int16&      gridX,
                                int16&      gridZ)
{
    BW_GUARD;
 ...
 *	number of entries can be very large ( e.g. 70km space ~ 1/2 million
 *thumbnails )
 */
...
void CacheChunkWalker::save(DataSectionPtr      pSection,
                            BW::vector<uint16>& modifiedTileNumbers)
{
    BW_GUARD;
...
bool FileChunkWalker::nextTile(BW::string& retChunkName,
                               int16&      gridX,
                               int16&      gridZ)
{
    BW_GUARD;
...
bool FileChunkWalker::partialFolderMatch(const BW::string& folder1,
                                         const BW::string& folder2)
{
    BW_GUARD;
...
bool FileChunkWalker::isValidFolder(const WIN32_FIND_DATA& fileInfo)
{
    ...
    if (findFileData_.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
        // don't allow "." and ".." folders.  that be bad.  also don't bother
        // looking in the "CVS" folder
...
bool ModifiedFileChunkWalker::isRelevant(const WIN32_FIND_DATA& fileInfo)
{
    ...
    if (gridFromThumbnailFileName(fileName.c_str(), gridX, gridZ)) {
        BW::string pathName =
          WorldManager::instance().geometryMapping()->path();
        BW::string chunkName;
        chunkID(chunkName, gridX, gridZ);

        if (gridX >= info_.localToWorld_.x && gridZ >= info_.localToWorld_.y &&
            ...
            if (thumbnailExists(pathName, chunkName)) {
                uint64 fileTime =
                  SpaceMap::instance().timestampCache().getUint64(
                ...
                if (cacheTime < fileTime) {
                    SpaceMapDebug::instance().onConsidered(
                      gridX, gridZ, 0x00ff0000);
                    return true;
                } else {
                    // Green - thumbnail is ok
                    SpaceMapDebug::instance().onConsidered(
                      gridX, gridZ, 0x0000ff00);
                }
            } else {
