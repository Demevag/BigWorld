...
namespace {
    ...
    static const uint32 TEX_MASK_BLIT_FULL_STRENGTH =
      1 << TEX_MASK_BLIT_STRENGTH_EXP;
    static const uint32 TEX_MASK_BLIT_HALF_STRENGTH =
      1 << (TEX_MASK_BLIT_STRENGTH_EXP - 1);
    ...
    class TerrainBlockInfo
    {
      public:
        ...
        bool setTerrainBlock(Terrain::EditorBaseTerrainBlock* pTerrainBlock);
     ...
     *
     *	@return				True if the chunk should be saved and unloaded,
     *false if it should remain in memory.
     */
     ...
     *
     *	@param force		If true then the chunk will be marked as needing
     *saving when set, if false then the chunk won't be saved.
     */
    ...
    class ChunkVisitor
    {
      public:
         ...
         *	@param irect		The area to import in this chunk inside the
         *giant bitmap coordinate system.
         ...
         */
        virtual bool visit(const BW::string&  chunkName,
     ...
     */
    bool CountVisitor::visit(const BW::string& /*chunkName*/,
                             const BW::RectInt& /*irect*/,
                             const BW::RectInt& /*trect*/,
                             uint32 /*polesWide*/,
                             uint32 /*polesHigh*/)
     ...
     *  Overrides ChunkVisitor::visit.
     */
    /*virtual*/ bool UndoVisitor::visit(const BW::string& chunkName,
                                        const BW::RectInt& /*irect*/,
                                        const BW::RectInt& /*trect*/,
                                        uint32 /*polesWide*/,
                                        uint32 /*polesHigh*/)
    ...
    {
      public:
        ImportVisitor(TerrainPaintBrushPtr pBrush,
     ...
     */
    ImportVisitor::ImportVisitor(TerrainPaintBrushPtr pBrush,
                                 const BW::string&    texture,
                                 const BW::string&    bumpTexture,
                                 const Vector4&       uProj,
                                 const Vector4&       vProj,
     ...
     *   Overrides ChunkVisitor::visit.
     */
    /*virtual*/ bool ImportVisitor::visit(const BW::string&  chunkName,
                                          const BW::RectInt& irect,
                                          const BW::RectInt& trect,
                                          uint32             polesWide,
                                          uint32             polesHigh)
    {
        ...
        bool   newLayer = false;
        size_t layer    = pTerrainBlock->findLayer(
          texture_,
          bumpTexture_,
          uProj_,
          vProj_,
        ...
        int dleft = Math::lerp(
          irect.xMin_, trect.xMin_, trect.xMax_, 0, (int)polesWide - 1);
        int dtop = Math::lerp(
          irect.yMin_, trect.yMax_, trect.yMin_, 0, (int)polesHigh - 1);
        int dright = Math::lerp(
          irect.xMax_, trect.xMin_, trect.xMax_, 0, (int)polesWide - 1);
        int dbottom = Math::lerp(
          irect.yMax_, trect.yMax_, trect.yMin_, 0, (int)polesHigh - 1);
        ...
        for (int dy = dbottom; dy <= dtop; ++dy) {
            ...
            for (int dx = dleft; dx <= dright; ++dx) {
                ...
                uint8     value      = (uint8)std::min(
                  (uint32)(startValue + mv.fuzziness_), (uint32)mv.maxValue_);
                ...
                *pLayers[layer] = value;
                ...
                if (remainder == 0) {
                    // If the remainder is zero, which can happen a lot, then
                    // the other layers at this pixel must be zero:
                    for (size_t i = 0; i < numLayers; ++i) {
                        if (i != layer) {
                            *pLayers[i] = 0;
                        }
                    ...
                    if (sumMask != 0) {
                        ...
                        uint32 maskRatio =
                          (incr << TEX_MASK_BLIT_STRENGTH_EXP) / sumMask;
                        maskRatio =
                          std::min(maskRatio, TEX_MASK_BLIT_FULL_STRENGTH);
                     ...
                     ChunkVisitor&     visitor)
    {
        ...
        float      gridSize;
        if (!TerrainUtils::spaceSettings(
              space, gridSize, gridWidth, gridHeight, localToWorld)) {
            return false;
        }
        ...
        int left = (int)((leftf / gridSize + gridWidth / 2) / gridWidth *
                         (widthPixels - 1));
        int top  = (int)((topf / gridSize + gridHeight / 2) / gridHeight *
                        (heightPixels - 1));
        int right =
          (int)((rightf / gridSize + gridWidth / 2) / gridWidth * widthPixels);
        int bottom = (int)((bottomf / gridSize + gridHeight / 2) / gridHeight *
                           heightPixels);
        ...
        if (showProgress) {
            pImportTask = new ProgressBarTask(
              description, float(gridWidth * gridHeight), false);
        }
        ...
        while (lcw.nextTile(chunkName, gridX, gridZ)) {
            ...
            if (TerrainUtils::isEditable(gridX, gridZ)) {
                ...
                if (TerrainUtils::rectanglesIntersect(tileLeft,
                                                      tileTop,
                                                      tileRight,
                                                      tileBottom,
                                                      left,
                    ...
                    visitor.visit(
                      chunkName, irect, trect, polesWide, polesHigh);
                }
            }
        }
...
size_t TextureMaskBlit::numChunksAffected(TerrainPaintBrushPtr pBrush)
{
    ...
    CountVisitor countVisitor;
    visitChunks(pBrush->importMaskTL_.x,
                pBrush->importMaskTL_.y,
                pBrush->importMaskBR_.x,
                pBrush->importMaskBR_.y,
                "",    // No progress text
 ...
 */
bool TextureMaskBlit::saveUndoForImport(TerrainPaintBrushPtr pBrush,
 ...
 */
bool TextureMaskBlit::import(TerrainPaintBrushPtr pBrush,
                             const BW::string&    texture,
                             const BW::string&    bumpTexture,
                             const Vector4&       uProj,
                             const Vector4&       vProj,
                             ...
                             bool                 forceSave)
{
    ...
    ImportVisitor importVisitor(
      pBrush, texture, bumpTexture, uProj, vProj, forceSave);
    visitChunks(pBrush->importMaskTL_.x,
                pBrush->importMaskTL_.y,
                pBrush->importMaskBR_.x,
                pBrush->importMaskBR_.y,
