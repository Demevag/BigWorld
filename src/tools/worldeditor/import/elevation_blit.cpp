...
namespace {
     ...
     */
    void tilePosition(TerrainUtils::TerrainFormat& format,
                      unsigned int                 gridX,
                      unsigned int                 gridY,
                      int&                         left,
                      int&                         top,
 ...
 */
size_t ElevationBlit::importUndoSize(float leftf,
                                     float topf,
                                     float rightf,
                                     float bottomf)
{
 ...
 */
bool ElevationBlit::saveUndoForImport(float             leftf,
                                      float             topf,
                                      float             rightf,
                                      float             bottomf,
                                      ...
                                      bool              showProgress)
{
    ...
    if (showProgress) {
        ...
        importTask   = new ProgressBarTask(
          LocaliseUTF8(
 ...
 */
bool ElevationBlit::import(ImportImage const& image,
                           float              leftf,
                           float              topf,
                           float              rightf,
                           float              bottomf,
                           ...
                           bool               forceToMemory)
{
    ...
    if (showProgress) {
        float length = float((gridRight - gridLeft) * (gridTop - gridBottom));
        importTask   = new ProgressBarTask(
          LocaliseUTF8(L"WORLDEDITOR/WORLDEDITOR/IMPORT/ELEVATION_BLIT/"
                       L"IMPORT_TERRAIN_DATA"),
          length,
          true);
    }
        ...
        if (TerrainUtils::rectanglesIntersect(tileLeft,
            ...
            if (TerrainUtils::isEditable(gridX, gridZ)) {
                ...
                TerrainUtils::getTerrain(
                  gridX, gridZ, terrainImage, forceToMemory, &getInfo);

                if (!terrainImage.isEmpty()) {
                    // Destination coordinates within this chunk:
                    int dleft   = Math::lerp(ileft,
                                           tileLeft,
                                           tileRight,
                                           0,
                                           (int)format.polesWidth - 1);
                    int dtop    = Math::lerp(itop,
                                          tileBottom,
                                          tileTop,
                                          0,
                                          (int)format.polesHeight - 1);
                    int dright  = Math::lerp(iright,
                                            tileLeft,
                                            tileRight,
                                            0,
                                            (int)format.polesWidth - 1);
                    int dbottom = Math::lerp(ibottom,
                                             tileBottom,
                                             tileTop,
                                             0,
                                             (int)format.polesHeight - 1);
                    ...
                    double sleft = Math::lerp(
                      ileft, left, right, 0.0, (float)image.width() - 1.0);
                    double stop = Math::lerp(
                      itop, top, bottom, 0.0, (float)image.height() - 1.0);
                    double sright = Math::lerp(
                      iright, left, right, 0.0, (float)image.width() - 1.0);
                    double sbottom = Math::lerp(
                      ibottom, top, bottom, 0.0, (float)image.height() - 1.0);
                    ...
                    for (int dy = dbottom; dy <= dtop; ++dy) {
                        ...
                        double sy  = Math::safeLerp((double)dy,
                                                   (double)dbottom,
                                                   (double)dtop,
                                                   (double)sbottom,
                                                   (double)stop);
                        for (int dx = dleft; dx <= dright; ++dx, ++dst) {
                            double sx     = Math::safeLerp((double)dx,
                                                       (double)dleft,
                                                       (double)dright,
                                                       (double)sleft,
                                                       (double)sright);
                            ...
                            double imgVal =
                              image.toHeight(rawVal) * scale + offset;
                            double terVal = *dst;
                            switch (mode) {
                                case ElevationBlit::OVERLAY: // fall thru
                                                             // deliberate
                            ...
                            *dst =
                              Math::clamp(TerrainUtils::MIN_TERRAIN_HEIGHT,
                                          (float)terVal,
                                          TerrainUtils::MAX_TERRAIN_HEIGHT);
                        }
                    }
                    TerrainUtils::patchEdges(gridX,
                                             gridZ,
                                             gridWidth,
                                             gridHeight,
                                             getInfo,
                    ...
                    TerrainUtils::setTerrain(
                      gridX, gridZ, terrainImage, getInfo, !forceToMemory);
                }
            }
        }
    }
 ...
 */
bool ElevationBlit::exportTo(ImportImage& image,
                             float        leftf,
                             float        topf,
                             float        rightf,
                             float        bottomf,
                             ...
                             bool         showProgress)
{
    ...
    if (!TerrainUtils::rectanglesIntersect(0,
                                           heightPixels,
                                           widthPixels,
                                           0,
                                           left,
    ...
    while (bcw.nextTile(chunkName, gridX, gridZ)) {
        ...
        if (TerrainUtils::rectanglesIntersect(tileLeft,
            ...
            if (TerrainUtils::isEditable(gridX, gridZ)) {
                Terrain::TerrainHeightMap::ImageType terrainImage;
                TerrainUtils::getTerrain(gridX, gridZ, terrainImage, false);

                if (!terrainImage.isEmpty()) {
                    // Source coordinates within this chunk:
                    int sleft   = Math::lerp(ileft,
                                           tileLeft,
                                           tileRight,
                                           0,
                                           (int)format.polesWidth - 1);
                    int stop    = Math::lerp(itop,
                                          tileBottom,
                                          tileTop,
                                          0,
                                          (int)format.polesHeight - 1);
                    int sright  = Math::lerp(iright,
                                            tileLeft,
                                            tileRight,
                                            0,
                                            (int)format.polesWidth - 1);
                    int sbottom = Math::lerp(ibottom,
                                             tileBottom,
                                             tileTop,
                                             0,
                                             (int)format.polesHeight - 1);
                    ...
                    int dbottom =
                      Math::lerp(ibottom, bottom, top, 0, dheight - 1);
 ...
 */
bool ElevationBlit::heightRange(float  leftf,
                                float  topf,
                                float  rightf,
                                float  bottomf,
                                float& minHeight,
