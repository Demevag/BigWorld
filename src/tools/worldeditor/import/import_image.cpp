  ...
  public:
    ...
    static ImportCodecPtr getCodec(const BW::string& filename)
    {
        if (s_pInstance_) {
            ...
            for (BW::vector<ImportCodecPtr>::iterator it =
                   s_pInstance_->codecs_.begin();
                 it != s_pInstance_->codecs_.end();
                 ++it) {
 ...
 */
ImportCodec::LoadResult ImportImage::load(BW::string const& filename,
                                          float*            left,
                                          float*            top,
                                          float*            right,
                                          float*            bottom,
                                          ...
                                          bool              configDialog)
{
    ...
    if (codec == NULL) {
        ...
        return codec->load(
          filename, *this, left, top, right, bottom, absolute, configDialog);
    }
}
 ...
 */
bool ImportImage::save(BW::string const& filename,
                       float*            left,
                       float*            top,
                       float*            right,
                       float*            bottom,
 ...
 */
void ImportImage::setScale(float  minV,
                           float  maxV,
                           uint16 minSrc /*= 0*/,
                           uint16 maxSrc /*= 65535*/
)
 ...
 */
void ImportImage::rangeRaw(uint16& minV,
                           uint16& maxV,
 ...
 *						use cached values.
 */
void ImportImage::rangeHeight(float& minV, float& maxV, bool recalc /*= false*/
) const
{
    BW_GUARD;
