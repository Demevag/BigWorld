...
namespace TerrainUtils {
    ...
    struct TerrainFormat
    {
        ...
        uint32
          polesHeight;     // Number of poles across entire block in y-direction
    ...
    class TerrainFormatStorage
    {
      public:
        ...
        void setTerrainInfo(Terrain::BaseTerrainBlockPtr& block);
        void clear();
    ...
    void terrainSize(BW::string const& space,
                     float&            gridSize,
                     int&              gridMinX,
                     int&              gridMinY,
                     int&              gridMaxX,
    ...
    bool isEditable(int gx, int gy);

    float heightAtPos(float gx, float gy, bool forceLoad = false);

    bool rectanglesIntersect(int  left1,
                             int  top1,
                             int  right1,
                             int  bottom1,
                             int  left2,
    ...
    bool spaceSettings(BW::string const& space,
                       float&            gridSize,
                       uint16&           gridWidth,
                       uint16&           gridHeight,
                       GridCoord&        localToWorld);

    bool sourceTextureHasAlpha(const BW::string fileName);
}
