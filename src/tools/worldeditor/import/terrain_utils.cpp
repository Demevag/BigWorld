    ...
    for (int i = dirMap->minGridX(); i <= dirMap->maxGridX(); ++i) {
        for (int j = dirMap->minGridY(); j <= dirMap->maxGridY(); ++j) {
            const BW::string resName = dirMap->path() +
                                       ...
                                       ".cdata/terrain";
            block = Terrain::BaseTerrainBlock::loadBlock(
              resName, Matrix::identity, Vector3::zero(), terrainSettings);
            if (block.hasObject()) {
                break;
            }
        }
  ...
  DataSectionPtr&              terrainSettingsSection)
{
    ...
    BW::string error;
    const bool ok = defaultTerrainBlock->create(terrainSettingsSection, &error);
    if (!ok || !error.empty()) {
 ...
 */
void TerrainUtils::terrainSize(BW::string const& space_,
                               float&            gridSize,
                               int&              gridMinX,
                               int&              gridMinY,
                               int&              gridMaxX,
...
)
{
    ...
    if (chunk != NULL && chunk->loaded()) {
        ...
        if (chunkTerrain != NULL) {
            terrainBlock = static_cast<Terrain::EditorBaseTerrainBlock*>(
              chunkTerrain->block().getObject());
            if (getInfo != NULL) {

                getInfo->block_ = terrainBlock;
                getInfo->chunkTerrain_ =
                  static_cast<EditorChunkTerrain*>(chunkTerrain);
            }
        }
    }
    ...
    if (terrainBlock == NULL) {
        ...
        terrainBlock         = EditorChunkTerrain::loadBlock(
          spaceName + '/' + chunkName + ".cdata/terrain",
          NULL,
  ...
  bool                                        forceToDisk)
{
    ...
    if (forceToDisk && getInfo.chunkTerrain_ == NULL) {
        ...
        xform.setTranslate(gx * getInfo.block_->blockSize(),
                           0.0f,
 ...
 *							loaded yet.
 */
bool TerrainUtils::isEditable(int gx, int gy)
{
    BW_GUARD;
 ...
 */
float TerrainUtils::heightAtPos(float gx, float gy, bool forceLoad /*= false*/
)
{
    BW_GUARD;
    ...
    if (forceLoad) {
        ...
        Chunk* chunk = chunkManager.findChunkByName(
          chunkName, WorldManager::instance().geometryMapping());
    ...
    float dist = chunkManager.cameraSpace()->collide(
      srcPos, dstPos, ClosestObstacle::s_default);
    float result = 0.0;
    if (dist > 0.0)
 ...
 */
bool TerrainUtils::rectanglesIntersect(int  left1,
                                       int  top1,
                                       int  right1,
                                       int  bottom1,
                                       int  left2,
 ...
 */
bool TerrainUtils::spaceSettings(BW::string const& space,
                                 float&            gridSize,
                                 uint16&           gridWidth,
                                 uint16&           gridHeight,
