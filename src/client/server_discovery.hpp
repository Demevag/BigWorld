  ...
  , public PyObjectPlus
{
    Py_Header(ServerDiscovery, PyObjectPlus)
      ...
      public : ServerDiscovery(PyTypeObject* pType = &s_type_);
    ~ServerDiscovery();
     ...
     *  status of a BigWorld server as might be considered useful in a game
     * lobby. Instances of this class are and can only be created by the
    ...
    class Details : public PyObjectPlus
    {
        Py_Header(Details, PyObjectPlus)
          ...
          public : Details(PyTypeObject* pType = &s_type_);
        ...
        PY_RO_ATTRIBUTE_DECLARE(ownerValid_ ? ownerName_ : BW::string(),
                                ownerName)
        PY_RO_ATTRIBUTE_DECLARE(usersCountValid_ ? int(usersCount_) : -1,
                                usersCount)
    ...
    PyObject* changeNotifier() { return changeNotifier_.getObject(); }
