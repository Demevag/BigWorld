...
bool CameraApp::init()
{
    ...
    pyAutoAim_ =
      AutoAimPtr(new AutoAim(entityPicker_), PyObjectPtr::STEAL_REFERENCE);
    pyTargeting_ = TargetingPtr(new Targeting(entityPicker_, this->autoAim()),
                                PyObjectPtr::STEAL_REFERENCE);
    pyCameraSpeed_ =
      CameraSpeedPtr(new CameraSpeed(), PyObjectPtr::STEAL_REFERENCE);
    ...
    MF_WATCH("Render/Near Plane",
             *clientCamera_->projAccess(),
             MF_ACCESSORS(float, ProjectionAccess, nearPlane),
             "Camera near plane.");

    MF_WATCH("Render/Far Plane",
             *clientCamera_->projAccess(),
             MF_ACCESSORS(float, ProjectionAccess, farPlane),
             "Camera far plane.");

    MF_WATCH("Render/Fov",
             *clientCamera_->projAccess(),
             MF_ACCESSORS(float, ProjectionAccess, fov),
             "Field of view.");

    // Set up the camera
    float farPlaneDistance = rootSection->readFloat("camera/farPlane", 250.f);
    ...
    CameraControl::strafeRate(rootSection->readFloat("ui/straferate", 100.0));

    CameraControl::initDebugInfo();

    // Add BigWorld.cameraSpeed to the BigWorld module
    ScriptModule pBigworldModule =
      ScriptModule::import("BigWorld", ScriptErrorPrint());
    MF_ASSERT(ScriptModule::check(pBigworldModule));

    pBigworldModule.setAttribute(
      "cameraSpeed", this->cameraSpeed(), ScriptErrorPrint());

    // Add BigWorld.targeting to the BigWorld module
    pBigworldModule.setAttribute(
      "targeting", this->targeting(), ScriptErrorPrint());
...
void CameraApp::fini()
{
    ...
    pBigworldModule.setAttribute(
      "targeting", ScriptObject::none(), ScriptErrorPrint());
    pBigworldModule.setAttribute(
      "cameraSpeed", ScriptObject::none(), ScriptErrorPrint());
 ...
 *					If this is not specified, the current camera will be
 *returned.
 *
...
PY_AUTO_MODULE_FUNCTION(RETOWN,
                        camera,
                        OPTARG(BaseCameraPtr, BaseCameraPtr(), END),
                        BigWorld)
