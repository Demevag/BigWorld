...
EntityType* EntityType::find(const BW::string& name)
{
    ...
    BW::map<BW::string, EntityTypeID>::iterator found = s_typeNames_.find(name);
...
bool EntityType::init()
{
    ...
    if (entityDescriptionMap.parse(
        ...
        s_entityDefConstants_ =
          EntityDefConstants(entityDescriptionMap.digest(),
    ...
    for (int i = 0; i < numEntityTypes; i++) {
        ...
        // Load the module
        PyObject* pModule =
          PyImport_ImportModule(const_cast<char*>(currDesc.name().c_str()));
        if (pModule == NULL) {
            PyErr_PrintEx(0);
        ...
        if (pModule != NULL) {
            pClass = PyObject_GetAttrString(
              pModule, const_cast<char*>(currDesc.name().c_str()));
        }
        ...
        if (pClass != NULL) {
            if (!currDesc.checkMethods(
                ...
                succeeded = false;
                         ...
                         pClass, (PyObject*)&PyEntity::s_type_) != 1) {
                PyErr_Clear();
                ...
                EntityType* pType =
                  new EntityType(currDesc, pModule, (PyTypeObject*)pClass);
...
bool EntityType::reload()
{
    ...
    for (uint i = 0; i < s_types_.size(); i++) {
        ...
        if (et.pClass_ != NULL) {
            ...
            if (!desc.checkMethods(
                  desc.client().internalMethods(),
...
void EntityType::swizzleClass(PyTypeObject*& pOldClass)
{
    ...
    if (!PyObject_IsSubclass(pObjNewClass, (PyObject*)&PyEntity::s_type_)) {
        WARNING_MSG("EntityType::reload: "
                    "Class '%s' does not derive from Entity!",
                    className);
        return;
    }
...
PyObject* EntityType::getPreloads(void)
{
    ...
    for (uint i = 0; i < s_types_.size(); i++) {
        PyObject* pFn =
          PyObject_GetAttrString(s_types_[i]->pModule_, "preload");
        if (pFn == NULL) {
            PyErr_Clear();
            continue;
        }
                                    ...
                                    bool           includeOwnClient) const
{
    ...
    for (uint i = 0; i < description_.propertyCount(); i++) {
        ...
        if (pSection && (pSubSection = pSection->openSection(pDD->name()))) {
            ScriptDataSink sink;
        ...
        PyDict_SetItemString(
          pDict, const_cast<char*>(pDD->name().c_str()), pValue.getObject());
    }
                                    ...
                                    StreamContents contents) const
{
    ...
    if (contents < TAGGED_CELL_PLAYER_DATA) {
        ...
        description_.readStreamToDict(
          stream,
          dataDomainsToRead,
          ScriptDict(pDict, ScriptObject::FROM_BORROWED_REFERENCE));
    } else {
        description_.readTaggedClientStreamToDict(
          stream,
          ScriptDict(pDict, ScriptObject::FROM_BORROWED_REFERENCE),
          includeOwnClient);
    }
 ...
 */
void EntityType::prepareCreationStream(
                                         ...
                                         const ScriptDict& props) const
{
    description_.addDictionaryToStream(
      props, outputStream, EntityDescription::FROM_BASE_TO_CLIENT_DATA);
}
...
{
    description_.addDictionaryToStream(
      props, outputStream, EntityDescription::FROM_CELL_TO_CLIENT_DATA);
}
