 ...
 *
 *	AutoAim is made up of essentially two components.  The first is friction and
 *defines to what degree a Camera will 'stick' to its target and the second is
 *adhesion, which describes to what degree the target reticle (eg, crosshairs,
 *target box, etc) will follow the target. Each has a field of view where the
 *friction and adhesion will be the maximum specified and a falloff field of
 *view, where the values linearly blend to 0.
 *
 *	The object is contained within the BigWorld.Targeting object. Defaults for
 *any number of values can be provided in engine_config.xml under
 *&lt;targeting&gt;/&lt;autoAim&gt;, eg;
 *
 *	@{
 *	&lt;root&gt;
 *		...
 ...
 *
 *	This describes to what degree the Camera will 'stick' to a target.  It is on
 *a scale from 0.0 to 1.0. A value of 0.0 would mean there was no friction at
 *all, so the camera would move freely when a target is present and a value
 *of 1.0 would mean that the camera would lock onto the target until it reaches
 *the friction falloff angle/distance.  Defaults to 0.5.
 *
 *	@type float
 */
 ...
 *
 *	This describes to what degree the target reticle will 'stick' to a target as
 *the targeter moves forward. The value is between 0.0 and 1.0. 0 would mean the
 *target reticle is not affected, whereas 1 would cause the target reticle to
 *follow the target until it slides out of view.  Defaults to 0.05.
 *
 *	@type float
 */
 ...
 *
 *	This describes to what degree the target reticle will 'stick' to a target as
 *the targeter moves sideways. The value is between 0.0 and 1.0. 0 would mean
 *the target reticle is not affected, whereas 1 would cause the target reticle
 *to follow the target until it slides out of view.  Defaults to 0.2.
 *
 *	@type float
 */
 ...
 *
 *	This describes to what degree the target reticle will 'stick' to a target as
 *the targeter turns. The value is between 0.0 and 1.0. 0 would mean the target
 *reticle is not affected, whereas 1 would cause the target reticle to follow
 *the target until it slides out of view.  Defaults to 0.1.
 *
 *	@type float
 */
 ...
 *
 *	This value is used in calculations to handle pitch and yaw.  Defaults to
 *0.5.
 *
 *	@type float
 */
 ...
 *
 *	Adjusts the forwardAdhesion for backwards movement.  The default, -1 will
 *perform the usually desired effect, 0 will deactivate it.  Other values will
 *adjust it proportionally.
 *
 *	@type int
 */
 ...
 *
 *	This attribute specifies the left and right field of view angles, in
 *radians, within which the friction will be the maximum specified by friction.
 *
 *	@type int
 */
 ...
 *
 *	This attribute specifies the upper and lower field of view angles, in
 *radians, within which the friction will be maximum specified by friction.
 *
 *	@type float
 */
 ...
 *
 *	This attribute specifies the distance within which the friction will be
 *maximum specified by friction.
 *
 *	@type float
 */
 ...
 *
 *	This attribute specifies the left and right field of view angles, in
 *radians, outside which the friction will be 0.  The friction will linearly
 *deplete from frictionHorizontalAngle to frictionHorizontalFalloffAngle.
 *
 *	@type float
 */
 ...
 *	This attribute specifies the upper and lower field of view angles, in
 *radians, outside which the friction will be 0.  The friction will linearly
 *deplete from frictionVerticalAngle to frictionVerticalFalloffAngle.
 *
 *	@type float
 */
 ...
 *	The friction will linearly deplete from frictionDistance to
 ...
 *
 *	This attribute specifies the left and right field of view angles, in
 *radians, within which the adhesion will be the maximum specified for the
 *current movement.
 *
 *	@type float
 */
 ...
 *
 *	This attribute specifies the left and right field of view angles, in
 *radians, outside which the adhesion will be 0.  The adhesion will linearly
 *deplete from adhesionHorizontalAngle to adhesionHorizontalFalloffAngle.
 *
 *	@type float
 */
 ...
 *	This attribute specifies the upper and lower field of view angles, in
 *radians, outside which the adhesion will be 0.  The adhesion will linearly
 *deplete from adhesionVerticalAngle to adhesionVerticalFalloffAngle.
 *
 *	@type float
 */
 ...
 *	The adhesion will linearly deplete from adhesionDistance to
...
void AutoAim::readInitialValues(DataSectionPtr rootSection)
{
    ...
    if (section) {
        ...
        forwardAdhesion_ =
          section->readFloat("forwardAdhesion", forwardAdhesion_);
        ...
        adhesionPitchToYawRatio_ = section->readFloat("adhesionPitchToYawRatio",
                                                      adhesionPitchToYawRatio_);
        reverseAdhesionStyle_ =
          section->readInt("reverseAdhesionStyle", reverseAdhesionStyle_);

        entityPicker_.autoAimFrictionHorizontalAngle(
          DEG_TO_RAD(section->readFloat(
            "frictionHorizontalAngle",
            RAD_TO_DEG(entityPicker_.autoAimFrictionHorizontalAngle()))));
        entityPicker_.autoAimFrictionVerticalAngle(
          DEG_TO_RAD(section->readFloat(
            "frictionVerticalAngle",
            RAD_TO_DEG(entityPicker_.autoAimFrictionVerticalAngle()))));
        entityPicker_.autoAimFrictionDistance(section->readFloat(
          "frictionDistance", entityPicker_.autoAimFrictionDistance()));
        entityPicker_.autoAimFrictionHorizontalFalloffAngle(
          DEG_TO_RAD(section->readFloat(
            "frictionHorizontalFalloffAngle",
            RAD_TO_DEG(2.0f *
                       entityPicker_.autoAimFrictionHorizontalAngle()))));
        entityPicker_.autoAimFrictionVerticalFalloffAngle(
          DEG_TO_RAD(section->readFloat(
            "frictionVerticalFalloffAngle",
            RAD_TO_DEG(2.0f * entityPicker_.autoAimFrictionVerticalAngle()))));
        entityPicker_.autoAimFrictionMinimumDistance(
          section->readFloat("frictionMinimumDistance", 0.0f));
        entityPicker_.autoAimFrictionFalloffDistance(
          section->readFloat("frictionFalloffDistance",
                             2.0f * entityPicker_.autoAimFrictionDistance()));

        entityPicker_.autoAimAdhesionHorizontalAngle(
          DEG_TO_RAD(section->readFloat(
            "adhesionHorizontalAngle",
            RAD_TO_DEG(entityPicker_.autoAimAdhesionHorizontalAngle()))));
        entityPicker_.autoAimAdhesionVerticalAngle(
          DEG_TO_RAD(section->readFloat(
            "adhesionVerticalAngle",
            RAD_TO_DEG(entityPicker_.autoAimAdhesionVerticalAngle()))));
        entityPicker_.autoAimAdhesionDistance(section->readFloat(
          "adhesionDistance", entityPicker_.autoAimAdhesionDistance()));
        entityPicker_.autoAimAdhesionHorizontalFalloffAngle(
          DEG_TO_RAD(section->readFloat(
            "adhesionHorizontalFalloffAngle",
            RAD_TO_DEG(2.0f *
                       entityPicker_.autoAimAdhesionHorizontalAngle()))));
        entityPicker_.autoAimAdhesionVerticalFalloffAngle(
          DEG_TO_RAD(section->readFloat(
            "adhesionVerticalAngle",
            RAD_TO_DEG(2.0f * entityPicker_.autoAimAdhesionVerticalAngle()))));
        entityPicker_.autoAimAdhesionFalloffDistance(section->readFloat(
          "adhesionDistance", 2.0f * entityPicker_.autoAimAdhesionDistance()));
    }
}
...
{
    BW_GUARD;
    return roundf(RAD_TO_DEG(entityPicker_.autoAimFrictionHorizontalAngle()),
                  4);
}
...
float AutoAim::frictionHorizontalFalloffAngle() const
{
    BW_GUARD;
    return roundf(
      RAD_TO_DEG(entityPicker_.autoAimFrictionHorizontalFalloffAngle()), 4);
}
...
float AutoAim::frictionVerticalFalloffAngle() const
{
    BW_GUARD;
    return roundf(
      RAD_TO_DEG(entityPicker_.autoAimFrictionVerticalFalloffAngle()), 4);
}
...
{
    BW_GUARD;
    return roundf(RAD_TO_DEG(entityPicker_.autoAimAdhesionHorizontalAngle()),
                  4);
}
...
float AutoAim::adhesionHorizontalFalloffAngle() const
{
    BW_GUARD;
    return roundf(
      RAD_TO_DEG(entityPicker_.autoAimAdhesionHorizontalFalloffAngle()), 4);
}
...
float AutoAim::adhesionVerticalFalloffAngle() const
{
    BW_GUARD;
    return roundf(
      RAD_TO_DEG(entityPicker_.autoAimAdhesionVerticalFalloffAngle()), 4);
}
 ...
 *	This class manages the field of view in which an Entity can become a
 *potential target for any given client. Default values can be specified in
 *engine_config.xml in the &lt;targeting&gt; data section. See BigWorld.PyTarget
 *to see how potential targets are used to select the final target.
 */
 ...
 *	This is the field of view angle, in degrees, at which an Entity will become
 *a potential target. Lower values will provide a tighter fov, whilst higher
 *values will capture more targets.
 *	@type	float
 */
PY_ATTRIBUTE(selectionAngle)

/*~	attribute Targeting.deselectionAngle
 *	This is the angle, in degrees, in the field of view at which a potential
 *target will no longer be considered viable.  Lower values will provide a
 *tighter fov, whilst higher values will keep more targets.
 *	@type	float
 */
 ...
 *	This is the default field of view angle, in degrees, at which an Entity will
 ...
 *	This is the default angle, in degrees, in the field of view at which a
 *potential target will no longer be considered viable.
 *	@type	float
 */
...
void Targeting::readInitialValues(DataSectionPtr rootSection)
{
    ...
    if (section) {
        ...
        deselectionAngle(
          section->readFloat("deselectionAngle", deselectionAngle()));
        selectionDistance(
          section->readFloat("selectionDistance", selectionDistance()));
        ...
        entityPicker_.defaultSelectionDistance(
          entityPicker_.selectionDistance());
    }
}
 ...
 *	can be specified in engine_config.xml through the &lt;camera&gt; data
 *section.
 *
 ...
 *		( 0.84, 1.00 )
 *	@}
 *	For example, An axis value of 0.55 would pick the 4th item, giving a value
 *of 0.25. The camera speed would then be:
 *	@{
 ...
 *	The minimum value used for camera control with axis oriented devices.  In
 *addition, if the position is less than the minimum, it is ignored (the
 *deadzone). If the position is more than the maximum, it is considered
 *"pinned", i.e. at the extreme.
 *	@type	float
 */
PY_ATTRIBUTE(lookAxisMin)

/*~	attribute CameraSpeed.lookAxisMax
 *	The maximum value used for camera control with axis oriented devices.  In
 *addition, if the position is less than the minimum, it is ignored (the
 *deadzone). If the position is more than the maximum, it is considered
 *"pinned", i.e. at the extreme.
 *	@type	float
 ...
 *	This is the basic turning speed (in degrees per second) for horizonal
 *turning. Horizontal turning can be accelerated.
 *	@type	float
 */
PY_ATTRIBUTE(cameraYawSpeed)

/*~	attribute CameraSpeed.cameraPitchSpeed
 *	This is the basic turning speed (in degrees per second) for vertical
 *turning.
 *	@type	float
 */
 ...
 *	Acceleration is linear, and the maximum speed multiplier is given by this
 *value. The speed increases linearly between the time acceleration starts and
 *ends.
 *	@type	float
 */
 ...
 *	This is the time in seconds at which acceleration stops, and the movement
 ...
 *	If true, the camera speed is permitted to accelerate if targeting, otherwise
 *it won't.  Defaults to false (0).
 *	@type	boolean (0 or 1)
 */
...
void CameraSpeed::readInitialValues(DataSectionPtr rootSection)
{
    ...
    if (section) {
        ...
        cameraYawSpeed_ =
          DEG_TO_RAD(float(section->readInt("cameraYawSpeed", 120)));
        cameraPitchSpeed_ =
          DEG_TO_RAD(float(section->readInt("cameraPitchSpeed", 60)));
 ...
 *	Constructor.
 */
ClientSpeedProvider::ClientSpeedProvider(const TargetingPtr&   targeting,
...
float ClientSpeedProvider::value(const AxisEvent& event)
{
    ...
    AxisEvent::Axis otherAxisID =
      doingHorizontal ? AxisEvent::AXIS_RY : AxisEvent::AXIS_RX;
    ...
    // if (otherAxisID == AxisEvent::AXIS_RY && axisValue != adjustment +
    // adjustment) { 	char message[256]; 	sprintf(message,
    //"SimpleSpeedProvider::value() Adjusting from %g to %g (other axis =
    ...
    axisValue       = Math::clamp(cameraSpeed_->lookAxisMin_,
                            axisValue + adjustment,
                            cameraSpeed_->lookAxisMax_);
    float lookValue = 1.0f;
    if (axisValue > cameraSpeed_->lookAxisMin_) {
        V2Vector::iterator last = cameraSpeed_->lookTable_.begin();
        for (V2Vector::iterator iter = cameraSpeed_->lookTable_.begin();
             iter != cameraSpeed_->lookTable_.end();
             ++iter) {
            if (iter->x >= axisValue)
                break;
            last = iter;
    ...
    if (targeting_->autoAimOn())
        target_ = targeting_->hasAnAutoAimTarget(
          autoAimDistance, autoAimAngle, true, doingHorizontal);
    else
        target_ = NULL;
    if (target_) {
        float level = 1.0f;

        // The friction is full between 0 and fullDistance. After that it
        // decreases to 0 at falloffDistance
        float fullDistance = targeting_->autoAim()->frictionDistance();
        float minimumDistance =
          targeting_->autoAim()->frictionMinimumDistance();
        float falloffDistance =
          targeting_->autoAim()->frictionFalloffDistance();
        if (autoAimDistance > fullDistance)
            level *= 1.0f - (autoAimDistance - fullDistance) /
                              (falloffDistance - fullDistance);
        else if (autoAimDistance < minimumDistance && minimumDistance > 0.f)
            level *= autoAimDistance / minimumDistance;

        // The friction is full within the autoAimAngle. After that it decreases
        // to 0 at falloffAngle
        float fullAngle = doingHorizontal
                            ? targeting_->autoAim()->frictionHorizontalAngle()
                            : targeting_->autoAim()->frictionVerticalAngle();
        float falloffAngle =
          doingHorizontal
            ? targeting_->autoAim()->frictionHorizontalFalloffAngle()
            : targeting_->autoAim()->frictionVerticalFalloffAngle();
        if (autoAimAngle > fullAngle)
            level *=
              1.0f - (autoAimAngle - fullAngle) / (falloffAngle - fullAngle);
    ...
    switch (event.axis()) {
        case AxisEvent::AXIS_RX:
            if (lookValue == 0.0f) {
                ...
                float  accelTime =
                  yawAccelerationTime_ == 0.0 || lookValue < 1.0f
                     ? 0.0f
                     : yawAccelerationTime_;
                if (lookValue == 1.0f &&
                    (cameraSpeed_->accelerateWhileTargeting_ ||
                     target_ == NULL))
                    yawAccelerationTime_ += event.dTime();

                float accel =
                  accelTime < cameraSpeed_->accelerateStart_
                    ? 1.0f
                    : 1.0f + (cameraSpeed_->accelerateRate_ - 1.0f) *
                               min(accelTime, cameraSpeed_->accelerateEnd_) /
                               cameraSpeed_->accelerateEnd_;

                result = axisValue * lookValue * (1.0f - friction) *
                         cameraSpeed_->cameraYawSpeed_ * accel;
            }
            break;

        case AxisEvent::AXIS_RY:
            result = axisValue * lookValue * (1.0f - friction) *
                     cameraSpeed_->cameraPitchSpeed_;
            break;
...
void ClientSpeedProvider::adjustDirection(float  dTime,
                                          Angle& pitch,
                                          Angle& yaw,
                                          Angle& roll)
{
    ...
    if (targeting_->autoAimOn())
        target_ = targeting_->hasAnAutoAimTarget(
          autoAimDistance, autoAimAngle, false, true);
    else
        target_ = NULL;
    ...
    if (previousHadATarget_ && target_ != NULL) {
        ...
        // Get the movement of the player, divided into forward and sideways
        // movement
        ...
        float falloffDistance =
          targeting_->autoAim()->adhesionFalloffDistance();
        Angle baseLevel = 1.0f;
        if (targetDistance > falloffDistance)
            baseLevel = 0.0f;
        else if (targetDistance > fullDistance)
            baseLevel = baseLevel * (1.0f - (targetDistance - fullDistance) /
                                              (falloffDistance - fullDistance));

        float fullHorizAngle = targeting_->autoAim()->adhesionHorizontalAngle();
        float falloffHorizAngle =
          targeting_->autoAim()->adhesionHorizontalFalloffAngle();
        ...
        if (absYawDeltaToTarget > falloffHorizAngle)
            ...
            baseYawLevel =
              baseYawLevel * (1.0f - (absYawDeltaToTarget - fullHorizAngle) /
        ...
        if (absPitchDeltaToTarget > falloffHorizAngle)
            ...
            basePitchLevel = basePitchLevel *
                             (1.0f - (absPitchDeltaToTarget - fullHorizAngle) /
                                       (falloffHorizAngle - fullHorizAngle));

        // Change in yaw from forward movement
        forwardYawLevel = baseYawLevel *
                          targeting_->autoAim()->forwardAdhesion_ *
                          absForwardDistance;
        ...
        strafingYawLevel = baseYawLevel *
                           targeting_->autoAim()->strafeAdhesion_ *
                           absStrafeDistance;
        ...
        turningYawLevel =
          baseYawLevel * targeting_->autoAim()->turnAdhesion_ * fabsf(angle);
        ...
        Angle forwardPitchLevel = basePitchLevel *
                                  targeting_->autoAim()->forwardAdhesion_ *
                                  absForwardDistance;
        float deltaPitch = forwardPitchLevel *
                           targeting_->autoAim()->adhesionPitchToYawRatio_ *
                           absPitchDeltaToTarget;
        ...
        Angle strafingPitchLevel = basePitchLevel *
                                   targeting_->autoAim()->strafeAdhesion_ *
                                   absStrafeDistance;
        deltaPitch = strafingPitchLevel *
                     targeting_->autoAim()->adhesionPitchToYawRatio_ *
                     absPitchDeltaToTarget;
        if (pitchDeltaToTarget < 0.0f)
            deltaPitch *= -1.0f;
        ...
        while (angle >= MATH_PI)
            angle = fabsf(angle - 2 * MATH_PI);
        Angle turningPitchLevel =
          basePitchLevel * targeting_->autoAim()->turnAdhesion_ * fabsf(angle);
        ...
        pitchChange =
          (forwardPitchLevel + strafingPitchLevel + turningPitchLevel) *
          targeting_->autoAim()->adhesionPitchToYawRatio_;
    } else {
    ...
    if (showTotalYaw_ || showForwardYaw_ || showStrafeYaw_ || showTurnYaw_ ||
        showTotalPitch_ || showFriction_) {
...
void ClientSpeedProvider::drawDebugStuff()
{
    ...
    if (yawChange_.size() > numSamples_)
        yawChange_.erase(yawChange_.begin(),
                         yawChange_.begin() + yawChange_.size() - numSamples_);
    if (yawForwardChange_.size() > numSamples_)
        yawForwardChange_.erase(yawForwardChange_.begin(),
                                yawForwardChange_.begin() +
                                  yawForwardChange_.size() - numSamples_);
    if (yawStrafeChange_.size() > numSamples_)
        yawStrafeChange_.erase(yawStrafeChange_.begin(),
                               yawStrafeChange_.begin() +
                                 yawStrafeChange_.size() - numSamples_);
    if (yawTurnChange_.size() > numSamples_)
        yawTurnChange_.erase(yawTurnChange_.begin(),
                             yawTurnChange_.begin() + yawTurnChange_.size() -
                               numSamples_);
    if (pitchChange_.size() > numSamples_)
        pitchChange_.erase(pitchChange_.begin(),
                           pitchChange_.begin() + pitchChange_.size() -
                             numSamples_);
    if (friction_.size() > numSamples_)
        friction_.erase(friction_.begin(),
                        friction_.begin() + friction_.size() - numSamples_);
 ...
 */
void drawPlane(const Vector3&     u,
               const Vector3&     v,
               const Vector3&     p,
...
void ClientCamera::drawDebugStuff()
{
    ...
    if (s_cameraState) {
        ...
        drawPlane(s_cameraState->uaxis,
                  s_cameraState->vaxis,
                  s_cameraState->entityPosition + s_cameraState->waxis,
                  Moo::Colour(0, 0, 1, 0));

        // draw near plane at camera position
        drawPlane(s_cameraState->uaxis,
                  s_cameraState->vaxis,
                  s_cameraState->cameraPosition + s_cameraState->waxis,
                  Moo::Colour(0, 1, 0, 0));

        // draw camera position
        Geometrics::instance().drawSphere(
          s_cameraState->cameraPosition, 0.02f, Moo::Colour(1, 0, 0, 0));
    }
}
...
static void debugNearPlane(PyObjectPtr camera)
{
    ...
    if (camera) {
        ...
        Vector3 entityPos =
          matrix.applyPoint(Vector3(0.f, cursorCamera->pivotPosition().y, 0.f));
