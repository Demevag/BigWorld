...
// Scaleform undefined Windows DrawText to avoid the effect on their DrawText
// class, We should use BWDrawText in case Scaleform DrawText is being used.
#if SCALEFORM_SUPPORT
#ifdef UNICODE
...
bool clientAreaHitTest(HWND hWnd)
{
    ...
    return SendMessage(hWnd, WM_NCHITTEST, 0, MAKELPARAM(pnt.x, pnt.y)) ==
           HTCLIENT;
}
              ...
              LPCTSTR   lpWindowName)
{
    ...
    try {
        ...
        preferencesFilename.init(rootSection->openSection("preferences"),
                                 "preferences.xml",
        ...
        if (dataRes.load(preferencesFilename.resolveName()) ==
        ...
        if (preferences) {
            ...
            // load ExDevice Preference explicitly as it is needed before gfx
            // settings are init
            if (graphicsPreferences) {
                ...
                for (; sectIt < sectEnd; ++sectIt) {
                    ...
                    if (label == "DEVICE_EX") {
                        exInterface =
                          (*sectIt)->readInt("activeOption", exInterface);
                        break;
                    }
                }
            }
        ...
        if (rootSection) {
            ...
            DataSectionPtr pSection =
              rootSection->openSection("appTitle", false);
            if (pSection) {
        ...
        while (running) {
            if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
                ...
                if ((msg.message == WM_KEYDOWN) || (msg.message == WM_KEYUP) ||
                    ...
                    BW::ScaleformBW::IME::handleIMMMessage(
...
#endif
    // if exiting while running open automate we return 0 (even if singeltons
    // issues cause us to return something else)
    if (BWOpenAutomate::s_doingExit) {
        LogMsg::logToFile("Exiting automated test normally");
        if (msg.wParam != 0) {
            WARNING_MSG(
              "Returning with 0 from main as open automate is calling exit");
            return 0;
        }
    }
...
/**
 *	This function is called when WM_CLOSE or alt+F4 are received in order to
...
LRESULT BWWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg) {
        ...
        case WM_SETCURSOR:
            // From DirectX "ShowCursor" documentation, one should prevent
            // Windows from setting a Windows cursor for this window by doing
            // this:
            if (Moo::rc().device() && !LogMsg::hasSeenCriticalMsg() &&
        ...
        case WM_ACTIVATE:
            if (App::pInstance() && !App::instance().isQuiting()) {
        ...
        case WM_MOVE:
            if (g_bActive && g_bAppStarted) {
                if (App::pInstance()) {
                    App::instance().moveWindow((int16)LOWORD(lParam),
                                               (int16)HIWORD(lParam));
                }
            }
            break;
        ...
        case WM_SIZE:
            g_bActive = !((SIZE_MAXHIDE == wParam) ||
                          (SIZE_MINIMIZED == wParam) || (0 == lParam));
        ...
        case WM_CLOSE: {
            ...
            // DefWindowProc call DestroyWindow(), and app get WM_DESTROY
            // message
            break;
        }
        ...
        case WM_DESTROY:
            ...
            // notify that we are quitting (send WM_QUIT message) and exit from
            // main message loop
            PostQuitMessage(0);
            break;
        ...
        case WM_IME_SETCONTEXT:
            return DefWindowProc(
              hWnd, msg, wParam, 0); // In order to hide the system IME windows
        ...
        case WM_INPUTLANGCHANGE:
            if (BW::ScaleformBW::IME::handleIMEMessage(
    ...
    if (InputDevices::handleWindowsMessage(
          hWnd, msg, wParam, lParam, inputResult)) {
        return inputResult;
    }

    if (msg == WM_SYSKEYUP && wParam == VK_MENU) {
        // Override these to stop some default windows behaviour (e.g. pressing
        // alt to bring up the system menu at the top left of the game window).
        // Since this is specific to the client it is here and not in
        // InputDevices.
        return TRUE;
    }
...
/**
 * Splits the given string in the style of Windows command line parsing (i.e.
 *the same style which is passed into a C main function under win32).
 *
 ...
 *  @param delim Possible argument delimeters. Defaults to whitespace
 *characters.
 ...
 */
size_t splitCommandArgs(const BW::wstring&      str,
                        BW::vector<BW::string>& out,
                        ...
                        wchar_t                 escape = L'\\')
{
    ...
    for (size_t i = 0; i < str.size(); ++i) {
        ...
        // bound = we are currently inside a quote, so process until we hit a
        // non-quote
        if (binding) {
            // Process \\" (escaped slash-with-a-quote)
            if (str[i] == escape && i < str.size() - 2 &&
                str[i + 1] == escape) {
            ...
            if (str[i] == binding) {
                // Hit matching binding char, add buffer to output (always -
            ...
            if (delim.find_first_of(str[i]) != BW::wstring::npos) {
                // Hit a delimeter
                if (!buf.empty()) {
                    // Add buffer to output (if nonempty - ignore empty strings
                    // when unbound)
                    out.push_back(BW::string());
                    bw_wtoutf8(buf, out.back());
                    buf.clear();
...
// Because in parseCommandLine( LPCTSTR str ), we have some reference in
// g_scriptArgv which point to some strings in it, if it's local, then those
// reference will be invalid after the function ends.
    ...
    while (argc < static_cast<int>(g_commandLine.size())) {
        ...
        if (argc + 1 < static_cast<int>(g_commandLine.size())) {
            ...
            if (arg == "--res" || arg == "-r" || arg == "--options") {
                ...
                bw_snprintf(logLine,
                            sizeof(logLine),
                            "Automated test initiated, %ls, built %s",
                            APP_TITLE,
                            compileTimeString);
