...
// const float INFLUENCE_RADIUS_SQUARED		= INFLUENCE_RADIUS *
// INFLUENCE_RADIUS; const float COLLISION_FRACTION				= 0.5f; const
// float INV_COLLISION_FRACTION			= 1.0f/(1.0f-COLLISION_FRACTION); const
// float NORMAL_SPEED					= 0.5f; const float ANGLE_TWEAK
// = 0.02f; const float PITCH_TO_SPEED_RATIO			= 0.002f; const float
// GOAL_APPROACH_RADIUS_SQUARED	= (10.0f * 10.0f); const float
// GOAL_STOP_RADIUS_SQUARED		= (1.0f * 1.0f);
...
void BoidsFilter::output(double time, MovementFilterTarget& target)
{
    ...
    for (int i = 0; i < size; i++) {
        ...
        boidData_[i].updateModel(
          goal, &*pCA, *this, dTime, timeMultiplier_, state_, radius_);
    ...
    if (boidsLanded) {
        ...
        entity.pPyEntity().callMethod("boidsLanded",
                                      ScriptErrorPrint(),
                                        ...
                                        const float        radius)
{
    ...
    if (delta.y > 0.01f) { // We're too low
        pitch_ += filter.angleTweak_ * DT;

        if (pitch_ > 0.8f) {
            pitch_ = 0.8f;
        }
    } else if (delta.y < -0.01f) { // We're too high
        pitch_ -= filter.angleTweak_ * DT;
        if (pitch_ < -0.8f) {
            pitch_ = -0.8f;
        }
    ...
    IF_NOT_MF_ASSERT_DEV(-LIMIT < pos_.x && pos_.x < LIMIT && -LIMIT < pos_.z &&
                         pos_.z < LIMIT)
    {
