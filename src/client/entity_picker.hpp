...
class EntityPicker
{
  public:
    ...
    // These two methods were moved from the private section, it didn't compile
    // before
  ...
  private:
    ...
    void prepareViewMatrix(float    fov,
                           float    aspectRatio,
                           Matrix&  lookFrom,
                           Matrix&  fovMatrix,
    ...
    bool twoPointLOSCheck(const Entity&      entity,
                          const BoundingBox& bb,
                          const Matrix&      lookFrom) const;
    bool fullBoundsLOSCheck(const Entity&      entity,
                            const BoundingBox& bb,
                            const Matrix&      lookFrom) const;
    ...
    void setCurrentEntity(Entity* pEntity, bool isFull, bool isHidden);
    ...
    float upperMarginFactor_; // percentage of the upper part of the bounding
                              // box that is unselectable
    ...
    /// special mode = when on, you cannot choose a target that would result in
    /// a partial lock.
    bool noPartial_;
...
class MouseTargetingMatrix : public MatrixProvider
{
    Py_Header(MouseTargetingMatrix, MatrixProvider)
      ...
      public
      : MouseTargetingMatrix(
          PyTypeObject* pType = &MouseTargetingMatrix::s_type_);
    void matrix(Matrix& lookFrom) const;
