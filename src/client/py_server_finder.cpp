  ...
  , pDoneCaller_(pDoneCaller)
{
    ...
    pDict_.setItem("uid", ScriptObject::createFrom(serverInfo.uid()).get());
}
...
{
    pDict_.setItemString(key.c_str(), ScriptObject::createFrom(value).get());
}
...
{
    Script::call(pCallback_.newRef(), PyTuple_Pack(1, pDict_.get(), NULL));
}
...
void PyServerFinder::onServerFound(const ServerInfo& serverInfo)
{
    ...
    if (pDetailsCallback_) {
        this->sendProbe(serverInfo.address(),
                        new PyServerProbeHandler(
                          pDetailsCallback_, serverInfo, pDoneCaller_));
    }
}
...
bool findServers(PyObjectPtr pCallback)
{
    ...
    ServerFinder* pFinder =
      new PyServerFinder(pFoundCallback, pDetailsCallback, pCompleteCallback);
...
PY_AUTO_MODULE_FUNCTION(RETOK, findServers, ARG(PyObjectPtr, END), BigWorld);
