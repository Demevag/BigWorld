...
PyObject* EntityDirProvider::pyNew(PyObject* args)
{
    ...
    int       yawIndex;
    if (!PyArg_ParseTuple(
          args, "O!ii", &PyEntity::s_type_, &pSource, &pitchIndex, &yawIndex)) {
        return NULL;
    }
...
void EntityDirProvider::getYawPitch(float& yaw, float& pitch) const
{
    ...
    if (pSource_.exists() && PyWeakref_CheckProxy(pSource_.getObject())) {
 ...
 *  @}
 *  @param source The MatrixProvider that gives the position from which the
 * matrix this provides points.
 *  @param target The MatrixProvider that gives the position towards which the
 * matrix this provides points.
...
PyObject* ScanDirProvider::pyNew(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(
          args, "ff|fO", &amplitude, &period, &offset, &pCallback) ||
        (pCallback != NULL && !PyCallable_Check(pCallback))) {
        PyErr_SetString(PyExc_TypeError,
                        "Amplitude, period and optional offset and hit limit "
                        "callback expected.\n");
        return NULL;
    }
...
void ScanDirProvider::getYawPitch(float& yaw, float& pitch) const
{
    ...
    if (oldPosInCycle_ >= 0.f && pHitLimitCallBack_) {
        ...
        bool wasGoingLeft =
          (0.25f < oldPosInCycle_) && (oldPosInCycle_ < 0.75f);
