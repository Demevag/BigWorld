 ...
 *	It provides the ability to conveniently construct it from a python
 *dictionary.
 */
 ...
 *	Constructor
 *
 *	Populates the data dictionary with every string:string entry in the provided
 *python dictionary.
 */
 ...
 */
bool BWVOIPDataDict::get(unsigned int index,
                         const char** key,
                         const char** value) const
{
    BW_GUARD;
 ...
 *
 *	@param	data			(dictionary)	Optional: Additional information in
 *the form of string value pairs
 *
 *	@return					(none)
 */
/**
 ...
 *
 *	This function instructs the VoIP client to clean up all its connections and
 *resources.
 *
 *	@param	data			(dictionary)	Optional: Additional information in
 *the form of string value pairs
 ...
 *
 *	@param	data			(dictionary)	Additional information in the form
 ...
 *
 *	@param	username		(string)		The string that uniquely identifies
 *the user's account
 *	@param	password		(string)		The corresponding password to access
 *the account
 *	@param	data			(dictionary)	Optional: Additional information in
 *the form of string value pairs
 *
...
PyObject* PyVOIP::py_login(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(
          args, "ss|O:VOIP.login", &username, &password, &pyData)) {
        return NULL;
    }
 ...
 *	Default password is "".
 *
 *	@param	channelName		(string)		The string that uniquely identifies
 *the new channel
 *	@param	password		(string)		Optional: The password that will be
...
PyObject* PyVOIP::py_createChannel(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(
          args, "s|sO:VOIP.createChannel", &channel, &password, &pyData)) {
        return NULL;
    }
    if (pyData != NULL && !PyDict_Check(pyData)) {
        PyErr_SetString(PyExc_TypeError,
...
PyObject* PyVOIP::py_joinChannel(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(
          args, "s|sO:VOIP.joinChannel", &channel, &password, &pyData)) {
        return NULL;
    }
...
PyObject* PyVOIP::py_queryChannels(PyObject* args)
{
    ...
    for (int i = 0; i < number; i++) {
        PyList_Append(
          channels,
          PyString_FromString(VOIPApp::getVOIPClient().getChannel(i)));
    }
 ...
 *
 *	@param	userName		(string)		The string identifying the user's
 *VoIP account
 *	@param	channelName		(string)		The string identifying the channel
 *	@param	data			(dictionary)	Optional: Additional information in
 *the form of string value pairs
...
PyObject* PyVOIP::py_inviteUser(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(
          args, "ss|O:VOIP.inviteUser", &user, &channel, &pyData)) {
        return NULL;
    }
...
PyObject* PyVOIP::py_kickUser(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(
          args, "ss|O:VOIP.kickUser", &user, &channel, &pyData)) {
        return NULL;
    }
...
PyObject* PyVOIP::py_enableActiveChannel(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(
          args, "s|O:VOIP.enableActiveChannel", &channel, &pyData)) {
        return NULL;
    }

    if (pyData != NULL && !PyDict_Check(pyData)) {
        PyErr_SetString(PyExc_TypeError,
                        "VOIP.enableActiveChannel: "
                        "expecting dict for optional argument 2 (data)");
        return NULL;
    }
...
PyObject* PyVOIP::py_disableActiveChannel(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(
          args, "s|O:VOIP.disableActiveChannel", &channel, &pyData)) {
        return NULL;
    }

    if (pyData != NULL && !PyDict_Check(pyData)) {
        PyErr_SetString(PyExc_TypeError,
                        "VOIP.disableActiveChannel: "
                        "expecting dict for optional argument 2 (data)");
        return NULL;
    }
 ...
 *
 *	@param	attenuation		(float)			The new speaker volume attenuation
 *in decibels(dB)
 *	@param	data			(dictionary)	Optional: Additional information in
 *the form of string value pairs
 *
...
PyObject* PyVOIP::py_setMasterVolume(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(
          args, "f|O:VOIP.setMasterVolume", &attenuation, &pyData)) {
        return NULL;
    }

    if (pyData != NULL && !PyDict_Check(pyData)) {
        PyErr_SetString(PyExc_TypeError,
                        "VOIP.setMasterVolume: "
                        "expecting dict for optional argument 2 (data)");
        return NULL;
    }
 ...
 *
 *	@param	attenuation		(float)			The new microphone attenuation in
 *decibels(dB)
 *	@param	data			(dictionary)	Optional: Additional information in
 *the form of string value pairs
 *
...
PyObject* PyVOIP::py_setMicrophoneVolume(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(
          args, "f|O:VOIP.setMicrophoneVolume", &attenuation, &pyData)) {
        return NULL;
    }

    if (pyData != NULL && !PyDict_Check(pyData)) {
        PyErr_SetString(PyExc_TypeError,
                        "VOIP.setMicrophoneVolume: "
                        "expecting dict for optional argument 2 (data)");
        return NULL;
    }
 ...
 *	@param	z			The location of the player in on the Z axis. +Z is
 *forward
 *	@param	yaw			The current yaw of the player. 0.0f yaw is forward, +/-
 *PI is backard
 *	@param	pitch		The current pitch of the player. 0.0f pitch is level,
 *+ve pitch is up (clockwise in Z-Y plane)
 *	@param	roll
 *	@param	spaceID		An integer uniquely identifying the space in which the
 *player currently resides
 */
void PyVOIP::tick(float x,
                  float y,
                  float z,
                  float yaw,
                  float pitch,
 ...
 *	Constructor
 *
 *	@param	voip		The VOIPClient the response will be handling callbacks
 *to.
 *	@param	callback	A callable python object that accepts an argument list
 *of the form (int, dictionary).
 */
