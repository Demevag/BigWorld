...
class MyDebug
{
  public:
    MyDebug()
    {
        ...
        MF_WATCH("Client Settings/Physics/reverse collision check",
                 g_reverseCollisionCheck);
        MF_WATCH("Client Settings/Physics/uphill slow down",
                 g_slowUpHill,
                 Watcher::WT_READ_WRITE,
                 "Toggle whether or not player entities slow down as they move "
                 "up hill.");
...
Physics::~Physics()
{
    ...
    Physicians::iterator it =
      std::find(s_physicians_.begin(), s_physicians_.end(), this);
...
void Physics::tick(double timeNow, double timeLast)
{
    ...
    if (style_ == STANDARD_PHYSICS) {
    ...
    } else if (style_ == CHASE_PHYSICS && chaseTarget_ != NULL &&
               chaseTarget_->isInWorld()) {
        ...
        this->transformCommonToLocal(spaceID, vehicleID, newPos, newDir);
...
void Physics::avatarStyleTick(double timeNow, double timeLast)
{
    ...
    this->transformLocalToCommon(spaceID, vehicleID, oldPos, oldDir);
    ...
    if (seekState_ == PATH) {
        ...
        this->doSeek(
          mc, useVelocity, disableVelocitySmoothing, newHeadYaw, newPos, dTime);
    } else if (chaseTarget_ != NULL) {
        this->doChaseTarget(
          mc, disableVelocitySmoothing, newHeadYaw, newPos, dTime);
    } else {
        // normal operation!
    ...
    newHeadYaw   = Math::clamp(float(HEAD_YAW_RANGE), float(newHeadYaw));
    newHeadPitch = Math::clamp(float(HEAD_PITCH_RANGE), float(newHeadPitch));
    ...
    if (seekState_ != PATH) {
        this->applyVelocitySmoothing(disableVelocitySmoothing,
                                     oldPos,
                                     newPos,
                                     thisDisplacement,
                                     dTime,
    ...
    this->transformCommonToLocal(spaceID, vehicleID, localNewPos, localOldDir);

    Vector3 localOldPos(oldPos);
    localOldDir = oldDir;
    this->transformCommonToLocal(spaceID, vehicleID, localOldPos, localOldDir);
    ...
    if (fall_ || (!oldStyleCollision_ &&
                  collideTerrain_)) // need to check here in new collision for
                                    // the scramble height
    {
        fallingVelocity_ =
          this->applyGravityAndBuoyancy(vehicleID, oldPos, newPos, mc, dTime);
    }
    ...
    this->doTeleport(
      pEmb, mc, newPos, newHeadYaw, newHeadPitch, spaceID, vehicleID);
    ...
    this->transformCommonToLocal(spaceID, vehicleID, localNewPos, localDir);
 ...
 *	@param	dTime		delta time.
 */
void Physics::doSeek(const MotionConstants& mc,
...
{
    BW_GUARD;
    Vector3 thisVelocity(dTime > 0.f ? thisDisplacement / dTime
                                     : Vector3(0, 0, 0));
    ...
    if (blendingFromFinish_ > timeNow) {
        float requiredSpeed =
          thisSpeed + LINEAR_SCALE(blendingFromSpeed_ - thisSpeed,
                                   float(blendingFromFinish_ - timeNow),
                                   smoothingTime_);

        if (thisSpeed <
            0.1f) { // go in the direction of blendingFromDirection_ if stopped.
                                 ...
                                 bool&                    posNotLoaded)
{
    ...
    // (overlapper) which hasn't loaded yet (to avoid falling through the
    // floor).
    if (pEmb) {
 ...
 */
float Physics::scaleUphillSpeed(float          maxDistance,
                                const Vector3& oldPos,
                                Vector3&       newPos)
{
    BW_GUARD;
    ...
    if (teleportPos_.x != FLT_MAX) {
        ...
        if (teleportDir_.x != FLT_MAX) {
            ...
            if (pEmb) {
                ...
                float  oldYaw =
                  atan2f(modelTransform.m[2][0], modelTransform.m[2][2]);
...
void Physics::weaponEmplacementStyleTick(double timeNow, double timeLast)
{
    ...
    this->transformLocalToCommon(spaceID, vehicleID, oldPos, unusedDir);
    ...
    if (seekState_ == LINE) {
        ...
        // Use current python interface to extract wanted head and seek
        // positions float difYaw = seek_[0] - mc.modelYaw; float difPitch =
        // seek_[1] - newHeadPitch;
 ...
 */
void Physics::transformCommonToLocal(SpaceID      spaceID,
                                     EntityID     vehicleID,
                                     Vector3&     pos,
 ...
 */
void Physics::transformLocalToCommon(SpaceID      spaceID,
                                     EntityID     vehicleID,
                                     Vector3&     pos,
                                     Direction3D& dir) const
{
...
void Physics::seekFinished(bool successfully)
{
    ...
    Script::call(pFunction,
                 Py_BuildValue("(i)", int(successfully)),
                 "Physics::seekFinished: ",
                 true);
}
 ...
 *	half the original velocity, or when the input maxDistance is reached.
 */
Vector3 Physics::movingForward(float              maxDistance,
                               Vector4ProviderPtr destinationPtr) const
{
    ...
    while ((oldPos - newPos).lengthSquared() < maxDistance * maxDistance) {
        ...
        if ((midPos - newPos).lengthSquared() < 0.1f * 0.1f)
            break;
    }
                                ...
                                float                  dTime) const
{
    ...
    if (g_debugDrawCollisions) {
        ...
        WorldTriangle sweepTriangle1Dest(
          v[0] + delta, v[1] + delta, v[2] + delta);
        WorldTriangle sweepTriangle2Dest(
          v[0] + delta, v[2] + delta, v[3] + delta);
    ...
    if (st.foundMin()) {
        ...
        if (norm.y < 1.f) {
            slope = 90.f - RAD_TO_DEG(
                             atanf(norm.y / Vector2(norm.x, norm.z).length()));
        }
                             ...
                             CollisionInfo&         ci) const
{
    ...
    // off.
    ci.widthVector_.set(
      mc.modelWidth * ci.flatDir_.z, 0.f, -mc.modelWidth * ci.flatDir_.x);
    ...
    // Make sure that we are at least this far away before we run into an
    // object.

    ci.frontDistance_ = 0.0f;
    if (!reverseCheck)
        ci.frontDistance_ =
          0.5f * sqrtf(sqr(mc.modelDepth) + sqr(mc.modelWidth));
 ...
 */
bool Physics::preventCollision(const Vector3&         oldPos,
                               Vector3&               newPos,
                               const MotionConstants& mc,
                               bool                   shouldSlide,
                               bool                   reverseCheck) const
{
    ...
    CollisionAdvance collisionAdvance(ci.bottomLeft_,
                                      ...
                                      ci.heightVector_,
                                      ci.flatDir_,
                                      ci.maxAdvance_ + ci.frontDistance_);
    ...
    WorldTriangle sweepTriangle(ci.bottomLeft_,
                                ci.bottomLeft_ + ci.heightVector_ +
                                  ci.halfWidthVector_,
    ...
    if (g_debugDrawCollisions) {
        ...
        WorldTriangle bufTri(sweepTriangle.v0() + ci.frontBuffer_,
                             sweepTriangle.v1() + ci.frontBuffer_,
                             sweepTriangle.v2() + ci.frontBuffer_);
        DebugDraw::triAdd(bufTri, col);
        WorldTriangle destTri(sweepTriangle.v0() + ci.delta_ + ci.frontBuffer_,
    ...
    if (shouldSlide && collided) {
        ...
        // collisionAdvance.hitTriangle().bounce( leftOver, g_collisionBounce
        // /*0.f*/ );
 ...
 */
float Physics::applyGravityAndBuoyancy(EntityID&              vehicleID,
                                       const Vector3&         oldPos,
                                       Vector3&               newPos,
                                       const MotionConstants& mc,
                                       float                  deltaTime) const
{
    ...
    float newFallingVelocity = oldStyleCollision_ || fall_
                                 ? fallingVelocity_ + (gravity_ * deltaTime)
                                 : 0;
    ...
    float dropDistance =
      (fallingVelocity_ + newFallingVelocity) / 2 * deltaTime;
    ...
    Vector3 corners[4];
    corners[0] =
      testPos - flatDir * modelDepth * 0.5f - perpFlatDir * modelWidth * 0.5f;
    corners[1] =
      testPos - flatDir * modelDepth * 0.5f + perpFlatDir * modelWidth * 0.5f;
    corners[2] =
      testPos + flatDir * modelDepth * 0.5f - perpFlatDir * modelWidth * 0.5f;
    ...
    CollisionAdvance collisionAdvance(corners[0],
                                      corners[1] - corners[0],
                                      corners[2] - corners[0],
                                      maxDrop > 0.f ? Vector3(0.f, -1.f, 0.f)
                                                    : Vector3(0.f, 1.f, 0.f),
                                      fabsf(maxDrop));
    ...
    if (g_debugDrawCollisions) {
        ...
        WorldTriangle dropOneDest(
          corners[0] + delta, corners[3] + delta, corners[1] + delta);
        WorldTriangle dropTwoDest(
          corners[0] + delta, corners[2] + delta, corners[3] + delta);
    ...
    if (vehicleID != 0 && fabsf(dropDistance) < collisionAdvance.advance()) {
...
bool Physics::handleAxisEvent(const AxisEvent& event)
{
    ...
    switch (event.axis()) {
        case AxisEvent::AXIS_LX:
            joystickPos_[0] = event.value();
            break;
        case AxisEvent::AXIS_LY:
            joystickPos_[1] = event.value();
            break;
        default:
            handled = false;
            break;
    ...
    if (pAxisEventNotifier_ &&
        joystickPos_.lengthSquared() >= axisEventNotifierThreshold_) {
...
bool Physics::teleportVehicle(PyEntityPtr pPyVehicle)
{
    ...
    teleportVehicle_ =
      pPyVehicle ? pPyVehicle->pEntity()->entityID() : NULL_ENTITY_ID;
...
PyObject* Physics::py_seek(PyObject* args)
{
    ...
    bool good = false;
    if (PyArg_ParseTuple(
          args, "O|ffO", &pyPose, &timeLimit, &heightTolerance, &pyCallback)) {
    ...
    if (!good) {
        ...
        if (pOldCallback != NULL) {
            ...
            Script::call(pOldCallback,
                         Py_BuildValue("(i)", 0),
                         "Physics::seekCancelled: ",
                         true);
        }
...
PyObject* getData(const Physics::MouseAxis data)
{
    ...
    char* str = NULL;
    switch (data) {
        case Physics::MA_MouseX:
            str = "MouseX";
            break;
        case Physics::MA_MouseY:
            str = "MouseY";
            break;
        case Physics::MA_Direction:
            str = "Direction";
            break;
        default:
            Py_RETURN_NONE;
 ...
 *	TODO: Make this work with the general PyObjectPlus-sponsored system
 */
int setData(PyObject* pObject, Physics::MouseAxis& rData, const char* varName)
{
    BW_GUARD;
...
void Physics::setIsMoving(bool newValue)
{
    ...
    if (newValue != moving_) {
        ...
        if (pIsMovingNotifier_) {
            ...
            Script::call(pIsMovingNotifier_.getObject(),
...
void Physics::hoverStyleTick(double timeNow, double timeLast)
{
    ...
    if (ripper_.zDrag_ < 1.f) {
        ...
        while (remainingTime > 0.f) {
            this->integrateHoverPosition(
              min(remainingTime, DESIRED_ELAPSED_TIME),
              position,
              yaw,
              pitch,
              roll);
...
{
  public:
    HoverFunction(const Physics::HoverSetting* settings,
                  const PlaneEq*               pPlaneEq,
                  float                        thrustA,
                  float                        brakeA,
                  const Vector3&               thrustAxisA,
                  Matrix34&                    matrixA)
      : settings_(settings)
      , pPlaneEq_(pPlaneEq)
      , thrust(thrustA)
    ...
    // const Vector3 thrustAxis(0.f, sin(hover_.thrustAngle()),
    // cos(hover_.thrustAngle()));
    ...
    Physics::Data operator()(float t, const Physics::Data& data) const
    {
        ...
        float groundHeight =
          pPlaneEq_ ? pPlaneEq_->y(data.position_.x, data.position_.z) : 0.f;
        float heightFromGround =
          data.position_.y - groundHeight - settings_->minHeightFromGround_;
        ...
        // Find the components of the momentum in each direction of the vehicle.
        // We want more drag on sideways momentum than forward momentum, for
        // example.

        float xMomentum =
          matrix.applyToUnitAxisVector(X_AXIS).dotProduct(data.momentum_);
        float yMomentum =
          matrix.applyToUnitAxisVector(Y_AXIS).dotProduct(data.momentum_);
        float zMomentum =
          matrix.applyToUnitAxisVector(Z_AXIS).dotProduct(data.momentum_);

        // Calculate the drag on each component of the momentum.
        const float X_DRAG =
          settings_->xDrag_; // Affects how much the vehicle slides
        const float Y_DRAG =
          yMomentum > 0.f
            ? settings_->yUpDrag_
            : settings_->yDownDrag_; // Have more drag when flying up.
        const float Z_DRAG =
          settings_->zDrag_ +
          settings_->brakeDrag_ * brake; // More drag if we have the brake on.
        ...
        const float heightMultiplier =
          (heightFromGround > settings_->thrustDropOffHeight_)
            ? powf(0.5f,
                   (heightFromGround - settings_->thrustDropOffHeight_) /
                     settings_->thrustDropOffRate_)
            : 1.f;

        rv.momentum_ +=
          0.5f * heightMultiplier * thrust * matrix.applyVector(thrustAxis);
...
void Physics::integrateHoverPosition(float    elapsedTime,
                                     Vector3& position,
                                     float&   yaw,
                                     float&   pitch,
                                     float&   roll)
...
//		pPlaneEq = &flat;
    ...
    */
    ...
    float sinRoll =
      transform.applyToUnitAxisVector(X_AXIS).dotProduct(planeEq.normal());
    float desiredRoll = -asinf(Math::clamp(-1.f, sinRoll, 1.f));

    float heightFromGround =
      location[1] - currGroundHeight - ripper_.minHeightFromGround_;

    if (heightFromGround < 0.01f)
        heightFromGround = 0.01f;

    const float heightFactor =
      1.f - 0.8f * heightFromGround / (2.0f + heightFromGround);
    ...
    if (currentSpeed > 0.f) {
        float turnFactor =
          (0.3f + 0.6f * currentSpeed / (currentSpeed + 20.f)) * -float(turn_);
        desiredRoll += turnFactor;
    }

    const float decayRate =
      0.3f + 0.4f * heightFromGround / (2.f + heightFromGround);
    pitch += (1 - powf(decayRate,
                       2 * elapsedTime /* * 20.f/(heightFromGround + 3) */)) *
             (desiredPitch - pitch);
    roll += (1 - powf(decayRate,
                      2 * elapsedTime /* * 20.f/(heightFromGround + 3)*/)) *
            (desiredRoll - roll);
    ...
    HoverFunction F(
      &ripper_, &planeEq, actualThrust, float(brake_), thrustAxis_, transform);
    ...
    if ((oldStyleCollision_ && collide_) ||
        (!oldStyleCollision_ && (collideObjects_ || collideTerrain_))) {
        Vector3 dirv(data_.position_ - oldPos);
        dirv.normalise(); // start ray from 10cm back from dir vector
        if (dirv.lengthSquared() < 0.9f)
            dirv = Vector3(0, -1, 0);
        ...
        if (cft.found_) {
            ...
            if (pos.y < planeHeight)
            {
                ...
                data_.momentum_ -=
                    (1 + elasticity) * normal.dotProduct( data_.momentum_ ) *
            normal;
            }
            */
            ...
            if (!collided_) {
                ...
                collisionPosition_ =
                  (cft.hit_.v0() + cft.hit_.v1() + cft.hit_.v2()) / 3.f;
