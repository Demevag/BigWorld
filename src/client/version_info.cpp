...
// typedef HRESULT(WINAPI * DIRECTINPUTCREATE)( HINSTANCE, DWORD,
// LPDIRECTINPUT*,
//                                              IUnknown* );
...
void VersionInfo::queryAll(void)
{
    queryOS();
    queryDX();
}
...
size_t VersionInfo::totalPhysicalMemory(void) const
{
    queryMemory();
    return memoryStatus_.dwTotalPhys;
}

size_t VersionInfo::availablePhysicalMemory(void) const
{
    queryMemory();
    return memoryStatus_.dwAvailPhys;
}

size_t VersionInfo::totalVirtualMemory(void) const
{
    queryMemory();
    return memoryStatus_.dwTotalVirtual;
}

size_t VersionInfo::availableVirtualMemory(void) const
{
    queryMemory();
    return memoryStatus_.dwAvailVirtual;
}

size_t VersionInfo::totalPagingFile(void) const
{
    queryMemory();
    return memoryStatus_.dwTotalPageFile;
}

size_t VersionInfo::availablePagingFile(void) const
{
    queryMemory();
    return memoryStatus_.dwAvailPageFile;
}

uint VersionInfo::memoryLoad(void) const
{
    queryMemory();
...
void VersionInfo::queryMemory(void) const
{
    ...
    if (!loadedDll_) {
        ...
        vi_getProcessMemoryInfo_ = (VI_GETPROCESSMEMORYINFO)GetProcAddress(
          loadedDll_, VI_GETPROCESSMEMORYINFO_NAME);
    }
    ...
    if ((tickCount - lastMemoryCheck_) > 1000) {
        ...
        if (vi_getProcessMemoryInfo_) {
            vi_getProcessMemoryInfo_(::GetCurrentProcess(),
                                     &processMemoryStatus_,
                                     sizeof(processMemoryStatus_));
 ...
 */
void VersionInfo::queryHardware(void) {}
...
//    DIRECTINPUTCREATE    DirectInputCreate  = NULL;
    ...
    DirectDrawCreate = (DIRECTDRAWCREATE)GetProcAddress( hDDrawDLL,
"DirectDrawCreate" ); if( DirectDrawCreate == NULL )
    {
        dwDXVersion = 0;
        FreeLibrary( hDDrawDLL );
        OutputDebugString( "VersionInfo: Couldn't GetProcAddress
DirectDrawCreate\r\n" ); return dwDXVersion;
    }
    ...
    ); FreeLibrary( hDDrawDLL ); return dwDXVersion;
    }
    ...
    if ( Direct3DCreate8 )
    {
        ...
        if( pD3D != NULL)
        {
            ...
            if ( SUCCEEDED( pD3D->GetAdapterIdentifier( D3DADAPTER_DEFAULT,
    D3DENUM_NO_WHQL_LEVEL, &adapterInfo ) ) )
            {
                adapterDriverMajorVer_ = LOWORD(
    adapterInfo.DriverVersion.HighPart ); adapterDriverMinorVer_ = HIWORD(
    adapterInfo.DriverVersion.LowPart ); adapterDriver_ = adapterInfo.Driver;
