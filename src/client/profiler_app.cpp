...
void ProfilerApp::tick(float /* dGameTime */, float dRenderTime)
{
    ...
    // Gather stats
    filteredDTime_ =
      (DAMPING_RATIO * filteredDTime_) + ((1.f - DAMPING_RATIO) * dRenderTime);
...
bool ProfilerFlyThroughSpline::update(float dTime, const FlyThroughNodes& nodes)
{
    BW_GUARD;
    ...
    if (edgePosition_ >= 1.f) {
        ...
        edgesPerSecond_ =
          SPLINE_SPEED / ((p0_ - p1_).length() + 2.f * (p1_ - p2_).length() +
                          (p2_ - p3_).length());
    }
    ...
    Vector3     pos = 0.5 * ((2 * p1_) + (-p0_ + p2_) * edgePosition_ +
                         (2 * p0_ - 5 * p1_ + 4 * p2_ - p3_) * t2 +
                         (-p0_ + 3 * p1_ - 3 * p2_ + p3_) * t3);

    Vector3 rot = 0.5 * ((2 * r1_) + (-r0_ + r2_) * edgePosition_ +
...
void profilerFinishedWithStats()
{
    ...
    float frameTimeDifferences;
    float frameTimePow2Differences;
    calculatePerformance(frameTimesAvg,
                         frameTimesStdDev,
    ...
    sentList.push_back(
      PairStringFloat("Total Frames", (float)s_frameTimes.size()));
    sentList.push_back(
      PairStringFloat("Milli per frame", 1000.0f * frameTimesAvg));
    sentList.push_back(PairStringFloat("FPS", 1.0f / frameTimesAvg));
    sentList.push_back(
      PairStringFloat("Standard Deviation", 1000.0f * frameTimesStdDev));
    sentList.push_back(
      PairStringFloat("Frame differences", 1000.0f * frameTimeDifferences));
    sentList.push_back(PairStringFloat("Frame differences Pow 2",
                                       1000.0f * frameTimePow2Differences));
...
static void reportCSVStats(FILE* file, double elapsedTime)
{
    ...
    fprintf(file,
            "%g, %d, %d, %lu, %d, %f, %f, %f, %f\n",
            elapsedTime,
            curStats.currentBytes_,
            curStats.currentAllocs_,
            pmc.PagefileUsage,
            s_frameTimes.size(),
            s_frameTimesTotal * 1000.0f,
            frameTimesAvg * 1000.0f,
            frameTimesStdDev * 1000.0f,
            1.0f / frameTimesAvg);
#endif // ENABLE_MEMORY_DEBUG
}
...
static void soakUpdate(double elapsedTime)
{
    ...
    if ((elapsedTime - s_prevValidateTime) > VALIDATE_TIME) {
        ...
        if (elapsedTime >= s_soakTime) {
            ...
            float secondsPerFrame = (float)elapsedTime / (float)s_numSoakFrames;
            ...
            /* TODO: Get the approximate peak based on our sampling from the app
             and
             * put this back

                        if (s_settled &&
                            (10 * curStats.peakBytes_) > (11 *
             s_settledStats.peakBytes_))
                        {
            ...
            */
            ListStringFloat sentList;
            sentList.push_back(
              PairStringFloat("Total Frames", (float)s_numSoakFrames));
            sentList.push_back(
              PairStringFloat("Milli per frame", 1000.0f * secondsPerFrame));
...
PyObject* py_setProfilerHistory(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(args,
                          "s|s:BigWorld.setProfilerHistory",
                          &historyFileName,
                          &threadName)) {
        return NULL;
    }
