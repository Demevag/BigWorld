...
bool FreeCamera::handleMouseEvent(const MouseEvent& event)
{
    ...
    CameraControl::pitchVelocity(deltaPitch + CameraControl::pitchVelocity());
...
void FreeCamera::update(float dTime)
{
    ...
    if (invViewProvider_) {
        ...
        view_.postMultiply(CameraControl::calculateDeltaMatrix(dTime));

        // Update the camera matrix.
        invView_.invert(view_);
        Vector3 t = invView_.applyToOrigin();
        invView_.postRotateY(CameraControl::yawVelocity() * dTime *
                             CameraControl::orientationDamper());
        invView_.preRotateX(CameraControl::pitchVelocity() * dTime *
                            CameraControl::orientationDamper());
        ...
        if (halfLife > 0.0) {
            CameraControl::yawVelocity(
              Math::decay(CameraControl::yawVelocity(), 0.f, halfLife, dTime));
