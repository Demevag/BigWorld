...
void SimpleSpeedProvider::adjustDirection(float  dTime,
                                          Angle& pitch,
                                          Angle& yaw,
                                          Angle& roll)
{
  ...
  , elapsedTimeForLookSpring_(0.0)
{
    ...
    if (section) {
        ...
        this->pitch(
          DEG_TO_RAD(section->readFloat("pitch", RAD_TO_DEG(this->pitch()))));
        this->yaw(
          DEG_TO_RAD(section->readFloat("yaw", RAD_TO_DEG(this->yaw()))));
        direction_.setPitchYaw(-this->pitch(), this->yaw());
        refreshDirection_ = false;

        // Read in general behaviour defaults.
        this->invertVerticalMovement(section->readBool(
          "invertVerticalMovement", this->invertVerticalMovement()));

        // Read in mouse behaviour defaults.
        this->mouseSensitivity(
          section->readFloat("mouseSensitivity", this->mouseSensitivity()));
        this->mouseHVBias(
          section->readFloat("mouseHVBias", this->mouseHVBias()));

        // Read in cursor limiting defaults.
        this->maxPitch(DEG_TO_RAD(
          section->readFloat("maxPitch", RAD_TO_DEG(this->maxPitch()))));
        this->minPitch(DEG_TO_RAD(
          section->readFloat("minPitch", RAD_TO_DEG(this->minPitch()))));

        // Read in look-spring behaviour.
        this->lookSpring(section->readBool("lookSpring", this->lookSpring()));
        this->lookSpringRate(
          section->readFloat("lookSpringRate", this->lookSpringRate()));
        this->lookSpringIdleTime(section->readDouble(
          "lookSpringIdleTime", this->lookSpringIdleTime()));
        this->lookSpringOnMove(
          section->readBool("lookSpringOnMove", this->lookSpringOnMove()));
    }
}
...
{
    BW_GUARD;
    return MatrixProviderPtr(new DCMProv(*this), PyObjectPtr::STEAL_REFERENCE);
}
...
void DirectionCursor::tick(float deltaTime)
{
    ...
    if (this->lookSpring()) {
        ...
        if (elapsedTimeForLookSpring_ > this->lookSpringIdleTime() ||
            ...
            pitch_ =
              Math::decay(pitch_, 0.f, this->lookSpringRate(), deltaTime);
    ...
    if (this->roll() != 0.0f) {
        ...
        this->roll(r > 0.f ? max(0.f, r - RADIANS_PER_SECOND * deltaTime)
...
bool DirectionCursor::handleMouseEvent(const MouseEvent& event)
{
    ...
    float msens =
      this->mouseSensitivity() * Moo::rc().camera().fov() / DEG_TO_RAD(60.f);
    ...
    this->pitch(Math::clamp(this->minPitch(), this->pitch(), this->maxPitch()));
