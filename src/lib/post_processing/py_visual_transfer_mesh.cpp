...
namespace PostProcessing {
     ...
     */
    PY_FACTORY_NAMED(PyVisualTransferMesh,
                     "VisualTransferMesh",
                     _PostProcessing)
...
#ifdef EDITOR_ENABLED
    void PyVisualTransferMesh::edEdit(GeneralEditor*       editor,
                                      FilterChangeCallback pCallback)
    {
        ...
        STATIC_LOCALISE_NAME(
          s_resource,
          "WORLDEDITOR/GUI/PAGE_POST_PROCESSING/PY_VISUALXFERMESH/RESOURCE");
        ...
        resourceProp->fileFilter(Name(
          "Visual files(*.visual)|*.visual|Visual files(*.visual)|*.visual||"));

        STATIC_LOCALISE_NAME(s_resourceDesc,
                             "WORLDEDITOR/GUI/PAGE_POST_PROCESSING/"
                             "PY_VISUALXFERMESH/RESOURCE_DESC");
    ...
    PyObject* PyVisualTransferMesh::pyNew(PyObject* args)
    {
        ...
        if (!PyArg_ParseTuple(args, "|s", &id)) {
            PyErr_SetString(PyExc_TypeError,
                            "PostProcessing.VisualTransferMesh: "
