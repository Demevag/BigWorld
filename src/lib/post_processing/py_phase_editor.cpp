    ...
    class PyPhaseFilterQuadTypeUndo : public UndoRedo::Operation
    {
      public:
        PyPhaseFilterQuadTypeUndo(PostProcessing::PyPhasePtr pPhase)
          : UndoRedo::Operation(
              size_t(typeid(PyPhaseFilterQuadTypeUndo).name()))
          , pPhase_(pPhase)
        {
...
namespace PostProcessing {
    ...
    BW::string PyPhaseEditor::exposedToScriptName(
    ...
    StringProxyPtr PyPhaseEditor::textureProxy(BaseMaterialProxyPtr proxy,
                                               ...
                                               const BW::string& descName) const
    {
        return new PyPhaseTextureProxy(proxy,
                                       getFn,
                                       setFn,
                                       descName,
                                       pPhase_,
                                       pPhase_->material()->pMaterial());
    }
    ...
    IntProxyPtr PyPhaseEditor::intProxy(BaseMaterialProxyPtr proxy,
                                        ...
                                        const BW::string&    descName) const
    {
        return new PyPhaseIntProxy(
          proxy, getFn, setFn, rangeFn, descName, pPhase_);
    }
    ...
    FloatProxyPtr PyPhaseEditor::floatProxy(BaseMaterialProxyPtr proxy,
                                            ...
                                            const BW::string&    descName) const
    {
        return new PyPhaseFloatProxy(
          proxy, getFn, setFn, rangeFn, descName, pPhase_);
    }

    Vector4ProxyPtr PyPhaseEditor::vector4Proxy(
    ...
    BW::string PyPhaseEditor::getOutputRenderTarget() const
    {
        ...
        PyObjectPtr pyRT(PyObject_GetAttrString(pPhase_, "renderTarget"),
    ...
    BW::string PyPhaseEditor::getMaterialFx() const
    {
        ...
        if (material && material->pMaterial() &&
    ...
    bool PyPhaseEditor::setMaterialFx(const BW::string& materialFx)
    {
        ...
        if (effect->initFromEffect(materialFx)) {
            pPhase_->material(PyMaterialPtr(new PyMaterial(effect),
                                            PyObjectPtr::STEAL_REFERENCE));
            ok = true;
    ...
    bool PyPhaseEditor::setFilterQuadType(const int& filterQuadType,
                                          bool       addBarrier)
    {
        int idx = 0;

        FilterQuadCreators::iterator it =
          FilterQuadCreators::instance().begin();
        ...
        if (it != FilterQuadCreators::instance().end()) {
            ...
            if (pNewFilterQuad) {
                UndoRedo::instance().add(
                  new PyPhaseFilterQuadTypeUndo(pPhase_));
        ...
        if (!ok) {
            ERROR_MSG("Could not change the post-processing phase's filter "
                      "quad type.\n");
        }
