 ...
 */
PY_FACTORY_NAMED(OAVersionStructWrapper,
                 "CreateOAVersionStructWrapper",
                 OpenAutomateWrapper)

OAVersionStructWrapper::OAVersionStructWrapper(PyTypeObject* pType)
  : PyObjectPlus(pType)
  , major_(0)
  , minor_(0)
  , custom_(0)
 ...
 *
 *  Contains the current benchmark name (invalid only for run benchmark
 * commands)
 *
 *  @type String
 */
 ...
 */
PY_FACTORY_NAMED(OACommandWrapper,
                 "CreateOACommandWrapper",
                 OpenAutomateWrapper)

OACommandWrapper::OACommandWrapper(PyTypeObject* pType)
  : PyObjectPlus(pType)
{
 ...
 */
PY_FACTORY_NAMED(OANamedOptionWrapper,
                 "CreateOANamedOptionWrapper",
                 OpenAutomateWrapper)

OANamedOptionWrapper::OANamedOptionWrapper(PyTypeObject* pType)
  : PyObjectPlus(pType)
{
    oaInitOption(&internalOption_);
}

OANamedOptionWrapper::OANamedOptionWrapper(oaNamedOptionStruct* option,
                                           PyTypeObject*        pType)
  : PyObjectPlus(pType)
...
{
    BWOpenAutomate::eOAOptionDataType temp =
      (BWOpenAutomate::eOAOptionDataType)internalOption_.DataType;
...
PyObject* OANamedOptionWrapper::pyGet_enumValue()
{
    ...
    PyErr_SetString(PyExc_TypeError, "Failed geting the type of an option");
    return NULL;
}
...
{
    BWOpenAutomate::eOAComparisonOpType temp =
      (BWOpenAutomate::eOAComparisonOpType)
        internalOption_.Dependency.ComparisonOp;
    return Script::getData(temp);
}

/*~ function OANamedOptionWrapper.setComparison
 *
 *	Set the value compared to the dependent parent option in order to see if
 *this option is actionve
 *
 ...
 *this option is actionve
 */
void OANamedOptionWrapper::setComparison(
  BWOpenAutomate::eOAOptionDataType optionType,
  PyObjectPtr                       comparisonValArg)
{
    ...
    switch (optionType) {
        ...
        case BWOpenAutomate::BWOA_TYPE_STRING: {
            ...
            internalOption_.Dependency.ComparisonVal.String =
              (oaString)comparisonVal_.c_str();
            break;
        }
        ...
        case BWOpenAutomate::BWOA_TYPE_ENUM: {
            Script::setData(comparisonValArg.get(), comparisonVal_, "");
            internalOption_.Dependency.ComparisonVal.Enum =
              (oaString)comparisonVal_.c_str();
            break;
        }
 ...
 *	@param		version an OAVersionStructWrapper which will receive the
 *OpenAutomate version information.
 *	@param		bwTestMode bool marking whether we should use the open automate
 *testing
 *
 ...
 *  This function initialises the open automate library.
 */
bool bwOAInit(const BW::string&                    initStr,
              SmartPointer<OAVersionStructWrapper> version,
              bool                                 bwTestMode)
{
    // first make sure we are registered
    bwOARegisterApp();
    // mark that exceptions will cause exit with an error code and not error
    // message
...
PY_AUTO_MODULE_FUNCTION(
  RETDATA,
  bwOAInit,
  ARG(BW::string, ARG(SmartPointer<OAVersionStructWrapper>, ARG(bool, END))),
  OpenAutomateWrapper)
 ...
 *
 *	@param		command the OACommandWrapper object which will get the command
 *additional info.
 *
 ...
 *  This function gets the next OpenAutomate command for the application to run.
 */
BWOpenAutomate::eOACommandType bwOAGetNextCommand(
  SmartPointer<OACommandWrapper> command)
{
    return (BWOpenAutomate::eOACommandType)oaGetNextCommand(
      &command->internalCommand_);
}
PY_AUTO_MODULE_FUNCTION(RETDATA,
                        bwOAGetNextCommand,
                        ARG(SmartPointer<OACommandWrapper>, END),
                        OpenAutomateWrapper)

/*~ function OpenAutomateWrapper.bwOAGetNextOption
 *
 *  This function gets the Option sent by OpenAutomate. The application should
 *set this option. based on the details available in the OANamedOptionWrapper
 *
...
PY_AUTO_MODULE_FUNCTION(RETVOID,
                        bwOAAddOption,
                        ARG(SmartPointer<OANamedOptionWrapper>, END),
                        OpenAutomateWrapper)

/*~ function OpenAutomateWrapper.bwOAAddOptionvValue
 *
 *  This function is used to inform OpenAutomate on the current active value for
 *a specific option
 *
 *	@param	name a String specifying the option name
 *	@param	valueType a String specifying the option value type
 *	@param	value the option value (string, int, float, enum value and bool are
 *supported)
 */
/**
 *  This function is used to inform OpenAutomate on the current active value for
    ...
    switch (valueType) {
        case BWOpenAutomate::BWOA_TYPE_STRING: {
            ...
            if (ret != 0) {
                ...
                oaAddOptionValue(
                  name.c_str(), (oaOptionDataType)valueType, &valueTemp);
            }
            break;
        }
        case BWOpenAutomate::BWOA_TYPE_INT: {
 ...
 *
 *  This function is used to inform OpenAutomate on a benchmark (based on the
 *benchmark name)
 *
 ...
 *
 *  This function is used to register an application as an open automate
 * supporting app
 */
/**
 *  This function is used to register an application as an open automate
...
bool bwOARegisterApp()
{
    ...
    for (int i = 0; i < allApps->NumAppBuilds; ++i) {
        if (memcmp(allApps->AppBuilds[i]->Id,
                   &buildId,
                   sizeof(oauAppBuildId)) == 0 &&
            memcmp(allApps->AppBuilds[i]->Info,
                   &appInfo,
                   sizeof(oauAppBuildInfo)) == 0) {
            alreadyRegistered = true;
        }
    }
    if (!alreadyRegistered) {
        oauRegisterAppBuild(OAU_REGISTER_REGISTRY_SYSTEM, &buildId, &appInfo);
    }
    return true;
}
 ...
 *	@param		name a string containing the result name (should be unique
 *foreach test)
...
void bwOAAddResultValue(const BW::string& name, float value)
{
    ...
    oaAddResultValue((oaChar*)name.c_str(), OA_TYPE_FLOAT, &usedValue);
}
PY_AUTO_MODULE_FUNCTION(RETVOID,
                        bwOAAddResultValue,
 ...
 *
 *  This function is called before calling quit to make sure quit doesn't fail
 * due to singeltons issues
 */
/**
 *  This function is called before calling quit to make sure quit doesn't fail
 * due to singeltons issues
