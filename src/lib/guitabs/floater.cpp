...
namespace GUITABS {
      ...
      , lastRollupSize_(0)
    {
        ...
        Create(0,
               L"",
               WS_POPUP | WS_THICKFRAME | MFS_SYNCACTIVE | WS_CAPTION |
                 WS_SYSMENU,
               CRect(0, 0, 100, 100),
               parentWnd,
               0);
     ...
     *	this floater window.  It can potentially adjust the position and the
     *size.
     *
    ...
    void Floater::validatePos(int& posX, int& posY, int& width, int& height)
    {
        ...
        HMONITOR mon =
          MonitorFromRect(CRect(posX, posY, posX + width, posY + height),
    ...
    /**
     *	This method returns the last size the floater had before being rolled
     *up.
     *
    ...
    /**
     *	This method adjusts the size of the floater to try to fit all its
     *internal panels.
     *
     *	@param rollUp	True to roll up the whole floater, false for normal
     *resize.
     */
    ...
    /**
     *	This MFC method overrides the default behaviour of "delete this", so we
     *can rely on smartpointers for objects of this class.
     */
    void Floater::PostNcDestroy()
    {
        // do nothing. The standard behaviour is "delete this", which conflicts
        // with smartpointers.
    }
    ...
    /**
     *	This MFC method is called as the window is resized in order to update
     *the sizes of the internal panels.
     *
    ...
    void Floater::OnSizing(UINT nSide, LPRECT rect)
    {
        ...
        if (rect && (rect->bottom - rect->top - 1 < minH ||
                     (dockTreeRoot_ && !dockTreeRoot_->isExpanded())))
            if (nSide == WMSZ_TOP || nSide == WMSZ_TOPLEFT ||
                nSide == WMSZ_TOPRIGHT)
