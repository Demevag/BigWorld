...
namespace GUITABS {
    ...
    /**
     *	This abstract class must be inherited by factories that create the
     *desired dialogs, property sheets, and other kinds of windows to be
     *inserted as tearoff panels. Since this class is an interface, there should
     *be no problem with multiple inheritance. IMPORTANT NOTES:
     ...
     *objects are deallocated by the Manager.
 ...
 *		BW::string getContentID() { return TestContent::contentID; }  // see the
 *sample for Content in content.hpp
 *	};
 *
 *
 ...
 *	{
 *		CDialogContent* p = new CDialogContent(); // CDialogContent inherits
 *from Content and CDialog p->Create( CDialogContent::IDD );  // the resource ID
 *is asumed to be defined in a const inside CDialogContent in this example
 *		return p;
 *	};
 *
 ...
 *	use your created window. GUITABS only creates a window if the GetSafeHwnd()
 *member is not a valid window handle ( it's tested using Win32's ::IsWindow()
 *).
 *
 *********************************************************
 *
 *  Sample 4: factory registration call in the cpp file that creates the panels,
 *etc:
 *
