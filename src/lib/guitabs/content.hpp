 ...
 *  methods.
 */
#define IMPLEMENT_ROOT_CONTENT_NOLOAD(                                         \
  shortName, longName, width, height, icon)                                    \
  public:                                                                      \
 ...
 *  See sample at the end of the file.
 */
#define IMPLEMENT_CLONE_CLOSE_CONTENT(                                         \
  shortName, longName, width, height, icon)                                    \
    IMPLEMENT_ROOT_CONTENT(shortName, longName, width, height, icon)           \
    bool isClonable()                                                          \
    {                                                                          \
...
namespace GUITABS {
     ...
     *  Make sure you override the PostNcDestroy() method for CView and
     *CFrameWnd derived classes. In these classes, the default behaviour of this
     *method is to do a "delete this", which is incompatible to the way Content
     *objects are deallocated by the Manager. For more information, please see
     *manager.hpp and content_factory.hpp
     */
    class Content : public ReferenceCount
    {
      public:
         ...
         *  @param section data section pointer where the data should be loaded
         * from.
         ...
         *  @param section data section pointer where the data should be saved
         * to.
         *  @return true if successful, false otherwise.
         */
        ...
        /**
         *  The deriving class should return here a short display string that
         * can displayed in the small tabs inside panels.
         ...
         *  @return true if the class supports cloning through the clone()
        ...
        /**
         *  This method is called when the close button of the panel containing
         *the content is clicked. If the this function returns true, the Manager
         *will look for other content classes with the same ID. If there are,
         *this content will be permanently removed, but if not, it will be
         *hidden, and if the application wants to completely destroy it, it can
         *later remove the panel/tab.
         *  @param isLastContent true if it's the last content with this ID,
         *false if not.
         *  @return true if the Content wishes to be removed/hidden, false if
         *not.
         */
 ...
 *	{
 *		IMPLEMENT_BASIC_CONTENT( L"A Test!", L"A Test Content Panel Class!",
 *265, 430, NULL ) public:
 *
 ...
 *		{
 *			if ( isLastContent )
 *				return GUITABS::OnCloseAction::CONTENT_HIDE; // if it's the last
 *content of this class, keep it hidden. else return
 *GUITABS::OnCloseAction::CONTENT_DESTROY; // if it's not the last, then just
 *delete it.
 *		}
 *		void handleRightClick( int x, int y ) { }
 *		void OnOK() { }
 *		void OnCancel() { }
 *		void PostNcDestroy() { }  // VERY IMPORTANT: required for CView and
 *CFrameWnd derived windows, to avoid the object doing a "delete this"
 *	};
 *
 *	const BW::string TestContent::contentID = "TestContentID";
 *
