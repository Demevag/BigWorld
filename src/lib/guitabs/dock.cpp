...
namespace GUITABS {
     ...
     *						dock's root splitter. This is the 3D view in
     *WE/PE/ME.
     */
    ...
    Dock::~Dock()
    {
        ...
        // dettach the mainView_ window from the dock tree, and put it as a
        // child of mainFrame_. Also sets the active view so we don't get any
        // strange MFC asserts on exit.
        if (IsWindow(mainView_->GetSafeHwnd()) &&
            IsWindow(mainFrame_->GetSafeHwnd())) {
        ...
        for (FloaterItr i = floaterList_.begin(); i != floaterList_.end();
             ++i) {
    ...
    DockNodePtr Dock::getNodeByPoint(int x, int y)
    {
        ...
        for (FloaterItr i = floaterList_.begin();
             i != floaterList_.end() && !ret;
             ++i) {
            if ((*i)->IsWindowVisible() &&
                ::IsChild((*i)->getCWnd()->GetSafeHwnd(),
                          ::WindowFromPoint(CPoint(x, y)))) {
    ...
    /**
     *	This method moves a tab from a panel and docks it according to the
     *insertAt param.  For example, if insertAt is LEFT, then the tab is
     *inserted as a panel to the left of destPanel.
     *
     ...
     *	@param dstX		Destination X coordinate, used for when floating the
     *panel.
     *	@param dstY		Destination Y coordinate, used for when floating the
     *panel.
     */
    void Dock::dockTab(PanelPtr panel,
                       TabPtr   tab,
                       CWnd*    destPanel,
                       InsertAt insertAt,
                       int      srcX,
                       ...
                       int      dstY)
    {
        ...
        newPanel->SetWindowPos(
          0, rect.left, rect.top, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
    ...
    /**
     *	This method docks a panel into another panel, or floats it if insertAt
     *is FLOATING, or inserts it as a tab if insertAt is TAB.
     *
     ...
     *panel.
     */
    void Dock::dockPanel(PanelPtr panel,
                         CWnd*    destPanel,
                         InsertAt insertAt,
                         int      srcX,
                         int      srcY,
    ...
    /**
     *	This method does all the hard work to make a panel float (a popup panel
     *as opposed to a docked panel).
     *
     ...
     *panel.
     */
    void Dock::floatPanel(PanelPtr panel,
                          int      srcX,
                          int      srcY,
                          int      dstX,
                          int      dstY)
    {
        ...
        if (panel->isFloating()) {
            ...
            if (floater && floater->getRootNode()->isLeaf()) {
                floater->SetWindowPos(
                  0, dstX, dstY, 0, 0, SWP_NOZORDER | SWP_NOSIZE);
     ...
     *	@param destPanel	Panel to use for the insertAt operation.  Should
     *never be FLOATING for this method.
     *	@param insertAt		How to insert the tab.
     */
    void Dock::insertPanelIntoPanel(PanelPtr panel,
                                    CWnd*    destPanel,
                                    InsertAt insertAt)
    {
        BW_GUARD;
    ...
    /**
     *	This method returns a node and its parent by the node's CWnd, no matter
     *if it's docked or floating.
     *
     ...
     */
    bool Dock::getNodeByWnd(CWnd*        ptr,
    ...
    void Dock::removeNodeByWnd(CWnd* ptr)
    {
        ...
        childNode->getCWnd()->SetWindowPos(
          0, 0, 0, 0, 0, SWP_NOZORDER | SWP_NOSIZE);
     ...
     */
    PanelPtr Dock::insertPanel(const BW::wstring& contentID,
    ...
    /**
     *	This method creates the appropriate dock node type for the specified
     *data section.
     *
     *	@param section	Data section containing the node to create.
     *	@return		The appropriate dock node type for the specified data
     *section.
     */
    ...
    /**
     *	This method saves the layout tree of docked and floating panels to a
     *data section.
     *
     ...
     *	@param index	If multiple contents of this type exist, get the
     *occurrence number "index".
    ...
    /**
     *	This method returns the leaf nodes under a node's subtree, or the same
     *node if the node is itself a leaf.
     *
    ...
    /**
     *	This method stores information about the position and layout of panels
     *for later use.  This is useful when hidding and showing panels, keeping
     *track of where the panel was before hiding so when showing it, it shows up
    ...
    bool Dock::buildPanelPosList(bool docked, DockNodePtr node, PanelPtr panel)
    {
        ...
        if (inLeft) {
            ...
            for (BW::vector<DockNodePtr>::iterator i = leaves.begin();
                 i != leaves.end();
                 ++i) {
                InsertAt ins;
            ...
            for (BW::vector<DockNodePtr>::reverse_iterator i = leaves.rbegin();
                 i != leaves.rend();
                 ++i) {
                InsertAt ins;
        ...
        while (panel->getNextPos(docked, pos)) {
            if (panel->isFloating()) {
                for (FloaterItr i = floaterList_.begin();
                    ...
                    DockNodePtr node;
                    DockNodePtr parent;
                    (*i)->getRootNode()->getNodeByWnd(
                      pos.destPanel, node, parent);
                    if (node) {
                        insertPanelIntoPanel(
                          panel, pos.destPanel, pos.insertAt);
    ...
    void Dock::toggleTabPos(PanelPtr panel, TabPtr tab)
    {
        ...
        // call restore to put this panel in the previous state of the original
        // panel
    ...
    void Dock::destroyFloater(FloaterPtr floater)
    {
        ...
        if (IsWindow(floater->GetSafeHwnd())) {
            ...
            for (PanelItr i = panelList_.begin(); i != panelList_.end(); ++i) {
                DockNodePtr node;
                DockNodePtr parent;
                floater->getRootNode()->getNodeByWnd(
                  (*i)->getCWnd(), node, parent);
                if (node) {
     ...
     *	@param lParam	Windows message LPARAM.
     */
    void Dock::sendMessage(const BW::wstring& contentID,
                           UINT               msg,
                           WPARAM             wParam,
                           LPARAM             lParam)
    {
    ...
    /**
     *	This method returns the number of instances of the content type
     *contentID in the dock.
     *
     *	@param contentID	Content id of the desired panel.
    ...
    /**
     *	This method detaches a tab from a panel and puts it into its own new
     *panel.
     *
     *	@param panel	Panel where the tab currently is.
