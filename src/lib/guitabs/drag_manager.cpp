...
namespace GUITABS {
    ...
    /**
     *	This method is called when the user starts dragging a panel. It
     *initialises the drag and drop operation and enters into the drag loop.
     *
    ...
    void DragManager::startDrag(int x, int y, PanelPtr panel, TabPtr tab)
    {
        ...
        FloaterPtr floater =
          Manager::instance().dock()->getFloaterByWnd(panel->getCWnd());
    ...
    void DragManager::prepareDrag()
    {
        ...
        //		dc_ = wnd->GetDCEx(NULL,
        // DCX_WINDOW|DCX_CACHE|DCX_LOCKWINDOWUPDATE); 	else
    ...
    void DragManager::dragLoop()
    {
        ...
        // This flag is used to properly validate when a panel is drag&dropped
        // too fast, or when a drag operation is interrupted by a modal dialog.
                ...
                case WM_MOUSEMOVE:
                    if (isDragging_) {
                        if (GetAsyncKeyState(VK_LBUTTON) < 0)
                            ...
                            // Mouse button is not down for some reason. It
                            // could be because it was drag&dropped too fast, or
                            // because the drag was interrupted, maybe by a
                            // modal dialog. Put a flag to process the next
                            // message and then end, in case the last message is
                            // actually the LBUTTONUP of a fast drag.
                            lastMessage = true;
                            continue;
                        }
                    }
    ...
    /**
     *	This method is called to update the drop target and give visual feedback
     *to the user about the potential resulting position for the panel.
     *
     *	@param x	Mouse X position.
     *	@param y	Mouse Y position.
     */
    void DragManager::updateDrag(int x, int y)
    {
        ...
        if (abs(downX_ - x) > MIN_DRAG_DISTANCE ||
            abs(downY_ - y) > MIN_DRAG_DISTANCE) {
            ...
            if (node && node->getCWnd() != panel_->getCWnd() && ins == TAB) {
                // dropping onto a different panel as a tab, setup the
                // destination panel and tab position
                PanelPtr curDestPanel =
                  Manager::instance().dock()->getPanelByWnd(node->getCWnd());
                if (destPanel_ != curDestPanel) {
                    // only change the destPanel if it has changed to avoid
                    // flicker
                    ...
                    if (!!destPanel_) {
                        if (tab_ || panel_->visibleTabCount() == 1) {
                            ...
                            tempTab_->setLabel(
                              Localise(L"GUITABS/DRAG_MANAGER/MANY_TABS"));
                ...
                // not dropping onto a different panel as a tab, remove tab
                // position
                if (!!destPanel_) {
                    drawDragRect(lastIns_, lastRect_, lastSize_);
                    destPanel_->removeTempTab();
            ...
            if (ins == TAB && node) {
                ...
                if (node->getCWnd() == panel_->getCWnd() && tab_) {
                    ...
                    // inserting as a tab in another panel, so update tab
                    // position
                    if (!!destPanel_ &&
                        (tab_ || panel_->visibleTabCount() == 1))
    ...
    /**
     *	This method is called when the user successfully finishes dragging a
     *panel.
     *
     *	@param x	Mouse X position.
     *	@param y	Mouse Y position.
    ...
    void DragManager::endDrag(int x, int y)
    {
        ...
        if (abs(downX_ - x) > MIN_DRAG_DISTANCE ||
            ...
            if (node && node->getCWnd() == panel->getCWnd() && tab &&
                ins == TAB)
                return;
            ...
            if (tab)
                Manager::instance().dock()->dockTab(
                  panel, tab, ptr, ins, startX_, startY_, x, y);
            else
                Manager::instance().dock()->dockPanel(
                  panel, ptr, ins, startX_, startY_, x, y);
        }
    }
    ...
    void DragManager::cancelDrag(bool leaveTempTab)
    {
        ...
        panel_->recalcSize();
        panel_->RedrawWindow(
          0, 0, RDW_INVALIDATE | RDW_FRAME | RDW_ALLCHILDREN);

        // remove references so these smart pointers don't keep owning the
        // objects after endDrag
        panel_     = 0;
        tab_       = 0;
        destPanel_ = 0;
    }
     ...
     */
    InsertAt DragManager::getInsertLocation(DockNodePtr node,
                                            int         x,
                                            int         y,
                                            CRect&      locationRect)
    {
        ...
        // the +/- 10 tweak improves feedback when changing from inserting as a
        // tab to inserting into top/bottom and the dest panel has 1 tab (which
        // toggles the tabbar on/off deppending on the insert location
        CRect topRect = insideRect;
        topRect.bottom =
          topRect.top + INSERT_AREA_SIZE + (tabCtrlAtTop_ ? 10 : 0);

        CRect bottomRect = insideRect;
        bottomRect.top =
          bottomRect.bottom - INSERT_AREA_SIZE - (!tabCtrlAtTop_ ? 10 : 0);
