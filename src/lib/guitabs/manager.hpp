...
namespace GUITABS {
    ...
    /**
     *  This singleton class is the only class accesible by the user of the
     * tearoff panel framework. The programmer must first register it's content
     * factories and insert a dock into his mainFrame (using registerFactory and
     * insertDock) in any order, and the the user can load a configuration file
     * or insert panels manually to actually create and display the desired
     * panels on the screen (using load or insertPanel). It is required that the
     * programmer calls removeDock() on exit, before destroying it's main frame
     * and view windows. It is recommended that the mainView window doesn't have
     * a border. For more information, please see content.hpp and
     * content_factory.hpp
     */
    class Manager
      ...
      , public ReferenceCount
    {
      public:
        ...
        PanelHandle insertPanel(const BW::wstring& contentID,
                                InsertAt           insertAt,
                                PanelHandle        destPanel = 0);
        ...
        bool load(const wchar_t*     defaultFilename,
                  const BW::wstring& fname = L"");
