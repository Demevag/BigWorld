...
namespace GUITABS {
     ...
     */
    void SplitterNode::finishInsert(const CRect* destRect,
                                    int          leftChildSize,
                                    int          rightChildSize)
    {
        BW_GUARD;
        ...
        if (dir_ == VERTICAL) {
            if (leftChildSize) {
                splitterWnd_.SetRowInfo(
                  0,
                  min(leftChildSize, destRect->Height() / 2),
                  MIN_PANE_SIZE + 1);
                splitterWnd_.SetRowInfo(1,
                                        max(destRect->Height() - leftChildSize,
                                            destRect->Height() / 2),
                                        MIN_PANE_SIZE + 1);
            } else if (rightChildSize) {
                splitterWnd_.SetRowInfo(0,
                                        max(destRect->Height() - rightChildSize,
                                            destRect->Height() / 2),
                                        MIN_PANE_SIZE + 1);
                splitterWnd_.SetRowInfo(
                  1,
                  min(rightChildSize, destRect->Height() / 2),
                  MIN_PANE_SIZE + 1);
            } else {
                splitterWnd_.SetRowInfo(
                  0, destRect->Height() / 2, MIN_PANE_SIZE + 1);
                splitterWnd_.SetRowInfo(
                  1, destRect->Height() / 2, MIN_PANE_SIZE + 1);
            }
        } else {
            if (leftChildSize) {
                splitterWnd_.SetColumnInfo(
                  0,
                  min(leftChildSize, destRect->Width() / 2),
                  MIN_PANE_SIZE + 1);
                splitterWnd_.SetColumnInfo(
                  1,
                  max(destRect->Width() - leftChildSize, destRect->Width() / 2),
                  MIN_PANE_SIZE + 1);
            } else if (rightChildSize) {
                splitterWnd_.SetColumnInfo(
                  0,
                  max(destRect->Width() - rightChildSize,
                      destRect->Width() / 2),
                  MIN_PANE_SIZE + 1);
                splitterWnd_.SetColumnInfo(
                  1,
                  min(rightChildSize, destRect->Width() / 2),
                  MIN_PANE_SIZE + 1);
            } else {
                splitterWnd_.SetColumnInfo(
                  0, destRect->Width() / 2, MIN_PANE_SIZE + 1);
                splitterWnd_.SetColumnInfo(
                  1, destRect->Width() / 2, MIN_PANE_SIZE + 1);
            }
        }
    ...
    /**
     *	This method adjustes the size of the node to allow for room to fit in
     *the node specified in the "newNode" param.
     *
    ...
    bool SplitterNode::adjustSizeToNode(DockNodePtr newNode, bool nodeIsNew)
    {
        ...
        bool isInLeft =
          getLeftChild()->getNodeByWnd(newNode->getCWnd(), node, parentNode);
        bool isInRight =
          getRightChild()->getNodeByWnd(newNode->getCWnd(), node, parentNode);
        ...
        // get the mainview window, in order to properly calculate preferred
        // sizes
        CWnd* mainViewWnd = 0;
        ...
        if (isInLeft) {
            // node in left subtree, proceed to resize the right subtree to fit
            // the node
            if (!getLeftChild()->getNodeByWnd(mainViewWnd, node, parentNode)) {
                // left subtree does not contain mainView, so preferred size is
                // ok
                getLeftChild()->getPreferredSize(w, h);
            } else {
                // left subtree contains mainView, so get the current size of
                // the pane
                w = getLeftSize();
                h = getLeftSize();
            }
            ...
            if (getSplitOrientation() == HORIZONTAL) {
                if (getLeftSize() < w ||
                    (getLeftChild() == newNode && nodeIsNew)) {
                    // only resize if the node requires more space than already
                    // available
                    setLeftSize(max(MIN_PANE_SIZE + 1,
                                    min(w, rect.Width() - MIN_PANE_SIZE - 30)));
                    setRightSize(
                      max(MIN_PANE_SIZE + 1, rect.Width() - w - SPLITTER_PAD));
                }
            } else {
                if (getLeftSize() < h ||
                    (getLeftChild() == newNode && nodeIsNew) ||
                    !getLeftChild()->isExpanded()) {
                    // resize if the node requires more space than already
                    // available or if it's collapsed
                    ...
                    setRightSize(
                      max(MIN_PANE_SIZE + 1, rect.Height() - h - SPLITTER_PAD));
                }
            }
        } else if (isInRight) {
            // node in right subtree, proceed to resize the left subtree to fit
            // the node
            if (!getRightChild()->getNodeByWnd(mainViewWnd, node, parentNode)) {
                // right subtree does not contain mainView, so preferred size is
                // ok
                getRightChild()->getPreferredSize(w, h);
            } else {
                // right subtree contains mainView, so get the current size of
                // the pane
                w = getRightSize();
                h = getRightSize();
            }
            ...
            if (getSplitOrientation() == HORIZONTAL) {
                if (getRightSize() < w ||
                    (getRightChild() == newNode && nodeIsNew)) {
                    // only resize if the node requires more space than already
                    // available
                    setLeftSize(
                      max(MIN_PANE_SIZE + 1, rect.Width() - w - SPLITTER_PAD));
                ...
                if (getRightSize() < h ||
                    ...
                    // available or if it's collapsed
                    if (!getLeftChild()->isExpanded()) {
                        // if left subtree is collapsed, minimize left and
                        // maximize right
                        if (!getLeftChild()->getNodeByWnd(
                              mainViewWnd, node, parentNode)) {
                            // left subtree does not contain mainView, so
                            // preferred size is ok
                            getLeftChild()->getPreferredSize(w, h);
                        } else {
                            // left subtree contains mainView, so calc maximum
                            // available size
                            w = rect.Width() - w;
                            h = rect.Height() - h;
                        }
                        ...
                        setLeftSize(max(MIN_PANE_SIZE + 1, h));
                        setRightSize(max(MIN_PANE_SIZE + 1,
                                         rect.Height() - h - SPLITTER_PAD));
                    } else {
    ...
    void SplitterNode::recalcLayout()
    {
        ...
        if (getSplitOrientation() == VERTICAL &&
            (!getLeftChild()->isExpanded() || !getRightChild()->isExpanded()))
            splitterWnd_.allowResize(false);
        else
            splitterWnd_.allowResize(true);
        splitterWnd_.RecalcLayout();
    ...
    bool SplitterNode::load(DataSectionPtr section, CWnd* parent, int wndID)
    {
        ...
        DockNodePtr rightNode =
          Manager::instance().dock()->nodeFactory(rightSec);
        if (!leftNode || !rightNode)
            return false;

        dir_ = nodeSec->readBool("layoutVertical", dir_ == VERTICAL)
                 ? VERTICAL
                 : HORIZONTAL;
     ...
     *	@param childNode	Return param, the node (panel or splitter) matching
     *the CWnd pointer.
     *	@param parentNode	Return param, the parent of the node.
     *	@return		True if found, false if not.
     */
    bool SplitterNode::getNodeByWnd(CWnd*        ptr,
    ...
    /**
     *	This method searches a subtree of nodes for a node that contains a
     *point.
     *
    ...
    /**
     *	This helper method performs all the hard work of resizing splitter pane
     *and its subtree.
     *
     ...
     */
    void SplitterNode::resizeTreeDimension(Orientation dir,
                                           int         lastSize,
                                           int         size)
    {
        BW_GUARD;
        ...
        if (getSplitOrientation() == dir) {
            ...
            if (getLeftChild()->getNodeByWnd(
                  mainViewWnd, childNode, parentNode)) {
                ...
                if (!getRightChild()->isExpanded() && dir == VERTICAL) {
                    ...
                    h += d;
                    setLeftSize(
                      max(MIN_PANE_SIZE + 1, size - h - SPLITTER_PAD));
                    setRightSize(h);
                } else {
                    ...
                    setLeftSize(
                      max(MIN_PANE_SIZE + 1, curSize + (size - lastSize) - d));
                }
            } else if (getRightChild()->getNodeByWnd(
                         mainViewWnd, childNode, parentNode)) {
                ...
                // if mainView isn't in the subtree, resize both panes
                // proportionally.
                int leftNewSize;
                ...
                if (!getLeftChild()->isExpanded() && dir == VERTICAL) {
                    ...
                    if (!getRightChild()->isExpanded() && dir == VERTICAL) {
                        ...
                        // simply resize both panes proportionally to the change
                        // of size of the splitter
                        leftNewSize =
                          min(size - MIN_PANE_SIZE - 11,
                              getLeftSize() + (size - lastSize) / 2);
                    }
                }

                setLeftSize(max(MIN_PANE_SIZE + 1, leftNewSize));
                setRightSize(
                  max(MIN_PANE_SIZE + 1, size - leftNewSize - SPLITTER_PAD));
            }
        }
    }
     ...
     */
    void SplitterNode::resizeSplitter(int lastWidth,
                                      int lastHeight,
                                      int width,
                                      int height)
    {
        ...
        // Needed to filter out the first resize messages sent from windows,
        // because windows first resizes to a very small window (0,0) and then
        // to the actual size
        if (width < 7 && height < 7)
            return;

        if (delayedLeftSize_ > -1 && delayedRightSize_ > -1) {
            // delayed resize activated. Validate and set sizes. Currently, this
            // is only called from the load method.
            CWnd*       mainViewWnd = Manager::instance().dock()->getMainView();
            DockNodePtr node;
            DockNodePtr parentNode;
            bool        isInLeft =
              getLeftChild()->getNodeByWnd(mainViewWnd, node, parentNode);
            bool isInRight =
              getRightChild()->getNodeByWnd(mainViewWnd, node, parentNode);
            ...
            if (isInLeft) {
                ...
                delayedLeftSize_ = size - d - delayedRightSize_ - SPLITTER_PAD;
                       ...
                       size < (delayedLeftSize_ + delayedRightSize_ +
                               SPLITTER_PAD)) {
