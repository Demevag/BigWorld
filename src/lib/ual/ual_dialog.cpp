...
UalDialog::UalDialog(const BW::wstring& configFile)
  ...
  , fileListProvider_(
      new ListFileProvider(UalManager::instance().thumbnailManager().postfix()))
...
bool UalDialog::load(DataSectionPtr section)
{
    ...
    if (lastLanguage_ ==
        section->readWideString("lastLanguage", lastLanguage_)) {
        // only read the custom names if the language is the same.
        dlgShortCaption_ =
          section->readWideString("shortCaption", dlgShortCaption_);
        dlgLongCaption_ =
          section->readWideString("longCaption", dlgLongCaption_);
    }
...
GUITABS::ContentPtr UalDialog::clone()
{
    ...
    if (lastItemInfo_.isFolder_ && lastItemInfo_.dialog_) {
        // is the result of dragging and dropping a folder, so clone using that
        // info
        ...
        if (newUal->folderTree_.GetCount())
            newUal->folderTree_.SelectItem(
              newUal->folderTree_.GetChildItem(TVI_ROOT));
    } else {
        // it's not being clone because of a drag&drop operation, so do standard
        // stuff
...
HICON UalDialog::iconFromXml(DataSectionPtr section, BW::string item)
{
    ...
    if (iconNum != 0) {
        ...
        if (!ret)
            error(BW::string("Couldn't load icon resource for VFolder ") +
                  section->asString());
    } else {
        ...
        if (!ret)
            error(BW::string("Couldn't load icon file for VFolder ") +
                  section->asString());
    }
...
void UalDialog::loadMain(DataSectionPtr section)
{
    ...
    dlgShortCaption_ = Localise(
      section->readWideString("shortCaption", dlgShortCaption_).c_str());
    dlgLongCaption_ =
      Localise(section->readWideString("longCaption", dlgLongCaption_).c_str());
    ...
    fileListProvider_->setThreadYieldMsec(section->readInt(
      "threadYieldMsec", fileListProvider_->getThreadYieldMsec()));

    fileListProvider_->setThreadPriority(section->readInt(
      "threadPriority", fileListProvider_->getThreadPriority()));
    ...
    smartList_.SetIconSpacing(section->readInt("iconSpacingX", 90),
                              section->readInt("iconSpacingY", 100));
    filtersCtrl_.setPushlike(section->readBool("pushlikeFilters", false));
    search_.idleText(section->readWideString(
      "searchIdleText", Localise(L"UAL/UAL_DIALOG/DEFAULT_SEARCH_IDLE_TEXT")));
}
...
void UalDialog::loadToolbar(DataSectionPtr section)
{
    ...
    toolbar_.SetWindowPos(
      0, 0, 0, tbSize.cx, tbSize.cy, SWP_NOMOVE | SWP_NOZORDER);
}
...
void UalDialog::loadFilters(DataSectionPtr section)
{
    ...
    for (BW::vector<DataSectionPtr>::iterator s = filters.begin();
         s != filters.end();
         ++s) {
        ...
        if ((*s)->readBool("separator", false)) {
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                for (BW::vector<DataSectionPtr>::iterator ss = sections.begin();
                     ss != sections.end();
                     ++ss) {
                    if (!str[i].empty())
                        str[i] += L";";
            ...
            if (str[0].empty() && str[1].empty()) {
                ...
                error(BW::string("Filter ") + nname +
                      " has no include nor exclude tags.");
            }
        }
 ...
 */
void UalDialog::loadVFolders(DataSectionPtr     section,
                             ...
                             VFolderPtr         parent)
{
    ...
    for (int i = 0; i < section->countChildren(); ++i) {
        ...
        if (vfolder != NULL) {
            ...
            UalVFolderLoaderPtr loader =
              LoaderRegistry::loader(child->sectionName());
            if (loader != NULL && loader->subVFolders()) {
                // look for loadOneName in the nested folders and load it at the
                // parent's level
 ...
 */
VFolderPtr UalDialog::loadVFolder(DataSectionPtr     section,
                                  const BW::wstring& loadOneName,
                                  VFolderPtr         parent,
                                  DataSectionPtr     customData)
{
    ...
    if (section->asString().empty()) {
        error(BW::string("A VFolder of type '") + section->sectionName() +
              "' has no name in the XML config file.");
        return 0;
    }
    ...
    if (loadOneName != L"***EXCLUDE_ALL***" &&
        ...
        UalVFolderLoaderPtr loader =
          LoaderRegistry::loader(section->sectionName());

        if (loader == NULL) {
            // it's not a recognized vfolder section, so return.
            // Note: This early error doesn't seem to get caught by WE at the
            // moment, probably because WE registers it's error callback after
            // this.
            error("VFolder type '" + section->sectionName() +
                  "' could not be loaded");
            return 0;
        }

        VFolderPtr vfolder = loader->load(
          this, section, parent, customData, true /*addToFolderTree*/);
        ...
        if (!folderTree_.getVFolder(section->asWideString()) &&
            std::find(excludeVFolders_.begin(),
    ...
    for (BW::vector<DataSectionPtr>::iterator s = excludeVFolders.begin();
        ...
        for (BW::vector<BW::wstring>::iterator i = excluded.begin();
            ...
            if (!(*i).empty()) {
                ...
                if (std::find(excludeVFolders_.begin(),
                              excludeVFolders_.end(),
                              *i) == excludeVFolders_.end())
 ...
 *						name is the same as the contents of loadOneName.
 */
void UalDialog::loadCustomVFolders(DataSectionPtr     section,
                                   const BW::wstring& loadOneName)
{
    BW_GUARD;
    ...
    for (BW::vector<DataSectionPtr>::iterator s = customVFolders.begin();
        ...
        if (loadOneName.empty() || loadOneName == (*s)->asWideString())
            VFolderPtr vfolder =
              loadFromBaseVFolder(vfolders, inheritsFrom, (*s));
    }
}
 ...
 */
VFolderPtr UalDialog::loadFromBaseVFolder(DataSectionPtr     section,
                                          ...
                                          VFolderPtr         parent)
{
    ...
    for (int i = 0; i < section->countChildren(); ++i) {
        ...
        VFolderPtr vfolder =
          loadFromBaseVFolder(child, baseName, customData, parent);
        if (vfolder)
            return vfolder;
    }
    return 0;
...
/*static*/ bool UalDialog::vfolderFindByTag(HTREEITEM item, void* testData)
{
    ...
    VFolderItemDataPtr data =
      (VFolderItemData*)dlg->folderTree_.GetItemData(item);
 ...
 */
void UalDialog::showItem(const BW::wstring& vfolder,
...
void UalDialog::adjustSearchSize(int width, int height)
{
    ...
    if (search_.GetSafeHwnd()) {
        ...
        if (width - trect.Width() < minSearchX) {
            if (toolbar_.GetSafeHwnd()) {
                toolbar_.SetWindowPos(
                  0, xmargin, ymargin, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
            }
            search_.SetWindowPos(&CWnd::wndBottom,
                                 xmargin,
                                 ymargin + trect.Height() + gap * 2,
                                 width - xmargin * 2,
                                 ysearch,
                                 0);
            ...
            search_.SetWindowPos(&CWnd::wndBottom,
                                 xmargin,
                                 ymargin,
                                 width - trect.Width() - xmargin * 2 - gap * 2,
                                 ysearch,
                                 0);
            search_.GetWindowRect(&rect);
            ScreenToClient(&rect);
            if (toolbar_.GetSafeHwnd()) {
                toolbar_.SetWindowPos(0,
                                      rect.right + gap * 2,
                                      rect.top,
                                      0,
                                      0,
...
void UalDialog::adjustFiltersSize(int width, int height)
{
    ...
    if (filtersCtrl_.GetSafeHwnd()) {
        ...
        if (!showFilters_ || filtersCtrl_.empty()) {
            ...
            filtersCtrl_.SetWindowPos(
              0, 4, top, width - 8, filtersCtrl_.getHeight(), SWP_NOZORDER);
        }
    }
}
...
void UalDialog::adjustSplitterSize(int width, int height)
{
    ...
    if (splitterBar_ && splitterBar_->GetSafeHwnd()) {
        ...
        splitterBar_->SetWindowPos(
          0, 3, top, width - 6, height - top - 15, SWP_NOZORDER);
    ...
    if (statusBar_.GetSafeHwnd()) {
        ...
        ScreenToClient(&rect);
        statusBar_.SetWindowPos(
          0, rect.left, rect.bottom, rect.right, 17, SWP_NOZORDER);
...
void UalDialog::setListStyle(SmartListCtrl::ViewStyle style)
{
    ...
    if (!sel)
        return;
    VFolderPtr vfolder =
      folderTree_.getVFolder((VFolderItemData*)folderTree_.GetItemData(sel));
    if (!vfolder)
        return;
...
void UalDialog::setFolderTreeStatusBar(VFolderItemData* data)
{
    ...
    if (data && !!data->getProvider())
        setStatusText(data->getProvider()->getDescriptiveText(
...
void UalDialog::callbackVFolderSelect(VFolderItemData* data)
{
    ...
    if (!data || data->isVFolder() ||
        !UalManager::instance().itemClickCallback())
        return;
...
void UalDialog::folderTreeSelect(VFolderItemData* data)
{
    ...
    if (!!vfolder) {
        ...
        if (folderData) {
            ...
            if (folderData->showInList_) {
                ...
                for (BW::vector<BW::wstring>::iterator i =
                       folderData->disabledFilters_.begin();
                     i != folderData->disabledFilters_.end();
                     ++i) {
    ...
    if (!!data->getProvider()) {
        ...
        if (showInList && data->getProvider()->getListProviderInfo(
...
void UalDialog::folderTreeItemDelete(VFolderItemData* data)
{
    ...
    if (data->getProvider().getObject() == historyFolderProvider_.getObject()) {
        if (data->isVFolder()) {
            if (MessageBox(Localise(L"UAL/UAL_DIALOG/CLEAR_HISTORY_TEXT"),
    ...
    } else if (data->getProvider().getObject() ==
               favouritesFolderProvider_.getObject()) {
        if (data->isVFolder()) {
            if (MessageBox(Localise(L"UAL/UAL_DIALOG/CLEAR_FAVOURITES_TEXT"),
...
void UalDialog::showItemContextMenu(UalItemInfo* ii)
{
    ...
    if (ii) {
        if (!ii->isFolder()) {
            if (smartList_.getProvider() == favouritesListProvider_.getObject())
                menu.addItem(Localise(L"UAL/UAL_DIALOG/REMOVE_FROM_FAVOURITES"),
                             removeFromFavCmd);
            else if (smartList_.getProvider() ==
                     historyListProvider_.getObject())
                menu.addItem(Localise(L"UAL/UAL_DIALOG/REMOVE_FROM_HISTORY"),
                             removeFromHistCmd);

            if (smartList_.getProvider() != favouritesListProvider_.getObject())
                menu.addItem(Localise(L"UAL/UAL_DIALOG/ADD_TO_FAVOURITES"),
                             addToFavCmd);
        }
        ...
        if (!ii->getNext()) {
            ...
            if (paths.size() == 1) {
                if (PathFileExists(paths[0].c_str())) {
                    menu.addItem(
                      Localise(L"UAL/UAL_DIALOG/OPEN_FOLDER_IN_EXPLORER"),
                      openExplorerCmd);
                    menu.addItem(
                      Localise(L"UAL/UAL_DIALOG/COPY_PATH_TO_CLIPBOARD"),
                      copyPathCmd);
                }
            } else {
                for (int i = 0;
                     i < (int)paths.size() && i < openExplorerCmdRange;
                     ++i) {
                    if (PathFileExists(paths[i].c_str())) {
                        menu.addItem(
                          Localise(L"UAL/UAL_DIALOG/OPEN_X_IN_EXPLORER",
                                   paths[i]),
                          openExplorerCmd + i);
                    }
                }

                for (int i = 0; i < (int)paths.size() && i < copyPathCmdRange;
                     ++i) {
                    if (PathFileExists(paths[i].c_str())) {
                        menu.addItem(
                          Localise(L"UAL/UAL_DIALOG/COPY_X_TO_CLIPBOARD",
                                   paths[i]),
                          copyPathCmd + i);
                    }
                }
    ...
    if (result >= openExplorerCmd &&
        result < openExplorerCmd + openExplorerCmdRange) {
    ...
    } else if (result >= copyPathCmd &&
               result < copyPathCmd + copyPathCmdRange) {
        if (OpenClipboard()) {
            BW::wstring path = paths[result - copyPathCmd];
            HGLOBAL     data =
              GlobalAlloc(GMEM_MOVEABLE, (path.length() + 1) * sizeof(TCHAR));
...
void UalDialog::showContextMenu(VFolderItemData* data)
{
    ...
    if (!data || data->isVFolder()) {
        ...
        if (data)
            plainVFolder = data->getProvider().getObject() !=
                             favouritesFolderProvider_.getObject() &&
                           data->getProvider().getObject() !=
                             historyFolderProvider_.getObject();
        ...
        check =
          smartList_.getStyle() == SmartListCtrl::SMALLICONS ? L"##" : L"";
        menu.addItem(check + Localise(L"UAL/UAL_DIALOG/SMALL_ICONS"),
                     smallViewCmd);
        ...
        menu.addItem(Localise(L"UAL/UAL_DIALOG/RELOAD_DEFAULT_FOLDERS"),
                     defaultFoldersCmd);

        if (data) {
            BW::wstring remove =
              Localise(L"UAL/UAL_DIALOG/REMOVE_X", data->assetInfo().text());
        ...
        if (result == removeFolderCmd && data) {
            ...
            if (folderTree_.GetCount() == 0) {
                UalManager::instance().thumbnailManager().resetPendingRequests(
                  &folderTree_);
            ...
            if (dlg.DoModal() == IDOK) {
                ...
                // Ugly hack: repaint all windows just to get the new panel
                // title repainted :S  Instead, should implement a notification
                // mecanism so the appropriate panel gets the repaint message
                if (GetDesktopWindow())
...
void UalDialog::listLoadingUpdate()
{
    ...
    if (!delayedListShowItem_.empty()) {
        BW::wstring textTmp = delayedListShowItem_.c_str() +
                              delayedListShowItem_.find_last_of(L'\\') + 1;
...
void UalDialog::listItemSelect()
{
    ...
    if (UalManager::instance().itemClickCallback()) {
        ...
        if (focusItem >= 0 && smartList_.GetItemState(
                                focusItem, LVIS_SELECTED) == LVIS_SELECTED) {
            POINT pt;
            GetCursorPos(&pt);
    ...
    if (!numSel)
        ...
        txt = Localise(
          L"UAL/UAL_DIALOG/SELECTED_ITEMS", numSel, smartList_.GetItemCount());
...
void UalDialog::listItemDelete()
{
    ...
    if (smartList_.getProvider() == historyListProvider_.getObject() ||
        ...
        // delete from the history or favourites, depending on the current list
        // provider
        int item   = -1;
        int numSel = smartList_.GetSelectedCount();
        if (numSel > 1) {
            if (MessageBox(Localise(L"UAL/UAL_DIALOG/MULTI_DELETE_TEXT"),
        ...
        for (int i = 0; i < numSel; i++) {
            item = smartList_.GetNextItem(item, LVNI_SELECTED);
            if (item >= 0) {
                if (smartList_.getProvider() ==
                    historyListProvider_.getObject())
                    UalManager::instance().history().remove(
                      smartList_.getAssetInfo(item), i == numSel - 1);
                else
...
void UalDialog::listItemRightClick(int index)
{
    ...
    UalItemInfo*                    iip = &ii;
    while (a != assets.end()) {
        iip->setNext(new UalItemInfo(this, *a++, pt.x, pt.y));
        iip = iip->getNext();
    }
...
void UalDialog::listItemToolTip(int index, BW::wstring& info)
{
    ...
    if (!assetInfo.longText().empty()) {
        ...
        BW::wstring path;
        bw_utf8tow(
          BWResource::getFilePath(BWResource::dissolveFilename(nlongText)),
          path);
        if (!path.empty()) {
 ...
 */
void UalDialog::handleDragMouseMove(UalItemInfo&  ii,
                                    const CPoint& srcPt,
 ...
 *							NULL otherwise.
 */
void UalDialog::dragLoop(BW::vector<AssetInfo>& assetsInfo,
                         bool                   isFolder,
                         void*                  folderExtraData)
{
    BW_GUARD;
    ...
    if (ii.isFolder_)
        lastItemInfo_ =
          ii; // used when cloneRequired, to know last item dragged to be cloned
    ...
    while (CWnd::GetCapture() == this) {
        ...
        if (msg.message == WM_LBUTTONUP) {
            ...
            if (!(endDialog = UalManager::instance().updateDrag(ii, true)))
                info = &ii; // if it's not an UAL to UAL drag, call the callback
                            // with the item info
            stopDrag();
...
void UalDialog::scrollWindow(CWnd* wnd, CPoint pt)
{
    ...
    if (wnd == &smartList_) {
        ...
        bool vertical   = ((GetWindowLong(smartList_.GetSafeHwnd(), GWL_STYLE) &
        ...
        if (speedDamping == 0)
            if (pt.y < scrollAreaHeight && pos > 0)
                folderTree_.SendMessage(
                  WM_VSCROLL, SB_THUMBPOSITION | ((pos - 1) << 16), 0);
            else if (pt.y >= rect.Height() - scrollAreaHeight &&
                     pt.y < rect.Height())
                folderTree_.SendMessage(
                  WM_VSCROLL, SB_THUMBPOSITION | ((pos + 1) << 16), 0);
        speedDamping++;
        ...
        if (!endDrag) {
            ...
            while (ii) {
                if (ii->assetInfo().equalTo(dropAssetInfo)) {
                    // the dragged items are being dropped onto one of it's
                    // items, so avoid adding it
                    doAdd = false;
                    break;
                }
                ii = ii->getNext();
            ...
            if (doAdd) {
                ...
                while (ii) {
                    UalManager::instance().favourites().remove(ii->assetInfo());
                    UalManager::instance().favourites().addAt(ii->assetInfo_,
                                                              dropAssetInfo);
                    ii = ii->getNext();
                }
            }
...
void UalDialog::updateFolderTreeDrag(const UalItemInfo& itemInfo, bool endDrag)
{
    ...
    if (itemInfo.isFolder_) {
        // dragging a folder, so do folder-related stuff like Drag&Drop cloning
        // or reordering
        if (!endDrag) {
            // update
            folderTree_.SelectDropTarget(0);
            ...
            // end drag
            VFolderPtr vfolder =
              folderTree_.getVFolder(itemInfo.assetInfo().text());
            if (!vfolder) {
                // add the dragged folder or vfolder
                UalManager::instance().copyVFolder(
                  itemInfo.dialog_, this, itemInfo);
                vfolder = folderTree_.getVFolder(itemInfo.assetInfo().text());
                VFolderPtr dropVFolder =
                  folderTree_.getVFolder(data.getObject());
        ...
        folderTree_.SelectDropTarget(0);
        folderTree_.SetInsertMark(0);
        if (data && data->getProvider().getObject() ==
                      favouritesFolderProvider_.getObject()) {
            ...
            if (!endDrag) {
                ...
                if (dropItemT) {
                    if (data->isVFolder())
                        folderTree_.SelectDropTarget(
                          dropItemT); // dropping on top of the favourites
                                      // folder
                    else
                        folderTree_.SetInsertMark(dropItemT, FALSE);
                }
                ...
                if (doAdd) {
                    ...
                    while (ii) {
                        if (!data->isVFolder()) {
                            // remove old item, if it exists, in order to add
                            // the new one in the proper location
                            UalManager::instance().favourites().remove(
                              ii->assetInfo_);
                        }
                        // add to favourites
                        if (!UalManager::instance().favourites().getItem(
                              ii->assetInfo_))
                            UalManager::instance().favourites().addAt(
                              ii->assetInfo_, data->assetInfo());
                        else
...
void UalDialog::error(const BW::string& msg)
{
    ...
    if (UalManager::instance().errorCallback())
        (*UalManager::instance().errorCallback())(
          BW::string("Asset Browser: ") + msg);
}
