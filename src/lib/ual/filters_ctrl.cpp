...
void FiltersCtrl::add(const wchar_t* name, bool pushed, void* data)
{
    ...
    if (pushlike_) {
        LONG style =
          GetWindowLong(newFilter->button.GetSafeHwnd(), GWL_EXSTYLE);
        SetWindowLong(newFilter->button.GetSafeHwnd(),
                      GWL_EXSTYLE,
                      style | WS_EX_STATICEDGE);
    }
    newFilter->button.SetFont(GetParent()->GetFont());
    newFilter->button.SetWindowPos(
      0, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_FRAMECHANGED);
...
void FiltersCtrl::addSeparator()
{
    ...
    newSep->separator.Create(
      L"", WS_CHILD | WS_VISIBLE | WS_DISABLED, CRect(0, 0, 2, 20), this, 0);
    LONG style = GetWindowLong(newSep->separator.GetSafeHwnd(), GWL_EXSTYLE);
    SetWindowLong(
      newSep->separator.GetSafeHwnd(), GWL_EXSTYLE, style | WS_EX_STATICEDGE);
    newSep->separator.SetWindowPos(
      0, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_FRAMECHANGED);
...
void FiltersCtrl::recalcWidth(int width)
{
    ...
    for (FilterItr i = filters_.begin(); i != filters_.end(); ++i) {
        if ((*i)->button.GetSafeHwnd()) {
            ...
            (*i)->separator.SetWindowPos(
              0,
              x + (separatorWidth_ - butSeparation_ - brect.Width()) / 2,
              y,
              0,
              0,
