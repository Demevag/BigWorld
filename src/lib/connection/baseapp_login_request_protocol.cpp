  ...
  LoginRequest& request) const
{
    ...
    // For BaseApp logins, the first request steals the existing
    // NetworkInterface from the ServerConnection, for the subsequent requests,
    // a new NetworkInterface is created (with different source port each time).
    // The NetworkInterface is always attached to
    ...
    if (pServerConnection->hasInterface()) {
        ...
        pInterface =
          new Mercury::NetworkInterface(&(pServerConnection->dispatcher()),
 ...
 *	Override from LoginRequestProtocol.
 */
void BaseAppLoginRequestProtocol::cleanUpInterface(
 ...
 *	Override from LoginRequestProtocol.
 */
void BaseAppLoginRequestProtocol::onAttemptsExhausted(
  ...
  bool          hasReceivedNoSuchPort) const
{
    ...
    if ((baseAppIPByte1 != loginAppIPByte1) && (loginAppIPByte1 != 0)) {
        ...
        ERROR_MSG(
          "LoginHandler::onAttemptsExhausted: "
 ...
 *	Override from LoginRequestProtocol.
 */
void BaseAppLoginRequestProtocol::sendRequest(LoginRequestTransport& transport,
                                              LoginRequest&          request,
                                              float timeout) const
{
    BW_GUARD;
    ...
    bundle.sendRequest(
      args, &request, NULL, int(timeout * 1000000), Mercury::RELIABLE_NO);
