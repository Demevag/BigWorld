...
namespace {
     ...
     *	static instance (s_avf_initer) is then used to then initiate their
     *creation during program start-up.
     */
    class AvatarFilterWatcherCreator
    {
      public:
        AvatarFilterWatcherCreator()
        {
            ...
            MF_WATCH(
              "Client Settings/Filters/Latency Velocity",
 ...
 */
void AvatarFilterHelper::input(double             time,
                               SpaceID            spaceID,
                               EntityID           vehicleID,
                               ...
                               const Direction3D& direction)
{
    ...
    currentInputIndex_ =
      (currentInputIndex_ + NUM_STORED_INPUTS - 1) % NUM_STORED_INPUTS;
...
double AvatarFilterHelper::output(double       time,
                                  SpaceID&     rSpaceID,
                                  EntityID&    rVehicleID,
                                  Position3D&  rPosition,
                                  Vector3&     rVelocity,
                                  Direction3D& rDirection)
{
    ...
    if (gotNewInput_) {
        ...
        const int maxLatencyFrame =
          std::min(inputCount_, (uint)NUM_STORED_INPUTS) - 1;

        if (maxLatencyFrame > 0) {
            const double newestTime = this->getStoredInput(0).time_;
            const double olderTime =
              this->getStoredInput(maxLatencyFrame).time_;
            ...
            double ratio =
              (maxLatencyFrame - latencyFrames) / (double)maxLatencyFrame;

            idealLatency_ =
              float(time - Math::lerp(ratio, olderTime, newestTime));
        }

        // idealLatency cannot be less than s_latencyMinimum_ (usually set to 1
        // frame)
        idealLatency_ =
          std::max(idealLatency_, AvatarFilterSettings::s_latencyMinimum_);
    }
    ...
    this->extract(
      outputTime, rSpaceID, rVehicleID, rPosition, rVelocity, rDirection);
  ...
  EntityID           vehicleID)
{
    ...
    environment.transformIntoCommon(
      spaceID_, vehicleID_, position_, direction_);

    spaceID_   = spaceID;
    vehicleID_ = vehicleID;

    environment.transformFromCommon(
      spaceID_, vehicleID_, position_, direction_);
}
...
void AvatarFilterHelper::chooseNextWaypoint(double time)
{
    ...
    previousWaypoint.changeCoordinateSystem(
      this->environment(), newWaypoint.spaceID_, newWaypoint.vehicleID_);

    currentWaypoint.changeCoordinateSystem(
      this->environment(), newWaypoint.spaceID_, newWaypoint.vehicleID_);
    ...
    BoundingBox newWaypointBB(newWaypoint.storedInput_.position_ -
