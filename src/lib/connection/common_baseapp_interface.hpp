...
#define MF_EMPTY_PROXY_MSG(NAME)                                               \
    MERCURY_HANDLED_EMPTY_MESSAGE(                                             \
      NAME, NoBlockProxyMessageHandler<void>, &Proxy::NAME)
...
#define MF_EMPTY_BLOCKABLE_PROXY_MSG(NAME)                                     \
    MERCURY_HANDLED_EMPTY_MESSAGE(NAME, ProxyMessageHandler<void>, &Proxy::NAME)

#undef MF_BEGIN_BLOCKABLE_PROXY_MSG
#define MF_BEGIN_BLOCKABLE_PROXY_MSG(NAME)                                     \
    BEGIN_HANDLED_STRUCT_MESSAGE(                                              \
      NAME, ProxyMessageHandler<INTERFACE_NAME::NAME##Args>, &Proxy::NAME)

#define MF_VARLEN_PROXY_MSG(NAME)                                              \
    MERCURY_HANDLED_VARIABLE_MESSAGE(                                          \
      NAME, 2, ProxyVarLenMessageHandler<false>, &Proxy::NAME)
...
#define MF_VARLEN_BLOCKABLE_PROXY_MSG(NAME)                                    \
    MERCURY_HANDLED_VARIABLE_MESSAGE(                                          \
      NAME, 2, ProxyVarLenMessageHandler<true>, &Proxy::NAME)
