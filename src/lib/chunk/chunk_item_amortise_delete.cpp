...
void AmortiseChunkItemDelete::tick()
{
    ...
    if (chunkItemListSize > curbAt_) {
        ...
        if (chunkItemListSize > maxChunkItemCount_) {
            releaseMinimumCount = static_cast<uint32>(
              chunkItemListSize - chunkItemListSizePreviousFrame_);

#if CIAD_DEBUG_INFO
            // Used for debugging only
            NOTICE_MSG("AmortiseChunkItemDelete: OVERSHOOT - %d",
                       chunkItemListSize);
#endif // CIAD_DEBUG_INFO
        }

        // Scale the time slice based on how close we are to the maximum limit
        // relative to the curb point.  Note that we subtract
        // releaseMinimumCount in case we have gone past the maximum limit, to
        // make sure curbMaxValue is less than 1.
        double curbMaxValue =
          double(chunkItemListSize - releaseMinimumCount - curbAt_) /
          double(maxChunkItemCount_ - curbAt_);
        // Prevent divide by zero
        double oneMinusCurbMaxValue =
          std::max<double>(0.0001, 1.0 - curbMaxValue);
        // Inverse square law - if it works for Gravity it works for me!!!
        scaleFactor = 1.0 / (oneMinusCurbMaxValue * oneMinusCurbMaxValue *
                             oneMinusCurbMaxValue);
    }
    ...
    chunkItemListSizePreviousFrame_ =
      static_cast<uint32>(chunkItemList_.size());
    // Use the minimum between the current time slice and
    // twice the previous time slice
    timeSlicePreviousFrame_ = std::min<double>(scaleFactor * baseTimeSlice_,
                                               2 * timeSlicePreviousFrame_);

#if CIAD_DEBUG_INFO
    // Used for debugging only
    std::strstream ss;
    ss << chunkItemList_.size() << " left. " << scaleFactor << " scaleFactor. "
