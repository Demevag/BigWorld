...
{
  public:
    SpaceGridTraversal(const Vector3& source,
                       const Vector3& extent_i,
                       const Vector3& range,
                       float          gridResF)
      : extent(extent_i)
      , gridResF_(gridResF)
      , altCells(0)
    {
        ...
        sx = int(source.x / gridResF_);
        if (source.x < 0.f)
            sx--;
        sz = int(source.z / gridResF_);
        if (source.z < 0.f)
            sz--;

        ex = int(extent.x / gridResF_);
        if (extent.x < 0.f)
            ex--;
        ez = int(extent.z / gridResF_);
        if (extent.z < 0.f)
            ez--;
        ...
        // dprintf( "Start sx %d sz %d ex %d ez %d dir(%f,%f,%f) xsense %d
  ...
  private:
    ...
    inline void decNear(int r, int a, float eTrav)
    {
        ...
        if (!--nearCount[r][a]) {
            CellSpec cs = {
                gx + (r ^ bx), gz + (a ^ bz), nearSTravel[r][a], eTrav
            };
