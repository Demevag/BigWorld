 ...
 */
ChunkStationNode::LinkInfoConstIter ChunkStationNode::findLink(
  UniqueID const& id) const
{
 ...
 */
ChunkLinkPtr ChunkStationNode::setLink(ChunkStationNode* other,
                                       bool              canTraverse)
{
    BW_GUARD;
    if (other == NULL)
...
/*virtual*/ bool ChunkStationNode::isValid(BW::string& failureMsg) const
{
    ...
    if (id() == UniqueID()) {
        failureMsg =
          LocaliseUTF8(L"CHUNK/INHABITANTS/CHUNK_STATIONNODE/INVALID_ID");
        return false;
    }

    if (graph() == NULL) {
        failureMsg = LocaliseUTF8(
          L"CHUNK/INHABITANTS/CHUNK_STATIONNODE/NOT_CONNECTED_TO_GRAPH");
        return false;
    }

    if (links_.size() != preloadLinks_.size()) {
        failureMsg = LocaliseUTF8(
          L"CHUNK/INHABITANTS/CHUNK_STATIONNODE/LINK_SIZE_UNEQUAL");
        return false;
    }
    ...
    for (size_t i = 0; i < links_.size(); ++i) {
        ...
        if (!link->isValid(failureMsg))
            return false;
        ChunkStationNode* myself =
          (ChunkStationNode*)link->startItem().getObject();
        ChunkStationNode* other =
          (ChunkStationNode*)link->endItem().getObject();
        if (myself != this && other != this) {
            failureMsg = LocaliseUTF8(
              L"CHUNK/INHABITANTS/CHUNK_STATIONNODE/NOT_MATCH_END_ITEM");
            return false;
        }
        if (myself != this)
        ...
        if (it1 == preloadLinks_.end() || it2 == other->preloadLinks_.end()) {
            failureMsg = LocaliseUTF8(
              L"CHUNK/INHABITANTS/CHUNK_STATIONNODE/PRELOAD_IS_MISSING_LINK");
            return false;
        }

        bool owns = id() < other->id();
        if (owns) {
            if (myself->preloadLinks_[other->id()] &&
                (link->direction() & ChunkLink::DIR_START_END) == 0) {
                failureMsg =
                  LocaliseUTF8(L"CHUNK/INHABITANTS/CHUNK_STATIONNODE/"
                               L"LINK_PRELOAD_WRONG_DIRECTION");
                return false;
            }
            if (!myself->preloadLinks_[other->id()] &&
                (link->direction() & ChunkLink::DIR_START_END) != 0) {
    ...
    for (LinkInfoConstIter it = beginLinks(); it != endLinks(); ++it) {
        ...
        if (!ok) {
            failureMsg = LocaliseUTF8(L"CHUNK/INHABITANTS/CHUNK_STATIONNODE/"
                                      L"LINK_IN_PRELOAD_NOT_IN_LINK");
            return false;
        }
    }
    ...
    if (graph()->getNode(id()) == NULL) {
        failureMsg = LocaliseUTF8(
          L"CHUNK/INHABITANTS/CHUNK_STATIONNODE/NODE_NOT_REGISTER_TO_GRAPH");
        return false;
    }

    if (chunk() == NULL) {
        failureMsg = LocaliseUTF8(
          L"CHUNK/INHABITANTS/CHUNK_STATIONNODE/NODE_NOT_IN_CHUNK");
        return false;
    }
...
ChunkLinkPtr ChunkStationNode::findLink(ChunkStationNode const* other) const
{
    ...
    for (BW::vector<ChunkLinkPtr>::const_iterator it = links_.begin();
        ...
        if (link->startItem() == other || link->endItem() == other) {
            return link;
        }
    }
    return NULL;
...
void ChunkStationNode::removeLink(ChunkLinkPtr link)
{
    ...
    ChunkStationNode* other =
      static_cast<ChunkStationNode*>(link->endItem().get());
    ChunkStationNode* myself =
      static_cast<ChunkStationNode*>(link->startItem().get());
    ...
    if (other == this) {
        other = static_cast<ChunkStationNode*>(link->startItem().getObject());
    }
    other->delLink(link);
 ...
 */
ChunkItemFactory::Result ChunkStationNode::create(Chunk*         pChunk,
...
ChunkItemFactory ChunkStationNode::factory_("station",
                                            0,
                                            ChunkStationNode::create);
