...
bool ChunkFlora::load(DataSectionPtr pSection, Chunk* pChunk)
{
    ...
    DataSectionPtr pData =
      BWResource::openSection(pChunk->mapping()->path() + resName);
    ...
    if (pData) {
        ...
        if (header->version_ != ChunkFlora::VERSION) {
            WARNING_MSG(
              "ChunkFlora data for chunk %s contains the wrong "
        ...
        uint32 nBytesReqd =
          width_ * height_ * sizeof(Ecotype::ID) + sizeof(Header);
 ...
 */
ChunkItemFactory::Result ChunkFlora::create(Chunk*         pChunk,
...
ChunkFloraManager::~ChunkFloraManager()
{
    ...
    if (!items_.empty()) {
        WARNING_MSG(
          "ChunkFloraManager - items are not empty.  This means the "
