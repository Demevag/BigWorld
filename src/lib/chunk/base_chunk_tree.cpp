...
{
  public:
    TreeChunkBSPObstacle(const BSPTree&     bsp,
    ...
    virtual ~TreeChunkBSPObstacle() { WARNING_MSG("~TreeChunkBSPObstacle\n"); }
};
    ...
    if (pChunk_ != NULL) {
        ...
        if (this->bspTree_ != NULL) {
            ChunkModelObstacle::instance(*pChunk_).addObstacle(
              new TreeChunkBSPObstacle(
                this->bspTree_, world, &this->boundingBox_, this));
        }
    }
}
