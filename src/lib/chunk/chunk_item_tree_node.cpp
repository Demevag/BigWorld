...
void ChunkItemTreeNode::setParent(ChunkItemTreeNodePtr parent)
{
    ...
    for (ChunkItemTreeNodePtr p = this; p; p = p->getParent()) {
        if (p == parent)
            return; // cannot make link
    }

    for (ChunkItemTreeNodePtr p = parent; p; p = p->getParent()) {
        if (p == this)
            return; // cannot make link
    }
...
{
    MF_ASSERT(std::find(children_.begin(), children_.end(), child) !=
              children_.end());
...
void ChunkItemTreeNode::addBindingToMe(ChunkBindingPtr binding)
{
    ...
    // check whether already exists
    BindingList::iterator it =
      std::find(bindingsToMe_.begin(), bindingsToMe_.end(), binding);
...
void ChunkItemTreeNode::getCopyOfChildren(
  BW::list<ChunkItemTreeNodePtr>& outList) const
{
...
void ChunkItemTreeNodeCache::add(ChunkItemTreeNodePtr node)
{
    ...
    ChunkItemTreeNodeListMap::iterator it =
      waitingNodeListMap_.find(node->id());
    if (it != waitingNodeListMap_.end()) {
        for (ChunkItemTreeNodeList::const_iterator itc = it->second.begin();
             itc != it->second.end();
             itc++) {
            (static_cast<ChunkItemTreeNodePtr>(*itc))
              ->ChunkItemTreeNode::setParent(node);
        }
