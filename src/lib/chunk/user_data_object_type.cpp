 ...
 *	@param pType The Python class that is associated with this user data object
 *type. This object steals the reference from the caller
 */
UserDataObjectType::UserDataObjectType(
  const UserDataObjectDescription& description,
  PyTypeObject*                    pType)
  ...
  const UniqueID& guid) const
{
    ...
    UserDataObject* pNewUserDataObject =
      new (pObject) UserDataObject(const_cast<UserDataObjectType*>(this), guid);
...
namespace {
    /**
     *	This utility method loads the user data object class object. Returns
     *NULL pointer if there is an error in loading the class object.
     */
    PyObjectPtr importUserDataObjectClass(const UserDataObjectDescription& desc)
    {
        ...
        if (!pModule) {
            WARNING_MSG("UserDataObjectType::importUserDataObjectClass: Could "
                        "not load module %s\n",
                        name);
            PyErr_Clear();
            pModule = PyObjectPtr(PyImport_ImportModule("BigWorld"),
                                  PyObjectPtr::STEAL_REFERENCE);
            if (!pModule) {
                ERROR_MSG("UserDataObjectType::importUserDataObjectClass: "
        ...
        PyObjectPtr pClass =
          PyObjectPtr(PyObject_GetAttrString(pModule.getObject(), name),
                      PyObjectPtr::STEAL_REFERENCE);
        ...
        if (!pClass) {
            WARNING_MSG("UserDataObjectType::importUserDataObjectClass: Could "
                        ...
                        name);
            pClass = PyObjectPtr(
              PyObject_GetAttrString(pModule.getObject(), "UserDataObject"),
              PyObjectPtr::STEAL_REFERENCE);
            if (!pClass) {
                ERROR_MSG("UserDataObjectType::importUserDataObjectClass: "
        ...
        if (result != 1) {
            if (result == -1)
                PyErr_Print();
            ERROR_MSG(
              "UserDataObject::init: %s does not derive from UserDataObject\n",
              name);
            return PyObjectPtr();
        }
...
/**
 *	This method needs to be called in order to initialise the different user
 *data object types.
 *
...
{
    return UserDataObjectType::load(s_curTypes_, s_typesInDifferentDomain_) &&
  ...
  UserDataObjectTypesInDifferentDomain& typesInDifferentDomain)
{
    ...
    for (UserDataObjectDescriptionMap::DescriptionMap::const_iterator i =
           userDataObjectDescriptionMap.begin();
         i != userDataObjectDescriptionMap.end();
         i++) {
        // Don't try to import the object classes if the domain is not our
        // current domain
        ...
        types[desc.name()] =
          new UserDataObjectType(desc, (PyTypeObject*)pClass.getObject());
    }
 ...
 */
void UserDataObjectType::migrate(
  UserDataObjectTypes&                  types,
  UserDataObjectTypesInDifferentDomain& typesInDifferentDomain)
{
    BW_GUARD;
...
/**
 *	This static method is used to get the user data object type associated with
 *the input name.
 */
...
/**
 *	This static method checks if a user data object type is defined, but in a
 *different domain
...
bool UserDataObjectType::knownTypeInDifferentDomain(const char* className)
{
    ...
    return s_typesInDifferentDomain_.find(className) !=
           s_typesInDifferentDomain_.end();
}
...
UserDataObjectTypesInDifferentDomain
  UserDataObjectType::s_typesInDifferentDomain_;
