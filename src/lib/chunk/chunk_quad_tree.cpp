  ...
  : topSize_(float(QUAD_TREE_CHUNK_DIM) * gridSize)
{
    ...
    corner_.set(float(gridX) * topSize_, float(gridZ) * topSize_);
 ...
 *	@param viewProjection the projection matrix used to determine if a chunk is
 *visible
 *	@param chunkList the list of chunks to fill in with visible chunks
 */
void ChunkQuadTree::calculateVisible(const Matrix&       viewProjection,
                                     BW::vector<Chunk*>& chunkList)
{
    BW_GUARD;
 ...
 *	@param chunkList the list of chunks to fill in with visible chunks
 */
void ChunkQuadTree::QuadTreeNode::calculateVisible(
  ...
  BW::vector<Chunk*>& chunkList)
{
    ...
    if (!bb_.insideOut()) {
        ...
        // If the bounding box is completely outside the view frustum we are
        // done
        if (!bb_.combinedOutcode()) {
            ...
            if (!bb_.outcode() || chunkCount_ == 1) {
                ...
                for (uint32 i = 0; i < 4; i++) {
                    nodes_[s_traverseOrder_[i]]->calculateVisible(
                      viewProjection, chunkList);
                }
            }
        }
    }
...
void ChunkQuadTree::QuadTreeNode::allVisible(BW::vector<Chunk*>& chunkList)
{
    BW_GUARD;
    if (chunkCount_ > 1) {
        // If there are more than 1 chunks in our list, check the bounding boxes
        // of the child nodes to see if we should skip some nodes
        if (nodes_[0]->bb_.insideOut() || nodes_[1]->bb_.insideOut() ||
            nodes_[2]->bb_.insideOut() || nodes_[3]->bb_.insideOut()) {
            // Traverse deeper into the quad tree
            for (uint32 i = 0; i < 4; i++) {
            ...
            // Add all chunks that have not already been traverse to the chunk
            // list
 ...
 */
void ChunkQuadTree::QuadTreeNode::init(
  Chunk**                       pStartChunk,
  uint32                        chunkCount,
  ChunkQuadTree::QuadTreeNode*& nextAllocation)
{
    BW_GUARD;
    // Set up bounding box
