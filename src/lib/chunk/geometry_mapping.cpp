 ...
 */
GeometryMapping::GeometryMapping(ChunkSpacePtr     pSpace,
                                 const Matrix&     m,
  ...
  , path_(path + "/")
  , name_(path.substr(
      1 + std::min(path.find_last_of('/'), path.find_last_of('\\'))))
  , pDirSection_(NULL)
  , condemned_(false)
  , singleDir_(false)
{
    ...
    if (gridSizeOK) {
...
#ifdef MF_SERVER
        // Override bounds with serverBounds if present -- not relevant for
        // clients.
        minLGridX_ = pSettings->readInt("serverBounds/minX", this->minLGridX());
        minLGridY_ = pSettings->readInt("serverBounds/minY", this->minLGridY());
        maxLGridX_ = pSettings->readInt("serverBounds/maxX", this->maxLGridX());
        maxLGridY_ = pSettings->readInt("serverBounds/maxY", this->maxLGridY());
#endif
        ...
        // TODO: Remove this and make a proper fix for occlusion culling in arid
        // space
        ...
        pSpace->scaleNavigation(pSettings->readBool("scaleNavigation", false));
...
DataSectionPtr GeometryMapping::pDirSection()
{
    ...
    if (!pDirSection_)
        pDirSection_ =
          BWResource::openSection(path_.substr(0, path_.size() - 1));
...
{
    // add 50 to make sure the point is fully in the centre of the chunk before
    // we map / rotate it this will give us a much better chance of getting the
    // correct grid location in the next step.
    Vector3 worldPt(pSpace()->gridToPoint(x) + pSpace()->gridSize() / 2,
                    0.f,
 ...
 */
void GeometryMapping::boundsToGrid(const BoundingBox& bb,
                                   int&               minGridX,
                                   int&               minGridZ,
                                   int&               maxGridX,
                                   int&               maxGridZ) const
 ...
 *	ie. doesn't differentiate between local or world coordinates.
 */
void GeometryMapping::gridToBounds(int          minGridX,
                                   int          minGridY,
                                   int          maxGridX,
                                   int          maxGridY,
...
{
    const float gridSize = pSpace()->gridSize();
    retBB.setBounds(Vector3(float(minGridX), 0, float(minGridY)) * gridSize,
                    Vector3(float(maxGridX + 1), 0, float(maxGridY + 1)) *
                      gridSize);
}
...
{
    return (gridX >= minLGridX_ && gridX <= maxLGridX_ && gridZ >= minLGridY_ &&
            gridZ <= maxLGridY_);
}
...
{
    return (gridX >= minGridX_ && gridX <= maxGridX_ && gridZ >= minGridY_ &&
            gridZ <= maxGridY_);
}
 ...
 *	string is returned instead.
 */
BW::string GeometryMapping::outsideChunkIdentifier(int  x,
                                                   int  z,
                                                   bool checkBounds) const
{
    BW_GUARD;
...
class HexLookup
{
  public:
    ...
    {
        value       = (hexLookup_[f[3]] + (hexLookup_[f[2]] << 4) +
                 (hexLookup_[f[1]] << 8) + (hexLookup_[f[0]] << 12));
        int8 factor = hexFactor_[f[0]] & hexFactor_[f[1]] & hexFactor_[f[2]] &
                      hexFactor_[f[3]];
        return (!!factor);
    }
...
/*static*/ bool GeometryMapping::gridFromChunkName(const BW::string& chunkName,
                                                   int16&            x,
                                                   int16&            z)
{
    BW_GUARD;
...
namespace Util {
                              ...
                              BW::set<BW::string>& result)
    {
        ...
        for (IFileSystem::Directory::iterator iter = dir.begin();
             iter != dir.end();
             ++iter) {
            if (iter->length() == 15 && iter->substr(8) == "i.chunk") {
                result.insert(subFolder + iter->substr(0, 9));
            } else if (fs->getFileType(folder + *iter) ==
                       IFileSystem::FT_DIRECTORY) {
                gatherInternalChunks(
                  spaceFolder, subFolder + *iter + '/', result);
            }
        }
    }
