 ...
 */
bool ChunkOverlapper::load(DataSectionPtr pSection,
                           Chunk*         pChunk,
                           BW::string*    errorString)
{
    ...
    if (!overlapperID_.empty()) {
        ...
        if (!pBBSect) {
            pBBSect = BWResource::openSection(
              pChunk->mapping()->path() + overlapperID_ + ".chunk/boundingBox");
        }

        if (!pTransformSect) {
            pTransformSect = BWResource::openSection(
              pChunk->mapping()->path() + overlapperID_ + ".chunk/transform");
        }
...
void ChunkOverlapper::findAppointedChunk()
{
    if (!pOverlappingChunk_->isAppointed()) {
        pOverlappingChunk_ =
          this->pOverlappedChunk()->space()->findOrAddChunk(pOverlappingChunk_);
    }
}
                ...
                for (pit2 = (*bit2)->unboundPortals_.begin();
                    ...
                    for (uint i = 0; i < pts.size(); i++) {
                        pChunk->transform().applyPoint(
                          waypoint,
                          Vector3(p2.origin + p2.uAxis * pts[i][0] +
                                  p2.vAxis * pts[i][1]));

                        if (chunk_.boundingBox().intersects(
                              waypoint, 1.f)) { // just check each pt of portal
                                                // with our bb
                            complete_ = false;
...
void ChunkOverlappers::del(ChunkOverlapperPtr pOverlapper, bool isForeign)
{
    ...
    for (Overlappers::iterator it = foreign_.begin(); it != foreign_.end();
         ++it) {
        if ((*it)->overlapperID() == pOverlapper->overlapperID()) {
            foreign_.erase(it);
