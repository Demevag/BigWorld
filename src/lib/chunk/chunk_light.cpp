...
ChunkDirectionalLight::ChunkDirectionalLight()
  : pLight_(
      new Moo::DirectionalLight(Moo::Colour(0, 0, 0, 1), Vector3(0, -1, 0)))
{
}
...
ChunkOmniLight::ChunkOmniLight(WantFlags wantFlags)
  ...
  , pLight_(new Moo::OmniLight(Moo::Colour(0, 0, 0, 1), Vector3(0, 0, 0), 0, 0))
{
}
...
// static
void ChunkOmniLight::load(Moo::OmniLight& light, const DataSectionPtr& pSection)
{
#ifdef EDITOR_ENABLED
    Vector3 colour = pSection->readVector3("colour") / 255.f;
...
ChunkSpotLight::ChunkSpotLight(WantFlags wantFlags)
  : ChunkMooLight(wantFlags)
  , pLight_(new Moo::SpotLight(Moo::Colour(0, 0, 0, 1),
                               Vector3(0, 0, 0),
                               Vector3(0, -1, 0),
                               0,
                               0,
...
// static
void ChunkSpotLight::load(Moo::SpotLight& light, const DataSectionPtr& pSection)
{
#ifdef EDITOR_ENABLED
    Vector3 colour = pSection->readVector3("colour") / 255.f;
    light.colour(Moo::Colour(colour[0], colour[1], colour[2], 1));
...
void ChunkPulseLight::tick(float dTime)
{
    ...
    if (pAnimation_) {
        ...
        if (positionAnimFrame_ >= pAnimation_->totalTime()) {
            positionAnimFrame_ =
              fmodf(positionAnimFrame_, pAnimation_->totalTime());
        }
        Matrix m;
        Matrix res = Matrix::identity;

        for (uint32 i = 0; i < pAnimation_->nChannelBinders(); i++) {
            Moo::AnimationChannelPtr pChannel =
              pAnimation_->channelBinder(i).channel();
            if (pChannel) {
    ...
    pLight_->colour(Moo::Colour(colour_.r * colourMod,
                                colour_.g * colourMod,
                                colour_.b * colourMod,
                                1.f));
...
const StringRef ChunkPulseLight::defaultAnimation(
  "system/animation/lightnoise.animation");
const StringRef ChunkPulseLight::defaultFrameSection(
  "system/data/pulse_light.xml");
...
bool ChunkPulseLight::load(DataSectionPtr pSection)
{
    ...
    BW::string animation = pSection->readString("animation", defaultAnimation);
    ...
    if (pSection) {
        ...
        for (uint32 i = 0; i < animFrames.size(); i++) {
            colourAnimation_.addKey(animFrames[i].x * timeScale,
                                    animFrames[i].y);
        }
...
namespace {
    ...
    struct PortalTraverseInfo
    {
          ...
          , portal_(&portal)
        {
            ...
            chunkToChunk_.multiply(portal.pChunk->transform(),
                                   chunk.transformInverse());
        }

        Chunk*                 chunk_;  // the chunk we traversed from
        ChunkBoundary::Portal* portal_; // the portal we traversed through
        Matrix chunkToChunk_; // transform to move a point from target chunk
                              // back to this chunk
    };
                                  ...
                                  Moo::LightContainer&       outLights)
    {
        ...
        for (uint32 i = 0; i < srcLights.nDirectionals(); i++) {
            ...
            if (std::find(outLights.directionals().begin(),
                          outLights.directionals().end(),
                          pDir) == outLights.directionals().end()) {
    ...
    /**
     *  Helper function to recursively check lights from connected chunks to see
     * if they intersect with portals back to the original chunk (inserting into
     * the given diffuse and specular light containers).
     */
                                ...
                                Moo::LightContainer& outLightsDiffuse)
    {
        ...
        // See if the lights in this chunk intersect with each portal in order,
        // right back up to the root.
        Moo::LightContainerPtr diffLC =
          ChunkLightCache::instance(topChunk).pOwnLights();
        ...
        if (tstack.size() <= ChunkLightCache::MAX_LIGHT_SEEP_DEPTH) {
            for (Chunk::piterator pit = topChunk.pbegin();
                 pit != topChunk.pend();
                 pit++) {
                if (!pit->hasChunk() || !pit->pChunk->isBound() ||
                    pit->pChunk == &lastChunk)
                    continue;
     ...
     */
    void intersectLightInternal(const Vector3&         oposl,
                                const Vector3&         nposl,
                                float                  oradius,
                                float                  nradius,
                                Chunk&                 chunk,
                                ...
                                int                    depth)
    {
        ...
        if (depth < ChunkLightCache::MAX_LIGHT_SEEP_DEPTH) {
            for (Chunk::piterator pit = portal.pChunk->pbegin();
                 pit != portal.pChunk->pend();
                 pit++) {
                if (!pit->hasChunk() || !pit->pChunk->isBound() ||
                    pit->pChunk == &chunk) {
                    continue;
                ...
                Matrix chunkToChunk;
                chunkToChunk.multiply(chunk.transform(),
                                      portal.pChunk->transformInverse());

                intersectLightInternal(chunkToChunk.applyPoint(oposl),
     ...
     *  intersect and traverse portals to discover which chunks were
     * intersecting and chunks which are now intersecting. Uses ref-counts to
     * allow for chunks to accessable via multiple different portal paths.
     */
    void intersectLight(const Vector3& opos,
                        const Vector3& npos,
                        float          orad,
                        float          nrad,
                        Chunk&         chunk,
    ...
    {
        BW_GUARD;
        for (Chunk::piterator pit = chunk.pbegin(); pit != chunk.pend();
             pit++) {
            ...
            intersectLightInternal(
              opos, npos, orad, nrad, chunk, portal, transient, counter, 1);
        }
    }
        ...
        for (size_t i = 0; i < counts.size(); i++) {
            ...
            if (count >= 0) {
                if (chunkAllLights->addLight(pLight, true)) {
                    // DEBUG_MSG( "Added light 0x%p to chunk %s
                    // (incount=%d)!\n",
                    //       pLight.get(), chunk.identifier().c_str(), count );
                }
            } else {
                if (chunkAllLights->delLight(pLight)) {
                    // DEBUG_MSG( "Removed light 0x%p from chunk %s
                    // (incount=%d)!\n",
                    //   pLight.get(), chunk.identifier().c_str(), count );
                }
            }
...
void ChunkLightCache::draw(Moo::DrawContext& drawContext)
{
    ...
    if (Renderer::instance().pipelineType() ==
        IRendererPipeline::TYPE_FORWARD_SHADING)
        return;
...
void ChunkLightCache::dirtySeep(int seepDepth, Chunk* parent)
{
    ...
    if (seepDepth > 0) {
        for (Chunk::piterator pit = chunk_.pbegin(); pit != chunk_.pend();
             pit++) {
            ChunkBoundary::Portal& portal = (*pit);

            if (!portal.hasChunk() || !portal.pChunk->isBound() ||
                portal.pChunk == parent) {
                continue;
            }

            ChunkLightCache::instance(*portal.pChunk)
              .dirtySeep(seepDepth - 1, &chunk_);
        }
    }
}

/**
 *  Notification that one of our omni lights has moved (but is still in this
 * chunk), so check if light seepage needs to be updated.
 */
...
/**
 *  Notification that one of our spot lights has moved (but is still in this
 * chunk), so check if light seepage needs to be updated.
 */
