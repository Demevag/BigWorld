...
enum
{
    ...
    TRIANGLE_MATERIALKIND_SHIFT =
      8 // how many bits to shift to get to the byte we want
};
...
class WorldTriangle
{
  public:
    ...
    bool intersects(const WorldTriangle& triangle, const Vector3& offset) const;
    ...
    void bounce(Vector3& v, float elasticity = 1.f) const;
    ...
    {
        return (flags & TRIANGLE_MATERIALKIND_MASK) >>
               TRIANGLE_MATERIALKIND_SHIFT;
    }
    Flags flags() const;
...
#ifdef BW_WORLDTRIANGLE_DEBUG
    static void BeginDraw(const Matrix& transform,
                          uint32        lifeTime       = 0,
                          uint32        hitFailureTime = 0);
    static void EndDraw();

  private:
    // Enable to draw debug triangles in collision tests. If you do you must
    // also set transform to correct mapping from chunk to world space.
    static bool   s_debugTriangleDraw_;
    static Matrix s_debugTriangleTransform_;
    // If the debug triangles are drawn every frame, then life = 0 (the
    // default). Otherwise set this to indicate how many frames to draw them.
