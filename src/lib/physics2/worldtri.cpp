...
#define BW_WT_DEBUG_DRAW_LINE(start, end, colour)                              \
    if (s_debugTriangleDraw_) {                                                \
        ...
        LineHelper::instance().drawLine(                                       \
          a, b, (colour), s_debugHitTestFailureLife_);                         \
    }
 ...
 */
WorldTriangle::Flags WorldTriangle::packFlags(int collisionFlags,
                                              int materialKind)
{
    return collisionFlags | ((materialKind << TRIANGLE_MATERIALKIND_SHIFT) &
...
bool WorldTriangle::intersects(const WorldTriangle& triangle) const
{
    ...
    PlaneEq planeEqA(
      triA.v0(), triA.v1(), triA.v2(), PlaneEq::SHOULD_NOT_NORMALISE);
    ...
    PlaneEq planeEqB(
      triB.v0(), triB.v1(), triB.v2(), PlaneEq::SHOULD_NOT_NORMALISE);
                               ...
                               const Vector3&       offset) const
{
    ...
    float   unitDotProduct =
      planeEq.normal().unitVector().dotProduct(offset.unitVector());
    ...
    if (almostZero(unitDotProduct, 0.005f)) {
        ...
        //  this case
        if (vd0 < 0.f || vd1 < 0.f || vd2 < 0.f || vd0 >= 1.f || vd1 >= 1.f ||
            vd2 >= 1.f) {
            bruteCheck = true;
        }
    }
    ...
    if (bruteCheck) {
        ...
        WorldTriangle wt2(
          wt1.v0() + offset, wt1.v1() + offset, wt1.v2() + offset);
    ...
    } else // a more efficient version of checking
    {
        ...
        Vector3 triB3D[3] = { triangle.v_[0] + offset * vd0,
                              triangle.v_[1] + offset * vd1,
                              triangle.v_[2] + offset * vd2 };
        Vector2 triA[3]   = { Vector2(v_[0][indexU], v_[0][indexV]),
                              Vector2(v_[1][indexU], v_[1][indexV]),
                              Vector2(v_[2][indexU], v_[2][indexV]) };
        Vector2 triB[3]   = { Vector2(triB3D[0][indexU], triB3D[0][indexV]),
