...
class SweepShape<Vector3>
{
  public:
    ...
    void boundingBox(BoundingBox& bb) const { bb.setBounds(pt_, pt_); }

    void transform(Vector3& shape,
                   Vector3& end,
                   const Matrix&,
                   float,
                   float,
 ...
 */
template <>
class SweepShape<WorldTriangle>
{
  public:
    SweepShape(const WorldTriangle& wt)
      : wt_(wt)
    {
    }
    ...
    void transform(WorldTriangle& shape,
                   Vector3&       end,
                   const Matrix&  tr,
                   float          sdist,
                   float          edist,
                   ...
                   const Vector3&) const
    {
        ...
        shape       = WorldTriangle(tr.applyPoint(wt_.v0() + off),
...
template <typename ShapeType>
SweepParams::SweepParams(const SweepShape<ShapeType>& start, const Vector3& end)
{
