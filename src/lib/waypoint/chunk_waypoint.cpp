...
namespace {
     ...
     */
    void closestPointOnLineSegment(const Vector2& start,
                                   const Vector2& end,
                                   Vector2&       point)
    {
  ...
  const WaypointSpaceVector3&        point) const
{
    ...
    const Vector2* pLastPoint =
      &(provider.vertexByIndex(edges_.back().vertexIndex_));

    while (iEdge != end) {
        const Vector2& thisPoint = provider.vertexByIndex(iEdge->vertexIndex_);
                         ...
                         WorldSpaceVector3&                 wpoint) const
{
    ...
    const Vector2* pPrevPoint =
      &(provider.vertexByIndex(edges_.back().vertexIndex_));

    while (eit != end) {
        const Vector2& thisPoint = provider.vertexByIndex(eit->vertexIndex_);

        Vector2 edgeVec(thisPoint - *pPrevPoint);
        Vector2 pointVec(point.x - pPrevPoint->x, point.z - pPrevPoint->y);
    ...
    if (eit != end) {
        ...
        Vector2 prev = provider.vertexByIndex((edges_.end() - 1)->vertexIndex_);
    ...
    if (!bb.intersects(wpoint)) {
        for (eit = edges_.begin(); eit != end; ++eit) {
            ...
            if (!edge.adjacentToChunk()) {
                const Edge& next =
                  (eit + 1 == end) ? edges_.front() : *(eit + 1);

                const Vector2& thisStart =
                  provider.vertexByIndex(edge.vertexIndex_);
                const Vector2& nextStart =
                  provider.vertexByIndex(next.vertexIndex_);

                WaypointSpaceVector3 start(thisStart.x, avgHeight, thisStart.y);
                WaypointSpaceVector3 end(nextStart.x, avgHeight, nextStart.y);
                ...
                // move in a bit
                wpoint = WorldSpaceVector3(middle + (wpoint - middle) * 0.99f);
...
void ChunkWaypoint::print(const ChunkWaypointVertexProvider& provider) const
{
    ...
    for (Edges::size_type i = 0; i < edges_.size(); ++i) {
        const Vector2& start = provider.vertexByIndex(edges_[i].vertexIndex_);
...
void ChunkWaypoint::calcCentre(const ChunkWaypointVertexProvider& provider)
{
    ...
    for (Edges::size_type i = 0; i < edgeCount_; ++i) {
        const Vector2& start = provider.vertexByIndex(edges_[i].vertexIndex_);
        const Vector2& end =
          provider.vertexByIndex(edges_[(i + 1) % edgeCount_].vertexIndex_);
