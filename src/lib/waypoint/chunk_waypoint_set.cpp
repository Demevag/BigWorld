  ...
  , backlinks_()
{
    ...
    for (wit = pSetData_->waypoints().begin();
        ...
        for (eit = wit->edges_.begin(); eit != wit->edges_.end(); ++eit) {
            ...
            MF_ASSERT(neighbourEdges_.find(pSetData_->getAbsoluteEdgeIndex(
                        *eit)) == neighbourEdges_.end());
            ...
            rNewEdge.yValues[1] = wymax + ChunkWaypoint::HEIGHT_RANGE_TOLERANCE;
            rNewEdge.yValues[2] = wymin - ChunkWaypoint::HEIGHT_RANGE_TOLERANCE;
            ...
            const Vector2& start = pSetData_->vertexByIndex(eit->vertexIndex_);
            const Vector2& end =
              pSetData_->vertexByIndex(nextEdgeIter->vertexIndex_);

            rNewEdge.v = WaypointSpaceVector3(
              (start.x + end.x) / 2.f, 0.f, (start.y + end.y) / 2.f);
        }
    }
    ...
    if (firstTime) {
        firstTime = false;
        MF_WATCH(
          "Client Settings/Physics/debug draw waypoints with arrows",
          s_drawArrows_,
          Watcher::WT_READ_WRITE,
          "Use arrows or lines to draw waypoint navpolys or connections");
        MF_WATCH(
          "Client Settings/Physics/debug draw waypoint connections",
          s_drawConnections_,
          Watcher::WT_READ_WRITE,
 ...
 *							connected.
 */
void ChunkWaypointSet::connect(ChunkWaypointSetPtr    pWaypointSet,
...
void ChunkWaypointSet::drawConnections() const
{
    ...
    darkerColour.r =
      Math::clamp(0.f, regularColour.r - adjacentToChunkColourValue.x, 1.0f);
    darkerColour.g =
      Math::clamp(0.f, regularColour.g - adjacentToChunkColourValue.y, 1.0f);
    darkerColour.b =
      Math::clamp(0.f, regularColour.b - adjacentToChunkColourValue.z, 1.0f);
    ...
    for (ChunkWaypoints::size_type i = 0; i != this->waypointCount(); ++i) {
        ...
        for (ChunkWaypoint::Edges::const_iterator edgeItr = wp.edges_.begin();
            ...
            if (edge.adjacentToChunk()) {
                ...
                MF_ASSERT(
                  edgeLabel.waypoint_ <
                  static_cast<WaypointIndex>(edgeLabel.pSet_->waypointCount()));
...
void ChunkWaypointSet::drawNavPolys() const
{
    ...
    for (ChunkWaypoints::size_type i = 0; i != this->waypointCount(); ++i) {
        ...
        for (; edgeItr != waypoint.edges_.end();
             lastEdge = &(*edgeItr), ++edgeItr) {
            this->drawNavPolyEdge(
              (*edgeItr), *lastEdge, navPolyTop.y, regularColour, darkerColour);
        }
                                       ...
                                       const Moo::Colour& darkerColour) const
{
    ...
    const Vector2& edgeStart = this->vertexByIndex(lastEdge.vertexIndex_);
    const Vector2& edgeEnd   = this->vertexByIndex(edge.vertexIndex_);
