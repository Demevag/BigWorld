...
namespace BW {
    ...
    TEST_F(ScriptUnitTestHarness, ScriptSequence_general)
    {
        ...
        CHECK(pSequence.setItem(
          0, ScriptString::create("test1"), ScriptErrorPrint()));
        CHECK(pSequence.setItem(
          1, ScriptString::create("test2"), ScriptErrorPrint()));
        CHECK(pSequence.setItem(
          2, ScriptString::create("test3"), ScriptErrorPrint()));
        CHECK(pSequence.setItem(
          3, ScriptString::create("test4"), ScriptErrorPrint()));
        CHECK(pSequence.size() == 4);

        ScriptString pStr =
          ScriptString::create(pSequence.getItem(0, ScriptErrorPrint()));
        CHECK(pStr);
