...
namespace BW {
    ...
    TEST_F(ScriptUnitTestHarness, ScriptArgs_createForSingleScriptObjectIsATrap)
    {
        ...
        // Explicitly call the template static method to ensure it works as
        // expected
        ScriptArgs oneScriptObjectGood =
          ScriptArgs::create<ScriptObject>(testScriptObject);
        ...
        ScriptTuple twoScriptObjectsTuple =
          ScriptTuple::create(twoScriptObjects);
