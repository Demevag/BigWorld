...
namespace {
     ...
     *	                       Identifier tag.
     *	@param entityTypeName  The name of the Entity type the property belongs
     *to.
     *
    ...
    {
        const char* pPropTypeName =
          dataDesc.dataType()->pMetaDataType()->name();
        bool isValid = true;
...
bool EntityDefs::init(DataSectionPtr pEntitiesSection)
{
    ...
    entityDescriptionMap_.addPersistentPropertiesToMD5(persistentPropertiesMD5);
                                ...
                                EntityTypeID             entityTypeID)
{
    ...
    for (uint i = 0; i < entityDesc.propertyCount(); ++i) {
        ...
        if (pDataDesc->isIdentifier()) {
            if (!hasIdentifier) {
                isOkay &=
                  checkIdentifierPropertyType(*pDataDesc, entityDesc.name());
