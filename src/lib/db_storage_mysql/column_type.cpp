                                   ...
                                   ColumnIndexType idxType) const
{
    ...
    struct ResultStream
    {
        ...
        void addType(const char* typeName) { ss << typeName; }
        ...
        void addAutoIncrement() { ss << " AUTO_INCREMENT"; }
        void addDefaultValue(const BW::string& defaultValue, MySql* pConnection)
        {
            if (!defaultValue.empty()) {
                ss << " DEFAULT ";
                if (pConnection) {
                    ss << '\'' << MySqlEscapedString(*pConnection, defaultValue)
                       << '\'';
                } else {
                    ss << defaultValue;
                }
        ...
        void addPrimaryKey() { ss << " PRIMARY KEY"; }
        void addNotNull() { ss << " NOT NULL"; }
        void addOnUpdate(const BW::string& cmd) { ss << " ON UPDATE " << cmd; }
    };
 ...
 */
BW::string ColumnType::getDefaultValueAsString(MySql& connection) const
{
...
bool ColumnType::operator==(const ColumnType& other) const
{
    ...
    {
        static bool equalOptionalStuff(const ColumnType& self,
                                       const ColumnType& other)
        {
        ...
        static bool equalDefaultValue(const ColumnType& self,
                                      const ColumnType& other)
        {
