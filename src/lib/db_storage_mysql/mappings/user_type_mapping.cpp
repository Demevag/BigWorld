                                           ...
                                           QueryRunner& queryRunner) const
{
    ...
    MemoryIStream encStrm(const_cast<char*>(encStr.c_str()), encStr.length());
    this->CompositePropertyMapping::fromStreamToDatabase(
      helper, encStrm, queryRunner);
 ...
 *
 *	@returns The new CompositePropertyMapping on success, an empty
 *PropertyMapping on failure.
 */
                                           ...
                                           DataSectionPtr      pDefaultValue)
{
    ...
    // get the object
    PyObjectPtr pObject(PyObject_GetAttrString(
      pModule.getObject(), const_cast<char*>(instanceName.c_str())));
    ...
    PyUserTypeBinderPtr pBinder(
      new PyUserTypeBinder(namer, propName, pDefaultValue), true);

    // call the method
    PyObjectPtr pResult(PyObject_CallMethod(
      pObject.getObject(), "bindSectionToDB", "O", pBinder.getObject()));
