...
{
  public:
    EntityTypeMapping(MySql&                   connection,
    ...
    bool hasNewerRecord(MySql& connection, DatabaseID id, GameTime time) const;

    bool deleteWithID(MySql& connection, DatabaseID id) const;

    void addLogOnRecord(MySql&                  connection,
                        DatabaseID              dbID,
                        const EntityMailBoxRef& mailbox) const;
    bool getLogOnRecord(MySql&            connection,
                        DatabaseID        dbID,
                        EntityMailBoxRef& mailbox) const;
    void removeLogOnRecord(MySql& conn, DatabaseID id) const;
    void updateAutoLoad(MySql& conn, DatabaseID id, bool shouldAutoLoad) const;

    // Inserting an entity
    DatabaseID insertNew(MySql& connection, BinaryIStream& strm) const;
    DatabaseID insertExplicit(MySql&         connection,
                              DatabaseID     dbID,
                              BinaryIStream& strm) const;
    bool       update(MySql&         connection,
                      DatabaseID     dbID,
                      BinaryIStream& strm,
    ...
    // Looking up entities
    bool lookUpEntitiesByProperties(
      MySql&                             connection,
  ...
  private:
    ...
    // Enums must be in the order that these properties are stored in the
    // stream.
    enum FixedCellProp
    {
        CELL_POSITION_INDEX,
