...
EntityTypeMapping::EntityTypeMapping(MySql&                   con,
  ...
  getNameFromIDQuery_("SELECT sm_" + identifierProperty + " FROM " +
                      this->getTableName() + " WHERE id=?")
  , getIDFromNameQuery_("SELECT id FROM " + this->getTableName() +
                        " WHERE sm_" + identifierProperty + "=?")
  ,

  idExistsQuery_("SELECT id FROM " + this->getTableName() + " WHERE id=?")
  , hasNewerQuery_("SELECT gameTime FROM " + this->getTableName() +
                   " WHERE id=? AND gameTime >= ?")
  , insertNewQuery_()
  ...
  , lookUpEntitiesQueryStart_(
      ...
      "lo.typeID = ? "
      "WHERE 1 = 1 ")
  , // 1=1 : support appending AND conditions
  propsNameMap_()
  , identifier_(identifierProperty)
{
    ...
    if (properties.size() > 0) {
        ...
        insertExplicitQuery_.init(
          createExplicitInsertStatement(tableName, properties));
        ...
        if (!identifier_.empty()) {
            getByNameQuery_.init(createSelectStatement(
              tableName, properties, "sm_" + identifier_ + "=?"));
        }
    } else {
 ...
 */
bool EntityTypeMapping::lookUpEntitiesByProperties(
  MySql&                             connection,
  ...
  IDatabase::ILookUpEntitiesHandler& handler) const
{
    ...
    const PropertyMapping* sortPropertyMapping =
      criteria.hasSortProperty()
...
class UpdateFromStreamVisitor : public EntityTypeMappingVisitor
{
  public:
    ...
    void pushGameTime(GameTime gameTime) { queryRunner_.pushArg(gameTime); }
 ...
 *	Constructor
 */
UpdateFromStreamVisitor::UpdateFromStreamVisitor(
  MySql&                   connection,
  const EntityTypeMapping& entityTypeMapping,
  DatabaseID               dbID,
  const Query&             query,
 ...
 *		doesn't exist.
 */
bool EntityTypeMapping::update(MySql&         connection,
                               DatabaseID     dbID,
                               BinaryIStream& strm,
                               GameTime*      pGameTime) const
{
    UpdateFromStreamVisitor visitor(
      connection, *this, dbID, updateQuery_, strm);
                                        ...
                                        BinaryIStream& strm) const
{
    ...
    UpdateFromStreamVisitor visitor(
      connection, *this, 0, insertNewQuery_, strm);
...
{
    UpdateFromStreamVisitor visitor(
      connection, *this, dbID, insertExplicitQuery_, strm);
...
namespace {
    const Query addLogOnQuery("INSERT INTO bigworldLogOns "
...
{
    updateLogOnAutoLoadQuery.execute(
      connection, shouldAutoLoad, dbID, this->getDatabaseTypeID(), NULL);
}
...
{
    ResultSet resultSet;
    getLogOnQuery.execute(
      connection, dbID, this->getDatabaseTypeID(), &resultSet);

    bool isOkay =
      resultSet.getResult(ref.id, ref.addr.ip, ref.addr.port, ref.addr.salt);
