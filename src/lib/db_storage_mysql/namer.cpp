 ...
 *	Constructor (1 of 2)
 */
Namer::Namer(const BW::string& entityName, const BW::string& tableNamePrefix)
 ...
 *	Constructor (2 of 2)
 */
Namer::Namer(const Namer& existing, const BW::string& propName, bool isTable)
...
{
    BW::string suffix =
      (propName.empty()) ? DEFAULT_SEQUENCE_COLUMN_NAME : correctName(propName);
...
{
    BW::string suffix =
      (propName.empty()) ? DEFAULT_SEQUENCE_TABLE_NAME : correctName(propName);
