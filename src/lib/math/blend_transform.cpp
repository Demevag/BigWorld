 ...
 */
BlendTransform::BlendTransform(const Quaternion& irotate,
...
{
    return (rotate_ == rhs.rotate_) && (scale_ == rhs.scale_) &&
 ...
 * and a reasonable translation, otherwise returns false and writes reason to
 * given string.
 */
bool BlendTransform::valid(BW::string* reason) const
{
    ...
    if (!almostEqual(rotLength, 1.0f)) {
        if (reason) {
            *reason =
              "BlendTransform::valid() - Rotation quaternion not normalised, "
              "possibly generated from a skewed matrix.\n";
        }
        return false;
    }

    if (fabsf(scale_.x) >= 1000.0f || fabsf(scale_.y) >= 1000.0f ||
    ...
    if (fabsf(translate_.x) >= 200.0f || fabsf(translate_.y) >= 200.0f ||
