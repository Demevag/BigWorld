...
namespace Math {
     ...
     */
    Polyhedron::Face::Face(Polyhedron&                     owner,
                           const BW::vector<unsigned int>& idxs)
      : owner_(owner)
      , idxs_(idxs)
    {
     ...
     */
    Polyhedron::Edge::Edge(Polyhedron&  owner,
                           unsigned int idx0,
                           unsigned int idx1)
      : owner_(owner)
      , idx0_(idx0)
     ...
     *  line, and returns the minimum and maximum position after projection.
     * Used when testing intersection in 'intersects', using the separation of
     * axes theorem.
     *
     ...
     *  @see intersects
     */
    std::pair<float, float> Polyhedron::calcMinMax(
      const Vector3& separationLine) const
    {
        ...
        for (unsigned int i = 0; i < numPoints(); i++) {
            ...
            if (minMax.first > projection)
                minMax.first = projection;
            if (minMax.second < projection)
                minMax.second = projection;
        }
        return minMax;
    }

    /**
     *  This method performs an intersection test against another Polyhedron,
     * using the separation of axes theorem. This implementation doesn't assume
     * any particular ordering of the polyhedron's points.
     *
    ...
    bool Polyhedron::intersects(const Polyhedron& other) const
    {
        ...
        for (unsigned int i = 0; i < numFaces(); i++) {
            ...
            if (thisMinMax.first > otherMinMax.second ||
                otherMinMax.first > thisMinMax.second)
                return false;
        }
        ...
        for (unsigned int i = 0; i < numEdges(); i++) {
            for (unsigned int j = 0; j < other.numEdges(); j++) {
                ...
                std::pair<float, float> thisMinMax =
                  this->calcMinMax(edgeXProd);
                std::pair<float, float> otherMinMax =
                  other.calcMinMax(edgeXProd);
                if (thisMinMax.first > otherMinMax.second ||
                    otherMinMax.first > thisMinMax.second)
                    return false;
            }
