...
namespace {
    ...
    template <typename DestinationStorage, typename SourceStorage>
    void copyStorageData(DestinationStorage& dst, SourceStorage& src)
    {
    ...
    void generateIdentityMapping(size_t numNodes, TranslationVector& order)
    {
    ...
    void generateRandomOrdering(size_t numNodes, TranslationVector& order)
    {
                                             ...
                                             TranslationVector& order)
    {
        ...
        for (size_t i = 0; i < numNodes; ++i) {
            ...
            for (size_t childIndex = 0; childIndex < StorageType::NUM_CHILDREN;
                 ++childIndex) {
                ...
                NodeIndex childNode =
                  StorageType::childIndexFromReference(nodeRef);
        ...
        for (size_t i = 0; i < numNodes; ++i) {
            ...
            for (size_t i = 0; i < LooseOctree_DataTypes::NUM_CHILDREN; ++i) {
                ...
                for (size_t candidateIndex = i + 1;
                    ...
                    // If the current child is not blank, then look at the next
                    // one
                    if (candidateRef != LooseOctree_DataTypes::INVALID_NODE) {
                        node.child[i] = candidateRef;
                        node.child[candidateIndex] =
                          LooseOctree_DataTypes::INVALID_NODE;
                        break;
                    }
                }
            }
