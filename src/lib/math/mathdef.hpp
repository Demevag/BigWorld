...
const Outcode OUTCODE_MASK   = OUTCODE_LEFT | OUTCODE_RIGHT | OUTCODE_BOTTOM |
...
#define ffrac(number) (number - (float)((int)number))
...
namespace Math {
                        ...
                        double deltaTime)
    {
        if (halfLife <= 0.0) {
            ...
            return dstValue +
                   pow(0.5, deltaTime / halfLife) * (srcValue - dstValue);
        }
    }
                       ...
                       float deltaTime)
    {
        if (halfLife <= 0.0f) {
            ...
            return dstValue +
                   powf(0.5f, deltaTime / halfLife) * (srcValue - dstValue);
        }
    }
     ...
     */
    template <typename SRC, typename DST>
    inline DST safeLerp(SRC const& x,
                        SRC const& src_a,
                        SRC const& src_b,
                        DST const& dst_a,
                        DST const& dst_b)
    ...
    inline TYPE lerp(TYPE const& t, TYPE const& a, TYPE const& b)
    {
        return a + t * (b - a);
    }
