...
namespace BW {
    ...
    inline bool watcherStringToValue(const char* valueStr, Rect& rect)
    {
        char          comma;
        istringstream istr(valueStr);
        istr.imbue(Locale::standardC());
        istr >> rect.xMin_ >> comma >> rect.yMin_ >> comma >> rect.xMax_ >>
          comma >> rect.yMax_;
        return !istr.fail() && istr.eof();
    }

    inline void streamFloat(ostringstream& ostr, float v)
    {
    ...
    inline string watcherValueToString(const BW::Rect& rect)
    {
