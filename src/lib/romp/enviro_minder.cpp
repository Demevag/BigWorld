...
namespace // anonymous
{
    ...
    class WatcherInitialiser
    {
      public:
        WatcherInitialiser()
        {
            MF_WATCH("Render/DecalClipPlaneBias",
                     decalClipPlaneBias,
...
class SkyBoxController : public Moo::EffectConstantValue
{
  ...
  public:
    void value(const Vector4& v) { value_ = v; }
  ...
  , allowUpdate_(true)
{
    ...
    if (!EnvironminderData::instance().s_skyBoxController) {
        ...
        *Moo::rc().effectVisualContext().getMapping("SkyBoxController") =
          EnvironminderData::instance().s_skyBoxController;
    }

    if (!EnvironminderData::instance().s_windAnimation) {
        EnvironminderData::instance().s_windAnimation = new WindAnimation;
        *Moo::rc().effectVisualContext().getMapping("WindAnimation") =
          EnvironminderData::instance().s_windAnimation;
    }
 ...
 *  @param  loadFromExternal    true if cofing data should be loaded from
 * external source
 *
...
bool EnviroMinder::load(DataSectionPtr pDS, bool loadFromExternal /*= true*/)
{
    ...
    if (pDS) {
        ...
        if (spSeas) {
            for (DataSectionIterator it = spSeas->begin(); it != spSeas->end();
                 ++it) {
                Sea* ns = new Sea();
                ns->load(*it);
                seas_->push_back(ns);
        ...
        if (pSections.size()) {
            for (BW::vector<DataSectionPtr>::iterator it = pSections.begin();
                 it != pSections.end();
                 it++) {
                Moo::VisualPtr spSkyDome =
                  Moo::VisualManager::instance()->get((*it)->asString());
        ...
        DataSectionPtr spFlora =
          BWResource::instance().openSection(floraXML, true);

        flora_->init(spFlora, pDS->readInt("terrain/version"));

        // note: farPlane may have been set in the Sky xml file. ( more general
        // ) that setting can be overridden in the space file.     ( less
        // general )
        this->farPlaneBaseLine_ =
          pDS->readFloat("farPlane", this->farPlaneBaseLine_);
 ...
 *
 *  @param  pDS                 the space.settings data section for this space
 *  @param  saveToExternal      true if cofing data should be saved to external
 * source
 *
 *  @return                     always returns true.
 */
bool EnviroMinder::save(DataSectionPtr pDS,
                        bool           saveToExternal /*= true*/) const
{
    BW_GUARD;
    // initialise ourselves from the space.settings or project file
    if (pDS) {
        ...
        if (saveToExternal) {
            if (!todFile_.empty())
                ...
                ERROR_MSG("EnviroMinder::save: Could not save Time Of Day "
                          "because its file path is empty.\n");
        } else {
            pTODSect = pDS;
        }
        if (pTODSect) {
        ...
        if (saveToExternal) {
            if (!sgdFile_.empty())
                ...
                ERROR_MSG("EnviroMinder::save: Could not save Sky Dome because "
                          "its file path is empty.\n");
        } else {
            pSkyDomeSect = pDS;
        }
        if (pSkyDomeSect) {
            skyGradientDome_->save(pSkyDomeSect);
            pSkyDomeSect->writeFloat("farPlane", this->farPlaneBaseLine());
            if (saveToExternal)
                pSkyDomeSect->save();

            //-- Notice: HDR and SSAO settings are saved into <skyGradientDome>
            ...
            //-- save HDR settings.
            {
                ...
                DataSectionPtr hdrDataSec =
                  pSkyDomeSect->openSection("HDR", true);
            ...
            //-- save SSAO settings.
            {
                ...
                DataSectionPtr ssaoDataSec =
                  pSkyDomeSect->openSection("SSAO", true);
        ...
        if (seas_ != NULL && !seas_->empty()) {
            ...
            int            idx    = 0;
            for (Seas::const_iterator it = seas_->begin(); it != seas_->end();
                 ++it, ++idx) {
                char buffer[64];
        ...
        for (BW::vector<Moo::VisualPtr>::const_iterator it = skyDomes_.begin();
             it != skyDomes_.end();
             ++it) {
    ...
    pDS->writeFloat("farPlane", this->farPlaneBaseLine());
 ...
 */
void EnviroMinder::tick(float                  dTime,
                        bool                   outside,
                        const WeatherSettings* pWeatherOverride)
{
    BW_GUARD;
    EnvironminderData::instance().s_skyBoxController->value(
      Vector4(0, 0, 0, 1));
    EnvironminderData::instance().s_windAnimation->tick(dTime,
                                                        weather_->wind());

    ParticleSystemManager::instance().windVelocity(
      Vector3(weather_->wind().x, 0, weather_->wind().y));
    ...
    float sunAngle =
      2.f * MATH_PI - DEG_TO_RAD((timeOfDay_->gameTime() / 24.f) * 360);
...
void EnviroMinder::activate()
{
    ...
    if (Renderer::instance().pipelineType() ==
        IRendererPipeline::TYPE_DEFERRED_SHADING) {
        Renderer::instance().pipeline()->ssaoSupport()->settings(
          &ssaoSettings_);
    }
 ...
 */
void EnviroMinder::drawHind(float  dTime,
                            uint32 drawWhat,
...
void EnviroMinder::decideLightingAndFog()
{
    ...
    outLight.ambientColour =
      outLight.ambientColour * lightDimmer * ambientControl;
    ...
    modcol =
      (extraFog * controlFog) + ((1.f - extraFog) * Vector3(1.f, 1.f, 1.f));
...
void EnviroMinder::drawSkySunCloudsMoon(float dTime, uint32 drawWhat)
{
...

    {
        ...
        // we must set these, because sky boxes + clouds are not meant to set
        // their own COLORWRITEENABLE flags, so if somebody does, then we must
        // restore the state in code.
        ...
        if (drawWhat & DRAW_SKY_BOXES) {
            // we must set these, because sky boxes + clouds are not meant to
            // set their own COLORWRITEENABLE flags, so if somebody does, then
            // we must restore the state in code.
            Moo::rc().setRenderState(D3DRS_COLORWRITEENABLE,
                                     D3DCOLORWRITEENABLE_RED |
                                       D3DCOLORWRITEENABLE_GREEN |
                                       D3DCOLORWRITEENABLE_BLUE);
...
void EnviroMinder::drawSkyDomes(bool isOcclusionPass)
{
    ...
    skyDomesPartition_ =
      Math::clamp((size_t)0, skyDomesPartition_, skyDomes_.size());
    ...
    for (size_t i = 0; i < pySkyDomes_.size(); i++) {
        ...
        if (!isOcclusionPass || value[2] > 0.001f) {
            if (value[3] > 0.001f) {
                ...
                // set skybox pointer to prevent being culled in water
                // reflection
 ...
 */
void EnviroMinder::addPySkyDome(PyAttachmentPtr    attachment,
                                Vector4ProviderPtr provider)
{
    BW_GUARD;
 ...
 */
void addSkyBox(const PyAttachmentPtr&    pAttachment,
               const Vector4ProviderPtr& pProv)
{
    BW_GUARD;
...
PY_AUTO_MODULE_FUNCTION(RETVOID,
                        addSkyBox,
                        ARG(PyAttachmentPtr, ARG(Vector4ProviderPtr, END)),
                        BigWorld)
 ...
 *  @param  provider    Vector4Provider that provides control over the skybox.
 */
void EnviroMinder::delPySkyDome(PyAttachmentPtr    attachment,
                                Vector4ProviderPtr provider)
{
    BW_GUARD;
    for (uint32 i = 0; i < pySkyDomes_.size(); i++) {
        if ((pySkyDomes_[i] == attachment) &&
            (skyDomeControllers_[i] == provider)) {
 ...
 *  @param p the Vector4Provider to set on the sky box
 */
void delSkyBox(const PyAttachmentPtr&    pAttachment,
               const Vector4ProviderPtr& pProv)
{
    BW_GUARD;
    ClientSpacePtr pSpace = DeprecatedSpaceHelpers::cameraSpace();
...
/**
 *  This method removes the static sky boxes, i.e. the ones added via
 * WorldEditor.
 */
...
void EnviroMinder::loadHDR(const DataSectionPtr& data, bool loadFromExternal)
{
    ...
    if (!sgdFile.empty() && loadFromExternal) {
        ...
        if (pSubSect) {
            ...
            //{
            //   float f_sun =
            //   hdr->readFloat("environment/sunLumMultiplier", 1.0f); float
            //   f_sky = hdr->readFloat("environment/skyLumMultiplier", 1.0f);
            //   float f_amb =
            //   hdr->readFloat("environment/ambientLumMultiplier", 1.0f); float
            //   f_fog = hdr->readFloat("environment/fogLumMultiplier", 1.0f);
            //   PS::SetSceneHDRMultipliers(f_sky,f_sun,f_amb,f_fog);
            // }else{
            //   PS::SetSceneHDRMultipliers(1.0f,1.0f,1.0f,1.0f);
            // }
        } else {
            ERROR_MSG("EnviroMinder::load: Cannot open skyGradientDome "
                      "resource '%s' to load HDR settings.\n",
                      sgdFile.c_str());
        }
    } else {
...
void EnviroMinder::loadSSAO(const DataSectionPtr& data, bool loadFromExternal)
{
    ...
    if (!sgdFile.empty() && loadFromExternal) {
        ...
        if (pSubSect) {
            ...
            ERROR_MSG("EnviroMinder::load: Cannot open skyGradientDome "
                      "resource '%s' to load SSAO settings.\n",
                      sgdFile.c_str());
        }
    } else {
 ...
 *  external file defined in the data section.
 *
 *  @param  pDS                 data section from where to load the sky gradient
 * dome.
 *  @param  loadFromExternal    true if cofing data should be loaded from
 * external source.
 *  @param  farPlane            (out) will store the farPlane defined in the
 * data section.
 */
void EnviroMinder::loadSkyGradientDome(DataSectionPtr data,
...
void EnviroMinderSettings::init(DataSectionPtr resXML)
{
    ...
    this->farPlaneSettings_ =
      Moo::makeCallbackGraphicsSetting("FAR_PLANE",
                                       "Far Plane",
                                       *this,
