 ...
 */
bool PyMaterial::findPropertyAndDesc(const char*             name,
                                     Moo::EffectPropertyPtr& prop,
                                     D3DXHANDLE&             handle,
                                     D3DXPARAMETER_DESC&     desc)
{
    BW_GUARD;
    Moo::EffectMaterial::Properties::iterator it =
      pMaterial_->properties().begin();
    Moo::EffectMaterial::Properties::iterator en =
      pMaterial_->properties().end();
    for (; it != en; it++) {
        handle = it->first;
        ...
        switch (propertyDesc.Type) {
            ...
            case D3DXPT_TEXTURECUBE: {
                ...
                if (!r.empty()) {
                    Moo::BaseTexturePtr pTex =
                      Moo::TextureManager::instance()->get(r);
                    return ScriptObject(new PyTextureProvider(NULL, pTex),
            ...
            default:
                PyErr_SetString(PyExc_TypeError,
                                "BigWorld.PyMaterial: "
                                "Could not determine the type of the requested "
                                "material attribute.");
                return ScriptObject();
        }
    }
        ...
        switch (propertyDesc.Type) {
            ...
            case D3DXPT_FLOAT:
                if (propertyDesc.Class == D3DXPC_VECTOR) {
                    ...
                    if (Vector4Provider::Check(value)) {
                        // INFO_MSG( "material property %s is now a
                        // Vector4Provider", propertyDesc.Name );
                        Vector4Provider* v4Prov =
                          static_cast<Vector4Provider*>(value.get());
                        v4Providers_[prop] = v4Prov;
                        v4Prov->output(v);
                    } else {
                        // INFO_MSG( "material property %s is not a
                        // Vector4Provider", propertyDesc.Name );
                    ...
                    if (MatrixProvider::Check(value)) {
                        // INFO_MSG( "material property %s is now a
                        // MatrixProvider", propertyDesc.Name );
                        MatrixProvider* mProv =
                          static_cast<MatrixProvider*>(value.get());
                        matrixProviders_[prop] = mProv;
                        mProv->matrix(m);
                    } else {
                        INFO_MSG("material property %s is not a MatrixProvider",
                                 propertyDesc.Name);
            ...
            case D3DXPT_TEXTURECUBE: {
                if (PyTextureProvider::Check(value)) {
                    PyTextureProvider* pTex =
                      static_cast<PyTextureProvider*>(value.get());
...
PyObject* PyMaterial::pyNew(PyObject* args)
{
    ...
    if (!m) {
        PyErr_SetString(PyExc_TypeError,
                        "BigWorld.PyMaterial: "
                        "Argument parsing error: Expected a d3dEffect filename "
                        "or a material data section. "
                        "If using d3dEffect filename, you may also specify a "
                        "diffuse map name.");
        return NULL;
    }
