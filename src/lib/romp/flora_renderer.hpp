...
class FloraVertexContainer
{
  public:
    ...
    void addVertices(const FloraVertex* vertices,
                     uint32             count,
    ...
    void uvOffset(float uOffset, float vOffset)
    {
        uOffset_ = (UV_TYPE)(uOffset * PACK_UV);
        vOffset_ = (UV_TYPE)(vOffset * PACK_UV);
    };
...
/**
 * This class defines the interface and implements common methods for the flora
 * renderer
 */
class FloraRenderer : public Moo::DeviceCallback
{
  public:
    ...
    void beginAlphaTestPass(float  drawDistance,
                            float  fadePercent,
                            uint32 alphaTestRef);
    void drawBlock(uint32            firstVertex,
                   uint32            nVertices,
