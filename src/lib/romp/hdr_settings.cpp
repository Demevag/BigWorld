...
bool HDRSettings::load(const DataSectionPtr& iData)
{
    ...
    m_avgLumUpdateInterval =
      iData->readFloat("avgLumUpdateInterval", m_avgLumUpdateInterval);
    ...
    //-- bloom parameters.
    {
        ...
        b.m_brightThreshold =
          iData->readFloat("bloom/brightThreshold", b.m_brightThreshold);
        b.m_brightOffset =
          iData->readFloat("bloom/brightOffset", b.m_brightOffset);
    ...
    //-- tone mapping parameters.
    {
        ...
        t.m_eyeDarkLimit =
          iData->readFloat("tonemapping/eyeDarkLimit", t.m_eyeDarkLimit);
        t.m_eyeLightLimit =
          iData->readFloat("tonemapping/eyeLightLimit", t.m_eyeLightLimit);
        t.m_middleGray =
          iData->readFloat("tonemapping/middleGray", t.m_middleGray);
        t.m_whitePoint =
          iData->readFloat("tonemapping/whitePoint", t.m_whitePoint);
    }
    ...
    //-- environment lighting parameters.
    {
        ...
        e.m_skyLumMultiplier = iData->readFloat("environment/skyLumMultiplier",
                                                e.m_skyLumMultiplier);
        e.m_sunLumMultiplier = iData->readFloat("environment/sunLumMultiplier",
                                                e.m_sunLumMultiplier);
        e.m_ambientLumMultiplier = iData->readFloat(
          "environment/ambientLumMultiplier", e.m_ambientLumMultiplier);
        //-- Note: for backward capability we use sun luminance multiplier as
        //fallback, in case if
        //--       fog luminance multiplier is not presented in cfg.
        e.m_fogLumMultiplier = iData->readFloat(
          "environment/fogLumMultiplier", m_environment.m_sunLumMultiplier);
    }
...
bool HDRSettings::save(DataSectionPtr& oData) const
{
    ...
    oData->writeFloat("tonemapping/eyeLightLimit",
                      m_tonemapping.m_eyeLightLimit);
    ...
    //-- environment lighting parameters.
    oData->writeFloat("environment/skyLumMultiplier",
                      m_environment.m_skyLumMultiplier);
    oData->writeFloat("environment/sunLumMultiplier",
                      m_environment.m_sunLumMultiplier);
    oData->writeFloat("environment/ambientLumMultiplier",
                      m_environment.m_ambientLumMultiplier);
    oData->writeFloat("environment/fogLumMultiplier",
                      m_environment.m_fogLumMultiplier);
