...
class StackedLabels : public Moo::DrawContext::UserDrawItem
{
  public:
    ...
    virtual void draw()
    {
        ...
        if (projectedPos.z <= 1.f) {
            ...
            if (FontManager::instance().begin(*font)) {
                ...
                while (it != end) {
                    float widthInClip = font->metrics().stringWidth(*it) /
                                        float(Moo::rc().halfScreenWidth());

                    projectedPos.y += clipHeight;
                    font->drawStringInClip(
                      *it, projectedPos - Vector3(widthInClip / 2.f, 0, 0));
                    ++it;
                }
                FontManager::instance().end();
            }
    ...
    void add(const BW::string& id) { this->labels_.push_back(id); }
