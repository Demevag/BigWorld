...
Vector4 Lightning::decideLightning(float dTime)
{
    ...
    if (rand() * conflict_ < RAND_MAX * 0.7 * dTime)
        return thunder;
    ...
    if (litype >= 13) // sheet lightning / forked lightning
    {
        if (litype == 15) {
            ...
            Vector2 topPos(cs.position.x + rand() * straySz / RAND_MAX,
                           cs.position.y + rand() * straySz / RAND_MAX);

            if (topPos.length() < 500) // TODO: should be set to 80% of farplane
                shouldFlashAmbient = true;

            Vector3 flashpoint(topPos[0],
                               (1 - topPos.length() / fullRange) *
                                 cloudStrata[0].height,
                               topPos[1]);
            ...
            thunder = Vector4(flashpoint + Moo::rc().invView().applyToOrigin(),
...
void Sky::lightningStrike(const Vector3& top)
{
    ...
    while (!stack.empty()) {
        ...
        for (int f = 0; f < nforks; f++) {
            // figure a direction
            Vector3 newdir(lf.dir[0] + (rand() * 0.5f / RAND_MAX) - 0.25f,
            ...
            for (int i = 0; i < 4; i++) {
                vprg.applyPoint(
                  v[i].pos_,
                  Vector4(((i & 2) ? nlf.pos[0] : lf.pos[0]) +
                            ((i & 1) ? 0.5f : -0.5f) *
                              ((i & 2) ? nlf.width : lf.width) * zrot[2],
                          ((i & 2) ? nlf.pos[1] : lf.pos[1]),
                          ((i & 2) ? nlf.pos[2] : lf.pos[2]) +
                            ((i & 1) ? 0.5f : -0.5f) *
                              ((i & 2) ? nlf.width : lf.width) * zrot[0],
                          1));
            ...
            //			Moo::rc().addSortedTriangle( SortedTriangle( &v[0],
            //&v[1], &v[2], &mat ) ); 			Moo::rc().addSortedTriangle(
            //SortedTriangle( &v[3], &v[2], &v[1], &mat ) );

            // and add it to the stack if it's worthy
            if (nlf.width >= 1.f && nlf.pos[1] > -100.f)
                stack.push_back(nlf);
