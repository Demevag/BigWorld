  ...
  , m_globalAnimation(64)
{
    ...
    DataSectionPtr pSection =
      BWResource::openSection(s_floraMFM.value().c_str());
...
FloraVertexContainer* FloraRenderer::lock(uint32 firstVertex, uint32 nVertices)
{
    ...
    if (!pContainer_) {
        ...
        void* pData;
        if (SUCCEEDED(pVB_.lock(sizeof(FloraVertex) * firstVertex,
                                sizeof(FloraVertex) * nVertices,
                                &pData,
                                lockFlags_))) {
                            ...
                            float         tintEnd)
{
    ...
    uint32 ambient =
      Colour::getUint32FromNormalised(timeOfDay->lighting().ambientColour);
    Moo::rc().setRenderState(D3DRS_TEXTUREFACTOR, ambient);
    parameters_->setVector("ambient", &timeOfDay->lighting().ambientColour);

    Vector4 vDirection(
      m_globalAnimation.direction().x, 0, m_globalAnimation.direction().y, 0);
    parameters_->setVector("g_windDirection", &vDirection);
    parameters_->setFloatArray(
      "localAnimations", &m_localAnimation.getGrid()->x, 64 * 2);
    parameters_->setFloatArray(
      "globalAnimations", &m_globalAnimation.getGrid()->x, 64 * 2);

    pVB_.set(0, 0, sizeof(FloraVertex));

    // Set up fog start and end, so that the vertex shader output will be
...
void FloraRenderer::beginAlphaTestPass(float  drawDistance,
                                       float  fadePercent,
                                       uint32 alphaTestRef)
{
    BW_GUARD;
...
void FloraRenderer::drawBlock(uint32            firstVertex,
                              uint32            nVertices,
...
void FloraRenderer::update(float dTime, EnviroMinder& enviro)
{
    ...
    if (bFirstTime) {
        MF_WATCH(
          "Client Settings/Flora/Local Noise Persistence",
          s_fLocalPersistence,
          Watcher::WT_READ_WRITE,
          "Alpha test reference value for the alpha tested drawing pass.");
        MF_WATCH(
          "Client Settings/Flora/Local Noise octaves",
          s_nLocalOctaves,
          Watcher::WT_READ_WRITE,
          "Alpha test reference value for the alpha tested drawing pass.");
        MF_WATCH(
          "Client Settings/Flora/Local Noise Scale",
          s_fLocalScale,
          Watcher::WT_READ_WRITE,
          "Alpha test reference value for the alpha tested drawing pass.");
        MF_WATCH(
          "Client Settings/Flora/Local Noise Frequency",
          s_fLocalFrequency,
          Watcher::WT_READ_WRITE,
          "Alpha test reference value for the alpha tested drawing pass.");

        MF_WATCH(
          "Client Settings/Flora/Global Noise Persistence",
          s_fGlobalPersistence,
          Watcher::WT_READ_WRITE,
          "Alpha test reference value for the alpha tested drawing pass.");
        MF_WATCH(
          "Client Settings/Flora/Global Noise octaves",
          s_nGlobalOctaves,
          Watcher::WT_READ_WRITE,
          "Alpha test reference value for the alpha tested drawing pass.");
        MF_WATCH(
          "Client Settings/Flora/Global Noise Scale",
          s_fGlobalScale,
          Watcher::WT_READ_WRITE,
          "Alpha test reference value for the alpha tested drawing pass.");
        MF_WATCH(
          "Client Settings/Flora/Global Noise Frequency",
          s_fGlobalFrequency,
          Watcher::WT_READ_WRITE,
          "Alpha test reference value for the alpha tested drawing pass.");
...
static bool canRender()
{
    ...
    return Moo::rc().vsVersion() >= 0x100 &&
           Moo::rc()
               ...
               .caps_.MaxSimultaneousTextures > 1;
}
                                       ...
                                       const Matrix*      pTransform)
{
    ...
    for (const FloraVertex *pVertex = pVertIn, *end = pVertIn + count;
         pVertex != end;
         ++pVertex, ++pVertOut) {
        const Vector3 tintPos(
          pVertex->tint_.x, pVertex->tint_.y, pVertex->tint_.z);
        ...
        pVertOut->tint_ =
          Vector4(pTransform->applyPoint(tintPos), pVertex->tint_.w);
        pVertOut->animation_[0] = pVertex->animation_[0];
        const float fRand =
          1.4142136f * pVertOut->pos_.x + 1.7320508f * pVertOut->pos_.z;
