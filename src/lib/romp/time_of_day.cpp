...
void TimeOfDay::load(DataSectionPtr root, bool ignoreTimeChanged /*= false*/)
{
    ...
    if (dayNightSection) {
        ...
        sunColor_ =
          dayNightSection->readVector3("sunColor", Vector3(255, 255, 255));
        moonAngle_ = dayNightSection->readFloat("moonAngle", 80.f);
        moonColor_ =
          dayNightSection->readVector3("moonColor", Vector3(255, 255, 255));
        ...
        for (uint i = 0; i < lightKeys.size(); i++) {
            ...
            colour =
              Vector3(lightKeys[i]->readVector3("colour", Vector3(0, 0, 0)));
        ...
        for (uint i = 0; i < fogKeys.size(); i++) {
            ...
            colour =
              Vector3(fogKeys[i]->readVector3("colour", Vector3(0, 0, 0)));
        ...
        for (uint i = 0; i < ambientKeys.size(); i++) {
            ...
            colour =
              Vector3(ambientKeys[i]->readVector3("colour", Vector3(0, 0, 0)));
...
/*static*/ const BW::string& TimeOfDay::gameTimeToString(float gt)
{
    ...
    bw_snprintf(timeAsCharArray,
                sizeof(timeAsCharArray),
                "%02d:%02d",
                hourTime,
                secondsTime);
 ...
 */
/*static*/ float TimeOfDay::gameTimeFromString(const BW::string& newTime,
                                               float             defaultTime)
{
    BW_GUARD;
    ...
    if (sscanf(newTime.c_str(), "%d:%d", &hours, &minutes) !=
        2) { // try it as a float
        BW::istringstream istr(newTime);
        istr.imbue(Locale::standardC());
        istr >> timeAsFloat;
        if (istr.fail()) {
            WARNING_MSG("TimeOfDay::gameTimeFromString: Incorrect Time Of Day "
                        "format: %s\n",
                        newTime.c_str());
        }
    } else {
