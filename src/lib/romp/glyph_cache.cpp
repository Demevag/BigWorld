    ...
    const char* gdiplusErrorString(Gdiplus::Status s)
    {
        ...
        switch (s) {
            ...
            case Gdiplus::GenericError:
                return "there was an error on the method call, which is "
                       "identified as something other than those defined by "
                       "the other elements of this enumeration.";
            case Gdiplus::InvalidParameter:
                return "one of the arguments passed to the method was not "
                       "valid. ";
            case Gdiplus::OutOfMemory:
                return "the operating system is out of memory and could not "
                       "allocate memory to process the method call. For an "
                       "explanation of how constructors use the OutOfMemory "
                       "status, see the Remarks section at the end of this "
                       "topic. ";
            case Gdiplus::ObjectBusy:
                return "one of the arguments specified in the API call is "
                       "already in use in another thread. ";
            case Gdiplus::InsufficientBuffer:
                return "a buffer specified as an argument in the API call is "
            ...
            case Gdiplus::WrongState:
                return "the object is in an invalid state to satisfy the API "
                       "call. For example, calling Pen::GetColor from a pen "
                       "that is not a single, solid color results in a "
                       "WrongState status. ";
            ...
            case Gdiplus::ValueOverflow:
                return "the method performed an arithmetic operation that "
                       "produced a numeric overflow. ";
            case Gdiplus::AccessDenied:
                return "a write operation is not allowed on the specified "
                       "file. ";
            case Gdiplus::UnknownImageFormat:
                return "the specified image file format is not known. ";
            case Gdiplus::FontFamilyNotFound:
                return "the specified font family cannot be found. Either the "
                       "font family name is incorrect or the font family is "
                       "not installed. ";
            case Gdiplus::FontStyleNotFound:
                return "the specified style is not available for the specified "
                       "font family. ";
            case Gdiplus::NotTrueTypeFont:
                return "the font retrieved from an HDC or LOGFONT is not a "
                       "TrueType font and cannot be used with GDI+. ";
            case Gdiplus::UnsupportedGdiplusVersion:
                return "the version of GDI+ that is installed on the system is "
                       "incompatible with the version with which the "
                       "application was compiled. ";
            case Gdiplus::GdiplusNotInitialized:
                return "the GDI+API is not in an initialized state. To "
            ...
            case Gdiplus::PropertyNotSupported:
                return "the specified property is not supported by the format "
                       "of the image and, therefore, cannot be set. ";
            /*case Gdiplus::ProfileNotFound:
                return "the color profile required to save an image in CMYK
...
// Section : SourceFont
//-----------------------------------------------------------------------------
GlyphCache::SourceFont::SourceFont(DataSectionPtr ds,
                                   bool           bold,
                                   bool           antialias,
                                   bool           dropShadow)
  : rangeMin_(0)
  ...
  , font_(NULL)
{
    ...
    if (ds->findChild("unicodeRange")) {
        if (!parseUnicodeRange(
              ds->readString("unicodeRange"), rangeMin_, rangeMax_) ||
            rangeMin_ > rangeMax_) {
            ERROR_MSG("GlyphCache::SourceFont: Invalid secondary Unicode range "
                      "specified.");
        }
    }
...
bool GlyphCache::SourceFont::createFont(int pointSize)
{
    ...
    if (s == Gdiplus::Ok) {
        font_ = new Gdiplus::Font(fontFamily_,
                                  (float)(abs(pointSize)),
                                  bold_ ? Gdiplus::FontStyleBold
                                        : Gdiplus::FontStyleRegular,
                                  Gdiplus::UnitPixel);

        s = font_->GetLastStatus();
        if (s != Gdiplus::Ok) {
            ERROR_MSG("SourceFont::createFont: Font family '%s' is valid, but "
                      "failed to create font (%d pt). %s\n",
                      fontName_.c_str(),
                      pointSize,
        ...
        if (warnIfMissing_) {
            ERROR_MSG(
              "SourceFont::createFont: Invalid font family '%s' (%d pt). %s\n",
              fontName_.c_str(),
              pointSize,
              gdiplusErrorString(s));
        }
...
const GlyphCache::SourceFontPtr& GlyphCache::charToFont(wchar_t c)
{
    ...
    for (BW::vector<SourceFontPtr>::reverse_iterator it = sourceFonts_.rbegin();
         it != sourceFonts_.rend();
         it++) {
        SourceFontPtr& sf = *it;
...
const GlyphCache::Glyph& GlyphCache::glyph(wchar_t c)
{
    ...
    ERROR_MSG(
      "A glyph (U+%x) was not found in the pre-generated font cache %s\n",
      (uint32)c,
      fontFileName_.c_str());
    return glyphs_.begin()->second;
}
...
bool GlyphCache::calcGlyphRect(wchar_t c, Vector2& dimensions)
{
    ...
    Gdiplus::Status s =
      graphics_->MeasureString(&character,
                               1,
                               font,
...
bool GlyphCache::grabSlot(GlyphCache::Slot& slot)
{
    ...
    GlyphReferenceHolder::GlyphReferenceCountMap::iterator it =
      refCounts_->begin();
    GlyphReferenceHolder::GlyphReferenceCountMap::iterator en =
      refCounts_->end();
    while (it != en) {
        bool eraseIt = false;
    ...
    if (this->expand()) {
        ...
        DEBUG_MSG("Could not grab space font cache, nor increase the cache "
                  "size ('%s'). "
                  "Too many characters on screen at the same time for glyph "
                  "height %d px (%d currently in cache). "
...
bool GlyphCache::expand()
{
    ...
    if (result) {
        ...
        DEBUG_MSG("GlyphCache::expand - failed to expand texture from %dx%d to "
                  "%dx%d.\n",
                  oldWidth,
                  oldHeight,
                  oldWidth,
...
void GlyphCache::uvToSlot(const Vector2& uv, Slot& ret) const
{
    ...
    ret.y_ = (uint32)floorf(
      (uv.y / uvDivisor_) * pTexture_->height() / slotSize_.y + 0.5f);
 ...
 */
const GlyphCache::Glyph& GlyphCache::addGlyphInternal(
  wchar_t                 c,
  const GlyphCache::Slot& slot)
{
    ...
    if (sourceFont && sourceFont->font() && pTexture_->pTexture()) {
        ...
        graphics_->SetTextRenderingHint(
          sourceFont->antialias()
        ...
        graphics_->Clear(Gdiplus::Color(
          1, 255, 255, 255)); // must have 1 alpha or it smartly decides to not
                              // do anything at all
        ...
        float pixelWidth =
          proportional_ ? dimensions.x + effectsMargin_ : maxWidth_;
        ...
        if (sourceFont->dropShadow()) {
            ...
            if (s != Gdiplus::Ok) {
                ERROR_MSG(
                  "GlyphCache::addGlyphInternal could not draw string (%s)",
                  gdiplusErrorString(s));
            }
        }

        s = graphics_->DrawString(&c,
                                  1,
                                  font,
                                  Gdiplus::PointF(0, 0),
                                  Gdiplus::StringFormat::GenericTypographic(),
        ...
        ::OffsetRect(&dstRect,
                     slot.x_ * (uint32)slotSize_.x,
                     slot.y_ * (uint32)slotSize_.y);

        // INFO_MSG( "%s - %d - %0.1f, %0.1f\t\tslot size\n",
        // fontFileName_.c_str(), (uint32)c, slotSize_.x, slotSize_.y );
        // INFO_MSG( "%s - %d - slot %d, %d\trect %d, %d, %d, %d\tglyph uv
        // %0.3f, %0.3f, %0.3f, %0.3f\n", fontFileName_.c_str(), (uint32)c,
        // slot.x_, slot.y_, dstRect.left, dstRect.top, dstRect.right,
        // dstRect.bottom, glyph.uv_.x, glyph.uv_.y, glyph.uvWidth_, slotSize_.y
        // / this->mapHeight() );
        ...
        s = bitmap_->LockBits(
          &gdiRect, Gdiplus::ImageLockModeRead, pixelFormat_, &bmpData);

        if (s == Gdiplus::Ok) {
            HRESULT        hr;
            unsigned char* cbits = (unsigned char*)(bmpData.Scan0);

            // Copy bits into system memory texture, and flag the driver to
            // update the vid mem one whenever it wants
            D3DLOCKED_RECT lr;
            lr.pBits          = 0;
            lr.Pitch          = 0;
            ...
            if (SUCCEEDED(hr)) {
                ...
                ERROR_MSG("GlyphCache::addGlyphInternal - could not get lock "
                          "texture surface rectangle (%s)",
            ...
            ERROR_MSG(
              "GlyphCache::addGlyphInternal - could not lock bitmap (%s)",
              gdiplusErrorString(s));
        }
    }
 ...
 */
void GlyphCache::loadCharacterSet(DataSectionPtr pSection,
                                  BW::wstring&   characterSet)
{
    BW_GUARD;
    ...
    for (size_t i = 0; i < codePoints.size(); i++) {
        ...
        if (!codePoint) {
            WARNING_MSG(
              "loadCharacterSet: invalid unicodeChar tag encountered: '%s'\n",
    ...
    for (size_t i = 0; i < codePointRanges.size(); i++) {
        ...
        if (parseUnicodeRange(codePointRanges[i]->asString(), start, end)) {
            ...
            WARNING_MSG(
              "loadCharacterSet: invalid unicodeRange tag encountered: '%s'\n",
...
bool GlyphCache::load(const BW::string& fontFileName, DataSectionPtr pSection)
{
    ...
    Vector2 mapDimensions =
      pSection->readVector2("mapDimensions", Vector2(512, 32));
    ...
    for (size_t i = 0; i < secondarySections.size(); i++) {
        sourceFonts_.push_back(new SourceFont(secondarySections[i],
                                              sourceFonts_[0]->bold(),
    ...
    BW::string preGeneratedName =
      s_glyphCacheFontRoot.value() + fontFileName_ + ".generated";
    DataSectionPtr pGenerated =
      BWResource::instance().openSection(preGeneratedName);
    ...
    if (pGenerated.hasObject()) {
        ...
        this->loadLegacyGeneratedSection(pLegacyGenerated,
                                         defaultCharacterSet_);
    } else {
        if (!this->createFont()) {
            return false;
        }
        ...
        if (slotSize_.x > mapDimensions.x) {
            ERROR_MSG("Horizontal map size %d is not big enough for point size "
                      "%d (needs to be at least %d). "
                      "Specify a larger mapDimensions in '%s'.\n",
                      (int)mapDimensions.x,
                      pointSize_,
                      (int)fabsf(slotSize_.x),
                      fontFileName.c_str());
            return false;
        }
        ...
        if (mapDimensions.y < slotSize_.y) {
            ...
            if (mapDimensions.y > (float)gdiSurfacePitch_) {
                ...
                bw_safe_delete(bitmap_);
                bw_safe_delete(graphics_);
                bitmap_ = new Gdiplus::Bitmap(
                  gdiSurfacePitch_, gdiSurfacePitch_, pixelFormat_);
        ...
        // There must be, by now, room for at least 1 row and 1 column in the
        // cache.
        ...
        pTexture_ = new Moo::ManagedTexture(mapName,
                                            (int)mapDimensions.x,
                                            (int)mapDimensions.y,
                                            1,
                                            0,
 ...
 */
bool GlyphCache::loadLegacyGeneratedSection(DataSectionPtr     pSection,
                                            const BW::wstring& characterSet)
{
    ...
    Moo::BaseTexturePtr pTex =
      Moo::TextureManager::instance()->get(mapName, false, true);
    pTexture_ = dynamic_cast<Moo::ManagedTexture*>(pTex.getObject());

    if (!pTexture_) {
        ASSET_MSG("Generated section exists, but could not load the generated "
                  "texture map %s\n",
                  mapName.c_str());
        return false;
    }

    Vector2 mapDimensions((float)pTexture_->width(),
                          (float)pTexture_->height());
    slotBounds_.x_ = (uint32)floorf(mapDimensions.x / slotSize_.x);
    slotBounds_.y_ = (uint32)floorf(mapDimensions.y / slotSize_.y);
    ...
    if (sourceHeight == 0.f) {
        ERROR_MSG(
          "Generated section exists, but does not contain the glyph height\n");
        return false;
    }
    ...
    if (positions.length() < numChars * 8) {
        ERROR_MSG("FontMetrics::load failed because numChars did not match the "
    ...
    // The positions string is a long hex string containing position data in the
    // bitmap. There should be 4 hex characters per font character, being
    // xxyy(xpos)(ypos) The widths string is similar, but has only a 2-character
    // hex code per string.
    maxWidth_ = 0;
    for (size_t i = 0; i < numChars; i++) {
        ...
        Vector2 sourceSize(sourceWidth / mapDimensions.x,
                           sourceHeight / mapDimensions.y);
...
bool GlyphCache::loadPreGeneratedData(DataSectionPtr pSection)
{
    ...
    DataSectionPtr binChild =
      pSection->openSection("glyphCache", true, BinSection::creator());
    ...
    if (headerString != "GlyphCache_v1") {
        ERROR_MSG("Generated section exists, but the header (%s) does not "
                  "match the expected version string (GlyphCache_v1).\n",
                  headerString.c_str());
        return false;
    }
    ...
    if (pDestRT->push()) {
        ...
        while (it != en) {
            ...
            Geometrics::createRectMesh(
              tl, br, sourceUV, sourceUV + sourceSize, 0xFF00FF00, mesh);
            mesh.draw();
            ...
            mesh.clear();
            Geometrics::createRectMesh(
              tl, br, sourceUV, sourceUV + sourceSize, 0x00FF0000, mesh);
            mesh.draw();

            // Glyph area plus Effect margin plus Texture margin in the blue
            // channel
            br.x += effectsMargin_;
            br.y += effectsMargin_;
            br.x += textureMargin_;
            ...
            mesh.clear();
            Geometrics::createRectMesh(
              tl, br, sourceUV, sourceUV + sourceSize, 0x000000FF, mesh);
            mesh.draw();
...
{
    tl.set(g.uv_.x * (float)pTexture_->width(),
           (g.uv_.y / uvDivisor_) * (float)pTexture_->height());
...
void GlyphCache::preloadGlyphs(const BW::wstring& characterSet)
{
    ...
    // Access each glyph in the character set, and artifically raise its
    // reference count in order to leak that glyph for the duration
    for (size_t i = 0; i < numChars; i++) {
 ...
 */
bool GlyphCache::save(DataSectionPtr    pSection,
                      ...
                      const BW::string& gridName)
{
    ...
    Moo::TextureManager::writeDDS(
      this->pTexture()->pTexture(), ddsName, D3DFMT_A8R8G8B8, 1);
    INFO_MSG("Font cache map for %s saved to %s\n",
             fontFileName_.c_str(),
             ddsName.c_str());

    Moo::RenderTargetPtr pGridMap =
      new Moo::RenderTarget("temporaryFontGridMap");
    pGridMap->create(pTexture_->width(), pTexture_->height(), false);
    this->createGridMap(pGridMap.get());
    Moo::TextureManager::writeDDS(
      pGridMap->pTexture(), gridName, D3DFMT_A8R8G8B8, 1);
...
bool GlyphCache::createFont()
{
    ...
    bitmap_ =
      new Gdiplus::Bitmap(gdiSurfacePitch_, gdiSurfacePitch_, pixelFormat_);
    graphics_ = new Gdiplus::Graphics(bitmap_);

    brush_       = new Gdiplus::SolidBrush(Gdiplus::Color(255, 255, 255, 255));
    shadowBrush_ = new Gdiplus::SolidBrush(
      Gdiplus::Color((BYTE)(shadowAlpha_ * 255.f), 0, 0, 0));
    Vector2 dimensions;
