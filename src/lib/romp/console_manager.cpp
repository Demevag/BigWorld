...
void ConsoleManager::activate(XConsole* pConsole)
{
    ...
    if (IME::pInstance()) {
        bool allowIME = pActiveConsole_ == NULL &&
...
void ConsoleManager::draw(float dTime)
{
    ...
    if (pActiveConsole_ != NULL) {
        if (this->darkenBackground() &&
            pActiveConsole_->allowDarkBackground()) {
            Geometrics::drawRect(
              Vector2::zero(),
...
/**
 *	Given a console, this determines the name of that console. Returns "" if not
 *found.
 */
...
bool ConsoleManager::handleKeyEvent(const KeyEvent& event)
{
    ...
    if (iter != keyedConsoles_.end()) {
        ...
        if (!handled && event.key() == KeyCode::KEY_ESCAPE &&
            event.isKeyDown()) {
            this->deactivate();
            handled = true;
        }
    ...
    if (PyTuple_Size(args) > 0) {
        ...
        if (!PyArg_ParseTuple(args, "b", &enable)) {
            PyErr_SetString(PyExc_TypeError,
                            "BigWorld.darkenConsoleBackground: "
