  ...
  , terrainVersion_(0)
{
    ...
    if (!s_createdWatchers) {
        MF_WATCH(
          "Client Settings/Flora/Alpha Test Ref",
          s_alphaTestRef,
        ...
        MF_WATCH(
          "Client Settings/Flora/Alpha Test Draw Distance",
          s_alphaTestDistance,
          Watcher::WT_READ_WRITE,
        ...
        MF_WATCH("Client Settings/Flora/Alpha Test Fade Pct",
                 s_alphaTestFadePct,
                 Watcher::WT_READ_WRITE,
                 "Percent of the alpha test drawing range over which the flora "
                 "fades out.");
        MF_WATCH("Client Settings/Flora/Decimation Fade Pct",
                 s_decimationFadePct,
                 Watcher::WT_READ_WRITE,
                 "Percent of the alpha blended drawing range over which the "
                 "flora fades out.");
        MF_WATCH("Client Settings/Flora/per frame block move max",
                 s_allowedBlockMovesPerFrame,
                 Watcher::WT_READ_WRITE,
                 "Number of flora blocks that may be updated per-frame.  Set "
                 "this value higher"
                 "if the flora seems to be lagging behind the camera.  Higher "
                 "values may incur"
                 "a higher per-frame cost depending on the speed at which the "
                 "camera moves.");
        MF_WATCH("Client Settings/Flora/Debug BB",
                 s_debugBB,
                 Watcher::WT_READ_WRITE,
                 "Enable visualisation of the flora blocks' bounding boxes, "
                 "and the flora's"
                 "macro bounding box culling system.");
        MF_WATCH("Client Settings/Flora/Debug Status",
                 s_debugStatus,
                 Watcher::WT_READ_WRITE,
                 "Enable visualisation of flora block's update status, and the "
                 "status of the"
                 "composite flora texture.");
        MF_WATCH("Client Settings/Flora/Cull",
                 s_cull,
                 Watcher::WT_READ_WRITE,
                 "Enabled or disable culling of flora.  If disabled, the flora "
                 "will not move"
        ...
        MF_WATCH("Client Settings/Flora/tintStart",
                 s_tintBegin,
                 Watcher::WT_READ_WRITE,
                 "Relative height where terrain tile textures start to blend "
                 "with flora texture.");

        MF_WATCH("Client Settings/Flora/tintEnd",
                 s_tintEnd,
                 Watcher::WT_READ_WRITE,
                 "Relative height where terrain color texture end to blend "
                 "with flora texture.");
...
bool Flora::init(DataSectionPtr pSection, uint32 terrainVersion)
{
    ...
    s_shadowAlphaTestRef =
      pSection->readInt("ShadowAlphaTestRef", s_shadowAlphaTestRef);
    s_alphaTestDistance =
      pSection->readFloat("AlphaTestDistance", s_alphaTestDistance);
    s_alphaTestFadePct =
      pSection->readFloat("AlphaTestFadePercent", s_alphaTestFadePct);
    s_allowedBlockMovesPerFrame =
      pSection->readInt("MaxPerFrameBlockMove", s_allowedBlockMovesPerFrame);
    ...
    if (pEcotypes) {
        ...
        while (it != pEcotypes->end()) {
            ...
            for (uint32 i = 0; i < pTextures.size(); i++) {
                BW::StringRef texRoot =
                  BWResource::removeExtension(pTextures[i]);
...
void Flora::activate()
{
    ...
    if (maxVBSize() != 0) {
        if (this->pRenderer() != NULL) {
            ...
            ERROR_MSG("EnviroMinder::activate: Could not activate flora, the "
                      "renderer is not initialised.");
        }
    }
}
...
{
    BW_GUARD;
    AABB spaceBounds = DeprecatedSpaceHelpers::cameraSpace()->bounds();
    ...
    if (moveX) {
        int left =
          (centerBlockX_ - (BLOCK_STRIDE / 2) + BLOCK_STRIDE) % BLOCK_STRIDE;
        ...
        int front =
          (centerBlockZ_ - (BLOCK_STRIDE / 2) + BLOCK_STRIDE) % BLOCK_STRIDE;
...
void Flora::fillBlocks()
{
    ...
    if (!movedBlocks_.size())
        return;
    /**
     *	Note : all flora blocks must be moved before any flora blocks can be
     *refilled. the reason being - when a flora block moves, it's ecotype
     *reference may be deallocated. if a whole row of flora blocks move out of
     *an ecotype area, then ecotypes may be fully deref'd, and therefore texture
     *memory freed up.
     *
    ...
    // If any new ecotypes are referenced, they will make sure their texture
    // memory is switched in.
    int numMovedBlocks = 0;
    for (BW::set<FloraBlock*>::iterator it = movedBlocks_.begin();
         it != movedBlocks_.end();) {
        FloraBlock* block = *it;
        if ((numMovedBlocks < s_allowedBlockMovesPerFrame) &&
            block->needsRefill()) {
...
{
    BW_GUARD;
    if (!s_enabled_ || !pRenderer_ || !this->vbSize_)
        return;
...
void Flora::accumulateBoundingBoxes()
{
    ...
    // TODO : remove hardcoding of blocks of 5.  using 5 now coz it is all 25
    // divides into easily
    int         idx = 0;
    int         x   = left;
    int         z   = front;
        ...
        for (int x = left; x < xEnd; x += 5) {
            ...
            for (int zz = 0; zz < 5; zz++) {
                for (int xx = 0; xx < 5; xx++) {
                    FloraBlock* block = &blocks_[(z + zz) % BLOCK_STRIDE]
                                                [(x + xx) % BLOCK_STRIDE];
                    if (block->blockID() != -1)
...
void Flora::draw(float dTime, EnviroMinder& enviro)
{
    ...
    if (!pRenderer_ || !s_enabled_ || !macroBB_[0].blocks_[0] || !vbSize_)
        return;
...
void Flora::drawSorted(EnviroMinder& enviro)
{
    ...
    const float squareDistance =
      s_alphaTestDistance * s_alphaTestDistance + adjBlockWidth * adjBlockWidth;
...

    {
        ...
        // draw alpha tested blocks front to back, only going for x blocks
        // distance. then draw alpha blended blocks back to front, only going
        // for y blocks distance.
        pRenderer_->beginAlphaTestPass(s_alphaTestDistance,
                                       s_alphaTestFadePct,
                                       s_alphaTestRef /*, s_floraBatching*/);

        const bool  bDecimation = s_decimationFadePct < 100.0f;
        const float fDecimationDistance =
          s_alphaTestDistance * (s_decimationFadePct * 0.01f);
        const float fDecimationDistanceSq =
          fDecimationDistance * fDecimationDistance;
        const float fDecimationMulK =
          1.0f /
          (s_alphaTestDistance * s_alphaTestDistance - fDecimationDistanceSq);
        ...
        for (size_t i = 0; i < numSortedBlocks; ++i) {
            ...
            if (bDecimation) {
                const float fDecimationK =
                  (pair.distance_ - fDecimationDistanceSq) * fDecimationMulK;
                if (fDecimationK > 1.f) {
                    numVertices = 0;
    ...
    if (s_debugStatus) {
        ...
        for (int z = 0; z < BLOCK_STRIDE; z++) {
            for (int x = 0; x < BLOCK_STRIDE; x++) {
                ...
                Moo::Colour c;
                if (block->culled()) {
                    c = Moo::Colour(block->needsRefill() ? 0x80800000
                                                         : 0x80008000);
                } else {
                    c = Moo::Colour(block->needsRefill() ? 0x80ff0000
                                                         : 0x8000ff00);
                }
...
void Flora::drawPostDeferred()
{
    if (s_debugBB) {
        ...
        for (int i = 0; i < 400; i++) {
            ...
            uint32 col =
              (fullyCulled ? 0xff000000
                           : (fullyInside ? 0xffffffff : 0xff808080));
...
Ecotype::ID Flora::generateEcotypeID(const Vector2& p)
{
    ...
    if (details.pBlock_ && details.pBlock_->dominantTextureMap()) {
        ...
        if (normal.y > cosMaxSlope_) {
            ...
            StringEcotypeHashMap::const_iterator it =
              texToEcotype_.find(texRoot);
...
void Flora::teleportCamera(const Vector2& camPos2)
{
    ...
    Vector2 topLeft =
      camPos2 + Vector2(-(float)BLOCK_STRIDE / 2.f * BLOCK_WIDTH,
                        (float)BLOCK_STRIDE / 2.f * BLOCK_WIDTH);
...
/**
 *	This method finds a terrain block at the given position, and returns the
 *relative position within that block.
 *
 *	TODO : cache the 4 terrain blocks used at any one time by flora.
 */
Terrain::BaseTerrainBlockPtr Flora::getTerrainBlock(const Vector3& pos,
                                                    Vector3&       relativePos,
                                                    const Vector2* refPt) const
{
    BW_GUARD;
 ...
 */
int Flora::getTerrainBlockID(const Matrix& terrainBlockTransform,
                             const float   terrainBlockSize) const
{
    BW_GUARD;
    Vector3 pos = terrainBlockTransform.applyPoint(
      Vector3(terrainBlockSize / 2.f, 0, terrainBlockSize / 2.f));
...
void Flora::resetBlockAt(float x, float z)
{
    ...
    int dxIdx =
      dx > 0.f ? int(dx / BLOCK_WIDTH + 0.5f) : int(dx / BLOCK_WIDTH - 0.5f);
    int dzIdx =
      dz > 0.f ? int(dz / BLOCK_WIDTH + 0.5f) : int(dz / BLOCK_WIDTH - 0.5f);

    // DEBUG_MSG( "investigated point is x %d by z %d blocks away from center
    // block\n", dxIdx, dzIdx );
...
void Flora::vbSize(uint32 numBytes)
{
    ...
    if (numVertices_ > maxPrims * 3) {
        ...
        INFO_MSG("Flora::createUnmanagedObjects - reduced memory usage to %d, "
                 "due to VB size restriction\n",
                 numBytes);
    }

    // Make a check - the number of vertices allocated to each flora block must
    // be exactly divisible by 3, or else the triangle lists won't work
    ...
    if (numVerticesPerBlock_ % 3 != 0) {
        ...
        numBytes     = numVertices_ * vSize;
        INFO_MSG("Flora::vbSize - reduced memory usage to %d, due to alignment "
                 "requirements\n",
                 numBytes);
    }
    numVertices_ = numVerticesPerBlock_ * blockSize;
    ...
    // Must refill all blocks, since our vertex buffer etc. will have been
    // recreated.
    cameraTeleport_ = true;
}
...
void FloraSettings::init(DataSectionPtr resXML)
{
    ...
    bool supported =
      Moo::GraphicsSettingsAutoDetector::getTotalVideoMemory() > 320;

    // far plane settings
    this->floraSettings_ =
      Moo::makeCallbackGraphicsSetting(getFloraSettingId(),
                                       "Flora Quality",
                                       *this,
                                       &FloraSettings::setFloraOption,
                                       -1,
                                       false,
                                       false);
    ...
    if (resXML.exists()) {
        ...
        while (sectIt != sectEnd) {
            ...
            if (!label.empty() && value != undefined) {
                ...
                this->floraSettings_->addOption(
                  label, label, isSupported, isAdvanced);
