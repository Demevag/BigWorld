...
class OcclusionTestAlphaRef : public Moo::EffectConstantValue
{
  public:
    ...
    void value(uint32 value) { value_ = value; }
  ...
  , enviroMinder_(enviroMinder)
{
    ...
    *Moo::rc().effectVisualContext().getMapping(
      "EnvironmentOcclusionAlphaRef") = s_occlusionTestAlphaRef;
 ...
 *	and the eye.
 */
float SkyDomeOccluder::collides(const Vector3&    photonSourcePosition,
                                ...
                                const LensEffect& le)
{
    ...
    return 1.f -
           Math::clamp(0.f, ((float)lastResult_ / (float)possiblePixels_), 1.f);
}
 ...
 *
 */
void getOccluderScissorRect(float   size,
                            uint32& x,
                            uint32& y,
                            uint32& width,
                            uint32& height)
...
uint32 SkyDomeOccluder::drawOccluders(float size)
{
    ...
    if (first) {
        first = false;
        MF_WATCH(
          "Render/Sky Dome Occlusion/Alpha Reference",
          s_alphaRef,
