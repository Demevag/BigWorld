...
void WaterCell::bindNeighbour(Moo::EffectMaterialPtr effect, int edge)
{
    ...
    if (edgeCells_[edge] && edgeCells_[edge]->isActive()) {
        ...
        effect->pEffect()->pEffect()->SetTexture(
          neighbours[edge],
                              ...
                              WaterCellPtrList&      edgeList)
{
    ...
    if (shouldActivate()) {
        ...
        // activation possibly failed if no available textures. check activation
        // state.
        if (isActive()) {
    ...
    if (Waters::instance()
          .simulationEnabled()) // only do for high detail settings
    {
        if (isActive()) {
            ...
            if (movements().size()) {
                ...
                if (edges > 0) {
                    ...
                    // Cells at the front of this list have a higher activation
                    // priority, cells on an edge have priority over cells on a
                    // corner.
                    edgeList.insert(
                      edgeList.end(), cornerCells.begin(), cornerCells.end());
                }
            }
            ...
            if (edgeActivation_) {
                ...
                for (int i = 0; i < 4 && deactivate; i++) {
                    if (edgeCells_[i] && edgeCells_[i]->isActive()) {
...
void WaterCell::updateDistance(const Vector3& camPos)
{
    ...
    box.setBounds(box.minBounds() + cellMin, box.minBounds() + cellMax);
                             ...
                             const Matrix&  mappingTransform)
{
    ...
    reflectionTint_ =
      pSection->readVector4("reflectionTint", Vector4(1, 1, 1, 1));
    reflectionScale_ = pSection->readFloat("reflectionStrength", 0.04f);

    refractionTint_ =
      pSection->readVector4("refractionTint", Vector4(1, 1, 1, 1));
    ...
    textureTessellation_ =
      pSection->readFloat("textureTessellation", tessellation_);
    ...
    deepColour_ =
      pSection->readVector4("deepColour", Vector4(0.f, 0.20f, 0.33f, 1.f));
...
{
    waveTexture_ = pSection->readString(
      "waveTexture", "system/maps/water_normal_animation/normal");

    foamTexture_ =
      pSection->readString("foamTexture", "system/maps/water_foam2.dds");

    reflectionTexture_ = pSection->readString(
      "reflectionTexture", "system/maps/cloudyhillscubemap2.dds");
                    ...
                    const BW::vector<uint32>& srcIndices)
{
    ...
    if (remappedVerts_.back().size() > maxVerts) {
        ...
        if (remappedVerts_.back().size() > maxVerts) {
            ERROR_MSG(
              "Water::remap( ): Maximum vertex count "
...
{
    BW_GUARD;
#define WIDX_CHECKMAX                                                          \
    maxIndex = indices_32.back() > maxIndex ? indices_32.back() : maxIndex;
    ...
    for (uint z = uint(zMin_); z < uint(zMax_) - 1; z++) {
        for (uint x = uint(xMin_); x < uint(xMax_); x++) {
            ...
            if (!instrip && !lastX && water_->wIndex_[gridIndex] >= 0 &&
                ...
                indices_32.push_back(
                  uint32(water_->wIndex_[gridIndex + water_->gridSizeX_]));
                WIDX_CHECKMAX
    ...
    if (indices.size() > 2) {
        ...
        // triangles.
        if (SUCCEEDED(indexBuffer_.create(
              nIndices_, format, D3DUSAGE_WRITEONLY, D3DPOOL_MANAGED))) {
 ...
 *
 * @param config a structure containing the configuration data for the water
 * surface.
 * @param pCollider the collider to use to intersect the water with the scene
 */
Water::Water(const WaterState&     state,
             const WaterResources& resources,
             RompColliderPtr       pCollider)
  : state_(state)
  ...
  , animTime_(0.0f)
{
    ...
    if (Water::backgroundLoad()) {
        ...
        FileIOTaskManager::instance().addBackgroundTask(
          new CStyleBackgroundTask("Water Load Task",
                                   &Water::doCreateTables,
                                   this,
                                   &Water::onCreateTablesComplete,
                                   this));
    } else {
    ...
    if (first) {
        first = false;
#ifdef DEBUG_WATER
        MF_WATCH("Client Settings/Water/debug cell",
                 s_debugCell_,
                 Watcher::WT_READ_WRITE,
                 "simulation debugging?");
        MF_WATCH("Client Settings/Water/debug cell2",
                 s_debugCell2_,
                 Watcher::WT_READ_WRITE,
                 "simulation debugging?");
        MF_WATCH("Client Settings/Water/debug sim",
                 s_debugSim_,
                 Watcher::WT_READ_WRITE,
                 "simulation debugging?");
#endif
        MF_WATCH("Client Settings/Water/cell cull enable",
                 s_cullCells_,
                 Watcher::WT_READ_WRITE,
                 "enable cell culling?");
        MF_WATCH("Client Settings/Water/cell cull dist",
                 s_cullDistance_,
                 Watcher::WT_READ_WRITE,
                 "cell culling distance (far plane)");

        MF_WATCH("Render/Performance/DrawPrim WaterCell",
                 s_enableDrawPrim,
                 Watcher::WT_READ_WRITE,
    ...
    pPyVolume_ =
      PyObjectPtr(new PyWaterVolume(this), PyObjectPtr::STEAL_REFERENCE);
}
...
bool Water::stillValid(Water* water)
{
    ...
    if (Water::backgroundLoad()) {
        if (deletionLock_) {
            ...
            BW::vector<Water*>::iterator it = std::find(
              Waters::instance().begin(), Waters::instance().end(), water);
...
void Water::rebuild(const WaterState& state, const WaterResources& resources)
{
    ...
    if (wRigid_.size() != gridSizeX_ * gridSizeZ_) { // don't clear it
        wRigid_.clear();
        wAlpha_.clear();
        bankDist.clear();
    ...
    enableSim_ = enableSim_ &&
                 Moo::rc().supportsTextureFormat(D3DFMT_A16B16G16R16F) &&
                 state_.useSimulation_;
...
uint32 Water::createIndices()
{
    ...
    if (waterIndex > 0xffff) {
        Commentary::instance().addMsg(
          "WARNING! Water surface contains excess"
          " vertices, please INCREASE the \"Mesh Size\" parameter",
          1);
    }
#endif // EDITOR
...
{
    BW::wstring fileName;
    bw_utf8tow(BWResource::resolveFilename(resources_.transparencyTable_),
               fileName);
 ...
 */
template <class C>
void writeVector(BW::string& data, BW::vector<C>& v)
{
    int clen = sizeof(C);
...
void compressVector(BW::string& data, BW::vector<C>& v)
{
    ...
    for (uint32 i = 0; i < imax; i++) {
        ...
        if ((cur_v != prev_v) || (c > 254) || (i == (imax - 1))) {
            if (prev_v ==
                static_cast<C>(RLE_KEY_VALUE)) // exception for keyvalue
            {
...
void Water::saveTransparencyTable()
{
    ...
    BinaryPtr binaryBlockAlpha =
      new BinaryBlock(data1.data(), data1.size(), "BinaryBlock/Water");
    BinaryPtr binaryBlockRigid =
      new BinaryBlock(data2.data(), data2.size(), "BinaryBlock/Water");
    ...
    if (!pSection) {
        ...
        // make it
        pSection = new BinSection(
          tagName, new BinaryBlock(NULL, 0, "BinaryBlock/Water"));
    ...
    if (!pSection->writeBinary("alpha", binaryBlockAlpha)) {
        CRITICAL_MSG("Water::saveTransparencyTable - error while writing "
                     "BinSection in %s/alpha\n",
                     dataName.c_str());
        return;
    }
    ...
    int            version        = 2;
    BinaryPtr      binaryBlockVer =
      new BinaryBlock(&version, sizeof(int), "BinaryBlock/Water");
    versionSection->setParent(pSection);
    if (!pSection->writeBinary("version", binaryBlockVer)) {
        CRITICAL_MSG("Water::saveTransparencyTable - error while writing "
                     "BinSection in %s/version\n",
                     dataName.c_str());
        return;
    }
...
{
    char p1 = pCompressedData[index];
    index++;
    char p2 = pCompressedData[index];
    index++;
    char p3 = pCompressedData[index];
    index++;
    char p4 = pCompressedData[index];
    index++;
...
{
    value = static_cast<bool>(pCompressedData[index]);
    index++;
}
 ...
 */
template <class C>
void decompressVector(const char*    pCompressedData,
                      uint32         length,
                      BW::vector<C>& v)
{
    uint32 i = 0;
...
bool Water::loadTransparencyTable()
{
    ...
    if (pVersionData) {
        const int* pVersion =
          reinterpret_cast<const int*>(pVersionData->data());
        fileVersion = pVersion[0];
    }

    BinaryPtr pAlphaData = pSection->readBinary("alpha");
    if (fileVersion < 2) {
        const uint32* pAlphaValues =
          reinterpret_cast<const uint32*>(pAlphaData->data());
        int nAlphaValues = pAlphaData->len() / sizeof(uint32);
        wAlpha_.assign(pAlphaValues, pAlphaValues + nAlphaValues);
    } else {
        wAlpha_.clear();
        const char* pCompressedValues =
          reinterpret_cast<const char*>(pAlphaData->data());
        decompressVector<uint32>(pCompressedValues, pAlphaData->len(), wAlpha_);
    }

    BinaryPtr pRigidData = pSection->readBinary("rigid");
    if (fileVersion < 2) {
        const bool* pRigidValues =
          reinterpret_cast<const bool*>(pRigidData->data());
        int nRigidValues =
          pRigidData->len() / sizeof(bool); // not needed.....remove..
        wRigid_.assign(pRigidValues, pRigidValues + nRigidValues);
    } else {
        wRigid_.clear();
        const char* pCompressedValues =
          reinterpret_cast<const char*>(pRigidData->data());
...
{
    //  float delta = Math::clamp<float>(0.f, vertexHeight - terrainHeight,
    //  MAX_DEPTH); delta = delta / MAX_DEPTH;
    //
    //  return uint32( delta*255.f ) << 24;

    float h =
      155.f +
      100.f * min(1.f, max(0.f, (1.f - (vertexHeight - terrainHeight))));
    h = h / 255.f;
...
void Water::buildTransparencyTable()
{
    ...
    for (uint32 zIndex = 0; zIndex < gridSizeZ_; zIndex++) {
        ...
        for (uint32 xIndex = 0; xIndex < gridSizeX_; xIndex++) {
            ...
            // float height = pCollider_ ? pCollider_->ground( v ) : (v.y -
            // 100); if (height > waterDepth_
            ...
            // else
            //{
            //  Get the terrain height, and calculate the transparency of this
            //  water grid point to be the height above the terrain.
            float height =
              pCollider_ ? pCollider_->ground(v) : (v.y - MAX_DEPTH);
...
#define IS_BANK_LINE(xIndex, zIndex, data)                                     \
    data[xIndex + (zIndex * gridSizeX_)] &&                                    \
      (((xIndex + 1) < gridSizeX_ &&                                           \
        !data[xIndex + 1 + (zIndex * gridSizeX_)]) ||                          \
       ((zIndex + 1) < gridSizeZ_ &&                                           \
        !data[xIndex + ((zIndex + 1) * gridSizeX_)]) ||                        \
...
void Water::buildBankDistTable()
{
    ...
    struct BWPolygon
    {
        ...
        inline void Add(float x, float y) { vertices.push_back(Vector2(x, y)); }
        // returns false if none of bank line vertices passed interpolation
        // factor or result dir == 0
        inline bool DirectionAndDistance(Vector2  pos,
                                         Vector2& dir,
                                         float&   minDist)
        {
            bool result = false;
            ...
            for (BW::list<Vector2>::iterator it = vertices.begin();
                 it != vertices.end();
                 it++) {
                Vector2 delta = (*it) - pos;
                float   dist  = delta.lengthSquared();
                if (dist < Water::s_fBankLineInterpolationFactor &&
                    dist > 0.0f) {
                    dir += delta / (dist * dist);
                    if (dist < minDist)
                        minDist = dist;
...
void Water::createCells()
{
    BW_GUARD;
    if (enableSim_) {
        float simGridSize = (ceilf(state_.simCellSize_ / state_.tessellation_) *
                             state_.tessellation_);
        ...
        for (float y = 0.f; y < state_.size_.y; y += simGridSize) {
            ...
            // TODO: if the extra bit is really small... just enlarge the
            // current cell??? hmmmm
        ...
        // If the simulation is disabled, dont divide the surface.. just use one
        // cell..
...
void Water::doCreateTables(void* self)
{
    ...
    if (Water::stillValid(selfWater)) {
        ...
        selfWater->enableSim_ =
          selfWater->enableSim_ &&
          Moo::rc().supportsTextureFormat(D3DFMT_A16B16G16R16F) &&
          selfWater->state_.useSimulation_;
    ...
    if (Water::backgroundLoad()) {
        ...
        FileIOTaskManager::instance().addBackgroundTask(
          new CStyleBackgroundTask("Water Res Loader Task",
                                   &Water::doLoadResources,
                                   this,
                                   &Water::onLoadResourcesComplete,
                                   this));
    } else {
...
void Water::setup2ndPhase()
{
    ...
    DEBUG_MSG("Water::Water: using %d vertices out of %d\n",
              nVertices_,
              gridSizeX_ * gridSizeZ_);

    float simGridSize =
      ceilf(state_.simCellSize_ / state_.tessellation_) * state_.tessellation_;

    int  cellX     = int(ceilf(state_.size_.x / simGridSize));
    int  cellY     = int(ceilf(
      state_.size_.y / simGridSize)); // TODO: need to use the new cell size! :D
    ...
    for (uint i = 0; i < cellCount; i++) {
        ...
        if (enableSim_) {
            ...
            cells_[i].initEdge(3,
                               (posX >= (col + cellX)) ? NULL : &cells_[posX]);
            cells_[i].initEdge(
              2, (uint(posY) >= cellCount) ? NULL : &cells_[posY]);
        }
    }
    ...
    bb_.setBounds(Vector3(-state_.size_.x * 0.5f, -1.f, -state_.size_.y * 0.5f),
                  Vector3(state_.size_.x * 0.5f, 1.f, state_.size_.y * 0.5f));
    ...
    // go through all the points and find the lowest terrain point.... use that
    // for the BB depth

    bbDeep_ = bb_;
    bbDeep_.setBounds(bbDeep_.minBounds() - Vector3(0.f, 2.f, 0.f),
                      bbDeep_.maxBounds());

    bbActual_.setBounds(
      Vector3(-state_.size_.x * 0.5f, -state_.depth_, -state_.size_.y * 0.5f),
      Vector3(state_.size_.x * 0.5f, 0.f, state_.size_.y * 0.5f));
...
void Water::doLoadResources(void* self)
{
    ...
    Water* selfWater = static_cast<Water*>(self);
    if (Water::stillValid(selfWater)) {
        Moo::TextureManager::instance()->getAnimationSet(
          selfWater->resources_.waveTexture_,
          &(selfWater->normalTextures_),
          selfWater->state_.waveTextureNumber_);

        selfWater->screenFadeTexture_ =
          Moo::TextureManager::instance()->get(s_screenFadeMap);
        selfWater->foamTexture_ = Moo::TextureManager::instance()->get(
          selfWater->resources_.foamTexture_);
        selfWater->reflectionCubeMap_ = Moo::TextureManager::instance()->get(
          selfWater->resources_.reflectionTexture_);
...
void Water::onLoadResourcesComplete(void* self)
{
    ...
    if (Water::stillValid(selfWater)) {
        if (selfWater && Waters::s_effect_) {
            selfWater->paramCache_->effect(
              Waters::s_effect_->pEffect()->pEffect());

            // Animate normal map texture
            int timeInt       = (int)floor(selfWater->animTime_ *
                                     selfWater->state_.animationSpeed_);
            int animFrameNum1 = timeInt % selfWater->state_.waveTextureNumber_;
            int animFrameNum2 =
              (timeInt + 1) % selfWater->state_.waveTextureNumber_;
            if (selfWater->normalTextures_) {
                if (selfWater->normalTextures_[animFrameNum1].exists())
                    selfWater->paramCache_->setTexture(
                      "normalMap1",
                      selfWater->normalTextures_[animFrameNum1]->pTexture());
                if (selfWater->normalTextures_[animFrameNum2].exists())
                    selfWater->paramCache_->setTexture(
                      "normalMap2",
                      selfWater->normalTextures_[animFrameNum2]->pTexture());
                selfWater->paramCache_->setFloat("animationInterpolator",
                                                 selfWater->animTime_ -
                                                   animFrameNum1);
            }
            ...
            if (selfWater->screenFadeTexture_)
                selfWater->paramCache_->setTexture(
                  "screenFadeMap", selfWater->screenFadeTexture_->pTexture());
            if (selfWater->foamTexture_)
                selfWater->paramCache_->setTexture(
                  "foamMap", selfWater->foamTexture_->pTexture());
            if (selfWater->reflectionCubeMap_ &&
                WaterSceneRenderer::s_simpleScene_)
                selfWater->paramCache_->setTexture(
                  "reflectionCubeMap",
                  selfWater->reflectionCubeMap_->pTexture());
        }

        if (selfWater && Waters::s_simulationEffect_) {
            ComObjectWrap<ID3DXEffect> pEffect =
              Waters::s_simulationEffect_->pEffect()->pEffect();

            float borderSize =
              Waters::s_highQuality_ ? (float)SIM_BORDER_SIZE : 0.0f;
            float pixSize = 1.0f / 256.0f;
            float pix2    = 2.0f * borderSize * pixSize;

            Vector4 cellInfo(
              borderSize * pixSize, 1 - (borderSize * pixSize), pix2, 0.0f);
...
void Water::createUnmanagedObjects()
{
    ...
    if (Waters::s_effect_) {
        ...
        WaterCell::SimulationCellPtrList::iterator cellIt =
          activeSimulations_.begin();
    ...
    if (Renderer::pInstance()->pipelineType() ==
        IRendererPipeline::TYPE_DEFERRED_SHADING)
        format = D3DFMT_A16B16G16R16F;
    bool inited = createRenderTarget(
      s_rt, bbDesc.Width, bbDesc.Height, format, "Water bbCopy", true);
}
...
void Water::createManagedObjects()
{
    ...
    if (Moo::rc().device()) {
        ...
        if (s_quadVertexBuffer_->reset(4) && s_quadVertexBuffer_->lock()) {
            ...
            ERROR_MSG("Water::createManagedObjects: "
...
void Water::renderSimulation(float dTime)
{
    ...
    // TODO: move the rain simulation stuff out of the individual water
    // simulation and
    ...
    if (raining_) {
        ...
        if (rainMark != Waters::s_nextMark_) {
            ...
            SimulationManager::instance().simulateRain(
              dTime, Waters::instance().rainAmount(), simulationEffect_);
        }
    }

    // TODO:
    //  -perturbations should not be passed to a cell if its too far away from
    //  the camera -cell with the closest perturbation to the camera should take
    //  priority (along with its neighbours)

    // TODO: only check activity when in view...however, keep simulating
    ...
    ComObjectWrap<ID3DXEffect> pEffect =
      simulationEffect_->pEffect()->pEffect();

    Vector4 psSimulationPositionWeighting(
      state_.consistency_ + 1.0f, state_.consistency_, 0.0f, 0.0f);
    ...
    if (activeSimulations_.size()) {
        ...
        WaterCell::SimulationCellPtrList::iterator it =
          activeSimulations_.begin();
        for (; it != activeSimulations_.end(); it++) {
            if ((*it)) {
                ...
                WATER_STAT(s_activeEdgeCellCount = (*it)->edgeActivation()
                                                     ? s_activeEdgeCellCount + 1
                                                     : s_activeEdgeCellCount);
                WATER_STAT(s_activeCellCount++);

                // only sim + tick if not already ticked
                if ((*it)->mark() < Waters::s_nextMark_) {
                    (*it)->simulate(
                      simulationEffect_, dTime, Waters::instance());
                    (*it)->tick();
              ...
              .simulationEnabled()) // only do for high detail settings
        {
            ...
            for (it = activeSimulations_.begin();
                 it != activeSimulations_.end();
                 it++) {
            ...
            for (uint32 xIndex = 0; xIndex < gridSizeX_; xIndex++) {
                ...
                if (wIndex_[index] >= 0) {
                    ...
                    for (uint vIdx = 0; vIdx < remappedVerts_.size(); vIdx++) {
                        BW::map<uint32, uint32>& mapping = remappedVerts_[vIdx];
                        ...
                        if (it != mapping.end()) {
                            ...
                            pVert.uv_.set(xT / state_.textureTessellation_,
                                          zT / state_.textureTessellation_);
                            pVert.normal_.set(bankDirIt->first.x,
                                              bankDirIt->first.y,
                                              bankDirIt->second);
                        }
                    }
                    if (wIndex_[index] <= (int32)Moo::rc().maxVertexIndex()) {
                        ...
                        vb0It->uv_.set(xT / state_.textureTessellation_,
                                       zT / state_.textureTessellation_);
                        vb0It->normal_.set(bankDirIt->first.x,
                                           bankDirIt->first.y,
                                           bankDirIt->second);
                        ++vb0It;
                    }
...
void Water::setupRenderState(float dTime)
{
    ...
    if (firstTime) {
        MF_WATCH("Client Settings/Water/texScale",
                 s_fTexScale,
                 Watcher::WT_READ_WRITE,
                 "test scaling");
        MF_WATCH("Client Settings/Water/BankLineInterpolationFactor",
                 s_fBankLineInterpolationFactor,
                 Watcher::WT_READ_WRITE,
                 "Minimal distance to bankline vertex to count it's weight in "
                 "bankDir");
        firstTime = false;
    }
    ...
    if (Renderer::instance().pipelineType() ==
        IRendererPipeline::TYPE_DEFERRED_SHADING) {
        paramCache_->setFloat("foamIntersectionFactor",
                              state_.foamIntersection_);
    ...
    // TODO: branch off and setup the surface for simple env. map. reflection if
    // the quality is set to low...

    Vector4 distortionScale(state_.reflectionScale_,
                            state_.reflectionScale_,
                            state_.refractionScale_,
                            state_.refractionScale_);
    ...
    paramCache_->setFloat(
      "sunScale", Waters::instance().insideVolume() ? 0.f : state_.sunScale_);
    paramCache_->setFloat(
      "smoothness",
      Math::lerp(Waters::instance().rainAmount(), state_.smoothness_, 1.f));

    if (waterScene_ && waterScene_->reflectionTexture()) {
        paramCache_->setTexture("reflectionMap",
                                waterScene_->reflectionTexture()->pTexture());
    }
    if (screenFadeTexture_)
        paramCache_->setTexture("screenFadeMap",
                                screenFadeTexture_->pTexture());
    if (reflectionCubeMap_ && WaterSceneRenderer::s_simpleScene_)
        paramCache_->setTexture("reflectionCubeMap",
                                reflectionCubeMap_->pTexture());
    ...
    if (normalTextures_) {
        if (normalTextures_[animFrameNum1].exists())
            paramCache_->setTexture("normalMap1",
                                    normalTextures_[animFrameNum1]->pTexture());
        if (normalTextures_[animFrameNum2].exists())
            paramCache_->setTexture("normalMap2",
                                    normalTextures_[animFrameNum2]->pTexture());
    }
    paramCache_->setFloat("animationInterpolator",
                          animTime_ * state_.animationSpeed_ - timeInt);

    if (reflectionCubeMap_ && state_.visibility_ == Water::INSIDE_ONLY &&
        WaterSceneRenderer::s_simpleScene_) {
        simpleReflection_ = 0.0f;
        paramCache_->setTexture("reflectionCubeMap",
                                reflectionCubeMap_->pTexture());
        ...
        float          camDist =
          (Vector3(camPos.x, camPos.y, camPos.z) - waterPos).length();
        float dist = camDist - this->size().length() * 0.5f;

        // lerp distance to water between cull fade start and cull distance
        // (fade end) as [0.0-1.0]
        dist = Math::clamp(s_sceneCullDistance_ - s_sceneCullFadeDistance_,
                           dist,
                           s_sceneCullDistance_);
        simpleReflection_ =
          Math::lerp(dist,
                     s_sceneCullDistance_ - s_sceneCullFadeDistance_,
                     s_sceneCullDistance_,
                     0.0f,
                     1.0f);
    }
 ...
 *
 * TODO: base activity on water visibility... (perhaps just restrict adding new
 * movements?)
 */
...
{
    BW_GUARD;
    if (this->shouldDraw() && this->drawMark() == (Waters::nextDrawMark()) &&
...
void Water::updateSimulation(float dTime)
{
    ...
    if (enableSim_) {
        raining_ = (Waters::instance().rainAmount() ||
                    SimulationManager::instance().rainActive());
        time_ += dTime * 30.f;
        while (time_ >= 1) {
            time_ -= floorf(time_);
...
void Water::draw(Waters& group, float dTime)
{
    ...
    if (visible_ && inited) {
        ...
        Moo::rc().setRenderState(D3DRS_FILLMODE,
                                 group.drawWireframe_ ? D3DFILL_WIREFRAME
                                                      : D3DFILL_SOLID);
        ...
        for (uint i = 0; i < cells_.size(); i++) {
            ...
            if (s_debugCell_ >= 0 && (i != s_debugCell_ && i != s_debugCell2_))
                ...
                //       should really be combining multiple cells into a single
                //       draw call..
                // TODO: better culling under terrain (linked to the vertex mesh
                // generation)
#ifdef EDITOR_ENABLED
...
#endif
            {
                if (cells_[i].cellDistance() >
                    s_cullDistance_ * Moo::rc().camera().farPlane())
                    continue;
            }
            ...
            if (Renderer::instance().pipelineType() ==
                ...
                if (raining_) {
                    ...
                    if (usingSim) {
                        ...
                        paramCache_->setTexture(
                          "simulationMap", cells_[i].simTexture()->pTexture());
                        paramCache_->setTexture("rainSimulationMap",
                        ...
                        paramCache_->setTexture("simulationMap",
                                                SimulationManager::instance()
                                                  .rainTexture()
                                                  ->pTexture());
                        paramCache_->setTexture("rainSimulationMap",
                    ...
                    paramCache_->setBool("useSimulation", true);
                    paramCache_->setTexture("simulationMap",
                                            cells_[i].simTexture()->pTexture());
                } else
                    paramCache_->setBool("useSimulation", false);

                if (usingSim) {
                    // TODO: clean this up..... move things into the cell
                    // class..
                    ...
                    float simGridSize =
                      ceilf(state_.simCellSize_ / state_.tessellation_) *
                      state_.tessellation_;
            ...
            if (cells_[i].bind()) {
                ...
                for (uint32 pass = 0; pass < Waters::s_effect_->numPasses();
                     pass++) {
    ...
    if (s_debugSim_) {
        for (uint i = 0; i < cells_.size(); i++) {
            ...
            Vector3 cX0Y0(
              cells_[i].min().x - xoff, 1.f, cells_[i].min().y - yoff);
            Vector3 cX0Y1(
              cells_[i].min().x - xoff, 1.f, cells_[i].max().y - yoff);
            Vector3 cX1Y0(
              cells_[i].max().x - xoff, 1.f, cells_[i].min().y - yoff);
            Vector3 cX1Y1(
              cells_[i].max().x - xoff, 1.f, cells_[i].max().y - yoff);
 ...
 */
bool Water::addMovement(const Vector3& from,
                        const Vector3& to,
...
void Waters::setQualityOption(int optionIndex)
{
    ...
    WaterSceneRenderer::s_drawDynamics_ =
      qo.isSet(QualityOption::DRAW_DYNAMICS);
    WaterSceneRenderer::s_drawPlayer_ = qo.isSet(QualityOption::DRAW_PLAYER);
    WaterSceneRenderer::s_simpleScene_ =
      qo.isSet(QualityOption::USE_SIMPLE_SCENE);
...
void Waters::init()
{
    ...
    isVisible = true;
#endif
    MF_WATCH("Visibility/water",
             isVisible,
             Watcher::WT_READ_WRITE,
             "Is water and water reflections visible.");
    ...
    bool supported =
      Moo::rc().vsVersion() >= 0x300 && Moo::rc().psVersion() >= 0x300;

    //--
    qualitySettings_ =
      Moo::makeCallbackGraphicsSetting("WATER_QUALITY",
                                       "Water Quality",
                                       *this,
    ...
    //-- ToDo (b_sviglo): reconsider initial state values. Try to avoid this set
    //of static constant
    ...
    m_qualityOptions.push_back(QualityOption(
      1.0f,
      QualityOption::DRAW_TREES | QualityOption::DRAW_DYNAMICS |
        QualityOption::DRAW_PLAYER | QualityOption::USE_HIGH_QUALITY |
        QualityOption::USE_SIMULATION)); //-- MAX

    m_qualityOptions.push_back(
      QualityOption(1.0f,
                    QualityOption::DRAW_TREES | QualityOption::DRAW_PLAYER |
                      QualityOption::USE_HIGH_QUALITY)); //-- HIGH

    m_qualityOptions.push_back(QualityOption(
      0.5f,
      QualityOption::DRAW_TREES | QualityOption::DRAW_PLAYER)); //-- MEDIUM

    m_qualityOptions.push_back(QualityOption(
      0.25f,
      QualityOption::USE_SIMPLE_SCENE | QualityOption::USE_CUBE_MAP)); //-- LOW
    ...
    if (Water::backgroundLoad()) {
        ...
        FileIOTaskManager::instance().addBackgroundTask(
          new CStyleBackgroundTask("Water Res Loader Task",
                                   &Waters::loadResources,
                                   this,
                                   &Waters::loadResourcesComplete,
                                   this));
    } else {
        loadResources(this);
  ...
  , time_(0.f)
{
    ...
    static bool firstTime = true;
    if (firstTime) {
        MF_WATCH("Client Settings/Water/draw",
                 s_drawWaters_,
                 Watcher::WT_READ_WRITE,
                 "Draw water?");
        MF_WATCH("Client Settings/Water/wireframe",
                 drawWireframe_,
                 Watcher::WT_READ_WRITE,
                 "Draw water in wire frame mode?");
        MF_WATCH("Client Settings/Water/character impact",
                 movementImpact_,
                 Watcher::WT_READ_WRITE,
                 "Character simulation-impact scale");
        MF_WATCH("Client Settings/Water/drawShadows",
                 s_enableShadows,
                 Watcher::WT_READ_WRITE,
                 "Draw shadows?");

#ifdef DEBUG_WATER
        MF_WATCH("Client Settings/Water/Stats/water count",
                 s_waterCount,
                 Watcher::WT_READ_ONLY,
                 "Water Stats");
        MF_WATCH("Client Settings/Water/Stats/visible count",
                 s_waterVisCount,
                 Watcher::WT_READ_ONLY,
                 "Water Stats");
        MF_WATCH("Client Settings/Water/Stats/cell count",
                 s_activeCellCount,
                 Watcher::WT_READ_ONLY,
                 "Water Stats");
        MF_WATCH("Client Settings/Water/Stats/edge cell count",
                 s_activeEdgeCellCount,
                 Watcher::WT_READ_ONLY,
                 "Water Stats");
        MF_WATCH("Client Settings/Water/Stats/impact movement count",
                 s_movementCount,
                 Watcher::WT_READ_ONLY,
                 "Water Stats");
#endif // DEBUG_WATER
 ...
 *
 * @param from  World space position, defining the start of the line moving
 * through the water.
 * @param to    World space position, defining the end of the line moving
 * through the water.
 * @param diameter  The diameter of the object moving through the water.
 */
void Waters::addMovement(const Vector3& from,
                         const Vector3& to,
                         const float    diameter)
{
    BW_GUARD;
...
void Waters::checkAllListeners()
{
    ...
    {
      public:
        CallbackArguments(PyObjectPtr pCallback,
                          bool        entering,
                          PyObjectPtr pWater)
    ...
    while (it != end) {
        ...
        if (!l.inside()) {
            ...
            for (uint i = 0; i < this->size(); i++) {
                ...
                if (inside) {
                    firedCallbacks.push_back(CallbackArguments(
                      l.callback(), true, water->pPyVolume()));
                    l.water(water);
                    break;
                }
            }
        } else {
            // If already in water, only check if you are getting out of that
            // water.
            ...
            for (uint i = 0; i < this->size(); i++) {
                ...
                if (l.water() == water) {
                    ...
                    if (!inside) {
                        firedCallbacks.push_back(CallbackArguments(
                          l.callback(), false, water->pPyVolume()));
                        l.water(NULL);
                    }
                    break;
                }
            ...
            if (!foundWater) {
                firedCallbacks.push_back(
                  CallbackArguments(l.callback(), false, NULL));
                l.water(NULL);
            }
        }
        it++;
 ...
 *  @param  callback        A callback fn that takes one boolean argument and
 * one PyWaterVolume argument.
 */
/**
 ...
 */
uint32 Waters::addWaterVolumeListener(MatrixProviderPtr dynamicSource,
                                      PyObjectPtr       callback)
{
    BW_GUARD;
    Waters::instance().listeners_.push_back(
      VolumeListener(dynamicSource, callback));
...
void Waters::drawDrawList(float dTime)
{
    ...
    if (s_drawWaters_) {
        ...
        Moo::rc().effectVisualContext().updateSharedConstants(
          Moo::CONSTANTS_PER_SCREEN | Moo::CONSTANTS_PER_VIEW);
...
void Waters::drawMasks()
{
    ...
    if (!isVisible)
        return;
#endif
    if (s_drawWaters_ && Water::s_quadVertexBuffer_ && s_waterDrawList.size()) {
        Moo::rc().setFVF(VERTEX_TYPE::fvf());
...
void Waters::checkVolumes()
{
    ...
    //
    //       //FogController::instance().overrideFog(s_near,s_far,
    //       Colour::getUint32(s_col) );
    //       FogController::instance().overrideFog(s_near,s_far, 0xffffffff );
    //       //FogController::instance().overrideFog(s_near,s_far,
    //       Colour::getUint32FromNormalised(fogColour) );
    //   }
    //   else
    //       FogController::instance().setOverride(false);
}
