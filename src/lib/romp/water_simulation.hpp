...
class SimulationTextureBlock
{
  public:
    ...
    Moo::RenderTargetPtr previous()
    {
        return simTexture_[(simulationIndex_ + 1) % 3];
    }
    Moo::RenderTargetPtr current()
    {
        return simTexture_[(simulationIndex_ + 2) % 3];
    }
    Moo::RenderTargetPtr result()
    {
        return (simulationIndex_ > 0) ? simTexture_[(simulationIndex_ - 1) % 3]
                                      : simTexture_[0];
    }
...
class Simulation
{
  public:
    ...
    virtual void addMovement(const Vector3& from,
                             const Vector3& to,
...
class SimulationCell : public Simulation
{
  public:
    ...
    void         bindAsNeighbour(Moo::EffectMaterialPtr effect,
    ...
    static float s_hitTime, s_waveSpeedSquared, s_landScaleX, s_landScaleY,
      s_landScaleZ;
  ...
  protected:
    ...
    // void addActiveEdge(int edge) { activeEdges_ |= (1 << edge); } //not used
    // yet... byte activeEdges_;
  private:
    int                     edge_;
    float                   cellSize_;
    SimulationTextureBlock* simulationTarget_;
