...
class FontMetrics : public ReferenceCount
{
  public:
    ...
    BW::vector<BW::wstring> breakString(
      BW::wstring        wstr,
      /* IN OUT */ int&  w,
      int&               h,
      int                minHyphenWidth = 0,
      const BW::wstring& wordBreak      = L" ",
      const BW::wstring& punctuation = L",<.>/?;:'\"[{]}\\|`~!@#$%^&*()-_=+");

    void stringDimensions(const BW::string& str,
                          int&              w,
                          int&              h,
                          bool              multiline        = false,
                          bool              colourFormatting = false);
    uint stringWidth(const BW::string& str,
                     bool              multiline        = false,
                     bool              colourFormatting = false);
    void stringDimensions(const BW::wstring& str,
                          int&               w,
                          int&               h,
                          bool               multiline        = false,
                          bool               colourFormatting = false);
    uint stringWidth(const BW::wstring& str,
                     bool               multiline        = false,
                     bool               colourFormatting = false);
    ...
    static int matchColourTag(const BW::wstring& str,
                              size_t             offset,
                              uint32&            outColour);
  ...
  private:
    void stringDimensionsInternal(const BW::wstring& s,
                                  bool               multline,
                                  bool               colourFormatting,
                                  int&               w,
                                  int&               h);
    void stringDimensionsInternal(const BW::string& s,
                                  bool              multline,
                                  bool              colourFormatting,
                                  int&              w,
                                  int&              h);
