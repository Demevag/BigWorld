    ...
    if (FontManager::instance().begin(*font_)) {
        for (int row = 0; row < this->visibleHeight(); row++) {
            ...
            font_->drawWConsoleString(
              line, 0, row, (int)offset_.x, (int)offset_.y);
        }
...
void XConsole::drawCursor(float dTime)
{
    ...
    if (drawCursor) {
        ...
        font_->drawConsoleString(
          "_", cursorX_, cursorY_, (int)offset_.x, (int)offset_.y);
    }
}
...
{
    BW_GUARD;
    WARNING_MSG("The use of console's font scaling in deprecated.\n"
...
void XConsole::newline()
{
    ...
    if (cursorY_ - scrollOffset_ == this->visibleHeight_) {
        if (scrollable_) {
            ...
            wcsncpy(&buffer_[0][0],
                    &buffer_[1][0],
                    MAX_CONSOLE_WIDTH * (this->visibleHeight_ - 1));
...
void XConsole::updateVisibleArea()
{
    ...
    if (this->font_.exists()) {
        if (!this->fitToScreen_) {
            int fontWidth  = 0;
            int fontHeight = 0;
            this->font_->metrics().stringDimensions(
              L"@", fontWidth, fontHeight);
            this->visibleWidth_ = std::min(
              int(Moo::rc().screenWidth() / fontWidth), MAX_CONSOLE_WIDTH);
            MF_ASSERT(this->visibleWidth_ * fontWidth <=
                      Moo::rc().screenWidth());
            ...
            font_->fitToScreen(
              this->fitToScreen_,
              Vector2(float(IDEAL_CONSOLE_WIDTH), float(IDEAL_CONSOLE_HEIGHT)));
        }
    }
    if (oldVisibleWidth != this->visibleWidth_ ||
        oldVisibleHeight != this->visibleHeight_) {
