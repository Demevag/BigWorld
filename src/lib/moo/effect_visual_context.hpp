...
namespace Moo {
    ...
    //-- Warning: watch the structure memory alignment because we send this
    //structure as is to
    ...
    enum ESharedConstantType
    {
        ...
        CONSTANTS_ALL =
          CONSTANTS_PER_FRAME | CONSTANTS_PER_SCREEN | CONSTANTS_PER_VIEW
    };

    /**
     * This class is a helper class for setting up automatic constants while
     * rendering a visual using effect files. The automatic constants are
     * referenced by using the d3dx effect file semantics.
     */
    class EffectVisualContext
    {
      public:
        ...
        void           worldMatrixPalette(const Vector4* worldPalette,
                                          uint32         worldPaletteSize);
        const Vector4* worldMatrixPalette() const
        {
            return worldMatrixPalette_;
        }
        uint32 worldMatrixPaletteSize() const
        {
            return worldMatrixPaletteSize_;
        }

        DX::CubeTexture* pNormalisationMap()
        {
            return pNormalisationMap_.pComObject();
        };
        ...
        DX::BaseTexture* pBitwiseLUT() const
        {
            return pBitwiseLUT_->pTexture();
        }
        DX::BaseTexture* pAtan2LUT() const { return pAtan2LUT_->pTexture(); }
        ID3DXEffectPool* pEffectPool() const
        {
            return pEffectPoolHandle_.pComObject();
        }
        ...
        //-- ToDo (b_sviglo): reconsider this. Currently this stuff is used only
        //for particles rendering. But
        //--	   we are using Influx's particle system. So maybe we can safely
        //remove this without
        //--	   any outside effects and to improve simplicity of the whole
        //source code.
        ...
        //-- Note: This method mostly used for adding shared auto-constant
        //without breaking
        //--	   encapsulation between different engine's modules, however you
        //be able to add a new
        //--	   per-object constant to the EffectVisualContext, but be sure
        //that you did this
        //--	   before creating managed effect because determination of
        //per-object auto-constant
        //--	   is performed during creating managed effect file.
        void registerAutoConstant(EAutoConstantType                  type,
                                  const BW::string&                  name,
                                  const Moo::EffectConstantValuePtr& autoConst);
        void unregisterAutoConstant(EAutoConstantType type,
                                    const BW::string& name);
      ...
      private:
        ...
        // we calculate inverse world transform matrix immediately after
        // worldMatrix is set because a lot of shader constants are using it and
        // we don't want to calculate it multiple times
        ...
        typedef BW::map<EffectConstantValuePtr*, EffectConstantValuePtr>
                         ConstantMappings;
