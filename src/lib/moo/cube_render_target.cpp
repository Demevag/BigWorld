...
namespace Moo {
    ...
    /**
     *	This method creates a cubemap and a z/stencil buffer, of the supplied
     *dimensions.
    ...
    /**
     *	This method sets a render surface, it does NOT store the previous render
     *target etc, so it is important to call RenderContext::pushRendertarget if
     *you want to restore the previous render target.
     *	@param face the identifier of the cubemap face to set as a render
     *target.
    ...
    /**
     *	This method sets up the view matrix for rendering to a specified cubemap
     *face.
     *	@param face the cubemap face we want to render to.
     *	@param centre the camera position we want to render from.
     */
    void CubeRenderTarget::setCubeViewMatrix(D3DCUBEMAP_FACES face,
        ...
        if (device) {
            ...
            if (SUCCEEDED(hr)) {
                ...
                bool stencilAvailable =
                  false; // we don't care whether we have one
                DX::Surface* pDepthStencil = NULL;
                hr                         = device->CreateDepthStencilSurface(
                  cubeDimensions_,
                  cubeDimensions_,
                  rc().getMatchingZBufferFormat(
                    D3DFMT_A8R8G8B8, rc().stencilAvailable(), stencilAvailable),
                  D3DMULTISAMPLE_NONE,
                  0,
                  TRUE,
                  &pDepthStencil,
...
#if ENABLE_RESOURCE_COUNTERS
                    pDepthStencilTarget_.addAlloc(
                      BW::StringRef("CubeRenderTarget"));
#endif
                    pDepthStencil->Release();
                } else {
                    ERROR_MSG("CubeRenderTarget::create: error creating the "
                              "zbuffer: %s\n",
                              DX::errorAsString(hr).c_str());
                    release();
                ...
                ASSET_MSG("CubeRenderTarget::create: error creating the "
                          "cubetexture: %s\n",
                          DX::errorAsString(hr).c_str());
