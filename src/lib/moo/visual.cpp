...
namespace Moo {
    ...
    namespace {
        ...
        {
          public:
            uint32 premadeBSPCount() const
            {
                return (uint32)s_premadeBSPs.size();
            }

            WatchOwner()
            {
                BW_GUARD;
                MF_WATCH("Render/Visual/premadeBSP_Count",
                         *this,
                         &WatchOwner::premadeBSPCount,
                         "The size of the premade BSP Cache");
            }
        };
    ...
    // The following methods were created because they are needed by
    // VisualLoader. Most of them are inline here because VisualLoader is only
    // instantiated here.
    // -----------------------------------------------------------------------------
    ...
    inline const ComplexEffectMaterialPtr Visual::Materials::find(
      const BW::string& identifier) const
    {
        BW_GUARD;
        for (const_iterator ppMat = this->begin(); ppMat != this->end();
             ++ppMat) {
    ...
    inline void Visual::BSPCache::add(const BW::string& resourceID,
                                      BSPProxyPtr&      pBSP)
    {
        BW_GUARD;
    ...
    inline BSPProxyPtr& Visual::BSPCache::find(const BW::string& resourceID)
    {
        ...
        BSPProxyPtr&     pFoundBSP =
          (bspit != s_premadeBSPs.end()) ? bspit->second : NULL_BSPPROXYPTR;
        s_premadeBSPsLock.give();
        return pFoundBSP;
    }
    ...
    inline BSPProxyPtr Visual::loadBSPVisual(const BW::string& resourceID)
    {
        ...
        return (bspVisual && bspVisual->pBSP_) ? bspVisual->pBSP_
                                               : NULL_BSPPROXYPTR;
    }
    ...
    bool Visual::reload(bool doValidateCheck)
    {
        ...
        // Visual::validateFile can cache the bsp model, and we want the bsp
        // reload.
        BSPCache::instance().del(resourceID_);
        BSPCache::instance().del(resourceID_ + ".processed");
    ...
    namespace {
         ...
         *  @a identifiers.
         *  @return True if any identifiers couldn't be added into @a
         * identifiers because they already existed (i.e., true if we encounter
         ...
         */
        void getOffsets(Vector3&                       translationOffset,
                        ...
                        ConstraintData::ConstraintType constraintType)
        {
            ...
            switch (constraintType) {
                ...
                case ConstraintData::CT_PARENT:
                    ...
                    rotationOffset =
                      dataSection->readVector3("rotateOffset", rotationOffset);
                    break;
         ...
         *  @param constraintIdentifiers/ikHandleIdentifiers Gives the
         * identifiers for all constraints and IK handles in the system.
          ...
          const BW::unordered_set<BW::string>& ikHandleIdentifiers)
        {
            ...
            for (BW::vector<DataSectionPtr>::const_iterator cItr =
                ...
                getOffsets(
                  translationOffset, rotationOffset, *cItr, constraintType);
                ...
                if (constraintType == ConstraintData::CT_AIM) {
                    aimVector = (*cItr)->readVector3("aimVector", Vector3::K);
                    upVector  = (*cItr)->readVector3("upVector", Vector3::J);
                    worldUpVector =
                      (*cItr)->readVector3("worldUpVector", Vector3::J);
                }
                ...
                if (sourceSections.size() > 1) {
                    ...
                    // If there are multiple sources, we need to choose a
                    // different identifier for each constraint (since the
                    // runtime constraints map one source to one target). The
                    // issue with this is that if the original constraint was an
                    // input to another constraint, it will now not be found.
                    // This can only be resolved by supporting a type of
                    // constraint that's actually a group of other constraints.
                ...
                for (BW::vector<DataSectionPtr>::const_iterator sItr =
                    ...
                    getOffsets(
                      translationOffset, rotationOffset, *sItr, constraintType);
                    ...
                    if (sourceSections.size() > 1) {
                        // Choose identifier that's not in use for this
                        // constraint tied to the next source.
                        do {
                            thisSrcIdentifier += "_";
                        } while (
                          constraintIdentifiers.find(thisSrcIdentifier) !=
                          constraintIdentifiers.end());
                    }

                    constraints.push_back(ConstraintData(constraintType,
                                                         thisSrcIdentifier,
                                                         targetType,
                                                         targetIdentifier,
                                                         sourceType,
         ...
         *      have missing data.
         */
        void loadIKHandleData(BW::vector<IKHandleData>&         ikHandles,
                              const BW::vector<DataSectionPtr>& ikSections)
        {
            ...
            for (BW::vector<DataSectionPtr>::const_iterator itr =
                ...
                ikHandles.push_back(IKHandleData(
                  identifier, startJoint, endJoint, poleVector, targetPos));
            }
        }
    }
     ...
     *   @param loadingFlags Flags for partial loading (geometry-only for flora
     * for example).
     */
    void Visual::load(uint32 loadingFlags /*= LOAD_ALL*/)
    {
        ...
        Moo::ScopedResourceLoadContext resLoadCtx(
          BWResource::getFilename(resourceID_));
        ...
        while (rsit != rsend) {
            ...
            renderSet.treatAsWorldSpaceObject_ =
              renderSetSection->readBool("treatAsWorldSpaceObject");
            BW::vector<BW::string> nodes;

            // Read the node identifiers of all nodes that affect this
            // renderset.
            ...
            if (renderSet.transformNodes_.empty()) {
                // The renderset is not affected by any nodes, so we will force
                // it to be affected by the rootNode.
            ...
            while (geit != geend) {
                ...
                BW::string verticesName =
                  geometrySection->readString("vertices");
                if (verticesName.find_first_of('/') == BW::string::npos)
                    verticesName = primitivesFileName + verticesName;

                // NOTE: this streamName is not used yet, it's holding it's spot
                // for when
                //  it could be used in any refactoring of our vertex data.
                // BW::string streamName = geometrySection->readString(
                // "stream", "" ); if (streamName.find_first_of( '/' ) >=
                // streamName.size())
                ...
                geometry.vertices_ = VerticesManager::instance()->get(
                  verticesName, numNodesValidate);
                ...
                BW::string indicesName =
                  geometrySection->readString("primitive");
                if (indicesName.find_first_of('/') == BW::string::npos)
                    indicesName = primitivesFileName + indicesName;
                geometry.primitives_ =
                  PrimitiveManager::instance()->get(indicesName);
                ...
                geometrySection->openSections("primitiveGroup",
                                              primitiveGroups);

                BW::vector<DataSectionPtr>::iterator prit =
                  primitiveGroups.begin();
                BW::vector<DataSectionPtr>::iterator prend =
                  primitiveGroups.end();
                ...
                while (prit != prend) {
                    ...
                    Vector3 processedGroupOrigin =
                      primitiveGroupSection->readVector3("groupOrigin",
                                                         maxVector);
                    ...
                    if (loadingFlags == LOAD_ALL) {
                        ComplexEffectMaterialPtr complexMat =
                          new ComplexEffectMaterial();
                        complexMat->load(
                          primitiveGroupSection->openSection("material"));

                        if (complexMat->baseMaterial()->pEffect() &&
                            !complexMat->skinned() &&
                            renderSet.treatAsWorldSpaceObject_) {
                            // report an error and disable this material
                            ASSET_MSG("Using non skinned material on skinned "
                                      "vertices: %s, effect:%s\n",
                                      geometry.vertices_->resourceID().c_str(),
                                      complexMat->baseMaterial()
                                        ->pEffect()
                                        ->resourceID()
                                        .c_str());
                ...
                if (isOK_ && (loadingFlags == LOAD_ALL) && bumped &&
                    !geometry.vertices_->hasBumpedFormat()) {
                    // warning: using bumped shader on non-bumped data.
                    WARNING_MSG("Geometry using a bumped material without "
                                "having bumped information generated,"
                ...
                if (validProcessedOrigins) {
                    validProcessedOrigins =
                      geometry.primitives_->adoptGroupOrigins(
                        primitiveGroupOrigins.storage());
                }

                // If origins didn't load correctly, fallback to calculating
                // them
                if (!validProcessedOrigins) {
        ...
        uvSpaceDensity_ =
          root->readFloat("uvSpaceDensity", ModelTextureUsage::INVALID_DENSITY);
        ...
        if (constraintsSection) {
            ...
            constraintsSection->openSections("orientConstraint",
                                             orientSections);
            ...
            constraintsSection->openSections("parentConstraint",
                                             parentSections);
            ...
            constraintsSection->openSections("poleVectorConstraint",
                                             poleVectorSections);

            const std::size_t numConstraintDescriptions =
              static_cast<int64>(orientSections.size() + pointSections.size() +
            ...
            // We need to get all of the constraint names and all of the IK
            // handle names. This is so we can correctly configure the
            // constraint source and target types. We need to be able to check
            // whether they're referring to an IK handle or a constraint, and if
            // not we assume they're referring to a model node.
            ...
            if (addIKConstraintIdentifiers(constraintNames, orientSections) ||
                ...
                addIKConstraintIdentifiers(constraintNames,
                                           poleVectorSections)) {
            ...
            loadConstraintData(constraints_,
                               orientSections,
                               ConstraintData::CT_ORIENT,
                               constraintNames,
                               ikHandleNames);
            loadConstraintData(constraints_,
                               pointSections,
                               ConstraintData::CT_POINT,
                               constraintNames,
                               ikHandleNames);
            loadConstraintData(constraints_,
                               parentSections,
                               ConstraintData::CT_PARENT,
                               constraintNames,
                               ikHandleNames);
            loadConstraintData(constraints_,
                               aimSections,
                               ConstraintData::CT_AIM,
                               constraintNames,
                               ikHandleNames);
            loadConstraintData(constraints_,
                               poleVectorSections,
                               ConstraintData::CT_POLE_VECTOR,
                               constraintNames,
                               ikHandleNames);
        ...
        if ((loadingFlags == LOAD_ALL) && pFoundBSP) {
            ...
            if (bspRequiresMapping) {
                loader.remapBSPMaterialFlags(*pBSP_->pTree(),
                                             ownMaterials_,
                                             bspMaterialIDs,
                                             missingBspMaterialIDs);
            }
        }

#ifdef EDITOR_ENABLED
        if (missingBspMaterialIDs.size() > 0) {
            // add materials to the first render set geometry since we don't
            ...
            for (; it != end; ++it) {
                ...
                DataSectionPtr missingBspMaterial =
                  root->newSection("missingBspMaterial");
                missingBspMaterial = missingBspMaterial->newSection("material");
                missingBspMaterial->writeString("identifier", *it);

                // create new a material, the material will be added to the
                // visual in the ME
                ...
                renderSets_[0].geometry_[0].primitiveGroups_.push_back(
                  primitiveGroup);
            }
        }
#endif
     ...
     * for example).
     */
    Visual::Visual(const BW::StringRef& resourceID,
    ...
    bool Visual::updateBSP()
    {
        ...
        BSPMaterialIDs       missingBspMaterialIDs;
        bool                 bspRequiresMapping =
          loader.loadBSPTreeNoCache(pFoundBSP, materialIDs);
     ...
     *  @param ignoreBoundingBox if true the bounding box is assumed to be in
     * the view volume.
    ...
    /*
     *   This method sets up the lighting and the common renderstates used by
     * visuals.
    ...
    namespace {
        ...
        DrawContext::InstanceData* captureInstanceData(
          ...
          const Visual::RenderSet& renderSet)
        {
            ...
            if (isRenderSetSkinned) {
                ...
                NodePtrVector::const_iterator it =
                  renderSet.transformNodes_.begin();
                NodePtrVector::const_iterator end =
                  renderSet.transformNodes_.end();
                ...
                *data->matrix() =
                  renderSet.transformNodes_.front()->worldTransform();
            }
            return data;
        }
    }
                         ...
                         bool              useDefaultPose)
    {
        ...
        if (helper.shouldDraw(ignoreBoundingBox, bb_)) {
            // If the world transforms in our node catalogue haven't been
            // updated, then do so by traversing our original hierarchy and
            // copying the results into the global node catalogue.
            if (useDefaultPose) {
                rootNode_->traverse();
            ...
            while (rsit != rsend) {
                ...
                Visual::GeometryVector::iterator git =
                  renderSet.geometry_.begin();
                Visual::GeometryVector::iterator gend =
                  renderSet.geometry_.end();

                DrawContext::InstanceData* instanceData =
                  captureInstanceData(drawContext, renderSet);
                ...
                while (git != gend) {
                    ...
                    while (pgit != pgend) {
                        ...
                        drawContext.drawRenderOp(primitiveGroup.material_.get(),
                                                 geometry.vertices_.get(),
                                                 geometry.primitives_.get(),
    ...
    void Visual::useExistingNodes(NodeCatalogue& nodeCatalogue)
    {
        ...
        while (rsit != rsend) {
            ...
            for (NodePtrVector::iterator nit =
                   renderSet.transformNodes_.begin();
    ...
    AnimationPtr Visual::addAnimation(const BW::string& animResourceID)
    {
        BW_GUARD;
        AnimationPtr anim =
          AnimationManager::instance().get(animResourceID, rootNode_);
        if (anim) {
     ...
     *      identifier for the result is not the same as for other constraints
     * in this visual.
     */
    ...
    /**
     *  This method overrides all materials that have the given
     * materialIdentifier with the given material.
     */
    void Visual::overrideMaterial(const BW::string& materialIdentifier,
                                  const ComplexEffectMaterial& mat)
    {
        ...
        while (rsit != rsend) {
            ...
            while (git != gend) {
                ...
                PrimitiveGroupVector::iterator pgit =
                  geometry.primitiveGroups_.begin();
                PrimitiveGroupVector::iterator pgend =
                  geometry.primitiveGroups_.end();
                ...
                while (pgit != pgend) {
                    ...
                    if (primitiveGroup.material_->identifier() ==
                        materialIdentifier) {
     ...
     *  It returns the number of material pointers found (the size of the
     * vector)
     *
     *  If materialIdentifier is a wildcard "*", this function returns all
     * materials.
     */
    int Visual::gatherMaterials(const BW::string&            materialIdentifier,
        ...
        while (rsit != rsend) {
            ...
            while (git != gend) {
                ...
                while (pgit != pgend) {
                    ...
                    if (everyMaterial || ownMaterials_[mindex]->identifier() ==
                                           materialIdentifier) {
    ...
    int Visual::collateOriginalMaterials(
      BW::vector<ComplexEffectMaterialPtr>& materials)
    {
        BW_GUARD;
        materials.clear();

        BW::vector<ComplexEffectMaterialPtr>::iterator it =
          ownMaterials_.begin();
        BW::vector<ComplexEffectMaterialPtr>::iterator end =
          ownMaterials_.end();
            ...
            while (git != gend) {
                ...
                while (pgit != pgend) {
                    ...
                    const Moo::PrimitiveGroup& prims =
                      geometry.primitives_->primitiveGroup(
                        primitiveGroup.groupIndex_);
                    const Moo::IndicesHolder& indices =
                      geometry.primitives_->indices();

                    // Iterate over triangles, calculating texel density,
                    // updating functor's min
                    PrimitiveHelper::generateTrianglesFromIndices(
                      indices,
                      prims.startIndex_,
                      prims.nPrimitives_,
                      PrimitiveHelper::TRIANGLE_LIST,
                      densityFunct,
                      indices.size());
                }
                ...
                minUvDensity =
                  min(minUvDensity, densityFunct.getMinUvDensity());
    ...
    void Visual::generateTextureUsage(Moo::ModelTextureUsageGroup& usageGroup)
    {
        ...
        // Iterate over each material used by the visual and extract it's
        // textures
        BW::vector<ComplexEffectMaterialPtr>::iterator it =
          ownMaterials_.begin();
        BW::vector<ComplexEffectMaterialPtr>::iterator end =
    ...
    namespace {
        ...
        template <>
        void memcpy_strided<0>(void*  dest,
                               void*  src,
                               size_t elements,
                               size_t elementSize)
        {
        ...
        bool copyData(const VertexFormat& format,
                      PrimitivePtr&       prims,
                      VerticesPtr&        verts,
                      ...
                      uint32&             retNumIndices)
        {
            ...
            if (vb.valid()) {
                ...
                const uint32 srcSize =
                  nVertices * format.streamStride(sourceStreamIndex);
                ...
                if (vl) {
                    ...
                    const uint32 dstSize =
                      verts->nVertices() *
                      retFormat.streamStride(destStreamIndex);
                    ...
                    VertexFormat::ConversionContext vfConversion(&retFormat,
                                                                 &format);
                    MF_ASSERT(vfConversion.isValid());
                    bool converted =
                      vfConversion.convertSingleStream(retPVertices,
                                                       destStreamIndex,
                                                       vl,
                                                       sourceStreamIndex,
                                                       nVertices);
     ...
     *  @param retNumVertices   an integer reference that will be set to the
     * number of vertices in the new array of vertices
     *  @param retNumIndices    an integer reference that will be set to the
     * number of indices in the new array of indices
     ...
     */
    bool Visual::createCopy(const VertexFormat& retFormat,
                            void*&              retPVertices,
                            IndicesHolder&      retPIndices,
                            uint32&             retNumVertices,
                            uint32&             retNumIndices,
        ...
        while (rsit != rsend) {
            ...
            while (git != gend) {
                ...
                if (geometry.primitiveGroups_[0].material_)
                    material =
                      geometry.primitiveGroups_[0].material_->baseMaterial();
                else
                    material = NULL;

                const VertexFormat& pFormat =
                  geometry.vertices_->pDecl()->format();
                success = copyData(pFormat,
                                   prims,
                                   verts,
                                   retFormat,
                                   retPVertices,
     ...
     *
     *  Currently this method is only used by MeshParticleRenderer's
     * BonedVisualMPT class.
     */
    bool Visual::primGroup0(Moo::VerticesPtr&  retVerts,
                            Moo::PrimitivePtr& retPrim,
                            PrimitiveGroup*&   retPPrimGroup)
    {
        BW_GUARD;
        // get render set #0
        ...
        while (rsit != rsend) {
            ...
            while (git != gend) {
                ...
                while (pgit != pgend) {
                    ...
                    geometry.primitives_->drawPrimitiveGroup(
                      primitiveGroup.groupIndex_);
                }
            }
        }
    ...
    {
        BW_GUARD;
        MF_ASSERT(numInstances > 0 &&
                  "Number of instances has to be at least 1.");
        ...
        Moo::rc().device()->SetStreamSourceFreq(
          0, (D3DSTREAMSOURCE_INDEXEDDATA | numInstances));
        Moo::rc().device()->SetStreamSourceFreq(
          InstancingStream::STREAM_NUMBER, (D3DSTREAMSOURCE_INSTANCEDATA | 1));
        instancingStream_->set(Moo::InstancingStream::STREAM_NUMBER,
        ...
        Moo::rc().device()->SetStreamSourceFreq(InstancingStream::STREAM_NUMBER,
                                                1);
    ...
    void Visual::getVertices(BW::map<void*, int>& vertices)
    {
        ...
        while (rsit != rsend) {
            ...
            while (git != gend) {
                ...
                ++git;
                vertices.insert(
                  std::make_pair(geometry.vertices_.get(),
                                 geometry.vertices_->vertexBuffer().size()));
            }
        }
    }
                ...
                if (geometry.vertices_) {
                    ...
                    if (SUCCEEDED(vb.lock(0, vb.size(), &pData, 0))) {
                        ...
                        for (size_t i = 0; i < nVetrs; ++i) {
                            ...
                            pData =
                              (char*)pData + geometry.vertices_->vertexStride();
                        }
                        vb.unlock();
                    }
                }
      ...
      Visual::RenderSet* pRenderSet)
    {
        if (pRenderSet->treatAsWorldSpaceObject_) {
            ...
            rc().effectVisualContext().worldMatrixPalette(matrixPalette_,
                                                          paletteSize);
        } else {
            // assign a single world matrix
            rc().effectVisualContext().worldMatrix(
              &pRenderSet->transformNodes_.front()->worldTransform());
        }
    }
    ...
    namespace BSPTreeHelper {
         ...
         */
        void createVertexList(const BSPTree&               source,
