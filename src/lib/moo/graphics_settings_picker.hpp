...
namespace Moo {
    ...
    class GraphicsSettingsPicker
    {
      public:
        ...
        bool getSettings(const D3DADAPTER_IDENTIFIER9& AdapterID,
    ...
    // This class auto detects the system configuration and chooses the best
    // matching options for the game
    class GraphicsSettingsAutoDetector
    {
      ...
      public:
        ...
        ~GraphicsSettingsAutoDetector();
        bool              init();
        bool              getSettings(SettingsVector& out,
                                      int             overrideSettingsGroup = NO_OVERRIDE);
        void              readConfigSetting(const DataSectionPtr& pConfig,
                                            const char*           name,
      ...
      private:
        ...
        void              addSettingsGroup(const DataSectionPtr& pSection,
                                           const char*           name,
                                           SystemPerformance     quality);
        static int64      getVideoMemory();
        int64             GetCPUFrequency();
        int64             GetTicks();
        static HRESULT    GetVideoMemoryViaDXGI(HMONITOR hMonitor,
        ...
        bool GetDeviceEnumerator(IWbemServices*         pSvc,
                                 IWbemLocator*          pLoc,
                                 BSTR                   wmiQuery,
                                 IEnumWbemClassObject** pEnumerator);
        INT  GetDeviceObjectsInfo(IEnumWbemClassObject* pEnumerator,
        ...
        SystemPerformance CheckNvidiaGPUId(
          const char*       deviceId,
          SystemPerformance curPerformance = MIN);
        SystemPerformance CheckAtiGPUId(const char*       deviceId,
                                        SystemPerformance curPerformance = MIN);
        SystemPerformance CheckIntelGPUId(
          const char*       deviceId,
          SystemPerformance curPerformance = MIN);
        void readVendorSetting(const DataSectionPtr& pSection,
                               SystemPerformance     level,
                               const char*           name,
                               Vendors               vendor);
