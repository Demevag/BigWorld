...
namespace Moo {
     ...
     *	option. For examples on how to use this class, see
     *moo/managed_effect.hpp or the CallbackGraphicsSetting class.
     *
     *	An option can be selected via the selectOption method. Calling this
     *	method triggers the onOptionSelected virtual function (unless the
     *setting is flagged as delayed). Client systems must always override this
     *function. GraphicsSetting objects should be registered with
     *GraphicsSettings::add(). They can be listed with
     *GraphicsSettings::settings().
     *
     *	Settings flagged with &lt;delayed> do not get their activeOption
     *property and onOptionSelected virtual method triggered when their active
     *option change. Instead, the call is delayed until
     *GraphicsSettings::commitPending() is called. Settings should set this flag
     *in two cases: (1) for closely related settings that can gain from being
     *processed as a batch (see texture_manager.cpp for an example) and (2) for
     *settings that may take a while to process. This gives the interface
     *programmer a chance to warn the user that processing the new settings may
     *take a while to complete before the client application blocks for a few
     *seconds (currently, there is no support for displaying a progress bar
     *while the settings are being processed).
     *
     *	Settings flagged with &lt;needsRestart>, when their active option
     *change, will set the global needsRestart flag in GraphicsSetting. Settings
     *that require the client application to be restarted to come into effect
     *should set this flag to true (see managed_effect for an example). The
     *global needsRestart flag can be queried with
     *GraphicsSetting::needsRestart().
     */
    class GraphicsSetting : public ReferenceCount
    {
      ...
      public:
        ...
        //-- advanced  - is this option available only in the advanced renderer
        //pipeline?
        struct Option
        {
            Option(const BW::string& label,
            ...
            bool operator==(const Option& rht) const
            {
                return m_label == rht.m_label && m_desc == rht.m_desc &&
                       m_supported == rht.m_supported &&
                       m_advanced == rht.m_advanced;
            }
        ...
        //-- Note (b_sviglo): Setting marked as advanced controls set of
        //availble options in the all other
        //--				  graphics settings.
        GraphicsSetting(const BW::string& label,
                        const BW::string& desc,
                        int               activeOption,
                        bool              delayed,
                        bool              needsRestart,
        ...
        static bool init(DataSectionPtr section,
                         int            overrideAutoDetectSettingsGroup =
                           GraphicsSettingsAutoDetector::NO_OVERRIDE);
    ...
    class CallbackGraphicsSetting : public GraphicsSetting
    {
      public:
        ...
        CallbackGraphicsSetting(const BW::string& label,
                                const BW::string& desc,
                                ClassT&           instance,
                                Function          function,
                                int               activeOption,
    ...
    /**
     *	Helper template function used to create a CallbackGraphicsSetting
     *object. Use this template function to avoid having to provide the type
     *parameters required to instantiate a CallbackGraphicsSetting object.
     */
    template <typename ClassT>
    GraphicsSetting::GraphicsSettingPtr makeCallbackGraphicsSetting(
      ...
      bool needsRestart)
    {
        ...
        return GraphicsSetting::GraphicsSettingPtr(
          new CallbackGraphicsSetting<ClassT>(label,
                                              desc,
                                              instance,
                                              function,
                                              activeOption,
    ...
    class StaticCallbackGraphicsSetting : public GraphicsSetting
    {
      public:
        ...
        StaticCallbackGraphicsSetting(const BW::string& label,
                                      const BW::string& desc,
                                      Function          function,
                                      int               activeOption,
                                      bool              delayed,
