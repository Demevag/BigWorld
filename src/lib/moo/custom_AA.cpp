...
namespace Moo {
    ...
    void CustomAA::mode(uint id)
    {
        if (id < m_modesCount) {
            ...
            //-- clamp results in case if user's cfg files already have invalid
            // mode indices.
    ...
    void CustomAA::apply()
    {
        //-- if multi-sampling enabled or MRT is disabled it means that we don't
        // want to use custom
        //-- anti-aliasing, so disable it internally but save desired custom AA
        // mode.
