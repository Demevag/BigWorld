...
namespace Moo {
    ...
    {
        BW_GUARD;
        pDXBaseTexture_->QueryInterface(IID_IDirect3DTexture9,
                                        (void**)&pDXTexture_);
        this->level(0);
    }
    ...
    bool TextureExposer::level(int lod)
    {
        ...
        if (lod < 0 || pDXTexture_ == NULL)
            return false;
        if (pDXTexture_->GetLevelDesc(lod, &desc) != D3D_OK)
            return false;

        D3DLOCKED_RECT lockedRect;

        textureLockWrapper_.init(pDXTexture_);
        HRESULT hr = textureLockWrapper_.lockRect(
          lod, &lockedRect, NULL, D3DLOCK_NOSYSLOCK | D3DLOCK_READONLY);
