...
namespace Moo {
     ...
     */
    DynamicIndexBufferBase::DynamicIndexBufferBase(DWORD     usage,
                                                   D3DFORMAT format)
      : locked_(false)
      , lockBase_(0)
      , lockIndex_(0)
     ...
     *
     *	@return A reference to the locked portion of memory representing the
     *buffer.
     */
    IndicesReference DynamicIndexBufferBase::lock(uint32 nLockIndices)
    {
        ...
        if (Moo::rc().device() != NULL) {
            ...
            if (maxIndices_ < nLockIndices) {
                // If not release the buffer so it can be recreated with the
                // right size.
                release();
            ...
            if (!indexBuffer_.valid()) {
                // Create a new buffer
                if (FAILED(indexBuffer_.create(
                      maxIndices_, format_, usage_, D3DPOOL_DEFAULT))) {
            ...
            if (lockBuffer) {
                ...
                // Can we fit our vertices in the so far unused portion of the
                // indexbuffer?
    ...
    IndicesReference DynamicIndexBufferBase::lock2(uint32 nLockIndices)
    {
        ...
        if (Moo::rc().device() != NULL) {
            ...
            if (lockBuffer) {
                ...
                // Can we fit our vertices in the sofar unused portion of the
                // indexbuffer?
     ...
     *	@return		A dynamic index buffer that corresponds to the desired
     *format.
     */
