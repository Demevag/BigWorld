...
namespace {
     ...
     */
    void PNGAPI pngWriteBuffer(png_structp pngPtr,
                               png_bytep   bytes,
                               png_size_t  size)
    {
        BW::vector<BinaryPtr>* buffers = (BW::vector<BinaryPtr>*)pngPtr->io_ptr;
        BinaryPtr              buffer =
          new BinaryBlock(bytes, size, "BinaryBlock/PNGWriter");
     ...
     */
    void PNGAPI pngReadBuffer(png_structp pngPtr,
                              png_bytep   bytes,
                              png_size_t  size)
    {
        BW_GUARD;
     ...
     */
    BinaryPtr concatenate(BW::vector<BinaryPtr> const& buffers,
    ...
    /**
     *  Error callback for png_error().  We have to throw an exception here
     * because you aren't allowed to return from a libpng error function (i.e.
     * expect a segfault very shortly if you attempt to continue execution after
     * this).
     */
...
namespace Moo {
                          ...
                          BW::string const&   resCounter)
    {
        ...
        png_structp pngPtr = png_create_write_struct_2(
          PNG_LIBPNG_VER_STRING, 0, 0, 0, 0, pngMalloc, pngFree);
        if (pngPtr == NULL)
            return NULL;
        ...
        png_set_pHYs(
          pngPtr, pngInfo, data.width_, data.height_, PNG_RESOLUTION_UNKNOWN);
        ...
        for (uint32 y = 0; y < data.height_; ++y) {
            if (data.upsideDown_)
                rowPointers[y] =
                  data.data_ + data.stride_ * (data.height_ - y - 1);
            else
    ...
    bool decompressPNG(BinaryPtr pngData, PNGImageData& imgData)
    {
        ...
        try {
            ...
            imgData.data_ =
              new uint8[imgData.width_ * imgData.height_ * bytesPP];
            if (!imgData.data_) {
            ...
            for (uint32 i = 0; i < pngInfo->height; ++i)
                png_read_row(
                  pngPtr, imgData.data_ + i * imgData.width_ * bytesPP, NULL);
