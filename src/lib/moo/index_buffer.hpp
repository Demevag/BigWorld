...
namespace Moo {
    ...
    class IndicesReference
    {
      ...
      public:
        ...
        bool valid() const { return !!indices_; }
        ...
        void copy(const IndicesReference& src,
                  int                     count,
                  int                     start      = 0,
                  int                     srcStart   = 0,
                  uint32                  vertexBase = 0)
        ...
        int            entrySize() const { return bestEntrySize(format_); }
        void*          indices() { return indices_; }
        const void*    indices() const { return indices_; }
        uint32         size() const { return size_; }
        IndexReference operator[](int32 index)
        {
            return IndexReference(*this, index);
        }
        uint32 operator[](int32 index) const { return get(index); }
    ...
    class IndicesHolder : public IndicesReference
    {
      ...
      public:
        ...
        ~IndicesHolder() { bw_safe_delete_array((unsigned char*&)indices_); }
     ...
     *	to realise the functionality. This is purely a helper for the creation
     *of the D3D index buffer objects, meaning an instance of this object can
    ...
    class IndexBuffer
    {
      ...
      public:
        ...
        HRESULT create(int         numOfIndices,
                       D3DFORMAT   format,
                       DWORD       usage,
                       D3DPOOL     pool,
                       const char* allocator = "index buffer/unknown")
        {
            ...
            // does not support it
            if (format == D3DFMT_INDEX32 && rc().maxVertexIndex() <= 0xffff)
                return hr;
            ...
            if (SUCCEEDED(
                  hr = Moo::rc().device()->CreateIndexBuffer(
        ...
        bool valid() const { return indexBuffer_.hasComObject(); }
        void release()
        {
        ...
        IndicesReference lock(DWORD flags = 0)
        {
            ...
            if (valid()) {
                ...
                // Try to lock the buffer, if we succeed update the return value
                // to use the locked vertices
                if (SUCCEEDED(indexBuffer_.pComObject()->Lock(
                      0, 0, (void**)&indices, flags))) {
        ...
        IndicesReference lock(UINT offset, UINT numOfIndices, DWORD flags)
        {
            ...
            if (valid()) {
                ...
                if (SUCCEEDED(indexBuffer_.pComObject()->Lock(
                      ...
                      numOfIndices * IndicesReference::bestEntrySize(format_),
                      (void**)&indices,
                      flags))) {
