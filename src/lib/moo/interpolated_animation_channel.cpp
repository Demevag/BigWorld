...
namespace Moo {
     ...
     */
    InterpolatedAnimationChannel::InterpolatedAnimationChannel(
      const InterpolatedAnimationChannel& other)
      ...
      const InterpolatedAnimationChannel& rhs) const
    {
        ...
        return compareChannelBase(rhs) && (scaleKeys_ == rhs.scaleKeys_) &&
      ...
      KeyVal&                                          out)
    {
        ...
        KeyIterator upperBound =
          std::upper_bound(keyFrames.begin(),
                           keyFrames.end(),
        ...
        if (upperBound == keyFrames.begin()) {
            ...
            interpolate(
              &out, &previouseKeyFrame->second, &upperBound->second, t);
        }
    }

    inline void buildTransform(const Quaternion& rot,
                               const Vector3&    scale,
                               const Vector3&    pos,
                               Matrix&           out)
    {
    ...
    void InterpolatedAnimationChannel::result(float time, Matrix& out) const
    {
        Quaternion rot;
        interpolateKeys(
          rotationKeys_, rotationIndex_, D3DXQuaternionSlerp, time, rot);
    ...
    void InterpolatedAnimationChannel::result(float           time,
    ...
    {
      public:
        bool operator()(const ::std::pair<X, Y>& pa,
                        const ::std::pair<X, Y>& pb)
        {
                    ...
                    InterpolatedAnimationChannel::IndexVector& index)
    {
        ...
        std::sort(
          vector.begin(), vector.end(), CompareFirstOfPair<float, VType>());
    ...
    void InterpolatedAnimationChannel::addScaleKey(float          time,
                                                   const Vector3& key)
    {
    ...
    void InterpolatedAnimationChannel::addPositionKey(float          time,
                                                      const Vector3& key)
    {
    ...
    void InterpolatedAnimationChannel::addRotationKey(float             time,
    ...
    void cleanKeys(BW::vector<std::pair<float, MapType>>& theMap)
    {
        ...
        // Only really should remove the end marker key if they're the only ones
        // in the file...
        if (theMap.size() == 2)
        // if ( theMap.size() > 1 )
        {
    ...
    void InterpolatedAnimationChannel::reduceRotationKeys(float cosAngleError,
                                                          Moo::NodePtr node)
    {
        BW_GUARD;
        ...
        // Adding a scaling of the error factor for nodes with a large depth
        // below them this allows the lower children nodes to be compressed alot
        // more than the ones higher on the hierachy, improving the compression.
        if (node) {
            ...
            while (stillGoing) {
                ...
                if (rit != rotationKeys_.end()) {
                    ...
                    D3DXQuaternionSlerp(&q,
                                        &first->second,
                                        &rit->second,
                                        (delit->first - first->first) /
                                          (rit->first - first->first));
        ...
        while (i != scaleKeys_.size()) {
            ...
            while (stillGoing) {
                ...
                if (sit != scaleKeys_.end()) {
                    Vector3 v = ((sit->second - first->second) /
                                 (sit->first - first->first));
                    ...
                    v.z /= delit->second.z;
                    v -= Vector3(1, 1, 1);
                    if (fabs(v.x) < scaleError && fabs(v.y) < scaleError &&
                        fabs(v.z) < scaleError) {
        ...
        while (i != positionKeys_.size()) {
            ...
            while (stillGoing) {
                ...
                if (pit != positionKeys_.end()) {
                    Vector3 v = ((pit->second - first->second) /
    ...
    void InterpolatedAnimationChannel::reduceKeyFramesTo(float rotationError,
                                                         float scaleError,
                                                         float positionError)
    {
        BW_GUARD;
    ...
    void InterpolatedAnimationChannel::preCombine(
      const AnimationChannel& rOther)
    {
        BW_GUARD;
    ...
    void InterpolatedAnimationChannel::postCombine(
      const AnimationChannel& rOther)
    {
        BW_GUARD;
        InterpolatedAnimationChannel hack;
    ...
    int InterpolatedAnimationChannel::size() const
    {
        return static_cast<int>(
          sizeof(uint) +
          scaleKeys_.size() * sizeof(ScaleKeyframes::value_type) +
          sizeof(uint) +
          positionKeys_.size() * sizeof(PositionKeyframes::value_type) +
          sizeof(uint) +
          rotationKeys_.size() * sizeof(RotationKeyframes::value_type) +
          sizeof(uint) + scaleIndex_.size() + sizeof(uint) +
          positionIndex_.size() + sizeof(uint) + rotationIndex_.size());
    }
    ...
    bool InterpolatedAnimationChannel::valid() const
    {
        ...
        for (uint i = 0; i < rotationIndex_.size(); ++i) {
            if (rotationIndex_[i] <= 0 ||
                rotationIndex_[i] > rotationKeys_.size())
                return false;
        }

        for (uint i = 0; i < positionIndex_.size(); ++i) {
            if (positionIndex_[i] <= 0 ||
                positionIndex_[i] > positionKeys_.size())
                return false;
        }
    ...
    std::ostream& operator<<(std::ostream&                       o,
                             const InterpolatedAnimationChannel& t)
    {
    ...
    */
    ...
    InterpolatedAnimationChannel::TypeRegisterer
      InterpolatedAnimationChannel::s_rego3_(
        INTERPOLATED_ANIMATION_CHANNEL_COMPRESSION_OFF,
        New3);
    InterpolatedAnimationChannel::TypeRegisterer
