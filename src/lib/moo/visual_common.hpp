...
namespace Moo {
    ...
    class VisualLoader
    {
      ...
      public:
        ...
        const BW::string& getPrimitivesResID() const
        {
            return primitivesResID_;
        }
      ...
      bool        useProcessedData)
    {
        ...
        std::replace(
          resourceBaseName.begin(), resourceBaseName.end(), '\\', '/');
     ...
     * 		1. Look in BSP cache.
     * 		2. Load pre-generated BSP from file (see loadBSPTreeNoCache()
     *method) The returned BSP is inserted into the BSP cache if it wasn't there
     *before.
     *
     ...
     * 	A bsp2 BSPTree will need the primitive groups information to complete
     *its loading. This is done by calling the remapFlags() method of the
     *BSPTree. Non-bsp2 BSPTrees are fully loaded by this function. A NULL
     *BSPTree is returned if it is not found in the cache or on disk i.e. needs
     *to be generated.
     *
     * 	Requires:
     ...
     */
    template <class VISUAL>
    bool VisualLoader<VISUAL>::loadBSPTree(
      typename VISUAL::BSPTreePtr& pBSPTree,
      BSPMaterialIDs&              materialIDs)
    {
        BW_GUARD;
        ...
        if (pBSPTree) {
            ...
            if (pPrimitivesSection) {
                ...
                if (pBSPSection) {
                    ...
                    if (pBin) {
                        ...
                        if (pMaterialIDs) {
                            ...
                            if (materialIDs.empty()) {
                                WARNING_MSG(
                                  "%s was exported using the old bsp2 "
                                  "exporter, and must be re-exported "
                                  "manually\n",
                                  resourceBaseName_.c_str());
                            }
#endif
                    ...
                    if (pBSPSection) {
                        WARNING_MSG(
                          "Primitives file contains \"bsp\" section, "
            ...
            if (pBSPData) {
                ...
                //			WARNING_MSG( "No BSP data found. Please re-export
                //%s\n", 				resourceBaseName_.c_str() );
            }
        }
     ...
     *							index currently in the world triangles of the
     *bsp.
     */
    template <class VISUAL>
    void VisualLoader<VISUAL>::remapBSPMaterialFlags(
      BSPTree&                          bspTree,
      const typename VISUAL::Materials& materials,
      const BSPMaterialIDs&             materialIDs,
      BSPMaterialIDs&                   missingBSPMaterials)
    {
        ...
        for (BSPMaterialIDs::const_iterator iterMatID = materialIDs.begin();
            ...
            // assigned materials from the object, so we didn't find the
            // material identifier in the object.  So that means we use the
            // object's material kind.
#ifdef EDITOR_ENABLED
            if (pMaterial == NULL) {

                WARNING_MSG("Model %s.model BSP is using unreferenced material "
                            "\"%s\". Please assign the correct materials and "
                            "re-export the model.\n",
                            resourceBaseName_.c_str(),
                            (*iterMatID).c_str());
            ...
            WorldTriangle::Flags flags =
              (pMaterial) ? pMaterial->getFlags(objectMaterialKind)
      ...
      typename VISUAL::Materials& materials)
    {
        ...
        for (DataSection::SearchIterator iterRenderSet =
             ...
             iterRenderSet != pVisualSection_->endOfSearch();
             ++iterRenderSet) {
