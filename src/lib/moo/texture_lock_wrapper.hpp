...
namespace Moo {
     ...
     *	It will use staging texture and propagate changes back to original
     *texture if IDirect3DDevice9Ex device is in use and original texture isn't
     *lockable
     */
    ...
    /**
     *	CubeTexture lock wrapper to make code IDirect3DDevice9Ex device
     *compatible It will use staging texture and propagate changes back to
     *original texture if IDirect3DDevice9Ex device is in use and original cube
     *texture isn't lockable
     */
    class CubeTextureLockWrapper
    {
      public:
        CubeTextureLockWrapper(
          const ComObjectWrap<DX::CubeTexture>& tex = NULL);
        ~CubeTextureLockWrapper();
        void init(const ComObjectWrap<DX::CubeTexture>& tex);
        void fini();

        HRESULT lockRect(D3DCUBEMAP_FACES face,
                         UINT             level,
                         D3DLOCKED_RECT*  pLockedRect,
                         DWORD            flags);
