...
void PyDeferredResponse::callback(ScriptTuple values)
{
    ...
    if (!methodDescription_.sendReturnValues(
          source, replyID_, replyAddr_, networkInterface_)) {
        WARNING_MSG("PyDeferredResponse::callback: "
...
void PyDeferredResponse::errback(ScriptObject failure)
{
    ...
    if (value) {
        methodDescription_.sendReturnValuesError(
          value, replyID_, replyAddr_, networkInterface_);
    }
}
...
bool MethodDescription::staticInitTwoWay()
{
    ...
    ScriptModule pModule = ScriptModule::import(
      "BWTwoWay",
    ...
    s_pCreateErrorFunc = pModule.getAttribute(funcName, ScriptErrorRetain());
                                                  ...
                                                  ScriptObject args)
{
    ...
    ScriptObject pExc(PyObject_CallFunction(
                        s_pCreateErrorFunc.get(), "sO", excType, args.get()),
...
{
    bool result =
      this->MemberDescription::parse(interfaceName,
                                     pSection,
                                     /* isForClient */ (component == CLIENT),
    ...
    if (pReplayExposureLevel) {
        ...
        if (replayExposureLevelString == "NONE") {
            ...
            ERROR_MSG(
              ...
              replayExposureLevelString.c_str(),
              name_.c_str());
 ...
 */
bool MethodDescription::areValidArgs(bool         exposedExplicit,
                                     ScriptObject args,
                                     bool         generateException) const
{
    ...
    bool result =
      args_.checkValid(ScriptTuple(args), name_.c_str(), firstOrdinaryArg);
 ...
 */
ScriptTuple MethodDescription::extractSourceEntityID(
  ScriptTuple args,
  EntityID&   sourceEntityID) const
{
 ...
 */
void MethodDescription::handleException(
  int                        replyID,
  const Mercury::Address&    replyAddr,
  Mercury::NetworkInterface& networkInterface,
  bool                       shouldPrintError) const
{
    ...
    if (shouldPrintError) {
        WARNING_MSG(
          "MethodDescription::handleException: "
 ...
 */
void MethodDescription::sendCurrentExceptionReturnValuesError(
  int                        replyID,
  const Mercury::Address&    replyAddr,
  Mercury::NetworkInterface& networkInterface)
{
    ...
    if (this->hasReturnValues()) {
        ...
        if (pAddCallbacks) {
            ScriptObject pDelayedReturn(
              new PyDeferredResponse(*this, replyID, *pReplyAddr, *pInterface),
              ScriptObject::STEAL_REFERENCE);
            ...
            this->sendReturnValues(
              returnSource, replyID, *pReplyAddr, *pInterface);
        }
    }
 ...
 *	@return	True if successful.
 */
bool MethodDescription::sendReturnValues(
  DataSource&                source,
  int                        replyID,
  const Mercury::Address&    replyAddr,
 ...
 */
void MethodDescription::sendReturnValuesError(
  const char*                excType,
  const char*                msg,
  int                        replyID,
  const Mercury::Address&    replyAddr,
...
{
#if defined(SCRIPT_PYTHON)
    ScriptObject pError = MethodDescription::createErrorObject(excType, msg);

    MethodDescription::sendReturnValuesError(
      pError, replyID, replyAddr, networkInterface);
#else  // !SCRIPT_PYTHON
                                             ...
                                             BinaryOStream& stream)
{
    ...
    ScriptObject args = pFailure.getAttribute("args", ScriptErrorClear());
...
{
    return ScriptObject(
      PyObject_GetAttrString(object.get(),
                                             ...
                                             EntityID       sourceID) const
{
    ...
    return args_.createFromStream(
      data, sink, name_, hasImplicitSourceID ? &sourceID : NULL);
}
  ...
  ScriptObject&  pResult) const
{
    ...
    if (isError) {
        ...
        pResult = MethodDescription::createErrorObject(errorType.c_str(), args);
    } else {
        ScriptDataSink sink;
 ...
 */
bool MethodDescription::createReturnValuesFromStream(BinaryIStream& data,
                                                     DataSink&      sink) const
{
 ...
 */
void MethodDescription::setExposedMsgID(int exposedID,
                                        int numExposed,
                                        const ExposedMethodMessageRange* pRange)
{
    ...
    if (pRange) {
        pRange->msgIDFromExposedID(
          exposedID, numExposed, exposedMsgID_, exposedSubMsgID_);
        ...
        if (exposedSubMsgID_ != -1 &&
            ...
            WARNING_MSG(
              "MethodDescription::setExposedMsgID: "
...
WatcherPtr MethodDescription::pWatcher()
{
    ...
    if (watchMe == NULL) {
        ...
        watchMe->addChild("interfaceName", makeWatcher(pNull->interfaceName_));
        watchMe->addChild("variableLengthHeaderSize",
                          makeWatcher(pNull->varLenHeaderSize_));
        watchMe->addChild(
          "oversizeWarnLevel",
          makeNonRefWatcher(
            *(static_cast<MemberDescription*>(pNull)),
        ...
        watchMe->addChild("internalIndex", makeWatcher(pNull->internalIndex_));
        watchMe->addChild("exposedIndex", makeWatcher(pNull->exposedIndex_));
        watchMe->addChild("exposedMsgID", makeWatcher(pNull->exposedMsgID_));
        ...
        watchMe->addChild("isReliable", makeWatcher(pNull->isReliable_));
        watchMe->addChild(
          "stats", EntityMemberStats::pWatcher(), &pNull->stats_);
    }
    return watchMe;
}
  ...
  const BW::string& componentName)
{
    if (!componentName.empty()) // it's a component
    {
        if (components_.find(BW::string()) != components_.end()) {
            ERROR_MSG(
              "MethodDescription::addImplementingComponent: "
    ...
    } else if (isComponentised_) // it's entity but method already delegated to
                                 // component
    {
        ERROR_MSG(
          "MethodDescription::addImplementingComponent: "
...
/**
 *	This method checks if the method is implemented by component with given
 *name.
 */
bool MethodDescription::isImplementedBy(const BW::string& componentName) const
{
...
{
    return flags_ == other.flags_ && args_ == other.args_ &&
