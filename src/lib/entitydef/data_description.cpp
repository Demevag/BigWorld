...
namespace {
    ...
    /*
     *	This function converts a string to the appropriate flags. It is used
     *when parsing the properties in the def files.
     *
     *	@param name The string to convert.
     *	@param flags This value is set to the appropriate flag values.
     *	@param parentName The name of the entity type to be used if a warning
     *needs to be displayed.
     *	@param propName The name of the property that these flags are associated
     *		with.
     *	@return True on success, otherwise false.
     */
    bool setEntityDataFlags(const BW::string& name,
                            int&              flags,
                            const BW::string& parentName,
    ...
    /*
     *	This helper function returns the string that is associated with the
     *input DataDescription flags.
     */
                            ...
                            PARSE_OPTIONS     options /*= PARSE_DEFAULT*/)
{
    ...
    if (options & PARSE_IGNORE_FLAGS) {
        ...
        if (!setEntityDataFlags(
              pSection->readString("Flags"), dataFlags_, parentName, name_)) {
            ERROR_MSG("DataDescription::parse: "
                      "Invalid Flags section '%s' for %s\n",
                      pSection->readString("Flags").c_str(),
                      name_.c_str());
            return false;
        }
    }
    ...
    if (pSection->readBool("Indexed", isIndexed)) {
        ...
        databaseIndexingType_ = pSection->readBool("Indexed/Unique", isUnique)
    ...
    if (this->isClientServerData()) {
        ...
        bool isExposedForReplay =
          pSection->readBool("ExposedForReplay", this->isOtherClientData());
...
//	MF_ASSERT( pInitialValue_ );
    ...
    bool isOkay = this->MemberDescription::parse(
      parentName,
      pSection,
      /* isForClient */ this->isClientServerData(),
 ...
 *								log output.
 */
bool DataDescription::addToStream(
  DataSource&    source,
...
WatcherPtr DataDescription::pWatcher()
{
    ...
    if (!watchMe) {
        ...
        watchMe->addChild(
          "type",
          new SmartPointerDereferenceWatcher(makeWatcher(&DataType::typeName)),
          &pNull->pDataType_);
        ...
        watchMe->addChild("localIndex", makeWatcher(pNull->localIndex_));
        ...
        watchMe->addChild("index", makeWatcher(&DataDescription::index));
        watchMe->addChild(
 ...
 *
 */
bool DataDescription::setAttrOn(const ScriptObject& object,
                                const ScriptObject& attr) const
{
                                         ...
                                         bool         isSlice) const
{
    ...
    char* callbackName = reinterpret_cast<char*>(alloca(callbackNameMaxSize));

    bool      needsChangePath = false;
    PyObject* pCallback       = NULL;

    if (pChangePath.get() != NULL) {
        bool* pHasNonSimpleCallback = const_cast<bool*>(
          isSlice ? &hasSliceSetterCallback_ : &hasNestedSetterCallback_);
    ...
    Script::call(pCallback, args, "DataDescription::callSetterCallback: ");
