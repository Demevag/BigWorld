    ...
    class BlobStreamElement : public DataType::StreamElement
    {
      public:
        ...
        bool fromStreamToSink(BinaryIStream& stream, DataSink& sink) const
        {
            ...
            if (stream.error()) {
                ERROR_MSG(
                  "BlobStreamElement: Not enough data on stream to read "
                  "value\n");
                return false;
            }
 ...
 *	Constructor.
 */
BlobDataType::BlobDataType(MetaDataType* pMeta)
  : StringDataType(pMeta)
{
}
...
DataType::StreamElementPtr BlobDataType::getStreamElement(
  size_t  index,
  size_t& size,
  bool& /* isNone */,
  bool /* isPersistentOnly */) const
