...
namespace BW {
    ...
    TEST_F(EntityDefUnitTestHarness, Script_ScriptDataSource_wstring_from_utf8)
    {
        ...
        ScriptString inputObject =
          ScriptString::create("\xe8\xbf\x99\xe3\x81\x93\xd0\xad");
        BW::wstring      result;
        ScriptDataSource source(inputObject);
        CHECK(source.read(result));
        // No CHECK_EQUAL for BW::wstring
    ...
    TEST_F(EntityDefUnitTestHarness, Script_ScriptDataSource_UDO_REF)
    {
        ...
        ScriptString inputObject = ScriptBlob::create(input.toString());
#endif // defined( SCRIPT_PYTHON )
    ...
    TEST_F(EntityDefUnitTestHarness, Script_ScriptDataSource_PyClass)
    {
        ...
        ClassDataType* pClassType = dynamic_cast<ClassDataType*>(pType.get());
        CHECK(pClassType);
        ...
        CHECK_EQUAL(1, index);
        pClass->setFieldValue(
          index, ScriptObject::createFrom(BW::string("TestString")));
    ...
    TEST_F(EntityDefUnitTestHarness,
           Script_ScriptDataSource_PyClass_from_mapping)
    {
        // Tests a specific behaviour of the ClassDataType. If given a Mapping
        // that isn't a PyClassDataInstance, then it will read the fields of the
        // mapping rather than its attributes.
        ScriptDict inputObject = ScriptDict::create(2);
        RETURN_ON_FAIL_CHECK(inputObject.exists());
        CHECK(inputObject.setItem(
          "int8_5", ScriptObject::createFrom((int8)5), ScriptErrorPrint()));
        CHECK(inputObject.setItem(
          "str_TestString",
    ...
    TEST_F(EntityDefUnitTestHarness,
           Script_ScriptDataSource_FixedDict_from_dict)
    {
        DataTypePtr pType = DataType::buildDataType(
        ...
        FixedDictDataType* pFixedDictType =
          dynamic_cast<FixedDictDataType*>(pType.get());
...
#if defined(SCRIPT_PYTHON)
    TEST_F(EntityDefUnitTestHarness,
           Script_ScriptDataSource_FixedDict_from_PyFixedDictDataInstance)
    {
        DataTypePtr pType = DataType::buildDataType(
          "<type> FIXED_DICT <Properties> "
          " <int8_5> <Type> INT8 </Type> </int8_5> "
    ...
    TEST_F(EntityDefUnitTestHarness,
           Script_ScriptDataSource_PyFixedDict_as_dict)
    {
        DataTypePtr pType = DataType::buildDataType(
          "<type> FIXED_DICT "
        ...
        ScriptModule fixedDictWrappers =
          ScriptModule::import("FixedDictWrappers", ScriptErrorPrint());
        RETURN_ON_FAIL_CHECK(fixedDictWrappers.exists());
        ScriptType wrapIntStrType = ScriptType::create(
          fixedDictWrappers.getAttribute("WrapIntStr", ScriptErrorPrint()));
    ...
    TEST_F(EntityDefUnitTestHarness,
           Script_ScriptDataSource_PyFixedDict_as_stream_default)
    {
        DataTypePtr pType = DataType::buildDataType(
          "<type> FIXED_DICT "
          "<implementedBy> FixedDictWrappers.noStream </implementedBy> "
        ...
        CHECK(source.readCustomType(*pFixedDictType,
                                    stream,
    ...
    TEST_F(EntityDefUnitTestHarness,
           Script_ScriptDataSource_PyFixedDict_as_stream_custom)
    {
        DataTypePtr pType = DataType::buildDataType(
          "<type> FIXED_DICT "
    ...
    TEST_F(EntityDefUnitTestHarness,
           Script_ScriptDataSource_PyFixedDict_as_section)
    {
        DataTypePtr pType = DataType::buildDataType(
          "<type> FIXED_DICT "
          "<implementedBy> FixedDictWrappers.noStream </implementedBy> "
        ...
        DataSectionPtr pStrSection =
          pSection->findChild(BW::string("str_TestString"));
        RETURN_ON_FAIL_CHECK(pStrSection);
        CHECK_EQUAL(BW::string("TestString"), pStrSection->asString());
    }

    TEST_F(EntityDefUnitTestHarness,
           Script_ScriptDataSource_PyUserType_as_stream)
    {
        DataTypePtr pType = DataType::buildDataType(
          "<type> USER_TYPE "
        ...
        UserDataType* pUserType = dynamic_cast<UserDataType*>(pType.get());
        CHECK(pUserType);

        // This should probably be in a fixture...
        ScriptModule userTypes =
          ScriptModule::import("UserTypes", ScriptErrorPrint());
        RETURN_ON_FAIL_CHECK(userTypes.exists());
        ScriptType intStrType = ScriptType::create(
          userTypes.getAttribute("IntStr", ScriptErrorPrint()));
        RETURN_ON_FAIL_CHECK(intStrType.exists());

        ScriptObject instance = intStrType.callFunction(
          ScriptArgs::create(
            ScriptObject::createFrom((int8)5),
            ScriptObject::createFrom(BW::string("TestString"))),
          ScriptErrorPrint());
        RETURN_ON_FAIL_CHECK(instance.exists());
        ...
        MemoryOStream stream;
        CHECK(source.readCustomType(*pUserType,
                                    stream,
                                    /* isPersistentOnly */ false));
    ...
    TEST_F(EntityDefUnitTestHarness,
           Script_ScriptDataSource_PyUserType_as_section)
    {
        DataTypePtr pType = DataType::buildDataType(
          "<type> USER_TYPE "
          "<implementedBy> UserTypes.intStr </implementedBy> </type>");
