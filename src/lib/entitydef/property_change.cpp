 ...
 *	This method adds the path to the owner of the changing property to a stream.
 */
void PropertyChange::addCompressedPathToStream(
  ...
  int            numClientServerProperties) const
{
    ...
    if (!this->isNestedChange()) {
        ...
        bits.add(BitReader::bitsRequired(numClientServerProperties),
                 clientServerID);
    } else {
...
void PropertyChange::addToExternalStream(BinaryOStream& stream,
                                         ...
                                         int numClientServerProperties) const
{
    this->addCompressedPathToStream(
      stream, clientServerID, numClientServerProperties);
...
void SlicePropertyChange::addExtraBits(BitWriter& writer) const
{
    ...
    const int numBitsRequired =
      BitReader::bitsRequired(static_cast<uint>(originalLeafSize_) + 1);
