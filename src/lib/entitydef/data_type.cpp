...
int EntityDef_Token = SingleTypeDataSinks_Token | SingleTypeDataSources_Token;
 ...
 */
bool DataType::addToStream(DataSource&    source,
 ...
 */
bool DataType::createFromStream(BinaryIStream& stream,
                                DataSink&      sink,
                                bool           isPersistentOnly) const
{
    bool                        result = true;
...
DataTypePtr DataType::buildDataType(DataSectionPtr pSection)
{
    ...
    if (found != s_aliases_.end()) {
        if (pSection->findChild("Default")) {
            WARNING_MSG(
              "DataType::buildDataType: New default value for "
...
DataTypePtr DataType::findOrAddType(DataTypePtr pDT)
{
    if (s_singletonMap_ == NULL)
        s_singletonMap_ = new SingletonMap();

    SingletonMap::iterator found =
      s_singletonMap_->find(SingletonPtr(pDT.get()));
...
bool DataType::initAliases()
{
    ...
    if (pAliases) {
        ...
        for (iter = pAliases->begin(); iter != pAliases->end(); ++iter) {
            ...
            if (pAliasedType) {
                s_aliases_.insert(
                  std::make_pair((*iter)->sectionName().c_str(), pAliasedType));

#ifdef EDITOR_ENABLED
                s_aliasWidgets_.insert(
                  std::make_pair((*iter)->sectionName().c_str(),
...
{
    return !pOldValue || !this->canIgnoreSelfAssignment() ||
 ...
 *	one exists.
 */
bool DataType::StreamElement::getChildType(ConstDataTypePtr& rpChildType) const
{
 ...
 *	iterating.
 */
bool DataType::StreamElement::getStructureName(BW::string& rStructureName) const
{
            ...
            : root_.getStreamElement(index_, size_, isNone_, isPersistentOnly_))
{
    ...
    MF_ASSERT((pCurrent_.get() != NULL) ||
              (!useChild_ && pChildIt_.get() == NULL && size_ == 0 &&
               index_ == 0 && !isNone_));
}
...
DataType::const_iterator::const_iterator(const const_iterator& other)
  ...
  , pChildIt_(other.pChildIt_.get() ? new const_iterator(*other.pChildIt_)
                                    : NULL)
  , index_(other.index_)
  , size_(other.index_)
                ...
                const DataType::const_iterator& rhs)
{
    ...
    if (lhs.useChild_ != rhs.useChild_ || lhs.index_ != rhs.index_ ||
        lhs.size_ != rhs.size_ || lhs.isNone_ != rhs.isNone_ ||
 ...
 *	Reference operator
 */
DataType::const_iterator::pointer DataType::const_iterator::operator->() const
{
...
DataType::const_iterator& DataType::const_iterator::operator++()
{
    ...
    if (pChildIt_.get()) {
        ...
        if (childIt != (isPersistentOnly_ ? childIt.root_.endPersistent()
                                          : childIt.root_.end())) {
    ...
    if (pCurrent_.get() == NULL) {
        ...
        pChildIt_.reset(new DataType::const_iterator(
          isPersistentOnly_ ? pChildType->beginPersistent()
                            : pChildType->begin()));
    }
