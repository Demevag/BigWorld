...
static void drawFMODGeometry(FMOD::Geometry* geometry)
{
    ...
    if (!SoundManager::FMOD_ErrCheck(
          result, "drawFMODGeometry: Couldn't get number of polygons.")) {
        return;
    }
    ...
    for (int ipoly = 0; ipoly < numPolygons; ipoly++) {
        ...
        for (int ivert = 0; ivert < numVerts; ivert += 1) {
            ...
            // v.pos_.x, v.pos_.y, v.pos_.z );
            v.colour_ = D3DCOLOR_RGBA(ivert == 0 ? 255 : 0, 255, 128, 255);
            verts.push_back(v);
        }

        Moo::rc().drawPrimitiveUP(
          D3DPT_TRIANGLEFAN, numVerts - 2, &verts[0], sizeof(Moo::VertexXYZL));
...
SoundOccluder::~SoundOccluder()
{
    ...
    for (unsigned int i = 0; i < geometries_.size(); i++) {
        ...
        SoundManager::FMOD_ErrCheck(
          result, "SoundOccluder::~SoundOccluder: Couldn't release geometry.");
    }
}
...
bool SoundOccluder::construct(Model* pModel)
{
    ...
    if (renderSets.size() && renderSets[0].geometry_.size()) {
        ...
        for (uint32 i = 0; i < geometry.primitiveGroups_.size(); i++) {
            ...
            bool  occluder =
              material->floatProperty(directOcclusion, "DirectOcclusion");
            float reverbOcclusion = 0.f;
            occluder &=
              material->floatProperty(reverbOcclusion, "ReverbOcclusion");
            bool doubleSided = false;
            material->boolProperty(doubleSided, "doubleSided");

            float tempTest;
            material->floatProperty(tempTest, "selfIllumination");

            if (occluder &&
                (directOcclusion != 0.f || reverbOcclusion != 0.f)) {
                const Moo::PrimitiveGroup& primGroup =
                  geometry.primitives_->primitiveGroup(pg.groupIndex_);
                if (geometry.primitives_->indices().format() ==
                    D3DFMT_INDEX16) {
                    const uint16* pInd =
                      (const uint16*)geometry.primitives_->indices().indices();
                    indices.insert(indices.end(),
                                   pInd + primGroup.startIndex_,
                                   pInd + primGroup.startIndex_ +
                                     primGroup.nPrimitives_ * 3);
                } else {
                    const uint32* pInd =
                      (const uint32*)geometry.primitives_->indices().indices();
                    indices.insert(indices.end(),
                                   pInd + primGroup.startIndex_,
                                   pInd + primGroup.startIndex_ +
                                     primGroup.nPrimitives_ * 3);
                ...
                if (indices.size()) {
                    ...
                    FMOD::System* pSystem;
                    FMOD_RESULT   result = SoundManager::instance()
                                           .getEventSystemObject()
                                           ->getSystemObject(&pSystem);
                    if (!SoundManager::FMOD_ErrCheck(
                          result,
                          "SoundOccluder::construct Couldn't get FMOD::System "
                          "handle"))
                        return false;
                    result =
                      pSystem->createGeometry(numTris, numVerts, &pGeometry);
                    if (!SoundManager::FMOD_ErrCheck(
                          result,
                          "SoundOccluder::SoundOccluder Couldn't create "
                          "geometry object."))
                        return false;
                    ...
                    for (int i = 0; i < numIndices; ++i) {
                        ...
                        int index; // TODO: remove 'index' (replace with NULL if
                                   // possible)
                        FMOD_RESULT result =
                          pGeometry->addPolygon(directOcclusion,
                                                ...
                                                vertices,
                                                &index);
                        ok &= SoundManager::FMOD_ErrCheck(
                          result, "SoundOccluder::SoundOccluder");
                    }
                              ...
                              float                            reverbOcclusion)
{
    ...
    if (!SoundManager::FMOD_ErrCheck(
          result, "SoundOccluder::construct Couldn't get FMOD::System handle"))
        return false;
    result = pSystem->createGeometry(numQuads, numVerts, &pGeometry);
    if (!SoundManager::FMOD_ErrCheck(
          result,
          "SoundOccluder::SoundOccluder Couldn't create geometry object."))
        return false;
    ...
    for (uint32 z = 0; z < MESH_SIZE; ++z) {
        for (uint32 x = 0; x < MESH_SIZE; ++x) {
            ...
            FMOD_RESULT result = pGeometry->addPolygon(directOcclusion,
                                                       reverbOcclusion,
                                                       doubleSided,
                                                       4,
                                                       vertices,
                                                       &index);
            ok &= SoundManager::FMOD_ErrCheck(result,
                                              "SoundOccluder::SoundOccluder");
        }
    }
...
bool SoundOccluder::update(const Vector3& position,
                           ...
                           const Vector3& up)
{
    ...
    for (unsigned int i = 0; i < geometries_.size(); i++) {
        FMOD_RESULT result =
          geometries_[i]->setPosition((FMOD_VECTOR*)&position);
        SoundManager::FMOD_ErrCheck(
          result, "SoundOccluder::update couldn't set geometry position");

        if (forward != Vector3::zero()) {
            MF_ASSERT(up != Vector3::zero());
            MF_ASSERT(fabs(forward.dotProduct(up)) < 1.00e-10f);
            FMOD_RESULT result = geometries_[i]->setRotation(
              (FMOD_VECTOR*)&forward, (FMOD_VECTOR*)&up);
            SoundManager::FMOD_ErrCheck(
              result, "SoundOccluder::update couldn't set geometry rotation");
        }
    }
    return ok;
}
