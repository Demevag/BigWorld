  ...
  , cognate_(pAnimSect->readString("cognate", ""))
{
    ...
    if (firstFrame_ < 0)
        firstFrame_ = rawFrames + firstFrame_ + 1;
    if (lastFrame_ < 0)
        lastFrame_ = rawFrames + lastFrame_ + 1;
    ...
    if (aSection) {
        for (DataSectionIterator iter = aSection->begin();
            ...
            BW::string id =
              SanitiseHelper::substringReplace(pDS->sectionName(),
    ...
    for (uint i = 0; i < anim_->nChannelBinders(); i++) {
        chanNodes.insert(
          std::make_pair(anim_->channelBinder(i).node().getObject(), i));

        // Initialise the alpha values with the loaded alpha values
        BW::string channelID = anim_->channelBinder(i).channel()->identifier();
    ...
    for (uint i = 0; i < rOwner.nodeTree_.size(); i++) {
        ...
        if (ntd.nChildren > 0) {
            ...
            if (sat >= NODE_STACK_SIZE) {
                --sat;
                ERROR_MSG("Node Hierarchy too deep, unable to determine alpha "
                          "values\n");
...
void NodefullModelAnimation::play(float time, float blendRatio, int flags)
{
    ...
    frame =
      firstFrame_ <= lastFrame_ ? firstFrame_ + frame : firstFrame_ - frame;

    if (flags > 0) {
        flags &= 7;
        pIR = anim_->itinerantRoot();
        if (!alternateItinerants_[flags])
            this->precalcItinerant(flags);
...
void NodefullModelAnimation::precalcItinerant(int flags)
{
    ...
    if (pNewAC != NULL) {
        ...
        {
            Matrix first;
            pIR->channel()->result(float(firstFrame_), first);
            Matrix final;
            pIR->channel()->result(float(lastFrame_), final);
        ...
        if (flags & 2) // isCoordinated
        {
            ...
            //			dprintf( "desired translation is (%f,%f,%f) q
            //(%f,%f,%f,%f)\n", 				oTo[0], oTo[1], oTo[2], oTq[0], oTq[1], oTq[2],
            //oTq[3] );

            // and what's the transform used in this animation
            Matrix offsetT;
            pIR->channel()->result(0, offsetT);
        ...
        if (flags & 4) // isImpacting
        {
            ...
            Matrix firstT;
            pNewAC->result(0, firstT);
            Matrix firstTI;
            firstTI.invert(firstT);
...
void NodefullModelAnimation::flagFactor(int flags, Matrix& mOut) const
{
    ...
    if (flags < 0) {
        if (flags == -1) {
            anim_->itinerantRoot()->channel()->result(float(firstFrame_), mOut);
        }
        return;
    }
