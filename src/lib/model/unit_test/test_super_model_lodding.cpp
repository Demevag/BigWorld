...
namespace TestModel {
    bool initLodSettings(BWResource& bwresource, LodSettings& lodSettings)
    {
        ...
        DataSectionPtr pSection = bwresource.openSection(s_graphicsSettingsXML);
                  ...
                  float         expectedNextLodDown)
    {
        ...
        finalResult &=
          almostEqual(superModel.getLodNextUp(), expectedNextLodUp);
        finalResult &=
          almostEqual(superModel.getLodNextDown(), expectedNextLodDown);
                              ...
                              float         expectedLod)
    {
        ...
        if (expectedResult) {
            finalResult &=
              almostEqual(superModel.getLodNextUp(), Model::LOD_HIDDEN);

            float expectedNextLodDown = Model::LOD_MAX;
            for (int i = 0; i < superModel.nModels(); ++i) {
                expectedNextLodDown = std::min(
                  expectedNextLodDown, superModel.topModel(i)->extent());
            }
            ...
            finalResult &=
              almostEqual(superModel.getLodNextUp(), Model::LOD_HIDDEN);
            finalResult &=
              almostEqual(superModel.getLodNextDown(), Model::LOD_MIN);
        }
     ...
     *	re-calculate.
     */
    bool testBehindCamera(SuperModel& superModel, LodSettings& lodSettings)
    {
        bool finalResult = true;
     ...
     *	Extents from 0-80.
     */
    bool testUpdateLods1(BWResource& bwresource, LodSettings& lodSettings)
    {
     ...
     */
    bool testUpdateLods2(BWResource& bwresource, LodSettings& lodSettings)
    {
        BW::vector<BW::string> modelIDs;
     ...
     */
    bool testUpdateLods3(BWResource& bwresource, LodSettings& lodSettings)
    {
        BW::vector<BW::string> modelIDs;
     ...
     *	Extents from 0-infinity.
     */
    bool testUpdateLods4(BWResource& bwresource, LodSettings& lodSettings)
    {
        BW::vector<BW::string> modelIDs;
        modelIDs.push_back("sets/minspec/shells/dungeon/dungeon_tunnel/model/"
                           "dungeon_tunnel_basic.model");
        SuperModel superModel(modelIDs);
        ...
        // Just behind camera, within shell, -0.1f
        {
            Matrix world(Matrix::identity);
            world.setTranslate(0.0f, 0.0f, -3.1f);
            finalResult &= checkShellLodBehindCamera(
              lodSettings,
              superModel,
              world,
              true,              // expected visibility
     ...
     */
    bool testUpdateLods5(BWResource& bwresource, LodSettings& lodSettings)
    {
        BW::vector<BW::string> modelIDs;
     ...
     *	Extents from 0-infinity.
     */
    bool testUpdateLods6(BWResource& bwresource, LodSettings& lodSettings)
    {
        BW::vector<BW::string> modelIDs;
     ...
     */
    bool testUpdateLods7(BWResource& bwresource, LodSettings& lodSettings)
    {
        BW::vector<BW::string> modelIDs;
