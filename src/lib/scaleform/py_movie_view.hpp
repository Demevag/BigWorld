...
namespace ScaleformBW {
    ...
    {
        Py_Header(PyMovieView, PyObjectPlus) public
        ...
        PY_AUTO_METHOD_DECLARE(RETOK,
                               gotoLabeledFrame,
                               ARG(BW::string, OPTARG(int32, 0, END)))
        ...
        {
            return pMovieView_->GetBackgroundAlpha();
        }
        void backgroundAlpha(float f)
        {
            pMovieView_->SetBackgroundAlpha((Float)f);
        }
