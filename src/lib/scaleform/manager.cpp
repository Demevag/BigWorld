...
// Scaleform::GFx::AS3::Classes::ClassRegistrationTable singleton, this #include
// must be put somewhere only once
...
namespace ScaleformBW {
    ...
    int tokenSet =
      PyMovieDef_token | FlashGUIComponent_token | FlashTextGUIComponent_token;
...
#endif
    {
        ...
        MF_WATCH("Render/Scaleform/DrawCalls",
                 lastNumDrawCalls_,
                 Watcher::WT_READ_ONLY);
        MF_WATCH("Render/Scaleform/Triangles",
                 lastNumTriangles_,
                 Watcher::WT_READ_ONLY);
    ...
    bool Manager::doInit()
    {
        ...
        if (!(pRenderer_ = *new Render::Renderer2D(pRenderHAL_.GetPtr()))) {
            ERROR_MSG("%s: %s\n",
                      "BW::Manager::doInit: ",
                      "Couldn't initialize Renderer2D object");
            return false;
        }

        // Configure renderer in "Dependent mode", honoring externally
        // configured device settings.
        if (!pRenderHAL_->InitHAL(Render::D3D9::HALInitParams(
              Moo::rc().device(),
              Moo::rc().presentParameters(),
              Render::D3D9::HALConfig_NoSceneCalls))) {
            ERROR_MSG("%s: %s\n",
                      "BW::Manager::doInit: ",
                      "Couldn't initialize RenderHAL object");
            return false;
        }
        ...
        pLoader_->SetLog(
          pLogger_); // this line was commented out in SF 3.3 integration, maybe
                     // for sake of optimization
        SF::Log::SetGlobalLog(pLogger_);

        Ptr<GFx::ActionControl> pactControl =
          *new GFx::ActionControl(GFx::ActionControl::Action_LogAllFilenames);
        pLoader_->SetActionControl(pactControl);

        // This would give us A LOT more debugging information - (while GFX/SWF
        // parsing)
        // Ptr<SF::GFx::ParseControl> pparseControl = *new
        // ParseControl(ParseControl::VerboseParse);
        ...
        if (isAmpEnabled_) {
            ...
            Ptr<GFx::AMP::ServerState> ampState =
              *SF_NEW                  GFx::AMP::ServerState();
        ...
        // Set the loader as our provider of fonts.  When fonts are added to the
        // loader, we inherit these as well.
        ...
        Ptr<GFx::XML::SupportBase> pxmlSupport =
          *new GFx::XML::Support(pexpatXmlParser);
        ...
        Ptr<GFx::FontProviderWin32> fontProvider =
          *new GFx::FontProviderWin32(::GetDC(0));
        ...
        pTexMgr_ =
          (Render::D3D9::TextureManager*)pRenderHAL_->GetTextureManager();
        ...
        if (SoundManager::instance().isInitialised()) {
            Ptr<Sound::SoundRendererFMOD> pSoundRenderer =
              Sound::SoundRendererFMOD::CreateSoundRenderer();

            FMOD::System*      system;
            FMOD::EventSystem* eventSystem =
              SoundManager::instance().getEventSystemObject();
            if (!eventSystem ||
                eventSystem->getSystemObject(&system) != FMOD_OK ||
                !pSoundRenderer->Initialize(system)) {
                ERROR_MSG("%s: %s\n",
                          "BW::Manager::doInit: ",
                          "Couldn't initialize GSoundRendererFMOD object");
                return false;
            }
    ...
    struct AdvanceFn
    {
        ...
        AdvanceFn(Float time, UINT catchup)
          : time_(time)
          , catchup_(catchup)
        {
        }
     ...
     */
    void mapFont(const BW::string  fontA,
                 const BW::string& fontB,
                 uint              style       = GFx::FontMap::MFF_Original,
                 float             scaleFactor = 1.0f)
    {
        BW_GUARD;
        Manager::instance().pFontMap()->MapFont(
          fontA.c_str(),
          fontB.c_str(),
          (GFx::FontMap::MapFontFlags)style,
          scaleFactor);
    }
    PY_AUTO_MODULE_FUNCTION(
      ...
      mapFont,
      ARG(BW::string,
          ARG(BW::string, OPTARG(uint, 0x0010, OPTARG(float, 1.0f, END)))),
      _Scaleform)
    ...
    void Manager::addMovieView(PyMovieView* obj)
    {
        ...
        // This would give us A LOT more debugging information - (for
        // ActionScript)
    ...
    {
        BW_GUARD;
        BW::list<PyMovieView*>::iterator itr =
          std::find(movies_.begin(), movies_.end(), obj);
        if (itr != movies_.end()) {
            movies_.erase(itr);
        }
    }
    ...
    bool Manager::onKeyEvent(const KeyEvent& event)
    {
        ...
        // BigWorld key events also contain mouse key events.  Scaleform
        // requires key and mouse events to be separate.
        if (event.key() >= KeyCode::KEY_MINIMUM_MOUSE &&
            event.key() <= KeyCode::KEY_MAXIMUM_MOUSE) {
            POINT mousePos;
            ::GetCursorPos(&mousePos);
            ::ScreenToClient(Moo::rc().windowHandle(), &mousePos);
            // TODO - remove below.  build mouse key state dynamically as
            // needed.
            ...
            if (gfxevent.KeyCode != SF::Key::None) {
                ...
                handled |=
                  std::for_each(movies_.begin(), movies_.end(), fn).handled();
            }
    ...
    //-------------------------------------------------------------------------
    NotifyMouseStateFn::NotifyMouseStateFn(float x_,
                                           float y_,
                                           int   buttons_,
                                           Float scrollDelta_)
      : x(x_)
    ...
    void NotifyMouseStateFn::operator()(PyMovieView* elem)
    {
        ...
        // TODO - can maybe hittest against the scaleform movie before
        // continuing
        GFx::Viewport view;
        ...
        s.Height =
          (movie->GetMovieDef()->GetFrameRect().Height() / view.Height);
        Float          mX = (x * s.Width);
        Float          mY = (y * s.Height);
        Render::Matrix m;
        ...
        if (scrollDelta == 0.0f) {
            ...
            if (movie->HitTest(x, y, GFx::Movie::HitTest_ShapesNoInvisible)) {
                GFx::MouseEvent gfxevent(
                  GFx::Event::MouseWheel, buttons, x, y, scrollDelta);
                handled_ |=
                  (movie->HandleEvent(gfxevent) == GFx::Movie::HE_Completed);
            }
        }
    }
    ...
    bool Manager::onMouse(int buttons, int nMouseWheelDelta, int xPos, int yPos)
    {
        BW_GUARD;
        bool               handled = false;
        NotifyMouseStateFn msFn =
          NotifyMouseStateFn((float)xPos,
                             (float)yPos,
                             buttons,
                             (Float)((nMouseWheelDelta / WHEEL_DELTA) * 3));
    ...
    void Manager::onDrawClipboard(HWND hWnd)
    {
        ...
        if (hglb != NULL) {
            ...
            if (pwstr != NULL) {
                Ptr<GFx::TextClipboard> pClipboard =
                  this->pLoader()->GetTextClipboard();
                if (pClipboard) {
    ...
    void Manager::updateAMPProfileResCounters()
    {
        ...
        static ResourceCounters::DescriptionPool poolDescrGPUImageMem(
          "ScaleformAMP/GPUMemory/Images",
          (uint)ResourceCounters::MP_DEFAULT,
          ResourceCounters::RT_TEXTURE);
        static ResourceCounters::DescriptionPool poolDescrGPUMeshCacheMem(
          "ScaleformAMP/GPUMemory/MeshCache",
          (uint)ResourceCounters::MP_DEFAULT,
          ResourceCounters::RT_VERTEX_BUFFER);

        static ResourceCounters::DescriptionPool poolDescrMovieDataMem(
          "ScaleformAMP/SysMemory/MovieData",
          (uint)ResourceCounters::MP_SYSTEM,
          ResourceCounters::RT_OTHER);
        static ResourceCounters::DescriptionPool poolDescrMovieViewMem(
          "ScaleformAMP/SysMemory/MovieView",
          (uint)ResourceCounters::MP_SYSTEM,
          ResourceCounters::RT_OTHER);
        static ResourceCounters::DescriptionPool poolDescrSoundMem(
          "ScaleformAMP/SysMemory/Sound",
          (uint)ResourceCounters::MP_SYSTEM,
          ResourceCounters::RT_OTHER);
        static ResourceCounters::DescriptionPool poolDescrFontCacheMem(
          "ScaleformAMP/SysMemory/FontCache",
          (uint)ResourceCounters::MP_SYSTEM,
          ResourceCounters::RT_OTHER);
        static ResourceCounters::DescriptionPool poolDescrSWFVideoDataMem(
          "ScaleformAMP/SysMemory/SWFVideo",
          (uint)ResourceCounters::MP_SYSTEM,
          ResourceCounters::RT_OTHER);
        ...
        if (lastAMPFrameProfile_ == NULL) {
            ...
            MF_WATCH("Render/Scaleform/SysMemoryTotal",
                     currAMPFrameProfile_->TotalMemory,
                     Watcher::WT_READ_ONLY);

            MF_WATCH("Render/Scaleform/GPUMemory/Images",
                     currAMPFrameProfile_->ImageMemory,
                     Watcher::WT_READ_ONLY);
            MF_WATCH("Render/Scaleform/GPUMemory/MeshCache",
                     currAMPFrameProfile_->MeshCacheGraphicsMemory,
                     Watcher::WT_READ_ONLY);

            MF_WATCH("Render/Scaleform/SysMemory/MovieData",
                     currAMPFrameProfile_->MovieDataMemory,
                     Watcher::WT_READ_ONLY);
            MF_WATCH("Render/Scaleform/SysMemory/MovieView",
                     currAMPFrameProfile_->MovieViewMemory,
                     Watcher::WT_READ_ONLY);
            MF_WATCH("Render/Scaleform/SysMemory/Sound",
                     currAMPFrameProfile_->SoundMemory,
                     Watcher::WT_READ_ONLY);
            MF_WATCH("Render/Scaleform/SysMemory/FontCache",
                     currAMPFrameProfile_->FontCacheMemory,
                     Watcher::WT_READ_ONLY);
            MF_WATCH("Render/Scaleform/SysMemory/SWFVideo",
                     currAMPFrameProfile_->VideoMemory,
                     Watcher::WT_READ_ONLY);
        } else {
            currAMPFrameProfile_->ImageMemory =
              currAMPFrameProfile_->ImageGraphicsMemory =
                currAMPFrameProfile_->MeshCacheMemory =
                  currAMPFrameProfile_->MeshCacheGraphicsMemory =
                    currAMPFrameProfile_->MovieDataMemory =
                      currAMPFrameProfile_->MovieViewMemory =
                        currAMPFrameProfile_->SoundMemory =
            ...
            GFx::AMP::Server& gfxAmpServer =
              *((GFx::AMP::Server*)&ampServerInst);
            ...
            RESOURCE_COUNTER_SUB(poolDescrGPUImageMem,
                                 lastAMPFrameProfile_->ImageMemory,
                                 lastAMPFrameProfile_->ImageMemory);
            RESOURCE_COUNTER_SUB(poolDescrGPUMeshCacheMem,
                                 lastAMPFrameProfile_->MeshCacheGraphicsMemory,
                                 lastAMPFrameProfile_->MeshCacheGraphicsMemory);

            RESOURCE_COUNTER_SUB(
              poolDescrMovieDataMem, lastAMPFrameProfile_->MovieDataMemory, 0);
            RESOURCE_COUNTER_SUB(
              poolDescrMovieViewMem, lastAMPFrameProfile_->MovieViewMemory, 0);
            RESOURCE_COUNTER_SUB(
              poolDescrSoundMem, lastAMPFrameProfile_->SoundMemory, 0);
            RESOURCE_COUNTER_SUB(
              poolDescrFontCacheMem, lastAMPFrameProfile_->FontCacheMemory, 0);
            RESOURCE_COUNTER_SUB(
              poolDescrSWFVideoDataMem, lastAMPFrameProfile_->VideoMemory, 0);
        }

        RESOURCE_COUNTER_ADD(poolDescrGPUImageMem,
                             currAMPFrameProfile_->ImageMemory,
                             currAMPFrameProfile_->ImageMemory);
        RESOURCE_COUNTER_ADD(poolDescrGPUMeshCacheMem,
                             lastAMPFrameProfile_->MeshCacheGraphicsMemory,
                             lastAMPFrameProfile_->MeshCacheGraphicsMemory);

        RESOURCE_COUNTER_ADD(
          poolDescrMovieDataMem, currAMPFrameProfile_->MovieDataMemory, 0);
        RESOURCE_COUNTER_ADD(
          poolDescrMovieViewMem, currAMPFrameProfile_->MovieViewMemory, 0);
        RESOURCE_COUNTER_ADD(
          poolDescrSoundMem, currAMPFrameProfile_->SoundMemory, 0);
        RESOURCE_COUNTER_ADD(
          poolDescrFontCacheMem, currAMPFrameProfile_->FontCacheMemory, 0);
        RESOURCE_COUNTER_ADD(
          poolDescrSWFVideoDataMem, currAMPFrameProfile_->VideoMemory, 0);
