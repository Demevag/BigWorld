...
namespace ScaleformBW {
    GFile* FileOpener::OpenFile(const char* pfilename, SInt flags, SInt mode)
    {
        BW_GUARD;
        BinaryPtr ptr =
          BWResource::instance().rootSection()->readBinary(pfilename);
        if (!ptr.hasObject())
            return NULL;
    ...
    Render::Image* BWToGFxImageLoader::LoadProtocolImage(
      const GFx::ImageCreateInfo& info,
      const GString&              url)
    {
        BW_GUARD;
        UInt   width, height;
        ...
        if (!pTex ||
            pTex->status() == BW::Moo::BaseTexture::STATUS_FAILED_TO_LOAD) {
        ...
        if (pTex->format() != D3DFMT_A8R8G8B8 &&
            pTex->format() != D3DFMT_DXT1 && pTex->format() != D3DFMT_DXT3 &&
            pTex->format() != D3DFMT_DXT5) {
            D3DFORMAT defaultFmt =
              (isCamouflage || isSticker) ? D3DFMT_A8R8G8B8 : D3DFMT_DXT5;
        ...
        switch (pTex->format()) {
            case D3DFMT_A8R8G8B8:
                fmt = isCamouflage ? Render::Image_R8G8B8A8
                                   : Render::Image_B8G8R8A8;
                break;
            case D3DFMT_DXT1:
                fmt = Render::Image_DXT1;
                break;
            case D3DFMT_DXT3:
                fmt = Render::Image_DXT3;
                break;
            case D3DFMT_DXT5:
                fmt = Render::Image_DXT5;
                break;
            default: {
        ...
        Ptr<Render::RawImage> pimage =
          *Render::RawImage::Create(fmt,
                                    1,
        ...
        if (tex) {
            ...
            if (D3D_OK == tex->LockRect(0, &rect, NULL, D3DLOCK_READONLY)) {
                ...
                if (pimage->GetImageData(&imgData)) {
                    ...
                    memcpy(imgPlane0.pData,
                           rect.pBits,
                           DX::surfaceSize(width, height, pTex->format()));
                    pimage->AddRef();
                }
                tex->UnlockRect(0);
            }
    ...
    bool BWToGFxImageLoader::ParseStickerUrl(const char* szUrl,
                                             uint&       width,
                                             uint&       height,
                                             BW::string& strFilePath,
                                             uint&       tileWidth,
                                             ...
                                             uint&       tileY)
    {
        ...
        if (*ptr++ != ',')
            return false;
        if (sscanf(
              ptr, "%d,%d,%d,%d", &tileWidth, &tileHeight, &tileX, &tileY) != 4)
            return false;
      ...
      : GFx::Loader(*new FileOpener)
    {
        ...
        Ptr<GFx::ImageFileHandlerRegistry> pimgReg =
          *new GFx::ImageFileHandlerRegistry();
