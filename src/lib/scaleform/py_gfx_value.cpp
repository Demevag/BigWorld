...
namespace ScaleformBW {
    ...
    PyGFxValue* PyGFxValue::createInternal(const char*  name,
                                           PyGFxValue*  pyParent,
                                           GFx::Value&  gfxValue,
    ...
    PyGFxValue* PyGFxValue::create(const char* name, PyGFxValue* pyParent)
    {
        ...
        if (pyParent->gfxValue_.GetMember(name, &gfxValue))
            return createInternal(
              name, pyParent, gfxValue, pyParent->pyMovieView_);
        else
            return NULL;
    }
    ...
    PyGFxValue* PyGFxValue::create(GFx::Value&  gfxValue,
                                   PyMovieView* pyMovieView)
    {
        BW_GUARD;
    ...
    PyObject* PyGFxValue::convertValueToPython(GFx::Value&  gfxValue,
                                               PyMovieView* pyMovieView,
                                               PyGFxValue*  pyOwner)
    {
        BW_GUARD;
        ...
        // for basic AS types, convert GFx::Value to one of integral Python
        // types, for AS Arrays, convert into Python list for DisplayObjects,
        // Objects and etc. unknown types - return reference to ourself
        ...
        switch (type) {
            ...
            case GFx::Value::VT_StringW:
                pyValue = PyUnicode_FromWideChar(gfxValue.GetStringW(),
                                                 wcslen(gfxValue.GetStringW()));
                break;
                ...
                for (uint i = 0; i < arrSize; ++i) {
                    ...
                    if (gfxValue.GetElement(i, &gfxArrElem))
                        PyList_SetItem(
                          pyValue,
                          i,
                          convertValueToPython(gfxArrElem, pyMovieView));
                }
            } break;

            default:
                if (pyOwner == NULL && pyMovieView == NULL) {
                    // don't know what to do if only have raw GFxValue and no
                    // connection with PyMovieView
                    pyValue = Py_None;
                    Py_XINCREF(pyValue);
                } else if (pyOwner == NULL) {
                    // orbitrary object-type value returned by AS method, make
                    // new nameless PyGFxValue from it
    ...
    GFx::Value PyGFxValue::convertValueToGFx(PyObject*    pyValue,
                                             PyMovieView* pyMovieView)
    {
        BW_GUARD;
        ...
        if (pyValue == NULL) {
            ...
            for (Py_ssize_t i = 0; i < numItems; ++i) {
                // all 3 are borrowed references, no need to decref them after
                // use
    ...
    void PyGFxValue::collectChildValues()
    {
        ...
        // note: collecting not values themself but names, because Visit() seem
        // to stick to the hierarchy obtained only once, on first try (TODO -
        // reconsider after next SF version)
        struct PyGFxChildrenCollector : public GFx::Value::ObjectVisitor
        {
            PyGFxValue*            pyParent_;
            BW::vector<BW::string> names;
            PyGFxChildrenCollector(PyGFxValue* pyParent)
              : pyParent_(pyParent)
            {
            }
            bool IncludeAS3PublicMembers() const { return true; }
            void Visit(const char* name, const GFx::Value& gfxValue)
            {
        ...
        for (BW::vector<BW::string>::iterator it = collector.names.begin();
             it != collector.names.end();
             ++it)
    ...
    bool PyGFxValue::bindToScript(PyObject* pyScript)
    {
        ...
        for (Children_t::iterator it = children_.begin(); it != children_.end();
             ++it) {
            ...
            PyGFxScriptFunction* pyScriptFn =
              SF_HEAP_NEW(SF::Memory::GetGlobalHeap())
                PyGFxScriptFunction(pyScript, pyMovieView_, funcName);

            pyMovieView_->pMovieView()->CreateFunction(&pyChild->gfxValue_,
                                                       pyScriptFn);
    ...
    void PyGFxValue::unbindFromScript()
    {
        ...
        if (pyScript_ != NULL) {
            for (Children_t::iterator it = children_.begin();
                 it != children_.end();
                 ++it) {
                PyGFxValue* pyChild = it->second.get();
    ...
    PyObject* PyGFxValue::children()
    {
        ...
        for (Children_t::iterator it = children_.begin(); it != children_.end();
            ...
            PyGFxValue* pyChild = it->second.get();
            if (pyChild != NULL)
                PyDict_SetItem(pyDict,
                               Script::getData(it->first),
                               pyChild->convertValueToPython());
        }
    ...
    void PyGFxValue::fromDict(PyObjectPtr dict)
    {
        ...
        if (!PyDict_Check(pyDict)) {
            PyErr_SetString(PyExc_TypeError,
                            "Failed to convert argument from dict to "
                            "PyGFxValue - given argument is not of dict type");
            return;
        }
        ...
        if (gfxValue_.GetType() != GFx::Value::VT_Object &&
            ...
            PyErr_SetString(
              PyExc_TypeError,
              "Failed to convert argument from dict to PyGFxValue - the type "
              "of GFx Value is not ActionScript Object.");
            return;
        }
        ...
        for (Py_ssize_t i = 0; i < numItems; ++i) {
            ...
            if (!gfxValue_.SetMember(objKey, objVal))
                PyErr_Format(
                  PyExc_ValueError,
                  "Failed to assign value by key '%s' to PyGFxValue - most "
                  "likely this key is missing in ActionScript Object.",
                  objKey);
        }
    ...
    PyObject* PyGFxValue::toDict()
    {
        ...
        if (gfxValue_.GetType() != GFx::Value::VT_Object &&
            gfxValue_.GetType() != GFx::Value::VT_DisplayObject) {
            PyErr_SetString(PyExc_TypeError,
                            "Failed to convert PyGFxValue to dict - the type "
                            "of GFx Value is not ActionScript Object.");
            Py_Return;
        }
                                    ...
                                    const ScriptObject& value)
    {
        ...
        if (strcmp(attr, "script") == 0) {
            if (bindToScript(value.get())) {
                ...
                PyErr_Format(PyExc_AttributeError,
                             ...
                             "failed to bind script to PyGFxValue '%s'",
                             name_.c_str());
                return false;
            }
        }
        ...
        if (it != children_.end()) {
            ...
            if (type == GFx::Value::VT_DisplayObject ||
                ...
                // assign nested values of DisplayObjects and Objects with
                // SetMember and resync nested hierarchy of PyGFxValue objects
                // to appropriate GFx Values
                if (gfxValue_.SetMember(attr, gfxValueNew)) {
                    resync();
                    return true;
                }
            } else {
                // assign integral types and arrays directly with
                // Value::operator=
    ...
    PyObject* PyGFxValue::pyCall(PyObject* args)
    {
        ...
        if ((parent_ == NULL && pyMovieView_ == NULL) || name_.empty()) {
            PyErr_Format(PyExc_TypeError,
                         "Cannot invoke Action Script method on object not "
                         "attached to the Movie clip");
            return NULL;
        }
        ...
        if (gfxValue_.GetType() != GFx::Value::VT_Object &&
            ...
            PyErr_Format(PyExc_TypeError,
                         "Cannot invoke Action Script method on object of "
                         "non-Object type (method name: %s)",
                         name_.c_str());
            return NULL;
        }
        ...
        if (numArgs == 0) {
            // empty argument list
            success = (parent_ == NULL)
                        ? pMovie->Invoke(name_.c_str(), &gfxResult, NULL, 0)
            ...
            for (uint i = 0; i < numArgs; ++i)
                gfxArgs[i] =
                  convertValueToGFx(PyTuple_GetItem(args, i), pyMovieView_);

            success =
              (parent_ == NULL)
                ? pMovie->Invoke(
                    name_.c_str(), &gfxResult, &gfxArgs[0], numArgs)
                : obj.Invoke(name_.c_str(), &gfxResult, &gfxArgs[0], numArgs);
        }

        if (!success) {
            PyErr_Format(PyExc_Exception,
                         "PyGFxValue - Failed to invoke method %s.",
                         name_.c_str());
            return NULL;
        }

        // mark all connected child values at same level with this function
        // object as out-of-sync, but don't forcibly sync them now
        // TODO: reconsider, maybe on-demand resync should be recursive instead
        // of single-level?
        ...
        // convert result into Python without referencing it to any PyGFxValue
        // (since we only have raw GFx Value there)
