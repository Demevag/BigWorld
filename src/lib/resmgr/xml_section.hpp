...
class XMLSection : public DataSection
{
  public:
    ...
    static XMLSectionPtr createFromStream(const BW::StringRef& tag,
                                          std::istream&        astream);
    static XMLSectionPtr createFromBinary(const BW::StringRef& tag,
                                          BinaryPtr            pBinary);
    ...
    static bool shouldReadXMLAttributes() { return s_shouldWriteXMLAttributes; }
    ...
    virtual bool isAttribute() const { return isAttribute_; }

    virtual bool isAttribute(bool isAttribute);

    virtual bool noXMLEscapeSequence() const { return noXMLEscapeSequence_; }
  ...
  protected:
    ...
    virtual BW::string   asString(
        const BW::StringRef& defaultVal = BW::StringRef(),
        int                  flags      = DS_IncludeWhitespace);
    virtual BW::wstring asWideString(
      const BW::WStringRef& defaultVal = BW::WStringRef(),
      int                   flags      = DS_IncludeWhitespace);
  ...
  private:
    ...
    static bool processBang(class WrapperStream& stream, XMLSection* pCurrNode);
