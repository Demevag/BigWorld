...
class Formatter
{
  ...
  public:
    Formatter() { wmemset(str_, L'\0', 1024); }
...
inline void formatString(const wchar_t*   format,
                         BW::wstring&     o_Result,
                         const Formatter& f1 = Formatter(),
                         const Formatter& f2 = Formatter(),
                         const Formatter& f3 = Formatter(),
                         const Formatter& f4 = Formatter(),
                         const Formatter& f5 = Formatter(),
                         const Formatter& f6 = Formatter(),
                         const Formatter& f7 = Formatter(),
                         const Formatter& f8 = Formatter())
{
...
class StringID
{
  public:
    ...
    bool               operator<(
      const StringID& that) const; // should be a free function, but ...
  private:
    BW::wstring  str_;
    unsigned int key_;
};
...
{
  public:
    virtual void               load(DataSectionPtr     language,
                                    const BW::wstring& root = L"") = 0;
    ...
    static std::pair<BW::wstring, BW::wstring> splitIsoLangCountryName(
...
class StringProvider
{
    ...
    void           formatString(const StringID&  formatID,
                                ...
                                const Formatter& f8 = Formatter())
    {
        ::BW_NAMESPACE formatString(
          str(formatID), o_OutputString, f1, f2, f3, f4, f5, f6, f7, f8);
    }
  ...
  public:
    ...
    void setLanguages(const BW::wstring& langName,
                      const BW::wstring& countryName);

    const wchar_t* str(const wchar_t* id,
                       DefResult      def = RETURN_PARAM_IF_NOT_EXISTING) const;

    void formatString(const wchar_t*   formatID,
                      BW::wstring&     o_OutputString,
                      const Formatter& f1 = Formatter(),
                      const Formatter& f2 = Formatter(),
                      const Formatter& f3 = Formatter(),
...
inline void formatLocalisedString(const wchar_t*   format,
                                  BW::wstring&     o_OutputString,
                                  const Formatter& f1,
                                  const Formatter& f2 = Formatter(),
                                  const Formatter& f3 = Formatter(),
                                  ...
                                  const Formatter& f8 = Formatter())
{
    StringProvider::instance().formatString(
      format, o_OutputString, f1, f2, f3, f4, f5, f6, f7, f8);
}

inline const wchar_t* Localise(
  const wchar_t*            key,
...
//-----------------------------------------------------------------------------
inline const wchar_t* Localise(const wchar_t*   format,
                               const Formatter& f1,
                               const Formatter& f2 = Formatter(),
                               const Formatter& f3 = Formatter(),
                               const Formatter& f4 = Formatter(),
                               ...
                               const Formatter& f8 = Formatter())
{
    ...
    formatLocalisedString(
      format, stackString.container(), f1, f2, f3, f4, f5, f6, f7, f8);
...
inline void LocaliseUTF8(BW::string&      o_OutputString,
...
inline BW::string LocaliseUTF8(const wchar_t*   format,
                               const Formatter& f1,
                               const Formatter& f2 = Formatter(),
                               const Formatter& f3 = Formatter(),
                               const Formatter& f4 = Formatter(),
                               ...
                               const Formatter& f8 = Formatter())
{
    ...
    formatLocalisedString(
      format, wlocalisedString.container(), f1, f2, f3, f4, f5, f6, f7, f8);
    bw_wtoutf8(wlocalisedString.container(), localisedString.container());
    return localisedString.container();
}

#ifdef WIN32
inline BW::string LocaliseACP(const wchar_t*   format,
                              const Formatter& f1,
                              const Formatter& f2 = Formatter(),
                              const Formatter& f3 = Formatter(),
                              const Formatter& f4 = Formatter(),
...
inline void LocaliseUTF8(
  BW::string&               o_OutputString,
  const wchar_t*            key,
  StringProvider::DefResult def = StringProvider::RETURN_PARAM_IF_NOT_EXISTING)
{
...
inline BW::string LocaliseStaticUTF8(const wchar_t* key)
{
    ...
    if (it == s_cache.end()) {
        it = s_cache.insert(std::make_pair(key, LocaliseUTF8(key))).first;
    }
...
class WindowTextNotifier : LanguageNotifier
{
  ...
  public:
    ...
    void insertComboString(HWND                            hwnd,
    ...
    static LRESULT CALLBACK    CallWndRetProc(int    nCode,
                                              WPARAM wParam,
                                              LPARAM lParam);
    static LRESULT CALLBACK    CallWndProc(int    nCode,
                                           WPARAM wParam,
                                           LPARAM lParam);
    static LRESULT CALLBACK    ComboProc(HWND   hwnd,
                                         UINT   msg,
                                         WPARAM wParam,
                                         LPARAM lParam);
    static LRESULT CALLBACK    ToolTipProc(HWND   hwnd,
                                           ...
                                           WPARAM wParam,
                                           LPARAM lParam);
    static LRESULT CALLBACK    ToolTipParentProc(HWND   hwnd,
                                                 UINT   msg,
                                                 WPARAM wParam,
                                                 LPARAM lParam);
};
...
#define STATIC_LOCALISE_STRING(name, key)                                      \
    static BW::string name(LocaliseUTF8(key))
#define STATIC_LOCALISE_WSTRING(name, key)                                     \
    static BW::wstring name(Localise(key))
