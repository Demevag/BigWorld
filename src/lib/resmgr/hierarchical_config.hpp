...
class HierarchicalConfig
{
  public:
    ...
    /// \param filename The name of the file that is recursively searched for in
    /// every directory \param rootDirectory The root directory to recursively
    /// search. If empty the base resource paths will be used instead
    HierarchicalConfig(const StringRef& filename,
                       const StringRef& rootDirectory);

    void load(const DataSectionPtr pRootSection);
    void load(const StringRef& filename);
    void load(const StringRef& filename, const StringRef& rootDirectory);
