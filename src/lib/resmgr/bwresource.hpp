...
class BWResource : public Singleton<BWResource>
{
  public:
    ...
    static void checkAccessFromCallingThread(const BW::StringRef& path,
                                             const char*          msg);

    // Static methods taken from the now deprecated FilenameResolver
    static IFileSystem::FileType getFileType(const BW::StringRef&   file,
    ...
    static bool searchForFile(const BW::StringRef& directory, BW::string& file);
    ...
    template <typename T>
    static bool             changeExtensionT(T*       file,
                                             const T* newExtension,
                                             bool     includingDot = true);
    ...
    static void       addPaths(const BW::StringRef& path,
                               const char*          desc,
    ...
    static DataSectionPtr openSectionInTree(const BW::StringRef& filePath,
                                            const BW::StringRef& sectionToOpen);
...
/*static*/ int BWResource::findExtensionPos(const T* file)
{
    ...
    if (bw_str_find_last_of(file, T('.'), dotPos)) {
        ...
        if (bw_str_find_last_of(file, T('\\'), pos) && pos < fileNamePos) {
            fileNamePos = pos;
        }
 ...
 */
template <typename T>
/*static*/ bool BWResource::getExtensionT(const T* file,
                                          T*       retExtension,
                                          size_t   retSize)
{
    BW_GUARD;
    ...
    if (dotPos != -1) {
        bw_str_substr(retExtension,
                      retSize,
                      file,
                      dotPos + 1,
                      bw_str_len(file) - dotPos - 1);
        return true;
    }
    // no extension
    return false;
        ...
        if (bw_str_find_first_of(formatedPath, bwPath, pos)) {
            ...
            if (formatedPath[index] == T('/')) {
                bw_str_substr(path,
                              ...
                              formatedPath,
                              index + 1,
                              bw_str_len(formatedPath) - index - 1);
                return true;
            }
        }
    }
