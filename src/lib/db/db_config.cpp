...
namespace DBConfig {
     ...
     *
     *	@param parentPath 	The parent block's path describing a section in
     *bw.xml, or empty if we are a top-level block.
     *
    ...
    bool ConfigBlock::init(const BW::string& parentPath)
    {
        ...
        for (Options::iterator iter = options_.begin(); iter != options_.end();
             ++iter) {
        ...
        for (ObsoleteNames::const_iterator iter = obsoleteNames_.begin();
             ...
             ++iter) {
            BW::string obsoletePath =
              !path_.empty() ? path_ + "/" + iter->first : iter->first;
    ...
    WatcherPtr ConfigBlock::pWatcher() const
    {
        ...
        for (Blocks::const_iterator iter = childBlocks_.begin();
            ...
            pWatcher->addChild(
              block.name().c_str(),
              block.pWatcher(),
              // We need to use the offset of the member relative to this
              // object, hence the ugliness here.
      ...
      , secondaryDB(*this)
    {
        ...
        // Some options were moved to DBApp. They are considered obsolete as
        // well, as they should not be in this section.
    ...
    /**
     *	This method returns the static instance of the database configuration
     *data.
     */
    const DBConfigBlock& get()
    {
        static TopLevelConfigBlock instance;
        // We hold onto the top-level object so it can be initialised to check
        // for the <dbMgr> top-level obsolete section.
