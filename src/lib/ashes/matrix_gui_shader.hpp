 ...
 *
 *	The MatrixGUIShader can also blend(linearly interpolate) from its current
 *target to a newly specified target.  It takes eta seconds from the last time
...
class MatrixGUIShader : public GUIShader
{
    Py_Header(MatrixGUIShader, GUIShader)
      ...
      public : MatrixGUIShader(PyTypeObject* pType = &s_type_);
    /// use this constructor for simple non-blended transforms ( static; ignores
    /// eta )
