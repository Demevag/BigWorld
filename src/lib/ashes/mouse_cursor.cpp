...
namespace { // anonymous
    ...
    // Named constants
    // Note: changes made to the AutoConfig parameters below must be also made
    // in bigworld/src/tools/res_packer/packer_helper.cpp.
    const AutoConfigString s_cursorDefFile("gui/cursorsDefinitions",
 ...
 *	@components{ client, tools }
 *
 *	Enables/disables mouse clipping. When true, the mouse cursor will be clipped
 *to the client region of the window whenever the window is in focus. Useful in
 *	conjunction with hiding the cursor (e.g. when in mode where the mouse is
...
void MouseCursor::position(const Vector2& pos)
{
    ...
    SimpleGUI::instance().clipRangesToPixel(
      float(pos.x + 1.0f), float(1.0f - pos.y), &x, &y);
 ...
 *
 *	@param	cursorName	name of the cursor to use (as defined in
 *mouse_cursors.xml)
 */
void MouseCursor::shape(const BW::string& cursorName)
{
    ...
    if (this->setShape(cursor)) {
        ...
        PyErr_Format(PyExc_ValueError,
                     "Could not change cursor shape: %s",
                     cursorName.c_str());
    }
}
...
{
    BW_GUARD;
    return this->curProvider_.exists() ? this->curProvider_.getObject()
                                       : Py_None;
}
...
/**
 *	Returns current setting as to whether the mouse cursor is clipped while
 *window is in focus.
 */
...
/**
 *	Sets the clip flag. When true, the mouse will be clipped to the region of
 *the window while it is in focus.
 */
...
void MouseCursor::tick(float dTime)
{
    ...
    } else if (this->isActive_ && this->curCursor_ != this->cursorDefs_.end() &&
               this->curCursor_->second.texture->isAnimated()) {
        // Updating cursor if it's an animated texture
        this->setTexture(this->curCursor_->second.texture->pTexture(),
                         this->curHotSpot_);
    }
}
    ...
    if (pTop) {
        for (int i = 0; i < pTop->countChildren(); ++i) {
            ...
            Moo::TextureManager::instance()->getDetailsManager()->setFormat(
              fileName, D3DFMT_A8R8G8B8);
            cursorDef.texture =
              Moo::TextureManager::instance()->getSystemMemoryTexture(fileName);
            if (cursorDef.texture->format() != D3DFMT_A8R8G8B8) {
                ERROR_MSG(
                  "Invalid cursor texture map format for cursor: %s.\n"
...
bool MouseCursor::setShape(const CursorDefMap::const_iterator& curIt)
{
    ...
    if (curIt != this->cursorDefs_.end()) {
        ...
        if (this->setTexture(curIt->second.texture->pTexture(),
                             this->curHotSpot_)) {
            return true;
        }

        ERROR_MSG("MouseCursor::setShape: "
 ...
 */
bool MouseCursor::setTexture(PyTextureProviderPtr provider,
                             const Vector2&       hotSpot)
{
    ...
    if (srcSurface && SUCCEEDED(srcSurface->GetDesc(&srcSurfaceDesc))) {
        if (srcSurfaceDesc.Usage & D3DUSAGE_RENDERTARGET) {
            ...
            dstTexture = Moo::rc().createTexture(srcTexture->width(),
                                                 srcTexture->height(),
                                                 1,
                                                 0,
                                                 D3DFMT_A8R8G8B8,
            ...
            if (dstTexture) {
                ...
                if (SUCCEEDED(hr)) {
                    ...
                    if (SUCCEEDED(hr)) {
                        res =
                          this->setTexture(dstTexture.pComObject(), hotSpot);
                    }
                }
            }
        } else {
...
bool MouseCursor::setTexture(DX::BaseTexture* texture, const Vector2& hotSpot)
{
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            ERROR_MSG("MouseCursor::setTexture: "
...
/**
 *	Callback triggered by Moo when the device is recreated. Flag cursor as
 *dirty.
 */
