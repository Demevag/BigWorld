 ...
 *
 * @type PyAttachment	This attribute stores the attachment that is to be
 ...
 *	@components{ client, tools }
 *
 *	Creates and returns a new MeshGUIAdaptor object, which is used to display 3D
 *objects in the GUI scene.
 *
  ...
  , transform_(NULL)
{
    ...
    transform_    = SmartPointer<MatrixProvider>(
      mat, SmartPointer<MatrixProvider>::STEAL_REFERENCE);
    Matrix m;
 ...
 */
void MeshGUIAdaptor::draw(Moo::DrawContext& drawContext,
                          bool              reallyDraw,
                          bool              overlay)
{
    BW_GUARD;
    if (visible()) {
        ...
        if (!momentarilyInvisible_ &&
            reallyDraw) // don't check the texture_ member ( see SGC.ipp )
        {
...
void MeshGUIAdaptor::addAsSortedDrawItem(Moo::DrawContext& drawContext,
                                         const Matrix&     worldTransform)
{
    ...
    if (visible()) {
        ...
        if (attachment_ && !momentarilyInvisible_) // don't check the texture_
                                                   // member ( see SGC.ipp )
        {
            // Draw mesh now.
            // How this works:
            ...
            attachment_->updateAnimations(Moo::rc().world(), Model::LOD_MIN);
            attachment_->draw(drawContext, Moo::rc().world());
