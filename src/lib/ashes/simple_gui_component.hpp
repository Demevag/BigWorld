...
namespace Script {
    ...
    {
        PyObject* ret =
          (data.good() ? const_cast<PyObject*>(data.getPyObj()) : Py_None);
        Py_INCREF(ret);
        return ret;
    }
}
...
#define COMPONENT_FACTORY_DECLARE(CONSTRUCT)                                   \
    ...
    virtual GUIComponentFactory& factory()                                     \
    {                                                                          \
        return s_factory;                                                      \
    }                                                                          \
    static SimpleGUIComponent* s_create()                                      \
    {                                                                          \
...
class SimpleGUIComponent : public PyObjectPlusWithWeakReference
{
    ...
    virtual void draw(Moo::DrawContext& drawContext,
                      bool              reallyDraw,
                      bool              overlay = true);
    virtual void addAsSortedDrawItem(Moo::DrawContext& drawContext,
                                     const Matrix&     worldTransform);

    //-------------------------------------------------
    // Hierarchy logic
    //-------------------------------------------------
    bool addChild(const BW::string& name, SimpleGUIComponent* child);
    ...
    // TODO: make this pure virtual (good practice: never inherit from a
    // concrete base)
    ...
    virtual bool handleDragEnterEvent(SimpleGUIComponent* /*draggedComponen*/,
                                      const MouseEvent& /*event*/);

    virtual bool handleDragLeaveEvent(SimpleGUIComponent* /*draggedComponen*/,
                                      const MouseEvent& /*event*/);

    virtual bool handleDropEvent(SimpleGUIComponent* /*draggedComponen*/,
    ...
    // Shaders
    //-------------------------------------------------
    void         addShader(const BW::string& name, GUIShader* shader);
    ...
    // adds bounding box in local or world space.  if gui component is drawn in
    // 2D, the bounding box represents clip coordinates, otherwise world
    // coordinates.
    void localBoundingBox(BoundingBox& bb, bool skinny = false);
    void worldBoundingBox(BoundingBox&  bb,
                          const Matrix& world,
                          bool          skinny = false);
  ...
  protected:
    ...
    virtual ConstSimpleGUIComponentPtr nearestRelativeParent(
      int depth = 0) const;
    ...
    virtual void updateChildren(float dTime,
                                float relParentWidth,
                                float relParentHeight);

    virtual void drawSelf(bool reallyDraw, bool overlay);
    void         drawChildren(Moo::DrawContext& drawContext,
                              bool              reallyDraw,
                              bool              overlay);
  ...
  private:
    ...
    bool invokeKeyEventHandler(PyObject*           pEventHandler_,
                               const char*         methodName,
                               const KeyEvent&     event,
                               SimpleGUIComponent* dragged,
                               const char*         callErrorPrefix,
    ...
    uint32 colour_;
    /// this is the dynamic, mutable colour.  it is modified by shaders per
    /// frame, without resorting to changing vertex colours.
    ...
    {
      public:
        DepthCompare(const ChildRecVector& crv)
          : crv_(crv)
        {
        }
