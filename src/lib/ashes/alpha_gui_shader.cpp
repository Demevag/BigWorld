 ...
 */
bool AlphaGUIShader::processComponent(SimpleGUIComponent& component,
                                      float               dTime)
{
    BW_GUARD;
    int        nVerts;
    ...
    currentAlpha_ =
      Math::clamp(0.f, oldAlpha_ + ((constants_[2] - oldAlpha_) * dt), 1.f);
    ...
    switch (fadeMode_) {
        case FADE_ALL: {
            Vector4 newCol =
              Colour::getVector4Normalised(component.runTimeColour());
        ...
        case FADE_RIGHT:
            // componentAlpha is the current alpha T-value multiplied by the
            // component's actual alpha ( set in the colour property )
            {
                float origAlpha =
                  (float)((component.runTimeColour() & 0xff000000) >> 24);
                origAlpha /= 255.f;
                float componentAlpha =
                  Math::clamp(0.f, currentAlpha_ * origAlpha, origAlpha);
                ...
                for (i = 0; i < nVerts; i++) {
                    ...
                    if (verts[i].pos_.x > constants_[1])
                        ...
                        t = (verts[i].pos_.x - constants_[0]) /
                            (constants_[1] - constants_[0]);
