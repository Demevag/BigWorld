...
namespace BW { namespace Memhook {
    typedef void*(MEMHOOK_CDECL* NewFuncType)(size_t);
    typedef void*(MEMHOOK_CDECL* NewNoThrowFuncType)(size_t,
                                                     const std::nothrow_t&);
    typedef void*(MEMHOOK_CDECL* NewArrayFuncType)(size_t);
    typedef void*(
      MEMHOOK_CDECL* NewArrayNoThrowFuncType)(size_t, const std::nothrow_t&);
    typedef void(MEMHOOK_CDECL* DeleteFuncType)(void*);
    typedef void(MEMHOOK_CDECL* DeleteNoThrowFuncType)(void*,
                                                       const std::nothrow_t&);
    typedef void(MEMHOOK_CDECL* DeleteArrayFuncType)(void*);
    typedef void(
      MEMHOOK_CDECL* DeleteArrayNoThrowFuncType)(void*, const std::nothrow_t&);
    ...
    // It is only safe to set these when there's just one thread running (i.e.
    // at start up)
