...
void SourcePSA::saveInternal(DataSectionPtr pSect) const
{
    ...
    if (pPositionSrc_) {
        DataSectionPtr pVGSect =
          pSect->newSection(SourcePSAPositionSourceString);
        pPositionSrc_->save(pVGSect);
    }

    if (pVelocitySrc_) {
        DataSectionPtr pVGSect =
          pSect->newSection(SourcePSAVelocitySourceString);
    ...
    if (motionTriggered() || timeTriggered() || (queuedTotal_ > 0)) {
        ...
        if (motionTriggered()) {
            ...
            if (speed > sensitivity()) {
                ...
                frameTotal = int(
                  (1 + 2 * ((speed - sensitivity()) / sensitivity())) * dTime);

                if (frameTotal + particleSystem.size() >
                    particleSystem.capacity()) {
                    frameTotal =
                      particleSystem.capacity() - particleSystem.size();
                }
            }

            // Make the motion triggered particles.
            if (frameTotal > 0) {
                Vector3 displacementPerParticle =
                  displacement / float(frameTotal);
                createParticles(particleSystem,
                                objectToWorld,
                                frameTotal,
        ...
        if (timeTriggered() || (queuedTotal_ > 0)) {
            if (rate() > 0.0f) {
                ...
                if (accumulatedTime_ > timeBetweenParticles) {
                    frameTotal = int(accumulatedTime_ / timeBetweenParticles);
            ...
            // TODO : This feature added for GDC Austin, but only with xml
            // support Implement fully in BigWorld 2.0 but no harm in leaving it
            // here. Hard limit time trigged particles if maximum velocity is
            // set.
...
{
    return sizeof(SourcePSA) + pPositionSrc_->sizeInBytes() +
           pVelocitySrc_->sizeInBytes();
}
                                ...
                                const Vector3&  dispPerParticle)
{
    ...
    for (int count = 0; (count < numberOfParticles) &&
                        ((allowedTime_ == 0.0f) ||
                         (timestamp() - currentTime_ < allowedTime_));
         ++count) {
        ...
        if (grounded()) {
            if (pGS_) {
                // Make it a tiny bit higher in case it is currently snapped to
                // the ground This needs to be done since the RompCollider will
                // not find a collision if it starts at or below the ground.
                // Notice that the drop distance is offset by this amount as
                // well.

                position.y += 0.05f;

                position.y =
                  pGS_->ground(position, dropDistance() + 0.05f, true);
            }
        }
        ...
        if (position.y != RompCollider::NO_GROUND_COLLISION) {
            ...
            // Get the world pitch and yaw from the forward vector of the
            // psystem For sprite particles, pitch means spin speed, and yaw
            // means initial orientation
            float pitch = initialRotation_.x +
                          randomInitialRotation_.x * (unitRand() - 0.5f);
            float yaw = initialRotation_.y +
                        randomInitialRotation_.y * (unitRand() - 0.5f);
 ...
 *	Factory function to create and return a new PySourcePSA object. SourcePSA is
 *a ParticleSystemAction that creates particles within a ParticleSystem.
 *	@param position_source VectorGenerator as a position source for the
 *particles.
 *	@param velocity_source VectorGenerator as a velocity source for the
 *particles.
 ...
 *	Used by Mesh particle systems. Randomise the initial Pitch/Yaw. Default is
 *(0.0, 0.0).
 *	@type Sequence of 2 floats.
 */
 ...
 *
 *	Specifies how much velocity particles should inherit from SourcePSA when
 *spawned, eg, a value of 0.0 will add no extra velocity to spawning particles
 *and a value of 2.0 will add double the SourcePSA's velocity to the spawning
 *particles.
 *
 *	@type float
 */
 ...
 *	If set particle systems will be imbued with a random amount of angular
 *velocity, and a random rotation axis.  This property is exclusively for mesh
 *particles.
 */
PY_ATTRIBUTE(randomSpin)
/*~ attribute PySourcePSA.minSpin
 *	When randomSpin is set, the particle is given a random amount of spin,
 *between minSpin and maxSpin.  [The units are 20 radians per second]
 */
...
PyObject* PySourcePSA::pyNew(PyObject* args)
{
    ...
    if (PyArg_ParseTuple(args, "OO", &positionDesc, &velocityDesc)) {
        SourcePSAPtr pAction =
          new SourcePSA(VectorGenerator::parseFromPython(positionDesc),
...
/**
 *	This Python method allows the script to determine the max size of the
 *position vector generator.
 */
