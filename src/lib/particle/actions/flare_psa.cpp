...
/*static*/ void FlarePSA::prerequisites(DataSectionPtr       pSection,
...
void FlarePSA::lateExecute(ParticleSystem& particleSystem, float dTime)
{
    ...
    Matrix worldTransform = particleSystem.pRenderer()->local()
                              ? particleSystem.worldTransform()
                              : Matrix::identity;

    if (flareStep_ == 0) {
        // This now uses a very new method to determine the last particle added
        // (bug 32689) find the youngest particle
        ...
        while (current != endOfParticles) {
            ...
            if (particle.isAlive()) {
                // Use the particle index to ensure that particles keep their
                // flare in the case where there is a sink (bug 5236).
...
PyObject* PyFlarePSA::pyNew(PyObject* args)
{
    ...
    if (PyArg_ParseTuple(args, "s|iii", &flareType, &flareStep, &col, &size)) {
        ...
        PyErr_SetString(PyExc_TypeError,
                        "FlarePSA:"
                        "Expected a lens flare filename and optional params "
                        "(flare step),(colourize),(useParticleSize).");
        return NULL;
    }
}
