 ...
 *	sorting error.
 */
void MeshParticleSorter::sortOptimised(Particles::iterator beg,
                                       Particles::iterator end)
{
    ...
    maxDist_ =
      pos.x * view[0][2] + pos.y * view[1][2] + pos.z * view[2][2] + view[3][2];
    ...
    for (uint16 i = 0; i < nParticles_; i++, p++) {
        ...
        if (p->isAlive()) {
            const Vector3& pos  = p->position();
            float          dist = pos.x * view[0][2] + pos.y * view[1][2] +
