    ...
    if (Geometrics::beginTexturedWorldLines()) {
        ...
        while (iter != end) {
            ...
            for (int i = 1; i < steps_; i++) {
                ...
                Geometrics::texturedWorldLine(a,
                                              b,
                                              width_,
...
Particle* TrailParticleRenderer::TrailParticles::addParticle(
...
Vector3 TrailParticleRenderer::TrailParticles::getPosition(uint index,
                                                           uint age) const
{
...
void TrailParticleRenderer::TrailParticles::copyPositions()
{
    for (iterator it = begin(); it != end(); ++it) {
        ...
        if (particle.isAlive()) {
            ...
            (this->positionHistories_[historyIndex_])[particle.index()] =
              particle.position();
        }
    }
...
void TrailParticleRenderer::TrailParticles::resetHistory(
  const Particle& particle)
{
    for (uint i = 0; i < steps_; i++) {
        Vector3 v                                 = particle.position();
...
void TrailParticleRenderer::loadInternal(DataSectionPtr pSect)
{
    BW_GUARD;
    this->serialise(
      BW::LoadFromDataSection<TrailParticleRenderer>(pSect, this));
}

void TrailParticleRenderer::saveInternal(DataSectionPtr pSect) const
{
    BW_GUARD;
    this->serialise(BW::SaveToDataSection<TrailParticleRenderer>(pSect, this));
}
 ...
 *	Constructor.
 */
PyTrailParticleRenderer::PyTrailParticleRenderer(TrailParticleRendererPtr pR,
                                                 PyTypeObject*            pType)
