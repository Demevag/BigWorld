...
class VisualParticleRenderer : public BaseMeshParticleRenderer
{
  public:
    ...
    {
        return sizeof(VisualParticleRenderer);
    }
 ...
 *	VisualParticleRenderer is a ParticleSystemRenderer which renders each
 *particle as a visual object.  Any visual can be used, although the TintShader
 *will only work on visuals that use effect files that use MeshParticleTint.
 */
class PyVisualParticleRenderer : public PyParticleSystemRenderer
{
    Py_Header(PyVisualParticleRenderer, PyParticleSystemRenderer)
      ...
      /// @name Constructor(s) and Destructor.
      //@{
      PyVisualParticleRenderer(VisualParticleRendererPtr pR,
