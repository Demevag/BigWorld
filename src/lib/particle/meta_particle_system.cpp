...
#ifdef EDITOR_ENABLED
static AutoConfigString s_mpsMetaDataConfig(
  "editor/metaDataConfig/metaParticleSystem",
  "helpers/meta_data/meta_particle_system.xml");
#endif
 ...
 */
/* static */ void MetaParticleSystem::prerequisites(DataSectionPtr       pDS,
                                                    BW::set<BW::string>& output)
{
    ...
    if (pDS) {
        if (pDS->readString(ParticleSystem::VERSION_STRING, "") != "") {
            ...
            for (DataSectionIterator it = pDS->begin(); it != pDS->end();
                 ++it) {
                DataSectionPtr pSystemDS = *it;
                BW::string     systemName =
                  pDS->unsanitise(pSystemDS->sectionName());
 ...
 *	@return bool				Success or failure of the load operation.
 */
bool MetaParticleSystem::load(DataSectionPtr       pDS,
                              ...
                              StringBuilder*       pError /*= NULL*/)
{
    ...
    if (pDS->openSection(ParticleSystem::VERSION_STRING)) {
        ...
        for (DataSectionIterator it = pDS->begin(); it != pDS->end(); ++it) {
            // Only process the system sections, ignore other data like editor
            // only ones.
            ...
            if (isSystemSection) {
                ...
                if (newSystem->load(pSystemDS,
                                    pDS->unsanitise((*it)->sectionName()),
                                    false,
                                    pError)) {
                    addSystem(newSystem);
                }
    ...
    if (helperModel != NULL) {
        ...
        helperModelCenterOnHardPoint_ =
          helperModel->readUInt("helperModelCenterOnHardPoint", (uint)-1);
    }
                              ...
                              bool                 transient)
{
    ...
    DataSectionPtr pDS = BWResource::openSection(
      directory + filename, true); // create a new directory if need to
    ...
    if (helperModel != NULL) {
        ...
        helperModel->writeUInt("helperModelCenterOnHardPoint",
                               helperModelCenterOnHardPoint_);
    }
 ...
 */
void MetaParticleSystem::reSerialise(DataSectionPtr pDS,
                                     bool           load,
                                     bool           transient)
{
    BW_GUARD;
    if (load) {
        ...
        for (ParticleSystems::iterator it  = systems_.begin(),
                                       end = systems_.end();
             it != end;
             ++it) {
            // save system properties in separate sections
...
ParticleSystemPtr MetaParticleSystem::system(const BW::StringRef& name)
{
    ...
    for (ParticleSystems::iterator it = systems_.begin(); it != systems_.end();
         ++it) {
        if ((*it)->name() == name)
            return *it;
    }
...
/*static */ void MetaParticleSystem::getHardPointTransforms(
  ModelPtr                model,
...
MetaParticleSystemPtr MetaParticleSystemCache::load(const StringRef& filename)
{
