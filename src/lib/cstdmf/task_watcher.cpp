 ...
 */
void TaskWatcher::update(double duration,
                         double timeInQueue,
                         double timeInThread)
{
    ++numTasks_;
...
WatcherPtr TaskWatcher::pWatcher()
{
    ...
    if (pWatcher == NULL) {
        ...
        pWatcher->addChild("numTasks", makeWatcher(&TaskWatcher::numTasks_));

        pWatcher->addChild("totalTime", makeWatcher(&TaskWatcher::totalTime_));
...
bool TaskWatchers::init(SimpleMutex& mutex,
                        const char*  bgPath,
                        const char*  mtPath,
                        float        warnThreshold)
{
    ...
    WatcherPtr pBackgroundWatcher =
      new TaskMapWatcher(pNull->backgroundWatchers_);

    pBackgroundWatcher->addChild("*", TaskWatcher::pWatcher());

    // Make it thread safe
    WatcherPtr pSafeBackgroundWatcher =
      new SafeWatcher(pBackgroundWatcher, mutex);
    ...
    WatcherPtr pMainThreadWatcher =
      new TaskMapWatcher(pNull->mainThreadWatchers_);
 ...
 *	and by task type.
 */
void TaskWatchers::update(const char* taskName,
                          double      duration,
                          double      timeInThread,
