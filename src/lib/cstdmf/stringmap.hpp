...
class StringMap
{
  public:
    ...
    StringMap(const StringMap& other) { this->copy(other); }
    ...
    iterator find(STRING_TYPE key) { return map_.find(key); }

    const_iterator find(STRING_TYPE key) const { return map_.find(key); }
...
class StringSet
{
  public:
    ...
    size_type count(const value_type& value) const { return set_.count(value); }

    std::pair<iterator, bool> insert(const value_type& value)
    {
        STRING_TYPE               newKey = policy_.insert(value);
        std::pair<iterator, bool> retval = set_.insert(newKey);
    ...
    iterator find(STRING_TYPE key) { return set_.find(key); }

    const_iterator find(STRING_TYPE key) const { return set_.find(key); }
...
class StringRefMap
  : public StringMap<mapped_type,
                     policy,
                     BW::StringRef,
                     BW::map<BW::StringRef, mapped_type>>
{};

template <class mapped_type, class policy = OwningStringRefPolicy>
class StringRefUnorderedMap
  : public StringMap<mapped_type,
                     policy,
                     BW::StringRef,
                     BW::unordered_map<BW::StringRef, mapped_type>>
{};

template <class policy = OwningStringRefPolicy>
class StringRefSet
  : public StringSet<policy, BW::StringRef, BW::set<BW::StringRef>>
{};
 ...
 */
template <class _Ty, class _I = int16>
class OrderedStringMap
{
  public:
    ...
    //@{
    OrderedStringMap() { tables_.push_back(new Table()); }

    OrderedStringMap(const _Myt& toCopy) { this->copy(toCopy); }

    ~OrderedStringMap() { this->destruct(); }
     ...
     */
    bool erase(const BW::string& key) { return this->erase(key.c_str()); }
     ...
     */
    iterator find(const BW::string key) { return this->find(key.c_str()); }
     ...
     */
    size_type max_size() const { return 1UL << ((8 * sizeof(index_type)) - 1); }
     ...
     */
    void erase(iterator iter) { this->erase_iterator(iter); }
  ...
  private:
    ...
    void destruct()
    {
        for (typename Tables::iterator i = tables_.begin(); i != tables_.end();
             i++) {
            bw_safe_delete(*i);
        }
        tables_.clear();

        for (typename _Imp::iterator i = entries_.begin(); i != entries_.end();
             i++) {
    ...
    iterator find_and_insert(const key_type key_in, const referent_type* pRef)
    {
        ...
        if (kk == 0 && ekk == 0) { // ok, exact replacement ... easy
            if (pRef != NULL)
                i->second = *pRef;
            return i;
        } else {
            if (pRef == NULL)
                return entries_.end();
            return this->complicated_insert(
              *pRef, lastTableIndex, key_in, i, key_dist);
        }
    }

    // insert into the table indicated
    iterator simple_insert(const referent_type& ref,
    ...
    iterator complicated_insert(const referent_type& ref,
                                index_type           tableIndex,
                                key_type             key_in,
                                iterator             contender,
                                int                  key_dist)
    ...
    void erase_iterator(iterator iter)
    {
        ...
        if (iter != entries_.begin() + entries_.size() - 1) {
            ...
            *iter = entries_.back();
            ...
            tables_[lastTableIndex]->e[kk] = ~((int)(iter - entries_.begin()));
        }
...
std::ostream& operator<<(std::ostream& o, const OrderedStringMap<_Ty, _I>& dmap)
{
...
uint32 memoryUse(const OrderedStringMap<T, I>& obj)
{
