...
namespace {
    ...
    {
      public:
        TestBinaryFileInitState(BinaryFile& testFile,
                                size_t      testFileSize,
        ...
        TEST_SUB_FUNCTOR_OPERATOR
        {
            ...
            while (bytesRemainingToRead > 0) {
                const size_t readOpSize = std::min(
                  static_cast<size_t>(operationSize_), bytesRemainingToRead);
                ...
                RETURN_ON_FAIL_CHECK(
                  readOpSize ==
                  testFile_.read(&testBuffer[testBufferOffset], readOpSize));
        ...
        TEST_SUB_FUNCTOR_OPERATOR
        {
            ...
            while (bytesRemainingToWrite > 0) {
                const size_t writeOpSize = std::min(
                  static_cast<size_t>(operationSize_), bytesRemainingToWrite);
                ...
                testFile_.write(&testContents_[testContentsOffset],
                                writeOpSize);
            ...
            if ((testFile_.file() == NULL) ||
                ...
                MF_ASSERT(static_cast<std::ptrdiff_t>(testContents_.size()) <=
                          testFile_.dataSize());

                if (!testContents_.empty()) {
                    CHECK_EQUAL(memcmp(&testContents_[0],
                                       testFile_.data(),
                                       testContents_.size()),
                                0);
                }
                ...
                CHECK_EQUAL(testBuffer.size(),
                            fread(testBuffer.empty()
                                    ? NULL
    ...
    BW::vector<uint8> getRandomVector(size_t size, uint32 extraSeed = 0)
    {
        ...
        std::generate(result.begin(), result.end(), &randomUint8);
        return result;
    }
    ...
    class TestFileRead
    {
      public:
        ...
        TEST_SUB_FUNCTOR_OPERATOR
        {
            ...
            ASSERT_WITH_MESSAGE(tempFile != NULL,
    ...
    class TestFileWrite
    {
      public:
        ...
        TEST_SUB_FUNCTOR_OPERATOR
        {
            ...
            TestBinaryFileInitState testInitStateFunctor(file, 0, tempFile);
            TEST_SUB_FUNCTOR(testInitStateFunctor);
    ...
    class TestMemoryRead
    {
      public:
        ...
        TEST_SUB_FUNCTOR_OPERATOR
        {
            ...
            BW::vector<uint8> fileContents = getRandomVector(testDataSize_);
            ...
            switch (mode_) {
                ...
                case M_DIRECT:
                    file.wrap(reinterpret_cast<void*>(&fileContents[0]),
                ...
                case M_RESERVE:
                    void* internalBuffer =
                      file.reserve((int)fileContents.size());
                    memcpy(
        ...
        TEST_SUB_FUNCTOR_OPERATOR
        {
            ...
            switch (mode_) {
                ...
                case M_DIRECT:
                    ...
                    file.wrap(reinterpret_cast<void*>(&userBuffer[0]),
                              static_cast<int>(fileContents.size()),
                              false);
                    expectedFilePtr = NULL;
...
TEST(BinaryFile_Cache)
{
    ...
    CHECK_EQUAL(memcmp(file.data(), &testContents[0], testContents.size()), 0);
}
