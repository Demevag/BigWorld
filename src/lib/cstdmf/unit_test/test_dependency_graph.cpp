...
namespace {
     ...
     */
    typedef BW::unordered_map<BW::string, BW::vector<BW::string>>
      ExpectedOrderings;

    /**
     *	@return True if @a ordering is valid given @a expectedOrderings.
     */
    bool checkExpectedOrdering(const BW::vector<BW::string>& ordering,
                               const ExpectedOrderings&      expectedOrderings)
    {
        ...
        typedef BW::unordered_map<BW::string, OrderingVectorSizeT>
                         OrderingIndexMap;
    ...
    class TestGraphOrdering
    {
      public:
         ...
         *		due to a cycle).
         */
        TestGraphOrdering(const ExpectedOrderings& expectedOrderings,
        ...
        TEST_SUB_FUNCTOR_OPERATOR
        {
            ...
            RETURN_ON_FAIL_CHECK(expectedOrderings_.size() == graph.size());
            ...
            RETURN_ON_FAIL_CHECK(expectOrderingSuccess_ ==
                                 graph.getOrder(ordering));
      ...
      private:
         ...
         */
        void addExpectedNodesToGraph(DependencyGraph<BW::string>& graph)
        {
            for (ExpectedOrderings::const_iterator expItr =
         ...
         */
        void addExpectedDependenciesToGraph(DependencyGraph<BW::string>& graph)
        {
            for (ExpectedOrderings::const_iterator expItr =
                   expectedOrderings_.begin();
                 expItr != expectedOrderings_.end();
