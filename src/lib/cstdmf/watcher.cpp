...
bool Watcher::visitChildren(const void*     base,
                            const char*     path,
                                   ...
                                   Watcher::Mode& mode) const
{
    if (isEmptyPath(path)) {
        ...
        if (pChild != NULL) {
            const void* addedBase =
              (const void*)(((const uintptr)base) +
                            ((const uintptr)pChild->base));
            return pChild->watcher->getAsString(
              addedBase, this->tail(path), result, desc, mode);
        } else {
            return false;
        }
    }
    ...
    if (isEmptyPath(path)) {
        ...
        if (pChild != NULL) {
            const void* addedBase =
              (const void*)(((const uintptr)base) +
                            ((const uintptr)pChild->base));
            return pChild->watcher->getAsStream(
              addedBase, this->tail(path), pathRequest);
        } else {
            return false;
        }
    }
                                     ...
                                     const char* value)
{
    ...
    if (pChild != NULL) {
        ...
        return pChild->watcher->setFromString(
          addedBase, this->tail(path), value);
    } else {
        return false;
    }
}
 ...
 *	Override from Watcher.
 */
bool DirectoryWatcher::setFromStream(void*                 base,
                                     const char*           path,
                                     WatcherPathRequestV2& pathRequest)
{
    ...
    if (pChild != NULL) {
        void* addedBase = (void*)(((uintptr)base) + ((uintptr)pChild->base));
        return pChild->watcher->setFromStream(
          addedBase, this->tail(path), pathRequest);
    } else {
        return false;
    }
 ...
 *	Override from Watcher
 */
bool DirectoryWatcher::visitChildren(const void*         base,
                                     const char*         path,
    ...
    if (isEmptyPath(path)) {
        ...
        while (iter != container_.end()) {
            ...
            if (!pathRequest.addWatcherPath(
                  addedBase, NULL, (*iter).label, *(*iter).watcher)) {
                break;
            }
        ...
        if (pChild != NULL) {
            ...
            handled = pChild->watcher->visitChildren(
              addedBase, this->tail(path), pathRequest);
        }
    }
 ...
 * Override from Watcher.
 */
bool DirectoryWatcher::addChild(const char* path,
                                WatcherPtr  pChild,
                                void*       withBase)
{
    bool wasAdded = false;
    ...
    } else // Must be for a child
    {
        ...
        if (pFound == NULL) {
            ...
            size_t compareLength =
              (pSeparator == NULL) ? strlen((char*)path) : (pSeparator - path);
        ...
        if (pFound != NULL) {
            wasAdded =
              pFound->watcher->addChild(this->tail(path), pChild, withBase);
        }
    }
        ...
        while (iter != container_.end()) {
            if (compareLength == (*iter).label.length() &&
                ...
                if (pSeparator == NULL) {
                    ...
                    DirData* pChild =
                      const_cast<DirData*>(&(const DirData&)*iter);
 ...
 *	Constructor.
 */
CallableWatcher::ArgDescription::ArgDescription(WatcherDataType   type,
 ...
 *	Override from Watcher.
 */
bool CallableWatcher::getAsString(const void*    base,
                                  const char*    path,
                                  BW::string&    result,
                                  BW::string&    desc,
                                  Watcher::Mode& mode) const
 ...
 *	Override from Watcher.
 */
bool CallableWatcher::setFromString(void*       base,
                                    const char* path,
                                    const char* valueStr)
{
  ...
  int                   returnValueSize) const
{
    ...
    resultSize +=
      this->tupleElementStreamSize(static_cast<int>(output.length()));
 ...
 *	Override from Watcher.
 */
bool CallableWatcher::getAsStream(const void*           base,
                                  const char*           path,
                                  WatcherPathRequestV2& pathRequest) const
{
    if (isEmptyPath(path)) {
        ...
        for (size_t i = 0; i < numArgs; i++) {
            ...
            watcherValueToStream(
              argStream, argList_[i].description(), Watcher::WT_READ_ONLY);
            watcherValueToStream(
              argStream, argList_[i].type(), Watcher::WT_READ_ONLY);
        ...
        Watcher::Mode mode = Watcher::WT_READ_ONLY;
        watcherValueToStream(
          pathRequest.getResultStream(), (int32)exposeHint_, mode);
        pathRequest.setResult(comment_, mode, this, base);
        return true;
    }
 ...
 *	Constructor
 */
NoArgCallableWatcher::NoArgCallableWatcher(ExposeHint hint, const char* comment)
                                         ...
                                         WatcherPathRequestV2& pathRequest)
{
    ...
    *pInput >> typeChar;
    ...
    if (type == WATCHER_TYPE_TUPLE) {
        ...
        if (parameterCount != 0) {
            WARNING_MSG(
              "NoArgCallableWatcher::setFromStream: "
    ...
    if (this->onCall(output, value)) {
        BinaryOStream& resultStream = this->startResultStream(
          pathRequest, output, static_cast<int>(value.size()));
...
bool NoArgFuncCallableWatcher::onCall(BW::string& output, BW::string& value)
{
 ...
 *	Constructor
 */
SimpleCallableWatcher::SimpleCallableWatcher(ExposeHint  hint,
                                             const char* comment)
  : CallableWatcher(hint, comment)
{
}
                                          ...
                                          WatcherPathRequestV2& pathRequest)
{
    ...
    *pInput >> type;
    ...
    if ((WatcherDataType)type != WATCHER_TYPE_TUPLE) {
        ERROR_MSG(
          "SimpleCallableWatcher::setFromStream: "
          "Function Watcher called with non-tuple data as argument list. "
          "Received %d, wanted %d\n",
...
bool SafeWatcher::getAsString(const void*    base,
                              const char*    path,
                              BW::string&    result,
                              BW::string&    desc,
                              Watcher::Mode& mode) const
...
bool SafeWatcher::setFromString(void*       base,
                                const char* path,
                                const char* valueStr)
{
    MutexHolder mh(*this);
...
bool SafeWatcher::getAsStream(const void*           base,
                              const char*           path,
                              WatcherPathRequestV2& pathRequest) const
{
    MutexHolder mh(*this);
...
bool SafeWatcher::setFromStream(void*                 base,
                                const char*           path,
                                WatcherPathRequestV2& pathRequest)
{
    MutexHolder mh(*this);
...
bool SafeWatcher::visitChildren(const void*         base,
                                const char*         path,
                                WatcherPathRequest& pathRequest)
{
    MutexHolder mh(*this);
...
bool SafeWatcher::addChild(const char* path, WatcherPtr pChild, void* withBase)
{
    MutexHolder mh(*this);
...
ReadWriteLockWatcher::ReadWriteLockWatcher(WatcherPtr     pWatcher,
...
bool ReadWriteLockWatcher::getAsString(const void*    base,
                                       const char*    path,
                                       BW::string&    result,
                                       BW::string&    desc,
                                       Watcher::Mode& mode) const
...
bool ReadWriteLockWatcher::setFromString(void*       base,
                                         const char* path,
                                         const char* valueStr)
{
...
bool ReadWriteLockWatcher::getAsStream(const void*           base,
                                       const char*           path,
                                       WatcherPathRequestV2& pathRequest) const
{
    ReadWriteLock::ReadGuard rg(readWriteLock_);
...
bool ReadWriteLockWatcher::setFromStream(void*                 base,
                                         const char*           path,
                                         WatcherPathRequestV2& pathRequest)
{
    ReadWriteLock::WriteGuard wg(readWriteLock_);
...
bool ReadWriteLockWatcher::visitChildren(const void*         base,
                                         const char*         path,
                                         WatcherPathRequest& pathRequest)
{
    ReadWriteLock::ReadGuard rg(readWriteLock_);
...
bool ReadWriteLockWatcher::addChild(const char* path,
                                    WatcherPtr  pChild,
                                    void*       withBase)
{
    ReadWriteLock::WriteGuard wg(readWriteLock_);
...
bool DereferenceWatcher::getAsString(const void*    base,
                                     const char*    path,
                                     BW::string&    result,
                                     BW::string&    desc,
                                     Watcher::Mode& mode) const
{
    ...
    return baseAddress == 0
             ...
             : watcher_->getAsString(
                 (void*)(sb_ + baseAddress), path, result, desc, mode);
}

bool DereferenceWatcher::setFromString(void*       base,
                                       const char* path,
                                       const char* valueStr)
{
    uintptr baseAddress = base ? dereference(base) : 0;
    return baseAddress == 0 ? false
                            : watcher_->setFromString(
                                (void*)(sb_ + baseAddress), path, valueStr);
}

bool DereferenceWatcher::getAsStream(const void*           base,
                                     const char*           path,
                                     WatcherPathRequestV2& pathRequest) const
{
    uintptr baseAddress = base ? dereference(base) : 0;
    return baseAddress == 0 ? false
                            : watcher_->getAsStream(
                                (void*)(sb_ + baseAddress), path, pathRequest);
}

bool DereferenceWatcher::setFromStream(void*                 base,
                                       const char*           path,
                                       WatcherPathRequestV2& pathRequest)
{
    uintptr baseAddress = base ? dereference(base) : 0;
    return baseAddress == 0 ? false
                            : watcher_->setFromStream(
                                (void*)(sb_ + baseAddress), path, pathRequest);
}

bool DereferenceWatcher::visitChildren(const void*         base,
                                       const char*         path,
                                       WatcherPathRequest& pathRequest)
{
    uintptr baseAddress = base ? dereference(base) : 0;
...
bool DereferenceWatcher::addChild(const char* path,
                                  WatcherPtr  pChild,
                                  void*       withBase)
{
 ...
 *	Override from Watcher.
 */
bool AbsoluteWatcher::getAsString(const void*    base,
                                  const char*    path,
                                  BW::string&    result,
                                  BW::string&    desc,
                                  Watcher::Mode& mode) const
/* override */
{
    return (base == NULL)
             ? false
             : pWatcher_->getAsString(
                 (void*)(absoluteAddress_), path, result, desc, mode);
}

/*
 *	Override from Watcher.
 */
bool AbsoluteWatcher::setFromString(void*       base,
                                    const char* path,
                                    const char* valueStr) /* override */
{
    return (base == NULL) ? false
                          : pWatcher_->setFromString(
                              (void*)(absoluteAddress_), path, valueStr);
}

/*
 *	Override from Watcher.
 */
bool AbsoluteWatcher::getAsStream(
  const void*           base,
  const char*           path,
  WatcherPathRequestV2& pathRequest) const /* override */
{
    return (base == NULL) ? false
                          : pWatcher_->getAsStream(
                              (void*)(absoluteAddress_), path, pathRequest);
}

/*
 *	Override from Watcher.
 */
bool AbsoluteWatcher::setFromStream(
  void*                 base,
  const char*           path,
  WatcherPathRequestV2& pathRequest) /* override */
{
    return (base == NULL) ? false
                          : pWatcher_->setFromStream(
                              (void*)(absoluteAddress_), path, pathRequest);
}
 ...
 *	Override from Watcher.
 */
bool AbsoluteWatcher::visitChildren(
  const void*         base,
  const char*         path,
  WatcherPathRequest& pathRequest) /* override */
{
    return (base == NULL) ? false
                          : pWatcher_->visitChildren(
                              (void*)(absoluteAddress_), path, pathRequest);
}
 ...
 *	Override from Watcher.
 */
bool AbsoluteWatcher::addChild(const char* path,
                               WatcherPtr  pChild,
