...
namespace BW {
    ...
    namespace {
        ...
        const size_t CALLSTACK_HEADER_SIZE =
          sizeof(MemoryDebug::CallstackHeader);
        const size_t GUARD_ITEM_SIZE     = sizeof(StackTracker::StackItem);
        const size_t CALLSTACK_ITEM_SIZE = sizeof(CallstackAddressType);
        const size_t MAX_CALLSTACK_SIZE =
          CALLSTACK_HEADER_SIZE + (GUARD_ITEM_SIZE * MAX_GUARD_DEPTH) +
        ...
        typedef std::pair<const BW::MemoryDebug::CallstackHeader*,
                          BW::MemoryDebug::CallstackStats>
                     CallstackStatPair;
        BW_NAMESPACE StaticArray<CallstackStatPair, 256 * 1024>
                     s_sortedCallstacks;
        ...
        typedef BW::map<CallstackAddressType,
                        CacheGrindEventInfo,
                        std::less<CallstackAddressType>,
                        detail::DebugStlAllocator<
                          std::pair<CallstackAddressType, CacheGrindEventInfo>>>
          CacheGrindEventMap;
        typedef BW::map<const char*,
                        CacheGrindEventInfo,
                        std::less<const char*>,
                        detail::DebugStlAllocator<
                          std::pair<const char*, CacheGrindEventInfo>>>
          CacheGrindCalleeEventMap;
          ...
          public:
            CacheGrindCalleeEventInfo(FixedSizedAllocator* allocator)
              : events(CacheGrindCalleeEventMap::key_compare(),
                       detail::DebugStlAllocator<
                         CacheGrindCalleeEventMap::value_type>(allocator))
            {
            }
        ...
        typedef BW::map<
          ...
          std::less<CallstackAddressType>,
          detail::DebugStlAllocator<
            std::pair<CallstackAddressType, CacheGrindCalleeEventInfo>>>
          CacheGrindCalleeMap;
        ...
        class CacheGrindFrameInfo
        {
          public:
            CacheGrindFrameInfo(FixedSizedAllocator* allocator)
              : callees(
                  CacheGrindCalleeMap::key_compare(),
                  detail::DebugStlAllocator<CacheGrindCalleeMap::value_type>(
                    allocator))
              , events(
                  CacheGrindEventMap::key_compare(),
        ...
        typedef BW::map<const char*,
                        ...
                        std::less<const char*>,
                        detail::DebugStlAllocator<
                          std::pair<const char*, CacheGrindFrameInfo>>>
          CacheGrindFrameMap;
    ...
    {
        return const_cast<CallstackAddressType*>(
          callstack(const_cast<const CallstackHeader*>(hdr)));
    }
    ...
    StackTracker::StackItem* MemoryDebug::CallstackHeader::guardstack(
      MemoryDebug::CallstackHeader* hdr)
    {
        return const_cast<StackTracker::StackItem*>(
          guardstack(const_cast<const MemoryDebug::CallstackHeader*>(hdr)));
    }
    ...
    class MemoryDebug::AllocationStats
    {
      public:
        ...
        typedef BW_NAMESPACE
            StaticArray<SlotStat, BW::ThreadSlotTracker::MAX_SLOTS>
            SlotStats;

        SlotStat heapStats_; // Heap only stats (Not including allocations from
                             // fixed sized pools)
        SlotStat globalStats_;   // Global allocation stats (via
                                 // Allocator::allocate calls, not including
                                 // internal fixed sized pool memory)
        SlotStat poolStats_;     // Pool allocation stats
        SlotStat internalStats_; // Internal allocation stats
        SlotStats
          slotStats_; // Slot allocation stats (via Allocator::allocate calls,
                      // not including internal fixed sized pool memory)
        ...
        void reportSlotHeader(FILE* f)
        {
            fprintf(
              f,
              "Slot name, Peak allocation (bytes), Total allocation count, "
        ...
        void reportSlotToFile(FILE* f, const SlotStat& slot)
        {
            ...
            size_t leakedBytes =
              slot.liveAllocationBytes_ - slot.ignoreLeaksAllocationBytes_;
            size_t leakedCount =
              slot.liveAllocationCount_ - slot.ignoreLeaksAllocationCount_;
    ...
    MemoryDebug::MemoryDebug(CallstackMode csMode,
                             bool          reportHotspots,
                             bool          reportIgnoredLeaks,
                             bool          reportCacheGrind)
      : allocator_(256 * 1024)
      , callstacks_(NULL)
    ...
    void MemoryDebug::init()
    {
        if (internalInit_) {
            ...
            debugAllocator_ = new (debugAllocatorMem_)
              FixedSizedAllocator(256, 512 * 1024, "MemoryDebug");
            ...
            callstacks_ = new (callstacksMem_)
              CallstackMap(256 * 1024,
            ...
            stringTable_ = new (stringTableMem_) HashStringMap(
              256 * 1024,
              HashCStr(),
              EqualsCStr(),
              detail::DebugStlAllocator<std::pair<const char*, size_t>>(
                debugAllocator_));

            lookupCache_ = new (lookupCacheMem_) LookupCacheMap(
              1 * 1024,
              BW::hash<const void*>(),
              std::equal_to<const void*>(),
...
#if ENABLE_SMARTPOINTER_TRACKING
            smartPointers_ = new (smartPointersMem_) SmartPointerMap(
              256 * 1024,
              BW::hash<const void*>(),
              std::equal_to<const void*>(),
              detail::DebugStlAllocator<
                std::pair<const void*, SmartPointerInfo>>(debugAllocator_));
#endif

            allocationStats_ =
              new (allocator_.allocate(sizeof(AllocationStats)))
                AllocationStats;
            ...
            if (stacktraceSymbolResolutionAvailable()) {
                ...
                // head of the stack, we use this address to figure out if the
                // fast callstack method made it to the top or not
                CallstackAddressType callstack[64];
                int                  depth =
                  stacktraceAccurate(callstack, ARRAY_SIZE(callstack));

                const char* mainFuncName = "BaseThreadInitThunk";

                for (int i = 0; i < depth; ++i) {
                    char name[SYMBOL_STRING_MAX_LENGTH] = { 0 };
                    if (convertAddressToFunction(callstack[i],
                                                 name,
                                                 ARRAY_SIZE(name),
                                                 NULL,
                                                 0,
                ...
                dprintf(
                  " MemoryDebug : Disabling callstacks as symbol resolution is "
                  "not available, reverting to using BW_GUARD mode\n");
    ...
    {
        strncpy(detailedCallstackForGuardString_,
                str,
                                           ...
                                           bool* requestDetailedCallstack)
    {
        ...
        for (int i = 0; i < guardDepth; ++i) {
            ...
            // make a copy of the temporary strings before generating hash for
            // the callstack
            if (stackItem.temp) {
            ...
            if (stackItem.annotation) {
                stackItem.annotation =
                  getStringTableEntry(stackItem.annotation);
            }

            if (showDetailedCallstackForGuard_ && requestDetailedCallstack) {
                if (strstr(guardStack[i].name,
                           detailedCallstackForGuardString_) != 0) {
    ...
    void MemoryDebug::allocate(const void* ptr,
                               size_t      sizeUsed,
                               ...
                               uint32      allocFlags)
    {
        ...
        // IF_NOTRACK_ALLOC means the allocation shouldn't be tracked, usually
        // because it's already been tracked by Allocator::allocate and again
        // through Allocator::heapAllocate.
        ...
        info.flags =
          (systemStage_ == Allocator::SS_PRE_MAIN) ? AF_ALLOCATION_PREMAIN : 0;
        info.flags |=
          (allocFlags & Allocator::IF_POOL_ALLOC) ? AF_ALLOCATION_POOL : 0;
        info.flags |= (allocFlags & Allocator::IF_INTERNAL_ALLOC)
                        ? AF_ALLOCATION_INTERNAL | AF_ALLOCATION_IGNORELEAK
                        : 0;
    ...
    void MemoryDebug::deallocate(const void* ptr)
    {
        ...
        if (it != liveAllocations_->end()) {
            // update stats
            allocationStats_->decBytes(
              it->second.sizeUsed, it->second.slot, it->second.flags);
    ...
    MemoryDebug::ProcessMemoryInfo MemoryDebug::getProcessMemoryInfo()
    {
        ...
        while (1) {
            ...
            switch (memInfo.Type) {
                case MEM_IMAGE:
                    procMemInfo.imageBytes += memInfo.RegionSize;
                    break;
                case MEM_MAPPED:
                    procMemInfo.mappedBytes += memInfo.RegionSize;
                    break;
                case MEM_PRIVATE:
                    procMemInfo.privateBytes += memInfo.RegionSize;
                    break;
            }
        ...
        procMemInfo.virtualAddressAvailable =
          (size_t)memoryStatus.ullTotalVirtual;
    ...
    std::pair<const MemoryDebug::CallstackHeader* const,
              MemoryDebug::CallstackStats>&
    MemoryDebug::getCallstackEntry(uint8 flags, size_t skipFrames)
    {
        ...
        tempHdr->guardDepth = recordGuardCallstack(
          guardStack, MAX_GUARD_DEPTH, &debugCompleteStackRequested);

        bool getCompleteStack = (callstackMode_ == CM_CALLSTACKS_COMPLETE) ||
                                (tempHdr->guardDepth == 0) ||
        ...
        // if mode is CM_BW_GUARD we do not attempt to collect any callstack
        // info unless there are no guards on the stack, in this case we just
        // grab a quick stacktrace.
        if (callstackMode_ != CM_BW_GUARD || getCompleteStack) {
            // first we attempt to use the fast method and see if this makes it
            // to the top of the stack as recorded when the system was
            // initialised, if it didn't we back to the slower method
            tempHdr->depth =
              stacktrace(callstack, MAX_CALLSTACK_DEPTH, skipFrames);
            ++totalStacksGenerated_;

#if defined(_WINDOWS_)
            // ideally we would potentially gather an accurate one but this is
            // too slow in the general case
            if (callstackMode_ != CM_BW_GUARD) {
                bool hasTop = false;
                for (int i = 0; i < tempHdr->depth; ++i) {
                    uint64 dist = (uint64)callstack[i] - (uint64)callstackTop_;
                    // callstack points to a return address within the function
                    // so see if we are near the top function
                    if (dist < 1024) {
                        hasTop = true;
                        break;
                    }
                ...
                if (!hasTop && (callstackMode_ == CM_CALLSTACKS_COMPLETE ||
                    ...
                    tempHdr->depth =
                      stacktraceAccurate(callstack, MAX_CALLSTACK_DEPTH);
                }
            }
#endif // _WINDOWS_
        }
        ...
        if (it == callstacks_->end()) {
            ...
            CallstackHeader* newHdr =
              (CallstackHeader*)allocator_.allocate(totalSize);
            ...
            MF_ASSERT_NOALLOC(memcmp(&callstack[0],
                                     ...
                                     sizeof(callstack[0]) * it->first->depth) ==
                              0);
#endif // BW_ENABLE_STACKTRACE
            MF_ASSERT_NOALLOC(
              memcmp(&guardStack[0],
    ...
    void MemoryDebug::formatShortGuardStack(
      const StackTracker::StackItem* items,
      size_t                         numItems,
      BW::StringBuilder&             builder)
    {
...
#if BW_ENABLE_STACKTRACE
    void MemoryDebug::formatShortCallstack(
...
//-----------------------------------------------------------------------------
    ...
    void MemoryDebug::saveAllocationsToFile(const char* filename)
    {
        ...
        if (f) {
            ...
            if (callstacks_) {
                ...
                for (CallstackMap::const_iterator itr = callstacks_->begin(),
                    ...
                    if ((callstackMode_ != CM_BW_GUARD) ||
                        ...
                        formatShortCallstack(
                          CallstackHeader::callstack(hdr), hdr->depth, builder);
                    } else
#endif // BW_ENABLE_STACKTRACE
                    {
            ...
            for (LiveAllocationMap::const_iterator
                   itr = liveAllocations_->begin(),
                   end = liveAllocations_->end();
                 itr != end;
                 ++itr) {
                const AllocationInfo& info = itr->second;
                const char*           slotName =
                  ThreadSlotTracker::instance().slotName(info.slot);
                fprintf(
                  f, "%s;%p;%d\n", slotName, info.callstack, info.sizeUsed);
            }
    ...
    {
        const char* ignoredFunctions[] = { "BW::MemoryDebug::allocate",
    ...
    void MemoryDebug::saveAllocationsToCacheGrindFile(const char* filename)
    {
        ...
        // Avoid "most vexing parse" issue by declaring frameMapKeyCompare
        // variable before usage instead of inlining it into the frameMap
        // constructor.
        CacheGrindFrameMap::key_compare frameMapKeyCompare;
        CacheGrindFrameMap              frameMap(
          frameMapKeyCompare,
          detail::DebugStlAllocator<CacheGrindFrameMap::value_type>(
            debugAllocator_));

        if (callstacks_ == NULL || liveAllocations_ == NULL) {
        ...
        for (CallstackMap::const_iterator itr = callstacks_->begin(),
            ...
            for (int i = 0; i < hdr->depth; ++i) {
                ...
                if (matchIgnoredFunction(name) && prevAddress == NULL) {
                    continue;
                }
                ...
                CacheGrindFrameMap::iterator findFrame =
                  frameMap
                    .insert(std::make_pair(
                      name, CacheGrindFrameInfo(debugAllocator_)))
                    .first;
                ...
                if (prevAddress == NULL) {
                    ...
                    CacheGrindCalleeMap::iterator findCallee =
                      frame.callees
                        .insert(std::make_pair(
                          address, CacheGrindCalleeEventInfo(debugAllocator_)))
                        .first;
        ...
        for (LiveAllocationMap::const_iterator itr = liveAllocations_->begin(),
            ...
            CallstackMap::iterator stackFind =
              callstacks_->find(info.callstack);
            ...
            for (int i = 0; i < hdr->depth; ++i) {
                ...
                if (prevAddress == NULL) {
                    // The first entry in the stack (adjust it's frame
                    // information)
                    ...
                    if (calleeInfo.iterationToken_ != iterationToken_) {
                        calleeEventInfo.addAllocation(info.sizeUsed,
                                                      info.flags);
                    }
                    calleeInfo.iterationToken_ = iterationToken_;
                }
        ...
        if (f) {
            fprintf(
              f,
              "events: TotalAllocs LiveCount LiveSize IgnoredCount IgnoredSize "
              "LeakCount LeakSize InternalCount InternalSize\n");
            ...
            for (CacheGrindFrameMap::iterator iter = frameMap.begin(),
                ...
                for (CacheGrindEventMap::iterator iter = frame.events.begin(),
                                                  end  = frame.events.end();
                     iter != end;
                     ++iter) {
                    CallstackAddressType address = iter->first;
                    CacheGrindEventInfo& events  = iter->second;

                    // Fetch the line number of the function the events occurred
                    // on
                    int line = 1;
                ...
                for (CacheGrindCalleeMap::iterator iter = frame.callees.begin(),
                    ...
                    for (CacheGrindCalleeEventMap::iterator
                        ...
                        size_t leakCount =
                          events.liveCount - events.ignoredCount;
                        size_t leakSize = events.liveSize - events.ignoredSize;
                        fprintf(f,
                                "%d %" PRIzu " %" PRIzu " %" PRIzu " %" PRIzu
                                " %" PRIzu " %" PRIzu " %" PRIzu " %" PRIzu
        ...
        dprintf(" Completed.\n");
#else
        dprintf(" ERROR: CacheGrind output requires compilation with "
                "BW_ENABLE_STACKTRACE and ENABLE_MEMORY_DEBUG.\n");
#endif // BW_ENABLE_STACKTRACE
        dprintf("--------------------------------------------------------------"
                "--------------------\n");
    }
    ...
    void MemoryDebug::saveStatsToFile(const char* filename)
    {
        ...
        if (f) {
            ...
            allocationStats_->reportSlotToFile(f, allocationStats_->heapStats_);
            allocationStats_->reportSlotToFile(f, allocationStats_->poolStats_);
            allocationStats_->reportSlotToFile(
              f, allocationStats_->internalStats_);

            // Print each slot:
            for (size_t i = 0; i < allocationStats_->slotStats_.size(); ++i) {
                AllocationStats::SlotStat& slot =
                  allocationStats_->slotStats_[i];
    ...
    size_t MemoryDebug::printLiveAllocations()
    {
        ...
        if (systemStage_ == Allocator::SS_PRE_MAIN) {
            ...
            // stage was not set. this can happen when loading and unloading
            // cstdmf as a dll without calling main - ie when the asset pipeline
            // scans for converter dlls to dynamically load and finds cstdmf.dll
            return 0;
        }
        ...
        for (LiveAllocationMap::iterator it = liveAllocations_->begin();
             it != liveAllocations_->end();
             ++it) {
            const AllocationInfo&              info = it->second;
            const CallstackMap::const_iterator csItr =
              callstacks_->find(info.callstack);
            ...
            if (ucEntry == uniqueCallstacks.size()) {
                if (uniqueCallstacks.size() == uniqueCallstacks.capacity()) {
                    MF_DEBUGPRINT_NOALLOC(
                      "Live allocation unique callstack size exeeded");
                    break;
                }
        ...
        size_t numLeaksDetected =
          numLeakedPreMainAllocs + numLeakedMainAllocs - numLeakedIgnoredAllocs;
        ...
        if (numLeaksDetected > 0 ||
            ...
            std::sort(uniqueCallstacks.begin(),
                      uniqueCallstacks.end(),
                      callstackSorter);
            ...
            for (int i = 0, size = (int)uniqueCallstacks.size(); i < size;
                 ++i) {
                const CallstackReport& report = uniqueCallstacks[i];

                if (!reportIgnoredLeaks_ && report.numAllocations == 0) {
                    continue;
                }
                ...
                builder.appendf("   Leaked %d bytes from %d allocations",
                                report.leakSize,
                                report.numAllocations);

                if (reportIgnoredLeaks_) {
                    builder.appendf(" (%d ignored)",
                                    report.numIgnoredAllocations);
                }
        ...
        if (reportHottestCallstacks_) {
            ...
            std::sort<CallstackStatPair*, CallstackPairSort>(
              s_sortedCallstacks.begin(),
              s_sortedCallstacks.end(),
            ...
            dprintf("----------------------------------------------------------"
                    "------------------------\n");
            for (int i = 0, size = (int)s_sortedCallstacks.size(); i < size;
                 ++i) {
        ...
        //	(int)((float)totalStacksGeneratedSlow_ / totalStacksGenerated_ *
        //100) );
#endif
...
#endif // BW_ENABLE_STACKTRACE
                formatLongGuardStack(
                  CallstackHeader::guardstack(hdr), hdr->guardDepth, builder);
                dprintf(builder.string());
                dprintf("------------------------------------------------------"
                        "----------------------------\n");
            }
    ...
    void MemoryDebug::addWatchers()
    {
        ...
        pSlotWatcher->addChild("name",
                               makeWatcher(&AllocationStats::SlotStat::name_));

        AllocationStats::SlotStat* pNullSlot = NULL;
        pSlotWatcher->addChild(
          "curBytes", makeWatcher((long&)pNullSlot->liveAllocationBytes_));
        pSlotWatcher->addChild(
          "curBlocks", makeWatcher((long&)pNullSlot->liveAllocationCount_));

        pWatchSlots->addChild("*", pSlotWatcher);
        pWatchSlots->setLabelSubPath("name");

        Watcher::rootWatcher().addChild(
          "MemoryTracker", pWatchSlots, allocationStats_);
                                                  ...
                                                  const void* object)
    {
        ...
        if (iter == smartPointers_->end()) {
            smartPointers_->insert(
              std::make_pair(smartPointer, smartPointerInfo));
        } else {
