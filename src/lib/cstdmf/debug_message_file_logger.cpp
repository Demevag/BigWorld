...
namespace { // anonymous
    const char* strSeverities[] = { "TRACE",    "DEBUG",   "INFO",
                                    "NOTICE",   "WARNING", "ERROR",
                                    "CRITICAL", "HACK",    "ASSET" };

    const char* strSources[] = { "CPP", "SCRIPT" };
     ...
     *								associated to 1<<1, stringTable[2]
     *associated to 1<<2.
     * @return						The bits result converted from the string
     *array.
     *
     */
    template <typename T>
    T stringArrayToBits(const BW::vector<BW::string>& stringArrayValue,
                        ...
                        size_t                        lenStringsTable,
                        BW::StringBuilder&            errorMsg)
    {
        MF_ASSERT(lenStringsTable < sizeof(T) * 8);
        T                                      retBits = 0;
        BW::vector<BW::string>::const_iterator sectIt =
...
BW::string DebugMessageFileLogger::severities() const
{
    ...
    char retString[1024] = "";
    bitsToString<uint16>(
      severities_, strSeverities, ARRAY_SIZE(strSeverities), retString, 1024);
    return retString;
}
...
BW::string DebugMessageFileLogger::sources() const
{
    ...
    char retString[1024] = "";
    bitsToString<uint8>(
      sources_, strSources, ARRAY_SIZE(strSources), retString, 1024);
    return retString;
}
...
{
    BW_GUARD;
    return stringArrayToBits<uint8>(
      sourceArray, strSources, ARRAY_SIZE(strSources), errorMsg);
}
...
{
    BW_GUARD;
    return stringArrayToBits<uint16>(
      serverityArray, strSeverities, ARRAY_SIZE(strSeverities), errorMsg);
}
                                        ...
                                        StringBuilder&    errorMsg)
{
    if (bw_str_equal(openMode.c_str(), "w", 1)) {
        ...
        errorMsg.appendf(
          "DebugMessageFileLogger::convertOpenMode:"
 ...
 * @param category			category, if it's "",means all categories are
 *included.
 * @param openMode			file open mode.
 * @param enableLog			if start to write into disk.
 */
void DebugMessageFileLogger::config(
  const BW::string& logFileName /*= BW::string()*/,
  ...
  bool              enableLog /*= false*/)
{
    ...
    bw_safe_delete(pOutFile_);
    pOutFile_ = new std::ofstream(
      fileName_, (openMode == APPEND) ? std::ios::app : std::ios::trunc);
    ...
    category_   = category;
    std::transform(
      category_.begin(), category_.end(), category_.begin(), tolower);
    sources_  = sources;
    openMode_ = openMode;
...
{
    BW_GUARD;
    return ((priority == MESSAGE_PRIORITY_TRACE) &&
            (severities_ & SERVERITY_TRACE)) ||
           ((priority == MESSAGE_PRIORITY_DEBUG) &&
            (severities_ & SERVERITY_DEBUG)) ||
           ((priority == MESSAGE_PRIORITY_INFO) &&
            (severities_ & SERVERITY_INFO)) ||
           ((priority == MESSAGE_PRIORITY_NOTICE) &&
            (severities_ & SERVERITY_NOTICE)) ||
           ((priority == MESSAGE_PRIORITY_WARNING) &&
            (severities_ & SERVERITY_WARNING)) ||
           ((priority == MESSAGE_PRIORITY_ERROR) &&
            (severities_ & SERVERITY_ERROR)) ||
           ((priority == MESSAGE_PRIORITY_CRITICAL) &&
            (severities_ & SERVERITY_CRITICAL)) ||
           ((priority == MESSAGE_PRIORITY_HACK) &&
            (severities_ & SERVERITY_HACK)) ||
           ((priority == MESSAGE_PRIORITY_ASSET) &&
            (severities_ & SERVERITY_ASSET));
}
 ...
 */
bool DebugMessageFileLogger::handleMessage(DebugMessagePriority messagePriority,
                                           ...
                                           va_list     argPtr)
{
    ...
    if (pOutFile_ == NULL || !this->enable() ||
        !this->matchSeverity(messagePriority) ||
        !this->matchSource(messageSource) || !this->matchCategory(pCategory)) {
        return false;
    }
...
void DebugMessageFileLogger::addLogHeader()
{
    ...
    *pOutFile_ << "BigWorld " << exe.c_str() << " "
               << BWVersion::versionString() << " (compiled at " << compileTime
               << ") starting on " << executeTime << "\n\n";
