                            ...
                            DWORD     dwStatusInformationLength)
    {
        if (dwInternetStatus == INTERNET_STATUS_REQUEST_COMPLETE) {
            ...
            if (status_ == NOT_CONNECTED) {
                ...
                status_ = SENDING;
                FtpPutFile(ftp_,
                           localFileName_.c_str(),
                           removeFileName_,
                           FTP_TRANSFER_TYPE_ASCII,
                           dwContext);
            } else if (status_ == SENDING) {
                status_ =
                  result->dwError == ERROR_SUCCESS ? COMPLETED : FAILURE;
  ...
  public:
      ...
      , removeFileName_(fileName)
    {
        ...
        if (!localFileName_.empty()) {
            HANDLE file = CreateFile(localFileName_.c_str(),
            ...
            if (file != INVALID_HANDLE_VALUE) {
                ...
                if (inet_) {
                    InternetSetStatusCallback(
                      inet_, (INTERNET_STATUS_CALLBACK)statusCallback);

                    InternetConnect(inet_,
                                    server,
    ...
    bool completed() const { return status_ == COMPLETED; }

    bool failure() const { return status_ == FAILURE; }
};
