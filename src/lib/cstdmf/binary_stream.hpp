...
inline BinaryOStream& operator<<(BinaryOStream& b, const BW::vector<T, A>& data)
{
...
inline int BinaryIStream::readPackedInt()
{
    ...
    if (value == 0xFF) { // line below is very slightly dodgy ('tho safe here)
...
// the MSVC compiler requires this operator because it doesn't find the
// binarystream operator<< for vectors
 ...
 */
#define TOKEN_ADD(stream, TOKEN) stream << *(int*)TOKEN;
