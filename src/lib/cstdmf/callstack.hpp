...
#define BW_ENABLE_STACKTRACE                                                   \
    ENABLE_MEMORY_DEBUG && !BWENTITY_DLL &&                                    \
...
namespace BW {
    ...
    /// \warning For this function to work correctly with optimisations enabled
    /// you
    ///          need to disable the frame pointer omission optimisation using
    ///          /Oy-
    ...
    bool stacktraceAccurate(char*  stackMsg,
                            size_t stackMsgSize,
                            size_t skipFrames = 0,
                            int    threadId   = 0);
