...
namespace {
    ...
    // Dummy declaration of default slot - set by ThreadSlotTracker::init().
    // This is required for using MEMTRACKER_BREAK_ON_ALLOC(slotId,allocId) on
    // default slot.
...
namespace BW {
    ...
    /*static*/ THREADLOCAL(BW::ThreadSlotTracker::ThreadState)
      ThreadSlotTracker::s_threadState;
    ...
    /*static*/ void ThreadSlotTracker::init()
    {
        if (!s_threadSlotTracker) {
            s_threadSlotTracker =
              new (s_threadSlotTrackerMem) ThreadSlotTracker;
            s_trackerSlot_Default =
              s_threadSlotTracker->declareSlot("Default", 0);
    ...
    int ThreadSlotTracker::declareSlot(const char* name, uint32 flags)
    {
        ...
        bw_str_copy(
          slotNames_[slot], ARRAY_SIZE(slotNames_[slot]), name, nameLength);
