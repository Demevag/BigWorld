...
namespace BW {
    ...
    class BasicStringRef
    {
        ...
        static inline char toLower(char ch) { return std::tolower(ch); }

        static inline wchar_t toLower(wchar_t ch) { return std::towlower(ch); }
      ...
      public:
        ...
        {
            MF_ASSERT(str_);
            return std::basic_string<CharT, Traits, StlAllocator<CharT>>(
              str_, length_);
        }
        ...
        int compare(const BasicStringRef<CharT>& str) const
        {
            ...
            int       result =
              traits_type::compare(str_, str.str_, std::min(lhs_sz, rhs_sz));
            if (result != 0) {
                return result;
            }
            if (lhs_sz < rhs_sz) {
        ...
        int compare_lower(const BasicStringRef<CharT>& str) const
        {
            size_type lhs_sz = length_;
            size_type rhs_sz = str.length_;
            for (size_t i = 0, min_sz = std::min(lhs_sz, rhs_sz); i != min_sz;
                 ++i) {
                       ...
                       size_type                    pos = 0) const
        {
            if (str.length_ < length_) {
                for (size_type i   = std::min(pos, length_),
                               end = length_ - str.length_;
                     i <= end;
                     ++i) {
                    if (substr(i, str.length_) == str)
                        return i;
                }
            }
            return npos;
        ...
        size_type find_first_of(const CharT* str,
                                size_type    pos,
        ...
        size_type find_first_not_of(const CharT* str,
                                    size_type    pos,
                                    size_type    count) const
        {
        ...
        size_type find_last_of(const CharT* str,
                               size_type    pos,
                               size_type    count) const
        {
        ...
        size_type find_last_not_of(const CharT* str,
                                   size_type    pos,
                                   size_type    count) const
        {
        ...
        CharT operator[](size_type index) const { return at(index); }
    ...
    // It would be preferable to not do this, but lots of existing string code
    // uses +
    ...
    namespace detail {
                                      ...
                                      std::size_t                        n)
        {
            ...
            std::fill_n(
              fill_chars, static_cast<std::size_t>(chunk_size), os.fill());
