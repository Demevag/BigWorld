...
#define BW_PLATFORM_SUPPORTS_CONSOLE                                           \
    (defined(MF_SERVER) || defined(PLAYSTATION3) || defined(__APPLE__))
...
{
    RecursiveMutexHolder        lock(criticalCallbackMutex_);
    CriticalCallbacks::iterator it = std::find(
      pCriticalCallbacks_.begin(), pCriticalCallbacks_.end(), pCallback);
...
{
    RecursiveMutexHolder     lock(messageCallbackMutex_);
    DebugCallbacks::iterator it = std::find(
      pMessageCallbacks_.begin(), pMessageCallbacks_.end(), pCallback);
 ...
 */
bool DebugFilter::handleMessage(DebugMessagePriority messagePriority,
                                ...
                                va_list              argPtr)
{
    ...
    for (size_t i = 0; !handled && i < pMessageCallbacks_.size(); ++i) {
        ...
        handled = pMessageCallbacks_[i]->handleMessage(messagePriority,
                                                       pCategory,
                                                       messageSource,
                                                       metaData,
                                                       pFormat,
 ...
 */
void DebugFilter::setCategoryFilter(
                                       ...
                                       DebugMessagePriority messagePriority)
{
    ...
    BW::string categoryAsString(pCategoryName);
    {
        ...
        CategoriesMap::const_iterator it =
          suppressedCategories_.find(categoryAsString);
...
void DebugFilter::addWatchers()
{
    ...
    pCategorySuppressionWatcher->setComment(
    ...
    pChildWatcher->setComment(
      "0:TRACE; 1:DEBUG; 2:INFO; 3:NOTICE; 4:WARNING; "
...
void DebugFilter::removeWatchers() const
{
    ...
    MF_VERIFY(Watcher::rootWatcher().removeChild("logger/categorySupression"));

    MF_VERIFY(
      Watcher::rootWatcher().removeChild("logger/shouldOutputErrorBackTrace"));
