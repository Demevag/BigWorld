...
void waitForRestarting()
{
    ...
    if (off != s_commandLine.npos) {
        DWORD  pid = (DWORD)_wtoi64(s_commandLine.c_str() + off +
                                   s_commandLineToken.size());
...
void startNewInstance()
{
    ...
    while (GetModuleFileName(NULL, &exeName[0], (DWORD)exeName.size()) + 1 >
           exeName.size()) {
    ...
    CreateProcess(
      NULL, &commandLine[0], NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi);
}
