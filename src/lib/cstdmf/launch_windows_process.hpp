 ...
 *
 *	@param pHandlesOut an optional structure used to store the handles of the
 *process created if not NULL, stdOut & stdErr are piped to handles within this
 *structure that can be read by passOnProcessOutput later on.
 *
 *  @param bCreatePipes if true creates some io pipes to pipe the stOut & StdErr
 *from the launched process to the parents stdOut & stdErr.
 *
 *	@return the process exit code
 */
CSTDMF_DLL DWORD launchProcess(const BW::string& process,
