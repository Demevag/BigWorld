...
namespace BW {
                            ...
                            int    threadID /*= 0*/)
    {
        ...
        if (threadID == 0 || OurThreadID() == (unsigned long)threadID) {
            ...
            stackStringBuilder.appendf("Thread #%d trace:", threadID);
        }
        ...
        if (stacktraceSymbolResolutionAvailable()) {
            ...
            int                  stackSize =
              stacktraceAccurate(callStack, 256, skipFrames, threadID);
            ...
            for (int i = 0; i < stackSize; i++) {
                ...
                convertAddressToFunction(callStack[i],
                                         nameBuf,
                                         bufferSize,
                                         fileBuf,
                                         bufferSize,
                                         &lineNo);
                stackStringBuilder.appendf(
                  "%s(%d) : %s", fileBuf, lineNo, nameBuf);
                stackStringBuilder.append(BW_NEW_LINE);
            }
