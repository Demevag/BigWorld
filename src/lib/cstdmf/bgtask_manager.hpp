...
class BackgroundTask : public SafeReferenceCount
{
  public:
    ...
    /**
     *	This method is called to perform a task in the background thread.
     *Derived classes will often add themselves back to the manager at the end
     *of this method by calling <code>mgr.addMainThreadTask( this )</code>. This
     *allows this object to complete in the main thread.
     */
...
class CStyleBackgroundTask : public BackgroundTask
{
  public:
    ...
    CSTDMF_DLL CStyleBackgroundTask(const char* name,
                                    FUNC_PTR    bgFunc,
                                    void*       bgArg,
                                    FUNC_PTR    fgFunc = NULL,
                                    void*       fgArg  = NULL);

    CSTDMF_DLL CStyleBackgroundTask(FUNC_PTR bgFunc,
                                    void*    bgArg,
                                    FUNC_PTR fgFunc = NULL,
                                    void*    fgArg  = NULL);
...
{
  public:
    BackgroundTaskThread(TaskManager& mgr, BackgroundThreadDataPtr pData);

    BackgroundTaskThread(TaskManager&            mgr,
 ...
 *	by default. Tasks which override stop() must also call
 *BackgroundTask::stop().
 *
...
class TaskManager
{
  public:
    ...
    CSTDMF_DLL virtual bool initWatchers(const char* path,
                                         float       warnThreshold = 0);
    ...
    void startThreads(int numThreads, StartThreadCallback* startThreadCallback)
    {
        this->startThreads("Unknown BG Task",
                           numThreads,
    ...
    {
        this->startThreads(threadNamePrefix,
                           numThreads,
                           new StartThreadCallbackWrapper(startThreadCallback));
    }
    ...
    CSTDMF_DLL void addBackgroundTask(BackgroundTaskPtr pTask,
     ...
     *	that have not told the main thread that they have stopped. This is
     *always no less than numUnstoppedThreads.
     */
    ...
    /**
     *	This method is called to see if we're currently running inside a
     *cancelled task. Any long-running code may call it. It should be very
     *quick, but may block or suspend the thread if the BgTaskManager for the
     *current task so wishes.
     *
...
//------------------------------------------------------------------------------
class BgTaskManager
  : public InitSingleton<BgTaskManager>
  , public TaskManager
{
  public:
    friend class BackgroundTaskThread;

    CSTDMF_DLL BgTaskManager(bool exclusive = true)
      : TaskManager(exclusive){};
...
class FileIOTaskManager
  : public InitSingleton<FileIOTaskManager>
  , public TaskManager
{
  public:
    friend class BackgroundTaskThread;

    CSTDMF_DLL FileIOTaskManager(bool exclusive = true)
      : TaskManager(exclusive){};
