...
CSTDMF_DLL TimeStamp ProfileVal::s_warningPeriod_ =
...
WatcherPtr ProfileVal::pSummaryWatcher()
{
    ...
    if (pWatcher == NULL) {
        ...
        pWatcher = new DataWatcher<ProfileVal>(*pNull, Watcher::WT_READ_WRITE);
    }
...
WatcherPtr ProfileVal::pWatcherStamps()
{
    ...
    if (pWatcher == NULL) {
        ...
        pWatcher->addChild("sumIntTime",
                           makeWatcher((uint64&)pNull->sumIntTime_));
        pWatcher->addChild("lastIntTime",
                           makeWatcher((uint64&)pNull->lastIntTime_));
    }
...
{
    TimeStamp lastTime =
      val.inProgress_ ? TimeStamp(timestamp() - val.lastTime_) : val.lastTime_;
    ...
    if (val.sumQuantity_ != 0) {
        s << "Quantity: " << val.sumQuantity_ << " (last " << val.lastQuantity_
          << ", avg " << ((double)val.sumQuantity_) / val.count_ << ')'
          << std::endl;
    }

    s << "Time: " << NiceTime(val.sumTime_) << " (last " << NiceTime(lastTime)
      << ')' << std::endl;
    ...
    if (val.sumTime_ != 0) {
        ...
        if (val.sumQuantity_) {
            s << "Time/Qty: " << NiceTime(val.sumTime_ / val.sumQuantity_)
              << std::endl;
        }
    }
...
std::ostream& operator<<(std::ostream& o, const NiceTime& nt)
{
    ...
    double truncatedStampDouble =
      (double)(int64)(nt.t_ - ((uint64)(((double)(int64)micros) /
                                        microsecondsPerStamp)));
    uint32 picos =
      (uint32)(truncatedStampDouble * 1000000.0 * microsecondsPerStamp);
    ...
    { // Only display this precision if <1min. Just silly otherwise.
        bw_snprintf(
          lastBit, sizeof(lastBit), ".%03d", (int)((picos / 1000) % 1000));
        o << lastBit;
    }
