...
void LogMsg::doWrite(const char* pFormat, va_list argPtr)
{
    ...
    if ((g_shouldWriteToSyslog) && ((priority_ == MESSAGE_PRIORITY_ERROR) ||
    ...
    if (!handled && shouldAccept) {
        ...
        if (priority_ == MESSAGE_PRIORITY_ERROR &&
            ...
            vdprintf(
              newFormatStringBuilder.string(), argPtr, priority_, pCategory_);
        } else
 ...
 *
 */
void LogMsg::messageBackTrace(StringBuilder* pBuilder, bool shouldAddToMetaData)
{
    BW_GUARD;
    ...
    for (uint32 i = 0; i < depth; i++) {
        ...
        if (gotFunctionName) {
            ...
            char*  demangledBuffer       = abi::__cxa_demangle(
              mangled.c_str(), 0, &demangledBufferLength, &status);
        ...
        stackBuilder.appendf("#%d %s\n",
                             i,
                             (gotFunctionName) ? functionName.c_str()
                                               : traceString.c_str());
    }
                                      ...
                                      va_list        argPtr)
{
    ...
    strftime(timeBuffer,
             ARRAY_SIZE(timeBuffer) - 1,
             "%m.%d.%Y at %H:%M:%S" BW_NEW_LINE BW_NEW_LINE
             "Message:" BW_NEW_LINE,
             timeinfo);
...
void LogMsg::win32StackTraceThreads(StringBuilder& builder)
{
    ...
    const size_t     frozenThreadsCount =
      freezeAllThreads(frozenThreads, MAX_FROZEN_THREADS);
    ...
    builder.appendf("Memory status:" BW_NEW_LINE
                    "System: %lld/%lld [%3.2f%% used]" BW_NEW_LINE,
                    Memory::usedMemory(),
                    Memory::maxAvailableMemory(),
                    Memory::memoryLoad());
}
 ...
 *	                     written to.
 */
/*static*/ void LogMsg::automatedTest(bool isTest, const char* pDebugLogPath)
{
...
void LogMsg::xbox360Stack(const char stackTrace[])
{
    ...
    MultiByteToWideChar(
      CP_UTF8, 0, tcbuffer, -1, wcbuffer, ARRAYSIZE(wcbuffer));
 ...
 */
/*static*/ void LogMsg::formatMessage(char* /*out*/ pMsgBuffer,
                                      size_t        lenMsgBuffer,
                                      const char*   pFormat,
                                      va_list       argPtr)
{
...
/*static*/ void LogMsg::createMiniDump()
{
    ...
    if (pMiniDumpWriteDump(GetCurrentProcess(),
        ...
        bw_snprintf(outputMessage,
                    ARRAY_SIZE(outputMessage),
