...
namespace {
    ...
    {
      public:
        ScopedReentryCounter() { this->incrementCounter(); }

        ~ScopedReentryCounter() { this->decrementCounter(); }
  ...
  , clipHeight_(0.05f)
{
    ...
    for (int i = 0; i < bufferSize; i++) {
        ...
        msgs_[i].component_ = new TextGUIComponent(
          FontManager::instance().getCachedFont("default_small.font"));
        msgs_[i].backing_ =
          new SimpleGUIComponent("resources/maps/gui/background.dds");
...
{
    if (!ScopedReentryCounter::checkReentryCounter(msg))
        return;
...
void ClosedCaptions::update(float dTime)
{
    if (font_) {
        ...
        for (uint i = 0; i < msgs_.size(); i++) {
            ...
            c.position(Vector3(-1.f + margin_.x,
                               -1.f + margin_.y + ((float)i * clipHeight_),
                               0.9f));

            SimpleGUIComponent& b = *msgs_[idx].backing_;
            b.height(clipHeight_);
            b.position(Vector3(-1.f + margin_.x,
                               -1.f + margin_.y + ((float)i * clipHeight_),
                               1.f));
        }
    }
