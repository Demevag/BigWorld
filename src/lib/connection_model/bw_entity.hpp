  ...
  , public SafeReferenceCount
{
  public:
     ...
     *	server.
     */
    virtual void onProperty(int            messageID,
                            BinaryIStream& data,
     ...
     *	the server.
     */
    virtual void onNestedProperty(BinaryIStream& data,
                                  bool           isSlice,
                                  bool           isInitialising) = 0;
    ...
    void onMoveFromServer(double             time,
                          const Position3D&  position,
                          EntityID           vehicleID,
                          SpaceID            spaceID,
                          const Direction3D& direction,
    ...
    BWENTITY_API void onMoveLocally(double             time,
                                    const Position3D&  position,
                                    EntityID           vehicleID,
                                    bool               is2DPosition,
                                    const Direction3D& direction);
    ...
    virtual EntityExtension* createExtension(
      EntityExtensionFactoryBase* pFactory)
    {
        return NULL;
    }
  ...
  private:
    ...
    void onMoveInternal(double             time,
                        const Position3D&  position,
                        EntityID           vehicleID,
                        SpaceID            spaceID,
                        const Direction3D& direction,
    ...
    void populateMoveDefaults(Position3D&  position,
                              EntityID&    vehicleID,
                              SpaceID&     spaceID,
                              Direction3D& direction,
