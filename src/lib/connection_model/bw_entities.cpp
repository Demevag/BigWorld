 ...
 *	Constructor.
 */
BWEntities::BWEntities(BWConnection& connection, BWEntityFactory& entityFactory)
                                        ...
                                        BinaryIStream& data)
{
    ...
    BWEntityPtr pEntity = entityFactory_.create(
      id, entityTypeID, NULL_SPACE_ID, data, &connection_);
 ...
 */
void BWEntities::handleCellPlayerCreate(EntityID          id,
                                        SpaceID           spaceID,
                                        EntityID          vehicleID,
                                    ...
                                    BinaryIStream&    data)
{
    ...
    BWEntityPtr pEntity =
      entityFactory_.create(id, entityTypeID, spaceID, data, &connection_);
    ...
    this->addEntity(pEntity.get(), spaceID, vehicleID, position, direction);
}
                           ...
                           const Direction3D& direction)
{
    ...
    pEntity->onMoveFromServer(connection_.lastMessageTime(),
                              position,
                              vehicleID,
                              spaceID,
                              direction,
                                      ...
                                      PassengersVector* pFormerPassengers)
{
    ...
    if (pFormerPassengers) {
        pFormerPassengers->reserve(pEntity->passengers().size() +
                                   pFormerPassengers->size());
    }
    ...
    while (!pEntity->passengers().empty()) {
        ...
        if (pFormerPassengers) {
            pFormerPassengers->push_back(std::make_pair(pPassenger, entityID));
        }

        MF_VERIFY(activeEntities_.removeEntityFromWorld(pPassenger->entityID(),
                                                        pFormerPassengers));
 ...
 */
void BWEntities::handleRestoreClient(EntityID           entityID,
                                     SpaceID            spaceID,
                                     EntityID           vehicleID,
                                     const Position3D&  pos,
                                     ...
                                     BinaryIStream&     data)
{
    ...
    if (!pPlayer_->isControlled()) {
        ...
        pPlayer_->onMoveFromServer(connection_.lastMessageTime(),
                                   pos,
                                   vehicleID,
                                   spaceID,
                                   dir,
    ...
    if (pLostException != NULL) {
        ...
        pPlayer_->setSpaceVehiclePositionAndDirection(
          spaceID, vehicleID, pos, dir);
                                        ...
                                        BinaryIStream&     data)
{
    ...
    BWEntityPtr pNewEntity =
      entityFactory_.create(newID, entityTypeID, spaceID, data, &connection_);
    ...
    this->addEntity(pNewEntity.get(), spaceID, vehicleID, position, direction);
...
void BWEntities::tick(double gameTime, double deltaTime)
{
    ...
    for (BW::list<BWEntity*>::iterator iEntityLostVehicle =
        ...
        vPassengers.push_back(std::make_pair(pEntity, pEntity->vehicleID()));
 ...
 */
void BWEntities::handleEntityMethod(EntityID       entityID,
                                    int            methodID,
                                    BinaryIStream& data)
{
    BWEntityPtr pEntity = this->findAny(entityID);
 ...
 */
void BWEntities::handleEntityProperties(EntityID entityID, BinaryIStream& data)
{
    BWEntityPtr pEntity = this->findAny(entityID);
 ...
 */
void BWEntities::handleEntityProperty(EntityID       entityID,
                                      int            propertyID,
                                      BinaryIStream& data)
{
    BWEntityPtr pEntity = this->findAny(entityID);
                                           ...
                                           bool              isVolatile)
{
    ...
    pEntity->onMoveFromServer(connection_.lastMessageTime(),
                              ...
                              vehicleID,
                              spaceID,
                              Direction3D(Vector3(roll, pitch, yaw)),
                              posError,
                              isTeleport);
...
class BWEntitiesWatcher : public Watcher
{
  public:
    ...
    {
        if (isEmptyPath(path)) {
            const Map& useMap = *(Map*)(((uintptr)&toWatch_) + ((uintptr)base));
