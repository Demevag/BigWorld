...
class WatcherPacketHandler : public WatcherPathRequestNotification
{
  ...
  private:
    static const BW::string
      v1ErrorIdentifier; //!< Error prefix used in v1 watcher responses when
                         //!< responses will exceed UDP packet size.
    static const BW::string
      v1ErrorPacketLimit; //!< Error message used in v1 watcher responses when
                          //!< responses will exceed UDP packet size.

    bool canDelete_; //!< Set to true when we aren't initiating path requests.

    WatcherEndpoint
      watcherEndpoint_; //!< The endpoint we should communicate through.
    ...
    int32 outgoingRequests_; //!< The number of separate watcher path requests
                             //!< we have initiated.
    int32 answeredRequests_; //!< The number of path requests that have replied
                             //!< to us.
    ...
    PathRequestList pathRequestList_; //!< List of all path requests belonging
                                      //!< to the current watcher query.

    int maxPacketSize_; //!< The maximum size of packet we should fill with
