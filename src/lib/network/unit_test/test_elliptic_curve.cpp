...
-----END PUBLIC KEY-----";
    ...
    const char PUBLIC_KEY_OTHER[] = "-----BEGIN PUBLIC KEY-----\n\
...
TEST(ECDSA_Sign)
{
...

    {
        ...
        privateKey =
          BWOpenSSL::PEM_read_bio_ECPrivateKey(pem, NULL, NULL, NULL);
    ...
    { // Compute SHA256 digest
        ...
        BWOpenSSL::EVP_DigestUpdate(digestContext, MESSAGE, sizeof(MESSAGE));
        CHECK(BWOpenSSL::EVP_DigestFinal(
          digestContext, digestBuffer, &digestLength));
    }

    { // Sign the digest
        signatureLength = BWOpenSSL::ECDSA_size(privateKey);
        signatureBuffer = new unsigned char[signatureLength];
        CHECK(BWOpenSSL::ECDSA_sign(0,
                                    digestBuffer,
                                    digestLength,
                                    signatureBuffer,
                                    &signatureLength,
    ...
    { // Read the public key
        ...
        publicKey = BWOpenSSL::PEM_read_bio_EC_PUBKEY(pem, NULL, NULL, NULL);
    ...
    // Verify the signature
    {
        ...
        CHECK(BWOpenSSL::ECDSA_verify(0,
                                      digestBuffer,
                                      digestLength,
                                      signatureBuffer,
                                      signatureLength,
    ...
    {
        unsigned char* BAD_SIGNATURE = new unsigned char[signatureLength];

        BWOpenSSL::RAND_bytes(BAD_SIGNATURE, signatureLength);

        CHECK(1 != BWOpenSSL::ECDSA_verify(0,
                                           digestBuffer,
                                           digestLength,
                                           BAD_SIGNATURE,
                                           signatureLength,
    ...
    // Fail to verify correct signature, but wrong key
    {
        ...
        wrongKey = BWOpenSSL::PEM_read_bio_EC_PUBKEY(pem, NULL, NULL, NULL);

        CHECK(wrongKey);

        CHECK(0 == BWOpenSSL::ECDSA_verify(0,
                                           digestBuffer,
                                           digestLength,
                                           signatureBuffer,
                                           signatureLength,
...
TEST(EllipticCurveChecksumScheme)
{
    ...
    ChecksumSchemePtr pScheme = ChainedChecksumScheme::create(
      ...
      EllipticCurveChecksumScheme::create(PRIVATE_KEY, /* isPrivate */ true));
    CHECK(pScheme);
