...
class FloodServerApp : public NetworkApp
{
  ...
  public:
    FloodServerApp(Mercury::EventDispatcher& mainDispatcher,
                   TEST_ARGS_PROTO,
...
FloodServerApp::FloodServerApp(Mercury::EventDispatcher& mainDispatcher,
                               ...
                               unsigned      payloadSizeBytes,
                               unsigned long maxRunTimeMicros)
  : NetworkApp(mainDispatcher, Mercury::NETWORK_INTERFACE_INTERNAL, TEST_ARGS)
  , channels_()
  , channelCounts_()
  , msg1Count_(0)
...
int FloodServerApp::run()
{
    ...
    INFO_MSG(
      "FloodServerApp(%d)::run: finished (#msg1 = %d)\n", getpid(), msg1Count_);
    return 0;
}
...
void FloodServerApp::handleTimeout(TimerHandle timerHandle, void* /*arg*/)
{
    ...
    while (iter != channels_.end()) {
        ...
        bundle.startRequest(
          FloodClientInterface::msg2, &s_emptyReplyHandler, NULL);
                          ...
                          BinaryIStream& data)
{
    ...
    const char* buf = reinterpret_cast<const char*>(data.retrieve(lenBytes));
    ...
    ASSERT_WITH_MESSAGE(iChannel != channels_.end(), "Channel not found");
    ...
    while ((unsigned)(pChar - buf) < lenBytes) {
        ASSERT_WITH_MESSAGE(*(pChar++) == 'a', "Client gave us invalid data");
    }
 ...
 */
template <class ARGS>
class ServerFloodStructMessageHandler : public Mercury::InputMessageHandler
{
  public:
...
{
  public:
    FloodClientApp(Mercury::EventDispatcher& mainDispatcher,
                   TEST_ARGS_PROTO,
...
void FloodClientApp::connect()
{
    ...
    FloodServerInterface::connectArgs& args =
      FloodServerInterface::connectArgs::start(bundle, Mercury::RELIABLE_NO);
    ...
    if (!pChannel_) {
        pChannel_ = new Mercury::UDPChannel(
          this->networkInterface(), serverAddr_, Mercury::UDPChannel::EXTERNAL);
    }
}
...
MEMTRACKER_DECLARE(TestFlood_testFlood, "TestFlood_testFlood", 0);
...
TEST(TestFlood_testFlood)
{
    ...
    const float maxRunTimeSeconds =
      NUM_ITERATIONS * (TICK_PERIOD / 1000000.) * 3.0;
    ...
    FloodServerApp serverApp(mainDispatcher,
                             TEST_ARGS,
                             PAYLOAD_SIZE,
    ...
    for (unsigned i = 0; i < numChildren; i++) {
        FloodClientApp* pApp =
          new FloodClientApp(mainDispatcher,
                             TEST_ARGS,
    ...
    ASSERT_WITH_MESSAGE(serverApp.getMsg1Count() >=
                          numChildren * NUM_ITERATIONS,
