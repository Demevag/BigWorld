...
namespace {
    ...
    // The number of seconds the test should run before it is considered a
    // failure with a timeout.
 ...
 */
Mercury::Reason MangleFilter::recv(
  Mercury::PacketReceiver&           receiver,
  const Mercury::Address&            addr,
  Mercury::Packet*                   pPacket,
  Mercury::ProcessSocketStatsHelper* pStatsHelper)
...
{
  public:
    MangleServerApp(Mercury::EventDispatcher& mainDispatcher, TEST_ARGS_PROTO)
    ...
    bool hasRunningClients() const { return numClients_ > 0; }
 ...
 */
template <class ARGS>
class ServerMangleStructMessageHandler : public Mercury::InputMessageHandler
{
  public:
...
class MangleClientApp : public NetworkApp
{
  public:
    MangleClientApp(
      ...
      Mercury::UDPChannel::Traits channelTraits = Mercury::UDPChannel::INTERNAL,
 ...
 */
template <class ARGS>
class ClientMangleStructMessageHandler : public Mercury::InputMessageHandler
{
  public:
...
void MangleClientApp::sendConnect()
{
    ...
    MangleServerInterface::connectArgs& args =
      MangleServerInterface::connectArgs::start(bundle, Mercury::RELIABLE_NO);

    args.traits = pChannel_->isExternal() ? Mercury::UDPChannel::EXTERNAL
 ...
 */
void MangleClientApp::ackConnect(
...
TEST_F(TestMangleFixture, mangledChannel)
{
    ...
    MangleClientApp clientApp(mainDispatcher,
                              TEST_ARGS,
    ...
    float corruptedPercent =
      100 * float(serverReceiveStats.numCorruptedPacketsReceived()) /
