...
namespace {
      ...
      , public TimerHandler
    {
      ...
      protected:
        ...
        void recreateChannel(TimerHandle           handle,
                             Mercury::UDPChannel** ppChannel)
        {
            if (g_numSent == NUM_SENDS) {
                // If we don't stop recreating the channels, resends don't get
                // processed because the last send time of unacked packets is
                // reset each time we initFromStream.
                return;
            }
...
TEST(Channel_stream)
{
    ...
    pFromChannel = new Mercury::UDPChannel(fromInterface,
                                           toInterface.address(),
                                           Mercury::UDPChannel::INTERNAL,
                                           1.f,
                                           NULL,
    ...
    pToChannel = new Mercury::UDPChannel(toInterface,
                                         fromInterface.address(),
                                         Mercury::UDPChannel::INTERNAL,
                                         1.f,
                                         NULL,
                                         1);
