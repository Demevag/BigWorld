    ...
    class ReplyHandler
    {
      public:
        ...
        virtual bool onHighPrecisionMachineMessage(
          ...
          uint32                       addr);
        virtual bool onWholeMachineMessage(WholeMachineMessage& wmm,
                                           uint32               addr);
        ...
        virtual bool onMachinedAnnounceMessage(MachinedAnnounceMessage& mam,
                                               uint32                   addr);
        virtual bool onQueryInterfaceMessage(QueryInterfaceMessage& wmm,
                                             uint32                 addr);
        virtual bool onCreateWithArgsMessage(CreateWithArgsMessage& cwam,
                                             uint32                 addr);
        virtual bool onMachinePlatformMessage(MachinePlatformMessage& mpm,
                                              uint32                  addr);
    };

    Mercury::Reason sendAndRecv(Endpoint&     ep,
                                uint32        destaddr,
                                ReplyHandler* pHandler = NULL);

    Mercury::Reason sendAndRecv(uint32        srcip,
                                uint32        destaddr,
                                ReplyHandler* pHandler = NULL);

    Mercury::Reason sendAndRecvFromEndpointAddr(Endpoint&     ep,
                                                uint32        destaddr,
                                                ReplyHandler* pHandler = NULL);
...
class ListenerMessage : public ProcessMessage
{
  public:
    ...
    ListenerMessage() { message_ = MachineGuardMessage::LISTENER_MESSAGE; }
...
class CreateWithArgsMessage : public CreateMessage
{
  public:
    ...
    CreateWithArgsMessage()
      : CreateMessage(CREATE_WITH_ARGS_MESSAGE)
    {
    }
