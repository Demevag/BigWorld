...
ForwardingStringHandler::ForwardingStringHandler(const char* fmt,
                                                 bool        isSuppressible)
  : fmt_(fmt)
  , numRecentCalls_(0)
...
void ForwardingStringHandler::onToken(char  type,
                                      int   cflags,
                                      int   min,
                                      int   max,
                                      int   flags,
    ...
    for (unsigned i = 0; i < fmtData_.size(); i++) {
        ...
        switch (fd.type_) {
            case 'd': {
                switch (fd.cflags_) {
                    case DP_C_SHORT:
                        os << (short)va_arg(args, int);
                        break;
                    case DP_C_LONG:
#if MESSAGE_LOGGER_VERSION < 7
                        os << (int32)va_arg(args, long);
                        break;
#else
                        os << (int64)va_arg(args, long);
                        break;
#endif
                    case DP_C_LLONG:
                        os << (int64)va_arg(args, long long);
                        break;
                    default:
                        os << (int32)va_arg(args, int);
                        break;
                }
                break;
            }
            ...
            case 'x': {
                switch (fd.cflags_) {
                    case DP_C_SHORT:
                        os << (unsigned short)va_arg(args, unsigned int);
                        break;
                    case DP_C_LONG:
#if MESSAGE_LOGGER_VERSION < 7
                        os << (uint32)va_arg(args, unsigned long);
                        break;
#else
                        os << (uint64)va_arg(args, unsigned long);
                        break;
#endif
                    case DP_C_LLONG:
                        os << (uint64)va_arg(args, unsigned long long);
                        break;
                    default:
                        os << (uint32)va_arg(args, unsigned int);
                        break;
                }
                break;
            }
            ...
            case 'g': {
                ...
                switch (fd.cflags_) {
                    case DP_C_LDOUBLE:
                        os << va_arg(args, LDOUBLE);
                        break;
                    default:
                        os << va_arg(args, double);
                        break;
                }
                break;
            }
