...
namespace Mercury {
    ...
    {
        PacketPtr pPacket =
          Packet::createFromStream(data, Packet::UNACKED_SEND);
     ...
     */
    void UDPChannel::UnackedPacket::addToStream(UnackedPacket* pInstance,
                                                BinaryOStream& data)
    {
        if (pInstance) {
            Packet::addToStream(
              data, pInstance->pPacket_.get(), Packet::UNACKED_SEND);
