...
namespace Mercury {
    ...
    /**
     *	This method updates the statics associated with the PacketReceiver's
     *socket.
     */
    void PacketReceiverStats::updateSocketStats(const Endpoint& socket)
    {
        ...
        {
            double elapsedTime =
              (currTime - lastGatherTime_) / stampsPerSecondD();
        ...
        if ((lastTxQueueSize_ > maxTxQueueSize_) &&
            (lastTxQueueSize_ > 128 * 1024)) {
            WARNING_MSG("Transmit queue peaked at new max (%d bytes)\n",
                        lastTxQueueSize_);
        }

        if ((lastRxQueueSize_ > maxRxQueueSize_) &&
            (lastRxQueueSize_ > 1024 * 1024)) {
    ...
    WatcherPtr PacketReceiverStats::pWatcher()
    {
        ...
        StatWatcherCreator::addWatchers(
          pWatcher, "bytesReceived", pNull->numBytesReceived_);
        StatWatcherCreator::addWatchers(
          pWatcher, "packetsReceived", pNull->numPacketsReceived_);
        StatWatcherCreator::addWatchers(pWatcher,
                                        "duplicatePacketsReceived",
                                        pNull->numDuplicatePacketsReceived_);
        StatWatcherCreator::addWatchers(pWatcher,
                                        "packetsReceivedOffChannel",
                                        pNull->numPacketsReceivedOffChannel_);
        StatWatcherCreator::addWatchers(
          pWatcher, "bundlesReceived", pNull->numBundlesReceived_);
        StatWatcherCreator::addWatchers(
          pWatcher, "messagesReceived", pNull->numMessagesReceived_);
        StatWatcherCreator::addWatchers(
          pWatcher, "overheadBytesReceived", pNull->numOverheadBytesReceived_);
        StatWatcherCreator::addWatchers(pWatcher,
                                        "corruptedPacketsReceived",
                                        pNull->numCorruptedPacketsReceived_);
        StatWatcherCreator::addWatchers(pWatcher,
                                        "corruptedBundlesReceived",
                                        pNull->numCorruptedBundlesReceived_);
        ...
        pWatcher->addChild("timing/mercury",
                           ProfileVal::pWatcherStamps(),
                           &pNull->mercuryTimer_);
        pWatcher->addChild(
          "timing/system", ProfileVal::pWatcherStamps(), &pNull->systemTimer_);

        pWatcher->addChild("timingInSeconds/mercury",
                           ProfileVal::pWatcherSeconds(),
                           &pNull->mercuryTimer_);
        pWatcher->addChild("timingInSeconds/system",
                           ProfileVal::pWatcherSeconds(),
                           &pNull->systemTimer_);

        pWatcher->addChild("timingSummary/mercury",
                           ProfileVal::pSummaryWatcher(),
                           &pNull->mercuryTimer_);
        pWatcher->addChild("timingSummary/system",
                           ProfileVal::pSummaryWatcher(),
                           &pNull->systemTimer_);
