...
namespace Mercury {
                                              ...
                                              int         maxWaitMicroSeconds)
    {
        ...
        // Since this channel might not be doing any sending while we're waiting
        // for the reply, we need to mark it as irregular temporarily to ensure
        // ACKs are sent until we're done.
        if (isRegularChannel) {
        ...
        if (maxWaitMicroSeconds > 0) {
            timerHandle_ = dispatcher.addTimer(
              maxWaitMicroSeconds, this, NULL, "BlockingReplyHandler");
        }
    ...
    /**
     *	This method handles the max timer going off. If this is called, we have
     *not received the response in the required time.
     */
     ...
     */
    void BlockingReplyHandler::handleMessage(const Address&         addr,
     ...
     */
    void BlockingReplyHandler::handleException(const NubException& ne,
                                               void*               arg)
    {
