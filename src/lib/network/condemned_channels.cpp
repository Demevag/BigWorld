...
namespace Mercury {
    ...
    void CondemnedChannels::add(UDPChannel* pChannel)
    {
        if (this->shouldDelete(pChannel, timestamp())) {
            ...
            if (!timerHandle_.isSet()) {
                ...
                timerHandle_ =
                  pChannel->networkInterface().dispatcher().addTimer(
                    int(seconds * 1000000), this, NULL, "CondemnedChannels");
            }
        }
    }
    ...
    {
        IndexedChannels::const_iterator iter = indexedChannels_.find(channelID);
                                                ...
                                                uint64      now) const
    {
        ...
        if (!shouldDelete && (now - pChannel->lastReceivedTime() > ageLimit) &&
    ...
    /**
     *	This method deletes any condemned channels that are now considered
     *finished. This can be from having no more unacked packets or timing out
     *
    ...
    int CondemnedChannels::numCriticalChannels() const
    {
        ...
        for (NonIndexedChannels::const_iterator iter =
               nonIndexedChannels_.begin();
    ...
    /**
     *	This method handles the timer event. It checks whether any condemned
