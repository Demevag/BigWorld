...
namespace Mercury {
    ...
    void OnceOffReceiver::init(EventDispatcher& dispatcher)
    {
        ...
        clearFragmentedBundlesTimerHandle_ =
          dispatcher.addTimer(FragmentedBundle::MAX_AGE * 1000000,
                              this,
                              NULL,
     ...
     *	needs to be done because we may receive this packet again but we should
    ...
    void OnceOffReceiver::initOnceOffPacketCleaning(EventDispatcher& dispatcher)
    {
        if (!onceOffPacketCleaningTimerHandle_.isSet()) {
            ...
            onceOffPacketCleaningTimerHandle_ = dispatcher.addTimer(
              onceOffPacketCleaningPeriod, this, NULL, "OnceOffPacketCleaning");
        }
    }
     ...
     *	information and another for the less recent information. This method
     *clears the less recent information and demotes the recent information to
     *be less recent.
     */
     ...
     *	Constructor.
     */
    OnceOffPacket::OnceOffPacket(const Address& addr,
                                              ...
                                              EventDispatcher& dispatcher)
    {
        ...
        OnceOffPackets::iterator actualIter =
          onceOffPackets_.insert(temp).first;
        OnceOffPacket* pOnceOffPacket =
          const_cast<OnceOffPacket*>(&(*actualIter));

        pOnceOffPacket->registerTimer(
          dispatcher, packetSender, onceOffResendPeriod_);
    ...
    /**
     *	This method removes the resend timer associated with a packet sent
     *once-off reliably. It should be called when receipt of the packet has been
     *	acknowledged.
     */
    ...
    /**
     *	This method cancels the timer that is responsible for resending a
     *packet.
     */
                                                 ...
                                                 PacketSender&  packetSender)
    {
        if (++oop.retries_ <= onceOffMaxResends_) {
            ...
            if (packetSender.rescheduleSend(
                  oop.addr_, oop.pPacket_.get(), NULL)) {
                return;
            }
    ...
    {
        // Iterate through all the unacked once-off sends and remove those going
        // to the dead address.
        OnceOffPackets::iterator iter       = onceOffPackets_.begin();
        int                      numRemoved = 0;
