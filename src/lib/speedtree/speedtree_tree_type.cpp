...
namespace speedtree {
    ...
    //-- Stores the current wind information. It will later be used to animate
    ...
    void TSpeedTreeType::WindAnimation::update()
    {
        ...
        if (m_lastTickTime != TSpeedTreeType::s_time_) {
            ...
            //-- setup SpeedTree's wind handler with our current wind direction
            // and strength.
            Vector3 wind(windVelX, 0, windVelZ);
            float strength = std::min(wind.length() / c_maxWindVelocity, 1.0f);
            wind.normalise();
            m_speedWind->SetWindStrengthAndDirection(
              strength, wind.x, wind.y, wind.z);

            //-- update SpeedTree's wind handler based on the current camera
            // position.
            float camDir[3], camPos[3];
            CSpeedTreeRT::GetCamera(camPos, camDir);
            m_speedWind->Advance(TSpeedTreeType::s_time_,
                                 true,
                                 true,
                                 camDir[0],
                                 camDir[1],
            ...
            if (m_hasLeaves) {
                ...
                bool success = (m_speedWind->GetRockAngles(windRockAngles) &&
                ...
                if (success) {
                    ...
                    for (uint i = 0; i < m_leafAnglesCount; ++i) {
                        m_anglesTable[i * 4 + 0] =
                          DEG_TO_RAD(windRockAngles[i]);
                        m_anglesTable[i * 4 + 1] =
                          DEG_TO_RAD(windRustleAngles[i]);
                    }
                }
            }
     ...
     *
     *	@param	speedTree		the speedtree object that will use this
     *speedwind.
     *	@param	datapath		data path where to look for the tree specific
     *setup.
     *	@param	defaultWindIni	default ini file to use if tree specific not
     *found.
     */
    //----------------------------------------------------------------------------------------------
    ...
    //----------------------------------------------------------------------------------------------
    void TSpeedTreeType::collectTreeDrawData(DrawData& drawData,
                                             ...
                                             bool      semiTransparent)
    {
        ...
        //-- collect all needed data for the later efficient rendering.
        {
            ...
            //-- process branches.
            {
                ...
                bool readyToDraw =
                  TSpeedTreeType::s_drawBranches_ && lod.branchDraw_ &&
                ...
                if (readyToDraw) {
                    ...
                    drawDataCollector_.m_branches[lod.branchLod_].push_back(
                      inst);
                }
            }
            ...
            {
                //-- check all needed states to be sure we really ready to draw.
                bool readyToDraw = TSpeedTreeType::s_drawFronds_ &&
                                   lod.frondDraw_ &&
            ...
            {
                //-- check all needed states to be sure we really ready to draw.
                bool readyToDraw = TSpeedTreeType::s_drawLeaves_ &&
                                   lod.leafDraw_ && lod.leafLodCount_ > 0 &&
                                   !treeData_.leaves_.lod_.empty();

                if (readyToDraw) {
                    for (int i = 0; i < lod.leafLodCount_; ++i) {
                        int lodID = lod.leafLods_[i];
                        if (lodID < G_MAX_LOD_COUNT &&
                            treeData_.leaves_.lod_[lodID].index_) {
                            inst.m_materialID =
            ...
            {
                //-- check all needed states to be sure we really ready to draw.
                bool readyToDraw = TSpeedTreeType::s_drawBillboards_ &&
                                   collectBillboards && lod.billboardDraw_ &&
                ...
                if (readyToDraw) {
                    ...
                    inst.m_alphaRef =
                      1.0f + fadeValue * (minAlpha / maxAlpha - 1.0f);
    ...
    void TSpeedTreeType::DrawDataCollector::sort()
    {
        ...
        struct Sorter
        {
            Sorter(const Instances& instances, const Vector3& camPos)
              : m_instances(instances)
              , m_camPos(camPos)
            {
            }

            bool operator()(const DrawData::Instance& lft,
                            const DrawData::Instance& rht) const
            {
        ...
        for (uint i = 0; i < G_MAX_LOD_COUNT; ++i) {
            std::sort(m_branches[i].begin(),
                      m_branches[i].end(),
                      Sorter(m_branches[i], camPos));
            std::sort(m_fronds[i].begin(),
                      m_fronds[i].end(),
                      Sorter(m_fronds[i], camPos));
            std::sort(m_leaves[i].begin(),
                      m_leaves[i].end(),
                      Sorter(m_leaves[i], camPos));
        }
        std::sort(m_billboards.begin(),
                  m_billboards.end(),
                  Sorter(m_billboards, camPos));
    }
    ...
    {
        DataSectionPtr pDS =
          BWResource::instance().openSection("speedtree/bushes.xml");
        if (!pDS)
            return;
