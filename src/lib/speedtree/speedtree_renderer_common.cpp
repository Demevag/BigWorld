...
namespace {
    ...
    const uint32 g_instanceSizeInBytes =
      sizeof(speedtree::TSpeedTreeType::DrawData::PackedGPU);
    ...
    //----------------------------------------------------------------------------------------------
    const char* const g_vertDclrs[] = { "speedtree_branch",
                                        "speedtree_branch",
                                        "speedtree_leaf",
                                        "speedtree_billboard" };

    //-- Warning: names should be in sync with ConstantsMap::EConstant
    // enumeration.
    //----------------------------------------------------------------------------------------------
    const char* const g_constNames[] = {
        "g_instance",         "alphaTestEnable",  "g_texturedTrees",
        "g_diffuseMap",       "g_normalMap",      "g_useNormalMap",
        "g_material",         "g_cullEnabled",    "g_leafLightAdj",
        "g_leafAngleScalars", "g_leafRockFar",    "g_windMatrices",
        "g_leafAngles",       "g_useHighQuality", "g_useZPrePass"
    };
}
//--------------------------------------------------------------------------------------------------
...
namespace speedtree {
    ...
    SpeedTreeSettings::SpeedTreeSettings(const DataSectionPtr& cfg)
    {
        ...
        m_qualitySettings =
          Moo::makeCallbackGraphicsSetting("SPEEDTREE_QUALITY",
                                           "Speedtree Quality",
                                           *this,
        ...
        m_qualitySettings->addOption(
          "MAX", "Max", Moo::rc().psVersion() >= 0x300, true);
        ...
        m_options.push_back(
          Option(true, cfg->readFloat("lods/MAX", 1.0f), 7.0f)); //-- MAX
        m_options.push_back(
          Option(true, cfg->readFloat("lods/HIGH", 0.8f), 6.0f)); //-- HIGH
        m_options.push_back(
          Option(false, cfg->readFloat("lods/MEDIUM", 0.6f), 5.0f)); //-- MEDIUM
        m_options.push_back(
          Option(false, cfg->readFloat("lods/LOW", 0.4f), 4.0f)); //-- LOW
    ...
    bool ConstantsMap::getHandles(const Moo::EffectMaterialPtr& effect)
    {
        ...
        //-- It is ok if some parameters returns invalid handle because we use
        // the same shaders map
    ...
    //----------------------------------------------------------------------------------------------
    SpeedTreeRendererCommon::SpeedTreeRendererCommon(const DataSectionPtr& cfg)
      : m_settings(cfg)
    {
        BW_GUARD;
        ...
        //-- Note: we have to invoke these methods implicitly here because
        // SpeedTreeRenderer is
        //--       called after render context creation therefore these method
        // won't be called
        //--       automatically.
    ...
    //----------------------------------------------------------------------------------------------
    bool SpeedTreeRendererCommon::pass(Moo::ERenderingPassType type,
                                       bool                    instanced)
    {
        BW_GUARD;

        //-- Check pipeline capabilities.
        instanced = (Renderer::instance().pipelineType() ==
        ...
        for (uint i = 0; i < TREE_PART_COUNT; ++i) {
            ...
            ds.m_material->pEffect()->pEffect()->SetBool(
              ds.m_constantsMap->handle(ConstantsMap::CONST_USE_HIGH_QUALITY),
              m_settings.activeOption().m_highQuality);
            ...
            for (uint j = 0; j < SPECIAL_DRAWING_STATE_COUNT; ++j) {
                ...
                ads.m_material->pEffect()->pEffect()->SetBool(
                  ads.m_constantsMap->handle(
                    ConstantsMap::CONST_USE_HIGH_QUALITY),
                  m_settings.activeOption().m_highQuality);
            }
        }
    ...
    void SpeedTreeRendererCommon::createUnmanagedObjects()
    {
        ...
        if (Renderer::instance().pipelineType() ==
            ...
            //-- ToDo: reconsider memory usage. Currently is 80 * 3000 * 3 ->
            // 720kb -> 0.72Mb
            //-- 80 bytes per instance, maximum 3000 instances per frame (for
            // all passes not only main
            //-- color pass but for shadows passes and reflection pass), and x3
            // multiplier to be sure
            //-- we won't start rewriting data if CPU will be 3 frames ahead
            // over GPU (that is default
            //-- behavior for almost every commodity graphics drivers, when GPU
            // is really busy CPU doesn't
            //-- wait while GPU will done its work, it just generate data in
            // advance with hope that
            //-- GPU has been busy for a short time and on the next frame it
            // will done both frames,
            //-- previous and current).
            for (uint i = 0; i < TREE_PART_COUNT; ++i) {
                success &= SUCCEEDED(
                  m_instVBs[i].create(g_maxInstBufferSizeInBytes,
    ...
    void SpeedTreeRendererCommon::createManagedObjects()
    {
        ...
        for (uint i = 0; i < TREE_PART_COUNT; ++i) {
            if (createMaterials) {
                ...
                success &= m_materials[i].second.getHandles(
                  m_materials[i].first->baseMaterial());

                //-- create special material.
                for (uint j = 0; j < SPECIAL_DRAWING_STATE_COUNT; ++j) {
                    m_specialMaterials[j][i].first = new Moo::EffectMaterial();
                    success &= m_specialMaterials[j][i].first->initFromEffect(
                      g_specialShader);
                    m_specialMaterials[j][i].first->hTechnique(
                      g_specialShaderTechniques[j][i]);
                    success &= m_specialMaterials[j][i].second.getHandles(
                      m_specialMaterials[j][i].first);
                }
            }
        ...
        Moo::VertexDeclaration* instancing =
          Moo::VertexDeclaration::get(g_instancingVertDclr);
        success &= (instancing != NULL);

        //-- create instancing versions.
        if (success) {
            for (uint i = 0; i < TREE_PART_COUNT; ++i) {
                m_vertDclrs[i][1] = Moo::VertexDeclaration::combine(
                  m_vertDclrs[i][0], instancing);
        ...
        if (Renderer::instance().pipelineType() ==
            IRendererPipeline::TYPE_DEFERRED_SHADING) {
            //-- disable billboard optimizer in deferred shading use instancing
            // instead.
