...
bool SkinSplitter::createList(uint32 nodeLimit, BW::vector<uint32>& nodeList)
{
    ...
    while ((index = findAppropriateRelationship(nodeLimit, nodeList)) != -1) {
        ...
        for (uint i = 0; i < r.size(); i++) {
            if (std::find(nodeList.begin(), nodeList.end(), r[i]) ==
                nodeList.end())
 ...
 */
void SkinSplitter::splitTriangles(TriangleVector&           triangles,
...
{
    BW_GUARD;
    // create a index list for the nodes, true means the node is included in the
    // list, false means not included.
...
/*
 *	This method tries to find a relationship that can be combined with the
 *current nodelist and not make the nodelist go over nodeLimit. It tries to find
 *the most suitable relationship to combine with ie the one that will increase
 *bonecount the least. param nodeLimit the max number of nodes in the nodelist
 *	param nodeList the current list of nodes
 *	return the index of the most appropriate relationship to combine with or -1
 *if none found.
 */
 ...
 *	pVHolder the vertices
 */
void SkinSplitter::addRelationship(const BloatVertex& v1,
                                   const BloatVertex& v2,
 ...
 */
int SkinSplitter::findRelationship(const BoneRelationship& relationship,
                                   bool                    perfectMatch)
{
    BW_GUARD;
    ...
    for (uint i = 0; i < relationships_.size(); i++) {
        if (relationships_[i].size() == relationship.size() || !perfectMatch) {
            // compare each element of the relationship to see if it can be
            // found
 ...
 */
void SkinSplitter::addRelationship(BoneRelationship&  relationship,
                                   const BloatVertex& v)
{
    BW_GUARD;
    for (size_t i = 0; i < 4; ++i) {
        if (v.weights[i] > 0.f) {
            if (std::find(relationship.begin(),
                          relationship.end(),
