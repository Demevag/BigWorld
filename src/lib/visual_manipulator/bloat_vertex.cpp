...
bool BloatVertex::operator==(const BloatVertex& bv) const
{
    return this->pos == bv.pos && this->normal == bv.normal &&
           this->uv == bv.uv && this->vertexIndex == bv.vertexIndex &&
           this->colour == bv.colour && this->uv2 == bv.uv2 &&
           this->smoothingGroup == bv.smoothingGroup &&
           this->binormal == bv.binormal && this->tangent == bv.tangent &&
           this->meshIndex == bv.meshIndex && weights == bv.weights &&
           indices[0] == bv.indices[0] && indices[1] == bv.indices[1] &&
           indices[2] == bv.indices[2] && indices[3] == bv.indices[3];
};
...
bool BloatVertex::operator<(const BloatVertex& bv) const
{
    ...
    if (weights < bv.weights)
        return true;
    if (weights > bv.weights)
        return false;

    for (uint32 i = 0; i < 4; i++) {
        if (indices[i] < bv.indices[i])
            return true;
        if (indices[i] > bv.indices[i])
            return false;
    }
