...
#define IMEID_CHT_VER52                                                        \
    (LANG_CHT |                                                                \
     MAKEIMEVERSION(                                                           \
       5, 2)) // New(Phonetic/ChanJie)IME5.2 : 5.2.x.x // IME2002a(w/WinXP)
#define IMEID_CHT_VER60                                                        \
    (LANG_CHT | MAKEIMEVERSION(6, 0)) // New(Phonetic/ChanJie)IME6.0 : 6.0.x.x
                                      // // New IME 6.0(web download)
...
static LONG(WINAPI* _ImmGetCompositionString)(HIMC   himc,
                                              DWORD  dwIndex,
                                              LPVOID lpBuf,
                                              DWORD  dwBufLen);
...
static DWORD(WINAPI* _ImmGetCandidateList)(HIMC            himc,
                                           DWORD           deIndex,
...
static UINT(WINAPI* _GetReadingString)(HIMC   himc,
                                       UINT   uReadingBufLen,
                                       LPWSTR lpwReadingBuf,
                                       PINT   pnErrorIndex,
                                       BOOL*  pfIsVertical,
                                       PUINT  puMaxReadingLen);
static BOOL(WINAPI* _ShowReadingWindow)(HIMC himc, BOOL bShow);

// Callbacks
void(CALLBACK* ImeUiCallback_DrawRect)(int   x1,
                                       int   y1,
                                       int   x2,
                                       int   y2,
                                       DWORD color);
...
static LRESULT(WINAPI* _SendMessage)(HWND   hwnd,
                                     UINT   msg,
                                     WPARAM wp,
                                     LPARAM lp) = SendMessageA;
static DWORD (*_GetCandidateList)(HIMC             himc,
                                  DWORD            dwIndex,
...
static TCHAR
  g_szReadingString[32];      // Used only in case of horizontal reading window
...
static LPTSTR g_aszIndicator[] = {
    TEXT("A"),
#ifdef UNICODE
    L"\x7B80", L"\x7E41", L"\xac00", L"\x3042",
#else
    "\xd6\xd0", "\xa4\xa4", "\xb0\xa1", "\x82\xa0",
#endif
};
...
static bool IsUILessMode()
{
    ...
    //			 way of doing things...
    return g_bUILessMode && g_hklCurrent != _CHS_HKL_GOOGLE &&
  ...
  protected:
      ...
      , public ITfCompartmentEventSink
    {
      public:
        ...
        // ITfUIElementSink
        //   Notifications for Reading Window events. We could process candidate
        //   as well, but we'll use IMM for simplicity sake.
        ...
        STDMETHODIMP OnActivated(DWORD    dwProfileType,
                                 LANGID   langid,
                                 REFCLSID clsid,
                                 REFGUID  catid,
                                 REFGUID  guidProfile,
    ...
    static void MakeReadingInformationString(
...
static void AW_SendCompString()
{
    ...
    iLen = WideCharToMultiByte(g_uCodePage,
                               0,
                               g_szCompositionString,
                               -1,
                               (LPSTR)szCompStr,
                               COUNTOF(szCompStr),
                               NULL,
                               NULL) -
           1; // don't need to send NUL terminator;
    for (i = 0; i < iLen; i++) {
...
// They can be deleted if the game doesn't supports them (i.e. games requires
// Win98 SE or later).
static DWORD AW_GetCandidateList(HIMC             himc,
                                 DWORD            dwIndex,
                                 LPCANDIDATELIST* ppCandList)
{
    ...
    if (dwBufLen) {
        LPCANDIDATELIST pCandList =
          (LPCANDIDATELIST)ImeUiCallback_Malloc(dwBufLen);
        if (pCandList) {
            ...
            if (dwBufLen) {
                ...
                for (i = 0; i < (int)pCandList->dwCount; i++) {
                    wideBufLen += MultiByteToWideChar(g_uCodePage,
                                                      0,
                                                      (LPSTR)pCandList +
                                                        pCandList->dwOffset[i],
                                                      -1,
                                                      NULL,
                                                      0) *
                                  sizeof(WCHAR);
                ...
                *ppCandList = (LPCANDIDATELIST)ImeUiCallback_Malloc(wideBufLen);
                ...
                LPWSTR pwz =
                  (LPWSTR)((LPSTR)pCandListW + pCandList->dwOffset[0]);
                for (i = 0; i < (int)pCandList->dwCount; i++) {
                    pCandListW->dwOffset[i] = (LPSTR)pwz - (LPSTR)pCandListW;
                    pwz += MultiByteToWideChar(g_uCodePage,
                                               0,
                                               (LPSTR)pCandList +
                                                 pCandList->dwOffset[i],
                                               -1,
...
static LONG WINAPI AW_ImmGetCompositionString(HIMC   himc,
                                              DWORD  dwIndex,
                                              LPVOID lpBuf,
                                              DWORD  dwBufLen)
{
    ...
    switch (dwIndex) {
        ...
        case GCS_COMPSTR:
            if (dwRet) {
                pszMb[dwRet] = 0;
                dwRet        = (DWORD)MultiByteToWideChar(
                  g_uCodePage, 0, pszMb, -1, (LPWSTR)lpBuf, dwBufLen);
                if (dwRet) {
                    // Note that ImmGetCompositionString() returns number of
                    // bytes copied, regardless of the width of character.
        ...
        case GCS_COMPATTR: {
            ...
            DWORD dwRet2 = ImmGetCompositionStringA(
              himc, GCS_COMPSTR, pszMb2, sizeof(pszMb2));
            if (!dwRet2) {
                dwRet2 = ImmGetCompositionStringA(
                  himc, GCS_RESULTSTR, pszMb2, sizeof(pszMb2));
                if (!dwRet2) {
                    return 0;
                }
            }
...
static int GetCharCountFromBytes(LPCSTR pszString, int iBytes)
{
    ...
    if (i != iBytes)
        iCount = -iCount; // indicate error - iBytes specifies wrong boundary
                          // (i.e. the last byte is leadbyte)
    return iCount;
}
...
static DWORD WA_GetCandidateList(HIMC             himc,
                                 DWORD            dwIndex,
                                 LPCANDIDATELIST* ppCandList)
{
    ...
    if (dwBufLen) {
        ...
        if (pCandList) {
            ...
            if (dwBufLen) {
                ...
                int mbBufLen = 0;
                for (i = 0; i < (int)pCandList->dwCount; i++) {
                    mbBufLen += WideCharToMultiByte(
                      g_uCodePage,
                      0,
                      (LPWSTR)((LPSTR)pCandList + pCandList->dwOffset[i]),
                      -1,
                      NULL,
                      0,
                      NULL,
...
static LONG WINAPI WA_ImmGetCompositionString(HIMC   himc,
                                              DWORD  dwIndex,
                                              LPVOID lpBuf,
                                              DWORD  dwBufLen)
{
    ...
    switch (dwIndex) {
        ...
        case GCS_COMPSTR:
            if (dwRet) {
                ...
                dwRet = (DWORD)WideCharToMultiByte(g_uCodePage,
                                                   0,
                                                   pwzUc,
                                                   -1,
                                                   (LPSTR)lpBuf,
        ...
        case GCS_CURSORPOS: {
            ...
            DWORD dwRet2 = ImmGetCompositionStringW(
              himc, GCS_COMPSTR, pwzUc2, sizeof(pwzUc2));
            if (!dwRet2) {
                dwRet2 = ImmGetCompositionStringW(
                  himc, GCS_RESULTSTR, pwzUc2, sizeof(pwzUc2));
                if (!dwRet2) {
                    return 0;
                }
            }
            dwRet2 /= 2;
            // The return value of WideCharToMultiByte() should probably be
            // checked/asserted for success. bounds violation (overflow)
            // 'pszMb[iRc]'
            const int bufSize = COUNTOF(g_szCompositionString) * 2;
            char      pszMb[bufSize];
            int       iRc = WideCharToMultiByte(
              g_uCodePage, 0, pwzUc2, dwRet2, pszMb, sizeof(pszMb), NULL, NULL);
            assert(iRc > 0); // WideCharToMultiByte returns 0 if it failed, and
                             // it should *never* be negative in the first place
            if (iRc >= bufSize) // if we wrote more bytes than the length of the
                                // buffer, we need to terminate it
            {
        ...
        case GCS_COMPATTR: {
            ...
            for (char* psz = pszMb; *psz; psz++, pSrc++) {
                *pOut++ = *pSrc; // copy attribute
                ...
                // buffer overrun protection, pOut is incremented in the loop,
                // but not part	of the loop	invariant test.	To make	the	code
                // more readable we have a test rather than rolling this	into
                // the for stmt.
...
static void ComposeCandidateLine(int index, LPCTSTR pszCandidate)
{
    ...
    // Thomas Cowell: We don't want to stick the candidate number into the
    // candidate string here.
    ...
    while (*pszCandidate &&
           (COUNTOF(g_szCandidate[index]) > (psz - g_szCandidate[index]))) {
        *psz++ = *pszCandidate++;
    }
    *psz = 0;
}
...
static void SendCompString()
{
    ...
    if (ImeUiCallback_OnChar) {
        ...
        pwz  = szUnicode;
        iLen = MultiByteToWideChar(g_uCodePage,
                                   0,
                                   g_szCompositionString,
                                   -1,
                                   szUnicode,
...
static DWORD GetCandidateList(HIMC             himc,
                              DWORD            dwIndex,
                              LPCANDIDATELIST* ppCandList)
{
...
static void CancelCompString(HWND hwnd,
                             bool bUseBackSpace = true,
                             int  iNewStrLen    = 0)
{
    if (g_dwIMELevel != 3)
        return;
    ...
    // till it's same long as the new one. Then move caret to the beginning of
    // comp string. New comp string will overwrite old one.
    if (iNewStrLen < cc) {
...
static void DrawCaret(DWORD x, DWORD y, DWORD height)
{
    if (g_bCaretDraw && ImeUiCallback_DrawRect)
        ImeUiCallback_DrawRect(x,
                               y + gSkinIME.caretYMargin,
                               x + gSkinIME.caretWidth,
                               y + height - gSkinIME.caretYMargin,
                               g_CaretInfo.colorComp);
}
...
static void DrawImeIndicator()
{
    ...
    switch (gSkinIME.symbolPlacement) {
        case 0: // vertical centering IME indicator
        {
            if (SizeOfPie + g_CaretInfo.margins.right + 4 > g_screenWidth) {
                ...
                PieData[0].sy =
                  (float)g_CaretInfo.margins.top +
                  (g_CaretInfo.margins.bottom - g_CaretInfo.margins.top) / 2;
            } else {
                PieData[0].sx = -(SizeOfPie / 2) + g_CaretInfo.margins.right +
                                gSkinIME.symbolHeight + 4;
                PieData[0].sy =
                  (float)g_CaretInfo.margins.top +
                  (g_CaretInfo.margins.bottom - g_CaretInfo.margins.top) / 2;
            }
    ...
    if (!gSkinIME.symbolColor && bOn) {
        {
            PieData[1].color =
              0xff0000 + (((DWORD)gSkinIME.symbolTranslucence) << 24);
            PieData[2].color =
              0xff3000 + (((DWORD)gSkinIME.symbolTranslucence) << 24);
            PieData[3].color =
              0xff6000 + (((DWORD)gSkinIME.symbolTranslucence) << 24);
            PieData[4].color =
              0xff9000 + (((DWORD)gSkinIME.symbolTranslucence) << 24);
            PieData[5].color =
              0xffC000 + (((DWORD)gSkinIME.symbolTranslucence) << 24);
            PieData[6].color =
              0xffff00 + (((DWORD)gSkinIME.symbolTranslucence) << 24);
            PieData[7].color =
              0xC0ff00 + (((DWORD)gSkinIME.symbolTranslucence) << 24);
            PieData[8].color =
              0x90ff00 + (((DWORD)gSkinIME.symbolTranslucence) << 24);
            PieData[9].color =
              0x60ff00 + (((DWORD)gSkinIME.symbolTranslucence) << 24);
            PieData[10].color =
              0x30c0ff + (((DWORD)gSkinIME.symbolTranslucence) << 24);
            PieData[11].color =
              0x00a0ff + (((DWORD)gSkinIME.symbolTranslucence) << 24);
            PieData[12].color =
              0x3090ff + (((DWORD)gSkinIME.symbolTranslucence) << 24);
            PieData[13].color =
              0x6060ff + (((DWORD)gSkinIME.symbolTranslucence) << 24);
            PieData[14].color =
              0x9030ff + (((DWORD)gSkinIME.symbolTranslucence) << 24);
            PieData[15].color =
              0xc000ff + (((DWORD)gSkinIME.symbolTranslucence) << 24);
        }
    } else {
        DWORD dwColor = bOn ? gSkinIME.symbolColor : gSkinIME.symbolColorOff;
        for (int t1 = 1; t1 < 16; t1++) {
            PieData[t1].color =
              dwColor + (((DWORD)gSkinIME.symbolTranslucence) << 24);
        }
    }
    ...
    // fix for Ent Gen #120 - reduce the height of character when Korean IME is
    // on
    ...
    if (gSkinIME.symbolFont) {
#ifdef DS2
        // save the font height here since DS2 shares editbox font and indicator
        // font
        DWORD _w, _h;
        g_CaretInfo.pFont->GetTextExtent(TEXT(" "), &_w, &_h);
#endif // DS2

        // GOS deals height in points that is 1/72nd inch and assumes display
        // device is 96dpi.
        fHeight = fHeight * 96 / 72;
        gSkinIME.symbolFont->SetHeight((UINT)fHeight);
        gSkinIME.symbolFont->SetColor(
        ...
        DWORD   w, h;
        LPCTSTR cszSymbol =
          (g_dwState == IMEUI_STATE_ON) ? g_pszIndicatior : g_aszIndicator[0];

        gSkinIME.symbolFont->GetTextExtent(cszSymbol, &w, &h);
        gSkinIME.symbolFont->SetPosition((int)(PieData[0].sx) - w / 2,
                                         (int)(PieData[0].sy) - h / 2);
        ...
        // Double-check: Confirm match by testing a range of font heights to
        // find best fit
        DWORD _h2;
...
static void DrawCompositionString(bool bDrawCompAttr)
{
    ...
    if (g_dwIMELevel == 3 && g_IMECursorBytes && g_szCompositionString[0]) {
        // shift starting point of drawing composition string according to the
        // current caret position.
        TCHAR temp = g_szCompositionString[g_IMECursorBytes];
        g_szCompositionString[g_IMECursorBytes] = 0;
        g_CaretInfo.pFont->GetTextExtent(
          g_szCompositionString, &wCompChar, &hCompChar);
    ...
    if (GETPRIMLANG() != LANG_KOREAN ||
        g_bCaretDraw) // Korean uses composition attribute as blinking block
                      // caret
        for (i = 0; i < len; i += cType) {
            ...
            if (cType == 2 &&
                g_szCompositionString[i + 1]) // in case we have 0 in trailbyte,
                                              // we don't count it.
            ...
            g_CaretInfo.pFont->GetTextExtent(
              pszCurrentCompLine, &bgXnext, &hCompChar);
            ...
            if (g_dwIMELevel == 3 && bDrawCompAttr) {
                if ((LONG)bgX >= g_CaretInfo.margins.left &&
                    (LONG)bgX <= g_CaretInfo.margins.right) {
                    if (g_dwImeUiFlags & IMEUI_FLAG_SUPPORT_CARET) {
                        if (ImeUiCallback_DrawRect)
                            ImeUiCallback_DrawRect(bgX,
                                                   bgY,
                                                   bgX + wCompChar,
                                                   bgY + hCompChar,
                                                   bgColor);
                        ...
                        if (ImeUiCallback_DrawRect)
                            ImeUiCallback_DrawRect(bgX - wCompChar,
                                                   bgY,
                                                   bgX,
                                                   bgY + hCompChar,
                                                   bgColor);
                ...
                // make sure enough buffer space (possible space, NUL for
                // current line, possible DBCS, 2 more NUL) are available in
                // multiline composition string buffer
                bool bWrite =
                ...
                if ((LONG)(bgX + wCompChar) >= g_CaretInfo.margins.right) {
                    ...
                    if (bWrite) {
                        if (pszMlcs == g_szMultiLineCompString ||
                            pszMlcs[-1] == 0)
                ...
                if (ImeUiCallback_DrawRect)
                    ImeUiCallback_DrawRect(
                      bgX, bgY, bgX + wCompChar, bgY + hCompChar, bgColor);
                if (bWrite) {
            ...
            if ((saveCandPos && candX == POSITION_UNINITIALIZED) ||
                (IMEID_LANG(GetImeId()) == LANG_CHS &&
                 i / (3 - sizeof(TCHAR)) == (int)g_IMECursorChars)) {
                candX = bgX;
                candY = bgY;
            }
            saveCandPos = false;
    ...
    bgX = bgXnext;
    if (g_dwIMELevel == 2) {
        // in case the caret in composition string is at the end of it, draw it
        // here
        ...
        //			sizeof(g_szMultiLineCompString) /
        // sizeof(g_szMultiLineCompString[0]) - 2);
        *pszMlcs++ = 0;
        *pszMlcs++ = 0;
        DWORD x, y;
        ...
        pszMlcs = g_szMultiLineCompString;
        while (*pszMlcs && pszMlcs - g_szMultiLineCompString <
                             sizeof(g_szMultiLineCompString) /
    ...
    if (g_dwCaretX != POSITION_UNINITIALIZED &&
        g_dwCaretY != POSITION_UNINITIALIZED) {
...
static void DrawCandidateList()
{
    ...
    if (candX == POSITION_UNINITIALIZED) {
        // CHT IME in Vista doesn't have ATTR_TARGET_CONVERTED attribute while
        // typing, so display the candidate list near the caret in the
        // composition string
        if (GETLANG() == LANG_CHT && GetImeId() != 0 &&
            g_dwCaretX != POSITION_UNINITIALIZED) {
            candX = g_dwCaretX;
            candY = g_dwCaretY;
        } else {
    ...
    if (g_bReadingWindow && g_bHorizontalReading)
        g_CaretInfo.pFont->GetTextExtent(
          g_szReadingString, (DWORD*)&largest.cx, (DWORD*)&largest.cy);
    else {
        for (i = 0; g_szCandidate[i][0] && i < (int)g_uCandPageSize; i++) {
            ...
            if (g_bReadingWindow)
                ...
                if (g_bVerticalCand)
                    g_CaretInfo.pFont->GetTextExtent(
                      g_szCandidate[i] + 2, &tx, &ty);
                else
                    g_CaretInfo.pFont->GetTextExtent(
                      g_szCandidate[i] + 1, &tx, &ty);
    ...
    // Show candidate list above composition string if there isn't enough room
    // below.
    DWORD dwCandHeight;
    ...
    if (ImeUiCallback_DrawRect)
        ImeUiCallback_DrawRect(
          left, top, right, bottom, gSkinIME.candColorBorder);
    left++;
    top++;
    right--;
    bottom--;
    if (g_bReadingWindow || IMEID_LANG(GetImeId()) == LANG_CHS) {
        if (ImeUiCallback_DrawRect)
            ImeUiCallback_DrawRect(
              left, top, right, bottom, gSkinIME.candColorBase);
        ...
        if (!g_bReadingWindow) {
            ...
            if (ImeUiCallback_DrawRect) {
                ImeUiCallback_DrawRect(
                  left, top, seperateLineX - 1, bottom, gSkinIME.candColorBase);
                ImeUiCallback_DrawRect(
                  seperateLineX, top, right, bottom, gSkinIME.candColorBase);
            }
        }
    } else {
        for (i = 0; (DWORD)i < slotsUsed; i++) {
            if (ImeUiCallback_DrawRect)
                ImeUiCallback_DrawRect(
                  left, top, left + largest.cx, bottom, gSkinIME.candColorBase);
            left += largest.cx + 1;
        }
    }
    ...
    if (g_bReadingWindow && g_bHorizontalReading) {
        ...
        if (g_iReadingError >= 0) {
            ...
            *psz = 0;
            g_CaretInfo.pFont->GetTextExtent(
              szTemp, (DWORD*)&iEnd, (DWORD*)&iDummy);
            TCHAR cSave             = szTemp[g_iReadingError];
            szTemp[g_iReadingError] = 0;
            g_CaretInfo.pFont->GetTextExtent(
              szTemp, (DWORD*)&iStart, (DWORD*)&iDummy);
            szTemp[g_iReadingError] = cSave;
            if (ImeUiCallback_DrawRect)
                ImeUiCallback_DrawRect(candX + iStart,
                                       candY,
                                       candX + iEnd,
                                       candY + largest.cy,
        ...
        if (iStart >= 0) {
            ...
            if (gSkinIME.candColorBase != 0xffffffff ||
                gSkinIME.candColorBorder != 0xff000000)
                g_CaretInfo.pFont->SetColor(g_CaretInfo.colorCand);
            else
                g_CaretInfo.pFont->SetColor(
                  0xff000000 + (~((0x00ffffff) & g_CaretInfo.colorCand)));
            ...
            if (g_dwSelection == (DWORD)i) {
                ...
                if (ImeUiCallback_DrawRect) {
                    if (g_bReadingWindow || g_bVerticalCand)
                        ImeUiCallback_DrawRect(candX,
                                               candY + i * largest.cy,
                                               candX - 1 + dwCandWidth,
                                               candY + (i + 1) * largest.cy,
                                               gSkinIME.candColorBorder);
                    else
                        ImeUiCallback_DrawRect(candX,
                                               candY,
                                               candX + adwCandWidth[i],
                                               candY + largest.cy,
                                               gSkinIME.candColorBorder);
                }
            } else
...
//
LPARAM ImeUi_ProcessMessage(HWND    hWnd,
                            UINT    uMsg,
                            WPARAM  wParam,
                            LPARAM& lParam,
                            bool*   trapped)
    ...
    *trapped = false;
    ...
    switch (uMsg) {
        ...
        case WM_INPUTLANGCHANGE:
            BW_IME_DEBUG_MSG(
              "WM_INPUTLANGCHANGE (HKL=%x, wParam=%x, lParam=%x)\n",
              g_hklCurrent,
              wParam,
              lParam);
            OnInputLangChange();
        ...
        case WM_IME_SETCONTEXT:
            //
            // We don't want anything to display, so we have to clear lParam and
            // pass it to DefWindowProc(). Expecially important in Vista to
            // receive IMN_CHANGECANDIDATE correctly.
            //
            BW_IME_DEBUG_MSG(
              "WM_IME_SETCONTEXT (HKL=%x, wParam=%x, lParam=%x)\n",
              g_hklCurrent,
              wParam,
              lParam);
            lParam = 0;
        ...
        case WM_IME_STARTCOMPOSITION:
            BW_IME_DEBUG_MSG(
              "WM_IME_STARTCOMPOSITION (HKL=%x, wParam=%x, lParam=%x)\n",
              g_hklCurrent,
              wParam,
              lParam);
            InitCompStringData();
        ...
        case WM_IME_COMPOSITION: {
            BW_IME_DEBUG_MSG(
              "WM_IME_COMPOSITION (HKL=%x, wParam=%x, lParam=%x)\n",
              g_hklCurrent,
              wParam,
              lParam);
            LONG  lRet;
            ...
            *trapped = true;
            ...
            if (lParam & GCS_RESULTSTR) {
                lRet = (LONG)_ImmGetCompositionString(
                         himc, GCS_RESULTSTR, szCompStr, COUNTOF(szCompStr)) /
                       sizeof(TCHAR);
                szCompStr[lRet] = 0;
                CancelCompString(g_hwndCurr, false, GetCharCount(szCompStr));
                wcscpy_s(g_szCompositionString,
                         COUNTOF(g_szCompositionString),
                         szCompStr);
                _SendCompString();
                InitCompStringData();
            }
            ...
            if (lParam & GCS_COMPSTR) {
                ...
                lRet = (LONG)_ImmGetCompositionString(
                         himc, GCS_COMPSTR, szCompStr, COUNTOF(szCompStr)) /
                       sizeof(TCHAR);
                szCompStr[lRet] = 0;
                //
                ...
                wcscpy_s(g_szCompositionString,
                         COUNTOF(g_szCompositionString),
                         szCompStr);
                lRet                     = _ImmGetCompositionString(himc,
                                                GCS_COMPATTR,
                                                g_szCompAttrString,
                ...
                if (GETLANG() == LANG_CHT && !GetImeId()) {
                    // int i, chars = lstrlen(g_szCompositionString) / (3 -
                    // sizeof(TCHAR));
                    if (g_szCompositionString[0] != '\0') {
                        // BigWorld: our presentation layer will cope, so just
                        // copy everything across.
                        wcscpy_s(g_szReadingString,
                                 COUNTOF(g_szReadingString),
                                 szCompStr);
                        memset(&g_szCompositionString,
                               0,
                               sizeof(g_szCompositionString));
                        g_bReadingWindow = true;
                        GetReadingWindowOrientation(0);
                        g_szCompositionString[0] =
                          TEXT(' '); // BigWorld: stick a dummy space in here.
                    } else
                        g_dwCount = 0;
                }
                ...
                g_IMECursorBytes =
                  _ImmGetCompositionString(himc, GCS_CURSORPOS, NULL, 0);
                g_IMECursorChars = GetCharCountFromBytes(g_szCompositionString,
                                                         g_IMECursorBytes);
                ...
                if (g_dwIMELevel == 3) {
                    ...
                    len =
                      GetCharCount(g_szCompositionString + g_IMECursorBytes);
        ...
        case WM_IME_ENDCOMPOSITION:
            BW_IME_DEBUG_MSG(
              "WM_IME_ENDCOMPOSITION (HKL=%x, wParam=%x, lParam=%x)\n",
              g_hklCurrent,
              wParam,
              lParam);
        ...
        case WM_IME_NOTIFY:
            switch (wParam) {
                case IMN_SETCONVERSIONMODE: {
                    BW_IME_DEBUG_MSG(
                      "IMN_SETCONVERSIONMODE (HKL=%x, wParam=%x, lParam=%x)\n",
                      g_hklCurrent,
                      wParam,
                      lParam);
                    ...
                    if (LANG_CHT == GETLANG() && !bNoReentrance) {
                        ...
                        _ImmGetConversionStatus( g_himcOrg, &dwConvMode,
                        &dwSentMode ); const DWORD dwFlag = IME_CMODE_SOFTKBD |
                        IME_CMODE_SYMBOL; if ( dwConvMode & dwFlag )
                            _ImmSetConversionStatus( g_himcOrg, dwConvMode &
                        ~dwFlag, dwSentMode );
                        */
                    }
                    bNoReentrance = false;
                ...
                case IMN_SETOPENSTATUS:
                    BW_IME_DEBUG_MSG("IMN_SETOPENSTATUS (bUILessMode=%d, "
                                     "HKL=%x, wParam=%x, lParam=%x)\n",
                                     IsUILessMode(),
                                     g_hklCurrent,
                                     wParam,
                                     lParam);
                ...
                case IMN_OPENCANDIDATE:
                    BW_IME_DEBUG_MSG("IMN_OPENCANDIDATE (bUILessMode=%d, "
                                     "HKL=%x, lCandidateList=%x)\n",
                                     IsUILessMode(),
                                     g_hklCurrent,
                                     lParam);
                case IMN_CHANGECANDIDATE:
                    ...
                    if (wParam != IMN_OPENCANDIDATE) {
                        BW_IME_DEBUG_MSG("IMN_CHANGECANDIDATE (bUILessMode=%d, "
                                         "HKL=%x, lCandidateList=%x)\n",
                                         IsUILessMode(),
                                         g_hklCurrent,
                                         lParam);
                    ...
                    }
                    {
                        ...
                        *trapped    = true;
                        ...
                        // BigWorld: Reading windows and candidate windows can
                        // co-exist?
                        // g_bReadingWindow = false;
                        dwIndex = 0;
                        dwBufLen =
                          _GetCandidateList(himc, dwIndex, &lpCandList);
                        ...
                        if (dwBufLen) {
                            ...
                            if (GETLANG() == LANG_CHS && GetImeId()) {
                                // MSPY (CHS IME) has variable number of
                                // candidates in candidate window find where
                                // current page starts, and the size of current
                                // page
                                const int maxCandChar =
                                  18 * (3 - sizeof(TCHAR));
                                UINT cChars = 0;
                                UINT i;
                                ...
                                // BigWorld: We've calculated the correct page
                                // size above, but now use the given offset to
                                // read out the candidate list strings.
                                startOfPage = lpCandList->dwPageStart;
                            } else {
                                g_uCandPageSize = min(lpCandList->dwPageSize,
                                                      DWORD(MAX_CANDLIST));
                                if (g_uCandPageSize >
                                    0) // BigWorld: avoid division by zero
                                {
                                    startOfPage =
                                      IsUILessMode()
                                        ? lpCandList->dwPageStart
                                        : (g_dwSelection / g_uCandPageSize) *
                                            g_uCandPageSize;
                                } else {
                                    startOfPage = 0;
                                }
                            ...
                            // don't display selection in candidate list in case
                            // of Korean and old Chinese IME.
                            if (GETPRIMLANG() == LANG_KOREAN ||
                ...
                case IMN_CLOSECANDIDATE:
                    BW_IME_DEBUG_MSG(
                      "IMN_CLOSECANDIDATE (bUILessMode=%d, HKL=%x)\n",
                      IsUILessMode(),
                      g_hklCurrent);
                    if (IsUILessMode()) {
                        break;
                ...
                case IMN_PRIVATE: {
                    // BigWorld: candidate lists and reading windows can
                    // coexist...?
                    // if ( !g_bCandList )
                    {
                    ...
                    BW_IME_DEBUG_MSG(
                      "IMN_PRIVATE (dwID=%x, HKL=%x, lParam=%x)\n",
                      dwId,
                      g_hklCurrent,
                      lParam);
                ...
                default:
                    BW_IME_DEBUG_MSG(
                      "Unhandled WM_NOTIFY (HKL=%x, wParam=%x, lParam=%x)\n",
                      g_hklCurrent,
                      wParam,
                      lParam);
                    // BigWorld: Setting this to true breaks some IME's such as
                    // Japanese on XP.
                    //*trapped = true;
                    break;
            }
        ...
        // fix for #15386 - When Text Service Framework is installed in Win2K,
        // Alt+Shift and Ctrl+Shift combination (to switch input locale /
        // keyboard layout) doesn't send WM_KEYUP message for the key that is
        // released first. We need to check if these keys are actually up
        // whenever we receive key up message for other keys.
        case WM_KEYUP:
        case WM_SYSKEYUP:
            if (!(lAlt & 0x80000000) && wParam != VK_MENU &&
                (GetAsyncKeyState(VK_MENU) & 0x8000) == 0) {
                PostMessageA(GetFocus(),
                             WM_KEYUP,
                             (WPARAM)VK_MENU,
                             (lAlt & 0x01ff0000) | 0xC0000001);
            } else if (!(lCtrl & 0x80000000) && wParam != VK_CONTROL &&
                       (GetAsyncKeyState(VK_CONTROL) & 0x8000) == 0) {
                PostMessageA(GetFocus(),
                             WM_KEYUP,
                             (WPARAM)VK_CONTROL,
                             (lCtrl & 0x01ff0000) | 0xC0000001);
            } else if (!(lShift & 0x80000000) && wParam != VK_SHIFT &&
                       (GetAsyncKeyState(VK_SHIFT) & 0x8000) == 0) {
                PostMessageA(GetFocus(),
                             WM_KEYUP,
                             (WPARAM)VK_SHIFT,
                             (lShift & 0x01ff0000) | 0xC0000001);
            }
...
void ImeUi_SetCompStringAppearance(CImeUiFont_Base* pFont,
                                   DWORD            color,
                                   const RECT*      prc)
{
    if (!g_bInitialized)
...
void ImeUi_SetState(DWORD dwState)
{
    ...
    if (NULL != (himc = _ImmGetContext(g_hwndCurr))) {
        ...
        switch (GETPRIMLANG()) {
            ...
            case LANG_KOREAN:
                // toggle Korean IME
                if ((bOn && g_dwState != IMEUI_STATE_ON) ||
                    (!bOn && g_dwState == IMEUI_STATE_ON)) {
...
#pragma warning(suppress : 4996)
    ...
    if (g_hImmDll) {
        _ImmLockIMC = (LPINPUTCONTEXT2(WINAPI*)(HIMC hIMC))GetProcAddress(
          g_hImmDll, "ImmLockIMC");
        _ImmUnlockIMC =
          (BOOL(WINAPI*)(HIMC hIMC))GetProcAddress(g_hImmDll, "ImmUnlockIMC");
        _ImmLockIMCC = (LPVOID(WINAPI*)(HIMCC hIMCC))GetProcAddress(
          g_hImmDll, "ImmLockIMCC");
        _ImmUnlockIMCC = (BOOL(WINAPI*)(HIMCC hIMCC))GetProcAddress(
          g_hImmDll, "ImmUnlockIMCC");
        BOOL(WINAPI * _ImmDisableTextFrameService)
        (DWORD) = (BOOL(WINAPI*)(DWORD))GetProcAddress(
          g_hImmDll, "ImmDisableTextFrameService");
    ...
    // There are the following combinations of code config, window type, and the
    // method of sending characters. Wnd: Unicode, Code: Unicode, Method:
    // SendMessageW (SendMessageW must be supported since RegisterClassW is
    // successful) Wnd: non Uni, Code: Unicode, Method: AW_SendCompString (Send
    // characters in multibyte after W->A conversion) Wnd: Unicode, Code: non
    // Uni, Method: SendMessageA (System does A->W conversion) - possible, but
    // unlikely to be used. Wnd: non Uni, Code: non Uni, Method: SendMessageA
#ifdef UNICODE
    ...
    // the following pointers to function has to be initialized before this
    // function is called.
    if (bDisable || !ImeUiCallback_Malloc || !ImeUiCallback_Free) {
        g_bDisableImeCompletely = true;
    ...
    // SetupTSFSinks has to be called before CheckToggleState to make it work
    // correctly.
    g_bUILessMode = CTsfUiLessMode::SetupSinks() != FALSE;
    CheckToggleState();
    if (g_bUILessMode) {
        g_bChineseIME = (GETPRIMLANG() == LANG_CHINESE) &&
...
//
//	Use IMEID_VER and IMEID_LANG macro to extract version and language
// information.
//
static DWORD GetImeId(UINT uIndex)
{
    ...
    hklPrev = kl;
    DWORD dwLang =
      (static_cast<DWORD>(reinterpret_cast<UINT_PTR>(kl)) & 0xffff);
    ...
    if (kl != _CHT_HKL_NEW_PHONETIC && kl != _CHT_HKL_NEW_CHANG_JIE &&
        kl != _CHT_HKL_NEW_QUICK && kl != _CHT_HKL_HK_CANTONESE &&
        kl != _CHS_HKL) {
        goto error;
    }
...
#define LCID_INVARIANT                                                         \
    MAKELCID(MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US), SORT_DEFAULT)
        if ((CompareStringA(LCID_INVARIANT,
                            NORM_IGNORECASE,
                            szTmp,
                            -1,
                            _CHT_IMEFILENAME,
                            -1) != 2) &&
            (CompareStringA(LCID_INVARIANT,
                            NORM_IGNORECASE,
                            szTmp,
                            -1,
                            _CHT_IMEFILENAME2,
    ...
    if (dwVerSize) {
        ...
        if (lpVerBuffer) {
            if (GetFileVersionInfoA(
                  szTmp, dwVerHandle, dwVerSize, lpVerBuffer)) {
...
#define pVerFixedInfo ((VS_FIXEDFILEINFO FAR*)lpVerData)
                    ...
                    dwVer = (dwVer & 0x00ff0000) << 8 | (dwVer & 0x000000ff)
                                                          << 16;
...
static void GetReadingString(HWND hWnd)
{
    ...
    DWORD dwlen = 0;
    DWORD dwerr = 0;
    WCHAR wzBuf[16]; // We believe 16 wchars are big enough to hold reading
                     // string after having discussion with CHT IME team.
    WCHAR*          wstr    = wzBuf;
    bool            unicode = FALSE;
    ...
    if (_GetReadingString) {
        BOOL bVertical;
        UINT uMaxUiLen;
        dwlen = _GetReadingString(
          himc, 0, NULL, (PINT)&dwerr, &bVertical, &uMaxUiLen);
        if (dwlen) {
            if (dwlen > COUNTOF(wzBuf)) {
                dwlen = COUNTOF(wzBuf);
            }
            dwlen = _GetReadingString(
              himc, dwlen, wstr, (PINT)&dwerr, &bVertical, &uMaxUiLen);
        }
    ...
    } else //	IMEs that doesn't implement Reading String API
    {
        ...
        switch (dwId) {
            ...
            case IMEID_CHT_VER42: // New(Phonetic/ChanJie)IME98  : 4.2.x.x //
                                  // Win98
            case IMEID_CHT_VER43: // New(Phonetic/ChanJie)IME98a : 4.3.x.x //
                                  // WinMe, Win2k
            case IMEID_CHT_VER44: // New ChanJie IME98b          : 4.4.x.x //
                                  // WinXP
            ...
            case IMEID_CHT_VER50: // 5.0.x.x // WinME
                ...
                p = *(LPBYTE*)((LPBYTE)_ImmLockIMCC(lpIMC->hPrivate) +
                               3 * 4); // PCKeyCtrlManager
                if (!p)
                    break;
                p = *(LPBYTE*)((LPBYTE)p + 1 * 4 + 5 * 4 +
                               4 * 2); // = PCReading = &STypingInfo
                if (!p)
                    break;
                dwlen   = *(DWORD*)(p + 1 * 4 + (16 * 2 + 2 * 4) + 5 * 4 +
                                  16); // m_dwDisplayStringLength;
                dwerr   = *(DWORD*)(p + 1 * 4 + (16 * 2 + 2 * 4) + 5 * 4 + 16 +
                                  1 * 4); // m_dwDisplayErrorStart;
            ...
            case IMEID_CHS_VER53: // 5.3.x.x // SCIME2k or MSPY3 (w/OfficeXP and
                ...
                if (!p)
                    break;
                p = *(LPBYTE*)((LPBYTE)p + 1 * 4 +
                               5 * 4); // = PCReading = &STypingInfo
                if (!p)
                    break;
                dwlen   = *(DWORD*)(p + 1 * 4 + (16 * 2 + 2 * 4) + 5 * 4 +
                                  16 * 2); // m_dwDisplayStringLength;
                dwerr   = *(DWORD*)(p + 1 * 4 + (16 * 2 + 2 * 4) + 5 * 4 +
                                  16 * 2 + 1 * 4); // m_dwDisplayErrorStart;
                wstr    = (WCHAR*)(p + 1 * 4 + (16 * 2 + 2 * 4) + 5 * 4);
                unicode = TRUE;
                break;
            ...
            case IMEID_CHS_VER42: // 4.2.x.x // SCIME98 or MSPY2 (w/Office2k,
                                  // Win2k, WinME, etc)
            {
                int nTcharSize = IsNT() ? sizeof(WCHAR) : sizeof(char);
                p = *(LPBYTE*)((LPBYTE)_ImmLockIMCC(lpIMC->hPrivate) + 1 * 4 +
                               1 * 4 + 6 * 4); // = PCReading = &STypintInfo
                if (!p)
                    break;
                dwlen = *(DWORD*)(p + 1 * 4 + (16 * 2 + 2 * 4) + 5 * 4 +
                                  16 * nTcharSize); // m_dwDisplayStringLength;
                dwerr =
                  *(DWORD*)(p + 1 * 4 + (16 * 2 + 2 * 4) + 5 * 4 +
                            16 * nTcharSize + 1 * 4); // m_dwDisplayErrorStart;
                wstr    = (WCHAR*)(p + 1 * 4 + (16 * 2 + 2 * 4) +
                                5 * 4); // m_tszDisplayString
    ...
    if ( unicode )
    {
        ...
        {
            if ( dwerr<=(DWORD)i && g_dwSelection==(DWORD)-1 ){ // select error
char g_dwSelection = i;
            }
#ifdef UNICODE
            ...
            mbc[1] = 0;
            mbc[2] = 0;
            WideCharToMultiByte(g_uCodePage, 0, wstr + i, 1, mbc, sizeof(mbc),
NULL, NULL);
    ...
    else
    {
        char *p = (char *)wstr;
        int i, j;
        for ( i=0, j=0; (DWORD)i < dwlen ; i++,j++ ) // dwlen > 0, if known IME
: yutakah
        {
...
#ifdef UNICODE
            MultiByteToWideChar( g_uCodePage, 0, p + i, 1 + ( _IsLeadByte( p[i]
) ? 1 : 0 ), g_szCandidate[j], 1 ); if ( _IsLeadByte( p[i] ) )
            {
                i++;
            }
#else
...
static struct
{
    ...
    false,
    true,
    0xbc, // Alt + Ctrl + ',': SW keyboard for
          // Trad. Chinese IME
    true,
    false,
    false,
    VK_TAB, // ATOK2005's Ctrl+TAB
};

//
// Ignores specific keys when IME is on. Returns true if the message is a hot
// key to ignore.
...
bool ImeUi_IgnoreHotKey(const MSG* pmsg)
{
    ...
    if (pmsg->wParam == VK_PROCESSKEY &&
        ...
        // special case #1 - VK_JUNJA toggles half/full width input mode in
        // Korean IME. This VK (sent by Alt+'=' combo) is ignored regardless of
        // the modifier state.
        if (uVkReal == VK_JUNJA) {
            return true;
        }
        // special case #2 - disable right arrow key that switches the candidate
        // list to expanded mode in CHT IME.
        if (uVkReal == VK_RIGHT && g_bCandList && GETLANG() == LANG_CHT) {
            return true;
        }
#ifndef ENABLE_HANJA_KEY
        // special case #3 - we disable VK_HANJA key because 1. some Korean
        // fonts don't Hanja and 2. to reduce testing cost.
        ...
        for (int i = 0; i < COUNTOF(aHotKeys); i++) {
            if (aHotKeys[i].m_bCtrl == bCtrl &&
                aHotKeys[i].m_bShift == bShift && aHotKeys[i].m_bAlt == bAlt &&
...
void ImeUi_FinalizeString(bool bSend)
{
    ...
    if (!g_bInitialized || bProcessing ||
        NULL == (himc = _ImmGetContext(g_hwndCurr)))
        return;
    bProcessing = true;
    ...
    if (IsUILessMode()) {
        ...
        ImmSetCompositionString(
          himc, SCS_SETSTR, TEXT(""), sizeof(TCHAR), TEXT(""), sizeof(TCHAR));
    }
    ...
    _ImmReleaseContext(g_hwndCurr, himc);
    // Zooty2 RAID #4759: Sometimes application doesn't receive
    // IMN_CLOSECANDIDATE on Alt+Tab So the same code for IMN_CLOSECANDIDATE is
    // replicated here.
    CloseCandidateList();
    // BigWorld: make sure the reading string doesn't get stuck on in some
    // languages
    g_bReadingWindow     = false;
    g_szReadingString[0] = NULL;
    bProcessing          = false;
...
static void SetCompStringColor()
{
    ...
    DWORD dwTranslucency    = (g_dwIMELevel == 2)
                                ? 0xff000000
                                : ((DWORD)gSkinIME.compTranslucence << 24);
    gSkinCompStr.colorInput = dwTranslucency | gSkinIME.compColorInput;
    gSkinCompStr.colorTargetConv =
      dwTranslucency | gSkinIME.compColorTargetConv;
    gSkinCompStr.colorConverted = dwTranslucency | gSkinIME.compColorConverted;
    gSkinCompStr.colorTargetNotConv =
      dwTranslucency | gSkinIME.compColorTargetNotConv;
...
static void CheckToggleState()
{
    ...
    bool bIme =
      _ImmIsIME(g_hklCurrent) != 0 &&
      ((0xF0000000 &
        static_cast<DWORD>(reinterpret_cast<UINT_PTR>(g_hklCurrent))) ==
    ...
    if (NULL != (himc = _ImmGetContext(g_hwndCurr))) {
        if (g_bChineseIME) {
            DWORD dwConvMode, dwSentMode;
            _ImmGetConversionStatus(himc, &dwConvMode, &dwSentMode);
            g_dwState = (dwConvMode & IME_CMODE_NATIVE) ? IMEUI_STATE_ON
                                                        : IMEUI_STATE_ENGLISH;
        } else {
            g_dwState = (bIme && _ImmGetOpenStatus(himc) != 0)
                          ? IMEUI_STATE_ON
                          : IMEUI_STATE_OFF;
        }
        _ImmReleaseContext(g_hwndCurr, himc);
...
// this function is used only in brief time in CHT IME handling, so accelerator
// isn't processed.
static void _PumpMessage()
{
    MSG msg;
...
static void GetReadingWindowOrientation(DWORD dwId)
{
    g_bHorizontalReading = (g_hklCurrent == _CHS_HKL) ||
                           (g_hklCurrent == _CHT_HKL_NEW_CHANG_JIE) ||
                           (dwId == 0);
    if (!g_bHorizontalReading && IMEID_LANG(dwId) == LANG_CHT) {
        ...
        strcat_s(szRegPath,
                 COUNTOF(szRegPath),
                 (dwVer >= MAKEIMEVERSION(5, 1)) ? "MSTCIPH" : "TINTLGNT");
        LONG lRc =
          RegOpenKeyExA(HKEY_CURRENT_USER, szRegPath, 0, KEY_READ, &hkey);
        if (lRc == ERROR_SUCCESS) {
            DWORD dwSize = sizeof(DWORD), dwMapping, dwType;
            lRc          = RegQueryValueExA(hkey,
                                   "keyboard mapping",
                                   NULL,
                                   &dwType,
                                   (PBYTE)&dwMapping,
...
void ImeUi_ToggleLanguageBar(BOOL bRestore)
{
    ...
    if (iShowStatusWindow == -1) {
        iShowStatusWindow =
          IsNT() && g_osi.dwMajorVersion >= 5 &&
              (g_osi.dwMinorVersion > 1 ||
               (g_osi.dwMinorVersion == 1 && lstrlenA(g_osi.szCSDVersion)))
            ? 1
            : 0;
    }
    ...
    if (hwndImeDef && !bRestore) {
        // The following OPENSTATUSWINDOW is required to hide ATOK16 toolbar
        // (FS9:#7546)
        SendMessageA(hwndImeDef, WM_IME_CONTROL, IMC_OPENSTATUSWINDOW, 0);
...
static void OnInputLangChange()
{
    ...
    if (uLang != GETPRIMLANG()) {
        ...
        // Other languages use the level that is specified by
        // ImeUi_SetSupportLevel()
...
static void CheckInputLocale()
{
    ...
    char szCodePage[8];
    int  iRc = GetLocaleInfoA(MAKELCID(GETLANG(), SORT_DEFAULT),
...
BOOL CTsfUiLessMode::SetupSinks()
{
    ...
    if (hr != S_OK) {
        BW_IME_DEBUG_MSG(
          "CTsfUiLessMode::SetupSinks CoCreateInstance Fail (hr=%d)\n", hr);
        return FALSE;
    }
            ...
            if (SUCCEEDED(hr = srcTm->AdviseSink(__uuidof(ITfUIElementSink),
                ...
                if (SUCCEEDED(
                      hr = srcTm->AdviseSink(
                        ...
                        &m_dwAlpnSinkCookie))) {
                    if (SetupCompartmentSinks()) // Setup compartment sinks for
                                                 // the first time
                    {
                        bRc = TRUE;
                        BW_IME_DEBUG_MSG("CTsfUiLessMode::SetupSinks - Sinks "
                                         "have been setup.\n");
                    }
                }
            }
...
void CTsfUiLessMode::ReleaseSinks()
{
    ...
    if (m_tm &&
        ...
        BW_IME_DEBUG_MSG(
          "CTsfUiLessMode::ReleaseSinks - Sinks have been released.\n");
    }
}
...
{
    BW_IME_DEBUG_MSG(
      "CUIElementSink::BeginUIElement (dwUIElementId=%d, *pbShow=%d)\n",
      dwUIElementId,
      *pbShow);
    ...
    if (!pElement) {
        BW_IME_DEBUG_MSG(
          "CUIElementSink::BeginUIElement - GetUIElement returned NULL\n");
        return E_INVALIDARG;
    }
    ...
    if (SUCCEEDED(pElement->GetDescription(&desc))) {
        BW_IME_DEBUG_MSG(
          "CUIElementSink::BeginUIElement: pElement->GetDescription=%s)\n",
    ...
    *pbShow                                    = FALSE;
    if (!g_bCandList &&
        SUCCEEDED(pElement->QueryInterface(
...
STDAPI CTsfUiLessMode::CUIElementSink::UpdateUIElement(DWORD dwUIElementId)
{
    ...
    if (!pElement) {
        BW_IME_DEBUG_MSG(
          "CUIElementSink::UpdateUIElement - GetUIElement returned NULL\n");
        return E_INVALIDARG;
    }
    ...
    if (SUCCEEDED(pElement->GetDescription(&desc))) {
        BW_IME_DEBUG_MSG(
          "CUIElementSink::UpdateUIElement: pElement->GetDescription=%s)\n",
          bw_wtoutf8(desc).c_str());
        SysFreeString(desc);
    }
#endif
...
STDAPI CTsfUiLessMode::CUIElementSink::EndUIElement(DWORD dwUIElementId)
{
    ...
    if (!pElement) {
        BW_IME_DEBUG_MSG(
          "CUIElementSink::EndUIElement - GetUIElement returned NULL\n");
        return E_INVALIDARG;
    }
    ...
    if (SUCCEEDED(pElement->GetDescription(&desc))) {
        BW_IME_DEBUG_MSG(
          "CUIElementSink::EndUIElement: pElement->GetDescription=%s)\n",
          bw_wtoutf8(desc).c_str());
        SysFreeString(desc);
    }
#endif
...
void CTsfUiLessMode::UpdateImeState(BOOL bResetCompartmentEventSink)
{
    ...
    if (GetCompartments(&pcm, &pTfOpenMode, &pTfConvMode)) {
        ...
        if (valOpenMode.vt == VT_I4) {
            if (g_bChineseIME) {
                g_dwState = valOpenMode.lVal != 0 && valConvMode.lVal != 0
                              ? IMEUI_STATE_ON
                              : IMEUI_STATE_ENGLISH;
            } else {
                g_dwState =
        ...
        if (bResetCompartmentEventSink) {
            SetupCompartmentSinks(
              FALSE, pTfOpenMode, pTfConvMode); // Reset compartment sinks
        }
        pTfOpenMode->Release();
        pTfConvMode->Release();
        pcm->Release();
...
STDAPI CTsfUiLessMode::CUIElementSink::OnActivated(DWORD    dwProfileType,
                                                   LANGID   langid,
                                                   REFCLSID clsid,
                                                   REFGUID  catid,
                                                   REFGUID  guidProfile,
...
{
    BW_IME_DEBUG_MSG(
      "CUIElementSink::OnActivated - dwProfileType=%x, langid=%x, clasid=%x, "
      "catid=%x, guidProfile=%x, hkl=%x, dwFlags=%x\n",
      dwProfileType,
      langid,
      clsid,
      catid,
      ...
      hkl,
      dwFlags);
    static GUID s_TF_PROFILE_DAYI = { 0x037B2C25, 0x480C, 0x4D7F, 0xB0,
                                      0x27,       0xD6,   0xCA,   0x6B,
                                      0x69,       0x78,   0x8A };
  ...
  ITfReadingInformationUIElement* preading)
{
    ...
    BW_IME_DEBUG_MSG(
      "CTsfUiLessMode::MakeReadingInformationString (dwFlags=0x%x, cchMax=%d, "
      "uErrorIndex=%d, fVertical=%d)\n",
      dwFlags,
      cchMax,
      uErrorIndex,
      fVertical);
    ...
    g_iReadingError--; // g_iReadingError is used only in horizontal window, and
                       // has to be -1 if there's no error.
#ifndef UNICODE
    if (g_iReadingError > 0) {
    ...
    if (SUCCEEDED(preading->GetString(&bstr))) {
        if (bstr) {
            ...
            szStr[0] = 0;
            int iRc  = WideCharToMultiByte(
              CP_ACP, 0, bstr, -1, szStr, sizeof(szStr), NULL, NULL);
            if (iRc >= sizeof(szStr)) {
            ...
            if (fVertical) {
                // for vertical reading window, copy each character to
                // g_szCandidate array.
                ...
                g_szCandidate[0][0] =
                  TEXT(' '); // hack to make rendering happen
            }
            SysFreeString(bstr);
        } else {
            g_szReadingString[0] =
              NULL; // BigWorld: make sure reading string doesn't hang around.
            BW_IME_DEBUG_MSG("CTsfUiLessMode::MakeReadingInformationString - "
                             "preading->GetString returned NULL string.\n");
        }
    }
}
...
void CTsfUiLessMode::MakeCandidateStrings(ITfCandidateListUIElement* pcandidate)
{
    ...
    for (UINT i = dwPageStart, j = 0;
         (DWORD)i < g_dwCount && j < g_uCandPageSize;
         i++, j++) {
                                     ...
                                     ITfCompartment**    ppTfConvMode)
{
    ...
    if (SUCCEEDED(
        ...
        if (SUCCEEDED(hr = pcm->GetCompartment(
                        GUID_COMPARTMENT_KEYBOARD_OPENCLOSE, &pTfOpenMode))) {
            if (SUCCEEDED(hr = pcm->GetCompartment(
                                           ...
                                           ITfCompartment* pTfConvMode)
{
    ...
    if (SUCCEEDED(hr = pTfOpenMode->QueryInterface(IID_ITfSource,
        ...
        if (bRemoveOnly || SUCCEEDED(hr = srcOpenMode->AdviseSink(
            ...
            if (SUCCEEDED(hr = pTfConvMode->QueryInterface(
                            IID_ITfSource, (void**)&srcConvMode))) {
                // Remove existing sink for open mode
                ...
                // Setup sink for open mode (toggle state) change
                if (bRemoveOnly ||
                    SUCCEEDED(hr = srcConvMode->AdviseSink(
                                IID_ITfCompartmentEventSink,
                                (ITfCompartmentEventSink*)m_TsfSink,
...
TCHAR* ImeUi_GetCompositionString()
{
    if (!g_szCompositionString[0] && g_dwCount > 0) {
        return TEXT(" "); // BigWorld: If we have candidates, make sure we
                          // always have a composition.
    } else {
