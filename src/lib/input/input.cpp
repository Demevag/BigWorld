...
namespace {
    ...
    inline Vector2 lastMessageCursorPos(HWND hWnd)
    {
        ...
        POINT msgCursorPos = { GET_X_LPARAM(dwCursorPos),
...
bool InputDevices::privateProcessEvents(InputHandler& handler)
{
    ...
    { // DogWatch scope
        ...
        // Sometimes new events can be added while handling events in the queue,
        // so we need to iterate by index, and avoid iterators.
 ...
 */
bool InputDevices::handleWindowsMessagePrivate(HWND     hWnd,
                                               UINT     msg,
                                               WPARAM&  wParam,
                                               LPARAM&  lParam,
                                               LRESULT& result)
{
    ...
    switch (msg) {
        ...
        case WM_SYSKEYUP: {
            ...
            */
            ...
            if (msg == WM_KEYDOWN && ignoringKey_ == key) {
                ...
                pushKeyEventWin32(
                  key, msg, modifiers(), lastMessageCursorPos(hWnd));
                keyWasIgnored_ = false;
                break;
            }
        }
        ...
        case WM_INPUT: {
            ...
            UINT res = GetRawInputData((HRAWINPUT)lParam,
                                       RID_INPUT,
                                       &rid,
                                       &dwSize,
                ...
                if (rkb->VKey == VK_SHIFT) {
                    ...
                    if (key == KeyCode::KEY_LSHIFT ||
                        ...
                        if (rkb->Message == WM_KEYUP ||
                            ...
                            forceKeyUp(KeyCode::KEY_PGDN,
                                       modifiers(),
                                       lastMessageCursorPos(hWnd));
                            forceKeyUp(KeyCode::KEY_PGUP,
                                       modifiers(),
                                       lastMessageCursorPos(hWnd));
                            forceKeyUp(KeyCode::KEY_HOME,
                                       modifiers(),
                                       lastMessageCursorPos(hWnd));
                            forceKeyUp(KeyCode::KEY_END,
                                       modifiers(),
                                       lastMessageCursorPos(hWnd));
                        }

                        pushKeyEventWin32(key,
                                          rkb->Message,
                                          modifiers(),
                                          lastMessageCursorPos(hWnd));
                    }
                ...
                */
                ...
                } else // Must be MOUSE_INPUT_RELATIVE
                {
                    if (rm->lLastX != 0 || rm->lLastY != 0 || dz != 0) {
                        pushMouseEvent(
                          rm->lLastX, rm->lLastY, dz, currentCursorPos(hWnd));
                    }
                }
            }
        ...
        case WM_LBUTTONDOWN: {
            pushMouseButtonEvent(KeyCode::KEY_LEFTMOUSE,
                                 true,
                                 modifiers(),
                                 lastMessageCursorPos(hWnd));
            break;
        ...
        case WM_RBUTTONDOWN: {
            pushMouseButtonEvent(KeyCode::KEY_RIGHTMOUSE,
                                 true,
                                 modifiers(),
                                 lastMessageCursorPos(hWnd));
            break;
        ...
        case WM_MBUTTONDOWN: {
            pushMouseButtonEvent(KeyCode::KEY_MIDDLEMOUSE,
                                 true,
                                 modifiers(),
                                 lastMessageCursorPos(hWnd));
            break;
        ...
        case WM_XBUTTONDOWN: {
            ...
            pushMouseButtonEvent(
              key, true, modifiers(), lastMessageCursorPos(hWnd));
            break;
        }
        ...
        case WM_XBUTTONUP: {
            ...
            pushMouseButtonEvent(
              key, false, modifiers(), lastMessageCursorPos(hWnd));
            break;
        }
        ...
        case WM_MOUSEMOVE: {
            ...
            for (EventQueue::reverse_iterator it = eventQueue_.rbegin();
                ...
                if (event.type_ == InputEvent::MOUSE) {
                    ...
                    mevent.fill(
                      mevent.dx(), mevent.dy(), mevent.dz(), cursorPos);
                }
            }
...
void InputDevices::updateCursorCapture(HWND hWnd)
{
    ...
    for (int i = KeyCode::KEY_MINIMUM_MOUSE; i < KeyCode::KEY_MAXIMUM_MOUSE;
         i++) {
...
void InputDevices::handleLostData(InputHandler& handler, int mask)
{
    ...
    if ((mask & JOY_DATA_LOST) && joystick_.pDIJoystick()) {
        ...
        hr =
          joystick_.pDIJoystick()->GetDeviceState(sizeof(joyState), &joyState);

        if (SUCCEEDED(hr)) {
            // success.  iterate through valid joystick codes and check the
            // state
            for (int k = KeyCode::KEY_MINIMUM_JOY;
                ...
                for (int k = KeyCode::KEY_MINIMUM_JOY;
                    ...
                    if (event.isKeyDown() !=
                        ...
                        keyStates_.setKeyStateNoRepeat(event.key(),
                                                       event.isKeyDown());
                        handler.handleKeyEvent(event);
                    }
                }
 ...
 */
void InputDevices::pushKeyEventWin32(KeyCode::Key   key,
                                     USHORT         msg,
                                     uint32         mods,
 ...
 */
void InputDevices::pushKeyEvent(KeyCode::Key   key,
                                int32          state,
                                uint32         mods,
                                const Vector2& cursorPos)
{
 ...
 */
void InputDevices::pushMouseButtonEvent(KeyCode::Key   key,
                                        bool           isDown,
                                        uint32         mods,
                                        const Vector2& cursorPos)
{
 ...
 */
void InputDevices::pushMouseEvent(long           dx,
                                  long           dy,
                                  long           dz,
                                  const Vector2& cursorPos,
                                  bool           accumulate)
{
    BW_GUARD;
    if (!accumulate || eventQueue_.empty() ||
        ...
        mevent.fill(
          mevent.dx() + dx, mevent.dy() + dy, mevent.dz() + dz, cursorPos);
    }
}
...
void InputDevices::forceKeysUp()
{
    ...
    for (size_t i = KeyCode::KEY_MINIMUM_KEY; i < KeyCode::NUM_KEYS; ++i) {
        ...
        if (this->isKeyDown(key)) {
            this->forceKeyUp(
              key, this->modifiers(), lastMessageCursorPos(hWnd_));
        }
    }
}
...
/*static*/ void InputDevices::pushIMECharEvent(wchar_t* chs)
{
    ...
    if (InputDevices::pInstance()) {
        ...
        InputDevices::instance().pushKeyEvent(
          KeyCode::KEY_IME_CHAR, 0, chs, 0, lastMessageCursorPos(hWnd));
    }
}
                                    ...
                                    void*                   pData)
{
    ...
    HRESULT hr = pCallbackData->pDirectInput->CreateDevice(
      GUID_Joystick, pCallbackData->ppDIJoystick, NULL);
                               ...
                               void*                         pJoystickAsVoid)
{
    ...
    if (FAILED(
          pJoystick->pDIJoystick()->SetProperty(DIPROP_RANGE, &diprg.diph))) {
        return DIENUM_STOP;
    }
 ...
 */
bool Joystick::init(IDirectInput8* pDirectInput, void* hWnd)
{
    BW_GUARD;
    EnumJoysticksCallbackData callbackData = { &pDIJoystick_, pDirectInput };
    ...
    if (FAILED(pDIJoystick_->SetCooperativeLevel(
          hWnd_, DISCL_EXCLUSIVE | DISCL_FOREGROUND))) {
        return false;
    }
...
static inline float scaleFromDIToUnit(int value)
{
    ...
    float floatValue = (float)value / (float)(DIRECT_INPUT_AXIS_MAX -
                                              DIRECT_INPUT_AXIS_DEAD_ZONE);
...
bool Joystick::updateFromJoystickDevice()
{
    ...
    if (pDIJoystick_) {
        ...
        /*
                ...
                {
                    bw_snwprintf( strText, sizeof(strText)/sizeof(wchar_t),
           "%ld", js.rgdwPOV[0] ); SetWindowText( GetDlgItem( hDlg, IDC_POV ),
           strText );
                }
...
static const KeyCode::Key s_joyKeys_PlayStation[32] = {
    KeyCode::KEY_JOYTRIANGLE, KeyCode::KEY_JOYCIRCLE, KeyCode::KEY_JOYCROSS,
    KeyCode::KEY_JOYSQUARE,   KeyCode::KEY_JOYL2,     KeyCode::KEY_JOYR2,
    KeyCode::KEY_JOYL1,       KeyCode::KEY_JOYR1,     KeyCode::KEY_JOYSELECT,
    KeyCode::KEY_JOYSTART,    KeyCode::KEY_JOYARPUSH, KeyCode::KEY_JOYALPUSH,
    KeyCode::KEY_JOYDUP,      KeyCode::KEY_JOYDRIGHT, KeyCode::KEY_JOYDDOWN,
    KeyCode::KEY_JOYDLEFT,

    KeyCode::KEY_JOY16,       KeyCode::KEY_JOY17,     KeyCode::KEY_JOY18,
    KeyCode::KEY_JOY19,       KeyCode::KEY_JOY20,     KeyCode::KEY_JOY21,
    KeyCode::KEY_JOY22,       KeyCode::KEY_JOY23,     KeyCode::KEY_JOY24,
    KeyCode::KEY_JOY25,       KeyCode::KEY_JOY26,     KeyCode::KEY_JOY27,
    KeyCode::KEY_JOY28,       KeyCode::KEY_JOY29,     KeyCode::KEY_JOY30,
    KeyCode::KEY_JOY31
};
                             ...
                             bool*         pLostDataFlag)
{
    ...
    switch (hr) {
        ...
        case DI_BUFFEROVERFLOW:
            //		DEBUG_MSG( "Joystick::processEvents: joystick buffer
            // overflow\n" );
        ...
        case DIERR_NOTACQUIRED:
            //		DEBUG_MSG( "Joystick::processEvents: input not acquired,
            // acquiring\n" );
