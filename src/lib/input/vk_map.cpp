...
namespace VKMap {
    const KeyCode::Key VKMap[] = {
        ...
        KeyCode::KEY_NONE,  // VK_SHIFT         0x10  Shift/Control is mapped by
        ...
        KeyCode::KEY_SLASH, // VK_OEM_2          0xBF  // '/?' for US// '/?' for
                            // US
    ...
    KeyCode::Key fromVKey(USHORT vkey, bool extended)
    {
        ...
        // Remap to specific left/right keys (shift is a little more message
        // specific so is handled by the caller).
        if (vkey == VK_CONTROL || vkey == VK_LCONTROL) {
            ...
            // There's no specific VK_ code for numpad enter, so just shortcut
            // it here.
            if (extended) {
    ...
    KeyCode::Key fromRawKey(RAWKEYBOARD* rkb)
    {
        ...
        if (vkey == VK_SHIFT) {
            ...
            vkey =
              MapVirtualKey(rkb->MakeCode, 0x03); // 0x03 == MAPVK_VSC_TO_VK_EX
        }
