...
namespace {
    ...
    void extend(WorldTriangle& wt)
    {
        ...
        wt = WorldTriangle(
          wt.v0() - a + c, wt.v1() - b + a, wt.v2() - c + b, TRIANGLE_TERRAIN);
    }
}
  ...
  , isQuadTreeInUse_(0)
{
    ...
    sizeInBytes_ =
      sizeof(*this) + heights_.width() * heights_.height() * sizeof(float);

    // Track memory usage
    RESOURCE_COUNTER_ADD(
      ResourceCounters::DescriptionPool("Terrain/HeightMap2",
...
void TerrainHeightMap2::allocateQuadTreeData() const
{
    ...
    // Track memory usage
    RESOURCE_COUNTER_ADD(
      ResourceCounters::DescriptionPool("Terrain/HeightMap2/QuadTree",
                                        (uint)ResourceCounters::MP_SYSTEM,
                                        ResourceCounters::RT_OTHER),
      numQuadTreeCells_ * sizeof(TerrainQuadTreeCell),
      0);
}
...
bool TerrainHeightMap2::save(DataSectionPtr pSection) const
{
    ...
    // data obtained from the heightmap. This will prevent issues when heightmap
    // is saved multiple times with loss of precision due to PNG compression and
    // quantization in compressHeightMap
    ...
    BinaryPtr bdata =
      new BinaryBlock(&data[0], data.size(), "Terrain/HeightMap2/BinaryBlock");
 ...
 */
/*virtual*/ bool TerrainHeightMap2::load(DataSectionPtr dataSection,
...
)
{
    ...
    if (!pBin) {
        if (error)
            *error = "dataSection is not binary";
        return false;
    }
                                      ...
                                      BW::string* error /*= NULL*/)
{
    ...
    if (header->magic_ != HeightMapHeader::MAGIC) {
        if (error)
            *error = "dataSection is not for a TerrainHeightMap2";
        return false;
    }
    ...
    if (header->version_ == HeightMapHeader::VERSION_REL_UINT16_PNG) {
        ...
        if (!decompressImage(pImageData, image)) {
            if (error)
                *error = "The height map data could not be decompressed.";
            return false;
        }

        const uint16* pRelHeights = image.getRow(0);
        heights_                  = Moo::Image<float>(
          header->width_, header->height_, "Terrain/HeightMap2/Image");

        for (uint32 i = 0; i < header->height_ * header->width_; i++) {
            *(heights_.getRow(0) + i) = convertHeight2Float(
              *pRelHeights++ + *((int32*)&header->minHeight_));
        }
    }
    // support raw float loading if new format or in editor
        ...
        BinaryPtr compData = new BinaryBlock((void*)(header + 1),
...
Vector3 TerrainHeightMap2::normalAt(int x, int z) const
{
...
#else
    //-- Note: we use this stuff for the y component of the normal to add more
    //power to the x and
    //--		  z components. By doing this we can make normal maps generated
    //by WorldEditor almost
    //--		  with the same quality and contrast as WorldMachine does. My
    //research points me out
    //--		  that BigWorld uses some fake formula for calculation normal
    //maps for terrain, so
    ...
    float val1 = 0.0f;
    float val2 = 0.0f;
    if (x - 1 < 0 || z - 1 < 0 || x + 1 >= int(heights_.width()) ||
        z + 1 >= int(heights_.height())) {
 ...
 */
bool TerrainHeightMap2::collide(Vector3 const&            start,
                                Vector3 const&            end,
                                TerrainCollisionCallback* pCallback) const
{
    ...
    if (s_optimiseCollisionAlongZAxis && almostEqual(start.z, end.z) &&
        pCallback) {
        return hmCollideAlongZAxis(start, start, end, pCallback);
    }

    if ((xDist < quadtreeThreshold && zDist < quadtreeThreshold) ||
        disableQuadTree) {
 ...
 *  @param pCallback    The terrain collision callback.
 */
bool TerrainHeightMap2::collide(WorldTriangle const&      start,
                                Vector3 const&            end,
                                TerrainCollisionCallback* pCallback) const
{
    BW_GUARD;
 ...
 */
bool TerrainHeightMap2::hmCollide(Vector3 const&            originalStart,
                                  Vector3 const&            start,
                                  Vector3 const&            end,
                                  TerrainCollisionCallback* pCallback) const
{
    ...
    BoundingBox bb(Vector3(0, minHeight(), 0),
                   Vector3(blockSize(), maxHeight(), blockSize()));

    const Vector3 gridAlign(float(blocksWidth()) / blockSize(),
                            1.f,
    ...
    if (bb.clip(s, e, BOUNDING_BOX_EPSILON)) {
        ...
        while ((gridX != gridEndX &&
            ...
            if (checkGrid(gridX, gridZ, originalStart, end, pCallback))
                return true;

            float xDistNextGrid =
              gridDirX < 0 ? float(gridX) - s.x : float(gridX + 1) - s.x;
            ...
            float zDistNextGrid =
              gridDirZ < 0 ? float(gridZ) - s.z : float(gridZ + 1) - s.z;
...
namespace // unnamed
{
    ...
    inline bool collideTerrainGrid(float&         outT,
                                   Vector3&       outTriNormal,
                                   const Vector3& segStart,
                                   const Vector3& segStop,
                                   const Vector3& gridAdj1,
                                   const Vector3& gridAdj2,
                                   const Vector3& gridPre,
                                   const Vector3& gridPost)
    {
 ...
 *	@param dist			length of the segment, such that: end = start + dist *
 *dir
 *	@return true if collision found
 */
bool TerrainHeightMap2::collideShortSegment(float&         outCollDist,
                                            Vector3&       outTriNormal,
                                            const Vector3& start,
                                            const Vector3& dir,
                                            float          dist) const
...
#define FAST_HEIGHT_AT(x, z)                                                   \
    heights_.getRow((z) +                                                      \
                    internalVisibleOffset())[(x) + internalVisibleOffset()]
    ...
    for (int gridZ = gridBegZ; gridZ <= gridEndZ; ++gridZ) {
        bottomRight.set(float(gridBegX) * internalSpacingX(),
                        FAST_HEIGHT_AT(gridBegX, gridZ),
                        float(gridZ) * internalSpacingZ());

        topRight.set(bottomRight.x,
        ...
        for (int gridX = gridBegX; gridX <= gridEndX; ++gridX) {
            ...
            if ((gridX ^ gridZ) & 1)
                gotCollision = collideTerrainGrid(t,
                                                  triNormal,
                                                  start,
                                                  end,
                                                  bottomRight,
            ...
            if (gotCollision) {
                // TODO: may be search for all collisions, not only first
                // detected.
                outCollDist  = t * dist;
 ...
 *	wich may significantly optimize selection in some cases (eg vehicle box
 *above smooth ground).
 *
 ...
 */
void TerrainHeightMap2::visitTriangles(RealWTriangleSet& outTris,
...
#define FAST_HEIGHT_AT(x, z)                                                   \
    ...
    int xe = std::min(int((boxCenter.x + dx) * invSpacingX) + 1,
                      int(internalBlocksWidth()));
    int zs = std::max(int((boxCenter.z - dz) * invSpacingZ), 0);
    int ze = std::min(int((boxCenter.z + dz) * invSpacingZ) + 1,
                      int(internalBlocksHeight()));
    ...
    Vector3 v(initX, 0.f, float(zs) * spacingZ);
    ...
    for (int z = zs; z < ze; ++z) {
        ...
        for (int x = xs; x < xe; ++x) {
            ...
            if ((bld > 0.f) | (tld > 0.f) | (brd > 0.f) | (trd > 0.f)) {
                ...
                bottomRight.set(v.x, FAST_HEIGHT_AT(x + 1, z), v.z - spacingZ);

                topLeft.set(v.x - spacingX, FAST_HEIGHT_AT(x, z + 1), v.z);

                bottomLeft.set(
                  v.x - spacingX, FAST_HEIGHT_AT(x, z), v.z - spacingZ);

                if ((x ^ z) & 1) {
                    if (bld > 0.f || tld > 0.f || brd > 0.f)
                        outTris.push_back(
                          WorldTriangle(bottomLeft, topLeft, bottomRight));

                    if (tld > 0.f || trd > 0.f || brd > 0.f)
                        outTris.push_back(
                          WorldTriangle(topLeft, topRight, bottomRight));
                } else {
                    if (bld > 0.f || tld > 0.f || trd > 0.f)
                        outTris.push_back(
                          WorldTriangle(bottomLeft, topLeft, topRight));

                    if (trd > 0.f || brd > 0.f || bld > 0.f)
                        outTris.push_back(
                          WorldTriangle(topRight, bottomRight, bottomLeft));
                }
            }
        }
...
#define FAST_HEIGHT_AT(x, z)                                                   \
    ...
    int xe = std::min(int((bb.maxBounds().x) * invSpacingX) + 1,
                      int(internalBlocksWidth()));
    int zs = std::max(int((bb.minBounds().z) * invSpacingZ), 0);
    int ze = std::min(int((bb.maxBounds().z) * invSpacingZ) + 1,
                      int(internalBlocksHeight()));
  ...
  TerrainCollisionCallback* pCallback) const
{
    ...
    if (bb.clip(s, e, BOUNDING_BOX_EPSILON)) {
        ...
        const float* rowZ = heights_.getRow(gridZ + internalVisibleOffset()) +
                            gridX + internalVisibleOffset();
        const float* rowZPlusOne =
          heights_.getRow(gridZ + 1 + internalVisibleOffset()) + gridX +
          internalVisibleOffset();
        ...
        for (; gridX != gridEndX; gridX += gridDirX) {
            ...
            if ((gridX ^ gridZ) & 1) {
                tri = WorldTriangle(
                  bottomLeft, topLeft, bottomRight, TRIANGLE_TERRAIN);
            } else {
                tri = WorldTriangle(
                  bottomLeft, topLeft, topRight, TRIANGLE_TERRAIN);
            }
            ...
            if (tri.intersects(originalStart, dir, dist1) &&
                ...
                if ((gridX ^ gridZ) & 1) {
                    tri = WorldTriangle(
                      topLeft, topRight, bottomRight, TRIANGLE_TERRAIN);
                } else {
                    tri = WorldTriangle(
                      topRight, bottomRight, bottomLeft, TRIANGLE_TERRAIN);
                }
 ...
 */
bool TerrainHeightMap2::hmCollide(WorldTriangle const&      triStart,
 ...
 *  @param pCallback    The collision callback.
 */
bool TerrainHeightMap2::checkGrid(int                       gridX,
                                  int                       gridZ,
                                  Vector3 const&            start,
                                  Vector3 const&            end,
                                  TerrainCollisionCallback* pCallback) const
{
    ...
    if (gridX >= 0 && gridX < (int)verticesWidth() && gridZ >= 0 &&
        gridZ < (int)verticesHeight()) {
        // Construct our four vertices that make up this grid cell
        Vector3 bottomLeft(float(gridX) * internalSpacingX(),
        ...
        if ((gridX ^ gridZ) & 1) {
            triA =
              WorldTriangle(bottomLeft, topLeft, bottomRight, TRIANGLE_TERRAIN);
            triB =
              WorldTriangle(topLeft, topRight, bottomRight, TRIANGLE_TERRAIN);
        } else {
            triA =
              WorldTriangle(bottomLeft, topLeft, topRight, TRIANGLE_TERRAIN);
            triB = WorldTriangle(
              topRight, bottomRight, bottomLeft, TRIANGLE_TERRAIN);
        }
                                  ...
                                  TerrainCollisionCallback* pCallback) const
{
    ...
    if (gridX >= 0 && gridX < (int)blocksWidth() && gridZ >= 0 &&
        gridZ < (int)blocksHeight()) {
        // Construct our four vertices that make up this grid cell
        Vector3 bottomLeft(float(gridX) * internalSpacingX(),
...
bool HeightMapResource::load()
{
    ...
    BW::string heightSectionName = TerrainHeightMap2::getHeightSectionName(
      terrainSectionName_ + "/heights", lodLevel_);
