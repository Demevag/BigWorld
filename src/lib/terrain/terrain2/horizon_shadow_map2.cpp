...
/*explicit*/ HorizonShadowMap2::HorizonShadowMap2(TerrainBlock2* block)
  : HorizonShadowMap(block ? block->blockSize() : DEFAULT_GRID_RESOLUTION)
  , // 22_MERGE: can receive NULL block via dummyHorizonShadowMap_
  block_(block)
  , texture_(NULL)
  , textureLock_(NULL)
  , width_(0)
...
bool HorizonShadowMap2::create(uint32 size, BW::string* error /*= NULL*/)
{
    ...
    if ((texture_ = Moo::rc().createTexture(
           size,
           size,
           1,
 ...
 */
/*virtual*/ HorizonShadowPixel HorizonShadowMap2::shadowAt(int32 x,
                                                           int32 z) const
{
    BW_GUARD;
...
/*virtual*/ bool HorizonShadowMap2::lock(bool readOnly)
{
    ...
    if (texture_ && lockCount_++ == 0) {
        ...
        HRESULT hr = textureLock_.lockRect(
          0, &lockedRect, NULL, readOnly ? D3DLOCK_READONLY : 0);
...
/*virtual*/ bool HorizonShadowMap2::save(DataSectionPtr pSection) const
{
    ...
    if (myself->lock(true)) {
        ...
        BinaryPtr binaryBlock = new BinaryBlock(
          &data.front(), data.size(), "BinaryBlock/HorizonShadowMap2");
 ...
 */
/*virtual*/ bool HorizonShadowMap2::load(DataSectionPtr dataSection,
                                         BW::string*    error /*= NULL*/)
{
    ...
    if (pHorizonData) {
        ...
        if ((texture_ =
               Moo::rc().createTexture(pHeader->width_,
                                       pHeader->height_,
                                       1,
                                       usage,
                                       D3DFMT_G16R16,
