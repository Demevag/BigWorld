 ...
 *  @param sizeMultiplier  Multiplier to affect the size of the dominant texture
 * map relative to the size of the biggest layer.
 */
...
/*virtual*/ bool DominantTextureMap2::save(DataSectionPtr pSection) const
{
    ...
    if (textureDataSize == 0 || image().rawDataSize() == 0) {
        ERROR_MSG("Failed to save dominant texture map: empty texture names "
                  "and/or image\n");
        return false;
    }
    ...
    BinaryPtr pImageBlock =
      new BinaryBlock(&imgData.front(),
                      imgData.size(),
    ...
    memcpy(
      &data.front() + hdrData.size(), pImageBlock->data(), pImageBlock->len());

    BinaryPtr pSaveData = new BinaryBlock(
      &data.front(), data.size(), "Terrain/DominantTextureMap2/BinaryBlock");
...
/**
 *  This function loads a dominant texture map saved with
 * DominantTextureMap2::save.
 *
 *  @param dataSection   The DataSection to load from.
 *  @param error         If not null and an error occurs
 */
/*virtual*/ bool DominantTextureMap2::load(DataSectionPtr dataSection,
                                           BW::string*    error /* = NULL*/)
{
    ...
    if (pData == NULL) {
        if (error)
            *error =
              "Couldn't load dominant texture map: dataSection not binary.";
        return false;
    }
    ...
    if (pHeader->width_ == 0 || pHeader->height_ == 0) {
        if (error)
            *error = "Failed to load dominant texture map: image has zero "
                     "width/height.";
        return false;
    }
    ...
    DominantTextureMap::ImageType newImage(
      pHeader->width_, pHeader->height_, "Terrain/DominantTextureMap2");
    if (newImage.rawDataSize() > (size_t)pImageData->len()) {
        if (error)
            *error =
              "Image data size overflow when loading dominant texture map.";
        return false;
    }
