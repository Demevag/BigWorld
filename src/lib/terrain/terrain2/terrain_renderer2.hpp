...
namespace Terrain {
    ...
    class TerrainRenderer2 : public BaseTerrainRenderer
    {
      ...
      public:
        ...
        void addBlock(BaseRenderTerrainBlock* pBlock, const Matrix& transform);

        void drawAll(Moo::ERenderingPassType passType,
                     Moo::EffectMaterialPtr  pOverride = NULL,
                     bool                    clearList = true);

        void drawSingle(Moo::ERenderingPassType passType,
        ...
        void zBufferIsClear(bool zBufferIsClear)
        {
            zBufferIsClear_ = zBufferIsClear;
        }

        static uint8 getLoadFlag(float minDistance,
        ...
        bool IsHorizonShadowMapSuppressed() const
        {
            return isHorizonMapSuppressed;
        }
        void IsHorizonShadowMapSuppressed(bool isSuppressed)
        {
            isHorizonMapSuppressed = isSuppressed;
        }

        void setHorizonShadowsBlendDistance(Vector2 distances)
        {
            horizonShadowsBlendDistance = distances;
        }

        bool isSpecialModeEnabled_; // if set to true, TerrainRenderer renders
                                    // terrain kind as defined by
                                    // material_kind_classify.xml
        bool forceRenderLowestLod_; // temporary hack! when turned ON, the
                                    // lowest terrain LOD is rendered forcibly
      ...
      public:
        ...
        bool editorGenerateLodTexture(BaseRenderTerrainBlock* pBlock,
                                      const Matrix&           transform);
      ...
      private:
        ...
        // obsolete TERRAIN_OVERLAY_MODE_COLORIZE_GROUND_STRENGTH mode with
        // colors auto-calculated from materialKind of each tile
        typedef BW::map<BW::string, Vector4> TerrainOverlayColorMap;
        TerrainOverlayColorMap
          colorOverlays_; // only for
                          // TERRAIN_OVERLAY_MODE_COLORIZE_GROUND_STRENGTH

      public:
        uint terrainOverlayMode(); // routes call to
                                   // OptionsTerrain::terrainOverlayMode(),
                                   // modes are listed in OptionsHelper.hpp
        ...
        struct TextureOverlayLayer : public SafeReferenceCount
        {
            ...
            Vector4 color; // texture color will be used as monochrome and
                           // modulated with this color
            float alpha;   // will be multiplified by overlayAlpha
            bool  visible;
        };
        typedef SmartPointer<TextureOverlayLayer> TextureOverlayLayerPtr;

        BW::vector<TextureOverlayLayerPtr>
          textureOverlays; // textures ordered as they are drawn (last is
                           // topmost)
        ...
        Vector4 overlayBounds; // (minX, minZ, sizeX, sizeZ) used for
                               // worldPos->textureCoords mapping
      ...
      private:
        enum DrawMethod
        {
            ...
            DM_LOD_NORMALS /** Draw with low resolution normal map and lod
                              texture */
        };
        ...
        void setAOMapConstants(const TerrainBlock2* pBlock,
                               BaseMaterial*        pMaterial);
        ...
        void setTextureLayerConstants(const TerrainBlendsPtr& pBlend,
                                      ...
                                      bool                    blended = false,
                                      bool                    bumped  = false);
        void setSingleLayerConstants(BaseMaterial*        pMaterial,
                                     uint32               layerNo,
        ...
        void drawSingleInternal(Moo::ERenderingPassType passType,
                                BaseRenderTerrainBlock* pBlock,
                                const Matrix&           transform,
        ...
        float specularFresnelConstant() const
        {
            return specularInfo_.fresnelConstant_;
        };
    ...
    inline uint8 TerrainRenderer2::getLoadFlag(
      float minDistance,
      float textureBlendsPreloadDistance,
      float normalPreloadDistance)
    {
