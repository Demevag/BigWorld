    ...
    if (pData) {
...
#endif
        pLodMap_ =
          Moo::rc().createTextureFromFileInMemoryEx(pData->data(),
                                                    pData->len(),
                                                    D3DX_DEFAULT,
                                                    D3DX_DEFAULT,
                                                    D3DX_DEFAULT,
        ...
        if (pLodMap_) {
            ...
            if (levelDesc.Width != levelDesc.Height) {
                WARNING_MSG(
                  "TerrainLodMap2::load - lod texture must be square.\n");
                pLodMap_ = NULL;
                ret      = false;
            } else {
        ...
        WARNING_MSG(
          "TerrainLodMap2::load - unable to get lod map from data section\n");
    }

    return ret;
}

bool TerrainLodMap2::save(const DataSectionPtr& parentSection,
