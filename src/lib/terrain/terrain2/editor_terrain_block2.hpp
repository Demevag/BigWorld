...
namespace Terrain {
    ...
    class EditorTerrainBlock2 : public TerrainBlock2
    {
      public:
        ...
        size_t                     insertTextureLayer(uint32 blendWidth  = 0,
        ...
        void rebuildCombinedLayers(bool compressTextures           = true,
                                   bool generateDominantTextureMap = true,
                                   bool invalidateLodMap           = true);

        void rebuildAOMap(uint32 newSize);
        bool importAOMap(const Moo::Image<uint8>& srcImage,
                         uint                     top,
                         uint                     left);
        ...
        //-- Note: this methods just changes size of the shadow map but doesn't
        //regenerate it, more
        //--	   over it even doesn't inform EditorTerrainChunk about need to
        //rebuild shadow map.
    ...
    /**
     * This is a callback class that allows the EditorTerrainBlock2 to be
     * notified when the height map changes.
     */
