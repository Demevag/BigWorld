...
namespace // anonymous
{
     ...
     */
    void calcOutcodes(const BoundingBox&   bb,
     ...
     */
    void projectTriangleOnBoundingBox(WorldTriangle&     tri,
                                      const Vector3&     dir,
                                      Outcode            crossedBoundaries,
                                      const BoundingBox& bb)
    {
        ...
        tri = WorldTriangle(tri.v0() + triangleDelta,
     ...
     */
    bool clipPrism(WorldTriangle&     triangle,
                   Vector3&           extent,
                   const BoundingBox& bb)
    {
        BW_GUARD;
        ...
        WorldTriangle triEnd(tri.v0() + dir, tri.v1() + dir, tri.v2() + dir);
        ...
        if (!combinedOC) {
            if (combinedOCTri1 || combinedOCTri2) {
                ...
                if (destCrossing) {
                    projectTriangleOnBoundingBox(
                      triEnd, -dir, destCrossing, bb);
                    ocTri2         = 0;
                ...
                // If the outcode is zero we know that we are completely inside
                // the bounding box and do not need to clip.
                res = true;
            }
        }
...
{
    BW_GUARD;
    const Vector3 BOUNDING_BOX_EPSILON(
      QUADTREE_COLLISION_EPSILON, 0.1f, QUADTREE_COLLISION_EPSILON);
    ...
    if (xSize > minCellSize && zSize > minCellSize) {
        ...
        *pCellArray += 4;
        ...
        boundingBox_.setBounds(Vector3(xMin, y, zMin) - BOUNDING_BOX_EPSILON,
 ...
 */
bool TerrainQuadTreeCell::collide(const Vector3&            source,
                                  ...
                                  TerrainCollisionCallback* pCallback) const
{
    ...
    if (expandedBB.clip(s, e)) {
        ...
        if (children_) {
            Vector3 diff(
              1.f / boundingBox_.width(), 0, 1.f / boundingBox_.depth());

            Vector3 sNormalised = (s - boundingBox_.minBounds()) * diff;
            Vector3 eNormalised = (e - boundingBox_.minBounds()) * diff;

            uint32 startQuadrant =
              (sNormalised.x > 0.5f ? 1 : 0) + (sNormalised.z > 0.5f ? 2 : 0);
            uint32 endQuadrant =
              (eNormalised.x > 0.5f ? 1 : 0) + (eNormalised.z > 0.5f ? 2 : 0);

            // Check the quadrants that the ray passes through to see if we hit
            // something.
            if (startQuadrant == endQuadrant) {
                return children_[startQuadrant].collide(
                  source, extent, pOwner, pCallback);
            } else if ((startQuadrant & endQuadrant) == 0) {
                if (children_[startQuadrant].collide(
                      source, extent, pOwner, pCallback) ||
                    children_[startQuadrant ^ 1].collide(
                      source, extent, pOwner, pCallback) ||
                    children_[endQuadrant ^ 1].collide(
                      source, extent, pOwner, pCallback) ||
                    children_[endQuadrant].collide(
                      source, extent, pOwner, pCallback)) {
                    return true;
                }
            } else {
...
bool TerrainQuadTreeCell::collide(const WorldTriangle&      source,
                                  const Vector3&            extent,
                                  const TerrainHeightMap2*  pOwner,
                                  TerrainCollisionCallback* pCallback) const
{
    ...
    if (clipPrism(wt, wtExt, boundingBox_)) {
        ...
        if (children_) {
            ...
            float xStart =
              std::max(boundingBox_.minBounds().x, bbCombined.minBounds().x);
            float xEnd =
              std::min(boundingBox_.maxBounds().x, bbCombined.maxBounds().x);
            float zStart =
              std::max(boundingBox_.minBounds().z, bbCombined.minBounds().z);
            float zEnd =
              std::min(boundingBox_.maxBounds().z, bbCombined.maxBounds().z);

            return pOwner->hmCollide(
              source, wtExt, xStart, zStart, xEnd, zEnd, pCallback);
        }
    }
