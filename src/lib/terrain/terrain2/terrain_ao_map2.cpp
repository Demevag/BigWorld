  ...
  : size_(0)
{
    RESOURCE_COUNTER_ADD(
      ResourceCounters::DescriptionPool("TerrainAOMap2/Base",
...
bool TerrainAOMap2::init(const BW::string& terrainResource)
{
    ...
    DataSectionPtr pTerrainSection =
      BWResource::instance().openSection(terrainResource);
    if (pTerrainSection) {
        pAOMap_ =
          loadMap(pTerrainSection->openSection("ambientOcclusion"), size_);
 ...
 */
ComObjectWrap<DX::Texture> TerrainAOMap2::loadMap(
  ...
  uint32&               size)
{
    ...
    if (pMapSection && (pData = pMapSection->asBinary())) {
        const TerrainAOMapHeader* pHeader =
          reinterpret_cast<const TerrainAOMapHeader*>(pData->data());

        // Check the header for 8 bit png compression
        if (pHeader->magic_ == TerrainAOMapHeader::MAGIC &&
            ...
            if (!decompressImage(pImageData, image)) {
                ERROR_MSG("TerrainAOMap2::load - the ao map data could not be "
                          "compressed");
                return NULL;
            }
            ...
            pAOMap = Moo::rc().createTexture(image.width(),
                                             image.height(),
                                             1,
                                             usage,
                                             D3DFMT_L8,
            ...
            if (pAOMap) {
                ...
                if (SUCCEEDED(hr)) {
                    ...
                    pAOMap = NULL;
                    ERROR_MSG("TerrainAOMap2::load - unable to lock texture - "
                              "dx error: %lx",
                              hr);
                }
            }
 ...
 *
 */
bool TerrainAOMap2::import(const Moo::Image<uint8>& srcImg,
                           uint                     top,
                           uint                     left,
                           uint                     size)
{
    ...
    //--	Note: ao has range [0,1] where 0.5 means no ao influence, 0 - 0.5
    //values make ambient,
    //--		  darker 0.5 - 1 values make ambient brighter.
    Moo::Image<uint8> dstImg(size, size, 127);
    {
        //-- calculate src rect with 1 pixel border around it. This border
        //needed for doing correct
        //-- bilinear interpolation in pixel shader.
        uint32 outsidesSize = size;
        uint32 insidesSize  = outsidesSize - 1;

        //-- Note: chunks located on the edges of the space have 1 pixel border
        //outside the srcImg
        ...
        uint32 srcW = outsidesSize -
                      ((((left + 1) * insidesSize) > srcImg.width()) ? 1 : 0) -
                      ((left == 0) ? 1 : 0);
        uint32 srcH = outsidesSize -
                      ((((top + 1) * insidesSize) > srcImg.height()) ? 1 : 0) -
                      ((top == 0) ? 1 : 0);
...
bool TerrainAOMap2::save(const DataSectionPtr& pTerrainSection)
{
    ...
    return saveMap(
      pTerrainSection->newSection("ambientOcclusion", BinSection::creator()),
      pAOMap_);
}
 ...
 *
 */
bool TerrainAOMap2::createMap(const Moo::Image<uint8>&    srcImg,
                              ComObjectWrap<DX::Texture>& pMap)
{
    ...
    // Create the texture for the ao map
    ComObjectWrap<DX::Texture> pAOMap =
      Moo::rc().createTexture(srcImg.width(),
                              srcImg.width(),
                              1,
                              usage,
                              D3DFMT_L8,
    ...
    if (pAOMap) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            for (uint32 y = 0; y < desc.Height; y++) {
                memcpy(
                  pOutData, srcImg.getRow(y), sizeof(uint8) * srcImg.width());
                pOutData += rect.Pitch;
            }
            textureLock.unlockRect(0);
            ...
            pAOMap = NULL;
            ERROR_MSG(
              "TerrainAOMap2::import - unable to lock texture - dx error: %1x",
              hr);
            return false;
        }
    } else {
 ...
 *	@return true if successful
 */
bool TerrainAOMap2::saveMap(const DataSectionPtr&             pSection,
                            const ComObjectWrap<DX::Texture>& pMap)
{
    ...
    Moo::Image<uint8> aoData(surfaceDesc.Width,
                             surfaceDesc.Height,
                             pRawData,
                             false,
                             rect.Pitch,
    ...
    if (pData && pSection) {
        ...
        TerrainAOMapHeader* pHeader =
          reinterpret_cast<TerrainAOMapHeader*>(&data.front());
        ...
        BinaryPtr pSaveData = new BinaryBlock(
          &data.front(), data.size(), "Terrain/AOMap2/BinaryBlock");
        pSaveData->canZip(
