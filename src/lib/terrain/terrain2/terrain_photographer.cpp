...
class SurfaceHelper
{
  public:
    ...
    bool initOffscreenPlain(uint32    width,
                            uint32    height,
                            D3DFORMAT format,
                            D3DPOOL   pool)
    {
        BW_GUARD;
        pSurface_  = NULL;
        HRESULT hr = Moo::rc().device()->CreateOffscreenPlainSurface(
          width, height, format, pool, &pSurface_, NULL);
        return SUCCEEDED(hr);
    }
    ...
    bool initRenderTarget(uint32 width, uint32 height, D3DFORMAT format)
    {
        ...
        HRESULT hr = Moo::rc().device()->CreateRenderTarget(width,
                                                            height,
                                                            format,
                                                            D3DMULTISAMPLE_NONE,
                                                            0,
    ...
    operator DX::Surface*() { return pSurface_.pComObject(); }

    DX::Surface* operator->() { return pSurface_.pComObject(); }
...
TerrainPhotographer::TerrainPhotographer(TerrainRenderer2& drawer)
  : drawer_(drawer)
{
}
                                          ...
                                          const Matrix&     transform)
{
    ...
    if (pBasePhoto_) {
        ...
        if (pBasePhoto_->push()) {
            Moo::rc().device()->Clear(
              0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, 0, 1, 0);
            ...
            MF_ASSERT(textureQualitySettingPtr &&
                      "Missing texture quality option");
            int oldTextureQualityOption =
              textureQualitySettingPtr->activeOption();
            ...
            Matrix view;
            view.lookAt(Vector3(imageSize * 0.50375f, 0, imageSize * 0.49625f) +
            ...
            Terrain::BaseTerrainRenderer::instance()->enableOverlays(
              isOverlaysEnabled);
...
/**
 *	This method copies the photograph to a texture. If supplied texture is
 *empty, then new one is created with given format.
 *
