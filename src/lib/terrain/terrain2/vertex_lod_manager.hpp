...
namespace Terrain {
     ...
     */
    struct VertexLodArray
      : public BW::vector<VertexLodEntryPtr>
      , public SafeReferenceCount
    {
        VertexLodArray(uint32 size)
          : BW::vector<VertexLodEntryPtr>(size)
        {
        }
    };
    ...
    class VertexLodManager : public Resource<VertexLodArray>
    {
      public:
        ...
        virtual void stream(
          ResourceStreamType streamType = defaultStreamType());
        virtual bool load();

        const VertexLodEntryPtr& getLod(uint32 level,
      ...
      protected:
        void getTargetWorkingSet(uint32      lod,
     ...
     */
    inline void VertexLodManager::getCurrentWorkingSet(
     ...
     */
    inline void VertexLodManager::getRequestedWorkingSet(
      WorkingSet& workingSet) const
    {
        workingSet = requestedWorkingSet_;
    }
     ...
     */
    inline void VertexLodManager::getTargetWorkingSet(uint32      lod,
                                                      WorkingSet& workingSet,
                                                      uint32      topLod) const
    {
        // start is one before or topLod
        workingSet.start_ = lod;
        if (workingSet.start_ > topLod)
            workingSet.start_--;

        // end is one after or max
        workingSet.end_ = lod;
        if (workingSet.end_ < object_->size() - 1)
            workingSet.end_++;
    }
