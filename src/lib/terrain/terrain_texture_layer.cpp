...
static BasicAutoConfig<float> s_terrainTextureSpacing(
  "environment/terrainTextureSpacing",
  10.0f);
 ...
 *	@param epsilon			The floating point epsilon to compare texture
 *projections with.
 */
bool TerrainTextureLayer::sameTexture(TerrainTextureLayer const& other,
                                      float                      epsilon) const
{
    BW_GUARD;
    return sameTexture(
 ...
 *	@returns				True if the layer uses the given texture and
 *projections. The (x, y, z) components of the projections must be within
 *							epsilon to be considered as the same.
 */
bool TerrainTextureLayer::sameTexture(BW::string const& texture,
 ...
 */
/*static*/ ComObjectWrap<DX::Texture> TerrainTextureLayer::createBlendTexture(
  ...
  bool*                  smallBlended)
{
    ...
    D3DPOOL initialPool = isCompressed ? D3DPOOL_SCRATCH : D3DPOOL_MANAGED;
    ...
    pTexture = Moo::rc().createTexture(width,
                                       height,
                                       1,
                                       0,
                                       format,
                                       initialPool,
                                       "Terrain/TextureLayer/BlendTexture");
    ...
    if (isCompressed) {
        ...
        pCompressed =
          Moo::rc().createTexture(width / divisor,
                                  height / divisor,
                                  1,
                                  0,
                                  fmt,
        ...
        if (pCompressed) {
            ...
            HRESULT hr = D3DXLoadSurfaceFromSurface(dest.pComObject(),
                                                    NULL,
                                                    NULL,
                                                    src.pComObject(),
                                                    NULL,
 ...
 */
/*static*/ void TerrainTextureLayer::defaultUVProjections(Vector4& u,
                                                          Vector4& v)
{
