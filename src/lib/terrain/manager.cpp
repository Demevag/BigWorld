...
namespace {
    ...
    class NullTerrainRenderer : public Terrain::BaseTerrainRenderer
    {
      public:
        ...
        void addBlock(Terrain::BaseRenderTerrainBlock* pBlock,
...
namespace Terrain {
    ...
    bool Manager::init()
    {
        ...
        if (!MaterialKinds::init()) {
            ERROR_MSG(
              "Terrain::Manager::init: Unable to load material kinds data.\n");
            return false;
        }
        instance().pTerrain2Defaults_ =
          BWResource::instance().openSection(g_terrain2Settings);
        if (!instance().pTerrain2Defaults_) {
            ERROR_MSG("Terrain::Manager::init: Unable to load terrain2 "
                      "settings file specified by "
                      "system/terrain2: '%s'.\n",
    ...
    void Manager::currentRenderer(BaseTerrainRenderer* newInstance)
    {
        ...
        if (currentRenderer_)
#if ENABLE_WATCHERS
            currentRenderer_->isVisible(
              !BaseTerrainRenderer::isEnabledInWatcher);
#else
    ...
    BaseTerrainRenderer* Manager::currentRenderer()
    {
        ...
        if (currentRenderer_ == NULL) {
            // if it's initialised, but the instance is NULL, it means that no
            // valid terrain renderer set. Return the NullTerrainRenderer to
            // avoid a crash. This can happen when rendering the terrain when
            // the blocks haven't yet loaded, which are the ones that set the
            // terrain renderer instance.
    ...
    {
        BW_GUARD;
        if (lastVertexLodConsts_ == NULL ||
            lastVertexLodConsts_->blockSize != blockSize) {
            delete lastVertexLodConsts_;
