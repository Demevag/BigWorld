...
namespace {
    ...
    int readIntSetting(BW::string     sectionName,
    ...
    // a second section for defaults
    float readFloatSetting(BW::string     sectionName,
                           DataSectionPtr pSection,
                           DataSectionPtr pDefaultsSection)
    {
        if (pDefaultsSection)
            return pSection->readFloat(
              sectionName, pDefaultsSection->readFloat(sectionName));
        return pSection->readFloat(sectionName);
    }

    // This helper method reads a bool value from a datasection, using
    // a second section for defaults
    bool readBoolSetting(BW::string     sectionName,
                         DataSectionPtr pSection,
                         DataSectionPtr pDefaultsSection)
    {
        if (pDefaultsSection)
                            ...
                            BW::vector<float>& distances)
    {
        ...
        for (uint32 i = 0; i < numDistances; i++) {
            char buffer[64];
            bw_snprintf(
              buffer, ARRAY_SIZE(buffer), "lodInfo/lodDistances/distance%u", i);
            distances.push_back(
              readFloatSetting(buffer, pSection, pDefaultsSection));
        }
    }
                            ...
                            BW::vector<float>& distances)
    {
        ...
        for (uint32 i = 0; i < distances.size(); i++) {
            char buffer[64];
            bw_snprintf(
              buffer, ARRAY_SIZE(buffer), "lodDistances/distance%u", i);
 ...
 *  @param oldBlockSize	The old chunk edge length. (if changed from the World
 *Editor)
 *	@return true if successful
 */
bool TerrainSettings::init(float          blockSize,
                           ...
                           float          oldBlockSize)
{
    ...
    if ((version_ == TerrainSettings::TERRAIN2_VERSION) &&
        ...
        heightMapSize_ =
          readIntSetting("heightMapSize", settings, pTerrain2Defaults);
        heightMapEditorSize_ =
          settings->readInt("heightMapEditorSize", heightMapSize_);
        normalMapSize_ =
          readIntSetting("normalMapSize", settings, pTerrain2Defaults);
        holeMapSize_ =
          readIntSetting("holeMapSize", settings, pTerrain2Defaults);
        shadowMapSize_ =
          readIntSetting("shadowMapSize", settings, pTerrain2Defaults);
        blendMapSize_ =
          readIntSetting("blendMapSize", settings, pTerrain2Defaults);

        // calculate the number of vertex lods
        numVertexLods_ =
          log2RoundUp(std::max(heightMapSize_, heightMapEditorSize_));

        MF_ASSERT((1 << numVertexLods_) ==
                  std::max(heightMapSize_, heightMapEditorSize_));

        // read the lod distances
        readLevelDistances(
          settings, pTerrain2Defaults, numVertexLods_, savedVertexLodValues_);
        ...
        vertexLod_.startBias(
          readFloatSetting("lodInfo/startBias", settings, pTerrain2Defaults));
        vertexLod_.endBias(
          readFloatSetting("lodInfo/endBias", settings, pTerrain2Defaults));

        // read lod texture information
        m_savedLodTextureParams.m_start = readFloatSetting(
          "lodInfo/lodTextureStart", settings, pTerrain2Defaults);
        m_savedLodTextureParams.m_distance = readFloatSetting(
          "lodInfo/lodTextureDistance", settings, pTerrain2Defaults);
        m_savedLodTextureParams.m_blendPreloadDistance = readFloatSetting(
          "lodInfo/blendPreloadDistance", settings, pTerrain2Defaults);

        // normal map lod
        m_savedLodNormalParams.m_start = readFloatSetting(
          "lodInfo/lodNormalStart", settings, pTerrain2Defaults);
        m_savedLodNormalParams.m_start = std::max(
          m_savedLodNormalParams.m_start,
          m_savedLodTextureParams.m_start + m_savedLodTextureParams.m_distance);
        m_savedLodNormalParams.m_distance = readFloatSetting(
          "lodInfo/lodNormalDistance", settings, pTerrain2Defaults);
        m_savedLodNormalParams.m_normalPreloadDistance = readFloatSetting(
          "lodInfo/normalPreloadDistance", settings, pTerrain2Defaults);

        // bump mapping
        m_savedBumpParams.m_fadingStart = readFloatSetting(
          "lodInfo/bumpFadingStart", settings, pTerrain2Defaults);
        m_savedBumpParams.m_fadingDistance = readFloatSetting(
          "lodInfo/bumpFadingDistance", settings, pTerrain2Defaults);
        m_savedBumpParams.m_enabled = graphicsCfg.bumpMapping();

        // Height map lods
        defaultHeightMapLod_ =
          numVertexLods_ -
          log2RoundUp(std::min(heightMapSize_, heightMapEditorSize_));
    ...
    directSoundOcclusion_ =
      settings->readFloat("soundOcclusion/directOcclusion", 0.0f);
    reverbSoundOcclusion_ =
      settings->readFloat("soundOcclusion/reverbOcclusion", 0.0f);
    ...
    if (version_ == TerrainSettings::TERRAIN2_VERSION) {
        ...
        // Load base height map lod setting
        serverHeightMapLod_ =
          readIntSetting("lodInfo/server/heightMapLod",
                         settings,
                         Manager::instance().pTerrain2Defaults());
...
void TerrainSettings::applyLodTextureModifier(float modifier)
{
    ...
    m_lodTextureParams.m_distance =
      m_savedLodTextureParams.m_distance * modifier;
    m_lodTextureParams.m_blendPreloadDistance =
      m_savedLodTextureParams.m_blendPreloadDistance * modifier;
}
...
void TerrainSettings::applyLodNormalModifier(float modifier)
{
    ...
    m_lodNormalParams.m_start =
      max(m_lodNormalParams.m_start,
          m_lodTextureParams.m_start + m_lodTextureParams.m_distance);
    m_lodNormalParams.m_distance = m_savedLodNormalParams.m_distance * modifier;
    m_lodNormalParams.m_normalPreloadDistance =
      m_savedLodNormalParams.m_normalPreloadDistance * modifier;
}
...
void TerrainSettings::applyBumpMappingModifier(float modifier)
{
    ...
    m_bumpParams.m_fadingDistance =
      m_savedBumpParams.m_fadingDistance * modifier;
...
void TerrainSettings::initDefaults(float blockSize, float oldBlockSize)
{
    BW_GUARD;
    this->init(
      blockSize, Manager::instance().pTerrain2Defaults(), oldBlockSize);
}
...
void TerrainSettings::save(DataSectionPtr pSection)
{
    ...
    if (version_ == TerrainSettings::TERRAIN2_VERSION) {
        ...
        if (pLodSection) {
            ...
            pLodSection->writeFloat("lodTextureStart",
                                    m_savedLodTextureParams.m_start);
            pLodSection->writeFloat("lodTextureDistance",
                                    m_savedLodTextureParams.m_distance);
            pLodSection->writeFloat(
              "blendPreloadDistance",
              m_savedLodTextureParams.m_blendPreloadDistance);
            pLodSection->writeFloat("bumpFadingStart",
                                    m_savedBumpParams.m_fadingStart);
            pLodSection->writeFloat("bumpFadingDistance",
                                    m_savedBumpParams.m_fadingDistance);

            // normal map lod
            pLodSection->writeFloat("lodNormalStart",
                                    m_savedLodNormalParams.m_start);
            pLodSection->writeFloat("lodNormalDistance",
                                    m_savedLodNormalParams.m_distance);
            pLodSection->writeFloat(
              "normalPreloadDistance",
              m_savedLodNormalParams.m_normalPreloadDistance);
            pLodSection->writeFloat("detailHeightMapDistance",
                                    detailHeightMapDistance_);
    ...
    pSection->writeFloat("soundOcclusion/directOcclusion",
                         directSoundOcclusion_);
    pSection->writeFloat("soundOcclusion/reverbOcclusion",
                         reverbSoundOcclusion_);
}
...
void TerrainSettings::updateLodDistances(float oldBlockSize)
{
    ...
    uint32 savedLods    = std::min(numVertexLods_ - 1, oldSavedLods);
 ...
 *
 *  @returns		True if loading dominant texture maps is enabled, false
 *otherwise.
 */
 ...
 *
 *  @param doLoad		True to enable loading dominant texture maps, false to
 *disable.
 */
...
#else
    return ((heightMapSize_ > MINIMUM_TERRAIN_HEIGHT_MAP) ? s_topVertexLod_
                                                          : 0) +
           defaultHeightMapLod_;
...
void TerrainSettings::lodTextureStart(float value)
{
    ...
    applyLodTextureModifier(
      TerrainGraphicsOptions::instance()->lodTextureModifier());
    applyLodNormalModifier(
      TerrainGraphicsOptions::instance()->lodNormalModifier());
}
...
void TerrainSettings::bumpMapping(bool value)
{
    ...
    applyBumpMappingModifier(
      TerrainGraphicsOptions::instance()->lodBumpModifier());
}
