...
class Picker : public ReferenceCount
{
  public:
    virtual bool intersects(const Vector3& origin,
                            ...
                            Vector3*       intersection = NULL) const = 0;
...
{
  public:
    BoxPicker(const Vector3& minValues,
...
{
  public:
    CylinderPicker(const Vector3& position,
                   float          radius,
                   float          length,
                   bool           capFront  = true,
                   bool           capBack   = true,
  ...
  protected:
    bool intersectsDisc(const Vector3& discPosition,
                        float          discSign,
...
{
  public:
    ConePicker(const Vector3& position,
               float          radius,
               float          length,
               bool           cap       = true,
               const Matrix&  transform = Matrix::identity);
...
{
  public:
    DiscPicker(const Vector3& position,
               float          innerRadius,
               float          outerRadius,
               const Matrix&  transform = Matrix::identity);
    bool intersects(const Vector3& origin,
...
class SolidShapeMesh : public ReferenceCount
{
  public:
    ...
    void addSphere(const Vector3& pos,
                   ...
                   ShapePartPtr   callback = NULL);
    void addCone(const Vector3& pos,
                 float          radius,
                 float          length,
                 bool           cap      = true,
                 uint32         colour   = 0xffffffff,
                 ShapePartPtr   callback = NULL);
    void addCylinder(const Vector3& pos,
                     float          radius,
                     float          length,
                     bool           capFront = true,
                     bool           capBack  = true,
                     uint32         colour   = 0xffffffff,
                     ShapePartPtr   callback = NULL);
    void addBox(const Vector3& minValues,
                const Vector3& maxValues,
                uint32         colour   = 0xffffffff,
                ShapePartPtr   callback = NULL);
    void addDisc(const Vector3& position,
                 float          innerRadius,
                 float          outerRadius,
                 uint32         colour   = 0xffffffff,
                 ShapePartPtr   callback = NULL);
    ...
    ShapePart*    intersects(const Vector3& origin,
                             const Vector3& direction,
                             float*         distance = NULL);
