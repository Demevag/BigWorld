...
namespace MetaData {
    ...
    class Property : public SafeReferenceCount
    {
      ...
      public:
        ...
        virtual GeneralProperty* createProperty(bool readOnly,
                                                bool useFullDateFormat)  = 0;
        ...
        const PropertyDesc& desc() const { return desc_; }
    ...
    class PropertyDesc : public SafeReferenceCount
    {
      ...
      public:
        PropertyDesc(const BW::string&    name,
          ...
          , type_(type)
          , descSection_(descSection)
        {
        }
        const BW::string&    name() const { return name_; }
        const PropertyType*  type() const { return type_; }
        const BW::string&    description() const { return description_; }
        const DataSectionPtr descSection() const { return descSection_; }
    ...
    class Operation : public UndoRedo::Operation
    {
      ...
      public:
        Operation(MetaData& metaData)
          : UndoRedo::Operation(
              reinterpret_cast<size_t>(typeid(UndoRedo).name()))
          , restore_(metaData)
        {
        }
        ...
        virtual void undo()
        {
            ...
            UndoRedo::instance().add(new Operation(restore_.metaData()));
            restore_.restore();
        }
     ...
     *	its methods to get information of the outside environment. An
     *application have to provide a derived class of it to handle certain
