...
class ChunkItemRevealer : public PyObjectPlus
{
    Py_Header(ChunkItemRevealer, PyObjectPlus)
      ...
      public
      : ChunkItemRevealer(PyTypeObject* pType)
      : PyObjectPlus(pType)
    {
    }
...
{
    Py_Header(ChunkItemGroup, ChunkItemRevealer) public
      : ChunkItemGroup(ChunkItems    items = ChunkItems(),
...
class ChunkItemView : public ToolView
{
    Py_Header(ChunkItemView, ToolView)
      ...
      public : ChunkItemView(PyTypeObject* pType = &s_type_);
    ~ChunkItemView();
...
{
    Py_Header(ChunkItemBounds, ChunkItemView) public
      : ChunkItemBounds(PyTypeObject* pType = &s_type_);

    PY_RW_ATTRIBUTE_DECLARE(colour_, colour)

    static ChunkItemBounds* New(SmartPointer<ChunkItemRevealer> spRevealer,
                                uint32                          col,
                                float                           growFactor,
                                BW::string                      texture,
                                float                           offset,
    ...
    PY_AUTO_FACTORY_DECLARE(
      ChunkItemBounds,
      OPTARG(SmartPointer<ChunkItemRevealer>,
             NULL,
             OPTARG(uint32,
                    0xffffffff, // colour
                    OPTARG(float,
