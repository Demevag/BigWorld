 ...
 *	Constructor.
 */
PositionGizmo::PositionGizmo(int            disablerModifiers,
...
void PositionGizmo::rebuildMesh(bool force)
{
    ...
    if (snapToObstableEnabled())
        selectionMesh_.addSphere(
          Vector3(0, 0, 0),
          sphereRadius,
          0x7fffffff,
          new PositionShapePart(Moo::Colour(1, 1, 1, 1)));
    else
        sphereRadius = 0.0;
    ...
    selectionMesh_.addCylinder(
      ...
      true,
      0xffff0000,
      new PositionShapePart(Moo::Colour(1, 0, 0, 0), Vector3(0, 0, 1)));
    selectionMesh_.addCone(
      Vector3(0, 0, length),
      radius_ * 2.f,
      1,
      true,
    ...
    selectionMesh_.addCylinder(
      ...
      true,
      0xff00ff00,
      new PositionShapePart(Moo::Colour(0, 1, 0, 0), Vector3(1, 0, 0)));
    selectionMesh_.addCone(
      Vector3(0, 0, length),
      radius_ * 2.f,
      1,
    ...
    if (!snapToTerrainEnabled() && !isPlanar_) {
        selectionMesh_.addCylinder(
          ...
          true,
          0xff0000ff,
          new PositionShapePart(Moo::Colour(0, 0, 1, 0), Vector3(0, 1, 0)));
        selectionMesh_.addCone(
          Vector3(0, 0, length),
          radius_ * 2.f,
          1,
    ...
    selectionMesh_.addBox(min1,
                          max1,
                          0xffffff00,
                          new PositionShapePart(Moo::Colour(1, 1, 0, 0), 1));
    selectionMesh_.addBox(min2,
                          max2,
                          0xffffff00,
                          new PositionShapePart(Moo::Colour(1, 1, 0, 0), 1));
    ...
    if (!snapToTerrainEnabled() && !isPlanar_) {
        ...
        selectionMesh_.addBox(
          ...
          max1,
          0xffff00ff,
          new PositionShapePart(Moo::Colour(1, 0, 1, 0), 0));
        selectionMesh_.addBox(
          min2,
          max2,
          0xffff00ff,
        ...
        selectionMesh_.addBox(
          ...
          max1,
          0xff00ffff,
          new PositionShapePart(Moo::Colour(0, 1, 1, 0), 2));
        selectionMesh_.addBox(
          min2,
          max2,
          0xff00ffff,
...
bool PositionGizmo::draw(Moo::DrawContext& drawContext, bool force)
{
    ...
    if (pModel_ && currentPart_ && currentPart_->isFree() &&
        snapToObstableEnabled() && lastTool_ &&
        lastTool_ == ToolManager::instance().tool()) {
        return true;
    }
...
void PositionGizmo::click(const Vector3& origin, const Vector3& direction)
{
    ...
    if (currentPart_ != NULL) {
        ...
        if (matrixProxy_)
            toolFunctor =
              ToolFunctorPtr(new MatrixPositioner(matrixProxy_), true);
        else {
            if (SnapProvider::instance()->snapMode() ==
                ...
                if (currentPart_->isFree() &&
                    ...
                    toolFunctor = ToolFunctorPtr(
                      new MatrixMover(matrixProxy_, true, true), true);
                else
                    toolFunctor = ToolFunctorPtr(
                      new MatrixMover(matrixProxy_, false, false), true);
            } else
                toolFunctor = ToolFunctorPtr(
                  new MatrixMover(matrixProxy_, true, false), true);
        }
        ...
        if (currentPart_->isPlane() && isPlanar_ &&
            ...
            if (snapToObstableEnabled()) {
                Vector3 normal = getCoordModifier().applyVector(
                  currentPart_->plane().normal());
                normal.normalise();
        ...
        } else if (currentPart_->isPlane()) {
            Vector3 normal =
              getCoordModifier().applyVector(currentPart_->plane().normal());
            normal.normalise();
            ...
            Vector3 dir =
              getCoordModifier().applyVector(currentPart_->direction());
            dir.normalise();

            ToolPtr moveTool(
              new Tool(
                ToolLocatorPtr(new LineToolLocator(
                                 this->objectTransform().applyToOrigin(), dir),
                               true),
                NULL,
                toolFunctor),
              true);
...
bool PositionGizmo::snapToObstableEnabled() const
{
    ...
    return SnapProvider::instance()->snapMode() ==
             SnapProvider::SNAPMODE_OBSTACLE &&
           CurrentPositionProperties::properties().size() == 1;
}
