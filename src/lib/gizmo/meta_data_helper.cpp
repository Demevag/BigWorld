...
namespace MetaData {
    ...
    {
      public:
        PropertyOperation(MetaData& metaData, const BW::string& name)
                            ...
                            const BW::string& username)
    {
        ...
        if (!metaData[METADATA_CREATED_ON] || !metaData[METADATA_CREATED_BY] ||
                                ...
                                const BW::string& username)
    {
        ...
        if (!s_updating) {
            if (metaData[METADATA_MODIFIED_ON] &&
                metaData[METADATA_MODIFIED_BY]) {
                s_updating = true;

                if (!UndoRedo::instance().isUndoing()) {
                    UndoRedo::instance().add(
                      new PropertyOperation(metaData, METADATA_MODIFIED_ON));
                    UndoRedo::instance().add(
                      new PropertyOperation(metaData, METADATA_MODIFIED_BY));
                }
                ...
                // TODO: This should never happen. Make sure Xiaoming fixes
                // this.
                WARNING_MSG(
                  "Metadata::updateModificationInfo: metadata is empty.\n");
                return;
            }
        }
    }
