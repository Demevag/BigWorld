...
template <>
LocatorFactory::ObjectMap* LocatorFactory::pMap_;
...
void PlaneToolLocator::calculatePosition(const Vector3& worldRay, Tool& tool)
{
    ...
    transform_.translation(
      planeEq_.intersectRay(Moo::rc().invView().applyToOrigin(), worldRay));
}
 ...
 *	Constructor.
 */
TriPlaneToolLocator::TriPlaneToolLocator(const Vector3& pos,
                                         PyTypeObject*  pType)
  : ToolLocator(pType)
  , fulcrum_(pos)
{
...
void TriPlaneToolLocator::calculatePosition(const Vector3& worldRay, Tool& tool)
{
    ...
    /*if ( lastPlane != ( use - planeEq_ ) )
    {
        ...
        DEBUG_MSG( "Plane is now %s\n", lastPlane == 0 ? "x" : lastPlane == 1 ?
    "y" : "z" );
    }*/

    if (use) {
        transform_.setTranslate(
          use->intersectRay(Moo::rc().invView().applyToOrigin(), worldRay));
    }
}
 ...
 *	Constructor.
 */
AdaptivePlaneToolLocator::AdaptivePlaneToolLocator(const Vector3& pos,
                                                   PyTypeObject*  pType)
  : ToolLocator(pType)
  , fulcrum_(pos)
{
 ...
 *	@param	tool		The tool for this locator.
 */
void AdaptivePlaneToolLocator::calculatePosition(const Vector3& worldRay,
                                                 Tool&          tool)
{
    BW_GUARD;
    ...
    if (dist > 0.f) {
        ...
        transform_.setTranslate(
          peq.intersectRay(Moo::rc().invView().applyToOrigin(), worldRay));
    }
}
...
PyObject* AdaptivePlaneToolLocator::pyNew(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(args, "fff", &x, &y, &z)) {
        PyErr_SetString(PyExc_TypeError,
                        "AdaptivePlaneToolLocator() expects "
 ...
 *	Constructor.
 */
LineToolLocator::LineToolLocator(const Vector3& origin,
...
void LineToolLocator::calculatePosition(const Vector3& worldRay, Tool& tool)
{
    ...
    if (normal.length() != 0) {
        ...
        transform_.translation(
          origin_ + direction_.dotProduct(posOnPlane - origin_) * direction_);
    } else
...
PyObject* LineToolLocator::pyNew(PyObject* args)
{
    ...
    Vector3   vFirst, vOther;
    if (!PyArg_ParseTuple(args, "OO", &pFirst, &pOther) ||
        !Script::setData(pFirst, vFirst) || !Script::setData(pOther, vOther)) {
        PyErr_SetString(PyExc_TypeError,
                        "LineToolLocator() expects "
...
void PlaneChunkLocator::calculatePosition(const Vector3& worldRay, Tool& tool)
{
    ...
    Vector3 pos =
      pe.intersectRay(Moo::rc().invView().applyToOrigin(), worldRay);
