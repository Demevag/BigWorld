...
#define LOCATOR_FACTORY_DECLARE(CONSTRUCT)                                     \
    ...
    virtual LocatorFactory& factory()                                          \
    {                                                                          \
        return s_factory;                                                      \
    }                                                                          \
    static ToolLocator* s_create()                                             \
    {                                                                          \
...
{
    Py_Header(ToolLocator, PyObjectPlus) public
...
{
    Py_Header(OriginLocator, ToolLocator) public
...
{
    Py_Header(PlaneToolLocator, ToolLocator) public
      : PlaneToolLocator(PlaneEq*      pPlane = NULL,
...
{
    Py_Header(TriPlaneToolLocator, ToolLocator) public
      : TriPlaneToolLocator(const Vector3& pos   = s_lastPos,
                            PyTypeObject*  pType = &s_type_);
    virtual void calculatePosition(const Vector3& worldRay, Tool& tool);
...
{
    Py_Header(AdaptivePlaneToolLocator, ToolLocator) public
      : AdaptivePlaneToolLocator(const Vector3& pos,
                                 PyTypeObject*  pType = &s_type_);
    virtual void calculatePosition(const Vector3& worldRay, Tool& tool);
...
{
    Py_Header(LineToolLocator, ToolLocator) public
      : LineToolLocator(const Vector3& origin    = Vector3(0, 0, 0),
...
{
    Py_Header(ChunkToolLocator, ToolLocator) public
...
{
    Py_Header(PlaneChunkLocator, ChunkToolLocator) public
