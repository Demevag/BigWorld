...
void AngleGizmo::init()
{
    ...
    if (!inited_) {
        ...
        mesh_.addCone(Vector3(0, 0, 0),
                      radius,
                      length,
                      true,
                      0xffffff00,
                      new AngleShapePart(Moo::Colour(1, 1, 1, 0)));
        inited_ = true;
    }
}
...
void AngleGizmo::drawZBufferedStuff(bool force)
{
    ...
    coneMesh.addCone(Vector3(0.f, 0.f, coneLength),
                     coneRadius,
                     -coneLength,
                     false,
                     0xffffff00,
    ...
    if (ind.size()) {
        ind.fill(&coneMesh.indices().front(),
                 (uint32)coneMesh.indices().size());
        dib.unlock();
        ...
        if (dvb.lockAndLoad(&coneMesh.verts().front(),
                            static_cast<uint32>(coneMesh.verts().size()),
                            vbLockIndex) &&
            SUCCEEDED(dvb.set()) && SUCCEEDED(dib.indexBuffer().set())) {
            rc.drawIndexedPrimitive(D3DPT_TRIANGLELIST,
                                    vbLockIndex,
                                    0,
                                    (uint32)(coneMesh.verts().size()),
                                    ibLockIndex,
                                    (uint32)(coneMesh.indices().size()) / 3);
        }
    }
...
bool AngleGizmo::draw(Moo::DrawContext& drawContext, bool force)
{
    ...
    if (ind.size()) {
        ...
        if (verts) {
            memcpy(
              verts, &mesh_.verts().front(), vertexSize * mesh_.verts().size());
            dvb.unlock();
            ...
            rc.drawIndexedPrimitive(D3DPT_TRIANGLELIST,
                                    dvb.lockIndex(),
                                    0,
                                    (UINT)mesh_.verts().size(),
                                    dib.lockIndex(),
                                    (UINT)mesh_.indices().size() / 3);
        }
    }
...
bool AngleGizmo::intersects(const Vector3& origin,
...
void AngleGizmo::click(const Vector3& origin, const Vector3& direction)
{
    ...
    if (currentPart_ != NULL) {
        ToolPtr angleTool(
          new Tool(
            ToolLocatorPtr(new AdaptivePlaneToolLocator(
                             this->objectTransform().applyToOrigin()),
                           true),
            ToolViewPtr(
              new FloatVisualiser(
                pMatrix_, pAngle_, 0xffffff00, "angle", AngleFormatter::s_def),
              true),
            ToolFunctorPtr(new DynamicFloatDevice(pMatrix_, pAngle_, 0.4f),
                           true)),
          true);
