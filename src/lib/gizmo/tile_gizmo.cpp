...
class UniformScaleProxy : public FloatProxy
{
  public:
    ...
    virtual float get() const { return curTiling_; }

    virtual void set(float tiling, bool transient, bool addBarrier = true)
    {
        BW_GUARD;
 ...
 */
TileGizmo::TileGizmo(MatrixProxyPtr matrixProxy,
  ...
  , scaledMinGridSize_(0)
{
    ...
    uniformScaleGizmo_ = new RadiusGizmo(uniformScaleProxy_,
                                         matrixProxy,
                                         "Uniform U,V scale",
                                         0xffffffff,
                                         8.f,
...
/*virtual*/ bool TileGizmo::draw(Moo::DrawContext& drawContext, bool force)
{
    ...
    if (!force && enableModifier_ != ALWAYS_ENABLED &&
    ...
    scaledMinGridSize_ =
      (camToGizmoDist < START_SCALING_GRID_SIZE_DIST)
        ? MIN_GRID_SIZE
        : MIN_GRID_SIZE * (camToGizmoDist / START_SCALING_GRID_SIZE_DIST);
    ...
    if ((drawOptions_ & DRAW_SCALE) != 0) {
        ok &= scaleGizmo_->draw(drawContext, force) &&
 ...
 */
/*virtual*/ bool TileGizmo::intersects(Vector3 const& origin,
 ...
 */
/*virtual*/ void TileGizmo::click(Vector3 const& origin,
                                  Vector3 const& direction)
{
    BW_GUARD;
 ...
 */
/*virtual*/ void TileGizmo::rollOver(Vector3 const& origin,
                                     Vector3 const& direction)
{
    BW_GUARD;
