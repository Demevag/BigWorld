...
namespace BW {
    ...
    {
        listeners_.erase(
          std::remove(listeners_.begin(), listeners_.end(), pListener),
          listeners_.end());
    }
    ...
    void TickOperation::tick(SceneTypeSystem::RuntimeTypeID typeID,
                             ...
                             float                          dTime)
    {
        ITickOperationTypeHandler* pTypeHandler = this->getHandler(typeID);
        if (pTypeHandler) {
            pTypeHandler->doTick(pObjects, objectCount, dTime);
        }
    }

    void TickOperation::updateAnimations(SceneObject* pObjects,
                                         size_t       objectCount,
                                         float        dTime)
    {
        for (size_t i = 0; i < objectCount; ++i) {
