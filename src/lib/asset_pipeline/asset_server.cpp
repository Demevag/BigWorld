...
void AssetServer::broadcastAsset(const StringRef& asset)
{
    ...
    for (BW::map<HANDLE, SimpleThread*>::iterator it = pipeThreads_.begin();
         it != pipeThreads_.end();) {
...
void AssetServer::processCommand(HANDLE hPipe, const StringRef& command)
{
    ...
    if (strncmp(command.data(), ASSET_PIPE_LOCK, strlen(ASSET_PIPE_LOCK)) ==
        0) {
        lock(hPipe);
    } else if (strncmp(command.data(),
                       ASSET_PIPE_UNLOCK,
                       strlen(ASSET_PIPE_UNLOCK)) == 0) {
        unlock(hPipe);
    }
}
...
void AssetServer::serverThreadFunc(void* arg)
{
    ...
    assetServer.hCommandMutex_ =
      CreateMutexW(NULL, false, commandMutex.c_str());
...
void AssetServer::pipeThreadFunc(void* arg)
{
    ...
    while (!assetServer.terminating_) {
        ...
        for (StringArray::const_iterator requestIt = requests.begin();
