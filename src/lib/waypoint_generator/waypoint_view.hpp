...
class IWaypointView
{
  ...
  public:
    ...
    virtual void  getVertex(int      polygon,
                            int      vertex,
                            Vector2& v,
                            int&     adjacency,
                            bool&    adjToAnotherChunk) const = 0;
    virtual void  setAdjacency(int polygon, int vertex, int newAdj) {}

    virtual BW::vector<int> findWaypoints(const Vector3&,
                                          float girth) const = 0;

    void writeToSection(DataSectionPtr pSection,
                        Chunk*         pChunk,
                        float          defaultGirth,
                        bool           removeOld = true) const;
    void saveOut(Chunk* pChunk, float defaultGirth, bool removeAllOld = true);
    BinaryPtr asBinary(const Matrix& transformToLocal, float defaultGirth);
    ...
    class VertexRef
    {
      ...
      private:
        ...
        {
            view_->getVertex(
              pindex_, vindex_, pos_, adjNavPoly_, adjToAnotherChunk_);
    ...
    class PolygonRef
    {
      public:
        ...
        float minHeight() const { return view_->getMinHeight(index_); }
        float maxHeight() const { return view_->getMaxHeight(index_); }
        int   navPolySetIndex() const { return view_->getSet(index_); }

        uint      size() const { return view_->getVertexCount(index_); }
