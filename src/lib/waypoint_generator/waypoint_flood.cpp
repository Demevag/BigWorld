...
class LogFile
{
  ...
  public:
    ...
    void println(int indent, const char* format, ...)
    {
        ...
        vsprintf_s(buffer + indent * INDENT,
                   sizeof(buffer) - indent * INDENT,
                   format,
                   args);
        strcat(buffer, "\r\n");
 ...
 */
bool WaypointFlood::setArea(const Vector3& min,
                            const Vector3& max,
                            float          resolution)
{
    BW_GUARD;
...
bool WaypointFlood::checkBounds(Vector3 src, Vector3 dst)
{
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        if (pChunk_->owns(isrc) && pChunk_->owns(idst)) {
            return true;
        }
 ...
 */
int WaypointFlood::fill(const Vector3& seedPoint,
                        IProgress*     pProgress,
                        bool           debug)
{
    BW_GUARD;
    ...
    log.println(
      0, "get seed point %g %g %g", seedPoint.x, seedPoint.y, seedPoint.z);
    ...
    std::stack<std::pair<Vector3, int>, BW::vector<std::pair<Vector3, int>>>
            floodStack;
    ...
    if (!this->checkBounds(src, src)) {
        ...
        src.y = seedPoint.y + 0.1f;
        if (!(pPhysics_->findDropPoint(src, dropPoint) &&
              (src.y = dropPoint, true) && this->checkBounds(src, src))) {
            src.x -= seedPoint.x >= 0.0 ? resolution_ : -resolution_;
    ...
    log.println(1,
                "drop to %g --- src_x: %d  src_z: %d  src_index: %d",
                dropPoint,
                src_x,
                src_z,
                src_index);
    ...
    if (g == 0) {
        ...
        if (g == 0) {
            log.println(1,
                        "More than %d different heights on (%g, %g, %g)!",
                        MAX_HEIGHTS - 1,
                        src.x,
                        src.y,
                        src.z);
            ERROR_MSG("WaypointFlood: More than %d different heights on (%g, "
                      "%g, %g)!\n",
                      MAX_HEIGHTS - 1,
                      src.x,
                      src.y,
        ...
        log.println(
          1, "got new height: %g    grid: %d", hgtGrids_[g][src_index], g);
    } else
    ...
    if (adjGrids_[src_grid][src_index].all == 0)
        ...
        log.println(
          1, "already filled, all: %u", adjGrids_[src_grid][src_index].all);
        return 0;
    }
    ...
    while (!floodStack.empty()) {
        ...
        log.println(
          1, "************************ new point ************************");
        log.println(1,
                    "pop point: %g %g %g  src_x: %d  src_z: %d  src_index: %d "
                    "src_grid: %d  height: %g",
                    src.x,
                    src.y,
                    src.z,
        ...
        for (angle = 0; angle < 8; ++angle) {
            ...
            */
            ...
            if (fabs(dst.x - dst2.x) > FLOOD_FLOAT_FUDGE ||
                ...
                log.println(1,
                            "adjustMove for angle %d: dst_x: %d dst_z: %d dst: "
                            "%g %g %g  dst2: %g %g %g failed",
                            angle,
                            dst_x,
                            dst_z,
                            dst.x,
            ...
            log.println(1,
                        "adjustMove for angle %d: dst_x: %d dst_z: %d dst: %g "
                        "%g %g  dst2: %g %g %g succeeded",
                        angle,
                        dst_x,
                        dst_z,
                        dst.x,
            ...
            if (g == 0) {
                ...
                if (g == 0) {
                    log.println(
                      ...
                      dst.y,
                      dst.z);
                    ERROR_MSG("WaypointFlood: More than %d different heights "
                              "on (%g, %g, %g)!\n",
                              MAX_HEIGHTS - 1,
                              dst.x,
                              dst.y,
            ...
            if ((fabs(src.x - src2.x) > FLOOD_FLOAT_FUDGE ||
                ...
                log.println(
                  1,
                  "adjustMove Back: src: %g %g %g  src2: %g %g %g failed",
                  src.x,
                  src.y,
                  src.z,
                  src2.x,
            ...
            log.println(
              1,
              "adjustMove Back: src: %g %g %g  src2: %g %g %g succeeded",
              src.x,
              src.y,
              src.z,
              src2.x,
            ...
            log.println(1,
                        "adjGrids_[dst_grid][dst_index]: %u  src_grid: %d",
                        adjGrids_[dst_grid][dst_index].angle(angle ^ 4),
                        src_grid);
            ...
            if (adjGrids_[dst_grid][dst_index].all == 0) {
                ...
                if (!this->checkBounds(src, dst)) {
                    ...
                    log.println(1,
                                "Check Bounds Failure on %s - %s!",
                                src.desc().c_str(),
                                dst.desc().c_str());
                    continue;
                }
                log.println(1,
                            "dst_grid: %d  dst_index: %d  height: %g",
                            dst_grid,
                            dst_index,
                            dst.y);
            ...
            if (adjGrids_[src_grid][src_index].angle(angle) == 0 &&
                ...
                log.println(1,
                            "make link: src_grid: %d  src_index: %d angle: %d "
                            "-- dst_grid: %d  dst_index: %d angle: %d",
                            src_grid,
                            src_index,
                            angle,
                            dst_grid,
...
#ifdef DEBUG_WAYPOINT_FLOOD
                MF_ASSERT(adjGrids_[src_grid][src_index].angle(angle) ==
                          dst_grid);
                MF_ASSERT(adjGrids_[dst_grid][dst_index].angle(angle ^ 4) ==
                          src_grid);
#endif // DEBUG_WAYPOINT_FLOOD
            }
...
void WaypointFlood::flashFlood(float height)
{
    ...
    for (int x = 0; x < xsize_; ++x) {
        ...
        adjGrids_[hgtGrid][aindex].each.dr = 0;
        adjGrids_[hgtGrid][bindex].each.ul = 0;
        adjGrids_[hgtGrid][aindex].each.d  = 0;
        adjGrids_[hgtGrid][bindex].each.u  = 0;
        adjGrids_[hgtGrid][aindex].each.dl = 0;
        adjGrids_[hgtGrid][bindex].each.ur = 0;
    ...
    for (int z = 0; z < zsize_; ++z) {
        ...
        adjGrids_[hgtGrid][aindex].each.dl = 0;
        adjGrids_[hgtGrid][bindex].each.ur = 0;
        adjGrids_[hgtGrid][aindex].each.l  = 0;
        adjGrids_[hgtGrid][bindex].each.r  = 0;
        adjGrids_[hgtGrid][aindex].each.ul = 0;
        adjGrids_[hgtGrid][bindex].each.dr = 0;
    ...
    const int dx[8] = { 0, 1, 1, 1, 0, -1, -1, -1 };
    const int dy[8] = {
        xsize_, xsize_, 0, -xsize_, -xsize_, -xsize_, 0, xsize_
    };
    ...
    for (int a = 0; a < 8; a += 2) {
        ...
        adjGrids_[hgtGrid][index + dx[(a + 0) & 7] + dy[(a + 0) & 7]].angle(
          (a + 4) & 7, 0);
        adjGrids_[hgtGrid][index + dx[(a + 1) & 7] + dy[(a + 1) & 7]].angle(
          (a + 5) & 7, 0);
        adjGrids_[hgtGrid][index + dx[(a + 2) & 7] + dy[(a + 2) & 7]].angle(
          (a + 6) & 7, 0);
    }
}
            ...
            for (int z = 1; z < zsize_; ++z) {
                ...
                */
                ...
                if ((lg = origGrids[g][index].each.l) &&
                    ...
                    (ug = origGrids[rg][index - xsize_].each.u) && ug == g) {
    ...
    for (int g = smallestHeight_; g < MAX_HEIGHTS; ++g) {
        for (int x = 1; x < xsize_ - 1; ++x) {
            for (int z = 1; z < zsize_ - 1; ++z) {
                ...
                if (boundarySet_.find(std::make_pair(index, g)) !=
                    boundarySet_.end())
                    continue;
                ...
                for (int a = 0; a < 8; a += 2) {
                    ...
                    int og[9];
                    for (int i = 0; i < 9; ++i) {
                        ondex[i] = index + xdirs[a >> 1] * ((i % 3) - 1) +
                                   zdirs[a >> 1] * ((i / 3) - 1);
                        og[i] =
                          (anglemap[i] != -1)
                            ? adjGrids_[g][index].angle((a + anglemap[i]) & 7)
                            : g;
                    }
                    if (og[3] &&
                        origGrids[og[3]][ondex[3]].angle((1 + a) & 7) == 0 &&
                        og[3] &&
                        origGrids[og[3]][ondex[3]].angle((2 + a) & 7) != 0 &&
                        og[3] &&
                        origGrids[og[3]][ondex[3]].angle((3 + a) & 7) != 0 &&
                        og[3] &&
                        origGrids[og[3]][ondex[3]].angle((4 + a) & 7) ==
                          og[0] &&
                        og[1] &&
                        origGrids[og[1]][ondex[1]].angle((1 + a) & 7) == 0 &&
                        og[1] &&
                        origGrids[og[1]][ondex[1]].angle((0 + a) & 7) != 0 &&
                        og[1] &&
                        origGrids[og[1]][ondex[1]].angle((7 + a) & 7) != 0 &&
                        og[1] &&
                        origGrids[og[1]][ondex[1]].angle((6 + a) & 7) ==
                          og[0] &&
                        ...
                        (og[3] &&
                           origGrids[og[3]][ondex[3]].angle((0 + a) & 7) != 0 ||
                         og[1] && origGrids[og[1]][ondex[1]].angle((2 + a) &
                                                                   7) != 0)) {
                        ...
                        if (og[5])
                            adjGrids_[og[5]][ondex[5]].angle((6 + a) & 7, 0);
                        if (og[7])
                            adjGrids_[og[7]][ondex[7]].angle((4 + a) & 7, 0);
                    }
                    /*
                ...
                for (int angle = 0; angle < 8; ++angle) {
                    ...
                    if (dg && adjGrids_[dg][dindex].angle(angle ^ 4) != g) {
                        ERROR_MSG("WaypointFlood: Failed consistency check "
                                  ...
                                  angle,
                                  dg,
                                  adjGrids_[dg][dindex].angle(angle ^ 4));
                        return false;
                    }
                }
            }
...
bool WaypointFlood::writeTGA(const char* filename) const
{
    ...
    unsigned char hdr[] = { sizeof(min_) +
                              sizeof(resolution_), // extra data size
                            1,
                            1,
                            0,
                            0,
                            ...
                            0,
                            0,
                            (unsigned char)((xsize_ * MAX_HEIGHTS * 4) & 0xff),
                            (unsigned char)((xsize_ * MAX_HEIGHTS * 4) >> 8),
                            (unsigned char)((zsize_ * MAX_HEIGHTS * 4) & 0xff),
                            (unsigned char)((zsize_ * MAX_HEIGHTS * 4) >> 8),
                            8,
                            0 };
