...
bool ChunkView::load(Chunk* pChunk)
{
    ...
    if (pSection)
        pNavmesh = pSection->readBinary("worldNavmesh");
    if (pNavmesh) {
...
void ChunkView::parseNavPoly(DataSectionPtr pSection, int set, Chunk* pChunk)
{
    ...
    // If chunk provided, we want to convert heights from local -> global
    // coords.
    if (pChunk) {
    ...
    for (it = pSection->begin(); it != pSection->end(); it++) {
        if ((*it)->sectionName() == "vertex") {
            ...
            BW::string adjacentChunk =
              pVertex->readString("adjacentChunk", notFound);
...
void ChunkView::getVertex(int      polygon,
                          int      vertex,
                          Vector2& v,
                          int&     adjacency,
