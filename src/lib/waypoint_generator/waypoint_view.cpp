                                   ...
                                   bool           removeOld) const
{
    ...
    for (int p = 0; p < this->getPolygonCount(); p++) {
        ...
        // if this navPolySetIndex isn't found in the map, then it is inserted
        // with the default value of DataSectionPtr.
        DataSectionPtr& ourNavPolySet =
          navPolySetSections[polygon.navPolySetIndex()];
        ...
        Vector3 mhv = pChunk->transformInverse().applyPoint(
          Vector3(pChunk->centre().x, polygon.minHeight(), pChunk->centre().z));
        Vector3 hv = pChunk->transformInverse().applyPoint(
          Vector3(pChunk->centre().x, polygon.maxHeight(), pChunk->centre().z));
        ...
        for (unsigned int v = 0; v < vertexCount; v++) {
            ...
            // if valid nav poly number then add maxpolynum to it for writing
            // out..
            ...
            Vector3 nc = pChunk->transformInverse().applyPoint(
              Vector3(vr.pos().x, polygon.maxHeight(), vr.pos().y));

            // and write to datasection.
            pVertex->setVector3(Vector3(nc.x, nc.z, float(aNavPoly)));
        }
    }
 ...
 */
void IWaypointView::saveOut(Chunk* pChunk,
                            float  defaultGirth,
                            bool   removeAllOld)
{
    BW_GUARD;
    ...
    if (!removeAllOld && pChunkBin) {
        ...
        while (dataPtr < dataEnd) {
            ...
            if (aGirth == defaultGirth)
                offset |= (1 << 31);
            else
                keepSize += dataPtr - dataElt;
        ...
        if (keepSize > 0) {
            BinaryPtr combo = new BinaryBlock(
              NULL, pNewNavmesh->len() + keepSize, "BinaryBlock/IWaypointView");
    ...
    if (!pChunkBin) {
        DataSectionPtr pTemp = new BinSection(
          "", new BinaryBlock(NULL, 0, "BinaryBlock/IWaypointView"));
                                  ...
                                  float         defaultGirth)
{
    ...
    for (int p = 0; p < this->getPolygonCount(); p++) {
        ...
        // determine heights in local coords.
        Vector3 mhv = transformToLocal.applyPoint(
          Vector3(polygon[0].pos().x, polygon.minHeight(), polygon[0].pos().y));
        Vector3 Mhv = transformToLocal.applyPoint(
          Vector3(polygon[0].pos().x, polygon.maxHeight(), polygon[0].pos().y));
        ...
        for (unsigned int v = 0; v < vertexCount; v++) {
            ...
            Vector3 nc = transformToLocal.applyPoint(
              Vector3(vr.pos().x, polygon.maxHeight(), vr.pos().y));
            dmos << nc.x << nc.z;
    ...
    for (NavPolySets_asBinary_map::iterator it = navPolySets.begin();
         it != navPolySets.end();
         ++it) {
        dataSize += navPolySetEltSize + it->second.pCtrl->size() +
                    it->second.pData->size();
    }
