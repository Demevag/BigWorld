...
void XMLBillingSystem::initLogOnMapping(XMLDatabase* pDatabase)
{
    for (DataSection::iterator it = pLogonMapSection_->begin();
        ...
        if (typeID != INVALID_ENTITY_TYPE_ID) {
            DatabaseID entityID = pSection->read("entityID", DatabaseID(0));

            if (entityID == 0) {
                entityID = pDatabase->findEntityByName(
                  typeID, pSection->readString("entityName"));
            }

            logonMap_[pSection->readString("logOnName")] = LogOnMapping(
              EntityKey(typeID, entityID), pSection->readString("password"));
        } else {
 ...
 *	Override from BillingSystem
 */
void XMLBillingSystem::setEntityKeyForAccount(
