...
namespace controls {
    ...
    /**
     *  This function replaces srcColour in bitmap with dstColour.  It only
     * works with 8bpp, 24bpp and 32bpp images.  It has only been tested with
     * 8bpp images.
     *
    ...
    /**
     *  This function replaces the colour in the top-left pixel of the bitmap
     * with dstColour.  It only works with 8bpp, 24bpp and 32bpp images.  It has
     * only been tested with 8bpp images.
     *
     *  @param hbitmap      The bitmap to change.
     *  @param dstColour    The colour to replace with.
     */
     ...
     *  @param left     The left coordinate of the child control.  If this is
     * set to NO_RESIZE then the coordinate is left as is.
     *  @param top      The top coordinate of the child control.  If this is set
     *                  to NO_RESIZE then the coordinate is left as is.
     *  @param right    The right coordinate of the child control.  If this is
     * set to NO_RESIZE then the coordinate is left as is.
     *  @param bottom   The bottom coordinate of the child control.  If this is
     * set to NO_RESIZE then the coordinate is left as is.
     */
    void childResize(CWnd& wnd, int left, int top, int right, int bottom)
    {
        ...
        if (wnd.GetSafeHwnd() != 0) {
            ...
            if (parent != NULL) {
                ...
                wnd.SetWindowPos(
                  NULL, left, top, right - left, bottom - top, SWP_NOZORDER);
            }
        }
    }
    ...
    void childResize(CWnd& wnd, CRect const& extents)
    {
        ...
        childResize(
          wnd, extents.left, extents.top, extents.right, extents.bottom);
    }
    ...
    void childResize(CWnd& parent, UINT id, CRect const& extents)
    {
        ...
        childResize(
          parent, id, extents.left, extents.top, extents.right, extents.bottom);
    }
    ...
    /**
     *  Makes sure a dialog gets positioned completely inside the screen, near
     * to the original desired position
     *
     *  @param size           Size of the dialog/window to position.
     *  @param pt             Desired point where the dialog should be
     * positioned.
