...
namespace controls {
    ...
    /**
     *  This function 'subclasses' an existing control with the toolbar.  This
     * is not proper subclassing, as we hide the subclassed control, but borrow
     * its position.
     *
    ...
    BOOL DialogToolbar::LoadToolBarEx(UINT id, UINT disabledId /*= 0*/)
    {
        ...
        if (disabledId != 0) {
            ...
            controls::replaceColour(bitmap, ::GetSysColor(COLOR_BTNFACE));
            disabledImgList_.Create(bitmapInfo.bmHeight,
                                    bitmapInfo.bmHeight,
