...
namespace controls {
     ...
     */
    BOOL ColorTimeline::Create(DWORD                     dwStyle,
                               CRect                     rcPos,
                               CWnd*                     pParent,
    ...
    void ColorTimeline::addScheduleItem(ColorScheduleItem const& item)
    {
        ...
        newItem.heightPos_        = vertBorder + (int)(newItem.normalisedTime_ /
                                                unitHeightTimeIncrement_);
        newItem.timeStatic_       = new ColorStatic();
        newItem.timeStatic_->Create(_T("0"), 0, CRect(0, 0, 0, 0), this, 2);
    ...
    void ColorTimeline::totalScheduleTime(float time)
    {
        ...
        // Fill the new times:
        for (ColorScheduleItems::const_iterator iter = colorSchedule_.begin();
    ...
    void ColorTimeline::setColorScheduleItemSelected(float time)
    {
        ...
        colorScheduleIterSelected_ = colorSchedule_.end();
        for (ColorScheduleItems::iterator iter = colorSchedule_.begin();
             iter != colorSchedule_.end();
             ++iter) {
     ...
     */
    void ColorTimeline::setColorScheduleItemSelectedColor(Vector4 const& color,
                                                          bool redraw /*= true*/
    )
    {
        BW_GUARD;
        ...
        if (selItem != NULL) {
            ...
            --lastItem;
            if (wrap_ && (colorScheduleIterSelected_ == firstItem ||
    ...
    void ColorTimeline::OnPaint()
    {
        ...
        bitmap.CreateCompatibleBitmap(
          &dcPaint, rectClient.Width(), rectClient.Height());
        ...
        if (colorSchedule_.empty()) {
            ...
            for (; next != iterEnd; ++iter, ++next) {
                ...
                float   colorIncrement =
                  unitHeightTimeIncrement_ /
                  (next->normalisedTime_ - iter->normalisedTime_);
                for (int y = iter->heightPos_; y < next->heightPos_; ++y) {
                    if (showAlpha_) {
                        Vector4 alphaColor(
                          currentColor.w, currentColor.w, currentColor.w, 1.0f);
                        COLORREF fillColor = RGBtoCOLORREF(alphaColor);
                        dcMem.FillSolidRect(
                          alphaRect.left, y, alphaRect.Width(), 1, fillColor);
                    }
                    COLORREF fillColor = RGBtoCOLORREF(currentColor);
                    dcMem.FillSolidRect(
                      clrRect.left, y, clrRect.Width(), 1, fillColor);
                    currentColor +=
                      (next->color_ - iter->color_) * colorIncrement;
                }
            }
            ...
            if (lineAtTime_ >= 0.0f) {
                int ypos =
                  vertBorder + (int)((rectClient.Height() - 2 * vertBorder) *
                                     lineAtTime_ / totalScheduleTime_);
            ...
            // Draw the markers:
            for (iter = colorSchedule_.begin(); iter != colorSchedule_.end();
                 ++iter) {
        ...
        dcPaint.BitBlt(rectClient.left,
                       rectClient.top,
                       rectClient.Width(),
                       rectClient.Height(),
                       &dcMem,
    ...
    void ColorTimeline::OnLButtonDown(UINT nFlags, CPoint point)
    {
        ...
        CalcColorRect(clrRect);
        CalcAlphaRect(alphaRect);
        if (addColorAtLButton_ && ((showAlpha_ && alphaRect.PtInRect(point)) ||
            ...
            ++next;
            for (; next != iterEnd; ++iter, ++next) {
                if (iter->heightPos_ <= point.y && point.y < next->heightPos_) {
                    ...
                    newItem.normalisedTime_ =
                      (newItem.heightPos_ - vertBorder) *
                      unitHeightTimeIncrement_;

                    // Interpolate between the existing colours:
                    Vector4 colorDifference = next->color_ - iter->color_;
                    float   proportion =
                      (float)(newItem.heightPos_ - iter->heightPos_) /
                      (next->heightPos_ - iter->heightPos_);
                    newItem.color_ =
                      iter->color_ + proportion * colorDifference;

                    // Create the label:
                    newItem.timeStatic_ = new ColorStatic();
                    newItem.timeStatic_->Create(
                      _T("0"), 0, CRect(0, 0, 0, 0), this, 2);
                    newItem.timeStatic_->SetBkColour(
                      ColorStatic::TransparentBackground());
                    CWnd*  parent = GetParent();
                    ...
                    if (font != NULL)
                        newItem.timeStatic_->SetFont(font);
                    BW::wstring timeStr = timeToString(newItem.normalisedTime_ *
                                                       totalScheduleTime_);
                    newItem.timeStatic_->SetWindowText(timeStr.c_str());

                    colorScheduleIterSelected_ =
                      colorSchedule_.insert(next, newItem);
        ...
        if (foundOne) {
            ...
            point.y = Math::clamp(
              client.top + vertBorder, point.y, client.bottom - vertBorder);
            float ratio = (float)(point.y - vertBorder) /
                          (client.Height() - 2 * vertBorder);
            float time   = totalScheduleTime_ * ratio;
            CWnd* parent = GetParent();
            if (parent != NULL) {
                parent->SendMessage(WM_CT_SEL_TIME,
                                    (WPARAM)GetDlgCtrlID(),
                                    (LPARAM)(time * 10000));
            }
        }
    ...
    void ColorTimeline::OnMouseMove(UINT nFlags, CPoint point)
    {
        ...
        if (dragging_) {
            ...
            GetClientRect(client);
            point.y = Math::clamp(
              vertBorder + 1, (int)point.y, client.Height() - vertBorder - 1);
            ...
            colorScheduleIterSelected_->normalisedTime_ =
              ...
              unitHeightTimeIncrement_;
            BW::wstring timeStr = timeToString(
              colorScheduleIterSelected_->normalisedTime_ * totalScheduleTime_);
            colorScheduleIterSelected_->timeStatic_->SetWindowText(
              timeStr.c_str());
            ...
            --previous;
            while (colorScheduleIterSelected_->heightPos_ <=
                   previous->heightPos_) {
    ...
    BOOL ColorTimeline::OnSetCursor(CWnd* /*wnd*/,
                                    UINT /*hittest*/,
                                    UINT /*message*/)
    {
        BW_GUARD;
    ...
    void ColorTimeline::drawColorMark(CDC&                     dc,
                                      ...
                                      CRect const&             clientRect)
    {
        ...
        if (isSelected) {
            dc.FillSolidRect(clrLeft + indicatorClrWidth - selIndClrWidth - 2,
                             clrTop,
                             selIndClrWidth + 2,
                             indicatorClrHeight,
                             ::GetSysColor(COLOR_BTNFACE));
            dc.FillSolidRect(clrLeft + indicatorClrWidth - selIndClrWidth,
                             clrTop,
                             selIndClrWidth,
                             indicatorClrHeight,
                             RGB(102, 102, 102));
        }
        item.timeStatic_->SetWindowPos(&CWnd::wndTop,
                                       horizBorder,
    ...
    void ColorTimeline::UpdateParent(UINT message)
    {
        ...
        if (parent != NULL) {
            parent->SendMessage(
              message, (WPARAM)GetDlgCtrlID(), (LPARAM)GetSafeHwnd());
        }
    }
    ...
    void ColorTimeline::CalcColorRect(CRect& clrRect) const
    {
        ...
        int gradWidth =
          client.Width() - 2 * horizBorder - staticWidth - markWidth;
        int alphaX1 = 0;
        int clrX1   = 0;
        if (showAlpha_) {
    ...
    void ColorTimeline::CalcMarkRect(ColorScheduleItem const& item,
                                     CRect const&             clientRect,
