...
class DirDialog
{
  public:
    ...
    static CString basePath()
    {
        return s_basePath_;
    } // A getter for the default directory
    static BW::vector<CString> paths()
    {
        return s_paths_;
    } // A getter for the paths vector
  ...
  private:
    ...
    static int __stdcall browseCtrlCallback(HWND   hwnd,
                                            UINT   uMsg,
                                            LPARAM lParam,
                                            LPARAM lpData);
};
