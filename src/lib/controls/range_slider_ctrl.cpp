...
namespace controls {
    ...
    void RangeSliderCtrl::getRangeAreaRect(CRect* rect)
    {
        ...
        GetClientRect(rect);
        rect->DeflateRect(RANGESLIDER_X_OUT_MARGIN, RANGESLIDER_Y_OUT_MARGIN);
    }
    ...
    int RangeSliderCtrl::applyExponent(int val) const
    {
        ...
        return int(0.5f +
                   powf((float)val * powf((float)w, exponent_ - 1.0f),
                        1.0f / exponent_) +
                   (float)start);
    }
    ...
    int RangeSliderCtrl::removeExponent(int val) const
    {
        ...
        // add 0.5f to round to the next integer in case it contains decimals
        return int(0.5f +
                   powf((float)val, exponent_) /
                     powf((float)w, exponent_ - 1.0f) +
                   (float)start);
    }
    ...
    void RangeSliderCtrl::drawThumb(CDC& dc, int val, bool focused, bool down)
    {
        ...
        COLORREF fillColour =
          down ? GetSysColor(COLOR_BTNSHADOW) : GetSysColor(COLOR_BTNFACE);
        ...
        if (focused) {
            ...
            dc.FillSolidRect(x,
                             ...
                             3,
                             2,
                             down ? GetSysColor(COLOR_BTNTEXT)
    ...
    int RangeSliderCtrl::valFromPoint(CPoint point)
    {
        ...
        return (point.x - rect.left) * (GetRangeMax() - GetRangeMin() + 1) /
                 rect.Width() +
               GetRangeMin();
    }
    ...
    RangeSliderCtrl::ThumbType RangeSliderCtrl::thumbHitTest(CPoint point)
    {
        ...
        if (point.x >= xmin - RANGESLIDER_THUMB_WIDTH / 2 &&
            point.x <= xmin + RANGESLIDER_THUMB_WIDTH / 2)
            return THUMB_MIN;

        int xmax = coordFromVal(max);
        if (point.x >= xmax - RANGESLIDER_THUMB_WIDTH / 2 &&
            point.x <= xmax + RANGESLIDER_THUMB_WIDTH / 2)
            return THUMB_MAX;
    ...
    void RangeSliderCtrl::sendParentMsg(int msg)
    {
        ...
        parent->SendMessage(msg, 0, (LPARAM)this);
    }
    ...
    void RangeSliderCtrl::OnPaint()
    {
        ...
        dc.Draw3dRect(
          &rect, GetSysColor(COLOR_BTNSHADOW), GetSysColor(COLOR_BTNHIGHLIGHT));
    ...
    void RangeLimitSliderCtrl::setRangeLimit(float min, float max)
    {
        ...
        if (minval < minLimit_ || maxLimit_ < maxval) {
            SetRange(minval < minLimit_ ? minLimit_ : minval,
