...
namespace controls {
     ...
     */
    RowSizer::RowSizer(Orientation orientation,
                       CWnd*       parent,
                       uint32      containerID)
      : orientation_(orientation)
      , wnd_(parent)
     ...
     */
    void RowSizer::addChild(SizerPtr child, uint32 size, SizeUnits sizeUnits)
    {
        BW_GUARD;
    ...
    /**
     *  This gets the gap between the top/bottom (if vertically orientated) or
     * the left/right (if horizontally orientated) and the first sub-sizer.
     *
    ...
    /**
     *  This sets the gap between the top/bottom (if vertically orientated) or
     * the left/right (if horizontally orientated) and the first sub-sizer.
     *
    ...
    /*virtual*/ void RowSizer::onSize(CRect const& e)
    {
        ...
        if (wnd != NULL) {
            wnd->SetWindowPos(
              NULL, e.left, e.top, e.Width(), e.Height(), SWP_NOZORDER);
        }
        ...
        if (orientation_ == HORIZONTAL)
            weightSize = extents.Width() - fixedSize -
                         ((int)children_.size() - 1) * childGap_;
        else if (orientation_ == VERTICAL)
            weightSize = extents.Height() - fixedSize -
                         ((int)children_.size() - 1) * childGap_;
        ...
        for (size_t i = 0; i < children_.size(); ++i) {
            ...
            if (children_[i].sizeUnits_ == PIXELS) {
                ...
                if (orientation_ == HORIZONTAL) {
                    childExtents.right =
                      cx + children_[i].size_ * weightSize / sumWeights;
                    childExtents.bottom = extents.bottom;
                } else if (orientation_ == VERTICAL) {
                    childExtents.right = extents.right;
                    childExtents.bottom =
                      cy + children_[i].size_ * weightSize / sumWeights;
                }
            }
