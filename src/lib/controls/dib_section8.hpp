...
namespace controls {
    ...
    {
      public:
        void getPalette(COLORREF* values,
                        uint32    start = 0,
                        uint32    count = 256) const;
        void setPalette(COLORREF const* values,
                        uint32          start = 0,
                        uint32          count = 256);
    };
...
namespace controls {
    ...
    {
        return ((width() + 3) / 4) * 4 *
               sizeof(uint8); // round up to nearest DWORD
    }

    inline /*virtual*/ bool DibSection<uint8>::createBuffer(uint32  w,
                                                            uint32  h,
                                                            uint8*& buffer,
                                                            bool&   owns,
                                                            size_t& stride,
                                                            bool&   flipped)
    {
        ...
        if (w != 0 && h != 0) {
            ...
            hbitmap_ = ::CreateDIBSection(0, // Use desktop DC
                                          &bmi_,
                                          DIB_RGB_COLORS,
                                          (void**)&buffer,
      ...
      uint32                h)
    {
        for (uint32 y = 0; y < h; ++y) {
            ...
            uint8*       q = p + w;
            uint8 const* src =
              reinterpret_cast<uint8 const*>(lockedRect.pBits) +
              y * lockedRect.Pitch;
            for (; p != q; ++p) {
                uint8 r = *src++;
                uint8 g = *src++;
                uint8 b = *src++;
                ++src; // skip alpha
                *p = (uint8)((54 * r + 182 * g + 19 * b) /
                             255); // just get luminance
            }
        }
    }
}
