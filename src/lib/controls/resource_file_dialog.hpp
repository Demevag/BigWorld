...
{
  public:
    ResourceFileDialog(BOOL    bOpenFileDialog,
      ...
      : CFileDialog(bOpenFileDialog,
                    lpszDefExt,
                    lpszFileName,
                    dwFlags | OFN_EXPLORER | OFN_NOCHANGEDIR,
                    lpszFilter,
                    pParentWnd)
    {
    }
    virtual BOOL OnFileNameOK()
    {
        ...
        BW::string filename =
          BWResource::dissolveFilename((LPCTSTR)GetPathName());
        if (filename[1] == ':') // absolute folder
        {
            BW::string msg =
              L("CONTROLS/RESOURCE_FILE_DIALOG/MUST_BE_IN_RESOURCE_DIR");
