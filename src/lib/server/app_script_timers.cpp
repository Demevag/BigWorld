...
namespace {
    ...
    class TimerFini : public Script::FiniTimeJob
    {
      public:
        virtual void fini()
        {
            // Currently, this is not needed as the timeQueue is destroyed and
            // all timers cancelled before Script::fini. g_pTimers should
            // already be NULL.
    ...
    {
      public:
        ScriptTimerHandler(PyObject* pObject)
          : pObject_(pObject)
        {
        }
    ...
    /**
     *	This method is exposed to scripting. It is used to register a timer
     *function to be called with a given period.
     */
    PyObject* py_addTimer(PyObject* args)
    {
        ...
        int id = ScriptTimersUtil::addTimer(
          &g_pTimers, initialOffset, repeatOffset, userArg, pHandler);
     ...
     *	Note: This method should only be used in rare situations. It is better
     *to use Entity.addTimer as entity timers are backed up and restored on
     *BaseApp failure.
     *
     *	The function addTimer registers a function to be called after
     *	"initialOffset" seconds. Optionally it can be repeated every
     *"repeatOffset" seconds, and have optional user data (an integer that is
     *simply passed to the timer function) "userArg".
     *
     *	The callback function must accept 2 arguments; the internal id of the
     *timer (useful to remove the timer via the "delTimer" method), and the
     *optional user supplied integer "userArg".
     *
     *	Example:
     *	@{
     *	import BigWorld
     ...
     *
     *	# call onTimer after 5 seconds, and then every 1 second with a userArg
     *of 9 BigWorld.addTimer( onTimer, 5, 1, 9 )
     *
     ...
     *	@}
     *
     *	@param callback Specifies the timer handler to be used. This is a
     *function that takes 2 integer parameters.
     *	@param initialOffset initialOffset is a float and specifies the interval
     *(in seconds) between registration of the timer, and the time of first
     *execution.
     *	@param repeatOffset=0 repeatOffset is an optional float and specifies
     *the interval (in seconds) between repeated executions after the first
     *execution. BigWorld.delTimer must be used to remove the timer or this will
     *repeat continuously.
     *	@param userArg=0 userArg is an optional integer which specifies an
     *optional user supplied value to be passed to the callback method.
     *	@return Integer. This function returns the internal id of the timer.
     *This can be used with BigWorld.delTimer to remove the timer.
     */
