                                              ...
                                              TimerHandler* pHandler)
{
    ...
    TimerHandle timerHandle =
      g_pTimeQueue->add(initialTicks,
                        repeatTicks,
                        pHandler,
                        reinterpret_cast<void*>(userArg),
                        "ScriptTimer");
...
void ScriptTimers::writeToStream(BinaryOStream& stream) const
{
    ...
    while (iter != map_.end()) {
        ...
        MF_VERIFY(
          g_pTimeQueue->getTimerInfo(iter->second, time, interval, pUser));
 ...
 */
void ScriptTimers::readFromStream(BinaryIStream& stream,
                                  uint32         numTimers,
                                  TimerHandler*  pHandler)
{
    for (uint32 i = 0; i < numTimers; ++i) {
        ...
        map_[timerID] = g_pTimeQueue->add(time,
                                          interval,
                                          pHandler,
 ...
 */
ScriptTimers::Map::const_iterator ScriptTimers::findTimer(
...
namespace ScriptTimersUtil {
                                    ...
                                    TimerHandler*  pHandler)
    {
        ...
        return rpTimers->addTimer(
          initialOffset, repeatOffset, userArg, pHandler);
    }
     ...
     */
    void readFromStream(ScriptTimers** ppTimers,
