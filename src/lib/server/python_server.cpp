  ...
  , multiline_()
{
    ...
    dispatcher_.registerFileDescriptor(
      socket_.fileno(), this, "PythonConnection");
...
void PythonConnection::handleLine()
{
    ...
    if (!currentLine_.empty()) {
        ...
        if (currentLine_[currentLine_.length() - 2] == ':') {
            ...
            // PyObject * pRes = PyRun_String( (char *)currentLine_.c_str(),
            // true );
            active_ = false;
        }
    }
 ...
 *
 *	@param dispatcher The event dispatcher with which to register file
 *descriptors.
                           ...
                           const char*               ifspec)
{
    ...
    listener_.setnonblocking(true);
               ...
               // int val = 1;
    // setsockopt(listener_, SOL_SOCKET, SO_REUSEADDR, (const char*)&val,
    // sizeof(val));
#endif

    u_int32_t ifaddr = ip;
    if (ifspec) { // try binding somewhere else if ifspec != 0
    ...
    pDispatcher_->registerFileDescriptor(
      listener_.fileno(), this, "PythonConnection");
