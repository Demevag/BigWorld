 ...
 */
bool PluginLibrary::loadAllFromDirRelativeToApp(bool        prefixWithAppName,
                                                const char* partialDir)
{
#if defined(__unix__)
    char rlbuf[512];
    ...
    for (StringSet::iterator it = dllNames.begin(); it != dllNames.end();
         it++) {
    ...
    for (StringSet::iterator it = dllNames.begin(); it != dllNames.end();
        ...
        HMODULE handle =
          LoadLibraryEx(dllName.c_str(), NULL, LOAD_WITH_ALTERED_SEARCH_PATH);
