 ...
 * in the registry
 *
 * @parm LPTSTR | pServiceDisplayName = NULL | "Friendly" named of the service,
 * as displayed by the Services Applet and 'net start'.  If pServiceDisplay name
 * is NULL, it assumes the same string a pServiceName
 *
 *
 * @rdesc None.
 *
...
CService::CService(LPTSTR pServiceName,
                   LPTSTR pServiceDisplayName /* = NULL */,
                   DWORD  svcType /*= SERVICE_WIN32_OWN_PROCESS*/)
  : m_isService(FALSE)
  , m_isInteractive(FALSE)
  , m_neverService(FALSE)
  , m_StatusHandle(0)
  ...
  , m_pDispatchTable(NULL)
{
    // set name of service
    ASSERT(pServiceName);
    m_pServiceName = (LPTSTR)HeapAlloc(
      GetProcessHeap(), 0, sizeof(TCHAR) * (lstrlen(pServiceName) + 1));
    ...
    if (pServiceDisplayName != NULL) {
        m_pServiceDisplayName =
          (LPTSTR)HeapAlloc(GetProcessHeap(),
                            0,
                            sizeof(TCHAR) * (lstrlen(pServiceDisplayName) + 1));
...
{
#if TRACE_SERVICE
    if ((g_hFile != INVALID_HANDLE_VALUE) && (g_pServiceName == m_pServiceName))
        CloseHandle(g_hFile);
#endif // TRACE_SERVICE
 ...
 * @doc ALL EXTERNAL CSERVICE
 *
 * @api DWORD | CService::Checkpoint | Notifies the Service Control Manager
 * (SCM) that progress is being made while the service is in a "pending" state.
 *
 ...
 *
 * @comm If the process is not running as a NT service, the function just
 * returns.
 *
 ...
 * @doc ALL EXTERNAL CSERVICE
 *
 * @api DWORD | CService::UpdateStatus | Notifies the Service Control Manager
 * (SCM) that the service has changed state or encountered an error
 *
 ...
 *        SetServiceStatus error codes ( Win32 API )
 *
 * @comm While the service is in a SERVICE_START_PENDING state, all accepted
 * service control functions are disabled.
 *
 ...
 *
 * If a service is reported as SERVICE_RUNNING or SERVICE_STOPPED, the
                             ...
                             DWORD dwError /* = NO_ERROR */)
{
    ...
    ASSERT(
      dwError == NO_ERROR ||
      ((dwState == SERVICE_STOP_PENDING) || (dwState == SERVICE_STOPPED)) &&
...
DWORD CService::Start(int argc, char** argv)
{
    ...
    ASSERT(g_pService ==
           NULL); // only allow one active service object per process
    g_pService = this;
 ...
 * @doc ALL EXTERNAL CSERVICE
 *
 * @api void | CService::StartWithExternalDispatch | Begins the real startup
 * work of the service
 *
 ...
 * @rdesc None
 *
 * @comm Derived classes are responible for calling Checkpoint while this
 * service starting and stopping.  They are also responsible for calling
 * UpdateStatus with SERVICE_RUNNING when the service is actually running.  The
 * function must also return after it has been signalled to stop
 *
 * @xref CService::onStop CService::UpdateStatus CService::Checkpoint
 * CService::Start
 */
 ...
 *
 * Other service control handlers should be implemented with the same thoughts
 * in mind. In addition, a control handler may what to change the controls that
 * the service accepts via SetControlsAccepted
 *
 * @xref CService::onStop CService::UpdateStatus CService::Checkpoint
 * CService::Start CService::SetControlsAccepted
 */
 ...
 * @comm Derived classes are responible for calling Checkpoint while this
 * service starting and stopping.  They are also responsible for calling
 * UpdateStatus with SERVICE_RUNNING when the service is actually running.
 *
 ...
 *
 * @api static void | CService::service_main | Internal wrapper for
 * CService::Main which is called by the service control dispatcher
 *
...
void WINAPI CService::service_main(DWORD dwArgc, LPTSTR* lpArgv)
{
...
#if TRACE_SERVICE
    {
        ...
        DWORD cbWritten;
        wsprintf(szBuf,
                 "Beginning CService::service_main %s\n",
    ...
    // register our service control handler:
    g_pService->m_StatusHandle = ::RegisterServiceCtrlHandler(
      g_pService->m_pServiceName, CService::service_ctrl);
 ...
 * @rdesc None
 *
 * @comm This functions just passes the control code of the the various onXXX
 * service control handlers.  All service control handlers are responsible for
 * changing state via UpdateStatus or changing the accepted service control list
 * via SetControlsAccepted themselves.
 *
 * @xref CService::onStop CService::SetControlsAccepted CService::UpdateStatus
 */
 ...
 */
DWORD CService::Install(
  LPTSTR pDependencies /* = NULL */,
  ...
  LPTSTR pszPath /*= NULL*/)
{
    ...
    if (schSCManager) {
        ...
        if (schService) {
            ...
            if (r != 0) {
                wsprintf(szBuf,
                         TEXT("%s installed and started.\n"),
                         m_pServiceDisplayName);
            } else {
                wsprintf(szBuf,
                         TEXT("%s installed but startup failed.\n"),
                         m_pServiceDisplayName);
            }
            ...
            DWORD bytes;
            wsprintf(
              szBuf, TEXT("CreateService failed - 0x%x\n"), GetLastError());
            WriteConsole(GetStdHandle(STD_OUTPUT_HANDLE),
                         szBuf,
                         lstrlen(szBuf) + 1,
                         &bytes,
...
DWORD CService::Remove()
{
    ...
    if (schSCManager) {
        schService =
          OpenService(schSCManager, m_pServiceName, SERVICE_ALL_ACCESS);
        ...
        if (schService) {
            ...
            if (DeleteService(schService))
                ...
                wsprintf(
                  szBuf, TEXT("DeleteService failed - 0x%x\n"), GetLastError());
            ...
            wsprintf(
              szBuf, TEXT("OpenService failed - 0x%x\n"), GetLastError());
            WriteConsole(GetStdHandle(STD_OUTPUT_HANDLE),
                         szBuf,
                         lstrlen(szBuf) + 1,
                         &bytes,
 ...
 * executing as a console process or a service by enumerating the process token.
...
DWORD CService::CheckSIDs(void)
{
    ...
    // open the token
    if (!::OpenProcessToken(
          ::GetCurrentProcess(), TOKEN_QUERY, &hProcessToken)) {
        dwRet = ::GetLastError();
        goto closedown;
    }
    ...
    if (!::GetTokenInformation(
          hProcessToken, TokenGroups, groupInfo, groupLength, &groupLength)) {
    ...
    if (!AllocateAndInitializeSid(
          &siaNt, 1, SECURITY_SERVICE_RID, 0, 0, 0, 0, 0, 0, 0, &pServiceSid)) {
        dwRet = ::GetLastError();
        goto closedown;
    }
    ...
    if (!(m_isService || m_isInteractive)) {
        //
        //  Neither Interactive or Service was present in the current users
        //  token, This implies that the process is running as a service, most
        //  likely running as LocalSystem.
        //
        ...
        wsprintf(szKey,
                 ...
                 m_pServiceName);
        if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, szKey, 0, KEY_READ, &hkey) ==
            ERROR_SUCCESS) {
            DWORD dwType = 0;
            DWORD dwSize = sizeof(DWORD);
            if (RegQueryValueEx(
                  hkey, TEXT("Type"), NULL, NULL, (LPBYTE)&dwType, &dwSize) ==
                ERROR_SUCCESS) {
...
closedown:
...
#if TRACE_SERVICE
    {
        ...
        wsprintf(
          szBuf,
          "CService::CheckSIDs - Interactive:%ld  Service:%ld  Error:%lu\n",
          m_isInteractive,
          m_isService,
          dwRet);
        WriteFile(g_hFile, szBuf, lstrlen(szBuf), &cbWritten, NULL);
 ...
 * @doc ALL INTERNAL CSERVICE
 *
 * @api BOOL | CService::control_handler | Implements the console control
 * handler
 *
