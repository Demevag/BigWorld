...
#define SERVER_APP_HEADER_CUSTOM_NAME(APP_NAME, CONFIG_PATH)                   \
    ...
    virtual const char* getConfigPath() const                                  \
    {                                                                          \
        return #CONFIG_PATH;                                                   \
    }
...
class ServerApp
{
  public:
    ...
    static APP_TYPE& getApp(const Mercury::UnpackedMessageHeader& header)
    {
        return *static_cast<APP_TYPE*>(
          reinterpret_cast<ServerApp*>(header.pInterface->pExtensionData()));
    }
  ...
  protected:
    ...
    static bool             bindToPrescribedPort(
                  Mercury::NetworkInterface& networkInterface,
                  Mercury::TCPServer&        server,
