...
class RendezvousHashSchemeTestFixture
{
  public:
    ...
    double calculateMeanAbsoluteDeviationRatio(
      const char*              name,
 ...
 */
template <typename ALGORITHM>
RendezvousHashSchemeTestFixture<ALGORITHM>::RendezvousHashSchemeTestFixture()
  : hashScheme_()
  , testData_()
{
 ...
 */
template <typename ALGORITHM>
void RendezvousHashSchemeTestFixture<ALGORITHM>::runAllTests(TestResult& result,
                                                             const char* name)
{
 ...
 */
template <typename ALGORITHM>
void RendezvousHashSchemeTestFixture<ALGORITHM>::basicTest(TestResult& result_,
                                                           const char* m_name)
{
    ...
    // Check balance.
    CHECK(this->calculateMeanAbsoluteDeviationRatio(m_name) < MAD_MAX_RATIO);
    ...
    for (iter = begin; iter != end; ++iter) {
        ...
        CHECK(hashScheme_.mappedValueForBucket(iter->first) == iter->second);
  ...
  const char* m_name)
{
    ...
    for (DatabaseID key = 0; static_cast<size_t>(key) < NUM_ITERATIONS; ++key) {
        if (oldHash.bucketForKey(key) != removeBucket) {
            CHECK(oldHash.bucketForKey(key) == hashScheme_.bucketForKey(key));
        } else {
  ...
  const char* m_name)
{
    ...
    for (DatabaseID key = 0; static_cast<size_t>(key) < NUM_ITERATIONS; ++key) {
        ...
        CHECK(hashScheme_.valueForKey(key) == outHash.valueForKey(key));
...
template <typename ALGORITHM>
double
RendezvousHashSchemeTestFixture<ALGORITHM>::calculateMeanAbsoluteDeviationRatio(
  const char* testName,
  size_t      numToTest /* = NUM_ITERATIONS */,
  ...
  /* = Key() */) const
{
    ...
    const double meanAbsoluteDeviation =
      sumAbsoluteDeviation / hashScheme_.size();

    HACK_MSG(
      "%s: meanAbsoluteDeviation = %.03lf\n", testName, meanAbsoluteDeviation);
