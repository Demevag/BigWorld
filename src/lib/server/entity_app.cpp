 ...
 *	Constructor.
 */
EntityAppTimeQueue::EntityAppTimeQueue(int updateHertz, EntityApp& entityApp)
  ...
  , tickStatsPeriod_(0) // Set below
{
    ...
    tickStatsPeriod_ = uint32(tickStatsPeriod * EntityAppConfig::updateHertz());
}
...
void EntityApp::tickStats()
{
    if (tickStatsPeriod_ > 0) {
        ...
        EntityMemberStats::Stat::tickSome(time_ % tickStatsPeriod_,
                                          tickStatsPeriod_,
                                          double(tickStatsPeriod_) /
                                            EntityAppConfig::updateHertz());
    }
}
...
void EntityApp::onSignalled(int sigNum)
{
    ...
    switch (sigNum) {
        case SIGQUIT:
            CRITICAL_MSG(
              "Received QUIT signal. This is likely caused by the "
