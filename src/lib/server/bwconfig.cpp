...
namespace BWConfig {
    ...
    /**
     *	Find the next matching element, iterating through each configuration
     *data section. If none is found, then this iterator will be equivalent to
     *the iterator returned by endOfSearch().
     */
    ...
    void init()
    {
        ...
        // Attempt to find a personalised configuration file first. This is of
        // the form "bw_<username>.xml".
        bool           isUserFilename = false;
        struct passwd* pPasswordEntry = ::getpwuid(::getUserId());

        if (pPasswordEntry && (pPasswordEntry->pw_name != NULL)) {
            filename =
              BW::string("server/bw_") + pPasswordEntry->pw_name + ".xml";
            isUserFilename = true;
        }
        ...
        DebugFilter::instance().filterThreshold(
          (DebugMessagePriority)BWConfig::get<uint32>(
            "outputFilterThreshold",
            DebugFilter::instance().filterThreshold()));
