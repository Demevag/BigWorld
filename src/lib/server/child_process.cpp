                                        ...
                                        bool shouldPipeStdErr)
{
    if (!shouldPipeStdOut && !shouldPipeStdErr) {
        WARNING_MSG(
          "ChildProcess::startProcessWithPipe: "
    ...
    if (shouldPipeStdOut) {
        ...
        eventDispatcher_.registerFileDescriptor(stdOutFD_, this, "stdout");
    ...
    if (shouldPipeStdErr) {
        ...
        eventDispatcher_.registerFileDescriptor(stdErrFD_, this, "stderr");
        close(stdErrFDs[1]);
    }
...
void ChildProcess::execInFork()
{
    ...
    int result = execv(commandToRun_.c_str(), const_cast<char* const*>(argv));
