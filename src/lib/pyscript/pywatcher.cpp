...
Watcher& PySequence_Watcher()
{
    ...
    if (pWatchMe == NULL) {
         ...
         *(PyObjectPtrRef*)NULL ) );*/
        pWatchMe->addChild("*", new PyObjectWatcher(*(PyObjectPtrRef*)NULL));
    }
    return *pWatchMe;
}
...
SmartPointer<DataWatcher<PyObjectPtrRef>> PyObjectWatcher::fallback_ =
  new DataWatcher<PyObjectPtrRef>(*(PyObjectPtrRef*)NULL,
                                  Watcher::WT_READ_WRITE);

/**
 *	Constructor
 */
PyObjectWatcher::PyObjectWatcher(PyObjectPtrRef& popr)
  : popr_(popr)
{
}
 ...
 *	Get method
 */
bool PyObjectWatcher::getAsString(const void*    base,
                                  const char*    path,
                                  BW::string&    result,
                                  BW::string&    desc,
                                  Watcher::Mode& mode) const
{
    ...
    // - decref correct?
    PyObjectPtrRef& popr =
      *(PyObjectPtrRef*)(((const uintptr)&popr_) + ((const uintptr)base));
    ...
    if (pSpecialWatcher != NULL) { // yes it is - let it handle everything then
        ...
        if (isEmptyPath(path)) { // call the datawatcher
            ret = static_cast<Watcher&>(*fallback_)
                    .getAsString(&popr, path, result, desc, mode);
        }
    }
 ...
 *	Set method
 */
bool PyObjectWatcher::setFromString(void*       base,
                                    const char* path,
                                    const char* valueStr)
{
    PyObjectPtrRef& popr =
    ...
    if (isEmptyPath(path)) {
        // ok, set away then
        ret = static_cast<Watcher&>(*fallback_)
                .setFromString(&popr, path, valueStr);
    } else {
...
bool PyObjectWatcher::getAsStream(const void*           base,
                                  const char*           path,
                                  WatcherPathRequestV2& pathRequest) const
{
    ...
    if (pSpecialWatcher != NULL) { // yes it is - let it handle everything then
        ...
        if (isEmptyPath(path)) { // call the datawatcher
            ret = static_cast<Watcher&>(*fallback_)
                    .getAsStream(&popr, path, pathRequest);
        }
    }
 ...
 *	Set method
 */
bool PyObjectWatcher::setFromStream(void*                 base,
                                    const char*           path,
                                    WatcherPathRequestV2& pathRequest)
{
    ...
    if (isEmptyPath(path)) {
        // ok, set away then
        ret = static_cast<Watcher&>(*fallback_)
                .setFromStream(&popr, path, pathRequest);
    } else {
        // it had better be a directory...
        PyObject* pObject         = popr;
        Watcher*  pSpecialWatcher = this->getSpecialWatcher(pObject);
        ...
        if (pSpecialWatcher != NULL) {
            ret = pSpecialWatcher->setFromStream(&pObject, path, pathRequest);
        }
 ...
 *	Visit method
 */
bool PyObjectWatcher::visitChildren(const void*         base,
                                    const char*         path,
                                    WatcherPathRequest& pathRequest)
{
    ...
    if (pSpecialWatcher != NULL) {
        ret = pSpecialWatcher->visitChildren(&pObject, path, pathRequest);
    }
 ...
 *	Add child method
 */
bool PyObjectWatcher::addChild(const char* /*path*/,
                               WatcherPtr /*pChild*/,
                               void* /*withBase*/)
{
...
static PyObject* py_setWatcher(PyObject* args)
{
    ...
    if (pStr) {
        ...
        if (!Watcher::rootWatcher().setFromString(NULL, path, pCStr)) {
            PyErr_Format(
              PyExc_TypeError, "Failed to set %s to %s.", path, pCStr);
            Py_DECREF(pStr);
            return NULL;
        }
...
namespace {
    ...
    bool putPyObjectOnStream(BinaryOStream&   stream,
                             PyObject*        pyObj,
        ...
        switch (type) {
            ...
            case WATCHER_TYPE_UINT: {
                ...
                if (Script::setData(pyObj, value, "int64") != -1) {
                    ...
                    ERROR_MSG(
                      "PyWatcher: putPyObjectOnStream failed to convert "
            ...
            case WATCHER_TYPE_UNKNOWN: {
                // In this case we will only pack on the type mode and a size of
                // 0, no data will be transferred.
            ...
            default: {
                ERROR_MSG(
                  "PyWatcher: putPyObjectOnStream: Unsupported type (%d) "
...
class PyAccessorWatcher : public Watcher
{
  public:
    ...
    //@{
    PyAccessorWatcher(const char* path,
    ...
    ~PyAccessorWatcher()
    {
        ...
        if (Py_IsInitialized()) {
            ...
            WARNING_MSG(
              "~PyAccessorWatcher: There were still Python references to a "
              "watcher after Python was finalized ('%s'). "
              "You should release all Python references to watchers on "
              "shutdown - did you get a Python exception?\n",
              path_.c_str());
        }
    }
  ...
  protected:
                             ...
                             Watcher::Mode& mode) const
    {
        if (isEmptyPath(path)) {
            Py_INCREF(getFunction_);
            PyObject* pValue =
              Script::ask(getFunction_, PyTuple_New(0), "PyAccessorWatcher");
                               ...
                               WatcherPathRequestV2& pathRequest)
    {
        ...
        if (isEmptyPath(path) && (setFunction_ != NULL)) {
            ...
            status = Script::call(
              setFunction_, PyTuple_Pack(1, pyObj), "PyAccessorWatcher");
            Py_DECREF(pyObj);
            pyObj = NULL;
...
class PyDeferredWatcher : public PyObjectPlus
{
    Py_Header(PyDeferredWatcher, PyObjectPlus)
      ...
      public
      : PyDeferredWatcher(PyFunctionWatcher&    pyFunctionWatcher,
                          void*                 base,
                          const char*           path,
                          WatcherPathRequestV2& pathRequest,
                          BW::string&           outputStr)
...
class PyFunctionWatcher : public CallableWatcher
{
  public:
    ...
    bool addArg(PyObject* pArgType, const char* argName = "")
    {
        ...
        if ((PyTypeObject*)pArgType == &PyBool_Type) {
            ...
            const char* typeStr =
              (pyStr != NULL) ? PyString_AsString(pyStr) : "(unknown)";
    ...
    {
        if (!PyCallable_Check(function) || (argList == NULL) ||
            !PySequence_Check(argList)) {
        ...
        for (Py_ssize_t i = 0; i < seqSize; i++) {
            ...
            if (PyType_Check(pArg)) {
                ...
                if ((pName == NULL) || (pType == NULL)) {
                    PyErr_Format(
                      PyExc_RuntimeError,
                      "Failed to decode argument tuple for element %" PRIzu " "
                      "of arg list sequence.",
                      size_t(i));
    ...
    void finishSetFromStream(void*                 base,
                             const char*           path,
                             WatcherPathRequestV2& pathRequest,
                             ...
                             const BW::string&     outputStr)
    {
        ...
        BinaryOStream& resultStream =
          this->startResultStream(pathRequest, outputStr, objectSize);
  ...
  protected:
                               ...
                               WatcherPathRequestV2& pathRequest)
    {
        ...
        if (!istream || !this->watcherStreamToValue(*istream, pArgs) ||
            !isEmptyPath(path) || (pFunction_ == NULL)) {
            Py_XDECREF(pArgs);
            return false;
        }
        ...
        ScriptObject returnValues(
          Script::ask(
            pFunction_.get(), pArgs, "PyFunctionWatcher", false, true),
        ...
        if (pAddCallbacks) {
            ScriptObject pDelayedReturn(
              new PyDeferredWatcher(*this, base, path, pathRequest, outputStr),
              ScriptObject::STEAL_REFERENCE);

            ScriptObject pCallback =
              pDelayedReturn.getAttribute("callback", ScriptErrorPrint());
            ScriptObject pErrback =
              pDelayedReturn.getAttribute("errback", ScriptErrorPrint());

            MF_ASSERT(pCallback && pErrback);

            ScriptObject result(
              PyObject_CallFunctionObjArgs(
                pAddCallbacks.get(), pCallback.get(), pErrback.get(), NULL),
              ScriptObject::STEAL_REFERENCE);
            ...
            this->finishSetFromStream(
              base, path, pathRequest, returnValues, outputStr);
        }
...
{
    pyFunctionWatcher_.finishSetFromStream(
      base_, path_, pathRequest_, values, outputStr_);
}

void PyDeferredWatcher::errback(const ScriptObject& failure)
{
    ScriptObject value = failure.getAttribute(
      "value",
      ScriptErrorPrint("PyDeferredWatcher::errback: Invalid failure object\n"));

    if (value) {
        pyFunctionWatcher_.finishSetFromStream(
          base_, path_, pathRequest_, value, outputStr_);
    }
}
 ...
 *	>>>
 *	>>> BigWorld.addWatcher( "Comms/Max bandwidth per second", getMaxBps,
 *setMaxBps )
 *	@}
 *
 *	This adds a watcher variable under "Comms" watcher directory. The function
 *getMaxBps is called to obtain the watcher value and setMaxBps is called when
 *the watcher is modified.
 *
 *	Raises a TypeError if getFunction or setFunction is not callable.
 *
 *	@param	path	the path to the item to create.
 *	@param	getFunction	the function to call when retrieving the watcher
 *variable. This function takes no argument and returns a string representing
 *						the watcher value.
 *	@param	setFunction	(optional)the function to call when setting the watcher
 *variable This function takes a string argument as the new watcher value. This
 *function does not return a value. If this function does not exist the created
 *watcher is a read-only watcher variable.
 */
static PyObject* py_addWatcher(PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(
          args, "sO|Os", &path, &getFunction, &setFunction, &desc)) {
        return NULL;
    }
    ...
    // TODO PyAccessorWatcher() + init() for better error handling + return
    // status
...
static PyObject* py_addFunctionWatcher(PyObject* args)
{
    ...
    PyFunctionWatcher* pyFW =
      new PyFunctionWatcher((CallableWatcher::ExposeHint)exposedHint, desc);

    if (pyFW) {
        if (!pyFW->init(path, function, argumentList)) {
            bw_safe_delete(pyFW);
        }
    }
...
PyObject* SimplePythonWatcher::pythonChildBase(PyObject*   pPyObject,
                                               const char* path)
{
    ...
    if (PyMapping_Check(pPyObject)) {
        PyObject* pPyChild =
          PyMapping_GetItemString(pPyObject, const_cast<char*>(head.c_str()));
...
bool SimplePythonWatcher::getAsString(const void*    base,
                                      const char*    path,
                                      BW::string&    result,
                                      BW::string&    desc,
                                      Watcher::Mode& mode) const
{
    ...
    if (isEmptyPath(path)) {
        if (PyString_Check(pPyObject)) {
            ...
            return true;
        } else if (PyMapping_Check(pPyObject) || PySequence_Check(pPyObject) ||
        ...
        if (pPyChild) {
            ...
            bool returnValue = this->getAsString(
              pPyChild, DirectoryWatcher::tail(path), result, desc, mode);
...
bool SimplePythonWatcher::setFromString(void*       base,
                                        const char* path,
                                        const char* valueStr)
{
    return true;
...
bool SimplePythonWatcher::getAsStream(const void*           base,
                                      const char*           path,
                                      WatcherPathRequestV2& pathRequest) const
{
    PyObject*     pPyObject = (PyObject*)base;
    ...
    if (pPyObject == NULL) {
        watcherValueToStream(pathRequest.getResultStream(), "NULL", mode);
        pathRequest.setResult(comment_, mode, this, base);
        return true;
    }
    ...
    if (isEmptyPath(path)) {
        ...
        if (pPyChild) {
            // We recurse here because PyWatcher is its own child
            bool returnValue = this->getAsStream(
              pPyChild, DirectoryWatcher::tail(path), pathRequest);
            Py_DECREF(pPyChild);
            return returnValue;
        } else {
            watcherValueToStream(pathRequest.getResultStream(), "", mode);
...
bool SimplePythonWatcher::setFromStream(void*                 base,
                                        const char*           path,
                                        WatcherPathRequestV2& pathRequest)
{
    return true;
...
bool SimplePythonWatcher::visitChildren(const void*         base,
                                        const char*         path,
                                        WatcherPathRequest& pathRequest)
{
    PyObject* pPyObject = (PyObject*)base;
    ...
    if (pPyObject == NULL) {
        ...
        for (Py_ssize_t iter = 0; iter < count; iter++) {
            ...
            if (!pathRequest.addWatcherPath(
                  PySequence_Fast_GET_ITEM(pValues, iter),
                  NULL,
                  label,
                  *this)) {
                break;
        ...
        if (pChild != NULL) {
            handled = this->visitChildren(
              pChild, DirectoryWatcher::tail(path), pathRequest);
            Py_DECREF(pChild);
        }
    }
