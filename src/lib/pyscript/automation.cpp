...
namespace Automation {
    void parseCommandLine(LPCTSTR lpCmdLine)
    {
        ...
        while (arg != NULL) {
            if ((strcmp(arg, "-s") == 0) || (strcmp(arg, "--script") == 0)) {
                char* scriptPath = strtok(NULL, sepsArgs);
                if (scriptPath == NULL) {
                    ERROR_MSG("Automation::ParseCommandLine: script argument "
                              "specified without scriptPath parameter\n");
                    return;
                }

                size_t scriptPathLen = strlen(scriptPath);
                if (scriptPathLen > 2 && scriptPathLen < MAX_PATH - 1 &&
                    scriptPath[0] == '\"' &&
                    scriptPath[scriptPathLen - 1] == '\"') {
                    char trimmedScriptPath[BW_MAX_PATH];
                    bw_str_substr(trimmedScriptPath,
                                  ARRAY_SIZE(trimmedScriptPath),
                                  scriptPath,
                                  1,
                                  scriptPathLen - 2);
    ...
    void executeScript(const char* scriptPath)
    {
        ...
        if (!module) {
            ERROR_MSG("Automation::ExecuteScript: Failed to import script "
                      "module '%s':\n",
                      scriptPath);
            PyErr_Print();
            return;
        ...
        if (!res) {
            ERROR_MSG(
              "Automation::ExecuteScript: Error running script module '%s':\n",
              scriptPath);
            PyErr_Print();
            return;
        }
