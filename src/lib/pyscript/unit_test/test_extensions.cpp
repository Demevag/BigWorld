...
namespace BW {
    ...
    TEST_F(PyScriptUnitTestHarness, Python_cPickle)
    {
        ...
        if (module) {
            PyObject* res = Script::ask(
              PyObject_GetAttrString(module, "testPickleAndUnpickle"),
              PyTuple_New(0),
              "",
              false);
    ...
    {
        // we don't need to explicitly initialise cPickle module if we are using
        // Pickler class
    ...
    TEST_F(PyScriptUnitTestHarness, Python_zlib)
    {
        ...
        if (module) {
            PyObject* res =
              Script::ask(PyObject_GetAttrString(module, "testZlib"),
                          PyTuple_New(0),
                          "",
                          false);
    ...
    TEST_F(PyScriptUnitTestHarness, Python_ImportHashlibModule)
    {
        ...
        if (module) {
            ...
            PyObject* res =
              Script::ask(PyObject_GetAttrString(module, "encryptTest"),
                          args,
                          "Python_ImportHashlibModule",
                          false);

            CHECK(PyString_Check(res));
            CHECK_EQUAL("e1537ae4352f712da08b93d34310f6de",
                        BW::string(PyString_AsString(res)));
