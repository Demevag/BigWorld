...
namespace BW {
            ...
            PythonScriptOutputWriter_testStdioDebugMessage)
    {
        ...
        CHECK_EQUAL(Py_None,
                    Script::runString("print 'Test stdio string: %s' % ( 25, )",
                                      /* printResult */ true));
            ...
            PythonScriptOutputWriter_testStderrDebugMessage)
    {
        ...
        CHECK_EQUAL(Py_None,
                    Script::runString("import sys; print >> sys.stderr, 'Test "
                                      "stderr string: %s' % ( 45, )",
                                      /* printResult */ true));
    ...
    // Test that we can hook when only stdout or stderr is redirected, and that
    // we cannot hook if neither is redirected. These tests need to be careful,
    // we must leave sys as we found it.
            ...
            PythonScriptOutputWriter_testStdioOnlyHookMessage)
    {
        ...
        CHECK_EQUAL(Py_None,
                    Script::runString("import sys; sys.savestderr = "
            ...
            PythonScriptOutputWriter_testStderrOnlyHookMessage)
    {
        ...
        CHECK_EQUAL(Py_None,
                    Script::runString("import sys; sys.savestdout = "
            ...
            PythonScriptOutputWriter_testCannotHookMessage)
    {
        ...
        CHECK_EQUAL(
          Py_None,
          Script::runString(
            "import sys; sys.savestdout = sys.stdout; sys.stdout = "
            "sys.__stdout__; "
    ...
    // Again, order of hooks added is not specified. But if they are ordered in
    // a way other than by insertion (e.g. sorted container) these two tests
    // need to be changed.
    TEST_FP(ScriptOutputHookLatchFixture,
            ScriptOutputHookLatchFixture("Latch", false),
