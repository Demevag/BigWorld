...
ScriptObject PyDataSection::pyGetAttribute(const ScriptString& attrObj)
{
    ...
    if (attr[0] == '_' && (attr[1] != '_' && attr[1] != 0)) {
        ...
        if (pChildSection) {
            return ScriptObject(new PyDataSection(pChildSection),
 ...
 */
template <class C>
bool harmlessSetData(PyObject* obj, C& res)
{
    ...
    if (attr[0] == '_') {
        ...
        if (pChildSection) {
            ...
            if (ScriptString strValue = ScriptString::create(value)) {
                ...
                wchar_t    buf[256];
                Py_ssize_t nChars =
                  PyUnicode_AsWideChar((PyUnicodeObject*)value.get(), buf, 255);
                if (nChars != -1) {
                    buf[nChars] = L'\0';
...
PyObject* PyDataSection::py_items(PyObject* /*args*/)
{
    ...
    while (iter != pSection_->end()) {
        ...
        PyTuple_SetItem(
          pTuple, 0, PyString_FromString(pChild->sectionName().c_str()));
...
{
    BinaryPtr pBinary =
      new BinaryBlock(v.data(), v.length(), "BinaryBlock/PyDataSection");
...
#define IMPLEMENT_WRITE_VALUES(METHOD, TYPE)                                   \
    ...
    {                                                                          \
        ...
        if (!PyArg_ParseTuple(args, "sO", &path, &pSeq) ||                     \
            ...
            PyErr_SetString(PyExc_TypeError,                                   \
                            "Expected string and a sequence.");                \
            return NULL;                                                       \
        }                                                                      \
                                                                               \
        ...
        for (Py_ssize_t i = 0; i < size; i++) {                                \
            ...
            if (res != 0) {                                                    \
                PyErr_Format(                                                  \
                  args, "Section %ld is not of of type " #TYPE ".", i);        \
                return NULL;                                                   \
            }                                                                  \
        }                                                                      \
                                                                               \
 ...
 *
 *	This function writes a subsection with the specified name, writing the
 *	specified BLOB string.  If one or more subsections with that name already
 *exist, then the value of the first one is replaced with the specified string.
 *	This will not effect any subsections of the named subsection, only its
 *	actual contents.
 *
 *	@param	sectionName	the name of the subsection to create or overwrite.
...
PyObject* PyDataSection::py_createSection(PyObject* args)
{
    ...
    // We do this, so that names such as foldername/filename.xml can be passed
    // in
...
PyObject* PyDataSection::py_createSectionFromString(PyObject* args)
{
    ...
    DataSectionPtr newSection = XMLSection::createFromStream("", sstream).get();

    if (newSection) {
        DataSectionPtr copiedSection =
          pSection_->newSection(newSection->sectionName());
...
PyObject* PyDataSection::py_deleteSection(PyObject* args)
{
    ...
    if (PyTuple_Size(args) != 1) {
        PyErr_SetString(
          PyExc_TypeError,
          "py_deleteSection: Expected a DataSection or String argument.");
        return NULL;
    }
...
PyObject* PyDataSection::child(int index)
{
    if (0 <= index && index < pSection_->countChildren()) {
        ...
        if (pChild)
            return new PyDataSection(pChild);
...
int PyDataSection::pySet_isAttribute(PyObject* value)
{
    ...
    if (pSection_->countChildren() != 0) {
        PyErr_Format(PyExc_AttributeError,
                     "isAttribute can only be used on "
