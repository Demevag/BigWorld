...
PyComponent::PyComponent(const IEntityDelegatePtr& delegate,
                         EntityID                  entityID,
                               ...
                               bool                   isPersistentOnly)
{
    ...
    if (!descr.addToStream(source, stream, isPersistentOnly)) {
        PyErr_Format(PyExc_TypeError, "Failed to convert value to stream");
        return false;
    }
