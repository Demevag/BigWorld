...
namespace BW { namespace CompiledSpace {
                                       ...
                                       size_t               maxObjects)
    {
        ...
        size_t sectionIdx =
          binFile.findSection(StaticSceneModelTypes::FORMAT_MAGIC);
        ...
        if (!pStream_->read(modelData_) || !pStream_->read(resourceRefs_)) {
    ...
    // ----------------------------------------------------------------------------
    void StaticSceneModelHandler::loadModelInstances(
      const StringTable& strings,
      SceneObject*       pLoadedObjects,
      size_t             maxObjects)
    {
        ...
        for (size_t i = 0; i < this->numModels(); ++i) {
            ...
            bool isCastingShadow =
              instance.pDef_->flags_ & StaticSceneModelTypes::FLAG_CASTS_SHADOW;
    ...
    bool StaticSceneModelHandler::loadSuperModel(
      ...
      Instance&                     instance)
    {
        // TODO: fix up super model to allow us not to use this vector of
        // strings.
        BW::vector<BW::string> modelIds;

        for (uint16 i = def.resourceIDs_.first_; i <= def.resourceIDs_.last_;
             ++i) {
            if (i != uint16(-1)) {
        ...
        if (bLoadSuccess) {
            ...
            if (animationName) {
                ...
                if (instance.pAnimation_->pSource(*instance.pSuperModel_) ==
                    NULL) {
            ...
            if (count > 0 && count % 2 == 0) {
                ...
                for (uint16 i = def.tintDyes_.first_; i < def.tintDyes_.last_;
                     i += 2) {
                    ...
                    if (dye && tint) {
                        ...
                        if (pDye) {
                            ...
                            ASSET_MSG(
                              "SuperModel cannot find dye->tint mapping: "
                              "%s->%s\n",
                              dye,
                              tint);
                        }
        ...
        if (!bLoadSuccess) {
            WARNING_MSG(
              "StaticModelHandler: No models loaded into SuperModel\n");
    ...
    // ----------------------------------------------------------------------------
    void StaticSceneModelHandler::Instance::setTransform(
      const Matrix& transform)
    {
        ...
        if (pSuperModel_) {
            pSuperModel_->cacheDefaultTransforms(pDef_->worldTransform_);

            pSuperModel_->updateLodsOnly(pDef_->worldTransform_,
    ...
    // ----------------------------------------------------------------------------
    // ----------------------------------------------------------------------------
    class StaticSceneModelHandler::TickHandler
            ...
            for (size_t i = 0; i < objectCount; ++i) {
                ...
                if (pModelAnim) {
                    ...
                    pInstance->pAnimation_->time +=
                      dTime * pInstance->pDef_->animationMultiplier_;
    ...
    // ----------------------------------------------------------------------------
    // ----------------------------------------------------------------------------
    class StaticSceneModelHandler::DrawHandler
    ...
    // ----------------------------------------------------------------------------
    // ----------------------------------------------------------------------------
    class StaticSceneModelHandler::CollisionHandler
      ...
      private:
                               ...
                               CollisionState&    cs) const
        {
            ...
            for (int i = 0; i < numModels; i++) {
                const ModelPtr& pModel = pInstance->pSuperModel_->topModel(i);
                if (!pModel) {
                    continue;
                }
    ...
    {
      public:
        virtual void getWorldTransform(const SceneObject& obj, Matrix* xform)
        {
        ...
        virtual void getWorldVisibilityBoundingBox(const SceneObject& obj,
                                                   AABB*              bb)
        {
            StaticSceneModelHandler::Instance* pItem =
              obj.getAs<StaticSceneModelHandler::Instance>();
      ...
      : public Moo::ITextureStreamingObjectOperationTypeHandler
    {
      public:
                          ...
                          Moo::ModelTextureUsageGroup&       usageGroup)
        {
            ...
            usageGroup.setWorldScale_FromTransform(
              pItem->pDef_->worldTransform_);
    ...
    void StaticSceneModelHandler::registerHandlers(Scene& scene)
    {
        ...
        pTextureStreamingOp
          ->addHandler<StaticSceneModelHandler::Instance,
