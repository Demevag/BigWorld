...
namespace BW {
    namespace CompiledSpace { namespace Terrain2Types {
        ...
        inline uint32 blockIndexFromPoint(const Vector3& point, Header& header)
        {
            int32 xCoord = pointToGrid(point.x, header.blockSize_);
            int32 zCoord = pointToGrid(point.z, header.blockSize_);

            // Check bounds
            if (xCoord < header.gridMinX_ || xCoord > header.gridMaxX_ ||
                zCoord < header.gridMinZ_ || zCoord > header.gridMaxZ_) {
