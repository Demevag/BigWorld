...
namespace BW { namespace CompiledSpace {
    ...
    namespace {
          ...
          LightSceneProvider::MooPulseLightPtrArray& mooLightArray)
        {
            ...
            for (; iter != end; ++iter) {
                ...
                const LightSceneTypes::OmniLight& omniLight =
                  pulseLight.omniLight;
                Vector2* pFirstFrame =
                  &pulseLightsAnimFrames[0] + pulseLight.firstFrameIdx;
                const ExternalArray<Vector2> animFrames(
                  pFirstFrame, pulseLight.numFrames, pulseLight.numFrames);
                Moo::PulseLightPtr mooLight(new Moo::PulseLight(
                  omniLight.colour,
                  omniLight.position,
                  omniLight.innerRadius,
                  omniLight.outerRadius,
    ...
    bool LightSceneProvider::doLoadFromSpace(
      ClientSpace*          pSpace,
      BinaryFormat&         reader,
      ...
      const StringTable&    strings)
    {
        ...
        pStream_ = pReader_->findAndOpenSection(LightSceneTypes::FORMAT_MAGIC,
...

        {
            ...
            convertSerialisedToMooOmniLightPtrs(
              omniLightsData, transform, omniLights_);
            convertSerialisedToMooSpotLightPtrs(
              spotLightsData, transform, spotLights_);
            convertSerialisedToMooPulseLightPtrs(pulseLightsData,
                                                 transform,
                                                 pulseLightsAnimFrames,
                                                 strings,
                                                 pulseLights_);
        }
    ...
    void LightSceneProvider::forEachObject(const SceneObjectCallback& function)
    {
    }

    namespace {
        // ConvexHull intersections.
        // NOTE: We should eventually do a full specialisation for SpotLight
        // types since this default intersection is way too conservative.
        template <typename LightType>
        bool lightAndObjectIntersect(const ConvexHull& hull,
                                     ...
                                     Moo::LightContainer& lightContainer)
        {
            ...
            for (; iter != end; ++iter) {
                ...
                if (lightAndObjectIntersect((IntersectingObjectType)obj,
                                            *pLight)) {
    ...
    size_t LightSceneProvider::intersect(
      ...
      Moo::LightContainer& lightContainer) const
    {
        ...
        numLightsAdded +=
          addIntersectingLights(hull, omniLights_, lightContainer);
        numLightsAdded +=
          addIntersectingLights(hull, spotLights_, lightContainer);
        numLightsAdded +=
          addIntersectingLights(hull, pulseLights_, lightContainer);
      ...
      Moo::LightContainer& lightContainer) const
    {
        ...
        numLightsAdded +=
          addIntersectingLights(bbox, omniLights_, lightContainer);
        numLightsAdded +=
          addIntersectingLights(bbox, spotLights_, lightContainer);
        numLightsAdded +=
          addIntersectingLights(bbox, pulseLights_, lightContainer);
