...
namespace BW { namespace CompiledSpace {
    ...
    bool StaticSceneFlareWriter::writeData(BinaryFormatWriter& writer)
    {
        BinaryFormatWriter::Stream* stream =
          writer.appendSection(StaticSceneFlareTypes::FORMAT_MAGIC,
    ...
    void StaticSceneFlareWriter::convertFlare(const ConversionContext& ctx,
                                              ...
                                              const BW::string&        uid)
    {
        addFromChunkFlare(pItemDS, ctx.chunkTransform, strings(), assets());
    }

    // ----------------------------------------------------------------------------
    bool StaticSceneFlareWriter::addFromChunkFlare(
      ...
      AssetListWriter&      assetList)
    {
...

        {
            ...
            result.maxDistance_ = pFlareRoot->readFloat(
              "maxDistance", LensEffect::DEFAULT_MAXDISTANCE);
            result.area_ =
              pFlareRoot->readFloat("area", LensEffect::DEFAULT_AREA);
            result.fadeSpeed_ = pFlareRoot->readFloat(
              "fadeSpeed", LensEffect::DEFAULT_FADE_SPEED);

            // Then use local data to override it
            result.maxDistance_ =
              pObjectDS->readFloat("maxDistance", result.maxDistance_);
            result.area_ = pObjectDS->readFloat("area", result.area_);
            result.fadeSpeed_ =
              pObjectDS->readFloat("fadeSpeed", result.fadeSpeed_);
        }
