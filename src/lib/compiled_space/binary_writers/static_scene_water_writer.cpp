...
namespace BW { namespace CompiledSpace {
    ...
    bool StaticSceneWaterWriter::writeData(BinaryFormatWriter& writer)
    {
        BinaryFormatWriter::Stream* stream =
          writer.appendSection(StaticSceneWaterTypes::FORMAT_MAGIC,
    ...
    {
        Vector3 chunkCentre =
          Vector3(ctx.gridSize * 0.5f, 0.0f, ctx.gridSize * 0.5f);
        ...
        if (!BWResource::fileExists(odata) &&
            BWResource::fileExists(legacyOdata)) {
            odata = legacyOdata;
        }
    ...
    // ----------------------------------------------------------------------------
    bool StaticSceneWaterWriter::addFromChunkVLO(
      const DataSectionPtr& pObjectDS,
      ...
      AssetListWriter&      assetList)
    {
        ...
        result.foamTexture_ =
          assetList.addAsset(CompiledSpace::AssetListTypes::ASSET_TYPE_TEXTURE,
                             resources.foamTexture_,
                             stringTable);

        result.waveTexture_ = stringTable.addString(resources.waveTexture_);

        result.reflectionTexture_ =
          assetList.addAsset(CompiledSpace::AssetListTypes::ASSET_TYPE_TEXTURE,
                             resources.reflectionTexture_,
                             stringTable);

        result.transparencyTable_ =
          stringTable.addString(resources.transparencyTable_);
        ...
        Vector3 size(
          result.state_.size_.x * 0.5f, 0, result.state_.size_.y * 0.5f);
