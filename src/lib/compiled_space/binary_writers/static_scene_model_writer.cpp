...
namespace BW { namespace CompiledSpace {
    ...
    namespace {
        ...
        AABB extractBounds(const BW::string& modelName)
        {
            ...
            // Now override that result with the model's visibility BB if there
            // is one. the model's visibility BB takes into account animation of
            // the object and should be larger than the other BB. If it isn't,
            // we should probably end up taking the merger of the two bb's to
    ...
    bool StaticSceneModelWriter::writeData(BinaryFormatWriter& writer)
    {
        BinaryFormatWriter::Stream* stream =
          writer.appendSection(StaticSceneModelTypes::FORMAT_MAGIC,
    ...
    // ----------------------------------------------------------------------------
    bool StaticSceneModelWriter::addFromChunkModel(
      const DataSectionPtr& pItemDS,
      ...
      AssetListWriter&      assetList)
    {
        ...
        IndicesRangeBuilder resourceNameRange(&stringIndices_,
                                              &result.resourceIDs_);
        for (size_t i = 0; i < models.size(); ++i) {
