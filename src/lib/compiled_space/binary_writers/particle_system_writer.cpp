...
namespace BW { namespace CompiledSpace {
    ...
    bool ParticleSystemWriter::write(BinaryFormatWriter& writer)
    {
        BinaryFormatWriter::Stream* stream =
          writer.appendSection(ParticleSystemTypes::FORMAT_MAGIC,
    ...
    void ParticleSystemWriter::convertParticles(const ConversionContext& ctx,
                                                ...
                                                const BW::string&     uid)
    {
        addFromChunkParticles(pItemDS, ctx.chunkTransform, strings(), assets());
    }
      ...
      AssetListWriter&      assetList)
    {
        ...
        bool isReflectionVisible = pDS->readBool("reflectionVisible", false);
