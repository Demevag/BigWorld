...
class ScopedTemporaryFile
{
  public:
    ...
    ~ScopedTemporaryFile() { DeleteFileA(filename_); }
                             ...
                             const BW::vector<BW::string>& filterOut)
{
    ...
    for (size_t i = 0; i < sections_.size(); ++i) {
        ...
        // Update transform
        Matrix m =
          sections_[i].pDS_->readMatrix34("transform", Matrix::identity);
                                     ...
                                     const BW::vector<BW::string>& filterOut)
{
    ...
    {
        FILE* fh = BWResource::instance().fileSystem()->posixFileOpen(
          tempXMLFile.filename(), "w");
    ...
    if (!PackedSection::convert(
          tempXMLFile.filename(), tempBINFile.filename(), NULL, true)) {
        return false;
    }
