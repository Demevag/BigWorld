...
EdgeGeometryMapping::EdgeGeometryMapping(ChunkSpacePtr         pSpace,
                                         ...
                                         EdgeGeometryMappings& mappings,
                                         const Vector3*        pInitialPoint)
  ...
  , mappings_(mappings)
{
    ...
    BW::RectInt bounds(this->minLGridX(),
                       this->minLGridY(),
                       this->maxLGridX() + 1,
                       this->maxLGridY() + 1);
...
bool EdgeGeometryMapping::hasFullyLoaded()
{
    ...
    bool fullyLoaded = edge_[0].isFullyLoaded() && edge_[1].isFullyLoaded() &&
                       edge_[2].isFullyLoaded() && edge_[3].isFullyLoaded();
 ...
 */
bool EdgeGeometryMapping::tick(const Vector3& minB,
                               const Vector3& maxB,
                               bool           unloadOnly)
{
    ...
    int lim[4];
    lim[LEFT] =
      Math::clamp(this->minLGridX(),
                  int(floorf(bb.minBounds().x / pSpace()->gridSize())),
                  this->maxLGridX() + 1);
    lim[BOTTOM] =
      Math::clamp(this->minLGridY(),
    ...
    if (unloadOnly) {
        ...
        lim[(little ^ 1) | 2] =
          std::min(lim[little ^ 1] + 2, lim[(little ^ 1) | 2]);
    }
    ...
    for (int i = 0; i < 4; i++) {
        currLoadedAll_ &= edge_[i].tick(this,
                                        edge_[(i ^ 1) & 1],
                                        edge_[(i ^ 1) | 2],
                                        edge_[i ^ 2],
                                        anyColumnsLoaded);
                                         ...
                                         bool      isLast) const
{
    ...
    if (isFirst) {
        loadedRect = BW::Rect(
          -veryBigNumber, -veryBigNumber, veryBigNumber, veryBigNumber);
    }
    ...
    if (isLast) {
        ...
        if (loadedRect.xMin_ < -prettyBigNumber)
            loadedRect.xMin_ = -FLT_MAX;
        if (loadedRect.yMin_ < -prettyBigNumber)
            loadedRect.yMin_ = -FLT_MAX;
        if (loadedRect.xMax_ > prettyBigNumber)
            loadedRect.xMax_ = FLT_MAX;
        if (loadedRect.yMax_ > prettyBigNumber)
            loadedRect.yMax_ = FLT_MAX;
    }
}
...
{
    rects.push_back(BW::Rect(this->minLGridX(),
                             this->minLGridY(),
                             this->maxLGridX(),
                             this->maxLGridY()));
