...
BW::string FunctorManager::text(const BW::string& textor, ItemPtr item)
{
    ...
    if (functors().find(functorName) != functors().end()) {
        if (!functors()
               .find(functorName)
               ->second->text(functor, item, result)) {
    ...
    if (functor.size() >= 2 && functor[0] == '\"' &&
        *functor.rbegin() == '\"') {
        ...
        if (const wchar_t* str = Localise(
              wfunctor.c_str(), StringProvider::RETURN_NULL_IF_NOT_EXISTING)) {
...
unsigned int FunctorManager::update(const BW::string& updater, ItemPtr item)
{
    ...
    if (functors().find(functorName) != functors().end()) {
        if (!functors()
               .find(functorName)
               ->second->update(functor, item, result)) {
            ; //// should give some error message here
        }
        return result;
    }
