...
As we know, any status bitmap could be
normal/disabled/enabled/hover/pressed/....... It is a bit hard to give a full
enumeration of is, so we use a string as BITMAP type
...
You may define your own freely since their are no real panelty except using a
bit more resources.

TODO: for now, we doesn't support any register + post processing, will be
finished when everything else is ok
**************************************************/
...
class Bitmap : public SafeReferenceCount
{
  ...
  public:
    Bitmap(const BW::wstring& name,
           COLORREF           transparent,
           const BW::string&  type   = "NORMAL",
           unsigned int       width  = SIZE_DEFAULT,
...
class BitmapManager
{
  ...
  public:
    BitmapPtr find(const BW::string& name,
                   const BW::string& type   = "NORMAL",
                   unsigned int      width  = SIZE_DEFAULT,
                   unsigned int      height = SIZE_DEFAULT);
    BitmapPtr get(const BW::string& name,
                  COLORREF          transparent,
                  const BW::string& type   = "NORMAL",
                  unsigned int      width  = SIZE_DEFAULT,
                  unsigned int      height = SIZE_DEFAULT);
