...
HBITMAP clipBitmap(HBITMAP srcBmp, RECT rect)
{
    ...
    if ((dcMem = CreateCompatibleDC(NULL)) &&
        ...
        if ((destBmp = CreateBitmap(rect.right - rect.left,
...
HBITMAP replaceButtonFace(HBITMAP srcBmp, COLORREF transparent)
{
    ...
    if ((dcMem = CreateCompatibleDC(NULL)) &&
        ...
        if ((destBmp = CreateBitmap(bitmap.bmWidth,
                                    bitmap.bmHeight,
                                    bitmap.bmPlanes,
                                    bitmap.bmBitsPixel,
                                    NULL))) {
        ...
        if (!bmpName.empty() && bmpName[0] == L'#') {
            bitmap_ =
              (HBITMAP)LoadImage(GetModuleHandle(NULL),
                                 MAKEINTRESOURCE(_wtoi(bmpName.c_str() + 1)),
                                 IMAGE_BITMAP,
                                 width == SIZE_DEFAULT ? 0 : width,
                                 height == SIZE_DEFAULT ? 0 : height,
                                 0);
...
BitmapPtr BitmapManager::find(const BW::string& name,
                              const BW::string& type /* = "NORMAL" */,
                              unsigned int      width /* = SIZE_DEFAULT*/,
                              unsigned int      height /* = SIZE_DEFAULT */)
{
    ...
    if (bitmaps_.find(name) != bitmaps_.end()) {
        ...
        for (BW::vector<BitmapPtr>::iterator iter = bitmaps.begin();
             iter != bitmaps.end();
             ++iter) {
            if ((*iter)->type() == type) {
...
BitmapPtr BitmapManager::get(const BW::string& name,
                             COLORREF          transparent,
                             const BW::string& type /* = "NORMAL" */,
                             unsigned int      width /* = SIZE_DEFAULT*/,
                             unsigned int      height /* = SIZE_DEFAULT */)
...
void BitmapManager::free(BitmapPtr& bitmap)
{
    ...
    for (BW::map<BW::string, BW::vector<BitmapPtr>>::iterator miter =
           bitmaps_.begin();
         miter != bitmaps_.end();
         ++miter) {
