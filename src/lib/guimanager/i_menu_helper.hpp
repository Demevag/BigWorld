...
class IMenuHelper
{
  public:
    ...
    virtual void       setMenuItemInfo(void*        menu,
                                       unsigned int index,
                                       void*        info)             = 0;
    virtual GUI::Item* getMenuItemInfo(void*        menu,
                                       unsigned int index,
                                       void*        typeData)         = 0;
    virtual void       insertMenuItem(void*        menu,
                                      unsigned int index,
                                      GUI::ItemPtr info)       = 0;
    virtual void       modifyMenu(void*          menu,
                                  unsigned int   index,
                                  GUI::ItemPtr   info,
                                  const wchar_t* string)       = 0;
    virtual void       enableMenuItem(void*        menu,
                                      unsigned int index,
                                      MenuState    state)         = 0;
    virtual void       checkMenuItem(void*          menu,
                                     unsigned int   index,
                                     MenuCheckState state)     = 0;

    virtual void setSeperator(void* menu, unsigned int index) = 0;

    virtual void updateText(void*              hMenu,
                            unsigned           index,
                            const BW::wstring& newName) = 0;
