 ...
 */
void CreateBaseReplyHandler::handleMessage(
  const Mercury::Address&         srcAddr,
...
void CreateBaseReplyHandler::handleShuttingDown(const Mercury::NubException& ne,
                                                void* arg)
{
  ...
  void*)
{
    DEBUG_MSG("AsyncControlledShutDownHandler::handleMessage: "
              ...
              srcAddr.c_str(),
              ServerApp::shutDownStageToString(stage_));
        ...
        while (true) {
            if (numToSend_ < int(addrs_.size())) {
                ...
                if (pChannelOwner != NULL &&
                    ...
                    bundle.startRequest(BaseAppIntInterface::controlledShutDown,
                                        this);
...
CheckStatusReplyHandler::CheckStatusReplyHandler(
  const Mercury::Address& srcAddr,
  Mercury::ReplyID        replyID)
  : ForwardingReplyHandler(srcAddr, replyID, BaseAppMgr::instance().interface())
{
}
