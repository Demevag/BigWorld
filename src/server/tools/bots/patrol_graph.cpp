...
namespace Patrol {
    ...
    {
        if (props_.minSpeed() <= 0.f || props_.maxSpeed() <= 0.f)
            return ospeed;
    ...
    bool Graph::init(DataSection* pSection, float scalePos, float scaleSpeed)
    {
        ...
        for (int i = 0; i < numNodes; ++i) {
            ...
            if (!nodes_[i].init2(nodeMap, pNode.get())) {
                ERROR_MSG(
                  "Graph::init: Failed to initialise edges for node %d\n", i);
                return false;
            }
        }
    ...
    {
        char dnbuf[1024];
        if (graphName.size() > sizeof(dnbuf) - 64)
            return NULL;
      ...
      , graph_(graph)
    {
        ...
        if (snap) { // startPosition is modified below:
     ...
     */
    bool GraphTraverser::nextStep(float&       speed,
                                  float        dTime,
                                  Vector3&     pos,
                                  Direction3D& dir)
    {
    ...
    namespace {
        ...
        class PatrolFactory : public MovementFactory
        {
          public:
             ...
             */
            MovementController* create(const BW::string& data,
                                       float&            speed,
                                       Vector3&          position)
            {
                BW::string graphName;
                ...
                Graph* pGraph =
                  graphs_.getGraph(graphName, scalePos, scaleSpeed);

                if (pGraph) {
                    return new GraphTraverser(
                      *pGraph, speed, position, randomise, snap);
                }

                PyErr_Format(PyExc_IOError,
                             "Unable to load graph '%s'",
                             graphName.c_str());
                return NULL;
            }
