...
int main(int argc, char* argv[])
{
    ...
    if (argc <= 1) {
        return becomeRootUser() ? SnapshotHelper::SUCCESS
                                : SnapshotHelper::FAILURE;
    }
...
bool validate_config(ConfigReader& config)
{
    ...
    const char* config_values[] = {
        "datadir", "lvgroup", "lvorigin", "lvsnapshot", "lvsizegb"
    };
                      ...
                      const char*   dbPass)
{
    ...
    if (!mysql_real_connect(
          &sql, "localhost", dbUser, dbPass, NULL, 0, NULL, 0)) {
    ...
    // TODO: anything past this point must LVREMOVE as the LVCREATE has already
    // worked,
