...
static PyMethodDef PyQueryResult_methods[] = {
    { "format",
      (PyCFunction)PyQueryResult_format,
      METH_VARARGS,
      "Docs for format" },
    { "metadata",
      (PyCFunction)PyQueryResult_metadata,
      METH_VARARGS,
      "Return the meta-data associated with the log message (if any)" },
    { "getTime",
      (PyCFunction)PyQueryResult_getTime,
      METH_VARARGS,
...
PyObject* PyQueryResult::py_format(PyObject* args)
{
    if (pQueryResult_ == NULL) {
        PyErr_Format(PyExc_ValueError,
                     "Query result has not been properly "
                     "initialised.");
        return NULL;
    }
...
PyQueryResult::PyQueryResult(const LogEntry&         entry,
                             PyBWLogPtr              pBWLog,
  ...
  pQueryResult_(new QueryResult(entry,
                                pBWLog->getBWLogReader(),
                                pUserLog.getObject(),
                                pComponent,
                                message,
                                metadata))
...
PyObject* PyQueryResult::pyGet_message()
{
    ...
    return PyString_FromStringAndSize(const_cast<char*>(message.data()),
                                      message.size());
}
...
{
    return PyString_InternFromString(const_cast<char*>(this->getUsername()));
}
...
{
    return PyString_InternFromString(const_cast<char*>(this->getComponent()));
}
