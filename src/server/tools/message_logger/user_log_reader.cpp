...
bool UserLogReader::loadSegments()
{
    ...
    int             numEntries =
      scandir(path_.c_str(), &namelist, UserSegmentReader::filter, NULL);
    ...
    for (int i = 0; i < numEntries; i++) {
        ...
        if (existingIndex != -1) {
            UserSegmentReader* pSegment =
              static_cast<UserSegmentReader*>(userSegments_[existingIndex]);
            if (pSegment->isDirty()) {
...
bool UserLogReader::getEntryAndQueryRange(
  const LogEntryAddress& logEntryAddress,
  LogEntry&              result,
                             ...
                             LogEntry&              result)
{
    ...
    return this->getEntryAndSegment(
      logEntryAddress, result, pSegment, /*shouldEmitWarnings*/ true);
}
...
bool UserLogReader::getEntryAndSegment(const LogEntryAddress& logEntryAddress,
                                       LogEntry&              result,
                                       UserSegmentReader*&    pSegmentResult,
