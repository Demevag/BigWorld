...
{
    if (!begin_.isGood() || !end_.isGood() || !curr_.isGood() ||
        !(curr_ <= end_)) {
        return false;
    }
    ...
    if (status) {
        args_ = QueryRangeIterator(
          this, curr_.getSegmentNumber(), entry.argsOffset());
 ...
 *
 */
bool QueryRange::getMetaDataFromEntry(const LogEntry& entry, BW::string& result)
{
    const UserSegmentReader* pSegment = args_.getSegment();
    return const_cast<UserSegmentReader*>(pSegment)->metadata(entry, result);
}
...
bool QueryRange::seek(int segmentNum,
                      int entryNum,
                      int metaOffset,
                      int postIncrement)
{
