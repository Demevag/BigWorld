...
namespace MongoDB {
     ...
     */
    UserLogWriter::UserLogWriter(ConnectionThreadData& connectionData,
     ...
     * server startup collection and shard the database if connecting to a
     * MongoDB cluster.
     */
    bool UserLogWriter::initDB()
    {
        ...
        try {
            ok = this->initUIDCollection() && this->initServerStartsColl() &&
    ...
    bool UserLogWriter::writeComponent(mongo::BSONObj& obj)
    {
        ...
        MongoDB::CollList::const_iterator iter = std::find(
          collList.begin(), collList.end(), componentCollName.c_str());
        ...
        try {
            ...
            std::auto_ptr<mongo::DBClientCursor> cursor =
              connectionData_.conn_.query(componentCollName.c_str(),
                                          mongo::BSONObj(objBuilder.obj()));

            if (!cursor.get()) {
                ERROR_MSG(
                  "UserLogWriter::writeComponent: Couldn't get cursor.\n");
                return false;
            }
    ...
    {
        BW::string startupCollName = dbName_ + "." + SERVER_STARTS_COLL_NAME;

        mongo::Date_t  timeVal(timestamp);
        mongo::BSONObj recObj = BSON(SERVER_STARTS_KEY_TIME << timeVal);
        ...
        try {
            ...
            ERROR_MSG("UserLogWriter::onServerStart: "
                      "Got exception when inserting server start time to %s, "
                      "error: %s\n",
                      startupCollName.c_str(),
                      ex.what());
     ...
     * collections whose last entry is older than seconds will be seen as
     * expired.
     */
    bool UserLogWriter::rollAndPurge(uint64 seconds)
    {
        // Roll up active collection by clearing the active collection name. A
        // new collection will be created when new log arrives. We don't create
        // a new one here because we may end up with empty collection if no new
        // logs.
    ...
    /**
     * Purge expired log collections. Log collections whose last entry is older
     * than seconds will be seen as expired.
     */
    bool UserLogWriter::purgeExpiredColls(uint64 seconds)
    {
        ...
        uint32 entryCollNameLength =
          entryCollNamePrefix.length() + COLL_SUFFIX_LENGTH;
        ...
        while (iter != collList.end()) {
            ...
            if ((collNamespace.length() == entryCollNameLength) &&
                (collNamespace.compare(0,
                                       entryCollNamePrefix.length(),
                                       entryCollNamePrefix) == 0)) {
                BW::string timeSuffix =
                  collNamespace.substr(entryCollNamePrefix.length());

                // drop the collection if it's a entry collection and expired
                if (this->isCollectionExpired(
                      collNamespace, timeSuffix, seconds)) {
    ...
    /**
     * Create the server start ups collection for the user database. Limit the
     * total size of it by creating it as Capped Collection.
     *
     * Exceptions raised by MongoDB should be handled by caller.
     */
    bool UserLogWriter::initServerStartsColl()
    {
        BW::string startupsCollName = dbName_ + "." + SERVER_STARTS_COLL_NAME;

        if (!connectionData_.conn_.createCollection(
              startupsCollName.c_str(),
    ...
    bool UserLogWriter::initComponentsColl()
    {
        ...
        if (!connectionData_.conn_.createCollection(
              componentCollName.c_str())) {
            ERROR_MSG(
              "UserLogWriter::initComponentsColl: "
     ...
     * parent database should be sharded as well. This should only be called
     * when Message Logger is connecting to a MongoDB cluster.
     *
     * Exceptions raised by MongoDB should be handled by caller.
     */
    bool UserLogWriter::shardDatabase()
    {
        ...
        ok = connectionData_.conn_.runCommand(ADMIN_DB_NAME, shardObj, result);
        if (!ok) {
            BW::string errMsg = result.getStringField("errmsg");
            ERROR_MSG(
              "UserLogWriter::shardDatabase: "
    ...
    /**
     * Create a new collection and use it as active collection. New logs after
     * this will be inserted into this active collection. Enable sharding of the
     * new collection if Message Logger is connected with a MongoDB cluster.
     */
    bool UserLogWriter::initActiveColl()
    {
        ...
        try {
            ...
            if (connectionData_.getIsCluster()) {
                ...
                // - low cardinality field from LogStorageMongoDB::getShardKey
                // that
                ...
                objBuilder.append("key",
                                  mongo::BSONObjBuilder()
                                    .append(ENTRIES_KEY_SHARD_KEY, 1)
                                    .append(ENTRIES_KEY_TIMESTAMP, 1)
                                    .obj());

                ok = connectionData_.conn_.runCommand(
                  ADMIN_DB_NAME, objBuilder.done(), result);

                if (!ok) {
                    BW::string errMsg = result.getStringField("errmsg");
                    ERROR_MSG("UserLogWriter::initActiveColl: "
                              "MongoDB returned error when sharding "
                              "collection(%s): %s\n",
                              newCollName.c_str(),
                              errMsg.c_str());
    ...
    /**
     * Get new collection name, in the form entries_<timestamp>. The timestamp
     * is in the form YYYYMMDDHHMMSS of current UTC time, like
     * entries_20140820101400
     */
    ...
    /**
     * Get the timestamp suffix based on current time and in the form
     * YYYYMMDDHHMMSS of UTC time, like entries_20140820101400
     */
    ...
    /**
     * Check if the given collection is expired by checking if its last record
     * is older than seconds.
     */
    bool UserLogWriter::isCollectionExpired(const BW::string& collNS,
                                            const BW::string& timeSuffix,
                                            uint64            seconds)
    {
        time_t now = time(NULL);

        // Check the collection creation time first. If the collection is
        // created no earlier than seconds, it shouldn't be expired
        struct tm timeInfo;
        int       count = sscanf(timeSuffix.c_str(),
                           "%04d%02d%02d%02d%02d%02d",
                           &timeInfo.tm_year,
                           &timeInfo.tm_mon,
                           &timeInfo.tm_mday,
        ...
        if (count != 6) {
            ERROR_MSG(
              "UserLogWriter::isCollectionExpired:"
        ...
        try {
            ...
            query.sort(
              BSON(ENTRIES_KEY_TIMESTAMP << -1 << ENTRIES_KEY_COUNTER << -1));

            // only return timestamp and counter so that the query can be
            // covered by index
            mongo::BSONObj returnPattern =
              BSON(ENTRIES_KEY_TIMESTAMP << 1 << ENTRIES_KEY_COUNTER << 1);

            mongo::BSONObj lastEntry = connectionData_.conn_.findOne(
              collNS.c_str(), query, &returnPattern);
            ...
            time_t logTime =
              lastEntry.getField(ENTRIES_KEY_TIMESTAMP).date().toTimeT();
