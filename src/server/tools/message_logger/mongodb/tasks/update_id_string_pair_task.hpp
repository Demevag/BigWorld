    ...
    class UpdateIDStringPairTask : public MongoDBBackgroundTask
    {
      ...
      protected:
                                   ...
                                   ConnectionThreadData& connectionData)
        {
            ...
            mongo::BSONObj existingRecord =
              mongo::BSONObjBuilder().append(idKey_, id_).obj();
            ...
            if (status == DB_TASK_GENERAL_ERROR) {
                ERROR_MSG(
                  "UpdateIDStringPairTask::performBackgroundTask: "
                ...
                // If the DB reconnects, ensure this task is performed before
                // any logs are flushed.
