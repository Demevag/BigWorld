...
namespace MongoDB {
    ...
    class UserLogBuffer : public SafeReferenceCount
    {
      public:
        ...
        void append(mongo::BSONObj logBson) { logBuffer_.push_back(logBson); }

        void clear() { logBuffer_.clear(); }

        BSONObjBuffer::size_type size() { return logBuffer_.size(); }

        // Must be publicly visible as non-const so that it can be swapped into
        // a flush task.
