...
namespace MongoDB {
    ...
    // Version 2: Split collections, multiple Message Loggers with one MongoDB,
    // BW2.10
    ...
    // Number of seconds to wait before reattempting connection when
    // autoreconnect has failed.
    ...
    // reallocations when many thousands of log messages are added per second in
    // a production environment.
    static const uint32 MAX_LOG_BUFFER_SIZE = 1024;

    // The amount of memory to reserve for each log buffer. This reduces vector
    // reallocations. In an environment with a lot of logs (such as production)
    // this becomes most effective. In an environment with few logs (such as
    // dev) then a little extra memory usage should not hurt when there are so
    // few logs anyway.
    ...
    // This must be a character valid for MongoDB database name but not valid
    // for a user name.
