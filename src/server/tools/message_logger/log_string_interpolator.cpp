...
namespace {
    ...
    const MessageLogger::FormatStringOffsetId FORMAT_STRING_EOF_MARKER(
      0xffffffff);
}
 ...
 *	Override from FormatStringHandler.
 */
void LogStringInterpolator::onToken(char  type,
                                    int   cflags,
                                    int   min,
                                    int   max,
                                    int   flags,
                                    ...
                                    int   vflags)
{
    fmtData_.push_back(FormatData(type, cflags, base, min, max, flags, vflags));
 ...
 *
 */
bool LogStringInterpolator::streamToString(
 ...
 *
 */
bool LogStringInterpolator::streamToLog(
                    ...
                    switch (fd.cflags_) {
                        ...
                        case DP_C_LONG: {
                            ...
                            if (version < MESSAGE_LOGGER_VERSION_64BIT) {
                                ...
                                // We've just read from a 32-bit version client,
                                // but write it out as if it were 64-bit
                                // version.
                                finalVal = val;
                            } else {
                                inputStream >> finalVal;
                                CHECK_STREAM();
                            }
