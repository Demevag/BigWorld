...
bool HostnamesValidatorMLDB::init(const char* logLocation)
{
    ...
    const char* hostnamesFullFilePath =
      this->join(tempDirPath_.c_str(), HostnamesMLDB::getHostnamesFilename());
...
BW::string HostnamesValidatorMLDB::getTempPath(const char* pBasePath) const
{
    ...
    char* pTempDirname =
      strdup(FileHandler::join(pBasePath, "/validate_hostnames-XXXXXX"));
