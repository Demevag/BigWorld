...
class MessageWithDestination : public MGM
{
  public:
     ...
     *	Destructor.
     */
    virtual ~MessageWithDestination() { ep_.detach(); }

    /**
     *	This method sets the values to be fed into MGM->sendto() later.
     */
    void target(const Endpoint& ep,
                const uint16    port,
                const uint32    addr      = BROADCAST,
                const uint8     packFlags = 0)
    {
...
typedef MessageWithDestination<HighPrecisionMachineMessage>
  HighPrecisionMachineMessageWithDestination;
typedef MessageWithDestination<WholeMachineMessage>
  WholeMachineMessageWithDestination;
typedef MessageWithDestination<ProcessMessage> ProcessMessageWithDestination;
typedef MessageWithDestination<ProcessStatsMessage>
  ProcessStatsMessageWithDestination;
...
typedef MessageWithDestination<MachinedAnnounceMessage>
  MachinedAnnounceMessageWithDestination;
typedef MessageWithDestination<QueryInterfaceMessage>
  QueryInterfaceMessageWithDestination;
typedef MessageWithDestination<CreateWithArgsMessage>
  CreateWithArgsMessageWithDestination;
