                             ...
                             in_addr               addr)
{
    ...
    while (iter != members_.end()) {
        ...
        if (lm.category_ == pm.category_ &&
            ...
            int msglen =
              lm.preAddr_.size() + sizeof(address) + lm.postAddr_.size();
            ...
            memcpy(
              data + preSize + sizeof(address), lm.postAddr_.c_str(), postSize);
...
void Listeners::checkListeners()
{
    for (Members::iterator it = members_.begin(); it != members_.end(); it++) {
        if (!serverPlatform_.isProcessRunning(it->lm_.pid_)) {
            syslog(LOG_INFO,
                   "Dropping dead listener (for %s's) with pid %d",
                   it->lm_.name_.c_str(),
                   it->lm_.pid_);
            members_.erase(it--);
        }
