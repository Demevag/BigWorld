  ...
  const EntityDefs&               entityDefs)
{
    for (int i = 0; i < numConnections_; ++i) {
        bgTaskMgr_.startThreads(1, new MySqlThreadData(connectionInfo));
    }
                                           ...
                                           GameTime         time)
{
    ...
    if (pEntityTypeMapping == NULL) {
        WARNING_MSG("PrimaryDatabaseUpdateQueue::addUpdate: Entity with id"
                    " \'%d\' is invalid. Skipping it.",
                    key.typeID);
        return;
    }

    EntityTask* pTask = new ConsolidateEntityTask(
      pEntityTypeMapping, key.dbID, data, time, *this);
                                                     ...
                                                     DatabaseID dbID)
{
    if (!isOkay) {
        ERROR_MSG("PrimaryDatabaseUpdateQueue::onPutEntity: "
                  "could not write entity with DBID=%" FMT_DBID
                  " to database\n",
                  dbID);
        hasError_ = true;
    }
