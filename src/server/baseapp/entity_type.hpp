...
class EntityType : public ReferenceCount
{
  public:
    ...
    EntityType(const EntityDescription& desc,
               PyTypeObject*            pType,
               bool                     isProxy);
    ~EntityType();
    ...
    BasePtr create(EntityID id, const BW::string& templateID);
    ...
    const EntityDescription& description() const { return entityDescription_; }
    ...
    static const EntityDescription* getDescription(
      EntityTypeID typeID,
      bool         shouldExcludeServices = false);
