 ...
 */
shared_ptr<EntityCreator> EntityCreator::create(
  DBApp&                     idOwner,
  Mercury::NetworkInterface& intInterface)
{
    shared_ptr<EntityCreator> pCreator(new EntityCreator(intInterface));
...
PyObject* EntityCreator::createBaseLocally(PyObject* args, PyObject* kwargs)
{
    ...
    if (pCellData) {
        ...
        PyDict_SetItemString(
          pCellData.get(), POSITION_PROPERTY_NAME, pVec3Position);
        Py_DECREF(pVec3Position);

        PyObject* pVec3Direction = Script::getData(Vector3(0, 0, 0));
        PyDict_SetItemString(
          pCellData.get(), DIRECTION_PROPERTY_NAME, pVec3Direction);
    ...
    for (uint i = 0; i < count; i++) {
        ...
        if (pValue) {
            if (!pDataDesc->dataType()->isSameType(ScriptObject(
...
class CreateBaseReplyHandler : public Mercury::ReplyMessageHandler
{
  ...
  private:
                       ...
                       void* /*arg*/)
    {
        ...
        this->callback(ScriptObject(PyEntityMailBox::constructFromRef(baseRef),
...
class CreateBaseFromDBReplyHandler : public Mercury::ReplyMessageHandler
{
  ...
  private:
                       ...
                       void* /*arg*/)
    {
        ...
        if (status != LOAD_FROM_DB_FAILED) {
            ...
            this->callback(PyEntityMailBox::constructFromRef(baseRef),
                           dbID,
                           (status == LOAD_FROM_DB_FOUND_EXISTING));
        } else {
            this->callback();
        }
    ...
    void callback(PyObject*  pArg      = NULL,
                  DatabaseID dbID      = 0,
                                          ...
                                          bool                    hasCallback)
{
    ...
    PyObjectPtr pDict =
      kwargs ? kwargs : PyObjectPtr(PyDict_New(), PyObjectPtr::STEAL_REFERENCE);

    EntityTypePtr pType =
      this->consolidateCreateBaseArgs(pDict, args, headOffset, tailOffset);
    ...
    if (baseAppAddr == intInterface_.address()) {
        ...
        Mercury::ReplyMessageHandler* pHandler = new CreateBaseReplyHandler(
        ...
        if (this->addCreateBaseData(
              *pBundle, pType, pDict, pCallback, &newID)) {
 ...
 */
EntityTypePtr EntityCreator::consolidateCreateBaseArgs(PyObjectPtr pDestDict,
                                                       PyObject*   args,
                                                       int         headOffset,
                                                       int tailOffset) const
{
    ...
    if (!PyString_Check(pPyTypeName)) {
        PyErr_SetString(PyExc_TypeError, "Expected a string as first argument");
        return NULL;
    }
    ...
    for (int i = headOffset; i < numArgs - tailOffset; ++i) {
        ...
        if (PyDict_Check(pCurr.get())) {
            ...
            if (!pType->description().addSectionToDictionary(
                  pDataSection,
                  ScriptDict(pCurr),
            ...
            PyErr_Format(PyExc_TypeError,
                         "Expect dict or PyDataSection for argument %d",
                         i + 1);
            return NULL;
        }
                                      ...
                                      EntityID*      pNewID)
{
    ...
    if (!pType->description().addDictionaryToStream(
          ScriptDict(pDict),
          stream,
                                     ...
                                     PyObjectPtr       pResultHandler)
{
    ...
    if (!pType) {
        PyErr_Format(
          PyExc_ValueError, "Invalid entity type %s", entityType.c_str());
        return false;
    }
    ...
    Mercury::ReplyMessageHandler* pHandler =
      new LoadEntityHandlerWithCallback(pResultHandler, pType->id(), entityID);
 ...
 */
void EntityCreator::createBaseFromDB(
  ...
  BinaryIStream&                        data)
{
    ...
    Mercury::ReplyMessageHandler* pHandler = new LoadEntityHandlerWithReply(
      header.replyID, srcAddr, entityTypeID, entityID);
 ...
 */
PyObject* EntityCreator::createRemoteBaseFromDB(
  ...
  const char*             origAPIFuncName)
{
    ...
    if (pEntityType == NULL) {
        PyErr_Format(PyExc_ValueError, "Invalid entity type %s", entityType);
        return NULL;
    }
    ...
    } else if (!pDestAddr) { // Create base anywhere
...
BasePtr EntityCreator::createBaseFromStream(BinaryIStream&    data,
                                            Mercury::Address* pClientAddr,
                                            BW::string*       pEncryptionKey)
{
    EntityID     id = 0;
    EntityTypeID type;
    ...
    BasePtr pBase =
      pType->create(id, databaseID, data, hasPersistentDataOnly, pLogOnData);
 ...
 *	proxy or base entities.
 */
void EntityCreator::createBaseWithCellData(
  const Mercury::Address&               srcAddr,
  const Mercury::UnpackedMessageHeader& header,
  BinaryIStream&                        data,
  ...
  const ScriptObject& callback)
{
    ...
    const Mercury::Address& baseAppAddr =
      this->shouldCreateLocally() ? intInterface_.address()
                                  : this->getAddressFromDestinationBaseApps();

    if (baseAppAddr != intInterface_.address()) // remote baseapp
    {
        return this->createBaseRemotelyFromTemplate(
          baseAppAddr, entityType, templateID, callback);
    }
 ...
 *	proxy or base entities.
 */
void EntityCreator::createBaseFromTemplate(
  const Mercury::Address&               srcAddr,
  const Mercury::UnpackedMessageHeader& header,
  BinaryIStream&                        data)
{
    ...
    if (data.error()) {
        ...
        if (!pBase) {
            ERROR_MSG(
              "EntityCreator::createBaseFromTemplate: "
...
void EntityCreator::handleBaseAppDeath(const Mercury::Address& addr)
{
    ...
    if (destinationBaseApps_.empty()) {
        INFO_MSG(
          "EntityCreator::handleBaseAppDeath: "
