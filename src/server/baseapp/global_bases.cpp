 ...
 *  @components{ base }
 *  values return a base (or base mailbox) with the input key or the default
 * value if not found.
...
namespace {
    ...
    {
      public:
        MakeGlobalHandler(GlobalBases* pGlobalBases,
                          Base*        pBase,
                          BW::string   label,
                          PyObject*    pCallback)
 ...
 */
bool GlobalBases::registerRequest(Base*     pBase,
                                  PyObject* pKey,
                                  PyObject* pCallback)
{
...
bool GlobalBases::deregister(Base* pBase, PyObject* pKey)
{
    ...
    if (range.first != range.second) {
        ...
        BaseEntityMailBox* pMailbox = static_cast<BaseEntityMailBox*>(pValue);
        ...
        if (bases.findEntity(pMailbox->id())) {
            ...
            // now deregister it
            BaseApp::instance().baseAppMgr().deregisterBaseGlobally(pickledKey);
...
void GlobalBases::add(BinaryIStream& data)
{
    ...
    EntityTypePtr pBaseType =
      EntityType::getType(mailboxRef.type(),
                          /*shouldExcludeServices*/ true);

    if (pBaseType) {
        BaseEntityMailBox* pMailBox =
          new BaseEntityMailBox(pBaseType, mailboxRef.addr, mailboxRef.id);
...
void GlobalBases::add(Base* pBase, const BW::string& pickledKey)
{
    ...
    if (!BaseAppConfig::shouldResolveMailBoxes()) {
        pMailBox =
          PyEntityMailBox::constructFromRef(pBase->baseEntityMailBoxRef());
    } else {
        Py_INCREF(pMailBox);
    }
 ...
 */
void GlobalBases::add(BaseEntityMailBox* pBase, const BW::string& pickledKey)
{
    ScriptObject pKey = Pickler::unpickle(pickledKey);
...
void GlobalBases::resolveInvalidMailboxes()
{
    ...
    while (pMap.next(pos, pKey, pValue)) {
        ...
        if (BaseEntityMailBoxPtr pMailbox =
              BaseEntityMailBoxPtr::create(pValue)) {
            if ((pMailbox->address() ==
                 BaseApp::instance().intInterface().address())) {
                if (!bases.findEntity(pMailbox->id())) {
                    const ScriptString pKeyStrObj(pKey.str(ScriptErrorPrint()));
