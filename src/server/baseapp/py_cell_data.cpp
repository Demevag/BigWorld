 ...
 */
PyCellData::PyCellData(EntityTypePtr  pEntityType,
                       BinaryIStream& data,
                       bool           persistentDataOnly,
                       PyTypeObject*  pType)
  : PyObjectPlus(pType)
...
namespace {
    SpaceID getSpaceIDFromMap(const ScriptMapping& map)
    {
        ...
        if (map) {
            ScriptObject id =
              map.getItem(SPACE_ID_PROPERTY_NAME, ScriptErrorClear());
            if (id) {
...
Vector3 PyCellData::getPosOrDir(PyObjectPtr pCellData,
                                const char* pName,
                                int         offset)
{
...
Vector3 PyCellData::getPos(PyObjectPtr pCellData)
{
    ...
    return PyCellData::getPosOrDir(
      pCellData, POSITION_PROPERTY_NAME, 2 * sizeof(Vector3) + sizeof(SpaceID));
}
...
Direction3D PyCellData::getDir(PyObjectPtr pCellData)
{
    ...
    Vector3 dirVector = PyCellData::getPosOrDir(
      pCellData, DIRECTION_PROPERTY_NAME, sizeof(Vector3) + sizeof(SpaceID));
...
SpaceID PyCellData::getSpaceID(PyObjectPtr pCellData)
{
    if (PyCellSpatialData::Check(pCellData.get())) {
        ...
        if (!pCellData) {
            return *(SpaceID*)(pCD->data_.data() + pCD->data_.size() -
                               sizeof(SpaceID));
        }
    }
...
void PyCellData::addSpatialData(PyObjectPtr        pCellData,
                                const Vector3&     pos,
 ...
 */
bool PyCellData::addToStream(BinaryOStream& stream,
                             bool           addPosAndDir,
                             bool           addPersistentOnly)
{
    ...
    if (pDict_) {
        isOK = PyCellData::addMapToStream(stream,
                                          pEntityType_,
                                          ScriptMapping(pDict_),
                                          addPosAndDir,
                                          addPersistentOnly);
    } else {
        if (dataHasPersistentOnly_ == addPersistentOnly) {
            ...
            if (isOK) {
                isOK = this->addMapToStream(stream,
                                            pEntityType_,
                                            ScriptMapping(pDict_),
                                            addPosAndDir,
                                            addPersistentOnly);
                                ...
                                bool                 addPersistentOnly)
{
    ...
    if (pEntityType->description().addDictionaryToStream(
          map, stream, dataDomain)) {
        if (addPosAndDir) {
...
PyObjectPtr PyCellData::createPyDictOnDemand()
{
    if (!pDict_) {
        if (!data_.empty()) {
            ...
            pDict_ =
              pEntityType_->createCellDict(stream, dataHasPersistentOnly_);
 ...
 *	@param addPersistentOnly	Indicates whether or not non-persistent data
 *should be added to the stream.
 */
bool PyCellData::addToStream(BinaryOStream& stream,
                             EntityTypePtr  pType,
                             PyObject*      pCellData,
                             ...
                             bool           addPersistentOnly)
{
    if (PyCellData::Check(pCellData)) {
        return ((PyCellData*)pCellData)
          ->addToStream(stream, addPosAndDir, addPersistentOnly);
    } else if (ScriptMapping map =
                 ScriptMapping::create(ScriptObject(pCellData))) {
        return PyCellData::addMapToStream(
          stream, pType, map, addPosAndDir, addPersistentOnly);
    }
