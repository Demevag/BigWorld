 ...
 * 		   "BigWorld.tcpEcho( &lt;ip_addr&gt;, &lt;port&gt;, &lt;message&gt;[,
 *<callback>] )", where the &lt;ip_addr&gt; and &lt;port&gt; is the IP address
 *and port of the TCP server and <message> is a message you want to send to the
 * 		   server. e.g.
 ...
 * 	is mainly because of the need to access the global Mercury::NetworkInterface
 *The way to access this global is similar but not identical on all server
 * 	components.
 */
...
class TCPEcho : public Mercury::InputNotificationHandler
{
  public:
    ...
    virtual ~TCPEcho() { delete[] pRecieveBuf_; }

    void init(const BW::string& ip,
              uint16            port,
              const BW::string& msg,
  ...
  protected:
    virtual int handleInputNotification(int fd)
    {
        ...
        Mercury::EventDispatcher& dispatcher =
          BaseApp::instance().mainDispatcher();
        ...
        if (!readyToWrite_) {
            ...
            if (sendSize < 0) {
                ...
                dispatcher.registerFileDescriptor(
                  endpoint_.fileno(), this, "TCPEcho");
            }
        } else {
            ...
            if (size <= 0) {
                if (size < 0) {
                    ...
                    this->doCallback(false,
                                     BW::string(pRecieveBuf_, numReceived_));
                }
    ...
    void doCallback(bool isError, const BW::string& msg)
    {
        if (pCallback_) {
            ...
            Script::call(
              pCallback_.get(),
              Py_BuildValue("(Os)", isError ? Py_True : Py_False, msg.c_str()),
              "TCPEcho callback",
              /*okIfFnNull:*/ false);
        } else {
            if (isError)
 ...
 */
bool tcpEcho(const BW::string& ip,
             uint16            port,
             const BW::string& msg,
             PyObjectPtr       pCallback)
{
...
PY_AUTO_MODULE_FUNCTION(
  RETOK,
  tcpEcho,
  ARG(BW::string, ARG(uint16, ARG(BW::string, OPTARG(PyObjectPtr, NULL, END)))),
  BigWorld)
