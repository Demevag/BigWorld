...
class Proxy : public Base
{
    Py_Header(Proxy, Base)
      ...
      public : Proxy(EntityID id, DatabaseID dbID, EntityType* pType);
    ~Proxy();
    ...
    void transferClient(const Mercury::Address&       dstAddr,
                        bool                          shouldReset,
    ...
    {
        return pClientChannel() != NULL && pClientChannel()->isConnected();
    }
    ...
    PY_AUTO_METHOD_DECLARE(RETOWN,
                           ...
                           ARG(PyObjectPtr,
                               OPTARG(PyObjectPtr, NULL, OPTARG(int, -1, END))))
    PyObject* streamStringToClient(PyObjectPtr pData,
                                   PyObjectPtr pDesc,
                                   int         id);
    ...
    void enterAoIOnVehicle(
      const BaseAppIntInterface::enterAoIOnVehicleArgs& args);
...
#undef MF_COMMON_OSTREAM
  ...
  private:
    ...
    {
      public:
        ClientBundlePrimer(Proxy& proxy)
          : proxy_(proxy)
        {
        }
    ...
    void sendMessageToClientHelper(BinaryIStream& data, bool isReliable = true);
