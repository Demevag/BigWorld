...
Mercury::UDPChannel* CommonCellEntityMailBox::pChannel() const
{
    ...
    // Disallow the call if we don't have a cell entity and we're not pending
    // cell creation, or if we're pending cell destruction.

    if ((!pBase->hasCellEntity() && !pBase->isGetCellPending()) ||
...
{
    return this->getStreamCommon(
      methodDesc, CellAppInterface::runScriptMethod, pHandler);
}
...
static class BaseAppPostOfficeAttendant
{
  public:
    BaseAppPostOfficeAttendant()
    {
        ...
        PyEntityMailBox::registerMailBoxRefEquivalent(Base::Check, baseReduce);
    }
    ...
    static PyObject* newCellMB(const EntityMailBoxRef& ref)
    {
        ...
        EntityTypePtr pLocalType =
          EntityType::getType(ref.type(),
