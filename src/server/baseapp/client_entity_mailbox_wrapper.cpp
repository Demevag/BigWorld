...
PY_TYPEOBJECT_SPECIALISE_REPR_AND_STR(ClientEntityMailBoxWrapper,
                                      &reprClientEntityMailBoxWrapper,
                                      &reprClientEntityMailBoxWrapper);
  ...
  const ScriptString& attrObj)
{
    const MethodDescription* pDescription =
      pMailBox_->findMethod(attrObj.c_str());

    if (pDescription) {
        return ScriptObject(
          new RemoteClientMethod(pMailBox_,
...
PyObject* ClientEntityMailBoxWrapper::pyRepr() const
{
    ...
    return PyString_FromFormat(
      "Client MailBox %s(%d).ownClient%s",
 ...
 */
PyObject* ClientEntityMailBoxWrapper::pyCall(PyObject* args, PyObject* kwargs)
{
