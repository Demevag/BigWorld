 ...
 *  Constructor.
 */
LoginHandler::LoginHandler(LogOnParamsPtr          pParams,
 ...
 */
void LoginHandler::onGetEntityKeyForAccountSuccess(
 ...
 *  IGetEntityKeyForAccountHandler override
 */
void LoginHandler::onGetEntityKeyForAccountCreateNew(
  EntityTypeID      typeID,
  bool              shouldRemember,
  const BW::string& dataForClient,
  const BW::string& dataForBaseEntity)
...
void LoginHandler::onSetEntityKeyForAccountFailure(const EntityKey& key)
{
    ...
    {
      public:
        virtual void onDelEntityComplete(bool isOK) { delete this; }
    };

    DBApp::instance().delEntity(EntityDBKey(key), 0, *(new DelEntityNoOp()));
 ...
 */
void LoginHandler::onGetEntityCompleted(
  bool                    isOK,
  ...
  const EntityMailBoxRef* pBaseEntityLocation)
{
    if (!isOK) {
        if (shouldCreateNewOnLoadFailure_) {
            ...
            this->sendFailureReply(LogOnStatus::LOGIN_REJECTED_NO_SUCH_USER,
...
void LoginHandler::checkOutEntity()
{
    if ((pBaseRef_ == NULL) &&
        ...
        DBApp::instance().setBaseEntityLocation(
          entityKey_, baseRef_, reserveBaseMailboxHandler_);
...
void LoginHandler::onPutEntityComplete(bool isOK, DatabaseID dbID)
{
    ...
    *pStrmDbID_ = dbID;
    ...
    } else { // Failed "rememberEntity" function.
...
void LoginHandler::onReservedBaseMailbox(bool isOK, DatabaseID dbID)
{
    if (isOK) {
        ...
        this->sendFailureReply(LogOnStatus::LOGIN_REJECTED_DB_GENERAL_FAILURE,
...
{
    DBApp::instance().interface().sendOnExistingChannel(replyAddr_, bundle_);
 ...
 *  also destroys this object.
 */
inline void LoginHandler::sendFailureReply(LogOnStatus status, const char* msg)
{
    DBApp::instance().sendFailure(replyID_, replyAddr_, status, msg);
    delete this;
}
                                   ...
                                   bool         shouldRemember)
{
    ...
    const BW::string& newEntityName =
      entityKey_.name.empty() ? pParams_->username() : entityKey_.name;
                                 ...
                                 void*                           arg)
{
    ...
    if (proxyAddr.ip == 0) {
        ...
        if (entityKey_.dbID != 0) {
            pBaseRef_ = &baseRef_;
            DBApp::instance().setBaseEntityLocation(
              entityKey_, baseRef_, setBaseMailboxHandler_);
 ...
 *  Mercury::ReplyMessageHandler override.
 */
void LoginHandler::handleException(const Mercury::NubException& ne, void* arg)
{
    MemoryOStream mos;
    mos << "BaseAppMgr timed out creating entity.";
    this->handleFailure(&mos, LogOnStatus::LOGIN_REJECTED_BASEAPPMGR_TIMEOUT);
}
