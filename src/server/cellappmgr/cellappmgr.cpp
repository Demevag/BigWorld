...
bool CellAppMgr::init(int argc, char* argv[])
{
    ...
    if (Config::metaLoadBalancePeriod() > 0.0) {
        metaLoadBalanceTimer_ = this->mainDispatcher().addTimer(
          ...
          this,
          (void*)TIMEOUT_META_LOAD_BALANCE,
          "MetaLoadBalance");
    }
    ...
    Mercury::MachineDaemon::registerBirthListener(
      interface_.address(),
      CellAppMgrInterface::handleBaseAppMgrBirth,
      "BaseAppMgrInterface");
    ...
    Mercury::MachineDaemon::registerBirthListener(
      interface_.address(),
      CellAppMgrInterface::handleDBAppMgrBirth,
      "DBAppMgrInterface");
    ...
    Mercury::MachineDaemon::registerBirthListener(
      interface_.address(),
      CellAppMgrInterface::handleCellAppMgrBirth,
      "CellAppMgrInterface");
    Mercury::MachineDaemon::registerDeathListener(
      interface_.address(),
      CellAppMgrInterface::handleCellAppDeath,
      "CellAppInterface");
...
void CellAppMgr::addWatchers()
{
    ...
    pWatchSpaces->addChild("*", new BaseDereferenceWatcher(Space::pWatcher()));
    rootWatcher.addChild("spaces", pWatchSpaces, (void*)&this->spaces_);
    ...
    MF_WATCH("numSpacesWithLoadedGeometry",
             *this,
             &CellAppMgr::numSpacesWithLoadedGeometry);

    // Watchers for load-balancing
    MF_WATCH(
      "loadBalancing/cellsPerSpaceMax", *this, &CellAppMgr::cellsPerSpaceMax);
    MF_WATCH("loadBalancing/cellsPerMultiCellSpaceAvg",
             *this,
             &CellAppMgr::cellsPerMultiCellSpaceAvg);
    MF_WATCH("loadBalancing/numPartitions", *this, &CellAppMgr::numPartitions);
    MF_WATCH("loadBalancing/cellAppGroups", *this, &CellAppMgr::cellAppGroups);
    MF_WATCH("loadBalancing/numCells", *this, &CellAppMgr::numCells);
    MF_WATCH("loadBalancing/numSpaces", *this, &CellAppMgr::numSpaces);
    MF_WATCH("loadBalancing/numMultiCellSpaces",
             *this,
             &CellAppMgr::numMultiCellSpaces);
    MF_WATCH("loadBalancing/numMultiMachineSpaces",
             *this,
             &CellAppMgr::numMultiMachineSpaces);
    MF_WATCH("loadBalancing/machinesPerMultiCellSpaceAvg",
             *this,
             &CellAppMgr::machinesPerMultiCellSpaceAvg);
    MF_WATCH("loadBalancing/machinesPerMultiCellSpaceMax",
             *this,
             &CellAppMgr::machinesPerMultiCellSpaceMax);
    MF_WATCH("loadBalancing/numMachinePartitions",
             *this,
    ...
    MF_WATCH("viewer server port",
             *pCellAppMgrViewerServer_,
             &CellAppMgrViewerServer::port);
     ...
     */
    MF_WATCH(
      "stats/maxCellAppTimeout", cellApps_, &CellApps::maxCellAppTimeout);

    rootWatcher.addChild(
      "baseAppMgr", Mercury::ChannelOwner::pWatcher(), &baseAppMgr_);
...
bool CellAppMgr::checkLoadingSpaces()
{
    ...
    while (spaceIter != spaces_.end()) {
        ...
        bool needsMoreLoadingCells =
          isLoading && (pSpace->numCells() < Config::maxLoadingCells()) &&
...
void CellAppMgr::overloadCheck()
{
    ...
    if (this->dbAppAlpha().channel().isEstablished()) {
        ...
        DBAppInterface::cellAppOverloadStatusArgs::start(bundle)
          .hasOverloadedCellApps = areAnyOverloaded || isAvgOverloaded;
 ...
 */
void CellAppMgr::createEntityCommon(
  Space*                                pSpace,
  const Mercury::Address&               srcAddr,
  const Mercury::UnpackedMessageHeader& header,
  BinaryIStream&                        data)
 ...
 */
void CellAppMgr::prepareForRestoreFromDB(
                        ...
                        BinaryIStream&                        data)
{
    ...
    if (!sharedCellAppData_.empty()) {
        ...
        while (iter != sharedCellAppData_.end()) {
            ...
            bundle << SharedDataType(SHARED_DATA_TYPE_CELL_APP) << iter->first
                   << iter->second;
            ++iter;
        }
    }
    ...
    if (!sharedGlobalData_.empty()) {
        ...
        while (iter != sharedGlobalData_.end()) {
            bundle.startMessage(CellAppInterface::setSharedData);
            bundle << SharedDataType(SHARED_DATA_TYPE_GLOBAL) << iter->first
                   << iter->second;
            ++iter;
        }
    }
 ...
 *	died unexpectedly.
 */
void CellAppMgr::handleCellAppDeath(
  const Mercury::Address&                            srcAddr,
  const Mercury::UnpackedMessageHeader&              header,
 ...
 */
void CellAppMgr::ackCellAppDeath(
  const Mercury::Address&                         srcAddr,
  const Mercury::UnpackedMessageHeader&           header,
...
void CellAppMgr::startTimer()
{
    ...
    gameTimer_ =
      this->mainDispatcher().addTimer(1000000 / Config::updateHertz(),
                                      this,
                                      (void*)TIMEOUT_GAME_TICK,
                                      "GameTick");
    pTimeKeeper_ =
      new TimeKeeper(interface_, gameTimer_, time_, Config::updateHertz());
    hasStarted_ = true;
}
...
void CellAppMgr::handleCellAppDeath(Mercury::Address addr)
{
    ...
    CellAppDeathHandler* pCellAppDeathHandler = new CellAppDeathHandler(
      addr, Config::secondsToTicks(MAX_WAIT_ON_CELLAPP_DEATH, 1));
    ...
    if (data.remainingLength() >= headerSize) {
        ...
        if (pSpace) {
            if (key == uint16(-1)) {
                ...
                pSpace->sendSpaceDataUpdate(srcAddr, entryID, uint16(-1), NULL);
            } else {
 ...
 */
void CellAppMgr::gameTimeReading(
  const Mercury::Address&                         srcAddr,
  const Mercury::UnpackedMessageHeader&           header,
...
bool CellAppMgr::startRecovery()
{
    if (!isRecovering_) {
        ...
        recoveryTimer_ = this->mainDispatcher().addOnceOffTimer(
          2000000, // 2 seconds
          this,
...
{
    if (pShutDownHandler_ && pShutDownHandler_->isPaused()) {
