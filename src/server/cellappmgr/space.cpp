...
{
  public:
    AddCellReplyHandler(const Mercury::Address& addr, SpaceID spaceID)
      : addr_(addr)
      , spaceID_(spaceID)
    {
    }
...
void Space::loadBalance()
{
    ...
    float loadSafetyBound =
      std::max(CellAppLoadConfig::safetyBound(),
...
void Space::addGeometryMappingsToStream(BinaryOStream& stream) const
{
    for (DataEntries::const_iterator it = dataEntries_.begin();
        ...
        if ((it->second.key == SPACE_DATA_MAPPING_KEY_CLIENT_SERVER) ||
            ...
            const SpaceData_MappingData& data =
              *reinterpret_cast<const SpaceData_MappingData*>(
                it->second.data.data());
...
CellData* Space::addCell(CellApp& cellApp, CellData* pCellToSplit)
{
    ...
    // prepare space data for sending
    {
        ...
        while (iter != dataEntries_.end()) {
            bundle << iter->first << iter->second.key << iter->second.data;
            ++iter;
        }
    }
...
CellData* Space::recoverCell(CellApp& app, const Mercury::Address& addr)
{
    ...
    if (pCell) {
        ...
        ERROR_MSG(
          "Space::recoverCell: No cell %s in space %u\n", addr.c_str(), id_);
    }
...
void Space::endRecovery()
{
    ...
    if (cellsToDelete_.empty()) {
        ...
        WARNING_MSG("Space::endRecovery: Num cells not recovered = %" PRIzu
                    "\n",
                    cellsToDelete_.size());
                    ...
                    bool                setLastMappedGeometry)
{
    ...
    if (setLastMappedGeometry &&
        (key == SPACE_DATA_MAPPING_KEY_CLIENT_SERVER)) {
...
WatcherPtr Space::pWatcher()
{
    ...
    if (pWatcher == NULL) {
        ...
        pWatcher->addChild("areaNotLoaded", makeWatcher(&Space::areaNotLoaded));
        ...
        pWatcher->addChild("geometry",
                           makeWatcher(&Space::geometryMappingAsString));
...
float Space::artificialMinLoadCellShare(float receivedCellLoad) const
{
    ...
    for (Cells::const_iterator it = cells_.begin(); it != cells_.end(); ++it) {
    ...
    if (addedLoad >= sumOfDeltas) {
        artificialLoadShare = thisDelta + (addedLoad - sumOfDeltas) / numCells;
    } else {
