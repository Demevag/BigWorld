...
{
  public:
    CollectAddresses(AddressList& list)
      : list_(list)
    {
    }
...
{
  public:
    CollectSomeAddresses(const ComponentIDList& targetList, AddressList& list)
    ...
    {
        return std::find(targetList_.begin(),
                         targetList_.end(),
                         pCellApp->id()) != targetList_.end();
    }
  ...
  const BW::string& targetInfo)
{
    ...
    if (targetInfo == TARGET_LEAST_LOADED) {
        ...
        if (pCellApp) {
            list->push_back(AddressPair(pCellApp->addr(),
                                        Component(CELL_APP, pCellApp->id())));
        }
