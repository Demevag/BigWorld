...
namespace {
    ...
    /*
     *	This class is used with make_heap, push_heap and pop_heap to maintain
     *the priority queue of EntityCaches.
     *
...
class AoITrigger : public RangeTrigger
{
  public:
    ...
    ~AoITrigger() { this->removeWithoutContracting(); }
 ...
 */
Witness::Witness(RealEntity&    owner,
                 BinaryIStream& data,
  ...
  , lastSentReliableDirection_()
{
    ...
    if (createRealInfo == CREATE_REAL_FROM_INIT) {
        ...
        if (!desc.addDictionaryToStream(
              tempDict, bundle, EntityDescription::FROM_CELL_TO_CLIENT_DATA)) {
            CRITICAL_MSG("Witness::Witness(%d): "
...
void Witness::readSpaceDataEntries(BinaryIStream& data, bool hasChangedSpace)
{
    ...
    if (hasChangedSpace) {
        ...
        data.retrieve(numSpaceDataEntries * (sizeof(RecentEntry::first_type) +
    ...
    for (seq = space.begDataSeq(); seq < space.endDataSeq(); seq++) {
        ...
        RecentEntriesSet::const_iterator found = recentEntries.find(entrySpec);
...
void Witness::readAoI(BinaryIStream& data)
{
    ...
    if (isAoIRooted) {
        float rootX, rootZ;
        data >> rootX >> rootZ;
        pAoIRoot_ = new MobileRangeListNode(rootX,
                                            rootZ,
                                            RangeListNode::FLAG_NO_TRIGGERS,
                                            RangeListNode::FLAG_NO_TRIGGERS);
        // TODO: Remove this const_cast.
        const_cast<RangeList&>(entity().space().rangeList()).add(pAoIRoot_);
    }
        ...
        if (pEntity == NULL) {
            ...
            if (!dummyCache.isEnterPending()) {
                this->entity().callback(
                  "onLeftAoIID", Py_BuildValue("(i)", id), "onLeftAoIID", true);
                Mercury::Bundle& bundle = this->bundle();
...
void Witness::init()
{
    ...
    while (i < entityQueue_.size()) {
        ...
        if (pEntity->isInAoIOffload()) {
            ...
            this->entity().callback(
              "onLeftAoI", PyTuple_Pack(1, pEntity), "onLeftAoI", true);
                              ...
                              bool           hasChangedSpace)
{
    ...
    if (createRealInfo == CREATE_REAL_FROM_OFFLOAD) {
        ...
        referencePosition_ =
          ::BW_NAMESPACE calculateReferencePosition(entity_.position());
    }
#endif /* !VOLATILE_POSITIONS_ARE_ABSOLUTE */
...
void Witness::vehicleChanged()
{
    ...
    sva.vehicleID =
      entity_.pVehicle() ? entity_.pVehicle()->id() : NULL_ENTITY_ID;
...
void Witness::addSpaceDataChanges(Mercury::Bundle& bundle)
{
    ...
    if (allSpacesDataChangeSeq_ != Space::s_allSpacesDataChangeSeq_) {
        ...
        if (knownSpaceDataSeq_ < space.begDataSeq()) {
            ...
            for (it = space.spaceDataMapping().begin(); it != endIter; it++) {
                ...
                bundle.addBlob(it->second.data().data(),
        ...
        while (knownSpaceDataSeq_ < space.endDataSeq()) {
            ...
            const BW::string* pValue =
              space.dataBySeq(knownSpaceDataSeq_++, seid, key);
...
bool* Witness::addReferencePosition(Mercury::Bundle& bundle)
{
    ...
    if (real_.controlledByRef().id == entity_.id() && hasReferencePosition_) {
        ...
        BaseAppIntInterface::relativePositionReferenceArgs::start(bundle)
          .sequenceNumber = referenceSeqNum_;
                                         ...
                                         bool isReferencePosition) const
{
    ...
    const bool lastSendWasRecentEnough =
      alwaysSkipUnchanged ||
      (timeSinceLastSend <=
 ...
 *				history events.
 */
bool Witness::selectEntity(Mercury::Bundle& bundle, EntityID targetID) const
{
    if (targetID == entity_.id()) {
        bundle.startMessage(BaseAppIntInterface::selectPlayerEntity);
    } else {
 ...
 *
 * @param ppCache   The current entity cache. Can be NULL after the call if it
 * was invalidated.
...
#if VOLATILE_POSITIONS_ARE_ABSOLUTE
    otherEntity.writeClientUpdateDataToBundle(
      bundle, Position3D::zero(), *pCache, distSqr);
#else  /* VOLATILE_POSITIONS_ARE_ABSOLUTE */
    hasAddedReliableRelativePosition |=
      otherEntity.writeClientUpdateDataToBundle(
        bundle, referencePosition_, *pCache, distSqr);
#endif /* VOLATILE_POSITIONS_ARE_ABSOLUTE */
...
void Witness::update()
{
    ...
    EntityCache::Priority startingPriority =
      entityQueue_.empty() ? 0.f : entityQueue_.front()->priority();
    ...
    EntityCache::Priority maxPriority =
      entityQueue_.empty()
        ? 0.f
    ...
    while ((queueBegin != queueEnd) &&
        ...
        if (!pCache->isUpdatable()) {
            ...
            // The entity has not gone anywhere, so we will proceed with the
            // update.
            hasAddedReliableRelativePosition |= this->sendQueueElement(pCache);
        ...
        if (wasPrioritised) {
            if (pCache) {
                ...
                // prioritised vehicles' priorities should be updated only after
                // pop_heap
                pCache->updatePriority(entity_.position());
    ...
    if (queueEnd == entityQueue_.begin()) {
        ...
        std::make_heap(
          entityQueue_.begin(), entityQueue_.end(), PriorityCompare());
        STOP_PROFILE_WITH_DATA(profileMakeHeap, entityQueue_.size());
    } else {
        EntityCache::Priority earliestPriority =
          entityQueue_.front()->priority();

        START_PROFILE(profileClearPrioritised);
        int count = 0;
        for (Entity* pVehicle = entity_.pVehicle();
             numPrioritised > 0 && pVehicle != NULL;
             pVehicle = pVehicle->pVehicle(), ++count) {
            EntityCache* pCache = aoiMap_.find(pVehicle->id());
            if (!pCache) {
                // this case should be already logged above so no additional
                // logs here
                continue;
            }
 ...
 */
void Witness::onSendReliablePosition(Position3D position, Direction3D direction)
{
...
void Witness::setAoIRoot(float x, float z)
{
    ...
    RangeListNode* pNewAoIRoot = new MobileRangeListNode(
      x, z, RangeListNode::FLAG_NO_TRIGGERS, RangeListNode::FLAG_NO_TRIGGERS);
...
void Witness::clearAoIRoot()
{
    ...
    MobileRangeListNode* pOldAoIRoot = static_cast<MobileRangeListNode*>(
...
int Witness::pySet_aoiRoot(PyObject* value)
{
    ...
    if (PyVector<Vector2>::Check(value) ||
        ...
        int     retVal = Script::setData(value, valueVector2, "aoiRoot");
        if (retVal == 0) {
            this->setAoIRoot(valueVector2.x, valueVector2.y);
        }
        return retVal;
    }
    ...
    if (PyVector<Vector3>::Check(value) ||
        ...
        int     retVal = Script::setData(value, valueVector3, "aoiRoot");
        if (retVal == 0) {
            this->setAoIRoot(valueVector3.x, valueVector3.z);
        }
        return retVal;
    }
...
void Witness::sendEnter(Mercury::Bundle& bundle, EntityCache* pCache)
{
    ...
    pEntity->pType()->stats().enterAoICounter().countSentToOtherClients(
      bundle.size() - oldSize);
}
...
void Witness::sendCreate(Mercury::Bundle& bundle, EntityCache* pCache)
{
...

    {
        CompressionOStream compressedStream(
          ...
          this->entity()
            .pType()
            ->description()
            .externalNetworkCompressionType());
        ...
        if (isVolatile) {
            compressedStream << PackedYawPitchRoll</* HALFPITCH */ false>(
              entity.localDirection().yaw,
              entity.localDirection().pitch,
    ...
    entity.pType()->stats().createEntityCounter().countSentToOtherClients(
      bundle.size() - oldSize);
}
...
IDAlias Witness::allocateIDAlias(const Entity& entity)
{
    ...
    if (entity.volatileInfo().hasVolatile(0.f) && numFreeAliases_ != 0) {
        numFreeAliases_--;
 ...
 *	object.
 */
bool Witness::sendToClient(EntityID           entityID,
...
void Witness::sendToProxy(Mercury::MessageID msgID, MemoryOStream& stream)
{
    ...
    bundle.startMessage(BaseAppIntInterface::gMinder.interfaceElement(msgID));
    bundle.transfer(stream, stream.size());
}
...
void Witness::addToAoI(Entity* pEntity, bool setManuallyAdded)
{
    ...
    if (!wasInAoIOffload) {
        ...
        this->entity().callback(
          "onEnteredAoI", Py_BuildValue("(O)", pEntity), "onEnteredAoI", true);
    }
...
void Witness::newPosition(const Vector3& position)
{
    ...
    if ((dur > 1) && !noiseMade_) {
        ...
        if (v.y < -NoiseConfig::verticalSpeed()) {
            ...
            float horizSpeedSqr = ServerUtil::sqr(v.x) + ServerUtil::sqr(v.z);
 ...
 *  @{
 *  451.8 DEBUG: Dumping AoI for entity 1000. Seen = 2 AoIMap = 2 AoIRadius =
 * 500.000 AoIHyst =  5.000 451.8 DEBUG: 16777218: volatile  129/ 130. event 45/
 * 45. priority !0.000000 451.8 DEBUG: 16778216: volatile 65535/   0. event
 * 1/   1. priority  0.000000
 *  @}
 ...
 *
 *  self.entitiesInAoI( withheld = False ) # All entities in AoI not being
 *withheld
 *  @}
 *
...
int Witness::pySet_bpsToClient(PyObject* value)
{
    ...
    int maxPacketSize =
      ServerUtil::bpsToPacketSize(newValue, CellAppConfig::updateHertz());
...
EntityCache* Witness::findEntityCacheFromPyArg(PyObject* pEntityOrID) const
{
    ...
    if (pCache == NULL) {
        PyErr_Format(PyExc_ValueError,
                     "Entity %d is not in the AoI of %d\n",
                     pEntity->id(),
                     this->entity().id());
    }
...
namespace {
    ...
    /**
     *	This class is a base for Witness' AoI visitor implementations that
     *create a Python list containing the entities in a Witness' AoI.
     */
    class BaseAoICollector : public EntityCacheVisitor
    {
      public:
        // Returns a new reference.
        ScriptList getList() const { return pList_; }
    ...
    class AoICollector : public BaseAoICollector
    {
      public:
         ...
         *	Constructor.
         *
         *	@param predicate	A functor to test which entities should be added
         *to the list.
         */
        AoICollector(const PRED& predicate)
    ...
    {
      public:
        bool operator()(const EntityCache& /*cache*/) const { return true; }
    };
    ...
    {
      public:
        FilterWithheldEntities(bool onlyWithheld)
          : onlyWithheld_(onlyWithheld)
        {
        }
...
PyObject* Witness::py_entitiesInAoI(PyObject* args, PyObject* kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(
          args, kwargs, "|O:entitiesInAoI", (char**)keywords, &pArg)) {
        return NULL;
    }
 ...
 */
bool Witness::setAoIUpdateScheme(PyObjectPtr pEntityOrID, BW::string schemeName)
{
    EntityCache* pCache = this->findEntityCacheFromPyArg(pEntityOrID.get());
...
{
    return (shouldAddReplayAoIUpdates_ && entity_.cell().pReplayData() &&
