 ...
 *  Sample Output:
 *  @{
 *  107.9 DEBUG: RangeListNode::debugX: me.PosX=-347.4982910, me=[Entity
 * 16777218], prevX=[Head], nextX=[Entity 16777216] 107.9 DEBUG:
 * RangeListNode::debugZ: me.PosZ=369.6944580, me=[Entity 16777218],
 * prevZ=[Entity 16777216], nextZ=[Tail]
 *  @}
 */
PY_METHOD(debug)
 ...
 *  @}
 *  Gives a list of entities that are instance of class Creature, but does not
 * include entities that are instances of subclasses of Creature. The search
 * radius is 100 metres using (100,0,100) as centre.
 *  @{
 *    [ e for e in self.entitiesInRange( 100, None, (100,0,100) ) if isinstance(
 * e, BaseType ) ]
 *  @}
 *  will give a list of entities that are instantiated from BaseType or
 * subclasses of BaseType
 ...
 *  will be used in matching entities. If entityType is a valid class name then
 * only entities of that class will be returned, otherwise all entities in range
 * are returned.
 ...
 *  @components{ cell }
 *  The isOnGround attribute will be True if the position of the Entity is on
 * the ground and false otherwise.
...
namespace {
     ...
     */
    void outputEntityPerfWarning(const char*   message,
                                 const Entity& entity,
                                 double        duration,
                                 int           size)
    {
        WARNING_MSG("%s (%s %u of size %d bytes took %f seconds)\n",
                    message,
                    entity.pType()->description().name().c_str(),
                    entity.id(),
                    size,
                    duration);
    }
...
class BuildEntityTypeDict : public Script::InitTimeJob
{
  public:
    ...
    virtual void init()
    {
        ...
        this->addAttributes(Entity::s_getAttributeDefs(),
                            /* mainMethods: */ true);

        // Add entity extra
        BW::vector<EntityExtraInfo*>& entityExtra = Entity::s_entityExtraInfo();
    ...
    const StringSet<ConstPolicy>& attributeSet() { return attributeSet_; }
  ...
  private:
    ...
    void addAttributes(PyGetSetDef* pMembers, bool mainMethods = false)
    {
        while (pMembers && pMembers->name != NULL) {
            if (mainMethods || (strcmp(pMembers->name, "__members__") != 0 &&
 ...
 *	@see initGhost
 */
bool Entity::initReal(BinaryIStream&          data,
                      ...
                      EntityPtr               pNearbyEntity)
{
    ...
    bool shouldUpdateGhostPositions =
      pReal_->init(data,
  ...
  const ScriptDict& properties)
{
    ...
    data >> localPosition_ >> flags >> localDirection_ >> lastEventNumber_ >>
      baseAddr_;
    ...
    if (isTemplate) {
        ...
        // The template may override localPosition_, isOnGround_ and
        // localDirection_
        ...
        if (properties) {
            ...
            if (!description.readStreamToDict(
                  data, EntityDescription::CELL_DATA, propDict)) {
                ERROR_MSG(
                  "Entity::readRealDataInEntityFromStreamForInitOrRestore(%u): "
        ...
        if (!populateDelegateWithDict(description,
    ...
    properties_.insert(properties_.end(),
                       pEntityType_->propCountGhostPlusReal() -
                         properties_.size(),
                       ScriptObject());
    ...
    for (uint i = 0; i < properties_.size(); ++i) {
        ...
        if (!pNewVal ||
            !(properties_[i] = dt.attach(
                pNewVal,
                &propertyOwner_,
                i))) { // prop currently null so assignment won't exec python so
            ...
            properties_[i] = dt.attach(ds.pInitialValue(), &propertyOwner_, i);
...
void Entity::readGhostDataFromStreamInternal(BinaryIStream& data)
{
    ...
    for (uint32 i = 0; i < properties_.size(); ++i) {
        ...
        ScriptDataSink sink;
        MF_VERIFY(dt.createFromStream(data,
                                      sink,
...
void Entity::writeGhostDataToStream(BinaryOStream& stream) const
{
    ...
    CompressionOStream compressionStream(
      stream, pEntityType_->description().internalNetworkCompressionType());
...
void Entity::callScriptInit(bool isRestore, EntityPtr pNearbyEntity)
{
    if (isRestore) {
        ...
        this->callback(
          "__init__",
          Py_BuildValue("(O)", pNearbyEntity ? pNearbyEntity.get() : Py_None),
          "__init__",
          true);
    ...
    //  (see the draft proposition below in the commented-out method
    //  DelegateEntityInit,
    if (pEntityDelegate_) {
 ...
 */
void Entity::writeRealDataToStream(BinaryOStream& data, bool isTeleport) const
{
    ...
    CompressionOStream compressionStream(
      data, pEntityType_->description().internalNetworkCompressionType());
...
bool Entity::readBasePropertiesExposedForReplayFromStream(BinaryIStream& data)
{
    ...
    if (basePropertiesDataSize) {
        ...
        if (!description.readStreamToDict(
              basePropertiesData, EntityDescription::BASE_DATA, basePropDict)) {
            return false;
        }
  ...
  BinaryOStream& data) const
{
    ...
    if (exposedForReplayClientProperties_ &&
        ...
        !edesc.addDictionaryToStream(exposedForReplayClientProperties_,
                                     propData,
...
{
  public:
    ReplyForwarder(const Mercury::Address& destAddr, Mercury::ReplyID replyID)
      : destAddr_(destAddr)
      , replyID_(replyID)
    {
    }
 ...
 */
void Entity::forwardMessageToReal(CellAppChannel&         realChannel,
                                  EntityID                entityID,
                                  uint8                   messageID,
                                  BinaryIStream&          data,
...
WatcherPtr Entity::pWatcher()
{
    ...
    if (!pWatcher) {
        ...
        PropWatcher* propertiesWatcher = new PropWatcher(pNull->properties_);
                                           ...
                                           float            lodPriority) const
{
    ...
    while (eventIter != eventEnd &&
        ...
        hasEventsToSend =
          hasEventsToSend || event.shouldSend(lodPriority, cache.detailLevel());
    ...
    // if (cache.lastVolatileUpdateNumber() != volatileUpdateNumber_)
    {
        ...
        if (this->volatileInfo().hasVolatile(lodPriority)) {
            ...
            if (cache.isAlwaysDetailed() ||
                (cache.isPrioritised() &&
                 CellAppConfig::sendDetailedPlayerVehicles())) {
                this->writeVolatileDetailedDataToBundle(
                  bundle, cache.idAlias(), isReliable);
            } else {
                hasAddedReliableRelativePosition =
                  this->writeVolatileDataToBundle(
                    bundle, basePos, cache.idAlias(), lodPriority, isReliable);
            }
...
#if ENABLE_WATCHERS
            pEntityType_->stats().countVolatileSentToOtherClients(oldSize -
                                                                  initSize);
#endif
        }
    }
    ...
    hasSelectedEntity |=
      cache.updateDetailLevel(bundle, lodPriority, hasSelectedEntity);
                                       ...
                                       bool             isReliable) const
{
    ...
    const Vector3 relativePos =
      isRelative ? localPosition_ - basePosition : localPosition_;
    ...
    if (shouldSendPosition) {
        ...
        const float maxLimit =
          sendOnGround ? PackedXZ::maxLimit(scale) : PackedXYZ::maxLimit(scale);

        const float minLimit =
          sendOnGround ? PackedXZ::minLimit(scale) : PackedXYZ::minLimit(scale);

        if ((relativePos.x < minLimit) || (maxLimit <= relativePos.x) ||
            (relativePos.z < minLimit) || (maxLimit <= relativePos.z)) {
            this->writeVolatileDetailedDataToBundle(
              bundle, idAlias, isReliable);
            return false;
        }
    }
    ...
    if (shouldSendPosition) {
        if (sendOnGround) {
            ...
            bundle << PackedXYZ(
              relativePos.x, relativePos.y, relativePos.z, scale);
        }
    }
...
void Entity::destroy()
{
    ...
    CellApp::instance().bufferedGhostMessages().playNewLifespanFor(this->id());
}
...
void Entity::createEntityDelegateWithTemplate(const BW::string& templateID)
{
    ...
    TRACE_MSG("Entity::createEntityDelegateWithTemplate: "
              "Creating entity delegate for '%s' entity #%d in space #%d from "
              "template '%s'\n",
              this->pType()->name(),
              this->id(),
              this->spaceID(),
...
void Entity::adjustForDeadBaseApp(const BackupHash& backupHash)
{
    ...
    if (this->pReal()) {
        ...
        // the entity's position and to generate traffic to see if we get a
        // response
        this->informBaseOfAddress(
          CellApp::instance().interface().address(), this->space().id(), true);
    }
}
                                      ...
                                      ScriptTuple              args) const
{
    ...
    ScriptTuple remainingArgs =
      pDescription->extractSourceEntityID(args, sourceEntityID);

    ScriptDataSource source(remainingArgs);

    bool succeeded =
      pDescription->addToServerStream(source, bundle, sourceEntityID);
...
const Mercury::Address& Entity::realAddr() const
{
    ...
    return pRealChannel_ ? pRealChannel_->addr()
...
bool Entity::checkIfZombied(const Mercury::Address& dyingAddr)
{
    ...
    if (pRealChannel_ && pRealChannel_->addr() != dyingAddr) {
        ...
        INFO_MSG("Entity::checkIfZombied: "
                 ...
                 "to next real %s\n",
                 id_,
                 pRealChannel_ ? pRealChannel_->addr().c_str()
                               : Mercury::Address::NONE.c_str(),
                 nextRealAddr_.c_str());
                               ...
                               ScriptTuple              args)
{
    ...
    return this->writeCellMessageToBundle(sender.bundle(), pDescription, args);
}
                                 ...
                                 bool                     isExposedForReplay)
{
    ...
    return this->writeClientMessageToBundle(
      pRealChannel_->bundle(), entityID, description, argStream, callingMode);
}
 ...
 */
bool Entity::physicallyPossible(const Position3D& newPosition,
                                Entity*           pVehicle,
                                float             propMove)
{
    ...
    uint64 delTime =
      std::min(stampsPerSecond(), nowTime - physicsLastValidated_);
    ...
    if (pVehicle == NULL && dstPos == srcPos) {
        Vector3 ldir;
...
bool Entity::traverseChunks(Chunk*              pCurChunk,
                            ...
                            BW::vector<Chunk*>& visitedChunks)
{
    ...
    for (; pit != pnd; pit++) {
        ...
        if (dsrc * ddst > 0)
            continue; // equiv test to above I hope
    ...
    if (!(status || pendingChunks.empty())) {
        ...
        for (BW::vector<Chunk*>::iterator iter = pendingChunks.begin();
            ...
            if (traverseChunks(
                  pNChunk, pDstChunk, srcPos, dstPos, visitedChunks)) {
                status = true;
                break;
            }
        }
                                           ...
                                           bool           isVehicleMovement)
{
    ...
    for (Triggers::iterator it = triggers_.begin(); it != triggers_.end();
         it++) {
        (*it)->shuffleXThenZExpand(
          increaseX, increaseZ, oldPosition.x, oldPosition.z);
    }
    ...
    if (Passengers::instance.exists(*this)) {
        Passengers::instance(*this).updateInternalsForNewPosition(oldPosition);
    }
...
void Entity::checkChunkCrossing()
{
    ...
    const Vector3 offsetPosition(
      globalPosition_.x, globalPosition_.y + 0.1f, globalPosition_.z);
    ...
    for (Chunk::piterator iter = pChunk_->pbegin(); iter != pChunk_->pend();
         iter++) {
        ...
        if (portal.hasChunk()) {
            ...
            if (chunk->contains(offsetPosition)) {
                ...
                if (chunk->isOutsideChunk()) {
                    ...
                    /*
                    DEBUG_MSG( "Changed Chunks: Entity %d gone through portal
                    [%s] from %s to inside %s (%6.3f, %6.3f, %6.3f)\n",
                        (int)this->id(),
                        portal.label.c_str(),
                        pChunk_->identifier().c_str(),
    ...
    if (throughPortal) {
        ...
        /*
        DEBUG_MSG( "Changed Chunks: Entity %d gone through portal [%s] from %s
        to %s (%6.3f, %6.3f, %6.3f)\n", (int)this->id(),
        ...
        */
        ...
        pNewChunk =
          this->pChunkSpace()->findChunkFromPointExact(offsetPosition);
        /*
        DEBUG_MSG( "Changed Chunks: Entity %d teleported or cheated from %s to
        %s (%6.3f, %6.3f, %6.3f)\n", (int)this->id(),
                        pChunk_->identifier().c_str(),
                        chunk->identifier().c_str(),
                        globalPosition_.x,
...
void Entity::reloadScript()
{
    ...
    if (PyObject_SetAttrString(
          this, "__class__", (PyObject*)pEntityType_->pPyType()) == -1) {
  ...
  const CellAppInterface::avatarUpdateImplicitArgs& args)
{
    ...
    if (!isZero(topSpeed_) && !this->physicallyPossible(args.pos, pVehicle_)) {
  ...
  const CellAppInterface::avatarUpdateExplicitArgs& args)
{
    ...
    if (!physicsError && args.vehicleID != curVehicleID) {
        ...
        if (args.vehicleID != 0) {
            Entity::nominateRealEntity(*this);
            if (!PassengerExtra::instance(*this).boardVehicle(args.vehicleID,
                                                              false)) {
                // just print it out
                PyErr_Print();
                PyErr_Clear();
 ...
 *	entities.
 */
void Entity::avatarUpdateCommon(const Position3D&   pos,
 ...
 *	stream.
 */
void Entity::readRealDataFromStreamForOnload(
  BinaryIStream&          data,
  const Mercury::Address* pBadHauntAddr)
{
    CompressionIStream compressionStream(data);
 ...
 */
void Entity::readRealDataFromStreamForOnloadInternal(
  BinaryIStream&          data,
  const Mercury::Address* pBadHauntAddr)
{
  ...
  const CellAppInterface::ghostSetNextRealArgs& args)
{
    ...
    CellApp::instance().bufferedGhostMessages().playSubsequenceFor(
      id_, nextRealAddr_);
}
 ...
 */
void Entity::witnessCapacity(const CellAppInterface::witnessCapacityArgs& args)
{
...
void Entity::ghostedDataUpdate(BinaryIStream& data)
{
    ...
    if (pDescription->isComponentised()) {
        ...
        bool success = propertyOwner_.setPropertyFromInternalStream(
          data, &pOldValue, &pChangePath, pDescription->localIndex(), &isSlice);
 ...
 *	away.
 */
void Entity::delControlledBy(const CellAppInterface::delControlledByArgs& args)
{
    MF_ASSERT(this->isReal());
...
/**
 *	This method is called when this entity's base part has been offloaded to
 *another BaseApp as part of BaseApp retirement.
 */
void Entity::onBaseOffloaded(const CellAppInterface::onBaseOffloadedArgs& args)
{
    ...
    for (RealEntity::Haunts::iterator iter = pReal_->hauntsBegin();
         ...
         ++iter) {
        CellAppInterface::onBaseOffloadedForGhostArgs::start(iter->bundle(),
                                                             this->id())
          .newBaseAddr = baseAddr_;
    }
}
...
void Entity::addDetailedPositionToHistory(bool isLocalOnly)
{
    ...
    const Mercury::InterfaceElement& ie = ClientInterface::detailedPosition;

    const int msgStreamSize = ie.streamSize();

    if (isLocalOnly) {
        this->addHistoryEventLocally(
          ie.id(),
          stream,
          pEntityType_->detailedPositionDescription(),
          msgStreamSize);
    } else {
        pReal_->addHistoryEvent(ie.id(),
                                stream,
                                pEntityType_->detailedPositionDescription(),
                                msgStreamSize);
    }
    ...
    if ((pReal_ != NULL) && (pReal_->pWitness() != NULL) &&
        ...
        pReal_->pWitness()->sendToClient(
          this->id(), ie.id(), stream, msgStreamSize);
...
void Entity::witnessed()
{
    ...
    if (periodsWithoutWitness_ == NOT_WITNESSED_THRESHOLD) {
        ...
        this->callback("onWitnessed",
                       Py_BuildValue("(O)", Py_True),
                       "onWitnessed(True) callback: ",
                       true);
    }
...
/**
 *	This method is called by the real to tell us that it is not being witnessed
 *by any entity. This ghost entity now checks whether anything is witnessing it
 *and will inform the real entity, if so.
 */
...
void Entity::callBaseMethod(BinaryIStream& data)
{
    ...
    if (baseAddr_.ip != 0) {
        ...
        if (pDescription != NULL) {
            BinaryOStream* pBOS = pBaseEntityMailBox->getStream(*pDescription);
            if (pBOS == NULL) {
                ERROR_MSG("Entity::callBaseMethod(%u): "
...
void Entity::recordClientMethod(BinaryIStream& data)
{
    ...
    if (this->cell().pReplayData()) {
        this->cell().pReplayData()->addEntityMethod(*this, *pDescription, data);
    } else {
        data.finish();
    }
}
...
void Entity::recordClientProperties(BinaryIStream& data)
{
    ...
    edesc.readStreamToDict(
      data, EntityDescription::BASE_DATA, exposedForReplayClientProperties_);
 ...
 */
void Entity::runMethodHelper(BinaryIStream&          data,
                             int                     methodID,
                             bool                    isExposed,
                             int                     replyID,
                             const Mercury::Address* pReplyAddr)
{
    ...
    if (isExposed) {
        ...
        pMethodDescription =
          pEntityType_->description().cell().exposedMethodFromMsgID(
            methodID, data, range);
    ...
    if (pMethodDescription != NULL) {
        Entity::nominateRealEntity(*this);
        {
            if (pMethodDescription->isComponentised()) {
                MF_ASSERT(pEntityDelegate_ != NULL);
                pEntityDelegate_->handleMethodCall(
                  *pMethodDescription, data, sourceID);
            } else {
                SCOPED_PROFILE(SCRIPT_CALL_PROFILE);
    ...
    STOP_PROFILE_WITH_CHECK(localProfile)
    {
        WARNING_MSG("\tin Entity::runExposedMethod(%u). Method = %s\n",
                    id_,
                    pMethodDescription ? pMethodDescription->name().c_str()
                                       : "<not found>");
    }
}
...
void Entity::requestEntityUpdate(BinaryIStream& data)
{
    ...
    if (pReal_ != NULL && pReal_->pWitness() != NULL) {
        ...
        pReal_->pWitness()->requestEntityUpdate(
          id,
...
bool Entity::isRealToScript() const
{
    return this->isReal() &&
           (!CellAppConfig::treatAllOtherEntitiesAsGhosts() ||
            (s_pTreatAsReal == this) || (s_pTreatAsReal == NULL) ||
                                    ...
                                    PropertyChange&        change)
{
    ...
    if (this->cell().pReplayData()) {
        this->cell().pReplayData()->addEntityProperty(
          *this, *pDescription, change);
    }
    ...
    if (pDescription->isGhostedData()) {
        ...
        if (pDescription->isOtherClientData()) {
            if (pDescription->shouldSendLatestOnly() &&
                ...
                WARNING_MSG(
                  "Entity::onOwnedPropertyChanged(%u): "
                ...
                MF_VERIFY_DEV(propertyOwner_.changeOwnedProperty(
            ...
            Mercury::MessageID msgID = this->addChangeToExternalStream(
              change, stream, *pDescription, &streamSize);
            ...
            HistoryEvent* pEvent =
              pReal_->addHistoryEvent(msgID,
                                      stream,
                                      *pDescription,
                                      streamSize,
                                      pDescription->detailLevel());
...
#if ENABLE_WATCHERS
            pDescription->stats().countSentToGhosts(bundle.size() -
                                                    oldBundleSize);
            pEntityType_->stats().countSentToGhosts(bundle.size() -
                                                    oldBundleSize);
#endif
    ...
    if (pDescription->isOwnClientData() && pReal_->pWitness() != NULL) {
        ...
        Mercury::MessageID messageID = this->addChangeToExternalStream(
          change, stream, *pDescription, &streamSize);
...
#endif
        pReal_->pWitness()->sendToClient(
          this->id(), messageID, stream, streamSize);
    }
  ...
  int*                   pStreamSize) const
{
    ...
    int16 msgID =
      messageRange.msgIDFromExposedID(dataDescription.clientServerFullIndex());

    if ((msgID == -1) || change.isNestedChange()) {
        const Mercury::InterfaceElement& ie =
          change.isSlice() ? ClientInterface::sliceEntityProperty
 ...
 */
ScriptObject Entity::propertyByDataDescription(
  const DataDescription* pDataDescr) const
{
    ...
    if (pDataDescr->isComponentised()) {
        ...
        ScriptObject result =
          PyComponent::getAttribute(*pEntityDelegate_,
                                    *pDataDescr,
                                    /* isPersistentOnly */ false);
...
ScriptObject Entity::pyGetAttribute(const ScriptString& attrObj)
{
    ...
    if (isDestroyed_ && strcmp(attr, "isDestroyed") != 0 &&
        ...
        PyErr_Format(PyExc_TypeError,
                     "Entity.%s cannot be accessed since "
    ...
    if (streq(attr, "base") && (baseAddr_.ip != 0)) {
        return ScriptObject(new BaseEntityMailBox(pEntityType_, baseAddr_, id_),
                            ScriptObject::FROM_NEW_REFERENCE);
    }
    ...
    if (pResult && !this->isRealToScript() && PyMethod_Check(pResult.get())) {
        bool         isGhost = false;
        ScriptObject pIsGhost =
          pResult.getAttribute("isGhost", ScriptErrorClear());
                            ...
                            const ScriptObject& value)
{
    ...
    if (pDescription != NULL && pDescription->isCellData()) {
        ...
        if (!propertyOwner_.changeOwnedProperty(
              properties_[cellIndex], value, dataType, cellIndex)) {
            if (!PyErr_Occurred()) {
...
int Entity::pySet_aoiUpdateScheme(PyObject* value)
{
    ...
    if (retVal == 0) {
        if (!AoIUpdateSchemes::getIDFromName(schemeName, aoiUpdateSchemeID_)) {
            PyErr_Format(
              PyExc_ValueError, "Invalid scheme name %s", schemeName.c_str());
            return -1;
        }
        ...
        for (RealEntity::Haunts::iterator iter = pReal_->hauntsBegin();
             iter != pReal_->hauntsEnd();
             ++iter) {
            CellAppInterface::aoiUpdateSchemeChangeArgs::start(iter->bundle(),
                                                               this->id())
              .scheme = aoiUpdateSchemeID_;
        }
    }
...
class PyEntityVector3 : public PyVectorRef<Vector3>
{
  public:
    ...
    virtual bool setVector(const Vector3& v)
    {
        ...
        {
            PyErr_SetString(PyExc_ValueError, "Can only move real entities");
            return false;
        }
...
{
    return new PyEntityVector3(
      this, &globalPosition_, &Entity::setGlobalPosition);
}
...
int Entity::pySet_direction(PyObject* value)
{
    ...
    if (retVal == 0) {
        this->setGlobalDirection(
          Vector3(Angle(newValue.x), Angle(newValue.y), Angle(newValue.z)));
    }
                          ...
                          bool                     isExposedForReplay)
{
    ...
    if (isForOthers) {
        ...
        pReal_->addHistoryEvent(description.exposedMsgID(),
                                argStream,
                                description,
                                description.streamSize(true),
    ...
    if (isExposedForReplay && this->cell().pReplayData()) {
        argStream.rewind();
        this->cell().pReplayData()->addEntityMethod(
          *this, description, argStream);
    }
...
PyObject* Entity::py_destroy(PyObject* /*args*/)
{
    if (this->isDestroyed()) {
        PyErr_Format(
          PyExc_TypeError, "Entity %d is already destroyed", int(this->id()));
        return NULL;
    }

    if (!this->isRealToScript()) {
        PyErr_Format(
          PyExc_TypeError, "Tried to destroy ghost entity %d", int(this->id()));
        return NULL;
    }
 ...
 *	a Python tuple.
 *
 *	@param args		A tuple containing the ID of the previously registered
 *controller.
 *
...
PyObject* Entity::py_clientEntity(PyObject* args)
{
    ...
    if (pDest == NULL) {
        PyErr_Format(
          PyExc_ValueError, "Could not find entity %d.", int(destID));
        return NULL;
    }
...
int Entity::pySet_volatileInfo(PyObject* value)
{
    ...
    int          retVal = Script::setData(value, newInfo, "volatileInfo");
                              ...
                              Mercury::ReplyID        replyID)
{
    ...
    Mercury::ChannelSender sender(
      isToBaseEntity ? pReal_->channel() : CellApp::getChannel(*pReplyAddr));
...
bool Entity::sendCellEntityLostToBase()
{
    ...
    ScriptDict tempDict =
      this->createDictWithProperties(EntityDescription::CELL_DATA);
    ...
    if (!description.addDictionaryToStream(
          tempDict, bundle, EntityDescription::CELL_DATA)) {
        CRITICAL_MSG("Entity::sendCellEntityLostToBase(%u): "
 ...
 */
void Entity::destroyEntity(const CellAppInterface::destroyEntityArgs& /*args*/)
{
    this->destroy();
}
    ...
    while ((pRLN != NULL) && (xDist <= range)) {
        if (pRLN->isEntity()) {
            ...
            if (!pType || (pType == pEntity->pType())) {
                if (ServerUtil::distSqrBetween(testPos, entityPos) <=
                    rangeSqr) {
...
PyObject* Entity::getComponent(const BW::string& name)
{
    ...
    return new PyCellComponent(
      pEntityDelegate_, id_, name, &pEntityType_->description());
}
...
bool Entity::destroySpace()
{
    ...
    if (CellAppConfig::useDefaultSpace() && this->space().id() == 1) {
        PyErr_Format(PyExc_ValueError,
                     "destroySpace called on entity %d in default space",
                     int(id_));
        return false;
    }
 ...
 */
PyObject* Entity::entitiesInRange(float        range,
                                  ScriptObject pClass,
                                  ScriptObject pActualPos)
{
    ...
    if (isDestroyed_) {
        PyErr_Format(PyExc_TypeError,
                     "entitiesInRange called on destroyed entity %d",
                     int(id_));
        return NULL;
    }
    ...
    class LocalVisitor : public EntityVisitor
    {
      public:
        ...
        void visit(Entity* pEntity) { PyList_Append(pList_.get(), pEntity); }
...
ControllerID Entity::addController(ControllerPtr pController, int userArg)
{
    ...
    {
        // PyErr_Format( PyExc_TypeError,
        ERROR_MSG("Entity::addController(%u): added to destroyed entity: "
 ...
 */
bool Entity::outdoorPropagateNoise(float range, int event, int info)
{
    AUTO_SCOPED_THIS_ENTITY_PROFILE;
    ...
    class LocalVisitor : public EntityVisitor
    {
      public:
        virtual ~LocalVisitor() {}
        void visit(Entity* pEntity) { entities_.push_back(pEntity); }
    ...
    for (LocalVisitor::Entities::iterator iter = visitor.entities_.begin();
        ...
        float dist = (float)sqrt(
          ServerUtil::distSqrBetween(this->position(), pEntity->position()));
        if (dist < range) {
...
bool Entity::makeNoise(float noiseLevel, int event, int info)
{
    ...
    float propagationRange = noiseLevel * NoiseConfig::standardRange();
    if (this->isRealToScript()) {
        if (pChunk_ != NULL) {
                        ...
                        int           info)
{
    ...
    */
    ...
    this->callback(
      "onNoise",
      Py_BuildValue("(Offii)", who, propRange, distance, event, info),
      "onNoise callback: ",
      true);
}
 ...
 *	chunks are loaded. In this case, True may be returned when False is
 *expected.
 */
 ...
 */
PyObject* Entity::trackEntity(int   entityId,
                              float velocity,
                              int   period,
                              int   userArg)
{
...
PyObject* Entity::getDict()
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        ScriptObject pValue =
          this->pyGetAttribute(ScriptString::create(propName.c_str()));

        if (pValue) {
            pDict.setItem(
              propName.c_str(), pValue, ScriptErrorPrint("Entity::getDict: "));
        } else {
            Script::clearError();
        }
    }
 ...
 */
int Entity::registerEntityExtra(PyMethodDef* pMethods, PyGetSetDef* pAttributes)
{
 ...
 */
void Entity::setAndDropGlobalPositionAndDirection(const Position3D&  position,
                                                  const Direction3D& direction)
{
    ...
    if (CellAppConfig::shouldNavigationDropPosition()) {
        if (this->pChunkSpace()) {
            ...
            WARNING_MSG("Entity::setAndDropGlobalPositionAndDirection(): "
...
/**
 *	This method trims the event history associated with this entity. It is
 *called at a frequency so that all entities in an AoI would have been visited
 *at least once over the period.
 *
 *	This method also calculates whether this entity is no longer being
 *witnessed.
 *
...
void Entity::trimEventHistory(GameTime /*cleanUpTime*/)
{
    ...
    if (this->isReal() && (periodsWithoutWitness_ != NOT_WITNESSED_THRESHOLD)) {
 ...
 */
bool Entity::callback(const char* funcName,
                      PyObject*   args,
                      ...
                      bool        okIfFnNull)
{
    ...
    // call it if we can
    PyObject* funcCallable =
      PyObject_GetAttrString(this, const_cast<char*>(funcName));
    ...
    if (!bufferIt) {
        bool ret =
          Script::call(funcCallable, args, errorPrefix, false /* okIfFnNull */);
    ...
    ScriptObject callable(funcCallable, ScriptObject::FROM_NEW_REFERENCE);

    ScriptArgs arguments(args, ScriptArgs::FROM_NEW_REFERENCE);

    s_callbackBuffer_.storeCallback(this, callable, arguments, errorPrefix);
...
{
    ScriptDict result = createDictWithAllProperties(pEntityType_->description(),
                                                    ScriptObject(),
                                                    pEntityDelegate_.get(),
                                                    dataDomains);
    if (!result) {
...
void Entity::nominateRealEntityPop()
{
    ...
    s_pTreatAsReal =
      s_nominatedEntities.empty() ? NULL : s_nominatedEntities.back().get();
}
