 ...
 *	(below).
 */
PortalConfigController::PortalConfigController(
  bool                 permissive,
                                            ...
                                            WorldTriangle::Flags collisionFlags)
{
    ...
    return pSpace_->setClosestPortalState(point_, permissive, collisionFlags);
}
...
void PortalConfigController::apply()
{
    ...
    if (!this->attemptToApply(permissive_, collisionFlags_)) {
        ...
        timerHandle_ = app.timeQueue().add(
          app.time() + ticks, ticks, this, NULL, "PortalConfigController");
    }
}
