 ...
 */
void HistoryEvent::recreate(Mercury::MessageID       msgID,
                            EventNumber              number,
                            void*                    msg,
                            int                      msgLen,
                            Level                    level,
...
void HistoryEvent::addToBundle(Mercury::Bundle& bundle)
{
    ...
    bundle.startMessage(isReliable_
...
EventNumber EventHistory::addFromStream(BinaryIStream& stream)
{
    ...
    HistoryEvent::extractFromStream(stream,
                                    eventNumber,
                                    type,
                                    level,
                                    latestEventIndex,
    ...
    if (!pNewEvent) {
        pNewEvent = new HistoryEvent(type,
                                     eventNumber,
                                     historyData,
                                     length,
                                     level,
...
void EventHistory::deleteEvent(HistoryEvent* pEvent)
{
    if (pEvent->latestEventIndex() != -1) {
        MF_ASSERT(pEvent == *latestEventPointers_[pEvent->latestEventIndex()]);
