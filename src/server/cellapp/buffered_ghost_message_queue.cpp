...
BufferedGhostMessageQueue::~BufferedGhostMessageQueue()
{
    for (Messages::iterator iter = messages_.begin(); iter != messages_.end();
         ++iter) {
        delete *iter;
    }
}
...
void BufferedGhostMessageQueue::promoteDelayedSubsequences()
{
    if (pDelayedMessages_ != NULL) {
        ...
        messages_.insert(messages_.begin(),
                         pDelayedMessages_->begin(),
                         pDelayedMessages_->end());
...
{
    return pDelayedMessages_ && !pDelayedMessages_->back()->isSubsequenceEnd();
}
