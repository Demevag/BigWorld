...
class EntityCache
{
  ...
  private:
    ...
    enum // Flags bits
    {
        ...
        MANUALLY_ADDED =
          1 << 7, ///< Indicating the entity added manually to the AoI
        ADDED_BY_TRIGGER = 1 << 8, ///< Indicating the entity added
                                   ///< automatically by the trigger to the AoI
        IS_ALWAYS_DETAILED =
          1 << 9, ///< Indicating whether or not volatile position
        ...
        NOT_UPDATABLE = ENTER_PENDING | REQUEST_PENDING | CREATE_PENDING |
                        GONE | WITHHELD | REFRESH,

        // Those are client related state and should be reset when this entity
        // is removed from client.
        CLIENT_STATE =
          ENTER_PENDING | REQUEST_PENDING | CREATE_PENDING | GONE | REFRESH,
    };
...
inline bool operator<(const EntityCache& left, const EntityCache& right)
{
