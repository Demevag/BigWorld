  ...
  BinaryIStream&                  data)
{
    ...
    if (app.nextTickPending()) {
        ...
        if (s_lastMessageTime != app.time()) {
            WARNING_MSG(
              "BufferedInputMessageHandler::handleMessage: No time "
        ...
        bufferedInputMessages.add(*this, srcAddr, header, data);
    } else {
                                         ...
                                         EntityID entityID)
{
    ...
    bool shouldBufferGhostMessage =
      !pEntity || pEntity->shouldBufferMessagesFrom(srcAddr) ||
    ...
    if (reality_ == GHOST_ONLY) {
        ...
        // Destroyed by restored real, then recreated when offload to sender.
                 ...
                 !BufferedGhostMessages::isSubsequenceStart(
                   header.identifier)) {
            isForDestroyedGhost = true;
        }
    ...
    // REAL_ONLY messages should be forwarded if we don't have the real.
        ...
        CellAppChannel* pChannel =
          pEntity ? pEntity->pRealChannel()
                  : Entity::population().findRealChannel(entityID);

        if (pChannel) {
            Entity::forwardMessageToReal(*pChannel,
                                         entityID,
                                         header.identifier,
                                         data,
                                         srcAddr,
    ...
    // Message is good, call through to the handler.
        ...
        if (s_lastMessageTime != app.time()) {
            WARNING_MSG(
              "EntityMessageHandler::handleMessage: No time left in "
        ...
        bufferedEntityMessages.add(*this, srcAddr, header, data, entityID);
    } else {
                                       ...
                                       EntityID                        entityID)
{
    ...
    if (header.replyID != Mercury::REPLY_ID_NONE) {
        ...
        errorMsg << "No such entity " << entityID << " on CellApp " << app.id()
                 << " at " << app.interface().address().c_str();
 ...
 */
template <class ARGS_TYPE>
class CellEntityMessageHandlerWithFailure
  : public CellEntityMessageHandler<ARGS_TYPE>
{
  public:
    /**
     *	This type is the function pointer type that handles the incoming
     ...
     *	Constructor.
     */
    CellEntityMessageHandlerWithFailure(
      const std::pair<FailureHandler, std::pair<Handler, EntityReality>>& args)
...
class CreateEntityNearEntityHandler : public EntityMessageHandler
{
  public:
     ...
     *	message.
     */
    typedef void (Cell::*Handler)(const Mercury::Address&               addr,
                                  const Mercury::UnpackedMessageHeader& header,
                                  BinaryIStream&                        stream,
  ...
  private:
                             ...
                             EntityID                        entityID)
    {
        ...
        data >> channelVersion >> isRestore >> entityToCreateID >> entityTypeID;
        ...
        Mercury::UDPChannel& channel = CellApp::getChannel(srcAddr);
