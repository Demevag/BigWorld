...
namespace {
    ...
    {
      public:
        MockEntity(EntityID id, const Mercury::Address& realAddr, SeqNum seqNum)
          : id_(id)
          , realAddr_(realAddr)
          , nextRealAddr_(0, 0)
          , seqNum_(seqNum)
        ...
        /**
         *	Returns where to expect the next message from the real entity to
         *come from.
         */
    ...
    {
      public:
        BufferedGhostMessages& bufferedMessages() { return bufferedMessages_; }

        MockEntities& entities() { return entities_; }
                     ...
                     BufferedGhostMessage*   pMsg)
        {
            ...
            if (pMsg->isCreateGhostMessage()) {
                shouldPlay =
                  !bufferedMessages_.hasMessagesFor(entityID, srcAddr);
            } else {
    ...
    /**
     *	Simulates CellAppInterface::createGhost - the first message of a
     *lifespan.
     */
    class CreateGhostMessage : public GhostMessage
    {
      public:
        CreateGhostMessage(TestApp&                app,
                           EntityID                id,
                           const Mercury::Address& srcAddr,
                           SeqNum                  num)
        ...
        virtual void play()
        {
            ...
            if (app_.entities().find(entityID_)) {
                app_.bufferedMessages().delaySubsequence(
                  entityID_,
                  srcAddr_,
    ...
    {
      public:
        GhostSetRealMessage(TestApp&                app,
                            EntityID                id,
                            const Mercury::Address& srcAddr,
                            SeqNum                  seqNum)
          : GhostMessage(app, id, true, false)
    ...
    {
      public:
        GhostSetNextRealMessage(TestApp&                app,
                                EntityID                id,
    ...
    class Messages
    {
      public:
        /**
         *	This method creates a set of messages that simulate what a ghost
         *would receive if the real entity travelled to the CellApps given.
         */
        void addLifespan(TestApp&         app,
                         EntityID         id,
                         const Addresses& reals,
                         SeqNum&          seqNum)
        {
    ...
    class TestRunner
    {
      ...
      private:
        ...
        /**
         *	This method populates a map with the counts for the number of
         *messages expected from each address. This map is used to generate the
         *	permutations.
         */
        void calcCounts(Counts& counts) const
        {
            ...
            while (iter1 != lifespans_.end()) {
                ...
                while (iter2 != iter1->end()) {
                    // There's a subsequence start and end message for each
                    // address.
                    const int numMsgs = 2;
    ...
    {
        if (seqNum != seqNum_ + 1) {
            app.bufferedMessages().delaySubsequence(
              id_, addr, new GhostSetRealMessage(app, id_, addr, seqNum));
            return;
        }
