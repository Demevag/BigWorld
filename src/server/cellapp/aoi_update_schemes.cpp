 ...
 *					away.
 */
bool AoIUpdateScheme::init(const BW::string& name,
                           float             minDelta,
                           float             maxDelta)
{
    ...
    if (isZero(minDelta) && isZero(maxDelta)) {
        ...
        weighting_ = minDelta;
        distanceWeighting_ =
          ((maxDelta / minDelta) - 1.f) / CellAppConfig::maxAoIRadius();
    }
...
BW::map<BW::string, AoIUpdateSchemes::SchemeID> AoIUpdateSchemes::nameToScheme_;

BW::map<AoIUpdateSchemes::SchemeID, BW::string> AoIUpdateSchemes::schemeToName_;
...
bool AoIUpdateSchemes::init()
{
    ...
    schemes_[0].init(
      "default", DEFAULT_AOI_SCHEME_MIN_DELTA, DEFAULT_AOI_SCHEME_MAX_DELTA);
    ...
    while ((iter != pTopSection->end()) && (currSchemeID != 0)) {
        ...
        if (pSection->sectionName() == "scheme") {
            ...
            const float      minDelta =
              pSection->readFloat("minDelta", DEFAULT_AOI_SCHEME_MIN_DELTA);
            const float maxDelta =
              pSection->readFloat("maxDelta", DEFAULT_AOI_SCHEME_MAX_DELTA);
