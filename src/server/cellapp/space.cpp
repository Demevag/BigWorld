                                     ...
                                     bool           isY) const
{
    ...
    while (pNode && (level >= 0)) {
        ...
        if (pNextNode) {
            ...
            if (almostEqual(
                  entityPosition, nextEntityPosition, POS_DIFF_EPSILON)) {
        ...
        if (shouldCountThisEntity) {
            if (currCPULoad > currCPULimit) {
                const float limitPosition =
                  pNextEntity ? (nextEntityPosition + entityPosition) / 2
...
WatcherPtr Space::pWatcher()
{
    ...
    if (!pWatcher) {
        ...
        pWatcher->addChild("cellInfos",
                           new MapWatcher<CellInfos>(pNull->cellInfos_));
        Watcher* pCellInfoWatcher =
          new SmartPointerDereferenceWatcher(CellInfo::pWatcher());
...
EntityPtr Space::newEntity(EntityID id, EntityTypeID entityTypeID)
{
    ...
    EntityPopulation::const_iterator deadEntity = Entity::population().find(id);
 ...
 */
bool Space::spaceDataEntry(const SpaceEntryID& entryID,
                           uint16              key,
                           const BW::string&   value,
                           UpdateCellAppMgr    cellAppMgrAction,
                           DataEffected        effected)
{
    ...
    if (cellAppMgrAction == UPDATE_CELL_APP_MGR) {
        ...
        if (key < SPACE_DATA_FIRST_CELL_ONLY_KEY) // && key != uint16(-1)
        {
            ...
            for (RecentDataEntries::reverse_iterator rit = recentData_.rbegin();
                 ...
                 ++rit) {
                if (rit->entryID == entryID && rit->key == uint16(-1)) {
                    WARNING_MSG(
                      "Space::spaceDataEntry: "
    ...
    // or that a space data value has been deleted.
    {
        ...
        const BW::string& actualValue = hasBeenAdded ? value : oldValue.data();
    ...
    if (effected == NEED_TO_EFFECT) {
        if (effectiveKey == SPACE_DATA_TOD_KEY) {
            ...
            if (hasBeenAdded) {
                ...
                pPhysicalSpace_->loadResource(
                  entryID, path, &mappingData.matrix[0][0]);
        ...
        */
            ...
            if (hasBeenAdded) {
                ...
                this->pCell()->startRecording(entryID,
                                              name,
                                              (shouldRecordAoIEvents != 0),
                                              /* isInitial */ false);
            } else {
...
void Space::updateGeometry(BinaryIStream& data)
{
    ...
    BW::Rect rect(-std::numeric_limits<float>::max(),
...
void Space::checkForShutDown()
{
    if (this->hasSingleCell() && entities_.empty() &&
        CellApp::instance().hasStarted() && !this->isShuttingDown() &&
        !CellApp::instance().mainDispatcher().processingBroken() &&
        !(CellAppConfig::useDefaultSpace() &&
          id_ == 1)) // Not for the default space.
    {
...
void Space::spaceGeometryLoaded(BinaryIStream& data)
{
    ...
    CellApp::instance().scriptEvents().triggerEvent(
      ...
      Py_BuildValue("iis#",
                    this->id(),
                    int(isBootStrap),
                    lastMappedGeometry.data(),
                    lastMappedGeometry.length()));
}
...
void Space::shutDownSpace(BinaryIStream& data)
{
    if (!shuttingDownTimerHandle_.isSet()) {
        ...
        shuttingDownTimerHandle_ =
          CellApp::instance().mainDispatcher().addTimer(
            1000000, this, NULL, "ShutdownSpace");
    } else {
...
/**
 *  This method updates space data entries with existing loadable bounds
 * information. It deletes the 'load bounds' entries for which no loadable
 * bounds exist.
 */
void Space::updateLoadBounds()
{
    ...
    for (SpaceDataMapping::const_iterator iter = spaceDataMapping_.begin();
        ...
        MF_ASSERT(SpaceData_LoadBounds::castFromString(value, pBounds) &&
                  pBounds);

        IPhysicalSpace::BoundsList::iterator boundsIter = std::find(
          loadableRects.begin(), loadableRects.end(), pBounds->loadRect);
    ...
    for (IPhysicalSpace::BoundsList::iterator iter = loadableRects.begin();
        ...
        this->spaceDataEntry(newEntryID,
                             SPACE_DATA_SERVER_LOAD_BOUNDS,
                             value,
...
void Space::chunkTick()
{
    ...
    if (unloadOnly) {
        ...
        // Inflate boundary of rect to load, but don't go outside
        // +-(nearIntMax);
        static const float nearIntMax = 1000000000.f;

        const float inflateBy =
          CellAppConfig::ghostDistance() * 2; // or something
...
void Space::writeChunkBounds(BinaryOStream& stream) const
{
    ...
    BW::Rect    spaceBoundary(bbox.minBounds().x,
                           bbox.minBounds().z,
                           bbox.maxBounds().x,
                           bbox.maxBounds().z);
 ...
 */
static PyObject* addSpaceData(SpaceID           spaceID,
                              uint16            key,
                              const BW::string& value,
                              bool              userOnly = true)
{
...
PY_AUTO_MODULE_FUNCTION(RETOWN,
                        addSpaceData,
                        ARG(SpaceID, ARG(uint16, ARG(BW::string, END))),
                        BigWorld)
...
PY_AUTO_MODULE_FUNCTION(RETOK,
                        delSpaceData,
                        ARG(SpaceID, ARG(SpaceEntryID, END)),
                        BigWorld)
...
static bool delSpaceDataForKey(SpaceID spaceID, uint16 key)
{
    ...
    SpaceDataMapping::const_iterator it = pSpace->spaceDataMapping().begin();
    SpaceDataMapping::const_iterator endIter = pSpace->spaceDataMapping().end();
    for (; it != endIter; ++it) {
    ...
    for (uint i = 0; i < deadEntries.size(); i++) {
        MF_VERIFY(
          pSpace->spaceDataEntry(deadEntries[i], uint16(-1), emptyString));
    }
 ...
 */
static PyObject* getSpaceData(SpaceID             spaceID,
...
PY_AUTO_MODULE_FUNCTION(RETOWN,
                        getSpaceData,
                        ARG(SpaceID,
                            ARG(SpaceEntryID, OPTARG(uint16, uint16(-1), END))),
                        BigWorld)
...
static PyObject* getSpaceDataFirstForKey(SpaceID spaceID, uint16 key)
{
    ...
    const BW::string* value = pSpace->spaceDataMapping().dataRetrieveFirst(key);
    if (value == NULL) {
        PyErr_Format(
          PyExc_ValueError,
          "BigWorld.getSpaceData(): "
 ...
 */
static PyObject* setSpaceData(SpaceID           spaceID,
                              uint16            key,
                              const BW::string& value,
                              bool              userOnly = true)
{
    ...
    if (!delSpaceDataForKey(spaceID, key))
        return NULL;
...
PY_AUTO_MODULE_FUNCTION(RETOWN,
                        setSpaceTimeOfDay,
                        ARG(SpaceID, ARG(float, ARG(float, END))),
                        BigWorld)
 ...
 */
static bool setSpaceArtificialMinLoad(SpaceID spaceID, float artificialMinLoad)
{
    if (!delSpaceDataForKey(spaceID, SPACE_DATA_ARTIFICIAL_LOAD)) {
        PyErr_Format(PyExc_ValueError,
                     "BigWorld.setSpaceArtificialMinLoad: "
                     "No space ID %d",
                     int(spaceID));
    ...
    bool res = pSpace->spaceDataEntry(
      entryID, SPACE_DATA_ARTIFICIAL_LOAD, value, Space::UPDATE_CELL_APP_MGR);
    if (!res) {
        PyErr_Format(PyExc_ValueError,
                     "BigWorld.setSpaceArtificialMinLoad: "
                     "failed to set space data entry for space ID %d",
                     int(spaceID));
...
PY_AUTO_MODULE_FUNCTION(RETOK,
                        setSpaceArtificialMinLoad,
                        ARG(SpaceID, ARG(float, END)),
                        BigWorld)
...
PY_AUTO_MODULE_FUNCTION(RETOWN,
                        ...
                        ARG(SpaceID,
                            ARG(MatrixProviderPtr,
                                ARG(BW::string, OPTARG(bool, true, END)))),
                        BigWorld)
...
void collectSpaceGeometryMappings(Space*    pSpace,
                                  bool      isClientOnly,
                                  PyObject* pResultList)
{
    uint16 key = isClientOnly ? SPACE_DATA_MAPPING_KEY_CLIENT_ONLY
                              : SPACE_DATA_MAPPING_KEY_CLIENT_SERVER;

    SpaceDataMapping::const_iterator iter = pSpace->spaceDataMapping().begin();
    SpaceDataMapping::const_iterator endIter = pSpace->spaceDataMapping().end();
    ...
    for (; iter != endIter; ++iter) {
        ...
        MF_VERIFY(
          tuple.setItem(0,
                        ScriptObject::createFrom(iter->second.data().data() +
                                                 sizeof(Matrix))));
        MF_VERIFY(tuple.setItem(
          1, ScriptObject::createFrom(*(Matrix*)iter->second.data().data())));
 ...
 *
 */
static bool startRecording(SpaceID           spaceID,
                           ...
                           bool              shouldRecordAoIEvents)
{
    ...
    if (!pSpace->pCell()->startRecording(
          entryID, name, shouldRecordAoIEvents)) {
        PyErr_Format(
          PyExc_ValueError, "Space %d is already recording", spaceID);
        return false;
    }
...
PY_AUTO_MODULE_FUNCTION(RETOK,
                        startRecording,
                        ARG(SpaceID, ARG(BW::string, OPTARG(bool, false, END))),
                        BigWorld)
...
PY_AUTO_MODULE_FUNCTION(RETOK, stopRecording, ARG(SpaceID, END), BigWorld)
 ...
 */
SpaceNode* Space::readTree(BinaryIStream& stream, const BW::Rect& rect)
{
    ...
    switch (type) {
        ...
        case 1:
            pResult =
              new SpaceBranch(*this, rect, stream, type == 0 /*isHorizontal*/);
            break;
...
{
    this->spaceDataEntry(entryID,
                         uint16(-1),
                         BW::string(),
                         Space::UPDATE_CELL_APP_MGR,
                         Space::NEED_TO_EFFECT);
}
