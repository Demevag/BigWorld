...
namespace Util {
    ...
    /**
     *	This helper function returns a list of points to navigate between the
     *given source location to the given destination, for the given ChunkSpace.
     *This is called by Entity.navigatePathPoints() and
     *BigWorld.navigatePathPoints().
     */
    PyObject* navigatePathPoints(const Navigator& navigator,
                                 ChunkSpace*      pChunkSpace,
                                 const Vector3&   src,
                                 const Vector3&   dst,
                                 ...
                                 bool             checkFirstPosition)
    {
        ...
        if (!navigator.findFullPath(pChunkSpace,
                                    src,
                                    dst,
                                    maxSearchDistance,
  ...
  Vector3&    result)
{
    ...
    return navigator.findRandomNeighbourPointWithRange(
      pSpace, position, minRadius, maxRadius, girth, result);
}
                                                      ...
                                                      float girth)
{
    ...
    return Util::navigatePathPoints(
      navigator, pSpace, src, dst, maxSearchDistance, girth, true);
}

/**
 *
 */
NavmeshEntityNavigationSystem::NavmeshEntityNavigationSystem(Entity& entity)
  : entity_(entity)
{
}
...
Controller* NavmeshEntityNavigationSystem::createController(
  ...
  float          maxSearchDistance,
  float          girth,
  float          closeEnough) const
{
    return new NavigationController(dstPosition,
                                    velocity,
                                    faceMovement,
                                    maxSearchDistance,
                                    girth,
  ...
  float          nearPortalDist)
{
    ...
    while ((fabs(position.x - destination.x) > 0.01f ||
        ...
        if (!this->getNavigatePosition(srcLoc,
                                       dstLoc,
                                       maxSearchDistance,
                                       position,
        ...
        srcLoc =
          navigator_.createPathNavLoc(entity_.pChunkSpace(), position, girth);
        ...
        if (passedActivatedPortal) {
            ...
            {
                dist += sqrtf((stepPoints[i].x - stepPoints[i - 1].x) *
            ...
            if (dist > nearPortalDist) {
                ...
                for (; i > 0; --i) {
                    lastLength =
                      sqrtf((stepPoints[i].x - stepPoints[i - 1].x) *
                              (stepPoints[i].x - stepPoints[i - 1].x) +
                            (stepPoints[i].z - stepPoints[i - 1].z) *
                              (stepPoints[i].z - stepPoints[i - 1].z));
                ...
                Vector3 dir((stepPoints[i - 1].x - stepPoints[i].x),
                            0.0,
                ...
                MF_VERIFY(
                  result.setItem(0, ScriptObject::createFrom(stopPosition)));
            ...
            // Otherwise starting point is close to this portal.
                ...
                if (!ignoreFirstStopPoint) {
                    ScriptTuple result = ScriptTuple::create(2);
                    MF_VERIFY(
                      result.setItem(0, ScriptObject::createFrom(source)));
                    MF_VERIFY(
                      result.setItem(1, ScriptObject::createFrom(false)));
                    return result.newRef();
                } else {
                    // sanity check.
  ...
  int                     userArg)
{
    ...
    // complain if invalid
    if (!srcLoc.valid()) {
        PyErr_Format(PyExc_ValueError, "Could not resolve source position.");
        return NULL;
    }
    ...
    bool foundPath = this->getNavigatePosition(srcLoc,
                                               dstLoc,
                                               maxSearchDistance,
                                               nextPosition,
                                               passedActivatedPortal,
    ...
    if (!foundPath) {
        PyErr_SetString(PyExc_ValueError, "No path found.");
        return NULL;
    }
    ...
    if (wayDistance > maxMoveDistance) {
        // shorten it
        nextPosition =
          entity_.position() + (disp * (maxMoveDistance / wayDistance));

        if (srcLoc.pSet()
              ->waypoint(srcLoc.waypoint())
              .containsProjection(*(srcLoc.pSet()),
                                  MappedVector3(nextPosition,
                                                srcLoc.pSet()->chunk(),
    ...
    entity_.setAndDropGlobalPositionAndDirection(position, entity_.direction());

    if (!pNSController) {
        pNSController =
          new NavigateStepController(nextPosition, velocity, faceMovement);
  ...
  float          girth) const
{
...

    {
        ...
        foundPath = navigator_.findPath(srcLoc,
                                        dstLoc,
                                        maxSearchDistance,
                                        /* blockNonPermissive: */ true,
                                        wayLoc,
  ...
  float          girth) const
{
    ...
    // be out of sync.
    return Util::navigatePathPoints(navigator_,
                                    entity_.pChunkSpace(),
                                    srcLoc.point(),
                                    dstPosition,
                                    maxSearchDistance,
 ...
 */
bool NavmeshEntityNavigationSystem::getNavigatePosition(
  const NavLoc& srcLoc,
  const NavLoc& dstLoc,
  float         maxSearchDistance,
  Vector3&      nextPosition,
  ...
  float         girth)
{
    ...
    */
    ...
    bool foundPath = navigator_.findPath(srcLoc,
                                         dstLoc,
                                         maxSearchDistance,
                                         /* blockNonPermissive */ true,
                                         wayLoc,
