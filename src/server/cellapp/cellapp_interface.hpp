...
#define MF_BUFFERED_STREAM_MESSAGE_EX(TYPE, NAME)                              \
    MERCURY_HANDLED_VARIABLE_MESSAGE(                                          \
      NAME,                                                                    \
      2,                                                                       \
      BufferedInputMessageHandler<StreamMessageHandlerEx<TYPE>>,               \
      &TYPE::NAME)

// For Cell referenced by Entity
#define MF_CREATE_ENTITY_MSG(NAME, METHOD_NAME)                                \
    MERCURY_HANDLED_VARIABLE_MESSAGE(                                          \
      NAME, 2, CreateEntityNearEntityHandler, &Cell::METHOD_NAME)

// For Entity
#define MF_EMPTY_ENTITY_MSG(NAME, IS_REAL_ONLY)                                \
    MERCURY_HANDLED_PREFIXED_EMPTY_MESSAGE(                                    \
      NAME,                                                                    \
      EntityID,                                                                \
...
#define MF_BEGIN_ENTITY_MSG(NAME, IS_REAL_ONLY)                                \
    BEGIN_HANDLED_PREFIXED_MESSAGE(                                            \
      NAME,                                                                    \
      EntityID,                                                                \
...
MERCURY_HANDLED_VARIABLE_MESSAGE(sendEntityPositions,
                                 2,
                                 EntityPositionSender,
                                 NULL)
