...
static class LeakTracer
{
  ...
  public:
    LeakTracer() { initialize(); }

    void initialize()
    {
        // Unfortunately we might be called before our constructor has actually
        // fired
        if (initialized)
            return;
        ...
        const char* filename =
          getenv("LEAKTRACE_FILE") ? getenv("LEAKTRACE_FILE") : "leak.out";
        struct stat dummy;
        if (stat(filename, &dummy) == 0) {
#ifndef _WIN32 // WIN32PORT
            bw_snprintf(
              uniqFilename, sizeof(uniqFilename), "%s.%d", filename, getpid());
#else  // ifndef _WIN32  // WIN32PORT
            bw_snprintf(uniqFilename,
                        sizeof(uniqFilename),
                        "%s.%d",
                        filename,
                        GetCurrentProcessId());
            ...
            bw_snprintf(uniqFilename, sizeof(uniqFilename), "%s", filename);
        }

        int reportfd =
          open(uniqFilename, O_WRONLY | O_CREAT | O_TRUNC, S_IREAD | S_IWRITE);
        if (reportfd < 0) {
...
#ifdef MAGIC
        fprintf(report,
                "# memory overrun protection of %d Bytes "
                "with magic 0x%4lX\n",
                SAVESIZE,
                MAGIC);
#endif
...
#else
        fprintf(report,
                "# not thread save; if you use threads, recompile with "
                "-DTHREAD_SAVE\n");
#endif
        fflush(report);
    }
...
void* LeakTracer::registerAlloc(size_t size, bool type)
{
    ...
    if (destroyed) {
        fprintf(stderr,
                "Oops, registerAlloc called after destruction of LeakTracer "
                "(size=%d)\n",
                size);
...
void LeakTracer::registerFree(void* p, bool type)
{
    ...
    if (destroyed) {
        fprintf(
          stderr, "Oops, allocation destruction of LeakTracer (p=%p)\n", p);
        return;
    }
        ...
        *lastPointer = leaks[i].nextBucket; // detach.
        ...
        if ((SAVESIZE >= sizeof(magic_t)) &&
            *((magic_t*)((char*)p + leaks[i].size)) != MAGIC) {
            ...
            hexdump((unsigned char*)p + leaks[i].size, SAVESIZE);
...
#endif
    fprintf(report,
            "D %10p             # delete non alloc or twice pointer %10p\n",
#ifndef _WIN32 // WIN32PORT
            __builtin_return_address(1),
#else  // ifndef _WIN32  // WIN32PORT
...
void LeakTracer::writeLeakReport()
{
    ...
    fprintf(report,
            "# total allocation requests: %6ld ; max. mem used"
            " %d kBytes\n",
            totalAllocations,
            maxAllocated / 1024);
    fprintf(report,
