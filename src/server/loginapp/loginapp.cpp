...
bool LoginApp::init(int argc, char* argv[]) /* override */
{
    ...
    MF_WATCH("numBannedIPAddresses", ipAddressBanMap_, &IPAddressBanMap::size);
    ...
    if (!BW_INIT_ANONYMOUS_CHANNEL_CLIENT(dbAppMgr_,
                                          this->intInterface(),
                                          LoginIntInterface,
                                          DBAppMgrInterface,
                                          numStartupRetries)) {
    ...
    if (Config::allowProbe() && Config::isProduction()) {
        CONFIG_ERROR_MSG(
          "Production Mode: bw.xml/loginApp/allowProbe "
    ...
    MF_WATCH(BW_OPTION_WATCHER_DIR "challengeType",
             *this,
                          ...
                          const Mercury::Address& dbAppAlphaAddress)
{
    ...
    Mercury::MachineDaemon::registerBirthListener(
      interface_.address(),
      LoginIntInterface::handleDBAppMgrBirth,
      "DBAppMgrInterface");
    ...
    pStatsWatcher->addChild("all", makeWatcher(loginStats_, &LoginStats::all));
    ...
    pChallengeStatWatcher->addChild(
      "calculationTime",
      makeWatcher(loginStats_, &LoginStats::challengeCalculationAverage));
    pChallengeStatWatcher->addChild(
      "verificationTime",
      makeWatcher(loginStats_, &LoginStats::challengeVerificationAverage));
    ...
    statsTimer_ = mainDispatcher_.addTimer(
      UPDATE_STATS_PERIOD, &loginStats_, NULL, "UpdateStats");
...
bool LoginApp::initLogOnParamsEncoder()
{
    ...
    RSAStreamEncoder* pEncoder = new RSAStreamEncoder(/* keyIsPrivate: */ true);
                             ...
                             LogOnParamsPtr          pParams /* = NULL */)
{
    ...
    if (numFailRepliesLeft_ > 0) {
        ...
        NETWORK_INFO_MSG(
          "LoginApp::handleFailure: "
          "LogOn for %s failed, LogOnStatus: %d, description '%s'.\n",
          addr.c_str(),
          status,
          pDescription ? pDescription : "none");
...
/**
 *	This method marks the address as banned and sends a failure message back to
 *the client.
 */
                     ...
                     BinaryIStream&                  data)
{
    ...
    if (currTimestamp > nextIPAddressBanMapCleanupTime_) {
        nextIPAddressBanMapCleanupTime_ =
          currTimestamp +
    ...
    INFO_MSG("LoginApp::login: %s from %s\n",
             isReattempt ? "Re-attempt" : "Attempt",
             source.c_str());
    ...
    bool isRateLimited =
      Config::rateLimitEnabled() && (numAllowedLoginsLeft_ == 0);
    if (isRateLimited) {
        if (Config::verboseLoginFailures()) {
    ...
    if (systemOverloaded_ != 0) {
        if (systemOverloadedTime_ + stampsPerSecond() < currTimestamp) {
            ...
            this->handleFailure(
              source, pChannel, header.replyID, systemOverloaded_);
            data.finish();
            return;
        }
    }

    if (this->processForLoginChallenge(
          source, pChannel, header.replyID, data)) {
        return;
    }
    ...
    DatabaseReplyHandler* pDBHandler = new DatabaseReplyHandler(
      *this, source, pChannel, header.replyID, pParams);
                                        ...
                                        BinaryIStream&          data)
{
    ...
    if (iter != loginRequests_.end()) {
        ...
        if (request.pLoginChallenge()) {
            ...
            this->sendChallengeReply(source,
                                     pChannel,
                                     replyID,
                                     request.challengeType(),
                                     request.pLoginChallenge());
            data.finish();
            return true;
        } else {
    ...
    this->sendChallengeReply(
      source, pChannel, replyID, challengeType, pChallenge);
                     ...
                     BinaryIStream& /*data*/)
{
    ...
    if (!Config::allowProbe() || header.length != 0)
        return;
                                 ...
                                 BinaryIStream&                  data)
{
    ...
    TRACE_MSG(
      "LoginApp::login: Client %s passed login challenge "
    ...
    calculationDuration =
      std::min((float)MAX_SANE_CALCULATION_SECONDS, calculationDuration);

    if (calculationDuration < (float)MAX_SANE_CALCULATION_SECONDS) {
        loginStats_.challengeCalculationTimeSample(calculationDuration);
    }

    loginStats_.challengeVerificationTimeSample(float(start.ageInSeconds()));
}
 ...
 */
const float LoginApp::LoginStats::COUNT_BIAS =
  2.f / (WEIGHTING_NUM_SAMPLES + 1);
